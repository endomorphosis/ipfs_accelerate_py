...
def main():
    ...
    manager = DatasetsManager(
        {"enable_audit": True, "enable_provenance": True, "enable_p2p": False}
    )
    ...
    # Track provenance
    cid = manager.track_provenance("example_operation", {"operation": "test", "data": "example"})
    if cid:
    ...
    logger.log_inference(
        "bert-base-uncased", {"input": "Example text", "output_dim": 768, "duration_ms": 100}
    )
    print("✓ Inference logged")

    # Log transformation
    logger.log_transformation("tokenization", {"tokenizer": "bert-tokenizer", "max_length": 512})
    print("✓ Transformation logged")

    # Log worker execution
    logger.log_worker_execution(
        "worker-001", "task-001", {"status": "completed", "duration_ms": 5000}
    )
    ...
    if task:
        ...
        # Complete task
        coordinator.complete_task(task["task_id"], {"status": "success", "output": "result"})
