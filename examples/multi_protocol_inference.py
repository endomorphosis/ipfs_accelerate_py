...
class MultiProtocolInferenceClient:
    ...
    async def http_text_completion(
        self, prompt: str, model: str = "gpt2", max_tokens: int = 50
    ) -> Dict[str, Any]:
        """Run text completion via HTTP"""
        async with aiohttp.ClientSession() as session:
            payload = {"model": model, "prompt": prompt, "max_tokens": max_tokens}
            async with session.post(f"{self.http_base_url}/v1/completions", json=payload) as resp:
                return await resp.json()

    async def http_chat_completion(self, messages: list, model: str = "gpt2") -> Dict[str, Any]:
        """Run chat completion via HTTP"""
        async with aiohttp.ClientSession() as session:
            payload = {"model": model, "messages": messages}
            async with session.post(
                f"{self.http_base_url}/v1/chat/completions", json=payload
            ) as resp:
                return await resp.json()
    ...
    async def websocket_subscribe(self, topics: list):
        ...
        await self.websocket_send({"type": "subscribe", "topics": topics})
    ...
    async def websocket_get_status(self):
        ...
        await self.websocket_send({"type": "status", "details": "full"})
    ...
    def mcp_get_backend_status(self) -> Dict[str, Any]:
        ...
        try:
            from ipfs_accelerate_py.mcp_server.tools.backend_management import get_backend_status

            return get_backend_status()
        except ImportError:
            return {"error": "MCP tools not available"}

    def mcp_select_backend(self, task: str, model: str = None) -> Dict[str, Any]:
...
async def run_http_examples(client: MultiProtocolInferenceClient):
    ...
    result = await client.http_text_completion(
        prompt="The quick brown fox", model="gpt2", max_tokens=20
    )
    ...
    result = await client.http_chat_completion(
        messages=[{"role": "user", "content": "Hello, how are you?"}], model="gpt2"
    )
...
def run_mcp_examples(client: MultiProtocolInferenceClient):
    ...
    backend = client.mcp_select_backend(task="text-generation", model="gpt2")
