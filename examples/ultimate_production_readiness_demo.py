...
try:
    from utils.comprehensive_model_hardware_compatibility import (
...
class UltimateProductionReadinessDemo:
    ...
    def _demonstrate_enhanced_performance_modeling(self) -> Dict[str, Any]:
        ...
        for model in test_models[:2]:  # Test first 2 models
            ...
            sorted_results = sorted(
                model_results.items(), key=lambda x: x[1].throughput_samples_per_sec, reverse=True
            )
            ...
            best_hw, best_metrics, reasons = (
                self.performance_modeling.get_optimal_hardware_recommendation(model)
            )
            ...
            print(
                f"    ✅ Best hardware for {model}: {best_hw} ({best_metrics.throughput_samples_per_sec:.1f} samples/sec)"
            )
        ...
        try:
            ...
            benchmarking_results = {
                ...
                "statistical_analysis": {
                    "hardware_rankings": len(
                        benchmark_report["performance_analysis"]["hardware_rankings"]
                    ),
    ...
    def _demonstrate_comprehensive_compatibility(self) -> Dict[str, Any]:
        ...
        for model in test_models[:2]:  # Test first 2 models
            ...
            if recommendations:
                ...
                print(
                    f"    ✅ Best hardware: {best_hw} ({best_result.compatibility_level.value.upper()})"
                )
                print(f"    📊 Performance score: {best_result.performance_score:.1f}/100")
                print(
                    f"    🎯 Optimal config: batch={best_result.optimal_batch_size}, {best_result.optimal_precision}"
                )
    ...
    def _demonstrate_advanced_integration_testing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            integration_results = {
                ...
                "hardware_performance": integration_report["analysis"].get(
            ...
            print(
                f"    ✅ Tests completed: {test_summary['successful_tests']}/{test_summary['total_tests']}"
            )
    ...
    def _demonstrate_enterprise_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if run_enterprise_validation:
                ...
                print(
                    f"    🏢 Enterprise features: {validation_report.automation_count} automation capabilities"
                )
    ...
    def _generate_overall_assessment(self, demo_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "integration_testing" in demo_results and demo_results["integration_testing"]:
            if "error" not in demo_results["integration_testing"]:
                integration_score = (
                    demo_results["integration_testing"]
                    .get("analysis", {})
                    .get("overall_assessment", {})
                    .get("integration_score", 75)
                )
    ...
    def _generate_final_recommendations(self, demo_results: Dict[str, Any]) -> List[str]:
        ...
        if components_tested < 4:
            recommendations.append(
                "Install additional ML libraries (torch, transformers) for full functionality"
            )
        ...
        if "benchmarking_results" in demo_results:
            if "error" in demo_results["benchmarking_results"]:
                recommendations.append(
                    "Resolve benchmarking dependencies for accurate performance measurement"
                )
            else:
        ...
        if "integration_testing" in demo_results:
            if "error" in demo_results["integration_testing"]:
                ...
                recommendations.append(
                    "Implement continuous integration testing for model deployments"
                )
    ...
    def _display_final_results(self, demo_results: Dict[str, Any]):
        ...
        print(f"   🏆 Status: {overall['status']}")
        print(
            f"   🧩 Components Tested: {overall['components_tested']}/{overall['total_components']}"
        )
        ...
        print(
            f"   📈 Performance Optimized: {'YES' if 'performance_analysis' in demo_results else 'BASIC'}"
        )
