...
async def run_example():
        ...
        "endpoints": {
            "local_endpoints": {
                "bert-base-uncased": [
                    ...
                    ["bert-base-uncased", "cuda:0", 32768]
                    if framework.hardware_detection.detect_hardware()
                    .get("cuda", {})
                    .get("available", False)
                    else None,
                ],
                ...
                "vit-base-patch16-224": [
                    ...
                    ["vit-base-patch16-224", "cuda:0", 32768]
                    if framework.hardware_detection.detect_hardware()
                    .get("cuda", {})
                    .get("available", False)
                    else None,
    ...
    for model in resources["endpoints"]["local_endpoints"]:
        resources["endpoints"]["local_endpoints"][model] = [
            endpoint
            for endpoint in resources["endpoints"]["local_endpoints"][model]
    ...
    try:
        result = await framework.process_async(
            "bert-base-uncased", "This is a test sentence.", "cpu:0"
        )
        logger.info(f"Result: {result}")
    except Exception as e:
    ...
    try:
        ...
        result = await framework.accelerate_inference(
            "llava-1.5-7b", "Describe this image", use_ipfs=True
        )
