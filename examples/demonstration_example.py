...
def demonstrate_basic_model_management():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        gpt_metadata = ModelMetadata(
            ...
            huggingface_config={"model_name": "gpt2", "trust_remote_code": False, "device": "cpu"},
            inference_code_location="transformers",
...
def demonstrate_bandit_recommendations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        recommender = BanditModelRecommender(
            model_manager=manager, algorithm="thompson_sampling", storage_path=bandit_path
        )
        ...
        for model_data in models_data:
            metadata = ModelMetadata(
                ...
                model_type=ModelType.ENCODER_ONLY,
                architecture="bert",
                inputs=[IOSpec(name="input_ids", data_type=DataType.TOKENS, shape=(512,))],
                outputs=[IOSpec(name="logits", data_type=DataType.LOGITS, shape=(768,))],
        ...
        for i in range(3):
            ...
            print(
                f"  {i + 1}. {recommendation.model_id} (confidence: {recommendation.confidence_score:.3f})"
            )
        ...
        try:
            ...
            for model_id, stats in performance_report.items():
                if "total_trials" in stats and stats["total_trials"] > 0:
                    print(
                        f"  {model_id}: {stats['total_trials']} trials, {stats['avg_reward']:.3f} avg reward"
                    )
...
def demonstrate_repository_structure():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        metadata = ModelMetadata(
            ...
            model_type=ModelType.DECODER_ONLY,
            architecture="gpt2",
            inputs=[IOSpec(name="input_ids", data_type=DataType.TOKENS, shape=(1024,))],
            outputs=[IOSpec(name="logits", data_type=DataType.LOGITS, shape=(50257,))],
        ...
        for doc_data in sample_docs:
            try:
                ...
                doc_index.add_document(
                    doc_data["file_path"], doc_data["content"], doc_data["section"]
                )
                print(f"✅ Added document: {doc_data['file_path']}")
            except Exception as e:
                print(
                    f"⚠️  Could not add document {doc_data['file_path']}: Requires internet access for model download"
                )
            ...
            try:
                ...
                if results:
                    for i, result in enumerate(results):
                        print(
                            f"  {i + 1}. {result.document.file_path} (similarity: {result.similarity_score:.3f})"
                        )
