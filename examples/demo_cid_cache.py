...
def demo_llm_cache():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, (prompt, model, temp) in enumerate(prompts, 1):
            ...
            # Check cache first
            start = time.time()
            cached = cache.get_completion(prompt=prompt, model=model, temperature=temp)
            ...
            if cached:
                ...
                response = {"text": f"Response to: {prompt}", "tokens": 100}
                elapsed = time.time() - start
                print(f"   ✗ Cache MISS - Making API call ({elapsed * 1000:.2f}ms)")

                # Cache the response
                cache.cache_completion(
                    prompt=prompt, response=response, model=model, temperature=temp
                )
...
def demo_cid_features():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for prompt, model, temp in queries:
            ...
            cache.cache_completion(prompt=prompt, response=response, model=model, temperature=temp)
