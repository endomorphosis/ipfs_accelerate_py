    ...
    if os.getenv("OPENROUTER_API_KEY"):
        ...
        try:
            embeddings = embed_texts(
                texts, provider="openrouter", model_name="text-embedding-3-small"
            )
...
def example_with_caching():
    ...
    try:
        ...
        print(
            f"Embeddings match: {np.allclose(embedding1, embedding2) if embedding1 and embedding2 else 'N/A'}"
        )
...
def example_custom_provider():
    ...
    try:
        texts = ["Text 1", "Text 2"]
        embeddings = embed_texts(texts, provider="mock")
...
def example_backend_manager_integration():
    ...
    try:
        ...
        if provider:
            ...
            embeddings = embed_texts(texts, provider="backend_manager")
...
def example_list_available_providers():
    ...
    providers_to_check = ["openrouter", "gemini_cli", "huggingface", "backend_manager"]
