...
def demonstrate_model_optimization():
    ...
    try:
        ...
        available_hardware = [
            hw for hw, avail in detector.get_available_hardware().items() if avail
        ]
    ...
    for scenario in scenarios:
        ...
        for model in scenario["models"]:
            recommendation = get_optimal_hardware(
                model, available_hardware, **scenario["constraints"]
            )
            ...
            print(
                f"     📝 {model:8} → {hw:8} (conf: {confidence:6}, perf: {performance:4.1f}x, mem: {min_mem}-{rec_mem}GB)"
            )
    ...
    for model, hardware, batch_size, seq_len in test_scenarios:
        ...
        if time_ms:
            ...
            print(
                f"  🕒 {model:8} on {hardware:8}: ~{time_ms:6.1f}ms{seq_info} (confidence: {confidence})"
            )
        else:
