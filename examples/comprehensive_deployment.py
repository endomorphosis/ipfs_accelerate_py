...
def main():
    ...
    parser.add_argument(
        "--hardware", default="auto", help="Hardware to use (auto for best available)"
    )
    parser.add_argument(
        "--mode",
    ...
    try:
        ...
        for hw in available_hardware[:3]:  # Show first 3 for brevity
            try:
                ...
                print(
                    f"   📊 {hw}: {info.get('status', 'unknown')} - {info.get('description', 'No details')}"
                )
            except:
    ...
    try:
        ...
        for i, (model_name, info) in enumerate(list(models_catalog.items())[:3]):
            ...
            print(
                f"      Size: {info['size_mb']}MB | Family: {info['family']} | {info['description']}"
            )
        ...
        print(
            f"✅ Matrix generated for {compatibility['models_count']} models and {compatibility['hardware_count']} hardware types"
        )
        ...
        if args.model == "auto":
            ...
            test_model = (
                args.model if args.model in models_catalog else list(models_catalog.keys())[0]
            )
        ...
        try:
            ...
            if benchmark_results:
                ...
                if hasattr(benchmark_results, "results") and benchmark_results.results:
                    avg_latency = sum(r.latency_ms for r in benchmark_results.results) / len(
        ...
        if "test_model" in locals() and "models_catalog" in locals():
            ...
            if model_size < 100:
                recommendations.append(
                    f"📱 Small model ({model_size}MB): Great for mobile/edge deployment"
                )
            elif model_size > 500:
                recommendations.append(
                    f"🏭 Large model ({model_size}MB): Consider quantization for deployment"
                )
