...
def example_register_cli_endpoints():
    ...
    openai = OpenAICodexAdapter(
        endpoint_id="openai_codex_primary", config={"model": "gpt-4", "max_tokens": 2048}
    )
    ...
    gemini = GeminiCLIAdapter(
        endpoint_id="gemini_pro_primary", config={"model": "gemini-pro", "temperature": 0.5}
    )
...
def example_cli_inference():
    ...
    result = execute_cli_inference(
        endpoint_id="claude_code_primary", prompt=prompt, task_type="text_generation", timeout=30
    )
    ...
    result = execute_cli_inference(
        endpoint_id="openai_codex_primary", prompt=prompt, task_type="text_generation", timeout=30
    )
