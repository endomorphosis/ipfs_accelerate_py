...
def demo_bandit_model_recommendation():
    ...
    with ModelManager(storage_path="demo_models.json") as manager:
        ...
        recommender = BanditModelRecommender(
            algorithm="thompson_sampling", model_manager=manager, storage_path="demo_bandit.json"
        )
        ...
        for context_key, context_data in report["contexts"].items():
            ...
            for model_id, arm_data in context_data["arms"].items():
                print(
                    f"    {model_id}: avg={arm_data['average_reward']:.3f}, "
...
def demo_integrated_ai_workflow():
    ...
    if doc_index.load_index():
        ...
        if results:
            ...
            for result in results:
                print(
                    f"  📄 {result.document.file_path} (similarity: {result.similarity_score:.3f})"
                )
    ...
    if recommendation:
        recommender.provide_feedback(
            model_id=recommendation.model_id, feedback_score=feedback_score, context=context
        )
...
def main():
    ...
    try:
        ...
        print("\n📊 Summary:")
        print(
            f"- 📚 Indexed {len(doc_index.documents) if doc_index.documents else 0} documentation sections"
        )
