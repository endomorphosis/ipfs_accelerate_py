    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            recommender = BanditModelRecommender(
                model_manager=manager, algorithm="thompson_sampling"
            )
        ...
        try:
            result = subprocess.run(
                [sys.executable, "test_model_manager.py"],
                capture_output=True,
                text=True,
                timeout=30,
            )
        ...
        try:
            result = subprocess.run(
                [sys.executable, "test_ai_model_discovery.py"],
                capture_output=True,
                text=True,
                timeout=30,
            )
        ...
        try:
            result = subprocess.run(
                [sys.executable, "test_repo_structure_offline.py"],
                capture_output=True,
                text=True,
                timeout=30,
            )
