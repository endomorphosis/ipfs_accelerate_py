...
def demonstrate_basic_performance_simulation():
    ...
    test_models = ["bert-base-uncased", "gpt2", "clip-vit-base-patch32", "whisper-base", "llama-7b"]
    ...
    for model in test_models:
        ...
        for hardware in hardware_options[:3]:  # Test top 3 for brevity
            try:
                ...
                print(
                    f"  {hardware.upper():>8}: {result.inference_time_ms:>6.1f}ms | "
...
def demonstrate_hardware_recommendations():
    ...
    for scenario in scenarios:
        ...
        recommendations = get_hardware_recommendations(scenario["model"], scenario["hardware"])
            ...
            if len(all_options) > 1:
                ...
                for hw, result in all_options.items():
                    if hw != best:
                        ...
                        print(
                            f"      {hw.upper()}: {speedup:.1f}x slower, "
...
def demonstrate_detailed_analysis():
    ...
    try:
        ...
        for hw, comp in analysis["hardware_comparison"].items():
            print(
                f"   {hw.upper():>8}: {comp['inference_time_ms']:>6.1f}ms | "
        ...
        if "error" not in results:
            ...
            for hardware, hw_results in results["hardware_results"].items():
                ...
                for batch_key, metrics in hw_results.items():
                    ...
                    print(
                        f"     Batch {batch_num:>2}: {metrics['inference_time_ms']:>6.1f}ms | "
            ...
            for hardware, config in results["optimal_configurations"].items():
                ...
                print(
                    f"   {hardware.upper():>8}: Batch size {batch_size} "
                    f"-> {perf['inference_time_ms']:.1f}ms "
...
def demonstrate_precision_optimization():
    ...
    for precision in precisions:
        try:
            ...
            print(
                f"   {precision.upper():>4}: {result.inference_time_ms:>6.1f}ms | "
                f"{result.memory_usage_mb:>6.0f}MB | "
                f"{result.efficiency_score:>5.3f} efficiency"
            )
        except Exception as e:
    ...
    if results:
        ...
        print(f"   🏃 Fastest: {fastest.upper()} ({results[fastest].inference_time_ms:.1f}ms)")
        print(
            f"   ⚡ Most efficient: {most_efficient.upper()} "
            f"({results[most_efficient].efficiency_score:.3f})"
        )
        print(
            f"   💾 Lowest memory: {lowest_memory.upper()} "
        ...
        if available_hw:
            ...
            if best_hw in available_hw:
                try:
                    ...
                    print(
                        f"   Expected: {perf.inference_time_ms:.1f}ms inference, "
...
Examples:
  ...
  python performance_analysis.py --hardware cuda   # Test specific hardware
    ...
    parser.add_argument("--quick", action="store_true", help="Run quick performance overview only")
    parser.add_argument("--model", type=str, help="Analyze specific model performance")
    parser.add_argument("--hardware", type=str, nargs="+", help="Test specific hardware options")
    parser.add_argument(
        "--precision",
        type=str,
    ...
    parser.add_argument("--benchmark", action="store_true", help="Run comprehensive benchmarking")
        ...
        if args.precision:
            try:
                result = simulate_model_performance(
                    args.model, hardware_list[0], precision=args.precision
                )
