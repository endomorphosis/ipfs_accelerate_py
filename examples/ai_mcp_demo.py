...
def create_sample_models(manager: ModelManager) -> None:
    ...
    gpt_model = ModelMetadata(
        ...
        architecture="gpt2",
        inputs=[IOSpec("input_ids", DataType.TOKENS, shape=(1024,), description="Token IDs")],
        outputs=[
...
def demonstrate_bandit_recommendations(
    manager: ModelManager, temp_dir: str
) -> BanditModelRecommender:
    ...
    recommender = BanditModelRecommender(
        model_manager=manager, storage_path=bandit_path, algorithm="thompson_sampling"
    )
    ...
    for ctx_info in contexts:
        ...
        if recommendation:
            ...
            recommender.provide_feedback(
                recommendation.model_id, feedback_score, ctx_info["context"]
            )
        ...
        try:
            ...
            if results:
                for i, result in enumerate(results):
                    print(
                        f"  {i + 1}. {result.document.file_path} (score: {result.similarity_score:.3f})"
                    )
            else:
...
async def demonstrate_mcp_server_tools(
    manager: ModelManager, recommender: BanditModelRecommender, doc_index: VectorDocumentationIndex
):
    ...
    with tempfile.TemporaryDirectory() as server_temp:
        ...
        if recommendation:
            print(
                f"  🎯 Recommendation: {recommendation.model_id} (confidence: {recommendation.confidence_score:.3f})"
            )
