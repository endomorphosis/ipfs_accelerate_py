...
class $1 extends $2 {
  $1($2) {
    ...
    this.resources = resources if ($1) ${$1}
      this.metadata = metadata if metadata else {}}}}}}}}}}}}}}}}}}}}}}
      this.deberta = hf_deberta())))))))))))resources=this.resources, metadata=this.metadata)
        ...
        try ${$1} catch($2: $1) {
          ...
          if ($1) {
            ...
            if ($1) {
              ...
              deberta_models = [],name for name in os.listdir())))))))))))cache_dir) if ($1) {
  ...
  $1($2) {
    ...
    try {
      ...
      endpoint, tokenizer, handler, queue, batch_size = this.deberta.init_cpu())))))))))))
      this.model_name,
      "cpu",
      "cpu"
      )
    ...
    if ($1) {
      try {
        ...
        endpoint, tokenizer, handler, queue, batch_size = this.deberta.init_cuda())))))))))))
        this.model_name,
        "cuda",
        "cuda:0"
        )
    ...
    try {
      ...
      if ($1) {
        ...
        try {
          ...
          endpoint, tokenizer, handler, queue, batch_size = this.deberta.init_openvino())))))))))))
