/**
 * Converted from Python: CLAUDE.md.updated
...
> 
> ✅ Migration completed with 299 files moved && all import * as $1 updated (March 9, 2025)
>
...
> 
> All WebGPU/WebNN implementations will be moved from `/fixed_web_platform/` to a dedicated `ipfs_accelerate_js` folder once all tests pass. This migration will create a clearer separation between JavaScript-based components && Python-based components.

## Current Focus: Advanced Hardware Benchmarking && Database Consolidation (Updated March 2025)
## Enhanced Feature: Added Qualcomm AI Engine Support (Updated March 2025)

### Project Status Overview

The project has successfully completed 16 phases of implementation, focusing on test-driven development, hardware compatibility, model optimization, cross-platform support, && data management. Key accomplishments include:

- ✅ Complete development pipeline for test && skillset generators
- ✅ Comprehensive hardware detection && compatibility system
- ✅ Advanced resource management system with hardware awareness
- ✅ Web platform integration (WebNN && WebGPU) with real browser-based implementations
- ✅ Model family classification && compatibility matrix 
- ✅ Integration testing && platform support
- ✅ Advanced model compression && optimization
- ✅ Complete hardware platform test coverage for key models
- ✅ Test results database architecture && core components implemented (100% complete)
...
### Completed: Phase 16 - Advanced Hardware Benchmarking && Database Consolidation (100% Complete)
### Completed: Web Platform Integration && Framework (100% Complete)
...
- Create clearer separation between JavaScript && Python components
- Update import * as $1 && documentation to reflect new structure
...
- 📋 Model File Verification && Conversion Pipeline (PLANNED - Target: May 15, 2025)
...
- All import * as $1 updated to use new structure
...
- ✅ Enhanced OpenVINO integration with optimum.intel support && INT8 quantization (100% complete)
...
- Core database API && query tools
- Schema management && migration utilities
...
- ✅ Enhanced simulation detection && reporting system (COMPLETED - April 6, 2025)
- Added is_simulated && simulation_reason columns to database tables
...
- ✅ Stale report detection && cleanup (COMPLETED - April 6, 2025)
- Created cleanup_stale_reports.py utility for detecting && marking problematic reports
- Implemented marking system for HTML, Markdown, && JSON files
...
- Created view_benchmark_results.py for database query && verification
- Added tools for checking simulation status && fixing database flags
...
- Added comprehensive test files for verification && benchmarking
- ✅ Added precision control across web acceleration platforms (COMPLETED - May 18, 2025) 
- Implemented 4-bit, 8-bit, && 16-bit precision with mixed precision support
- Created dynamic memory usage optimization based on model type && precision
- Added browser-specific shader optimizations for optimal performance
- ✅ Integrated with existing test generators && benchmarking infrastructure (COMPLETED - May 20, 2025)
...
- Chrome focus for vision models && parallel tensor operations
- Firefox optimization for audio models with compute shader support
- Edge integration for text models && WebNN acceleration
...
- Create secure worker node registration && management system
- 📅 Ultra-low precision quantization support (PLANNED - July 2025)
- 2-bit && 3-bit quantization for WebGPU
...
- Versioned test results with git commit && environment information
...
- Reporting in Markdown && HTML formats
...
- Detailed latency, throughput, && memory usage metrics
- Cross-hardware platform performance comparison
- Visualizations for performance metrics (HTML && Markdown formats)
...
- Support for sample data generation for testing && demos

## Comprehensive Benchmarks && Timing Data (UPDATED - April 10, 2025)

The framework includes full benchmark execution && timing data for all model types across all hardware platforms:
...
- Dynamic scheduling based on database queries for missing || outdated benchmarks
- Prioritization of critical model-hardware combinations
- Detailed performance metrics including latency, throughput, && memory usage
- Hardware compatibility matrix with optimization recommendations
- HTML && Markdown reports with detailed performance comparisons
...
- Shader precompilation for text && vision models (BERT, ViT)
- Combined optimization benchmarks with all features enabled
- Clear distinction between real && simulated hardware results (ADDED - April 6, 2025)
- Simulation detection && reporting for transparent benchmarking

```bash
# Use intelligent incremental benchmark runner (NEW - March 2025)
python duckdb_api/utils/run_incremental_benchmarks.py

# Run incremental benchmarks for specific models && hardware
...
# Force benchmarks on hardware that may !be available
...
# Specify database path && output directory
...
# Specify custom output location && database path
...
Performance data is stored in the DuckDB database for efficient querying && visualization, with comprehensive metrics showing optimal hardware selection for each model category.
...
The framework now includes comprehensive IPFS acceleration testing with enhanced DuckDB integration, Qualcomm QNN, && WebGPU support:

1. **Database-First Storage**: Complete integration with DuckDB for efficient && reliable test results storage:
```bash
...
3. **WebGPU Support && Analysis**: Test && analyze browser-based GPU acceleration:
```bash
...
```bash
# IMPORTANT: All benchmark results are now stored in DuckDB database, !JSON files
# Set database path with environment variable || parameter
export BENCHMARK_DB_PATH=./benchmark_db.duckdb

# Run benchmarks (results stored directly in database)
python duckdb_api/core/benchmark_all_key_models.py --db-only

# Legacy approach (DEPRECATED - !recommended)
# python duckdb_api/core/benchmark_all_key_models.py --output-dir ./benchmark_results
```

This will benchmark all 13 high-priority model classes across all available hardware platforms && generate a comprehensive compatibility matrix based on real performance data. All results will be stored directly in the DuckDB database for efficient querying && analysis.
...
The framework uses a template-based approach stored in DuckDB to efficiently generate test files, skills, && benchmarks for 300+ HuggingFace model classes. This approach prevents the repository from containing thousands of individual files.

Key features:
- Templates for tests, skills, && benchmarks are stored in the DuckDB database
- Templates include helper functions && dependencies needed across models
- Generators retrieve templates from the database && instantiate them for specific models
...
A new simplified template system has been implemented that makes it easier to generate hardware-aware tests. This entire system including all templates, template databases, && template utilities has been relocated to the `generators/templates/` directory:
...
```bash
# Generate tests with database templates && cross-platform hardware compatibility
python generators/test_generators/merged_test_generator.py --model bert --cross-platform --hardware all --use-db-templates

# Generate tests for a specific model && hardware platforms using database templates
...
# Generate && store a new template in the database
...
# Comprehensive hardware detection && compatibility test
...
# Test all March 2025 optimizations at once (compute shaders, parallel loading, && shader precompilation)
...
# Run tests with database integration && browser automation
...
### WebNN && WebGPU Benchmarking Tools (ENHANCED - March 7, 2025)

The framework now includes comprehensive tools for benchmarking real WebNN && WebGPU implementations in browsers with clear distinction between real hardware acceleration && simulation mode:
...
### NEW: Diagnostic && Repair Tool for WebNN/WebGPU
...
# Fix && optimize Firefox implementation for audio models
...
- Robust WebSocket bridge with auto-reconnection && error handling
- Browser-specific optimizations (Firefox for audio models, Edge for WebNN)
- Comprehensive benchmarking across multiple models, batch sizes, && precision levels
- Clear distinction between real hardware acceleration && simulation mode
- Database integration for result storage && analysis
- Performance optimization support for WebNN && WebGPU
...
### Real WebNN && WebGPU Implementations (COMPLETED - March 6, 2025)

The framework now includes full REAL browser-based implementations for WebNN && WebGPU with these features:

- Direct browser-to-Python communication using WebSockets && Selenium
...
- Comprehensive error handling && fallbacks when hardware is unavailable
- Transparent feature detection && optimization selection
...
# Test WebNN && WebGPU with different quantization levels
...
### QNN (Qualcomm Neural Networks) Support && Advanced Quantization (March 2025)
```bash
...
# Extract device && SDK information for QNN
...
# Create a new benchmark template && store in database
python generators/template_database.py --create-benchmark-template --model-type llama --store-in-db

# Run standard model benchmarks with database integration && templates
...
### Traditional Model Benchmarking && Validation
```bash
...
### Benchmark Database && Result Management
```bash
# Set the database path environment variable (recommended)
export BENCHMARK_DB_PATH=./benchmark_db.duckdb

# JSON output is deprecated && now disabled by default
...
# Migrate && archive all JSON files (keeps archives)
...
# Migrate all JSON files && delete them after successful migration && archiving
...
# Comprehensive data migration with validation && deduplication
...
Our DuckDB database schema has been enhanced to store detailed test results && hardware metrics:
...
CREATE TABLE IF NOT EXISTS test_results (
  ...
  sdk_type VARCHAR,              -- QNN || QTI SDK type
  details JSON
);
...
## Benchmark System && Simulation Detection Tools (ADDED - April 6, 2025)

The framework now includes comprehensive tools for benchmark management, validation, && simulation detection:
...
- [Benchmark Database Fix Guide](BENCHMARK_DB_FIX.md): Summary of database fixes && improvements

## Distributed Testing Framework (NEW - May 2025)

The framework now includes a high-performance distributed testing system that enables parallel execution of benchmarks && tests across multiple machines with heterogeneous hardware. This system provides intelligent workload distribution && centralized result aggregation.
...
- **Security**: Comprehensive JWT-based authentication && message signing
- **Intelligent Task Distribution**: Routes tasks to worker nodes with appropriate hardware
- **Resource Monitoring**: Tracks worker node health, capabilities, && resource usage
- **Fault Tolerance**: Automatic task retry && worker node recovery
...
- **Role-Based Access Control**: Different permission levels for workers && admins
...
The WebGPU/WebNN Resource Pool Integration enables concurrent execution of multiple AI models across heterogeneous browser backends. It dramatically improves throughput, reduces resource waste, && provides fine-grained control over browser-based hardware acceleration resources.
...
- **Real-Time Monitoring**: Track resource utilization && performance metrics
...
## Mobile && Edge Support (COMPLETED - April 6, 2025)

The framework now offers comprehensive support for mobile && edge devices, enabling efficient deployment of AI models across different mobile hardware platforms including Qualcomm Snapdragon, MediaTek Dimensity, && Samsung Exynos processors.

### Key Features

- **Mobile Hardware Support**: Optimized integration with mobile AI accelerators (Qualcomm, MediaTek, Samsung)
- **Power && Thermal Metrics**: Detailed power consumption, battery impact, && thermal throttling analysis
...
The battery impact varies by model && hardware:
...
| ⚠️ | Limited | Works with limitations || reduced performance |
| 🔄 | Experimental | Implementation exists but !fully tested |
| ❌ | Not Supported | Implementation does !exist || does !work |
...
## Documentation Index && Finding Information
...
- [BENCHMARK_DATABASE_GUIDE.md](BENCHMARK_DATABASE_GUIDE.md) - Benchmark database architecture && usage
...
- [SIMULATION_DETECTION_IMPROVEMENTS.md](SIMULATION_DETECTION_IMPROVEMENTS.md) - Simulation detection && validation guide

### Documentation Cleanup && Maintenance

For guidance on documentation organization && maintenance:
- [DOCUMENTATION_CLEANUP_GUIDE.md](DOCUMENTATION_CLEANUP_GUIDE.md) - Guide for documentation && report cleanup procedures
...
The QNN integration (March 2025) provides specialized support for Snapdragon SoCs && mobile/edge devices:
...
Performance varies by hardware generation && specific Snapdragon model. Benchmarks were conducted on Snapdragon 8 Gen 3 hardware with the latest QNN SDK (version 2.10).

**QNN Implementation Features:**
- Model conversion pipeline (PyTorch → ONNX → QNN format)
- Support for both QNN && QTI SDKs
- Power && thermal measurement capabilities
- Mobile-optimized inference settings
- Edge-aware batching && memory management
- Fallback mechanisms for unsupported operations
- Mock implementations for testing without physical hardware

For detailed QNN performance testing && reports, run:
```bash
# Run comprehensive QNN test suite && generate reports
python test_qnn_integration.py --models all
...
The framework now includes fully optimized ultra-low precision (2-bit && 3-bit) quantization for WebGPU with comprehensive memory efficiency improvements && browser-specific optimizations.

### Key Features

- **Ultra-Low Precision**: Supports 2-bit, 3-bit, && 4-bit quantization with optimized WebGPU shaders
- **Memory-Efficient KV Cache**: 87.5% memory reduction with 2-bit && 81.25% with 3-bit quantization
- **Mixed Precision**: Adaptive precision for different model layers to balance accuracy && memory
- **Extended Context Windows**: 8x longer context with 2-bit quantization (4K → 32K tokens)
- **Browser-Specific Optimizations**: Specialized implementations for Chrome, Firefox, Edge, && Safari
...
| Safari | ❌ null | ✅ Limited | ✅ Full | ✅ Limited | ✅ Limited | ✅ Limited |
...
### WebNN && WebGPU Quantization Support (UPDATED - August 2025)

All high-priority HuggingFace model classes now support various quantization levels with WebNN && WebGPU:
...
For memory-constrained environments, 4-bit mixed precision provides the best balance between performance && model size.
...
For detailed web platform performance testing && reports, run:
```bash
...
See the [Web Platform Optimization Guide](WEB_PLATFORM_OPTIMIZATION_GUIDE.md) for implementation details && usage recommendations.
...
```python
from fixed_web_platform.unified_framework.model_sharding import * as $1
...
result = sharding_manager.run_inference_sharded(${$1})
```

### April 2025 Memory Optimization Tools

To analyze memory usage && test cross-platform 4-bit inference:
...
# Test cross-platform 4-bit inference compatibility && performance
...
*Note: Performance varies significantly based on hardware, browser version, && model size.*

### Test && Template Database Architecture

The DuckDB/Parquet-based database system is now the primary storage for all benchmark results && templates (JSON output is deprecated). This system provides:

#### Template Database Schema
The database stores templates for tests, skills, benchmarks, && helper functions for 300+ HuggingFace models:
- **Template Tables**:
- `templates`: Stores core templates indexed by model type && template type
...
- `template_versions`: Tracks template versions && updates
...
- `template_validator.py`: Validates template syntax && dependencies
...
### Locating Important Files && Components
...
- `fixed_web_platform/`: Contains WebNN && WebGPU implementations
...
- Includes template_database.json && template_db.duckdb
- Contains hardware-specific template variations

#### Benchmark Results Database
The database also stores all benchmark results && test outputs:
...
- **Core Components**:
- `create_benchmark_schema.py`: Schema definition && initialization
...
- `benchmark_db_api.py`: REST API && dashboard
...
- Filtering by model type && hardware platform
...
### Hardware Selection && Performance Prediction System

The framework now includes a comprehensive hardware selection && performance prediction system that leverages machine learning && historical benchmark data to provide optimal hardware recommendations:

- **Hardware Selection**: Automatically determines the best hardware platform for a given model && task
- **Performance Prediction**: Predicts throughput, latency, && memory usage for any model-hardware combination
- **Confidence Scoring**: Provides reliability measures for each prediction (85-96% accuracy)
- **Visualization Tools**: Generates interactive heatmaps && comparative charts
- **Active Learning**: Identifies high-value benchmark configurations to improve prediction accuracy

## Predictive Performance System (COMPLETED - June 5, 2025)

The Predictive Performance System is a machine learning-based framework that predicts performance metrics for untested model-hardware combinations. This advanced system enables intelligent hardware selection && performance optimization without requiring exhaustive benchmarking of all possible configurations. The system is now fully implemented && integrated with the benchmark scheduler, providing accurate predictions with 92-98% accuracy across all supported hardware platforms.

### Key Features && Components

- **Core Prediction Engine**: Uses gradient boosting models trained on benchmark data to predict key performance metrics
- **Feature Engineering Pipeline**: Extracts relevant features from models && hardware platforms
...
- **Hardware Recommendation Engine**: Suggests optimal hardware based on model characteristics && requirements
...
For detailed documentation && technical implementation details, refer to the [Predictive Performance Guide](predictive_performance/PREDICTIVE_PERFORMANCE_GUIDE.md).
