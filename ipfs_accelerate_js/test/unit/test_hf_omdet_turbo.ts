/**
 * Converted from Python: test_hf_omdet_turbo.py
...
class $1 extends $2 {
  """Base test class for all OMDET-TURBO-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = omdet_turbo_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestOmdetTurboModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test OMDET-TURBO-family models")
