/**
 * Converted from Python: test_hf_depth_anything.py
...
class $1 extends $2 {
  """Base test class for all Depth-Anything-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = DEPTH-ANYTHING_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestDepthAnythingModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test Depth-Anything-family models")
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "LiheYoung/depth-anything-small"
    logger.info())))`$1`)
