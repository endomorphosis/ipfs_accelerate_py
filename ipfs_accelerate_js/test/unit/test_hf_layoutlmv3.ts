/**
 * Converted from Python: test_hf_layoutlmv3.py
...
class $1 extends $2 {
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = LAYOUTLMV3_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  try {:
    ...
    # Prepare test input
    if ($1) ${$1} else {
      pipeline_input = this.test_image_url
...
$1($2) {
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "microsoft/layoutlmv3-base"
    logger.info())))`$1`)
