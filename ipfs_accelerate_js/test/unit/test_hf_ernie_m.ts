/**
 * Converted from Python: test_hf_ernie_m.py
...
class $1 extends $2 {
  """Base test class for all ERNIE_M-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = ernie_m_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestErnie_MModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test ERNIE_M-family models")
