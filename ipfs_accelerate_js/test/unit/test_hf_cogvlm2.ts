/**
 * Converted from Python: test_hf_cogvlm2.py
...
class $1 extends $2 {
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = COGVLM2_MODELS_REGISTRY[],this.model_id]
      ,
    ...
    # Define test inputs
      ...
      this.test_text = "Describe this image in detail with emphasis on objects, colors, && context"
      this.test_texts = [],
      "Describe this image in detail with emphasis on objects, colors, && context",
      "Describe this image in detail with emphasis on objects, colors, && context ())))alternative)"
      ]
...
$1($2) {
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "THUDM/cogvlm2-llama3-8b"
    logger.info())))`$1`)
