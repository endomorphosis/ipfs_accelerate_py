/**
 * Converted from Python: test_hf_swin2sr.py
...
class $1 extends $2 {
  """Base test class for all SWIN2SR-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = swin2sr_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestSwin2SrModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test SWIN2SR-family models")
