/**
 * Converted from Python: test_hf_llava_next_video.py
...
class $1 extends $2 {
  """Base test class for all LLaVA-NeXT-Video-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = LLAVA-NEXT-VIDEO_MODELS_REGISTRY[],this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestLlavaNextVideoModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test LLaVA-NeXT-Video-family models")
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "llava-hf/llava-v1.6-vicuna-7b-video"
    logger.info())))`$1`)
