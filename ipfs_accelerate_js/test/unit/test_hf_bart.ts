/**
 * Converted from Python: test_hf_bart.py
...
class $1 extends $2 {
  """Base test class for all BART-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = BART_MODELS_REGISTRY[],this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestBartModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test BART-family models")
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "facebook/bart-large-cnn"
    logger.info())))`$1`)
