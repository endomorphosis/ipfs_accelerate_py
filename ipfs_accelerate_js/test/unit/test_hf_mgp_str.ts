/**
 * Converted from Python: test_hf_mgp_str.py
...
class $1 extends $2 {
  """Base test class for all MGP-STR-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = mgp_str_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestMgpStrModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test MGP-STR-family models")
