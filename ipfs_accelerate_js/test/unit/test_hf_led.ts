/**
 * Converted from Python: test_hf_led.py
...
class $1 extends $2 {
  $1($2) {
    ...
    this.resources = resources if ($1) ${$1}
      ...
      this.led = hf_led())))))))resources=this.resources, metadata=this.metadata)
        ...
        try ${$1} catch($2: $1) {
          ...
          # If all alternatives failed, check local cache
            ...
            if ($1) {
              ...
              led_models = [],name for name in os.listdir())))))))cache_dir) if ($1) {
  ...
  $1($2) {
    ...
    $1: string: Path to the created model
      """
    try {
      console.log($1))))))))"Creating local test model for LED testing...")
      ...
      if ($1) {
        ...
        model_state[],"encoder.layer.0.attention.this.query.weight"] = torch.randn())))))))512, 512)
        model_state[],"encoder.layer.0.attention.this.key.weight"] = torch.randn())))))))512, 512)
        model_state[],"encoder.layer.0.attention.this.value.weight"] = torch.randn())))))))512, 512)
  ...
  $1($2) {
    ...
    try {
      ...
      endpoint, tokenizer, handler, queue, batch_size = this.led.init_cpu())))))))
      this.model_name,
      "summarization",
      "cpu"
      )
    ...
    if ($1) {
      try {
        ...
        endpoint, tokenizer, handler, queue, batch_size = this.led.init_cuda())))))))
        this.model_name,
        "summarization",
        "cuda:0"
        )
    ...
    try {
      ...
      if ($1) {
        ...
        try {
          ...
          endpoint, tokenizer, handler, queue, batch_size = this.led.init_openvino())))))))
