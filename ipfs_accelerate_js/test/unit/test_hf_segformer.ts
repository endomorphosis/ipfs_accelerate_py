/**
 * Converted from Python: test_hf_segformer.py
...
class $1 extends $2 {
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = SEGFORMER_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "nvidia/segformer-b0-finetuned-ade-512-512"
    logger.info())))`$1`)
