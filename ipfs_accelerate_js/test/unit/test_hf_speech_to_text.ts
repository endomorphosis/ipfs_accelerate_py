/**
 * Converted from Python: test_hf_speech_to_text.py
...
class $1 extends $2 {
  """Base test class for all SPEECH_TO_TEXT-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = speech_to_text_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestSpeech_To_TextModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test SPEECH_TO_TEXT-family models")
