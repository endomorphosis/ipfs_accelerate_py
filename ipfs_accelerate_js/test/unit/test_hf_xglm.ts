/**
 * Converted from Python: test_hf_xglm.py
...
class $1 extends $2 {
  """Base test class for all XGLM-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = xglm_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestXglmModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test XGLM-family models")
