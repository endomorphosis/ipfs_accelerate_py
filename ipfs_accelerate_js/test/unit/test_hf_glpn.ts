/**
 * Converted from Python: test_hf_glpn.py
...
class $1 extends $2 {
  """Base test class for all GLPN-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = glpn_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestGlpnModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test GLPN-family models")
