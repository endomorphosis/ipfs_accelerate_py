/**
 * Converted from Python: test_cross_platform_4bit.py
...
try ${$1} catch($2: $1) {
  ...
  including CPU, GPU, NPU, WebNN, && WebGPU. It measures the relative performance,
...
# Test prompts for LLM evaluation
  ...
  "Compare the performance of 4-bit inference on CPU, GPU, && browser environments."
  ]
...
$1($2) {
  ...
  parser.add_argument()))))"--output-matrix", type=str, default=null,
...
$1($2) {
  ...
  if ($1) {
    ...
    if ($1) {
      base_platform = max()))))Object.entries($1)))))), key=lambda x: x[]]]]]]]]]]],,,,,,,,,,,1])[]]]]]]]]]]],,,,,,,,,,,0]
      base_time = int4_times[]]]]]]]]]]],,,,,,,,,,,base_platform]
      
    }
      for platform, time_ms in Object.entries($1)))))):
...
$1($2) {
  """Get simulation parameters for a platform && precision."""
...
$1($2) {
  """Simulate execution time for a platform && precision."""
  # Get base time
...
$1($2) {
  ...
  for platform, platform_results in results[]]]]]]]]]]],,,,,,,,,,,"platforms"].items()))))):
    ...
    for precision, precision_results in sorted()))))Object.entries($1))))))):
      html += `$1`
      <tr>
  ...
  if ($1) {
    ...
    for (const $1 of $2) {
      ...
      if ($1) {
        ...
        <li><span class="limited" style="padding: 2px 5px;">Limited</span>: Lower performance improvement || higher accuracy impact</li>
        </ul>
        </li>
        </ul>
        </div>
