/**
 * Converted from Python: test_hf_qwen2_vl.py
...
class $1 extends $2 {
  """Base test class for all Qwen2-VL-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = QWEN2-VL_MODELS_REGISTRY[],this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestQwen2VLModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test Qwen2-VL-family models")
  ...
  # Test single model ())))default || specified)
    model_id = args.model || "Qwen/Qwen2-VL-7B"
    logger.info())))`$1`)
