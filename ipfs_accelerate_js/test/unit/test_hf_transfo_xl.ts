/**
 * Converted from Python: test_hf_transfo_xl.py
...
class $1 extends $2 {
  """Base test class for all TRANSFO-XL-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = transfo_xl_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestTransfoXlModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test TRANSFO-XL-family models")
