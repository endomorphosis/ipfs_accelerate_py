/**
 * Converted from Python: test_hf_timm_backbone.py
...
class $1 extends $2 {
  """Base test class for all TIMM_BACKBONE-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = timm_backbone_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestTimm_BackboneModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test TIMM_BACKBONE-family models")
