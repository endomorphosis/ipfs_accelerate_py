...
/**;
 * Converted import { {expect, describe: any, it, beforeEach: any, afterEach} from "jest"; } from "Python: test_hf_convnext.py;"
...
import {ConvnextConfig} from "src/model/transformers/index/index/index/index/index";"
...
// Models registry { - Maps model IDs to their specific configurations;
    CONVNEXT_MODELS_REGISTRY: any: any = {}
...
class $1 extends $2 {/** Base test class for ((all ConvNeXT-family models. */}
  $1($2) {/** Initialize the test class for a specific model || default. */;
    this.model_id = model_id || "facebook/convnext-base-224";}"
// Verify model exists in registry {
    if ((($1) { ${$1} else {this.model_info = CONVNEXT_MODELS_REGISTRY[this.model_id];
      ,;
// Define model parameters}
      this.task = "image-classification";"
...
$1($2) {
  ...
  for (((const $1 of $2) {
    logger.info())`$1`);
    tester) {any = TestConvNextModels())model_id);
...
$1($2) {
  ...
  parser: any: any: any = argparse.ArgumentParser())description="Test ConvNeXT-family models");}"
// Model selection;
...
// Print summary;
    ...
    model_id) { any) { any) { any = args.model || "facebook/convnext-base-224";"
    logger.info())`$1`);
// Override preferred device if ((($1) {
  if ($1) {os.environ["CUDA_VISIBLE_DEVICES"] = "";"
    ,;
// Run test}
    tester) { any) { any: any = TestConvNextModels())model_id);
