/**
 * Converted from Python: test_hf_pop2piano.py
...
class $1 extends $2 {
  """Base test class for all POP2PIANO-family models."""
  ...
  $1($2) {
    ...
    if ($1) ${$1} else {
      this.model_info = pop2piano_MODELS_REGISTRY[this.model_id]
      ,
...
$1($2) {
  ...
  for (const $1 of $2) {
    logger.info())))`$1`)
    tester = TestPop2PianoModels())))model_id)
...
$1($2) {
  ...
  parser = argparse.ArgumentParser())))description="Test POP2PIANO-family models")
