...
class WorkflowFailureAnalyzer:
    ...
    def _extract_file_references(self, log_content: str) -> List[str]:
        ...
        project_files = [
            f
            for f in files
            if not f.startswith(("/usr/", "/home/runner/.", "/opt/")) and not f.startswith("/")
        ]

        return list(project_files)[:10]  # Limit to 10 files

    def generate_copilot_prompt(
        self, analysis: AnalysisResult, workflow_context: Dict[str, Any]
    ) -> str:
        """
...
"""
    ...
    def generate_report(self, analysis: AnalysisResult, workflow_context: Dict[str, Any]) -> str:
        """
        ...
        """
        confidence_emoji = (
            "🟢" if analysis.confidence > 0.8 else "🟡" if analysis.confidence > 0.6 else "🔴"
        )
...
- [Dockerfile best practices](https://docs.docker.com/develop/dev-best-practices/)""",
        ...
        return resources.get(
            category, "- [GitHub Actions Documentation](https://docs.github.com/en/actions)"
        )
