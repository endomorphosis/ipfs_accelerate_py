...
class AutoFixer:
    ...
    def _fix_dependency_issues(
        self, analysis: Dict[str, Any], failure_data: Dict[str, Any]
    ) -> bool:
            ...
            for i, line in enumerate(lines):
                if "timeout-minutes:" in line:
                    ...
                    if match:
                        ...
                        new_line = re.sub(
                            r"timeout-minutes:\s*\d+", f"timeout-minutes: {new_timeout}", line
                        )
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python auto_fix_common_issues.py <failure_analysis.json> <detailed_analysis.json>"
        )
        sys.exit(1)
