...
``ProVerifToolchainCertification@1``
...
* exercises secure, attack, mutated claim/model, replay, malformed output,
  cancellation, and version-mismatch cases;
* binds symbolic model ceiling, claims, execution bounds, and exact binaries;
...
so offline tests can prove corpus behavior without a live ProVerif process.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            f"OPAM {LOCKED_OPAM_VERSION}, isolated root)."
        )
    )
