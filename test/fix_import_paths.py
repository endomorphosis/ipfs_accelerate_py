...
SDK_DIR = os.path.abspath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "ipfs_accelerate_js")
)
            ...
            for match in re.finditer(pattern, content):
                        ...
                        if os.path.isdir(parent_dir):
                            ...
                            for name in os.listdir(parent_dir):
                                if name.lower() == base_name.lower():
                                    ...
                                    path_parts[-1] = (
                                        name  # Replace the last part with the correct case
                                    )
                ...
                if fixed_path.startswith("src/"):
                    ...
                    rel_path = os.path.relpath(
                        os.path.join(SDK_DIR, fixed_path), os.path.dirname(file_path)
                    )
                ...
                if fixed_path != import_path:
                    new_import = (
                        match.group(0)
                        .replace(f'"{import_path}"', f'"{fixed_path}"')
                        .replace(f"'{import_path}'", f"'{fixed_path}'")
                    )
