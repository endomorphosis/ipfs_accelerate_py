...
class ServerManager:
    ...
    def start_unified_api(self, gateway_port=8080, predictive_port=8500):
        ...
        cmd = [
            ...
            "--gateway-port",
            str(gateway_port),
            "--predictive-performance-api-port",
            str(predictive_port),
        ]
    ...
    def start_predictive_api(self, host="127.0.0.1", port=8500, db_path=None):
        ...
        try:
            ...
            if process.poll() is not None:
                logger.error(
                    f"Predictive Performance API server failed to start. Return code: {process.poll()}"
                )
                return False
    ...
    def run_both_servers(self, gateway_port=8080, predictive_port=8500, db_path=None):
        ...
        print(
            f"Predictive Performance API: http://localhost:{predictive_port}/api/predictive-performance"
        )
...
def main():
    ...
    parser.add_argument(
        "--predictive-port", type=int, default=8500, help="Port for the Predictive Performance API"
    )
    parser.add_argument("--db-path", type=str, help="Path to DuckDB database file")
    parser.add_argument(
        "--predictive-only",
        action="store_true",
        help="Run only the Predictive Performance API server",
    )
    parser.add_argument(
        "--unified-only", action="store_true", help="Run only the Unified API server"
    )
    ...
    try:
        if args.predictive_only:
            ...
            if not manager.start_unified_api(
                gateway_port=args.gateway_port, predictive_port=args.predictive_port
            ):
                return 1
