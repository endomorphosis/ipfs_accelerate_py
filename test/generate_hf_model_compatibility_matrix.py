...
HARDWARE_PLATFORMS = {
    "cpu": {"name": "CPU", "flag": "--device cpu", "priority": "high"},
    "cuda": {"name": "CUDA (NVIDIA GPU)", "flag": "--device cuda", "priority": "high"},
    "rocm": {"name": "ROCm (AMD GPU)", "flag": "--device rocm", "priority": "high"},
    "mps": {
        "name": "Metal Performance Shaders (Apple Silicon)",
        "flag": "--device mps",
        "priority": "high",
    },
    "openvino": {"name": "OpenVINO (Intel)", "flag": "--device openvino", "priority": "high"},
    "qualcomm": {"name": "Qualcomm AI Engine", "flag": "--device qualcomm", "priority": "medium"},
    "webnn": {"name": "WebNN", "flag": "--web-platform webnn", "priority": "medium"},
    "webgpu": {"name": "WebGPU", "flag": "--web-platform webgpu", "priority": "medium"},
}
...
def analyze_test_files():
    ...
    test_files = [
        f
        for f in FIXED_TESTS_DIR.glob("test_hf_*.py")
...
def run_tests(hardware_platforms=None, max_models=None, priority=None):
    ...
    if priority and priority != "all":
        model_info = {
            ...
            or (priority == "high" and info.get("priority") == "critical")
        }
    ...
    for family, info in model_info.items():
        ...
        for hw in hardware_platforms:
            ...
            # Construct command
            cmd = [sys.executable, test_file, "--model", family]
...
def generate_compatibility_matrix(test_results=None):
    ...
    # JSON format
    json_file = (
        REPORTS_DIR / f"compatibility_matrix_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    return {"json": str(json_file), "markdown": str(md_file), "csv": str(csv_file)}
...
def update_duckdb_matrix(matrix):
    ...
    try:
        ...
        result = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='model_compatibility'"
        ).fetchall()
        ...
        for family, info in matrix.items():
            ...
            result = conn.execute(
                "SELECT * FROM model_compatibility WHERE model_family = ?", [family]
            ).fetchall()
...
def generate_coverage_report():
    ...
    # Generate report
    report_file = (
        REPORTS_DIR / f"model_coverage_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
...
def identify_gaps():
    ...
    with open(report_file, "w") as f:
        ...
        if critical_models:
            ...
            for model in critical_models:
                ...
                f.write(
                    f"   - Suggested template: `templates/{model['architecture_type'].replace('-', '_')}_template.py`\n"
                )
                f.write("\n")
    ...
    return {"report": str(report_file), "data": str(json_file), "missing_models": missing_models}
...
def main():
    ...
    action_group.add_argument("--analyze", action="store_true", help="Analyze existing test files")
    action_group.add_argument(
        "--run-tests", action="store_true", help="Run tests on available hardware platforms"
    )
    action_group.add_argument(
        "--generate-matrix",
        action="store_true",
    ...
    action_group.add_argument(
        "--generate-report", action="store_true", help="Generate coverage report"
    )
    action_group.add_argument(
        "--identify-gaps", action="store_true", help="Identify gaps in model coverage"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        choices=HARDWARE_PLATFORMS.keys(),
    ...
    parser.add_argument(
        ...
        default="all",
        help="Priority level of models to test",
    )
    ...
    if args.analyze:
        ...
        analysis_file = (
            REPORTS_DIR / f"model_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        # Run tests
        results = run_tests(
            hardware_platforms=args.hardware, max_models=args.max_models, priority=args.priority
        )
