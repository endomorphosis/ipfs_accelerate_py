...
if not ENHANCED_INTEGRATION_AVAILABLE:
    ...
    class MockEnhancedResourcePoolIntegration:
        ...
        async def get_model(self, model_name, model_type=None, platform="webgpu", **kwargs):
            ...
            self.metrics["platform_distribution"][platform] = (
                self.metrics["platform_distribution"].get(platform, 0) + 1
            )
            self.metrics["browser_distribution"]["chrome"] = (
                self.metrics["browser_distribution"].get("chrome", 0) + 1
            )
    ...
    class MockResourcePoolBridgeIntegration:
        ...
        def get_execution_stats(self):
            return {
                ...
                "resource_metrics": {
                    "connection_util": 0.5,
                    "browser_usage": {"chrome": 1, "firefox": 0, "edge": 0, "safari": 0},
                },
            }
...
def verify_database_schema(db_path):
    ...
    try:
        ...
        if table_check < 3:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_webnn_webgpu_model_name ON webnn_webgpu_results(model_name)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_webnn_webgpu_browser ON webnn_webgpu_results(browser)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_webnn_webgpu_platform ON webnn_webgpu_results(platform)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_webnn_webgpu_timestamp ON webnn_webgpu_results(timestamp)"
            )
        ...
        try:
            ...
            conn.execute(
                "SELECT id, timestamp, model_name, browser, ipfs_accelerated FROM webnn_webgpu_results LIMIT 0"
            )

            # Validate resource_pool_test_results table
            conn.execute(
                "SELECT id, timestamp, total_tests, ipfs_accelerated_count FROM resource_pool_test_results LIMIT 0"
            )

            # Validate browser_connection_metrics table
            conn.execute(
                "SELECT id, timestamp, browser_name, connection_id FROM browser_connection_metrics LIMIT 0"
            )
...
class WebResourcePoolTester:
    ...
    def __init__(self, args):
        ...
        if (
            hasattr(args, "verify_db_schema")
            and args.verify_db_schema
            and hasattr(args, "db_path")
            and args.db_path
        ):
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"ResourcePoolBridgeIntegration initialized successfully with: {feature_str}"
            )
            ...
            try:
                ...
                if not model:
                    if retry_count < max_retries:
                        ...
                        logger.error(
                            f"Failed to get model after {max_retries} retries: {model_name}"
                        )
                ...
                try:
                    ...
                    if (
                        hardware_preferences["mixed_precision"]
                        or hardware_preferences["precision"] <= 4
                    ):
                    ...
                    try:
                        ...
                        result = await wait_for(
                            anyio.to_thread.run_sync(lambda: model(test_input)), timeout=timeout
                        )
                    except TimeoutError:
                ...
                mixed_precision = result.get(
                    "mixed_precision", hardware_preferences["mixed_precision"]
                )

                logger.info(
                    f"Test complete - Browser: {browser_name}, Real Implementation: {is_real}, "
                ...
                logger.info(
                    f"Test completed in {execution_time:.2f}s (load: {loading_time:.2f}s): {model_name}"
                )
                ...
                if retry_count < max_retries:
                    ...
                    return self._create_error_result(
                        model_name, model_type, platform, str(e), browser_specific_errors
                    )

        # Should never reach here due to return in the loop
        return self._create_error_result(
            model_name, model_type, platform, "Unknown error", browser_specific_errors
        )
        ...
        try:
            ...
            for model_info in models:
                ...
                if model_type == "audio":
                    ...
                    hardware_preferences["precompile_shaders"] = True
                    logger.info(
                        f"Using Chrome with shader precompilation for vision model: {model_name}"
                    )
            ...
            for i, result in enumerate(results):
                if i < len(model_configs):
                    ...
                    test_result = {
                        ...
                        "ipfs_cache_hit": result.get("ipfs_cache_hit", False),
                        "browser": result.get(
                            "browser", config["preferences"].get("browser", "unknown")
                        ),
                        "compute_shader_optimized": result.get("compute_shader_optimized", False),
                        "precompile_shaders": result.get("precompile_shaders", False),
                        "parallel_loading": result.get("parallel_loading", False),
                    ...
                    logger.info(
                        f"Concurrent model {model_name} - Browser: {browser_name}, "
            ...
            ipfs_accelerated = sum(
                1 for r in concurrent_results if r.get("ipfs_accelerated", False)
            )
            real_impl = sum(1 for r in concurrent_results if r.get("is_real_implementation", False))

            logger.info(
                f"Concurrent execution summary: {len(models)} models in {execution_time:.2f}s"
            )
            logger.info(f"Performance: {len(models) / execution_time:.2f} models/second")
            logger.info(
                f"Stats: IPFS Acceleration: {ipfs_accelerated}/{len(models)}, "
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                for i in range(0, len(models), batch_size):
                    ...
                    ipfs_accelerated_count += sum(
                        1 for r in results if r.get("ipfs_accelerated", False)
                    )
                    ipfs_cache_hits += sum(1 for r in results if r.get("ipfs_cache_hit", False))
                    real_implementations += sum(
                        1 for r in results if r.get("is_real_implementation", False)
                    )
                ...
                logger.info(
                    f"Progress: {elapsed:.1f}s / {duration}s - "
                    f"Remaining: {remaining:.1f}s - "
                    f"Executions: {total_executions} (Success: {successful_executions})"
                )
                logger.info(
                    f"Current Throughput: {current_throughput:.2f} models/sec, "
                ...
                if "resource_metrics" in stats and "browser_usage" in stats["resource_metrics"]:
                    ...
                    logger.info(
                        ...
                        + ", ".join(
                            [
                                f"{browser}: {count}"
                                for browser, count in browser_usage.items()
                                if count > 0
                            ]
                        )
                    )
            ...
            logger.info(
                f"Success Rate: {(successful_executions / total_executions * 100) if total_executions else 0:.1f}%"
            )
            logger.info(f"Total Throughput: {total_executions / total_time:.2f} executions/sec")
            logger.info(
                f"IPFS Acceleration: {ipfs_accelerated_count}/{total_executions} ({ipfs_accelerated_count / total_executions * 100 if total_executions else 0:.1f}%)"
            )
            logger.info(
                f"IPFS Cache Hits: {ipfs_cache_hits}/{ipfs_accelerated_count} ({ipfs_cache_hits / ipfs_accelerated_count * 100 if ipfs_accelerated_count else 0:.1f}%)"
            )
            logger.info(
                f"Real Implementations: {real_implementations}/{total_executions} ({real_implementations / total_executions * 100 if total_executions else 0:.1f}%)"
            )
            ...
            if "bridge_stats" in final_stats:
                ...
                logger.info(
                    f"  - Created Connections: {bridge_stats.get('created_connections', 0)}"
                )
                logger.info(
                    f"  - Current Connections: {bridge_stats.get('current_connections', 0)}"
                )
    ...
    def save_results(self):
        ...
        real_implementations = sum(
            1 for r in self.results if r.get("is_real_implementation", False)
        )
        ...
        report = {
            ...
            "ipfs_acceleration": {
                ...
                "cache_hit_rate": (ipfs_cache_hits / ipfs_accelerated * 100)
                if ipfs_accelerated
                else 0,
            },
        ...
        for browser, results in by_browser.items():
            ...
            cache_hits = sum(1 for r in results if r.get("ipfs_cache_hit", False))
            real_count = sum(1 for r in results if r.get("is_real_implementation", False))
            compute_shader_count = sum(
                1 for r in results if r.get("compute_shader_optimized", False)
            )
        ...
        if (
            self.args.db_path
            and hasattr(self.integration, "db_connection")
            and self.integration.db_connection
        ):
            try:
                # Store in database
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(
        description="Test WebNN/WebGPU Resource Pool Integration with IPFS Acceleration"
    )
    ...
    parser.add_argument(
        "--stress-test", action="store_true", help="Run a stress test on the resource pool"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        default=os.environ.get("BENCHMARK_DB_PATH"),
        help="Path to DuckDB database for storing test results",
    )
    parser.add_argument(
        "--db-only", action="store_true", help="Store results only in database (no JSON files)"
    )

    # Browser-specific options
    parser.add_argument(
        "--firefox", action="store_true", help="Use Firefox for all tests (best for audio models)"
    )
    parser.add_argument(
        "--chrome", action="store_true", help="Use Chrome for all tests (best for vision models)"
    )
    parser.add_argument(
        "--edge", action="store_true", help="Use Edge for all tests (best for WebNN)"
    )
    ...
    parser.add_argument(
        "--mixed-precision", action="store_true", help="Enable mixed precision inference"
    )
    ...
    parser.add_argument(
        "--error-retry", type=int, default=1, help="Number of times to retry on error"
    )
    ...
    if args.test_enhanced:
        ...
        try:
            from test.web_platform.enhanced_resource_pool_tester import (
                EnhancedWebResourcePoolTester,
            )

            tester = EnhancedWebResourcePoolTester(args)
        except ImportError:
            logger.info("Using mock EnhancedWebResourcePoolTester")
            from test.web_platform.enhanced_resource_pool_tester_mock import (
                EnhancedWebResourcePoolTester,
            )
        ...
        if args.stress_test:
            ...
            for model_type, model_name in models:
                ...
                # For text embedding with WebNN, prefer Edge
                    ...
                    and args.platform == "webnn"
                    and not args.chrome
                    and not args.firefox
                ):
                    logger.info(f"Using Edge for text model with WebNN: {model_name}")
                    os.environ["TEST_BROWSER"] = "edge"
