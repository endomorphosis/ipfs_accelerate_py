...
def generate_model_tests(output_dir: str) -> List[str]:
    ...
    for test_params in model_tests:
        ...
        model_dir = os.path.join(
            ...
            test_params["model_name"].split("/")[-1],
        )
...
def generate_hardware_tests(output_dir: str) -> List[str]:
    ...
    hardware_tests = [
        # WebGPU tests
        {
            ...
            "test_name": "matmul_performance",
            "test_operation": "matmul",
            "test_category": "compute",
        },
        {
            "hardware_platform": "webgpu",
            "test_name": "conv_performance",
            "test_operation": "conv",
            "test_category": "compute",
        },
        {
            "hardware_platform": "webgpu",
            "test_name": "memory_transfer",
            "test_operation": "memory",
            "test_category": "memory",
        },
        # WebNN tests
        {
            "hardware_platform": "webnn",
            "test_name": "graph_execution",
            "test_operation": "inference",
            "test_category": "compute",
        },
        {
            "hardware_platform": "webnn",
        ...
        # MPS tests
        {
            ...
            "test_name": "apple_silicon_performance",
            "test_operation": "inference",
            "test_category": "compute",
        },
    ]
    ...
    for test_params in hardware_tests:
        ...
        hardware_dir = os.path.join(
            output_dir, "hardware", test_params["hardware_platform"], test_params["test_category"]
        )
        os.makedirs(hardware_dir, exist_ok=True)

        # Create the template
        template = HardwareTestTemplate(parameters=test_params, output_dir=hardware_dir)
...
def generate_api_tests(output_dir: str) -> List[str]:
    ...
    for test_params in api_tests:
        ...
        # Create the template
        template = APITestTemplate(parameters=test_params, output_dir=api_dir)
...
def main() -> int:
    ...
    parser.add_argument(
        "--types",
        choices=["model", "hardware", "api", "all"],
        default="all",
