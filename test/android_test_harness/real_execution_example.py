    ...
    try:
        ...
        for accelerator, batch_results in accelerator_results.items():
            ...
            for batch_size, configs in sorted(batch_results.items()):
                avg_latency = sum(
                    config.get("latency_ms", {}).get("mean", 0) for config in configs
                ) / max(1, len(configs))
        ...
        for accelerator, batch_results in accelerator_results.items():
            ...
            for batch_size, configs in sorted(batch_results.items()):
                avg_throughput = sum(
                    config.get("throughput_items_per_second", 0) for config in configs
                ) / max(1, len(configs))
...
def run_thermal_analysis(
    ...
    analysis_results = {
        ...
        "temperature_time_series": [
            {"timestamp": timestamp, "temperatures": temps}
...
def plot_thermal_analysis(results: Dict[str, Any], output_file: str) -> None:
    ...
    try:
        ...
        for zone in important_zones:
            zone_temps = [entry["temperatures"].get(zone, float("nan")) for entry in time_series]
        ...
        for i, timestamp in enumerate(timestamps):
            ...
            for event in throttling_events:
                event_time = (
                    event["timestamp"]
                    - results.get("thermal_report", {}).get("timestamp", 0)
                    + timestamps[0]
                )
        ...
        plt.yticks([0, 1, 2, 3, 4, 5], ["None", "Mild", "Moderate", "Heavy", "Severe", "Emergency"])
        plt.grid(True, linestyle="--", alpha=0.7)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Android Test Harness - Real Model Execution Example"
    )

    parser.add_argument("--model", required=True, help="Path to model file")
    parser.add_argument("--name", help="Model name (defaults to filename)")
    parser.add_argument(
        "--type", default="onnx", choices=["onnx", "tflite", "tflite_quantized"], help="Model type"
    )
    ...
    parser.add_argument(
        "--analysis",
        default="comparison",
        choices=["comparison", "thermal", "both"],
        help="Type of analysis to perform",
    )

    # Comparison options
    parser.add_argument(
        "--batch-sizes", default="1,2,4", help="Comma-separated batch sizes for comparison"
    )
    parser.add_argument(
        "--accelerators", default="auto,cpu,gpu", help="Comma-separated accelerators for comparison"
    )
    parser.add_argument("--iterations", type=int, default=50, help="Number of iterations per test")

    # Thermal analysis options
    parser.add_argument(
        "--duration", type=int, default=300, help="Duration of thermal analysis in seconds"
    )
    ...
    if args.analysis == "comparison" or args.analysis == "both":
        ...
        if comparison_results.get("status") == "success":
            ...
            if best_config:
                ...
                print(
                    f"- Best configuration: batch_size={config_info.get('batch_size', 1)}, "
    ...
    if args.analysis == "thermal" or args.analysis == "both":
        ...
        if thermal_results.get("status") == "success":
            ...
            if throttling_stats.get("throttling_detected", False):
                print(
                    f"- Throttling level: {throttling_stats.get('throttling_level', 0)} "
                    f"({throttling_stats.get('level_description', 'Unknown')})"
                )
                print(
                    f"- Throttling duration: {throttling_stats.get('throttling_time_seconds', 0):.1f} seconds"
                )
                print(
                    f"- Performance impact: {throttling_stats.get('performance_impact', 0) * 100:.1f}%"
                )
