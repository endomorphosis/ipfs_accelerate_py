...
def run_thermal_analysis(
    ...
    try:
        ...
        results = {
            ...
            "baseline": {"temperatures": baseline_temps, "battery": baseline_battery},
            "configuration": {
        ...
        temp_deltas = {
            zone: final_temps[zone] - baseline_temps.get(zone, 0) for zone in final_temps.keys()
        }
        ...
        battery_impact = {
            ...
            "percent_per_hour": (baseline_battery["level"] - final_battery["level"])
            * (3600 / (final_time - start_time)),
        }
...
def generate_report(
    results: Dict[str, Any], report_format: str = "markdown", output_path: Optional[str] = None
) -> str:
    """
...
def _generate_markdown_report(results: Dict[str, Any]) -> str:
    ...
    report += (
        f"- **Estimated Drain**: {battery_impact.get('percent_per_hour', 0):.1f}% per hour\n\n"
    )
    ...
    if perf:
        ...
        if correlation:
            ...
            max_corr_zone, max_corr = max(
                correlation.items(), key=lambda x: abs(x[1]), default=(None, 0)
            )
...
def main():
    ...
    parser.add_argument(
        "--interval", type=float, default=1.0, help="Thermal sampling interval in seconds"
    )
    ...
    parser.add_argument(
        "--report-format", default="markdown", choices=["markdown", "html"], help="Report format"
    )
    ...
    try:
        ...
        if results.get("status") == "success":
            ...
            if args.report:
                generate_report(
                    results=results, report_format=args.report_format, output_path=args.report
                )
            ...
            print(
                f"Battery Impact: {impact.get('battery_impact', {}).get('percent_per_hour', 0):.1f}% per hour"
            )
