...
try:
    ...
    logger.warning(
        "Could not import Android database integration. Some functionality will be limited."
    )
    ANDROID_DB_AVAILABLE = False


def get_cross_platform_comparison(
    db_path: str, model_name: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
...
def get_benchmark_results(
    db_path: str, model_name: Optional[str] = None, device_model: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
...
def generate_cross_platform_report(
    db_path: str, output_path: Optional[str] = None, model_name: Optional[str] = None
) -> str:
    """
    ...
    # Find average ratios
    avg_throughput_ratio = sum(item.get("throughput_ratio", 0) for item in comparison) / len(
        comparison
    )
...
def generate_device_comparison_report(db_path: str, output_path: Optional[str] = None) -> str:
    ...
    for accelerator, accel_devices in accelerators.items():
        ...
        avg_battery_impact = sum(d.get("avg_battery_impact", 0) for d in accel_devices) / len(
            accel_devices
        )
        avg_throttling = (
            sum(d.get("throttling_frequency", 0) for d in accel_devices) / len(accel_devices) * 100
        )
    ...
    report += (
        "- **Thermal Management**: Implement cooling breaks for devices with high throttling\n"
    )
    report += (
        "- **Battery Optimization**: Apply model quantization for better battery efficiency\n\n"
    )
...
def generate_model_optimization_report(db_path: str, output_path: Optional[str] = None) -> str:
    ...
    report += (
        "| Model | Family | Performance Gap | Battery Impact | Priority | Key Optimizations |\n"
    )
    report += (
        "|-------|--------|----------------|----------------|----------|-------------------|\n"
    )
    ...
    for family, family_models in families.items():
        ...
        if family.lower() in ["bert", "distilbert", "albert"]:
            ...
            report += (
                "- **Feature Extraction Optimization**: Optimize mel spectrogram computation\n"
            )
    ...
    sorted_models = sorted(
        ...
        key=lambda x: priority_map.get(
            "Critical"
            if x.get("model_name") in comparison_map
            and comparison_map[x.get("model_name")].get("throughput_ratio", 1) > 10
            else "High"
            if x.get("model_name") in comparison_map
            and comparison_map[x.get("model_name")].get("throughput_ratio", 1) > 5
            else "Medium"
            if x.get("model_name") in comparison_map
            and comparison_map[x.get("model_name")].get("throughput_ratio", 1) > 2
            else "Low",
            3,
        ),
    )
    ...
    critical_models = [
        ...
        if model.get("model_name") in comparison_map
        and comparison_map[model.get("model_name")].get("throughput_ratio", 1) > 10
    ]

    for model in critical_models:
        model_name = model.get("model_name", "Unknown")
        report += (
            f"- **{model_name}**: Apply pruning, knowledge distillation, and INT4 quantization\n"
        )
    ...
    high_models = [
        ...
        for model in sorted_models
        if model.get("model_name") in comparison_map
        and 5 < comparison_map[model.get("model_name")].get("throughput_ratio", 1) <= 10
    ]
    ...
    medium_models = [
        ...
        for model in sorted_models
        if model.get("model_name") in comparison_map
        and 2 < comparison_map[model.get("model_name")].get("throughput_ratio", 1) <= 5
    ]
...
def main():
    ...
    cross_parser = subparsers.add_parser(
        "cross-platform", help="Generate cross-platform comparison report"
    )
    ...
    device_parser = subparsers.add_parser(
        "device-comparison", help="Generate device comparison report"
    )
    ...
    model_parser = subparsers.add_parser(
        "model-optimization", help="Generate model optimization report"
    )
