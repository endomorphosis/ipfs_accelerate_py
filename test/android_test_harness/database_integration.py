...
class AndroidDatabaseSchema:
    ...
    def verify_schema(self) -> bool:
        ...
        try:
            ...
            tables = ["android_benchmark_results", "android_thermal_analysis"]
...
class AndroidDatabaseAPI:
    ...
    def store_benchmark_result(self, result: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            max_temp = (
                max(thermal_metrics.get("post", {}).values()) if thermal_metrics.get("post") else 0
            )
    ...
    def store_thermal_analysis(
        self, analysis: Dict[str, Any], benchmark_id: Optional[int] = None
    ) -> Optional[int]:
        """
    ...
    def get_benchmark_results(
        self, model_name: Optional[str] = None, device_model: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
            ...
            for row in rows:
                result = {
                    ...
                    "throughput_ratio": row[
                        7
                    ],  # desktop / android (higher means desktop is faster)
...
def main():
    ...
    store_parser.add_argument(
        "--result-file", required=True, help="Path to benchmark result JSON file"
    )

    # Store thermal analysis command
    store_thermal_parser = subparsers.add_parser(
        "store-thermal", help="Store thermal analysis result"
    )
    store_thermal_parser.add_argument("--db-path", required=True, help="Path to DuckDB database")
    store_thermal_parser.add_argument(
        "--analysis-file", required=True, help="Path to thermal analysis JSON file"
    )
    store_thermal_parser.add_argument(
        "--benchmark-id", type=int, help="Optional benchmark ID to link with"
    )

    # Get benchmark results command
    get_benchmark_parser = subparsers.add_parser(
        "get-benchmarks", help="Get Android benchmark results"
    )
    ...
    get_benchmark_parser.add_argument(
    ...
    get_comparison_parser = subparsers.add_parser(
        "get-comparison", help="Get cross-platform performance comparison"
    )
    ...
    get_device_parser = subparsers.add_parser(
        "get-devices", help="Get Android device performance summary"
    )
        ...
        if args.command == "create-schema":
            ...
            if args.output:
                ...
                for result in results:
                    print(f"Model: {result['model_name']}")
                    print(
                        f"  Android: {result['android_throughput']:.2f} items/s, {result['android_latency_ms']:.2f} ms"
                    )
                    print(
                        f"  Desktop: {result['desktop_throughput']:.2f} items/s, {result['desktop_latency_ms']:.2f} ms"
                    )
