...
def parse_args():
    ...
    validate_parser.add_argument(
        "--output-dir",
        default="./simulation_validation_results",
    ...
    validate_parser.add_argument(
        "--hardware-types", nargs="+", help="List of hardware types to include"
    )
    validate_parser.add_argument("--model-types", nargs="+", help="List of model types to include")
    validate_parser.add_argument(
        "--time-range", type=int, default=90, help="Time range in days to include"
    )
    validate_parser.add_argument("--metrics", nargs="+", help="List of metrics to validate")
    validate_parser.add_argument(
    ...
    analyze_parser.add_argument(
        "--input-file", required=True, help="Path to the validation results file"
    )
    analyze_parser.add_argument(
        "--output-file", help="Path to save analysis results (defaults to input_file_analysis.json)"
    )
    analyze_parser.add_argument(
        "--include-drift", action="store_true", help="Include drift analysis"
    )
    analyze_parser.add_argument(
        "--db-path", help="Path to the DuckDB database (required for drift analysis)"
    )
    analyze_parser.add_argument(
        "--time-window", type=int, default=90, help="Time window in days for drift analysis"
    )
    analyze_parser.add_argument(
        "--by-hardware", action="store_true", help="Analyze by hardware type"
    )
    analyze_parser.add_argument("--by-model", action="store_true", help="Analyze by model type")
    analyze_parser.add_argument("--by-metric", action="store_true", help="Analyze by metric")
    analyze_parser.add_argument(
        "--worst-configs", action="store_true", help="Identify worst-performing configurations"
    )
    analyze_parser.add_argument(
        "--calibration-candidates", action="store_true", help="Identify calibration candidates"
    )
    ...
    full_parser.add_argument("--model-types", nargs="+", help="List of model types to include")
    full_parser.add_argument(
        "--time-range", type=int, default=90, help="Time range in days to include"
    )
    full_parser.add_argument("--metrics", nargs="+", help="List of metrics to validate")
    full_parser.add_argument(
        "--confidence-threshold",
        type=float,
        default=0.9,
    ...
    full_parser.add_argument("--include-drift", action="store_true", help="Include drift analysis")
    full_parser.add_argument("--all-analyses", action="store_true", help="Run all analyses")
        ...
        if run_drift:
            if not HAS_DB_API or not args.db_path:
                ...
                results["drift_over_time"] = analyzer.analyze_drift_over_time(
                    time_window=args.time_window, db_api=db_api
                )
