...
class TestBenchmarkDatabaseMerger:
    ...
    def test_find_input_files(self, temp_db_files):
        ...
        merger = BenchmarkDatabaseMerger(output_db=output_db, input_dbs=[android_db, ios_db])
        ...
        merger = BenchmarkDatabaseMerger(
            output_db=output_db, input_dir=temp_dir, pattern="*.duckdb"
        )
        input_files = merger.find_input_files()
        assert len(input_files) == 2
        ...
        # Test with both
        merger = BenchmarkDatabaseMerger(
            output_db=output_db, input_dbs=[android_db], input_dir=temp_dir, pattern="ios_*.duckdb"
        )
        input_files = merger.find_input_files()
        assert len(input_files) == 2
        ...
        merger = BenchmarkDatabaseMerger(
            output_db=output_db, input_dbs=[android_db, "non_existent.duckdb"]
        )
        input_files = merger.find_input_files()
    ...
    def test_merge_all_databases(self, mock_benchmark_db_api_class, mock_db_api, temp_db_files):
        ...
        merger = BenchmarkDatabaseMerger(output_db=output_db, input_dbs=[])
