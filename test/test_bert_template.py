...
class TestTextEmbeddingModel:
    ...
    def create_qualcomm_handler(self):
        ...
        try:
            ...
            if not (has_qnn or has_qti):
                print(
                    "Warning: Neither Qualcomm QNN SDK nor QTI SDK found, using mock implementation"
                )
                return lambda text, **kwargs: {
                    "embeddings": [0.0] * 768,
                    "implementation_type": "MOCK_QUALCOMM",
                }
            ...
            if has_qnn:
                try:
                    ...
                    qnn.convert_model(input_model=onnx_path, output_model=qnn_path)
            ...
            # If QTI SDK is available but not QNN
                ...
                try:
                    ...
                    qti.aisw.dlc_utils.convert_onnx_to_dlc(
                        input_model=onnx_path, output_model=dlc_path
                    )
    ...
    def run(self, platform="CPU", mock=False):
        ...
        try:
            ...
            print(
                f"Got embedding with shape: {result['embedding'].shape if hasattr(result['embedding'], 'shape') else 'N/A'}"
            )
