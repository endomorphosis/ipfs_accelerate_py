...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"vision_text_visualization_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
    ...
    try:
        ...
        for model_type in model_types:
            ...
            fig = px.bar(
                ...
                category_orders={
                    "hardware_platform": sorted(model_data["hardware_platform"].unique())
                },
                color_discrete_map={
                    "cpu": "#1f77b4",
                    "cuda": "#ff7f0e",
            ...
            for i, trace in enumerate(fig.data):
                fig.data[i].customdata = model_data[model_data["hardware_platform"] == trace.name][
                    "test_count"
                ]
            ...
            if export_format and export_format != "html":
                export_file = (
                    VISUALIZATIONS_DIR
                    / f"{model_type}_performance_comparison_{timestamp}.{export_format}"
                )
                fig.write_image(str(export_file))
                logger.info(
                    f"Exported {model_type} performance comparison as {export_format}: {export_file}"
                )
...
def create_compatibility_heatmap(data, theme="light", export_format=None) -> Tuple[bool, str]:
    ...
    try:
        ...
        for model_type in model_types:
            ...
            # Create heatmap
            fig = px.imshow(
                pd.pivot_table(matrix_df, values="Value", index="Model", columns="Hardware"),
            ...
            fig.update_layout(
                ...
                coloraxis_colorbar=dict(
                    title="Compatible", tickvals=[0, 1], ticktext=["No", "Yes"]
                ),
                margin=dict(t=50, b=100),
            )
            ...
            tasks = {
                model: task for model, task in zip(model_data["short_model_id"], model_data["task"])
            }
            ...
            fig.update_traces(
                customdata=[[tasks.get(y, "")] for y in fig.data[0].y], hovertemplate=hovertemplate
            )
            ...
            if export_format and export_format != "html":
                export_file = (
                    VISUALIZATIONS_DIR
                    / f"{model_type}_compatibility_heatmap_{timestamp}.{export_format}"
                )
                fig.write_image(str(export_file))
                logger.info(
                    f"Exported {model_type} compatibility heatmap as {export_format}: {export_file}"
                )
        ...
        for model_type in model_types:
            ...
            for model in models:
                ...
                if export_format and export_format != "html":
                    export_file = (
                        VISUALIZATIONS_DIR
                        / f"{short_model_id}_time_series_{timestamp}.{export_format}"
                    )
                    fig.write_image(str(export_file))
                    logger.info(
                        f"Exported {short_model_id} time series as {export_format}: {export_file}"
                    )
...
def create_interactive_dashboard(
    performance_data, compatibility_data, theme="light"
) -> Tuple[bool, str]:
    ...
    try:
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "bar"}, {"type": "heatmap"}], [{"type": "pie"}, {"type": "pie"}]],
    ...
    try:
        ...
        for model_type in model_types:
            ...
            if export_format and export_format != "html":
                export_file = (
                    VISUALIZATIONS_DIR
                    / f"{model_type}_statistical_analysis_{timestamp}.{export_format}"
                )
                fig.write_image(str(export_file))
                logger.info(
                    f"Exported {model_type} statistical analysis as {export_format}: {export_file}"
                )
...
def main():
    ...
    viz_group.add_argument(
        "--time-series", action="store_true", help="Create time series visualization"
    )
    viz_group.add_argument(
    ...
    viz_group.add_argument("--dashboard", action="store_true", help="Create interactive dashboard")
    viz_group.add_argument("--all", action="store_true", help="Create all visualization types")
    ...
    parser.add_argument("--db-path", type=str, default=str(DB_PATH), help="Path to DuckDB database")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--browser", action="store_true", help="Open visualizations in browser")
