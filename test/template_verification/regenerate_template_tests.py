...
def generate_model_registry_entry(
    model_name: str, model_info: Dict[str, Any], template_path: str
) -> str:
    ...
    try:
        ...
        entry_format = entry_format.replace(
            '"description": "[^"]+",', f'"description": "{model_name.upper()} model",'
        )
        entry_format = entry_format.replace(
            '"class": "[^"]+",', f'"class": "{model_info["class"]}",'
        )
        entry_format = entry_format.replace(
            '"default_model": "[^"]+",', f'"default_model": "{model_info["model_id"]}",'
        )
...
def regenerate_test_file(
    model_name: str, model_info: Dict[str, Any], verify: bool = True
) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if os.path.exists(model_info["destination_file"]):
            backup_path = (
                f"{model_info['destination_file']}.bak.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        if registry_entry:
            ...
            if registry_start != -1 and registry_end != -1:
                ...
                content = (
                    content[: registry_start + len(f"{registry_name} = {{") + 1]
                    + "\n"
                    + registry_entry
                    + content[registry_start + len(f"{registry_name} = {{") + 1 :]
                )
...
def update_architecture_types_file(model_name: str, architecture: str) -> bool:
    ...
    try:
        ...
        if (
            f'"{model_name}"' in arch_type_section
            or f'"{model_name.replace("_", "-")}"' in arch_type_section
        ):
            logger.info(
                f"Model '{model_name}' already exists in architecture type '{architecture}'"
            )
            return True
...
def regenerate_specific_model(
    model_name: str, verify: bool = True, apply: bool = False
) -> Tuple[bool, List[str]]:
...
def main():
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Apply changes to architecture types dictionary"
    )
    ...
    if args.model:
        ...
        success, errors = regenerate_specific_model(
            args.model, verify=args.verify, apply=args.apply
        )
