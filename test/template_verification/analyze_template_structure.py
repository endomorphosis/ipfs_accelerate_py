...
def compare_with_template(test_path: str, template_path: str) -> Dict[str, Any]:
    ...
    for section in [
        ...
        pattern = f"# {section}"
        if (
            pattern.lower() in template_content.lower()
            and pattern.lower() not in test_content.lower()
        ):
...
def generate_template_fix_recommendation(
    test_path: str, template_path: str, architecture: str
) -> str:
    ...
    if conformance_score < 50:
        ...
        recommendations.append(
            "This file has significant structural differences from the template."
        )
    ...
    missing_components = [
        comp for comp, present in test_analysis.get("components", {}).items() if not present
    ]
    ...
    if "test_class" in missing_components:
        ...
        recommendations.append(
            "Add a properly structured test class with methods for pipeline and from_pretrained testing."
        )
        recommendations.append("")
...
def analyze_all_manual_models():
    ...
    for model, architecture in MANUAL_MODEL_ARCHITECTURES.items():
        ...
        recommendations = generate_template_fix_recommendation(
            model_path, template_path, architecture
        )
...
def generate_summary_report(results):
    ...
    summary.append("")
    summary.append(
        "| Model | Architecture | Conformance Score | Syntax Errors | Missing Components |"
    )
    summary.append(
        "|-------|--------------|------------------|---------------|-------------------|"
    )
    ...
    for model, result in results.items():
        ...
        missing_components = ", ".join(
            [comp for comp, present in analysis.get("components", {}).items() if not present][:3]
        )
        if (
            len([comp for comp, present in analysis.get("components", {}).items() if not present])
            > 3
        ):
            missing_components += ", ..."
        ...
        summary.append(
            f"| {model} | {architecture} | {conformance_score:.1f}% | {syntax_errors} | {missing_components} |"
        )
    ...
    avg_score = (
        sum(result.get("analysis", {}).get("conformance_score", 0) for result in results.values())
        / len(results)
        if results
        else 0
    )
    ...
    if avg_score < 50:
        summary.append(
            "The manually created model tests have significant template conformance issues."
        )
        summary.append(
            "It is recommended to regenerate all of these tests using the template system."
        )
    elif avg_score < 80:
        summary.append(
            "The manually created model tests have moderate template conformance issues."
        )
