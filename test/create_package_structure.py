...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show actions without creating directories"
    )
    ...
    if args.dry_run:
        print(
            f"Would create {total_dirs_created} directories and {total_inits_created} __init__.py files"
        )
        print("Run without --dry-run to actually create the structure")
    else:
        print(
            f"Created {total_dirs_created} directories and {total_inits_created} __init__.py files"
        )
        print("\nNext steps:")
