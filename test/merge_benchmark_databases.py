...
class BenchmarkDatabaseMerger:
    ...
    def find_input_files(self) -> List[str]:
        ...
        if self.input_dir and os.path.isdir(self.input_dir):
            ...
            logger.info(
                f"Found {len(dir_files)} files matching pattern '{self.pattern}' in '{self.input_dir}'"
            )
    ...
    def connect_to_output_db(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(self.output_db):
                backup_path = (
                    f"{self.output_db}.bak.{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
                )
...
def main():
    ...
    parser.add_argument(
        "--pattern",
        default="*.duckdb",
        help="Glob pattern to match database files (default: *.duckdb)",
    )
