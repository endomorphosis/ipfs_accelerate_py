...
class MobileRegressionDetector:
    ...
    def load_historical_data(self) -> bool:
        ...
        try:
            ...
            self.historical_data = {"platforms": {}, "models": {}}
            ...
            for run in benchmark_runs:
                ...
                if platform not in self.historical_data["platforms"]:
                    self.historical_data["platforms"][platform] = {"devices": {}, "models": {}}

                # Initialize model data if needed
                if model_name not in self.historical_data["models"]:
                    self.historical_data["models"][model_name] = {"platforms": {}}
                ...
                for result in results:
                    ...
                    if model_name not in platform_models:
                        platform_models[model_name] = {"batch_sizes": {}}
                    ...
                    platform_models[model_name]["batch_sizes"][batch_size]["throughput"].append(
                        throughput
                    )
                    platform_models[model_name]["batch_sizes"][batch_size]["latency"].append(
                        latency
                    )
                    ...
                    if platform not in model_platforms:
                        model_platforms[platform] = {"batch_sizes": {}}
                    ...
                    model_platforms[platform]["batch_sizes"][batch_size]["throughput"].append(
                        throughput
                    )
            ...
            logger.info(
                f"Loaded historical data for {len(self.historical_data['platforms'])} platforms "
    ...
    def detect_regressions(self) -> List[Dict[str, Any]]:
                        ...
                        if model_name in platform_hist.get("models", {}):
                            ...
                            if batch_size in model_hist.get("batch_sizes", {}):
                                ...
                                if hist_throughput > 0 and current_throughput > 0:
                                    throughput_change = (
                                        (hist_throughput - current_throughput) / hist_throughput
                                    ) * 100
                                ...
                                if hist_latency > 0 and current_latency > 0:
                                    ...
                                    latency_change = (
                                        (current_latency - hist_latency) / hist_latency
                                    ) * 100
        ...
        logger.info(
            f"Detected {len(self.regressions)} regressions above threshold {self.threshold}%"
        )
        return self.regressions
    ...
    def _get_severity_level(self, change_percent: float) -> str:
        ...
        for level, config in sorted(
            self.SEVERITY_LEVELS.items(), key=lambda x: x[1]["threshold"], reverse=True
        ):
    ...
    def generate_report(self, output_path: str, format: str = "markdown") -> str:
        ...
        if format == "json":
            ...
            for i, reg in enumerate(self.regressions):
                ...
                report_content += (
                    f"**Historical Average:** {historical:.2f} (from {samples} samples)\n"
                )
