...
def check_server_status(
    host: str, port: int, max_retries: int = 30, retry_interval: float = 1.0
) -> bool:
    """
...
def start_api_server(
    ...
    try:
        ...
        if export_update_script.exists():
            ...
            if result.returncode != 0:
                logger.error(
                    f"Error updating Unified API Server with Export integration: {result.stderr}"
                )
                logger.info("Proceeding anyway, in case server is already updated")
        else:
        ...
        if sync_benchmark_data:
            ...
            bridge_script = (
                parent_dir / "test" / "integration" / "benchmark_predictive_performance_bridge.py"
            )
...
def serve_dashboard(
    host: str = "localhost", port: int = 8000, dashboard_path: str = None
) -> subprocess.Popen:
    """
    ...
    try:
        ...
        if dashboard_path is None:
            dashboard_path = (
                parent_dir / "test" / "web_interface" / "hardware_optimization_dashboard.html"
            )
        else:
...
def main():
    ...
    parser.add_argument("--api-host", type=str, default="localhost", help="API server host")
    parser.add_argument("--api-port", type=int, default=8080, help="API server port")
    parser.add_argument(
        "--dashboard-host", type=str, default="localhost", help="Dashboard server host"
    )
    parser.add_argument("--dashboard-port", type=int, default=8000, help="Dashboard server port")
    parser.add_argument("--api-key", type=str, help="Optional API key")
    parser.add_argument(
        "--no-sync", action="store_true", help="Skip benchmark data synchronization"
    )
    parser.add_argument(
    ...
    dashboard_server = serve_dashboard(host=args.dashboard_host, port=args.dashboard_port)
