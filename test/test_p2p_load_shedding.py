...
class MockGitHubAPI:
    ...
    def make_api_call(self, operation: str, **kwargs) -> Dict:
        ...
        with self.lock:
            ...
            logger.info(
                f"{self.instance_name}: 🌐 GitHub API call #{len(self.api_calls)}: {operation}"
            )
...
def test_p2p_load_shedding_simulation():
    ...
    try:
        ...
        cache_a = GitHubAPICache(enable_p2p=False, cache_dir="/tmp/cache_a", default_ttl=300)
        cache_b = GitHubAPICache(enable_p2p=False, cache_dir="/tmp/cache_b", default_ttl=300)
        ...
        logger.info(
            f"✓ Instance A: Cached response (API calls: {api_instance_a.get_api_call_count()})"
        )
        ...
        if cached_a:
            logger.info(
                f"✓ Instance A: Got from local cache (API calls: {api_instance_a.get_api_call_count()})"
            )
        ...
        if cached_b:
            logger.info(
                f"✓ Instance B: Got from P2P cache (API calls: {api_instance_b.get_api_call_count()})"
            )
    ...
    try:
        ...
        def thread_worker(thread_id: int):
            ...
            if cached:
                ...
                results.append(
                    {"thread": thread_id, "source": "cache", "api_calls": api.get_api_call_count()}
                )
            else:
                ...
                results.append(
                    {"thread": thread_id, "source": "api", "api_calls": api.get_api_call_count()}
                )
        ...
        logger.info(f"")
        logger.info(
            f"Load shedding ratio: {cache_hits}/{num_threads} = {cache_hits / num_threads * 100:.1f}%"
        )
