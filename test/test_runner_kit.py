...
class TestRunnerKit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = RunnerConfig(owner="test-owner", poll_interval=60, max_runners=5)
        ...
        self.kit = RunnerKit(
            config=self.config, docker_kit=self.mock_docker_kit, github_kit=self.mock_github_kit
        )
    ...
    def test_workflow_queue_dataclass(self):
        ...
        queue = WorkflowQueue(
            repo="owner/repo", workflows=[{"id": 1, "status": "queued"}], running=1, pending=2
        )
    ...
    def test_autoscaler_status_dataclass(self):
        ...
        status = AutoscalerStatus(
            running=True, start_time=now, iterations=5, active_runners=3, queued_workflows=2
        )
    ...
    def test_get_workflow_queues(self, mock_run):
        ...
        self.mock_github_kit.list_workflow_runs.return_value = Mock(
            success=True,
            data={"workflows": [{"id": 1, "status": "queued"}, {"id": 2, "status": "in_progress"}]},
        )
    ...
    def test_generate_runner_token(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout='{"token": "GITHUB_TOKEN_12345"}', stderr=""
        )
    ...
    def test_launch_runner_container(self):
        ...
        self.mock_docker_kit.run_container.return_value = Mock(
            success=True, data={"container_id": "container123"}
        )

        container_id = self.kit.launch_runner_container(repo="owner/repo", token="fake-token")
    ...
    def test_check_and_scale(self):
        ...
        mock_queues = [WorkflowQueue(repo="owner/repo", pending=2)]
        ...
        with patch.object(self.kit, "get_workflow_queues", return_value=mock_queues):
            with patch.object(self.kit, "provision_runners_for_queues", return_value=1):
                ...
                self.assertTrue(
                    self.kit.get_workflow_queues.called or hasattr(self.kit, "check_and_scale")
                )
