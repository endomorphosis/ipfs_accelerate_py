/**
 * Browser-Specific GELU Operation Test
 * Tests the performance of browser-specific GELU operations
 */
...
function generateRandomTensor(shape: number[]): Tensor<number> {
  ...
  const data = Array(size).fill(0).map(() => Math.random() * 4 - 2); // Range [-2, 2] for GELU testing
  return new Tensor<number>(shape, data, { dataType: 'float32' });
}

/**
 * Compute GELU on CPU for reference
...
function cpuGelu(input: Tensor<number>): Tensor<number> {
  ...
  const SQRT_2_OVER_PI = 0.7978845608; // sqrt(2/π)
  const GELU_COEF1 = 0.044715;
  
  for (let i = 0; i < input.data.length; i++) {
    const x = input.data[i];
    const x3 = x * x * x;
    const inner = SQRT_2_OVER_PI * (x + GELU_COEF1 * x3);
    output[i] = x * 0.5 * (1 + Math.tanh(inner));
  }
  return new Tensor<number>(input.shape, Array.from(output), { dataType: 'float32' });
}

/**
 * Compute fast GELU approximation on CPU for reference
 * FastGELU(x) = x * sigmoid(1.702 * x)
 */
function cpuFastGelu(input: Tensor<number>): Tensor<number> {
  const output = new Float32Array(input.data.length);
...
/**
 * Test elementwise GELU performance across browsers
 */
async function testGeluPerformance(backend: WebGPUBackend, input: Tensor<number>) {
  console.log('\n=== Testing Elementwise GELU Performance ===');
  ...
  const cpuStandardResult = cpuGelu(input);
  const cpuFastResult = cpuFastGelu(input);
  ...
  await loadBrowserShader(ShaderType.ELEMENTWISE_GELU);
  
  // Test standard GELU implementations
  console.log('\nTesting standard GELU implementations:');
  for (const browser of browserTypes) {
    ...
    for (let i = 0; i < iterations; i++) {
      result = await backend.gelu(input, {
        useBrowserOptimizations: browser.type !== null,
        browserType: browser.type,
        useFastMath: false
      });
  ...
  // Test fast GELU implementations
  console.log('\nTesting fast GELU implementations:');
  for (const browser of browserTypes) {
...
/**
 * Test fusion of GELU with other operations for performance
 */
async function testGeluFusion(backend: WebGPUBackend, inputA: Tensor<number>, inputB: Tensor<number>) {
  console.log('\n=== Testing GELU Fusion Performance ===');
  
  const actualBrowserType = detectBrowserType();
  const iterations = 10;
  const warmupIterations = 5;
  
  // Test sequences: add followed by GELU (common in transformer models)
  console.log('Testing Add+GELU fusion...');
  ...
  // GELU on CPU
  const cpuGeluResult = cpuGelu(cpuAddTensor);
  
  // Unfused execution (separate operations)
  console.log('Running unfused operations (separate add and GELU)...');
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    unfusedResult = await backend.gelu(addResult, {
      useBrowserOptimizations: true
    });
  }
  ...
  // Verify correctness
  const isUnfusedCorrect = areTensorsEqual(cpuGeluResult, unfusedResult!, 1e-4);
  console.log(`Unfused correctness: ${isUnfusedCorrect ? 'PASSED' : 'FAILED'}`);
  
  // Fused execution (executeOperations)
  console.log('Running fused operations (add+GELU combined)...');
  ...
  // Verify correctness
  const isFusedCorrect = areTensorsEqual(cpuGeluResult, fusedResult!, 1e-4);
 ...
 */
async function main() {
  console.log('Browser-Specific GELU Operation Test');
  console.log('===================================');
  ...
  try {
    ...
    // Run tests
    await testGeluPerformance(backend, input);
    await testGeluFusion(backend, inputA, inputB);
