...
class hf_git:
    """HuggingFace GIT implementation.
    
    This class provides standardized interfaces for working with GIT models
    across different hardware backends (CPU, CUDA, ROCm, OpenVINO, Apple, Qualcomm).
    
    This is a generative image-to-text model.
    """
    
    def __init__(self, resources=None, metadata=None):
        """Initialize the GIT model.
    ...
    def __test__(self, endpoint_model, endpoint_handler, endpoint_label, tokenizer):
        ...
        try:
            ...
            print(test_batch)
            print("hf_git test passed")
            return True
        except Exception as e:
            print(e)
            print("hf_git test failed")
            return False
    
    def init_cpu(self, model_name, device, cpu_label):
        """Initialize GIT model for CPU inference."""
        self.init()
        ...
        try:
            ...
            # Load model
            model = self.transformers.GitModel.from_pretrained(
                model_name,
            ...
            handler = self.create_cpu_image_to_text_endpoint_handler(
    ...
    def init_cuda(self, model_name, device, cuda_label):
        """Initialize GIT model for CUDA inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_cuda_image_to_text_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_rocm(self, model_name, device, rocm_label):
        """Initialize GIT model for ROCm (AMD GPU) inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_rocm_image_to_text_endpoint_handler(
                endpoint_model=model_name,
    ...
    def init_openvino(self, model_name, device, openvino_label):
        """Initialize GIT model for OpenVINO inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_openvino_image_to_text_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_apple(self, model_name, device, apple_label):
        """Initialize GIT model for Apple Silicon (MPS) inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_apple_image_to_text_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_qualcomm(self, model_name, device, qualcomm_label):
        """Initialize GIT model for Qualcomm inference."""
        self.init()
    ...
    def create_cpu_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create handler function for CPU image_to_text endpoint."""
    ...
    def create_cuda_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create handler function for CUDA image_to_text endpoint."""
        def handler(text, *args, **kwargs):
            try:
                # Convert single string to list for batch processing
                if isinstance(text, str):
    ...
    def create_rocm_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create ROCm (AMD GPU) handler for image_to_text endpoint."""
        try:
            def handler(input_text, **kwargs):
                """ROCm handler for image_to_text inference."""
                ...
                if 'image_to_text' == 'text_embedding':
                ...
                elif 'image_to_text' == 'text_generation' or 'image_to_text' == 'causal_lm':
                    # For text generation
                ...
                elif 'image_to_text' == 'image_classification':
    ...
    def create_openvino_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create handler function for OpenVINO image_to_text endpoint."""
        def handler(text, *args, **kwargs):
            try:
                # Convert single string to list for batch processing
                if isinstance(text, str):
    ...
    def create_apple_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create handler function for Apple Silicon image_to_text endpoint."""
        def handler(text, *args, **kwargs):
            try:
                # Convert single string to list for batch processing
                if isinstance(text, str):
    ...
    def create_qualcomm_image_to_text_endpoint_handler(self, endpoint_model, device, hardware_label, endpoint, tokenizer):
        """Create handler function for Qualcomm image_to_text endpoint."""
        def handler(text, *args, **kwargs):
            try:
