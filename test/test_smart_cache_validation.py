...
def test_cache_persistence_with_validation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cache = GitHubAPICache(cache_dir=tmpdir, enable_persistence=True)
        ...
        if "content_hash" in saved_data and saved_data["content_hash"]:
            ...
            print(
                f"   ✅ Validation fields saved: {len(saved_data.get('validation_fields', {}))} repos"
            )
        else:
        ...
        cache2 = GitHubAPICache(cache_dir=tmpdir, enable_persistence=True)
