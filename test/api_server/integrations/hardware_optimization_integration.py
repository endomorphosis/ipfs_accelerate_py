...
class HardwareOptimizationIntegration:
    ...
    def register_routes(self, app):
        ...
        if not OPTIMIZATION_AVAILABLE:
            logger.warning(
                "Hardware Optimization components not available, skipping route registration"
            )
            return
        ...
        try:
            ...
            class HardwareOptimizationRequest(BaseModel):
                ...
                current_precision: Optional[str] = Field(
                    None, description="Optional current precision"
                )
            ...
            @router.post(
                ...
                summary="Get hardware optimization recommendations",
            )
            async def get_recommendations(
                request: HardwareOptimizationRequest, background_tasks: BackgroundTasks
            ):
                """
            ...
            @router.post(
                "/analyze-performance",
                response_model=TaskResponse,
                summary="Analyze performance data",
            )
            async def analyze_performance(
                request: PerformanceAnalysisRequest, background_tasks: BackgroundTasks
            ):
                """
            ...
            @router.get(
                "/strategies/{hardware_platform}", summary="Get available optimization strategies"
            )
...
def create_integration(
    ...
    return HardwareOptimizationIntegration(
        benchmark_db_path=benchmark_db_path, api_url=api_url, api_key=api_key, config=config
    )
