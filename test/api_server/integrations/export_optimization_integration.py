    ...
    def register_routes(self, app):
        ...
        try:
            ...
            class ExportRequest(BaseModel):
                ...
                recommendation_name: Optional[str] = Field(
                    None, description="Specific recommendation name"
                )
                output_format: str = Field(
                    "all", description="Output format (python, json, yaml, script, all)"
                )
                framework: Optional[str] = Field(None, description="Deep learning framework")

            class BatchExportRequest(BaseModel):
                recommendations_report: Dict[str, Any] = Field(
            ...
            # Routes
            @router.post(
                "/export", response_model=TaskResponse, summary="Export optimization to files"
            )
            async def export_optimization(
                ...
                output_dir = os.path.join(
                    self.output_dir, f"{request.model_name}_{request.hardware_platform}_{timestamp}"
                )
            ...
            @router.post(
                "/batch-export", response_model=TaskResponse, summary="Export batch optimizations"
            )
            async def batch_export(request: BatchExportRequest, background_tasks: BackgroundTasks):
                """
                ...
                # Create custom output directory
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                output_dir = os.path.join(self.output_dir, f"batch_export_{timestamp}")
            ...
            async def download_export(task_id: str):
                ...
                if "result" not in task:
                    raise HTTPException(
                        status_code=500, detail=f"No result available for task {task_id}"
                    )
