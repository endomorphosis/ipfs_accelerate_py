...
class SampleDataRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"num_models": 10}}
        ...
        try:
            ...
            if HARDWARE_MODEL_PREDICTOR_AVAILABLE:
                ...
                predictor = HardwareModelPredictor(
                    benchmark_dir=benchmark_dir, database_path=benchmark_db
                )

            # Create adapter
            repo = get_repository()
            hardware_adapter = HardwareModelPredictorDuckDBAdapter(
                predictor=predictor, repository=repo, user_id="api-server"
            )
...
def get_ml_adapter():
    ...
    if ml_adapter is None and DUCKDB_AVAILABLE:
        try:
            repo = get_repository()
            ml_adapter = ModelPerformancePredictorDuckDBAdapter(repository=repo)
...
async def update_task_status(
    task_id: str, status: str, progress: float = 0, message: str = "", result: Any = None
):
...
async def record_feedback_task(task_id: str, request: FeedbackRequest):
    ...
    try:
        ...
        await update_task_status(
            ...
            {"success": result, "recommendation_id": request.recommendation_id},
        )
...
async def store_ml_model_task(task_id: str, request: MLModelRequest):
    ...
    try:
        ...
        await update_task_status(
            task_id, "completed", 1.0, "Task completed successfully", {"model_id": model_id}
        )
...
@router.post(
    "/predict-performance", summary="Predict performance for a model on specified hardware"
)
...
@router.post(
    "/generate-matrix", summary="Generate a prediction matrix for various models and hardware"
)
...
async def list_feature_importance(
    ...
    importance = repo.get_feature_importance(
        model_id=model_id, feature_name=feature_name, method=method, limit=limit
    )

    return {"count": len(importance), "feature_importance": importance}
...
async def websocket_endpoint(websocket: WebSocket, task_id: str):
    ...
    try:
        ...
        if task_id in task_status:
            await websocket.send_json({"task_id": task_id, **task_status[task_id]})
