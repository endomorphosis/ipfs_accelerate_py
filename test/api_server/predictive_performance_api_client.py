...
class ApiClient:
    ...
    def record_feedback(
        self, recommendation_id: str, accepted: bool, feedback: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary with task ID and status
        """
        data = {"recommendation_id": recommendation_id, "accepted": accepted, "feedback": feedback}
    ...
    def wait_for_task(
        self, task_id: str, poll_interval: float = 0.5, timeout: float = 60
    ) -> Dict[str, Any]:
        """
...
class AsyncApiClient:
    ...
    async def predict_hardware(
        ...
        return await self._make_request(
            "post", "/api/predictive-performance/predict-hardware", data
        )
    ...
    async def predict_performance(
        ...
        return await self._make_request(
            "post", "/api/predictive-performance/predict-performance", data
        )
        ...
        async with websockets.connect(ws_url) as ws:
            while True:
                try:
                    ...
                    if "progress" in data:
                        logger.info(
                            f"Task progress: {data['progress']:.0%} - {data.get('message', '')}"
                        )
...
def main():
    ...
    predict_hw_parser = subparsers.add_parser(
        "predict-hardware", help="Predict optimal hardware for a model"
    )
    ...
    predict_hw_parser.add_argument(
    ...
    predict_hw_parser.add_argument(
        "--hardware", type=str, help="Comma-separated list of available hardware"
    )
    predict_hw_parser.add_argument(
        "--predict-performance", action="store_true", help="Also predict performance"
    )

    # predict-performance command
    predict_perf_parser = subparsers.add_parser(
        "predict-performance", help="Predict performance for a model"
    )
    ...
    predict_perf_parser.add_argument(
        "--hardware", type=str, required=True, help="Comma-separated list of hardware"
    )
    ...
    record_meas_parser = subparsers.add_parser(
        "record-measurement", help="Record a performance measurement"
    )
    ...
    record_meas_parser.add_argument(
        "--precision", choices=["fp32", "fp16", "int8"], default="fp32", help="Precision"
    )
    record_meas_parser.add_argument(
        "--mode", choices=["inference", "training"], default="inference", help="Mode"
    ...
    record_meas_parser.add_argument(
        "--source", type=str, default="cli", help="Source of the measurement"
    )

    # analyze-predictions command
    analyze_parser = subparsers.add_parser(
        "analyze-predictions", help="Analyze prediction accuracy"
    )
    ...
    analyze_parser.add_argument(
        "--metric", choices=["throughput", "latency", "memory_usage"], help="Metric"
    )
    analyze_parser.add_argument("--days", type=int, help="Number of days to analyze")

    # record-feedback command
    feedback_parser = subparsers.add_parser(
        "record-feedback", help="Record feedback for a recommendation"
    )
    feedback_parser.add_argument(
        "--recommendation-id", type=str, required=True, help="Recommendation ID"
    )
    feedback_parser.add_argument(
        "--accepted",
        choices=["yes", "no"],
        required=True,
        help="Whether recommendation was accepted",
    )
    feedback_parser.add_argument("--feedback", type=str, help="Feedback text")

    # list-recommendations command
    list_rec_parser = subparsers.add_parser(
        "list-recommendations", help="List hardware recommendations"
    )
    ...
    list_pred_parser = subparsers.add_parser(
        "list-predictions", help="List performance predictions"
    )
    ...
    list_meas_parser = subparsers.add_parser(
        "list-measurements", help="List performance measurements"
    )
    ...
    sample_parser = subparsers.add_parser(
        "generate-sample-data", help="Generate sample data for testing"
    )
    sample_parser.add_argument(
        "--num-models", type=int, default=5, help="Number of sample models to generate"
    )
    ...
    try:
        if args.command == "predict-hardware":
            ...
            if "result" in result:
                ...
                print(
                    f"  Compatible Hardware: {', '.join(result_data.get('compatible_hardware', []))}"
                )
                print(f"  Model Family: {result_data.get('model_family')}")
                print(
                    f"  Model Size: {result_data.get('model_size_category', 'unknown')} ({result_data.get('model_size', 'unknown')} parameters)"
                )
            ...
            if "result" in result:
                ...
                if "overall" in stats:
                    ...
                    print(
                        f"  Overall Mean Relative Error: {overall.get('overall_mean_relative_error', 0) * 100:.2f}%"
                    )
                ...
                for metric, metric_stats in stats.items():
                    ...
                    print(
                        f"  Mean Relative Error: {metric_stats.get('mean_relative_error', 0) * 100:.2f}%"
                    )
            ...
            response = client.record_feedback(
                recommendation_id=args.recommendation_id, accepted=accepted, feedback=args.feedback
            )
            ...
            # Call API
            data = {"num_models": args.num_models}

            response = client._make_request(
                "post", "/api/predictive-performance/generate-sample-data", data
            )
