...
def get_error_metrics(self):
    ...
    with self.metrics_lock:
        ...
        return {
            ...
            "most_common_error": max(self.metrics["error_types"].items(), key=lambda x: x[1])[0]
...
def get_throughput_metrics(self, time_window=60):
    ...
    with self.metrics_lock:
        ...
        recent_timestamps = [
            ts for ts in self.metrics["timestamps"] if current_time - ts <= time_window
        ]
...
def get_model_metrics(self, model_name):
    ...
    with self.metrics_lock:
        if model_name not in self.metrics["models"]:
            return {"requests": 0, "success_rate": 0, "avg_latency": 0}
