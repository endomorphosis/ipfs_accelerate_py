...
try:
    ...
    from data.duckdb.simulation_validation.statistical.basic_validator import (
        BasicStatisticalValidator,
    )
...
def save_results(validation_results, summary, output_path):
    ...
    logger.info(
        f"Results saved to {output_path}_validation_results.json and {output_path}_summary.json"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", default="./simulation_validation_results", help="Path prefix for output files"
    )
    parser.add_argument("--no-save", action="store_true", help="Skip saving results to files")
    parser.add_argument("--verbose", action="store_true", help="Print detailed results to console")
    ...
    try:
        ...
        if args.verbose:
            ...
            for model, model_summary in summary["models"].items():
                print(
                    f"  {model}: {model_summary['mean_accuracy_score']:.2f}% (n={model_summary['count']})"
                )
