...
class TestGitHubActionsP2PCache:
    ...
    def test_cache_checked_before_api_call(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            self.assert_true(mock_cache.get.called, "Cache.get() should be called before API call")
    ...
    def test_api_not_called_on_cache_hit(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(stdout="gh version 2.0.0", stderr="", returncode=0)
            ...
            self.assert_equal(
                api_calls, 0, "subprocess.run should not be called when cache has data"
            )
    ...
    def test_api_called_on_cache_miss(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(stdout=api_response, stderr="", returncode=0)
            ...
            self.assert_true(
                mock_run.call_count > 0, "subprocess.run should be called on cache miss"
            )
    ...
    def test_results_cached_after_api_call(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cache = configure_cache(cache_dir=tmpdir, enable_p2p=False, enable_persistence=False)
            ...
            self.assert_true(stats["cache_size"] > 0, "Cache size should be greater than 0")
    ...
    def test_cache_stats_tracking(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check stats
            stats = cache.get_stats()
            self.assert_true(stats["hits"] > initial_hits, "Hit count should increase")
            self.assert_true(stats["misses"] > initial_misses, "Miss count should increase")
    ...
    def test_workflow_integration_scenario(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(stdout=workflows_response, stderr="", returncode=0)
    ...
    def run_all_tests(self):
        ...
        # Run all tests
        self.run_test("Cache checked before API call", self.test_cache_checked_before_api_call)

        self.run_test("API not called on cache hit", self.test_api_not_called_on_cache_hit)

        self.run_test("API called on cache miss", self.test_api_called_on_cache_miss)

        self.run_test("Results cached after API call", self.test_results_cached_after_api_call)

        self.run_test("Cache key includes parameters", self.test_cache_key_includes_parameters)

        self.run_test("P2P cache environment variables", self.test_p2p_cache_environment_variables)

        self.run_test("Cache statistics tracking", self.test_cache_stats_tracking)

        self.run_test("Workflow integration scenario", self.test_workflow_integration_scenario)
