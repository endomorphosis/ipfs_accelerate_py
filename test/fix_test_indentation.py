...
def fix_if_conditionals(content):
    ...
    content = re.sub(
        r"(\s+)if not HAS_TOKENIZERS:(\s+)class", r"\1if not HAS_TOKENIZERS:\n\1    class", content
    )

    # Fix if not HAS_SENTENCEPIECE with class definition
    content = re.sub(
        r"(\s+)if not HAS_SENTENCEPIECE:(\s+)class",
...
def fix_tokenizer_mock(content):
    ...
    content = re.sub(
        r'logger\.warning\("tokenizers not available, using mock"\)\s+if not HAS_TOKENIZERS:',
    ...
    content = re.sub(
        r'HAS_TORCH = False\s+logger\.warning\("torch not available, using mock"\)',
        r'HAS_TORCH = False\n    logger.warning("torch not available, using mock")',
        content,
    )
    content = re.sub(
        r'HAS_TRANSFORMERS = False\s+logger\.warning\("transformers not available, using mock"\)',
        r'HAS_TRANSFORMERS = False\n    logger.warning("transformers not available, using mock")',
        content,
    )
    content = re.sub(
        r'HAS_TOKENIZERS = False\s+logger\.warning\("tokenizers not available, using mock"\)',
        r'HAS_TOKENIZERS = False\n    logger.warning("tokenizers not available, using mock")',
        content,
    )
    content = re.sub(
        r'HAS_SENTENCEPIECE = False\s+logger\.warning\("sentencepiece not available, using mock"\)',
        r'HAS_SENTENCEPIECE = False\n    logger.warning("sentencepiece not available, using mock")',
        content,
    )
    content = re.sub(
        r'HAS_PIL = False\s+logger\.warning\("PIL or requests not available, using mock"\)',
        ...
        if decode_index > 0 and init_index > 0:
            ...
            if not decode_line and init_line.startswith(" "):
                ...
                content = content.replace(
                    "def decode(self, ids, **kwargs):", f"{indent}def decode(self, ids, **kwargs):"
                )
...
def fix_method_parameter_indentation(content):
    ...
    content = re.sub(
        r" def test_pipeline\(self,\(self,\(self,", r"    def test_pipeline(self,", content
    )
    ...
    content = re.sub(
        r" def test_from_pretrained\(self,\(self,\(self,",
        r"    def test_from_pretrained(self,",
        content,
    )
    content = re.sub(
        r"def test_from_pretrained\(self,\(self,\(self,", r"def test_from_pretrained(self,", content
    )
...
def fix_indented_line_continuation(content):
    ...
    content = re.sub(
        r'(\s+)if HW_CAPABILITIES\["cuda"\]:', r'        if HW_CAPABILITIES["cuda"]:', content
    )
    content = re.sub(
        r"(\s+)if hasattr\(tokenizer, .pad_token.\)",
        r'        if hasattr(tokenizer, "pad_token")',
        content,
    )

    # Fix for the logger line often indented incorrectly
    content = re.sub(
        r'(\s+)logger.info\(f"Testing model:', r'        logger.info(f"Testing model:', content
    )
...
def fix_if_else_indentation(content):
    ...
    content = re.sub(
        r'(\s+)if "cuda" in error_str.*?:\n(\s+)results',
        r'\1if "cuda" in error_str or "cuda" in traceback_str:\n\1    results',
        content,
    )
    content = re.sub(
        r'(\s+)elif "memory" in error_str:\n(\s+)results',
        r'\1elif "memory" in error_str:\n\1    results',
        content,
    )
    content = re.sub(
        r'(\s+)elif "no module named" in error_str:\n(\s+)results',
    ...
    try:
        ...
        if is_valid:
            ...
            for i, line in enumerate(lines):
                ...
                if "def decode(self, ids, **kwargs):" in line and not line.strip().startswith(
                    "def"
                ):
                    fixed_lines.append("        def decode(self, ids, **kwargs):\n")
                elif (
                    "if not HAS_SENTENCEPIECE:" in line
                    and i + 1 < len(lines)
                    and "class" in lines[i + 1]
                ):
            ...
            if is_valid:
                logger.info(
                    f"✅ Successfully fixed indentation with aggressive approach: {file_path}"
                )
                return True
            else:
...
def main():
    ...
    parser.add_argument(
        "--check-only", action="store_true", help="Only check for syntax errors, don't fix"
    )
