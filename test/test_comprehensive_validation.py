...
def validate_phase1_backend_tools():
    ...
    try:
        ...
        if results and len(results) > 0:
            ...
            print(
                f"   📦 Sample: {results[0].model_id if hasattr(results[0], 'model_id') else results[0]}"
            )
        else:
...
def validate_phase3_dashboard_apis():
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"   ⚠️ Server not running - start with: python3 -m ipfs_accelerate_py.mcp_dashboard"
            )
            return False
        ...
        if response.status_code == 200:
            ...
            if len(results) > 0:
                ...
                download_response = requests.post(
                    f"{base_url}/api/mcp/models/download", json={"model_id": model_id}, timeout=30
                )
            ...
            if results_div:
                ...
                if "model-result" in content:
                    ...
                    if download_btn:
                        ...
                        time.sleep(3)
                        page.screenshot(
                            path=str(screenshots_dir / "05_download_clicked.png"), full_page=True
                        )
