...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"comprehensive_hf_test_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
...
MODEL_ARCHITECTURES = {
    ...
    "vision-text": ["clip", "blip", "blip_2", "git", "flava", "paligemma"],
    "speech": ["wav2vec2", "hubert", "whisper", "encodec", "clap", "speecht5", "unispeech", "sew"],
    "multimodal": ["llava", "video_llava", "idefics", "imagebind"],
}
...
def run_test(test_file: str, args: Any) -> Dict:
    ...
    try:
        ...
        return {"test": os.path.basename(test_file), "success": False, "error": str(e)}
...
def collect_results(results_dir: str) -> Dict:
    ...
    aggregated_results = {
        "models": {},
        "summary": {"total": 0, "successful": 0, "failed": 0, "architectures": {}},
        ...
        try:
            ...
            if "metadata" in result_data:
                ...
                arch_type = next(
                    (
                        arch
                        for arch, models in MODEL_ARCHITECTURES.items()
...
def run_tests_for_architecture(arch_type: str, args: Any) -> List[Dict]:
    ...
    for model_family in MODEL_ARCHITECTURES.get(arch_type, []):
        ...
        if test_file:
            ...
            results.append(
                {"test": f"test_hf_{model_family}", "success": False, "error": "no_test_file"}
            )
...
def main():
    ...
    arch_group.add_argument(
        "--encoder-only", action="store_true", help="Test encoder-only models (BERT, RoBERTa, etc.)"
    )
    arch_group.add_argument(
        "--decoder-only", action="store_true", help="Test decoder-only models (GPT-2, LLaMA, etc.)"
    )
    arch_group.add_argument(
        "--encoder-decoder",
        action="store_true",
        help="Test encoder-decoder models (T5, BART, etc.)",
    )
    arch_group.add_argument(
        "--vision", action="store_true", help="Test vision models (ViT, Swin, etc.)"
    )
    arch_group.add_argument(
        "--vision-text", action="store_true", help="Test vision-text models (CLIP, BLIP, etc.)"
    )
    arch_group.add_argument(
        "--speech", action="store_true", help="Test speech models (Wav2Vec2, Whisper, etc.)"
    )
    arch_group.add_argument(
        "--multimodal", action="store_true", help="Test multimodal models (LLaVA, etc.)"
    )

    # Model selection
    model_group = parser.add_argument_group("Model Selection")
    model_group.add_argument(
        "--models", type=str, help="Comma-separated list of model families to test"
    )
    ...
    output_group.add_argument(
        "--output-dir", type=str, default=str(RESULTS_DIR), help="Directory for output files"
    )
    output_group.add_argument("--summary-file", type=str, help="Path to write summary JSON file")
    output_group.add_argument(
        "--store-results", action="store_true", help="Store results in DuckDB database"
    )
    ...
    # Otherwise run tests for selected architectures
        ...
        logger.error(
            "No architectures or models selected. Use --all or specify architectures/models."
        )
        parser.print_help()
        return 1
    ...
    if args.store_results:
        try:
            ...
            if any(arch == "vision-text" for arch in selected_archs):
                ...
                from vision_text_duckdb_integration import (
                    connect_to_db,
                    create_tables,
                    import_results,
                )
                ...
                if conn:
                    ...
                    logger.info(
                        f"Imported {result['imported']} vision-text files, {result['failed']} failed"
                    )
