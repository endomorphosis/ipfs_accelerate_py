...
class TestWebResourcePoolAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_resource_pool.get_metrics.return_value = {
            "base_metrics": {"peak_memory_usage": 1500, "execution_time": 0.5}
        }
    ...
    def test_get_optimal_strategy(self):
        ...
        strategy = self.adapter.get_optimal_strategy(
            model_configs=large_configs, browser="chrome", optimization_goal="latency"
        )
        ...
        strategy = self.adapter.get_optimal_strategy(
            model_configs=medium_configs, browser="chrome", optimization_goal="throughput"
        )
        ...
        with patch.object(self.adapter, "_estimate_total_memory", return_value=5000):
            strategy = self.adapter.get_optimal_strategy(
                model_configs=self.model_configs, browser="chrome", optimization_goal="latency"
            )
            self.assertEqual(strategy, "batched")
    ...
    def test_memory_estimation(self):
        ...
        configs_with_sharing = self.model_configs + [
            {"model_name": "bert-base-uncased", "model_type": "text_embedding", "batch_size": 4}
        ]
        ...
        expected_no_sharing = original_memory + self.adapter._estimate_total_memory(
    ...
    def test_compare_strategies(self):
        ...
        def mock_execute_models(
            model_configs, execution_strategy, optimization_goal, browser, return_metrics
        ):
        ...
        comparison = self.adapter.compare_strategies(
            model_configs=self.model_configs, browser="chrome", optimization_goal="throughput"
        )
        ...
        comparison = self.adapter.compare_strategies(
            model_configs=self.model_configs, browser="chrome", optimization_goal="memory"
        )
    ...
    def test_browser_auto_selection(self):
        ...
        with patch.object(
            self.adapter, "get_optimal_browser", return_value="edge"
        ) as mock_get_browser:
            # Execute models without specifying browser
            result = self.adapter.execute_models(
                model_configs=self.model_configs, execution_strategy="parallel"
            )
    ...
    def test_execution_statistics(self):
        ...
        self.adapter.execute_models(
            model_configs=self.model_configs, execution_strategy="parallel", browser="chrome"
        )

        self.adapter.execute_models(
            model_configs=self.model_configs, execution_strategy="sequential", browser="firefox"
        )
