...
class MultiModelEmpiricalValidator:
    ...
    def __init__(
        ...
        self.validation_metrics = {
            ...
            "error_trends": {"throughput": [], "latency": [], "memory": []},
            "hardware_specific": {},
            "strategy_specific": {},
        }
        ...
        logger.info(
            f"MultiModelEmpiricalValidator initialized with "
    ...
    def _store_validation_record(self, validation_record: Dict[str, Any]):
        ...
        if len(self.validation_metrics["records"]) > self.validation_history_size:
            self.validation_metrics["records"] = self.validation_metrics["records"][
                -self.validation_history_size :
            ]

        # Update error rates
        self.validation_metrics["error_rates"]["throughput"].append(
    ...
    def _calculate_validation_metrics(self, validation_record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_throughput_error = (
            np.mean(self.validation_metrics["error_rates"]["throughput"][-10:])
            if self.validation_metrics["error_rates"]["throughput"]
            else 0
        )
        avg_latency_error = (
            np.mean(self.validation_metrics["error_rates"]["latency"][-10:])
            if self.validation_metrics["error_rates"]["latency"]
            else 0
        )
        avg_memory_error = (
            np.mean(self.validation_metrics["error_rates"]["memory"][-10:])
            if self.validation_metrics["error_rates"]["memory"]
            else 0
        )
        ...
        if (
            ...
            and len(self.validation_metrics["error_rates"]["throughput"]) >= 20
        ):
        ...
        if hardware_platform in self.validation_metrics["hardware_specific"]:
            ...
            hw_metrics = {
                "avg_throughput_error": np.mean(hw_data["throughput_errors"][-10:])
                if hw_data["throughput_errors"]
                else 0,
                "avg_latency_error": np.mean(hw_data["latency_errors"][-10:])
                if hw_data["latency_errors"]
                else 0,
                "avg_memory_error": np.mean(hw_data["memory_errors"][-10:])
                if hw_data["memory_errors"]
                else 0,
                "count": hw_data["count"],
            }
        ...
        if execution_strategy in self.validation_metrics["strategy_specific"]:
            ...
            strategy_metrics = {
                "avg_throughput_error": np.mean(strategy_data["throughput_errors"][-10:])
                if strategy_data["throughput_errors"]
                else 0,
                "avg_latency_error": np.mean(strategy_data["latency_errors"][-10:])
                if strategy_data["latency_errors"]
                else 0,
                "avg_memory_error": np.mean(strategy_data["memory_errors"][-10:])
                if strategy_data["memory_errors"]
                else 0,
    ...
    def _check_if_refinement_needed(self) -> bool:
        ...
        recent_throughput_errors = self.validation_metrics["error_rates"]["throughput"][
            -self.refinement_interval :
        ]
        recent_latency_errors = self.validation_metrics["error_rates"]["latency"][
            -self.refinement_interval :
        ]
        recent_memory_errors = self.validation_metrics["error_rates"]["memory"][
            -self.refinement_interval :
        ]
        ...
        if (
            avg_throughput_error > self.error_threshold
            or avg_latency_error > self.error_threshold
            or avg_memory_error > self.error_threshold
        ):
            return True
        ...
        if (
            self.enable_trend_analysis
            and len(self.validation_metrics["error_trends"]["throughput"]) > 0
        ):
            for metric_name in ["throughput", "latency", "memory"]:
    ...
    def record_model_refinement(
        ...
        throughput_improvement = (
            ...
            / pre_refinement_errors.get("throughput", 1)
            * 100
        )
        latency_improvement = (
            (pre_refinement_errors.get("latency", 0) - post_refinement_errors.get("latency", 0))
            / pre_refinement_errors.get("latency", 1)
            * 100
        )
        memory_improvement = (
            (pre_refinement_errors.get("memory", 0) - post_refinement_errors.get("memory", 0))
            / pre_refinement_errors.get("memory", 1)
            * 100
        )

        # Calculate overall improvement
        overall_improvement = (
            throughput_improvement + latency_improvement + memory_improvement
        ) / 3
    ...
    def get_validation_metrics(self, include_history: bool = False) -> Dict[str, Any]:
        ...
        for metric_name in ["throughput", "latency", "memory"]:
            ...
            if len(values) >= 20:
                ...
                error_rates[f"{metric_name}_error_trend_direction"] = (
                    "improving" if trend < 0 else "worsening"
                )
        ...
        for platform, data in self.validation_metrics["hardware_specific"].items():
            hardware_metrics[platform] = {
                "count": data["count"],
                "avg_throughput_error": np.mean(data["throughput_errors"][-10:])
                if data["throughput_errors"]
                else 0,
                "avg_latency_error": np.mean(data["latency_errors"][-10:])
                if data["latency_errors"]
                else 0,
                "avg_memory_error": np.mean(data["memory_errors"][-10:])
                if data["memory_errors"]
                else 0,
            }
    ...
    def get_refinement_recommendations(self) -> Dict[str, Any]:
        ...
        if refinement_needed:
            ...
            if avg_throughput_error > 0.3 or avg_latency_error > 0.3 or avg_memory_error > 0.3:
                ...
                for metric_name in ["throughput", "latency", "memory"]:
                    ...
                    recent_avg = np.mean(self.validation_metrics["error_rates"][metric_name][-10:])
                    older_avg = np.mean(
                        self.validation_metrics["error_rates"][metric_name][-20:-10]
                    )
        ...
        for platform, metrics in self.validation_metrics["hardware_specific"].items():
            ...
            needs_refinement = (
                avg_throughput > self.error_threshold
                or avg_latency > self.error_threshold
                or avg_memory > self.error_threshold
            )
    ...
    def generate_validation_dataset(self) -> Dict[str, Any]:
        ...
        try:
            ...
            traceback.print_exc()
            return {"success": False, "reason": f"Error generating dataset: {str(e)}"}
    ...
    def visualize_validation_metrics(self, metric_type: str = "error_rates") -> Dict[str, Any]:
        ...
        if not self.enable_visualization:
            return {"success": False, "reason": "Visualization is disabled"}
        ...
        try:
            ...
            if metric_type == "error_rates":
                ...
                ax.axhline(
                    ...
                    color="r",
                    linestyle="--",
                    label=f"Error Threshold ({self.error_threshold:.2f})",
                )
                ...
                for metric_name in ["throughput", "latency", "memory"]:
                    ...
                    ax.plot(
                        range(len(trends)),
                        avg_10,
                        label=f"{metric_name.capitalize()} (10-point avg)",
                    )
                    ax.plot(
                        range(len(trends)),
                        avg_20,
                        linestyle="--",
                        label=f"{metric_name.capitalize()} (20-point avg)",
                    )
                ...
                for i, metric in enumerate(metrics):
                    error_values = [
                        np.mean(self.validation_metrics["hardware_specific"][p][f"{metric}_errors"])
                        for p in platforms
                    ]
                ...
                for i, metric in enumerate(metrics):
                    error_values = [
                        np.mean(self.validation_metrics["strategy_specific"][s][f"{metric}_errors"])
                        for s in strategies
                    ]
                    ax.bar(x + i * width, error_values, width, label=f"{metric.capitalize()} Error")
                ...
                return {"success": False, "reason": f"Unknown metric type: {metric_type}"}
            ...
            # Return figure data
            return {"success": True, "figure": fig, "image_data": buf, "metric_type": metric_type}

        except ImportError as e:
            return {"success": False, "reason": f"Visualization requires matplotlib: {str(e)}"}
        except Exception as e:
            logger.error(f"Error visualizing validation metrics: {e}")
            traceback.print_exc()
            return {"success": False, "reason": f"Error generating visualization: {str(e)}"}
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Average throughput error: {metrics['error_rates'].get('avg_throughput_error', 0):.2%}"
    )
