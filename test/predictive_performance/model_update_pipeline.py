...
class ModelUpdatePipeline:
    ...
    def _load_data(self) -> bool:
        ...
        try:
            ...
            self.feature_columns = [
                col for col in self.data.columns if col not in self.target_columns
            ]
    ...
    def update_models(
        ...
        if len(new_data) < self.min_samples_for_update:
            logger.warning(
                f"Not enough samples for update. Minimum required: {self.min_samples_for_update}"
            )
        ...
        for metric in metrics:
            ...
            try:
                ...
                baseline_mape = np.mean(
                    np.abs((y_val - baseline_predictions) / np.maximum(y_val, 1e-10))
                )

            logger.info(
                f"Baseline metrics - RMSE: {baseline_rmse:.4f}, R²: {baseline_r2:.4f}, MAPE: {baseline_mape:.4f}"
            )
            ...
            if update_strategy == "incremental":
                ...
                updated_model, update_info = self._window_update(model, X, y, X_val, y_val)
            ...
            if update_info["improvement_percent"] > self.update_threshold:
                ...
                logger.info(
                    f"Update accepted for {metric}. Improvement: {update_info['improvement_percent']:.2f}%"
                )
                update_metrics[metric] = update_info
            else:
                # Reject the update
                logger.info(
                    f"Update rejected for {metric}. Improvement below threshold: {update_info['improvement_percent']:.2f}%"
                )
        ...
        if overall_improvement > 0:
            self._save_models()
            logger.info(
                f"Models updated and saved. Overall improvement: {overall_improvement:.2f}%"
            )
        else:
            logger.info("No overall improvement. Models not saved.")

        return {"success": True, "update_record": update_record, "metric_details": update_metrics}
    ...
    def _incremental_update(
        ...
        if isinstance(updated_model, GradientBoostingRegressor):
            ...
            n_new_estimators = min(
                30, original_n_estimators // 5
            )  # Add 20% more or 30, whichever is less
            ...
            new_model = RandomForestRegressor(
                n_estimators=min(30, len(updated_model.estimators_) // 5), random_state=42
            )
        ...
        update_info = {
            ...
            "improvement_percent": float(
                rmse_improvement
            ),  # Using RMSE improvement as primary metric
        ...
        logger.debug(
            f"Incremental update results - RMSE improvement: {rmse_improvement:.2f}%, R² improvement: {r2_improvement:.2f}%"
        )

        return updated_model, update_info

    def _window_update(
        self, model, X: np.ndarray, y: np.ndarray, X_val: np.ndarray, y_val: np.ndarray
    ) -> Tuple[Any, Dict[str, Any]]:
        """
        ...
        logger.debug(
            f"Window update results - RMSE improvement: {rmse_improvement:.2f}%, R² improvement: {r2_improvement:.2f}%"
        )
    ...
    def _weighted_update(
        ...
        logger.debug(
            f"Weighted update results - RMSE improvement: {rmse_improvement:.2f}%, R² improvement: {r2_improvement:.2f}%, Weight: {best_weight:.2f}"
        )
    ...
    def evaluate_model_improvement(self, metric: str) -> Dict[str, Any]:
        ...
        r2_improvement = (
            (r2_current - r2_original) / (1 - r2_original) * 100 if r2_original < 1 else 0
        )
        ...
        logger.info(
            f"Model improvement for {metric} - RMSE: {rmse_improvement:.2f}%, R²: {r2_improvement:.2f}%, MAPE: {mape_improvement:.2f}%"
        )

        return evaluation

    def determine_update_need(
        self, new_data: pd.DataFrame, threshold: float = 0.1
    ) -> Dict[str, Any]:
        """
        ...
        overall_error_increase = (
            overall_error_increase / metrics_analyzed if metrics_analyzed > 0 else 0
        )
        ...
        logger.info(
            f"Update need analysis - Overall error increase: {overall_error_increase:.2f}, Needs update: {overall_needs_update}"
        )
    ...
    def integrate_with_active_learning(
        ...
        for round_idx in range(sequential_rounds):
            ...
            logger.info(
                f"Round {round_idx + 1} complete - Improvement: {round_result['improvement']:.2f}%"
            )
        ...
        integration_result = {
            ...
            "next_batch": batch.to_dict(orient="records")
...
def main():
    ...
    parser.add_argument(
        "--update-strategy",
        choices=["incremental", "window", "weighted"],
        default="incremental",
    ...
    parser.add_argument(
        "--determine-need", action="store_true", help="Determine if update is needed"
    )
    ...
    if args.evaluate:
        ...
        # Determine if update is needed
        result = {"action": "determine_need", "analysis": pipeline.determine_update_need(new_data)}
        ...
        if result["action"] == "evaluate":
            ...
            for metric, res in result["metric_results"].items():
                if isinstance(res, dict) and "improvement" in res:
                    imp = res["improvement"]
                    print(
                        f"  {metric}: RMSE: {imp.get('rmse_percent', 0):.2f}%, R²: {imp.get('r2_percent', 0):.2f}%"
                    )
