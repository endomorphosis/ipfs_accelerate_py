...
class TestBatchGenerator:
    ...
    def generate_test_configs(self):
        ...
        self.configs_df = self.configs_df.sort_values(
            by="expected_information_gain", ascending=False
        )

    def suggest_test_batch(
        self,
        configurations,
        batch_size=10,
        ...
        if hardware_availability is not None:
            ...
            configs_df = self._apply_hardware_availability(
                configs_df, hardware_availability, score_column
            )
        ...
        if not ensure_diversity:
            ...
            if hardware_constraints is not None:
                batch = self._apply_hardware_constraints(
                    sorted_configs, hardware_constraints, batch_size
                )
            else:
        ...
        return self._diversity_sampling(
            configs_df, score_column, batch_size, diversity_weight, hardware_constraints
        )
    ...
    def _diversity_sampling(
        self, configs_df, score_column, batch_size, diversity_weight, hardware_constraints=None
    ):
        """
        ...
        numeric_columns = [
            col for col in configs_df.columns if configs_df[col].dtype in [np.int64, np.float64]
        ]
        categorical_columns = [
            col
        ...
        while len(selected_indices) < batch_size and remaining_indices:
            ...
            for idx in remaining_indices:
                ...
                combined_score = (
                    1 - diversity_weight
                ) * norm_score + diversity_weight * norm_distance
...
def test_basic_batch_generation():
    ...
    batch = batch_generator.suggest_test_batch(
        configurations=batch_generator.configs_df, batch_size=10, ensure_diversity=True
    )
    ...
    print(
        batch[
            ["model_type", "hardware", "batch_size", "expected_information_gain", "selection_order"]
        ].head(3)
    )
...
def test_hardware_constrained_batch():
    ...
    hardware_constraints = {"cpu": 2, "cuda": 3, "openvino": 1, "webgpu": 1}
...
def test_combined_constraints():
    ...
    # Define constraints
    hardware_constraints = {"cpu": 3, "cuda": 3, "openvino": 2, "webgpu": 1}

    hardware_availability = {"cpu": 1.0, "cuda": 0.7, "openvino": 0.5, "webgpu": 0.3}
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["basic", "hardware", "availability", "diversity", "combined", "all"],
        default="all",
        help="Test to run",
    )
    parser.add_argument("--batch-size", type=int, default=10, help="Batch size for test generation")
