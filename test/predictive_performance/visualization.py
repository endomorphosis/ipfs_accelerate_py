...
class AdvancedVisualization:
        ...
        if use_interactive and PLOTLY_AVAILABLE:
            ...
            if size_metric and size_metric in df.columns:
                ...
                if sizes.max() > sizes.min():
                    sizes = min_size + (max_size - min_size) * (sizes - sizes.min()) / (
                        sizes.max() - sizes.min()
                    )
                else:
            ...
            scatter = ax.scatter(
                df[x_metric], df[y_metric], df[z_metric], c=point_colors, s=sizes, alpha=0.7
            )
            ...
            if output_file is None:
                output_file = (
                    f"3d_visualization_{x_metric}_{y_metric}_{z_metric}.{self.output_format}"
                )
            output_path = Path(self.output_dir) / output_file
    ...
    def create_performance_dashboard(
        ...
        if use_interactive and PLOTLY_AVAILABLE:
            ...
            fig = make_subplots(
                ...
                cols=1,
                subplot_titles=[
                    f"{metric.replace('_', ' ').title()} by {' and '.join(groupby)}"
            ...
            for i, metric in enumerate(metrics):
                ...
                if len(groupby) >= 2:
                    ...
                    fig.add_trace(
                        go.Bar(x=grouped_df[groupby[0]], y=grouped_df[metric], name=metric),
                        row=i + 1,
                        col=1,
                    )
            ...
            # Update layout
            fig.update_layout(
                title_text=title, height=300 * num_metrics, width=1000, barmode="group"
            )
            ...
            fig, axes = plt.subplots(
                num_metrics,
                1,
                figsize=(self.figure_size[0], self.figure_size[1] * num_metrics / 2),
                dpi=self.dpi,
            ...
            for group_name, group_df in df.groupby(groupby):
                ...
                if include_trend and len(group_df) >= window_size:
                    ...
                    group_df[f"{metric}_trend"] = (
                        group_df[metric].rolling(window=window_size, min_periods=1).mean()
                    )
    ...
    def create_power_efficiency_visualization(
        ...
        if power_metric in df.columns and performance_metric in df.columns:
            ...
            raise ValueError(
                f"Performance metric '{performance_metric}' or power metric '{power_metric}' not found in data"
            )
        ...
        if use_interactive and PLOTLY_AVAILABLE:
            ...
            # Group by specified columns and calculate mean values
            grouped_df = (
                df.groupby(groupby)[["efficiency", performance_metric, power_metric]]
                .mean()
                .reset_index()
            )
            ...
            if len(groupby) > 0:
                ...
                # Create scatter plot
                for (first_val, second_val), group in (
                    grouped_df.groupby(groupby[:2])
                    if len(groupby) > 1
                    else grouped_df.groupby(groupby[0])
                ):
                    marker = marker_map[second_val] if len(groupby) > 1 else marker_map["all"]
                    label = f"{groupby[0]}={first_val}" + (
                        f", {groupby[1]}={second_val}" if len(groupby) > 1 else ""
                    )
            ...
            for eff in eff_values:
                perf_values = eff * power_range
                ax.plot(power_range, perf_values, "k--", alpha=0.5, linewidth=1)
    ...
    def create_dimension_reduction_visualization(
        ...
        reduced_df = pd.DataFrame(
            X_reduced, columns=[f"{reducer_name}{i + 1}" for i in range(n_components)]
        )

        # Add target and groupby columns
        reduced_df[target] = (
            df[target].values if target in df.columns else np.zeros(len(reduced_df))
        )
        ...
        if use_interactive and PLOTLY_AVAILABLE:
            ...
            if n_components == 2:
                ...
                fig = px.scatter(
                    ...
                    hover_data=[groupby, target]
                    if groupby in reduced_df.columns and target in reduced_df.columns
                    else None,
                    title=title,
                )
                ...
                raise ValueError(
                    f"Interactive visualization only supports 2 or 3 components, got {n_components}"
                )
                ...
                if groupby in reduced_df.columns:
                    ...
                    for group_name, group_df in reduced_df.groupby(groupby):
                        ax.scatter(
                            ...
                            label=group_name,
                            alpha=0.7,
                            s=group_df[target] / reduced_df[target].max() * 100 + 30
                ...
                if method.lower() == "pca":
                    ...
                    for i, feature in enumerate(feature_names):
                        ax.arrow(
                            0,
                            0,
                            pca_components[0, i] * scale_factor,
                            pca_components[1, i] * scale_factor,
                            head_width=0.2,
                            head_length=0.2,
                            fc="black",
                            ec="black",
                        ...
                        ax.text(
                            pca_components[0, i] * scale_factor * 1.1,
                            pca_components[1, i] * scale_factor * 1.1,
                            feature,
                            fontsize=9,
                        )
                        ...
                        if groupby in reduced_df.columns:
                            ...
                            for group_name, group_df in reduced_df.groupby(groupby):
                                ax.scatter(
                                    ...
                                    label=group_name
                                    if i == 0 and j == 1
                                    else None,  # Only show legend once
                                    alpha=0.7,
                                    s=30,
                                )
                        else:
    ...
    def export_visualization(
        ...
        json_match = re.search(
            r"Plotly\.newPlot\((.*?),\s*({.*}),\s*({.*})", html_content, re.DOTALL
        )
    ...
    def create_prediction_confidence_visualization(
        ...
        if use_interactive and PLOTLY_AVAILABLE:
            ...
            if has_bounds:
                grouped_df[f"{metric}_lower_bound"] = (
                    df.groupby(groupby)[f"{metric}_lower_bound"].mean().values
                )
                grouped_df[f"{metric}_upper_bound"] = (
                    df.groupby(groupby)[f"{metric}_upper_bound"].mean().values
                )
            ...
            # Update layout
            fig.update_layout(yaxis_title=metric, xaxis_title=groupby[0], width=1000, height=600)
            ...
            if has_confidence:
                ...
                legend_elements = [
                    Patch(
                        facecolor=colors[0]
                        if len(groupby) >= 2
                        else sns.color_palette(self.palette, n_colors=1)[0],
                        alpha=conf,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate advanced visualizations for performance data"
    )
    parser.add_argument("--data", required=True, help="Path to data file (JSON or CSV)")
    parser.add_argument(
        "--output-dir", default="./visualizations", help="Output directory for visualizations"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Generate interactive visualizations"
    )
    parser.add_argument(
        "--metrics", nargs="+", default=DEFAULT_METRICS, help="Metrics to visualize"
    )
    parser.add_argument(
        "--groupby", nargs="+", default=["model_name", "hardware"], help="Columns to group by"
    )
    parser.add_argument("--no-3d", action="store_true", help="Skip 3D visualizations")
    parser.add_argument(
        "--no-time-series", action="store_true", help="Skip time-series visualizations"
    )
    parser.add_argument(
        "--no-power-efficiency", action="store_true", help="Skip power efficiency visualizations"
    )
    parser.add_argument(
        "--no-dimension-reduction",
        action="store_true",
        help="Skip dimension reduction visualizations",
    )
    parser.add_argument(
        "--no-confidence", action="store_true", help="Skip confidence visualizations"
    )
    parser.add_argument(
        "--report", action="store_true", help="Generate HTML report with all visualizations"
    )
    parser.add_argument(
        "--report-title", default="Performance Visualization Report", help="Report title"
    )
    parser.add_argument(
        "--report-file", default="visualization_report.html", help="Report output file"
    )

    args = parser.parse_args()

    # Create visualization system
    vis = AdvancedVisualization(output_dir=args.output_dir, interactive=args.interactive)
