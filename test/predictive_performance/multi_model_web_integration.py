...
try:
    from ipfs_accelerate_py.predictive_performance.multi_model_resource_pool_integration import (
...
class MultiModelWebIntegration:
    ...
    def __init__(
        ...
        if validator is not None:
            ...
            if enable_validation:
                logger.warning(
                    "MultiModelEmpiricalValidator not available, validation will be disabled"
                )
        ...
        logger.info(
            f"MultiModelWebIntegration created "
    ...
    def initialize(self) -> bool:
        ...
        logger.info(
            f"MultiModelWebIntegration initialization {'successful' if success else 'failed'}"
        )
        return success
    ...
    def execute_models(
        ...
        if browser and self.web_adapter:
            ...
            self._update_execution_stats(
                result, browser, execution_strategy or result.get("execution_strategy", "auto")
            )
    ...
    def compare_strategies(
        ...
        if browser and self.web_adapter:
            ...
            comparison = self.web_adapter.compare_strategies(
                model_configs=model_configs, browser=browser, optimization_goal=optimization_goal
            )
        ...
        # Otherwise use the resource pool integration
            ...
            logger.info(
                f"Comparing strategies for {len(model_configs)} models on {hardware_platform}"
            )
    ...
    def get_validation_metrics(self, include_history: bool = False) -> Dict[str, Any]:
        ...
        if self.resource_pool_integration:
            metrics = self.resource_pool_integration.get_validation_metrics(
                include_history=include_history
            )

            # Update execution stats with validation metrics
            if "validation_count" in metrics:
                self.execution_stats["validation_metrics"]["validation_count"] = metrics[
                    "validation_count"
                ]

            if "error_rates" in metrics:
                self.execution_stats["validation_metrics"]["average_errors"] = metrics[
                    "error_rates"
                ]
    ...
    def _update_execution_stats(self, result: Dict[str, Any], backend: str, strategy: str):
        ...
        self.execution_stats["browser_executions"][backend] = (
            self.execution_stats["browser_executions"].get(backend, 0) + 1
        )

        # Update strategy executions
        self.execution_stats["strategy_executions"][strategy] = (
            self.execution_stats["strategy_executions"].get(strategy, 0) + 1
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Execution complete with strategy: {result.get('execution_strategy', 'unknown')}"
        )
