...
spec = importlib.util.spec_from_file_location(
    "active_learning", str(Path(__file__).parent / "active_learning.py")
)
...
def test_basic_batch_generation():
    ...
    batch = active_learning.suggest_test_batch(
        configurations=configs, batch_size=10, ensure_diversity=True
    )
...
def test_diversity_weighting():
    ...
    for weight in [0.1, 0.5, 0.9]:
        batch = active_learning.suggest_test_batch(
            configurations=configs, batch_size=10, ensure_diversity=True, diversity_weight=weight
        )
