...
class ModelOperation:
    ...
    def calculate_memory_usage(self) -> int:
        ...
        if self.input_shapes and self.output_shapes:
            ...
            input_size = sum(
                np.prod(shape) * 4 for shape in self.input_shapes
    ...
    def is_compatible_for_fusion(self, other: "ModelOperation") -> bool:
        ...
        if self.model_name == other.model_name:
            return any(output in other.inputs for output in self.outputs) or any(
                output in self.inputs for output in other.outputs
            )

        # Different models but shareable
        if self.is_shared and other.is_shared:
            # Check shape compatibility
            if len(self.input_shapes) != len(other.input_shapes) or len(self.output_shapes) != len(
                other.output_shapes
            ):
                return False
...
class Tensor:
    ...
    def overlaps_with(self, other: "Tensor") -> bool:
        ...
        return max(self.first_use_time, other.first_use_time) <= min(
            self.last_use_time, other.last_use_time
        )
...
class MemoryOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Memory Optimizer initialized "
    ...
    def _generate_model_profile(
        self, model_name: str, model_type: str, model_size: str
        ...
        profile = {
            ...
            "operations": self._generate_synthetic_operations(
                model_name, template["ops"], model_type
            ),
            "tensors": self._generate_synthetic_tensors(
                model_name, template["tensors"], template["memory_mb"], model_type
            ),
    ...
    def _generate_synthetic_operations(
        self, model_name: str, count: int, model_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_shapes_for_op(
        self, op_type: str, model_type: str
        ...
        if model_type == "text_embedding" or model_type == "text_generation":
            ...
            if op_type == "matmul":
                return [[batch_size, seq_len, hidden_dim], [batch_size, hidden_dim, hidden_dim]], [
            ...
            if op_type == "conv2d":
                return [[batch_size, channels, img_size, img_size]], [
                    [batch_size, 64, img_size // 2, img_size // 2]
                ]
            elif op_type == "max_pool":
                return [[batch_size, 64, img_size // 2, img_size // 2]], [
                    [batch_size, 64, img_size // 4, img_size // 4]
                ]
            else:
    ...
    def _generate_synthetic_tensors(
        self, model_name: str, count: int, memory_mb: int, model_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_tensor_shape(
        self, model_type: str, index: int, is_input: bool, is_output: bool, is_constant: bool
    ) -> List[int]:
    ...
    def build_computation_graph(self, model_names: List[str]) -> bool:
        ...
        if self.operations:
            ...
            logger.info(
                f"Built computation graph with {len(self.operations)} operations, "
    ...
    def _calculate_peak_memory(self) -> int:
        ...
        for op_name in self.execution_order:
            ...
            for tensor_name in list(active_tensors):
                ...
                if (
                    tensor.consumed_by
                    and op_name == tensor.consumed_by[-1]
                    and not tensor.is_output
                ):
    ...
    def analyze_tensor_lifetimes(self):
        ...
        for tensor_name, tensor in self.tensors.items():
            ...
            logger.debug(
                f"Tensor {tensor_name} lifetime: {tensor.first_use_time:.2f} - {tensor.last_use_time:.2f}"
            )
    ...
    def identify_tensor_reuse_opportunities(self) -> List[List[str]]:
        ...
        while unassigned:
            ...
            largest_tensor = max(
                [t for t in sorted_tensors if t.name in unassigned], key=lambda t: t.size_bytes
            )
        ...
        logger.info(
            f"Identified {len(reuse_groups)} tensor reuse groups with {sum(len(g) for g in reuse_groups)} tensors"
        )
    ...
    def optimize_memory_allocation(self) -> Dict[str, Any]:
        ...
        optimization_results = {
            ...
            "memory_savings_percent": (self.memory_savings / memory_before * 100)
            if memory_before > 0
            else 0,
        ...
        logger.info(
            f"Memory optimization complete: {optimization_results['memory_savings_percent']:.2f}% savings "
            ...
            if op1_name in self.operations and op2_name in self.operations:
                ...
                for tensor_name in shared_outputs:
                    if tensor_name in self.tensors:
                        ...
                        logger.debug(
                            f"Tensor {tensor_name} eliminated by fusion of {op1_name} and {op2_name}"
                        )
    ...
    def _apply_tensor_reuse(self):
        ...
        for group in self.tensor_reuse_groups:
            ...
            largest_tensor = max([self.tensors[name] for name in group], key=lambda t: t.size_bytes)
            ...
            if max_slot < len(tensors) - 1:
                ...
                for tensor_name in tensors:
                    ...
                    if tensor_name in tensor_to_slot:
                        ...
                        tensor.memory_address = -(
                            size * tensor_to_slot[tensor_name] + 2
                        )  # Negative for pools

                logger.debug(
                    f"Created memory pool {pool_name} with {len(tensors)} tensors and {max_slot + 1} slots"
                )
    ...
    def generate_memory_plan(self) -> Dict[str, Any]:
        ...
        for op1_name, op2_name in self.operation_fusions:
            fusion_info = {
                ...
                "models": [
                    self.operations[op1_name].model_name,
                    self.operations[op2_name].model_name,
                ],
                "type": self.operations[op1_name].op_type,
                "eliminated_tensors": list(
                    set(self.operations[op1_name].outputs) & set(self.operations[op2_name].inputs)
                ),
            }
    ...
    def optimize_models(self, model_names: List[str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Completed memory optimization for {len(model_names)} models: "
...
if __name__ == "__main__":
    ...
    model_names = ["bert-base-uncased", "vit-base-patch16-224", "t5-small"]
    ...
    logger.info(
        f"Memory savings: {results['memory_savings_mb']:.2f} MB ({results['memory_savings_percent']:.2f}%)"
    )
    logger.info(f"Operation fusions: {results['optimization_summary']['operation_fusions']}")
    logger.info(
        f"Tensor reuse groups: {results['optimization_summary']['tensor_reuse_groups']} "
