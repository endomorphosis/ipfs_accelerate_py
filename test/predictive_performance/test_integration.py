...
def test_simple_recommendations(active_learner):
    ...
    try:
        ...
        if len(recommendations) > 0:
            ...
            logger.info(
                f"First recommendation: {first_rec['model_name']} on {first_rec['hardware']} with batch size {first_rec['batch_size']}"
            )
            logger.info(
                f"Expected information gain: {first_rec.get('expected_information_gain', 'N/A')}"
            )
...
def test_integration(active_learner, hw_recommender):
    ...
    try:
        ...
        logger.info(
            f"Total candidates considered: {integrated_results.get('total_candidates', 'N/A')}"
        )
        logger.info(f"Enhanced candidates: {integrated_results.get('enhanced_candidates', 'N/A')}")
        logger.info(
            f"Final recommendations: {integrated_results.get('final_recommendations', 'N/A')}"
        )
