...
def main():
    ...
    parser.add_argument(
        "--dashboard-port", type=int, default=8085, help="DRM dashboard port (default: 8085)"
    )
    parser.add_argument(
        "--update-interval", type=int, default=5, help="Update interval in seconds (default: 5)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--export-only",
        action="store_true",
        help="Only export Grafana dashboard without starting services",
    )
    parser.add_argument("--save-guide", action="store_true", help="Save integration guide to file")

    # Integration options
    parser.add_argument(
        "--drm-url", help="URL of DRM coordinator (optional, for connecting to live DRM)"
    )
    parser.add_argument("--api-key", help="API key for DRM coordinator authentication")
    parser.add_argument(
        "--simulation",
        action="store_true",
        help="Use simulated DRM data (default if no DRM URL provided)",
    )
    parser.add_argument(
    ...
    try:
        # Import modules
        from data.duckdb.distributed_testing.dashboard.drm_real_time_dashboard import (
            DRMRealTimeDashboard,
        )
