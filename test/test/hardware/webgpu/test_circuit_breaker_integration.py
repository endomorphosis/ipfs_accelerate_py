...
try:
    ...
    logger.error(
        "Error importing selenium_browser_bridge. Make sure it exists at the expected path."
    )
...
try:
    ...
    logger.error(
        "Error importing browser_failure_injector. Make sure it exists at the expected path."
    )
...
class CircuitBreakerIntegrationTest:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized circuit breaker integration test with browser={browser_name}, platform={platform}"
        )
        ...
        logger.info(
            f"Created circuit breaker with threshold={self.circuit_breaker.failure_threshold}"
        )
    ...
    async def test_circuit_closed_state(self) -> Dict[str, Any]:
        ...
        test_results = {"test_name": "circuit_closed_state", "success": True, "failures": []}
        ...
        try:
            ...
            for intensity in self.all_intensities:
                ...
                # Inject the failure
                logger.info(
                    f"Injecting {failure_type.value} with {intensity} intensity in CLOSED state"
                )
                ...
                if not result.get("success", False):
                    ...
                    test_results["failures"].append(
                        f"Failure injection failed with {intensity} intensity in CLOSED state"
                    )
                ...
                if result.get("circuit_breaker_open", False):
                    test_results["success"] = False
                    test_results["failures"].append(
                        f"Circuit breaker incorrectly blocked {intensity} intensity in CLOSED state"
                    )
            ...
            if not state == "closed":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to remain 'closed' after mild/moderate failures, got '{state}'"
                )
    ...
    async def test_circuit_open_transition(self) -> Dict[str, Any]:
        ...
        # Results for this test
        test_results = {"test_name": "circuit_open_transition", "success": True, "failures": []}
        ...
        try:
            ...
            for i in range(failures_needed):
                ...
                if not result.get("circuit_breaker_updated", False):
                    test_results["success"] = False
                    test_results["failures"].append(
                        f"Circuit breaker was not updated after severe failure {i + 1}"
                    )
                ...
                if i < failures_needed - 1:
                    ...
                    if not bridge or not injector:
                        test_results["success"] = False
                        test_results["failures"].append(
                            f"Failed to create new browser after failure {i + 1}"
                        )
                        return test_results
            ...
            if not state == "open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'open' after {failures_needed} severe failures, got '{state}'"
                )
                return test_results
    ...
    async def test_circuit_half_open_transition(self) -> Dict[str, Any]:
        ...
        if current_state != "open":
            test_results["success"] = False
            test_results["failures"].append(
                f"Expected circuit to be 'open' at start of test, got '{current_state}'"
            )
            return test_results
        ...
        try:
            ...
            logger.info(
                f"Waiting {self.circuit_breaker.half_open_after} seconds for half-open transition"
            )
            ...
            if not state == "half-open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'half-open' after timeout, got '{state}'"
                )
                return test_results
            ...
            if not bridge or not injector:
                test_results["success"] = False
                test_results["failures"].append(
                    "Failed to create browser or injector for half-open test"
                )
                return test_results
            ...
            for intensity in self.all_intensities:
                ...
                result = await injector.inject_random_failure(
                    exclude_severe=(intensity == "severe")
                )
                ...
                actual_allowed = not result.get("circuit_breaker_open", False) and result.get(
                    "success", False
                )
                ...
                if expected_allowed != actual_allowed:
                    test_results["success"] = False
                    test_results["failures"].append(
                        f"{intensity} intensity was {'allowed' if actual_allowed else 'blocked'} in half-open state, expected {'allowed' if expected_allowed else 'blocked'}"
                    )
                ...
                if intensity == "severe" and actual_allowed:
                    ...
                    if not state == "open":
                        test_results["success"] = False
                        test_results["failures"].append(
                            f"Circuit did not transition back to 'open' after severe failure in half-open state, got '{state}'"
                        )
    ...
    async def test_circuit_reclosing(self) -> Dict[str, Any]:
        ...
        # Results for this test
        test_results = {"test_name": "circuit_reclosing", "success": True, "failures": []}
        ...
        if not state == "closed":
            test_results["success"] = False
            test_results["failures"].append(
                f"Failed to reset circuit breaker to closed state, got '{state}'"
            )
            return test_results
        ...
        try:
            ...
            if not state == "open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'open' after recording failures, got '{state}'"
                )
                return test_results
            ...
            if not state == "closed":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'closed' after success in half-open state, got '{state}'"
                )
                return test_results
        ...
        try:
            ...
            if not state == "open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'open' for browser test, got '{state}'"
                )
                return test_results

            # Wait for half-open transition
            logger.info(
                f"Waiting {self.circuit_breaker.half_open_after} seconds for half-open transition for browser test"
            )
            await anyio.sleep(self.circuit_breaker.half_open_after + 0.5)
            ...
            if not bridge or not injector:
                test_results["success"] = False
                test_results["failures"].append(
                    "Failed to create browser or injector for reclosing test"
                )
                return test_results
            ...
            if not result.get("success", False) or result.get("circuit_breaker_open", False):
                test_results["success"] = False
                test_results["failures"].append(
                    "Mild failure was unexpectedly blocked in half-open state"
                )
            ...
            if not state == "half-open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to remain 'half-open' after mild failure, got '{state}'"
                )
            ...
            if not state == "closed":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Expected state to be 'closed' after recording success, got '{state}'"
                )
            ...
            for intensity in self.all_intensities:
                ...
                if not result.get("success", False) or result.get("circuit_breaker_open", False):
                    test_results["success"] = False
                    test_results["failures"].append(
                        f"{intensity} intensity was unexpectedly blocked in closed state"
                    )

        except Exception as e:
            test_results["success"] = False
            test_results["failures"].append(
                f"Exception during browser test for reclosing: {str(e)}"
            )
    ...
    async def test_adaptive_failure_injection(self) -> Dict[str, Any]:
        ...
        if not bridge or not injector:
            test_results["success"] = False
            test_results["failures"].append(
                "Failed to create browser or injector for closed state test"
            )
            return test_results
        ...
        try:
            ...
            if not bridge or not injector:
                test_results["success"] = False
                test_results["failures"].append(
                    "Failed to create browser or injector for open state test"
                )
                return test_results
            ...
            if not bridge or not injector:
                test_results["success"] = False
                test_results["failures"].append(
                    "Failed to create browser or injector for half-open state test"
                )
                return test_results
            ...
            if intensity == "severe":
                test_results["success"] = False
                test_results["failures"].append(
                    "inject_random_failure selected severe intensity in HALF-OPEN state"
                )

        except Exception as e:
            test_results["success"] = False
            test_results["failures"].append(
                f"Exception during adaptive failure injection test: {str(e)}"
            )
    ...
    async def test_failure_metrics(self) -> Dict[str, Any]:
        ...
        # Results for this test
        test_results = {"test_name": "failure_metrics", "success": True, "failures": []}
        ...
        try:
            ...
            if "circuit_breaker" not in stats:
                test_results["success"] = False
                test_results["failures"].append(
                    "Circuit breaker metrics not found in failure stats"
                )
                return test_results
            ...
            if cb_metrics["state"] != "closed":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected initial state in metrics: {cb_metrics['state']}"
                )

            if cb_metrics["failure_count"] != 0:
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected initial failure count: {cb_metrics['failure_count']}"
                )
            ...
            for i in range(self.circuit_breaker.failure_threshold - 1):
                ...
                if not result.get("success", False):
                    test_results["success"] = False
                    test_results["failures"].append(
                        f"Failed to inject failure {i + 1} for metrics test"
                    )
            ...
            if "circuit_breaker" not in stats:
                test_results["success"] = False
                test_results["failures"].append(
                    "Circuit breaker metrics not found in updated stats"
                )
                return test_results
            ...
            if cb_metrics["state"] != "closed":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected state in updated metrics: {cb_metrics['state']}"
                )
            ...
            if cb_metrics["failure_count"] != expected_count:
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected failure count: {cb_metrics['failure_count']}, expected {expected_count}"
                )
            ...
            if abs(cb_metrics["threshold_percent"] - expected_percent) > 0.1:
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected threshold percent: {cb_metrics['threshold_percent']}, expected {expected_percent}"
                )
            ...
            if cb_metrics["state"] != "open":
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected final state in metrics: {cb_metrics['state']}, expected 'open'"
                )
            ...
            if cb_metrics["failure_count"] != self.circuit_breaker.failure_threshold:
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected final failure count: {cb_metrics['failure_count']}, expected {self.circuit_breaker.failure_threshold}"
                )
            ...
            if abs(cb_metrics["threshold_percent"] - 100.0) > 0.1:
                test_results["success"] = False
                test_results["failures"].append(
                    f"Unexpected final threshold percent: {cb_metrics['threshold_percent']}, expected 100.0"
                )
    ...
    async def _create_browser_and_injector(self):
        ...
        try:
            ...
            injector = BrowserFailureInjector(
                bridge, circuit_breaker=self.circuit_breaker, use_circuit_breaker=True
            )
    ...
    def _save_results(self) -> None:
        ...
        try:
            ...
            with open(markdown_path, "w") as f:
                ...
                f.write(
                    f"- **Failure Threshold:** {self.results['circuit_breaker']['failure_threshold']}\n"
                )
                f.write(
                    f"- **Recovery Timeout:** {self.results['circuit_breaker']['recovery_timeout']} seconds\n"
                )
                f.write(
                    f"- **Half-Open After:** {self.results['circuit_breaker']['half_open_after']} seconds\n\n"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Circuit Breaker Integration with Browser Failure Injector"
    )
    parser.add_argument(
        "--browser",
        default="chrome",
    ...
    if not save_path:
        ...
        save_path = os.path.join(
            reports_dir, f"circuit_breaker_integration_test_{args.browser}_{timestamp}.json"
        )
