...
class TestConverterIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.python_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "ipfs_accelerate_py/api_backends"
        )
    ...
    def test_convert_sample_backend(self):
        ...
        converter = APIBackendConverter(
            python_file=self.sample_backend_path, output_dir=self.output_dir, dry_run=False
        )
        ...
        for file_path in expected_files:
            self.assertTrue(
                os.path.exists(file_path), f"Expected output file not found: {file_path}"
            )
    ...
    def test_convert_ollama_backend(self):
        ...
        converter = APIBackendConverter(
            python_file=self.ollama_backend_path, output_dir=self.output_dir, dry_run=False
        )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.output_dir, "ollama_clean", "ollama_clean.ts"))
        )
    ...
    def test_run_converter_cli(self):
        ...
        script_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "convert_api_backends.py"
        )
...
class TestValidTypeScriptOutput(unittest.TestCase):
    ...
    def test_typescript_syntax_validation(self):
        ...
        self.assertTrue(
            os.path.exists(types_file_path),
            f"Generated TypeScript types file not found: {types_file_path}",
        )
