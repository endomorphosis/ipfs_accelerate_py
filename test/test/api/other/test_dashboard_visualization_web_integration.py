...
class TestWebServerIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        def start_server():
            ...
            cls.httpd = HTTPServer(
                ("localhost", cls.server_port),
                lambda *args, **kwargs: TestHandler(
                    *args, dashboard_dir=cls.dashboard_dir, **kwargs
                ),
            )
    ...
    def setUp(self):
        ...
        self.integration = VisualizationDashboardIntegration(dashboard_dir=self.dashboard_dir)
    ...
    def test_dashboard_file_serving(self):
        ...
        try:
            response = urllib.request.urlopen(
                f"http://localhost:{self.server_port}{dashboard_relative_path}"
            )
            self.assertEqual(response.getcode(), 200)
            content = response.read().decode("utf-8")
    ...
    def test_mock_monitoring_dashboard_routes(self):
        ...
        def mock_handle_results(dashboard, request):
            ...
            above_dashboards = {
                name: details
                for name, details in results_dashboards.items()
