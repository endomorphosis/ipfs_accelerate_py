...
class TestReporterArtifactIntegration(unittest.TestCase):
        ...
        artifacts_to_create = [
            {
                ...
                "content": json.dumps({"tests": [{"name": "test1", "result": "pass"}]}),
            },
            {"name": "performance.csv", "content": "metric,value\nthroughput,125.4\nlatency,7.9"},
            {
                "name": "test.log",
                "content": "INFO: Starting tests\nERROR: Test2 failed\nINFO: Tests completed",
            },
        ]
    ...
    async def test_with_provider(self, provider_name):
        ...
        reporter = TestResultReporter(
            ci_provider=provider, report_dir=self.report_dir, artifact_dir=self.artifact_dir
        )
        ...
        urls = await reporter.get_artifact_urls(
            test_run_id=test_result.test_run_id, artifact_names=artifact_names
        )

        # Verify that we got URLs for all artifacts
        self.assertEqual(
            len(urls), len(artifact_names), f"Expected {len(artifact_names)} URLs, got {len(urls)}"
        )
        ...
        artifacts = await reporter.collect_and_upload_artifacts(
            test_run_id=f"{provider_name}-test-456", artifact_patterns=self.artifact_files
        )
        ...
        report_files = await reporter.report_test_result(
            test_result, formats=["markdown", "html", "json"]
        )
        ...
        for artifact in artifacts:
            self.assertIn(
                artifact["name"], html_content, f"HTML report missing artifact {artifact['name']}"
            )
            self.assertIn(
                artifact["url"], html_content, f"HTML report missing URL for {artifact['name']}"
            )
        ...
        self.assertIn(
            "artifacts", json_content["metadata"], "JSON report missing artifacts in metadata"
        )
        ...
        await reporter.report_test_result(test_result, formats=["markdown"])
        ...
        for artifact in artifacts:
            self.assertIn(
                artifact["name"], pr_comment, f"PR comment missing artifact {artifact['name']}"
            )
            self.assertIn(
                artifact["url"], pr_comment, f"PR comment missing URL for {artifact['name']}"
            )
        ...
        report_files = await reporter.report_test_result(
            updated_result, formats=["markdown", "html", "json"]
        )
        ...
        report_artifacts = [
            a for a in updated_result.metadata["artifacts"] if a.get("type") == "report"
        ]

        # Should have 3 report artifacts (markdown, html, json)
        self.assertEqual(
            len(report_artifacts), 3, f"Expected 3 report artifacts, got {len(report_artifacts)}"
        )
        ...
        for report_artifact in report_artifacts:
            ...
            self.assertIsNotNone(
                report_artifact["url"], f"URL for report {report_artifact['name']} is None"
            )
        ...
        urls = await reporter.get_artifact_urls(
            test_run_id=test_result.test_run_id, artifact_names=["non_existent.json"]
        )
...
async def test_parallel_url_retrieval_performance():
    ...
    reporter = TestResultReporter(
        ci_provider=provider, report_dir=report_dir, artifact_dir=artifact_dir
    )
...
async def test_edge_cases():
    ...
    try:
        ...
        reporter = TestResultReporter(
            ci_provider=None, report_dir=report_dir, artifact_dir=artifact_dir
        )

        # Try to get artifact URLs without a provider
        urls = await reporter.get_artifact_urls(
            test_run_id="test-123", artifact_names=["test.json", "test.log"]
        )
        ...
        reporter = TestResultReporter(
            ci_provider=MinimalProvider(), report_dir=report_dir, artifact_dir=artifact_dir
        )
        ...
        # Should return None for all artifacts
        assert len(urls) == 2, f"Expected 2 URL entries, got {len(urls)}"
        assert urls["test.json"] is None, (
            "URL should be None when provider doesn't support get_artifact_url"
        )
        assert urls["test.log"] is None, (
            "URL should be None when provider doesn't support get_artifact_url"
        )
        ...
        urls = await reporter.get_artifact_urls(test_run_id="test-123", artifact_names=[])
        ...
        await provider.upload_artifact(
            test_run_id, os.path.join(artifact_dir, "dummy.txt"), "test.json"
        )
        await provider.upload_artifact(
            test_run_id, os.path.join(artifact_dir, "dummy.txt"), "failing.json"
        )

        # Try to get URLs with one failing
        urls = await reporter.get_artifact_urls(
            test_run_id=test_run_id, artifact_names=["test.json", "failing.json"]
        )
...
async def test_ci_coordinator_integration():
    ...
    try:
        ...
        logger.warning(
            "DistributedTestingCoordinator not available, skipping CI coordinator integration test"
        )
        return
    ...
    try:
        ...
        coordinator = DistributedTestingCoordinator(
            db_path=db_path, enable_batch_processing=True, batch_size_limit=5
        )
        ...
        artifacts = await reporter.collect_and_upload_artifacts(
            test_run_id=test_run_id, artifact_patterns=artifact_files
        )
        ...
        assert len(task_artifacts) == len(artifacts), (
            f"Expected {len(artifacts)} artifacts, got {len(task_artifacts)}"
        )
