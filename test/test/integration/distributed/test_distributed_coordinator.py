...
class MockCoordinator:
    ...
    def add_task(self, task_name: str, params: Dict[str, Any]):
        ...
        self.tasks.put(
            {"task_id": task_id, "task_name": task_name, "params": params, "status": "pending"}
        )
        return task_id
...
class MockWorker:
    ...
    def __init__(
        ...
        self.capabilities = capabilities or {
            "hardware": {"cpu": "mock_cpu", "gpu": "mock_gpu", "memory": 16384},
            "platforms": ["webgpu", "cuda"],
            "browser": {"chrome": True, "firefox": True},
        }
    ...
    def run(self, max_tasks: int = 5):
        ...
        while not self.stopped and self.tasks_completed < max_tasks:
            ...
            if response["status"] == "task":
                task = response["task"]
                print(
                    f"Worker {self.worker_id} processing task {task['task_id']}: {task['task_name']}"
                )
...
def workers(coordinator):
    ...
    for i in range(4):
        capabilities = {
            ...
            "browser": {"chrome": i % 2 == 0, "firefox": i % 3 == 0},
        }
...
class TestDistributedCoordinator:
    ...
    def test_result_submission(self, coordinator, workers):
        ...
        for i, worker in enumerate(workers):
            ...
            submit_response = coordinator.submit_result(worker.worker_id, task["task_id"], result)
