...
try:
    ...
    logger.error(
        "Make sure you're running this script from the distributed_testing directory or the PYTHONPATH is set correctly."
    )
    sys.exit(1)
...
async def test_performance_metrics_storage():
    ...
    for _ in range(20):
        ...
        await error_handling.handle_error(
            error,
            {"component": error_type, "operation": "test_operation", "error_type": error_type},
        )
...
async def test_schema_migration():
    ...
    try:
        ...
        if count != 2:
            logger.error(
                f"Failed to insert new record after migration: found {count} rows, expected 2"
            )
            return False
...
async def test_date_functions():
    ...
    try:
        ...
        if delta.days < 29 or delta.days > 31:
            logger.error(
                f"Date function returned unexpected result: {thirty_days_ago}, delta: {delta.days} days"
            )
            return False
...
async def test_recovery_history_persistence():
    ...
    try:
        ...
        for i in range(3):
            success, info = await error_handling.handle_error(
                error, {"component": "network", "operation": "connect", "error_type": "network"}
            )
        ...
        if len(history2["history"]) != len(history["history"]):
            logger.error(
                f"History length mismatch: {len(history2['history'])} vs {len(history['history'])}"
            )
            return False
...
async def run_all_tests():
    ...
    logger.info(
        f"Performance record integrity test: {'SUCCESS' if integrity_result else 'FAILURE'}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test database integration for error recovery system"
    )
    parser.add_argument(
        "--test",
        choices=["schema", "date", "migration", "metrics", "history", "integrity", "all"],
        default="all",
