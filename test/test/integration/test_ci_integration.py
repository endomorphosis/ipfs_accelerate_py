...
from test.distributed_testing.ci.api_interface import (
    CIProviderFactory,
    CIProviderInterface,
    TestRunResult,
)
...
class TestCIIntegration(unittest.TestCase):
    ...
    async def _test_create_test_run(self):
        ...
        test_run_data = {"name": "Test Run", "build_id": "build-123", "commit_sha": "abcdef123456"}
    ...
    async def _test_update_test_run(self):
        ...
        update_data = {"status": "completed", "end_time": datetime.now().isoformat()}
    ...
    async def _test_upload_artifact(self):
        ...
        result = await self.mock_provider.upload_artifact(
            test_run_id, str(self.artifact_path), "sample.json"
        )
        self.assertTrue(result)
    ...
    async def _test_result_reporter_report_test_result(self):
        ...
        test_result.metadata = {
            "performance_metrics": {"average_throughput": 123.4, "average_latency_ms": 8.5}
        }
    ...
    async def _test_result_reporter_collect_artifacts(self):
        ...
        artifacts = await self.reporter.collect_and_upload_artifacts(
            test_run_id, [str(self.artifacts_dir / "*.json")]
        )
