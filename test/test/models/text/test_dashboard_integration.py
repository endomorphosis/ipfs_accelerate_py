...
def start_dashboard_server(host="localhost", port=8081, db_path=None):
    ...
    dashboard = MonitoringDashboard(host=host, port=port, db_path=db_path, auto_open=False)
    ...
    def test_alert_management(self):
        ...
        with start_dashboard_server(host=self.host, port=self.port, db_path=self.db_path):
            ...
            response = requests.post(
                f"{self.base_url}/api/alerts", json={"alert_id": alert_id, "action": "acknowledge"}
            )
            self.assertEqual(response.status_code, 200)
            ...
            # Resolve alert
            response = requests.post(
                f"{self.base_url}/api/alerts", json={"alert_id": alert_id, "action": "resolve"}
            )
            self.assertEqual(response.status_code, 200)
            result = response.json()
            self.assertTrue(result["success"])
