...
def test_ultra_low_precision(
    model_name, model_type, precision_bits, browser, extended_context=False
):
    """
    ...
    try:
        ...
        if result["success"]:
            ...
            print(
                f"  Saved: {memory_savings['saved_mb']:.1f} MB ({memory_savings['reduction_percent']:.1f}%)"
            )
...
def test_resource_pool_with_ulp(model_name, model_type, precision_bits=2, browser=None):
    ...
    try:
        ...
        if has_ulp:
            ...
            print(
                f"Memory reduction: {ulp_config['ultra_low_precision']['memory_reduction_percent']:.1f}%"
            )
            if ulp_config["ultra_low_precision"]["extended_context"]:
                print(
                    f"Context extension: {ulp_config['ultra_low_precision']['context_extension_factor']:.1f}x"
                )
...
def main():
    ...
    parser.add_argument(
        "--type", type=str, default="text", choices=["text", "vision", "audio"], help="Model type"
    )
    parser.add_argument(
        "--bits", type=int, default=2, choices=[2, 3, 4], help="Bits for quantization"
    )
    parser.add_argument(
        "--browser",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="basic",
        choices=["basic", "context", "resource-pool", "all"],
        help="Test mode",
    )
    ...
    if args.test_mode == "basic" or args.test_mode == "all":
        test_ultra_low_precision(
            args.model, args.type, args.bits, args.browser, args.extended_context
        )
