...
class TestSimulationValidationDBIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db_integration = SimulationValidationDBIntegration(db_path=self.temp_db_path)
        ...
        for i in range(len(self.simulation_results)):
            ...
            for metric in [
                ...
                if (
                    metric in self.simulation_results[i].metrics
                    and metric in self.hardware_results[i].metrics
                ):
        ...
        self.calibration_params = {
            ...
            "hardware_specific_factors": {
                "gpu_rtx3080": {"throughput_items_per_second": 1.1, "average_latency_ms": 0.85}
            },
            "model_specific_factors": {
                "bert-base-uncased": {"throughput_items_per_second": 1.15, "memory_peak_mb": 0.92}
            },
    ...
    def test_initialize_database(self):
        ...
        tables = self.db_integration.execute_query(
    ...
    def test_get_simulation_results_by_hardware(self):
        ...
        self.assertEqual(
            len(results), expected_count, f"Expected {expected_count} results, got {len(results)}"
        )
    ...
    def test_get_hardware_results_by_model(self):
        ...
        for result in results:
            self.assertEqual(
                result.model_id, model_id, f"Expected model_id {model_id}, got {result.model_id}"
            )
    ...
    def test_get_validation_results_by_criteria(self):
        ...
        results = self.db_integration.get_validation_results_by_criteria(
            hardware_id=hardware_id, model_id=model_id
        )
    ...
    def test_get_latest_calibration_parameters(self):
        ...
        new_params["calibration_timestamp"] = (
        ...
        self.assertEqual(
            latest_params["calibration_version"],
            "v1.1",
        ...
        self.assertEqual(
            latest_params["correction_factors"]["throughput_items_per_second"],
            1.25,
    ...
    def test_get_drift_detection_history(self):
        ...
        new_drift["new_window_end"] = (
            datetime.datetime.now() + datetime.timedelta(days=1)
        ).isoformat()
        new_drift["drift_metrics"]["throughput_items_per_second"]["mean_change_pct"] = (
            15.0  # Changed value
        )
        ...
        history = self.db_integration.get_drift_detection_history(
            hardware_type=hardware_type, model_type=model_type
        )
        ...
        self.assertEqual(
            history[0]["drift_metrics"]["throughput_items_per_second"]["mean_change_pct"],
            15.0,
    ...
    def test_get_mape_by_hardware_and_model(self):
        ...
        self.assertTrue(
            len(mape_results) <= expected_count,
    ...
    def test_analyze_calibration_effectiveness(self):
        ...
        for result in self.validation_results:
            ...
            for metric, comparison in result.metrics_comparison.items():
                ...
                calibrated_comparison["simulation_value"] = (
                    comparison["hardware_value"] * 1.02
                )  # Only 2% difference
                calibrated_comparison["absolute_error"] = abs(
                    calibrated_comparison["hardware_value"]
                    - calibrated_comparison["simulation_value"]
                )
                calibrated_comparison["relative_error"] = calibrated_comparison[
                    "absolute_error"
                ] / abs(calibrated_comparison["hardware_value"])
                calibrated_comparison["mape"] = (
                    calibrated_comparison["relative_error"] * 100
                )  # Much lower MAPE
        ...
        effectiveness = self.db_integration.analyze_calibration_effectiveness(
            before_version="test_v1.0", after_version="calibrated_v1.0"
        )

        # Check the analysis results
        self.assertIn(
            "overall_improvement", effectiveness, "Missing overall_improvement in analysis"
        )
        self.assertIn(
            "metric_improvements", effectiveness, "Missing metric_improvements in analysis"
        )
        self.assertIn(
            "hardware_improvements", effectiveness, "Missing hardware_improvements in analysis"
        )
        ...
        self.assertTrue(
            effectiveness["overall_improvement"] > 0,
    ...
    def test_get_calibration_history_for_hardware_model(self):
        ...
        hw_params["calibration_timestamp"] = (
            datetime.datetime.now() + datetime.timedelta(hours=1)
        ).isoformat()
        hw_params["hardware_specific_factors"]["gpu_rtx3080"]["throughput_items_per_second"] = (
            1.3  # Modified for GPU
        )
        ...
        model_params["calibration_timestamp"] = (
            datetime.datetime.now() + datetime.timedelta(hours=2)
        ).isoformat()
        model_params["model_specific_factors"]["bert-base-uncased"][
            "throughput_items_per_second"
        ] = 1.4  # Modified for BERT
    ...
    def test_integration_with_framework(self):
        ...
        try:
            ...
            if hasattr(framework, "validate"):
                ...
                logger.warning(
                    "Framework doesn't have validate method, skipping full integration test"
                )
                return
