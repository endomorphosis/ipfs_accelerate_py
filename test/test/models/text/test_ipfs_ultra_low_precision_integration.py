...
def parse_args():
    ...
    # Model selection
    parser.add_argument(
        "--model", type=str, default="bert", help="Model to test (llama, bert, t5, whisper, clip)"
    )

    parser.add_argument(
        "--model-path", type=str, default=None, help="Path to model (if not using default)"
    )
    ...
    parser.add_argument(
        "--mixed-precision", action="store_true", help="Use mixed precision across model components"
    )

    parser.add_argument(
        "--compare-all", action="store_true", help="Compare all precision levels (2, 3, 4, 8, 16)"
    )

    # Context extension testing
    parser.add_argument(
        "--extended-context", action="store_true", help="Test extended context window capabilities"
    )

    parser.add_argument(
        "--context-length", type=int, default=16384, help="Target context length for extension test"
    )
    ...
    parser.add_argument(
        "--compare-browsers", action="store_true", help="Compare performance across browsers"
    )
    ...
    # Benchmark options
    parser.add_argument(
        "--benchmark", action="store_true", help="Run comprehensive performance benchmarks"
    )
    ...
    parser.add_argument(
        "--measure-memory", action="store_true", help="Measure memory usage (approximate)"
    )
    ...
    parser.add_argument(
        "--db-path", type=str, default="./benchmark_db.duckdb", help="Path to benchmark database"
    )

    parser.add_argument(
        "--no-db", action="store_true", help="Disable database storage (use JSON output)"
    )
...
def setup_ipfs_with_ultra_low_precision(
    ...
    logger.info(
        f"Setting up IPFS with {precision_bits}-bit precision for {model_info['model_name']}"
    )
    ...
    try:
        ...
        logger.info(
            f"Memory reduction: {result['ultra_low_precision']['memory_reduction_percent']:.1f}%"
        )

        if extended_context:
            logger.info(
                f"Context extension: {result['ultra_low_precision']['context_extension_factor']:.1f}x"
            )
...
def test_with_resource_pool(
    ...
    logger.info(
        f"Setting up resource pool for {model_info['model_name']} with {precision_bits}-bit precision"
    )
...
def run_comprehensive_benchmark(
    ...
    for bits in precision_options:
        ...
        results["precision_results"][str(bits)] = {
            ...
            "memory_reduction_percent": setup_result["ultra_low_precision"][
        ...
        if verify_accuracy:
            ...
            results["precision_results"][str(bits)]["accuracy_impact_percent"] = (
                accuracy_impact.get(bits, 0.0)
            )
    ...
    if len(browsers) > 1:
        ...
        for browser in browsers:
            ...
            # Run inference test
            inference_result = run_inference_test(
                setup_result=setup_result, input_text=test_input, iterations=5
            )
...
def generate_benchmark_summary(benchmark_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "precision_results" in benchmark_results:
        ...
        for bits, data in benchmark_results["precision_results"].items():
            ...
            fp16_throughput = (
                benchmark_results["precision_results"]
                .get("16", {})
                .get("throughput_items_per_second", 0)
            )
    ...
    if summary["optimal_precision"]:
        findings.append(
            f"{summary['optimal_precision']}-bit precision provides the best balance of memory reduction and performance"
        )

    # Finding 2: Memory reduction
    if "2" in benchmark_results.get("precision_results", {}):
        memory_reduction = benchmark_results["precision_results"]["2"].get(
    ...
    if "2" in benchmark_results.get("precision_results", {}) and "16" in benchmark_results.get(
        "precision_results", {}
    ):
        bit2_throughput = benchmark_results["precision_results"]["2"].get(
            "throughput_items_per_second", 0
        )
        bit16_throughput = benchmark_results["precision_results"]["16"].get(
            "throughput_items_per_second", 0
        )
        ...
        if bit16_throughput > 0:
            ...
            findings.append(
                f"Ultra-low precision provides {speedup:.2f}x throughput compared to FP16"
            )

    # Finding 4: Browser comparison
    if summary["optimal_browser"]:
        findings.append(
            f"{summary['optimal_browser'].capitalize()} provides best performance for this model type"
        )
    ...
    try:
        ...
        if not (args.resource_pool or args.benchmark or args.compare_all):
            ...
            if setup_result.get("success", False):
                # Run inference test
                inference_result = run_inference_test(setup_result=setup_result, iterations=5)
        ...
        if args.output:
            ...
            default_output = (
                f"ultra_low_precision_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
