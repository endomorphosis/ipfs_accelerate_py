...
class TestDuckDBIntegration(unittest.TestCase):
    ...
    def test_06_end_to_end_integration(self):
        ...
        coordinator = CoordinatorDuckDBIntegration(
            db_path=self.db_path, batch_size=3, auto_vacuum_interval_hours=0, enable_dashboard=False
        )

        # Create workers
        worker1 = WorkerDuckDBIntegration(
            worker_id="e2e-worker-1", db_path=self.db_path, batch_size=2, submit_interval_seconds=1
        )

        worker2 = WorkerDuckDBIntegration(
            worker_id="e2e-worker-2", db_path=self.db_path, batch_size=2, submit_interval_seconds=1
        )
