...
class hf_whisper:
    """HuggingFace Whisper implementation.

    This class provides standardized interfaces for working with Whisper models
    across different hardware backends (CPU, CUDA, ROCm, OpenVINO, Apple, Qualcomm).

    This is a speech-to-text model used for automatic speech recognition.
    """
    ...
    def init_cpu(self, model_name, device, cpu_label):
        """Initialize Whisper model for CPU inference."""
        self.init()
    ...
    def init_cuda(self, model_name, device, cuda_label):
        """Initialize Whisper model for CUDA inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_cuda_speech_recognition_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_rocm(self, model_name, device, rocm_label):
        """Initialize Whisper model for ROCm (AMD GPU) inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_rocm_speech_recognition_endpoint_handler(
                endpoint_model=model_name,
    ...
    def init_openvino(self, model_name, device, openvino_label):
        """Initialize Whisper model for OpenVINO inference."""
        self.init()
        ...
        try:
            ...
            model = OVModelForSpeechSeq2Seq.from_pretrained(model_name, device=device, export=True)

            # Create handler function
            handler = self.create_openvino_speech_recognition_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_apple(self, model_name, device, apple_label):
        """Initialize Whisper model for Apple Silicon (MPS) inference."""
        self.init()
        ...
        try:
            ...
            # Create handler function
            handler = self.create_apple_speech_recognition_endpoint_handler(
                endpoint_model=model_name,
                device=device,
    ...
    def init_qualcomm(self, model_name, device, qualcomm_label):
        """Initialize Whisper model for Qualcomm inference."""
        self.init()
    ...
    def create_cpu_speech_recognition_endpoint_handler(
    ...
    def create_cuda_speech_recognition_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint, tokenizer
    ):
        """Create handler function for CUDA speech_recognition endpoint."""
    ...
    def create_rocm_speech_recognition_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint, tokenizer
    ):
        """Create ROCm (AMD GPU) handler for speech_recognition endpoint."""
        try:

            def handler(input_text, **kwargs):
                """ROCm handler for speech_recognition inference."""
                ...
                if "speech_recognition" == "text_embedding":
                    ...
                    "speech_recognition" == "text_generation" or "speech_recognition" == "causal_lm"
                ):
                    # For text generation
                ...
                elif "speech_recognition" == "image_classification":
    ...
    def create_openvino_speech_recognition_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint, tokenizer
    ):
        """Create handler function for OpenVINO speech_recognition endpoint."""
    ...
    def create_apple_speech_recognition_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint, tokenizer
    ):
        """Create handler function for Apple Silicon speech_recognition endpoint."""
    ...
    def create_qualcomm_speech_recognition_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint, tokenizer
    ):
        """Create handler function for Qualcomm speech_recognition endpoint."""
