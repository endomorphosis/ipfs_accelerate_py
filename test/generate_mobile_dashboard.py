...
class MobileDashboardGenerator:
    ...
    COLOR_SCHEMES = {
        "light": {
            ...
            "text": "#333333",
            "grid": "#eeeeee",
            "platforms": {"android": "#3ddc84", "ios": "#007aff"},
            "metrics": {
                "throughput": "#4caf50",
                "latency": "#2196f3",
                "memory": "#ff9800",
    ...
    def load_historical_data(self) -> bool:
        ...
        try:
            ...
            if not self.historical_data.empty:
                ...
                self.historical_data["throughput"] = self.historical_data["throughput"].astype(
                    float
                )
                self.historical_data["latency_mean"] = self.historical_data["latency_mean"].astype(
                    float
                )
                self.historical_data["memory_peak"] = self.historical_data["memory_peak"].astype(
                    float
                )
                self.historical_data["battery_impact"] = self.historical_data[
                    "battery_impact"
                ].astype(float)
        ...
        for i, platform in enumerate(platforms):
            fig.add_trace(
                go.Bar(
                    ...
                    marker_color=self.colors["platforms"].get(
                        platform, f"hsl({(i * 120) % 360}, 70%, 50%)"
                    ),
    ...
    def _create_model_comparison_chart(self) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Throughput (items/s)"),
        )
    ...
    def _create_historical_trends_chart(self) -> Optional[go.Figure]:
        ...
        key_models = ["bert-base-uncased", "mobilenet-v2", "clip-vit-base-patch32", "whisper-tiny"]
        ...
        trend_data = (
            df.groupby(["date", "platform", "model_name"])["throughput"].mean().reset_index()
        )
        ...
        for model in key_models:
            for platform in ["android", "ios"]:
                model_platform_data = trend_data[
                    (trend_data["model_name"] == model) & (trend_data["platform"] == platform)
                ]
                ...
                if not model_platform_data.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                size=6, symbol="circle" if platform == "android" else "square"
                            ),
                        )
                    )
    ...
    def _create_batch_scaling_chart(self) -> go.Figure:
                ...
                if x_values:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            line=dict(color=self.colors["platforms"].get(platform, "blue")),
                            marker=dict(
                                size=8, symbol="circle" if platform == "android" else "square"
                            ),
                        ),
                        row=row,
                        col=col,
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            showlegend=i == 0,  # Only show legend for first model
                            line=dict(
                                color=self.colors["platforms"].get(platform, "blue"), dash="dot"
                            ),
                        ),
                        row=row,
                        col=col,
        ...
        # Update all xaxes
        fig.update_xaxes(title="Batch Size", type="log", gridcolor=self.colors["grid"])

        # Update all yaxes
        fig.update_yaxes(title="Throughput (items/s)", gridcolor=self.colors["grid"])
...
def main():
    ...
    parser.add_argument(
        "--data-file", required=True, help="Path to JSON data file with benchmark results"
    )
    parser.add_argument(
        "--output", default="mobile_dashboard.html", help="Path to output HTML dashboard"
    )
    parser.add_argument("--db-path", help="Optional path to DuckDB database for historical data")
    parser.add_argument(
        "--title", default="Mobile Edge Performance Dashboard", help="Dashboard title"
    )
    parser.add_argument(
