...
async def run_coordinator(args):
    ...
    if coordinator.task_scheduler:
        coordinator.task_scheduler.max_tasks_per_worker = (
            2  # Allow 2 tasks per worker for demonstration
        )
...
async def create_benchmark_task(args, model, batch_sizes, hardware, min_memory_gb, priority=1):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        task_data = {
            ...
            "requirements": {"hardware": hardware, "min_memory_gb": min_memory_gb},
        }
        ...
        async with session.post(api_url, json=task_data) as resp:
            if resp.status == 200:
                ...
                logger.info(
                    f"Task created: {result['task_id']} (model: {model}, hardware: {hardware})"
                )
                return result
            else:
...
async def create_test_task(args, test_file, hardware, min_memory_gb, priority=1):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        task_data = {
            "type": "test",
            "priority": priority,
            "config": {"test_file": test_file, "test_args": ["--verbose"]},
            "requirements": {"hardware": hardware, "min_memory_gb": min_memory_gb},
        }
        ...
        async with session.post(api_url, json=task_data) as resp:
            if resp.status == 200:
                result = await resp.json()
                logger.info(
                    f"Task created: {result['task_id']} (file: {test_file}, hardware: {hardware})"
                )
                return result
            else:
                error_text = await resp.text()
...
async def test_intelligent_scheduler(args):
    ...
    for config in worker_configs[: args.num_workers]:  # Limit to requested number of workers
        ...
        logger.info(
            f"Started worker {config['id']} with capabilities: {config['capabilities']['hardware']}"
        )
    ...
    tasks.append(
        await create_benchmark_task(args, "bert-base-uncased", [1, 2, 4], ["cuda"], 6, priority=2)
    )
    tasks.append(
        await create_benchmark_task(args, "bert-large-uncased", [1, 2], ["cuda"], 10, priority=1)
    )
    tasks.append(
        await create_benchmark_task(
            args, "distilbert-base-uncased", [1, 2, 4, 8], ["cuda"], 4, priority=3
        )
    )
    ...
    # CPU-only tasks
    tasks.append(
        await create_benchmark_task(args, "bert-tiny", [1, 2, 4, 8, 16], ["cpu"], 2, priority=2)
    )
    ...
    if any("rocm" in w.capabilities.get("hardware", []) for w in workers):
        tasks.append(
            await create_benchmark_task(
                args, "bert-base-uncased", [1, 2, 4], ["rocm"], 6, priority=2
            )
        )
        tasks.append(
            await create_benchmark_task(args, "resnet50", [1, 4, 8, 16], ["rocm"], 4, priority=1)
        )
    ...
    while (datetime.now() - start_time).total_seconds() < run_time:
        ...
        if status:
            # Log status
            logger.info(
                f"Workers: {status['workers']['total']} ({status['workers']['active']} active, {status['workers']['busy']} busy)"
            )
            logger.info(
                f"Tasks: {sum(status['tasks'].values())} (pending: {status['tasks']['pending']}, running: {status['tasks']['running']}, completed: {status['tasks']['completed']})"
            )
        ...
        if (datetime.now() - start_time).total_seconds() % 10 < 5:
            ...
            await create_benchmark_task(
                args, model, [1, 2], hardware, memory, priority=random.randint(1, 3)
            )
...
async def main():
    ...
    parser.add_argument(
        "--db-path", default="./test_intelligent_scheduler.duckdb", help="Path to database"
    )
    parser.add_argument(
        "--security-config", default="./test_security_config.json", help="Path to security config"
    )
    parser.add_argument("--num-workers", type=int, default=3, help="Number of workers to start")
    parser.add_argument(
        "--run-time", type=int, default=60, help="How long to run the test in seconds"
    )
