...
try:
    ...
    _log_optional_dependency(
        "Visualization dependencies not available. Visualizations will be disabled."
    )
...
class CircuitBreakerTester:
    ...
    def __init__(self, quick_test: bool = False, simulate: bool = False):
        ...
        self.failure_types = ["timeout", "connection", "resource", "memory", "api"]
        self.test_results = []
    ...
    async def test_basic_functionality(self) -> None:
        ...
        try:
            ...
            assert state["state"] == "closed", (
                f"Expected circuit to be closed, got {state['state']}"
            )
    ...
    async def test_failure_simulation(self) -> None:
        ...
        async def operation_with_random_failures():
            ...
            if (
                self.simulate
                or (hash(f"{time.time()}{id(circuit_breaker)}") % 100) / 100 < self.success_rate
            ):
    ...
    async def test_optimization(self) -> None:
        ...
        async def operation_with_pattern(pattern_type: str = "normal"):
            ...
            if (
                self.simulate
                or (hash(f"{time.time()}{id(circuit_breaker)}") % 100) / 100 < success_prob
            ):
                # Successful operation
                await anyio.sleep(0.01)
                return "Success"
    ...
    async def test_prediction(self) -> None:
        ...
        async def create_failure_pattern(
            circuit_breaker: AdaptiveCircuitBreaker, pattern_type: str
        ) -> None:
            """
        ...
        assert os.path.exists(prediction_model_file), (
            f"Prediction model file {prediction_model_file} was not created"
        )
        ...
        try:
            ...
            assert circuit_breaker.circuit_breaker.state == "closed", (
                "Circuit should be closed at this point"
            )
        ...
        logger.info(
            f"Prediction test results: {preemptive_opens} preemptive opens, {regular_opens} regular opens"
        )

        # Print out model feature importances if available
        if VISUALIZATION_AVAILABLE and hasattr(
            circuit_breaker.prediction_model, "feature_importances_"
        ):
    ...
    async def test_hardware_specific(self) -> None:
        ...
        async def gpu_operation(success_rate: float = 0.5):
            if (
                self.simulate
                or (hash(f"{time.time()}{id(gpu_circuit_breaker)}") % 100) / 100 < success_rate
            ):
                await anyio.sleep(0.01)
                return "GPU Success"
            else:
        ...
        async def cpu_operation(success_rate: float = 0.7):
            if (
                self.simulate
                or (hash(f"{time.time()}{id(cpu_circuit_breaker)}") % 100) / 100 < success_rate
            ):
                await anyio.sleep(0.01)
                return "CPU Success"
            else:
        ...
        logger.info(
            f"GPU Circuit Breaker parameters: threshold={gpu_circuit_breaker.current_failure_threshold}, "
            f"recovery_timeout={gpu_circuit_breaker.current_recovery_timeout:.2f}s"
        )
        logger.info(
            f"CPU Circuit Breaker parameters: threshold={cpu_circuit_breaker.current_failure_threshold}, "
        ...
        assert (
            gpu_circuit_breaker.current_failure_threshold
            >= cpu_circuit_breaker.current_failure_threshold
            or gpu_circuit_breaker.current_recovery_timeout
            > cpu_circuit_breaker.current_recovery_timeout
    ...
    def generate_visualizations(self) -> None:
        ...
        if not VISUALIZATION_AVAILABLE:
            _log_optional_dependency(
                "Visualization dependencies not available. Skipping visualizations."
            )
            return
        ...
        if self.test_results:
            ...
            plt.bar(
                ...
                color=["green" if s else "red" for s in df["success"]],
            )
            ...
            logger.info(
                f"Visualization saved to {os.path.join(self.test_dir, 'visualizations/success_failure_pattern.png')}"
            )
            ...
            if "error" in df.columns:
                ...
                if error_types:
                    ...
                    # Save figure
                    plt.savefig(
                        os.path.join(self.test_dir, "visualizations/error_type_distribution.png")
                    )
                    plt.close()

                    logger.info(
                        f"Error distribution visualization saved to {os.path.join(self.test_dir, 'visualizations/error_type_distribution.png')}"
                    )
