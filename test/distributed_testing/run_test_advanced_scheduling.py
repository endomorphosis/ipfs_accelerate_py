...
class HardwareAwareScheduler(SchedulingAlgorithm):
    ...
    def _test_to_workload_profile(self, test_requirements):
        ...
        return create_workload_profile(
            workload_id=test_requirements.test_id, workload_type=WorkloadType.MIXED
        )
    ...
    def _update_worker_hardware_profiles(self, available_workers):
        ...
        for worker_id, capabilities in available_workers.items():
            self.worker_hardware_cache[worker_id] = [
                HardwareCapabilityProfile(
                    hardware_class=HardwareClass.GPU,
                    architecture="GPU_CUDA",
                    vendor="NVIDIA",
    ...
    def _adjust_efficiency_for_load_and_thermal(
        self, efficiency, worker_id, load, hardware_profile
    ):
    ...
    def create_scheduler(strategy_type, hardware_workload_manager, hardware_taxonomy, config=None):
        ...
        if strategy_type == SchedulingStrategyType.HARDWARE_AWARE:
            ...
            return HistoricalPerformanceScheduler(
                hardware_workload_manager, hardware_taxonomy, config.get("db_path")
            )
...
class TestEnvironment:
    ...
    def _create_simulated_workers(self, num_workers: int) -> Dict[str, WorkerCapabilities]:
        ...
        for i in range(num_workers):
            ...
            load = WorkerLoad(
                ...
                current_memory_usage=random.random()
                * 0.2
                * available_memory,  # Initial low memory usage
                warming_state=False,
                cooling_state=False,
            )
    ...
    def create_worker_loads(self) -> Dict[str, WorkerLoad]:
        ...
        for worker_id, capabilities in self.workers.items():
            ...
            memory_usage = (
                min(0.1 + (current_tests / max_tests) * 0.8 + random.random() * 0.1, 1.0)
                * capabilities.available_memory
            )
            ...
            def has_capacity_for(test_req):
                return (
                    current_tests < max_tests
                    and memory_usage + test_req.min_memory_gb < capabilities.available_memory * 0.9
                )
            ...
            if worker_id in self.workload_manager.thermal_tracking:
                ...
                new_temp = min(
                    current_temp + (current_tests / max_tests) * 0.1 + random.random() * 0.05, 1.0
                )
    ...
    def test_hardware_aware_scheduler(self) -> None:
        ...
        scheduler = self.scheduler_factory.create_scheduler(
            SchedulingStrategyType.HARDWARE_AWARE, self.workload_manager, self.hardware_taxonomy
        )
    ...
    def _run_scheduling_test(
        self, scheduler, scheduler_type: str, num_tests: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        for test in tests:
            ...
            selected_worker = scheduler.select_worker(
                test, self.workers, worker_loads, self.worker_performance
            )
            end_time = time.time()
            ...
            if selected_worker:
                ...
                logger.info(
                    f"[{scheduler_type}] Selected worker {selected_worker} ({self.workers[selected_worker].worker_type}) "
                ...
                if hasattr(scheduler, "record_execution_result"):
                    scheduler.record_execution_result(
                        test.test_id, selected_worker, execution_time, success
                    )

                # Update deadline status if applicable
                if hasattr(scheduler, "update_test_deadline_status"):
                    scheduler.update_test_deadline_status(test.test_id, success, execution_time)

                results.append(result)
            else:
                logger.warning(
                    f"[{scheduler_type}] No suitable worker found for test {test.test_id} ({test.test_type})"
                )
    ...
    def _add_simulated_training_data(self, scheduler) -> None:
        ...
        for i in range(100):
            ...
            # Record result
            scheduler.record_execution_result(test.test_id, worker_id, execution_time, success)

    def _record_scheduling_decision(
        self, scheduler, test_id: str, worker_id: str, features: Dict[str, float]
    ) -> None:
        """
    ...
    def _simulate_test_execution(
        self, test: TestRequirements, worker_id: str
    ) -> Tuple[float, bool]:
        """
    ...
    def _compare_scheduler_results(
        ...
        for scheduler_type, scheduler_results in results.items():
            ...
            total_decision_time = sum(
                result.get("decision_time_ms", 0) for result in scheduler_results
            )
            avg_decision_time = (
                total_decision_time / len(scheduler_results) if scheduler_results else 0
            )
...
def run_tests():
    ...
    parser.add_argument(
        "--db-path", type=str, default=None, help="Path to database for persistence"
    )
    parser.add_argument(
        "--output", type=str, default="scheduling_results.json", help="Output file for results"
    )
    ...
    for scheduler_type, metrics in results["comparison"].items():
        try:
            print(
                f"{scheduler_type:<20} {metrics.get('avg_execution_time', 0):<15.2f} {metrics.get('success_rate', 0):<15.2f} {metrics.get('avg_decision_time_ms', 0):<20.2f}"
            )
