...
class EnhancedHardwareTaxonomy(HardwareTaxonomy):
    ...
    def _initialize_common_capabilities(self):
        ...
        self.register_capability(
            CapabilityDefinition(
                ...
                properties={"operations": ["matmul", "gemm"], "dimensionality": ["2D"]},
        ...
        self.register_capability(
            CapabilityDefinition(
                ...
                capability_type="compute",
                properties={"operations": ["matmul", "conv2d"], "precisions": ["fp16", "int8"]},
                related_capabilities=["compute.matrix_multiplication", "compute.convolution"],
                hardware_requirements={"features": ["tensor_cores"]},
        ...
        self.register_capability(
            CapabilityDefinition(
                ...
                properties={"min_bandwidth_gbps": 500.0},
                hardware_requirements={"min_bandwidth_gbps": 500.0},
        ...
        self.register_capability(
            CapabilityDefinition(
                ...
                capability_type="memory",
                properties={"shared_address_space": True},
                hardware_requirements={"unified_memory": True},
    ...
    def _initialize_hardware_hierarchies(self):
        ...
        self.define_hardware_hierarchy(HardwareClass.CPU, HardwareClass.CPU, 1.0)

        # GPU-based hierarchies
        self.define_hardware_hierarchy(HardwareClass.GPU, HardwareClass.GPU, 0.8)

        # CPU to other accelerators
        self.define_hardware_hierarchy(
            HardwareClass.CPU, HardwareClass.GPU, 0.5, capability_filters={"memory.unified"}
        )
        self.define_hardware_hierarchy(HardwareClass.CPU, HardwareClass.TPU, 0.3)
        self.define_hardware_hierarchy(HardwareClass.CPU, HardwareClass.NPU, 0.4)

        # GPU to other accelerators
        self.define_hardware_hierarchy(HardwareClass.GPU, HardwareClass.TPU, 0.4)
        self.define_hardware_hierarchy(HardwareClass.GPU, HardwareClass.NPU, 0.4)
        ...
        self.define_hardware_hierarchy(HardwareClass.HYBRID, HardwareClass.CPU, 0.9)
    ...
    def register_hardware_capability(
        self, hardware_id: str, capability_id: str, properties: Dict[str, Any] = None
    ):
        """
    ...
    def define_hardware_hierarchy(
        ...
        logger.debug(
            f"Defined hardware hierarchy: {parent_class.value} -> {child_class.value} "
    ...
    def register_hardware_relationship(self, relationship: HardwareRelationship):
        ...
        logger.debug(
            f"Registered hardware relationship: {relationship.source_id} -> {relationship.target_id} "
        ...
        if capabilities:
            ...
            for cap_id, props in capabilities.items():
                ...
                if cap_def and cap_def.hardware_requirements:
                    if not self._hardware_profile_meets_requirements(
                        hardware_profile, cap_def.hardware_requirements
                    ):
                        continue
    ...
    def _infer_capabilities(self, hardware_profile: HardwareCapabilityProfile) -> Dict[str, Any]:
        ...
        feature_values = {
            getattr(feature, "value", feature) for feature in hardware_profile.features
        }
        precision_values = {
            getattr(precision, "value", precision)
            for precision in hardware_profile.supported_precisions
        }
        hardware_class_value = getattr(
            hardware_profile.hardware_class, "value", hardware_profile.hardware_class
        )

        # Infer compute capabilities
        if hardware_profile.compute_units > 0:
            inferred_capabilities["compute.matrix_multiplication"] = {
                "performance": hardware_profile.compute_units
                * (hardware_profile.clock_speed_mhz or 1000)
                * 0.001
            }
        ...
        if hardware_profile.memory.has_unified_memory:
            inferred_capabilities["memory.unified"] = {"shared_address_space": True}
        ...
        if (
            PrecisionType.INT8.value in precision_values
            or PrecisionType.INT4.value in precision_values
        ):
        ...
        if (
            hardware_class_value == HardwareClass.CPU.value
            and AcceleratorFeature.AVX2.value in feature_values
        ):
            inferred_capabilities["specialized.audio"] = {"effectiveness": 0.8}

        return inferred_capabilities

    def _get_inherited_capabilities_from_hierarchies(
        self, hardware_profile: HardwareCapabilityProfile
    ) -> Dict[str, Any]:
        """
            ...
            for hierarchy in hierarchies:
                if hierarchy.child_class == profile_class:
                    ...
                    for parent_profile in parent_profiles:
                        parent_id = (
                            f"{parent_profile.hardware_class.value}_{parent_profile.model_name}"
                        )
                        ...
                        for cap_id, props in parent_capabilities.items():
                            if cap_id not in hierarchy.capability_filters:
                                ...
                                if cap_id not in inherited_capabilities:
                                    ...
                                    inherited_props = (
                                        props.copy() if isinstance(props, dict) else props
                                    )
                                    ...
                                    if isinstance(inherited_props, dict):
                                        for key, value in inherited_props.items():
                                            if isinstance(value, (int, float)):
                                                inherited_props[key] = (
                                                    value * hierarchy.inheritance_factor
                                                )
    ...
    def find_hardware_with_capability(
        self, capability_id: str, min_threshold: float = 0.0
    ) -> List[Tuple[HardwareCapabilityProfile, Dict[str, Any]]]:
        """
    ...
    def calculate_workload_hardware_match(
        self, required_capabilities: Dict[str, Any], hardware_profile: HardwareCapabilityProfile
    ) -> float:
        """
                ...
                if isinstance(required_props, dict) and isinstance(hardware_props, dict):
                    ...
                    for key, required_value in required_props.items():
                        if key in hardware_props:
                            ...
                            elif isinstance(required_value, list) and isinstance(
                                hardware_value, list
                            ):
    ...
    def find_optimal_hardware_for_workload(
        self, required_capabilities: Dict[str, Any], available_workers: Optional[List[str]] = None
        ...
        valid_worker_ids = (
            ...
            else list(self.worker_hardware_map.keys())
        )
        ...
        for worker_id in valid_worker_ids:
            if worker_id in self.worker_hardware_map:
                ...
                for profile in worker_profiles:
                    match_score = self.calculate_workload_hardware_match(
    ...
    def register_hardware_profile_with_capabilities(self, profile: HardwareCapabilityProfile):
        ...
        logger.info(
            f"Registered hardware profile with {len(inferred_capabilities)} inferred capabilities: {profile_id}"
        )
    ...
    def find_related_hardware(
        self, hardware_id: str, relationship_type: Optional[str] = None, min_strength: float = 0.0
