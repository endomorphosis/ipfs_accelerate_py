...
class ArtifactRetriever:
    ...
    def __init__(
        ...
        logger.info(
            f"ArtifactRetriever initialized with cache_dir={cache_dir}, max_cache_size_mb={max_cache_size_mb}"
        )
    ...
    async def _clean_cache_if_needed(self) -> None:
        ...
        try:
            ...
            if total_size_mb > self.max_cache_size_mb:
                logger.info(
                    f"Cache size ({total_size_mb:.2f} MB) exceeds limit ({self.max_cache_size_mb} MB). Cleaning..."
                )
    ...
    async def retrieve_latest_artifact_by_type(
        ...
        latest_artifact = ArtifactDiscovery.find_latest_artifact(
            artifacts=artifacts, artifact_type=artifact_type, test_run_id=test_run_id
        )
        ...
        for metric_name, values in metrics_data.items():
            ...
            sorted_artifacts = sorted(
                [
                    a
                    for a in artifacts
                    if "metrics" in a.content_metadata
                    and metric_name in a.content_metadata["metrics"]
                ],
            ...
            if len(values) >= window_size:
                ...
                for i in range(len(values) - window_size + 1):
                    ...
                    moving_avg.append(
                        {"timestamp": timestamps[i + window_size - 1].isoformat(), "value": avg}
                    )
    ...
    if args.action == "list-types":
        ...
        if not artifacts:
            ...
            for i, artifact in enumerate(artifacts):
                print(
                    f"{i + 1}. {artifact.artifact_name} ({artifact.artifact_type}) - {artifact.creation_time}"
                )
        ...
        artifact1 = ArtifactMetadata(
            artifact_name=os.path.basename(args.artifact1), artifact_path=args.artifact1
        )

        artifact2 = ArtifactMetadata(
            artifact_name=os.path.basename(args.artifact2), artifact_path=args.artifact2
        )
