...
class BitbucketClient(CIProviderInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if aiohttp is None:
            self._offline = True
            logger.warning(
                "aiohttp not available; BitbucketClient running in offline/simulation mode"
            )
    ...
    async def update_test_run(self, test_run_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            bitbucket_result_map = {"running": "PENDING", "completed": "PASSED", "failed": "FAILED"}
    ...
    async def upload_artifact(
        ...
        try:
            ...
            logger.info(
                f"Artifact upload for Bitbucket not fully implemented. Would upload {artifact_path} as {artifact_name}"
            )
            return True
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 201 or response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error setting Bitbucket build status: {response.status} - {error_text}"
                    )
                    return False
            ...
            async with self.session.get(downloads_url) as response:
                if response.status == 200:
                    ...
                    for download in downloads_data.get("values", []):
                        ...
                        if (
                            download.get("name") == artifact_name
                            or download.get("name") == f"{report_id}-{artifact_name}"
                        ):
                            ...
                            if artifact_url:
                                ...
                                logger.info(
                                    f"Found artifact URL for {artifact_name}: {artifact_url}"
                                )
                                return artifact_url
                    ...
                    logger.warning(
                        f"Artifact not found in downloads API, using fallback URL: {fallback_url}"
                    )
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error getting Bitbucket downloads list: {response.status} - {error_text}"
                    )
                    return None
