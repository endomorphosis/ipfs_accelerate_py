...
class TeamCityClient(CIProviderInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if aiohttp is None:
            self._offline = True
            logger.warning(
                "aiohttp not available; TeamCityClient running in offline/simulation mode"
            )
    ...
    async def create_test_run(self, test_run_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            comment = (
                f"Starting distributed test run: {name} (ID: distributed-test-{int(time.time())})"
            )

            try:
                async with self.session.put(
                    api_url, data=comment, headers={"Content-Type": "text/plain"}
                ) as response:
                    if response.status == 200 or response.status == 204:
                        ...
                        logger.warning(
                            f"Failed to add comment to build: {response.status} - {error_text}"
                        )
        ...
        try:
            ...
            try:
                async with self.session.put(
                    ...
                    if response.status == 200 or response.status == 204:
                        logger.info(
                            f"Updated comment for test run {test_run_id} on build {build_id}"
                        )
                        return True
                    else:
                        error_text = await response.text()
    ...
    async def add_pr_comment(self, pr_number: str, comment: str) -> bool:
        ...
        """
        logger.info(
            f"TeamCityClient doesn't support adding PR comments directly. Would add comment to PR #{pr_number}"
        )
        return False
    ...
    async def upload_artifact(
        ...
        try:
            ...
            logger.info(
                f"Artifact upload for TeamCity would publish {artifact_path} as {artifact_name} to build {build_id}"
            )
            return True
    ...
    async def get_test_run_status(self, test_run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_run_id.startswith("teamcity-simulated-"):
                ...
                return {"id": test_run_id, "status": "running"}
        ...
        try:
            ...
            async with self.session.put(
                ...
                if response.status == 200 or response.status == 204:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error setting build status comment: {response.status} - {error_text}"
                    )
                    return False
        ...
        try:
            ...
            async with self.session.get(api_url) as response:
                ...
                artifacts_url = urljoin(
                    self.url, f"app/rest/builds/id:{build_id}/artifacts/children"
                )
                ...
                async with self.session.get(artifacts_url) as artifacts_response:
                    if artifacts_response.status != 200:
                        ...
                        logger.error(
                            f"Error getting artifacts list: {artifacts_response.status} - {error_text}"
                        )
                        return None
                    ...
                    if found_artifact:
                        ...
                        artifact_url = urljoin(
                            self.url,
                            f"app/rest/builds/id:{build_id}/artifacts/content/{quote(artifact_path)}",
                        )
                    ...
                    fallback_url = urljoin(
                        self.url,
                        f"repository/download/{build_type_id}/{build_id}:id/{quote(artifact_name)}",
                    )
                    logger.warning(
                        f"Artifact not found in API response, using fallback URL: {fallback_url}"
                    )
