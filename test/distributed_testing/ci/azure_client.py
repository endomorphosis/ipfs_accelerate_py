...
class AzureDevOpsClient(CIProviderInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if aiohttp is None:
            self._offline = True
            logger.warning(
                "aiohttp not available; AzureDevOpsClient running in offline/simulation mode"
            )

        logger.info(
            f"AzureDevOpsClient initialized for organization {self.organization}, project {self.project}"
        )
        return True
                ...
                async with self.session.post(url, json=payload) as response:
                    if response.status == 200:
                        ...
                        test_run = {
                            ...
                            "url": data.get(
                                "url",
                                f"https://dev.azure.com/{self.organization}/{self.project}/_testManagement/runs?runId={data['id']}",
                            ),
                        }
                        ...
                        logger.error(
                            f"Error creating Azure DevOps test run: {response.status} - {error_text}"
                        )
    ...
    async def update_test_run(self, test_run_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            state_map = {"running": "InProgress", "completed": "Completed", "failed": "Completed"}

            # Map our status to Azure DevOps test run outcome
            outcome_map = {"completed": "Passed", "failed": "Failed"}

            # Set Azure state based on our status
            azure_state = state_map.get(status, "InProgress")

            # Create the update payload
            payload = {"state": azure_state}
            ...
            async with self.session.patch(url, json=payload) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error updating Azure DevOps test run: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def add_pr_comment(self, pr_number: str, comment: str) -> bool:
        ...
        try:
            ...
            url = (
                f"{self.api_url}git/repositories/{{repositoryId}}/pullRequests/{pr_number}/threads"
            )
    ...
    async def upload_artifact(
        ...
        if self._offline:
            ...
            self._artifact_urls[test_run_id][artifact_name] = (
                f"file://{os.path.abspath(artifact_path)}"
            )
        ...
        try:
            ...
            async with self.session.post(container_url, json=container_payload) as response:
                if response.status == 200:
                    ...
                    upload_url = (
                        f"{self.api_url}test/runs/{test_run_id}/attachments/{attachment_id}"
                    )
                    ...
                    async with self.session.put(
                        ...
                        if upload_response.status in (200, 201):
                            logger.info(
                                f"Successfully uploaded artifact {artifact_name} to Azure DevOps test run {test_run_id}"
                            )
                            return True
                        else:
                            error_text = await upload_response.text()
                            logger.error(
                                f"Error uploading artifact content to Azure DevOps: {upload_response.status} - {error_text}"
                            )
                            return False
                else:
                    error_text = await response.text()
                    logger.error(
                        f"Error creating artifact container in Azure DevOps: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def get_test_run_status(self, test_run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_run_id.startswith("azure-simulated-"):
                ...
                return {"id": test_run_id, "status": "running", "conclusion": None}
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    return {
                        ...
                        "url": data.get("webAccessUrl")
                        or f"https://dev.azure.com/{self.organization}/{self.project}/_testManagement/runs?runId={test_run_id}",
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error getting Azure DevOps test run: {response.status} - {error_text}"
                    )
            ...
            # Return a simulated status for easier recovery
            return {"id": test_run_id, "status": "unknown", "error": f"Exception: {str(e)}"}
    ...
    async def get_artifact_url(self, test_run_id: str, artifact_name: str) -> Optional[str]:
        ...
        try:
            ...
            if (
                hasattr(self, "_artifact_attachments")
                and test_run_id in self._artifact_attachments
                and artifact_name in self._artifact_attachments[test_run_id]
            ):
            ...
            async with self.session.get(attachments_url) as response:
                if response.status == 200:
                    ...
                    for attachment in attachments_data.get("value", []):
                        if attachment.get("fileName") == artifact_name:
                            ...
                            if attachment_id:
                                ...
                                logger.info(
                                    f"Found artifact URL for {artifact_name}: {attachment_url}"
                                )
                    ...
                    logger.warning(
                        f"Failed to get attachments for test run {test_run_id}: {response.status}"
                    )
                    return None
    ...
    async def set_build_status(self, status: str, description: str) -> bool:
        ...
        try:
            ...
            payload = {
                ...
                "status": "completed",
                "properties": {"StatusDescription": description},
            }
            ...
            async with self.session.patch(url, json=payload) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error setting Azure DevOps build status: {response.status} - {error_text}"
                    )
                    return False
