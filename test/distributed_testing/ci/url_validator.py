...
class ArtifactURLValidator:
        ...
        try:
            while True:
                ...
                for url in self._registered_urls:
                    try:
                        is_valid, status_code, error_message = await self.validate_url(
                            url, use_cache=False
                        )
                        logger.info(
                            f"Health check for {url}: {'Valid' if is_valid else 'Invalid'} "
    ...
    async def validate_url(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"Error validating URL {url} (attempt {attempt + 1}/{self.max_retries}): {str(e)}"
                )
                error_message = str(e)
    ...
    async def _check_url(
        ...
        try:
            ...
            async with session.head(
                url, timeout=self.check_timeout, allow_redirects=True
            ) as response:
                is_valid = response.status < 400
                return (
                    is_valid,
                    response.status,
                    None if is_valid else f"HTTP error: {response.status}",
                )
    ...
    def get_url_health(self, url: str, timespan: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def get_all_url_health(self, timespan: Optional[int] = None) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def generate_health_report(
        self, timespan: Optional[int] = 86400, format: str = "dict"
        ...
        overall_availability = (
            sum(info["availability"] for info in url_health.values()) / total_urls
            if total_urls > 0
            else 0
        )
    ...
    def _generate_markdown_report(self, report: Dict[str, Any]) -> str:
        ...
        valid_percent = (
            (report["valid_urls"] / report["total_urls"] * 100.0)
        ...
        sorted_urls = sorted(
            report["urls"].items(), key=lambda item: (item[1]["is_valid"], item[1]["availability"])
        )

        for url, info in sorted_urls:
            status = "✅ Valid" if info["is_valid"] else f"❌ Invalid ({info['error_message']})"
            last_checked = (
                datetime.fromtimestamp(info["last_checked"]).strftime("%Y-%m-%d %H:%M:%S")
...
"""
        ...
        valid_percent = (
            report["valid_urls"] / report["total_urls"] * 100 if report["total_urls"] > 0 else 0
        )
...
async def generate_health_report(
    timespan: Optional[int] = 86400, format: str = "dict"
) -> Union[Dict[str, Any], str]:
    """
