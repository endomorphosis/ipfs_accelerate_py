...
class TestResultReporter:
    ...
    def __init__(
        ...
        logger.info(
            f"Test result reporter initialized with report_dir={self.report_dir}, artifact_dir={self.artifact_dir}"
        )

    async def get_artifact_urls(
        self, test_run_id: str, artifact_names: List[str], validate: bool = False
                ...
                if isinstance(run_cache, dict):
                    ...
                    if validate and urls:
                        try:
                            ...
                            valid_urls = {
                                name: url for name, url in urls.items() if url is not None
                            }
                            if valid_urls:
                ...
                if valid_urls:
                    ...
                    for url, (is_valid, status_code, error_message) in validation_results.items():
                        ...
                        if name is not None:
                            if is_valid:
                                logger.debug(
                                    f"Validated artifact URL for {name}: {url} (Status: {status_code})"
                                )
                            else:
                                logger.warning(
                                    f"Artifact URL for {name} ({url}) is not accessible: {error_message}"
                                )
            except ImportError:
                logger.info("URL validator not available, skipping URL validation")
            except Exception as e:
    ...
    async def report_test_result(
        self, result: TestRunResult, formats: List[str] = None
    ) -> Dict[str, str]:
        """
        ...
        if self.ci_provider and result.test_run_id:
            ...
            for fmt, file_path in report_files.items():
                try:
                    ...
                    success = await self.ci_provider.upload_artifact(
                        result.test_run_id, file_path, artifact_name
                    )
                    if success:
            ...
            if report_artifact_names and hasattr(self.ci_provider, "get_artifact_url"):
                try:
                    ...
                    report_urls = await self.get_artifact_urls(
                        result.test_run_id, list(report_artifact_names.values())
                    )
                    ...
                    for fmt, artifact_name in report_artifact_names.items():
                        if artifact_name in report_urls and report_urls[artifact_name]:
                            ...
                            logger.info(
                                f"Added {fmt} report URL to result metadata: {report_urls[artifact_name]}"
                            )
                except Exception as e:
            ...
            try:
                await self.ci_provider.update_test_run(
                    result.test_run_id,
                    {
            ...
            for file_path in glob.glob(pattern, recursive=True):
                ...
                if self.ci_provider:
                    try:
                        # Upload the artifact
                        success = await self.ci_provider.upload_artifact(
                            test_run_id, artifact_path, file_name
                        )
                        ...
                        if success:
                            ...
                            try:
                                if hasattr(self.ci_provider, "get_artifact_url"):
                                    artifact_url = await self.ci_provider.get_artifact_url(
                                        test_run_id, file_name
                                    )
                                    if artifact_url:
                                        logger.info(
                                            f"Retrieved artifact URL for {file_name}: {artifact_url}"
                                        )
                                    else:
                                        # Fallback to a placeholder URL if get_artifact_url returned None
                                        artifact_url = f"ci://artifacts/{test_run_id}/{file_name}"
                                        logger.warning(
                                            f"Could not retrieve URL for artifact {file_name}, using placeholder"
                                        )
                                else:
                                    # Fallback if get_artifact_url method doesn't exist
                                    artifact_url = f"ci://artifacts/{test_run_id}/{file_name}"
                                    logger.warning(
                                        f"CI provider doesn't support get_artifact_url, using placeholder URL"
                                    )
                            except Exception as e:
                                # Fallback if get_artifact_url fails
                                artifact_url = f"ci://artifacts/{test_run_id}/{file_name}"
                                logger.warning(
                                    f"Error retrieving artifact URL for {file_name}: {str(e)}, using placeholder"
                                )
                ...
                if (
                    validate_urls
                    and artifact_url
                    and not artifact_url.startswith("ci://artifacts/")
                ):
                    try:
                        # Import the URL validator
                        ...
                        if not is_valid:
                            logger.warning(
                                f"Artifact URL for {file_name} ({artifact_url}) is not accessible: {error_message}"
                            )
                        ...
                        if include_health_info:
                            try:
                                ...
                                logger.error(
                                    f"Failed to get URL health info for {file_name}: {str(e)}"
                                )
...
async def main():
    ...
    parser.add_argument(
        "--status", required=True, choices=["success", "failure", "running"], help="Test run status"
    )
    ...
    # Report options
    parser.add_argument(
        "--formats", default="markdown,html,json", help="Comma-separated list of output formats"
    )
    ...
    parser.add_argument(
        "--collect-artifacts", help="Comma-separated list of glob patterns for artifacts to collect"
    )
    ...
    reporter = TestResultReporter(
        ci_provider=ci_provider, report_dir=args.report_dir, artifact_dir=args.artifact_dir
    )
