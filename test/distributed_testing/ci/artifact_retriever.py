...
class ArtifactRetriever:
    ...
    async def _cleanup_cache(self) -> None:
        ...
        if cache_size_mb < self.max_cache_size_mb:
            logger.debug(
                f"Cache size {cache_size_mb:.2f} MB is below limit of {self.max_cache_size_mb} MB"
            )
            return

        logger.info(
            f"Cache size {cache_size_mb:.2f} MB exceeds limit of {self.max_cache_size_mb} MB, cleaning up"
        )
        ...
        for artifact in cached_artifacts:
            ...
            if os.path.exists(artifact["path"]):
                ...
                try:
                    ...
                    removed += 1
                    logger.debug(
                        f"Removed cached artifact: {artifact['path']}, saved {file_size:.2f} MB"
                    )
                except Exception as e:
    ...
    def _get_cache_path_for_artifact(
        self, test_run_id: str, artifact_name: str, provider_name: str
    ) -> str:
        """
    ...
    async def retrieve_artifact(
        ...
        cache_path, cache_key = self._get_cache_path_for_artifact(
            test_run_id=test_run_id, artifact_name=artifact_name, provider_name=provider_name
        )
        ...
        if use_cache and cache_key in self.cache_metadata and os.path.exists(cache_path):
            ...
            if verify_hash and "content_hash" in self.cache_metadata[cache_key]:
                ...
                if artifact_metadata.content_hash != stored_hash:
                    logger.warning(
                        f"Hash mismatch for cached artifact {cache_path}, retrieving fresh copy"
                    )
        ...
        if not artifact_url:
            logger.error(
                f"Failed to get URL for artifact {artifact_name} from provider {provider_name}"
            )
            return None
        ...
        try:
            ...
            async with self.session.get(artifact_url) as response:
                if response.status != 200:
                    logger.error(
                        f"Failed to download artifact: {response.status} - {await response.text()}"
                    )
                    return None
    ...
    async def retrieve_artifacts_batch(
        self, artifacts: List[Dict[str, str]], use_cache: bool = True
    ...
    async def analyze_metrics_trend(
        ...
        metrics = ArtifactDiscovery.extract_metrics_from_artifacts(
            artifacts=relevant_artifacts, metric_names=[metric_name]
        )
    ...
    async def compare_artifacts(
        self, artifact1: Dict[str, str], artifact2: Dict[str, str], use_cache: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if not metadata1.is_binary and not metadata2.is_binary:
            ...
            if (
                metadata1.mimetype
                and metadata1.mimetype.startswith("text/")
                and metadata2.mimetype
                and metadata2.mimetype.startswith("text/")
            ):
                try:
                    with (
                        open(path1, "r", encoding="utf-8", errors="replace") as f1,
                    ...
                    comparison["content_differences"]["error"] = (
                        f"Error comparing text files: {str(e)}"
                    )
            ...
            # Compare JSON files
                ...
                try:
                    with (
                        open(path1, "r", encoding="utf-8") as f1,
                    ...
                    comparison["content_differences"]["error"] = (
                        f"Error comparing JSON files: {str(e)}"
                    )
        else:
    ...
    def _calculate_difference(self, value1: Any, value2: Any) -> Any:
        ...
        if isinstance(value1, (int, float)) and isinstance(value2, (int, float)):
            ...
            return {"absolute": abs_diff, "percent": percent_diff}
        else:
