...
async def test_artifact_lifecycle():
    ...
    try:
        ...
        with open(report_file_path, "w") as f:
            json.dump({"tests": 10, "passed": 8, "failed": 2}, f, indent=2)
        ...
        for name, test_run in test_runs.items():
            ...
            results[name] = {"log": log_result, "report": report_result}
        ...
        for name, test_run in test_runs.items():
            ...
            assert log_artifact.artifact_type == "log", (
                f"Wrong artifact type for log: {log_artifact.artifact_type}"
            )
            ...
            assert report_artifact.artifact_type == "report", (
                f"Wrong artifact type for report: {report_artifact.artifact_type}"
            )
        ...
        for name, test_run in test_runs.items():
            ...
            assert len(artifacts) == 0, (
                f"Expected 0 artifacts after purge for {name}, found {len(artifacts)}"
            )
