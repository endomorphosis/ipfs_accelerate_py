...
class CircleCIClient(CIProviderInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if aiohttp is None:
            self._offline = True
            logger.warning(
                "aiohttp not available; CircleCIClient running in offline/simulation mode"
            )
    ...
    async def update_test_run(self, test_run_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Updated CircleCI test run: {test_run_id} with status: {update_data.get('status')}"
            )
    ...
    async def add_pr_comment(self, pr_number: str, comment: str) -> bool:
        ...
        """
        logger.info(
            f"CircleCIClient doesn't support adding PR comments directly. Would add comment to PR #{pr_number}"
        )
        return False
    ...
    async def upload_artifact(
        ...
        try:
            ...
            logger.info(
                f"Would upload artifact {artifact_path} as {artifact_name} for {project_slug}/{job_number}"
            )
            return True
    ...
    async def get_artifact_url(self, test_run_id: str, artifact_name: str) -> Optional[str]:
        ...
        if getattr(self, "_offline", False):
            ...
            logger.warning(
                f"Offline mode: no cached artifact URL for {artifact_name} in {test_run_id}"
            )
            return None
                    ...
                    if hasattr(self, "pipeline_id") and hasattr(self, "workflow_id"):
                        ...
                        async with self.session.get(artifacts_url) as artifacts_response:
                            if artifacts_response.status == 200:
                                ...
                                # Artifact not found
                                logger.warning(
                                    f"Artifact {artifact_name} not found in CircleCI job {job_number}"
                                )
                                return None
    ...
    async def set_build_status(self, status: str, description: str) -> bool:
        ...
        logger.info(
            f"CircleCIClient doesn't support setting build status directly. Would set status to {status}: {description}"
        )
        return False
