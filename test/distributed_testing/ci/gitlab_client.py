...
class GitLabClient(CIProviderInterface):
    ...
    async def _ensure_session(self):
        ...
        if self.session is None:
            self.session = aiohttp.ClientSession(
                headers={"PRIVATE-TOKEN": self.token, "User-Agent": "DistributedTestingFramework"}
            )
            ...
            # If commit_sha is provided, we can use GitLab Commit Status API
                ...
                async with self.session.post(url, params=payload) as response:
                    if response.status == 201:
                        ...
                        logger.error(
                            f"Error creating GitLab commit status: {response.status} - {error_text}"
                        )
    ...
    async def update_test_run(self, test_run_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            status_map = {"running": "running", "completed": "success", "failed": "failed"}
            ...
            if test_run_id.startswith("gl-pipeline-"):
                ...
                logger.info(
                    f"Would update GitLab pipeline for test run {test_run_id} with status {gitlab_status}"
                )
                return True
                ...
                if len(parts) >= 3:
                    ...
                    async with self.session.post(url, params=payload) as response:
                        if response.status == 201:
                            ...
                            error_text = await response.text()
                            logger.error(
                                f"Error updating GitLab commit status: {response.status} - {error_text}"
                            )
                            return False
                else:
    ...
    async def upload_artifact(
        ...
        try:
            ...
            branch_url = (
                f"{self.api_url}/projects/{self.project}/repository/branches/{artifact_branch}"
            )
            ...
            if should_create_branch:
                ...
                branch_create_data = {"branch": artifact_branch, "ref": default_branch}

                async with self.session.post(
                    branch_create_url, json=branch_create_data
                ) as response:
                    if response.status not in (201, 200):
                        error_text = await response.text()
                        logger.error(
                            f"Failed to create artifact branch: {response.status} - {error_text}"
                        )
                        return False
            ...
            async with self.session.post(file_url, json=file_data) as response:
                if response.status in (201, 200):
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Failed to upload artifact to GitLab: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def get_artifact_url(self, test_run_id: str, artifact_name: str) -> Optional[str]:
        ...
        if self._offline:
            if (
                hasattr(self, "_artifact_urls")
                and test_run_id in self._artifact_urls
                and artifact_name in self._artifact_urls[test_run_id]
            ):
        ...
        try:
            ...
            async with self.session.get(file_url) as response:
                if response.status == 200:
                    ...
                    logger.warning(
                        f"Artifact {artifact_name} not found in repository for test run {test_run_id}"
                    )
                    return None
                ...
                if len(parts) >= 3:
                    ...
                    async with self.session.get(url) as response:
                        if response.status == 200:
                            ...
                            error_text = await response.text()
                            logger.error(
                                f"Error getting GitLab pipeline: {response.status} - {error_text}"
                            )
                ...
                if len(parts) >= 3:
                    ...
                    async with self.session.get(url) as response:
                        if response.status == 200:
                            ...
                            our_statuses = [
                                s for s in data if s.get("name", "").startswith("Distributed Test")
                            ]
                            ...
                            if our_statuses:
                                ...
                                return {
                                    ...
                                    "status": status_map.get(
                                        latest.get("status"), latest.get("status")
                                    ),
                                ...
                                logger.warning(
                                    f"No distributed testing statuses found for commit {commit_sha}"
                                )
                                return {
                                    "id": test_run_id,
                                    "status": "unknown",
                            ...
                            error_text = await response.text()
                            logger.error(
                                f"Error getting GitLab commit statuses: {response.status} - {error_text}"
                            )
        ...
        try:
            ...
            async with self.session.post(url, params=payload) as response:
                if response.status == 201:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error setting GitLab build status: {response.status} - {error_text}"
                    )
                    return False
