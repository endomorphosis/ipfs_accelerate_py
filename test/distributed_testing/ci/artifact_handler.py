...
class ArtifactStorage:
        ...
        try:
            ...
            for test_run_id, artifacts_data in data.items():
                self.artifacts_by_test_run[test_run_id] = [
                    ArtifactMetadata.from_dict(artifact_data) for artifact_data in artifacts_data
                ]
    ...
    def _save_metadata(self):
        ...
        try:
            ...
            for test_run_id, artifacts in self.artifacts_by_test_run.items():
                data[test_run_id] = [artifact.to_dict() for artifact in artifacts]
    ...
    def get_artifact_by_name(
        self, test_run_id: str, artifact_name: str
    ) -> Optional[ArtifactMetadata]:
        """
...
class ArtifactHandler:
    ...
    async def upload_artifact(
        ...
        try:
            ...
            upload_success = await provider.upload_artifact(
                test_run_id=test_run_id, artifact_path=source_path, artifact_name=artifact_name
            )

            if not upload_success:
                logger.warning(
                    f"Failed to upload artifact {artifact_name} to provider {provider_name}"
                )
            ...
            logger.info(
                f"Uploaded artifact {artifact_name} for test run {test_run_id} to provider {provider_name}"
            )
            return True, metadata
    ...
    async def upload_artifacts_batch(
        self, artifacts: List[Dict[str, Any]], provider_name: str
