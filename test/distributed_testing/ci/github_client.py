...
class GitHubClient(CIProviderInterface):
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 201:
                    ...
                    logger.error(
                        f"Error creating GitHub check run: {response.status} - {error_text}"
                    )
    ...
    async def update_test_run(self, test_run_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            status_map = {"running": "in_progress", "completed": "completed", "failed": "completed"}

            # Map our status to GitHub conclusion
            conclusion_map = {"completed": "success", "failed": "failure"}
            ...
            async with self.session.patch(url, json=payload) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error updating GitHub check run: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def _upload_text_artifact(
        ...
        try:
            ...
            payload = {
                ...
                "public": False,
                "files": {artifact_name: {"content": file_content}},
            }
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 201:
                    ...
                    if gist_url:
                        ...
                        await self._update_check_run_with_artifact(
                            test_run_id, artifact_name, gist_url
                        )
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error creating Gist for artifact: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def _upload_binary_artifact(
        self, test_run_id: str, artifact_path: str, artifact_name: str
    ) -> bool:
        """
            ...
            async with self.session.post(release_url, json=release_payload) as response:
                ...
                if response.status in (201, 200):
                    ...
                    async with self.session.post(
                        asset_url, data=content, headers=headers
                    ) as upload_response:
                        if upload_response.status == 201:
                            ...
                            if browser_download_url:
                                logger.info(
                                    f"Uploaded binary artifact {artifact_name} to {browser_download_url}"
                                )
                                ...
                                # Update the check run to include a link to the artifact
                                await self._update_check_run_with_artifact(
                                    test_run_id, artifact_name, browser_download_url
                                )
                            ...
                            logger.error(
                                f"Error uploading binary artifact: {upload_response.status} - {error_text}"
                            )
                            return False
                else:
                    error_text = await response.text()
    ...
    async def _update_check_run_with_artifact(
        self, test_run_id: str, artifact_name: str, artifact_url: str
    ) -> bool:
        """
        ...
        try:
            ...
            async with self.session.get(check_run_url) as get_response:
                if get_response.status == 200:
                    ...
                    updated_output = {"title": output_title, "summary": output_summary}

                    # Update the check run
                    update_payload = {"output": updated_output}

                    async with self.session.patch(
                        check_run_url, json=update_payload
                    ) as update_response:
                        if update_response.status == 200:
                            ...
                            logger.error(
                                f"Error updating check run with artifact link: {update_response.status} - {error_text}"
                            )
                            return False
                else:
                    error_text = await get_response.text()
                    logger.warning(
                        f"Could not get check run to update with artifact link: {get_response.status} - {error_text}"
                    )
                    return False
            ...
            async with self.session.get(check_run_url) as response:
                if response.status == 200:
                    ...
                    if match:
                        ...
                        logger.warning(
                            f"Artifact {artifact_name} not found in check run {test_run_id}"
                        )
                else:
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error getting GitHub check run: {response.status} - {error_text}"
                    )
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 201:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Error setting GitHub build status: {response.status} - {error_text}"
                    )
                    return False
