...
class TravisClient(CIProviderInterface):
    ...
    async def add_pr_comment(self, pr_number: str, comment: str) -> bool:
        ...
        """
        logger.info(
            f"TravisClient doesn't support adding PR comments directly. Would add comment to PR #{pr_number}"
        )
        return False
    ...
    async def upload_artifact(
        ...
        try:
            ...
            logger.info(
                f"Artifact upload for Travis CI not fully implemented. Would upload {artifact_path} as {artifact_name}"
            )
            return True
    ...
    async def set_build_status(self, status: str, description: str) -> bool:
        ...
        if self._offline:
            ...
            return True
        logger.info(
            f"TravisClient doesn't support setting build status directly. Would set status to {status}: {description}"
        )
        return False
        ...
        try:
            ...
            try:
                async with self.session.get(api_url) as response:
                    ...
                    logger.info(
                        f"Created simulated artifact URL for {artifact_name}: {simulated_s3_url}"
                    )
