...
def test_provider_interface_implementation():
    ...
    for provider_name, provider_class in providers:
        ...
        if not_implemented:
            logger.error(
                f"  - FAILED: {provider_name} does not implement methods: {', '.join(not_implemented)}"
            )
            success = False
        else:
...
def test_artifact_handling_standardization():
    ...
    for provider_name, provider_class in providers:
        ...
        try:
            ...
            if missing_params:
                logger.error(
                    f"  - FAILED: {provider_name} upload_artifact method is missing parameters: {', '.join(missing_params)}"
                )
                success = False
                continue
            ...
            if return_annotation != bool and return_annotation != inspect.Signature.empty:
                logger.warning(
                    f"  - WARNING: {provider_name} upload_artifact method has unexpected return type annotation: {return_annotation}"
                )
