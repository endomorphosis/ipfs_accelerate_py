...
class GitHubNotificationChannel(NotificationChannel):
    ...
    def _send_impl(self, context):
        ...
        if self.config.get("commit_status", False):
            ...
            if commit_sha:
                success = success and self._update_commit_status(
                    token, repository, commit_sha, context
                )
            else:
    ...
    def _update_commit_status(self, token, repository, commit_sha, context):
        ...
        if success:
            ...
            logger.error(
                f"Failed to update GitHub status: {response.status_code} - {response.text}"
            )
    ...
    def _add_pr_comment(self, token, repository, pr_number, context):
        ...
        if success:
            ...
            logger.error(
                f"Failed to add GitHub PR comment: {response.status_code} - {response.text}"
            )
...
def build_notification_context(args, config):
    ...
    if args.test_report:
        if github_run_id != "Unknown":
            # For GitHub Actions
            report_url = (
                f"https://github.com/{github_repository}/actions/runs/{github_run_id}/artifacts"
            )
        else:
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="CI Notification System for Hardware Monitoring Tests"
    )
    parser.add_argument(
        "--test-status",
