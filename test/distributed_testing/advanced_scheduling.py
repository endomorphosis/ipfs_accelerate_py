...
class Worker:
    ...
    def __init__(
        ...
        self.resource_utilization: Dict[ResourceType, float] = {k: 0.0 for k in capabilities.keys()}
...
class AdvancedScheduler:
    ...
    def complete_task(
        self, worker_id: WorkerId, success: bool, result: Any = None
    ) -> Optional[TaskId]:
        """
    ...
    def _handle_task_failure(self, task: Task, error: Any = None) -> None:
        ...
        if task.attempt_count < self.max_task_retries:
            ...
            logger.info(
                f"Task {task.task_id} failed, requeueing for retry "
            ...
            for worker_id in available_workers:
                ...
                if worker.can_execute_task(task):
                    ...
                    if confidence < self.prediction_confidence_threshold:
                        ...
                        expected_duration = confidence * expected_duration + (1 - confidence) * (
                            task.estimated_duration or 60.0
                        )
    ...
    def _maybe_adapt_scheduling_algorithm(self) -> None:
        ...
        if algorithm_scores:
            self.current_best_algorithm = max(algorithm_scores.items(), key=lambda x: x[1])[0]

            logger.info(
                f"Adaptive scheduler selected algorithm: {self.current_best_algorithm} "
    ...
    def get_task_queue_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "users": {user_id: count for user_id, count in self.user_task_counts.items()},
            "task_types": {task_type: count for task_type, count in self.task_type_counts.items()},
    ...
    def get_worker_stats(self) -> Dict[str, Any]:
        ...
        for worker in self.workers.values():
            ...
            for res_type, usage in worker.resource_utilization.items():
                ...
                stats["resource_utilization"][res_type]["total"] += worker.capabilities.get(
                    res_type, 0
                )
...
if __name__ == "__main__":
    ...
    # Create scheduler
    scheduler = create_advanced_scheduler(config_file=args.config, algorithm=args.algorithm)
    ...
    for task_id, worker_id in assignments:
        ...
        print(
            f"  {task_id} ({task.task_type}, priority={task.priority}) -> {worker_id} ({worker.worker_type})"
        )
