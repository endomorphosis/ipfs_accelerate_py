...
class AdaptiveLoadBalancer:
    ...
    def select_worker_for_task(
        self, task: Dict[str, Any], workers: Dict[str, Any]
    ) -> Optional[str]:
    ...
    def _init_database_table(self):
        ...
        try:
            ...
            if not db:
                logger.debug(
                    "Coordinator database not available; skipping load balancer metrics table init"
                )
                return
    ...
    async def _initialize_hardware_profiles(self):
        ...
        logger.info(
            f"Initialized hardware profiles for {len(self.hardware_profiles)} hardware types"
        )
        ...
        try:
            ...
            for migration in self.migration_history:
                try:
                    end_time = datetime.fromisoformat(
                        migration.get("end_time", "1970-01-01T00:00:00")
                    )
            ...
            if hasattr(self, "previous_workload_prediction") and self.previous_workload_prediction:
                if "previous_prediction_accuracy" in self.previous_workload_prediction:
                    prediction_accuracy = self.previous_workload_prediction[
            ...
            metrics = {
                ...
                "migrations": {
                    ...
                    "success_rate": migrations_successful / migrations_initiated
    ...
    async def update_performance_metrics(self):
        ...
        try:
            ...
            for worker_id, worker in self.coordinator.workers.items():
                ...
                running_tasks = sum(
                    1
                    for task_id, w_id in self.coordinator.running_tasks.items()
                    if w_id == worker_id
                )
                ...
                self.worker_performance_history[worker_id] = [
                    p
                    for p in self.worker_performance_history[worker_id]
    ...
    async def _log_system_utilization(self):
        ...
        if total_workers > 0:
            ...
            logger.debug(
                f"System utilization: {avg_utilization:.2%} across {total_workers} workers"
            )
    ...
    async def _update_dynamic_thresholds(self):
        ...
        try:
            ...
            avg_system_load = sum(
                record.get("avg_utilization", 0) for record in recent_loads
            ) / len(recent_loads)
            ...
            if avg_system_load > 0.75:  # High load
                ...
                )  # Lower high threshold (more aggressive balancing)
            ...
            if (
                new_high != self.utilization_threshold_high
                or new_low != self.utilization_threshold_low
            ):
                logger.info(
                    f"Adjusting thresholds: {self.utilization_threshold_low:.2f}-{self.utilization_threshold_high:.2f} -> {new_low:.2f}-{new_high:.2f}"
                )
        ...
        try:
            ...
            for worker_id, history in self.worker_performance_history.items():
                ...
                prediction["worker_predictions"][worker_id] = {
                    ...
                    "trend": {"slope": slope, "intercept": intercept, "r_squared": r_squared},
                }
            ...
            if worker_utils_predictions:
                prediction["system_prediction"]["avg_utilization"] = sum(
                    worker_utils_predictions
                ) / len(worker_utils_predictions)
                ...
                prediction["system_prediction"]["imbalance_score"] = (
                    prediction["system_prediction"]["max_utilization"]
                    - prediction["system_prediction"]["min_utilization"]
                )

                # Calculate overall confidence as average of worker confidences
                confidences = [
                    pred["confidence"] for pred in prediction["worker_predictions"].values()
                ]
            ...
            if self.previous_workload_prediction:
                try:
                    ...
                    prev_time = datetime.fromisoformat(
                        self.previous_workload_prediction.get("timestamp", "")
                    )
                    prev_window = (
                        self.previous_workload_prediction.get("prediction_window_minutes", 0) * 60
                    )
                    ...
                    if (now - prev_time).total_seconds() >= prev_window:
                        ...
                        for worker_id, pred in self.previous_workload_prediction.get(
                            "worker_predictions", {}
                        ).items():
                            if (
                                worker_id in self.worker_performance_history
                                and self.worker_performance_history[worker_id]
                            ):
                                # Get current (actual) value
                                actual = self.worker_performance_history[worker_id][-1].get(
                                    "utilization", 0
                                )
                        ...
                        if actual_values and predicted_values:
                            ...
                            accuracy = max(
                                0, 1.0 - (mae * 2)
                            )  # Scale by 2 since utilization is 0-1
            ...
            logger.info(
                f"Predicted future system load: {prediction['system_prediction']['avg_utilization']:.2%} (confidence: {prediction['confidence']:.2f})"
            )
    ...
    async def detect_load_imbalance(self, future_load_prediction=None) -> bool:
        ...
        current_imbalance_detected = (
            max_util > self.utilization_threshold_high
            and min_util < self.utilization_threshold_low
            and max_util - min_util > 0.4
        ...
        if self.enable_predictive_balancing and future_load_prediction:
            ...
            if prediction_confidence >= 0.7 and len(worker_predictions) >= 2:
                ...
                predicted_utils = {
                    worker_id: pred.get("predicted_utilization", 0)
                ...
                if predicted_utils:
                    ...
                    future_imbalance_detected = (
                        max_pred_util > self.utilization_threshold_high + 0.05
                        and min_pred_util < self.utilization_threshold_low - 0.05
                        and max_pred_util - min_pred_util > 0.45
                    )  # Require slightly larger imbalance

                    if future_imbalance_detected:
                        logger.info(
                            f"Predicted future load imbalance: Worker {max_pred_id} at {max_pred_util:.2%}, "
        ...
        if current_imbalance_detected:
            logger.info(
                f"Current load imbalance detected: Worker {max_worker_id} at {max_util:.2%}, "
    ...
    async def balance_load(self, future_load_prediction=None):
        ...
        if len(self.active_migrations) >= self.max_simultaneous_migrations:
            logger.info(
                f"Too many active migrations ({len(self.active_migrations)}), skipping load balancing"
            )
            return
        ...
        try:
            ...
            overloaded_workers = [
                (worker_id, util)
                for worker_id, util in worker_utilization.items()
                    ...
                    for task_id, task in migratable_tasks.items():
                        ...
                        if await self._can_worker_handle_task(underloaded_id, task):
                            ...
                            if self.enable_cost_benefit_analysis:
                                ...
                                cost = await self._analyze_migration_costs(
                                    task, overloaded_id, underloaded_id
                                )

                                # Calculate migration benefit
                                benefit = await self._analyze_migration_benefits(
                                    task, overloaded_id, underloaded_id, worker_utilization
                                )

                                # Skip if cost outweighs benefit
                                if cost > benefit:
                                    logger.debug(
                                        f"Migration cost ({cost:.2f}) exceeds benefit ({benefit:.2f}) for task {task_id}"
                                    )
                                    continue

                                logger.debug(
                                    f"Migration analysis: cost={cost:.2f}, benefit={benefit:.2f}, net={benefit - cost:.2f} for task {task_id}"
                                )

                            # Initiate migration
                            success = await self._migrate_task(
                                task_id, overloaded_id, underloaded_id
                            )

                            if success:
                                migrations_initiated += 1
                                logger.info(
                                    f"Initiated migration of task {task_id} from worker {overloaded_id} to {underloaded_id}"
                                )
    ...
    async def _analyze_migration_costs(self, task, source_worker_id, target_worker_id):
        ...
        if task_type in self.migration_cost_history and self.migration_cost_history[task_type]:
            ...
            historical_cost = sum(self.migration_cost_history[task_type]) / len(
                self.migration_cost_history[task_type]
            )
            # Cap at 2.0
    ...
    async def _analyze_migration_benefits(
        self, task, source_worker_id, target_worker_id, worker_utilization
    ):
        """
                ...
                if task.get("requirements", {}).get("power_efficient", False):
                    ...
                    for hw in source_hardware:
                        if hw in self.hardware_profiles:
                            source_efficiency = max(
                                source_efficiency, self.hardware_profiles[hw].energy_efficiency
                            )

                    for hw in target_hardware:
                        if hw in self.hardware_profiles:
                            target_efficiency = max(
                                target_efficiency, self.hardware_profiles[hw].energy_efficiency
                            )
    ...
    async def _migrate_task(
        self, task_id: str, source_worker_id: str, target_worker_id: str
    ) -> bool:
        """
        ...
        if (
            source_worker_id not in self.coordinator.workers
            or target_worker_id not in self.coordinator.workers
        ):
            ...
            if source_worker_id in self.coordinator.worker_connections:
                try:
                    await self.coordinator.worker_connections[source_worker_id].send_json(
                        {"type": "cancel_task", "task_id": task_id, "reason": "migration"}
                    )
                    logger.info(
                        f"Sent cancellation request for task {task_id} to worker {source_worker_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"Error sending cancellation request to worker {source_worker_id}: {str(e)}"
                    )
                    return False
    ...
    async def handle_task_cancelled_for_migration(self, task_id: str, source_worker_id: str):
        ...
        if migration["source_worker_id"] != source_worker_id:
            logger.warning(
                f"Task {task_id} cancellation received from unexpected worker {source_worker_id}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Task {task_id} cancelled on source worker, now pending reassignment to target worker"
            )
            ...
            if task_id in self.coordinator.running_tasks:
                ...
                if actual_worker_id == target_worker_id:
                    logger.info(
                        f"Task {task_id} successfully migrated to target worker {target_worker_id}"
                    )
                else:
                    logger.warning(
                        f"Task {task_id} assigned to different worker {actual_worker_id} than target {target_worker_id}"
                    )
            else:
        ...
        for task_id, migration in list(self.active_migrations.items()):
            ...
            try:
                ...
                if age > 600:  # 10 minutes
                    logger.warning(
                        f"Cleaning up stale migration for task {task_id} (age: {age:.1f}s)"
                    )
                    migration["end_time"] = now.isoformat()
    ...
    def get_load_balancer_stats(self) -> Dict[str, Any]:
        ...
        utilization_stdev = (
            (sum((u - avg_utilization) ** 2 for u in worker_utils) / len(worker_utils)) ** 0.5
            if worker_utils
            else 0
        )
