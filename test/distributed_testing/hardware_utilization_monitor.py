...
class HardwareUtilizationMonitor:
    ...
    def __init__(
        ...
        if not self.hardware_detector and db_path and DUCKDB_AVAILABLE:
            self.hardware_detector = HardwareCapabilityDetector(
                worker_id=self.worker_id, db_path=self.db_path
            )
    ...
    def start_monitoring(self):
        ...
        logger.info(
            f"Hardware utilization monitoring started with {self.monitoring_level.value} level"
        )
    ...
    def _collect_metrics(self) -> ResourceUtilization:
        ...
        if PSUTIL_AVAILABLE:
            ...
            if disk_io and self.prev_disk_io:
                ...
                metrics.disk_read_mb = (disk_io.read_bytes - self.prev_disk_io.read_bytes) / (
                    1024**2
                )
                metrics.disk_write_mb = (disk_io.write_bytes - self.prev_disk_io.write_bytes) / (
                    1024**2
                )
            ...
            try:
                ...
                for i, gpu in enumerate(gpus):
                    gpu_metrics = {
                        ...
                        "memory_percent": (gpu.memoryUsed / gpu.memoryTotal) * 100.0
    ...
    def _check_alerts(self, metrics: ResourceUtilization):
        ...
        for gpu_info in metrics.gpu_utilization:
            ...
            # Check GPU memory
            if gpu_info.get("memory_percent", 0.0) >= self.alert_thresholds.get(
                ...
                alerts.append(
                    HardwareAlert(
                        ...
                        severity="warning"
                        if gpu_info.get("memory_percent", 0.0) < 95.0
                        else "critical",
    ...
    def start_task_monitoring(self, task_id: str):
        ...
        self.task_resources[task_id] = TaskResourceUsage(task_id=task_id, start_time=datetime.now())
    ...
    def _update_task_metrics(self, metrics: ResourceUtilization):
        ...
        for gpu_info in metrics.gpu_utilization:
            task_usage.peak_gpu_percent = max(
                task_usage.peak_gpu_percent, gpu_info.get("load", 0.0)
            )
        ...
        if task_usage.metrics:
            task_usage.avg_cpu_percent = sum(m.cpu_percent for m in task_usage.metrics) / len(
                task_usage.metrics
            )
            task_usage.avg_memory_percent = sum(m.memory_percent for m in task_usage.metrics) / len(
                task_usage.metrics
            )
        ...
        try:
            ...
            """,
                [
                    task_usage.task_id,
                    self.worker_id,
                    task_usage.start_time,
                    task_usage.end_time,
                    task_usage.peak_cpu_percent,
                    task_usage.peak_memory_percent,
                    task_usage.peak_gpu_percent,
                    task_usage.avg_cpu_percent,
                    task_usage.avg_memory_percent,
                    task_usage.avg_gpu_percent,
                    task_usage.total_disk_read_mb,
                    task_usage.total_disk_write_mb,
                    task_usage.total_net_sent_mb,
                    task_usage.total_net_recv_mb,
                    task_usage.completed,
                    task_usage.success,
                    task_usage.error,
                ],
            )
    ...
    def get_resource_utilization_from_db(
        self,
        start_time: Optional[datetime] = None,
        end_time: Optional[datetime] = None,
        task_id: Optional[str] = None,
    ...
    def export_metrics_to_json(self, file_path: str):
        ...
        try:
            ...
            data = {
                ...
                "current_metrics": self._serialize_metrics(self.current_metrics)
        ...
        if self.historical_metrics:
            ...
            if gpu_values:
                report["summary"]["gpu"] = {
                    ...
                    "current": self.current_metrics.gpu_utilization[0].get("load", 0.0)
                    if self.current_metrics and self.current_metrics.gpu_utilization
                    else None,
                }

            # Network stats
            net_sent_values = [
                m.net_sent_mb for m in self.historical_metrics if m.net_sent_mb is not None
            ]
            net_recv_values = [
                m.net_recv_mb for m in self.historical_metrics if m.net_recv_mb is not None
            ]
...
def main():
    ...
    parser.add_argument(
        "--level",
        default="standard",
        choices=["basic", "standard", "detailed", "intensive"],
        ...
        for i in range(args.duration):
            ...
            if current:
                ...
                for gpu in current.gpu_utilization:
                    print(
                        f"GPU {gpu.get('id', 0)}: {gpu.get('load', 0.0):.1f}%, Memory: {gpu.get('memory_percent', 0.0):.1f}%"
                    )
