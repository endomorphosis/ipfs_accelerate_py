...
class HardwareSchedulingVisualizer:
    ...
    def visualize_hardware_efficiency(
        ...
        colors = [
            class_colors.get(hardware_classes.get(hw_id, "unknown"), "gray")
        ...
        details = (
            f"Workload Profile: {workload_profile.workload_id}\n"
        ...
        legend_elements = [
            plt.Rectangle((0, 0), 1, 1, color=color, label=hw_class)
    ...
    def visualize_workload_distribution(
        self,
        worker_assignments: Dict[str, List[str]],
        worker_types: Dict[str, str],
        filename: str = "workload_distribution",
    ) -> str:
        """
        ...
        colors = [
            type_colors.get(worker_types.get(worker_id, "generic"), "gray")
        ...
        legend_elements = [
            plt.Rectangle((0, 0), 1, 1, color=color, label=worker_type)
    ...
    def visualize_thermal_states(
        self, thermal_states: Dict[str, Dict[str, float]], filename: str = "thermal_states"
    ) -> str:
        """
        ...
        warming_states = [
            1 if state.get("warming_state", False) else 0 for state in thermal_states.values()
        ]
        cooling_states = [
            1 if state.get("cooling_state", False) else 0 for state in thermal_states.values()
        ]
    ...
    def visualize_resource_utilization(
        self, worker_loads: Dict[str, Dict[str, float]], filename: str = "resource_utilization"
    ) -> str:
        """
    ...
    def visualize_execution_times(
        self, execution_data: Dict[str, Dict[str, Any]], filename: str = "execution_times"
    ) -> str:
        """
        ...
        legend_elements = [
            plt.Line2D(
                [0], [0], marker="o", color="w", markerfacecolor=color, markersize=8, label=wl_type
            )
        ...
        for i, (est, act, wl_id) in enumerate(zip(estimated_times, actual_times, workload_ids)):
            if (
                abs(est - act) / max(est, 1.0) > 0.3
            ):  # If actual time differs by more than 30% from estimate
    ...
    def visualize_history(
        self, history_data: Dict[str, Any], filename_prefix: str = "history"
    ) -> List[str]:
        """
        ...
        if "efficiency_scores" in history_data and history_data["efficiency_scores"]:
            ...
            eff_path = self._visualize_efficiency_history(
                history_data["efficiency_scores"], eff_filename
            )
        ...
        if "thermal_states" in history_data and history_data["thermal_states"]:
            ...
            thermal_path = self._visualize_thermal_history(
                history_data["thermal_states"], thermal_filename
            )
        ...
        if "resource_utilization" in history_data and history_data["resource_utilization"]:
            ...
            resource_path = self._visualize_resource_history(
                history_data["resource_utilization"], resource_filename
            )
            output_paths.append(resource_path)

        return output_paths

    def _visualize_efficiency_history(
        self, efficiency_history: Dict[str, List[Tuple[datetime, float]]], filename: str
    ) -> str:
        """Visualize efficiency scores over time."""
        plt.figure(figsize=(12, 6))
    ...
    def _visualize_thermal_history(
        self, thermal_history: Dict[str, List[Tuple[datetime, Dict[str, Any]]]], filename: str
    ) -> str:
        """Visualize thermal states over time."""
        plt.figure(figsize=(12, 6))
        ...
        for worker_id, data_points in thermal_history.items():
            ...
            warming_events = [
                (ts, 0.05) for ts, state in data_points if state.get("warming_state", False)
            ]
            cooling_events = [
                (ts, 0.1) for ts, state in data_points if state.get("cooling_state", False)
            ]

            if warming_events:
                w_timestamps, w_markers = zip(*warming_events)
                plt.scatter(
                    w_timestamps, w_markers, marker="^", color="red", label=f"{worker_id} Warming"
                )

            if cooling_events:
                c_timestamps, c_markers = zip(*cooling_events)
                plt.scatter(
                    c_timestamps, c_markers, marker="v", color="blue", label=f"{worker_id} Cooling"
                )
    ...
    def _visualize_resource_history(
        self, resource_history: Dict[str, List[Tuple[datetime, Dict[str, float]]]], filename: str
    ) -> str:
        """Visualize resource utilization over time."""
        ...
        for i, resource_type in enumerate(resource_types):
            ...
            for worker_id, data_points in resource_history.items():
                ...
                plt.plot(
                    timestamps, utilization, label=worker_id, marker="o", markersize=3, alpha=0.7
                )
    ...
    def record_resource_utilization(
        self, worker_id: str, utilization: Dict[str, float], timestamp: datetime = None
    ) -> None:
        """
    ...
    def generate_summary_report(
        self, filename: str = "scheduling_summary_report.html", include_visualizations: bool = True
    ) -> str:
        """
        ...
        workers_with_assignments = set(
            assignment["worker_id"] for assignment in self.history["assignments"]
        )
        workload_types = set(
            assignment["workload_type"] for assignment in self.history["assignments"]
        )
        ...
        avg_time_accuracy = (
            sum(execution_time_accuracy.values()) / len(execution_time_accuracy)
            if execution_time_accuracy
            else 0.0
        )
        ...
        for worker_id, avg_eff in sorted(
            worker_avg_efficiency.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for wl_type, avg_eff in sorted(
            workload_type_avg_efficiency.items(), key=lambda x: x[1], reverse=True
        ):
            eff_class = "good" if avg_eff >= 0.7 else "warning" if avg_eff >= 0.4 else "poor"
            html_content += f"""
                <tr>
        ...
        if include_visualizations and MATPLOTLIB_AVAILABLE:
            ...
            if self.history["resource_utilization"]:
                ...
                resource_viz_path = self.visualize_resource_utilization(
                    worker_loads, "report_resource_utilization"
                )
            ...
            if self.history["thermal_states"]:
                ...
                thermal_viz_path = self.visualize_thermal_states(
                    thermal_states, "report_thermal_states"
                )
            ...
            if self.history["execution_times"]:
                ...
                execution_viz_path = self.visualize_execution_times(
                    execution_data, "report_execution_times"
                )
