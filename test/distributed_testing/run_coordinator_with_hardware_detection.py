...
async def run_coordinator_demo(args):
    ...
    # Create visualizer
    visualizer = HardwareSchedulingVisualizer(output_dir=args.output_dir, file_format="html")
...
async def run_simulated_tasks(args):
    ...
    tasks = [
        ...
        },
        {
            "task_id": "task-3",
            "type": "test",
            "requirements": {"hardware": {"required_types": ["cpu"], "min_memory_gb": 16.0}},
        },
        {
            "task_id": "task-4",
            "type": "benchmark",
            "requirements": {"hardware": {"required_types": ["webgpu"]}},
        },
        {
            "task_id": "task-5",
            "type": "test",
            "requirements": {"hardware": {"required_types": ["tpu", "cpu"], "min_memory_gb": 32.0}},
        },
    ]
        ...
        for task in tasks:
            ...
            if is_compatible and "precision_types" in hardware_requirements:
                ...
                for hw in worker_caps.hardware_capabilities:
                    ...
                    if all(
                        req_type in supported_precisions for req_type in required_precision_types
                    ):
    ...
    worker_types = {"worker-1": "cpu", "worker-2": "gpu", "worker-3": "cpu", "worker-4": "browser"}
...
""")
        ...
        for worker in workers:
            ...
            hw_caps = worker["hardware_capabilities"]
            hw_components = ", ".join(
                [h["hardware_type"] for h in hw_caps.get("hardware_capabilities", [])]
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Coordinator with Hardware Capability Detection"
    )
    ...
    parser.add_argument(
        "--browser-executable-path", help="Path to browser executable for browser-based detection"
    )
    parser.add_argument(
        "--output-dir", default="./visualizations", help="Directory to save visualizations"
    )
    parser.add_argument(
        "--store-capabilities", action="store_true", help="Store capabilities in database"
    )
    parser.add_argument("--run-coordinator", action="store_true", help="Run the coordinator server")
    parser.add_argument(
        "--run-simulation", action="store_true", help="Run a simulation of task assignment"
    )
