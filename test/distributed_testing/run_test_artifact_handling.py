...
async def create_test_artifacts(temp_dir: str):
    ...
    artifacts["log"] = {"path": log_path, "name": "test.log", "type": "log"}
    ...
    artifacts["report"] = {"path": report_path, "name": "test_report.json", "type": "report"}
...
"""
    ...
    artifacts["coverage"] = {"path": coverage_path, "name": "coverage.html", "type": "coverage"}
...
async def run_demo(provider_name: str, config: Dict[str, Any]):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            # Create a test run
            test_run_data = {"name": f"Artifact Handling Demo - {datetime.now().isoformat()}"}
...
async def main():
    ...
    parser.add_argument(
        "--provider",
        choices=["github", "gitlab", "jenkins", "azure"],
        default="github",
