    ...
    async def invoke_hook(self, hook_type: HookType, *args, **kwargs) -> List[Any]:
        ...
        for callback in self.hooks[hook_type]:
            try:
                ...
                logger.error(
                    f"Error invoking hook {hook_type.value} in plugin {self.name}: {str(e)}"
                )
    ...
    async def unload_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            for hook_type in HookType:
                self.hooks[hook_type] = [
                    (pid, cb) for pid, cb in self.hooks[hook_type] if pid != plugin_id
                ]
    ...
    async def invoke_hook(self, hook_type: HookType, *args, **kwargs) -> List[Any]:
        ...
        for plugin_id, callback in self.hooks[hook_type]:
            ...
            try:
                ...
                logger.error(
                    f"Error invoking hook {hook_type.value} in plugin {plugin.name}: {str(e)}"
                )
