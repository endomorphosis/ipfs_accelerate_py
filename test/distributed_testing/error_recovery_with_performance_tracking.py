...
class PerformanceBasedErrorRecovery:
            ...
            if current_version < 1:
                ...
                if result:
                    ...
                    try:
                        ...
                        # Rename new table
                        self.db_connection.execute(
                            "ALTER TABLE recovery_performance_new RENAME TO recovery_performance"
                        )

                        logger.info(
                            "Successfully migrated recovery_performance table to use AUTOINCREMENT"
                        )
                    except Exception as e:
    ...
    def _load_performance_history(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {sum(len(records) for records in self.performance_history.values())} performance records"
            )
        except Exception as e:
    ...
    async def recover(self, error_report: Any) -> Tuple[bool, Dict[str, Any]]:
        ...
        strategy, strategy_id = await self._select_best_strategy(
            error_type, recovery_level, error_dict
        )
        ...
        try:
            ...
            impact_score = self._calculate_impact_score(
                resource_diff, execution_time, recovery_info["affected_tasks"], recovered_tasks
            )
            ...
            if success:
                ...
                if (
                    ...
                    logger.info(
                        f"Progressive recovery: escalating from level {recovery_level} to {next_level}"
                    )
            ...
            self._record_performance(
                ...
                context={"error_id": error_id, "recovery_level": recovery_level, "timeout": True},
            )
            ...
            if next_level <= ProgressiveRecoveryLevel.LEVEL_5.value and next_level > recovery_level:
                logger.info(
                    f"Progressive recovery after timeout: escalating from level {recovery_level} to {next_level}"
                )
                return await self.recover(error_report)
            ...
            if next_level <= ProgressiveRecoveryLevel.LEVEL_5.value and next_level > recovery_level:
                logger.info(
                    f"Progressive recovery after exception: escalating from level {recovery_level} to {next_level}"
                )
                return await self.recover(error_report)
    ...
    async def _select_best_strategy(
        ...
        for strategy_id, strategy in candidates.items():
            ...
            if recovery_level == ProgressiveRecoveryLevel.LEVEL_1.value and level_value == "low":
                ...
                recovery_level == ProgressiveRecoveryLevel.LEVEL_2.value and level_value == "medium"
            ):
                level_match_bonus = 0.2
            elif (
                recovery_level == ProgressiveRecoveryLevel.LEVEL_3.value and level_value == "medium"
            ):
                level_match_bonus = 0.2
        ...
        logger.info(
            f"Selected strategy {strategy_id} with score {score:.2f} for error type {error_type}"
        )
    ...
    def _map_error_type_to_category(self, error_type: str) -> str:
        ...
        for category in ErrorCategory:
            if (
                category.name.lower() == error_type.lower()
                or category.value.lower() == error_type.lower()
            ):
    ...
    def _update_adaptive_timeout(
        ...
        logger.debug(
            f"Updated adaptive timeout for {key}: {current_value:.1f}s -> {new_timeout:.1f}s"
        )
    ...
    def _calculate_impact_score(
        ...
        impact_score = (
            memory_impact + cpu_impact + time_impact + task_count_impact + recovery_impact
        )
    ...
    async def _check_stability(self) -> float:
        ...
        if hasattr(self.coordinator, "running_tasks") and hasattr(
    ...
    async def _get_affected_tasks(self, error_report: Any) -> Set[str]:
        ...
        if not affected_tasks:
            ...
            if "worker" in component.lower() and hasattr(self.coordinator, "running_tasks"):
                ...
                worker_id = None
                if hasattr(error_report, "context") and hasattr(
                    error_report.context, "related_entities"
                ):
    ...
    def _update_strategy_scores(self, strategy_id: str, error_type: str):
        ...
        records = [
            r for r in self.performance_history.get(strategy_id, []) if r.error_type == error_type
        ]
        ...
        if successful_records:
            avg_execution_time = sum(r.execution_time for r in successful_records) / len(
                successful_records
            )
        else:
        ...
        logger.debug(
            f"Updated strategy score for {strategy_id} with error type {error_type}: {overall_score:.2f}"
        )
    ...
    def _record_progression(
        ...
        if self.db_connection:
            try:
                ...
                VALUES (?, ?, ?, ?)
                """,
                    (error_id, new_level, datetime.now(), history_json),
                )
            except Exception as e:
                logger.error(f"Error recording progressive recovery: {str(e)}")

    def get_performance_metrics(
        self, error_type: Optional[str] = None, strategy_id: Optional[str] = None, days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        if self.db_connection:
            try:
                ...
                total_success_rate = (
                    sum(s["success_rate"] * s["total_executions"] for s in strategies.values())
                    / total_executions
                ...
                for error_type in set(
                    et for s in strategies.values() for et in s["error_types"].keys()
                ):
                    best_sid = None
                    best_score = -1.0
                ...
                return {
                    ...
                    "overall": {
                        ...
                        "error_type_count": len(
                            set(et for s in strategies.values() for et in s["error_types"].keys())
                        ),
                    },
                }
            except Exception as e:
        ...
        for strategy_id, records in self.performance_history.items():
            ...
            strategies[strategy_id] = {
                ...
                "overall_score": self.strategy_scores.get(error_type, {}).get(strategy_id, 0.0)
                if error_type
                else 0.0,
            }
            ...
            strategies.append(
                {
                    ...
                    "samples": sum(
                        1
                        for r in self.performance_history.get(strategy_id, [])
    ...
    async def run_tests():
        ...
        class MockRecoveryManager:
            def __init__(self):
                ...
                self.strategies = {
                    ...
                    "coordinator_restart": MockRecoveryStrategy(
                        "coordinator_restart", "high", 0.6, 2.0
                    ),
                    "system_recovery": MockRecoveryStrategy(
                        "system_recovery", "critical", 0.9, 3.0
                    ),
                }
            ...
            for error_type in error_types:
                ...
                error_report = error_handler.create_error_report(
                    Exception(f"Test {error_type} error"),
                    {
                        ...
                        "operation": "test",
                        "related_entities": {"task_id": "task-1", "worker_id": "worker-1"},
                    },
                )
        ...
        if args.get_metrics:
            ...
            for error_type, strategy in metrics["top_strategies"].items():
                print(
                    f"  {error_type}: {strategy['strategy_name']} (score: {strategy['score']:.2f})"
                )
