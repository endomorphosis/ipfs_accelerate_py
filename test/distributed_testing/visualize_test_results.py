...
def parse_args():
    ...
    parser.add_argument(
        "--input-dir", type=str, required=True, help="Directory containing test result artifacts"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="Directory to save visualization outputs"
    )
    parser.add_argument(
        "--format", type=str, default="png", help="Output image format (png, pdf, svg)"
    )
    parser.add_argument(
        "--style", type=str, default="seaborn-v0_8-whitegrid", help="Matplotlib style to use"
    )
...
def create_performance_summary(results, output_dir, fmt="png"):
    ...
    df = pd.DataFrame(
        {"test_name": test_names, "execution_time": exec_times, "status": test_statuses}
    )
    ...
    bars = plt.barh(plot_df["test_name"], plot_df["mean_time"], color=cm(plot_df["success_rate"]))
    ...
    plt.errorbar(
        ...
        plot_df["test_name"],
        xerr=[
            (plot_df["mean_time"] - plot_df["min_time"]),
        ...
        if "component_interactions" in result:
            for interaction in result["component_interactions"]:
                ...
                if key in interactions:
                    ...
                    interactions[key] = {"count": count, "success_count": (count if success else 0)}
...
def create_scaling_analysis(results, output_dir, fmt="png"):
    ...
    for idx, row in df.iterrows():
        if abs(row["worker_delta"]) > 2:  # Only label significant changes
            ax1.annotate(
                row["reason"][:20] + "..." if len(row["reason"]) > 20 else row["reason"],
...
def create_test_results_summary(results, output_dir, fmt="png"):
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"test_type": test_types, "status": test_results, "execution_time": execution_times}
    )
    ...
    result_summary = df.groupby("test_type").agg(
        {"passed": ["mean", "count"], "execution_time": ["mean", "sum"]}
    )
    ...
    ax1.bar(
        result_summary["test_type"],
        result_summary["pass_rate"] * 100,
        color=[
            SUCCESS_COLOR if rate > 0.8 else WARNING_COLOR if rate > 0.5 else FAILURE_COLOR
    ...
    for i, (idx, row) in enumerate(result_summary.iterrows()):
        ax1.text(i, row["pass_rate"] * 50, f"n={int(row['count'])}", ha="center", va="center")
    ...
    ax2.bar(result_summary["test_type"], result_summary["avg_time"], color=INFO_COLOR, alpha=0.7)

    # Add total time as text
    for i, (idx, row) in enumerate(result_summary.iterrows()):
        ax2.text(
            i, row["avg_time"] / 2, f"total: {row['total_time']:.1f}s", ha="center", va="center"
        )
...
def main():
    ...
    visualizations["performance_summary"] = create_performance_summary(
        results, args.output_dir, args.format
    )
    visualizations["component_interaction"] = create_component_interaction_graph(
        results, args.output_dir, args.format
    )
    visualizations["resource_usage"] = create_resource_usage_visualization(
        results, args.output_dir, args.format
    )
    visualizations["scaling_analysis"] = create_scaling_analysis(
        results, args.output_dir, args.format
    )
    visualizations["test_results"] = create_test_results_summary(
        results, args.output_dir, args.format
    )
