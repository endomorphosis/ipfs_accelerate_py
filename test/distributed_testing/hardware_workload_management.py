    ...
    def get_efficiency_score(self, hardware: HardwareCapabilityProfile) -> float:
        ...
        if hardware.hardware_class == HardwareClass.GPU:
            if self.workload_type == WorkloadType.VISION:
                ...
                self.workload_type == WorkloadType.NLP
                and AcceleratorFeature.TENSOR_CORES in hardware.features
            ):
...
class HardwareWorkloadManager:
    ...
    def __init__(self, hardware_taxonomy: HardwareTaxonomy, db_path: Optional[str] = None):
        ...
        ] = {}  # workload_type -> list of execution records
        ...
        self.hardware_optimizations: Dict[
            str, Dict[str, Any]
        ] = {}  # hardware_class -> optimization settings

        # Workload-specific device preferences
        self.workload_device_preferences: Dict[
            str, Dict[str, float]
        ] = {}  # workload_type -> hardware_id -> preference score
    ...
    def register_workload(self, workload_profile: WorkloadProfile) -> str:
        ...
        with self.lock:
            ...
            heapq.heappush(
                self.workload_queue, (workload_profile.priority, workload_profile.workload_id)
            )

            logger.info(
                f"Registered workload {workload_profile.workload_id} of type {workload_profile.workload_type.value}"
            )
            ...
            if workload_id in self.execution_plans:
                ...
                if status == "executing":
                    ...
                    for hardware_id, _ in plan.hardware_assignments:
                        if (
                            hardware_id in self.active_executions
                            and workload_id in self.active_executions[hardware_id]
                        ):
    ...
    ) -> List[Tuple[str, HardwareCapabilityProfile, float]]:
        """
            ...
            for profile in profiles:
                ...
                if profile.is_compatible_with(
                    ...
                    for (
                        worker_id,
                        hw_profiles,
                    ) in self.hardware_taxonomy.worker_hardware_map.items():
                        for hw_profile in hw_profiles:
                            if (
                                hw_profile.hardware_class == profile.hardware_class
                                and hw_profile.architecture == profile.architecture
                                and hw_profile.vendor == profile.vendor
                                and hw_profile.model_name == profile.model_name
                            ):
    ...
    def predict_execution_time(
        self, workload_profile: WorkloadProfile, hardware_profile: HardwareCapabilityProfile
    ) -> float:
        """
        ...
        if workload_type in self.performance_history:
            ...
            for execution in self.performance_history[workload_type]:
                ...
                if (
                    execution["hardware_class"] == hardware_profile.hardware_class.value
                    and execution["hardware_architecture"] == hardware_profile.architecture.value
                ):
    ...
    def predict_energy_usage(
        ...
        tdp_watts = (
            hardware_profile.thermal_design_power_w or 100.0
        )  # Default to 100W if not specified
    ...
    def _create_single_device_execution_plan(
        ...
        estimated_energy = self.predict_energy_usage(
            workload_profile, hardware_profile, estimated_time
        )
    ...
    def _create_sharded_execution_plan(
        ...
        avg_efficiency = sum(efficiency for _, _, efficiency in selected_hardware) / len(
            selected_hardware
        )
        ...
        slowest_hardware = next(
            hw_profile for _, hw_profile, eff in selected_hardware if eff == min_efficiency
        )
        ...
        for _, hw_profile, _ in selected_hardware:
            device_energy = self.predict_energy_usage(workload_profile, hw_profile, estimated_time)
        ...
        with self.lock:
            ...
            while self.workload_queue:
                ...
                if plan:
                    ...
                    logger.info(
                        f"Scheduled workload {workload_id} to {len(plan.hardware_assignments)} devices, "
                    ...
                    logger.warning(
                        f"No suitable hardware found for workload {workload_id}, requeued with priority {priority + 1}"
                    )
    ...
    def _update_thermal_tracking(self) -> None:
        ...
        for hardware_id, state in self.device_state_cache.items():
            ...
            if abs(thermal_state["temperature"] - current_temp) > 0.1:
                logger.debug(
                    f"Hardware {hardware_id} temperature: {thermal_state['temperature']:.2f}, "
                    f"load: {load:.2f}"
                )
        ...
        for workload_id, plan in self.execution_plans.items():
            if plan.execution_status == "executing" and plan.started_at:
                ...
                if elapsed > 2.0 * plan.estimated_execution_time:
                    logger.warning(
                        f"Workload {workload_id} has been running for {elapsed:.1f}s, "
    ...
    def _record_execution_performance(self, workload_id: str, plan: WorkloadExecutionPlan) -> None:
        ...
        logger.info(
            f"Recorded performance for workload {workload_id}: "
            f"actual={record['duration_seconds']:.2f}s vs. "
            f"estimated={plan.estimated_execution_time:.2f}s"
        )

    def _trigger_event(
        self, event_type: str, workload_id: str, plan: WorkloadExecutionPlan
    ) -> None:
...
class WorkloadExecutionGraph:
    ...
    def get_critical_path_remaining(self) -> List[str]:
        ...
        incomplete = {
            ...
            if self.status_map.get(id, SubtaskStatus(subtask)).status
...
class MultiDeviceOrchestrator:
    ...
    def get_next_critical_subtasks(
        self, workload_id: str, limit: int = 5
    ...
    def start_subtask_execution(self, workload_id: str, subtask_id: str) -> bool:
        ...
        with self.lock:
            ...
            # Update subtask status
            execution_graph.update_subtask_status(
                subtask_id=subtask_id, status="executing", start_time=datetime.now()
            )
    ...
    def complete_subtask(self, workload_id: str, subtask_id: str, result: Any) -> bool:
        ...
        with self.lock:
            ...
            # Update subtask status
            execution_graph.update_subtask_status(
                subtask_id=subtask_id, status="completed", result=result, end_time=datetime.now()
            )
            ...
            if execution_graph.is_completed():
                ...
                logger.info(
                    f"Completed workload {workload_id} with {len(execution_graph.subtasks)} subtasks"
                )
    ...
    def fail_subtask(self, workload_id: str, subtask_id: str, error: str) -> bool:
        ...
        with self.lock:
            ...
            recovery_strategy = subtask.custom_config.get(
            ...
            if recovery_strategy in self.recovery_strategies:
                recovery_successful = self.recovery_strategies[recovery_strategy](
                    workload_id, subtask_id, error
                )
                logger.info(
                    f"Applied recovery strategy '{recovery_strategy}' for failed subtask {subtask_id}, success: {recovery_successful}"
                )
                return recovery_successful
            else:
                logger.warning(
                    f"Unknown recovery strategy '{recovery_strategy}' for failed subtask {subtask_id}"
                )
                return False
    ...
    def _record_workload_completion(
        self, workload_id: str, execution_graph: WorkloadExecutionGraph
    ) -> None:
        """Record workload completion for performance history."""
        # Calculate overall statistics
        total_subtasks = len(execution_graph.subtasks)
        completion_time = (
            execution_graph.completed_at - execution_graph.created_at
        ).total_seconds()
        ...
        self.workload_history[workload_id].append(
            {
                ...
                "subtask_count_by_status": {
                    status: sum(
                        1 for s in execution_graph.status_map.values() if s.status == status
                    )
    ...
    def _recovery_strategy_retry(self, workload_id: str, subtask_id: str, error: str) -> bool:
        ...
        if status.retry_count >= subtask.retry_count:
            logger.warning(
                f"Retry limit exceeded for subtask {subtask_id} in workload {workload_id}"
            )
            return False
        ...
        logger.info(
            f"Retrying subtask {subtask_id} in workload {workload_id} (attempt {status.retry_count})"
        )
        return True
    ...
    def _recovery_strategy_reassign(self, workload_id: str, subtask_id: str, error: str) -> bool:
        ...
        logger.info(
            f"Reassigning subtask {subtask_id} in workload {workload_id} to different hardware"
        )
        return True
    ...
    def _recovery_strategy_skip(self, workload_id: str, subtask_id: str, error: str) -> bool:
        ...
        execution_graph.update_subtask_status(
            subtask_id=subtask_id, status="skipped", error=error, end_time=datetime.now()
        )
        ...
        for other_id, other_subtask in execution_graph.subtasks.items():
            if subtask_id in other_subtask.dependencies:
                logger.warning(
                    f"Skipping subtask {subtask_id} affects dependent subtask {other_id}"
                )
    ...
    def _recovery_strategy_alternate(self, workload_id: str, subtask_id: str, error: str) -> bool:
        ...
        logger.info(
            f"Using alternate implementation for subtask {subtask_id} in workload {workload_id}"
        )
        return True
        ...
        for subtask_id, status in execution_graph.status_map.items():
            if status.status == "executing" and status.start_time:
                ...
                if elapsed > subtask.timeout_seconds:
                    logger.warning(
                        f"Subtask {subtask_id} in workload {workload_id} timed out after {elapsed:.1f}s"
                    )
            ...
            if status.assigned_hardware:
                for hw_id, _ in status.assigned_hardware:
                    ...
                    if utilization and utilization.get("cpu_utilization", 0) > 90:
                        logger.warning(
                            f"High CPU utilization ({utilization['cpu_utilization']:.1f}%) for subtask {subtask_id} on hardware {hw_id}"
                        )
...
class FailureDetectionSystem:
    ...
    def record_failure(
        self, workload_id: str, subtask_id: str, error: str, hardware_id: Optional[str] = None
    ) -> None:
                ...
                for hw_id, count in hardware_counts.items():
                    if count >= 3:
                        ...
                        if pattern_key not in self.failure_patterns:
                            ...
                            logger.warning(
                                f"Detected failure pattern: {error_type} occurring on hardware {hw_id} ({count} times)"
                            )
                        else:
                            self.failure_patterns[pattern_key]["occurrences"] += 1
                            self.failure_patterns[pattern_key]["last_seen"] = max(
                                f["timestamp"] for f in failures if f.get("hardware_id") == hw_id
                            )
...
class PerformanceTracker:
    ...
    def calculate_execution_rate(self, workload_id: str) -> Optional[float]:
        ...
        with self.lock:
            if (
                workload_id not in self.progress_history
                or len(self.progress_history[workload_id]) < 2
            ):
                return None
