...
class AutoRecoveryManager:
    ...
    def __init__(
        ...
        self.task_recovery_history: Dict[
            str, List[Dict[str, Any]]
        ] = {}  # task_id -> recovery history
    ...
    async def _load_recovery_data_from_db(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded recovery data from database: {len(self.recovery_attempts)} workers, {len(self.task_recovery_attempts)} tasks"
            )
    ...
    async def _check_unhealthy_workers(self):
        ...
        for worker_id, metrics in self.health_monitor.worker_health_metrics.items():
            ...
            if (
                ...
                or heartbeat_age > self.health_monitor.heartbeat_timeout
            ):
                ...
                if attempts >= self.max_recovery_attempts:
                    logger.warning(
                        f"Worker {worker_id} has failed {attempts} times, marking as permanent failure"
                    )
    ...
    async def _schedule_worker_recovery(self, worker_id: str):
        ...
        logger.info(
            f"Scheduling recovery for worker {worker_id} (attempt {attempts}/{self.max_recovery_attempts})"
        )
    ...
    async def _mark_worker_permanent_failure(self, worker_id: str):
        ...
        logger.error(
            f"Marking worker {worker_id} as permanently failed after {self.recovery_attempts.get(worker_id, 0)} recovery attempts"
        )
    ...
    async def _check_stalled_tasks(self):
        ...
        for task_id, worker_id in list(self.coordinator.running_tasks.items()):
            ...
            if running_time > timeout:
                logger.warning(
                    f"Task {task_id} has been running for {running_time:.1f} seconds (timeout: {timeout}s), may be stalled"
                )

                # Check if worker is healthy
                worker_healthy = True
                if worker_id in self.health_monitor.worker_health_metrics:
                    worker_status = self.health_monitor.worker_health_metrics[worker_id].get(
                        "status", "unknown"
                    )
                ...
                if not worker_healthy:
                    logger.warning(
                        f"Worker {worker_id} appears unhealthy, recovering task {task_id}"
                    )
    ...
    async def _handle_system_critical_state(self):
                    ...
                    json.dumps(
                        {
                            ...
                            "active_workers_percentage": self.health_monitor.get_worker_health_summary().get(
                                "active_percentage", 0
                            ),
        ...
        try:
            ...
            """,
                (True, datetime.now().isoformat(), requeued_tasks),
            )
        except Exception as e:
