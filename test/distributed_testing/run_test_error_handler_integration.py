...
def create_test_dependency_graph(dependency_manager):
    ...
    dependency_manager.register_test(
        "test_3", [Dependency("test_1"), Dependency("test_2")], ["group_b"]
    )
    dependency_manager.register_test("test_4", [Dependency("group_a", is_group=True)], ["group_b"])
    dependency_manager.register_test(
        "test_5", [Dependency("test_3"), Dependency("test_4", dependency_type=DependencyType.SOFT)]
    )
    dependency_manager.register_test("test_6", [Dependency("test_5")])
    dependency_manager.register_test(
        "test_7",
        [Dependency("test_1"), Dependency("test_6", dependency_type=DependencyType.OPTIONAL)],
    )
...
def setup_error_hooks(error_handler):
    ...
    error_handler.register_error_hook(
        [ErrorType.CONFIGURATION, ErrorType.VALIDATION],
...
def setup_retry_policies(error_handler):
    ...
    error_handler.default_retry_policy = RetryPolicy(
        max_retries=3, initial_delay_ms=1000, backoff_factor=2.0, jitter=True
    )

    # Dependency manager-specific policy
    error_handler.set_retry_policy(
        "dependency_manager",
        RetryPolicy(
            max_retries=2,
...
def add_execution_hooks(orchestrator, error_handler):
    ...
    def pre_execution_hook(max_workers, strategy, total_tests):
        logger.info(
            f"Starting test execution with {max_workers} workers, "
    ...
    def post_execution_hook(metrics):
        logger.info(
            f"Execution completed: {metrics['completed_tests']} successful, "
...
async def run_integration_demo():
    ...
    error_handler.create_error_report(
        ConnectionError("Connection failed"), {"component": "network"}
    )
    ...
    result, error = await error_handler.retry_operation(
        flaky_func, context={"component": "demo", "operation": "flaky_operation"}
    )
    ...
    if unresolved:
        ...
        resolved = error_handler.resolve_error(
            error.error_id, "Manual resolution", {"notes": "Resolved during demo"}
        )
        if resolved:
