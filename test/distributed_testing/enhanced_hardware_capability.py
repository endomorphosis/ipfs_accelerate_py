...
class HardwareCapabilityDetector:
        ...
        try:
            ...
            capability = HardwareCapability(
                ...
                capabilities={
                    ...
                    "frequency_mhz": _safe_float(cpu_info.get("hz_advertised_raw", [0]), 0.0)
                    / 1_000_000.0,
                    "l1_cache_kb": _safe_size_to_kb(cpu_info.get("l1_data_cache_size", None), 0.0)
                    if "l1_data_cache_size" in cpu_info
                    else None,
                    "l2_cache_kb": _safe_size_to_kb(cpu_info.get("l2_cache_size", None), 0.0)
                    if "l2_cache_size" in cpu_info
                    else None,
                    "l3_cache_kb": _safe_size_to_kb(cpu_info.get("l3_cache_size", None), 0.0)
                    ...
                    "sse4": "sse4_1" in cpu_info.get("flags", [])
                    or "sse4_2" in cpu_info.get("flags", []),
                    ...
                    capability.capabilities.update(
                        {
                            ...
                            "neural_engine": "Apple"
                            in cpu_info.get("brand_raw", ""),  # Assumption for Apple Silicon
                        }
                    )
            ...
            score_values = [
                score.value for score in scores.values() if score != CapabilityScore.UNKNOWN
            ]
            if score_values:
    ...
    def _calculate_cpu_precision_score(self, capability: HardwareCapability) -> CapabilityScore:
        ...
        if PrecisionType.FP64 in precisions and PrecisionType.BF16 in precisions:
            return CapabilityScore.EXCELLENT
        elif (
            PrecisionType.FP64 in precisions
            and PrecisionType.FP32 in precisions
            and any(p in precisions for p in [PrecisionType.FP16, PrecisionType.INT8])
        ):
            return CapabilityScore.GOOD
                ...
                for i in range(device_count):
                    try:
                        ...
                        score_values = [
                            ...
                            if score != CapabilityScore.UNKNOWN
                        ]
                        if score_values:
                            avg_score = sum(score_values) / len(score_values)
                            overall_score = CapabilityScore.UNKNOWN
                            for score in CapabilityScore:
                                if (
                                    score != CapabilityScore.UNKNOWN
                                    and abs(score.value - avg_score) <= 0.5
                                ):
                                    overall_score = score
                                    break
                            scores["overall"] = overall_score
                        ...
                        msg = str(e)
                        if (
                            msg.strip().lower() == "not supported"
                            or "nvmLError_notSupported".lower() in e.__class__.__name__.lower()
                        ):
    ...
    def _calculate_nvidia_gpu_compute_score(
    ...
    def _detect_amd_gpus(self) -> List[HardwareCapability]:
        ...
        try:
            ...
            result = subprocess.run(
                ["which", "rocm-smi"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    ...
    def _detect_apple_gpus(self) -> List[HardwareCapability]:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    f"Error running system_profiler: {result.stderr.decode('utf-8').strip()}"
                )
                return []
            ...
            try:
                ...
                for display_info in displays_info:
                    if (
                        "spdisplays_metal" not in display_info
                        and "spdisplays_mtlgpufamilysupport" not in display_info
                    ):
                        continue
                    ...
                    is_apple_silicon = "Apple" in gpu_model and not (
                        "Intel" in gpu_model or "AMD" in gpu_model
                    )
    ...
    def _calculate_gpu_precision_score(self, capability: HardwareCapability) -> CapabilityScore:
        ...
        if (
            PrecisionType.FP32 in precisions
            and PrecisionType.FP16 in precisions
            and PrecisionType.INT8 in precisions
            and any(p in precisions for p in [PrecisionType.INT4, PrecisionType.INT2])
        ):
            return CapabilityScore.EXCELLENT
        elif (
            PrecisionType.FP32 in precisions
            ...
            return CapabilityScore.GOOD
        elif PrecisionType.FP32 in precisions and any(
            p in precisions for p in [PrecisionType.FP16, PrecisionType.INT8]
        ):
            return CapabilityScore.AVERAGE
        elif PrecisionType.FP32 in precisions:
            return CapabilityScore.BASIC
        ...
        try:
            ...
            try:
                ...
                qnn_capability = HardwareCapability(
                    ...
                    capabilities={"sdk_version": qnn_version, "backend": "QNN"},
                )
                ...
                try:
                    ...
                    snpe_capability = HardwareCapability(
                        ...
                        capabilities={"sdk_version": snpe_version, "backend": "SNPE"},
                    )
        ...
        try:
            ...
            try:
                ...
                one_capability = HardwareCapability(
                    ...
                    capabilities={"sdk_version": one_version, "backend": "ONE"},
                )
        ...
        try:
            ...
            try:
                ...
                nnapie_capability = HardwareCapability(
                    ...
                    capabilities={"sdk_version": nnapie_version, "backend": "NNAPIE"},
                )
    ...
    def _detect_apple_neural_engine(self) -> List[HardwareCapability]:
        ...
        try:
            ...
            result = subprocess.run(
                ["sysctl", "hw.optional.arm64"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            try:
                ...
                ane_capability = HardwareCapability(
                    ...
                    capabilities={"sdk_version": ct.__version__, "backend": "CoreML"},
                )
    ...
    def detect_all_capabilities(self) -> WorkerHardwareCapabilities:
        ...
        logger.info(
            f"Detected {len(all_capabilities)} hardware capabilities for worker {self.worker_id}"
        )
...
class HardwareCapabilityComparator:
    ...
    def compare_capabilities(
        self, capability1: HardwareCapability, capability2: HardwareCapability
    ) -> Dict[str, Any]:
        """
        ...
        precision_comparison = {
            "first_only": [
                p.name
                for p in capability1.supported_precisions
                if p not in capability2.supported_precisions
            ],
            "second_only": [
                p.name
                for p in capability2.supported_precisions
    ...
    def is_compatible(
        self, capability: HardwareCapability, requirements: Dict[str, Any]
    ...
    def estimate_performance(
        ...
        # Base estimates
        estimates = {"relative_score": 1.0, "confidence": 0.5, "recommendation_level": "unknown"}
    ...
    def find_best_hardware(
        self,
        capabilities: List[HardwareCapability],
        workload_type: str,
        workload_params: Dict[str, Any] = None,
    ) -> Tuple[HardwareCapability, Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    for idx, capability in enumerate(worker_capabilities.hardware_capabilities):
        ...
        print(
            f"  Memory: {capability.memory_gb:.2f} GB"
            if capability.memory_gb
            else "  Memory: Unknown"
        )
        print(
            f"  Supported Precisions: {', '.join(p.name for p in capability.supported_precisions)}"
        )
    ...
    if worker_capabilities.hardware_capabilities:
        ...
        estimate = comparator.estimate_performance(
            capability, "inference", {"precision": PrecisionType.FP32, "batch_size": 4}
        )
