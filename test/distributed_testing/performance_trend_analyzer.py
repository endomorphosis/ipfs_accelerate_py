...
if not logging.getLogger().handlers:
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"trend_analyzer_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            ),
        ],
    )
...
class PerformanceTrendAnalyzer:
    ...
    def __init__(
        ...
        self.config = {
            ...
            "anomaly_detection": {
                ...
                "moving_average": {"enabled": True, "window_size": 5, "threshold_factor": 2.0},
            },
            "trend_analysis": {
            ...
            "metrics_grouping": ["model_name", "worker_id"],
        }
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with self._request(
                "get", f"{self.coordinator_url}/status", headers=headers
            ) as response:
                ...
                if status == 200:
                    ...
                    logger.info(
                        f"Connected to coordinator. Status: {status_data.get('status', 'unknown')}"
                    )
                    return True
                else:
                ...
                for metric_name, metrics in self.metrics_cache.items():
                    if len(metrics) >= self.config["trend_analysis"]["minimum_data_points"]:
                        ...
                        for group_key, group_metrics in grouped_metrics.items():
                            ...
                            if (
                                len(group_metrics)
                                < self.config["trend_analysis"]["minimum_data_points"]
                            ):
                                continue
                            ...
                            if trend:
                                ...
                                logger.info(
                                    f"Detected {trend.trend_type} trend for {metric_name} in {group_key}: {trend.description}"
                                )
    ...
    async def _collect_metrics(self) -> None:
        ...
        try:
            ...
            # Get task results
            async with self._request(
                "get", f"{self.coordinator_url}/task_results", headers=headers
            ) as response:
                status = getattr(response, "status", None)
                if status is None:
                    status = getattr(response, "status_code", None)
                ...
                if status == 200:
                    ...
                    logger.debug(
                        f"Collected {len(metrics)} metrics from {len(task_results)} task results"
                    )
                else:
            ...
            # Get system metrics
            async with self._request(
                "get", f"{self.coordinator_url}/system_metrics", headers=headers
            ) as response:
                status = getattr(response, "status", None)
                if status is None:
                    status = getattr(response, "status_code", None)
    ...
    def _save_metrics_to_db(self, metrics: List[PerformanceMetric]) -> None:
        ...
        try:
            ...
            for metric in metrics:
                hardware_info_json = (
                    json.dumps(metric.hardware_info) if metric.hardware_info else None
                )
                execution_context_json = (
                    json.dumps(metric.execution_context) if metric.execution_context else None
                )
    ...
    def _detect_anomalies(self, metrics: List[PerformanceMetric]) -> List[PerformanceMetric]:
        ...
        if self.config["anomaly_detection"]["moving_average"]["enabled"] and len(values) >= 5:
            ...
            threshold_factor = self.config["anomaly_detection"]["moving_average"][
                "threshold_factor"
            ]

            # Calculate moving average
            moving_avg = np.convolve(
                values.flatten(), np.ones(window_size) / window_size, mode="valid"
            )
    ...
    def _generate_alerts(
        self, anomalies: List[PerformanceMetric], all_metrics: List[PerformanceMetric]
        ...
        for anomaly in anomalies:
            ...
            deviation_percent = (
                ((anomaly.value - baseline) / baseline) * 100 if baseline != 0 else 0.0
            )
            ...
            improvement_direction = self.config["trend_analysis"]["trend_classification"][
                "improvement_direction"
            ].get(anomaly.name, "decreasing")
    ...
    def _analyze_trend(self, metrics: List[PerformanceMetric]) -> Optional[PerformanceTrend]:
        ...
        relative_change = (
            abs(end_value - start_value) / abs(start_value) if start_value != 0 else 0.0
        )
        ...
        if relative_change <= stable_threshold:
            ...
            improvement_direction = self.config["trend_analysis"]["trend_classification"][
                "improvement_direction"
            ].get(sorted_metrics[0].name, "decreasing")

            if (improvement_direction == "decreasing" and slope < 0) or (
                improvement_direction == "increasing" and slope > 0
            ):
            ...
            if plt is None:
                ...
                for metric_name, metrics in self.metrics_cache.items():
                    ...
                    for group_key, group_metrics in grouped_metrics.items():
                        ...
                        timestamps = [
                            datetime.datetime.fromtimestamp(m.timestamp) for m in sorted_metrics
                        ]
                    ...
                    chart_filename = os.path.join(
                        self.output_dir, f"{metric_name.replace(' ', '_')}_{int(time.time())}.png"
                    )
            ...
            with open(report_filename, "w") as f:
                ...
                if self.latest_trends:
                    for trend_key, trend in self.latest_trends.items():
                        ...
                        f.write(
                            f"Trend: {trend.trend_type.upper()} (confidence: {trend.confidence:.2f})\n"
                        )
                        f.write(f"Description: {trend.description}\n")
                        f.write(
                            f"Period: {datetime.datetime.fromtimestamp(trend.start_timestamp).strftime('%Y-%m-%d %H:%M:%S')} to {datetime.datetime.fromtimestamp(trend.end_timestamp).strftime('%Y-%m-%d %H:%M:%S')}\n"
                        )
                ...
                if self.latest_alerts:
                    ...
                    for alert in sorted_alerts:
                        ...
                        f.write(
                            f"Value: {alert.value:.2f} (Expected range: {alert.expected_range[0]:.2f} - {alert.expected_range[1]:.2f})\n"
                        )
                        f.write(f"Deviation: {alert.deviation_percent:.1f}%\n")
                        f.write(f"Description: {alert.description}\n")
                        f.write(
                            f"Time: {datetime.datetime.fromtimestamp(alert.timestamp).strftime('%Y-%m-%d %H:%M:%S')}\n"
                        )
                        f.write(
                            f"Model: {alert.model_name or 'N/A'}, Worker: {alert.worker_id or 'N/A'}\n"
                        )
                        f.write("\n")
    ...
    async def get_performance_trends(
        self, metric_name: Optional[str] = None, model_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
                ...
                trends.append(
                    {
                        ...
                        "trend_type": row[3],
                        "confidence": row[4],
                        "start_timestamp": row[5].timestamp()
                        if hasattr(row[5], "timestamp")
                        else row[5],
                        "end_timestamp": row[6].timestamp()
    ...
    async def get_metrics_summary(
        self, metric_name: Optional[str] = None, group_by: Optional[str] = None
    ) -> Dict[str, Any]:
        """
