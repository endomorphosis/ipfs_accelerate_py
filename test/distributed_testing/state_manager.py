...
class StateManager:
    ...
    async def update_state(self, key: str, value: Any) -> bool:
        ...
        self.logger.info(
            f"State {key} {'created' if is_new else 'updated'} to version {self.state_versions[key]}"
        )
        return True
    ...
    async def get_all_state_with_metadata(self) -> Dict[str, Dict[str, Any]]:
        ...
        for key in self.state:
            ...
            result[key] = {"value": self.state[key], "metadata": metadata}
    ...
    async def _persist_state(self) -> bool:
        ...
        try:
            ...
            snapshot = {
                ...
                "timestamp": time.time(),
                "state": self.state,
                "metadata": {"versions": self.state_versions, "timestamps": self.state_timestamps},
            }
    ...
    async def load_state_from_disk(self) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"No state file found at {self.persistence_path}/{self.state_id}_state.json"
            )
            return False
        except Exception as e:
