...
class DistributedStateManager:
    ...
    def __init__(
        ...
        self.state_dir = state_dir or os.path.join(
            os.path.dirname(coordinator.db_path) if coordinator.db_path else ".", "state"
        )
            ...
            try:
                if os.path.exists(file_path):
                    with open(file_path, "r") as f:
                        ...
                        logger.info(
                            f"Loaded partition {partition_name} from disk: {len(partition.data)} items, version {partition.version}"
                        )
            except Exception as e:
    ...
    def _save_state_to_disk(self, partition_name: str = None):
        ...
        for name in partitions:
            ...
            try:
                ...
                logger.debug(
                    f"Saved partition {name} to disk: {len(partition.data)} items, version {partition.version}"
                )
            except Exception as e:
        ...
        while self.running:
            try:
                ...
                if current_time - self.last_sync_time >= self.sync_interval or (
                    ...
                    and current_time - self.last_sync_time >= self.max_sync_delay
                ):
    ...
    async def _synchronize_state(self):
        ...
        partition_names = sorted(
            self.partitions.keys(), key=lambda name: self.partitions[name].priority, reverse=True
        )
    ...
    async def _sync_partition(self, partition_name: str):
        ...
        if checksums and all(
            checksum == partition.checksum
            for node_id, checksum in checksums.items()
        ...
        for node in self.cluster_nodes:
            ...
            try:
                async with self.session.get(
                    f"{node}/state/checksum/{partition_name}", timeout=2
                ) as response:
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    if applied > 0:
                        logger.info(
                            f"Applied {applied} transactions from {node_url} for partition {partition_name}"
                        )
            ...
            async with self.session.get(
                f"{node_url}/state/partition/{partition_name}", timeout=5
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    ...
                    # Compare versions
                    if version > partition.version or (
                        version == partition.version and last_modified > partition.last_modified
                    ):
                        ...
                        # Save partition to disk
                        self._save_state_to_disk(partition_name)
                    elif version < partition.version or (
                        version == partition.version and last_modified < partition.last_modified
                    ):
    ...
    async def _push_partition(self, node_url: str, partition_name: str):
        ...
        try:
            ...
            async with self.session.post(
                f"{node_url}/state/update", json=data, timeout=5
            ) as response:
                if response.status == 200:
                    logger.info(f"Successfully pushed partition {partition_name} to {node_url}")
                else:
                    logger.warning(
                        f"Failed to push partition {partition_name} to {node_url}: {response.status}"
                    )
        except Exception as e:
    ...
    async def handle_checksum_request(self, partition_name: str):
        ...
        partition = self.partitions.get(partition_name)
        if not partition:
            return {"error": f"Partition {partition_name} not found", "node_id": self.node_id}
