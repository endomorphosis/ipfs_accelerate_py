...
class MockCoordinator:
    ...
    async def start_recovery(self, component_id, error):
        ...
        if self.plugin_manager:
            await self.plugin_manager.invoke_hook(HookType.RECOVERY_STARTED, component_id, error)
    ...
    async def complete_recovery(self, component_id, result):
        ...
        if self.plugin_manager:
            await self.plugin_manager.invoke_hook(HookType.RECOVERY_COMPLETED, component_id, result)
    ...
    def get_plugin_status(self, plugin_type=None):
        ...
        for plugin_id, plugin in plugins.items():
            ...
            if hasattr(plugin, "get_resource_pool_status") and callable(
                getattr(plugin, "get_resource_pool_status")
            ):
...
async def main():
    ...
    parser.add_argument(
        "--resource-pool-test", action="store_true", help="Test resource pool features"
    )
    parser.add_argument(
        "--simulate-recovery", action="store_true", help="Simulate recovery scenario"
    )
