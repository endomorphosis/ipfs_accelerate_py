...
class EnhancedReporterDemo:
    ...
    async def initialize(self):
        ...
        if self.args.provider:
            try:
                ...
                self.ci_provider = await CIProviderFactory.create_provider(
                    self.args.provider, config
                )
        ...
        self.reporter = TestResultReporter(
            ci_provider=self.ci_provider, report_dir=self.report_dir, artifact_dir=self.artifact_dir
        )
    ...
    async def create_test_artifacts(self):
        ...
        artifacts_to_create = [
            ...
            {
                "name": "test.log",
                "content": "2025-03-16 10:15:32 INFO: Starting tests\n"
    ...
    async def demonstrate_bulk_url_retrieval(self):
        ...
        for file_path in self.artifact_files:
            ...
            success = await self.ci_provider.upload_artifact(
                test_run_id=self.test_run_id, artifact_path=file_path, artifact_name=artifact_name
            )
        ...
        parallel_urls = await self.reporter.get_artifact_urls(
            test_run_id=self.test_run_id, artifact_names=artifact_names
        )
    ...
    async def demonstrate_automatic_url_retrieval(self):
        ...
        if not self.ci_provider:
            print(
                f"{YELLOW}Skipping automatic URL retrieval demo (no CI provider available){RESET}"
            )
            return False
        ...
        if self.artifacts:
            print(
                f"{GREEN}Successfully collected and uploaded {len(self.artifacts)} artifacts{RESET}"
            )
            print("\nArtifacts with URLs:")
            ...
            for artifact in self.artifacts:
                ...
                if URL_VALIDATOR_AVAILABLE:
                    if "url_validated" in artifact and artifact["url_validated"]:
                        ...
                        validation_status = (
                            f"{GREEN}Valid{RESET}" if is_valid else f"{RED}Invalid{RESET}"
                        )
    ...
    async def demonstrate_enhanced_reports(self):
        ...
        if report_files:
            ...
            if "artifacts" in test_result.metadata:
                report_artifacts = [
                    a for a in test_result.metadata["artifacts"] if a.get("type") == "report"
                ]
                if report_artifacts:
    ...
    async def demonstrate_pr_comments(self):
        ...
        await self.reporter.report_test_result(test_result, formats=["markdown"])
    ...
    async def demonstrate_error_handling(self):
        ...
        if urls:
            ...
            if len(urls) == 2 and all(
                name in urls for name in ["non_existent_1.json", "non_existent_2.log"]
            ):
        ...
        try:
            ...
            if dashboard_items:
                ...
                for item in dashboard_items:
                    ...
                    print(
                        f"  Task: {item.get('name')} - Status: {status_color}{item.get('status')}{RESET}"
                    )
