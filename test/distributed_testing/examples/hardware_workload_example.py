    ...
    return {
        ...
        "subtasks": {
            "tokenize": {
                ...
                "min_memory_gb": 1.0,
                "min_compute_units": 2,
                "metrics": {"compute_intensity": 0.3, "memory_intensity": 0.3},
                "priority": 1,
                "dependencies": [],
            ...
            "encode": {
                ...
                "min_memory_gb": 4.0,
                "min_compute_units": 8,
                "metrics": {"compute_intensity": 0.7, "memory_intensity": 0.5},
                "priority": 2,
            ...
            "generate": {
                ...
                "min_memory_gb": 8.0,
                "min_compute_units": 16,
                "metrics": {"compute_intensity": 0.8, "memory_intensity": 0.7},
                "priority": 1,
                "dependencies": ["encode"],
                "preferred_hardware_class": "GPU",
            },
            "postprocess": {
                ...
                "min_memory_gb": 1.0,
                "min_compute_units": 2,
                "metrics": {"compute_intensity": 0.2, "memory_intensity": 0.2},
                "priority": 3,
...
def workload_callback(workload_id: str, plan: WorkloadExecutionPlan) -> None:
    ...
    if plan.execution_status == "completed":
        ...
        if duration:
            logger.info(
                f"Workload {workload_id} completed in {duration:.2f}s "
...
def simulate_workload_execution(workload_manager: HardwareWorkloadManager) -> None:
    ...
    for workload_id, plan in workload_manager.execution_plans.items():
        if plan.execution_status == "planned":
            ...
            logger.info(
                f"Simulating execution of workload {workload_id} for {simulated_duration:.2f}s"
            )
...
def print_execution_summary(
    workload_manager: HardwareWorkloadManager, workload_names: Dict[str, str]
) -> None:
...
def run_multi_device_example(workload_manager: HardwareWorkloadManager) -> None:
    ...
    fault_tolerance_config = {
        ...
        "checkpointing": {"enabled": True, "interval_seconds": 5},
        "alternative_implementations": {"generate": "generate_fallback"},
    }
    ...
    orchestrator.configure(
        {
            ...
            "performance_tracking": {"enabled": True, "history_size": 50},
            "resource_monitoring": {
                "enabled": True,
            ...
            "critical_path_analysis": {"enabled": True, "path_priority_boost": 2},
        }
    )
    ...
    for workload_id_full, plan in workload_manager.execution_plans.items():
        if plan.execution_status == "completed" and "_" in workload_id_full:
            ...
            if len(parts) >= 3:
                ...
                logger.info(
                    f"Recorded result for subtask {subtask_id} of workload {main_workload_id}"
                )
    ...
    while not orchestrator.is_workload_completed(workload_id) and iteration < max_iterations:
        ...
        logger.info(
            f"Next batch of ready subtasks (iteration {iteration}): {[st['subtask_id'] for st in ready_subtasks]}"
        )
        ...
        for subtask in ready_subtasks:
            ...
            full_subtask_id = workload_manager.register_workload(workload_profile)
            logger.info(
                f"Registered subtask {subtask_id} with workload manager as {full_subtask_id}"
            )

            # For the last subtask, demonstrate checkpoint/resume functionality
            if subtask_id == "postprocess":
                logger.info(
                    "Demonstrating checkpoint/resume functionality for 'postprocess' subtask"
                )
        ...
        for workload_id_full, plan in workload_manager.execution_plans.items():
            if plan.execution_status == "completed" and "_" in workload_id_full:
                ...
                if len(parts) >= 3:
                    ...
                    orchestrator.record_subtask_result(main_workload_id, subtask_id, result)
                    logger.info(
                        f"Recorded result for subtask {subtask_id} with performance metrics"
                    )
    ...
    if orchestrator.is_workload_completed(workload_id):
        ...
        logger.info(
            "Multi-device workload completed successfully with enhanced monitoring and fault tolerance"
        )
    else:
        logger.warning(
            f"Multi-device workload not completed, remaining subtasks: "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Hardware-Aware Workload Management Example"
    )
    parser.add_argument(
        "--multi-device", action="store_true", help="Run multi-device orchestration example"
    )
    parser.add_argument(
        "--fault-tolerance", action="store_true", help="Enable fault tolerance features"
    )
    parser.add_argument(
        "--thermal-tracking", action="store_true", help="Enable thermal state tracking"
    )
    parser.add_argument(
        "--resource-monitoring", action="store_true", help="Enable resource monitoring"
    )
    parser.add_argument(
        "--performance-tracking", action="store_true", help="Enable performance tracking"
    )
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Visualize execution graphs and timelines"
    )
    parser.add_argument(
        "--simulate-failures", action="store_true", help="Simulate hardware failures"
    )
    ...
    if args.fault_tolerance or args.all_features:
        ...
        workload_manager.register_event_callback(
            "workload_retry", lambda wid, data: logger.info(f"Retrying workload {wid}")
        )
        workload_manager.register_event_callback(
            "workload_reassigned", lambda wid, data: logger.info(f"Reassigned workload {wid}")
        )
        workload_manager.register_event_callback(
    ...
    for config in workload_configs:
        ...
        if (
            args.thermal_tracking
            or args.all_features
            and config.get("preferred_hardware_class") == "GPU"
        ):
    ...
    if args.all_features:
        logger.info(
            "Enhanced Hardware-Aware Workload Management example completed successfully with all features enabled"
        )
    else:
