...
try:
    ...
    logger.error(
        "AdaptiveCircuitBreaker not available. Run this example from the distributed_testing directory."
    )
...
async def advanced_example():
    ...
    for cycle in range(pattern_cycles):
        for pattern in patterns:
            ...
            logger.info(
                f"  Half-open timeout: {state.get('current_half_open_timeout', 'N/A'):.2f}s"
            )
    ...
    if circuit_breaker.threshold_model is not None:
        ...
        logger.info(
            f"  Recovery model created: {circuit_breaker.recovery_timeout_model is not None}"
        )
        logger.info(
            f"  Half-open model created: {circuit_breaker.half_open_timeout_model is not None}"
        )
    ...
    if circuit_breaker.prediction_model is not None:
        ...
        for i in range(10):
            try:
                result = await circuit_breaker.execute(lambda: operation_with_pattern(pattern, i))
                logger.info(f"  Operation {i + 1} succeeded")
            except Exception as e:
                error_msg = str(e)
                if "preemptively opened" in error_msg:
                    logger.info(
                        f"  Operation {i + 1} detected early warning! Circuit opened preemptively."
                    )
        ...
        if opened_preemptively:
            ...
            logger.info(
                "Circuit was not opened preemptively (try running the example longer or adjusting patterns)"
            )
...
async def hardware_specific_example():
    ...
    for cb, operation, base_success_rate, name in circuit_breakers:
        ...
        for i, success_rate in enumerate(success_rates):
            ...
            logger.info(f"    Failure threshold: {state.get('current_failure_threshold', 'N/A')}")
            logger.info(
                f"    Recovery timeout: {state.get('current_recovery_timeout', 'N/A'):.2f}s"
            )
...
async def browser_testing_example():
    ...
    if not BROWSER_TESTING_AVAILABLE:
        ...
        for test_type in test_types:
            ...
            success_rates = {"load": 0.8, "render": 0.7, "compute": 0.6, "interactive": 0.75}
        ...
        browser_config = BrowserConfiguration(
            browser_name="chrome", platform="webgpu", headless=True
        )
        ...
        failure_injector = BrowserFailureInjector(browser_bridge, circuit_breaker=circuit_breaker)
            ...
            for j in range(5):
                try:
                    ...
                    await failure_injector.inject_failure(
                        failure_type, intensity=random.choice(["mild", "moderate", "severe"])
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"  Test {j * 2 + 2} failed as expected after failure injection: {str(e)}"
                        )
