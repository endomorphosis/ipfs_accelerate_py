...
async def run_example(gitlab_token=None, project_id=None, commit_sha=None, mr_iid=None):
    ...
    if not project_id and gitlab_token:
        logger.warning(
            "GitLab project ID not provided. Running in demo mode with simulated responses."
        )
    ...
    try:
        ...
        reporter = TestResultReporter(
            ci_provider=gitlab_client, report_dir=str(report_dir), artifact_dir=str(artifact_dir)
        )
...
async def main():
    ...
    if gitlab_token and project_id:
        ...
        logger.info(
            "Set GITLAB_TOKEN and CI_PROJECT_ID environment variables for actual GitLab integration"
        )
