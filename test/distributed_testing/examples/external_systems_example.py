...
async def test_jira_connector(config):
    ...
    try:
        ...
        logger.info(
            f"Retrieved issue details: {issue_details.get('key')} - {issue_details.get('fields', {}).get('summary')}"
        )
        ...
        # Update the issue
        update_success = await jira.update_item(
            "issue",
            issue.get("key"),
    ...
    try:
        message_data = {
            ...
            "blocks": [
                ...
                {
                    "type": "context",
                    "elements": [{"type": "mrkdwn", "text": "🕒 " + str(time.time())}],
                },
            ],
        }
...
async def test_testrail_connector(config):
    ...
    try:
        ...
        test_results = await testrail.execute_operation(
            "get_test_results", {"run_id": test_run.get("id")}
        )
        ...
        # Close the test run
        if config.get("cleanup", True):
            close_result = await testrail.execute_operation(
                "close_test_run", {"run_id": test_run.get("id")}
            )
...
async def test_prometheus_connector(config):
    ...
    try:
        ...
        query_data = {"query": "process_resident_memory_bytes"}
        ...
        logger.info(
            f"Range query results: {len(range_metrics.get('result_data', {}).get('result', []))} data points"
        )
        ...
        if config.get("pushgateway_url"):
            ...
            if config.get("cleanup", True):
                delete_result = await prometheus.delete_item(
                    "metrics", "distributed_testing_example"
                )
...
async def main():
    ...
    parser.add_argument(
        "--connector",
        choices=["jira", "slack", "testrail", "prometheus", "all"],
        default="all",
        help="Connector to test (default: all)",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Don't clean up test data after tests"
    )
