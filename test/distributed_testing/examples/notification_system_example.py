...
class MockCoordinator:
    ...
    async def initialize(self):
        ...
        for plugin_id, plugin in loaded_plugins.items():
            logger.info(
                f"Loaded plugin: {plugin.name} v{plugin.version} ({plugin.plugin_type.value})"
            )
    ...
    async def generate_test_notifications(self):
        ...
        for worker_id in self.worker_ids:
            capabilities = {"cpu": 8, "memory": 16384, "disk": 512, "gpu": True, "os": "Linux"}
            await self.plugin_manager.invoke_hook(
                HookType.WORKER_REGISTERED, worker_id, capabilities
            )
        ...
        for i in range(5):
            ...
            task_data = {
                ...
                "priority": i % 3,
                "parameters": {"param1": f"value{i + 1}", "param2": i * 10},
            }
...
async def main():
    ...
    if args.discord:
        ...
        if "DISCORD_WEBHOOK_URL" not in os.environ and "DISCORD_BOT_TOKEN" not in os.environ:
            logger.error(
                "Either DISCORD_WEBHOOK_URL or DISCORD_BOT_TOKEN environment variable is required for Discord notifications"
            )
            return
    ...
    if args.telegram:
        ...
        if "TELEGRAM_BOT_TOKEN" not in os.environ:
            logger.error(
                "TELEGRAM_BOT_TOKEN environment variable is required for Telegram notifications"
            )
            return
    ...
    if args.email:
        ...
        if missing_vars:
            logger.error(
                f"The following environment variables are required for Email notifications: {', '.join(missing_vars)}"
            )
            return
    ...
    if args.msteams:
        ...
        if "MSTEAMS_WEBHOOK_URL" not in os.environ and not (
            "MSTEAMS_USE_GRAPH_API" in os.environ
            and os.environ["MSTEAMS_USE_GRAPH_API"].lower() == "true"
            and "MSTEAMS_TENANT_ID" in os.environ
            and "MSTEAMS_CLIENT_ID" in os.environ
            and "MSTEAMS_CLIENT_SECRET" in os.environ
        ):
            logger.error(
                "Either MSTEAMS_WEBHOOK_URL or Graph API credentials (MSTEAMS_USE_GRAPH_API, MSTEAMS_TENANT_ID, MSTEAMS_CLIENT_ID, MSTEAMS_CLIENT_SECRET) are required for MS Teams notifications"
            )
            return
    ...
    if args.jira:
        ...
        if missing_vars:
            logger.error(
                f"The following environment variables are required for JIRA notifications: {', '.join(missing_vars)}"
            )
            return
