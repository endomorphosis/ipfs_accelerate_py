...
def detect_hardware_capabilities():
    ...
    try:
        ...
        if hasattr(torch, "hip") and torch.hip.is_available():
            ...
            capabilities["rocm_info"] = {"device_count": torch.hip.device_count()}
    except (ImportError, AttributeError):
    ...
    try:
        ...
        capabilities["openvino_info"] = {"version": openvino.__version__}
    except ImportError:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for i in range(num_workers):
                ...
                if i % 2 == 0:
                    ...
                    if "cuda" not in worker_capabilities["hardware"]:
                        ...
                        worker_capabilities["cuda_info"] = {
                            "device_count": 1,
                            "devices": [
                                {"index": 0, "name": "NVIDIA RTX 4090", "memory_total": 24}
                            ],
                        }
                ...
                logger.info(
                    f"Worker {worker_id} connected with capabilities: {worker_capabilities['hardware']}"
                )
            ...
            test_result.metadata = {
                ...
                "environment": {
                    ...
                    "Total CPU Cores": sum(
                        w.capabilities.get("cpu_info", {}).get("physical_cores", 0) for w in workers
                    ),
                    "Total GPU Count": sum(
                        w.capabilities.get("cuda_info", {}).get("device_count", 0) for w in workers
                    ),
                },
            }
            ...
            for model in model_types:
                ...
                for i in range(model["count"]):
                    ...
                    logger.info(
                        f"Submitted {model['name']} task {i + 1} with ID {task_data['task_id']}"
                    )
            ...
            for model in model_types:
                model_name = model["name"]
                worker_discovery_metrics["task_assignment_metrics"]["expected_distribution"][
                    model_name
                ] = []
                for i, worker in enumerate(workers):
                    ...
                    if any(
                        ...
                        for m in worker.capabilities.get("models", [])
                    ):
                        # Worker supports this model, it's a candidate for tasks
                        worker_discovery_metrics["task_assignment_metrics"][
                            "expected_distribution"
                        ][model_name].append(
            ...
            for worker in workers:
                worker_tasks = [
                    task
                    for task in task_history["tasks"]
                ...
                worker_performance["worker_performance"].append(
                    {
                        ...
                        / max(1, len(completed_tasks)),
                        "task_details": {"by_model": {}, "by_hardware": {}},
                    }
                )
                ...
                for model_name, data in model_counts.items():
                    worker_performance["worker_performance"][-1]["task_details"]["by_model"][
                        model_name
                    ] = {
                        "count": data["count"],
                ...
                for hardware_type, data in hardware_counts.items():
                    worker_performance["worker_performance"][-1]["task_details"]["by_hardware"][
                        hardware_type
                    ] = {
                        "count": data["count"],
                        "average_duration": data["duration"] / data["count"],
            ...
            test_result.metadata["performance_metrics"] = {
                ...
                "average_latency_ms": round(
                    sum(task.get("duration_seconds", 0) * 1000 for task in task_history["tasks"])
                    / len(task_history["tasks"]),
                    2,
                ),
                ...
                "worker_utilization_percent": round(
                    ...
                    * sum(
                        len(
                            [
                                t
                                for t in task_history["tasks"]
                                if t.get("assigned_worker") == w.worker_id
                            ]
                        )
                        for w in workers
                    )
                    / (max(1, len(workers) * total_task_count)),
                    2,
                ),
            ...
            artifacts = await reporter.collect_and_upload_artifacts(test_run_id, artifact_patterns)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Worker Auto-Discovery with CI Integration Example"
    )
    parser.add_argument(
        "--ci-provider", default="local", help="CI provider type (github, gitlab, jenkins, etc.)"
    )
    parser.add_argument("--config", help="Path to JSON configuration file for the CI provider")
    parser.add_argument(
        "--workers", type=int, default=2, help="Number of simulated workers to create"
    )
