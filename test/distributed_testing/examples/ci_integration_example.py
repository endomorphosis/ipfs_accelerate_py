...
def run_tests(args):
    ...
    try:
        result = subprocess.run(
            cmd, env=env, cwd=parent_dir, check=False, capture_output=True, text=True
        )
...
def generate_badge(args):
    ...
    try:
        ...
        status_json = {
            ...
            "runs": {"passing": passing_runs, "total": total_runs},
        }
...
def parse_arguments():
    ...
    parser.add_argument(
        "--test-mode",
        choices=["standard", "basic", "full", "long"],
        default="standard",
        help="Test mode",
    )
    parser.add_argument("--notification", action="store_true", help="Enable test notifications")
    parser.add_argument("--generate-badge", action="store_true", help="Generate status badge")
    parser.add_argument("--ci-integration", action="store_true", help="Run CI integration tests")
    parser.add_argument(
        "--output-dir", default="./example_output", help="Output directory for reports and badges"
    )
    parser.add_argument("--db-path", help="Path to test database")
    parser.add_argument("--all-channels", action="store_true", help="Use all notification channels")
