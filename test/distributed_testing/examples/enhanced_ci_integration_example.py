...
async def main():
    ...
    try:
        ...
        ci_plugin = None
        for plugin_id, plugin in coordinator.plugin_manager.get_plugins_by_type(
        ...
        success = await ci_client.upload_artifact(
            test_run["id"], str(html_report_path), "Test Report"
        )
        ...
        for run_num in range(2):
            ...
            for i in range(5):
                ...
                await coordinator.plugin_manager.invoke_hook(
                    ...
                    {
                        "status": "completed",
                        "result": f"Result for run {run_num + 2}, task {i + 1}",
                    },
                )
                ...
                await ci_client.record_performance_metric(
                    ...
                    metric_value=100
                    + (i % 3) * 50
                    - run_num * 10,  # Uses less memory in newer runs
                    unit="MB",
                )
                ...
                await ci_client.record_performance_metric(
                    ...
                    metric_value=0.9
                    - (i % 3) * 0.05
                    + run_num * 0.03,  # Gets more accurate in newer runs
                    unit="pct",
                )
        ...
        for idx, run in enumerate(history):
            logger.info(
                f"  Run {idx + 1}: {run.get('id')} - {run.get('status')} - Branch: {run.get('branch')}"
            )

        # Get performance metrics for execution time
        logger.info("Getting performance metrics for execution time")
        metrics = await ci_client.get_performance_metrics(metric_name="execution_time")
        ...
        branch_trends = await ci_client.analyze_performance_trends(
            metric_name="execution_time", grouping="branch", timeframe="all"
        )
        ...
        if "error" not in branch_trends:
            ...
            for group in branch_trends.get("groups", []):
                logger.info(
                    f"  Branch {group['name']}: avg={group['avg']:.2f} {branch_trends.get('unit', '')}, min={group['min']:.2f}, max={group['max']:.2f}"
                )

            if branch_trends.get("overall"):
                logger.info(
                    f"  Overall average: {branch_trends['overall']['avg']:.2f} {branch_trends.get('unit', '')}"
                )
        else:
            logger.warning(
                f"Performance trend analysis by branch failed: {branch_trends.get('error')}"
            )
        ...
        type_trends = await ci_client.analyze_performance_trends(
            metric_name="execution_time", grouping="task_type", timeframe="all"
        )
        ...
        if "error" not in type_trends:
            ...
            for group in type_trends.get("groups", []):
                logger.info(
                    f"  Type {group['name']}: avg={group['avg']:.2f} {type_trends.get('unit', '')}, min={group['min']:.2f}, max={group['max']:.2f}"
                )

            if type_trends.get("overall"):
                logger.info(
                    f"  Overall average: {type_trends['overall']['avg']:.2f} {type_trends.get('unit', '')}"
                )
        else:
            logger.warning(
                f"Performance trend analysis by task type failed: {type_trends.get('error')}"
            )
