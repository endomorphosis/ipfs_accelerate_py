...
class RedundancyBenchmark:
    ...
    def __init__(self, cluster_sizes=[1, 3, 5], base_port=8080, operations_per_run=1000, runs=3):
    ...
    async def _start_cluster(self, size):
        ...
        for i in range(size):
            ...
            cmd = [
                ...
                str(port),
                "--db-path",
                os.path.join(self.temp_dirs[i], "coordinator.duckdb"),
                "--data-dir",
                self.temp_dirs[i],
                "--enable-redundancy",
                "--peers",
                peers,
                "--log-level",
                "ERROR",  # Use ERROR level to reduce logging noise
            ]
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        json={
                            ...
                            "batch_sizes": [1, 2, 4, 8],
                            "priority": 1,
                            "worker_requirements": {"capabilities": ["cuda"]},
                        },
                    ) as response:
                        if response.status != 200:
...
async def main():
    ...
    parser.add_argument(
        "--operations", type=int, default=1000, help="Number of operations per run (default: 1000)"
    )
    parser.add_argument(
        "--runs", type=int, default=3, help="Number of runs for each configuration (default: 3)"
    )
