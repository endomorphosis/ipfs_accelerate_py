...
async def run_example(provider_type, provider_config, test_artifacts_dir=None):
    ...
    try:
        ...
        if provider_type not in available_providers:
            logger.warning(
                f"Provider type '{provider_type}' not found. Available types: {', '.join(available_providers)}"
            )
        ...
        reporter = TestResultReporter(
            ci_provider=ci_provider, report_dir=str(report_dir), artifact_dir=str(artifact_dir)
        )
        ...
        if pr_number:
            ...
            logger.info(
                f"{pr_label} comment {'added successfully' if comment_success else 'failed'}"
            )
