...
async def run_example(github_token=None, repository=None, commit_sha=None, pr_number=None):
    ...
    if not repository and github_token:
        logger.warning(
            "GitHub repository not provided. Running in demo mode with simulated responses."
        )
    ...
    try:
        ...
        github_config = {"token": github_token, "repository": repository, "commit_sha": commit_sha}
        ...
        reporter = TestResultReporter(
            ci_provider=github_client, report_dir=str(report_dir), artifact_dir=str(artifact_dir)
        )
...
async def main():
    ...
    if github_token and repository:
        ...
        logger.info(
            "Set GITHUB_TOKEN and GITHUB_REPOSITORY environment variables for actual GitHub integration"
        )
