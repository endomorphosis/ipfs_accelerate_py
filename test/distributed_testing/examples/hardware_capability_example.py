...
class HardwareAwareTaskScheduler:
    ...
    def assign_task(self, task_id: str, worker_id: str) -> bool:
        ...
        if self.workers[worker_id]["status"] != "idle":
            logger.error(
                f"Worker {worker_id} is not idle (status: {self.workers[worker_id]['status']})"
            )
            return False
    ...
    def schedule_pending_tasks(self) -> int:
        ...
        # Get pending tasks
        pending_tasks = {
            task_id: task for task_id, task in self.tasks.items() if task["status"] == "pending"
        }

        # Sort by priority
        sorted_tasks = sorted(pending_tasks.items(), key=lambda x: x[1]["priority"])
    ...
    def simulate_task_execution(self) -> None:
        ...
        # Find assigned tasks
        assigned_tasks = {
            task_id: task for task_id, task in self.tasks.items() if task["status"] == "assigned"
        }
    ...
    def print_scheduler_status(self) -> None:
        ...
        for worker_id, worker in self.workers.items():
            ...
            print(
                f"Worker {worker_id}: {status}, Current Task: {current_task}, Completed Tasks: {completed_tasks}"
            )
...
def run_hardware_capability_example(options: argparse.Namespace) -> None:
    ...
    capabilities = (
        detector.detect_all_capabilities_with_browsers()
        if options.enable_browser_detection
        else detector.detect_all_capabilities()
    )
...
def create_sample_tasks(scheduler: HardwareAwareTaskScheduler) -> None:
    ...
    scheduler.create_task(
        task_type="benchmark",
        hardware_requirements={"hardware_type": HardwareType.GPU, "vendor": HardwareVendor.NVIDIA},
        priority=1,
        min_memory_gb=16.0,
    )
    ...
    scheduler.create_task(
        task_type="test",
        hardware_requirements={"hardware_type": HardwareType.WEBGPU},
        priority=3,
        min_memory_gb=1.0,
    )
    ...
    scheduler.create_task(
        task_type="benchmark",
        hardware_requirements={"hardware_type": HardwareType.TPU},
        priority=2,
        min_memory_gb=16.0,
    )
    ...
    scheduler.create_task(
        task_type="inference",
        hardware_requirements={"hardware_type": HardwareType.WEBNN},
        priority=3,
        min_memory_gb=0.5,
    )

    # Generic task (no specific hardware)
    scheduler.create_task(task_type="utility", hardware_requirements={}, priority=5)
...
def run_worker_compatibility_example(detector: HardwareCapabilityDetector) -> None:
    ...
    workloads = [
        {
            "name": "BERT Inference",
            "requirements": {"hardware_type": HardwareType.GPU},
            "min_memory_gb": 4.0,
        ...
        },
        {
            ...
            "requirements": {"hardware_type": HardwareType.GPU, "vendor": HardwareVendor.NVIDIA},
            "min_memory_gb": 16.0,
            "preferred_hardware_types": [HardwareType.GPU, HardwareType.TPU],
        },
        {
            "name": "WebGPU Visualization",
            "requirements": {"hardware_type": HardwareType.WEBGPU},
            "min_memory_gb": 1.0,
            "preferred_hardware_types": [HardwareType.WEBGPU, HardwareType.GPU],
        },
        {
            "name": "Mobile NPU Inference",
            "requirements": {"hardware_type": HardwareType.NPU},
            "min_memory_gb": 2.0,
            "preferred_hardware_types": [HardwareType.NPU, HardwareType.TPU, HardwareType.GPU],
        },
    ]
...
def create_simulated_workers_in_db(detector: HardwareCapabilityDetector) -> None:
    ...
    try:
        from .enhanced_hardware_capability import WorkerHardwareCapabilities, HardwareCapability
...
def save_capabilities_to_json(capabilities, output_file: str) -> None:
    ...
    try:
        ...
        for hw in capabilities.hardware_capabilities:
            hw_type = (
                hw.hardware_type.value if hasattr(hw.hardware_type, "value") else hw.hardware_type
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Hardware Capability Example for Distributed Testing Framework"
    )
    parser.add_argument("--worker-id", help="Worker ID (default: auto-generated)")
    parser.add_argument(
        "--db-path",
        default="hardware_capabilities.duckdb",
