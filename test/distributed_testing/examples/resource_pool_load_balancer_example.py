...
try:
    from .load_balancer_resource_pool_bridge import LoadBalancerResourcePoolBridge, create_bridge
...
async def run_example():
    ...
    try:
        ...
        logger.info(
            f"Applied optimizations: {len(results['applied'])} applied, {len(results['failed'])} failed"
        )
