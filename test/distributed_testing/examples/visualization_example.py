...
from .hardware_aware_visualization import HardwareSchedulingVisualizer, create_visualizer
from .load_balancer_integration import create_hardware_aware_load_balancer, shutdown_integration
...
def run_efficiency_visualization_example(output_dir: str) -> None:
    ...
    workload_types = [WorkloadType.VISION, WorkloadType.NLP, WorkloadType.AUDIO]
...
def run_thermal_states_example(output_dir: str) -> None:
    ...
    visualizer.visualize_thermal_states(
        thermal_states=thermal_states, filename="thermal_states_example"
    )
...
def run_resource_utilization_example(output_dir: str) -> None:
    ...
    visualizer.visualize_resource_utilization(
        worker_loads=worker_loads, filename="resource_utilization_example"
    )
...
def run_execution_times_example(output_dir: str) -> None:
    ...
    visualizer.visualize_execution_times(
        execution_data=execution_data, filename="execution_times_example"
    )
...
def run_history_tracking_example(output_dir: str) -> None:
    ...
    for i in range(50):
        ...
        for worker_id in worker_ids:
            ...
            gpu_util = (
                max(0.0, min(100.0, random.gauss(70.0, 25.0))) if worker_id == "worker2" else 0.0
            )
    ...
    visualizer.visualize_history(history_data=visualizer.history, filename_prefix="history_example")
    ...
    report_file = visualizer.generate_summary_report(
        filename="scheduling_summary_example.html", include_visualizations=True
    )

    logger.info(
        f"Created history visualizations, saved history to {history_file}, and generated report at {report_file}"
    )
...
def run_integrated_example(output_dir: str) -> None:
    ...
    for worker_id in worker_types:
        ...
        # Record thermal state
        temperature = (
            load.get_effective_load_score()
            if hasattr(load, "get_effective_load_score")
        ...
        for test_id in test_ids:
            ...
            if assignment:
                ...
                execution_data[test_id] = {
                    "estimated_time": estimated_time,
                    "actual_time": actual_time,
                    "workload_type": assignment.test_requirements.test_type.upper()
                    if hasattr(assignment.test_requirements, "test_type")
                    else "UNKNOWN",
                }
                ...
                logger.info(
                    f"Completed test {test_id} with status {status} and execution time {actual_time:.2f}s (estimated: {estimated_time:.2f}s)"
                )

    # Visualize execution times
    visualizer.visualize_execution_times(
        execution_data=execution_data, filename="integrated_execution_times"
    )

    # Generate HTML report
    report_file = visualizer.generate_summary_report(
        filename="integrated_summary.html", include_visualizations=True
    )
...
def run_visualization_examples() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run hardware-aware scheduler visualization examples"
    )
