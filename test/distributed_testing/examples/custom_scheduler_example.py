...
async def setup_coordinator():
    ...
    coordinator = Coordinator(host="localhost", port=8080, enable_advanced_scheduler=True)
    ...
    scheduler_coordinator = SchedulerCoordinator(
        ...
        plugin_dirs=["distributed_testing/plugins/scheduler", "plugins/scheduler", "scheduler"],
    )
...
async def setup_workers(coordinator, num_workers=4):
    ...
    for i in range(num_workers):
        ...
        if i == 0:
            ...
            capabilities = {
                "hardware": ["CPU", "CUDA"],
                "gpu": {"name": "RTX 4090", "cuda_compute": 8.9, "memory_gb": 24},
                "memory": {"total_gb": 64},
                "cpu": {"cores": 16},
            ...
            capabilities = {
                "hardware": ["CPU", "CUDA"],
                "gpu": {"name": "RTX 3070", "cuda_compute": 8.6, "memory_gb": 8},
                "memory": {"total_gb": 32},
                "cpu": {"cores": 8},
...
async def create_test_tasks(coordinator, num_tasks=20):
    ...
    for i in range(num_tasks):
        ...
        if "llama" in model and "7b" in model:
            ...
            hardware_requirements = {"hardware": ["CUDA"], "min_memory_gb": 4}
...
async def run_scheduling_cycle(coordinator, scheduler_coordinator, strategy=None):
    ...
    logger.info(
        f"Running scheduling cycle with plugin '{active_plugin}' and strategy '{active_strategy}'"
    )
...
async def main():
    ...
    parser.add_argument(
        "--num-workers", type=int, default=4, help="Number of workers to create (default: 4)"
    )
    parser.add_argument(
        "--num-tasks", type=int, default=20, help="Number of tasks to create (default: 20)"
    )
    parser.add_argument(
        "--cycles", type=int, default=5, help="Number of scheduling cycles to run (default: 5)"
    )
    ...
    for cycle in range(args.cycles):
        ...
        logger.info(
            f"Tasks: {active_tasks} active, {completed_tasks} completed, {failed_tasks} failed"
        )
