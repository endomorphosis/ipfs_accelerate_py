...
class ResourcePoolWorker:
    ...
    def __init__(
        ...
        # Performance metrics
        self.metrics = {"cpu_percent": 0.0, "memory_percent": 0.0, "gpu_utilization": 0.0}
    ...
    def _build_capabilities(self) -> WorkerCapabilities:
        ...
        hardware_specs = {
            "cpu": {"cores": os.cpu_count() or 4, "arch": "x64"},
            "memory": {
        ...
        if has_gpu:
            hardware_specs["gpu"] = {"name": "Test GPU", "vram_gb": 8}
    ...
    def _update_browser_capacities(self) -> None:
        ...
        for browser_type, count in browser_counts.items():
            if count > 0:
                ...
                capacity = max(
                    0.0, 1.0 - (active / (count * 3))
                )  # Assume each browser can handle 3 models
                ...
                if browser_type in self.browser_metrics:
                    ...
                    self.browser_metrics[browser_type]["memory_usage"] = browser_memory[
                        browser_type
                    ]
    ...
    def _update_load_prediction(self) -> None:
        ...
        for browser_type in ["chrome", "firefox", "edge"]:
            ...
            if browser_count > 0:
                ...
                predicted_utilization = min(
                    1.0, predicted_active / (browser_count * model_capacity)
                )
            else:
        ...
        for browser_type, prediction in predicted_loads.items():
            if prediction["predicted_utilization"] > 0.8:
                logger.debug(
                    f"Predicted high utilization for {browser_type}: {prediction['predicted_utilization']:.2f} "
    ...
    async def execute_test(self, test_req: TestRequirements) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Executing test {test_req.test_id} ({test_req.model_type}/{test_req.model_id})"
        )
    ...
    async def _execute_sharded_test(self, test_req: TestRequirements) -> Dict[str, Any]:
        ...
        fault_tolerance_level = test_req.sharding_requirements.get(
        ...
        try:
            ...
            test_result = {
                ...
                "status": "success",
                "result": result,
                "sharding": {"strategy": sharding_strategy, "num_shards": num_shards},
            }
    ...
    def _prepare_inputs(self, test_req: TestRequirements) -> Any:
        ...
        if test_req.model_type == "vision":
            ...
            test_req.model_type == "text_embedding" or test_req.model_type == "large_language_model"
        ):
    ...
    def _compute_browser_capability_scores(self, test_req: TestRequirements) -> Dict[str, float]:
                ...
                if "avg_latency" in history and history["avg_latency"] > 0:
                    ...
                    if avg_latencies:
                        ...
                        if overall_avg > 0:
                            ...
                            latency_factor = 1.0 / max(
                                0.5, min(1.5, latency_ratio)
                            )  # Invert and clamp
        ...
        if max_score > 0:
            normalized_scores = {
                browser: score / max_score for browser, score in base_scores.items()
            }
        else:
    ...
    async def update_metrics(self) -> None:
        ...
        if self.resource_pool and self.resource_pool.connection_pool:
            ...
            for browser_type in self.browser_metrics:
                self.browser_metrics[browser_type] = {
                    "utilization": 0.0,
                    "memory_usage": 0.0,
                    "active_models": 0,
                }
        ...
        if hasattr(self.worker_load, "set_custom_property"):
            ...
            self.worker_load.set_custom_property(
                "predicted_loads", self.load_prediction["predicted_loads"]
            )

        logger.debug(
            f"Updated metrics for worker {self.worker_id}: CPU={self.metrics['cpu_percent']:.1f}%, "
...
class LoadBalancerResourcePoolBridge:
    ...
    async def submit_test(self, test_requirements: TestRequirements) -> str:
        ...
        """
        logger.info(
            f"Submitting test {test_requirements.test_id} ({test_requirements.model_type}/{test_requirements.model_id})"
        )
    ...
    async def _process_assignments(self) -> None:
        ...
        for worker_id, assignments in all_assignments.items():
            for assignment in assignments:
                ...
                if not worker:
                    logger.warning(
                        f"Worker {worker_id} not found for assignment {assignment.test_id}"
                    )
                    continue
    ...
    async def _execute_test(
        self, worker: ResourcePoolWorker, test_id: str, test_req: TestRequirements
    ) -> None:
        """
        ...
        try:
            ...
            # Mark test as failed
            self.load_balancer.update_assignment_status(test_id, "failed", {"error": str(e)})
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                for worker_id, worker in self.workers.items():
                    try:
                        ...
                        self.load_balancer.update_worker_load(worker_id, worker.worker_load)
                    except Exception as e:
            ...
            try:
                await wait_for(self._stop_monitoring.wait(), timeout=self.monitoring_interval)
            except TimeoutError:
    ...
    async def get_model_performance_history(
        self, model_type: str = None, time_range: str = "7d"
    ) -> Dict[str, Any]:
        """
            ...
            if worker.resource_pool:
                try:
                    history = await worker.resource_pool.get_performance_history(
                        model_type=model_type, time_range=time_range
                    )
                    ...
                    logger.error(
                        f"Error getting performance history for worker {worker_id}: {str(e)}"
                    )
    ...
    def _analyze_model_type_assignment(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "browser_preferences": {
                model_type: list(browsers) for model_type, browsers in browser_preferences.items()
            },
    ...
    async def apply_optimization_recommendations(
        ...
        for worker_id, worker_recs in recommendations.get("worker_analysis", {}).items():
            ...
            if browser_prefs:
                try:
                    ...
                    logger.error(
                        f"Error applying browser preferences for worker {worker_id}: {str(e)}"
                    )
                    results["failed"].append(
                        {"worker_id": worker_id, "type": "browser_preferences", "error": str(e)}
                    )
        ...
        if load_balance_rec == "rebalance":
            try:
                ...
                results["applied"].append({"type": "load_balance", "action": "rebalanced"})
            except Exception as e:
                logger.error(f"Error rebalancing loads: {str(e)}")
                results["failed"].append({"type": "load_balance", "error": str(e)})
        ...
        if browser_recs:
            try:
                ...
                results["applied"].append({"type": "browser_distribution", "actions": browser_recs})
            except Exception as e:
                logger.error(f"Error applying browser distribution recommendations: {str(e)}")
                results["failed"].append({"type": "browser_distribution", "error": str(e)})

        logger.info(
            f"Applied optimization recommendations: {len(results['applied'])} applied, {len(results['failed'])} failed, {len(results['ignored'])} ignored"
        )
...
def create_bridge(config: Dict[str, Any]) -> LoadBalancerResourcePoolBridge:
    ...
    browser_preferences = config.get(
