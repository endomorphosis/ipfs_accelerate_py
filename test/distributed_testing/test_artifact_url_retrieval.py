...
class MockAzureDevOpsClient(AzureDevOpsClient):
    ...
    async def get_artifact_url(self, test_run_id: str, artifact_name: str) -> Optional[str]:
        """Override to use mock implementation."""
        if (
            test_run_id in self._artifact_attachments
...
class MockBitbucketClient(BitbucketClient):
    ...
    async def upload_artifact(
        ...
        url = (
            f"https://bitbucket.org/{workspace}/{repository}/downloads/{report_id}-{artifact_name}"
        )
...
async def test_jenkins_get_artifact_url():
    ...
    test_run = await jenkins.create_test_run(
        {"name": "Jenkins Test Run", "job_name": "test_job", "build_id": "456"}
    )
    ...
    try:
        ...
        result = await jenkins.upload_artifact(
            test_run_id=test_run_id, artifact_path=file_path, artifact_name="test_result.json"
        )
        ...
        url = await jenkins.get_artifact_url(
            test_run_id=test_run_id, artifact_name="test_result.json"
        )
        ...
        url = await jenkins.get_artifact_url(
            test_run_id="jenkins-mock_job-123", artifact_name="test.log"
        )
        ...
        url = await jenkins.get_artifact_url(
            test_run_id=test_run_id, artifact_name="non_existent.json"
        )
...
async def test_circleci_get_artifact_url():
    ...
    try:
        ...
        # Test with a pre-cached artifact
        url = await circleci.get_artifact_url(
            test_run_id="circleci-github/mock-org/mock-repo-123", artifact_name="test.log"
        )
...
async def test_azure_get_artifact_url():
    ...
    # Create a test run
    test_run = await azure.create_test_run({"name": "Azure DevOps Test Run", "test_run_id": "456"})
    ...
    try:
        ...
        assert url is not None, "Failed to get artifact URL"
        assert "test_result.json" in url or "attachments" in url, (
            f"URL doesn't have expected pattern: {url}"
        )

        logger.info(f"Azure DevOps artifact URL: {url}")

        # Test with a pre-cached artifact
        url = await azure.get_artifact_url(test_run_id="123", artifact_name="test.log")
...
async def test_github_get_artifact_url():
    ...
    # Create a test run
    test_run = await github.create_test_run({"name": "GitHub Test Run"})
    ...
    try:
        ...
        # Test with a pre-cached artifact
        url = await github.get_artifact_url(
            test_run_id="github-mock-org/mock-repo-123", artifact_name="test.log"
        )
...
async def test_bitbucket_get_artifact_url():
    ...
    try:
        ...
        # Test with a pre-cached artifact
        url = await bitbucket.get_artifact_url(
            test_run_id="bitbucket-mock_workspace-mock_repo-report123", artifact_name="test.log"
        )
...
async def test_teamcity_get_artifact_url():
    ...
    # Create a test run
    test_run = await teamcity.create_test_run({"name": "TeamCity Test Run", "build_id": "789"})
    ...
    try:
        ...
        # Test with a pre-cached artifact
        url = await teamcity.get_artifact_url(
            test_run_id="teamcity-123-456", artifact_name="test.log"
        )
...
async def test_travis_get_artifact_url():
    ...
    test_run = await travis.create_test_run(
        {"name": "Travis CI Test Run", "repository": "test-org/test-repo", "build_id": "789"}
    )
    ...
    try:
        ...
        # Test with a pre-cached artifact
        url = await travis.get_artifact_url(
            test_run_id="travis-mock-org/mock-repo-123", artifact_name="test.log"
        )
