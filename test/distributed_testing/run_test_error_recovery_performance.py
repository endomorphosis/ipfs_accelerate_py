...
class DemoCoordinator:
    ...
    def add_task(self, task_id, task_type="test", status="pending"):
        ...
        task = {
            ...
            "worker_id": None,
            "attempts": 0,
            "config": {"retry_policy": {"max_retries": 3, "backoff_factor": 2.0}},
        }
    ...
    def assign_task(self, task_id, worker_id):
        ...
        WHERE task_id = ?
        """,
            (task["status"], task["started"], task["worker_id"], task_id),
        )
    ...
    async def _assign_pending_tasks(self):
        ...
        # Get active workers
        active_workers = [
            worker_id for worker_id, worker in self.workers.items() if worker["status"] == "active"
        ]
...
class ErrorSimulator:
    ...
    def _create_worker_offline_error(self, worker_id):
        ...
        if worker_id not in self.coordinator.workers:
            worker_id = (
                list(self.coordinator.workers.keys())[0] if self.coordinator.workers else "worker-1"
            )
        ...
        context = {
            "component": "worker",
            "operation": "heartbeat",
            "related_entities": {"worker_id": worker_id},
        }
    ...
    def _create_task_timeout_error(self, task_id):
        ...
        context = {
            ...
            "operation": "execute",
            "related_entities": {"task_id": task_id, "worker_id": worker_id},
        }
    ...
    def _create_resource_error(self, resource_type="memory"):
        ...
        context = {
            ...
            "related_entities": {"resource_type": resource_type},
...
async def run_recovery_scenario(scenario_type, recovery_system, error_simulator, iterations=5):
    ...
    results = {"successful": 0, "failed": 0, "recovery_info": []}
...
async def run_progressive_recovery_test(
    recovery_system, error_simulator, error_type="worker_offline"
):
    ...
    try:
        ...
        if success:
            ...
            print(
                f"Final level: {recovery_system.error_recovery_levels.get(error_report.error_id, 1)}"
            )
        else:
        ...
        return {"error_id": error_report.error_id, "success": success, "info": info}
    finally:
...
async def main():
    ...
    parser.add_argument(
        "--db-path", default=":memory:", help="Path to database file (default: in-memory)"
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=3,
        help="Number of iterations for each scenario (default: 3)",
    )
    parser.add_argument(
        "--scenario",
        choices=[
    ...
    (
        ...
        error_simulator,
        recovery_analyzer,
    ) = await setup_test_environment(args.db_path)
    ...
    if args.scenario == "all":
        ...
        results = await run_recovery_scenario(
            args.scenario, recovery_system, error_simulator, args.iterations
        )
