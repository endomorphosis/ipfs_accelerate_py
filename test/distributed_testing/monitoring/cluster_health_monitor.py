            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=2) as response:
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"Failed to get status from {node.node_id}: HTTP {response.status}"
                        )
        except Exception as e:
    ...
    def _update_dashboard(self, nodes):
        ...
        if leader_node:
            ...
            if not hasattr(self, "last_leader") or self.last_leader != leader_node.node_id:
                self._add_log_entry(
                    f"Leader changed to {leader_node.node_id} (Term: {leader_node.term})"
                )
            ...
            if not hasattr(self, "last_term") or self.last_term != leader_node.term:
                if hasattr(self, "last_term"):
                    self._add_log_entry(
                        f"Term advanced from {self.last_term} to {leader_node.term}"
                    )
        ...
        for i, node in enumerate(nodes):
            ...
            self.nodes_canvas.create_text(
                x + node_width // 2, y + 15, text=node.node_id, font=("Arial", 12, "bold")
            )

            # Draw role
            self.nodes_canvas.create_text(
                x + node_width // 2, y + 35, text=f"Role: {node.role}", font=("Arial", 10)
            )

            # Draw term
            self.nodes_canvas.create_text(
                x + node_width // 2, y + 55, text=f"Term: {node.term}", font=("Arial", 10)
            )
            ...
            # Draw task count
            self.nodes_canvas.create_text(
                x + node_width // 2, y + 95, text=f"Tasks: {node.task_count}", font=("Arial", 10)
            )
            ...
            if node.is_stale():
                if not hasattr(node, "logged_stale") or not node.logged_stale:
                    self._add_log_entry(
                        f"Node {node.node_id} appears to be offline or unresponsive"
                    )
        ...
        for i, node in enumerate(self.nodes):
            ...
            if len(history["time"]) > 0:
                ...
                if len(history["time"]) > 0:
                    ...
                    # Plot CPU usage
                    self.cpu_ax.plot(
                        relative_times, history["cpu"], label=node.node_id, color=colors[i]
                    )

                    # Plot memory usage
                    self.memory_ax.plot(
                        relative_times, history["memory"], label=node.node_id, color=colors[i]
                    )

                    # Plot latency
                    self.latency_ax.plot(
                        relative_times, history["latency"], label=node.node_id, color=colors[i]
                    )
