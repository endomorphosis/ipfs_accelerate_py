...
try:
    ...
    _optional_dep_warning(
        "Performance Analyzer not available. Advanced performance analysis features will be disabled."
    )
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("result_aggregator.log")],
)
...
class ResultAggregatorService:
    ...
    def get_aggregated_results(
        ...
        try:
            ...
            # Complete query
            query = (
                "SELECT {select_clause} ".format(select_clause=select_clause)
                + base_from
                + where_clause
                + group_by_clause
            )
        ...
        try:
            ...
            if filter_criteria:
                ...
                if metrics:
                    if conditions:
                        base_query += (
                            " AND m.metric_name IN (" + ", ".join(["?"] * len(metrics)) + ")"
                        )
                    else:
                        base_query += (
                            " WHERE m.metric_name IN (" + ", ".join(["?"] * len(metrics)) + ")"
                        )
            ...
            for metric_name, group in df.groupby("metric_name"):
                ...
                group["moving_avg"] = (
                    group["value"].rolling(window=min(window_size, len(group))).mean()
                )
    ...
    def _detect_anomalies_for_result(self, result_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            historical_results = [
                r
                for r in self.get_results(filter_criteria=filter_criteria, limit=200)
                if r.get("id") != result_id
            ]

            if len(historical_results) < 10:
                logger.info(
                    f"Not enough historical data for anomaly detection (need at least 10, have {len(historical_results)})"
                )
                return []
            ...
            current_feature_vector = current_feature_vector + [0] * (
                max_len - len(current_feature_vector)
            )
        ...
        try:
            ...
            for test_type, type_results in test_types.items():
                if len(type_results) < 10:
                    logger.info(
                        f"Not enough results for test type {test_type} (need at least 10, have {len(type_results)})"
                    )
                    continue
                ...
                with self._db_lock:
                    if report_type == "performance":
                        ...
                        report_data["aggregated_metrics"] = self.get_aggregated_results(
                            filter_criteria=filter_criteria, aggregation_type="mean"
                        )
                        ...
                        report_data["recent_results"] = self.get_results(
                            filter_criteria=filter_criteria, limit=10
                        )
                ...
                if report_type == "performance":
                    ...
                    if report_data["recent_results"]:
                        ...
                        for result in report_data["recent_results"]:
                            markdown += (
                                f"| {result['id']} | {result['task_id']} | {result['worker_id']} | "
                            )
                    ...
                    if report_data["anomalies"]:
                        ...
                        for anomaly in report_data["anomalies"]:
                            ...
                            markdown += (
                                f"{anomaly['anomaly_score']:.4f} | {anomaly['anomaly_type']} | "
                            )
                            markdown += (
                                f"{anomaly['is_confirmed']} | {anomaly['detection_time']} |\n"
                            )
                        ...
                        for i, anomaly in enumerate(report_data["anomalies"][:5]):
                            ...
                            if "anomalous_features" in anomaly["details"]:
                                ...
                                for feature in anomaly["details"]["anomalous_features"]:
                                    markdown += (
                                        f"| {feature['feature']} | {feature['value']:.4f} | "
                                    )
                                    markdown += (
                                        f"{feature['z_score']:.4f} | {feature['mean']:.4f} | "
                                    )
                ...
                if report_type == "performance":
                    ...
                    for metric, trend_data in report_data["performance_trends"].items():
                        ...
                        if (
                            ...
                            and "time_series" in trend_data
                            and len(trend_data["time_series"]) > 1
                        ):
                            ...
                            try:
                                ...
                                moving_avgs = [
                                    point["moving_avg"] if point["moving_avg"] is not None else None
                                    for point in trend_data["time_series"]
                                ]
                                ...
                                # Plot moving average
                                valid_indices = [
                                    i for i, v in enumerate(moving_avgs) if v is not None
                                ]
                                if valid_indices:
            ...
            if format and format != "json":
                try:
                    ...
                    return self.generate_analysis_report(
                        filter_criteria=filter_dict, report_type=report_type, format=format
                    )
    ...
    def analyze_performance_regression(
        ...
        if not hasattr(self, "performance_analyzer") or not self.performance_analyzer:
            logger.warning(
                "Performance Analyzer not available. Cannot analyze performance regression."
            )
            return {}
    ...
    def compare_hardware_performance(
        self, metrics: List[str] = None, test_type: str = None, time_period: str = "30d"
    ):
        """
        ...
        if not hasattr(self, "performance_analyzer") or not self.performance_analyzer:
            logger.warning(
                "Performance Analyzer not available. Cannot compare hardware performance."
            )
            return {}

        return self.performance_analyzer.compare_hardware_performance(
            metrics=metrics, test_type=test_type, time_period=time_period
        )
    ...
    def analyze_resource_efficiency(self, test_type: str = None, time_period: str = "30d"):
        ...
        if not hasattr(self, "performance_analyzer") or not self.performance_analyzer:
            logger.warning(
                "Performance Analyzer not available. Cannot analyze resource efficiency."
            )
            return {}

        return self.performance_analyzer.analyze_resource_efficiency(
            test_type=test_type, time_period=time_period
        )
    ...
    def analyze_performance_over_time(
        ...
        if not hasattr(self, "performance_analyzer") or not self.performance_analyzer:
            logger.warning(
                "Performance Analyzer not available. Cannot analyze performance over time."
            )
            return {}

        return self.performance_analyzer.analyze_performance_over_time(
            metric_name=metric_name, grouping=grouping, test_type=test_type, time_period=time_period
        )
    ...
    def generate_performance_report(
        ...
        if not hasattr(self, "performance_analyzer") or not self.performance_analyzer:
            logger.warning(
                "Performance Analyzer not available. Cannot generate performance report."
            )
    ...
    result_id = aggregator.store_result(
        {
            ...
            "details": {"model": "example_model", "batch_size": 8, "precision": "fp16"},
        }
    )
