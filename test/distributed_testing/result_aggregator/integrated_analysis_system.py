...
if DATA_ANALYSIS_AVAILABLE:
    try:
        from test.distributed_testing.result_aggregator.analysis.analysis import (
            ...
            analyze_circuit_breaker_performance,
            analyze_multi_dimensional_performance,
...
try:
    from test.distributed_testing.result_aggregator.pipeline.pipeline import (
        DataSource,
        ProcessingPipeline,
    )
...
try:
    from test.distributed_testing.result_aggregator.ml_detection.ml_anomaly_detector import (
        MLAnomalyDetector,
    )
    ...
    ML_AVAILABLE = False
    warnings.warn(
        "ML detection components not available. ML-based anomaly detection will be disabled."
    )
...
class IntegratedAnalysisSystem:
    ...
    def register_with_coordinator(self, coordinator):
        ...
        if integration_cls is None:
            try:
                from test.distributed_testing.result_aggregator.coordinator_integration import (
                    ResultAggregatorIntegration as integration_cls,
                )
        ...
        logger.info(
            f"Registered with coordinator (ID: {coordinator.coordinator_id if hasattr(coordinator, 'coordinator_id') else 'unknown'})"
        )
    ...
    def _start_analysis_thread(self):
        ...
        self.analysis_thread = threading.Thread(target=self._run_periodic_analysis, daemon=True)
    ...
    def _perform_periodic_analysis(self, service=None):
        ...
        try:
            ...
            report = analysis_service.generate_analysis_report(
                filter_criteria=filter_criteria, report_type="summary", format="json"
            )
    ...
    def register_notification_handler(self, handler):
        ...
        if callable(handler):
            ...
            logger.info(
                f"Registered notification handler {handler.__name__ if hasattr(handler, '__name__') else 'anonymous'}"
            )
            return True
        else:
    ...
    def unregister_notification_handler(self, handler):
        ...
        if handler in self.notification_handlers:
            ...
            logger.info(
                f"Unregistered notification handler {handler.__name__ if hasattr(handler, '__name__') else 'anonymous'}"
            )
            return True
        else:
            if _is_test_mode():
    ...
    def get_results(
        self, filter_criteria: Dict[str, Any] = None, limit: int = 100, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
    ...
    def analyze_results(
        ...
        if "trends" in analysis_types:
            trends = self.service.analyze_performance_trends(
                filter_criteria=filter_criteria, metrics=metrics
            )
        ...
        if DATA_ANALYSIS_AVAILABLE and results:
            ...
            if (
                "performance" in analysis_types
                and group_by
                and analyze_multi_dimensional_performance
            ):
                dimensions = [group_by]
            ...
            if (
                "recovery" in analysis_types
                and any("recovery" in col for col in df.columns)
                and analyze_recovery_performance
            ):
            ...
            if (
                "circuit_breaker" in analysis_types
                and any("circuit_breaker" in col for col in df.columns)
                and analyze_circuit_breaker_performance
            ):
                circuit_breaker_data = [r for r in data if "circuit_breaker_state" in r]
                if circuit_breaker_data:
                    circuit_breaker_analysis = analyze_circuit_breaker_performance(
                    ...
                    if metric in df.columns:
                        ...
                        if len(ts_data) >= 5:  # Need at least 5 points for forecasting
                            ...
                            for _, row in ts_data.iterrows():
                                ts.append({"timestamp": row["timestamp"], "value": row[metric]})
        ...
        if report_type == "performance":
            ...
            if hasattr(self.service, "generate_performance_report"):
                ...
                report = self.service.generate_analysis_report(
                    filter_criteria=filter_criteria, report_type="performance", format=format
                )
        else:
            # Use the standard report generation
            report = self.service.generate_analysis_report(
                filter_criteria=filter_criteria, report_type=report_type, format=format
            )
    ...
    def _visualize_trends(self, trend_data, metrics, output_path):
        ...
        try:
            ...
            fig, axes = plt.subplots(
                nrows=len(metrics), figsize=(12, 5 * len(metrics)), squeeze=False
            )
            ...
            for i, metric in enumerate(metrics):
                ...
                timestamps = [
                    datetime.fromisoformat(ts["timestamp"].replace("Z", "+00:00"))
                    if isinstance(ts["timestamp"], str)
                    else ts["timestamp"]
                ...
                if valid_indices:
                    ...
                    ax.plot(
                        valid_timestamps, valid_moving_avg, "r-", label="Moving Avg", linewidth=2
                    )
    ...
    def _visualize_anomalies(self, anomaly_data, output_path):
        ...
        try:
            ...
            timestamps = [
                anomaly.get("detection_time", i) for i, anomaly in enumerate(anomaly_data)
            ]
    ...
    def _visualize_performance_comparison(self, data, metrics, output_path):
        ...
        try:
            if not DATA_ANALYSIS_AVAILABLE:
                logger.warning(
                    "Pandas/NumPy not available. Performance comparison visualization not possible."
                )
                return False
            ...
            if not metrics:
                ...
                metrics = [
                    ...
                    if col in ["latency", "throughput", "memory_usage", "cpu_usage"]
                ]
    ...
    def _visualize_workload_distribution(self, workload_data, output_path):
        ...
        try:
            if not VISUALIZATION_AVAILABLE:
                logger.warning(
                    "Matplotlib not available. Workload distribution visualization not possible."
                )
                return False
    ...
    def _visualize_failure_patterns(self, failure_data, output_path):
        ...
        try:
            if not VISUALIZATION_AVAILABLE:
                logger.warning(
                    "Matplotlib not available. Failure patterns visualization not possible."
                )
                return False
                ...
                if correlations:
                    ...
                    for i, corr in enumerate(correlations[:5]):  # Limit to top 5
                        ...
                        if corr_type == "worker_issue":
                            ...
                            corr_text += (
                                f"{i + 1}. Test type {test_type}: {total_failures} failures\n"
                            )
    ...
    def _visualize_circuit_breaker(self, circuit_breaker_data, output_path):
        ...
        try:
            if not VISUALIZATION_AVAILABLE:
                logger.warning(
                    "Matplotlib not available. Circuit breaker visualization not possible."
                )
                return False
            ...
            if "parameter_stats" in circuit_breaker_data:
                ...
                threshold_color = (
                    "green"
                    if threshold_trend == "increasing"
                    else "red"
                    if threshold_trend == "decreasing"
                    else "blue"
                )
                timeout_color = (
                    "green"
                    if timeout_trend == "increasing"
                    else "red"
                    if timeout_trend == "decreasing"
                    else "blue"
                )

                ax3.bar(
                    [0], [1], color=threshold_color, label=f"Threshold Trend: {threshold_trend}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Integrated Analysis System for Distributed Testing Framework"
    )

    parser.add_argument(
        "--db-path", type=str, default="./benchmark_db.duckdb", help="Path to DuckDB database file"
    )

    # Analysis options
    parser.add_argument("--analyze", action="store_true", help="Perform analysis on test results")
    parser.add_argument("--filter", type=str, help="JSON filter criteria for analysis")
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days to look back for analysis"
    )
    ...
    parser.add_argument("--viz-output", type=str, help="Path to save the visualization")

    # Cleanup options
    parser.add_argument("--cleanup", action="store_true", help="Clean up old data")
    parser.add_argument(
        "--keep-days", type=int, default=90, help="Number of days to keep when cleaning up old data"
    )
    ...
    try:
        ...
        if args.analyze:
            analysis_results = system.analyze_results(
                filter_criteria=filter_criteria, time_period_days=args.days
            )
