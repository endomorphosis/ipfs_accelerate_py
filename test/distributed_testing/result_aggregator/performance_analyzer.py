...
if not logging.getLogger().handlers:
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("performance_analyzer.log")],
    )
...
try:
    ...
    _log_optional_dependency(
        "Visualization libraries not available. Visualization features will be disabled."
    )
...
try:
    ...
    ML_AVAILABLE = False
    _log_optional_dependency(
        "Scikit-learn not available. Advanced regression analysis will be disabled."
    )
...
class PerformanceAnalyzer:
    ...
    def compare_hardware_performance(
        ...
        try:
            ...
            results = {
                ...
                "metrics": {},
                "summary": {"best_overall_hardware": None, "best_by_metric": {}},
            }
            ...
            for metric in metrics_to_analyze:
                ...
                for hardware in hardware_profiles:
                    ...
                    if values:
                        ...
                        if (
                            best_value is None
                            or (is_higher_better and mean > best_value)
                            or (not is_higher_better and mean < best_value)
                        ):
                        ...
                        if hardware in metric_results:
                            ...
                            if is_higher_better and best_value > 0:
                                ...
                                relative_performance = (
                                    metric_results[best_hardware]["mean"]
                                    / metric_results[hardware]["mean"]
                                )
            ...
            if hardware_scores:
                results["summary"]["best_overall_hardware"] = max(
                    hardware_scores.items(), key=lambda x: x[1]
                )[0]
    ...
    def analyze_resource_efficiency(
        self, test_type: str = None, time_period: str = "30d"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results = {
                "efficiency_metrics": {},
                "summary": {"most_efficient_setup": None, "efficiency_scores": {}},
            }
            ...
            has_throughput_memory = (
                len(self.db.execute(throughput_memory_query, params).fetchall()) > 0
            )
            ...
            for config in configurations:
                ...
                if (
                    ...
                    and "memory_usage_mb" in metrics
                    and metrics["memory_usage_mb"] > 0
                ):
                ...
                if (
                    "throughput" in metrics
                    and "power_consumption" in metrics
                    and metrics["power_consumption"] > 0
                ):
                ...
                if (
                    ...
                    time_memory_efficiency = 1 / (
                        metrics["execution_time"] * metrics["memory_usage_mb"]
                    )
            ...
            if efficiency_scores:
                ...
                results["summary"]["most_efficient_setup"] = max(
                    efficiency_scores.items(), key=lambda x: x[1]
                )[0]
    ...
    def _get_config_string(self, config: Dict[str, Any]) -> str:
        ...
        hardware_str = (
            ...
            if isinstance(config["hardware"], list)
        ...
        if report_type == "comprehensive" or report_type == "all":
            ...
            for metric in self.key_metrics:
                regression = self.detect_performance_regression(
                    metric_name=metric, filter_criteria=filter_criteria, baseline_period=time_period
                )
            ...
            report_data["hardware_comparison"] = self.compare_hardware_performance(
                metrics=self.key_metrics, time_period=time_period
            )
            ...
            if "throughput" in self.key_metrics:
                report_data["time_analysis"]["throughput"] = self.analyze_performance_over_time(
                    metric_name="throughput", time_period=time_period
                )
            ...
            if "latency_ms" in self.key_metrics:
                report_data["time_analysis"]["latency"] = self.analyze_performance_over_time(
                    metric_name="latency_ms", time_period=time_period
                )
            ...
            report_data["regression_analysis"] = self.detect_performance_regression(
                filter_criteria=filter_criteria, baseline_period=time_period
            )
            ...
            for metric in self.key_metrics:
                report_data["time_analysis"][metric] = self.analyze_performance_over_time(
                    metric_name=metric, time_period=time_period
                )
    ...
    def _generate_markdown_report(self, report_data: Dict[str, Any]) -> str:
        ...
        if "regression_analysis" in report_data:
            ...
            if "summary" in report_data["regression_analysis"]:
                ...
                markdown += "### Summary\n\n"
                markdown += (
                    f"- **Total metrics analyzed**: {summary.get('total_metrics_analyzed', 0)}\n"
                )
                markdown += (
                    f"- **Regressions detected**: {summary.get('regressions_detected', 0)}\n"
                )
                markdown += (
                    f"- **Improvements detected**: {summary.get('improvements_detected', 0)}\n"
                )
            ...
            if metric_data:
                ...
                for metric, data in metric_data.items():
                    ...
                    if status == "insufficient_data":
                        ...
                        markdown += (
                            f"- Minimum samples required: {data.get('min_samples_required', 0)}\n\n"
                        )
                        continue
