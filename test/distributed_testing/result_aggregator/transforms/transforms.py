...
def _normalize_series(
    series: pd.Series, method: str, target_range: Optional[Tuple[float, float]] = None
) -> pd.Series:
    """
...
def extract_features(
    ...
    for fs in feature_sets:
        if fs == "performance":
            result = _extract_performance_features(result, include_derived, include_statistical)
        elif fs == "hardware":
...
def _extract_performance_features(
    data: pd.DataFrame, include_derived: bool = True, include_statistical: bool = True
) -> pd.DataFrame:
    """
    ...
    if include_derived:
        ...
        if "throughput" in available_metrics:
            if "latency" in available_metrics:
                result["throughput_latency_ratio"] = result["throughput"] / result[
                    "latency"
                ].replace(0, np.nan)

            if "memory_usage" in available_metrics:
                result["memory_efficiency"] = result["throughput"] / result["memory_usage"].replace(
                    0, np.nan
                )

            if "cpu_usage" in available_metrics:
                result["cpu_efficiency"] = result["throughput"] / result["cpu_usage"].replace(
                    0, np.nan
                )

            if "gpu_usage" in available_metrics:
                result["gpu_efficiency"] = result["throughput"] / result["gpu_usage"].replace(
                    0, np.nan
                )
...
def _extract_hardware_features(data: pd.DataFrame, include_derived: bool = True) -> pd.DataFrame:
    """
    ...
    if "hardware_type" in available_columns:
        ...
        try:
            hardware_dummies = pd.get_dummies(result["hardware_type"], prefix="hw", dummy_na=False)
    ...
    if "architecture" in available_columns:
        ...
        try:
            arch_dummies = pd.get_dummies(result["architecture"], prefix="arch", dummy_na=False)
    ...
    if include_derived:
        ...
        if "compute_units" in available_columns and "memory_capacity" in available_columns:
            result["memory_per_cu"] = result["memory_capacity"] / result["compute_units"].replace(
                0, np.nan
            )
        ...
        if "driver_version" in available_columns:
            try:
                ...
                result["driver_major"] = (
                    result["driver_version"].str.extract(r"(\d+)\.").astype(float)
                )
                result["driver_minor"] = (
                    result["driver_version"].str.extract(r"\d+\.(\d+)").astype(float)
                )
            except Exception as e:
...
def _extract_error_features(data: pd.DataFrame, include_statistical: bool = True) -> pd.DataFrame:
    """
    ...
    if "error_message" in available_columns and "error_category" not in result.columns:
        ...
        try:
            if "error_category" in result.columns:
                cat_dummies = pd.get_dummies(result["error_category"], prefix="err", dummy_na=False)
...
def _add_time_features(data: pd.DataFrame, time_column: str, time_window: int) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if time_range > 0:
            result["recency_score"] = (
                result[time_column] - result[time_column].min()
...
def clean_outliers(
    ...
    for col in columns:
        ...
        logger.info(
            f"Detected {outlier_count} outliers in column '{col}' ({outlier_count / len(result) * 100:.1f}%)"
        )
...
def transform_for_analysis(
    data: pd.DataFrame, target: str, config: Optional[Dict[str, Any]] = None
) -> pd.DataFrame:
    """
    ...
    if target == "performance_analysis":
        ...
        result = extract_features(result, feature_set="hardware", include_derived=True)

        result = extract_features(
            result, feature_set="performance", include_derived=True, include_statistical=False
        )
        ...
        result = extract_features(
            result, feature_set="error", include_statistical=True, time_window=time_window
        )

        # Extract performance features if requested
        if config.get("include_performance", False):
            result = extract_features(
                result, feature_set="performance", include_derived=True, include_statistical=True
            )
...
def prepare_for_visualization(
    data: pd.DataFrame, viz_type: str, config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    result = {
        ...
        "metadata": {
            "viz_type": viz_type,
            "config": config,
            "data_summary": {"row_count": len(data), "column_count": len(data.columns)},
        },
    }
...
def _prepare_time_series_viz(
    data: pd.DataFrame, config: Dict[str, Any], result: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    if group_by:
        ...
        for group_name, group_df in data_copy.groupby(group_cols):
            group_name = (
                group_name if not isinstance(group_name, tuple) else "_".join(map(str, group_name))
            )
    ...
    result["metadata"]["time_range"] = {
        ...
        "duration_hours": (
            data_copy[time_column].max() - data_copy[time_column].min()
        ).total_seconds()
        / 3600,
    }
...
def _prepare_correlation_viz(
    ...
    for i, row_col in enumerate(corr_matrix.index):
        for j, col_col in enumerate(corr_matrix.columns):
            matrix_data.append(
                {"row": row_col, "column": col_col, "value": float(corr_matrix.iloc[i, j])}
            )

    result["data"] = {"matrix": matrix_data, "columns": list(corr_matrix.columns)}
    ...
    if config.get("include_stats", False):
                ...
                if i != j:  # Skip diagonal (self-correlation)
                    if method == "pearson":
                        corr, p_value = stats.pearsonr(
                            data_copy[row_col].dropna(), data_copy[col_col].dropna()
                        )
        ...
        for i, row_col in enumerate(p_values.index):
            for j, col_col in enumerate(p_values.columns):
                p_value_data.append(
                    {"row": row_col, "column": col_col, "value": float(p_values.iloc[i, j])}
                )
...
def _prepare_anomaly_viz(
    ...
    if not anomaly_column:
        ...
        anomaly_candidates = [
            col for col in data.columns if "anomaly" in col.lower() or "outlier" in col.lower()
        ]
    ...
    if not pd.api.types.is_bool_dtype(
        data_copy[anomaly_column]
    ) and not pd.api.types.is_numeric_dtype(data_copy[anomaly_column]):
        logger.warning(
            f"Anomaly column '{anomaly_column}' is not boolean or numeric, attempting to convert"
        )
        # Try to convert to boolean
        if (
            data_copy[anomaly_column]
            .isin([0, 1, True, False, "True", "False", "true", "false"])
            .all()
        ):
    ...
    for metric in metrics:
        metric_data = {"name": metric, "normal": {}, "anomalies": {}}
...
if __name__ == "__main__":
    ...
    try:
        ...
        normalized_data = normalize_metrics(
            data, metrics=["latency", "throughput", "memory_usage"], method="min-max"
        )
        print(normalized_data[["latency", "throughput", "memory_usage"]].describe())

        print("\nExample 2: Extract features")
        features = extract_features(data, feature_set="performance", include_derived=True)
