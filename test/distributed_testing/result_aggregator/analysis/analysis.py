...
class AdvancedAnalysis:
        ...
        try:
            ...
            for name, group in grouped:
                if len(group) < min_samples:
                    logger.warning(
                        f"Group '{name}' has fewer than {min_samples} samples. Skipping."
                    )
                    continue
    ...
    def _visualize_correlation_matrices(
        self, correlations: Dict[str, pd.DataFrame], method: str, output_path: Optional[str] = None
    ):
        """
            ...
            for (group_name, metric_name), group_data in groups:
                ...
                if "seasonality" in detection_methods and SCIPY_AVAILABLE:
                    try:
                        ...
                        patterns[group_name][metric_name]["seasonality"] = {
                            "detected": False,
                            "error": str(e),
                        }
                ...
                if "forecast" in detection_methods and SCIPY_AVAILABLE:
                    try:
                        ...
                        patterns[group_name][metric_name]["forecast"] = {
                            "success": False,
                            "error": str(e),
                        }
    ...
    def _detect_trend(self, time_series: pd.Series) -> Dict:
        ...
        if SCIPY_AVAILABLE:
            ...
            return {
                ...
                "percent_change": (time_series.iloc[-1] - time_series.iloc[0])
                / time_series.iloc[0]
                * 100
                if time_series.iloc[0] != 0
                else float("inf"),
            }
        else:
    ...
    def _detect_seasonality(self, time_series: pd.Series) -> Dict:
        ...
        try:
            ...
            decomposition = seasonal_decompose(
                time_series.dropna(), model="additive", period=period
            )
    ...
    def _generate_forecast(self, time_series: pd.Series, forecast_periods: int = 7) -> Dict:
        """
        ...
        try:
            ...
            return {
                ...
                "confidence_intervals": {
                    "lower": model_fit.get_forecast(steps=forecast_periods)
                    .conf_int()
                    .iloc[:, 0]
                    .tolist(),
                    "upper": model_fit.get_forecast(steps=forecast_periods)
                    .conf_int()
                    .iloc[:, 1]
                    .tolist(),
                },
                "model_info": {"aic": model_fit.aic, "bic": model_fit.bic},
            }
    ...
    def _visualize_patterns(
        self, data: pd.DataFrame, patterns: Dict, group_by: str, output_path: Optional[str] = None
    ):
        """
        ...
        for group_name, metrics in patterns.items():
            for metric_name, pattern_results in metrics.items():
                ...
                group_data = data[
                    (data[group_by] == group_name) & (data["metric_name"] == metric_name)
                ]
                ...
                ax_ts = axes[0, 0]
                ax_ts.plot(
                    group_data["timestamp"], group_data["metric_value"], "b-", label="Actual"
                )
                ...
                if "trend" in pattern_results and pattern_results["trend"]["detected"]:
                    ...
                    if (
                        "slope" in pattern_results["trend"]
                        and "intercept" in pattern_results["trend"]
                    ):
                ...
                if "outliers" in pattern_results and pattern_results["outliers"]["detected"]:
                    ...
                    outlier_timestamps = [
                        datetime.strptime(o["timestamp"], "%Y-%m-%d %H:%M:%S") for o in outliers
                    ]
                ...
                ax_seas = axes[1, 0]
                if "seasonality" in pattern_results and pattern_results["seasonality"].get(
                    "detected", False
                ):
                ...
                if "forecast" in pattern_results and pattern_results["forecast"].get(
                    "success", False
                ):
    ...
    def compare_test_runs(
        ...
        try:
            ...
            comparison_results = {"metadata": run_metadata, "metrics": {}}
    ...
    def _visualize_comparison(self, comparison_results: Dict, output_path: Optional[str] = None):
        """
        ...
        # Extract metadata
        run1_name = comparison_results["metadata"]["run1"].get(
            "run_name", f"Run {comparison_results['metadata']['run1']['run_id']}"
        )
        run2_name = comparison_results["metadata"]["run2"].get(
            "run_name", f"Run {comparison_results['metadata']['run2']['run_id']}"
        )
            ...
            for metric in metrics:
                ...
                if len(time_series) >= 5 and SCIPY_AVAILABLE:
                    ...
                    time_values = np.array(
                        [(ts - time_series.index[0]).total_seconds() for ts in time_series.index]
                    )
    ...
    def _visualize_predictions(
        ...
        for metric, prediction in prediction_result["predictions"].items():
            ...
            ax.plot(
                prediction_timestamp, predicted_value, "ro", markersize=10, label="Predicted Value"
            )
            ...
            if "trend_analysis" in prediction:
                ...
                time_values = np.array(
                    [
                        (ts - metric_data["timestamp"].min()).total_seconds()
                ...
                # Plot trend line
                ax.plot(metric_data["timestamp"], trend_line, "r--", label="Trend Line")
    ...
    def benchmark_comparison(
        ...
        try:
            ...
            benchmark_result = {
                "metrics": {},
                "rankings": {},
                "overall_ranking": overall_rankings.to_dict()
                if overall_rankings is not None
                else None,
                "metadata": {
            ...
            # Generate visualization if requested
            if visualize and PLOTTING_AVAILABLE:
                self._visualize_benchmark_comparison(
                    pivot_metrics, rankings, overall_rankings, output_path
                )
    ...
    def _visualize_benchmark_comparison(
        self,
        ...
        for metric, pivot in pivot_metrics.items():
            ...
            # Create heatmap
            sns.heatmap(pivot, annot=True, cmap="YlGnBu", linewidths=0.5, ax=ax)
        ...
        for metric, ranking in rankings.items():
            ...
            sns.heatmap(ranking, annot=True, cmap="coolwarm_r", linewidths=0.5, ax=ax)
        ...
        if overall_rankings is not None:
            ...
            # Create heatmap
            sns.heatmap(overall_rankings, annot=True, cmap="coolwarm_r", linewidths=0.5, ax=ax)
...
if __name__ == "__main__":
    ...
    try:
        ...
        comparison = analyzer.compare_test_runs(
            run_id1="run1", run_id2="run2", visualize=True, output_path="comparison.png"
        )
