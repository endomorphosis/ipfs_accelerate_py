...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("result_aggregator_integration.log")],
)
...
class ResultAggregatorIntegration:
    ...
    def __init__(
        ...
        self.service = ResultAggregatorService(
            db_path=db_path, enable_ml=enable_ml, enable_visualization=enable_visualization
        )
        ...
        self.analysis_interval = analysis_interval or timedelta(
            minutes=5
        ...
        try:
            ...
            if plugin_manager and hasattr(plugin_manager, "register_hook"):
                ...
                try:
                    ...
                    logger.info(
                        f"Plugin architecture not available ({e}); falling back to method patching"
                    )
                    plugin_manager = None

            if plugin_manager and hasattr(plugin_manager, "register_hook"):
                # Register for task completion
                plugin_manager.register_hook(HookType.TASK_COMPLETED, self._hook_task_completed)

                # Register for task failure
                plugin_manager.register_hook(HookType.TASK_FAILED, self._hook_task_failed)

                # Register for periodic analysis
                if hasattr(HookType, "PERIODIC"):
                    plugin_manager.register_hook(HookType.PERIODIC, self._hook_periodic_analysis)
                else:
                    logger.info(
                        "HookType.PERIODIC not available; skipping periodic hook registration"
                    )
    ...
    def _wrap_task_completed(self, original_method: Callable) -> Callable:
        ...
        async def wrapped(
            task_id: str, worker_id: str, result: Dict[str, Any], execution_time: float
        ):
            ...
            try:
                ...
                if result_id > 0:
                    logger.debug(
                        f"Stored completed task {task_id} in result aggregator with ID {result_id}"
                    )
    ...
    def _wrap_task_failed(self, original_method: Callable) -> Callable:
        ...
        async def wrapped(task_id: str, worker_id: str, error: str, execution_time: float):
            ...
            try:
                ...
                # Add metrics
                metrics = {"execution_time": execution_time, "error_occurred": 1.0}
                ...
                if result_id > 0:
                    logger.debug(
                        f"Stored failed task {task_id} in result aggregator with ID {result_id}"
                    )
    ...
    async def _hook_task_completed(
        ...
        try:
            ...
            for metric, trend_data in trends.items():
                if (
                    ...
                    and abs(trend_data["percent_change"]) > 10
                ):
            ...
            if significant_trends:
                ...
                if self.enable_notifications:
                    for trend in significant_trends:
                        ...
                        message = (
                            f"{trend['metric']} {trend['trend']} by {abs(trend['percent_change']):.1f}% "
    ...
    async def _analyze_result(self, result_id: int, result: Dict[str, Any]):
        ...
        try:
            ...
            if anomalies:
                logger.info(
                    f"Detected {len(anomalies)} anomalies in real-time for result {result_id}"
                )
