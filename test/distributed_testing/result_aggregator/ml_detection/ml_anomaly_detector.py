...
class MLAnomalyDetector:
    ...
    def __init__(self, db_path: str = None, connection=None):
        ...
        if not SKLEARN_AVAILABLE:
            _log_optional_dependency(
                "Scikit-learn not available. ML-based anomaly detection will be disabled."
            )
            _log_optional_dependency("Install scikit-learn using: pip install scikit-learn")
        if not SCIPY_AVAILABLE:
            _log_optional_dependency(
                "SciPy not available. Some statistical functions will be limited."
            )
        if not PLOTTING_AVAILABLE:
            _log_optional_dependency(
                "Matplotlib/Seaborn not available. Visualization will be disabled."
            )
    ...
    def detect_anomalies(
        ...
        try:
            ...
            results["test_result_id"] = (
                ...
                + "_"
                + results["batch_size"].astype(str)
                + "_"
                + results["test_name"]
                + "_"
                + results["timestamp"].dt.strftime("%Y%m%d%H%M%S")
                + "_"
            ...
            pivot = results.pivot(
                index="test_result_id", columns="metric_name", values="metric_value"
            )
            ...
            for idx in anomaly_indices:
                ...
                sorted_metrics = sorted(metric_z_scores.items(), key=lambda x: x[1], reverse=True)
            ...
            result = {
                ...
                "anomaly_percentage": len(anomaly_indices) / len(pivot) * 100
                if len(pivot) > 0
                else 0,
    ...
    def _feature_engineering(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "latency" in data.columns and "throughput" in data.columns:
            enhanced["derived_efficiency"] = 1 / (
                data["latency"] * (1 / data["throughput"] + 1e-10) + 1e-10
            )

        if "memory_usage" in data.columns and "throughput" in data.columns:
            enhanced["derived_memory_efficiency"] = data["throughput"] / (
                data["memory_usage"] + 1e-10
            )
    ...
    def _visualize_anomalies(
        ...
        if data.shape[1] > 2:
            ...
            plt.scatter(
                normal["PC1"], normal["PC2"], c="blue", marker="o", alpha=0.5, label="Normal"
            )

            # Plot anomalies
            anomalies = pca_df[pca_df["anomaly"] == 1]
            plt.scatter(
                anomalies["PC1"], anomalies["PC2"], c="red", marker="x", s=100, label="Anomaly"
            )
            ...
            feature_weights = pd.DataFrame(
                pca.components_.T, columns=["PC1", "PC2"], index=data.columns
            )
            ...
            pc1_text = "PC1 top features:\n" + "\n".join(
                [f"{feat}: {feature_weights.loc[feat, 'PC1']:.3f}" for feat in top_pc1.index]
            )

            pc2_text = "PC2 top features:\n" + "\n".join(
                [f"{feat}: {feature_weights.loc[feat, 'PC2']:.3f}" for feat in top_pc2.index]
            )
            ...
            if data.shape[1] == 1:
                ...
                plt.title(
                    f"Anomaly Detection for {feature} using {method.replace('_', ' ').title()}"
                )
                plt.ylabel(feature)
                ...
                plt.scatter(
                    normal[feat1], normal[feat2], c="blue", marker="o", alpha=0.5, label="Normal"
                )

                # Plot anomalies
                anomalies = viz_data[viz_data["anomaly"] == 1]
                plt.scatter(
                    anomalies[feat1], anomalies[feat2], c="red", marker="x", s=100, label="Anomaly"
                )
        ...
        if output_path:
            ...
            file_path = (
                os.path.join(output_path, file_name) if os.path.isdir(output_path) else output_path
            )
    ...
    def _visualize_feature_distributions(
        self,
        data: pd.DataFrame,
        anomaly_indices: np.ndarray,
        method: str,
        ...
        fig.suptitle(
            f"Feature Distributions with Anomalies ({method.replace('_', ' ').title()})",
            fontsize=16,
        )
    ...
    def train_anomaly_model(
        ...
        try:
            ...
            if results.empty:
                ...
                logger.warning(msg)
                return {"status": "error", "message": msg, "model_info": None}
            ...
            pivot = results.pivot_table(
                index=["result_id", "timestamp"], columns="metric_name", values="metric_value"
            ).reset_index()
    ...
    def predict_anomalies(
        ...
        if model_info is None:
            raise ValueError(
                f"No trained model found for {model_name} on {hardware_type} using {method}"
            )
    ...
    def load_model(self, model_path: str) -> Dict:
        ...
        try:
            ...
            # Store the model in memory
            model_key = (
                f"{model_info['model_name']}_{model_info['hardware_type']}_{model_info['method']}"
            )
            self._trained_models[model_key] = model_info
    ...
    def evaluate_model(
        ...
        try:
            ...
            if results.empty:
                msg = "No data found for the specified criteria"
                logger.warning(msg)
                return {"status": "error", "message": msg, "evaluation": None}
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                data, y_true, test_size=test_size, random_state=42, stratify=y_true
            )
            ...
            # Create the result
            result = {"status": "success", "evaluation": evaluation}
    ...
    def delete_model(self, model_name: str, hardware_type: str, method: str) -> Dict:
        ...
        return {"status": "success", "message": f"Model {model_key} deleted from memory"}
