...
class DataSource:
    ...
    def get_data(
        self, custom_query: Optional[str] = None, query_params: Optional[List[Any]] = None
    ) -> pd.DataFrame:
        """
...
class PipelineManager:
    ...
    def create_pipeline(
        ...
        name: str,
        transforms: List[Transform],
        custom_data_source: Optional[DataSource] = None,
    ) -> ProcessingPipeline:
        """
        ...
        pipeline = ProcessingPipeline(name=name, data_source=data_source, transforms=transforms)
    ...
    def create_pipeline_from_config(self, config: Dict) -> ProcessingPipeline:
        ...
        for transform_config in config.get("transforms", []):
            ...
            try:
                ...
                transform = get_transform_by_name(
                    transform_type, **transform_config.get("params", {})
                )
    ...
    def save_pipeline_config(self, name: str, file_path: str) -> bool:
        ...
        config = {"name": pipeline.name, "transforms": []}

        # Add transforms
        for transform in pipeline.transforms:
            transform_config = {"type": transform.__class__.__name__}
            ...
            try:
                ...
                for param_name, param in signature.parameters.items():
                    if param_name not in ["self", "args", "kwargs"] and hasattr(
                        transform, param_name
                    ):
                ...
                logger.warning(
                    f"Could not extract parameters for transform {transform.__class__.__name__}: {str(e)}"
                )
    ...
    try:
        ...
        manager.create_pipeline(
            ...
            transforms=[
                TimeWindowTransform(days=30),
                AggregateTransform(
                    group_by=["model_name", "hardware_type"], metrics=["latency", "throughput"]
                ),
            ],
        )
