...
class FilterTransform(Transform):
    ...
    def __init__(self, **filter_criteria: Any):
        """
    ...
    def _apply_comparison_filter(
        self, data: pd.DataFrame, column: str, filter_value: Tuple
    ) -> pd.DataFrame:
        """
...
class TimeWindowTransform(Transform):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.timestamp_column not in data.columns:
            logger.warning(
                f"Column '{self.timestamp_column}' not found in data, returning original data"
            )
            return data
        ...
        logger.info(
            f"Time window filter {time_range} reduced data from {initial_count} to {filtered_count} rows"
        )
...
class PivotTransform(Transform):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Pivoted data: {len(data)} rows → {len(result)} rows with {len(pivot_table.columns)} columns"
        )
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.group_by is None:
            self.group_by = [
                ...
                if col not in [self.metric_name_column, self.metric_value_column]
            ]
        ...
        try:
            ...
            logger.info(
                f"Pivoted metrics: {data[self.metric_name_column].nunique()} unique metrics as columns"
            )
...
class ZScoreOutlierTransform(Transform):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.remove_outliers:
            ...
            logger.info(
                f"Removed {outlier_count} outliers ({outlier_count / len(data):.1%} of data)"
            )
...
class NormalizationTransform(Transform):
    ...
    def __init__(
        ...
        if self.method not in valid_methods:
            raise ValueError(
                f"Invalid normalization method: {method}. Valid methods: {', '.join(valid_methods)}"
            )
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in valid_columns:
            ...
            if self.method == "min-max":
                ...
                if max_val == min_val:
                    logger.warning(
                        f"Column '{col}' has equal min and max values, setting to constant"
                    )
                ...
                if std == 0 or pd.isna(std):
                    logger.warning(
                        f"Column '{col}' has zero or NaN standard deviation, setting to constant"
                    )
                ...
                if max_val == min_val:
                    logger.warning(
                        f"Column '{col}' has equal min and max values, setting to middle of target range"
                    )
...
class MergeTransform(Transform):
    ...
    def __init__(
        ...
        if self.how not in valid_join_types:
            raise ValueError(
                f"Invalid join type: {how}. Valid types: {', '.join(valid_join_types)}"
            )
...
class SortTransform(Transform):
    ...
    def __init__(self, by: Union[str, List[str]], ascending: Union[bool, List[bool]] = True):
        ...
        if isinstance(ascending, bool):
            ...
            if len(self.ascending) != len(self.by):
                logger.warning(
                    f"Length mismatch between 'by' ({len(self.by)}) and 'ascending' ({len(self.ascending)})"
                )
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_columns:
            ...
            valid_ascending = [
                self.ascending[i] for i, col in enumerate(self.by) if col in data.columns
            ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        norm_transform = NormalizationTransform(columns=["latency", "throughput"], method="min-max")
        ...
        # Apply SortTransform
        sort_transform = SortTransform(by=["model_name", "throughput"], ascending=[True, False])
