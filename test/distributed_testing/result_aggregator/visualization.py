...
class ResultVisualizer:
    ...
    def _check_visualization_libraries(self, interactive: bool = False) -> bool:
        ...
        if interactive and not self.enable_plotly:
            logger.warning(
                "Interactive visualizations require Plotly. Install with 'pip install plotly'."
            )
            return False

        if not interactive and not self.enable_matplotlib:
            logger.warning(
                "Static visualizations require Matplotlib. Install with 'pip install matplotlib seaborn'."
            )
            return False
    ...
    def generate_performance_chart(
        ...
        for result in results:
            ...
            # Extract all metrics
            row = {"timestamp": timestamp, "task_type": task_type, "worker_id": worker_id}
        ...
        available_metrics = [
            col for col in df.columns if col not in ["timestamp", "task_type", "worker_id"]
        ]
        ...
        if metrics:
            ...
            if not plot_metrics:
                logger.warning(
                    f"None of the specified metrics {metrics} are available. Available metrics: {available_metrics}"
                )
                return None
        else:
    ...
    def _generate_interactive_trend_analysis(self, trends: Dict[str, Dict[str, Any]]) -> Any:
        ...
        colors = {"increasing": "green", "decreasing": "red", "stable": "blue", "unknown": "gray"}
    ...
    def _generate_static_trend_analysis(self, trends: Dict[str, Dict[str, Any]]) -> Any:
        ...
        # Create a DataFrame for easier plotting
        df = pd.DataFrame(
            {"metric": metrics, "percent_change": percent_changes, "trend": trend_types}
        )
        ...
        # Create bar chart
        bars = ax.bar(df["metric"], df["percent_change"], color=[colors[t] for t in df["trend"]])
        ...
        legend_elements = [
            Patch(facecolor=color, label=trend)
            for trend, color in colors.items()
            if trend in df["trend"].values
        ]
    ...
    def generate_anomaly_dashboard(
        self, filter_criteria: Dict[str, Any] = None, output_path: str = None
    ) -> Any:
        """
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "bar"}, {"type": "scatter"}], [{"type": "box"}, {"type": "pie"}]],
        )
        ...
        fig.add_trace(go.Box(x=df["metric"], y=df["value"], name="Anomalous Values"), row=2, col=1)

        # 4. Anomaly Types Distribution (Pie chart)
        type_counts = df["anomaly_type"].value_counts()
        fig.add_trace(
            go.Pie(labels=type_counts.index, values=type_counts.values, name="Anomaly Types"),
            row=2,
            col=2,
        )
    ...
    def generate_summary_dashboard(
        ...
        numeric_metrics = [
            ...
            if col not in ["timestamp", "task_type", "worker_id", "status"]
        ...
        fig.add_trace(
            go.Pie(labels=type_counts.index, values=type_counts.values, name="Task Types"),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Pie(labels=status_counts.index, values=status_counts.values, name="Task Status"),
            row=1,
            col=2,
        )
        ...
        fig.add_trace(
            go.Bar(x=worker_counts["worker_id"], y=worker_counts["count"], name="Worker Tasks"),
            row=2,
            col=1,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", default="./visualizations", help="Directory to save visualizations"
    )
    parser.add_argument(
        "--type",
        choices=["performance", "trend", "anomaly", "summary"],
        default="performance",
    ...
    if args.type == "performance":
        output_path = os.path.join(
            args.output_dir,
            "performance_chart.html" if args.interactive else "performance_chart.png",
        )
        visualizer.generate_performance_chart(
            metrics=args.metrics, output_path=output_path, interactive=args.interactive
        )
        print(f"Generated performance chart: {output_path}")

    elif args.type == "trend":
        output_path = os.path.join(
            args.output_dir, "trend_analysis.html" if args.interactive else "trend_analysis.png"
        )
        ...
        visualizer.generate_anomaly_dashboard(output_path=output_path)
        print(f"Generated anomaly dashboard: {output_path}")

    elif args.type == "summary":
        output_path = os.path.join(args.output_dir, "summary_dashboard.html")
        visualizer.generate_summary_dashboard(output_path=output_path)
