...
class TestResultAggregatorService(unittest.TestCase):
    ...
    def test_store_result(self):
        ...
        result = {
            ...
            "status": "completed",
            "duration": 10.5,
            "metrics": {"throughput": 120.5, "latency": 5.2, "memory_usage": 1024.0},
            "details": {"model": "test_model", "batch_size": 8, "precision": "fp16"},
        }
        ...
        self.assertAlmostEqual(
            stored_result["metrics"]["throughput"], result["metrics"]["throughput"], places=6
        )
        self.assertAlmostEqual(
            stored_result["metrics"]["latency"], result["metrics"]["latency"], places=6
        )
        self.assertAlmostEqual(
            stored_result["metrics"]["memory_usage"], result["metrics"]["memory_usage"], places=6
        )
    ...
    def test_get_results(self):
        ...
        benchmark_results = self.service.get_results(filter_criteria={"test_type": "benchmark"})
        self.assertEqual(len(benchmark_results), 5)
        for result in benchmark_results:
            self.assertEqual(result["type"], "benchmark")

        # Test filtering by worker_id
        worker_results = self.service.get_results(filter_criteria={"worker_id": "test_worker_0"})
        for result in worker_results:
            self.assertEqual(result["worker_id"], "test_worker_0")

        # Test filtering by status
        completed_results = self.service.get_results(filter_criteria={"status": "completed"})
    ...
    def test_aggregated_results(self):
        ...
        aggregated = self.service.get_aggregated_results(aggregation_type="mean")
        ...
        grouped = self.service.get_aggregated_results(
            aggregation_type="mean", group_by=["worker_id"]
        )
    ...
    def test_performance_trends(self):
        ...
        for i in range(10):
            ...
            result = {
                ...
                "status": "completed",
                "duration": 10.0,
                "metrics": {"throughput": throughput, "latency": latency, "memory_usage": memory},
                "details": {"model": "trend_model", "batch_size": 8, "precision": "fp16"},
            }
            self.service.store_result(result)
    ...
    def test_report_generation(self):
        ...
        for i in range(5):
            result = {
                ...
                "details": {"model": "report_model", "batch_size": 8, "precision": "fp16"},
            }
            self.service.store_result(result)

        # Generate a performance report in Markdown format
        report = self.service.generate_analysis_report(report_type="performance", format="markdown")
        ...
        html_report = self.service.generate_analysis_report(
            report_type="performance", format="html"
        )
    ...
    def test_anomaly_detection(self):
        ...
        for i in range(15):
            result = {
                ...
                "details": {"model": "anomaly_model", "batch_size": 8, "precision": "fp16"},
            }
            self.service.store_result(result)
        ...
        self.assertTrue(
            any(
                feature in detected_features
                for feature in ["throughput", "latency", "memory_usage", "duration"]
            )
        )
...
class TestResultAggregatorIntegration(unittest.TestCase):
    ...
    async def _test_task_completed_hook(self, mock_store_result):
        ...
        result = {
            "status": "success",
            "metrics": {"throughput": 100.0, "latency": 10.0, "memory_usage": 1000.0},
        }
    ...
    @patch(
        "result_aggregator.coordinator_integration.ResultAggregatorIntegration._send_notification"
    )
    ...
    def test_periodic_analysis(
        self, mock_analyze_trends, mock_detect_anomalies, mock_send_notification
    ):
        """Test periodic analysis."""
        anyio.run(
            self._test_periodic_analysis,
            mock_analyze_trends,
            mock_detect_anomalies,
            mock_send_notification,
        )

    async def _test_periodic_analysis(
        self, mock_analyze_trends, mock_detect_anomalies, mock_send_notification
    ):
        # Setup mocks
        mock_analyze_trends.return_value = {
            ...
            "latency": {"trend": "stable", "percent_change": 0.5, "statistics": {"mean": 10.0}},
        }
