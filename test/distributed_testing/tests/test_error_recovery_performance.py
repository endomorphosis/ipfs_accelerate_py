...
class MockEnhancedErrorRecoveryManager(EnhancedErrorRecoveryManager):
    ...
    def __init__(self, coordinator=None):
        ...
        self.strategies["worker_restart"] = MockRecoveryStrategy(
            "worker_restart", "medium", True, 0.2
        )
        self.strategies["db_reconnect"] = MockRecoveryStrategy("db_reconnect", "medium", True, 0.3)
        self.strategies["coordinator_restart"] = MockRecoveryStrategy(
            "coordinator_restart", "high", True, 0.4
        )
        self.strategies["system_recovery"] = MockRecoveryStrategy(
            "system_recovery", "critical", True, 0.5
        )
...
class TestPerformanceBasedErrorRecovery(unittest.TestCase):
    ...
    async def _test_recover_success(self):
        ...
        error_report = self.error_handler.create_error_report(
            Exception("Test error"), {"error_type": ErrorType.NETWORK, "component": "test"}
        )
