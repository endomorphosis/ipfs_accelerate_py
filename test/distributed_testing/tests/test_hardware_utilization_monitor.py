...
class TestCoordinatorHardwareMonitoringIntegration(unittest.TestCase):
    ...
    def test_hardware_aware_find_best_worker(self):
        ...
        task = {"task_id": "test-task", "type": "benchmark", "requirements": {"hardware": ["gpu"]}}
        ...
        self.integration._hardware_aware_find_best_worker = lambda *args, **kwargs: (
            patched_find_best_worker(self, *args, **kwargs)
        )

        try:
            # Call the patched method
            result = self.integration._hardware_aware_find_best_worker(
...
class TestHardwareMonitoringEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_demo(self):
        ...
        with (
            tempfile.NamedTemporaryFile(suffix=".duckdb") as db_file,
