...
class TestCIClientImplementations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.configs = {
            ...
            "gitlab": {"token": "fake_gitlab_token", "project_id": "12345", "ref": "main"},
            "jenkins": {
    ...
    async def _test_client_implementation(self, provider_type: str, config: Dict[str, Any]):
        ...
        try:
            ...
            result = await client.update_test_run(
                test_run_id,
                {
                    "status": "running",
                    "summary": {
            ...
            result = await client.upload_artifact(
                test_run_id, str(self.artifact_path), "test_artifact.txt"
            )
    ...
    def test_all_client_implementations(self):
        ...
        async def run_all_tests():
            ...
            for provider_type, config in self.configs.items():
                results[provider_type] = await self._test_client_implementation(
