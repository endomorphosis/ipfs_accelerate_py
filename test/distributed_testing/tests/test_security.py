...
class TestSecurityManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.security = SecurityManager(secret_key=self.secret_key, token_expiry=60)
    ...
    def test_sign_and_verify_message(self):
        ...
        message = {"type": "test", "id": "123", "data": {"field1": "value1", "field2": 42}}
    ...
    def test_save_and_load_config(self):
        ...
        security = SecurityManager(
            secret_key="test_save_key", token_expiry=300, required_roles=["worker", "admin"]
        )
        ...
        try:
            ...
            for api_key in api_keys:
                if loaded_security.api_keys[api_key]["name"] == "key1":
                    ...
                    self.assertEqual(
                        loaded_security.api_keys[api_key]["roles"], ["worker", "admin"]
                    )
...
class TestAuthMiddleware:
    ...
    async def test_auth_middleware_api_key(self, setup_app):
        ...
        request = make_mocked_request("GET", "/protected", headers={"X-API-Key": api_key})
        response = await middleware_handler(request)
        ...
        # Test with invalid API key
        request = make_mocked_request("GET", "/protected", headers={"X-API-Key": "invalid_key"})
        response = await middleware_handler(request)
    ...
    async def test_auth_middleware_token(self, setup_app):
        ...
        request = make_mocked_request(
            "GET", "/protected", headers={"Authorization": f"Bearer {token}"}
        )
        response = await middleware_handler(request)
        ...
        request = make_mocked_request(
            "GET", "/protected", headers={"Authorization": "Bearer invalid_token"}
        )
        response = await middleware_handler(request)
