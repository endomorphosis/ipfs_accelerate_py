...
def test_error_aggregation(error_handler):
    ...
    error1 = error_handler.create_error_report(
        ValueError("Common error"), {"component": "component1", "operation": "operation1"}
    )
    ...
    error3 = error_handler.create_error_report(
        ValueError("Different message"), {"component": "component1", "operation": "operation1"}
    )
...
async def test_retry_operation_success(error_handler):
    ...
    result, error_report = await error_handler.retry_operation(
        flaky_function, context={"component": "test_component", "operation": "flaky_operation"}
    )
...
async def test_retry_operation_failure(error_handler):
    ...
    # Execute with retry
    result, error_report = await error_handler.retry_operation(
        failing_function, context={"component": "test_component", "operation": "failing_operation"}
    )
...
async def test_safe_execute_async(error_handler):
    ...
    result = await safe_execute_async(
        error_func, 2, 3, error_handler=error_handler, component="test"
    )
    assert result is None
...
def test_resolve_error(error_handler, mock_exception):
    ...
    assert (
        updated_report.recovery_details["manual_resolution"]["details"]["notes"] == "Fixed by test"
    )
