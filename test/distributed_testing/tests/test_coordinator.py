...
class TestDistributedTestingCoordinator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock database
        with (
            patch("distributed_testing.coordinator.duckdb.connect") as mock_duckdb_connect,
            ...
            self.coordinator = DistributedTestingCoordinator(
                db_path=self.db_path, host="localhost", port=8080
            )
...
async def coordinator_setup():
    ...
    with (
        patch("distributed_testing.coordinator.duckdb.connect"),
        patch("distributed_testing.coordinator.SecurityManager") as mock_security_manager,
        ...
        # Create coordinator instance
        coordinator = DistributedTestingCoordinator(
            db_path="test_db.duckdb", host="localhost", port=8080
        )
...
class TestCoordinatorAsync:
    ...
    async def test_handle_worker_registration(self, coordinator_setup):
        ...
        with patch.object(coordinator, "_send_response", AsyncMock()):
            ...
            await coordinator._handle_worker_registration(
                mock_ws,
                {
                    "type": "register",
                    "worker_id": "test_worker_id",
    ...
    async def test_handle_worker_heartbeat(self, coordinator_setup):
        ...
        heartbeat = {
            ...
            "health_status": {
                "is_healthy": True,
                "health_metrics": {"resource_healthy": True, "connection_healthy": True},
            },
        }
    ...
    async def test_handle_task_result(self, coordinator_setup):
        ...
        task = {
            ...
            "status": "running",
            "worker_id": worker_id,
            "config": {"model": "test_model", "batch_sizes": [1, 2, 4]},
        ...
        result = {
            ...
            "hardware_metrics": {"cpu_percent": 50, "memory_percent": 60},
            "result": {
                "model": "test_model",
                "precision": "fp32",
                "iterations": 3,
        ...
        with (
            patch.object(coordinator, "_send_response", AsyncMock()),
    ...
    async def test_handle_task_error(self, coordinator_setup):
        ...
        task = {
            ...
            "status": "running",
            "worker_id": worker_id,
            "config": {"test_file": "test_worker.py"},
            "created": datetime.now().isoformat(),
            "started": datetime.now().isoformat(),
        }
        coordinator.tasks[task_id] = task
    ...
    async def test_authenticate_worker_token(self, coordinator_setup):
        ...
        # Create auth message
        auth_message = {"type": "auth", "auth_type": "token", "token": "test_token"}
    ...
    async def test_find_worker_for_task(self, coordinator_setup):
        ...
        coordinator.workers = {
            "worker1": {
                "worker_id": "worker1",
                "status": "idle",
                "capabilities": {"hardware": ["cpu"], "cpu": {"cores": 4}},
                "connected": True,
        ...
        cpu_task = {
            "task_id": "cpu_task",
            "type": "benchmark",
            "config": {"model": "model1", "hardware": "cpu"},
        }
        ...
        gpu_task = {
            "task_id": "gpu_task",
            "type": "benchmark",
            "config": {"model": "model2", "hardware": "cuda"},
        }
        ...
        with patch.object(
            ...
            side_effect=lambda task, workers: (
                "worker2" if "cuda" in task["config"]["hardware"] else "worker1"
            ),
        ):
