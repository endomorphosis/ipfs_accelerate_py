...
class FailoverSimulator:
    ...
    def __init__(self, node_count=3, base_port=8080):
        ...
        self.db_paths = [
            os.path.join(temp_dir, "coordinator.duckdb") for temp_dir in self.temp_dirs
        ]
    ...
    async def start_cluster(self):
        ...
        for i in range(self.node_count):
            ...
            peers = ",".join(
                [f"localhost:{self.base_port + j}" for j in range(self.node_count) if j != i]
            )
    ...
    def restart_node(self, node_index):
        ...
        if 0 <= node_index < len(self.processes):
            ...
            cmd = [
                ...
                str(port),
                "--db-path",
                self.db_paths[node_index],
                "--data-dir",
                self.temp_dirs[node_index],
                "--enable-redundancy",
                "--peers",
                peers,
                "--log-level",
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        url, json={"worker_id": worker_id, "host": host, "port": port}
                    ) as response:
                        if response.status == 200:
...
class TestCoordinatorFailover(unittest.TestCase):
    ...
    def test_basic_failover(self):
        ...
        self.assertIn(
            "worker-1", workers_after, "Worker should still be in the list after failover"
        )
    ...
    def test_leader_rejoin(self):
        ...
        # Add some state
        self.event_loop.run_until_complete(self.simulator.add_worker("worker-1", "test-host", 9001))
        ...
        restarted_node = next(
            (
                node
                for node in status
                if node["node_id"] == f"node-{leader_index + 1}" and node["status"] == "running"
            ),
            None,
        )

        self.assertIsNotNone(restarted_node, "Restarted node should be running")
        self.assertEqual(restarted_node["role"], "FOLLOWER", "Restarted node should be a follower")
        ...
        for i in range(3):
            if i == leader_index:
                ...
                try:
                    ...
                    response_data = self.event_loop.run_until_complete(self._get_request(url))
                    self.assertIn(
                        "worker-1", response_data, "worker-1 should be in restarted node's state"
                    )
                    self.assertIn(
                        "worker-2", response_data, "worker-2 should be in restarted node's state"
                    )
                except Exception as e:
