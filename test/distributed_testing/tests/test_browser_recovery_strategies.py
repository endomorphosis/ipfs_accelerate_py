...
from ..browser_recovery_strategies import (
    ...
    SettingsAdjustmentStrategy,
    BrowserFallbackStrategy,
    SimulationFallbackStrategy,
    ModelSpecificRecoveryStrategy,
...
class TestSimpleRetryStrategy(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test environment."""
        self.strategy = SimpleRetryStrategy(
            BrowserType.CHROME, ModelType.TEXT, retry_delay=0.1, max_retries=3
        )
    ...
    async def async_test_execute_impl_failure(self):
        ...
        failure_info = {"operation": operation_mock, "args": [], "kwargs": {}, "retry_count": 0}
...
class TestBrowserRestartStrategy(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test environment."""
        self.strategy = BrowserRestartStrategy(
            BrowserType.CHROME, ModelType.TEXT, cooldown_period=0.1
        )
...
class TestBrowserFallbackStrategy(unittest.TestCase):
    ...
    async def async_test_execute_impl_custom_fallback_order(self):
        ...
        strategy = BrowserFallbackStrategy(
            BrowserType.CHROME, ModelType.TEXT, fallback_order=custom_fallback_order
        )
...
class TestModelSpecificRecoveryStrategy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.multimodal_strategy = ModelSpecificRecoveryStrategy(
            BrowserType.CHROME, ModelType.MULTIMODAL
        )
...
class TestProgressiveRecoveryManager(unittest.TestCase):
    ...
    async def async_test_execute_progressive_recovery_success(self):
        ...
        with patch.object(
            SimpleRetryStrategy, "_execute_impl", new_callable=AsyncMock, return_value=True
        ):
    ...
    async def async_test_execute_progressive_recovery_multiple_attempts(self):
        ...
        with (
            patch.object(
                SimpleRetryStrategy, "_execute_impl", new_callable=AsyncMock, return_value=False
            ),
            patch.object(
                BrowserRestartStrategy, "_execute_impl", new_callable=AsyncMock, return_value=False
            ),
            patch.object(
                SettingsAdjustmentStrategy,
                "_execute_impl",
    ...
    async def async_test_execute_progressive_recovery_all_fail(self):
        ...
        with (
            ...
            patch.object(
                BrowserFallbackStrategy, "_execute_impl", new_callable=AsyncMock, return_value=False
            ),
            patch.object(
                SimulationFallbackStrategy,
                "_execute_impl",
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_detect_model_type(self):
        ...
        self.assertEqual(
            detect_model_type("clip-vit-base"), ModelType.VISION
        )  # Should ideally be MULTIMODAL
