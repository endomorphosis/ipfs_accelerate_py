...
class MockCIProvider(CIProviderInterface):
    ...
    async def upload_artifact(self, test_run_id, artifact_path, artifact_name):
        ...
        self.artifacts[test_run_id].append({"path": artifact_path, "name": artifact_name})
        return True
...
class TestWorkerAutoDiscoveryWithCI(unittest.TestCase):
    ...
    async def _async_set_up(self):
        ...
        self.coordinator_start_patcher = patch.object(
            DistributedTestingCoordinator, "_setup_server", new_callable=AsyncMock
        )
    ...
    async def create_test_workers(self, count=2):
        ...
        for i in range(count):
            ...
            capabilities = {"hardware": ["cpu"], "memory_gb": 8 + (i * 8), "models": ["bert", "t5"]}
            ...
            worker._execute_task = AsyncMock(
                return_value={"status": "completed", "result": f"Result from {worker_id}"}
            )
    ...
    async def _test_worker_capability_awareness(self):
        ...
        worker_capabilities = {
            w: self.coordinator.get_worker_capabilities(w)
            for w in self.coordinator.get_registered_workers()
        }
    ...
    async def _test_ci_integration_with_auto_discovery(self, mock_assign_task):
        ...
        mock_assign_task.side_effect = lambda task, worker_id: (
            self.coordinator._mark_task_completed(task["task_id"], {"status": "completed"})
        )
        ...
        with open(worker_info_path, "w") as f:
            json.dump(
                {"workers": [{"id": w.worker_id, "capabilities": w.capabilities} for w in workers]},
                f,
            )
        ...
        artifacts = await self.reporter.collect_and_upload_artifacts(
            test_run_id, [str(worker_info_path)]
        )
        ...
        self.assertTrue(
            any(
                ...
                for name in (f"{test_run_id}_report.markdown", f"{test_run_id}_report.md")
            )
        )
