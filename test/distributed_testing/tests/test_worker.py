...
class TestDistributedTestingWorker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("worker.SecurityManager") as mock_security_manager:
            ...
            self.mock_security.sign_message.side_effect = lambda msg: {
                **msg,
            ...
            self.worker = DistributedTestingWorker(
                coordinator_url=self.coordinator_url, worker_id=self.worker_id, api_key=self.api_key
            )
...
class TestDistributedTestingWorkerAsync:
    ...
    async def worker_setup(self):
        ...
        mock_ws.recv = AsyncMock(
            return_value=json.dumps(
                {"type": "register_response", "worker_id": worker_id, "status": "success"}
            )
        )

        # Create worker with mocked components
        with (
            patch("worker.SecurityManager") as mock_security_manager,
            ...
            mock_security.sign_message.side_effect = lambda msg: {
                **msg,
                "signature": "test_signature",
            }
            ...
            worker = DistributedTestingWorker(
                coordinator_url=coordinator_url, worker_id=worker_id, api_key=api_key
            )
    ...
    async def test_connect_to_coordinator(self, worker_setup):
        ...
        mock_ws.recv.side_effect = [
            ...
            json.dumps(
                {"type": "register_response", "worker_id": worker.worker_id, "status": "success"}
            ),
        ]
    ...
    async def test_authenticate_failure(self, worker_setup):
        ...
        mock_ws.recv.return_value = json.dumps(
            {"type": "auth_response", "status": "failure", "message": "Invalid API key"}
        )
        # Test authentication
        worker.ws_connected = True  # Assume we're connected
        result = await worker._authenticate()
    ...
    async def test_send_heartbeat(self, worker_setup):
        ...
        mock_ws.recv.return_value = json.dumps({"type": "heartbeat_response", "status": "success"})
        worker.ws_connected = True  # Assume we're connected
    ...
    async def test_task_execution_success(self, worker_setup):
        ...
        task = {
            "task_id": "test_task_1",
            "type": "benchmark",
            "config": {"model": "test_model", "precision": "fp32", "iterations": 3},
            "status": "received",
                ...
                AsyncMock(
                    return_value={
                        ...
                        "batch_sizes": {
                            ...
                            "2": {
                                "latency_ms": 15,
                                "throughput_items_per_second": 133,
                                "memory_mb": 1152,
                            },
                            "4": {
                                "latency_ms": 25,
                                "throughput_items_per_second": 160,
                                "memory_mb": 1280,
                            },
                        },
                    }
    ...
    async def test_task_execution_failure(self, worker_setup):
        ...
        with (
            patch.object(
                worker, "_execute_test_task", AsyncMock(side_effect=Exception("Test failed"))
            ),
    ...
    async def test_check_health(self, worker_setup):
        ...
        with (
            patch("worker.psutil.cpu_percent", return_value=50),
        ...
        with (
            patch("worker.psutil.cpu_percent", return_value=96),
            patch("worker.psutil.virtual_memory", return_value=MagicMock(percent=60)),
        ):
            result = await worker.check_health()
    ...
    async def test_collect_hardware_metrics(self, worker_setup):
        ...
        with (
            ...
            patch("worker.HAS_TORCH", False),
        ):  # Disable GPU metrics for this test
            # Collect metrics
