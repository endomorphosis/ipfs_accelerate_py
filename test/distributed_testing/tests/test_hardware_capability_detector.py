...
class TestHardwareCapabilityDetector(unittest.TestCase):
    ...
    def test_detect_all_capabilities(self, mock_virtual_memory, mock_cpu_count):
        ...
        cpu_capabilities = [
            cap
            for cap in capabilities.hardware_capabilities
    ...
    def test_store_and_retrieve_capabilities(self):
        ...
        capabilities = WorkerHardwareCapabilities(
            ...
            hardware_capabilities=[
                HardwareCapability(
                    ...
                    scores={"compute": CapabilityScore.GOOD, "memory": CapabilityScore.AVERAGE},
                ),
                HardwareCapability(
                    hardware_type=HardwareType.GPU,
                    vendor=HardwareVendor.NVIDIA,
                    ...
                    scores={"compute": CapabilityScore.EXCELLENT, "memory": CapabilityScore.GOOD},
                ),
            ],
        )
        ...
        # Verify CPU capabilities
        cpu_capabilities = [
            cap for cap in retrieved.hardware_capabilities if cap.hardware_type == HardwareType.CPU
        ]
        self.assertEqual(len(cpu_capabilities), 1)
        ...
        gpu_capabilities = [
            cap for cap in retrieved.hardware_capabilities if cap.hardware_type == HardwareType.GPU
        ]
    ...
    def test_find_compatible_workers(self):
        ...
        high_memory = self.detector.find_compatible_workers(
            hardware_requirements={"hardware_type": HardwareType.CPU}, min_memory_gb=100.0
        )
        ...
        preferred = self.detector.find_compatible_workers(
            hardware_requirements={}, preferred_hardware_types=[HardwareType.GPU, HardwareType.CPU]
        )
