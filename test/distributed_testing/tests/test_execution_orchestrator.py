...
class ExecutionOrchestratorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dependency_manager.register_test(
            "test3", [Dependency("test1"), Dependency("test2")], ["group2"]
        )
        self.dependency_manager.register_test(
            "test4", [Dependency("group1", is_group=True)], ["group2"]
        )
        self.dependency_manager.register_test(
    ...
    def test_adjust_max_workers(self):
        ...
        for strategy in ExecutionStrategy:
            orchestrator = ExecutionOrchestrator(
                dependency_manager=self.dependency_manager, max_workers=4, strategy=strategy
            )
    ...
    def test_execute_all_tests(self, mock_sleep, mock_executor):
        ...
        with patch.object(
            self.orchestrator, "update_group_status", side_effect=mock_update_group_status
        ):
            # Execute all tests
