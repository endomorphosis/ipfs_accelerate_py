...
class TestPerformanceTrendAnalyzerIntegration(unittest.TestCase):
    ...
    async def _test_analyzer_collects_metrics(self):
        ...
        try:
                            ...
                            "results": [
                                {
                                    ...
                                    "result": {
                                        ...
                                        "batch_sizes": {
                                            ...
                                            "8": {
                                                "latency_ms": 250.0,
                                                "throughput_items_per_second": 32.0,
                                            },
                                        },
                                        "precision": "fp32",
                                        "iterations": 100,
                            ...
                            "workers": [
                                {
                                    ...
                                    "hardware_metrics": {
                                        ...
                                        "memory_percent": 60.0,
                                        "memory_used_gb": 4.5,
                                        "gpu": [{"memory_utilization_percent": 75.0}],
                                    },
                                },
                                {
                                    "id": "worker2",
            ...
            with patch.object(self.analyzer.session, "get", side_effect=mock_get):
                ...
                throughput_metrics = self.analyzer.metrics_cache.get(
    ...
    async def _test_analyzer_detects_anomalies(self):
        ...
        try:
            ...
            throughput_metrics = [
                PerformanceMetric(
                    name="throughput_items_per_second", value=100.0, timestamp=timestamps[0]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=105.0, timestamp=timestamps[1]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=98.0, timestamp=timestamps[2]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=30.0, timestamp=timestamps[3]
                ),  # Anomaly
                PerformanceMetric(
                    name="throughput_items_per_second", value=102.0, timestamp=timestamps[4]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=103.0, timestamp=timestamps[5]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=101.0, timestamp=timestamps[6]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=99.0, timestamp=timestamps[7]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=100.0, timestamp=timestamps[8]
                ),
                PerformanceMetric(
                    name="throughput_items_per_second", value=102.0, timestamp=timestamps[9]
                ),
            ]
    ...
    async def _test_database_integration(self):
        ...
        try:
            ...
            # Check metrics
            result = db.execute(
                "SELECT COUNT(*) FROM performance_metrics WHERE name = 'test_metric'"
            ).fetchone()
            self.assertGreaterEqual(result[0], 2)

            # Check alerts
            result = db.execute(
                "SELECT COUNT(*) FROM performance_alerts WHERE metric_name = 'test_metric'"
            ).fetchone()
            self.assertGreaterEqual(result[0], 1)

            # Check trends
            result = db.execute(
                "SELECT COUNT(*) FROM performance_trends WHERE metric_name = 'test_metric'"
            ).fetchone()
            self.assertGreaterEqual(result[0], 1)

            # Verify metric values
            result = db.execute(
                "SELECT value FROM performance_metrics WHERE name = 'test_metric' ORDER BY value"
            ).fetchall()
            ...
            result = db.execute(
                "SELECT severity FROM performance_alerts WHERE metric_name = 'test_metric'"
            ).fetchone()
            self.assertEqual(result[0], "critical")

            # Verify trend type
            result = db.execute(
                "SELECT trend_type FROM performance_trends WHERE metric_name = 'test_metric'"
            ).fetchone()
...
def run_tests():
    ...
    try:
        ...
        suite.addTest(
            TestPerformanceTrendAnalyzerIntegration("test_analyzer_connects_to_coordinator")
        )
        ...
        suite.addTest(
            TestPerformanceTrendAnalyzerIntegration("test_analyzer_generates_visualizations")
        )
