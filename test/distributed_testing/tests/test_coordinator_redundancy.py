...
class MockCoordinator:
    ...
    def __init__(self, node_id="node-1", host="localhost", port=8080):
        ...
        self.host = host
        self.port = port
        self.state = {"workers": {}, "tasks": {}, "status": {}, "results": {}}
...
class TestRedundancyManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.coordinators = [
            MockCoordinator(f"node-{i + 1}", "localhost", 8080 + i) for i in range(3)
        ]
    ...
    def test_become_leader(self):
        ...
        with (
            patch.object(manager, "_reset_leader_state") as mock_reset,
    ...
    def test_handle_request_vote(self):
        ...
        request = {"term": 0, "candidate_id": "node-2", "last_log_index": 0, "last_log_term": 0}
        ...
        request = {"term": 2, "candidate_id": "node-2", "last_log_index": 0, "last_log_term": 0}
        response = self.event_loop.run_until_complete(manager.handle_request_vote(request))
    ...
    def test_failover(self, mock_sleep):
        ...
        with patch.object(follower1, "http_request", new_callable=AsyncMock) as mock_request:
            ...
            with patch.object(
                follower1, "_start_election", new_callable=AsyncMock
            ) as mock_election:
    ...
    def test_forward_to_leader(self):
        ...
        with patch.object(manager, "http_request", new_callable=AsyncMock) as mock_request:
            ...
            response = self.event_loop.run_until_complete(
                manager._forward_to_leader("/api/workers/register", "POST", request)
            )
