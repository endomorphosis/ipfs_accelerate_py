...
class TestDynamicResourceManagerIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.provider_config_path, "w") as f:
            json.dump({"LOCAL": {"regions": {"local": {"enabled": True}}}}, f)
                ...
                if "/statistics" in url:
                    ...
                    mock_response.json = AsyncMock(
                        return_value={
                            ...
                            "tasks_failed": 0,
                            "tasks_created": 10,
                            "resource_usage": {"cpu_percent": 50.0, "memory_percent": 40.0},
                        }
                    )
    ...
    async def _test_resource_manager_scales_down_with_low_queue(self):
        ...
        try:
            ...
            self.resource_manager.config["min_workers"] = self.resource_manager.config[
                "max_workers"
            ]
    ...
    async def _test_worker_registration_with_coordinator(self):
        ...
        try:
            ...
            with patch(
                "distributed_testing.worker.Worker.start", new_callable=AsyncMock
            ) as mock_start:
                ...
                active_workers_in_coordinator = sum(
                    1
                    for worker in self.coordinator.state.workers.values()
                    if worker.status == "idle"
                )

                active_workers_in_manager = sum(
                    1
                    for r in self.resource_manager.resources.values()
                    if r.state in [ResourceState.RUNNING, ResourceState.INITIALIZING]
                )
                ...
                self.assertGreaterEqual(
                    active_workers_in_coordinator, active_workers_in_manager - 1
                )
                ...
                if "/statistics" in url:
                    ...
                    mock_response.json = AsyncMock(
                        return_value={
                            ...
                            "tasks_failed": 0,
                            "tasks_created": 7,
                            "resource_usage": {"cpu_percent": 60.0, "memory_percent": 40.0},
                        }
                    )
                    return mock_response
                elif "/workers" in url:
                    ...
                    for resource in self.resource_manager.resources.values():
                        if resource.state in [ResourceState.RUNNING, ResourceState.INITIALIZING]:
                            cpu_percent = (
                                98.0 if resource.resource_id == test_worker.resource_id else 40.0
                            )
                            workers.append(
                                {
                                    "id": resource.resource_id,
            ...
            with patch.object(self.resource_manager.session, "get", side_effect=mock_get):
                ...
                if worker_state == ResourceState.ERROR:
                    ...
                    healthy_workers = sum(
                        1
                        for r in self.resource_manager.resources.values()
                        if r.state == ResourceState.RUNNING
                        and r.resource_id != test_worker.resource_id
                    )
                    self.assertGreaterEqual(
                        healthy_workers, self.resource_manager.config["min_workers"]
                    )
        finally:
            await self.async_tearDown()
