    ...
    def get_error_context(self, error_type):
        ...
        contexts = {
            "network": {
                ...
                "related_entities": {"worker_id": "worker-1"},
            },
            "database": {
                "component": "database",
                ...
                "related_entities": {"query": "SELECT * FROM tasks"},
            },
            "worker": {
                "component": "worker",
                ...
                "error_severity": ErrorSeverity.MEDIUM,
                "related_entities": {"worker_id": "worker-2", "task_id": "task-3"},
            },
            "timeout": {
...
async def test_error_resolution(error_handler):
    ...
    resolved = error_handler.resolve_error(
        error_id, "Manual resolution for testing", {"test": True}
    )
    ...
    return {"error_id": error_id, "resolved": resolved, "level_reset": level_reset}
...
async def test_reset_recovery_levels(error_handler):
    ...
    return {"levels_before": levels_before, "levels_after": levels_after, "reset_success": reset}
...
async def run_all_tests(coordinator=None, with_db=True):
    ...
    for error_type in ["network", "database", "worker", "timeout", "system"]:
        results["basic_error_handling"][error_type] = await test_handle_error(
            error_handler, error_type
        )

    # Test persistent error with progressive recovery
    results["progressive_recovery"] = await test_persistent_error(
        error_handler, "network", attempts=3
    )
    ...
    print(
        f"  Recovery Level Escalation: {results['progressive_recovery'][0]['recovery_level']} -> {final_attempt['recovery_level']}"
    )
...
if __name__ == "__main__":
    ...
    if args.json:
        ...
        json_results = {"timestamp": datetime.now().isoformat(), "success": success, "results": {}}
                ...
                for key, value in test_results.items():
                    ...
                    if isinstance(value, dict):
                        ...
                        for inner_key, inner_value in value.items():
                            ...
                            if isinstance(
                                inner_value, (dict, list, str, int, float, bool, type(None))
                            ):
                                json_results["results"][test_name][key][inner_key] = inner_value
                            else:
                                json_results["results"][test_name][key][inner_key] = str(
                                    inner_value
                                )
                    else:
                        # Convert values to serializable types
