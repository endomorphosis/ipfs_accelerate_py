...
class MockCIProvider(CIProviderInterface):
    ...
    async def upload_artifact(
        ...
        self.artifact_urls[test_run_id][artifact_name] = (
            f"https://{self.provider_name}.example.com/artifacts/{test_run_id}/{artifact_name}"
        )

        logger.info(
            f"Uploaded artifact {artifact_name} for test run {test_run_id} with {self.provider_name}"
        )
        return True
...
async def test_artifact_metadata_extraction():
    ...
    try:
        ...
        artifacts.append(ArtifactMetadata(artifact_name="test.log", artifact_path=log_path))
        ...
        artifacts.append(
            ArtifactMetadata(artifact_name="test_report.json", artifact_path=report_path)
        )
        ...
        for artifact in artifacts:
            ...
            if artifact.content_metadata:
                logger.info(
                    f"  Content Metadata: {json.dumps(artifact.content_metadata, indent=2)}"
                )

                # Verify metrics extraction for reports
                if (
                    artifact.artifact_type in ["test_report", "performance_report"]
                    and "metrics" in artifact.content_metadata
                ):
                    logger.info(
                        f"  Metrics: {json.dumps(artifact.content_metadata['metrics'], indent=2)}"
                    )
...
async def test_artifact_discovery():
    ...
    try:
        ...
        log_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, artifact_type="log"
        )
        ...
        logs_and_test_logs = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, name_pattern=r"test_.*\.log"
        )
        ...
        provider1_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, provider_name="provider1"
        )
        ...
        test_run_0_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, test_run_id="test-run-0"
        )
        ...
        regression_test_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, labels=["regression-test"]
        )
        ...
        cuda_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, metadata_query={"hardware": "cuda"}
        )
        ...
        perf_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, name_pattern="perf_.*"
        )
        ...
        combined_artifacts = ArtifactDiscovery.discover_artifacts(
            artifacts=artifacts, provider_name="provider1", labels=["daily-test"]
        )
        ...
        latest_perf = ArtifactDiscovery.find_latest_artifact(
            artifacts=artifacts, artifact_type="performance_report"
        )
...
async def test_artifact_retrieval():
    ...
    try:
        ...
        with open(report_path, "w") as f:
            json.dump({"tests": 10, "passed": 8, "failed": 2, "duration": 15.3}, f, indent=2)

        # Create providers
        providers = {"github": MockCIProvider("github"), "gitlab": MockCIProvider("gitlab")}
        ...
        test_runs = {}
        for name, provider in providers.items():
            test_run = await provider.create_test_run({"name": f"{name} Test Run"})
            test_runs[name] = test_run

        # Upload artifacts to providers
        for name, provider in providers.items():
            await provider.upload_artifact(
                test_run_id=test_runs[name]["id"], artifact_path=log_path, artifact_name="test.log"
            )
        ...
        retriever = ArtifactRetriever(
            cache_dir=cache_dir, max_cache_size_mb=100, providers=providers
        )
        ...
        async def test_retrieval_without_real_http():
            ...
            for name, provider in providers.items():
                ...
                log_cache_path, log_cache_key = retriever._get_cache_path_for_artifact(
                    test_run_id=test_run_id, artifact_name="test.log", provider_name=name
                )
                ...
                report_cache_path, report_cache_key = retriever._get_cache_path_for_artifact(
                    test_run_id=test_run_id, artifact_name="test_report.json", provider_name=name
                )
                ...
                assert os.path.exists(cached_log_path), (
                    f"Cached log file doesn't exist: {cached_log_path}"
                )
                assert cached_log_metadata.artifact_type == "log", (
                    f"Unexpected artifact type: {cached_log_metadata.artifact_type}"
                )
                ...
                assert os.path.exists(cached_report_path), (
                    f"Cached report file doesn't exist: {cached_report_path}"
                )
                assert cached_report_metadata.artifact_type == "json", (
                    f"Unexpected artifact type: {cached_report_metadata.artifact_type}"
                )
                ...
                batch_results = await retriever.retrieve_artifacts_batch(
                    artifacts=batch_artifacts, use_cache=True
                )

                assert "test.log" in batch_results, f"Missing log in batch results for {name}"
                assert "test_report.json" in batch_results, (
                    f"Missing report in batch results for {name}"
                )
                ...
                for i in range(3):
                    ...
                    perf_cache_path, perf_cache_key = retriever._get_cache_path_for_artifact(
                        test_run_id=test_run_id, artifact_name=f"perf_{i}.json", provider_name=name
                    )
        ...
        if "error" not in trend_result:
            ...
            if "trend" in trend_result and trend_result["trend"] is not None:
                assert trend_result["trend"] in ["increasing", "decreasing", "stable"], (
                    f"Invalid trend: {trend_result['trend']}"
                )
...
async def test_provider_artifact_url_retrieval():
    ...
    try:
        ...
        await jenkins_client.initialize(
            {"url": "https://jenkins.example.com/", "user": "test_user", "token": "test_token"}
        )

        jenkins_test_run = await jenkins_client.create_test_run(
            {"name": "Jenkins Test Run", "job_name": "test-job", "build_id": "123"}
        )
        ...
        await circleci_client.initialize(
            {"token": "test_token", "project_slug": "github/test-owner/test-repo"}
        )
        ...
        circleci_test_run_id = circleci_test_run.get(
            "id", "circleci-github/test-owner/test-repo-123"
        )
        ...
        await azure_client.initialize(
            {"token": "test_token", "organization": "test-org", "project": "test-project"}
        )

        azure_test_run = await azure_client.create_test_run(
            {"name": "Azure Test Run", "build_id": "123"}
        )
