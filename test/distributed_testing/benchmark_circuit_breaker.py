...
class CircuitBreakerBenchmark:
    ...
    def __init__(
        ...
        # Results storage
        self.results = {"with_circuit_breaker": {}, "without_circuit_breaker": {}, "summary": {}}
        ...
        logger.info(
            f"Initialized circuit breaker benchmark with {num_browsers} browsers, {num_iterations} iterations"
        )
    ...
    async def _run_test_suite(self, use_circuit_breaker: bool) -> Dict[str, Any]:
        ...
        results = {
            ...
            "intensity_results": {},
            "overall": {"recovery_times_ms": [], "success_rate": 0, "resource_usage": {}},
        }
            ...
            for platform in self.platforms:
                for failure_type in self.failure_types:
                    for intensity in self.intensities:
                        ...
                        if (
                            not use_circuit_breaker
                            and intensity == "severe"
                            and len(test_cases)
                            < len(self.browser_types)
                            * len(self.platforms)
                            * len(self.failure_types)
        ...
        for i in range(self.num_iterations):
            logger.info(
                f"Running iteration {i + 1}/{self.num_iterations} {'WITH' if use_circuit_breaker else 'WITHOUT'} circuit breaker"
            )
            ...
            for test_case in test_cases:
                ...
                logger.info(
                    f"Testing {browser_type} + {platform} with {failure_type.value} ({intensity}) {'WITH' if use_circuit_breaker else 'WITHOUT'} circuit breaker"
                )
                ...
                self._add_to_category(
                    results, "browser_results", f"{browser_type}_{platform}", test_result
                )
                self._add_to_category(
                    results, "failure_type_results", failure_type.value, test_result
                )
        ...
        if results["overall"]["recovery_times_ms"]:
            results["overall"]["avg_recovery_time_ms"] = statistics.mean(
                results["overall"]["recovery_times_ms"]
            )
            results["overall"]["median_recovery_time_ms"] = statistics.median(
                results["overall"]["recovery_times_ms"]
            )
            results["overall"]["min_recovery_time_ms"] = min(
                results["overall"]["recovery_times_ms"]
            )
            results["overall"]["max_recovery_time_ms"] = max(
                results["overall"]["recovery_times_ms"]
            )

            if len(results["overall"]["recovery_times_ms"]) > 1:
                results["overall"]["stdev_recovery_time_ms"] = statistics.stdev(
                    results["overall"]["recovery_times_ms"]
                )
        ...
        for category_name in ["browser_results", "failure_type_results", "intensity_results"]:
            for key in results[category_name]:
                if results[category_name][key]["recovery_times_ms"]:
                    results[category_name][key]["avg_recovery_time_ms"] = statistics.mean(
                        results[category_name][key]["recovery_times_ms"]
                    )
                    results[category_name][key]["success_rate"] = (
                        results[category_name][key]["successes"]
                        / results[category_name][key]["total"]
                    )
        ...
        results["overall"]["resource_usage"] = self._calculate_resource_diff(
    ...
    async def _run_test_case(
        ...
        config = BrowserConfiguration(
            browser_name=browser_type, platform=platform, headless=True, timeout=30
        )
        ...
        try:
            ...
            if not injection_result.get("success", False):
                ...
                result["error"] = (
                    f"Failed to inject failure: {injection_result.get('error', 'unknown error')}"
                )
                await bridge.close()
                return result
    ...
    def _add_to_category(
        self, results: Dict[str, Any], category: str, key: str, test_result: Dict[str, Any]
    ) -> None:
        """
        ...
        if key not in results[category]:
            results[category][key] = {"recovery_times_ms": [], "successes": 0, "total": 0}
            ...
            if with_resources and without_resources:
                ...
                for key in with_resources:
                    if key in without_resources:
                        ...
                        pct_diff = (
                            (diff / without_resources[key]) * 100
                            if without_resources[key] != 0
                            else 0
                        )
                        resource_diff[key] = {"difference": diff, "percent_diff": pct_diff}
        ...
        for category in ["browser_results", "failure_type_results", "intensity_results"]:
            ...
            for key in self.results["with_circuit_breaker"][category]:
                if key in self.results["without_circuit_breaker"][category]:
                    ...
                    if not with_data.get("recovery_times_ms") or not without_data.get(
                        "recovery_times_ms"
                    ):
                        continue
    ...
    def _generate_visualizations(self) -> None:
        ...
        if not VISUALIZATION_AVAILABLE:
            logger.warning(
                "Visualization dependencies not available. Cannot generate visualizations."
            )
            return
    ...
    def _generate_recovery_time_graph(self) -> None:
        ...
        plt.savefig(
            f"benchmark_reports/recovery_time_comparison_{timestamp}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_success_rate_graph(self) -> None:
        ...
        without_rate = (
            self.results["without_circuit_breaker"]["overall"].get("success_rate", 0) * 100
        )
        ...
        if without_rate > 0:
            ...
            plt.figtext(
                0.5,
                0.02,
                f"{improvement:.1f}% improvement with Circuit Breaker",
                ha="center",
                fontsize=12,
            )
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plt.savefig(
            f"benchmark_reports/success_rate_comparison_{timestamp}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_category_graph(self, category: str, title: str, ylabel: str) -> None:
        ...
        plt.bar(
            x + width / 2, without_times, width, label="Without Circuit Breaker", color="orange"
        )
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plt.savefig(
            f"benchmark_reports/{category}_comparison_{timestamp}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _generate_summary_dashboard(self) -> None:
        ...
        with_cb_time = self.results["with_circuit_breaker"]["overall"].get(
            "avg_recovery_time_ms", 0
        )
        without_cb_time = self.results["without_circuit_breaker"]["overall"].get(
            "avg_recovery_time_ms", 0
        )
        ...
        for intensity in intensities:
            ...
            without_data = self.results["without_circuit_breaker"]["intensity_results"].get(
                intensity, {}
            )
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plt.savefig(
            f"benchmark_reports/circuit_breaker_summary_{timestamp}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _generate_markdown_report(self) -> None:
        ...
        try:
            ...
            with open(markdown_path, "w") as f:
                ...
                f.write(
                    f"- Failure types tested: {', '.join([f.value for f in self.failure_types])}\n"
                )
                ...
                f.write(
                    f"| Median | {with_median:.1f} | {without_median:.1f} | {improvement_str} |\n"
                )
                ...
                for key in with_resources:
                    if key in without_resources:
                        ...
                        f.write(
                            f"| {key} | {with_value:.2f} | {without_value:.2f} | {diff:.2f} |\n"
                        )
                ...
                if overall_rating in ["Excellent", "Very Good"]:
                    f.write(
                        "The circuit breaker pattern demonstrates significant benefits for fault tolerance and recovery in the distributed testing framework. It substantially improves recovery times and success rates while providing system stability during failure conditions.\n\n"
                    )
                elif overall_rating in ["Good", "Positive"]:
                    f.write(
                        "The circuit breaker pattern shows measurable benefits for fault tolerance and recovery in the distributed testing framework. It improves recovery times and success rates while helping maintain system stability during failures.\n\n"
                    )
                elif overall_rating == "Mixed":
                    f.write(
                        "The circuit breaker pattern shows mixed results in this benchmark. While there are some benefits in certain scenarios, the overall impact is not consistently positive across all test cases.\n\n"
                    )
                else:
                    f.write(
                        "The benchmark results are inconclusive regarding the benefits of the circuit breaker pattern in this specific implementation and test environment.\n\n"
                    )
                ...
                if VISUALIZATION_AVAILABLE:
                    ...
                    f.write(
                        "Detailed visualizations have been generated in the `benchmark_reports/` directory, including:\n\n"
                    )
    ...
    def _write_category_breakdown(self, file, category_name: str, category_key: str) -> None:
        ...
        # Setup table
        file.write(
            "| {} | With CB Recovery Time (ms) | Without CB Recovery Time (ms) | Time Improvement | With CB Success Rate | Without CB Success Rate | Rate Improvement |\n".format(
                category_name
            )
        )
        file.write(
        ...
        for key in common_keys:
            ...
            file.write(
                f"| {key} | {with_rec:.1f} | {without_rec:.1f} | {time_improvement_str} | {with_rate:.1f}% | {without_rate:.1f}% | {rate_improvement_str} |\n"
            )
...
async def main():
    ...
    parser.add_argument(
        "--browsers", type=int, default=2, help="Number of browser types to include in test"
    )
    parser.add_argument(
        "--iterations", type=int, default=3, help="Number of iterations for each test case"
    )
    parser.add_argument(
        "--failure-types",
        nargs="+",
    ...
    parser.add_argument(
        "--report",
        type=str,
        default="benchmark_reports/circuit_breaker_benchmark.json",
        help="Path to save JSON report",
    )
    parser.add_argument(
        "--simulate", action="store_true", help="Run in simulation mode (no real browsers)"
    )
