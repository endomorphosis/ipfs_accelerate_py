...
class CoordinatorHardwareIntegration:
            ...
            try:
                ...
                if "hardware_capabilities" in request_data:
                    ...
                    worker_capabilities = self._parse_hardware_capabilities(
                        worker_id, hardware_capabilities
                    )
    ...
    def _parse_hardware_capabilities(
        self, worker_id: str, capabilities_data: Dict[str, Any]
    ) -> Optional[WorkerHardwareCapabilities]:
        """
    ...
    async def _enhanced_can_worker_handle_task(self, worker_id: str, task: Dict[str, Any]) -> bool:
        ...
        if not worker_capabilities:
            ...
            if not hardware_requirements.get("critical", False):
                logger.warning(
                    f"No hardware capabilities for worker {worker_id}, but continuing as requirements not critical"
                )
                return True
            else:
                logger.warning(
                    f"No hardware capabilities for worker {worker_id}, can't verify critical hardware requirements"
                )
                return False
        ...
        if "precision_types" in hardware_requirements:
            ...
            if not self._has_required_precision_types(
                worker_capabilities, required_precision_types
            ):
                return False
    ...
    async def _enhanced_assign_pending_tasks(self):
        ...
        for worker_id, worker in self.coordinator.workers.items():
            if (
                ...
                and worker_id in self.coordinator.worker_connections
            ):
    ...
    def _has_required_hardware_types(
        self, worker_capabilities: WorkerHardwareCapabilities, required_types: List[str]
    ) -> bool:
        """
    ...
    def _has_required_hardware_features(
        self, worker_capabilities: WorkerHardwareCapabilities, required_features: Dict[str, Any]
    ) -> bool:
        """
        ...
        for hw_type, features in required_features.items():
            ...
            for feature_name, feature_value in features.items():
                ...
                if (
                    feature_name not in matching_hardware.capabilities
                    or matching_hardware.capabilities[feature_name] != feature_value
                ):
    ...
    def _has_sufficient_memory(
        self, worker_capabilities: WorkerHardwareCapabilities, min_memory_gb: float
    ) -> bool:
        """
    ...
    def _has_required_precision_types(
        self, worker_capabilities: WorkerHardwareCapabilities, required_precision_types: List[str]
    ) -> bool:
        """
    ...
    async def _get_worker_capabilities(
        self, worker_id: str
    ) -> Optional[WorkerHardwareCapabilities]:
        """
        Get worker capabilities from cache or database.
...
async def main():
    ...
    parser.add_argument(
        "--enable-browser-detection", action="store_true", help="Enable browser detection"
    )
    ...
    integration = CoordinatorHardwareIntegration(
        coordinator, db_path=args.db_path, enable_browser_detection=args.enable_browser_detection
    )
