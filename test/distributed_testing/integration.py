...
class DistributedTestingFramework:
    ...
    def __init__(
        ...
        self.coordinator_id = coordinator_id or self.config.get(
            "coordinator_id", f"coordinator-{int(time.time())}"
        )
        ...
        logger.info(
            f"Initialized Distributed Testing Framework (Coordinator: {self.coordinator_id})"
        )
    ...
    def start(self) -> bool:
        ...
        scheduling_thread = threading.Thread(target=self._scheduling_loop, daemon=True)
        ...
        for task_id, worker_id in assignments:
            if task_id in self.scheduler.tasks and worker_id in self.scheduler.workers:
                ...
                if len(self.metrics["assignments"]["recent"]) > 100:
                    self.metrics["assignments"]["recent"] = self.metrics["assignments"]["recent"][
                        -100:
                    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prometheus-port", type=int, default=8000, help="Port for Prometheus metrics"
    )

    args = parser.parse_args()

    # Create monitoring config from CLI args
    monitoring_config = {"prometheus_port": args.prometheus_port}
