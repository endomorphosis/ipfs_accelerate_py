...
from .enhanced_hardware_taxonomy import EnhancedHardwareTaxonomy
...
class WorkloadDecomposition:
    ...
    )  # subworkload_id -> list of dependencies
    metadata: Dict[str, Any] = field(default_factory=dict)
...
class HardwareAwareWorkloadManager:
    ...
    def __init__(self, hardware_taxonomy: EnhancedHardwareTaxonomy, db_path: Optional[str] = None):
        ...
        # Scheduler state
        self.scheduled_workloads: List[
            Tuple[int, str]
        ] = []  # List of (priority, workload_id) tuples
        ...
        with self.lock:
            ...
            for hardware_id, state in self.current_hardware_states.items():
                ...
                if state.current_allocation_slots:
                    ...
                    state.utilization_percentage = min(
                        0.9, 0.2 + (len(state.current_allocation_slots) * 0.1)
                    )
                else:
                ...
                state.power_usage_watts = base_power + (
                    state.utilization_percentage * (max_power - base_power)
                )
                ...
                if plan:
                    ...
                    for slot in plan.allocations:
                        ...
                        if hardware_id in self.current_hardware_states:
                            self.current_hardware_states[
                                hardware_id
                            ].current_allocation_slots.append(slot)

                    # Trigger event
                    self._trigger_event("plan_created", plan)

                    logger.info(
                        f"Created execution plan for workload {workload_id} with {len(plan.allocations)} allocations"
                    )
                    ...
                    logger.debug(
                        f"Couldn't create plan for workload {workload_id}, re-queued with priority {new_priority}"
                    )

            # Remove processed workloads
            self.scheduled_workloads = [
                item for item in self.scheduled_workloads if item not in processed_workloads
            ]
        ...
        with self.lock:
            ...
            for hardware_id, state in self.current_hardware_states.items():
                ...
                state.current_allocation_slots = [
                    slot for slot in state.current_allocation_slots if slot not in completed_slots
                ]
            ...
            for plan_id in plans_to_update:
                ...
                all_completed = all(slot.status == "completed" for slot in plan.allocations)

                any_failed = any(slot.status == "failed" for slot in plan.allocations)
    ...
    def register_workload(self, workload: Workload) -> str:
        ...
        with self.lock:
            ...
            logger.info(
                f"Registered workload {workload.workload_id} of type {workload.workload_type}"
            )
    ...
    def _create_single_device_execution_plan(self, workload: Workload) -> Optional[ExecutionPlan]:
        ...
        for worker_id, profile, match_score in hardware_matches:
            ...
            if hardware_id in self.current_hardware_states:
                ...
                if state.status in ["available", "busy"]:
                    ...
                    if workload.min_memory_bytes <= (
                        profile.memory.total_bytes - state.memory_usage_bytes
                    ):
        ...
        plan.estimated_energy_joules = self._estimate_energy_usage(
            optimized_workload, selected_profile, execution_time_seconds
        )
    ...
    def _create_decomposed_execution_plan(self, workload: Workload) -> Optional[ExecutionPlan]:
        ...
        latest_completion = (
            max(completion_times) if completion_times else (datetime.now() + timedelta(minutes=10))
        )
    ...
    def _optimize_workload_for_hardware(
        self, workload: Workload, hardware_profile: HardwareCapabilityProfile
    ) -> Workload:
        """
    ...
    def _optimize_batch_size(
        ...
        optimization_result = WorkloadOptimizationResult(
            ...
            + (0.1 * (suggested_batch_size / max(1, current_batch_size) - 1)),
            trade_offs={
    ...
    def _optimize_precision(
        ...
        for op in optimized.operations:
            ...
            if (
                op.operation_type in fp16_candidates
                and supports_fp16
                and supports_mixed_precision
                and has_tensor_cores
            ):
            ...
            elif op.operation_type in int8_candidates and supports_int8 and supports_quantization:
    ...
    def _optimize_memory_usage(
        ...
        sorted_ops = sorted(
            optimized.operations, key=lambda op: op.estimated_memory_bytes or 0, reverse=True
        )
    ...
    def _optimize_operation_fusion(
        self, workload: Workload, hardware_profile: HardwareCapabilityProfile
    ) -> Workload:
        ...
        for op_id, op in op_map.items():
            if op.operation_type in ["relu", "sigmoid", "tanh"]:
                ...
                if len(op_graph.get(op_id, set())) == 1:
                    ...
                    if dep_op and dep_op.operation_type in [
                        "matmul",
                        "gemm",
                        "conv1d",
                        "conv2d",
            ...
            for fusion_type, *args in fusions:
                if fusion_type == "LinearActivation":
                    ...
                    fused_op = OperationProfile(
                        ...
                        estimated_flops=(linear_op.estimated_flops or 0)
                        + (activation_op.estimated_flops or 0),
                        estimated_memory_bytes=max(
                            linear_op.estimated_memory_bytes or 0,
                            activation_op.estimated_memory_bytes or 0,
                        ),
                        parallelism_degree=min(
                            linear_op.parallelism_degree, activation_op.parallelism_degree
                        ),
                    ...
                    fused_op = OperationProfile(
                        ...
                        estimated_memory_bytes=max(
                            op.estimated_memory_bytes or 0 for op in chain_ops
                        ),
                ...
                for dep in op.dependencies:
                    if dep in removed_ops:
                        ...
                        for fused_op in new_operations:
                            if (
                                hasattr(fused_op, "metadata")
                                and fused_op.metadata
                                and "original_ops" in fused_op.metadata
                            ):
            ...
            if has_matmul and has_conv:
                ...
                if hardware_profile.hardware_class == HardwareClass.GPU:
                    if (
                        ...
                        in self.taxonomy.get_inherited_capabilities(hardware_profile)
                    ):
    ...
    def get_workload_optimization_history(
        self, workload_id: str
    ) -> List[WorkloadOptimizationResult]:
        """
