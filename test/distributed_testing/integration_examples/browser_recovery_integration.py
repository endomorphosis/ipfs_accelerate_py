    ...
    class BrowserAutomationBridge:
        ...
        async def launch(self, allow_simulation=False):
            ...
            if hasattr(self, "fail_launch_count") and self.fail_launch_count > 0:
                ...
                logger.warning(
                    f"Simulating launch failure ({self.fail_launch_count} more will occur)"
                )
                return False
...
try:
    ...
    class CircuitBreaker:
        ...
        async def execute(self, action, fallback=None):
            ...
            try:
                ...
                if (
                    ...
                    logger.info(
                        f"Circuit {self.name} is now closed after {self.consecutive_successes} consecutive successes"
                    )
                    self.state = CircuitState.CLOSED
                    self.failure_count = 0
        ...
        def get_metrics(self):
            ...
            recovery_rate = (
                self.metrics["recoveries"] / self.metrics["failures"]
                if self.metrics["failures"] > 0
                else 0
            )
            avg_downtime = (
                self.metrics["total_downtime_seconds"] / self.metrics["circuit_open_count"]
                if self.metrics["circuit_open_count"] > 0
                else 0
            )
...
class BrowserRecoveryDemo:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized browser recovery demo with {browser_name}, {model_name}, {platform}"
        )
    ...
    async def _run_protected_test(self, bridge, circuit, test_index):
            ...
            if self.inject_failures and test_index in [1, 4, 7]:
                ...
                if recovered:
                    ...
                    result = await bridge.run_test(
                        self.model_name, "This is a test input after recovery"
                    )
        ...
        def fallback():
            return {"success": False, "error": f"Circuit {circuit.name} is open"}
                ...
                try:
                    if not bridge.initialized:
                        ...
                        if not success:
                            return {
                                "success": False,
                                "error": "Failed to launch browser after recovery",
                            }
    ...
    async def _show_recovery_statistics(self):
        ...
        for browser, browser_stats in stats.get("browsers", {}).items():
            ...
            logger.info(
                f"{browser}: {success_rate:.2%} success rate ({browser_stats.get('successes', 0)}/{browser_stats.get('attempts', 0)})"
            )
        logger.info("-" * 50)
        ...
        for model, model_stats in stats.get("models", {}).items():
            ...
            logger.info(
                f"{model}: {success_rate:.2%} success rate ({model_stats.get('successes', 0)}/{model_stats.get('attempts', 0)})"
            )
        logger.info("-" * 50)
        ...
        for browser, model_strategies in analysis.get("best_strategies", {}).items():
            for model, strategy_info in model_strategies.items():
                if strategy_info.get("strategy"):
                    logger.info(
                        f"{browser}/{model}: {strategy_info.get('strategy')} (score: {strategy_info.get('score', 0):.2f})"
                    )
            ...
            if hasattr(circuit, "get_metrics"):
                ...
                if recovery_history:
                    ...
                    for i, recovery in enumerate(recovery_history[-3:]):  # Show last 3 recoveries
                        logger.info(
                            f"  {i + 1}. {recovery.get('timestamp', 'Unknown')} - {recovery.get('recovery_method', 'Unknown')} ({recovery.get('browser', 'Unknown')})"
                        )
...
async def main():
    ...
    parser.add_argument(
        "--browser", type=str, default="chrome", help="Browser to use (chrome, firefox, edge)"
    )
    ...
    parser.add_argument(
        "--no-failures", action="store_true", help="Don't inject artificial failures"
    )
