...
class RecoveryStrategy:
    ...
    async def execute(self, error_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Recovery strategy {self.name} succeeded in {execution_time:.2f}s (success rate: {self.success_rate:.2f})"
                )
            else:
                logger.warning(
                    f"Recovery strategy {self.name} failed after {execution_time:.2f}s (success rate: {self.success_rate:.2f})"
                )
    ...
    async def _execute_impl(self, error_info: Dict[str, Any]) -> bool:
        ...
        for retry in range(self.max_retries):
            try:
                ...
                logger.info(
                    f"Retry {retry + 1}/{self.max_retries} failed: {str(e)}. Retrying in {delay:.2f}s..."
                )
        ...
        try:
            ...
            try:
                ...
                if result and result[0] == 1:
                    ...
                    logger.warning(
                        f"Reconnection to database at {db_path} returned unexpected result"
                    )
                    return False
            except Exception as e:
            ...
            if hasattr(self.coordinator, "db") and self._db_table_exists("distributed_tasks"):
                try:
                    ...
                    """,
                        ...
                        ),  # Reset tasks running for >30 minutes
                    )
                except Exception as e:
                    ...
                    try:
                        ...
                        for row in result:
                            ...
                            workers[worker_id] = {
                                ...
                                "hardware_metrics": json.loads(hardware_metrics)
                                if hardware_metrics
                                else {},
...
class SystemRecoveryStrategy(RecoveryStrategy):
    ...
    async def _handle_resource_exhaustion(self, error_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if hasattr(self.coordinator, "health_monitor"):
                self.coordinator.health_monitor.check_interval = (
                    30  # Increase health check interval
                )
    ...
    async def _handle_disk_full(self, error_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} temporary files, freed {deleted_bytes / 1024 / 1024:.2f} MB"
            )
            ...
            logger.info(
                f"Deleted {deleted_count} old log files, freed {deleted_bytes / 1024 / 1024:.2f} MB total"
            )
            ...
            if hasattr(self.coordinator, "db_path"):
                ...
                logger.info(
                    f"Deleted {deleted_count} old database backups, freed {deleted_bytes / 1024 / 1024:.2f} MB total"
                )
    ...
    async def _handle_system_overload(self, error_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if hasattr(self.coordinator, "health_monitor"):
                self.coordinator.health_monitor.check_interval = (
                    60  # Increase health check interval
                )
                logger.info("Increased health check interval")

            # 3. Reduce concurrency
            if hasattr(self.coordinator, "max_concurrent_tasks"):
                self.coordinator.max_concurrent_tasks = max(
                    1, self.coordinator.max_concurrent_tasks // 2
                )
                logger.info(
                    f"Reduced max concurrent tasks to {self.coordinator.max_concurrent_tasks}"
                )
            ...
            if hasattr(self.coordinator, "health_monitor"):
                self.coordinator.health_monitor.check_interval = (
                    30  # Partially restore health check interval
                )
...
class EnhancedErrorRecoveryManager:
    ...
    async def recover(
        ...
        if not strategy:
            logger.error(
                f"No recovery strategy available for error category: {error_info['category']}"
            )
    ...
    def record_error_recovery(
        self, error_info: Dict[str, Any], strategy_name: str, success: bool, duration: float
    ):
        """
