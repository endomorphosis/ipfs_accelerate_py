...
class RetryPolicy:
    ...
    def should_retry(self, error_report: ErrorReport) -> bool:
        ...
        if self.retry_on_error_types is not None:
            ...
            if isinstance(error_report.error_type, str):
                if error_report.error_type not in [
                    et.value if hasattr(et, "value") else et for et in self.retry_on_error_types
                ]:
                    return False
            else:
        ...
        if self.skip_on_error_types is not None:
            # Handle both Enum and string value comparisons
            if isinstance(error_report.error_type, str):
                if error_report.error_type in [
                    et.value if hasattr(et, "value") else et for et in self.skip_on_error_types
                ]:
                    return False
            else:
        ...
        if self.skip_if_severity_above is not None:
            if isinstance(error_report.error_severity, str):
                ...
                skip_value = (
                    self.skip_if_severity_above.value
                    if hasattr(self.skip_if_severity_above, "value")
                    else self.skip_if_severity_above
                )
                ...
                # Compare enum values
                if getattr(
                    error_report.error_severity, "value", error_report.error_severity
                ) > getattr(self.skip_if_severity_above, "value", self.skip_if_severity_above):
                    return False
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "jitter": self.jitter,
            "retry_on_error_types": [et.value for et in self.retry_on_error_types]
            if self.retry_on_error_types
            else None,
            "skip_on_error_types": [et.value for et in self.skip_on_error_types]
            if self.skip_on_error_types
            else None,
            "skip_if_severity_above": self.skip_if_severity_above.value
            if self.skip_if_severity_above
            else None,
        }
...
class DistributedErrorHandler:
    ...
    def register_error_hook(
        self,
        error_type: Union[str, ErrorType, List[Union[str, ErrorType]]],
        hook: Callable[[ErrorReport], None],
    ) -> None:
        """
    ...
    def categorize_error(
        ...
        if any(kw in error_message.lower() for kw in ["configuration", "config", "settings"]):
            ...
            kw in error_message.lower() for kw in ["dependency", "require", "import", "module"]
        ):
    ...
    def create_error_report(
        self, exception: Exception, context_data: Dict[str, Any] = None
    ) -> ErrorReport:
        """
        ...
        if error_severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]:
            _log_error(
                f"SEVERE ERROR [{error_id}]: {error_type.value} in {context.component}/{context.operation}: {str(exception)}"
            )
        else:
            _log_warning(
                f"Error [{error_id}]: {error_type.value} in {context.component}/{context.operation}: {str(exception)}"
            )
    ...
    ) -> Tuple[Any, Optional[ErrorReport]]:
        """
        ...
        while True:
            try:
                ...
                if not error_report:
                    ...
                    context_data = {"component": component, "operation": operation_name, **context}
                    ...
                    if str(e) != error_report.message:
                        error_report.message = (
                            f"{error_report.message} | Retry {retry_count}: {str(e)}"
                        )

                # Check if we should retry
                retry_count += 1
                if (
                    not retry_policy.should_retry(error_report)
                    or retry_count > retry_policy.max_retries
                ):
                    ...
                    # Log failure
                    _log_error(
                        f"Operation {operation_name} failed after {retry_count} retries: {str(last_error)}"
                    )
                ...
                # Log retry attempt
                _log_warning(
                    f"Retrying operation {operation_name} after error: {str(e)} (attempt {retry_count}/{retry_policy.max_retries}, delay {delay_ms}ms)"
                )
    ...
    def handle_error(self, exception: Exception, context: Dict[str, Any] = None) -> ErrorReport:
        ...
        if error_report.error_severity == ErrorSeverity.CRITICAL:
            logger.critical(
                f"Critical error in {error_report.context.component}/{error_report.context.operation}: {error_report.message}"
            )
        elif error_report.error_severity == ErrorSeverity.HIGH:
            _log_error(
                f"High severity error in {error_report.context.component}/{error_report.context.operation}: {error_report.message}"
            )
        else:
            _log_warning(
                f"Error in {error_report.context.component}/{error_report.context.operation}: {error_report.message}"
            )
    ...
    def get_error_metrics(self) -> Dict[str, Any]:
        ...
        retry_success_rates = {}
        for error_type in set(self.retry_success_counts.keys()).union(
            self.retry_failure_counts.keys()
        ):
        ...
        aggregation_metrics = {
            ...
            "avg_group_size": sum(len(group) for group in error_groups.values()) / len(error_groups)
            if error_groups
            else 0,
        }
        ...
        return {
            ...
            "unresolved_errors": sum(
                1 for e in self.errors.values() if e.resolution_status != "resolved"
            ),
        }

    def get_unresolved_errors(
        self, component: Optional[str] = None, severity: Optional[ErrorSeverity] = None
    ) -> List[ErrorReport]:
        """
    ...
    def get_error_history(
        self, component: Optional[str] = None, period_hours: Optional[int] = None
    ...
    def integrate_with_dependency_manager(self, dependency_manager):
        ...
        def error_wrapped_validate():
            try:
                ...
                # Handle the error
                context = {"component": "dependency_manager", "operation": "validate_dependencies"}
    ...
    def clear_resolved_errors(self, older_than_hours: Optional[int] = None) -> int:
        ...
        for error_id, error_report in self.errors.items():
            if error_report.resolution_status == "resolved":
                if (
                    cutoff
                    and error_report.resolution_time
                    and error_report.resolution_time < cutoff
                ):
...
def safe_execute(func, *args, error_handler=None, **kwargs):
    ...
    try:
        ...
        if error_handler:
            context = {"component": component, "operation": operation, **context_arg}
...
if __name__ == "__main__":
    ...
    for test_value in [-5, 50, 200]:
        try:
            ...
            print(
                f"Error handled: {error_report.error_id} - {error_report.error_type.value} - {error_report.message}"
            )
    ...
    async def test_retry():
        ...
        error_handler.set_retry_policy(
            "example", RetryPolicy(max_retries=3, initial_delay_ms=100, backoff_factor=1.5)
        )
        ...
        result, error = await error_handler.retry_operation(
            flaky_operation, context={"component": "example", "operation": "flaky_operation"}
        )
