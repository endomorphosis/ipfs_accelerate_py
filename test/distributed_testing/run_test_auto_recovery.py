...
async def main():
    ...
    parser.add_argument(
        "--db-path", default="./test_auto_recovery.duckdb", help="Path to DuckDB database"
    )
    ...
    try:
        ...
        if worker_procs:
            ...
            new_worker_proc = await run_worker(
                worker_id, f"http://localhost:{args.port}", worker_key
            )
