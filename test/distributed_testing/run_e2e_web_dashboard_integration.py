...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("e2e_integration_test.log")],
)
...
# Hardware types
HARDWARE_TYPES = ["cpu", "cuda", "rocm", "mps", "webgpu", "webnn"]
...
def notification_callback(notification):
    ...
    logger.info(
        f"NOTIFICATION: {notification['type'].upper()} - {notification['severity'].upper()}"
    )
        ...
        if notification["type"] == "anomaly":
            if "anomalous_features" in notification["details"]:
                for feature in notification["details"]["anomalous_features"]:
                    logger.info(
                        f"  • {feature['feature']}: value={feature['value']:.2f}, z-score={feature['z_score']:.2f}"
                    )
...
def start_coordinator(args):
    ...
    if not is_port_available(args.coordinator_port):
        logger.error(
            f"Port {args.coordinator_port} is already in use. Please choose a different port."
        )
        sys.exit(1)
...
def start_worker(args, worker_id, capabilities):
    ...
    cmd = [
        ...
        "--coordinator",
        f"http://localhost:{args.coordinator_port}",
        "--worker-id",
        worker_id,
        "--capabilities",
...
def start_dashboard(args):
    ...
    if not is_port_available(args.dashboard_port):
        logger.error(
            f"Port {args.dashboard_port} is already in use. Please choose a different port."
        )
        sys.exit(1)
    ...
    logger.info(
        f"Web dashboard started on port {args.dashboard_port} with WebSocket update interval {args.update_interval}s"
    )
...
def generate_random_task(
    coordinator, task_id, task_type, hardware_requirements=None, priority=None
):
...
async def simulate_tasks(coordinator, integration, args):
    ...
    for i in range(args.num_tasks):
        ...
        generate_anomaly = (
            args.generate_anomalies and random.random() < 0.2
        )  # 20% anomaly rate if enabled

        # Generate result
        result, execution_time = generate_task_result(
            task, worker, success=success, generate_anomaly=generate_anomaly
        )
        ...
        try:
            if success:
                ...
                logger.info(
                    f"Task {task_id} completed by worker {worker_id} in {execution_time:.2f}s"
                )
            else:
                # Call task failure handler
                await coordinator._handle_task_failed(
                    task_id, worker_id, result["error"], execution_time
                )
                failed_count += 1
    ...
    if args.generate_trends:
        ...
        for i in range(num_trend_tasks):
            ...
            task["metadata"] = {"model": "trending_model", "batch_size": 8, "precision": "fp16"}
            ...
            try:
                ...
                await coordinator._handle_task_completed(task_id, worker_id, result, execution_time)
                logger.info(
                    f"Trend task {task_id} completed with throughput {result['metrics']['throughput']:.2f}"
                )
            except Exception as e:
    ...
    for metric, trend_data in trends.items():
        if (
            "trend" in trend_data
            and trend_data["trend"] != "stable"
            and abs(trend_data.get("percent_change", 0)) > 5
        ):
    ...
    summary_report = integration.service.generate_analysis_report(
        report_type="summary", format="markdown"
    )
    ...
    performance_report = integration.service.generate_analysis_report(
        report_type="performance", format="markdown"
    )
    ...
    if anomalies:
        anomaly_report = integration.service.generate_analysis_report(
            report_type="anomaly", format="markdown"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="End-to-End Integration Test for Result Aggregator Web Dashboard"
    )
    parser.add_argument(
        "--db-path", default="./e2e_test_results.duckdb", help="Path to DuckDB database"
    )
    ...
    parser.add_argument(
        "--generate-anomalies", action="store_true", help="Generate anomalous test results"
    )
    parser.add_argument(
        "--generate-trends", action="store_true", help="Generate performance trends"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument(
        "--open-browser", action="store_true", help="Open web browser when dashboard is ready"
    )
