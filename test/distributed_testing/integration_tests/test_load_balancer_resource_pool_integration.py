...
class TestLoadBalancerResourcePoolIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch(
            "distributed_testing.resource_pool_bridge.ResourcePoolBridgeIntegration",
    ...
    async def test_test_assignment(self):
        ...
        self.assertEqual(
            len(assignments), len(tests) - 1
        )  # -1 because the large model needs manual handling
    ...
    async def test_execute_models_across_workers(self):
        ...
        for i in range(5):
            ...
            test = TestRequirements(
                ...
                browser_requirements={
                    "preferred": "chrome" if i % 3 == 0 else "firefox" if i % 3 == 1 else "edge"
                },
            )
        ...
        self.load_balancer.update_worker_utilization(
            high_load_worker, cpu_utilization=85.0, memory_utilization=90.0, gpu_utilization=95.0
        )
    ...
    async def test_sharded_model_execution(self):
        ...
        test = TestRequirements(
            ...
            priority=3,
            requires_sharding=True,
            sharding_requirements={"strategy": "layer_balanced", "num_shards": 3},
        )
        ...
        for worker_id, capabilities in self.load_balancer.workers.items():
            if (
                capabilities.has_webgpu and capabilities.available_memory >= 4.0
            ):  # Need at least 4GB per shard
                suitable_workers.append(worker_id)

        # Ensure we have enough workers
        self.assertGreaterEqual(
            len(suitable_workers),
            test.sharding_requirements["num_shards"],
        ...
        for i, worker_id in enumerate(suitable_workers[: test.sharding_requirements["num_shards"]]):
            ...
            sharded_execution.run_inference = MagicMock(
                return_value={"output": f"Shard output from worker {worker_id}"}
            )
        ...
        test_workers = [
            worker_id
            for worker_id in self.workers
            if test.test_id in self.workers[worker_id]["active_tests"]
        ]
        ...
        self.load_balancer.update_worker_utilization(
            heavy_worker, cpu_utilization=90.0, memory_utilization=95.0, gpu_utilization=95.0
        )
...
class TestLoadBalancerResourcePoolIntegrationAsync(
    AsyncTestCase, TestLoadBalancerResourcePoolIntegration
):
