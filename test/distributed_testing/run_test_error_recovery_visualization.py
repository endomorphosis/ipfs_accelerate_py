...
async def run_visualization_test(output_dir=None, generate_sample_data=False, format="png"):
    ...
    class PerformanceBasedErrorRecovery:
        ...
        def get_progressive_recovery_history(self, error_id=None):
            ...
            if error_id:
                ...
                current_level = (
                    current_level_result[0]
                    if current_level_result and current_level_result[0]
                    else 1
                )

                return {"error_id": error_id, "history": history, "current_level": current_level}
            else:
                ...
                return {
                    ...
                    "summary": {
                        ...
                        "successful_recoveries": success_counts_result[0]
                        if success_counts_result
                        else 0,
                        "failed_recoveries": success_counts_result[1]
                        if success_counts_result
                        else 0,
                    },
                }
    ...
    trends_path = visualizer.visualize_performance_trends(
        time_series_data=analysis_data.get("time_series", []), filename="performance_trend_graphs"
    )
...
async def generate_performance_data(db_connection):
    ...
    for i in range(200):  # Generate 200 performance records
        ...
        stability_score = (
            0.9 if success else 0.2 + random.random() * 0.4
        )  # 0.2 to 0.6 if failed, 0.9 if success
    ...
    for strategy in strategies:
        for error_type in error_types:
            ...
            if result[0] > 0:  # If we have records
                ...
                task_recovery_rate = (
                    total_recovered_tasks / total_affected_tasks
                    if total_affected_tasks > 0
                    else 1.0
                )
...
def main():
    ...
    parser.add_argument(
        "--generate-sample-data", action="store_true", help="Generate sample performance data"
    )
    parser.add_argument(
        "--format",
        type=str,
