...
def _is_test_mode() -> bool:
    return bool(
        os.environ.get("PYTEST_CURRENT_TEST") or os.environ.get("CI") or "pytest" in sys.modules
    )
...
class BrowserRecoveryStrategy:
    ...
    def __init__(self, name: str, browser_type: BrowserType, model_type: ModelType):
        ...
        logger.debug(
            f"Initialized browser recovery strategy: {name} for {browser_type.value} and {model_type.value} models"
        )
    ...
    async def execute(self, bridge: Any, failure_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Browser recovery strategy '{self.name}' completed with success={success} in {execution_time:.2f}s (success rate: {self.success_rate:.2f})"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        avg_execution_time = (
            sum(self.execution_times) / len(self.execution_times) if self.execution_times else 0
        )
...
class SimpleRetryStrategy(BrowserRecoveryStrategy):
    ...
    async def _execute_impl(self, bridge: Any, failure_info: Dict[str, Any]) -> bool:
        ...
        if retry_count >= self.max_retries:
            logger.warning(
                f"Maximum retries ({self.max_retries}) exceeded for {self.browser_type.value}"
            )
            return False

        # Log the retry attempt
        logger.info(
            f"Retry attempt {retry_count + 1}/{self.max_retries} for {self.browser_type.value}"
        )
...
class BrowserRestartStrategy(BrowserRecoveryStrategy):
    ...
    def __init__(
        ...
        preserve_browser_args: bool = True,
    ):
        """
...
class SettingsAdjustmentStrategy(BrowserRecoveryStrategy):
    ...
    async def _execute_impl(self, bridge: Any, failure_info: Dict[str, Any]) -> bool:
        ...
        """
        logger.info(
            f"Executing settings adjustment for {self.browser_type.value} with {self.model_type.value} models"
        )
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully relaunched {self.browser_type.value} with adjusted settings"
                )
                return True
            else:
                logger.warning(
                    f"Failed to relaunch {self.browser_type.value} with adjusted settings"
                )
                return False
    ...
    async def _adjust_chrome_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
        ...
        bridge.add_browser_arg(
            "--disable-features=BackForwardCache"
        )  # Disable caching that can cause issues

    async def _adjust_firefox_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_edge_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_safari_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_text_model_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_vision_model_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_audio_model_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
    ...
    async def _adjust_multimodal_model_settings(
        self, bridge: Any, failure_type: FailureType, failure_info: Dict[str, Any]
    ):
...
class ModelSpecificRecoveryStrategy(BrowserRecoveryStrategy):
    ...
    def __init__(self, browser_type: BrowserType, model_type: ModelType):
        ...
        """
        super().__init__(
            f"model_specific_{model_type.value}_{browser_type.value}", browser_type, model_type
        )
        self.level = RecoveryLevel.AGGRESSIVE
    ...
    async def _execute_impl(self, bridge: Any, failure_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Applying {self.model_type.value}-specific recovery for {self.browser_type.value}"
            )
            ...
            if success:
                logger.info(
                    f"Successfully launched browser with {self.model_type.value}-specific optimizations"
                )
                return True
            else:
                logger.warning(
                    f"Failed to launch browser with {self.model_type.value}-specific optimizations"
                )
                return False
    ...
    async def _recover_text_model(self, bridge: Any, failure_info: Dict[str, Any]):
        ...
        if hasattr(bridge, "set_resource_settings"):
            bridge.set_resource_settings(
                max_batch_size=1, optimize_for="latency", progressive_loading=False
            )
...
class ProgressiveRecoveryManager:
    ...
    def get_strategies(
        self, browser_type: BrowserType, model_type: ModelType
    ) -> List[BrowserRecoveryStrategy]:
        """
        ...
        strategies = browser_strategies.get(
            model_type, browser_strategies.get(ModelType.GENERIC, [])
        )
    ...
    async def execute_progressive_recovery(
        ...
        start_level: RecoveryLevel = RecoveryLevel.MINIMAL,
    ) -> bool:
        """
        ...
        if not strategies:
            logger.error(
                f"No recovery strategies available for {browser_type.value}/{model_type.value}"
            )
            return False
        ...
        strategies = [
            s
            for s in strategies
            if getattr(s, "level", RecoveryLevel.MINIMAL).value >= start_level.value
        ]

        # Try each strategy in sequence
        for strategy in strategies:
            logger.info(
                f"Trying recovery strategy: {strategy.name} (level {getattr(strategy, 'level', 'unknown')})"
            )
    ...
    def get_strategy_stats(self) -> Dict[str, Any]:
        ...
        for browser_type, browser_strategies in self.strategies_by_browser.items():
            browser_stats = {"attempts": 0, "successes": 0, "success_rate": 0.0, "models": {}}

            for model_type, strategies in browser_strategies.items():
                model_stats = {"attempts": 0, "successes": 0, "success_rate": 0.0, "strategies": {}}
            ...
            if browser_stats["attempts"] > 0:
                browser_stats["success_rate"] = (
                    browser_stats["successes"] / browser_stats["attempts"]
                )
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        for entry in history:
            ...
            if timestamp:
                ...
                days[day]["strategies"][strategy_name]["total_execution_time"] += entry.get(
                    "execution_time", 0.0
                )
        ...
        for day, day_stats in sorted(days.items()):
            success_rate = (
                day_stats["successes"] / day_stats["attempts"] if day_stats["attempts"] > 0 else 0.0
            )
            avg_execution_time = (
                day_stats["total_execution_time"] / day_stats["attempts"]
        ...
        return {"stats": stats, "time_series": time_series, "best_strategies": best_strategies}
...
def detect_model_type(model_name: str) -> ModelType:
    ...
    # Text models
    if any(
        text_model in model_name_lower
        for text_model in [
            "bert",
            "t5",
            "gpt",
    ...
    # Vision models
        ...
        vision_model in model_name_lower
        for vision_model in ["vit", "resnet", "efficientnet", "yolo", "detr", "dino", "swin"]
    ):
        return ModelType.VISION

    # Audio models
    elif any(
        audio_model in model_name_lower
        for audio_model in ["whisper", "wav2vec", "hubert", "audioclip", "clap"]
    ):
        return ModelType.AUDIO

    # Multimodal models
    elif any(
        multimodal_model in model_name_lower
        for multimodal_model in ["clip", "llava", "blip", "xclip", "flamingo", "qwen-vl"]
    ):
...
def categorize_browser_failure(error: Exception, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if any(
        launch_error in error_message.lower()
        for launch_error in ["failed to launch", "could not start", "executable not found"]
    ):
        failure_type = FailureType.LAUNCH_FAILURE

    elif any(
        connection_error in error_message.lower()
    ...
    elif any(timeout_error in error_message.lower() for timeout_error in ["timeout", "timed out"]):
        failure_type = FailureType.TIMEOUT

    elif any(
        crash_error in error_message.lower()
        for crash_error in ["crashed", "crash", "terminated", "killed"]
    ):
        failure_type = FailureType.CRASH

    elif any(
        resource_error in error_message.lower()
        ...
        gpu_error in error_message.lower()
        for gpu_error in ["gpu", "webgpu", "graphics", "shader", "driver"]
    ):
        failure_type = FailureType.GPU_ERROR

    elif any(
        api_error in error_message.lower()
        for api_error in ["webnn", "webn", "api", "interface", "unsupported operation"]
    ):
        failure_type = FailureType.API_ERROR

    elif any(
        internal_error in error_message.lower()
        for internal_error in ["internal error", "internal failure", "unexpected error"]
    ):
...
async def recover_browser(bridge: Any, error: Exception, context: Dict[str, Any] = None) -> bool:
    ...
    if hasattr(bridge, "model_type"):
        model_type = (
            detect_model_type(bridge.model_type)
            if isinstance(bridge.model_type, str)
            else bridge.model_type
        )
    ...
    logger.info(
        f"Starting progressive recovery for {browser_type.value}/{model_type.value} from level {start_level.value}"
    )
