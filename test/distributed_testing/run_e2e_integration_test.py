...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run E2E integration tests for the Distributed Testing Framework."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--temp-dir",
        type=str,
        default="/tmp/dt_e2e_test",
        help="Temporary directory for test files (default: /tmp/dt_e2e_test)",
    )
    parser.add_argument(
        "--test-filter", type=str, default="Test*", help="Pattern to filter tests (e.g., TestE2E*)"
    )
    parser.add_argument(
        "--timeout", type=int, default=1800, help="Test timeout in seconds (default: 1800)"
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Don't clean up temporary files after tests"
    )
    parser.add_argument(
        "--ci", action="store_true", help="Run in CI mode (adjust settings for CI environment)"
    )
...
"""
    ...
    for test_result in result.test_results:
        status_class = {"pass": "pass", "fail": "fail", "error": "fail", "skip": "skip"}.get(
            test_result["status"], ""
        )
...
"""
    ...
    logger.info(
        f"Tests completed: {success_count} passed, {failure_count} failed, {error_count} errors, {skip_count} skipped"
    )
