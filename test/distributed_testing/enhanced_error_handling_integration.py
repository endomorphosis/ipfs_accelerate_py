...
class EnhancedErrorHandlingIntegration:
    ...
    async def handle_error(
        ...
        if success:
            logger.info(
                f"Successfully recovered from error {error_report.error_id} using strategy {recovery_info['strategy_name']}"
            )
        else:
            logger.warning(
                f"Failed to recover from error {error_report.error_id} after using strategy {recovery_info['strategy_name']}"
            )
            ...
            result.append(
                {
                    ...
                    "recovery_level": self.performance_recovery.error_recovery_levels.get(
                        report.error_id, 1
                    ),
                }
            )
    ...
    async def run_diagnostics(self) -> Dict[str, Any]:
        ...
        for strategy_id, strategy in recovery_metrics.get("strategies", {}).items():
            if strategy.get("total_executions", 0) > 5 and strategy.get("success_rate", 1.0) < 0.5:
                issues.append(
                    f"Low success rate for strategy {strategy.get('strategy_name', strategy_id)}: {strategy.get('success_rate', 0):.2f}"
                )
...
async def handle_resolve_error(request):
    ...
    if resolved:
        ...
        return web.json_response(
            {"error": f"Error {error_id} not found or already resolved"}, status=404
        )
...
async def handle_reset_recovery(request):
    ...
    if error_id:
        ...
        message = (
            f"Recovery level for error {error_id} reset"
            if result
            else f"Error {error_id} not found"
        )
    else:
        # Reset all errors
...
if __name__ == "__main__":
    ...
    async def run_example():
        ...
        try:
            ...
            # Handle the error
            success, info = await enhanced_error_handling.handle_error(
                e, {"component": "database", "operation": "connect"}
            )
