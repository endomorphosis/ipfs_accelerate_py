...
class RedundancyManager:
        ...
        **kwargs,
        ...
        if (
            ...
            self.cluster_nodes = [f"http://{legacy_host}:{legacy_port}"] + [
                f"http://{p['host']}:{p['port']}"
                for p in legacy_peers
                if isinstance(p, dict) and p.get("host") and p.get("port")
            ]
    ...
    def _get_node_url(self, node_id: str, cluster_nodes: List[str]) -> str:
        ...
        if match:
            ...
            if 0 <= index < len(cluster_nodes):
                ...
                def _port_value(url: str) -> int:
                    match_port = re.search(r":(\d+)$", url or "")
                    return int(match_port.group(1)) if match_port else 0

                ordered_nodes = sorted(cluster_nodes, key=_port_value)
                if 0 <= index < len(ordered_nodes):
    ...
    async def http_request(
        self, url: str, method: str = "GET", data: Any = None, timeout: float = 5
    ):
        ...
        try:
            if os.path.exists(self.state_path):
                with open(self.state_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded persistent state: term={self.current_term}, voted_for={self.voted_for}, log_entries={len(self.log_entries)}"
                    )
            else:
    ...
    def _save_persistent_state_sync(self):
        ...
        try:
            ...
            logger.debug(
                f"Saved persistent state: term={self.current_term}, voted_for={self.voted_for}, log_entries={len(self.log_entries)}"
            )
        except Exception as e:
    ...
    async def start(self):
        ...
        if self._compat_mode == "legacy":
            ...
            with _LEGACY_CLUSTER_LOCK:
                ...
                term = (
                    max((m.current_term for m in _LEGACY_CLUSTER_REGISTRY.values()), default=0) or 1
                )
                ...
                for node_key, manager in _LEGACY_CLUSTER_REGISTRY.items():
                    ...
                    manager.current_role = (
                        NodeRole.LEADER if node_key == leader_id else NodeRole.FOLLOWER
                    )
            return
        ...
        logger.info(
            f"RedundancyManager started for node {self.node_id} (role: {self.current_role.value})"
        )
            ...
            try:
                ...
                if time_since_heartbeat > self.election_timeout:
                    ...
                    if self.current_role != NodeRole.CANDIDATE:
                        logger.info(
                            f"Election timeout ({time_since_heartbeat:.2f}s > {self.election_timeout:.2f}s), starting election"
                        )
                        await self._start_election()
                    else:
            ...
            async with self.session.post(f"{node}/raft", json=data, timeout=2) as response:
                if response.status == 200:
                    ...
                    if vote_granted:
                        ...
                        logger.info(
                            f"Received vote from node {node}, votes received: {len(self.votes_received)}/{len(self.cluster_nodes)}"
                        )
    ...
    async def _send_append_entries(self, node: str):
        ...
        # Get entries to send
        entries = (
            self.log_entries[prev_log_index:] if prev_log_index < len(self.log_entries) else []
        )
        ...
        for n in range(self.commit_index + 1, len(self.log_entries) + 1):
            ...
            if count > len(self.cluster_nodes) // 2:
                ...
                if (
                    ...
                    and self.log_entries[n - 1]["term"] == self.current_term
                ):
                    self.commit_index = n
    ...
    async def _apply_add_task(self, task_data: Dict[str, Any]):
        ...
        if hasattr(self.coordinator, "task_manager"):
            ...
            # Add to database if available
            if hasattr(self.coordinator, "db_manager"):
                self.coordinator.db_manager.add_task(
                    task_id, task_type, priority, config, requirements
                )
    ...
    async def _apply_complete_task(
        self, task_id: str, worker_id: str, results: Dict[str, Any], metadata: Dict[str, Any]
    ):
    ...
    async def _apply_fail_task(
        self, task_id: str, worker_id: str, error: str, metadata: Dict[str, Any]
    ):
    ...
    async def _get_current_state(self) -> Dict[str, Any]:
        ...
        """
        state = {"workers": {}, "tasks": {}, "running_tasks": {}}
    ...
    async def handle_append_entries(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if prev_log_index > 0:
            ...
            if (
                prev_log_index <= len(self.log_entries)
                and self.log_entries[prev_log_index - 1]["term"] != prev_log_term
            ):
    ...
    async def handle_state_sync(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if term < self.current_term:
            return {"status": "rejected", "term": self.current_term}
    ...
    async def append_log(self, command: Dict[str, Any]) -> bool:
        ...
        if self.current_role != NodeRole.LEADER:
            logger.warning(
                f"Cannot append log entry, not leader (current role: {self.current_role.value})"
            )
            return False
    ...
    async def update_worker_status(self, worker_id: str, status: str) -> bool:
        ...
        if (
            ...
            return await self._forward_to_leader(
                "update_worker_status", {"worker_id": worker_id, "status": status}
            )
    ...
    async def add_task(self, task_data: Dict[str, Any]) -> bool:
        ...
        if (
            ...
            return await self._forward_to_leader("add_task", {"task_data": task_data})

        # Create command
        command = {"type": "add_task", "data": task_data}

        # Append to log
        return await self.append_log(command)

    async def update_task_status(
        self, task_id: str, status: str, worker_id: Optional[str] = None
    ) -> bool:
        ...
        if (
            ...
            return await self._forward_to_leader(
                "update_task_status", {"task_id": task_id, "status": status, "worker_id": worker_id}
            )

        # Create command
        command = {
            "type": "update_task",
            "data": {"task_id": task_id, "status": status, "worker_id": worker_id},
        }
    ...
    async def fail_task(
        ...
        if (
            ...
            return await self._forward_to_leader(
                "fail_task",
                {"task_id": task_id, "worker_id": worker_id, "error": error, "metadata": metadata},
            )
    ...
    async def _forward_to_leader(
        self, method: str, params: Dict[str, Any] | str, data: Dict[str, Any] | None = None
    ):
        ...
        # Prepare request
        data = {"method": method, "params": params, "forwarded": True, "source_node": self.node_id}

        try:
            async with self.session.post(
                f"{leader_url}/raft/forward", json=data, timeout=5
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    success = result.get("success", False)
        ...
        try:
            ...
            if method == "register_worker":
                ...
                success = await self.update_worker_status(
                    params.get("worker_id"), params.get("status")
                )
            elif method == "add_task":
                success = await self.add_task(params.get("task_data", {}))
            elif method == "update_task_status":
                success = await self.update_task_status(
                    params.get("task_id"), params.get("status"), params.get("worker_id")
                )
