...
class TestEnhancedHardwareTaxonomy(unittest.TestCase):
    ...
    def test_capability_registry(self):
        ...
        tensor_core_def = self.taxonomy.get_capability_definition(
    ...
    def test_hardware_capability_registration(self):
        ...
        self.taxonomy.register_hardware_capability(
            hardware_id, "test.custom_capability", {"performance": 100.0}
        )
    ...
    def test_capability_compatibility(self):
        ...
        self.assertEqual(
            self.taxonomy.get_capability_compatibility(
                "compute.matrix_multiplication", "compute.matrix_multiplication"
            ),
            1.0,
        )
        ...
        self.assertAlmostEqual(
            self.taxonomy.get_capability_compatibility(
                "compute.matrix_multiplication", "compute.tensor_core_acceleration"
            ),
            0.8,
        )
        ...
        self.assertAlmostEqual(
            self.taxonomy.get_capability_compatibility(
                "compute.matrix_multiplication", "compute.convolution"
            ),
            0.5,
        )
        ...
        self.assertAlmostEqual(
            self.taxonomy.get_capability_compatibility(
                "compute.matrix_multiplication", "memory.high_bandwidth"
            ),
            0.1,
        )
    ...
    def test_find_hardware_with_capability(self):
        ...
        tensor_core_hardware = self.taxonomy.find_hardware_with_capability(
            "compute.tensor_core_acceleration"
        )
        ...
        high_bandwidth_hardware = self.taxonomy.find_hardware_with_capability(
    ...
    def test_workload_hardware_match(self):
        ...
        cpu_match = self.taxonomy.calculate_workload_hardware_match(
            vision_requirements, self.cpu_profile
        )
        gpu_match = self.taxonomy.calculate_workload_hardware_match(
            vision_requirements, self.gpu_profile
        )
        npu_match = self.taxonomy.calculate_workload_hardware_match(
            vision_requirements, self.npu_profile
        )
        browser_match = self.taxonomy.calculate_workload_hardware_match(
            vision_requirements, self.browser_profile
        )
        ...
        cpu_match_nlp = self.taxonomy.calculate_workload_hardware_match(
            nlp_requirements, self.cpu_profile
        )
        gpu_match_nlp = self.taxonomy.calculate_workload_hardware_match(
            nlp_requirements, self.gpu_profile
        )
