...
from test.distributed_testing.result_aggregator.coordinator_integration import (
...
TASK_TYPES = ["benchmark", "unit_test", "integration_test", "functional_test", "performance_test"]
...
async def run_simulation(args):
    ...
    coordinator = DistributedTestingCoordinator(
        db_path=args.db_path, port=8081, enable_advanced_scheduler=True, enable_plugins=True
    )
    ...
    for task in tasks:
        ...
        result, execution_time = generate_task_result(
            task, worker, success=success, generate_anomaly=args.generate_anomalies
        )
    ...
    if not args.quick_test:
        ...
        for i in range(num_trend_tasks):
            ...
            task = generate_random_task(
                task_id, task_type, hardware_requirements=["cuda"], priority=3
            )
...
def main():
    ...
    parser.add_argument(
        "--enable-ml", action="store_true", default=True, help="Enable machine learning"
    )
    parser.add_argument(
        "--enable-visualization", action="store_true", default=True, help="Enable visualization"
    )
    parser.add_argument(
        "--quick-test", action="store_true", help="Run a quick test with minimal tasks"
    )
    parser.add_argument(
        "--generate-anomalies", action="store_true", help="Generate anomalous test results"
    )
    parser.add_argument(
        "--output-dir", default="./reports", help="Directory to store generated reports"
    )
