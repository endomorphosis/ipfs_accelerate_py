...
from hardware_workload_management import (
    HardwareWorkloadManager,
    WorkloadProfile,
    WorkloadType,
    WorkloadProfileMetric,
...
class HistoricalPerformanceScheduler(HardwareAwareScheduler):
    ...
    def select_worker(
        ...
        for hardware_id, hardware_profile, efficiency in compatible_hardware:
            ...
            if worker_id in available_workers:
                ...
                if load and load.has_capacity_for(test_requirements):
                    ...
                    available_hardware.append(
                        (worker_id, hardware_profile, performance_adjusted_efficiency)
                    )

        if not available_hardware:
            logger.warning(
                f"No available workers with capacity for test {test_requirements.test_id}"
            )
            return None
        ...
        logger.info(
            f"Selected worker {selected_worker_id} for test {test_requirements.test_id} "
        ...
        self._update_workload_preferences(
            workload_profile.workload_type.value, selected_worker_id, selected_efficiency
        )
    ...
    def _adjust_for_historical_performance(
        ...
        adjusted_efficiency = (
            1.0 - self.performance_weight
        ) * base_efficiency + self.performance_weight * performance_factor

        logger.debug(
            f"Adjusted efficiency for test {test_id} on worker {worker_id}: "
    ...
    def record_performance(self, performance_record: HistoricalPerformanceRecord) -> None:
        ...
        if len(self.performance_history[test_key]) > self.performance_history_window * 10:
            ...
            self.performance_history[test_key] = sorted(
                self.performance_history[test_key], key=lambda r: r.timestamp, reverse=True
    ...
    def _update_prediction_model(self, test_key: str) -> None:
        ...
        for record in successful_records:
            ...
            if worker_key not in worker_averages:
                worker_averages[worker_key] = {"times": [], "average": 0.0}
...
class DeadlineAwareScheduler(HistoricalPerformanceScheduler):
    ...
    def select_worker(
        ...
        for hardware_id, hardware_profile, efficiency in compatible_hardware:
            ...
            if worker_id in available_workers:
                ...
                if load and load.has_capacity_for(test_requirements):
                    ...
                    available_hardware.append(
                        (worker_id, hardware_profile, deadline_adjusted_efficiency)
                    )
        ...
        if self._is_test_urgent(test_requirements.test_id):
            ...
            fastest_worker = self._find_fastest_worker(
                test_requirements.test_id, available_hardware
            )
            ...
            if fastest_worker:
                ...
                logger.info(
                    f"Selected worker {selected_worker_id} for URGENT test {test_requirements.test_id} "
    ...
    def register_test_deadline(self, deadline: TestDeadline) -> None:
        ...
        logger.info(
            f"Registered deadline for test {deadline.test_id}: "
            f"{deadline.deadline.isoformat()}, priority {deadline.priority}"
        )

    def update_test_deadline_status(
        self, test_id: str, completed: bool, actual_duration: Optional[float] = None
    ) -> None:
        """
        ...
        logger.debug(
            f"Updated deadline status for test {test_id}: "
    ...
    def _update_test_priority(self, test_requirements: TestRequirements) -> None:
        ...
        if urgency_factor > 0:
            ...
            if boosted_priority < deadline.priority:
                logger.info(
                    f"Boosted priority for test {test_id} from {deadline.priority} to {boosted_priority} "
                    f"due to deadline in {time_remaining / 60:.1f} minutes"
                )

    def _adjust_for_deadline_pressure(
        self, base_efficiency: float, test_id: str, worker_id: str, hardware_id: str
    ) -> float:
        """
        ...
        adjusted_efficiency = (
            1.0 - self.deadline_weight
        ) * base_efficiency + self.deadline_weight * base_efficiency * deadline_factor

        logger.debug(
            f"Adjusted efficiency for test {test_id} based on deadline: "
    ...
    def _find_fastest_worker(
        self, test_id: str, available_hardware: List[Tuple[str, Any, float]]
...
class TestTypeSpecificScheduler(DeadlineAwareScheduler):
    ...
    def _initialize_test_type_configs(self) -> None:
        ...
        self.test_type_configs["default"] = TestTypeConfiguration(
            ...
            scheduling_parameters={"batch_scheduling": False, "dynamic_scaling": False},
    ...
    def register_test_type_config(self, config: TestTypeConfiguration) -> None:
        ...
        logger.info(
            f"Registered configuration for test type {config.test_type} with strategy {config.strategy.value}"
        )
...
class MLBasedScheduler(TestTypeSpecificScheduler):
    ...
    def select_worker(
        ...
        for hardware_id, hardware_profile, efficiency in compatible_hardware:
            ...
            if worker_id in available_workers:
                ...
                if load and load.has_capacity_for(test_requirements):
                    ...
                    combined_score = (
                        1.0 - self.ml_weight
                    ) * deadline_adjusted_efficiency + self.ml_weight * ml_score
        ...
        # Update preferences for this workload type
        self._update_workload_preferences(
            workload_profile.workload_type.value, selected_worker_id, selected["combined_score"]
        )
        ...
        self._record_scheduling_decision(
            test_requirements.test_id, selected_worker_id, selected["features"]
        )

        return selected_worker_id

    def record_execution_result(
        self, test_id: str, worker_id: str, execution_time: float, success: bool
    ) -> None:
        """
        ...
        for decision in self.scheduling_decisions:
            if decision.test_id == test_id and decision.worker_id == worker_id:
                ...
                logger.debug(
                    f"Updated scheduling decision for test {test_id} with execution time {execution_time:.2f}s"
                )
    ...
    def _extract_features(
        ...
        features["estimated_duration"] = (
            float(test_requirements.expected_duration or 60.0) / 3600.0
        )  # Normalize to hours

        # Hardware features
        features["hardware_type"] = self._encode_categorical(hardware_profile.hardware_class.value)
        features["compute_units"] = (
            float(hardware_profile.compute_units) / 100.0
        )  # Normalize to reasonable range
        features["memory_gb"] = float(hardware_profile.memory.total_bytes) / (
            1024**3 * 32.0
        )  # Normalize to 32GB

        # Worker state
        features["worker_load"] = (
            worker_load.calculate_load_score()
            if hasattr(worker_load, "calculate_load_score")
            else 0.5
        )
        ...
        if test_key in self.performance_history:
            worker_records = [
                r for r in self.performance_history[test_key] if r.worker_id == worker_id
            ]

            if worker_records:
                # Success rate
                features["historical_success_rate"] = sum(
                    1 for r in worker_records if r.success
                ) / len(worker_records)
                ...
                if successful_times:
                    features["historical_avg_time"] = (
                        sum(successful_times) / len(successful_times) / 3600.0
                    )  # Normalize to hours
                else:
        ...
        if test_requirements.test_id in self.test_deadlines:
            ...
            if time_remaining <= 0:
                ...
                urgency_seconds = self.urgency_threshold * 60
                features["deadline_pressure"] = max(
                    0, min(1.0, 1.0 - (time_remaining / urgency_seconds))
                )
        else:
    ...
    def _record_scheduling_decision(
        self, test_id: str, worker_id: str, features: Dict[str, float]
    ) -> None:
        """
        ...
        if len(self.scheduling_decisions) > 1000:
            ...
            self.scheduling_decisions = sorted(
                self.scheduling_decisions, key=lambda d: d.timestamp, reverse=True
            )[:1000]
    ...
    def _train_model(self) -> None:
        ...
        if len(training_data) < self.min_training_samples:
            logger.info(
                f"Not enough training data: {len(training_data)} samples, need {self.min_training_samples}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Trained ML scheduler model version {self.model_version} with {len(training_data)} samples"
            )
