...
def run_tests(args):
    ...
    if args.test_all:
        ...
        if args.test_worker_discovery:
            ...
            test_suite.addTests(
                test_loader.discover(tests_dir, pattern="test_worker_auto_discovery_with_ci.py")
            )
...
def parse_args():
    ...
    example_parser.add_argument(
        "--provider", help="CI provider type (github, gitlab, jenkins, etc.)"
    )
    example_parser.add_argument("--config", help="Path to CI provider configuration file")
    example_parser.add_argument(
        "--workers", type=int, help="Number of workers (for worker_auto_discovery example)"
    )

    # Test mode
    test_parser = subparsers.add_parser("test", help="Run CI/CD integration tests")
    test_parser.add_argument(
        "--test-all", action="store_true", help="Run all CI/CD integration tests"
    )
    test_parser.add_argument(
        "--test-ci-providers", action="store_true", help="Run CI provider interface tests"
    )
    test_parser.add_argument(
        "--test-worker-discovery",
        action="store_true",
        help="Run worker auto-discovery with CI integration tests",
    )
    ...
    if args.mode == "test" and not (
        args.test_all or args.test_ci_providers or args.test_worker_discovery
    ):
        args.test_all = True
