...
from test_dependency_manager import TestDependencyManager, DependencyType, Dependency
...
class HardwareTestMatcherDemo:
    ...
    def __init__(self):
        ...
        self.test_matcher = TestHardwareMatcher(
            hardware_capability_detector=self.hardware_detector, error_handler=self.error_handler
        )
    ...
    def setup_simulated_hardware(self):
        ...
        gpu_mid_capability = HardwareCapability(
            ...
            compute_units=22,
            memory_gb=6.0,
            supported_precisions=[PrecisionType.FP32, PrecisionType.FP16, PrecisionType.INT32],
            scores={
    ...
    def match_and_execute_tests(self):
        ...
        for test_id in test_ids:
            ...
            if test_id == "compute_test":
                ...
                if match.hardware_type == HardwareType.GPU:
                    ...
                    for hw in self.test_matcher.get_worker_capability(
                        match.worker_id
                    ).hardware_capabilities:
                        if hw.hardware_type == HardwareType.GPU and hw.capabilities.get(
                            "tensor_cores", False
                        ):
        ...
        for test_id in test_ids:
            if test_id in matches:
                ...
                metrics = self.test_matcher._get_test_performance_metrics(
                    test_id, match.hardware_id
                )

                # Update dependency manager
                if metrics["success_rate"] == 1.0:
                    self.dependency_manager.update_test_status(
                        test_id=test_id, status="completed", result={"success": True}
                    )
                else:
                    self.dependency_manager.update_test_status(
                        test_id=test_id, status="failed", result={"success": False}
                    )
    ...
    def analyze_performance_impact(self):
        ...
        for test_id, match in updated_matches.items():
            ...
            print(f"Test: {test_id}")
            print(
                f"  Original match: {original_match.hardware_type.value} ({original_match.hardware_id})"
            )
    ...
    def demonstrate_specialized_matchers(self):
        ...
        # Print header
        print(
            f"{'Factor':<25} {'Default':<10} {'Compute':<10} {'Memory':<10} {'GPU':<10} {'Precision':<10}"
        )
        print("-" * 75)
        ...
        for factor in factor_names:
            ...
            print(
                f"{factor:<25} {default_weight:<10.2f} {compute_weight:<10.2f} "
        ...
        print(f"Test: {test_id}")
        print(
            f"  Default matcher: {default_match.hardware_type.value} (Score: {default_match.match_score:.4f})"
        )
        print(
            f"  Compute matcher: {compute_match.hardware_type.value} (Score: {compute_match.match_score:.4f})"
        )
        print(
            f"  Memory matcher: {memory_match.hardware_type.value} (Score: {memory_match.match_score:.4f})"
        )
        print(
            f"  GPU matcher: {gpu_match.hardware_type.value} (Score: {gpu_match.match_score:.4f})"
        )
        print(
            f"  Precision matcher: {precision_match.hardware_type.value} (Score: {precision_match.match_score:.4f})"
        )
