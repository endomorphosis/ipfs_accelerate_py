...
class BrowserFailureInjectorTest:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized failure injector test with browser={browser_name}, platform={platform}"
        )
    ...
    async def test_failure_type(self, failure_type: FailureType) -> Dict[str, Any]:
        ...
        failure_results = {"failure_type": failure_type.value, "intensities": {}, "success": True}
    ...
    async def test_failure_with_intensity(
        self, failure_type: FailureType, intensity: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            with open(markdown_path, "w") as f:
                ...
                for failure_type, results in self.results["failure_types"].items():
                    ...
                    moderate = (
                        "✅" if intensities.get("moderate", {}).get("success", False) else "❌"
                    )
                    severe = "✅" if intensities.get("severe", {}).get("success", False) else "❌"

                    # Calculate success rate
                    success_count = sum(
                        1 for i, r in intensities.items() if r.get("success", False)
                    )
                    total_count = len(intensities)
                    success_rate = success_count / total_count if total_count > 0 else 0

                    f.write(
                        f"| {failure_type} | {mild} | {moderate} | {severe} | {success_rate:.2%} |\n"
                    )
                ...
                if self.results["success_rate"] > 0.9:
                    f.write(
                        "The browser failure injector is working correctly for most failure types and intensities.\n"
                    )
                elif self.results["success_rate"] > 0.7:
                    f.write(
                        "The browser failure injector is working for most failure types, but some issues exist.\n"
                    )
                else:
                    f.write(
                        "The browser failure injector has significant issues that need to be addressed.\n"
                    )
    ...
    def print_summary(self) -> None:
        ...
        for failure_type, results in self.results["failure_types"].items():
            ...
            # Format each intensity result
            mild = (
                "✅ SUCCESS" if intensities.get("mild", {}).get("success", False) else "❌ FAILED"
            )
            moderate = (
                "✅ SUCCESS"
                if intensities.get("moderate", {}).get("success", False)
                else "❌ FAILED"
            )
            severe = (
                "✅ SUCCESS" if intensities.get("severe", {}).get("success", False) else "❌ FAILED"
            )
        ...
        if self.results["success_rate"] > 0.9:
            ...
            for failure_type, results in self.results["failure_types"].items():
                success_count = sum(
                    1 for i, r in results.get("intensities", {}).items() if r.get("success", False)
                )
...
async def main():
    ...
    parser.add_argument(
        "--failure", help="Specific failure type to test (or omit for all failure types)"
    )
    parser.add_argument(
    ...
    if not save_path:
        ...
        if args.failure:
            save_path = os.path.join(
                reports_dir, f"failure_injector_{args.browser}_{args.failure}_{timestamp}.json"
            )
        else:
            save_path = os.path.join(
                reports_dir, f"failure_injector_{args.browser}_{timestamp}.json"
            )
