...
async def test_hardware_integration(db_path=None, enable_browser_detection=False):
    ...
    hardware_integration = CoordinatorHardwareIntegration(
        coordinator=coordinator, db_path=db_path, enable_browser_detection=enable_browser_detection
    )
    ...
    detector = HardwareCapabilityDetector(
        db_path=db_path, enable_browser_detection=enable_browser_detection
    )
    ...
    task_basic = {
        "task_id": "task-basic",
        "type": "test",
        "requirements": {"hardware": {"required_types": ["cpu"]}},
    }
    ...
    can_handle_basic = await hardware_integration._enhanced_can_worker_handle_task(
        worker_id, task_basic
    )
    can_handle_advanced = await hardware_integration._enhanced_can_worker_handle_task(
        worker_id, task_advanced
    )
    can_handle_impossible = await hardware_integration._enhanced_can_worker_handle_task(
        worker_id, task_impossible
    )
...
def main():
    ...
    anyio.run(
        test_hardware_integration(
            db_path=args.db_path, enable_browser_detection=args.enable_browser_detection
        )
    )
