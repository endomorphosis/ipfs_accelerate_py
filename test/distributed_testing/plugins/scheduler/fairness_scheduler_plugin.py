...
class FairnessSchedulerPlugin(BaseSchedulerPlugin):
    ...
    async def schedule_task(
        ...
        if self.config["enable_quotas"]:
            ...
            if project_id in self.project_quotas:
                ...
                if project_usage >= self.project_quotas[project_id]:
                    logger.info(
                        f"Project {project_id} exceeded quota {self.project_quotas[project_id]}"
                    )
                    return None
        ...
        if user_id in self.consecutive_allocations:
            if self.consecutive_allocations[user_id] >= self.config["max_consecutive_same_user"]:
                ...
                if len(self.user_allocation) > 1:  # Only apply if there are other users
                    logger.info(
                        f"User {user_id} has had {self.consecutive_allocations[user_id]} consecutive tasks, prioritizing other users"
                    )
        ...
        if worker_id:
            ...
            self.historical_usage[user_id]["tasks"].append(
                {"task_id": task_id, "timestamp": timestamp, "worker_id": worker_id}
            )

            # Limit history size
            max_history = self.config["history_window_size"]
            if len(self.historical_usage[user_id]["tasks"]) > max_history:
                self.historical_usage[user_id]["tasks"] = self.historical_usage[user_id]["tasks"][
                    -max_history:
                ]

            if self.config["detailed_logging"]:
                logger.info(
                    f"Scheduled task {task_id} to worker {worker_id} using {self._active_strategy.value} strategy"
                )
                logger.info(
                    f"User {user_id} allocation: {self.user_allocation[user_id]}, Project {project_id} allocation: {self.project_allocation[project_id]}"
                )
        ...
        if task_id in self.tasks:
            ...
            if status == "completed" or status == "failed":
                ...
                if project_id in self.project_allocation:
                    self.project_allocation[project_id] = max(
                        0, self.project_allocation[project_id] - 1
                    )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        fairness_metrics = {
            ...
            "active_projects": len(
                [p for p, count in self.project_allocation.items() if count > 0]
            ),
        }
    ...
    def _calculate_fair_shares(self) -> None:
        ...
        total_weight = sum(self.user_weights.values()) or (
            total_users * self.config["default_user_weight"]
        )
        ...
        project_weight_sum = sum(self.project_weights.values()) or (
            total_projects * self.config["default_project_weight"]
        )
    ...
    def _fair_share_scheduling(
        self, task_id: str, task_data: Dict[str, Any], available_workers: Dict[str, Dict[str, Any]]
    ) -> Optional[str]:
        """
        ...
        user_share_ratio = (
            current_user_allocation / user_fair_share if user_fair_share > 0 else float("inf")
        )
        project_share_ratio = (
            current_project_allocation / project_fair_share
            if project_fair_share > 0
            else float("inf")
        )
        ...
        if is_over_share:
            ...
            other_active_users = sum(
                1 for u, count in self.user_allocation.items() if u != user_id and count > 0
            )
        ...
        for worker_id, worker_data in available_workers.items():
            ...
            if "hardware_requirements" in task_data and self._match_hardware(
                task_data, {worker_id: worker_data}
            ):
                score += 5.0
    ...
    def _priority_based_scheduling(
        self, task_id: str, task_data: Dict[str, Any], available_workers: Dict[str, Dict[str, Any]]
    ) -> Optional[str]:
        """
    ...
    def _match_hardware(
        self, task_data: Dict[str, Any], available_workers: Dict[str, Dict[str, Any]]
    ) -> Optional[str]:
        """
