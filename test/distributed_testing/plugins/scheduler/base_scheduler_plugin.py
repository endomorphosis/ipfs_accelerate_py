...
class BaseSchedulerPlugin(SchedulerPluginInterface):
    ...
    def __init__(
        self, name: str, version: str, description: str, strategies: List[SchedulingStrategy] = None
    ):
        """
        ...
        self._active_strategy = (
            self._strategies[0] if self._strategies else SchedulingStrategy.ROUND_ROBIN
        )
    ...
    async def update_task_status(
        ...
        if status == "completed":
            ...
            if worker_id:
                ...
                if execution_time is not None:
                    self.metrics["worker_utilization"][worker_id]["total_execution_time"] += (
                        execution_time
                    )
    ...
    async def update_worker_status(
        self, worker_id: str, status: str, capabilities: Optional[Dict[str, Any]] = None
    ) -> None:
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        self.metrics["active_workers"] = len(
            [status for status in self.worker_status.values() if status == "active"]
        )

        # Calculate worker load
        total_workers = len(self.workers)
        self.metrics["avg_worker_load"] = (
            sum(self.worker_load.values()) / total_workers if total_workers > 0 else 0.0
        )
    ...
    def _performance_based_scheduling(
