...
class SchedulerPluginRegistry:
    ...
    def __init__(self, plugin_dirs: List[str] = None):
        ...
        logger.info(
            f"SchedulerPluginRegistry initialized with {len(self.plugin_dirs)} plugin directories"
        )
                ...
                try:
                    ...
                    for attr_name in dir(module):
                        ...
                        if (
                            ...
                            and issubclass(attr, SchedulerPluginInterface)
                            and attr is not SchedulerPluginInterface
                        ):
                            # Register the plugin
    ...
    async def unload_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if self.active_plugin == plugin_name:
                self.active_plugin = (
                    next(iter(self.plugin_instances)) if self.plugin_instances else None
                )
    ...
    async def initialize_plugin(
        self, plugin_name: str, coordinator: Any, config: Dict[str, Any] = None
    ) -> bool:
        """
    ...
    async def schedule_task(
        ...
        try:
            ...
            worker_id = await plugin.schedule_task(
                task_id, task_data, available_workers, worker_load
            )

            if worker_id:
                logger.debug(
                    f"Scheduled task {task_id} to worker {worker_id} using {self.active_plugin} plugin"
                )
            else:
                logger.debug(
                    f"No suitable worker found for task {task_id} using {self.active_plugin} plugin"
                )
            ...
            logger.error(
                f"Error scheduling task {task_id} with plugin {self.active_plugin}: {str(e)}"
            )
            return None
