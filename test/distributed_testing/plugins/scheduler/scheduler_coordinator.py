...
class SchedulerCoordinator:
    ...
    async def initialize(self) -> bool:
        ...
        logger.info(
            f"Discovered {len(discovered_plugins)} scheduler plugins: {', '.join(discovered_plugins)}"
        )
...
class SchedulerPluginWrapper:
    ...
    async def schedule_pending_tasks(self) -> int:
        ...
        if not active_plugin:
            if self.original_scheduler and hasattr(
                self.original_scheduler, "schedule_pending_tasks"
            ):
        ...
        for worker_id, worker in (
            coordinator.workers.items() if hasattr(coordinator, "workers") else {}
        ):
        ...
        for task_id, worker_id in (
            coordinator.running_tasks.items() if hasattr(coordinator, "running_tasks") else {}
        ):
        ...
        for task_id in list(pending_tasks):
            ...
            worker_id = await self.registry.schedule_task(
                task_id, task_data, available_workers_with_capacity, worker_task_count
            )
