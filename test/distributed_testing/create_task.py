...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create tasks for the distributed testing framework"
    )
    parser.add_argument(
        "--coordinator", default="http://localhost:8080", help="URL of coordinator server"
    )
    ...
    parser.add_argument(
        "--type",
        choices=["benchmark", "test", "custom"],
        required=True,
        help="Type of task to create",
    )
    parser.add_argument(
        "--priority", type=int, default=1, help="Task priority (higher number = higher priority)"
    )
    ...
    parser.add_argument(
        "--precision", default="fp32", help="Precision for benchmark task (fp32, fp16, int8, etc.)"
    )
    parser.add_argument(
        "--iterations", type=int, default=10, help="Number of iterations for benchmark task"
    )

    # Arguments for test tasks
    parser.add_argument("--test-file", help="Path to test file for test task")
    parser.add_argument(
        "--test-args",
    ...
    parser.add_argument("--name", help="Name for custom task")
    parser.add_argument(
        "--args",
        type=lambda s: s.split(","),
    ...
    parser.add_argument("--min-memory-gb", type=int, help="Minimum memory required (GB)")
    parser.add_argument("--min-cuda-compute", type=float, help="Minimum CUDA compute capability")
    ...
    if not api_key:
        ...
        if not api_key:
            logger.warning(
                "No API key provided or found in security config. Authentication may fail."
            )
