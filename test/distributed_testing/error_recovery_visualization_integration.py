...
class ErrorRecoveryVisualizationIntegration:
    ...
    async def generate_dashboard(self, days: int = 30, file_format: str = "png") -> str:
        ...
        self.visualizer = ErrorRecoveryVisualizer(
            output_dir=self.output_dir, file_format=file_format
        )

        # Generate dashboard
        dashboard_path = self.visualizer.generate_recovery_dashboard(
            performance_data, time_series_data, progressive_data
        )

        return dashboard_path

    async def generate_individual_visualizations(
        self, days: int = 30, file_format: str = "png"
    ) -> Dict[str, str]:
        """
        ...
        results["strategy_performance"] = self.visualizer.visualize_strategy_performance(
            performance_data, filename="strategy_performance_dashboard"
        )

        # Error recovery heatmap
        results["error_recovery_heatmap"] = self.visualizer.visualize_error_recovery_heatmap(
            performance_data, filename="error_recovery_heatmap"
        )

        # Performance trends
        results["performance_trends"] = self.visualizer.visualize_performance_trends(
            time_series_data, filename="performance_trend_graphs"
        )

        # Progressive recovery
        results["progressive_recovery"] = self.visualizer.visualize_progressive_recovery(
            progressive_data, filename="progressive_recovery_analysis"
        )

        return results

    async def generate_visualization_for_error_type(
        self, error_type: str, file_format: str = "png"
    ) -> str:
        """
        ...
        try:
            ...
            for i, name in enumerate(strategy_ids):
                ax2.annotate(
                    name,
                    (recovery_times[i], success_rates[i]),
                    xytext=(5, 0),
            ...
            output_path = os.path.join(
                self.output_dir, f"error_type_{error_type.lower()}.{file_format}"
            )
...
def create_visualization_integration(
    recovery_system=None, db_connection=None, output_dir: str = None
) -> ErrorRecoveryVisualizationIntegration:
    """
    ...
    return ErrorRecoveryVisualizationIntegration(
        recovery_system=recovery_system, db_connection=db_connection, output_dir=output_dir
    )
...
async def async_main(args):
    ...
    if args.db_path:
        ...
        try:
            db_connection = duckdb.connect(
                "./error_recovery_with_performance_test_db/recovery_data.duckdb"
            )
        except:
    ...
    integration = create_visualization_integration(
        recovery_system=recovery_system, db_connection=db_connection, output_dir=args.output_dir
    )

    if args.command == "dashboard":
        # Generate dashboard
        dashboard_path = await integration.generate_dashboard(
            days=args.days, file_format=args.format
        )
        ...
        vis_paths = await integration.generate_individual_visualizations(
            days=args.days, file_format=args.format
        )
        ...
        vis_path = await integration.generate_visualization_for_error_type(
            args.error_type, file_format=args.format
        )
...
def main():
    ...
    individual_parser = subparsers.add_parser(
        "individual", help="Generate individual visualizations"
    )

    # Error type visualization command
    error_type_parser = subparsers.add_parser(
        "error-type", help="Generate visualization for specific error type"
    )
