...
from test_dependency_manager import TestDependencyManager, DependencyStatus
...
class ExecutionOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"Execution Orchestrator initialized with strategy: {strategy.name}, "
    ...
    def create_execution_groups(self) -> None:
        ...
        logger.info(
            f"Created {len(self.execution_groups)} execution groups with "
    ...
    def adjust_max_workers(self) -> None:
        ...
        if self.strategy == ExecutionStrategy.SIMPLE:
            ...
            new_max_workers = min(
                len(self.execution_contexts), max(cpu_count * 2, memory_based_workers)
            )
        ...
        if new_max_workers != self.max_workers:
            logger.info(
                f"Adjusted max workers from {self.max_workers} to {new_max_workers} "
    ...
    def select_tests_for_execution(self) -> List[str]:
        ...
        if self.strategy == ExecutionStrategy.SIMPLE:
            ...
            prioritized_queue = sorted(
                ...
                key=lambda test_id: self.execution_contexts[test_id].metadata.get(
                    "deadline", float("inf")
                ),
            )
    ...
    def execute_test(self, test_id: str, worker_id: str) -> None:
        ...
        try:
            ...
            # Update the dependency manager
            self.dependency_manager.update_test_status(
                test_id=test_id, status="failed", result={"success": False, "error": str(e)}
            )
    ...
    def execute_all_tests(self) -> Dict[str, Any]:
        ...
        if self.pre_execution_hook:
            try:
                self.pre_execution_hook(
                    self.max_workers, self.strategy, len(self.execution_contexts)
                )
            except Exception as e:
        ...
        logger.info(
            f"Starting execution of {len(self.execution_contexts)} tests "
        ...
        while True:
            ...
            if selected_tests:
                ...
                self.metrics["max_parallelism"] = max(
                    self.metrics["max_parallelism"], current_parallelism
                )

                logger.debug(
                    f"Executing {len(selected_tests)} tests, "
            ...
            all_completed = all(
                context.status
                in [ExecutionStatus.COMPLETED, ExecutionStatus.FAILED, ExecutionStatus.SKIPPED]
        ...
        logger.info(
            f"Execution completed in {self.metrics['execution_time']:.2f} seconds with "
    ...
    async def execute_all_tests_async(self) -> Dict[str, Any]:
        ...
        self.start_time = time.time()
        logger.info(
            f"Starting async execution of {len(self.execution_contexts)} tests "
            f"with {self.max_workers} workers using {self.strategy.name} strategy"
        )
        ...
        logger.info(
            f"Async execution completed in {self.metrics['execution_time']:.2f} seconds with "
            f"{self.metrics['completed_tests']} successful, "
            f"{self.metrics['failed_tests']} failed, and "
            f"{self.metrics['skipped_tests']} skipped tests"
        )
    ...
    def visualize_execution_plan(self, output_format: str = "text") -> str:
        ...
        if output_format == "text":
            ...
            lines = []
            lines.append(
                f"Execution Plan ({self.strategy.name} strategy with {self.max_workers} workers)"
            )
            lines.append("-" * 80)

            for group_id, group in sorted(
                self.execution_groups.items(), key=lambda x: x[1].metadata.get("level", 0)
            ):
...
if __name__ == "__main__":
    ...
    for test_id, result in results["test_results"].items():
        ...
        print(
            f"{test_id}: {status} in {execution_time:.2f}s"
            if execution_time
            else f"{test_id}: {status}"
        )
