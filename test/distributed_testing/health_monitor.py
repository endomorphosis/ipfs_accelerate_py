...
class HealthMonitor:
    ...
    async def check_worker_health(self):
        ...
        for worker_id, worker in list(self.coordinator.workers.items()):
            ...
            try:
                ...
                logger.warning(
                    f"Invalid heartbeat timestamp for worker {worker_id}: {last_heartbeat_str}"
                )
            ...
            if (
                worker.get("status") in ["active", "idle"]
                and heartbeat_age > self.heartbeat_timeout
            ):
                logger.warning(
                    f"Worker {worker_id} heartbeat is {heartbeat_age:.1f} seconds old (timeout: {self.heartbeat_timeout}s)"
                )
    ...
    async def check_task_health(self):
        ...
        for task_id, worker_id in list(self.coordinator.running_tasks.items()):
            ...
            if running_time > timeout:
                logger.warning(
                    f"Task {task_id} has been running for {running_time:.1f} seconds on worker {worker_id} (timeout: {timeout}s)"
                )
    ...
    async def handle_worker_failure(self, worker_id: str):
        ...
        for task_id, assigned_worker_id in list(self.coordinator.running_tasks.items()):
            if assigned_worker_id == worker_id:
                ...
                # Update task status
                await self.coordinator._update_task_status(
                    task_id, "failed", {"error": f"Worker {worker_id} failed or disconnected"}
                )
                ...
                """,
                    (
                        ...
                        datetime.fromisoformat(
                            self.coordinator.tasks[task_id].get(
    ...
    async def handle_task_timeout(self, task_id: str, worker_id: str):
        ...
        # Update task status
        await self.coordinator._update_task_status(
            task_id, "failed", {"error": "Task execution timed out"}
        )
        ...
        """,
            (
                ...
                "failed",
                datetime.fromisoformat(
                    self.coordinator.tasks[task_id].get("started", datetime.now().isoformat())
                ),
                datetime.now(),
                0,
    ...
    async def attempt_worker_recovery(self, worker_id: str):
        ...
        if attempts > self.max_recovery_attempts:
            logger.warning(
                f"Worker {worker_id} has failed {attempts} times, marking as permanently offline"
            )
            ...
            logger.error(
                f"Worker {worker_id} has been marked as permanently failed after {attempts} recovery attempts"
            )
                ...
                if worker_id in self.coordinator.workers:
                    ...
                    if worker.get("status") in ["active", "idle"]:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error updating recovery status for worker {worker_id}: {str(e)}"
                            )
                        ...
                        logger.warning(
                            f"Worker {worker_id} has not recovered, status: {worker.get('status')}"
                        )
            ...
            try:
                ...
                """,
                    (
                        ...
                        json.dumps(
                            {"worker_summary": worker_summary, "task_summary": task_summary}
                        ),
                    ),
                )
                ...
                if status == "critical":
                    logger.critical(
                        f"SYSTEM HEALTH CRITICAL: Only {worker_summary['active_percentage']:.1f}% of workers are active!"
                    )
    ...
    def update_task_timeout_estimate(self, task_type: str, execution_time: float):
        ...
        logger.debug(
            f"Updated timeout estimate for {task_type}: {self.task_timeout_estimates[key]:.2f}s"
        )
    ...
    def get_worker_health_summary(self) -> Dict[str, Any]:
        ...
        active_workers = sum(
            1 for w in self.coordinator.workers.values() if w.get("status") == "active"
        )
        idle_workers = sum(
            1 for w in self.coordinator.workers.values() if w.get("status") == "idle"
        )
        offline_workers = sum(
            1 for w in self.coordinator.workers.values() if w.get("status") == "offline"
        )
        recovery_pending = sum(
            1 for w in self.coordinator.workers.values() if w.get("status") == "recovery_pending"
        )
        permanent_failure = sum(
            1 for w in self.coordinator.workers.values() if w.get("status") == "permanent_failure"
        )
