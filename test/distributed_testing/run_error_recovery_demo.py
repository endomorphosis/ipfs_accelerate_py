...
class ErrorRecoveryDemo:
    ...
    def __init__(
        ...
        if failure_types is None or "all" in failure_types:
            ...
            self.failure_types = [
                FailureType(ft) for ft in failure_types if hasattr(FailureType, ft)
            ]
        ...
        logger.info(
            f"Initialized error recovery demo with browser={browser_name}, model={model_name}, platform={platform}"
        )
    ...
    def _create_browser_config(self) -> BrowserConfiguration:
        ...
        config = BrowserConfiguration(
            browser_name=self.browser_name, platform=self.platform, headless=True, timeout=30
        )
    ...
    async def run_test_with_failure(
        self, failure_type: FailureType, iteration: int, intensity: str = "moderate"
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Running test with {failure_type.value} failure (iteration {iteration}, intensity {intensity})"
        )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Recovery successful, running test with model {self.get_model_name_for_type()}"
                )
                test_start = time.time()
                test_result = await bridge.run_test(
                    model_name=self.get_model_name_for_type(), input_data="This is a test input"
                )
                ...
                if not result["success"]:
                    logger.error(
                        f"Test failed after recovery: {test_result.get('error', 'Unknown error')}"
                    )
                    result["error"] = (
                        f"Test failed after recovery: {test_result.get('error', 'Unknown error')}"
                    )
    ...
    def _print_summary(self):
        ...
        print("-" * 60)
        print(
            f"{'Failure Type':<20} {'Tests':<8} {'Recovery':<12} {'Success':<12} {'Avg Time (ms)':<15}"
        )
        print("-" * 60)

        for failure_type, stats in failure_stats.items():
            print(
                f"{failure_type:<20} {stats['total']:<8} {stats['recovery_rate']:.2%}{'':<5} {stats['success_rate']:.2%}{'':<5} {stats['avg_recovery_time']:.2f}"
            )
    ...
    def _save_markdown_report(self, path: str, report: Dict[str, Any]):
        ...
        try:
            with open(path, "w") as f:
                ...
                f.write(
                    f"- **Circuit Breaker:** {'Enabled' if report.get('circuit_breaker_enabled', False) else 'Disabled'}\n\n"
                )
                ...
                for failure_type in failure_stats:
                    ...
                    failure_stats[failure_type]["recovery_rate"] = (
                        recovered / total if total > 0 else 0
                    )
                    failure_stats[failure_type]["success_rate"] = (
                        success / total if total > 0 else 0
                    )

                f.write(f"## Results by Failure Type\n\n")
                f.write(
                    f"| Failure Type | Tests | Recovery Rate | Success Rate | Avg Recovery Time (ms) |\n"
                )
                f.write(
                    f"|--------------|-------|---------------|--------------|------------------------|\n"
                )
                ...
                for failure_type, stats in failure_stats.items():
                    f.write(
                        f"| {failure_type} | {stats['total']} | {stats['recovery_rate']:.2%} | {stats['success_rate']:.2%} | {stats['avg_recovery_time']:.2f} |\n"
                    )
                ...
                for i, result in enumerate(report["results"]):
                    f.write(
                        f"### Test {i + 1}: {result.get('failure_type', 'Unknown')} ({result.get('intensity', 'Unknown')})\n\n"
                    )
                    ...
                    f.write(
                        f"- **Simulation Mode:** {'Yes' if result.get('simulation_mode', False) else 'No'}\n"
                    )
                ...
                f.write(
                    f"Based on the test results, the following strategies were most effective:\n\n"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Browser Error Recovery Demo with Failure Injection"
    )
    parser.add_argument(
        "--browser",
        default="chrome",
    ...
    parser.add_argument(
        "--failures", default="all", help="Comma-separated list of failure types to test, or 'all'"
    )
    parser.add_argument(
        "--iterations", type=int, default=3, help="Number of iterations for each failure type"
    )
    parser.add_argument("--report", type=str, help="Path to save test report")
    parser.add_argument(
        "--circuit-breaker",
        action="store_true",
        dest="use_circuit_breaker",
        default=True,
    ...
    if not report_path:
        ...
        report_path = os.path.join(
            report_dir, f"error_recovery_demo_{args.browser}_{args.model}_{timestamp}.json"
        )
