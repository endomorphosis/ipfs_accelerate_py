...
class ErrorRecoveryWithPerformance:
    ...
    def __init__(
        ...
        if self.error_handler:
            ...
            for error_type in ErrorType:
                self.error_handler.register_error_hook(error_type, self._error_notification_hook)
    ...
    def _load_performance_history(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded performance history: {sum(len(records) for records in self.performance_history.values())} records, {sum(len(scores) for scores in self.strategy_scores.values())} scores"
            )
        except Exception as e:
    ...
    def _error_notification_hook(self, error_report: ErrorReport):
        ...
        logger.info(
            f"Error notification received: {error_report.error_id} - {error_report.error_type.value}"
        )
    ...
    async def recover(self, error_report: ErrorReport) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            success = await wait_for(
                strategy.execute(self._convert_error_report(error_report)), timeout=timeout
            )
            ...
            impact_score = self._calculate_impact_score(
                resource_diff, recovery_info["execution_time"], affected_tasks
            )
            ...
            if success:
                ...
                if (
                    next_level <= ProgressiveRecoveryLevel.LEVEL_5.value
                    and next_level > recovery_level
                ):
                    logger.info(
                        f"Progressive recovery: escalating from level {recovery_level} to {next_level} for error {error_report.error_id}"
                    )
            ...
            # Recovery timed out
            logger.error(
                f"Recovery timed out after {timeout} seconds for error {error_report.error_id}"
            )
            ...
            if next_level <= ProgressiveRecoveryLevel.LEVEL_5.value and next_level > recovery_level:
                logger.info(
                    f"Progressive recovery: escalating from level {recovery_level} to {next_level} for error {error_report.error_id} after timeout"
                )
                return await self.recover(error_report)
            ...
            if next_level <= ProgressiveRecoveryLevel.LEVEL_5.value and next_level > recovery_level:
                logger.info(
                    f"Progressive recovery: escalating from level {recovery_level} to {next_level} for error {error_report.error_id} after exception"
                )
                return await self.recover(error_report)
    ...
    async def _select_best_strategy(self, error_type: str, recovery_level: int) -> Tuple[Any, str]:
        ...
        if not strategies:
            logger.warning(
                f"No strategies found for error type {error_type}, using default retry strategy"
            )
        ...
        for strategy_id, strategy in strategies.items():
            ...
            if not score:
                ...
                if (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_1.value
                    and strategy.level.value == "low"
                ):
                    base_score = 0.8
                elif (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_2.value
                    and strategy.level.value == "low"
                ):
                    base_score = 0.7
                elif (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_2.value
                    and strategy.level.value == "medium"
                ):
                    base_score = 0.8
                elif (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_3.value
                    and strategy.level.value == "medium"
                ):
                    base_score = 0.8
                elif (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_4.value
                    and strategy.level.value == "high"
                ):
                    base_score = 0.8
                elif (
                    recovery_level == ProgressiveRecoveryLevel.LEVEL_5.value
                    and strategy.level.value == "critical"
                ):
                    base_score = 0.9
        ...
        if scored_strategies:
            ...
            logger.info(
                f"Selected strategy {strategy_id} for error type {error_type} with score {score:.2f}"
            )
            return strategy, strategy_id

        # Fallback to default retry strategy
        logger.warning(
            f"No scored strategies found for error type {error_type}, using default retry strategy"
        )
        strategy = RetryStrategy(self.coordinator)
        return strategy, "retry"
    ...
    def _record_timeout(self, error_type: str, strategy_id: str, timeout: float, success: bool):
        ...
        if self.db_connection:
            try:
                ...
                """,
                    (error_type, strategy_id, timeout, datetime.now(), success),
                )
            except Exception as e:
    ...
    def _update_strategy_scores(self, strategy_id: str, error_type: str):
        ...
        records = [
            record
            for record in self.performance_history.get(strategy_id, [])
        ...
        average_recovery_time = (
            sum(record.execution_time_seconds for record in successful_records)
        ...
        task_recovery_rate = (
            sum(task_recovery_rates) / len(task_recovery_rates) if task_recovery_rates else 1.0
        )
            ...
            try:
                ...
                """,
                    (
                        ...
                        datetime.now(),
                        success,
                        json.dumps({"old_level": old_level, "new_level": new_level, **details}),
                    ),
                )
            except Exception as e:
    ...
    def _calculate_impact_score(
        self, resource_diff: Dict[str, float], execution_time: float, affected_tasks: int
    ) -> float:
        """
        ...
        process_memory_impact = (
            abs(resource_diff.get("process_memory_mb", 0.0)) / 1000.0
        )  # Normalize to 0-1 range
        process_memory_impact = min(process_memory_impact, 1.0)  # Cap at 1.0

        # Time impact (normalized to 0-1 range)
        time_impact = min(
            execution_time / 60.0, 1.0
        )  # Cap at 1.0 (anything over 60s is high impact)
    ...
    async def _check_stability(self) -> float:
        ...
        try:
            ...
            if self.coordinator and hasattr(self.coordinator, "worker_connections"):
                ...
                active_workers = sum(
                    1 for ws in self.coordinator.worker_connections.values() if not ws.closed
                )
                worker_connections_active = (
                    active_workers == total_workers if total_workers > 0 else True
                )
            ...
            if (
                self.coordinator
                and hasattr(self.coordinator, "running_tasks")
                and hasattr(self.coordinator, "pending_tasks")
            ):
                # Healthy if either tasks are running or we've processed all pending tasks
                tasks_processing = (
                    len(self.coordinator.running_tasks) > 0
                    or len(self.coordinator.pending_tasks) == 0
                )
    ...
    def get_performance_metrics(
        self, strategy_id: Optional[str] = None, error_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "summary": {
                ...
                "average_success_rate": sum(s["success_rate"] for s in strategy_stats.values())
                / len(strategy_stats)
                if strategy_stats
                else 0.0,
                "average_recovery_time": sum(
                    s["avg_recovery_time"] for s in strategy_stats.values()
                )
                / len(strategy_stats)
                if strategy_stats
                else 0.0,
    ...
    def get_adaptive_timeouts(
        self, error_type: Optional[str] = None, strategy_id: Optional[str] = None
    ) -> Dict[str, float]:
        """
    ...
    async def analyze_recovery_performance(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_metrics = {
                ...
                "avg_stability_score": sum(r["stability_score"] for r in records) / len(records)
                if records
                else 0.0,
            ...
            for et, et_records in by_error_type.items():
                error_type_metrics[et] = {
                    ...
                    "avg_execution_time": sum(r["execution_time"] for r in et_records)
                    / len(et_records)
                    if et_records
                    else 0.0,
                    "avg_impact_score": sum(r["impact_score"] for r in et_records) / len(et_records)
                    if et_records
                    else 0.0,
                    "avg_stability_score": sum(r["stability_score"] for r in et_records)
                    / len(et_records)
                    if et_records
                    else 0.0,
            ...
            for sid, s_records in by_strategy.items():
                ...
                strategy_metrics[sid] = {
                    ...
                    "avg_execution_time": sum(r["execution_time"] for r in s_records)
                    / len(s_records)
                    if s_records
                    else 0.0,
                    "avg_impact_score": sum(r["impact_score"] for r in s_records) / len(s_records)
                    if s_records
                    else 0.0,
                    "avg_stability_score": sum(r["stability_score"] for r in s_records)
                    / len(s_records)
                    if s_records
                    else 0.0,
                    "success_rate": sum(1 for r in s_records if r["success"]) / len(s_records)
                    if s_records
                    else 0.0,
                }
            ...
            for record in records:
                ...
                if day_str in days_map:
                    ...
                    if current_count > 1:
                        days_map[day_str]["avg_execution_time"] = (
                    ...
                    # Update success rate
                    days_map[day_str]["success_rate"] = (
                        days_map[day_str]["successful_recoveries"]
                        / days_map[day_str]["total_recoveries"]
                    )
...
if __name__ == "__main__":
    ...
    async def run_demo():
        ...
        error_report = error_handler.create_error_report(
            Exception("Test error"), {"component": "test", "operation": "demo", "critical": False}
        )
