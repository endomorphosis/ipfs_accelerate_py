...
class DiscordConnector(ExternalSystemInterface):
            ...
            if self.use_bot_api:
                ...
                async with self.session.post(url, headers=headers, json=payload) as response:
                    if response.status != 200:
                        ...
                        logger.error(
                            f"Failed to send message via Bot API: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
                ...
                async with self.session.post(self.webhook_url, json=payload) as response:
                    if response.status not in (200, 204):
                        error_text = await response.text()
                        logger.error(
                            f"Failed to send message via webhook: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
                            "error": f"Failed to send message: {response.status} - {error_text}",
    ...
    async def _send_embed(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Send as embed
        return await self._send_message({"channel_id": channel_id, "embeds": [embed]})
