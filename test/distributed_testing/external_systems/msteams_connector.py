...
class MSTeamsConnector(ExternalSystemInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        self.graph_api_endpoint = config.get(
        ...
        if self.use_graph_api:
            # Graph API mode
            if not self.graph_api_token and not (
                self.tenant_id and self.client_id and self.client_secret
            ):
                logger.error(
                    "When using Graph API, either graph_api_token or (tenant_id, client_id, client_secret) are required"
                )
                return False

            if not self.default_team_id:
                logger.warning(
                    "No default_team_id provided for Graph API, some operations may not work"
                )
        ...
        logger.info(
            f"MSTeamsConnector initialized with {'Graph API' if self.use_graph_api else 'Webhook'} mode"
        )
        return True
        ...
        try:
            ...
            async with self.session.post(token_url, data=data) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get access token. Status: {response.status}, Error: {error_text}"
                    )
                    return False
    ...
    async def _test_webhook_connection(self) -> bool:
        ...
        try:
            ...
            test_message = {
                ...
                "potentialAction": [
                    {"@type": "ActionCard", "name": "Connection test successful", "inputs": []}
                ],
            }
            ...
            async with self.session.post(self.webhook_url, json=low_visibility_test) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Teams webhook connection test failed. Status: {response.status}, Error: {error_text}"
                    )
                    return False
        ...
        try:
            ...
            async with self.session.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Teams Graph API connection test failed. Status: {response.status}, Error: {error_text}"
                    )
                    return False
    ...
    async def _send_message_webhook(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        if sections:
            ...
            message_card["sections"] = [{"facts": [{"name": k, "value": v} for k, v in facts]}]
        ...
        try:
            async with self.session.post(webhook_url, json=message_card) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Failed to send message to Teams webhook. Status: {response.status}, Error: {error_text}"
                    )
        ...
        try:
            async with self.session.post(endpoint, headers=headers, json=message) as response:
                if response.status in (200, 201):
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Failed to send message via Graph API. Status: {response.status}, Error: {error_text}"
                    )
        ...
        try:
            async with self.session.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"Failed to get channels. Status: {response.status}, Error: {error_text}"
                    )
            ...
            async with self.session.post(webhook_url, json=message) as response:
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True,
                        operation="send_adaptive_card",
                        result_data={"message_type": "adaptive_card", "webhook_url": webhook_url},
                    )
                else:
                    error_text = await response.text()
                    logger.error(
                        f"Failed to send adaptive card to Teams webhook. Status: {response.status}, Error: {error_text}"
                    )
            ...
            async with self.session.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True,
                        operation="get_channels",
                        result_data={"team_id": team_id, "channels": channels},
                    )
                else:
                    error_text = await response.text()
                    logger.error(
            ...
            async with self.session.get(endpoint, headers=headers) as response:
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True,
                        operation="get_team_members",
                        result_data={"team_id": team_id, "members": members},
                    )
                else:
                    error_text = await response.text()
                    logger.error(
                        f"Failed to get team members. Status: {response.status}, Error: {error_text}"
                    )
    ...
    async def create_item(self, item_type: str, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if item_type not in ["message", "adaptive_card"]:
            raise Exception(
                f"Unsupported item type: {item_type}. Only 'message' and 'adaptive_card' are supported."
            )
