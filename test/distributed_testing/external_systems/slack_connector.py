...
class SlackConnector(ExternalSystemInterface):
        ...
        try:
            ...
            async with self.session.post(url) as response:
                ...
                if response.status == 200 and data.get("ok"):
                    ...
                    self.bot_info = data
                    logger.info(
                        f"Connected to Slack as {data.get('user')} (team: {data.get('team')})"
                    )
                    ...
                    logger.error(
                        f"Failed to connect to Slack: {data.get('error', 'Unknown error')}"
                    )
                    return False
            ...
            async with self.session.post(url, json=payload) as response:
                ...
                if data.get("ok"):
                    return ExternalSystemResult(
                        success=True,
                        operation="delete_message",
                        result_data={"channel": data.get("channel"), "ts": ts},
                    )
                else:
                    return ExternalSystemResult(
                        success=False,
    ...
    async def _add_reaction(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        try:
            payload = {"channel": channel, "timestamp": ts, "name": name}
            ...
            async with self.session.post(url, json=payload) as response:
                ...
                if data.get("ok"):
                    return ExternalSystemResult(
                        success=True,
                        operation="add_reaction",
                        result_data={"channel": channel, "ts": ts, "name": name},
                    )
                else:
                    return ExternalSystemResult(
                        success=False,
    ...
    async def _archive_channel(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                ...
                if data.get("ok"):
                    return ExternalSystemResult(
                        success=True, operation="archive_channel", result_data={"channel": channel}
                    )
                else:
                    return ExternalSystemResult(
                        success=False,
        ...
        try:
            async with self.session.get(url, params={"channel": channel}) as response:
                ...
                if data.get("ok"):
                    ...
                    return ExternalSystemResult(
                        success=True, operation="get_channel_info", result_data=channel_info
                    )
                else:
                    return ExternalSystemResult(
                        success=False,
    ...
    async def _get_message_history(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        try:
            ...
            async with self.session.get(url, params=query_params) as response:
                ...
                if data.get("ok"):
                    return ExternalSystemResult(
                        success=True, operation="get_message_history", result_data=data
                    )
                else:
                    return ExternalSystemResult(
                        success=False,
    ...
    async def query(self, query_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if item_type == "message":
            ...
            history_params = {"channel": channel}
