...
class JiraConnector(ExternalSystemInterface):
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    self.user_info = data
                    logger.info(
                        f"Connected to JIRA as {data.get('displayName')} ({data.get('emailAddress')})"
                    )
        ...
        try:
            ...
            async with self.session.put(url, json=payload) as response:
                ...
                if response.status in [200, 204]:
                    return ExternalSystemResult(
                        success=True, operation="update_issue", result_data={"issue_key": issue_key}
                    )
                else:
                    error_text = await response.text()
            ...
            async with self.session.post(url, json=payload) as response:
                ...
                if response.status in [200, 201]:
                    ...
                    return ExternalSystemResult(
                        success=True,
                        operation="add_comment",
                        result_data={"id": data.get("id"), "self": data.get("self")},
                    )
                else:
                    error_text = await response.text()
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True, operation="get_issue_transitions", result_data=data
                    )
                else:
                    error_text = await response.text()
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                ...
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True, operation="search_issues", result_data=data
                    )
                else:
                    error_text = await response.text()
        ...
        try:
            async with self.session.get(url, params=query_params) as response:
                ...
                if response.status == 200:
                    ...
                    return ExternalSystemResult(
                        success=True, operation="get_issue", result_data=data
                    )
                else:
                    error_text = await response.text()
    ...
    async def _delete_issue(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        try:
            async with self.session.delete(url) as response:
                ...
                if response.status in [200, 204]:
                    return ExternalSystemResult(
                        success=True, operation="delete_issue", result_data={"issue_key": issue_key}
                    )
                else:
                    error_text = await response.text()
    ...
    async def query(self, query_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for key, value in query_params.items():
            ...
            if isinstance(value, list):
                formatted_value = (
                    f"({','.join([f'"{v}"' if isinstance(v, str) else str(v) for v in value])})"
                )
                if operator == "=":
                    operator = "IN"
                elif operator == "!=":
