...
class PrometheusConnector(ExternalSystemInterface):
    ...
    def __init__(self):
        ...
        self.cache = {"metric_metadata": {}, "targets": [], "alerts": []}
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                        ...
                        if self.pushgateway_url:
                            try:
                                async with self.session.get(
                                    self.pushgateway_url + "/-/ready"
                                ) as pgw_response:
                                    if pgw_response.status == 200:
                                        logger.info(
                                            f"Connected to Push Gateway at {self.pushgateway_url}"
                                        )
                                    else:
                                        logger.warning(
                                            f"Push Gateway not ready: {pgw_response.status}"
                                        )
                    ...
                    logger.error(
                        f"Failed to connect to Prometheus: {response.status} - {error_text}"
                    )
                    return False
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        ...
                        logger.debug(
                            f"Cached metadata for {len(self.cache['metric_metadata'])} metrics"
                        )
        ...
        try:
            async with self.session.get(url, params=query_params) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="query", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url, params=query_params) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="query_range", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
    ...
    async def _push_metrics(self, params: Dict[str, Any]) -> ExternalSystemResult:
        ...
        try:
            async with self.session.post(
                url, data=metric_data, headers={"Content-Type": "text/plain"}
            ) as response:
                response = await self._handle_rate_limit(response)
            ...
            async with self.session.delete(url) as response:
                ...
                if response.status in [200, 202]:
                    return ExternalSystemResult(
                        success=True,
                        operation="delete_metrics",
                        result_data={"job_name": job_name, "instance": instance},
                    )
                else:
                    error_text = await response.text()
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_metadata", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_targets", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_alerts", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_rules", result_data=data.get("data", {})
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url, params=query_params) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_series", result_data=data.get("data", [])
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
        ...
        try:
            async with self.session.get(url, params=query_params) as response:
                ...
                if response.status == 200:
                    ...
                    if data.get("status") == "success":
                        return ExternalSystemResult(
                            success=True, operation="get_labels", result_data=data.get("data", [])
                        )
                    else:
                        return ExternalSystemResult(
                            success=False,
                ...
                try:
                    async with self.session.get(self.pushgateway_url + "/-/ready") as pgw_response:
                        pushgateway_status = (
                            "ready"
                            if pgw_response.status == 200
                            else f"error_{pgw_response.status}"
                        )
                except Exception as e:
