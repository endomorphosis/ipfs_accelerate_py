...
class TelegramConnector(ExternalSystemInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        if not self.default_chat_id:
            logger.warning(
                "default_chat_id is not provided, it will need to be specified with each message"
            )
        ...
        try:
            ...
            async with self.session.post(f"{self.api_base_url}/sendPhoto", json=data) as response:
                if response.status != 200:
                    ...
                    return {
                        "success": False,
                        "error": f"Failed to send photo: {response.status} - {error_text}",
                    }
    ...
    async def _send_document(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.api_base_url}/sendDocument", json=data
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
                    ...
                    return {
                        "success": False,
                        "error": f"Failed to send document: {response.status} - {error_text}",
                    }
    ...
    async def query(self, query_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if query_params.get("type") == "updates":
            try:
                ...
                async with self.session.post(
                    f"{self.api_base_url}/getUpdates", json=data
                ) as response:
                    if response.status != 200:
    ...
    async def update_item(self, item_type: str, item_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.api_base_url}/editMessageText", json=data
            ) as response:
                if response.status != 200:
    ...
    async def delete_item(self, item_type: str, item_id: str) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.api_base_url}/deleteMessage", json=data
            ) as response:
                if response.status != 200:
