...
class CoordinatorHardwareMonitoringIntegration:
    ...
    def __init__(
        ...
        if not self.hardware_detector and db_path:
            self.hardware_detector = HardwareCapabilityDetector(db_path=self.db_path)
    ...
    def _register_coordinator_callbacks(self):
        ...
        if hasattr(self.coordinator, "register_worker_callback"):
            ...
            self.coordinator.register_worker_callback(
                event="register", callback=self._on_worker_registered
            )

            # Register for worker deregistration events
            self.coordinator.register_worker_callback(
                event="deregister", callback=self._on_worker_deregistered
            )
        else:
            ...
            try:
                ...
                if hasattr(self.coordinator, "workers") and worker_id in self.coordinator.workers:
                    ...
                    self.coordinator.workers[worker_id]["hardware_metrics"] = {
                        ...
                        "memory_percent": metrics.memory_percent,
                        "gpu_percent": max(
                            [gpu.get("load", 0.0) for gpu in metrics.gpu_utilization]
                        )
    ...
    async def _hardware_aware_find_best_worker(
        self,
        task: Dict[str, Any],
        ...
        if utilization:
            ...
            if memory_percent > 70.0:
                ...
                logger.debug(
                    f"Applied memory penalty of {memory_penalty:.2f} to worker {worker_id}"
                )
            ...
            if (
                gpu_utilization
                and "hardware" in task.get("requirements", {})
                and "gpu" in task["requirements"]["hardware"]
            ):
                max_gpu_load = (
                    max([gpu.get("load", 0.0) for gpu in gpu_utilization])
                    if gpu_utilization
                    else 0.0
                )
            ...
            if self._is_worker_critically_overloaded(utilization):
                logger.warning(
                    f"Worker {worker_id} is critically overloaded, preventing assignment"
                )
                return None, 0.0
        ...
        if abs(adjusted_score - score) > 1.0:
            logger.info(
                f"Adjusted worker {worker_id} score from {score:.2f} to {adjusted_score:.2f} based on hardware utilization"
            )
        ...
        if adjusted_score < score * 0.7:  # If adjusted score is much worse
            ...
            if alternate_worker_id and alternate_score > adjusted_score:
                logger.info(
                    f"Selected alternate worker {alternate_worker_id} with better utilization (score: {alternate_score:.2f} vs {adjusted_score:.2f})"
                )
    ...
    def _calculate_hardware_match_score(
        self, task: Dict[str, Any], worker: Dict[str, Any]
    ) -> float:
        """
    ...
    def _hardware_aware_update_worker_performance(
        self, worker_id: str, task_result: Dict[str, Any]
    ):
        """
            ...
            if task_id:
                ...
                if task_usage:
                    ...
                    if len(self.task_resource_history[task_type]) >= 100:
                        self.task_resource_history[task_type] = self.task_resource_history[
                            task_type
                        ][-99:]
                    ...
                    self.task_resource_history[task_type].append(
                        {
                            ...
                            "end_time": task_usage.end_time.isoformat()
                            if task_usage.end_time
                            else None,
                    ...
                    if hasattr(self.scheduler, "task_type_stats"):
                        ...
                        stats["count"] += 1
                        stats["avg_cpu_percent"] = (1 - alpha) * stats[
                            "avg_cpu_percent"
                        ] + alpha * task_usage.avg_cpu_percent
                        stats["avg_memory_percent"] = (1 - alpha) * stats[
                            "avg_memory_percent"
                        ] + alpha * task_usage.avg_memory_percent
                        stats["avg_gpu_percent"] = (1 - alpha) * stats[
                            "avg_gpu_percent"
                        ] + alpha * task_usage.avg_gpu_percent
                        stats["peak_cpu_percent"] = max(
                            stats["peak_cpu_percent"], task_usage.peak_cpu_percent
                        )
                        stats["peak_memory_percent"] = max(
                            stats["peak_memory_percent"], task_usage.peak_memory_percent
                        )
                        stats["peak_gpu_percent"] = max(
                            stats["peak_gpu_percent"], task_usage.peak_gpu_percent
                        )
    ...
    def stop_task_monitoring(
        self, task_id: str, worker_id: str, success: bool = True, error: Optional[str] = None
    ):
        """
    ...
    def generate_resource_report(
        self, worker_id: Optional[str] = None, task_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        report = {
            ...
            "workers": {},
            "tasks": {},
            "summary": {"worker_count": len(self.worker_monitors), "active_tasks": 0},
        }
        ...
        report["summary"]["active_tasks"] = sum(
            1 for m in self.worker_monitors.values() if m.active_task_id
        )
    ...
    def generate_html_report(
        self, file_path: str, worker_id: Optional[str] = None, task_id: Optional[str] = None
    ):
        """
...
"""
        ...
        for w_id, utilization in report_data["workers"].items():
            ...
            gpu_utilization = utilization.get("gpu_utilization", [])
            max_gpu_load = (
                max([gpu.get("load", 0.0) for gpu in gpu_utilization]) if gpu_utilization else 0.0
            )
            gpu_class = ""
...
"""
                ...
                for task in tasks:
                    ...
                    status_text = (
                        "Success" if task.get("completed") and task.get("success") else status_text
                    )
                    status_text = (
                        "Failed"
                        if task.get("completed") and not task.get("success")
                        else status_text
                    )
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="./hardware_metrics.duckdb", help="Path to DuckDB database"
    )
