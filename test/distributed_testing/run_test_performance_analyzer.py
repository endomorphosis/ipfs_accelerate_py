...
def generate_sample_data(
    service, num_results=500, with_trend=True, with_regression=True, with_hardware_diversity=True
):
    ...
    for i in range(num_results):
        ...
        result = {
            ...
            "details": {
                ...
                "metadata": {"model": model, "batch_size": batch_size, "precision": precision},
            },
...
def run_performance_analysis(service, analyzer, args):
    ...
    regression_analysis = analyzer.detect_performance_regression(
        metric_name="throughput", baseline_period="30d", comparison_period="7d"
    )
    ...
    if output_dir:
        ...
        if format == "json":
            ...
            report = analyzer.generate_performance_report(report_type="regression", format=format)
            with open(file_path, "w") as f:
                f.write(report)
    ...
    hardware_comparison = analyzer.compare_hardware_performance(
        metrics=["throughput", "latency_ms", "memory_usage_mb"], time_period="60d"
    )
    ...
    if output_dir:
        ...
        if format == "json":
            ...
            report = analyzer.generate_performance_report(
                report_type="hardware_comparison", format=format
            )
            with open(file_path, "w") as f:
                f.write(report)
    ...
    efficiency_analysis = analyzer.analyze_resource_efficiency(time_period="60d")
    ...
    if output_dir:
        ...
        if format == "json":
            ...
            report = analyzer.generate_performance_report(report_type="efficiency", format=format)
            with open(file_path, "w") as f:
                f.write(report)
    ...
    time_analysis = analyzer.analyze_performance_over_time(
        metric_name="throughput", time_period="90d"
    )
    ...
    if output_dir:
        ...
        if format == "json":
            ...
            report = analyzer.generate_performance_report(
                report_type="time_analysis", format=format
            )
            with open(file_path, "w") as f:
                f.write(report)
    ...
    comprehensive_report = analyzer.generate_performance_report(
        report_type="comprehensive", format=format, time_period="90d"
    )
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="./test_performance_analyzer.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument(
        "--output-dir", default="./performance_reports", help="Directory to store generated reports"
    )
    parser.add_argument("--generate-data", action="store_true", help="Generate sample data")
    parser.add_argument(
        "--num-results", type=int, default=500, help="Number of sample results to generate"
    )
    parser.add_argument(
        "--format", choices=["json", "markdown", "html"], default="markdown", help="Report format"
    )
    parser.add_argument(
        "--mode",
        choices=["all", "regression", "hardware", "efficiency", "time"],
        default="all",
        help="Analysis mode",
    )
    ...
    try:
        ...
        service = ResultAggregatorService(
            db_path=args.db_path, enable_ml=True, enable_visualization=True
        )
