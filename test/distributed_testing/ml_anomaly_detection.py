...
class MLAnomalyDetection:
    ...
    def start(self, interval: int = 60):
        ...
        self.analysis_thread = threading.Thread(
            target=self._analysis_loop, args=(interval,), daemon=True
        )
    ...
    def add_data_point(
        self, metrics: Dict[str, float], timestamp: Optional[datetime.datetime] = None
    ):
        """
    ...
    def analyze_metrics(self, force: bool = False) -> Dict[str, Any]:
        ...
        results = {
            ...
            "results": {"anomalies": {}, "forecasts": {}, "trends": {}},
        }
        ...
        if len(self.timestamps) < min_data_points and not force:
            logger.warning(
                f"Not enough data points for analysis: {len(self.timestamps)} < {min_data_points}"
            )
            results["status"] = "insufficient_data"
            results["message"] = (
                f"Insufficient data for analysis. Need at least {min_data_points} data points."
            )
            return results
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "trends": {"metrics_with_trends": len(self.trends), "summary": {}},
            "models": {
        ...
        for metric_name, anomalies in self.anomalies.items():
            if anomalies:
                ...
                recent_anomaly = max(
                    anomalies, key=lambda x: x["timestamp"] if "timestamp" in x else ""
                )
    ...
    def _detect_anomalies(
        ...
        if "isolation_forest" in methods and SKLEARN_AVAILABLE:
            try:
                ...
                logger.debug(
                    f"Isolation Forest found {len(anomaly_indices)} anomalies in {metric_name}"
                )

            except Exception as e:
                logger.warning(
                    f"Error in Isolation Forest anomaly detection for {metric_name}: {e}"
                )
                    ...
                    if 0 <= idx < len(values) and 0 <= idx < len(ts_datetime):
                        ...
                        if np.any(core_samples_mask):
                            ...
                            distances = np.linalg.norm(
                                X_scaled[idx].reshape(1, -1) - core_samples, axis=1
                            )
        ...
        if "threshold" in methods:
            try:
                ...
                anomaly_indices = np.where((values > upper_threshold) | (values < lower_threshold))[
                    0
                ]
                ...
                for idx in anomaly_indices:
                    ...
                    if 0 <= idx < len(values) and 0 <= idx < len(ts_datetime):
                        ...
                        confidence = min(
                            1.0, max(0.0, (deviations - config["std_multiplier"]) / 2.0)
                        )
                        ...
                        anomaly = {
                            ...
                            "confidence": float(confidence),
                            "threshold": float(
                                upper_threshold if values[idx] > mean else lower_threshold
                            ),
                ...
                logger.debug(
                    f"Threshold-based detection found {len(anomaly_indices)} anomalies in {metric_name}"
                )
        ...
        if "mad" in methods:
            try:
                ...
                for idx in anomaly_indices:
                    ...
                    if 0 <= idx < len(values) and 0 <= idx < len(ts_datetime):
                        ...
                        anomaly = {
                            ...
                            "confidence": float(confidence),
                            "threshold": float(
                                upper_threshold if values[idx] > median else lower_threshold
                            ),
                ...
                logger.debug(
                    f"MAD-based detection found {len(anomaly_indices)} anomalies in {metric_name}"
                )
        ...
        for anomaly in anomalies:
            idx = anomaly["index"]
            if (
                idx not in unique_anomalies
                or anomaly["confidence"] > unique_anomalies[idx]["confidence"]
            ):
    ...
    def _analyze_time_series(
        ...
        if len(ts_datetime) >= 2:
            ...
            intervals = [
                (ts_datetime[i] - ts_datetime[i - 1]).total_seconds()
        ...
        if "arima" in methods and STATSMODELS_AVAILABLE:
            try:
                ...
                model = ARIMA(values, order=config["order"])
                model_fit = model.fit()
                ...
                forecast_ci = model_fit.get_forecast(steps=forecast_horizon).conf_int(
                    alpha=ci_alpha
                )
        ...
        if "prophet" in methods and PROPHET_AVAILABLE and results["forecast_values"] == []:
            try:
                ...
                df = pd.DataFrame({"ds": ts_datetime, "y": values})
        ...
        if results["forecast_values"] == []:
            try:
                ...
                results["lower_bound"] = lower_bound.tolist()
                results["upper_bound"] = upper_bound.tolist()
                results["model_info"] = {"name": "Exponential Smoothing", "alpha": alpha}
        ...
        if results["forecast_values"]:
            ...
            results["latest_timestamps"] = [
                ts.isoformat() for ts in ts_datetime[-min(10, len(ts_datetime)) :]
            ]
    ...
    def _analyze_trend(
        ...
        results = {"methods_used": [], "direction": "stable", "confidence": 0.0}
        ...
        if "regression" in methods:
            try:
                ...
                slope_at_end = p_derivative(
                    1.0
                )  # Derivative at latest point (normalized time = 1.0)
                ...
                confidence = min(
                    1.0, max(0.0, r2 * min(1.0, abs(slope_at_end) / (np.std(values) / 2)))
                )
                ...
                logger.debug(
                    f"Regression trend analysis completed for {metric_name}: {direction} (conf: {confidence:.2f})"
                )
        ...
        if "moving_average" in methods and results["methods_used"] == []:
            try:
                ...
                if len(ma_values) >= 2:
                    ...
                    logger.debug(
                        f"Moving average trend analysis completed for {metric_name}: {direction} (conf: {confidence:.2f})"
                    )
    ...
    def _generate_visualizations(
        self, metric_name: str, values: np.ndarray, timestamps: np.ndarray, results: Dict[str, Any]
    ):
        """
        ...
        metric_dir = os.path.join(
            self.config["visualization"]["directory"], metric_name.replace(" ", "_")
        )
        ...
        try:
            ...
            if "anomalies" in results and results["anomalies"]:
                ...
                anomaly_times = [
                    ts_datetime[i] for i in anomaly_indices if 0 <= i < len(ts_datetime)
                ]

                ax.scatter(
                    anomaly_times, anomaly_values, color="red", s=80, label="Anomalies", zorder=5
                )
        ...
        if "anomalies" in results and results["anomalies"]:
            try:
                ...
                for i, anomaly in enumerate(anomalies):
                    if 0 <= anomaly["index"] < len(values) and 0 <= anomaly["index"] < len(
                        ts_datetime
                    ):
                        idx = anomaly["index"]
    ...
    def _train_models(self):
        ...
        try:
            ...
            if len(self.timestamps) < min_data_points:
                logger.warning(
                    f"Not enough data points for training: {len(self.timestamps)} < {min_data_points}"
                )
                return
            ...
            for metric_name in self.config["metrics"]:
                ...
                if (
                    "isolation_forest" in self.config["anomaly_detection"]["methods"]
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error training Isolation Forest model for {metric_name}: {e}"
                        )

                # Train ARIMA model if enabled
                if (
                    "arima" in self.config["time_series_analysis"]["methods"]
                    and STATSMODELS_AVAILABLE
                ):
                    try:
                        # Skip if not enough data
                        min_ts_points = max(
                            self.config["time_series_analysis"]["forecast_horizon"] * 2, 8
                        )
...
def create_grafana_dashboard(
    metrics: List[str], output_file: str = "grafana_ml_dashboard.json"
) -> str:
    """
            ...
            dashboard["panels"].append(
                {
                    ...
                    "fill": 1,
                    "fillGradient": 0,
                    "gridPos": {"h": 8, "w": 12, "x": i % 2 * 12, "y": y_pos + (i // 2) * 8},
                    "hiddenSeries": False,
                    "id": panel_id,
                    "legend": {
                        "avg": False,
            ...
            dashboard["panels"].append(
                {
                    ...
                    "fill": 1,
                    "fillGradient": 0,
                    "gridPos": {"h": 8, "w": 12, "x": (i % 2) * 12, "y": y_pos + (i // 2) * 8 + 4},
                    "hiddenSeries": False,
                    "id": panel_id,
                    "legend": {
                        "avg": False,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ML Anomaly Detection for Distributed Testing Framework"
    )
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument(
        "--metrics-port", type=int, default=9100, help="Port for Prometheus metrics"
    )
    ...
    parser.add_argument(
        "--export-prometheus", action="store_true", help="Export Prometheus configuration"
    )
