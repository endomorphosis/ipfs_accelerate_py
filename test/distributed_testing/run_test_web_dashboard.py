...
def generate_sample_data(service, num_results=100):
    ...
    task_templates = [
        {"type": "benchmark", "metrics": ["throughput", "latency", "memory_usage"]},
        {"type": "unit", "metrics": ["execution_time"]},
        {"type": "integration", "metrics": ["execution_time", "memory_usage"]},
        {
            "type": "performance",
            "metrics": ["throughput", "latency", "memory_usage", "execution_time"],
        },
        {"type": "compatibility", "metrics": ["execution_time"]},
    ]
    ...
    for i in range(num_results):
        ...
        for metric_name in template["metrics"]:
            ...
            if random.random() < 0.3:  # 30% chance
                result_metrics[metric_name] = {"value": value, "unit": metric_config["unit"]}
            else:
...
async def main():
    ...
    try:
        ...
        service = ResultAggregatorService(
            db_path=db_path, enable_ml=True, enable_visualization=True
        )
        ...
        report = service.generate_analysis_report(report_type="summary", format="markdown")
