    ...
    class TaskScheduler:
        ...
        async def schedule_pending_tasks(self) -> int:
            ...
            if coordinator is None:
                return 0
            pending = (
                list(coordinator.pending_tasks)
                if isinstance(coordinator.pending_tasks, (set, list))
                else []
            )
...
async def simulate_worker(
    worker_id: str, coordinator: Coordinator, hardware_capabilities: Dict[str, Any]
):
    """
...
async def create_test_task(
    ...
    logger.info(
        f"Created task {task_id} (type: {task_type}, priority: {priority}, requires_gpu: {requires_gpu})"
    )
...
async def simulate_coordinator(args):
    ...
    # Create coordinator
    try:
        coordinator = Coordinator(worker_registry=worker_registry, task_scheduler=task_scheduler)
    except TypeError:
        coordinator = Coordinator()
    ...
    hardware_detector = HardwareCapabilityDetector(db_path=detector_db_path)
    ...
    for i in range(args.num_workers):
        ...
        if i % 3 == 0:
            ...
            capabilities = {
                ...
                "memory": {"total_gb": 16.0, "available_gb": 14.5},
            }
        elif i % 3 == 1:
            # CPU + GPU worker
            capabilities = {
                "hardware": ["cpu", "gpu"],
                "cpu": {"cores": 16, "architecture": "x86_64"},
                "gpu": {"name": "NVIDIA RTX 3080", "cuda_compute": 8.6, "memory_gb": 10.0},
                "memory": {"total_gb": 32.0, "available_gb": 28.5},
            }
        else:
            # High-end worker
            capabilities = {
                ...
                "gpu": {"name": "NVIDIA RTX 4090", "cuda_compute": 8.9, "memory_gb": 24.0},
                "memory": {"total_gb": 64.0, "available_gb": 58.5},
            }
    ...
    try:
        while time.time() - start_time < args.duration and (
            coordinator.pending_tasks or coordinator.running_tasks
        ):
            ...
            current_running = list(
                coordinator.running_tasks.items()
            ...
            if int(time.time() - start_time) % 10 == 0:
                logger.info(
                    f"Status: {len(coordinator.pending_tasks)} pending, {len(coordinator.running_tasks)} running, {len(coordinator.completed_tasks)} completed, {len(coordinator.failed_tasks)} failed"
                )
                ...
                for worker_id, utilization in integration.get_worker_utilization().items():
                    ...
                    logger.info(
                        f"Worker {worker_id}: CPU {cpu_percent:.1f}%, Memory {memory_percent:.1f}%, GPU {gpu_percent:.1f}%"
                    )
        ...
        logger.info(
            f"Final status: {len(coordinator.completed_tasks)} completed, {len(coordinator.failed_tasks)} failed"
        )
            ...
            results = {
                ...
                "task_details": {
                    task_id: {
                        ...
                        "worker_id": task.get("result", {}).get("worker_id")
                        if "result" in task
                        else None,
                        "execution_time": task.get("result", {}).get("execution_time_seconds")
                        if "result" in task
                        else None,
                    }
...
def main():
    ...
    parser.add_argument(
        "--max-tasks-per-worker", type=int, default=2, help="Maximum tasks per worker"
    )
    parser.add_argument(
        "--report", default="hardware_utilization_report.html", help="Path for HTML report"
    )
    ...
    print("=" * 80)
    print(
        f"Simulating {args.num_workers} workers and {args.num_tasks} tasks for {args.duration} seconds"
    )
