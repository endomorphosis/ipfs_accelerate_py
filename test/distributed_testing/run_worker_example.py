...
async def run_worker(args):
    ...
    if args.log_dir:
        ...
        log_file = os.path.join(
            args.log_dir, f"worker_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    if args.coordinator_registry:
        coordinator_registry = [
            url.strip() for url in args.coordinator_registry.split(",") if url.strip()
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="IPFS Accelerate Distributed Testing Framework - Worker Example"
    )
    ...
    # Capability options
    parser.add_argument(
        "--tags", help="Comma-separated list of capability tags (e.g., 'gpu,cuda,transformers')"
    )
    ...
    parser.add_argument(
        "--coordinator-registry", help="Comma-separated list of coordinator URLs for fallback"
    )
    ...
    parser.add_argument(
        "--enable-swap", action="store_true", help="Allow use of disk space for memory overflow"
    )
    parser.add_argument(
        "--cache-models", action="store_true", help="Keep frequently used models in memory"
    )
    parser.add_argument("--cuda-streams", type=int, help="Number of CUDA streams for GPU tasks")
    parser.add_argument(
        "--optimize-for",
    ...
    # Recovery options
    parser.add_argument("--recover", action="store_true", help="Trigger recovery mode on startup")
    parser.add_argument("--reset-state", action="store_true", help="Reset worker state on startup")
