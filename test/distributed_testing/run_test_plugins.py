...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("plugin_test.log")],
)
...
async def simulate_tasks(coordinator, num_tasks=5):
    ...
    for i in range(num_tasks):
        ...
        if coordinator.plugin_manager:
            await coordinator.plugin_manager.invoke_hook(HookType.TASK_CREATED, task_id, task_data)
    ...
    for i, (task_id, _) in enumerate(tasks):
        ...
        # Assign task
        if coordinator.plugin_manager:
            await coordinator.plugin_manager.invoke_hook(HookType.TASK_ASSIGNED, task_id, worker_id)
    ...
    for i, (task_id, _) in enumerate(tasks):
        ...
        # Start task
        if coordinator.plugin_manager:
            await coordinator.plugin_manager.invoke_hook(HookType.TASK_STARTED, task_id, worker_id)
...
async def main():
    ...
    parser.add_argument(
        "--plugins",
        default="sample_reporter_plugin.py,ci_integration_plugin.py",
    ...
    parser.add_argument(
        "--ci-system", default="github", help="CI system to use (github, jenkins, gitlab)"
    )
    parser.add_argument("--simulate-tasks", type=int, default=5, help="Number of tasks to simulate")
