...
try:
    ...
    logger.error(
        f"Error importing selenium_browser_bridge. Make sure it exists at the expected path."
    )
...
try:
    ...
    logger.error(
        f"Error importing browser_recovery_strategies. Make sure it exists at the expected path."
    )
...
class RecoveryTestMetrics:
    ...
    def record_test_result(
        ...
        if model not in self.model_metrics:
            self.model_metrics[model] = {"tests": 0, "successes": 0, "failures": 0, "recoveries": 0}
    ...
    def record_recovery_metrics(
        ...
        if browser not in metrics["browsers"]:
            metrics["browsers"][browser] = {"attempts": 0, "successes": 0}
        ...
        if model not in metrics["models"]:
            metrics["models"][model] = {"attempts": 0, "successes": 0}
...
class SeleniumE2ERecoveryTest:
    ...
    def __init__(
        ...
        self.browser_types = {browser: detect_browser_type(browser) for browser in self.browsers}

        # Convert model names to ModelType
        self.model_types = {model: detect_model_type(model) for model in self.models}
        ...
        self.browser_preferences = {
            ...
            ModelType.AUDIO: BrowserType.FIREFOX
        ...
        logger.info(
            f"Initialized E2E recovery test with browsers: {self.browsers}, models: {self.models}"
        )
    ...
    def create_browser_config(self, browser_name: str, model_name: str) -> BrowserConfiguration:
        ...
        config = BrowserConfiguration(
            browser_name=browser_name, platform=platform, headless=True, timeout=30
        )
    ...
    def inject_failure(
        self, test_index: int, browser_type: BrowserType, model_type: ModelType
    ...
    async def run_test_case(
        self, browser_name: str, model_name: str, test_index: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if failure_type:
                ...
                error_msg = (
                    f"Artificial {failure_type.value} failure in {browser_name} for {model_name}"
                )
            ...
            # Execute test
            result = await bridge.run_test(model_name=model_name, input_data="This is a test input")
            ...
            try:
                ...
                self.metrics.record_recovery_metrics(
                    ...
                    browser=browser_name,
                    model=model_name,
                    details={"error": str(e), "recovery_duration": recovery_duration},
                )
                ...
                self.metrics.record_test_result(
                    ...
                    recovered=recovered,
                    recovery_attempts=1,
                    details={"error": str(e), "recovery_attempted": True, "recovered": recovered},
                )
    ...
    async def run_all_tests(self):
        ...
        logger.info(
            f"Running E2E browser recovery tests for {len(self.browsers)} browsers and {len(self.models)} models"
        )
        ...
        try:
            ...
            with open(summary_path, "w") as f:
                ...
                for strategy, metrics in summary["recovery_metrics"].items():
                    ...
                    f.write(
                        f"| {strategy} | {attempts} | {success_rate:.2%} | {avg_duration:.2f} |\n"
                    )
    ...
    def print_summary(self):
        ...
        for browser, rate in summary["browser_success_rates"].items():
            tests = summary["browser_metrics"][browser]["tests"]
            print(
                f"  {browser:10}: {rate:.2%} ({summary['browser_metrics'][browser]['successes']}/{tests})"
            )
        ...
        for model, rate in summary["model_success_rates"].items():
            tests = summary["model_metrics"][model]["tests"]
            print(
                f"  {model:20}: {rate:.2%} ({summary['model_metrics'][model]['successes']}/{tests})"
            )
        ...
        for strategy, metrics in summary["recovery_metrics"].items():
            ...
            print(
                f"  {strategy:20}: {success_rate:.2%} ({successes}/{attempts}) - Avg: {avg_duration:.2f}s"
            )
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of test iterations per browser-model combination",
    )
