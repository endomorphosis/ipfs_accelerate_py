...
class TestHardwareMatcher:
    ...
    def _create_schema(self) -> None:
        ...
        try:
            ...
            if self.error_handler:
                context = {"component": "test_hardware_matcher", "operation": "create_schema"}
    ...
    def register_test_profile(self, test_profile: TestProfile) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Registered test profile for {test_profile.test_id} of type {test_profile.test_type.value}"
            )
            return True
        except Exception as e:
            if self.error_handler:
    ...
    def _persist_test_profile(self, test_profile: TestProfile) -> None:
        ...
        try:
            ...
            for req in test_profile.requirements:
                ...
                value_str = (
                    json.dumps(req.value) if isinstance(req.value, (dict, list)) else str(req.value)
                )
    ...
    def register_worker_capabilities(self, worker_capabilities: WorkerHardwareCapabilities) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Registered capabilities for worker {worker_capabilities.worker_id} "
    ...
    def register_test_performance(self, performance_record: TestPerformanceRecord) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Registered performance for test {performance_record.test_id} "
    ...
    def _adjust_weights_based_on_performance(
        self, performance_record: TestPerformanceRecord
    ) -> None:
        """
        ...
        for factor in factors_to_adjust:
            if factor in self.match_factor_weights:
                ...
                new_weight = min(
                    self.weight_max, self.match_factor_weights[factor] + self.adaptation_rate
                )

                # Log adjustment
                logger.debug(
                    f"Adjusting weight for {factor} from {self.match_factor_weights[factor]:.2f} to {new_weight:.2f}"
                )
    ...
    def _get_test_performance_metrics(self, test_id: str, hardware_id: str) -> Dict[str, Any]:
        ...
        memory_usages = [
            r.memory_usage_mb for r in records if r.success and r.memory_usage_mb is not None
        ]
    ...
    def match_test_to_hardware(
        self, test_id: str, available_workers: List[str] = None
    ) -> Optional[HardwareTestMatch]:
        """
        ...
        if matches:
            ...
            logger.info(
                f"Matched test {test_id} to worker {best_match.worker_id} "
    ...
    def _evaluate_match_score(
        ...
        match_factors["load_balancing"] = self._evaluate_load_balancing(worker_capability.worker_id)

        # Evaluate feature support
        match_factors["feature_support"] = self._evaluate_feature_support(test_profile, hardware)
            ...
            if req.requirement_type == TestRequirementType.MEMORY:
                ...
                if isinstance(feature, str):
                    ...
                    feature_supported = all(
                        hardware.capabilities.get(attr) == value for attr, value in feature.items()
                    )
    ...
    def _evaluate_hardware_type_compatibility(
        self, test_profile: TestProfile, hardware_type: HardwareType
    ) -> float:
        """
    ...
    def _evaluate_compute_capability(
        self, test_profile: TestProfile, hardware: HardwareCapability
    ) -> float:
        """
    ...
    def _evaluate_memory_compatibility(
        self, test_profile: TestProfile, hardware: HardwareCapability
    ) -> float:
        """
    ...
    def _evaluate_precision_compatibility(
        self, test_profile: TestProfile, hardware: HardwareCapability
    ) -> float:
        """
        ...
        if required_precision in precision_order and any(
            precision in hardware.supported_precisions
            for precision in precision_order[precision_order.index(required_precision) :]
        ):
    ...
    def _evaluate_hardware_preference(
        self, test_profile: TestProfile, hardware_type: HardwareType
    ) -> float:
        """
    ...
    def match_tests_to_workers(
        self, test_ids: List[str], available_workers: List[str] = None
    ) -> Dict[str, HardwareTestMatch]:
        """
    ...
    def get_test_performance_history(
        self, test_id: str, hardware_id: Optional[str] = None
...
if __name__ == "__main__":
    ...
    memory_test = TestProfile(
        ...
        requirements=[
            TestRequirement(requirement_type=TestRequirementType.MEMORY, value=3000, importance=0.9)
        ],
        tags=["memory"],
    )
    ...
    for test_id in ["compute_test", "memory_test", "precision_test"]:
        ...
        if match:
            print(
                f"{test_id}: Matched to {match.worker_id} ({match.hardware_type.value}) with score {match.match_score:.4f}"
            )
    ...
    matcher.register_test_performance(
    ...
    if match:
        print(
            f"compute_test: Matched to {match.worker_id} ({match.hardware_type.value}) with score {match.match_score:.4f}"
        )
        print(f"  Match factors: {match.match_factors}")
