        ...
        while True:
            ...
            try:
                ...
                if msg_type == "execute_task":
                    ...
                    logger.info(f"Received batch execution request: {len(tasks_data)} tasks {task_ids}")
                    ...
                    reason = data.get("reason", "user_request")
                    logger.info(f"Received batch cancellation request: {len(task_ids)} tasks (reason: {reason})")
            ...
            for task_data in tasks_data:
                ...
                for batch_size in task_batch_sizes:
                    if str(batch_size) in combined_results.get("batch_sizes", {}):
                        task_results[str(batch_size)] = combined_results["batch_sizes"][str(batch_size)]
                ...
                execution_time = (datetime.now() - datetime.fromisoformat(combined_task["started"])).total_seconds()
