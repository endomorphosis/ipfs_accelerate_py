...
try:
    ...
    print(
        "Warning: compute shader modules not available, running without compute shader optimizations"
    )
...
WEBGPU_IMPL_TYPE = (
    "REAL_WEBGPU"  # Updated from "SIMULATED_WEBGPU_TRANSFORMERS_JS" to match fixed_web_platform
)
...
class WebPlatformTestRunner:
    ...
    def __init__(
        ...
        logger.info(
            f"Available browsers: {', '.join(self.available_browsers) if self.available_browsers else 'None'}"
        )
    ...
    def _detect_browsers(self) -> List[str]:
        ...
        if webnn_simulation or webnn_available or webgpu_simulation or webgpu_available:
            ...
            if feature_str:
                logger.info(
                    f"Web platform simulation mode detected with {feature_str}, enabling all browsers"
                )
            else:
            ...
            for path in firefox_paths:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        is_audio_test = os.environ.get(
                            "WEBGPU_COMPUTE_SHADERS_ENABLED"
                        ) == "1" and any(
                            audio_model in str(path)
                            for audio_model in ["whisper", "wav2vec2", "clap", "audio"]
                        )

                        # Log WebGPU support status in Firefox with enhanced audio model performance
                        if firefox_version >= 122 and is_audio_test:
                            logger.info(
                                f"Found Firefox v{firefox_version} with exceptional WebGPU compute shader support"
                            )
                            logger.info(
                                f"Firefox provides 55% improvement over standard WebGPU for audio models"
                            )
                            logger.info(
                                f"Use --MOZ_WEBGPU_ADVANCED_COMPUTE=1 flag for optimal performance"
                            )
                        elif firefox_version >= 117:
                            logger.info(
                                f"Found Firefox v{firefox_version} with WebGPU support (51% compute shader performance improvement)"
                            )
                        elif firefox_version > 0:
                            logger.warning(
                                f"Found Firefox v{firefox_version}, but WebGPU support may be limited (v117+ recommended)"
                            )
                        else:
        ...
        if not available_browsers and (
            os.environ.get("WEBNN_ENABLED") == "1" or os.environ.get("WEBGPU_ENABLED") == "1"
        ):
        ...
        try:
            ...
            if modality == "text":
                with open(file_path, "w") as f:
                    f.write(
                        "This is a sample text file for testing natural language processing models.\n"
                    )
                    f.write(
                        "It contains multiple sentences that can be used for inference tasks.\n"
                    )
    ...
    def check_web_platform_support(self, platform: str = "webnn") -> Dict[str, bool]:
        ...
        if platform == "webnn":
            support["available"] = support["web_browser"] and (
                support["onnx_runtime"] or support["transformers_js"]
            )
        else:  # webgpu
    ...
    def open_test_in_browser(
        self, test_file: str, platform: str = "webnn", headless: bool = False
    ) -> bool:
        """
    ...
    def run_model_test(
        self, model_key: str, platform: str = "webnn", headless: bool = False
    ) -> Dict:
        """
    ...
    def run_all_models(self, platform: str = "webnn", headless: bool = False) -> Dict:
        ...
        for model_key, model_info in self.models.items():
            ...
            if model_info["modality"] == "audio":
                logger.info(
                    f"Skipping audio model {model_key}, handled by web_audio_platform_tests.py"
                )
                continue
    ...
    def generate_test_report(
        self, results_file: Optional[str] = None, platform: Optional[str] = None
    ) -> str:
        """
        ...
        with open(report_file, "w") as f:
            ...
            for result in model_results:
                ...
                if platform_support:
                    ...
                    elif not platform_support.get(
                        "transformers_js", False
                    ) and not platform_support.get("onnx_runtime", False):
            ...
            if models_with_issues:
                f.write(
                    "   - Focus on improving support for these models: "
                    + ", ".join(models_with_issues)
                    + "\n"
                )
            ...
            f.write(
                "   - Create API-compatible wrappers across WebNN and WebGPU for model inference\n"
            )
            f.write("   - Implement automatic hardware selection based on available features\n")
            f.write(
                "   - Develop model splitting techniques for larger models that exceed browser memory limits\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="./web_platform_results", help="Directory for output results"
    )
    parser.add_argument(
        "--model", required=False, help="Model to test (bert, vit, clip, etc. or 'all-key-models')"
    )
    parser.add_argument(
        "--platform", choices=["webnn", "webgpu"], default="webnn", help="Web platform to test"
    )
    parser.add_argument(
        "--browser",
    ...
    parser.add_argument(
        "--compute-shaders", action="store_true", help="Enable WebGPU compute shader optimizations"
    )
    parser.add_argument(
        "--transformer-compute",
        action="store_true",
    ...
    parser.add_argument(
        "--shader-precompile", action="store_true", help="Enable WebGPU shader precompilation"
    )
    parser.add_argument(
        "--parallel-loading",
        action="store_true",
    ...
    parser.add_argument(
        "--all-optimizations", action="store_true", help="Enable all optimization features"
    )
    parser.add_argument(
        "--generate-report", action="store_true", help="Generate a report from test results"
    )
    parser.add_argument("--results-file", help="Path to the results file for report generation")
    parser.add_argument("--db-path", help="Path to the DuckDB database to store results")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    args = parser.parse_args()

    # Create test runner
    tester = WebPlatformTestRunner(
        output_dir=args.output_dir, use_small_models=args.small_models, debug=args.debug
    )
