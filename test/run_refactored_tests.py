...
def generate_report(results: Dict[str, unittest.TestResult], output_path: str) -> None:
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"- Success rate: {((total_tests - total_failures - total_errors) / total_tests * 100) if total_tests > 0 else 0:.2f}%\n\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="refactored_test_results.md",
        help="Output file for test results (default: refactored_test_results.md)",
    )
    parser.add_argument("--subdirs", type=str, nargs="+", help="Specific subdirectories to test")
