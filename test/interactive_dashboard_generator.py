...
def main():
    ...
    dashboard = CustomizableDashboard(
        theme=args.theme, output_dir=args.output_dir, debug=args.debug
    )
    ...
    if use_template:
        ...
        while True:
            ...
            if comp_type == "3d":
                ...
                dimensions_str = input(
                    "Dimensions (comma-separated, e.g. model_family,hardware_type): "
                )
                ...
                if comp_type == "heatmap":
                    model_families = input(
                        "Model families (comma-separated, leave empty for all): "
                    )
                    if model_families:
                        config["model_families"] = [m.strip() for m in model_families.split(",")]

                    hardware_types = input(
                        "Hardware types (comma-separated, leave empty for all): "
                    )
                    if hardware_types:
