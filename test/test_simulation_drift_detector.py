...
def create_test_validation_results(drift=False, num_historical=10, num_new=10):
    ...
    for i in range(num_historical):
        ...
        sim_result = SimulationResult(
            ...
            timestamp=(
                datetime.datetime.now() - datetime.timedelta(days=30) + datetime.timedelta(days=i)
            ).isoformat(),
        )
        ...
        for metric, comparison in base_metrics_comparison.items():
            metrics_comparison[metric] = {
                k: v + np.random.normal(0, v * 0.05) for k, v in comparison.items()
            }
    ...
    for i in range(num_new):
        ...
        sim_result = SimulationResult(
            ...
            timestamp=(
                datetime.datetime.now() - datetime.timedelta(days=10) + datetime.timedelta(days=i)
            ).isoformat(),
        )
        ...
        for metric, comparison in base_metrics_comparison.items():
            ...
            drifted_mape = comparison["mape"] * drift_factor + np.random.normal(
                0, comparison["mape"] * 0.05
            )
...
def test_threshold_adjustment():
    ...
    historical_results, new_results = create_test_validation_results(
        drift=True, num_historical=20, num_new=20
    )
...
def main():
    ...
    parser.add_argument(
        "--output", default="./drift_detection_results", help="Path prefix for output files"
    )
    ...
    try:
        ...
        logger.info(
            "No drift detection result: "
            + (
                "No significant drift"
                if not no_drift_result["is_significant"]
        ...
        logger.info(
            ...
            + (
                "Significant drift detected"
                if high_threshold_result["is_significant"]
                else "No significant drift"
            )
        )
        logger.info(
            "Low threshold result: "
            + (
                "Significant drift detected"
                if low_threshold_result["is_significant"]
                else "No significant drift"
            )
        )
        ...
        logger.info(
            ...
            + ("Drifting" if drift_status["is_drifting"] else "Not drifting")
        )
        ...
        if args.verbose:
            ...
            print(
                f"  High Threshold (overall): {high_threshold_result['thresholds_used']['overall']:.1f}%"
            )
            print(f"    Significant: {high_threshold_result['is_significant']}")
            print(
                f"  Low Threshold (overall): {low_threshold_result['thresholds_used']['overall']:.1f}%"
            )
