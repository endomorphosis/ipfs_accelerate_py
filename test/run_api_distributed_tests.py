...
try:
    from api_unified_testing_interface import APIDistributedTesting, APITestType, APIProvider
except ImportError:
    from test.api_unified_testing_interface import APIDistributedTesting, APITestType, APIProvider
...
def setup_argument_parser():
    ...
    api_group.add_argument(
        "--compare", type=str, help="Compare multiple API types (comma-separated)"
    )
    ...
    # Test configuration
    parser.add_argument(
        "--test-type", type=str, choices=[t.value for t in APITestType], help="Type of test to run"
    )
    parser.add_argument(
        "--prompt", type=str, default="Hello, world!", help="Prompt to use for testing"
    )
    ...
    parser.add_argument(
        "--duration", type=int, default=10, help="Duration in seconds for throughput tests"
    )
    ...
    parser.add_argument(
        "--priority", type=int, default=10, help="Priority of the task (for distributed tests)"
    )
    parser.add_argument(
        "--workers", type=int, default=1, help="Number of workers to use (for distributed tests)"
    )
    parser.add_argument(
        "--worker-tags",
        type=str,
    ...
    # Result monitoring
    parser.add_argument("--monitor", type=str, help="Monitor distributed test by task ID")
    parser.add_argument(
        "--poll-interval",
        type=int,
        default=5,
    ...
    parser.add_argument(
        "--lookback-days", type=int, default=7, help="Days to look back for anomaly detection"
    )
    ...
    if "results_by_api" in results:
        ...
        if "summary" in results:
            if test_type == "latency" and "latency_ranking" in results["summary"]:
                ...
                output.append(
                    f"Latency Ranking: {', '.join(results['summary']['latency_ranking'])}"
                )
            elif test_type == "throughput" and "throughput_ranking" in results["summary"]:
                output.append(
                    f"Highest Throughput API: {results['summary']['highest_throughput_api']}"
                )
                output.append(
                    f"Throughput Ranking: {', '.join(results['summary']['throughput_ranking'])}"
                )
                ...
                output.append(
                    f"Reliability Ranking: {', '.join(results['summary']['reliability_ranking'])}"
                )
        ...
        for api, api_results in results["results_by_api"].items():
            ...
            if "summary" in api_results:
                ...
                if test_type == "latency":
                    ...
                    output.append(
                        f"Success Rate: {summary.get('successful_iterations', 0)}/{api_results.get('iterations', 0)}"
                    )

                elif test_type == "throughput":
                    output.append(
                        f"Requests/Second: {summary.get('requests_per_second', 'N/A'):.2f}"
                    )
                    ...
                    output.append(f"Avg Latency: {summary.get('avg_latency', 'N/A'):.3f}s")
                    output.append(
                        f"Successful: {summary.get('successful_iterations', 0)}/{api_results.get('iterations', 0)}"
                    )
    ...
    # Handle single API test results
        ...
        if "summary" in results:
            ...
            if test_type == "latency":
                ...
                output.append(f"Max Latency: {summary.get('max_latency', 'N/A'):.3f}s")
                output.append(
                    f"Success Rate: {summary.get('successful_iterations', 0)}/{results.get('iterations', 0)}"
                )
                ...
                output.append(f"Avg Latency: {summary.get('avg_latency', 'N/A'):.3f}s")
                output.append(
                    f"Successful: {summary.get('successful_iterations', 0)}/{results.get('iterations', 0)}"
                )

            elif test_type == "cost_efficiency":
                cost_metrics = summary.get("cost_efficiency_metrics", {})
                output.append(
                    f"Tokens per Dollar: {cost_metrics.get('tokens_per_dollar', 'N/A'):.2f}"
                )
                output.append(
                    f"Output Tokens per Dollar: {cost_metrics.get('output_tokens_per_dollar', 'N/A'):.2f}"
                )
                output.append(
                    f"Tokens per Second: {cost_metrics.get('tokens_per_second', 'N/A'):.2f}"
                )
                output.append(
                    f"Output Tokens per Second: {cost_metrics.get('output_tokens_per_second', 'N/A'):.2f}"
                )
                ...
                if "concurrency_effects" in summary:
                    ...
                    for level, data in summary["concurrency_effects"].items():
                        output.append(
                            f"  Level {level}: {data.get('throughput', 'N/A'):.2f} req/s, "
...
def format_api_status(status: Dict[str, Any]) -> str:
    ...
    for api_type, api_status in status["api_status"].items():
        output.append(
            f"  {api_type}: {'Registered' if api_status['registered'] else 'Not Registered'}"
        )
    ...
    output.append(
        f"  APIs with Latency Data: {', '.join(metrics['apis_with_latency_data']) if metrics['apis_with_latency_data'] else 'None'}"
    )
    output.append(
        f"  APIs with Throughput Data: {', '.join(metrics['apis_with_throughput_data']) if metrics['apis_with_throughput_data'] else 'None'}"
    )
    output.append(
        f"  APIs with Reliability Data: {', '.join(metrics['apis_with_reliability_data']) if metrics['apis_with_reliability_data'] else 'None'}"
    )
...
def format_anomaly_detection(results: Dict[str, Any]) -> str:
    ...
    output.append(
        f"Anomaly Detection Results - API: {results['api_type']}, Test: {results['test_type']}"
    )
    output.append("=" * 50)
    ...
    if "anomalies" in results:
        ...
        for (metric, algorithm), anomaly_data in results["anomalies"].items():
            output.append(
                f"  {algorithm} detected {len(anomaly_data.get('indices', []))} anomalies"
            )
            ...
            if "anomalies" in anomaly_data and len(anomaly_data["anomalies"]) > 0:
                ...
                for i, anomaly in enumerate(anomaly_data["anomalies"][:5]):  # Show top 5
                    ...
                    output.append(
                        f"    {i + 1}. {timestamp}: {anomaly['value']:.3f} (score: {anomaly['score']:.2f})"
                    )
    ...
    if "forecast" in results:
        ...
        for method, forecast_data in results["forecast"].items():
            if "prediction" in forecast_data:
                output.append(
                    f"  {method} predicts values ranging from "
        ...
        if "result" in status:
            ...
            if "summary" in result:
                ...
                if test_type == "latency":
                    ...
                    output.append(f"Success Rate: {summary.get('success_rate', 'N/A'):.2%}")
                    output.append(
                        f"Successful: {summary.get('successful_iterations', 0)}/{result.get('iterations', 0)}"
                    )
...
def main():
    ...
    try:
        ...
        if args.monitor:
            ...
            status = monitor_task(
                api_testing, task_id, args.poll_interval, args.timeout, args.quiet
            )
        ...
        if args.api_type or args.compare:
            ...
            test_parameters = {"messages": messages, "model": args.model}
            ...
            if args.compare:
                ...
                if args.distributed:
                    ...
                    results = api_testing.compare_apis(
                        api_types=api_types, test_type=args.test_type, parameters=test_parameters
                    )
                ...
                if args.distributed:
                    ...
                    results = api_testing.run_test(
                        api_type=api_type, test_type=args.test_type, parameters=test_parameters
                    )
