...
def test_browser_args_webgpu():
    ...
    chrome_args = get_browser_args(
        platform="webgpu", browser="chrome", compute_shaders=True, precompile_shaders=True
    )
    ...
    # Firefox WebGPU args
    firefox_args = get_browser_args(platform="webgpu", browser="firefox", compute_shaders=True)
...
def test_browser_args_webnn():
    ...
    # Edge WebNN args
    edge_args = get_browser_args(platform="webnn", browser="edge")
    ...
    # Chrome WebNN args
    chrome_args = get_browser_args(platform="webnn", browser="chrome")
...
async def test_launch_with_subprocess(mock_popen, mock_find_browser):
    ...
    automation = BrowserAutomation(platform="webgpu", browser_name="chrome", headless=True)
...
def test_find_available_browsers():
    ...
    with patch(
        "check_browser_webnn_webgpu.find_browser_executable", return_value="/path/to/browser"
    ):
...
async def test_check_browser_capabilities(mock_browser_automation, mock_create_html):
    ...
    mock_automation.run_test = AsyncMock(
        return_value={
            "success": True,
    ...
    with patch(
        "check_browser_webnn_webgpu.check_browser_capabilities", new=AsyncMock()
    ) as mock_check:
        mock_check.return_value = {
            ...
            "webgpu": {
                "supported": True,
                "real": True,
                "details": {"vendor": "NVIDIA", "device": "GeForce RTX 3080"},
            },
        }
...
def test_format_capability_report():
    ...
    capabilities = {
        ...
        "webnn": {"supported": False, "error": "WebNN API not available"},
    }
