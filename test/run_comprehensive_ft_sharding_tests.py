...
def main():
    ...
    parser.add_argument("--all-models", action="store_true", help="Test all representative models")
    ...
    parser.add_argument(
        "--failure-mode", type=str, choices=FAILURE_MODES, help="Failure mode to test"
    )
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run a quick test with minimal combinations"
    )

    # Output options
    parser.add_argument(
        "--output-dir", type=str, default=DEFAULT_OUTPUT_DIR, help="Directory for output files"
    )
    ...
    for test_config in test_matrix:
        ...
        cmd = [
            "python",
            "run_web_resource_pool_fault_tolerance_test.py",
        ...
        try:
            ...
            # Log completion
            logger.info(
                f"Test {completed + 1}/{total_tests} completed with status: {result['status']}"
            )
        ...
        if completed > 0 and tests_remaining > 0:
            ...
            logger.info(
                f"Elapsed time: {elapsed_time / 60:.1f} minutes, Estimated remaining: {estimated_remaining / 60:.1f} minutes"
            )
    ...
    summary["success_rate"] = (
        summary["successful_tests"] / summary["total_tests"] if summary["total_tests"] > 0 else 0
    )
    ...
    for recovery, data in by_recovery.items():
        print(
            f"  {recovery.ljust(15)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
        )

    print("\nRESULTS BY FAILURE MODE:")
    for failure, data in by_failure.items():
        print(
            f"  {failure.ljust(25)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
        )

    print("\nRESULTS BY MODEL:")
    for model, data in by_model.items():
        print(
            f"  {model.ljust(20)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
        )
