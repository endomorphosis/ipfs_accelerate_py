...
class PredictiveAnalyticsUI:
        ...
        try:
            ...
            if not self.db_repository:
                ...
                self.db_repository = DuckDBAPIMetricsRepository(
                    db_path=self.db_path, create_if_missing=True
                )
            ...
            logger.info(
                f"Loaded data from database: {metrics_count} metrics, {apis_count} API providers"
            )
        ...
        try:
            for metric in self.historical_data:
                ...
                for idx in daily_indices:
                    ...
                    comparative_data[metric].append({"timestamp": timestamp, "values": values})
                ...
                dbc.Container(
                    [
                            ...
                            [
                                dbc.Col(
                                    [
                                        ...
                                        html.P(
                                            "Visualize performance forecasts, anomaly predictions, and optimization recommendations"
                                        ),
                                    ]
                                ),
                            ]
                        ...
                        html.Footer(
                            [
                                html.P(
                                    "API Predictive Analytics UI • IPFS Accelerate • "
                            ...
                            dbc.Col(
                                [
                                    ...
                                    dcc.Dropdown(
                                        id="api-dropdown",
                                        options=[
                                            {"label": api, "value": api} for api in api_options
                                        ],
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Trend and seasonality detection with pattern classification"
                                ),
                            ]
                        ),
                    ]
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Cost and performance optimization suggestions based on predictive analytics"
                                ),
                            ]
                        ),
                    ]
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Compare performance metrics across different API providers"
                                ),
                            ]
                        ),
                    ]
                        ...
                        dbc.Col(
                            [
                                ...
                                dcc.Dropdown(
                                    ...
                                    if hasattr(self, "comparative_data") and self.comparative_data
                                    else [],
                                    value=list(self.comparative_data.keys())[0]
                                    if hasattr(self, "comparative_data") and self.comparative_data
                                    else None,
                                    clearable=False,
                                ),
    ...
    def _setup_callbacks(self) -> None:
        ...
        def update_forecast_graph(api, metric, start_date, end_date, horizon, confidence):
            ...
            has_predictions = metric in self.predictions and api in self.predictions[metric]
        ...
        def update_anomaly_graph(api, metric, start_date, end_date, sensitivity, anomaly_types):
            ...
            if not api or not metric:
                return go.Figure().update_layout(title="No data selected"), html.P(
                    "No data selected"
                )
            ...
            if metric not in self.historical_data or api not in self.historical_data[metric]:
                return (
                    go.Figure().update_layout(title=f"No data available for {api} - {metric}"),
                    html.P(f"No data available for {api} - {metric}"),
                )

            # Check if we have anomalies
            has_anomalies = metric in self.anomalies and api in self.anomalies[metric]
            ...
            if has_anomalies:
                ...
                sorted_anomalies = sorted(
                    filtered_anomalies, key=lambda x: x.get("confidence", 0), reverse=True
                )
                ...
                visible_anomalies = [
                    a for a in sorted_anomalies if a.get("confidence", 0) >= threshold
                ]
        ...
        def update_pattern_graphs(api, metric, components):
            ...
            if metric not in self.historical_data or api not in self.historical_data[metric]:
                empty_fig = go.Figure().update_layout(
                    title=f"No data available for {api} - {metric}"
                )
                return (empty_fig, html.P(f"No data available for {api} - {metric}"), empty_fig)
                    ...
                    dbc.Card(
                        dbc.CardBody(
                            [
                                ...
                                dbc.Progress(
                                    value=confidence * 100, color=color, style={"height": "10px"}
                                ),
                            ]
                        )
                    )
                ...
                for component in components:
                    if component == "trend":
                        decomp_fig.add_trace(
                            go.Scatter(
                                x=times, y=trend, mode="lines", name="Trend", line=dict(color="red")
                            ),
                            row=row,
                            col=1,
                        )
            ...
            decomp_fig.update_layout(
                title=f"{api} - {metric_name} Decomposition", height=400, showlegend=True
            )
            ...
            if "timestamp" in df_hist.columns and "value" in df_hist.columns:
                ...
                try:
                    ...
                    seasonal_fig.update_layout(
                        title=f"{api} - {metric_name} Weekly Patterns", height=400
                    )
                except Exception as e:
                    if self.debug:
                    ...
                    seasonal_fig = go.Figure()
                    seasonal_fig.update_layout(
                        title=f"Insufficient data for seasonal pattern", height=400
                    )
        ...
        def update_recommendations(api, metric):
            ...
            if (
                metric == "cost"
                and metric in self.historical_data
                and api in self.historical_data[metric]
            ):
                hist_data = self.historical_data[metric][api]
                df_hist = pd.DataFrame(hist_data)
                ...
                if "value" in df_hist.columns:
                    ...
                    dates_future = pd.date_range(start=last_date, periods=30, freq="D")
                ...
                for i, rec in enumerate(self.recommendations[api][:5]):  # Show top 5
                    ...
                    rec_card = dbc.Card(
                        dbc.CardBody(
                            [
                                html.H5(
                                    rec.get("title", f"Recommendation {i + 1}"),
                                ...
                                html.P(
                                    [
                                        html.Strong("Impact: "),
                                        html.Span(
                                            f"{rec.get('impact', 0) * 100:.1f}% potential savings"
                                        ),
                                    ]
                                ),
                                html.P(
                    ...
                    rec_card = dbc.Card(
                        dbc.CardBody(
                            [
                                ...
                                html.P([html.Strong("Effort: "), html.Span(rec["effort"])]),
                            ]
                        ),
                        className="mb-3",
                    )
            ...
            if (
                "throughput" in self.historical_data
                and api in self.historical_data["throughput"]
                and "cost" in self.historical_data
                and api in self.historical_data["cost"]
            ):
                ...
                if "timestamp" in df_throughput.columns and "timestamp" in df_cost.columns:
                    ...
                    if (
                        "value_throughput" in df_merged.columns
                        and "value_cost" in df_merged.columns
                    ):
                        df_merged["efficiency"] = df_merged["value_throughput"] / df_merged[
                            "value_cost"
                        ].clip(lower=0.001)
        ...
        def update_comparative_analysis(metric, chart_type):
            ...
            if (
                not metric
                or not hasattr(self, "comparative_data")
                or not self.comparative_data
                or metric not in self.comparative_data
            ):
            ...
            if not data:
                empty_fig = go.Figure().update_layout(
                    title=f"No comparative data available for {metric}"
                )
                return empty_fig, html.P("No data available"), html.P("No data available")
            ...
            if chart_type == "line":
                ...
                for api in apis:
                    ...
                    valid_data = [
                        (ts, val) for ts, val in zip(timestamps, values) if val is not None
                    ]
                    if not valid_data:
                        continue
                ...
                normalized_data = {
                    api: (recent_data.get(api, 0) - min_val) / range_val for api in apis
                }
            ...
            for i, api in enumerate(sorted_apis):
                ...
                if api in trend_data and trend_data[api] is not None:
                    if trend_data[api] > 0:
                        ...
                        trend_display = html.Span(
                            [f"+{trend_data[api]:.1f}% ", html.I(className="fas fa-arrow-up")],
                            className=f"text-{trend_color}",
                        )
                    elif trend_data[api] < 0:
                        trend_color = "danger" if metric not in ["latency", "cost"] else "success"
                        trend_display = html.Span(
                            [f"{trend_data[api]:.1f}% ", html.I(className="fas fa-arrow-down")],
                            className=f"text-{trend_color}",
                        )
                    else:
                        trend_display = html.Span(
                            ["0% ", html.I(className="fas fa-equals")], className="text-secondary"
                        )
                ...
                row = html.Tr(
                    [
                        html.Td(api),
                        html.Td(
                            f"{recent_data.get(api, 'N/A'):.2f}" if api in recent_data else "N/A"
                        ),
                        html.Td(
                            f"{avg_data.get(api, 'N/A'):.2f}"
                            if api in avg_data and avg_data[api] is not None
                            else "N/A"
                        ),
            ...
            filtered_matrix = {
                api: feature_matrix.get(api, {}) for api in apis if api in feature_matrix
            }

            # Create the matrix table
            matrix_header = [
                html.Thead(
                    html.Tr([html.Th("Feature"), *[html.Th(api) for api in filtered_matrix.keys()]])
                )
            ]
            ...
            matrix_table = dbc.Table(
                matrix_header + matrix_body, bordered=True, striped=True, hover=True, size="sm"
            )
