...
class TestBackendManager:
    ...
    def test_backend_registry_state_persists_and_reloads(self):
        ...
        backend_entry = next(
            item for item in report["backends"] if item["id"] == "persistent_backend"
        )
    ...
    def test_backend_manager_finalize_inference_result_uses_result_recorder(self):
        ...
        manager = InferenceBackendManager(
            {"result_recorder": fake_result_recorder, "persist_registry": False}
        )
        manager.register_backend(
            ...
            instance=Mock(),
            capabilities=BackendCapabilities(
                supported_tasks={"text-generation"}, protocols={"http"}, hardware_types={"cpu"}
            ),
