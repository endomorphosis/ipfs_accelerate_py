...
def verify_directory(directory: str, output_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    report_file = (
        ...
        if output_file
        else os.path.join(directory, "verification_report.md")
    )
...
def generate_verification_report(summary: Dict[str, Any], output_file: str) -> None:
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"- **Valid files:** {summary['valid_files']} ({round(summary['valid_files'] / summary['total_files'] * 100, 1)}%)\n"
        )
        f.write(
            f"- **Invalid files:** {summary['invalid_files']} ({round(summary['invalid_files'] / summary['total_files'] * 100, 1)}%)\n\n"
        )
