...
def classify_model_type(file_path):
    ...
    if any(name in file_name for name in ["bert", "gpt", "t5", "gemma", "llama", "llm", "text"]):
        ...
        name in file_name for name in ["vit", "vision", "image", "clip", "resnet", "convnext"]
    ):
        return "vision"
    ...
    for file_path in test_files:
        ...
        if valid:
            results["valid"].append(
                {"path": relative_path, "class": class_name, "message": message, "type": model_type}
            )
            results["by_type"][model_type]["valid"] += 1
        else:
            results["invalid"].append(
                {"path": relative_path, "message": message, "type": model_type, "missing": missing}
            )
...
def print_results(results, verbose=False):
    ...
    print(
        f"Overall Compliance: {compliance:.1f}% ({len(results['valid'])}/{results['total']} files)"
    )
    ...
    if verbose:
        ...
        for file_info in results["invalid"]:
            missing_str = (
                f" Missing: {', '.join(file_info['missing'])}" if file_info["missing"] else ""
            )
            print(
                f"❌ {file_info['path']} ({file_info['type']}) - {file_info['message']}{missing_str}"
            )
...
def generate_report(results, output_file=None):
    ...
    for model_type, counts in results["by_type"].items():
        ...
        if total > 0:
            type_compliance = (counts["valid"] / total) * 100
            report += (
                f"- {model_type.title()}: {type_compliance:.1f}% ({counts['valid']}/{total})\n"
            )
