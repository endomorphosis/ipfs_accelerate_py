...
async def run_validation():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./fault_tolerance_reports",
    ...
    try:
        ...
        try:
            ...
            if args.mock:
                from mock_cross_browser_sharding import (
                    MockCrossBrowserModelShardingManager as ModelManager,
                )
            else:
                from cross_browser_model_sharding import (
                    CrossBrowserModelShardingManager as ModelManager,
                )
        ...
        if args.comparative:
            ...
            summary_path = os.path.join(
                args.output_dir, f"{args.model.replace('-', '_')}_comparative_summary.json"
            )
            ...
            summary_path = os.path.join(
                args.output_dir, f"{args.model.replace('-', '_')}_stress_test_summary.json"
            )
            with open(summary_path, "w") as f:
                json.dump(results, f, indent=2)
