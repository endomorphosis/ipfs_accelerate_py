...
class TestFlavaModels:
    """Test class for flava models."""

    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "flava-base"

        # Use registry information
        if self.model_id not in FLAVA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = FLAVA_MODELS_REGISTRY["flava-base"]
        else:
            self.model_info = FLAVA_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test flava models")
    ...
    # Run test
    model_id = args.model or "flava-base"
