...
def run_simple_tests())):
    ...
    if detector.is_available())):
        ...
        print())f"\1{summary['device_name']}\3"),
        print())f"\1{summary['compute_units']}\3"),
        print())f"  Memory: {summary['memory_mb']} MB"),
        print())f"\1{', '.join())summary['precision_support'])}\3"),
    ...
    for model in models:
        ...
        print())f"\1{', '.join())recommendations['recommended_optimizations'])}\3"),
        print())f"\1{recommendations['estimated_memory_reduction']}\3"),
