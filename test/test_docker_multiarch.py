...
def run_command(cmd, description="", return_output=True, timeout=120):
    ...
    try:
        result = subprocess.run(
            cmd, shell=True, capture_output=return_output, text=True, timeout=timeout
        )
        if return_output:
...
def test_project_structure():
    ...
    required_files = ["pyproject.toml", "requirements.txt", "Dockerfile", "docker-compose.yml"]
...
def test_docker_build():
    ...
    for target in targets_to_test:
        build_cmd = (
            f"docker build --platform {platform} --target {target} "
            f"-t ipfs-accelerate-py:test-{target} ."
        )

        success = run_command(
            build_cmd, f"Building {target} image", return_output=False, timeout=600
        )

        if success:
            # Verify image was created
            images = run_command(
                f"docker images ipfs-accelerate-py:test-{target} "
...
def test_container_functionality():
    ...
    run_command(
        f"docker stop {container_name} 2>/dev/null || true", "Cleaning up existing container"
    )
    ...
    start_cmd = (
        f"docker run --platform {platform} -d --name {container_name} "
    ...
    status = run_command(
        f"docker ps -f name={container_name} --format '{{.Status}}'", "Checking container status"
    )
    ...
    if status and "Up" in status:
        ...
        import_test = run_command(
            f"docker exec {container_name} python -c 'import ipfs_accelerate_py; print(\"Import successful\")'",
            "Testing package import inside container",
        )

        cli_test = run_command(
            f"docker exec {container_name} python -m ipfs_accelerate_py.cli_entry --help",
...
def test_docker_compose():
    ...
    if build_result:
        ...
        start_result = run_command(
            "docker compose up -d ipfs-accelerate-py", "Starting main service"
        )
        if start_result:
            time.sleep(5)
            ...
            ps_result = run_command(
                "docker compose ps ipfs-accelerate-py", "Checking service status"
            )
...
def test_hardware_detection():
    ...
    if run_command(start_cmd, "Starting hardware test container"):
        ...
        hw_test = run_command(
            f'docker exec {container_name} python -c "import platform; print("Platform:", platform.platform()); print("Machine:", platform.machine()); print("Processor:", platform.processor()); import sys; print("Python:", sys.version)"',
...
def cleanup_test_resources():
    ...
    test_containers = ["ipfs-accelerate-test-minimal", "ipfs-accelerate-hw-test"]
    ...
    run_command(
        "docker compose down --volumes 2>/dev/null || true", "Stopping Docker Compose services"
    )
