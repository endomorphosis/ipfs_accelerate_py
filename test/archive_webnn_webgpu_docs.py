...
"""
        try:
            ...
            logger.info(f"Created archive notice for {file_path}")
        except Exception as e:
            logger.error(f"Error creating archive notice for {file_path}: {str(e)}")
...
def create_documentation_index(archived_files, new_docs, archive_dir):
    ...
    for file_path in sorted(archived_files):
        archive_name = f"{file_path.stem}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}{file_path.suffix}"
...
def main():
    ...
    print(f"Archived files to: {args.archive_dir}")
