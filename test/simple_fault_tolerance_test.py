...
async def run_fault_tolerance_test():
    ...
    try:
        ...
        logger.info(
            f"Initial inference successful in {inference_result['metrics']['inference_time']:.2f}s"
        )
        ...
        for scenario in test_scenarios:
            ...
            if "error" in fault_inference_result:
                logger.info(
                    f"Inference with fault present failed as expected: {fault_inference_result['error']}"
                )
            else:
            ...
            logger.info(
                f"Recovery result: {recovery_result['status']}, recovered: {recovery_result.get('recovered', False)}"
            )
            ...
            if "error" in recovery_inference_result:
                logger.error(
                    f"Inference after recovery failed: {recovery_inference_result['error']}"
                )
                return 1

            logger.info(
                f"Inference after recovery successful in {recovery_inference_result['metrics']['inference_time']:.2f}s"
            )
