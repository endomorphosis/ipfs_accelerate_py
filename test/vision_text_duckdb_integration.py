...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"vision_text_db_integration_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def import_result_file(conn, file_path: str) -> bool:
    ...
    try:
        ...
        if not success and results:
            ...
            error_type = (
                first_test.get("pipeline_error_type")
                or first_test.get("from_pretrained_error_type")
                or first_test.get("openvino_error_type")
            )
        ...
        if existing:
            ...
            if hardware_platform in ["cpu", "cuda", "openvino", "rocm", "mps", "webnn", "webgpu"]:
                ...
                logger.warning(
                    f"Unknown hardware platform: {hardware_platform}, not updating database"
                )
        else:
            # Insert new record
            ...
            """,
                [
                    ...
                    hardware_flags["cpu"],
                    hardware_flags["cuda"],
                    hardware_flags["openvino"],
                    hardware_flags["rocm"],
                    hardware_flags["mps"],
                    hardware_flags["webnn"],
                    hardware_flags["webgpu"],
                    timestamp,
                ],
            )
...
def generate_compatibility_matrix(conn) -> Dict:
    ...
    try:
        ...
        matrix = {"timestamp": datetime.datetime.now().isoformat(), "models": {}}
        ...
        json_file = (
            REPORTS_DIR
            / f"vision_text_compatibility_matrix_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        md_file = (
            REPORTS_DIR
            / f"vision_text_compatibility_matrix_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(md_file, "w") as f:
            ...
            for model_type in ["clip", "blip"]:
                ...
                for model_id, info in sorted(type_models.items()):
                    ...
                    if info["last_tested"]:
                        try:
                            last_tested = datetime.datetime.fromisoformat(
                                info["last_tested"]
                            ).strftime("%Y-%m-%d")
                        except:
                            pass
                    ...
                    f.write(
                        f"| {model_id} | {task} | {cpu} | {cuda} | {openvino} | {rocm} | {mps} | {webnn} | {webgpu} | {last_tested} |\n"
                    )
            ...
            openvino_compat = len(
                [v for v in matrix["models"].values() if v["compatibility"]["openvino"]]
            )
            ...
            webnn_compat = len(
                [v for v in matrix["models"].values() if v["compatibility"]["webnn"]]
            )
            webgpu_compat = len(
                [v for v in matrix["models"].values() if v["compatibility"]["webgpu"]]
            )
            ...
            f.write(
                f"- CPU: {cpu_compat}/{total_models} models ({cpu_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- CUDA: {cuda_compat}/{total_models} models ({cuda_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- OpenVINO: {openvino_compat}/{total_models} models ({openvino_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- ROCm: {rocm_compat}/{total_models} models ({rocm_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- MPS: {mps_compat}/{total_models} models ({mps_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- WebNN: {webnn_compat}/{total_models} models ({webnn_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
            f.write(
                f"- WebGPU: {webgpu_compat}/{total_models} models ({webgpu_compat / total_models * 100 if total_models > 0 else 0:.1f}%)\n"
            )
...
def generate_performance_report(conn) -> Dict:
    ...
    try:
        ...
        try:
            ...
            count = conn.execute(
                "SELECT COUNT(*) FROM vision_text_results WHERE success = TRUE"
            ).fetchone()[0]
        ...
        # Prepare report data
        report_data = {"timestamp": datetime.datetime.now().isoformat(), "models": {}}
        ...
        csv_file = (
            REPORTS_DIR
            / f"vision_text_performance_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        ...
        md_file = (
            REPORTS_DIR
            / f"vision_text_performance_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(md_file, "w") as f:
            ...
            for model_type in ["clip", "blip"]:
                models_of_type = {
                    k: v for k, v in report_data["models"].items() if v["type"] == model_type
                }
                ...
                for model_id, info in sorted(models_of_type.items()):
                    ...
                    for hardware, perf in sorted(info["performance"].items()):
                        ...
                        if first_row:
                            f.write(
                                f"| {model_id} | {task} | {hardware} | {avg_time:.4f} | {test_count} |\n"
                            )
                            first_row = False
                        else:
        ...
        return {"success": True, "csv_file": str(csv_file), "md_file": str(md_file)}
    except Exception as e:
...
def main():
    ...
    action_group.add_argument(
        "--list-models", action="store_true", help="List all vision-text models in the database"
    )
    action_group.add_argument(
        "--performance-report",
        action="store_true",
