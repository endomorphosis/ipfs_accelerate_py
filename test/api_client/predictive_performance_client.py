...
class PredictivePerformanceClient:
    ...
    def predict_hardware(
        ...
        if available_hardware is not None:
            available_hardware = [
                hw.value if isinstance(hw, HardwarePlatform) else hw for hw in available_hardware
            ]
        ...
        response = self.session.post(
            self._get_url("predict-hardware"), headers=self.headers, json=payload
        )
    ...
    def predict_performance(
        ...
        # Send request
        response = self.session.post(
            self._get_url("predict-performance"), headers=self.headers, json=payload
        )
    ...
    def record_measurement(
        ...
        # Send request
        response = self.session.post(
            self._get_url("record-measurement"), headers=self.headers, json=payload
        )
    ...
    def analyze_predictions(
        ...
        # Send request
        response = self.session.post(
            self._get_url("analyze-predictions"), headers=self.headers, json=payload
        )
    ...
    def record_feedback(self, recommendation_id, accepted, feedback=None, wait=False, timeout=60):
        ...
        """
        # Prepare request
        payload = {"recommendation_id": recommendation_id, "accepted": accepted}
        ...
        # Send request
        response = self.session.post(
            self._get_url("record-feedback"), headers=self.headers, json=payload
        )
    ...
    def generate_sample_data(self, num_models=5, wait=False, timeout=60):
        ...
        # Send request
        response = self.session.post(
            self._get_url("generate-sample-data"), headers=self.headers, json=payload
        )
    ...
    def get_task_status(self, task_id):
        ...
        """
        response = self.session.get(self._get_url(f"task/{task_id}"), headers=self.headers)
    ...
    def list_recommendations(
        self, model_name=None, model_family=None, hardware=None, accepted=None, days=None, limit=10
    ):
        ...
        if hardware:
            params["hardware"] = (
                hardware.value if isinstance(hardware, HardwarePlatform) else hardware
            )
        ...
        response = self.session.get(
            self._get_url("recommendations"), headers=self.headers, params=params
        )
    ...
    def list_measurements(
        ...
        # Send request
        response = self.session.get(
            self._get_url("measurements"), headers=self.headers, params=params
        )
    ...
    def list_predictions(
        self, model_name=None, model_family=None, hardware=None, batch_size=None, limit=10
    ):
        ...
        # Send request
        response = self.session.get(
            self._get_url("predictions"), headers=self.headers, params=params
        )
