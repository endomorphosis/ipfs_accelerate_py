...
def run_benchmark(
    model_name="bert-base-uncased", hardware="cpu", batch_sizes=None, warmup=2, runs=5
):
...
def main():
    ...
    parser.add_argument("--model", type=str, default="bert-base-uncased", help="Model to benchmark")
    parser.add_argument(
        "--hardware",
        type=str,
        nargs="+",
    ...
    for hardware, result in benchmark_results.items():
        ...
        if model_type == "bert":
            logger.info(
                f"{'Batch Size':<10} {'Latency (ms)':<15} {'Throughput (items/s)':<25} {'Memory (MB)':<15}"
            )
            logger.info("-" * 80)
            ...
            for batch_result in result.get("results", []):
                ...
                logger.info(
                    f"{batch_size:<10} {latency:<15.2f} {throughput:<25.2f} {memory:<15.2f}"
                )

        elif model_type == "t5":
            logger.info(
                f"{'Batch Size':<10} {'Forward Latency':<15} {'Forward Throughput':<25} {'Generation Latency':<20} {'Generation Throughput':<25}"
            )
            logger.info("-" * 100)
            ...
            for batch_result in result.get("results", []):
                ...
                logger.info(
                    f"{batch_size:<10} {forward_latency:<15.2f} {forward_throughput:<25.2f} {generation_latency:<20.2f} {generation_throughput:<25.2f}"
                )
