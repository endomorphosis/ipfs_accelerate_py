...
def check_package_structure():
    ...
    try:
        ...
        results["installed_package"]["main_package"] = {
            ...
            "location": ipfs_accelerate_py.__file__
            if hasattr(ipfs_accelerate_py, "__file__")
            else "Unknown",
        }
        ...
        results["installed_package"]["main_package"] = {"status": "Missing", "error": str(e)}
...
def check_repo_structure():
    ...
    for parent in [current_dir] + list(current_dir.parents):
        if any(
            (parent / indicator).exists() for indicator in [".git", "setup.py", "pyproject.toml"]
        ):
            repo_root = parent
            break
...
def analyze_compatibility(package_results, framework_results, repo_results):
    ...
    if (
        package_results.get("installed_package", {}).get("main_package", {}).get("status")
        == "Present"
    ):
    ...
    expected_attrs = (
        framework_results.get("test_framework", {})
        .get("expected_structure", {})
        .get("core_modules", [])
    )
    present_attrs = []
    missing_attrs = []
    ...
    passed_checks = len(present_attrs) + (
        1 if results["compatibility_analysis"]["main_package"] == "Compatible" else 0
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check compatibility between IPFS Accelerate Python package and test framework"
    )
    ...
    # Combine results
    results = {**package_results, **framework_results, **repo_results, **compatibility_results}
