...
def _reset_control_configuration(
    ...
    configure_agent_supervisor_control()
    monkeypatch.delenv("IPFS_ACCELERATE_AGENT_REPOSITORY_ALLOWLIST", raising=False)
...
def _service(
    ...
    *,
    ...
    def operation_handler(request: OperationRequest) -> BackendResponse:
        if mutation_calls is not None and request.operation.mutating and not request.dry_run:
...
def test_registration_covers_every_operation_with_shared_schema() -> None:
    ...
    assert {item["name"] for item in manager.tools} == {operation.value for operation in Operation}
    ...
    for definition in manager.tools:
        ...
        assert {"bounded", "policy-controlled", "redacted"}.issubset(definition["tags"])
...
def test_discovery_safety_evidence_uses_observed_python_cli_and_mcp_runs(
    ...
    population_ids = {
        observation.manifest.schema_population_id for observation in evidence.observations
    }
...
async def test_mcp_result_is_exactly_the_python_service_record(
    ...
    actual = await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.STATUS](request=request.to_record())

    assert actual == expected
    assert agent_supervisor_service_resolution_count() == resolutions_before + 1
...
async def test_unconfigured_mcp_adapter_fails_closed_without_request_roots(
    ...
    with pytest.raises(
        ...
        await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.STATUS](request=request.to_record())
...
async def test_mcp_rejects_every_unsafe_real_mutation_before_service_resolution(
    ...
    with pytest.raises(error_type):
        await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.PAUSE](request=payload)
...
async def test_python_cli_mcp_matrix_emits_typed_parity_evidence(
    ...
    for scenario, request in _matrix_requests(repo_root, state_root):
        python_record = service.execute(request).to_record()
        command = next(
            name for name, operation in COMMAND_OPERATIONS.items() if operation is request.operation
        )
        exit_codes.append(
            cli.main(
                [
        ...
        mcp_record = await AGENT_SUPERVISOR_OPERATION_TOOLS[request.operation](
            request=request.to_record()
        )
        cases.append(
            ControlSurfaceParityCase(
    ...
    mutation_request = next(
        case.request for case in cases if case.scenario == "independent_mutation_success"
    )
    ...
    assert dict(evidence.request_schema_ids) == dict(mcp_manifest.request_schema_ids)
    assert dict(evidence.result_schema_ids) == dict(mcp_manifest.result_schema_ids)
    ...
    no_independent_completion_proof = evidence.evaluate_objective_completion(tasks_complete=True)
    assert no_independent_completion_proof.state.value == "provisionally_complete"
...
async def test_mcp_read_pagination_and_allowlists_are_enforced(
    ...
    events.write_text(
        "".join(json.dumps({"event_id": f"event:{index}"}) + "\n" for index in range(3)),
        encoding="utf-8",
    )
    ...
    page = await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.EVENTS](request=request.to_record())
    ...
    oversized_result = await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.EVENTS](
    ...
    outside_result = await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.STATUS](
...
async def test_mcp_dry_run_is_proposal_only_and_skips_backend_and_lease(
    ...
    record = await AGENT_SUPERVISOR_OPERATION_TOOLS[Operation.PAUSE](request=proposal.to_record())
...
async def test_mcp_mutation_is_fenced_idempotent_and_audited(
    ...
    assert receipts["data"]["items"][0]["receipt_id"] == first["audit_receipt_id"]
...
async def test_shared_redaction_preserves_python_cli_mcp_result_parity(
    ...
    assert mcp_record["data"]["tokens"] == [{"refresh_token": CONTROL_REDACTION_MARKER}]
