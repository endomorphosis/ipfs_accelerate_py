...
def test_metadata_declares_closed_bounded_profile_graph() -> None:
    ...
    assert tuple(project["dependencies"]) == CORE_REQUIREMENTS
    ...
    all_requirements = [
        ...
        *(value for values in extras.values() for value in values),
    ]
    parsed = [Requirement(value) for value in all_requirements]
...
def test_artifacts_bind_metadata_entrypoint_and_evaluation_surface(
    ...
    core = [requirement for requirement in requirements if requirement.marker is None]

    assert metadata["Name"] == "ipfs_accelerate_py"
    assert metadata["Version"] == "0.0.45"
    assert metadata["Requires-Python"] == ">=3.12"
    assert sorted(_canonical_requirement(str(item)) for item in core) == sorted(
        _canonical_requirement(item) for item in CORE_REQUIREMENTS
    )
    assert all(item.url is None and _is_bounded(item) for item in requirements)
    assert set(PROFILE_REQUIREMENTS) <= set(metadata.get_all("Provides-Extra", []))
