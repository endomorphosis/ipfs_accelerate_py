...
class TestErrorReporter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.reporter = ErrorReporter(
            github_token="test_token", github_repo="test_owner/test_repo", enabled=True
        )
    ...
    def test_initialization_from_env(self):
        ...
        with patch.dict(
            os.environ, {"GITHUB_TOKEN": "env_token", "GITHUB_REPO": "env_owner/env_repo"}
        ):
    ...
    def test_determine_labels(self):
        ...
        # MCP server error
        error_info1 = {"error_type": "RuntimeError", "source_component": "mcp-server"}
        ...
        # Dashboard error
        error_info2 = {"error_type": "TypeError", "source_component": "dashboard"}
        labels2 = self.reporter._determine_labels(error_info2)
        self.assertIn("dashboard", labels2)

        # Critical error
        error_info3 = {"error_type": "CriticalError", "source_component": "docker"}
    ...
    def test_save_and_load_reported_errors(self):
        ...
        new_reporter = ErrorReporter(github_token="test_token", github_repo="test_owner/test_repo")
    ...
    def test_create_github_issue_success(self, mock_post):
        ...
        mock_response.json.return_value = {"html_url": "https://github.com/test/repo/issues/123"}
    ...
    def test_report_error_with_exception(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"html_url": "https://github.com/test/repo/issues/124"}
        mock_post.return_value = mock_response
        ...
        try:
            ...
            issue_url = self.reporter.report_error(
                exception=e, source_component="test", context={"test": True}
            )
    ...
    def test_report_error_duplicate_prevention(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"html_url": "https://github.com/test/repo/issues/125"}
        mock_post.return_value = mock_response

        # Report first error
        issue_url1 = self.reporter.report_error(
            error_type="TestError", error_message="Same error", source_component="test"
        )
    ...
    def test_report_error_disabled(self):
        ...
        issue_url = reporter.report_error(
            error_type="TestError", error_message="Test", source_component="test"
        )
