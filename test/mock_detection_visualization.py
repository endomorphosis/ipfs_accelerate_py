...
def create_dependency_heatmap(
    ...
    title: str = "Mock Detection Implementation in Test Files",
) -> go.Figure:
    """
...
def create_mock_detection_summary(
    scan_results: List[Dict[str, Any]], output_file: str = None
) -> go.Figure:
    """
    ...
    # Update layout
    fig.update_layout(title="Mock Detection Implementation Summary", height=700, showlegend=False)
...
def create_model_family_analysis(
    scan_results: List[Dict[str, Any]], output_file: str = None
) -> go.Figure:
    """
...
def create_test_result_analysis(
    mock_results: List[Dict[str, Any]], output_file: str = None
) -> go.Figure:
    """
    ...
    # Update layout
    fig.update_layout(
        title="Mock vs. Real Dependencies Test Result Analysis", height=800, showlegend=False
    )
...
def create_interactive_dashboard(
    ...
    family_stats = (
        df.groupby("model_family")
        .agg({"has_mock_detection": "mean", "file": "count"})
        .reset_index()
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate visualizations for mock detection results"
    )
    parser.add_argument(
        "--test-dir",
        type=str,
        default="skills/fixed_tests",
    ...
    parser.add_argument(
        "--results-pattern",
        type=str,
        default="mock_detection_*.json",
        help="Pattern to match mock detection result files",
    )
    parser.add_argument(
        "--no-heatmap", action="store_true", help="Skip generating the dependency heatmap"
    )
    parser.add_argument(
        "--no-summary", action="store_true", help="Skip generating the mock detection summary"
    )
    parser.add_argument(
        "--no-family", action="store_true", help="Skip generating the model family analysis"
    )
    parser.add_argument(
        "--no-test-results", action="store_true", help="Skip generating the test result analysis"
    )
    parser.add_argument(
        "--no-dashboard", action="store_true", help="Skip generating the interactive dashboard"
    )
    parser.add_argument(
        "--no-report", action="store_true", help="Skip generating the markdown report"
    )
