...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--test-type",
        choices=["model", "hardware", "api", "integration", "all"],
        default="all",
        help="Type of tests to run",
    )
    parser.add_argument("--model", type=str, help="Specific model to test (e.g., bert, t5, vit)")
    parser.add_argument(
        "--platform", type=str, help="Hardware platform (e.g., webgpu, webnn, cuda, rocm, cpu)"
    )
    parser.add_argument("--api", type=str, help="API to test (e.g., openai, hf, vllm)")

    # Test execution options
    parser.add_argument(
        "--markers", type=str, help='pytest markers expression (e.g., "slow or webgpu")'
    )
    ...
    parser.add_argument(
        "--worker-count", type=int, default=4, help="Number of workers for distributed testing"
    )
    parser.add_argument(
        "--coordinator", type=str, help="Coordinator address for distributed testing"
    )
...
def build_pytest_command(args: argparse.Namespace) -> List[str]:
    ...
    if args.distributed:
        cmd.extend([f"--distributed", f"--worker-count={args.worker_count}"])
        if args.coordinator:
