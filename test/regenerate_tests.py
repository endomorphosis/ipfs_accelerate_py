...
def fix_indentation_issues(file_path):
    ...
    content = re.sub(
        r"self\.performance_stats = \{\}\s+def", "self.performance_stats = {}\n\n    def", content
    )
    ...
    pipeline_pattern = (
        r"(\s+def test_pipeline\(self,.*?(?=\s+def test_from_pretrained|\s+def run_tests|$))"
    )
...
def fix_method_content(method_text, method_name):
    ...
    for line in lines[1:]:
        ...
        if content.startswith('"""') or content.endswith('"""'):
            ...
            or content.startswith("try:")
            or content.startswith("else:")
            or content.startswith("elif ")
        ):
...
def regenerate_test_file(family, generator_path, output_dir):
    ...
    cmd = [sys.executable, generator_path, "--family", family, "--output", output_dir]
...
def add_model_to_generator(family, template_family, generator_path):
    ...
    try:
        ...
        updated_model_families = model_families_match.group(0).replace("}", f"{new_family_entry}}}")

        # Update the file content
        updated_content = content.replace(model_families_match.group(0), updated_model_families)
...
def main():
    parser = argparse.ArgumentParser(
        description="Regenerate test files for HuggingFace model families"
    )
    ...
    parser.add_argument(
        ...
        default=BASE_MODEL_FAMILIES,
        help=f"Model families to regenerate (default: {', '.join(BASE_MODEL_FAMILIES)})",
    )
    parser.add_argument("--all", action="store_true", help="Regenerate all known model families")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="fixed_tests",
    ...
    parser.add_argument(
        "--generator",
        type=str,
        default="test_generator_fixed.py",
        help="Path to the generator script (default: test_generator_fixed.py)",
    )

    # Advanced options
    parser.add_argument(
        "--list", action="store_true", help="List all known model families and exit"
    )
    parser.add_argument("--add-family", type=str, help="Add a new model family to the generator")
    parser.add_argument(
        "--template",
        type=str,
        choices=BASE_MODEL_FAMILIES,
