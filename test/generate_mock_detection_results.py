...
def generate_sample_result(num_models: int = 10) -> Dict[str, Any]:
    ...
    for family in model_families:
        if family == "bert":
            model_ids.extend(
                ["bert-base-uncased", "bert-large-uncased", "bert-base-cased", "bert-base-chinese"]
            )
        elif family == "gpt2":
            model_ids.extend(["gpt2", "gpt2-medium", "gpt2-large", "gpt2-xl"])
        elif family == "t5":
            model_ids.extend(["t5-small", "t5-base", "t5-large"])
        elif family == "vit":
            model_ids.extend(["vit-base-patch16-224", "vit-large-patch16-224"])
        elif family == "clip":
            model_ids.extend(["openai/clip-vit-base-patch32", "openai/clip-vit-large-patch14"])
        elif family == "blip":
            model_ids.extend(
                ["Salesforce/blip-image-captioning-base", "Salesforce/blip-image-captioning-large"]
            )
        elif family == "roberta":
            model_ids.extend(["roberta-base", "roberta-large"])
        elif family == "llama":
            model_ids.extend(["meta-llama/Llama-2-7b-hf", "meta-llama/Llama-2-13b-hf"])
        elif family == "whisper":
            model_ids.extend(["openai/whisper-tiny", "openai/whisper-base", "openai/whisper-small"])
        elif family == "wav2vec2":
            model_ids.extend(["facebook/wav2vec2-base-960h", "facebook/wav2vec2-large-960h"])
    ...
    for model_id in model_ids:
        ...
        duration_ms = int(
            random.uniform(duration_base - duration_variation, duration_base + duration_variation)
        )
...
def generate_multiple_results(
    num_results: int = 5, models_per_result: int = 10
) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    parser.add_argument(
        "--models-per-result",
        type=int,
        default=DEFAULT_MODELS_PER_RESULT,
