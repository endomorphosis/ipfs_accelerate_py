...
class ExportRequest(BaseModel):
    ...
    data_type: str = Field(
        ..., description="Type of data to export (forecast, anomaly, pattern, recommendation)"
    )
    ...
    if database_path:
        ...
        try:
            ...
            db_repository = DuckDBAPIMetricsRepository(
                db_path=database_path, create_if_missing=True
            )
            ...
            if generate_db_sample:
                ...
                cmd = [
                    sys.executable,
                    "duckdb_api/api_management/duckdb_api_metrics.py",
                    "--db-path",
                    database_path,
                    "--generate-sample",
                ]
...
async def get_metrics() -> List[str]:
    ...
    if not dashboard or not hasattr(dashboard, "historical_data"):
        raise HTTPException(
            status_code=503, detail="Dashboard not initialized or no data available"
        )
...
async def get_data(
    metric: str, provider: str, start_date: Optional[str] = None, end_date: Optional[str] = None
) -> List[Dict]:
...
async def get_anomalies(
    ...
    if not dashboard or not hasattr(dashboard, "anomalies"):
        raise HTTPException(
            status_code=503, detail="Dashboard not initialized or no anomalies available"
        )

    if metric not in dashboard.anomalies or provider not in dashboard.anomalies[metric]:
        raise HTTPException(
            status_code=404, detail=f"No anomalies available for {provider} - {metric}"
        )
...
async def get_recommendations(provider: str) -> List[Dict]:
    ...
    if not dashboard or not hasattr(dashboard, "recommendations"):
        raise HTTPException(
            status_code=503, detail="Dashboard not initialized or no recommendations available"
        )
...
async def filter_data(request: DataFilterRequest) -> Dict[str, Any]:
    ...
    result = {"historical_data": None, "predictions": None, "anomalies": None}

    # Get historical data
    if (
        hasattr(dashboard, "historical_data")
        and request.metric_type in dashboard.historical_data
        and request.api_name in dashboard.historical_data[request.metric_type]
    ):
        result["historical_data"] = dashboard.historical_data[request.metric_type][request.api_name]

    # Get predictions
    if (
        hasattr(dashboard, "predictions")
        and request.metric_type in dashboard.predictions
        and request.api_name in dashboard.predictions[request.metric_type]
    ):
        result["predictions"] = dashboard.predictions[request.metric_type][request.api_name]

    # Get anomalies
    if (
        hasattr(dashboard, "anomalies")
        and request.metric_type in dashboard.anomalies
        and request.api_name in dashboard.anomalies[request.metric_type]
    ):
    ...
    if request.data_type == "forecast":
        if request.api_name and request.metric_type:
            if (
                ...
                export_data["data"]["historical"] = dashboard.historical_data[request.metric_type][
                    request.api_name
                ]
            ...
            if (
                ...
                export_data["data"]["predictions"] = dashboard.predictions[request.metric_type][
                    request.api_name
                ]
        ...
        if request.api_name and request.metric_type:
            if (
                ...
                export_data["data"]["anomalies"] = dashboard.anomalies[request.metric_type][
                    request.api_name
                ]
        ...
        if request.api_name:
            if (
                hasattr(dashboard, "recommendations")
                and request.api_name in dashboard.recommendations
            ):
    ...
    try:
        while True:
            ...
            if dashboard:
                ...
                await websocket.send_json({"status": "not_initialized"})
...
def main():
    ...
    parser.add_argument(
        "--connect-dashboard", action="store_true", help="Connect to live monitoring dashboard"
    )
    ...
    parser.add_argument(
        "--theme", type=str, default="cosmo", help="UI theme (cosmo, darkly, flatly, etc.)"
    )
    parser.add_argument(
        "--enable-caching", action="store_true", help="Enable data caching for improved performance"
    )
