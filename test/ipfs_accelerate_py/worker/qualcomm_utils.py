...
class qualcomm_utils:
    ...
    def build_deployable_asset(self, model_path=None, output_path=None):
        ...
        try:
            export_quantized_onnx_cmd = (
                "python -m qai_hub_models.models." + model_path + ".export --target-runtime onnx"
            )
            export_quantized_onnx_cmd_results = subprocess.run(
                export_quantized_onnx_cmd,
                shell=True,
                check=True,
        ...
        try:
            ...
            convert_to_deployable_onnx_cmd_results = subprocess.run(
                convert_to_deployable_onnx_cmd,
                shell=True,
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            results["convert_to_deployable_onnx_cmd_results"] = (
                convert_to_deployable_onnx_cmd_results
            )
        except subprocess.CalledProcessError as e:
            print("Error: ", e)
            return None
    ...
    def install_qnn_model(self, model=None, path=None):
        ...
        pip_install_cmd_results = subprocess.run(
            pip_install_cmd, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def quantize_export_qnn_model(self, model=None, path=None):
        ...
        try:
            quantize_export_cmd = (
                ...
                + model
                + '.export --device "Snapdragon X Elite CRD" --skip-inferencing --skip-profiling --output-dir '
                + path
            )
            quantize_export_cmd_results = subprocess.run(
                quantize_export_cmd,
                shell=True,
                check=True,
                stdout=subprocess.PIPE,
        ...
        try:
            ...
            cleanup_dryrun_results = subprocess.run(
                cleanup_dryrun,
                shell=True,
                check=True,
                stdout=subprocess.PIPE,
    ...
    def remove_qnn_model(self, model=None):
        ...
        try:
            ...
            pip_uninstall_cmd_results = subprocess.run(
                pip_uninstall_cmd,
                shell=True,
                check=True,
                stdout=subprocess.PIPE,
    ...
    def setup_env(self, env_var=None):
        if platform.system() == "Windows":
            shutil.copytree(
                os.path.join(os.environ["QNN_SDK_ROOT"], "lib", "hexagon-v73", "unsigned"),
                "genie_bundle",
                dirs_exist_ok=True,
            )
            shutil.copytree(
                os.path.join(os.environ["QNN_SDK_ROOT"], "lib", "aarch64-windows-msvc"),
                "genie_bundle",
                dirs_exist_ok=True,
            )
            shutil.copy(
                os.path.join(
                    os.environ["QNN_SDK_ROOT"], "bin", "aarch64-windows-msvc", "genie-t2t-run.exe"
                ),
                "genie_bundle",
            )
        ...
        if platform.system() == "Android":
            ...
            if "hexagon" in arch:
                if "v73" in arch:
                    ...
                    shutil.copytree(
                        os.path.join(os.environ["QNN_SDK_ROOT"], "lib", "hexagon-v75", "unsigned"),
                        "genie_bundle",
                        dirs_exist_ok=True,
                    )
                elif "v79" in arch:
                    shutil.copytree(
                        os.path.join(os.environ["QNN_SDK_ROOT"], "lib", "hexagon-v79", "unsigned"),
                        "genie_bundle",
                        dirs_exist_ok=True,
                    )
                else:
                ...
                pass
            shutil.copytree(
                os.path.join(os.environ["QNN_SDK_ROOT"], "lib", "aarch64-android"),
                "genie_bundle",
                dirs_exist_ok=True,
            )
            shutil.copy(
                os.path.join(os.environ["QNN_SDK_ROOT"], "bin", "aarch64-android", "genie-t2t-run"),
                "genie_bundle",
            )
            return None
        return None
    ...
    def export_tokenizer(self, config, tokenizer, model, path):
        ...
        if os.path.isfile(path):
            os.remove(path)
            custom_tokenizer = self.transformers.AutoTokenizer(
                model.BPE(vocab=tokenizer.get_vocab(), merges=[])
            )
    ...
    def genie_t2t_run(self, config=None, text=None):
        ...
        with tempfile.TemporaryFile() as tmpfile:
            ...
            genie_t2t_run_cmd = (
                self.genie_t2t_run_path_linux + " --config " + filepath + " --text " + text
            )
            genie_t2t_run_cmd_results = subprocess.run(
                genie_t2t_run_cmd,
                shell=True,
                check=True,
                stdout=subprocess.PIPE,
