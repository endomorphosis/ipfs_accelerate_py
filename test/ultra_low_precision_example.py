...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="llama", help="Model to use (llama, t5, bert, clip, whisper)"
    )
    ...
    parser.add_argument(
        "--extended-context", action="store_true", help="Test context extension capabilities"
    )
    ...
    parser.add_argument(
        "--memory-constraint", type=int, default=None, help="Test with memory constraint (MB)"
    )

    parser.add_argument(
        "--output-json", type=str, default=None, help="Output file for results (JSON)"
    )
...
def example_ultra_low_precision_setup(
    model_name, bits, browser, mixed_precision=False, extended_context=False
):
    ...
    if not result["success"]:
        logger.error(
            f"Error setting up ultra-low precision: {result.get('error', 'Unknown error')}"
        )
        return None
    ...
    if extended_context:
        ...
        logger.info(
            f"Extended context: {config['context_extension_factor'] * 4096:.0f} tokens (from 4096)"
        )
    ...
    logger.info(
        f"Memory saved: {memory_savings['saved_mb']:.1f} MB ({memory_savings['reduction_percent']:.1f}%)"
    )
...
def main():
    ...
    if not ULTRA_LOW_PRECISION_AVAILABLE:
        ...
        logger.error(
            "Please make sure the fixed_web_platform.webgpu_ultra_low_precision module is installed."
        )
        return 1
    ...
    try:
        ...
        if setup_result:
            ...
            if args.extended_context:
                results["examples"]["setup"]["context_extension"] = setup_result[
                    "ultra_low_precision"
                ]["context_extension_factor"]

        # Example 2: Mixed Precision Configuration
        if args.mixed_precision:
            logger.info("\n=== Example 2: Mixed Precision Configuration ===")
            mp_config = example_mixed_precision_config(
                model_type="text", default_bits=args.bits, memory_mb=args.memory_constraint
            )
        ...
        if args.extended_context:
            ...
            context_result = example_context_extension(
                model_name=args.model, bits=args.bits, browser=args.browser
            )
