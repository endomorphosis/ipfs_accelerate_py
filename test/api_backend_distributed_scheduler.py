...
class APIRateLimit:
    ...
    def can_make_request(self, tokens: int = 0) -> bool:
        ...
        if (
            ...
            and len(self.minute_request_times) >= self.requests_per_minute
        ):
            return False
    ...
    def get_utilization(self) -> Dict[str, float]:
        ...
        if self.requests_per_minute > 0:
            utilization["minute_requests"] = (
                len(self.minute_request_times) / self.requests_per_minute
            )
...
class APIPerformanceProfile:
    ...
    def record_latency(self, latency: float) -> None:
        ...
        if len(self.historical_performance["latency"]) > self.max_history_length:
            self.historical_performance["latency"] = self.historical_performance["latency"][
    ...
    def record_success_rate(self, success_rate: float) -> None:
        ...
        if len(self.historical_performance["success_rate"]) > self.max_history_length:
            self.historical_performance["success_rate"] = self.historical_performance[
                "success_rate"
            ][-self.max_history_length :]
    ...
    def record_throughput(self, throughput: float) -> None:
        ...
        if len(self.historical_performance["throughput"]) > self.max_history_length:
            self.historical_performance["throughput"] = self.historical_performance["throughput"][
                -self.max_history_length :
            ]
...
class APIBackendProfile:
    ...
    def record_request_success(
        self, latency: float, input_tokens: int = 0, output_tokens: int = 0
    ) -> None:
...
class APIBackendScheduler:
    ...
    def _load_config(self, config_file: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            "rate_limits": {
                ...
                "groq": {"requests_per_minute": 100, "concurrent_requests": 100},
            },
            "costs": {
                "openai": {
                    "gpt-3.5-turbo": {
                    ...
                    "gpt-4": {"cost_per_1k_input_tokens": 0.01, "cost_per_1k_output_tokens": 0.03},
                },
                "claude": {
                    "claude-3-opus": {
    ...
    def _update_scheduling_metrics(self):
        ...
        with self.task_lock:
            ...
            for api_type, profile in self.api_profiles.items():
                self.scheduling_metrics["api_utilization"][api_type] = (
                    profile.rate_limits.get_utilization()
                )

            # Update cost tracking
            for api_type, profile in self.api_profiles.items():
                self.scheduling_metrics["cost_tracking"][api_type] = (
                    profile.cost_profile.get_cost_report()
                )
            ...
            if api_type in self.api_profiles:
                if success:
                    ...
                    self.api_profiles[api_type].record_request_success(
                        latency=duration, input_tokens=input_tokens, output_tokens=output_tokens
                    )
    ...
    def _select_api_for_task(self, task: Task, available_apis: List[str]) -> Optional[str]:
        ...
        for api_type in available_apis:
            ...
            if "required_capabilities" in task.metadata:
                ...
                if not all(
                    cap in profile.capabilities and profile.capabilities[cap] for cap in required
                ):
                    continue
    ...
    def get_api_status(self, api_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self.profile_lock:
            if api_type:
                if api_type not in self.api_profiles:
                    return {"error": f"API backend '{api_type}' not found"}

                return self.api_profiles[api_type].get_health_status()

            # Return status for all APIs
            return {api: profile.get_health_status() for api, profile in self.api_profiles.items()}
    ...
    def get_cost_report(self) -> Dict[str, Any]:
        ...
        with self.profile_lock:
            report = {"total_cost": 0.0, "api_costs": {}, "cost_by_model": {}}
...
if __name__ == "__main__":
    ...
    try:
        ...
        for api_type, api_status in status.items():
            print(
                f"  {api_type}: Circuit breaker = {api_status['circuit_breaker_status']}, "
