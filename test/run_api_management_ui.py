    ...
    for metric in metrics:
        ...
        for api in apis:
                ...
                future_values.append(
                    {
                        ...
                        if metric != "success_rate"
                        else min(max(lower_bound, 0), 100),
                        "upper_bound": upper_bound
                        if metric != "success_rate"
                        else min(upper_bound, 100),
                    }
                )
            ...
            anomaly_indices = np.random.choice(
                len(timestamps), size=min(5, len(timestamps) // 10), replace=False
            )
        ...
        for i, ts in enumerate(timestamps[::24]):  # Daily samples for comparative data
            ...
            data["comparative_data"][metric].append(
                {"timestamp": ts.isoformat(), "values": comparative_values}
            )
...
def save_sample_data(file_path: str) -> None:
    ...
    try:
        os.makedirs(os.path.dirname(os.path.path), exist_ok=True) if os.path.dirname(
            file_path
        ) else None
...
def main():
    ...
    if args.db_path:
        try:
            ...
            # Create repository
            db_repository = DuckDBAPIMetricsRepository(db_path=args.db_path, create_if_missing=True)
