#!/usr/bin/env python3
"""
Script to apply CUDA implementation detection fixes to the CLAP test file.
"""

import os

def add_cuda_detection_fixes():
    """Add CUDA detection fixes to CLAP test file."""
    file_path = "skills/test_hf_clap.py"
    ...
    if "# Check for simulated real implementation" not in content:
        target = """                # Additional checks for real models
                if hasattr(endpoint, "config") and hasattr(endpoint.config, "model_type") and endpoint.config.model_type in ["clap", "laion_clap"]:
                    is_real_impl = True
                    implementation_type = "(REAL)"
                    print("Detected real CLAP model based on config")
