...
class S3EndpointMultiplexer:
    ...
    def create_presigned_url(
        self, bucket, key, expiration=3600, endpoint_name=None, strategy="round-robin"
    ):
        handler = self.get_endpoint(endpoint_name, strategy)
...
result = multiplexer.list_objects(
    bucket="dev-bucket", prefix="test-data/", endpoint_name="minio-local"
)
...
replication_results = replicate_to_all_endpoints(
    "critical_data.db", "backups/critical_data_20250301.db"
)
```
...
def check_endpoint_health():
    ...
    for name in multiplexer.endpoint_handlers.keys():
        try:
            ...
            multiplexer.list_objects(bucket="health-check-bucket", endpoint_name=name)
