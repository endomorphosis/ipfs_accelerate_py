...
try:
    ...
    try:
        from ipfs_accelerate_py.predictive_performance.hardware_model_predictor import (
            HardwareModelPredictor,
        )
    ...
    try:
        ...
        if HARDWARE_MODEL_PREDICTOR_AVAILABLE:
            predictor = HardwareModelPredictor(
                benchmark_dir=args.benchmark_dir, database_path=args.benchmark_database
            )
        else:
            predictor = None

        adapter = HardwareModelPredictorDuckDBAdapter(
            predictor=predictor, repository=repository, user_id=args.user_id
        )
        ...
        print(
            f"  Model Size: {result.get('model_size_category', 'unknown')} ({result.get('model_size', 'unknown')} parameters)"
        )
...
def record_measurement(args):
    ...
    try:
        ...
        adapter = HardwareModelPredictorDuckDBAdapter(repository=repository, user_id=args.user_id)
...
def analyze_predictions(args):
    ...
    try:
        ...
        for metric, metric_stats in stats.items():
            ...
            print(
                f"  Range: {metric_stats.get('min_absolute_error', 0):.2f} - {metric_stats.get('max_absolute_error', 0):.2f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--database",
        type=str,
        default="predictive_performance.duckdb",
    ...
    parser.add_argument(
        "--benchmark-database", type=str, default=None, help="Path to benchmark database"
    )
    ...
    predict_hw_parser.add_argument(
        "--hardware", type=str, help="Comma-separated list of available hardware platforms"
    )
    predict_hw_parser.add_argument(
        "--predict-performance",
        action="store_true",
    ...
    predict_hw_parser.add_argument(
        "--user-id", type=str, default="cli-user", help="User ID for tracking recommendations"
    )
    predict_hw_parser.add_argument(
        "--no-store", action="store_true", help="Don't store prediction in database"
    )
    predict_hw_parser.set_defaults(func=predict_hardware)

    # predict-performance command
    predict_perf_parser = subparsers.add_parser(
        "predict-performance", help="Predict performance for a model on specified hardware"
    )
    ...
    predict_perf_parser.add_argument(
        "--hardware", type=str, required=True, help="Comma-separated list of hardware platforms"
    )
    ...
    predict_perf_parser.add_argument(
        "--user-id", type=str, default="cli-user", help="User ID for tracking predictions"
    )
    predict_perf_parser.add_argument(
        "--no-store", action="store_true", help="Don't store prediction in database"
    )
    ...
    record_meas_parser = subparsers.add_parser(
        "record-measurement", help="Record an actual performance measurement"
    )
    ...
    record_meas_parser.add_argument(
        "--prediction-id", type=str, help="ID of a previous prediction to compare with"
    )
    record_meas_parser.add_argument(
        "--source", type=str, default="cli", help="Source of the measurement"
    )
    record_meas_parser.add_argument(
        "--user-id", type=str, default="cli-user", help="User ID for tracking measurements"
    )
    ...
    analyze_parser.add_argument("--model", type=str, help="Filter by model name")
    analyze_parser.add_argument("--hardware", type=str, help="Filter by hardware platform")
    ...
    analyze_parser.add_argument("--days", type=int, help="Analyze predictions from the last N days")
    analyze_parser.set_defaults(func=analyze_predictions)

    # list-models command
    list_models_parser = subparsers.add_parser("list-models", help="List stored prediction models")
    list_models_parser.add_argument("--model-type", type=str, help="Filter by model type")
    list_models_parser.add_argument("--target-metric", type=str, help="Filter by target metric")
    list_models_parser.add_argument("--hardware", type=str, help="Filter by hardware platform")
    list_models_parser.add_argument("--family", type=str, help="Filter by model family")
    list_models_parser.add_argument(
        "--limit", type=int, default=10, help="Maximum number of models to list"
    )
    ...
    list_rec_parser.add_argument("--model", type=str, help="Filter by model name")
    list_rec_parser.add_argument("--family", type=str, help="Filter by model family")
    list_rec_parser.add_argument("--hardware", type=str, help="Filter by primary recommendation")
    list_rec_parser.add_argument("--user-id", type=str, help="Filter by user ID")
    list_rec_parser.add_argument("--accepted", type=bool, help="Filter by acceptance status")
    list_rec_parser.add_argument(
        "--days", type=int, help="List recommendations from the last N days"
    )
    list_rec_parser.add_argument(
        "--limit", type=int, default=10, help="Maximum number of recommendations to list"
    )
    list_rec_parser.set_defaults(func=list_recommendations)

    # record-feedback command
    feedback_parser = subparsers.add_parser(
        "record-feedback", help="Record feedback for a hardware recommendation"
    )
    ...
    feedback_parser.add_argument("--feedback", type=str, help="Optional feedback text")
    feedback_parser.add_argument("--user-id", type=str, default="cli-user", help="User ID")
