...
try:
    ...
    print(
        "Warning: Visualization libraries not available. Install matplotlib, pandas, numpy, and seaborn for visualizations."
    )
...
class APITestingExample:
    ...
    def __init__(self, output_dir: str = "api_test_results"):
        ...
        self.api_testing = APIDistributedTesting(resources={}, metadata={"api_keys": api_keys})
    ...
    def run_basic_tests(self) -> Dict[str, Any]:
        ...
        # Initialize results
        results = {"api_type": api_type, "timestamp": time.time(), "tests": {}}
        ...
        reliability_result = self.api_testing.run_distributed_test(
            ...
            parameters={"prompt": "What is machine learning?", "iterations": 5, "interval": 1},
        )
    ...
    def _print_basic_results_summary(self, results: Dict[str, Any]) -> None:
        ...
        if "latency" in results["tests"]:
            ...
            if "summary" in latency and latency.get("status") == "success":
                ...
                print(
                    f"  Success Rate: {summary.get('successful_iterations', 0)}/{latency.get('iterations', 0)}"
                )
            else:
        ...
        if "reliability" in results["tests"]:
            ...
            if "summary" in reliability and reliability.get("status") == "success":
                ...
                print(f"  Avg Latency: {summary.get('avg_latency', 'N/A'):.3f}s")
                print(
                    f"  Successful: {summary.get('successful_iterations', 0)}/{reliability.get('iterations', 0)}"
                )
            else:
    ...
    def run_api_comparison(self) -> Dict[str, Any]:
        ...
        if len(available_apis) < 2:
            available_apis = [
                available_apis[0],
                available_apis[0],
            ]  # Compare with itself as a fallback
            logger.warning(
                f"Only one API available ({available_apis[0]}), comparison will be limited."
            )
        ...
        # Initialize results
        results = {"apis_compared": apis_to_compare, "timestamp": time.time(), "comparisons": {}}
    ...
    def _generate_comparison_visualization(
        self, comparison_results: Dict[str, Any], test_type: str, output_file: str
    ) -> str:
        """
            ...
            if "summary" in throughput_comp:
                ...
                if "throughput_ranking" in summary:
                    ...
                    if "highest_throughput_api" in summary:
                        print(
                            f"Highest Throughput API: {summary.get('highest_throughput_api', 'N/A')}"
                        )
                ...
                for api in apis:
                    if api in throughput_comp.get("results_by_api", {}):
                        ...
                        if "summary" in api_result and api_result.get("status") == "success":
                            api_summary = api_result["summary"]
                            print(
                                f"  {api}: {api_summary.get('requests_per_second', 'N/A'):.2f} req/s"
                            )
            else:
    ...
    def run_cost_optimized_testing(self, budget: float = 1.0) -> Dict[str, Any]:
        ...
        for test_type in test_types:
            ...
            for api_type in available_apis:
                ...
                try:
                    ...
                    results["tests"][test_type][api_type] = {"status": "error", "error": str(e)}
        ...
        if VISUALIZATION_AVAILABLE:
            ...
            vis_file = self._generate_cost_optimization_visualization(
                results, os.path.join(self.output_dir, "cost_optimization.png")
            )
    ...
    def _estimate_test_cost(self, api_type: str, test_type: str) -> float:
        ...
        """
        if (
            not hasattr(self.scheduler, "api_profiles")
            or api_type not in self.scheduler.api_profiles
        ):
    ...
    def _generate_cost_optimization_visualization(
        self, results: Dict[str, Any], output_file: str
    ) -> str:
        """
        ...
        for api in apis_tested:
            if (
                ...
                and api in results["tests"]["latency"]
                and results["tests"]["latency"][api].get("status") == "success"
            ):
        ...
        for api in apis_tested:
            if (
                "throughput" in results["tests"]
                and api in results["tests"]["throughput"]
                and results["tests"]["throughput"][api].get("status") == "success"
            ):
        ...
        for test_type in results["tests"]:
            ...
            for api, result in results["tests"][test_type].items():
                if result.get("status") == "success" and "summary" in result:
                    if test_type == "latency":
                        print(
                            f"  {api}: {result['summary'].get('avg_latency', 'N/A'):.3f}s avg latency"
                        )
                    elif test_type == "throughput":
                        print(
                            f"  {api}: {result['summary'].get('requests_per_second', 'N/A'):.2f} req/s"
                        )
                else:
    ...
    def _execute_distributed_task(
        self, api_type: str, test_type: str, parameters: Dict[str, Any], priority: int
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_simulated_historical_data(self, apis: List[str], days: int) -> Dict[str, Any]:
        ...
        for api in apis:
            ...
            for i, ts in enumerate(timestamps):
                ...
                reliability_data.append({"timestamp": ts, "success_rate": success_rate})
    ...
    def _generate_historical_visualization(
        self, historical_data: Dict[str, Any], metric: str, apis: List[str], output_file: str
    ) -> str:
        """
        ...
        for api in apis:
            ...
            if metric == "latency":
                ...
                plt.fill_between(
                    timestamps, min_values, max_values, alpha=0.2, color=api_colors[api]
                )
...
def main():
    ...
    parser.add_argument(
        "--demo",
        choices=["basic", "comparison", "cost", "distributed", "visualization", "all"],
        default="all",
        help="Type of demo to run",
    )
    parser.add_argument(
        "--output-dir", type=str, default="api_test_results", help="Directory to save test results"
    )
