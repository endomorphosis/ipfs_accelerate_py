...
class TestBert(unittest.TestCase):
    ...
    def test_qualcomm(self):
        ...
        if not HAS_QUALCOMM: self.skipTest("Qualcomm AI Engine not available")
        
        # Set up device
        device = "cpu"  # Qualcomm uses CPU for PyTorch API
        ...
        try:
            ...
            # Log success
            logger.info(f"Successfully tested {self.model_name} on qualcomm")
            
        except Exception as e:
            logger.error(f"Error testing {self.model_name} on qualcomm: {str(e)}")
            raise
