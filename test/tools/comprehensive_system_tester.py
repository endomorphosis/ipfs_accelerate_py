...
class ComprehensiveSystemTester:
    ...
    def test_api_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.server_url}/api/inference/generate", json=test_data
            )
            if response.status_code == 200:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.server_url}/api/inference/classify", json=test_data
            )
            if response.status_code == 200:
        ...
        # Test embeddings
        try:
            test_data = {"text": "Generate embeddings for this text", "normalize": True}
            response = self.session.post(f"{self.server_url}/api/inference/embed", json=test_data)
            if response.status_code == 200:
        ...
        try:
            ...
            response = self.session.post(f"{self.server_url}/api/recommend", json=test_data)
            if response.status_code == 200:
    ...
    def generate_comprehensive_report(
        self, api_results: Dict[str, Any], screenshots: Dict[str, str]
    ) -> str:
...
"""
        ...
        successful_tests = sum(
            1 for result in api_results.values() if result.get("status", "").startswith("✅")
        )
...
def main():
    ...
    parser.add_argument(
        "--server-url", default="http://127.0.0.1:8090", help="Kitchen Sink server URL"
    )
