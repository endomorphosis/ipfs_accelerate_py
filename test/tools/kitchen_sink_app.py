...
try:
    ...
    from ipfs_accelerate_py.mcp_server.ai_model_server import create_ai_model_server
    ...
    def __init__(self):
        ...
        if HAVE_MODEL_MANAGER:
            ...
            self.bandit_recommender = BanditModelRecommender(
                model_manager=self.model_manager, storage_path="./kitchen_sink_bandit.json"
            )
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                for model in models:
                    if (
                        ...
                        or query in model["model_name"].lower()
        ...
        def generate_text():
            ...
            try:
                ...
                if model_id:
                    generated_text = (
                        f"[Generated by {model_id}] This is a continuation of: {prompt}..."
                    )
                else:
        ...
        def classify_text():
            ...
            try:
                ...
                prediction = (
                    "POSITIVE" if "good" in text.lower() or "great" in text.lower() else "NEUTRAL"
                )
        ...
        def search_huggingface():
            ...
            try:
                ...
                models = self.hf_search_engine.search_huggingface_models(
                    query=query, limit=limit, filter_dict=filter_dict, sort=sort_by
                )
            ...
            try:
                ...
                if model_info.repository_structure:
                    ...
                    response["repository"] = {
                        ...
                        "main_files": [
                            f
                            for f in repo.get("files", {}).keys()
                            if f.endswith((".json", ".bin", ".safetensors", ".py", ".md"))
                        ][:10],
                    }
        ...
        def get_popular_models_by_task(task):
            ...
            try:
                ...
                models = self.hf_search_engine.get_popular_models(limit=limit, task_filter=task)
