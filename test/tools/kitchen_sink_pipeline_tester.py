...
""",
    ...
    async def _test_text_generation_api(self, session):
        ...
        try:
            payload = {"prompt": "The future of AI is", "max_length": 50, "temperature": 0.7}
    ...
    async def _test_text_classification_api(self, session):
        ...
        try:
            payload = {"text": "This movie is amazing!"}
            async with session.post(f"{self.server_url}/api/classify", json=payload) as resp:
                if resp.status == 200:
                    ...
                    if "classification" in data:
                        ...
                        self.test_results["text_classification_api"] = (
                            "no_classification_in_response"
                        )
    ...
    async def _test_text_embeddings_api(self, session):
        ...
        try:
            payload = {"text": "Machine learning is transforming the world"}
