...
# Main Speech-to-Text model registry
SPEECH_TO_TEXT_MODELS_REGISTRY = {
    "facebook/s2t-small-librispeech-asr": {
        "full_name": "Speech2Text Small LibriSpeech ASR",
        "architecture": "encoder-decoder",
        "description": "Speech2Text model trained on LibriSpeech dataset for ASR",
        ...
        "attention_heads": 4,
        "layers": 12,
        "recommended_tasks": ["automatic-speech-recognition"]
    },
    "facebook/s2t-medium-librispeech-asr": {
        "full_name": "Speech2Text Medium LibriSpeech ASR",
        "architecture": "encoder-decoder",
        "description": "Speech2Text model trained on LibriSpeech dataset for ASR",
        "speech_to_text": "speech_to_text",
        "parameters": "100M",
    ...
    "facebook/s2t-large-librispeech-asr": {
        "full_name": "Speech2Text Large LibriSpeech ASR",
        "architecture": "encoder-decoder",
        "description": "Speech2Text model trained on LibriSpeech dataset for ASR",
        "speech_to_text": "speech_to_text",
        "parameters": "300M",
...
class TestModelTypeModels:
    def __init__(self, model_id="facebook/s2t-small-librispeech-asr", device=None):
        """Initialize the test class for SPEECH_TO_TEXT models.
        
        Args:
            model_id: The model ID to test (default: "facebook/s2t-small-librispeech-asr")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing SPEECH_TO_TEXT model {self.model_id} with pipeline API on {self.device}")
            ...
            # Create a simple dummy audio input (1 second of silence)
            import numpy as np
            sample_rate = 16000
            dummy_audio = np.zeros(sample_rate)  # 1 second of silence
            
            # Record inference start time
            inference_start = time.time()
            
            # Run inference
            outputs = pipe(dummy_audio, sampling_rate=sample_rate)
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing SPEECH_TO_TEXT model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and processor
            model = transformers.AutoModelForSpeechSeq2Seq.from_pretrained(self.model_id)
            processor = transformers.Speech2TextProcessor.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing SPEECH_TO_TEXT model {self.model_id} with OpenVINO")
            ...
            processor = transformers.Speech2TextProcessor.from_pretrained(self.model_id)
            inputs_pt = processor(audio=dummy_audio, sampling_rate=sample_rate, return_tensors="pt")
            inputs_np = {k: v.numpy() for k, v in inputs_pt.items()}
    
            # Record inference start time
            inference_start = time.time()
    
            # Forward pass
            outputs = compiled_model(inputs_np)[0]
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_speech_to_text_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SPEECH_TO_TEXT HuggingFace models")
    parser.add_argument("--model", type=str, default="facebook/s2t-small-librispeech-asr", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available SPEECH_TO_TEXT models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable SPEECH_TO_TEXT models:")
        for model_id, info in SPEECH_TO_TEXT_MODELS_REGISTRY.items():
    ...
    if pipeline_success:
        transcription = results["pipeline"].get("transcription", "N/A")
    ...
    if from_pretrained_success:
        transcription = results["from_pretrained"].get("transcription", "N/A")
    ...
    if "openvino" in results:
        ...
        if openvino_success:
            transcription = results["openvino"].get("transcription", "N/A")
    ...
    print(f"\nSuccessfully tested Speech-to-Text model: {args.model}")
