...
PYTHIA_MODELS_REGISTRY = {
    "EleutherAI/pythia-70m": {
        "description": "Pythia 70M base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-160m": {
        "description": "Pythia 160M base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-410m": {
        "description": "Pythia 410M base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-1b": {
        "description": "Pythia 1B base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-1.4b": {
        "description": "Pythia 1.4B base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-2.8b": {
        "description": "Pythia 2.8B base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-6.9b": {
        "description": "Pythia 6.9B base model",
        "class": "AutoModelForCausalLM",
    },
    "EleutherAI/pythia-12b": {
        "description": "Pythia 12B base model",
        "class": "AutoModelForCausalLM",
    }
}

class TestPythiaModels:
    """Base test class for all Pythia-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "EleutherAI/pythia-1b"
        
        # Verify model exists in registry
        if self.model_id not in PYTHIA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = PYTHIA_MODELS_REGISTRY["EleutherAI/pythia-1b"]
        else:
            self.model_info = PYTHIA_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Pythia was a priestess of Apollo who"
        self.test_texts = [
            "Pythia was a priestess of Apollo who",
            "In Greek mythology, Pythia served as"
        ]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            # Check pad token setup for Pythia models
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for Pythia tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for Pythia padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for Pythia tokenizer")
    ...
    def test_batch_inference(self, device="auto"):
        """Test batch inference capabilities of the model."""
        if device == "auto":
            device = self.preferred_device
        ...
        if not HAS_TRANSFORMERS or not HAS_TORCH:
            results["batch_error_type"] = "missing_dependency"
            results["batch_missing_core"] = ["transformers", "torch"]
            results["batch_success"] = False
            return results
        
        try:
            logger.info(f"Testing {self.model_id} with batch inference on {device}...")
            ...
            # Process logits for each sequence in batch
            batch_results = []
            if hasattr(outputs, "logits"):
                ...
                for i in range(batch_size):
                    ...
                    next_token_logits = sequence_logits[-1, :]
                    next_token_id = torch.argmax(next_token_logits).item()
                    
                    if hasattr(tokenizer, "decode"):
                        next_token = tokenizer.decode([next_token_id])
                        batch_results.append({
                            "input": self.test_texts[i],
            ...
            # Store results
            results["batch_success"] = True
            results["batch_inference_time"] = inference_time
            results["batch_size"] = batch_size
            results["per_item_time"] = inference_time / batch_size
            results["batch_results"] = batch_results
            results["batch_error_type"] = "none"
            
            # Store in performance stats
            self.performance_stats[f"batch_{device}"] = {
            ...
            # Add to examples
            self.examples.append({
                "method": f"batch_inference on {device}",
            ...
            results["batch_traceback"] = traceback.format_exc()
            logger.error(f"Error testing batch inference on {device}: {e}")
            ...
            if "cuda" in error_str or "cuda" in traceback_str:
                results["batch_error_type"] = "cuda_error"
            elif "memory" in error_str:
                results["batch_error_type"] = "out_of_memory"
            elif "no module named" in error_str:
                results["batch_error_type"] = "missing_dependency"
            else:
                results["batch_error_type"] = "other"
        
        # Add to overall results
        self.results[f"batch_{device}"] = results
        return results
    ...
    def run_tests(self, all_hardware=False):
        ...
        if all_hardware:
            ...
            if self.preferred_device != "cpu":
                ...
                self.test_batch_inference(device="cpu")
            ...
            if HW_CAPABILITIES["cuda"] and self.preferred_device != "cuda":
                ...
                self.test_batch_inference(device="cuda")
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_pythia_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Pythia models in the registry."""
    return list(PYTHIA_MODELS_REGISTRY.keys())
    
def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Pythia models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestPythiaModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_pythia_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test EleutherAI Pythia models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable EleutherAI Pythia models:")
        for model in models:
            info = PYTHIA_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "EleutherAI/pythia-1b"
    logger.info(f"Testing model: {model_id}")
