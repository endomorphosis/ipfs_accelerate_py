...
"""
Template for models with hyphenated names like xlm-rocaninea.
...
class TestXlmRocanineaModels:
    """
    Test class for XLM-RoCANINEa models.
        """
...
def main():
    ...
    xlm_rocaninea_tester = TestXlm_rocanineaModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = xlm_rocaninea_tester.run_tests()
    ...
    if success:              print(f"  Successfully tested {args.model}")
                      print(f"  - Device: {xlm_rocaninea_tester.device}")
