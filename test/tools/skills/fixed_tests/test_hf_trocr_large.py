...
# Main TrocrLarge model registry
TROCR_LARGE_MODELS_REGISTRY = {
    "trocr-large": {
        "full_name": "TrocrLarge Base Uncased",
        "architecture": "encoder-only",
        "description": "TrocrLarge Base model with uncased vocabulary",
        "model_type": "trocr_large",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
        "attention_heads": 12,
    ...
    "trocr_large-large-uncased": {
        "full_name": "TrocrLarge Large Uncased",
        "architecture": "encoder-only",
        "description": "TrocrLarge Large model with uncased vocabulary",
        "model_type": "trocr_large",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "trocr_large-base-cased": {
        "full_name": "TrocrLarge Base Cased",
        "architecture": "encoder-only",
        "description": "TrocrLarge Base model with cased vocabulary",
        "model_type": "trocr_large",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestTrocr_largeModels:
    def __init__(self, model_id="trocr-large", device=None):
        """Initialize the test class for TrocrLarge models.
        
        Args:
            model_id: The model ID to test (default: "trocr-large")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrLarge model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrLarge model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.TrocrLargeForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrLarge model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.TrocrLargeForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_trocr_large_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test TrocrLarge HuggingFace models")
    parser.add_argument("--model", type=str, default="trocr-large", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available TrocrLarge models")
    
    args = parser.parse_args()
    
    # List models if requested
    if args.list_models:
        print(f"\nAvailable TrocrLarge models:")
        for model_id, info in TROCR_LARGE_MODELS_REGISTRY.items():
    ...
    trocr_large_tester = TestTrocr_largeModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = trocr_large_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {trocr_large_tester.device}")
    ...
    print(f"\nSuccessfully tested TrocrLarge model: {args.model}")
