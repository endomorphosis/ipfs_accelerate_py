...
"""
Template for models with hyphenated names like xlm-rolayoutlma.
...
try:      import transformers
             ...
             XLM_ROLAYOUTLMA_MODELS_REGISTRY = {
                 "xlm-roberta-base": {
...
class TestXlmRolayoutlmaModels:
    """
    Test class for XLM-RoLAYOUTLMa models.
        """
...
def main():
    ...
    xlm_rolayoutlma_tester = TestXlm_rolayoutlmaModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = xlm_rolayoutlma_tester.run_tests()
    ...
    if success:              print(f"  Successfully tested {args.model}")
                      print(f"  - Device: {xlm_rolayoutlma_tester.device}")
