...
# Main TrocrBase model registry
TROCR_BASE_MODELS_REGISTRY = {
    "trocr-base": {
        "full_name": "TrocrBase Base Uncased",
        "architecture": "encoder-only",
        "description": "TrocrBase Base model with uncased vocabulary",
        "model_type": "trocr_base",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
        "attention_heads": 12,
    ...
    "trocr_base-large-uncased": {
        "full_name": "TrocrBase Large Uncased",
        "architecture": "encoder-only",
        "description": "TrocrBase Large model with uncased vocabulary",
        "model_type": "trocr_base",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "trocr_base-base-cased": {
        "full_name": "TrocrBase Base Cased",
        "architecture": "encoder-only",
        "description": "TrocrBase Base model with cased vocabulary",
        "model_type": "trocr_base",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestTrocr_baseModels:
    def __init__(self, model_id="trocr-base", device=None):
        """Initialize the test class for TrocrBase models.
        
        Args:
            model_id: The model ID to test (default: "trocr-base")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrBase model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrBase model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.TrocrBaseForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing TrocrBase model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.TrocrBaseForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_trocr_base_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test TrocrBase HuggingFace models")
    parser.add_argument("--model", type=str, default="trocr-base", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available TrocrBase models")
    
    args = parser.parse_args()
    
    # List models if requested
    if args.list_models:
        print(f"\nAvailable TrocrBase models:")
        for model_id, info in TROCR_BASE_MODELS_REGISTRY.items():
    ...
    trocr_base_tester = TestTrocr_baseModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = trocr_base_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {trocr_base_tester.device}")
    ...
    print(f"\nSuccessfully tested TrocrBase model: {args.model}")
