...
class SDKDashboardApp:
    ...
    def _setup_dashboard_routes(self):
        ...
        def static_files(filename):
            ...
            return send_from_directory(
                os.path.join(os.path.dirname(__file__), "..", "ipfs_accelerate_py", "static"),
                filename,
            )
    ...
    def run_jsonrpc_server(self):
        ...
        uvicorn.run(
            self.jsonrpc_server.app, host="127.0.0.1", port=self.jsonrpc_port, log_level="info"
        )
    ...
    def run_dashboard(self):
        ...
        self.dashboard_app.run(
            host="127.0.0.1", port=self.dashboard_port, debug=False, use_reloader=False
        )
    ...
    def run(self):
        ...
        jsonrpc_thread = threading.Thread(target=self.run_jsonrpc_server, daemon=True)
