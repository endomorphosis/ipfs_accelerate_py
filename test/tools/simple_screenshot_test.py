    ...
    try:
        ...
        try:
            response = requests.post(
                base_url + "/api/generate", json={"prompt": "Test", "max_length": 50}, timeout=15
            )
        ...
        try:
            response = requests.post(
                base_url + "/api/classify", json={"text": "This is a test"}, timeout=15
            )
        ...
        try:
            response = requests.post(
                base_url + "/api/embeddings", json={"text": "This is a test"}, timeout=15
            )
...
def capture_with_wkhtmltopdf():
    ...
    try:
        ...
        subprocess.run(
            ["sudo", "apt-get", "install", "-y", "wkhtmltopdf"], check=False, capture_output=True
        )
        ...
        for i, tab_id in enumerate(tabs, 1):
            try:
                ...
                driver.save_screenshot(
                    str(screenshots_dir / f"{i:02d}_{tab_id.replace('-tab', '')}.png")
                )
