    ...
    def test_all_endpoints(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/inference/generate",
            ...
            print(
                f"✅ Generation endpoint: Generated text in {gen_data.get('processing_time', 0):.3f}s"
            )
        except Exception as e:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/inference/classify",
            ...
            print(
                f"✅ Classification endpoint: Prediction '{class_data.get('prediction')}' with {class_data.get('confidence', 0):.2f} confidence"
            )
        except Exception as e:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/inference/embed",
                json={"text": "Sample text for embedding"},
                timeout=10,
            )
        ...
        try:
            response = requests.post(
                ...
                json={"task_type": "generation", "hardware": "cpu"},
                timeout=10,
            )
            ...
            if "model_id" in rec_data:
                ...
                print(
                    f"⚠️ Recommendations endpoint: {rec_data.get('error', 'No recommendation available')}"
                )
        except Exception as e:
        ...
        report_data = {
            ...
            "summary": {
                ...
                "total_improvements": sum(
                    len(cat["improvements"]) for cat in self.improvements_made
                ),
...
"""
        ...
        for endpoint, result in self.test_results.items():
            ...
            details = ", ".join(
                [f"{k}: {v}" for k, v in result.items() if k not in ["success", "error"]]
            )
