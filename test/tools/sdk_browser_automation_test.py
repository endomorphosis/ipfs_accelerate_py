...
class SDKBrowserTester:
    ...
    async def test_server_connection(self):
        ...
        try:
            ...
            if response and response.status == 200:
                ...
                print(
                    f"❌ Dashboard server error: {response.status if response else 'No response'}"
                )
    ...
    async def test_sdk_initialization(self):
        ...
        try:
            ...
            await self.page.wait_for_function(
                "typeof window.MCPClient !== 'undefined'", timeout=10000
            )
            ...
            if sdk_loaded:
                ...
                # Take screenshot
                await self.page.screenshot(
                    path=self.screenshots_dir / "02_sdk_connected.png", full_page=True
                )
    ...
    async def test_text_generation(self):
        ...
        try:
            ...
            await self.page.screenshot(
                path=self.screenshots_dir / "03_text_generation_form.png", full_page=True
            )
            ...
            if result_content and len(result_content.strip()) > 0:
                ...
                await self.page.screenshot(
                    path=self.screenshots_dir / "04_text_generation_result.png", full_page=True
                )
            else:
    ...
    async def test_text_classification(self):
        ...
        try:
            ...
            await self.page.fill(
                "#classification-text", "I love this product! It works amazingly well."
            )

            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "05_text_classification_form.png", full_page=True
            )
            ...
            await self.page.wait_for_selector(
                "#text-classification-result .classification-result", timeout=15000
            )
            ...
            if "Classification Result" in result_content:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "06_text_classification_result.png", full_page=True
                )
            else:
    ...
    async def test_text_embeddings(self):
        ...
        try:
            ...
            # Fill in the form
            await self.page.fill(
                "#embeddings-text", "This is a sample text for embedding generation."
            )

            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "07_text_embeddings_form.png", full_page=True
            )
            ...
            # Wait for result
            await self.page.wait_for_selector(
                "#text-embeddings-result .embeddings-result", timeout=15000
            )
            ...
            if "Text Embeddings" in result_content and "Dimensions:" in result_content:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "08_text_embeddings_result.png", full_page=True
                )
            else:
    ...
    async def test_model_recommendations(self):
        ...
        try:
            ...
            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "09_recommendations_form.png", full_page=True
            )
            ...
            # Wait for result
            await self.page.wait_for_selector(
                "#model-recommendations-result .recommendations-result", timeout=15000
            )
            ...
            if "Model Recommendations" in result_content:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "10_recommendations_result.png", full_page=True
                )
            else:
    ...
    async def test_model_manager(self):
        ...
        try:
            ...
            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "11_model_manager_form.png", full_page=True
            )
            ...
            # Wait for result
            await self.page.wait_for_selector(
                "#model-search-result .model-search-result", timeout=15000
            )
            ...
            if "Model Search Results" in result_content:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "12_model_manager_result.png", full_page=True
                )
            else:
    ...
    async def test_vision_models(self):
        ...
        try:
            ...
            # Fill in the form
            await self.page.fill(
                "#image-prompt", "A beautiful sunset over the ocean with sailing boats"
            )
            ...
            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "13_vision_models_form.png", full_page=True
            )
            ...
            # Wait for result
            await self.page.wait_for_selector(
                "#image-generation-result .result-item", timeout=15000
            )
            ...
            if result_content and len(result_content.strip()) > 0:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "14_vision_models_result.png", full_page=True
                )
            else:
    ...
    async def test_specialized_tools(self):
        ...
        try:
            ...
            # Take screenshot before submission
            await self.page.screenshot(
                path=self.screenshots_dir / "15_specialized_tools_form.png", full_page=True
            )
            ...
            if "Generated Code" in result_content:
                ...
                # Take screenshot of result
                await self.page.screenshot(
                    path=self.screenshots_dir / "16_specialized_tools_result.png", full_page=True
                )
            else:
    ...
    async def generate_summary_report(self):
        ...
        self.test_results["total_tests"] = (
            len(self.test_results["inference_tests"]) + 3
        )  # +3 for basic tests
        ...
        self.test_results["failed_tests"] = (
            self.test_results["total_tests"] - self.test_results["passed_tests"]
        )
        ...
        await self.page.screenshot(
            path=self.screenshots_dir / "17_final_dashboard_overview.png", full_page=True
        )
