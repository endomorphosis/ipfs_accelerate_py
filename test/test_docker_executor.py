...
class TestDockerExecutor(unittest.TestCase):
    ...
    def test_build_docker_command_basic(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            config = DockerExecutionConfig(
                image="python:3.9", command=["python", "-c", "print('hello')"]
            )
    ...
    def test_build_docker_command_with_resources(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            executor = DockerExecutor()
            config = DockerExecutionConfig(
                image="ubuntu:20.04", memory_limit="1g", cpu_limit=2.0, command=["echo", "test"]
            )
    ...
    def test_execute_container_success(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(returncode=0, stdout="Hello from container", stderr="")

            config = DockerExecutionConfig(
                image="ubuntu:20.04", command=["echo", "Hello from container"]
            )
    ...
    def test_execute_container_failure(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            config = DockerExecutionConfig(image="ubuntu:20.04", command=["false"])
    ...
    def test_execute_container_timeout(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            config = DockerExecutionConfig(
                image="ubuntu:20.04", command=["sleep", "1000"], timeout=1
            )
            ...
            self.assertTrue(
                ...
                or (result.error_message and "timeout" in result.error_message.lower())
            )
    ...
    def test_stop_container_failure(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            # Mock stop failure
            mock_run.return_value = Mock(returncode=1, stderr="No such container")
    ...
    def test_pull_image_success(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            # Mock pull success
            mock_run.return_value = Mock(returncode=0, stdout="Successfully pulled python:3.9")
...
class TestGitHubDockerBuild(unittest.TestCase):
    ...
    def test_github_config_creation(self):
        ...
        config = GitHubDockerConfig(
            repo_url="https://github.com/user/repo", branch="main", dockerfile_path="Dockerfile"
        )
    ...
    def test_build_and_execute_github_repo(self, mock_rmtree, mock_mkdtemp, mock_run):
        ...
        github_config = GitHubDockerConfig(repo_url="https://github.com/user/repo", branch="main")

        execution_config = DockerExecutionConfig(image="placeholder", command=["python", "app.py"])

        result = executor.build_and_execute_github_repo(github_config, execution_config)
