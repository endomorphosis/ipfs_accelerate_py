...
def test_provider_discovery():
    ...
    providers_to_check = ["codex_cli", "copilot_cli", "gemini_cli", "claude_code"]
...
def test_caching():
    ...
    key1 = _response_cache_key(
        provider="test", model_name="gpt-4", prompt="test prompt", kwargs={"temperature": 0.7}
    )
    ...
    key3 = _response_cache_key(
        provider="test", model_name="gpt-4", prompt="different prompt", kwargs={"temperature": 0.7}
    )
...
def test_xai_llm_router_provider_names():
    ...
    try:
        ...
        assert hasattr(provider, "chat_completions"), (
            "xAI provider must have chat_completions method"
        )
    finally:
