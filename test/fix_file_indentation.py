...
def fix_string_patterns(content):
    ...
    fixes = [
        ...
        (
            "return results    def test_from_pretrained",
...
def fix_spacing(content):
    ...
    content = re.sub(
        r"(class \w+:.*?performance_stats = \{\})\s+(\s+def)", r"\1\n\n\2", content, flags=re.DOTALL
    )

    # Add proper spacing between functions outside classes
    content = re.sub(
        r"(def \w+\(.+\):.*?return [^}]+})\s+(\s*def)", r"\1\n\n\n\2", content, flags=re.DOTALL
    )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", help="Output path (defaults to overwriting the input file)"
    )
    parser.add_argument(
