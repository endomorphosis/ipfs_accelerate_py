...
from ..style_doc import format_code_example  # TODO: Fix this relative import after reorganization
...
:
            if len()))outputs) > 0:
                ...
                # when needed.
                code_lines = [],f"\1{l}\3" if not len()))l) == 0 or l.isspace()))) else l for l in code_lines]:
                    code = "\n".join()))code_lines)
...
:
        if src_path.is_dir()))) and args.open_notebook_prefix is not None:
            ...
            colab_link = f"\1{str()))relative_path)}\3"
