...
python test/scripts/benchmark_db_query.py --sql "SELECT * FROM test_results WHERE hardware_type='qualcomm'" --format table

# Generate power efficiency comparison report
python test/scripts/benchmark_db_query.py --report qualcomm_power_efficiency --format html --output power_report.html
```
...
python test/simple_test_generator.py -g vit -p qualcomm -o test_vit_qualcomm.py

# Generate a test with all hardware platforms including Qualcomm
python test/simple_test_generator.py -g bert -p all
```
