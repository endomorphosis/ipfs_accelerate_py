...
class GeminiClient:
    ...
    def create_embeddings(self, text: Union[str, List[str]]) -> np.ndarray:
        ...
        try:
            ...
            if isinstance(text, list):
                result = self.embeddings_client.batch_embed_content(model=model, content=text)
                return np.array([item.embedding for item in result.embeddings])
            else:
                result = self.embeddings_client.embed_content(model=model, content=text)
    ...
    def _text_to_pseudo_embedding(self, text: str) -> np.ndarray:
        ...
        pseudo_embedding = np.array(
            [(hash_val >> (i * 8)) & 0xFF for i in range(16)], dtype=np.float32
        )
...
class GeminiClient:
    ...
    def __init__(self, api_key=None, **kwargs):
        ...
        if self.use_semantic_cache:
            ...
            self.logger.info(
                f"Initialized semantic cache with threshold {self.similarity_threshold}"
            )
        else:
...
async def generate_content(self, prompt, temperature=0.7, max_tokens=None, **kwargs):
    ...
    with self.stats_lock:
        ...
        self.cache_stats["avg_similarity"] = (
            self.cache_stats["avg_similarity"] * (self.cache_stats["total_requests"] - 1)
            + similarity
