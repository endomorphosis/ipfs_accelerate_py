...
class OpenAISemanticCache:
    ...
    def __init__(
        ...
        logger.info(f"Initialized OpenAI semantic cache with threshold {similarity_threshold}")
...
class SemanticCacheOpenAIClient:
    ...
    def __init__(
        ...
        embedding_dimensions: int = 1536  # OpenAI embeddings dimensions
    ):
        """
        ...
        self.cache = OpenAISemanticCache(
    ...
    def get_embedding(self, text: Union[str, List[str]]) -> np.ndarray:
        """
        Get embeddings for text using OpenAI's embedding API if available.
        ...
        """
        # Try to use the embeddings endpoint if available
        try:
            if hasattr(self.base_client, 'embeddings'):
                # OpenAI Python SDK
            ...
            elif hasattr(self.base_client, 'create_embedding'):
                # Custom client implementation
                return self.base_client.create_embedding(text)
            else:
...
async def example_usage():
    ...
    print("Testing OpenAI semantic cache with various queries...\n")
    
    for messages in example_messages:
        print(f"\nProcessing: {messages[-1]['content']}")
        response = await cached_client.create_chat_completion(messages, temperature=0.0)
