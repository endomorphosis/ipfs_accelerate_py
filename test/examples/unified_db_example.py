...
def search_across_components(
    base_url: str, api_key: str, query: str, limit: int = 10
) -> Dict[str, Any]:
    """
    ...
    response = requests.post(
        f"{base_url}/api/db/search", params={"query": query, "limit": limit}, headers=headers
    )
...
def get_component_specific_data(
    base_url: str, api_key: str, component: str, endpoint: str
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:8080", help="Base URL of the Unified API Server"
    )
    parser.add_argument("--api-key", required=True, help="API key for authentication")
    parser.add_argument(
        "--model", default="bert-base-uncased", help="Model name for unified queries"
    )
    ...
    if overview:
        # Print test stats
        if (
            "test_stats" in overview
            and overview["test_stats"]
            and not isinstance(overview["test_stats"], dict)
        ):
            test_stats = overview["test_stats"]
            print(f"Test Stats: {len(test_stats)} models")
            for i, stat in enumerate(test_stats[:3]):  # Show top 3
                print(
                    f"  - {stat.get('model_name', 'Unknown')}: {stat.get('total_tasks', 0)} tasks, "
        ...
        if (
            "generator_stats" in overview
            and overview["generator_stats"]
            and not isinstance(overview["generator_stats"], dict)
        ):
        ...
        if (
            "benchmark_stats" in overview
            and overview["benchmark_stats"]
            and not isinstance(overview["benchmark_stats"], dict)
        ):
            ...
            for i, stat in enumerate(bench_stats[:3]):  # Show top 3
                print(
                    f"  - {stat.get('model_name', 'Unknown')}: {stat.get('total_runs', 0)} runs, "
    ...
    if model_data and "overview" in model_data:
        ...
        if "recent_test_runs" in model_data and model_data["recent_test_runs"]:
            ...
            for i, run in enumerate(recent_tests[:2]):  # Show top 2
                print(
                    f"  - {run.get('run_id', 'Unknown')}: Status: {run.get('status', 'Unknown')}, "
        ...
        if "recent_generator_tasks" in model_data and model_data["recent_generator_tasks"]:
            ...
            for i, task in enumerate(recent_gens[:2]):  # Show top 2
                print(
                    f"  - {task.get('task_id', 'Unknown')}: Status: {task.get('status', 'Unknown')}, "
    ...
    if search_results and "result_counts" in search_results:
        ...
        if "test_results" in search_results and search_results["test_results"]:
            ...
            for i, result in enumerate(test_results[:2]):  # Show top 2
                print(
                    f"  - {result.get('run_id', 'Unknown')}: Model: {result.get('model_name', 'Unknown')}, "
    ...
    if args.verbose:
        ...
        if test_runs:
            ...
            for i, run in enumerate(test_runs[:3]):  # Show top 3
                print(
                    f"  - {run.get('run_id', 'Unknown')}: Model: {run.get('model_name', 'Unknown')}, "
        ...
        gen_tasks = get_component_specific_data(
            args.base_url, args.api_key, "generator", "tasks?limit=5"
        )
        ...
        if gen_tasks:
            ...
            for i, task in enumerate(gen_tasks[:3]):  # Show top 3
                print(
                    f"  - {task.get('task_id', 'Unknown')}: Model: {task.get('model_name', 'Unknown')}, "
