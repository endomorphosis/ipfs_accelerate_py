...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="WebGPU/WebNN Resource Pool Database Integration Example"
    )
    ...
    parser.add_argument("--days", type=int, default=30, help="Number of days to include in reports")
    parser.add_argument("--model", type=str, help="Specific model to analyze")
    parser.add_argument("--browser", type=str, help="Specific browser to analyze")
...
async def run_example(args):
    ...
    try:
        ...
        vision_conn_id, vision_conn = await pool.get_connection(
            model_type="vision", model_name="vit-base", platform="webgpu", browser="chrome"
        )
        ...
        audio_conn_id, audio_conn = await pool.get_connection(
            model_type="audio", model_name="whisper-tiny", platform="webgpu", browser="firefox"
        )
        ...
        report_filename = (
            f"performance_report{model_part}{browser_part}_{timestamp}.{args.report_format}"
        )
        ...
        print(
            f"  - Database integration: {stats.get('database_integration', {}).get('enabled', False)}"
        )
        print(
            f"  - Database path: {stats.get('database_integration', {}).get('db_path', 'unknown')}"
        )
