...
class KitchenSinkUITester:
    ...
    async def capture_console_logs(self, page: Page):
        ...
        def handle_page_error(error):
            error_entry = {"message": str(error), "timestamp": time.time()}
    ...
    def run_basic_api_tests(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.server_url}/api/inference/generate", json=test_data, timeout=10
            )
        ...
        for test_name, result in self.test_results.items():
            if not result.get("success", False):
                recommendations.append(
                    f"Fix {test_name} functionality - {result.get('error', 'Unknown error')}"
                )
