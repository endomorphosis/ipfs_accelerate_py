...
class TestFileMigrator:
    ...
    def _determine_target_path(self) -> str:
        ...
        if relative_path in self.migration_plan.get("migration_targets", {}):
            return os.path.join(
                REFACTORED_DIR, self.migration_plan["migration_targets"][relative_path]
            )
    ...
    def migrate(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                tree = None
                print(
                    f"Warning: Syntax error in {self.source_path}: {str(e)} - Will copy file without transformation"
                )
    ...
    def _transform_code(self, source_code: str, tree: ast.AST) -> str:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if node.name.startswith("Test") or any(
                    base.id == "TestCase" for base in node.bases if isinstance(base, ast.Name)
                ):
        ...
        migration_note = (
            f'"""Migrated to refactored test suite on {datetime.now().strftime("%Y-%m-%d")}\n\n"""'
        )
...
def find_test_files(directory: str, pattern: str = "**/test_*.py") -> List[str]:
    ...
    return [
        f for f in glob.glob(os.path.join(directory, pattern), recursive=True) if os.path.isfile(f)
    ]


def migrate_files(
    file_paths: List[str], migration_plan: Dict[str, Any], dry_run: bool = False
) -> Dict[str, Any]:
    """
    ...
    for source_path in file_paths:
        ...
        if dry_run:
            print(
                f"Would migrate {source_path} to {migrator.target_path} (category: {migrator.category}, base class: {migrator.base_class})"
            )
...
def generate_migration_report(results: Dict[str, Any], output_path: str) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Summary\n\n")
        f.write(
            f"- Total files processed: {len(results['success']) + len(results['failure']) + len(results['skipped'])}\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--plan",
        type=str,
        default="refactored_test_suite/migration_plan.json",
        help="Path to the migration plan JSON file",
    )
    parser.add_argument("--files", type=str, nargs="+", help="Specific test files to migrate")
    parser.add_argument("--dir", type=str, default=".", help="Directory to search for test files")
    parser.add_argument(
        "--pattern", type=str, default="**/test_*.py", help="Glob pattern for finding test files"
    )
    parser.add_argument(
        "--limit", type=int, default=0, help="Limit the number of files to migrate (0 for no limit)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--report",
        type=str,
        default="refactored_test_suite/migration_report.md",
