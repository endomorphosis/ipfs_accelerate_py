...
def test_export_with_archive(model_name, hardware_platform, output_dir, create_archive=True):
    ...
    try:
        ...
        result = exporter.export_optimization(
            model_name=model_name, hardware_platform=hardware_platform, output_format="all"
        )
        ...
        print(
            f"Exported {result['recommendations_exported']} optimization(s) to {result['base_directory']}"
        )
...
def test_batch_export_with_archive(output_dir, create_archive=True):
    ...
    try:
        ...
        result = exporter.export_batch_optimizations(
            recommendations_report=report, output_dir=output_dir, output_format="all"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./test_optimization_exports", help="Output directory"
    )
    ...
    if args.test_batch:
        ...
        success = test_batch_export_with_archive(
            output_dir=args.output_dir, create_archive=not args.no_archive
        )
    else:
        # Test single export
