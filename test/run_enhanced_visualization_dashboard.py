...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run the enhanced visualization dashboard with regression detection"
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="./visualizations/dashboard",
        help="Output directory for visualizations",
    )
    parser.add_argument(
        "--theme", default="dark", choices=["light", "dark"], help="Dashboard theme"
    )
    ...
    try:
        ...
        try:
            ...
            if not args.no_regression:
                logger.warning(
                    "Enhanced regression visualization module not found. Using basic visualization."
                )
        ...
        logger.error(
            "Make sure the enhanced_visualization_dashboard.py file is in the correct path."
        )
        logger.error(
            "Required dependencies: dash dash-bootstrap-components plotly pandas scipy duckdb aiohttp"
        )
        sys.exit(1)
