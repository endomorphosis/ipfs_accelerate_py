...
PHI_MODELS_REGISTRY = {
    "phi": {
        "description": "PHI small model",
        "class": "PHILMHeadModel",
    },
    "phi-medium": {
        "description": "PHI medium model",
        "class": "PHILMHeadModel",
    },
    "distilphi": {
        "description": "DistilPHI model",
        "class": "PHILMHeadModel",
    }
}

class TestPhiModels:
    """Base test class for all PHI-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "phi"
        
        # Verify model exists in registry
        if self.model_id not in PHI_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = PHI_MODELS_REGISTRY["phi"]
        else:
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            if tokenizer.pad_token is None:
                ...
                logger.info("Set pad_token to eos_token for PHI tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for PHI padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for PHI tokenizer")
            ...
            model_class = None
            if self.class_name == "PHILMHeadModel":
                model_class = transformers.PHILMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_phi_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available PHI models in the registry."""
    return list(PHI_MODELS_REGISTRY.keys())
    
def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered PHI models."""
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_phi_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test PHI-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable PHI-family models:")
        for model in models:
            info = PHI_MODELS_REGISTRY[model]
