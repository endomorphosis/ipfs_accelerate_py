...
class WorkflowClient:
    ...
    def run_workflow(self, model_name: str, hardware: list = ["cpu"]) -> Dict[str, Any]:
        ...
        if not generation_result.get("success", False):
            self.logger.error(
                f"Model generation failed: {generation_result.get('error', 'Unknown error')}"
            )
            return {
                "success": False,
        ...
        if not benchmark_result.get("success", False):
            self.logger.error(
                f"Benchmarks failed: {benchmark_result.get('error', 'Unknown error')}"
            )
            return {
                "success": False,
    ...
    def generate_model(self, model_name: str, hardware: list) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {"model_name": model_name, "hardware": hardware, "force": True}
            ...
            while True:
                ...
                self.logger.info(
                    f"Generation progress: {status['progress']:.1%} - {status['current_step']}"
                )
    ...
    def run_benchmarks(self, model_path: str, hardware: list) -> Dict[str, Any]:
        ...
        try:
            ...
            while True:
                ...
                self.logger.info(
                    f"Benchmark progress: {status['progress']:.1%} - {status['current_step']}"
                )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="bert-base-uncased", help="Model name to process"
    )
    parser.add_argument(
        "--hardware", type=str, default="cpu", help="Hardware to target (comma separated)"
    )
    parser.add_argument(
        "--test-api", type=str, default="http://localhost:8000", help="Test API URL"
    )
    parser.add_argument(
        "--generator-api", type=str, default="http://localhost:8001", help="Generator API URL"
    )
    parser.add_argument(
        "--benchmark-api", type=str, default="http://localhost:8002", help="Benchmark API URL"
    )
    parser.add_argument(
        "--gateway", type=str, default="http://localhost:8080", help="Gateway API URL"
    )
