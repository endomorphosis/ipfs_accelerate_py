...
class BasicResourcePoolFaultTester:
    ...
    async def test_scenario(self, scenario: str) -> Dict[str, Any]:
        ...
        try:
            ...
            phase2_result = {
                ...
                "message": "Initial inference successful"
                if "error" not in inference_result
                else inference_result.get("error"),
            }
            ...
            phase4_result = {
                ...
                "message": "Inference with fault succeeded (fault tolerance working)"
                if fault_inference_success
                else f"Inference with fault failed as expected: {fault_inference_result.get('error', 'Unknown error')}",
            }
            ...
            phase6_result = {
                ...
                "message": "Post-recovery inference successful"
                if recovery_inference_success
                else f"Post-recovery inference failed: {recovery_inference_result.get('error', 'Unknown error')}",
            }
            ...
            scenario_results["overall_success"] = (
                ...
                and phase2_result["success"]
                and phase3_result["success"]
                and (
                    phase4_result["success"] or not phase4_result["success"]
                )  # Either outcome is acceptable for fault inference
                and phase5_result["success"]
                and phase6_result["success"]
            )
    ...
    async def run_test(self, scenario: str = None) -> Dict[str, Any]:
        ...
        successful_scenarios = [
            s for s, r in self.results["scenarios"].items() if r.get("overall_success", False)
        ]
        ...
        self.results["summary"] = {
            ...
            "success_rate": len(successful_scenarios) / total_scenarios
        ...
        # Print summary
        logger.info(
            f"Test summary: {len(successful_scenarios)}/{total_scenarios} scenarios successful"
        )
        logger.info(
            f"Overall test result: {'SUCCESS' if self.results['summary']['overall_success'] else 'FAILURE'}"
        )
...
def format_results_for_display(results: Dict[str, Any]) -> str:
    ...
    for scenario, data in results["scenarios"].items():
        ...
        for phase, phase_data in data.get("phases", {}).items():
            ...
            output.append(
                f"  {success} {phase.ljust(20)} - {duration.ljust(8)} - {phase_data.get('message', '')}"
            )
    ...
    if "summary" in results:
        ...
        output.append(
            f"Scenarios: {results['summary']['successful_scenarios']}/{results['summary']['total_scenarios']} successful"
        )
...
async def main():
    ...
    parser.add_argument(
        "--scenario",
        type=str,
        choices=["connection_lost", "component_failure", "browser_crash", "multiple_failures"],
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./fault_tolerance_basic_test_results",
    ...
    tester = BasicResourcePoolFaultTester(
        model_name=args.model, recovery_strategy=args.recovery_strategy, output_dir=args.output_dir
    )
