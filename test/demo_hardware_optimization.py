...
def demonstrate_optimization_workflow(
    ...
    try:
        ...
        bridge = BenchmarkPredictivePerformanceBridge(
            benchmark_db_path=benchmark_db_path, predictive_api_url=api_url, api_key=api_key
        )
    ...
    analysis = opt_client.analyze_performance(model_name=model_name, hardware_platform=hardware)
    ...
    if "error" in analysis:
        ...
        if model_name == "bert-base-uncased":
            ...
            analysis = opt_client.analyze_performance(
                model_name=model_name, hardware_platform=hardware
            )
    ...
    if recs:
        ...
        print_table(
            rows, ["Recommendation", "Description", "Throughput ↑", "Latency ↓", "Confidence"]
        )
    ...
    print(
        f"Generating report for {len(model_names)} models and {len(hardware_platforms)} hardware platforms..."
    )
    report = opt_client.generate_report(
        model_names=model_names, hardware_platforms=hardware_platforms
    )
        ...
        if top_recs:
            ...
            for i, rec in enumerate(top_recs[:5]):
                ...
                throughput_imp = (
                    ...
                    .get("expected_improvements", {})
                    .get("throughput_improvement", 0)
                )

                rows.append([i + 1, model, hw, name, f"{throughput_imp * 100:.1f}%"])
...
def main():
    ...
    parser.add_argument("--model", type=str, default="bert-base-uncased", help="Model to analyze")
    parser.add_argument("--hardware", type=str, default="cuda", help="Hardware platform to analyze")
    parser.add_argument(
        "--sample-data", action="store_true", help="Generate sample data before running demo"
    )
