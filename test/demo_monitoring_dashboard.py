...
try:
    ...
    logger.error(
        "Could not import ValidationVisualizerDBConnector. Make sure test_basic_dashboard_integration.py exists."
    )
    sys.exit(1)
...
def create_dashboard_demo():
    ...
    result = connector.create_comprehensive_monitoring_dashboard(
        ...
        include_panels=["hardware_heatmap", "mape_comparison", "simulation_vs_hardware"],
    )
...
def create_monitoring_demo():
    ...
    logger.info(
        f"  Alert Thresholds: {monitor_result.get('monitoring_config', {}).get('alert_thresholds')}"
    )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["panel", "dashboard", "monitor", "all"], default="all", help="Demo mode"
    )
