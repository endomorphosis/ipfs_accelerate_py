...
class TestExportVisualization(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if HAS_ADVANCED_VISUALIZATION and cls.has_db:
            try:
                cls.viz = AdvancedVisualizationSystem(db_api=cls.db_api, output_dir=cls.temp_dir)
    ...
    def test_export_simple_figure(self):
        ...
        self.assertTrue(
            os.path.exists(exported_html_path),
            f"Exported HTML file not found: {exported_html_path}",
        )
        ...
        try:
            ...
            self.assertTrue(
                os.path.exists(exported_png_path),
                f"Exported PNG file not found: {exported_png_path}",
            )
        except Exception as e:
    ...
    def test_export_data(self):
        ...
        # Create a DataFrame
        df = pd.DataFrame(
            {"model": ["bert", "vit", "t5"], "throughput": [100, 200, 150], "latency": [10, 5, 8]}
        )
        ...
        # Verify the file exists
        self.assertTrue(
            os.path.exists(exported_csv_path), f"Exported CSV file not found: {exported_csv_path}"
        )
        ...
        # Verify the file exists
        self.assertTrue(
            os.path.exists(exported_json_path),
            f"Exported JSON file not found: {exported_json_path}",
        )
    ...
    def test_export_3d_visualization(self):
        ...
        if not self.db_api:
            ...
            dummy_result = {"component_type": "3d", "figure": fig, "title": "Test 3D Visualization"}
            ...
            try:
                ...
                exports = self.viz.export_3d_visualization(
                    visualization_data=result, formats=["html", "png"], visualization_id="test_3d"
                )
            except Exception as e:
        ...
        if "html" in exports:
            self.assertTrue(
                os.path.exists(exports["html"]), f"Exported HTML file not found: {exports['html']}"
            )

        # Check PNG export (may fail if kaleido isn't installed)
        if "png" in exports:
            self.assertTrue(
                os.path.exists(exports["png"]), f"Exported PNG file not found: {exports['png']}"
            )
