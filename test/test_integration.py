    ...
    def test_detection_performance_under_load(self):
        ...
        def detect_hardware():
            try:
                ...
                results.append(
                    {"duration": end_time - start_time, "cpu_available": result.get("cpu", False)}
                )
            except Exception as e:
                errors.append(e)
    ...
    def test_environment_variable_integration(self):
        ...
        for env_vars, config_name in test_configs:
            with patch.dict(os.environ, env_vars, clear=False):
                ...
                self.assertIsInstance(result, dict)
                self.assertTrue(
                    result.get("cpu", False), f"CPU should be available in {config_name}"
                )
...
def run_integration_tests():
    ...
    print("\n" + "=" * 70)
    print(
        f"INTEGRATION TEST SUMMARY: {passed_tests}/{total_tests} tests passed ({failed_tests} failed)"
    )
