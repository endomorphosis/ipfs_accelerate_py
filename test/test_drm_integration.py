...
class MockCloudProvider:
    ...
    def create_worker(self, config):
        ...
        return {"worker_id": worker_id, "status": "running", "provider": self.name}
...
def test_drm_integration():
    ...
    if scaling_decision.action == "scale_down":
        logger.info(
            f"Scaling down {len(scaling_decision.worker_ids)} workers: {scaling_decision.worker_ids}"
        )
