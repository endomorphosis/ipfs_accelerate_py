...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("validate_import_paths.log")],
)
...
def setup_args():
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix import issues automatically"
    )
...
def fix_import_path(import_path: str, file_path: str) -> str:
    ...
    for key_dir in key_dirs:
        ...
        for file in os.listdir(key_dir_path):
            file_base, ext = os.path.splitext(file)
            if ext in [".ts", ".tsx"] and (
                file_base.lower() == module_name.lower()
                or file_base.lower() == module_name.lower() + "_fixed"
                or file_base.lower() == "test_" + module_name.lower()
            ):
            ...
            if import_path.startswith("./") or import_path.startswith("../"):
                ...
                try:
                    ...
                    if os.path.isdir(target_path):
                        for ext in [".ts", ".tsx", ".js", ".jsx"]:
                            if os.path.exists(os.path.join(target_path, "index" + ext)):
                                import_graph[file_path].append(
                                    os.path.join(target_path, "index" + ext)
                                )
                                break
                except Exception as e:
                    if Config.VERBOSE:
...
def generate_report():
    ...
    report_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "import_validation_report.md"
    )
