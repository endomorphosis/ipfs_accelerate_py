...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("enhanced_ts_fixer.log")],
)
...
def parse_args():
    ...
    parser.add_argument(
        "--no-core-components", action="store_true", help="Skip core component fixes"
    )
...
def fix_imports(content: str) -> Tuple[str, int]:
    ...
    patterns = [
        ...
        (
            r'import\s+(?:{[^}]+}|\*\s+as\s+\w+|\w+)\s+from\s+[\'"]([^\'"]+)\.(py|ts|tsx)[\'"]',
            r'import \1 from "\3"',
        ),
...
def fix_interfaces(content: str) -> Tuple[str, int]:
    ...
    for interface_match in interfaces:
        ...
        fixed_body = re.sub(
            r"(\s+)(\w+)\s*:\s*([^;]+)(?!\s*;)$", r"\1\2: \3;", fixed_body, flags=re.MULTILINE
        )
        ...
        fixed_body = re.sub(
            r"(\w+\([^)]*\)(?:\s*:\s*[^;]+)?(?!\s*;))$", r"\1;", fixed_body, flags=re.MULTILINE
        )
...
def fix_classes(content: str) -> Tuple[str, int]:
    ...
    for class_match in classes:
        ...
        # Fix properties without types
        fixed_body = re.sub(
            r"(\s+)(private|protected|public)?\s*(\w+)(?:\s*=|\s*;)(?![:\(])",
            r"\1\2 \3: any",
            fixed_body,
        )
        ...
        fixed_body = re.sub(
            r"constructor\s*\(([^:\)]+)(?=[,\)])", r"constructor(\1: any", fixed_body
        )
        ...
        fixed_body = re.sub(
            r"(\s+)(private|protected|public)?\s*async\s+(\w+)\s*\(([^)]*)(?!\):\s*)",
            r"\1\2 async \3(\4): Promise<any>",
            fixed_body,
        )

        # Fix methods returning Promise but not marked as async
        fixed_body = re.sub(
            r"(\s+)(private|protected|public)?\s*(\w+)\s*\(([^)]*)\)\s*:\s*Promise<([^>]+)>([^{]*){",
            r"\1\2 async \3(\4): Promise<\5>\6{",
            fixed_body,
        )
...
def fix_function_returns(content: str) -> Tuple[str, int]:
    ...
    function_fixed = re.sub(
        async_arrow_pattern, r"const \1 = async \2: Promise<any> =>", function_fixed
    )
        ...
        try:
            ...
            if "compilerOptions" in existing_config:
                ...
                if "lib" not in existing_config["compilerOptions"]:
                    existing_config["compilerOptions"]["lib"] = [
                        "dom",
                        "dom.iterable",
                        "esnext",
                        "webworker",
        ...
        try:
            ...
            if "devDependencies" not in existing_pkg:
                ...
                if "typescript" not in existing_pkg["devDependencies"]:
                    existing_pkg["devDependencies"]["typescript"] = package_json["devDependencies"][
                        "typescript"
                    ]
                ...
                for type_def in ["@types/node", "@types/react", "@types/jest"]:
                    if type_def not in existing_pkg["devDependencies"]:
                        existing_pkg["devDependencies"][type_def] = package_json["devDependencies"][
                            type_def
                        ]
...
def process_all_files():
    ...
    for i, file_path in enumerate(ts_files):
        ...
        if any(comp_path in file_path for comp_path in core_paths) and Config.FIX_CORE_COMPONENTS:
            logger.info(
                f"Skipping core component (handled separately): {os.path.relpath(file_path, Config.TARGET_DIR)}"
            )
            continue
        ...
        if Config.VERBOSE or (i + 1) % 25 == 0:
            logger.info(
                f"Processing file {i + 1}/{len(ts_files)}: {os.path.relpath(file_path, Config.TARGET_DIR)}"
            )
