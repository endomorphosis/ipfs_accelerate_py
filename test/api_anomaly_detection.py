...
class AnomalyDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Anomaly detector initialized with algorithm: {algorithm.value}, sensitivity: {sensitivity}, notifications: {notification_enabled}"
        )

    def update_baseline(
        self, api: str, metric_type: str, historical_data: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        baseline_data = [point for point in sorted_data if point["timestamp"] >= baseline_cutoff]
        ...
        logger.debug(
            f"Updated baseline for {api} {metric_type}: mean={baseline['mean']:.4f}, std_dev={baseline['std_dev']:.4f}"
        )
        ...
        if len(short_term_slopes) >= 2:
            ...
            if normalized_change > threshold:
                ...
                anomaly = {
                    ...
                    "metric_type": metric_type,
                    "value": values[-1],
                    "expected_value": values[-self.window_size - 1]
                    + (prev_slope * (timestamps[-1] - timestamps[-self.window_size - 1])),
    ...
    def _determine_anomaly_type(
        self, metric_type: str, value: float, expected: float
    ) -> AnomalyType:
        """
    ...
    def get_recent_anomalies(
        self, hours: int = 24, min_severity: AnomalySeverity = AnomalySeverity.LOW
    ) -> List[Dict[str, Any]]:
        """
        ...
        recent_anomalies = [
            ...
            and severity_order.get(anomaly["severity"], 0) >= min_severity_level
        ]
