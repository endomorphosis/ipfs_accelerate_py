...
def detect_available_hardware() -> Dict[str, bool]:
    ...
    logger.info(
        f"Available hardware: {[hw for hw, available in available_hardware.items() if available]}"
    )
...
def get_device_for_hardware(hardware: str) -> torch.device:
    ...
    if hardware == "cuda" and torch.cuda.is_available():
        ...
        logger.warning(
            f"Hardware {hardware} not directly supported in PyTorch, falling back to CPU"
        )
        device = torch.device("cpu")
    ...
    try:
        ...
        for batch_size in batch_sizes:
            ...
            if hasattr(outputs, "items"):
                result["output_shape"] = {
                    k: str(v.shape) for k, v in outputs.items() if hasattr(v, "shape")
                }

            results.append(result)

            logger.info(
                f"Batch size {batch_size}: Latency = {latency:.2f} ms, Throughput = {throughput:.2f} items/sec"
            )
...
def benchmark_t5(
    ...
    try:
        ...
        for batch_size in batch_sizes:
            ...
            logger.info(
                f"Batch size {batch_size} forward pass: Latency = {forward_latency:.2f} ms, "
                f"Throughput = {forward_throughput:.2f} items/sec"
            )
            logger.info(
                f"Batch size {batch_size} generation: Latency = {generation_latency:.2f} ms, "
...
def benchmark_vit(
    ...
    try:
        ...
        for batch_size in batch_sizes:
            ...
            # Prepare input (replicate for batch size)
            batch_inputs = {
                k: v.repeat(batch_size, 1, 1, 1) if v.dim() == 4 else v.repeat(batch_size, 1)
    ...
    try:
        ...
        if hardware == "openvino":
            ...
            try:
                ...
                # Load the model with OpenVINO backend
                model = OVModelForSpeechSeq2Seq.from_pretrained(model_name, export=True)
                logger.info("Model loaded with OpenVINO backend")
        ...
        if hasattr(model, "generation_config") and hasattr(
            ...
            decoder_input_ids = (
                torch.tensor(model.generation_config.forced_decoder_ids).unsqueeze(0).to(device)
            )
        else:
            # Default for English
        ...
        for batch_size in batch_sizes:
            ...
            batch_inputs = {
                k: v.repeat(batch_size, *([1] * (v.dim() - 1))) for k, v in inputs.items()
            }
    ...
    with open(report_file, "w") as f:
            ...
            for hardware, hw_result in model_results["hardware_results"].items():
                ...
                if model_type in ["bert", "vit", "whisper"]:
                    ...
                    if results:
                        ...
                        f.write(
                            f"| {model_name} | {hardware} | {batch_size} | {latency:.2f} | {throughput:.2f} |\n"
                        )
                    ...
                    if results:
                        ...
                        forward_throughput = batch_result.get(
                            "forward_throughput_items_per_sec", "N/A"
                        )

                        f.write(
                            f"| {model_name} | {hardware} | {batch_size} | {forward_latency:.2f} | {forward_throughput:.2f} |\n"
                        )
        ...
        for model_name, model_results in all_results.items():
            for batch_size in batch_sizes_to_show:
                ...
                for hardware in hardware_backends:
                    hw_result = model_results["hardware_results"].get(
                        hardware, {"error": "Not tested"}
                    )
                    ...
                    batch_result = next(
                        (r for r in results if r.get("batch_size") == batch_size), None
                    )
            ...
            for hardware, hw_result in model_results["hardware_results"].items():
                ...
                if model_type in ["bert", "vit", "whisper"]:
                    ...
                    for batch_result in hw_result.get("results", []):
                        ...
                        f.write(
                            f"| {batch_size} | {latency:.2f} | {throughput:.2f} | {memory:.2f} |\n"
                        )

                elif model_type == "t5":
                    f.write(
                        "| Batch Size | Forward Latency (ms) | Forward Throughput (items/s) | Generation Latency (ms) | Generation Throughput (items/s) |\n"
                    )
                    f.write(
                        "|------------|----------------------|----------------------------|-------------------------|-------------------------------|\n"
                    )
                    ...
                    for batch_result in hw_result.get("results", []):
                        ...
                        generation_throughput = batch_result.get(
                            "generation_throughput_items_per_sec", "N/A"
                        )

                        f.write(
                            f"| {batch_size} | {forward_latency:.2f} | {forward_throughput:.2f} | {generation_latency:.2f} | {generation_throughput:.2f} |\n"
                        )
...
def generate_csv_report(all_results: Dict[str, Dict[str, Any]], output_dir: Path) -> Path:
    ...
    with open(report_file, "w", newline="") as f:
        ...
        if "bert" in model_types or "vit" in model_types or "whisper" in model_types:
            header.extend(
                ["Latency (ms)", "Latency Std (ms)", "Throughput (items/s)", "Memory (MB)"]
            )
        ...
        if "t5" in model_types:
            header.extend(
                [
                    "Forward Latency (ms)",
                    "Forward Latency Std (ms)",
                    "Forward Throughput (items/s)",
                    "Forward Memory (MB)",
                    "Generation Latency (ms)",
                    "Generation Latency Std (ms)",
                    "Generation Throughput (items/s)",
                    "Generation Memory (MB)",
                ]
            )
            ...
            for hardware, hw_result in model_results["hardware_results"].items():
                ...
                for batch_result in results:
                    ...
                    if model_type in ["bert", "vit", "whisper"]:
                        ...
                        if (
                            "bert" in model_types
                            or "vit" in model_types
                            or "whisper" in model_types
                        ):
...
def parse_args():
    ...
    parser.add_argument(
        "--models", type=str, nargs="+", help="Specific models to benchmark (overrides model-set)"
    )
    ...
    # Batch sizes
    parser.add_argument(
        "--batch-sizes", type=int, nargs="+", default=[1, 4, 16], help="Batch sizes to test"
    )
...
def main():
    ...
    logger.info(
        f"Available hardware platforms: {[hw for hw, available in available_hardware.items() if available]}"
    )
