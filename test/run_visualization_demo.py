...
from ipfs_accelerate_py.predictive_performance.visualization import (
...
def generate_sample_data():
    ...
    print(
        f"Generating data for {len(DEFAULT_TEST_MODELS)} models, {len(DEFAULT_TEST_HARDWARE)} hardware platforms, {len(DEFAULT_TEST_BATCH_SIZES)} batch sizes, and {len(DEFAULT_TEST_PRECISIONS)} precision formats..."
    )
...
def run_visualization_demo(data_path=None, advanced_vis=False):
    ...
    vis = AdvancedVisualization(output_dir=str(vis_dir), interactive=True)
    ...
    metrics = (
        DEFAULT_METRICS + ["power_consumption"]
    ...
    report_title = (
        "Predictive Performance System - Advanced Visualization Demo"
        if advanced_vis
        else "Predictive Performance System - Visualization Demo"
    )
                            ...
                            for metric in DEFAULT_METRICS:
                                if metric in pred_values:
                                    ...
                                    if metric in uncertainties:
                                        ...
                                        pred_record[f"{metric}_lower_bound"] = uncertainty.get(
                                            "lower_bound", pred_values[metric] * 0.8
                                        )
                                        pred_record[f"{metric}_upper_bound"] = uncertainty.get(
                                            "upper_bound", pred_values[metric] * 1.2
                                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Visualization Demo for the Predictive Performance System"
    )
    ...
    parser.add_argument(
        "--advanced-vis", action="store_true", help="Enable advanced visualization features"
    )
    ...
    if args.data:
        ...
        visualization_files, report_path = run_visualization_demo(
            args.data, advanced_vis=args.advanced_vis
        )
    elif args.generate:
        # Generate predictions and visualize them
        visualization_files, report_path = generate_predictions_for_visualization(
            advanced_vis=args.advanced_vis
        )
    else:
