...
try:
    ...
    class BrowserPerformanceOptimizer:
        ...
        def get_optimized_configuration(
            self, model_type, model_name=None, available_browsers=None, user_preferences=None
        ):
...
class MockBrowserHistory:
    ...
    def __init__(
        self, capability_scores=None, recommendations=None, performance_recommendations=None
    ):
...
class TestBrowserPerformanceOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_capability_scores = {
            "firefox": {
                "audio": {"score": 90.0, "confidence": 0.8, "sample_size": 20},
                "vision": {"score": 70.0, "confidence": 0.6, "sample_size": 15},
                "text_embedding": {"score": 60.0, "confidence": 0.5, "sample_size": 10},
            },
            "chrome": {
                "audio": {"score": 70.0, "confidence": 0.7, "sample_size": 18},
                "vision": {"score": 85.0, "confidence": 0.8, "sample_size": 25},
                "text_embedding": {"score": 75.0, "confidence": 0.7, "sample_size": 15},
            },
            "edge": {
                "audio": {"score": 65.0, "confidence": 0.6, "sample_size": 12},
                "vision": {"score": 70.0, "confidence": 0.7, "sample_size": 14},
                "text_embedding": {"score": 85.0, "confidence": 0.9, "sample_size": 30},
            },
        }
    ...
    def test_get_best_browser_for_model(self):
        ...
        browser, confidence, reason = self.optimizer.get_best_browser_for_model("audio", ["chrome"])
        self.assertEqual(browser, "chrome")  # Only option

        # Test with empty list
        browser, confidence, reason = self.optimizer.get_best_browser_for_model("audio", [])
    ...
    def test_get_optimization_parameters(self):
        ...
        # Test unknown model type
        params = self.optimizer.get_optimization_parameters("unknown", OptimizationPriority.LATENCY)
    ...
    def test_get_optimized_configuration(self):
        ...
        self.assertTrue(
            "audio_thread_priority" in config.parameters
        )  # Should have audio optimizations
    ...
    def test_apply_runtime_optimizations(self):
        ...
        optimized = self.optimizer.apply_runtime_optimizations(audio_model, "firefox", context)
        self.assertEqual(optimized["batch_size"], 2)  # Should keep user setting
        self.assertTrue(
            optimized["compute_shader_optimization"]
        )  # Should add Firefox audio optimization

        # Test Chrome vision optimizations
        context = {}
        optimized = self.optimizer.apply_runtime_optimizations(vision_model, "chrome", context)
        self.assertTrue(
            optimized["parallel_compute_pipelines"]
        )  # Should add Chrome vision optimization
        self.assertTrue(
            optimized["vision_optimized_shaders"]
        )  # Should add Chrome vision optimization

        # Test Edge text optimizations
        context = {"priority_list": ["webnn", "cpu"]}
        optimized = self.optimizer.apply_runtime_optimizations(text_model, "edge", context)
