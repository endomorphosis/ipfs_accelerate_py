...
class RealWorldModelTester:
    ...
    def _estimate_inference_time(self, model_type, hardware):
        ...
        hardware_multipliers = {"cpu": 1.0, "cuda": 0.2, "mps": 0.3, "webnn": 0.7, "webgpu": 0.8}
    ...
    def run_comprehensive_model_tests(self, use_actual_loading=False):
        ...
        for model_info in self.TEST_MODELS:
            try:
                ...
                error_result = {"model_name": model_info["name"], "status": f"test_error: {str(e)}"}
        ...
        def test_actual_model_loading_if_available(self, model_tester):
            ...
            if transformers and torch:
                ...
                assert result["status"] in [
                    "actual_success",
                    "load_failed: permission denied",
                    "load_failed: network error",
                ]
            else:
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="Run quick compatibility tests only")
