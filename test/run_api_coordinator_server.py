...
class APICoordinatorServer:
    ...
    def __init__(
        ...
        self.notification_manager = NotificationManager(config=notification_config or {})
    ...
    def unregister_worker(self, worker_id: str) -> bool:
        ...
        with self.stats_lock:
            if worker_id in self.worker_api_mapping:
                ...
                for provider in api_providers:
                    if (
                        provider in self.provider_registry
                        and worker_id in self.provider_registry[provider]
                    ):
    ...
    def process_result(self, task_id: str, result: Dict[str, Any]) -> None:
        ...
        if self.enable_anomaly_detection and self.anomaly_detector and provider and test_type:
            ...
            try:
                ...
                if anomalies:
                    logger.warning(
                        f"Anomalies detected in task {task_id}: {len(anomalies)} issues found"
                    )
    ...
    def create_api_test(
        ...
        task_params = {"api_type": api_type, "test_type": test_type, "test_parameters": parameters}

        # Create task
        task_id = self.coordinator.create_task(
            test_path=f"api_test_{api_type}_{test_type}", parameters=task_params, priority=priority
        )
    ...
    def create_api_comparison(
        ...
        for api_type in api_types:
            task_id = self.create_api_test(
                api_type=api_type, test_type=test_type, parameters=parameters, priority=priority
            )
    ...
    def get_provider_stats(self, provider: str) -> Dict[str, Any]:
        ...
        for task in provider_tasks:
            ...
            if test_type == "latency":
                ...
                if (
                    "cost_efficiency_metrics" in task
                    and "tokens_per_dollar" in task["cost_efficiency_metrics"]
                ):
            ...
            try:
                ...
                if self.enable_predictive_analytics and self.time_series_predictor:
                    ...
                    if len(latency_data) >= 5:
                        ...
                        if (
                            prediction.get("trend") == "increasing"
                            and prediction.get("confidence", 0) > 0.7
                        ):
                            # Send notification
                            self.notification_manager.send_notification(
                    ...
                    if len(throughput_data) >= 5:
                        ...
                        # Check if prediction indicates deteriorating performance
                        if (
                            prediction.get("trend") == "decreasing"
                            and prediction.get("confidence", 0) > 0.7
                        ):
                            # Send notification
                            self.notification_manager.send_notification(
...
def main():
    ...
    # Feature flags
    parser.add_argument(
        "--disable-anomaly-detection", action="store_true", help="Disable anomaly detection"
    )
    parser.add_argument(
        "--disable-predictive-analytics", action="store_true", help="Disable predictive analytics"
    )
    parser.add_argument(
        "--disable-dashboard", action="store_true", help="Disable monitoring dashboard"
    )
    ...
    try:
        ...
        if not args.disable_dashboard:
            logger.info(
                f"Monitoring dashboard available at http://{args.host}:{args.dashboard_port}"
            )
