...
def check_servers(api_url, timeout=5):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"Predictive Performance API returned status code {response.status_code}"
            )
    except Exception as e:
...
def main():
    ...
    # Server options
    parser.add_argument(
        "--api-url", type=str, default="http://localhost:8080", help="URL of the Unified API Server"
    )
    parser.add_argument("--benchmark-db", type=str, help="Path to the benchmark database file")
    parser.add_argument("--config", type=str, help="Path to configuration file")

    # Operation modes
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--service", action="store_true", help="Run as a continuous service")
    group.add_argument("--sync", action="store_true", help="Run a single synchronization cycle")
    group.add_argument("--report", action="store_true", help="Generate a synchronization report")
    group.add_argument(
    ...
    parser.add_argument(
        "--model", type=str, help="Synchronize results for a specific model (--sync mode)"
    )
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days to look back (--sync mode)"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument("--output", type=str, help="Path to write report JSON (--report mode)")
    parser.add_argument("--log-file", type=str, help="Path to log file (--service mode)")
    ...
    if args.service:
        ...
        service = BridgeService(config_path=args.config, log_file=args.log_file)
        service.start()
        return 0
    ...
    # Single synchronization cycle
        ...
        bridge = BenchmarkPredictivePerformanceBridge(
            benchmark_db_path=benchmark_db, predictive_api_url=args.api_url
        )
        ...
        try:
            ...
            if args.model:
                ...
                result = bridge.sync_by_model(
                    model_name=args.model, days=args.days, limit=args.limit
                )
            else:
            ...
            print(
                f"Total: {result['total']}, Synced: {result['synced']}, Failed: {result['failed']}"
            )
    ...
    # Generate report
        ...
        try:
            ...
            for model in report["model_coverage"][:5]:
                print(
                    f"  {model['model_name']}: {model['total_results']} results, {model['synced_results']} synced ({model['sync_percentage']}%)"
                )
            ...
            for hw in report["hardware_coverage"]:
                print(
                    f"  {hw['hardware_type']}: {hw['total_results']} results, {hw['synced_results']} synced ({hw['sync_percentage']}%)"
                )
