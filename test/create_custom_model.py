...
def generate_model_report(
    config: Dict[str, Any],
    files: Dict[str, str],
    test_result: Tuple[bool, str, Optional[str]],
    output_file: str,
) -> None:
    """
    ...
    with open(output_file, "w") as f:
        ...
        f.write("```\n\n")
        f.write(
            "3. Add the model to the appropriate architecture in `ADDITIONAL_ARCHITECTURE_MAPPINGS`:\n\n"
        )
        f.write("```python\n")
...
def parse_args():
    ...
    parser.add_argument(
        "--architecture", choices=ARCHITECTURE_TYPES.keys(), help="Architecture type"
    )
...
def main():
    ...
    # Determine mode
    interactive = args.interactive or not (
        args.model_name and args.architecture and args.task and args.model_class
    )
