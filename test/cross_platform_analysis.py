...
class CrossPlatformAnalyzer:
    ...
    def get_model_performance(
        ...
        try:
            ...
            return {"android": android_data, "ios": ios_data}
    ...
    def analyze_cross_platform_performance(
        ...
        android_metrics = {
            ...
            "avg_latency": np.mean(
                [c.get("android_latency_ms", 0) for c in comparisons if c.get("android_latency_ms")]
            ),
            "avg_throughput": np.mean(
                [c.get("android_throughput", 0) for c in comparisons if c.get("android_throughput")]
            ),
            "avg_battery_impact": np.mean(
                [
                    c.get("android_battery_impact", 0)
                    for c in comparisons
                    if c.get("android_battery_impact")
                ]
            ),
            "avg_temperature": np.mean(
                [
                    c.get("android_temperature", 0)
                    for c in comparisons
                    if c.get("android_temperature")
                ]
            ),
        }
        ...
        ios_metrics = {
            ...
            "avg_latency": np.mean(
                [c.get("ios_latency_ms", 0) for c in comparisons if c.get("ios_latency_ms")]
            ),
            "avg_throughput": np.mean(
                [c.get("ios_throughput", 0) for c in comparisons if c.get("ios_throughput")]
            ),
            "avg_battery_impact": np.mean(
                [c.get("ios_battery_impact", 0) for c in comparisons if c.get("ios_battery_impact")]
            ),
            "avg_temperature": np.mean(
                [c.get("ios_temperature", 0) for c in comparisons if c.get("ios_temperature")]
            ),
        }
        ...
        cross_platform_metrics = {
            "model_count": sum(
                1
                for c in comparisons
                if c.get("android_benchmark_count", 0) > 0 and c.get("ios_benchmark_count", 0) > 0
            ),
            "avg_throughput_ratio": np.mean(
                [
                    c.get("ios_android_throughput_ratio", 0)
                    for c in comparisons
                    if c.get("ios_android_throughput_ratio")
                ]
            ),
            "avg_latency_ratio": np.mean(
                [
                    c.get("android_ios_latency_ratio", 0)
                    for c in comparisons
                    if c.get("android_ios_latency_ratio")
                ]
            ),
            "avg_battery_ratio": np.mean(
                [
                    c.get("ios_android_battery_ratio", 0)
                    for c in comparisons
                    if c.get("ios_android_battery_ratio")
                ]
            ),
        }
    ...
    def _generate_recommendations(self, comparisons: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for comparison in comparisons:
            if (
                comparison.get("ios_android_throughput_ratio", 1) > 1.2
                ...
                comparison.get("ios_android_throughput_ratio", 1) < 0.8
                and comparison.get("android_benchmark_count", 0) > 0
                and comparison.get("ios_benchmark_count", 0) > 0
            ):
                family = comparison.get("model_family", "Unknown")
        ...
        for comparison in comparisons:
            if (
                comparison.get("android_benchmark_count", 0) > 0
                and comparison.get("ios_benchmark_count", 0) > 0
            ):
                android_battery = comparison.get("android_battery_impact", 0)
                ios_battery = comparison.get("ios_battery_impact", 0)
    ...
    def generate_visualization(
        ...
        try:
            ...
            ax2.text(
                0.02,
                0.95,
                "Green = iOS faster, Red = Android faster",
    ...
    def generate_comparison_report(
        self, model_name: Optional[str] = None, format: str = "markdown"
    ) -> Dict[str, str]:
        """
...
<body>
    ...
    <ul>
        <li><strong>Android Models</strong>: """
            + str(android_metrics.get("model_count", 0))
            + """</li>
        <li><strong>iOS Models</strong>: """
            + str(ios_metrics.get("model_count", 0))
            + """</li>
        <li><strong>Cross-Platform Models</strong>: """
            + str(cross_metrics.get("model_count", 0))
            + """</li>
    </ul>
    ...
    <table>
        ...
        <tr>
            ...
            <td>"""
            + f"{android_metrics.get('avg_throughput', 0):.2f}"
            + """</td>
            <td>"""
            + f"{ios_metrics.get('avg_throughput', 0):.2f}"
            + """</td>
            <td class="""
            + (
                "ios-faster"
                if cross_metrics.get("avg_throughput_ratio", 1) > 1
                else "android-faster"
            )
            + """>"""
            + f"{cross_metrics.get('avg_throughput_ratio', 0):.2f}x"
            + """</td>
        </tr>
        <tr>
            <td>Latency (ms)</td>
            <td>"""
            + f"{android_metrics.get('avg_latency', 0):.2f}"
            + """</td>
            <td>"""
            + f"{ios_metrics.get('avg_latency', 0):.2f}"
            + """</td>
            <td class="""
            + ("android-faster" if cross_metrics.get("avg_latency_ratio", 1) > 1 else "ios-faster")
            + """>"""
            + f"{1 / cross_metrics.get('avg_latency_ratio', 1):.2f}x"
            + """</td>
        </tr>
        <tr>
            <td>Battery Impact (%)</td>
            <td>"""
            + f"{android_metrics.get('avg_battery_impact', 0):.2f}"
            + """</td>
            <td>"""
            + f"{ios_metrics.get('avg_battery_impact', 0):.2f}"
            + """</td>
            <td class="""
            + ("android-faster" if cross_metrics.get("avg_battery_ratio", 1) < 1 else "ios-faster")
            + """>"""
            + f"{cross_metrics.get('avg_battery_ratio', 0):.2f}x"
            + """</td>
        </tr>
        <tr>
            <td>Temperature (°C)</td>
            <td>"""
            + f"{android_metrics.get('avg_temperature', 0):.2f}"
            + """</td>
            <td>"""
            + f"{ios_metrics.get('avg_temperature', 0):.2f}"
            + """</td>
            <td>-</td>
        </tr>
    </table>
