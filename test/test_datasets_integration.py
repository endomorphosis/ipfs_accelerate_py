...
def test_availability_check():
    ...
    from ipfs_accelerate_py.datasets_integration import is_datasets_available, get_datasets_status
...
def test_provenance_logger_inference():
    ...
    # Should not raise error
    cid = logger.log_inference("test-model", {"input": "test", "output": "result"})
    # CID is optional (None if IPFS unavailable)
    assert cid is None or isinstance(cid, str)
