...
class SkillsetGenerator:
    ...
    def generate_model_file(self, model_name: str, dry_run: bool = False) -> bool:
        ...
        if not template_model:
            print(f"No template found for model: {model_name}")
            return False
            
        # Load the template
        template_content = self.load_template(template_model)
        if not template_content:
            print(f"Failed to load template: {template_model}")
            return False
    ...
    def _replace_template_values(self, 
        ...
        # Replace class name
        content = content.replace(f"class hf_{template_model}", f"class hf_{model_name}")
...
def main():
    ...
    if args.model:
        print(f"Generating skillset for model: {args.model}")
        ...
        print(f"Generating skillsets for all models in family: {args.family}")
