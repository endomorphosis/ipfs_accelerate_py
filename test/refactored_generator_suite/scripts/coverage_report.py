...
def parse_roadmap_file(roadmap_path):
    ...
    try:
        ...
        for line in lines:
            ...
            if (
                "## Phase 3: Architecture Expansion" in line
                or "## Phase 4: Medium-Priority Models" in line
            ):
                high_priority_section = False
                    ...
                    if "Text" in current_section or "Text Models" in current_section:
                        if "encoder-only" in current_section.lower():
                            ...
                            if any(
                                ...
                                x in model_name.lower()
                                for x in ["gpt", "llama", "bloom", "mistral", "phi", "falcon"]
                            ):
                                architecture = "decoder-only"
                            elif any(
                                x in model_name.lower()
                                for x in ["t5", "bart", "pegasus", "longt5", "led"]
                            ):
                                architecture = "encoder-decoder"
                ...
                if architecture:
                    ...
                    if not is_implemented and architecture in architecture_types:
                        architecture_types[architecture].append(
                            {"name": model_name, "type": model_type, "architecture": architecture}
                        )
...
def generate_coverage_report(roadmap_path, tests_dir, output_dir):
    ...
    with open(report_file, "w") as f:
        ...
        implemented = sum(
            1 for model in models_data.get("high_priority_models", []) if model["implemented"]
        )
        implemented += sum(
            1 for model in models_data.get("medium_priority_models", []) if model["implemented"]
        )
        ...
        for arch, stats in models_data["models_by_architecture"].items():
            ...
            f.write(
                f"| {arch} | {stats['implemented']} | {stats['missing']} | {stats['total']} | {percentage:.1f}% |\n"
            )
        ...
        f.write(
            "To complete the model coverage, implement tests for the following models in order of priority:\n\n"
        )

        # High priority models
        f.write("### High Priority\n\n")
        high_missing = [
            model for model in models_data["high_priority_models"] if not model["implemented"]
        ]
        ...
        f.write("\n### Medium Priority\n\n")
        medium_missing = [
            model for model in models_data["medium_priority_models"] if not model["implemented"]
        ]
...
def main():
    ...
    parser.add_argument(
        "--roadmap",
        default="/home/barberb/ipfs_accelerate_py/test/skills/HF_MODEL_COVERAGE_ROADMAP.md",
        help="Path to HF_MODEL_COVERAGE_ROADMAP.md",
    )
    parser.add_argument(
        "--tests-dir",
        default="/home/barberb/ipfs_accelerate_py/test/skills/fixed_tests",
