...
class ModelSelector:
    ...
    def select_model(
        ...
        if model_type:
            ...
            models = [
                self.registry.get_model(model_id) for model_id in self.registry.get_model_ids()
            ]
        ...
        sorted_models = sorted(
            filtered_models,
            key=lambda m: (0 if m.get("default", False) else 1, m.get("size_mb", 1000000)),
        )

        # Return the best model ID
        return (
            sorted_models[0]["id"]
            if "id" in sorted_models[0]
            else list(self.registry.models.keys())[0]
        )
