...
def parse_args():
    ...
    output_group.add_argument(
        "--no-verify", action="store_true", help="Skip verification of generated files"
    )
    output_group.add_argument(
        "--summary-file",
        "-s",
...
def generate_models(args):
    ...
    if args.model:
        ...
        logger.info(
            f"Generating reference implementations for {len(models_to_generate)} models from file"
        )
        ...
        logger.info(
            f"Generating reference implementations for {len(models_to_generate)} {args.priority} models"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_model):
                ...
                try:
                    ...
                    results[model_type] = {"success": success, "file": file_path, "error": error}
                    ...
                    results[model] = {"success": False, "file": None, "error": str(e)}
    else:
        # Sequential generation
        for model in models_to_generate:
            model_type, success, file_path, error = generate_model_wrapper(
                model, args.output_dir, args.force
            )
            results[model_type] = {"success": success, "file": file_path, "error": error}
