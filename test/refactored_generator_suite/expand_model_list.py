...
def modify_comprehensive_generator(model_mapping: Dict[str, Dict[str, str]]):
    ...
    if search_marker in modified_content:
        ...
        if section_end > start_section:
            modified_content = (
                modified_content[:start_section] + hyphenated_fix + modified_content[section_end:]
            )
    ...
    logger.info(
        f"Updated {file_path} with {len(model_mapping)} models and fixed hyphenated name handling"
    )
