...
class TemplateComposer:
    ...
    def select_architecture_template(self, arch_type: str) -> BaseArchitectureTemplate:
        ...
        if arch_type in self.architecture_templates:
            ...
            logger.warning(
                f"Unknown architecture type: {arch_type}, using encoder-only as fallback"
            )
    ...
    def select_templates_for_model(
        ...
        for hw_type in hardware_types:
            ...
            if hw_template.is_compatible_with_architecture(arch_type):
                ...
                logger.warning(
                    f"Hardware {hw_type} is not compatible with architecture {arch_type}"
                )
    ...
    def generate_model_implementation(
        ...
        logger.info(
            f"Generating implementation for {model_name} ({arch_type}) on {', '.join(hardware_types)}"
        )
    ...
    def _generate_class_definition(
        self, model_name: str, arch_template: BaseArchitectureTemplate
    ) -> str:
        """Generate class definition."""
...
"""
    ...
    def _generate_hardware_init_methods(
        self,
        model_name: str,
        arch_template: BaseArchitectureTemplate,
        hardware_templates: List[BaseHardwareTemplate],
    ...
    def _generate_utilities(
        self, arch_template: BaseArchitectureTemplate, pipeline_template: BasePipelineTemplate
    ) -> str:
...
{pipeline_template.get_pipeline_utilities()}"""
    ...
    def _generate_mock_implementations(
        self,
        model_name: str,
        arch_template: BaseArchitectureTemplate,
        hardware_templates: List[BaseHardwareTemplate],
