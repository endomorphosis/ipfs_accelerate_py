...
This module provides the template for decoder-only models like GPT-2, LLaMA, etc.
"""
...
class DecoderOnlyTemplate(TemplateBase):
    """
    Template for decoder-only models like GPT-2, LLaMA, Mistral, etc.
    
    This template provides specialized support for models with causal/unidirectional 
    attention and autoregressive text generation capabilities.
    """
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        metadata.update({
            "name": "DecoderOnlyTemplate",
            "description": "Template for decoder-only models like GPT-2, LLaMA, Mistral, etc.",
            "supported_architectures": ["decoder-only"],
            "supported_models": [
                "gpt2", "gpt-j", "gpt-neo", "bloom", "llama", "mistral", "falcon", 
                "phi", "mixtral", "mpt", "codellama", "qwen2", "qwen3", "opt", "gemma"
            ]
        })
        return metadata
    ...
    def get_template_str(self) -> str:
        """
        Get the template string for decoder-only models.
...
which is a decoder-only model from the {{ model_info.type }} family.
...
class {{ model_info.type|capitalize }}Test:
    ...
    def __init__(self, model_name=None, output_dir=None, device=None):
        ...
        # Set test input text
        self.test_text = "{{ model_info.type|capitalize }} is a transformer model that"
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            # Run inference
            results = text_generation(self.test_text, **gen_kwargs)
            ...
            if isinstance(results, list) and len(results) > 0:
                ...
                if isinstance(first_result, dict) and "generated_text" in first_result:
                    ...
                    logger.info(f"Pipeline result: {generated_text[:50]}...")
                else:
    ...
    def test_model(self):
        ...
        try:
            ...
            logger.info(f"Model result: {generated_text[:50]}...")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            # Load tokenizer and model
            tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            model = OVModelForCausalLM.from_pretrained(self.model_name)
            ...
            gen_kwargs = {
                "max_length": 50,
                "do_sample": False,  # Deterministic for OpenVINO
                "num_return_sequences": 1
            }
            ...
            logger.info(f"OpenVINO result: {generated_text[:50]}...")
