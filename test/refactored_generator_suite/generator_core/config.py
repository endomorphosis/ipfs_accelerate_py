...
class ConfigManager:
    ...
    def _set_defaults(self) -> None:
        ...
        if "templates_dir" not in self.config:
            self.config["templates_dir"] = os.path.join(
        ...
        if "dependencies" not in self.config:
            self.config["dependencies"] = {"use_mocks": False, "ignore_missing": False}

        # Default syntax options
        if "syntax" not in self.config:
            self.config["syntax"] = {"auto_fix": True, "strict_validation": False}
