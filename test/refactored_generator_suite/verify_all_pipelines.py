...
def verify_pipeline_mapping():
    ...
    composer = TemplateComposer(
        hardware_templates, architecture_templates, pipeline_templates, "test_output"
    )
    ...
    architectures = [
        ...
        {"name": "vit-base-patch16-224", "type": "vision", "expected_pipeline": "image"},
        {
            ...
            "type": "vision-encoder-text-decoder",
            "expected_pipeline": "vision-text",
        },
    ...
    for arch in architectures:
        ...
        _, _, pipeline_template = composer.select_templates_for_model(
            model_name=model_name, arch_type=arch_type, hardware_types=["cpu"]
        )
        ...
        if actual_pipeline == expected_pipeline:
            ...
            logger.error(
                f"❌ {arch_type} -> {actual_pipeline} pipeline (expected {expected_pipeline})"
            )
            success = False
...
def check_pipeline_compatibility():
    ...
    for arch_type in architecture_types:
        compatible_pipelines = [
            p for p, c in compatibility_matrix.items() if c.get(arch_type, False)
        ]
        ...
        if not compatible_pipelines:
            ...
            logger.warning(
                f"⚠️ Architecture '{arch_type}' has multiple compatible pipelines: {', '.join(compatible_pipelines)}"
            )
