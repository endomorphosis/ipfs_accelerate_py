...
class GeneratorDatabaseIntegration:
    ...
    def track_task_update(
        ...
        if status in ["completed", "error"]:
            ...
            if "started_at" in step_data:
                step_data["duration"] = (
                    step_data["completed_at"] - step_data["started_at"]
                ).total_seconds()
    ...
    def track_batch_completion(self, batch_id: str) -> bool:
        ...
        if batch_data["status"] in ["completed", "partial"]:
            ...
            batch_data["duration"] = (
                batch_data["completed_at"] - batch_data["started_at"]
            ).total_seconds()
    ...
    def get_task_history(
        self, limit: int = 100, model_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
