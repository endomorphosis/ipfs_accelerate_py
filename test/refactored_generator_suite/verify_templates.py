...
def check_hardware_support(file_path, hardware_type):
    ...
    try:
        ...
        if hardware_type == "rocm":
            ...
            rocm_specifics["handles_half_precision"] = (
                "half precision" in content.lower() and "AMD GPU" in content
            )
        ...
        return {"has_support": False, "error": str(e), "file_name": os.path.basename(file_path)}
...
def main():
    ...
    for template in results["templates"]:
        if template.get("syntax_valid") and template.get("hardware_support", {}).get(
            "rocm", {}
        ).get("has_support", False):
