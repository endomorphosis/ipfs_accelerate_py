...
def analyze_skillset_file(file_path: str) -> Dict[str, Any]:
    ...
    skillset_classes = [name for name in classes.keys() if "Skillset" in name]
...
if __name__ == "__main__":
    ...
    if args.file:
        ...
        if args.json:
            ...
            print(
                f"  Missing common methods: {', '.join(result.get('missing_common_methods', []))}"
            )
            print(
                f"  Missing architecture methods: {', '.join(result.get('missing_architecture_methods', []))}"
            )
            print(
                f"  Compliance: {'Complete' if result.get('has_all_required_methods', False) else 'Incomplete'}"
            )
        ...
        skillset_files = [
            f
            for f in os.listdir(skillset_dir)
        ...
        if args.json:
            ...
            complete_count = sum(
                1 for r in results.values() if r.get("has_all_required_methods", False)
            )
