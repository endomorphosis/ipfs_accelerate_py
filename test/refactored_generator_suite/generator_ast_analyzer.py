...
class GeneratorASTAnalyzer:
    ...
    def analyze_directory(
        self, directory_path: str, pattern: str = "**/*generator*.py"
    ) -> Dict[str, Any]:
    ...
    def _analyze_ast(self, tree: ast.AST, file_path: str) -> Dict[str, Any]:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if "template" in node.name.lower() or any(
                    "template" in base.id.lower()
            ...
            # Analyze functions
                ...
                if "model" in node.name.lower() and (
                    "registry" in node.name.lower() or "register" in node.name.lower()
                ):
            ...
            if isinstance(child, ast.Call):
                ...
                if func_name:
                    if any(
                        ...
                        for keyword in ["hardware", "cuda", "gpu", "device", "detect"]
                    ):
                        function_info["has_hardware_detection"] = True
                    if any(
                        keyword in func_name
                        for keyword in ["model", "select", "registry", "transform"]
                    ):
    ...
    def _analyze_arguments(self, args: ast.arguments) -> Dict[str, Any]:
        ...
        arg_info = {"positional": [], "keyword": [], "vararg": None, "kwarg": None}
        ...
        for arg in args.args:
            arg_info["positional"].append(
                {
                    "name": arg.arg,
                    "annotation": self._get_name(arg.annotation)
        ...
        if args.vararg:
            arg_info["vararg"] = {
                ...
                "annotation": self._get_name(args.vararg.annotation)
                if hasattr(args.vararg, "annotation") and args.vararg.annotation
                else None,
            }
        ...
        if args.kwarg:
            arg_info["kwarg"] = {
                "name": args.kwarg.arg,
                "annotation": self._get_name(args.kwarg.annotation)
                if hasattr(args.kwarg, "annotation") and args.kwarg.annotation
                else None,
            }
    ...
    def _analyze_try_except(self, node: ast.Try) -> Dict[str, Any]:
        ...
        try_except_info = {"lineno": node.lineno, "has_import": False, "handlers": []}
    ...
    def generate_report(
        self, results: Dict[str, Any], summary_only: bool = False
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            "most_common_imports": sorted(
                [(k, v) for k, v in self.import_info.items()], key=lambda x: x[1], reverse=True
            )[:20],
    ...
    def generate_template_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "template_methods": sum(
                len(cls.get("methods", {})) for cls in self.template_info.values()
            ),
        }
    ...
    def generate_hardware_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hardware_related_functions": sum(
                1
                for func in self.function_info.values()
                if func.get("has_hardware_detection", False)
            ),
        }
    ...
    def generate_model_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_selection_functions": sum(
                1 for func in self.function_info.values() if func.get("has_model_selection", False)
            ),
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Python files with a focus on generator components"
    )
    ...
    parser.add_argument(
        "--pattern", default="**/*generator*.py", help="Glob pattern for files to analyze"
    )
    ...
    parser.add_argument(
        "--hardware", action="store_true", help="Focus on hardware detection analysis"
    )
