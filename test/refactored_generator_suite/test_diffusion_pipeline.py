...
def test_stable_diffusion_generation():
    ...
    if success:
        ...
        assert 'pipeline_type = "diffusion"' in content or "'diffusion'" in content, (
            "Missing diffusion pipeline type"
        )
...
def test_sam_segmentation():
    ...
    # Generate model implementation
    success, output_file = composer.generate_model_implementation(
        model_name="sam", arch_type="diffusion", hardware_types=["cpu"], force=True
    )
