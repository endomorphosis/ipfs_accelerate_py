...
def check_rocm_environment():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            is_rocm_available = details.get("has_amd_gpu", False) or details.get(
                "hip_available", False
            )
    ...
    if is_rocm_available:
        ...
        if "devices" in env_details:
            for device in env_details["devices"]:
                print(
                    f"  - GPU {device['index']}: {device['name']} ({device['total_memory_gb']} GB)"
                )
        ...
        if args.run_model:
            ...
            if success:
                ...
                print(
                    f"  - Half precision: {'Yes' if model_details.get('half_precision_supported') else 'No'}"
                )
