...
def parse_args():
    ...
    output_group.add_argument(
        "--output-dir", "-o", type=str, help="Directory to write generated skillsets to"
    )
    output_group.add_argument(
    ...
    output_group.add_argument(
        "--summary-file", "-s", type=str, help="Write generation summary to the specified JSON file"
    )

    # Template options
    template_group = parser.add_argument_group("Template Options")
    template_group.add_argument(
        "--template-dir", "-t", type=str, help="Directory containing template files"
    )
...
def generate_reference_implementations(args):
    ...
    for model in models_to_generate:
        ...
        results[model] = {"success": success, "files": files}
