...
def create_test_environment():
    ...
    output_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "transformers_implementations"
    )
...
def test_moe_generation():
    ...
    if success:
        ...
        assert "num_experts_per_token" in content or "num_active_experts" in content, (
            "Missing experts parameter"
        )
        ...
        assert "# moe pipeline imports" in content or "# MoE pipeline imports" in content, (
            "Missing MoE pipeline imports"
        )
...
def test_moe_classification():
    ...
    if success:
        ...
        assert "router_logits" in content or "router_probs" in content, (
            "Missing router probability information"
        )
...
def verify_pipeline(task_type="text_generation"):
    ...
    assert any(
        expert_term in preprocessing
        for expert_term in ["num_active_experts", "expert_routing", "num_experts"]
...
def test_template_composer_integration():
    ...
    _, _, pipeline_template = composer.select_templates_for_model(
        model_name="mixtral-8x7b", arch_type="mixture-of-experts", hardware_types=["cpu"]
    )

    # Check that the pipeline template is the MoE pipeline
    assert pipeline_template.pipeline_type == "moe", (
        "Template composer did not map mixture-of-experts to MoE pipeline"
    )
