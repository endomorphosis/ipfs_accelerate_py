...
try:
    ...
    class ComponentRegistry:
        ...
        def get_model_info(self, model_type):
            return {"architecture": "mock", "name": model_type}
...
class GeneratorTaskManager:
    ...
    async def generate_model(
        self, request: GenerateModelRequest, background_tasks: BackgroundTasks
    ...
    async def _generate_model(
        ...
        try:
            ...
            if self.db_integration:
                try:
                    self.db_integration.track_task_update(
                        task_id, "running", 0.1, "Preparing generation environment"
                    )
                except Exception as e:
            ...
            if self.db_integration:
                try:
                    self.db_integration.track_task_update(
                        task_id, "running", 0.15, "Determining output filename"
                    )
                except Exception as e:
                    logger.error(f"Error updating task in database: {e}")
            ...
            if self.db_integration:
                try:
                    self.db_integration.track_task_update(
                        task_id, "running", 0.3, f"Generating implementation for {model_name}"
                    )
                except Exception as e:
                    logger.error(f"Error updating task in database: {e}")
            ...
            await self._send_ws_update(task_id)
            logger.info(
                f"Model generation task {task_id} completed with status: {self.active_tasks[task_id]['status']}"
            )

        except Exception as e:
            logger.error(
                f"Error generating model {model_name} in task {task_id}: {e}", exc_info=True
            )
            self.active_tasks[task_id]["status"] = "error"
            ...
            if self.db_integration:
                try:
                    self.db_integration.track_task_update(
                        task_id, "error", 1.0, "Error during generation", str(e)
                    )
    ...
    def get_task_result(self, task_id: str) -> Dict[str, Any]:
        ...
        if task_data["status"] not in ["completed", "error"]:
            raise HTTPException(
                status_code=400, detail=f"Generation task {task_id} is not completed"
            )
        ...
        result = {
            ...
            "model_name": task_data["model_name"],
            "started_at": task_data["started_at"].isoformat(),
            "completed_at": task_data["completed_at"].isoformat()
            if task_data.get("completed_at")
            else None,
            "duration": (
                task_data.get("completed_at", datetime.now()) - task_data["started_at"]
            ).total_seconds(),
    ...
    def get_available_hardware(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Format the response
            return [
                {"name": hw_type, "available": info.get("available", False), "details": info}
    ...
    async def handle_websocket_connection(self, websocket: WebSocket, task_id: str):
        ...
        if task_id not in self.active_tasks:
            await websocket.send_json({"error": f"Generation task {task_id} not found"})
...
async def startup_event():
    ...
    if __name__ == "__main__":
        ...
        db_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "data", "generator_tasks.duckdb"
        )
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, default="./data/generator_tasks.duckdb", help="Path to database file"
    )
    args = parser.parse_args()
    ...
    uvicorn.run("generator_api_server:app", host=args.host, port=args.port, reload=False)
