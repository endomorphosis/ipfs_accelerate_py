    ...
    try:
        if args.viz_type == "3d":
            ...
            result = viz.create_3d_performance_visualization(
                ...
                or ["throughput_items_per_second", "average_latency_ms", "memory_peak_mb"],
            ...
            result = viz.create_hardware_comparison_heatmap(
                ...
                model_families=args.dimensions[0].split(",")
                if args.dimensions and len(args.dimensions) > 0
                else None,
                hardware_types=args.dimensions[1].split(",")
            ...
            result = viz.create_power_efficiency_visualization(
                hardware_types=args.dimensions[0].split(",")
                if args.dimensions and len(args.dimensions) > 0
                else None,
                model_families=args.dimensions[1].split(",")
                if args.dimensions and len(args.dimensions) > 1
                else None,
                batch_sizes=[int(b) for b in args.dimensions[2].split(",")]
            ...
            output_path = viz.export_dashboard(
                dashboard_name=dashboard_name, format=args.format, output_path=args.export_path
            )
...
def export_all(args):
    ...
    try:
        ...
        result_heatmap = viz.create_hardware_comparison_heatmap(
            metric="throughput", title="Hardware Comparison Heatmap"
        )
        ...
        exports = viz.export_all_visualizations(
            ...
            formats={
                ...
                "heatmap": args.formats.split(",")
                if args.formats
                else ["html", "png", "pdf", "json"],
                "power": args.formats.split(",")
                if args.formats
                else ["html", "png", "pdf", "json"],
                "time-series": args.formats.split(",")
                if args.formats
                else ["html", "png", "pdf", "json", "mp4", "gif"],
            },
            settings={
                "width": args.width,
                "height": args.height,
...
def main():
    ...
    parser.add_argument(
        "--no-open", action="store_true", help="Don't automatically open visualizations in browser"
    )
    parser.add_argument(
    ...
    export_args.add_argument("--name", help="Name for the exported visualization")
    export_args.add_argument("--title", help="Title for the visualization")
    export_args.add_argument(
        "--formats", help="Comma-separated list of export formats (html,png,pdf,json,mp4,gif)"
    )
    export_args.add_argument(
        "--width", type=int, default=1200, help="Width of the exported visualization"
    )
    export_args.add_argument(
        "--height", type=int, default=800, help="Height of the exported visualization"
    )
    export_args.add_argument(
        "--scale", type=float, default=2.0, help="Scale factor for the exported visualization"
    )

    # Animation arguments
    animation_args = argparse.ArgumentParser(add_help=False)
    animation_args.add_argument(
        "--fps", type=int, default=30, help="Frames per second for animations"
    )
    animation_args.add_argument(
        "--duration", type=int, default=10, help="Duration of animations in seconds"
    )
    animation_args.add_argument(
        "--frame-duration", type=int, default=50, help="Duration of each frame in milliseconds"
    )
    animation_args.add_argument(
        "--transition-duration",
        type=int,
    ...
    export_parser = subparsers.add_parser(
        "export", help="Export a specific visualization", parents=[export_args, animation_args]
    )
    ...
    export_parser.add_argument("--metrics", nargs="+", help="Performance metrics to visualize")
    export_parser.add_argument(
        "--dimensions",
        nargs="+",
    ...
    export_parser.add_argument(
        "--time-range", type=int, default=90, help="Time range in days (for time-series)"
    )
    export_parser.add_argument(
        "--time-interval",
        choices=["hour", "day", "week", "month"],
        default="day",
        help="Time interval (for time-series)",
    )
    export_parser.add_argument(
        "--window-size",
        type=int,
    ...
    export_parser.add_argument(
        "--no-trend", action="store_true", help="Don't include trend lines (for time-series)"
    )
    export_parser.add_argument(
        "--no-animation",
        action="store_true",
    ...
    export_parser.add_argument(
        "--batch-size", type=int, default=1, help="Batch size for heatmap visualization"
    )
    export_parser.add_argument(
        "--dashboard-template",
    ...
    export_parser.add_argument("--description", help="Description for the dashboard")
    export_parser.add_argument(
        "--columns", type=int, default=2, help="Number of columns in the dashboard grid"
    )
    export_parser.add_argument(
        "--row-height", type=int, default=500, help="Height of each row in pixels"
    )
    export_parser.add_argument(
        "--format",
    ...
    export_parser.add_argument("--export-path", help="Path for the exported file")

    # Export all command
    export_all_parser = subparsers.add_parser(
        "export-all", help="Export all visualization types", parents=[export_args, animation_args]
    )

    # Export animation command
    export_animation_parser = subparsers.add_parser(
        "export-animation",
        help="Export an animated time-series visualization",
        parents=[export_args, animation_args],
    )
    export_animation_parser.add_argument(
        "--metric", default="throughput_items_per_second", help="Performance metric to visualize"
    )
    export_animation_parser.add_argument(
        "--dimensions",
        nargs="+",
        default=["model_family", "hardware_type"],
        help="Dimensions for grouping (model_family, hardware_type, etc.)",
    )
    export_animation_parser.add_argument(
        "--time-range", type=int, default=90, help="Time range in days"
    )
    export_animation_parser.add_argument(
        "--time-interval",
        choices=["hour", "day", "week", "month"],
    ...
    export_animation_parser.add_argument(
        "--window-size", type=int, default=5, help="Window size for trend analysis"
    )
    export_animation_parser.add_argument(
        "--no-trend", action="store_true", help="Don't include trend lines"
    )
    export_animation_parser.add_argument(
        "--format", choices=["mp4", "gif"], default="mp4", help="Animation format"
    )

    # Configure export command
    configure_parser = subparsers.add_parser(
        "configure", help="Configure export settings", parents=[export_args, animation_args]
    )
    configure_parser.add_argument(
        "--no-plotlyjs", action="store_true", help="Don't include plotly.js in HTML exports"
    )
    configure_parser.add_argument(
        "--include-mathjax", action="store_true", help="Include MathJax in HTML exports"
    )
    configure_parser.add_argument(
        "--no-full-html", action="store_true", help="Don't include HTML/body tags in HTML exports"
    )
