// Tests for OllamaClean API Backend
import { OllamaClean } from '../src/api_backends/ollama_clean';
import { ApiMetadata, Message } from '../src/api_backends/types';

// Mock fetch for testing
global.fetch = jest.fn();

describe('OllamaClean API Backend', () => {
  let backend: OllamaClean;
  ...
  beforeEach(() => {
    ...
    // Set up test data
    mockMetadata = {
      ollama_clean_api_key: 'test-api-key'
    };
    
    // Create backend instance
    backend = new OllamaClean({}, mockMetadata);
  });
