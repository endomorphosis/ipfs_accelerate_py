...
class FaultToleranceVisualizer:
    ...
    def __init__(self, validation_results: Dict[str, Any]) -> None:
        ...
        if not self.available:
            self.logger.warning(
                "Visualization libraries not available. Install matplotlib and jinja2."
            )
    ...
    def generate_success_rate_dashboard(self, output_path: str) -> Optional[str]:
        ...
        try:
            ...
            color_map = mcolors.LinearSegmentedColormap.from_list(
                "success_gradient", ["red", "orange", "green"]
            )
            ...
            if multiple_runs:
                ...
                for scenario in scenario_success_counts:
                    success_rate = (
                        scenario_success_counts[scenario] / scenario_run_counts[scenario]
                        if scenario_run_counts[scenario] > 0
                        else 0
                    )
                    scenarios.append(scenario.replace("_", " "))
            ...
            bars = ax.barh(
                scenarios, success_rates, color=[color_map(rate) for rate in success_rates]
            )
            ...
            ax.axvline(
                x=0.7, color="orange", linestyle="--", alpha=0.7, label="Warning Threshold (70%)"
            )
            ax.axvline(
                x=0.9, color="green", linestyle="--", alpha=0.7, label="Success Threshold (90%)"
            )
            ax.legend()
    ...
    def generate_performance_impact_visualization(self, output_path: str) -> Optional[str]:
        ...
        try:
            ...
            if not perf_impact or not perf_impact.get("summary", {}).get(
    ...
    def generate_recovery_strategy_comparison(
        self, output_path: str, results_by_strategy: Dict[str, Dict[str, Any]]
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            for i, strategy in enumerate(strategies):
                ...
                bars = ax.bar(
                    positions + offset, strategy_times[i], bar_width, label=strategy, alpha=0.8
                )
