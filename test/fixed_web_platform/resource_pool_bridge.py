...
class EnhancedWebModel:
    ...
    def __init__(self, model_name, model_type, hardware_type, browser=None, **kwargs):
        ...
        # Debug init
        logger.debug(
            f"EnhancedWebModel initialized with optimization flags: compute_shader_optimized={self.compute_shader_optimized}, precompile_shaders={self.precompile_shaders}, parallel_loading={self.parallel_loading}"
        )
    ...
    def __call__(self, inputs):
        ...
        if "text_embedding" in self.model_type.lower() or "bert" in self.model_type.lower():
            ...
            output_tensors["text_embedding"] = (
                f"Simulated text embedding tensor for {self.model_name}"
            )
        elif "vision" in self.model_type.lower():
            # For vision models, we could share image features
            output_tensors["vision_embedding"] = (
                f"Simulated vision embedding tensor for {self.model_name}"
            )
        ...
        if self.uses_shared_tensors:
            ...
            result["shared_tensor_speedup"] = (
                1.0 / shared_tensor_speedup - 1.0
...
class ResourcePoolBridgeIntegration:
    ...
    def __init__(
        ...
        # Initialize logger
        logger.info(
            f"ResourcePoolBridgeIntegration created with max_connections={max_connections}, adaptive_scaling={'enabled' if adaptive_scaling else 'disabled'}, IPFS={'enabled' if enable_ipfs else 'disabled'}"
        )
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.adaptive_scaling:
                ...
                logger.info(
                    f"Adaptive scaling enabled, starting with {initial_connections} browser connections"
                )
                ...
                try:
                    from test.web_platform.resource_pool_circuit_breaker import (
                        ResourcePoolCircuitBreakerManager,
                    )

                    self.circuit_breaker_manager = ResourcePoolCircuitBreakerManager(
                        self.browser_connections
                    )
                    await self.circuit_breaker_manager.initialize()
                    logger.info(
                        "Circuit breaker manager initialized for connection health monitoring"
                    )
                ...
                logger.info(
                    f"Adaptive scaling disabled, creating {self.max_connections} browser connections"
                )
                await self._setup_initial_connections(self.max_connections)

            logger.info(
                f"Resource pool bridge initialized with {len(self.browser_connections)} browser connections"
            )
            return True
            ...
            if self.adaptive_scaling:
                try:
                    ...
                    logger.warning(
                        "Could not import AdaptiveConnectionManager, adaptive scaling disabled"
                    )
    ...
    async def _setup_initial_connections(self, num_connections):
                ...
                try:
                    ...
                    try:
                        ...
                        connection_errors[connection_id] = (
                            f"browser_launch_error: {type(launch_error).__name__}"
                        )
                        failed_connections += 1
                        continue
                    ...
                    if success:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Timeout while creating WebSocket bridge for {connection_id}"
                            )
                            ...
                            logger.error(
                                f"Error creating WebSocket bridge for {connection_id}: {bridge_error}"
                            )
                            await automation.close()
                            # Record the error for diagnostics
                            connection_errors[connection_id] = (
                                f"websocket_bridge_error: {type(bridge_error).__name__}"
                            )
                            failed_connections += 1
                            continue
                        ...
                        if bridge:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Timeout while waiting for WebSocket connection for {connection_id}"
                                )
                                await automation.close()
                                # Record the error for diagnostics
                                ...
                                logger.error(
                                    f"Error establishing WebSocket connection for {connection_id}: {connection_error}"
                                )
                                await automation.close()
                                # Record the error for diagnostics
                                connection_errors[connection_id] = (
                                    f"websocket_connection_error: {type(connection_error).__name__}"
                                )
                                failed_connections += 1
                                continue
                            ...
                            if connected:
                                ...
                                logger.info(
                                    f"Successfully created browser connection: {connection_id}"
                                )
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Error checking browser capabilities for {connection_id}: {cap_error}"
                                    )
                                    capabilities = None

                                if capabilities:
                                    # Update connection info with capabilities
                                    self.browser_connections[connection_id]["capabilities"] = (
                                        capabilities
                                    )
                                    ...
                                    logger.info(
                                        f"Connection {connection_id} supports: WebGPU={webgpu_support}, WebNN={webnn_support}"
                                    )

                                    if platform == "webgpu" and not webgpu_support:
                                        logger.warning(
                                            f"Connection {connection_id} is configured for WebGPU but does not support it"
                                        )
                                    elif platform == "webnn" and not webnn_support:
                                        logger.warning(
                                            f"Connection {connection_id} is configured for WebNN but does not support it"
                                        )
                            else:
                                logger.warning(
                                    f"Failed to establish WebSocket connection for {connection_id}"
                                )
                                await automation.close()
                                # Record the error for diagnostics
        ...
        logger.info(
            f"Connection setup complete: {successful_connections} successful, {failed_connections} failed out of {attempted_connections} attempted"
        )
        ...
        if successful_connections < num_connections // 2 and successful_connections > 0:
            logger.warning(
                f"Only {successful_connections} connections created. Some operations may be slower than expected."
            )
        ...
        if not self.browser_connections and self.real_browser_available:
            logger.warning(
                "No browser connections could be established, falling back to simulation mode"
            )
            ...
            if failed_connections > 0:
                ...
                for error in connection_errors.values():
                    error_type = (
                        error if isinstance(error, str) else error.get("error_type", "unknown")
                    )
    ...
    def _calculate_browser_distribution(self, num_connections):
        ...
        distribution = {"chrome": 0, "firefox": 0, "edge": 0}
    ...
    async def get_model(self, model_type, model_name, hardware_preferences=None):
        ...
        if hasattr(self, "adaptive_manager"):
            ...
            if len(hardware_priority_list) > 0:
                ...
                self.adaptive_manager.update_model_type_metrics(
                    model_type, 0.5
            ...
            if not preferred_browser:
                if (
                    "audio" in model_type.lower()
                    or "whisper" in model_type.lower()
                    or "wav2vec" in model_type.lower()
                ):
                    preferred_browser = (
                        "firefox"  # Firefox has better WebGPU compute shader performance for audio
                    )
                elif (
                    "vision" in model_type.lower()
                    or "clip" in model_type.lower()
                    or "vit" in model_type.lower()
                ):
                    ...
                    preferred_browser = (
                        "edge"  # Edge has excellent WebNN support for text embeddings
                    )
                else:
        ...
        if (
            hasattr(self, "browser_connections")
            and self.browser_connections
            and hasattr(self, "real_browser_available")
            and self.real_browser_available
        ):
            # Try to get a connection with the preferred browser and hardware platform
            connection = await self._get_connection_for_model(
                model_type, model_name, preferred_browser, preferred_hardware, **kwargs
            )

            if connection:
                # Create real browser model
                return await self._create_real_browser_model(
                    connection, model_type, model_name, **kwargs
                )
            else:
                # Fall back to simulation
                logger.warning(
                    f"No suitable browser connection available for {model_name}, falling back to simulation"
                )
        ...
        if hasattr(self, "tensor_sharing_manager") and self.tensor_sharing_manager:
            ...
            shared_tensor = self.tensor_sharing_manager.get_shared_tensor(
                embedding_tensor_name, model_name
            )
            if shared_tensor is not None:
                ...
                kwargs["shared_tensors"] = {tensor_type: embedding_tensor_name}

        # Either we don't have real browser connections or we couldn't get a suitable one
        # Fall back to simulation
        logger.debug(
            f"Using simulated model for {model_name} ({model_type}) using {preferred_hardware} with {preferred_browser}"
        )
        return EnhancedWebModel(
            model_name, model_type, preferred_hardware, preferred_browser, **kwargs
        )

    async def _get_connection_for_model(
        self, model_type, model_name, preferred_browser, preferred_hardware, **kwargs
    ):
        """
        ...
        if hasattr(self, "circuit_breaker_manager"):
            try:
                healthy_connections = (
                    await self.circuit_breaker_manager.circuit_breaker.get_healthy_connections()
                )
        ...
        for connection_id, connection in self.browser_connections.items():
            ...
            if (
                hasattr(self, "circuit_breaker_manager")
                and healthy_connections
                and connection_id not in healthy_connections
            ):
                logger.warning(
                    f"Skipping unhealthy connection {connection_id} based on circuit breaker health check"
                )
                continue
            ...
            if hasattr(self, "circuit_breaker_manager"):
                try:
                    allowed, reason = await self.circuit_breaker_manager.pre_request_check(
                        connection_id
                    )
                    if not allowed:
                        logger.warning(
                            f"Circuit breaker prevented use of connection {connection_id}: {reason}"
                        )
                        continue
                except Exception as e:
                    logger.warning(
                        f"Error checking circuit breaker for connection {connection_id}: {e}"
                    )
            ...
            if (
                "audio" in model_type.lower()
                and connection["browser"] == "firefox"
                and connection["compute_shaders"]
            ):
                score += 40  # Major bonus for audio models on Firefox with compute shaders

            # Check for WebNN support for text embedding models
            if (
                "text_embedding" in model_type.lower() or "bert" in model_type.lower()
            ) and connection["platform"] == "webnn":
            ...
            if (
                hasattr(self, "circuit_breaker_manager")
                and connection_id in self.circuit_breaker_manager.circuit_breaker.health_metrics
            ):
                health_score = self.circuit_breaker_manager.circuit_breaker.health_metrics[
                    connection_id
                ].health_score
                ...
                score += health_bonus
                logger.debug(
                    f"Added health bonus of {health_bonus:.1f} to connection {connection_id} (health score: {health_score:.1f})"
                )
        ...
        if connection_scores:
            ...
            logger.info(
                f"Selected connection {best_connection_id} with score {best_score} for {model_name} ({model_type})"
            )
    ...
    async def _create_real_browser_model(self, connection, model_type, model_name, **kwargs):
        ...
        if model_key not in connection["initialized_models"]:
            ...
            logger.info(
                f"Initializing model {model_name} ({model_type}) in browser using {platform}"
            )
            ...
            if not init_result or init_result.get("status") != "success":
                logger.error(
                    f"Failed to initialize model {model_name} in browser: {init_result.get('error', 'Unknown error')}"
                )
                # Release the connection and fall back to simulation
                connection["active"] = False
                self.active_connections -= 1
                return EnhancedWebModel(
                    model_name, model_type, platform, connection["browser"], **kwargs
                )
                ...
                try:
                    ...
                    try:
                        result = await wait_for(
                            self.bridge.run_inference(self.model_name, inputs, self.platform),
                        ...
                        if hasattr(self.pool, "circuit_breaker_manager") and connection_id:
                            try:
                                timeout_error = TimeoutError(
                                    f"Inference timeout after 60 seconds for {self.model_name}"
                                )
                                ...
                                logger.warning(
                                    f"Error handling timeout with circuit breaker: {circuit_error}"
                                )
                    ...
                    if not result or result.get("status") != "success":
                        error_msg = (
                            result.get("error", "Unknown error") if result else "Empty response"
                        )
                        ...
                        if hasattr(self.pool, "circuit_breaker_manager") and connection_id:
                            try:
                                ...
                                await self.pool.circuit_breaker_manager.circuit_breaker.record_model_performance(
                                    connection_id, self.model_name, inference_time_ms, False
                                )
                    ...
                    if hasattr(self.pool, "circuit_breaker_manager") and connection_id:
                        try:
                            await self.pool.circuit_breaker_manager.record_request_result(
                                connection_id, True, response_time_ms=inference_time_ms
                            )

                            # Record model performance
                            await self.pool.circuit_breaker_manager.circuit_breaker.record_model_performance(
                                connection_id, self.model_name, inference_time_ms, True
                            )
                        except Exception as e:
                    ...
                    if hasattr(self.pool, "circuit_breaker_manager") and connection_id:
                        try:
                            ...
                            logger.warning(
                                f"Error recording failure with circuit breaker: {circuit_error}"
                            )
                    ...
                    if not is_recoverable:
                        try:
                            ...
                            if hasattr(self.bridge, "websocket") and hasattr(
                                self.bridge.websocket, "state"
                            ):
                                error_response["websocket_state"] = self.bridge.websocket.state

                            # Check for browser status
                            if (
                                hasattr(self.connection["automation"], "process")
                                and self.connection["automation"].process
                            ):
                                error_response["browser_running"] = (
                                    self.connection["automation"].process.poll() is None
                                )
                        except Exception:
    ...
    def get_metrics(self):
        ...
        metrics = {
            ...
            "models": {},
            "performance": {"inference_times": {}, "throughput": {}, "memory_usage": {}},
            "adaptive_scaling": {
                "enabled": self.adaptive_scaling,
                "scaling_events": [],
                "current_metrics": {},
            },
            "resources": {"system_memory_percent": 0, "process_memory_mb": 0},
        }
        ...
        if PSUTIL_AVAILABLE:
            try:
                ...
                try:
                    ...
                    metrics["resources"]["process_memory_mb"] = process.memory_info().rss / (
                        1024 * 1024
                    )
                except (
        ...
        if hasattr(self, "circuit_breaker_manager"):
            try:
                ...
                for connection_id in self.browser_connections.keys():
                    state = anyio.run(
                        self.circuit_breaker_manager.circuit_breaker.get_connection_state(
                            connection_id
                        )
                    )
                    if state:
                        circuit_states[connection_id] = {
                            ...
                            "health_score": state["health_metrics"]["health_score"]
                ...
                metrics["circuit_breaker"] = {
                    ...
                    "healthy_count": len(
                        anyio.run(
                            self.circuit_breaker_manager.circuit_breaker.get_healthy_connections()
                        )
                    ),
                }
            except Exception as e:
                    ...
                    try:
                        ...
                        if inspect.iscoroutine(result) or hasattr(result, "__await__"):
                            try:
                                ...
                                model_timeout = min(
                                    60, timeout_seconds * 0.8
                                )  # 80% of total timeout or 60s, whichever is smaller
                                result = await wait_for(result, timeout=model_timeout)
                            except TimeoutError:
                                logger.error(
                                    f"Individual model timeout: {model_info['model_name']} after {model_timeout}s"
                                )
                                return {
                                    "success": False,
                        ...
                        logger.error(
                            f"Runtime error executing model {model_info['model_name']}: {e}"
                        )
                        error_obj = error_handler.handle_error(e, model_info)
                        return {
                            "success": False,
                            ...
                            "execution_time": time.time() - model_start_time,
                            "timestamp": time.time(),
                            "recovery_suggestion": error_handler.get_recovery_strategy(e).get(
                        ...
                        # Catch any other unexpected errors
                        logger.error(
                            f"Unexpected error executing model {model_info['model_name']}: {e}"
                        )
                        error_category = error_handler.categorize_error(e)
                        recovery_strategy = error_handler.get_recovery_strategy(e)
                        return {
    ...
    async def close(self):
            ...
            try:
                ...
                if hasattr(self.circuit_breaker_manager, "force_cleanup"):
                    try:
                        ...
                        logger.critical(
                            f"Force cleanup of circuit breaker failed: {force_cleanup_error}"
                        )
                        cleanup_status["errors"]["circuit_breaker_force_cleanup"] = str(
                            force_cleanup_error
                        )
            except Exception as e:
                ...
                try:
                    ...
                    if "bridge" in connection:
                        ...
                        async def cleanup_bridge():
                            try:
                                ...
                                await wait_for(connection["bridge"].shutdown_browser(), timeout=5)
                                connection_cleanup_status["browser_shutdown"] = True

                                # Then stop the bridge itself
                                await wait_for(connection["bridge"].stop(), timeout=5)
                                ...
                                logger.warning(
                                    f"Error shutting down bridge for {connection_id}: {bridge_error}"
                                )
                                return False
                    ...
                    if "automation" in connection:
                        ...
                        async def cleanup_automation():
                            try:
                                await wait_for(connection["automation"].close(), timeout=5)
                                ...
                                logger.warning(
                                    f"Error closing automation for {connection_id}: {automation_error}"
                                )
                                return False
                    ...
                    if any(result is not None for result in cleanup_results):
                        ...
                        connection_errors[connection_id] = {
                            "bridge_error": str(cleanup_results[0])
                            if cleanup_results[0] is not None
                            else None,
                            "automation_error": str(cleanup_results[1])
                            if len(cleanup_results) > 1 and cleanup_results[1] is not None
                            else None,
            ...
            if hasattr(self.adaptive_manager, "close"):
                try:
                    if inspect.iscoroutinefunction(self.adaptive_manager.close):
                        await wait_for(self.adaptive_manager.close(), timeout=5)
                    else:
        ...
        if cleanup_status["success"]:
            logger.info(
                f"Resource pool bridge closed successfully in {cleanup_status['total_cleanup_time']:.2f}s"
            )
        else:
            error_count = len(cleanup_status["errors"])
            logger.warning(
                f"Resource pool bridge closed with {error_count} errors in {cleanup_status['total_cleanup_time']:.2f}s"
            )
        ...
        try:
            ...
            if max_memory_mb is None:
                ...
                try:
                    ...
                    logger.info(
                        f"Automatically set tensor sharing memory limit to {max_memory_mb} MB (25% of available memory)"
                    )
                except ImportError:
            ...
            logger.warning(
                f"Cross-model tensor sharing not available: {e}. The 'cross_model_tensor_sharing' module could not be imported."
            )
            ...
            if "No module named" in str(e):
                ...
                logger.info(
                    f"To enable tensor sharing, install the required package: pip install {package_name}"
                )
    ...
    async def share_tensor_between_models(
        self,
        tensor_data,
        tensor_name,
        producer_model,
        ...
        try:
            ...
            if hasattr(self, "tensor_sharing_stats"):
                ...
                try:
                    ...
                    self.tensor_sharing_stats["tensors_by_type"][tensor_type]["memory_mb"] += (
                        memory_mb
                    )
            ...
            logger.info(
                f"Registered shared tensor {tensor_name} for models: {producer_model} -> {consumer_models}"
            )
            ...
            return {
                ...
                "dtype": dtype,
                "memory_mb": shared_tensor.get_memory_usage() / (1024 * 1024),
                "total_shared_tensors": getattr(self, "tensor_sharing_stats", {}).get(
                    "total_tensors", 1
                ),
            ...
            error_obj = error_handler.handle_error(
                e,
                {
                    "tensor_name": tensor_name,
                    "shape": shape,
    ...
    async def test_resource_pool():
        ...
        try:
            ...
            model = await integration.get_model(
                "text", "bert-base-uncased", {"priority_list": ["webgpu", "cpu"]}
            )
            ...
            model2 = await integration.get_model(
                "vision", "vit-base", {"priority_list": ["webgpu"]}
            )

            print("Getting audio model (Whisper)...")
            model3 = await integration.get_model(
                "audio",
                "whisper-tiny",
                {
