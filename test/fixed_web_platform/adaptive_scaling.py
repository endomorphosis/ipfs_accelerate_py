...
class AdaptiveConnectionManager:
    ...
    def __init__(
        ...
        self.browser_usage = {"chrome": 0, "firefox": 0, "edge": 0, "safari": 0}
        ...
        logger.info(
            f"Adaptive Connection Manager initialized with {min_connections}-{max_connections} connections"
        )
    ...
    def update_metrics(
        ...
        if self.avg_utilization == 0:
            ...
            self.avg_utilization = (
                self.avg_utilization * (1 - self.smoothing_factor)
    ...
    def _get_scaling_recommendation(self, metrics: Dict[str, Any]) -> Tuple[int, str]:
        ...
        if time_since_last_scaling < self.scaling_cooldown:
            return (
                recommended,
                f"In scaling cooldown period ({time_since_last_scaling:.1f}s < {self.scaling_cooldown}s)",
            )
        ...
        # Scale down if utilization below threshold for sustained period
            ...
            utilization < self.scale_down_threshold and avg_utilization < self.scale_down_threshold
        ):
            # Only scale down if above min connections
            if current_connections > self.min_connections:
                # Need at least this many to handle current active load
                min_needed = math.ceil(
                    active_connections / 0.8
                )  # Target 80% utilization for active connections
                ...
                if NUMPY_AVAILABLE:
                    ...
                    if util_trend > 0.005 and predicted_util > self.scale_up_threshold:
                        if current_connections < self.max_connections:
                            ...
                            predicted_min_connections = math.ceil(
                                predicted_models / model_per_connection
                            )
    ...
    def update_connection_startup_time(self, startup_time: float):
        ...
        # Update average
        self.avg_connection_startup_time = sum(self.connection_startup_times) / len(
            self.connection_startup_times
        )
    ...
    def _normalize_model_type(self, model_type: str) -> str:
        ...
        if (
            ...
            or "wav2vec" in model_type
            or "clap" in model_type
        ):
            return "audio"
        elif (
            "vision" in model_type
            or "image" in model_type
            or "vit" in model_type
            or "clip" in model_type
        ):
            return "vision"
        elif "embedding" in model_type or "bert" in model_type:
            return "text_embedding"
        elif (
            "generation" in model_type
...
if __name__ == "__main__":
    ...
    manager = AdaptiveConnectionManager(
        min_connections=1, max_connections=8, scale_up_threshold=0.7, scale_down_threshold=0.3
    )
    ...
    for i in range(20):
        ...
        logger.info(
            f"Utilization: {utilization:.2f}, Recommendation: {result['scaling_recommendation']}, Reason: {result['reason']}"
        )
