...
class RealWebGPUConnection:
    ...
    async def initialize(self):
        ...
        try:
            ...
            success = await self.integration.initialize_platform(
                platform="webgpu", browser_name=self.browser_name, headless=self.headless
            )
            ...
            if self.feature_detection:
                ...
                if webgpu_supported:
                    ...
                    if webgpu_adapter:
                        logger.info(
                            f"WebGPU Adapter: {webgpu_adapter.get('vendor')} - {webgpu_adapter.get('architecture')}"
                        )
                else:
    ...
    async def initialize_model(
        ...
        try:
            ...
            if self._is_shader_precompilation_supported() and options.get(
