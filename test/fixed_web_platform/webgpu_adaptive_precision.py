...
class WebGPUAdaptivePrecision:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized WebGPU adaptive precision with default={default_bits}-bit, "
    ...
    def get_layer_precision(self, layer_name: str) -> int:
        ...
        if "embed" in layer_name.lower():
            ...
            "output" in layer_name.lower()
            or "lm_head" in layer_name.lower()
    ...
    def create_layer_precision_map(self, model_structure: Dict) -> Dict:
        ...
        if browser_info["detected"]:
            ...
            if browser_info["browser"] == "firefox":
                ...
                self.layer_groups["attention"]["bits"] = max(
                    self.layer_groups["attention"]["bits"], 8
                )
                ...
                self.layer_groups["embedding"]["bits"] = max(
                    self.layer_groups["embedding"]["bits"], 16
                )
    ...
    def adjust_precision_for_memory(
        self, available_memory_mb: float, required_memory_mb: float
    ) -> bool:
        """
            ...
            self.memory_stats["precision_history"].append(
                {
                    ...
                    "new_precision": {
                        name: info["bits"] for name, info in self.layer_precision.items()
                    },
    ...
    def estimate_memory_savings(
        self, current_bits: int, target_bits: int, tensor_size_mb: float
    ) -> float:
        """
        ...
        current_storage_bits = (
            16 if current_bits > 8 else 8 if current_bits > 4 else 8 if current_bits > 2 else 8
        )
        target_storage_bits = (
            16 if target_bits > 8 else 8 if target_bits > 4 else 8 if target_bits > 2 else 8
        )
        ...
        current_adjusted_size = (
            tensor_size_mb / current_packing if current_bits < 8 else tensor_size_mb
        )
        target_adjusted_size = (
            tensor_size_mb * (target_storage_bits / current_storage_bits) / target_packing
            if target_bits < 8
            else tensor_size_mb * (target_bits / current_bits)
        )
    ...
    def get_memory_usage_estimate(
        self, model_structure: Dict, precision_map: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        def process_tensor(name, shape, dtype):
            ...
            layer_memory[name] = {
                ...
                "bits": bits,
                "reduction_percent": (1 - (optimized_size_mb / fp16_size_mb)) * 100
                if fp16_size_mb > 0
                else 0,
            }
        ...
        self.memory_stats["peak_memory_mb"] = max(
            self.memory_stats["peak_memory_mb"], total_optimized_mb
        )
    ...
    def track_accuracy_impact(self, layer_name: str, baseline_output: Any, quantized_output: Any):
        ...
        try:
            ...
            if baseline.shape == quantized.shape:
                ...
                rel_l2_err = np.sqrt(np.sum((baseline - quantized) ** 2)) / (
                    l2_norm if l2_norm > 0 else 1.0
                )
                ...
                logger.debug(
                    f"Layer {layer_name} ({bits}-bit): MSE={mse:.6f}, Rel L2={rel_l2_err:.6f}"
                )
    ...
    def get_accuracy_report(self) -> Dict:
        ...
        for layer, stats in self.accuracy_stats["layer_impact"].items():
            ...
            if bits not in by_precision:
                by_precision[bits] = []
            by_precision[bits].append({"layer": layer, **stats})
    ...
    def optimize_for_target_accuracy(self, target_rel_l2_err: float = 0.01) -> Dict:
        ...
        for layer_name, bits in optimized_precision.items():
            if (
                layer_name in self.layer_precision
                and self.layer_precision[layer_name]["bits"] != bits
            ):
                self.layer_precision[layer_name]["bits"] = bits
                precision_changes += 1

        logger.info(
            f"Optimized {precision_changes} layers to meet target accuracy of {target_rel_l2_err:.4f}"
        )
    ...
    def _validate_precision_settings(self):
        ...
        if self.critical_layers_bits not in valid_bits:
            raise ValueError(
                f"Critical layers bits must be one of {valid_bits}, got {self.critical_layers_bits}"
            )
    ...
    def _lower_precision_by_group_priority(self, required_mb: float) -> bool:
        ...
        groups_by_priority = sorted(self.layer_groups.items(), key=lambda x: x[1]["priority"])

        # Filter to only include groups that can be reduced further
        reducible_groups = [
            (name, info)
            for name, info in groups_by_priority
        ...
        for group_name, group_info in reducible_groups:
            ...
            logger.info(
                f"Reducing {group_name} group precision from {current_bits}-bit to {target_bits}-bit"
            )
...
class WebGPU4BitLayerController:
    ...
    def optimize_layer(self, layer_name: str, tensor_type: str, tensor_info: Dict) -> Dict:
        ...
        if "attention" in layer_name.lower() or any(
            k in layer_name.lower() for k in ["query", "key", "value"]
        ):
            ...
            for layer_idx, layer_info in self.model_structure["layers"].items():
                if "tensors" in layer_info:
                    for tensor_name, tensor_info in layer_info["tensors"].items():
                        ...
                        all_optimizations[layer_name] = self.optimize_layer(
                            layer_name, tensor_type, tensor_info
                        )
        ...
        if "layers" in self.model_structure:
            for layer_idx, layer_info in self.model_structure["layers"].items():
                if "tensors" in layer_info:
                    for tensor_name in layer_info["tensors"]:
                        if any(
                            k in tensor_name.lower() for k in ["attention", "query", "key", "value"]
                        ):
        ...
        for layer_name in self.critical_layers:
            ...
            if "key" in layer_name.lower() or "value" in layer_name.lower():
                ...
                self.set_layer_optimization(
                    layer_name, bits=self.kv_cache_bits, per_channel=True, block_size=32
                )
...
def optimize_model_with_adaptive_precision(
    ...
    model_structure = {"embeddings": {}, "layers": {}}
    ...
    for i in range(num_hidden_layers):
        ...
        layer_struct["tensors"]["attention.query"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention.key"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention.value"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention.output"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }

        # MLP components
        layer_struct["tensors"]["mlp.gate"] = {
            "shape": (hidden_size, 4 * hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["mlp.up"] = {
            "shape": (hidden_size, 4 * hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["mlp.down"] = {
        ...
        layer_struct["tensors"]["post_attention_layernorm"] = {
            "shape": (hidden_size,),
            "dtype": "float32",
        }
    ...
    memory_estimates = precision_controller.get_memory_usage_estimate(
        model_structure, precision_map
    )

    # Apply browser-specific optimizations if enabled
    browser_optimizations = {}
    if browser_specific_optimizations:
        browser_optimizations = generate_browser_specific_optimizations(
            model_type, device, model_config
        )
    ...
    logger.info(
        f"Optimized {model_type} model for WebGPU with default {precision_controller.default_bits}-bit precision"
    )
    logger.info(
        f"Memory reduction: {memory_estimates['memory_reduction_percent']:.2f}% "
...
def generate_browser_specific_optimizations(
    model_type: str, device: str, model_config: Optional[Dict] = None
    ...
    default_optimizations = {
        ...
        if "vision" in model_type.lower() or model_type.lower() in ["clip", "llava"]
        else False,
        "compute_shaders": True
        if "audio" in model_type.lower() or model_type.lower() in ["whisper", "wav2vec2", "clap"]
        else False,
    ...
    chrome_optimizations = {
        **default_optimizations,
        ...
        "thread_optimization": {"worker_threads": 4, "use_offscreen_canvas": True},
    ...
    safari_optimizations = {
        **default_optimizations,
        ...
        "thread_optimization": {"worker_threads": 1, "use_offscreen_canvas": False},
        "adaptive_precision_config": {
    ...
    if model_type.lower() in ["llama", "qwen2", "mistral"]:
        ...
        for browser in [chrome_optimizations, edge_optimizations, firefox_optimizations]:
            ...
            if browser == firefox_optimizations:
                browser["adaptive_precision_config"]["llm_optimizations"]["use_flash_attention"] = (
                    False
                )
                browser["adaptive_precision_config"]["llm_optimizations"][
                    "use_optimized_rotary_computation"
                ] = True
                browser["adaptive_precision_config"]["llm_optimizations"]["optimize_layernorm"] = (
                    True
                )
                browser["adaptive_precision_config"]["llm_optimizations"][
                    "sync_reduction_operations"
                ] = True
        ...
        for browser in [
            chrome_optimizations,
            edge_optimizations,
        ]:  # Skip Firefox due to inconsistent support
...
if __name__ == "__main__":
    ...
    print(
        f"  Reduction: {result['memory_estimates']['memory_reduction_mb']:.2f} MB "
    ...
    for browser, browser_opts in result["browser_optimizations"].items():
        ...
        print(
            f"    Memory-Efficient Attention: {browser_opts.get('memory_efficient_attention', False)}"
        )
        matrix_kernels = browser_opts.get("matrix_multiplication_kernels", {})
        if matrix_kernels:
            print(
                f"    Matrix Kernel Workgroup: {matrix_kernels.get('workgroup_size_x', 'N/A')}x{matrix_kernels.get('workgroup_size_y', 'N/A')}"
            )
