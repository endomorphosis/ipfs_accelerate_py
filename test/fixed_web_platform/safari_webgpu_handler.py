...
class MetalAPIIntegrationLayer:
    ...
    def _initialize_metal_device(self):
        ...
        major_version = (
            int(version_parts[0]) if version_parts and version_parts[0].isdigit() else 17
        )
        minor_version = (
            int(version_parts[1]) if len(version_parts) > 1 and version_parts[1].isdigit() else 6
        )
    ...
    def create_compute_pipeline(self, shader, workgroup_size, entry_point="main"):
        ...
        pipeline = {
            ...
            "threadgroup_memory_length": min(
                self.metal_device["max_threadgroup_memory_length"], 16384
            ),
            "cache_key": cache_key,
        }
    ...
    def optimize_for_model_type(self, model_type, input_shapes=None):
        ...
        if (
            "bert" in model_type.lower()
            or "t5" in model_type.lower()
            or "embedding" in model_type.lower()
        ):
            # Embedding models
            ...
            "vit" in model_type.lower()
            or "clip" in model_type.lower()
            or "vision" in model_type.lower()
        ):
            # Vision models
            optimizations.update(
                {
            ...
            "llama" in model_type.lower()
            or "llm" in model_type.lower()
            or "qwen" in model_type.lower()
        ):
            # LLMs
            optimizations.update(
                {
...
class SafariWebGPUHandler:
    ...
    def __init__(
        self, fallback_to_wasm=True, enable_metal_api=True, safari_version=None, user_agent=None
    ):
        """
        ...
        self.metal_api = None
        if self.enable_metal_api and (
            self.metal_optimizations or (self.capabilities.get("browser_version", "0") >= "17.2")
        ):
            try:
        ...
        logger.info(
            f"Initialized Safari WebGPU handler with fallback_to_wasm={fallback_to_wasm}, "
    ...
    def _map_browser_capabilities(self) -> Dict[str, Any]:
        ...
        capabilities = {
            ...
            in caps.get("special_optimizations", []),
            "quantization": {
    ...
    def should_use_fallback(self, operation_type: str) -> bool:
        ...
        if operation_type == "compute_shader" and not self.capabilities["compute_shaders"]:
            ...
            operation_type == "shader_precompilation"
            and not self.capabilities["shader_precompilation"]
        ):
            return True
            ...
            operation_type == "flash_attention"
            and not self.capabilities["memory_efficient_attention"]
        ):
            return True
    ...
    def run_native(self, operation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.metal_optimizations and self.metal_api and self._can_use_metal_api(operation_type):
            ...
            self.metrics["operations"][operation_type]["metal_count"] = (
                self.metrics["operations"][operation_type].get("metal_count", 0) + 1
            )
            self.metrics["operations"][operation_type]["metal_time_ms"] = (
                self.metrics["operations"][operation_type].get("metal_time_ms", 0) + elapsed_ms
            )
    ...
    def run_with_fallback(self, operation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation_type == "matmul":
            result = self.wasm_fallback.matrix_multiply(operation.get("a"), operation.get("b"))
        elif operation_type == "4bit_matmul":
            result = self.wasm_fallback.quantized_matrix_multiply(
                operation.get("inputs"), operation.get("weights_quantized"), operation.get("scales")
            )
    ...
    def _optimize_shader_for_metal(self, shader_code: str) -> str:
        ...
        if "reverse_bits_metal" in shader_code:
            ...
            if struct_end_index > 0:
                ...
                shader_code = (
                    shader_code[:insertion_point] + metal_compat + shader_code[insertion_point:]
                )
            else:
    ...
    def _simulate_native_operation(self, operation: Dict[str, Any]) -> Any:
        ...
        if operation_type == "matmul":
            ...
            if cols_a != rows_b:
                raise ValueError(
                    f"Matrix dimensions don't match: {rows_a}x{cols_a} and {rows_b}x{cols_b}"
                )
            ...
            return [[10.5, 11.2, 9.8], [8.7, 12.3, 10.1]]
    ...
    def _recover_from_timeout(self):
        ...
        if hasattr(self, "_current_batch_size"):
            ...
            recovery_actions.append(
                f"reduced_batch_size_{old_batch_size}_to_{self._current_batch_size}"
            )
            success = True
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        for op_type, stats in self.metrics["operations"].items():
            ...
            if op_total > 0:
                ...
                op_avg_time_native = (
                    stats["native_time_ms"] / stats["native_count"]
                    if stats["native_count"] > 0
                    else 0
                )
                op_avg_time_fallback = (
                    stats["fallback_time_ms"] / stats["fallback_count"]
                ...
                operation_metrics[op_type] = {
                    ...
                    "speedup_factor": op_avg_time_fallback / op_avg_time_native
                    if op_avg_time_native > 0
                    and stats["native_count"] > 0
    ...
    def create_optimized_pipeline(
        self, model_type: str, tensor_shapes: Dict[str, List[int]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if model_type == "bert" or model_type == "t5":
            ...
            pipeline["use_kv_cache_optimization"] = self.capabilities.get(
...
def optimize_for_safari(
    ...
    handler = SafariWebGPUHandler(
        fallback_to_wasm=fallback_to_wasm, enable_metal_api=enable_metal_api, user_agent=user_agent
    )
