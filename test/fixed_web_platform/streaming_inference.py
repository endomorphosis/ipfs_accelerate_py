...
class AdaptiveBatchSizeController:
    ...
    def _adjust_batch_size_from_history(self):
        ...
        if avg_tokens_per_second > 0 and avg_latency < self.config.get("target_latency_ms", 100):
...
class LowLatencyOptimizer:
    ...
    def optimize_token_generation(self, model, inputs, generated_tokens, generated_token_list=None):
        ...
        if generated_length == 0:
            ...
            optimizations.update({"reduce_batch_size": True, "aggressive_prefetch": True})
        else:
            # Later tokens - balance latency and throughput
            optimizations.update({"enable_batch_processing": True, "adaptive_prefetch": True})
...
class MemoryPressureMonitor:
    ...
    def initialize(self, device_info):
        ...
        logger.info(
            f"Memory monitor initialized with {self.memory_limit_mb:.2f}MB limit, "
...
class StreamingInferencePipeline:
    ...
    def __init__(self, model, config=None):
        ...
        self.latency_optimizer = LowLatencyOptimizer(config=config.get("latency_optimizer_config"))

        self.memory_monitor = MemoryPressureMonitor(config=config.get("memory_monitor_config"))
    ...
    def _on_memory_warning(self):
        ...
        """
        logger.warning(
            f"Memory usage warning: {self.memory_monitor.get_current_memory_mb():.2f}MB "
    ...
    def _on_memory_critical(self):
        ...
        """
        logger.error(
            f"Memory usage critical: {self.memory_monitor.get_current_memory_mb():.2f}MB "
            f"({self.memory_monitor.get_memory_percentage():.1f}%)"
        )
        ...
        if current_batch_size != new_batch_size:
            logger.info(
                f"Reduced batch size from {current_batch_size} to {new_batch_size} "
    ...
    async def generate_stream(self, prompt, max_tokens=100, **kwargs):
        ...
        try:
            ...
            yield {"is_summary": True, "session_summary": summary}
    ...
    def _generate_tokens(
        self, input_tokens, tokens_generated, batch_size=1, temperature=0.7, optimizations=None
    ):
        """
