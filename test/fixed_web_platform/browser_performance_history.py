...
class BrowserPerformanceHistory:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.config = {
            ...
            "browser_specific_optimizations": {
                "firefox": {
                    "audio": {"compute_shader_optimization": True, "audio_thread_priority": "high"}
                },
                "edge": {
                    "text_embedding": {"webnn_optimization": True, "quantization_level": "int8"}
                },
                "chrome": {
                    "vision": {"webgpu_compute_pipelines": "parallel", "batch_processing": True}
                },
            },
        }
    ...
    def _load_history(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(result)} performance records, "
    ...
    def record_execution(
        self, browser: str, model_type: str, model_name: str, platform: str, metrics: Dict[str, Any]
    ):
        ...
        if (
            len(self.history[browser][model_type][model_name][platform])
    ...
    def _update_recommendations_for_model(self, model_type: str, model_name: str):
        ...
        for browser in self.history:
            if (
                model_type in self.history[browser]
                and model_name in self.history[browser][model_type]
            ):
        ...
        for browser in browsers_used:
            ...
            for platform in platforms:
                ...
                platform_performance[platform] = {"metrics": metric_stats, "score": score}
        ...
        recommendation = {
            ...
            "confidence": min(
                1.0, browser_data["sample_size"] / 20
            ...
            try:
                ...
                """,
                    [
                        ...
                        model_type,
                        model_name,
                        recommendation["recommended_browser"],
        ...
        logger.info(
            f"Updated recommendation for {model_type}/{model_name}: "
            ...
            for model_type in self.history[browser]:
                ...
                for model_name in self.history[browser][model_type]:
                    ...
                    browsers_used = [
                        ...
                        if model_type in self.history[b]
                        and model_name in self.history[b][model_type]
                    ]
                ...
                normalized_ranks = [
                    (rank - 1) / (total - 1) if total > 1 else 0.5 for rank, total in model_ranks
                ]
                ...
                consistency = 1 - (
                    statistics.stdev(normalized_ranks) if len(normalized_ranks) > 1 else 0.5
                )
                ...
                logger.info(
                    f"Updated capability score for {browser} / {model_type}: "
            ...
            for model_type in list(self.history[browser].keys()):
                for model_name in list(self.history[browser][model_type].keys()):
                    for platform in list(self.history[browser][model_type][model_name].keys()):
                        ...
                        metrics_list = self.history[browser][model_type][model_name][platform]
                        filtered_metrics = [
                            m for m in metrics_list if m.get("timestamp") >= cutoff_date
                        ]
                        ...
                        if not filtered_metrics:
                            ...
                            self.history[browser][model_type][model_name][platform] = (
                                filtered_metrics
                            )
    ...
    def detect_anomalies(
        ...
        if browser in self.baselines and model_type in self.baselines[browser]:
            # Check each metric
            for metric_name in self.config["optimization_metrics"]:
    ...
    def get_browser_recommendations(
        self, model_type: str, model_name: str = None
    ) -> Dict[str, Any]:
        ...
        if model_name:
            ...
            if (
                model_type in self.recommendations
                and model_name in self.recommendations[model_type]
            ):
        ...
        if not models_of_type:
            ...
            for browser, model_types in self.capability_scores.items():
                if model_type in model_types:
                    ...
                    if score > best_score or (
                        abs(score - best_score) < 5 and confidence > highest_confidence
                    ):
            ...
            if best_browser:
                ...
                return {
                    ...
                    "sample_size": self.capability_scores[best_browser][model_type].get(
                        "sample_size", 0
                    ),
                    "config": config,
                }
        ...
        if models_of_type:
            ...
            best_browser = (
                max(browser_counts.items(), key=lambda x: x[1])[0] if browser_counts else None
            )
            best_platform = (
        ...
        default = default_recommendations.get(
            model_type, {"browser": "chrome", "platform": "webgpu"}
        )
    ...
    def get_performance_history(
        self, browser: str = None, model_type: str = None, model_name: str = None, days: int = None
    ) -> Dict[str, Any]:
                ...
                if model_type in filtered_history[b]:
                    if model_name:
                        ...
                        if model_name in filtered_history[b][model_type]:
                            # Filter by timestamp
                            filtered_models = {}
                            for platform, metrics_list in filtered_history[b][model_type][
                                model_name
                            ].items():
                                filtered_metrics = [
                                    m for m in metrics_list if m.get("timestamp") >= cutoff_date
