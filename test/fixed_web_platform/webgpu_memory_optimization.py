...
class WebGPUMemoryOptimizer:
    ...
    def allocate_tensor(self, name: str, shape: Tuple[int, ...], dtype: str) -> Any:
        ...
        self.memory_stats["peak_memory_mb"] = max(
            self.memory_stats["peak_memory_mb"], self.allocated_memory_mb
        )
        ...
        logger.debug(
            f"Allocated tensor '{name}' ({size_mb:.2f}MB), total memory: {self.allocated_memory_mb:.2f}MB"
        )
        return tensor
    ...
    def access_tensor(self, name: str) -> Any:
        ...
        if tensor_info["location"] == "cpu" and self.offload_cpu:
            ...
            tensor_info["tensor"] = self._cpu_to_gpu_tensor(
                tensor_info["tensor"], tensor_info["shape"], tensor_info["dtype"]
            )
            ...
            logger.debug(
                f"Moved tensor '{name}' back to GPU ({size_mb:.2f}MB), total memory: {self.allocated_memory_mb:.2f}MB"
            )
    ...
    def free_tensor(self, name: str) -> bool:
        ...
        logger.debug(
            f"Freed tensor '{name}' ({tensor_info['size_mb']:.2f}MB), total memory: {self.allocated_memory_mb:.2f}MB"
        )
        return True
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_utilization": self.allocated_memory_mb / self.total_memory_mb
    ...
    def _offload_least_recently_used(self, required_mb=0, exclude_names=None):
        ...
        if not self.offload_cpu:
            raise MemoryError(
                f"WebGPU memory limit exceeded. Need additional {required_mb}MB but CPU offloading is disabled."
            )
        ...
        sorted_tensors = [
            (name, info)
            for name, info in self.cached_tensors.items()
        ...
        for name, info in sorted_tensors:
            ...
            tensor = info["tensor"]
            self.cached_tensors[name]["tensor"] = self._gpu_to_cpu_tensor(
        ...
        if offloaded_tensors:
            logger.debug(
                f"Offloaded {len(offloaded_tensors)} tensors to CPU, freed {freed_mb:.2f}MB"
            )
...
class ProgressiveTensorLoader:
            ...
            for layer_idx, layer_info in layers.items():
                ...
                for name, tensor_info in layer_info["tensors"].items():
                    loading_plan["layers"][layer_idx]["tensors"][name] = {
                        "shape": tensor_info["shape"],
                        "dtype": tensor_info["dtype"],
                        "chunks": self._plan_tensor_chunks(
    ...
    def ensure_tensor_loaded(self, name, priority=0):
        ...
        if not self.enable_streaming:
            ...
            if loaded_count < chunk_count:
                ...
                pending_chunks = [
                    i for i in range(chunk_count) if i not in chunk_info["loaded_chunks"]
                ]
    ...
    def _plan_tensor_chunks(self, shape, dtype):
        ...
        for i in range(num_chunks):
            ...
            chunks.append({"start_idx": start_idx, "end_idx": end_idx, "size_mb": chunk_size_mb})
    ...
    def _load_tensor_chunk(self, name, chunk_idx):
        ...
        if chunk_idx >= len(chunks):
            raise ValueError(
                f"Invalid chunk index {chunk_idx}, tensor '{name}' has {len(chunks)} chunks"
            )
        ...
        logger.debug(
            f"Loaded chunk {chunk_idx} of tensor '{name}', {len(chunk_info['loaded_chunks'])}/{len(chunks)} chunks loaded"
        )
...
class WebGPUAttentionOptimizer:
    ...
    def optimize_attention_for_webgpu(self, model_config):
        ...
        if seq_length > max_seq_length:
            if seq_length <= 8192:
                ...
                logger.info(
                    f"Enabling sliding window attention with window size {sliding_window_size}"
                )
            else:
    ...
    def setup_kv_cache(self, batch_size, num_heads, head_dim, max_seq_length):
        ...
        logger.info(
            f"Initialized KV cache for sequence length {max_seq_length}, "
...
def optimize_model_for_webgpu(model, config=None, device="webgpu"):
    ...
    memory_optimizer = WebGPUMemoryOptimizer(
        total_memory_mb=memory_limit, offload_cpu=enable_offload
    )
    ...
    attention_optimizer = WebGPUAttentionOptimizer(
        max_memory_mb=memory_limit * 0.8
    )  # Use 80% of memory for attention
    ...
    if model_type in ["bert", "roberta"]:
        ...
        model_structure["embeddings"] = {
            "word_embeddings": {
                "shape": (config.get("vocab_size", 30522), hidden_size),
                "dtype": "float32",
            },
            "position_embeddings": {"shape": (seq_length, hidden_size), "dtype": "float32"},
            "token_type_embeddings": {
                "shape": (config.get("type_vocab_size", 2), hidden_size),
                "dtype": "float32",
            },
        ...
        model_structure["embeddings"] = {
            "word_embeddings": {
                "shape": (config.get("vocab_size", 50257), hidden_size),
                "dtype": "float32",
            },
        }
        ...
        if model_type == "gpt2":
            model_structure["embeddings"]["position_embeddings"] = {
        ...
        model_structure["embeddings"] = {
            "shared_embeddings": {
                "shape": (config.get("vocab_size", 32128), hidden_size),
                "dtype": "float32",
            },
        }
    ...
    for i in range(num_hidden_layers):
        ...
        layer_struct["tensors"]["attention_q"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention_k"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention_v"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["attention_out"] = {
            "shape": (hidden_size, hidden_size),
            "dtype": "float32",
        }

        # Add MLP components
        layer_struct["tensors"]["mlp_in"] = {
            "shape": (hidden_size, 4 * hidden_size),
            "dtype": "float32",
        }
        layer_struct["tensors"]["mlp_out"] = {
    ...
    logger.info(
        f"Optimized model for WebGPU with max sequence length: {attention_config['max_seq_length']}"
    )
    if enable_streaming:
