...
class ConnectionPoolManager:
    ...
    def __init__(
        ...
        self.connections_by_browser = {"chrome": {}, "firefox": {}, "edge": {}, "safari": {}}
        self.connections_by_platform = {"webgpu": {}, "webnn": {}, "cpu": {}}
        ...
        logger.info(
            f"Connection Pool Manager initialized with {min_connections}-{max_connections} connections"
        )
    ...
    async def initialize(self):
        ...
        with self.lock:
            ...
            try:
                ...
                logger.info(
                    f"Connection Pool Manager initialized with {len(self.connections)} connections"
                )
                return True
    ...
    def _start_background_tasks(self):
        ...
        logger.info(
            f"Started background tasks (health check: {self.health_check_interval}s, cleanup: {self.cleanup_interval}s)"
        )
    ...
    async def _create_initial_connection(self):
        ...
        try:
            ...
            logger.info(
                f"Created initial connection: id={connection_id}, browser={browser}, platform={platform}"
            )
            return True
        except Exception as e:
        ...
        with self.lock:
            ...
            for conn_id, conn in self.connections.items():
                if conn["browser"] == browser and conn["platform"] == platform:
                    ...
                    if conn["status"] == "ready" and conn["health_status"] in [
                        "healthy",
                        "degraded",
                    ]:
            ...
            if matching_connections:
                ...
                logger.info(
                    f"Using existing connection {conn_id} for {model_type} model ({browser}/{platform})"
                )
            ...
            if current_connections >= self.max_connections:
                ...
                logger.warning(
                    f"At max connections ({current_connections}/{self.max_connections}), finding best available"
                )
                ...
                for conn_id, conn in self.connections.items():
                    if conn["status"] == "ready" and conn["health_status"] in [
                        ...
                        logger.info(
                            f"Using non-optimal connection {conn_id} ({conn['browser']}/{conn['platform']}) for {model_type}"
                        )
            ...
            if self.adaptive_manager:
                ...
                self.adaptive_manager.update_metrics(
                    ...
                    active_connections=sum(
                        ...
                        if c["last_used_time"] > time.time() - 300
                    ),
                    ...
                    browser_counts={
                        b: len(conns) for b, conns in self.connections_by_browser.items()
                    },
    ...
    async def _check_connection_health(self):
        ...
        with self.lock:
            ...
            if health_stats["unhealthy"] > 0:
                logger.warning(
                    f"Connection health: {health_stats['healthy']} healthy, {health_stats['degraded']} degraded, {health_stats['unhealthy']} unhealthy"
                )
            else:
                logger.info(
                    f"Connection health: {health_stats['healthy']} healthy, {health_stats['degraded']} degraded"
                )

            # Check if we need to scale connections based on health
            if (
                health_stats["unhealthy"] > 0
                and health_stats["total"] - health_stats["unhealthy"] < self.min_connections
            ):
    ...
    async def _cleanup_connections(self):
        ...
        with self.lock:
            ...
            if self.adaptive_manager:
                ...
                if recommended_connections is not None and recommended_connections != len(
                    self.connections
                ):
                    if recommended_connections > len(self.connections):
                        ...
                        logger.info(
                            f"Adaptive scaling: removing {to_remove} connections ({reason})"
                        )
                        ...
                        for conn_id, conn in sorted(
                            ...
                            if (
                                time.time() - conn["last_used_time"] < 300
            ...
            for conn_id, conn in list(self.connections.items()):
                ...
                if time.time() - conn["last_used_time"] > 1800:  # 30 minutes
                    ...
                    if len(self.connections) > self.min_connections:
                        logger.info(
                            f"Cleaning up idle connection {conn_id} (idle for {(time.time() - conn['last_used_time']) / 60:.1f} minutes)"
                        )
                        await self._close_connection(conn_id)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            status_counts = {"ready": 0, "initializing": 0, "error": 0, "closing": 0}
            ...
            browser_counts = {
                browser: len(conns) for browser, conns in self.connections_by_browser.items()
            }
            platform_counts = {
                platform: len(conns) for platform, conns in self.connections_by_platform.items()
            }

            # Get adaptive scaling stats
            adaptive_stats = (
                self.adaptive_manager.get_scaling_stats() if self.adaptive_manager else {}
            )
...
if __name__ == "__main__":
    ...
    async def test_pool():
        ...
        pool = ConnectionPoolManager(min_connections=1, max_connections=4, adaptive_scaling=True)
