...
def detect_browser_capabilities(user_agent: Optional[str] = None) -> Dict[str, Any]:
    ...
    logger.info(
        f"Detected browser: {capabilities['browser_name']} {capabilities['browser_version']}"
    )
...
def _parse_browser_info(user_agent: str) -> Dict[str, Any]:
    ...
    """
    browser_info = {"browser_name": "Unknown", "browser_version": 0}
...
def _parse_platform_info(user_agent: str) -> Dict[str, Any]:
    ...
    """
    platform_info = {"platform": "Unknown", "os_version": "Unknown", "is_mobile": False}
...
def _determine_recommended_backend(capabilities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if capabilities["webgpu_supported"]:
        ...
        if capabilities["browser_name"] == "Safari" and capabilities["metal_api_supported"]:
...
def is_safari_with_metal_api(capabilities: Dict[str, Any]) -> bool:
    ...
    """
    return capabilities["browser_name"] == "Safari" and capabilities["metal_api_supported"]


def get_optimized_config(
    model_name: str, browser_capabilities: Dict[str, Any], model_size_mb: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    if memory_ratio > 2.0:
        ...
        config["use_quantization"] = browser_capabilities["webnn_features"].get(
            "quantized_operations", False
        )
    ...
    if (
        ...
        and browser_capabilities["webgpu_supported"]
        and browser_capabilities["webgpu_features"]["compute_shaders"]
    ):
...
if __name__ == "__main__":
    ...
    for ua in user_agents:
        ...
        for model in ["bert-base-uncased", "llama-7b"]:
            ...
            print(
                f"  Special optimizations: {', '.join(config['special_optimizations']) if config['special_optimizations'] else 'None'}"
            )
