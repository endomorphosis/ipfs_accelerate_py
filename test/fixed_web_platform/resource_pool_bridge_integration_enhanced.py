...
from test.web_platform.performance_trend_analyzer import (
...
class ResourcePoolBridgeIntegrationEnhanced:
    ...
    def __init__(
        ...
        self.enable_performance_trend_analysis = (
            enable_performance_trend_analysis and PERFORMANCE_ANALYZER_AVAILABLE
        )

        # Initialize logger
        logger.info(
            f"ResourcePoolBridgeIntegrationEnhanced created with max_connections={max_connections}, "
        ...
        try:
            ...
            if self.enable_performance_trend_analysis and PERFORMANCE_ANALYZER_AVAILABLE:
                ...
                self.performance_analyzer = PerformanceTrendAnalyzer(
                    db_path=self.db_path, logger=logger
                )
            ...
            if self.enable_circuit_breaker and CIRCUIT_BREAKER_AVAILABLE:
                ...
                if browser_connections:
                    ...
                    self.circuit_breaker_manager = BrowserCircuitBreakerManager(
                        logger=logger, db_path=self.db_path, enable_performance_history=True
                    )
            ...
            if (
                ...
                and ADVANCED_POOLING_AVAILABLE
                and not hasattr(self.bridge, "connection_pool")
            ):
            ...
            logger.info(
                f"ResourcePoolBridgeIntegrationEnhanced initialized successfully "
                f"(recovery={'enabled' if self.enable_recovery else 'disabled'}, "
                            ...
                            if recommendation.get("confidence", 0) >= 0.6:
                                ...
                                if "browser" not in hardware_preferences:
                                    ...
                                    if recommended_browser:
                                        ...
                                        logger.info(
                                            f"Using trend analyzer recommended browser '{recommended_browser}' for {model_type}/{model_name} "
                                ...
                                if (
                                    ...
                                    if recommended_browser:
                                        ...
                                        logger.info(
                                            f"Using platform priority list for {recommended_browser}: {hardware_preferences['priority_list']}"
                                        )

                                # Check for regressions that should affect our strategy
                                model_trends = self.performance_analyzer.analyze_model_trends(
                                    model_name=model_name, time_window_days=7.0
                                )

                                if model_trends and "latency" in model_trends:
                                    latency_trend = model_trends["latency"]
                                    if (
                                        latency_trend.direction == TrendDirection.DEGRADING
                                        and latency_trend.regression_severity
                                        in [RegressionSeverity.SEVERE, RegressionSeverity.CRITICAL]
                                    ):
                                        # Severe regression detected, apply mitigation
                                        logger.warning(
                                            f"Severe performance regression detected for {model_name}: {latency_trend.percent_change:.1f}% degradation"
                                        )
                                        ...
                                        if "fallback_threshold" not in hardware_preferences:
                                            hardware_preferences["fallback_threshold"] = (
                                                1.5  # Lower threshold for faster fallback
                                            )

                                        # Increase retry limit for degraded model
                                        if "retry_limit" not in hardware_preferences:
                                            hardware_preferences["retry_limit"] = (
                                                2  # Lower retry limit to fail faster
                                            )
                    except Exception as e:
                        logger.warning(
                            f"Error using performance trend analyzer for browser selection: {e}"
                        )
                ...
                if "browser" not in (hardware_preferences or {}):
                    try:
                        ...
                        if basic_recommendation:
                            ...
                            # Add recommended browser if not explicitly specified by user
                            if "browser" not in hardware_preferences:
                                recommended_browser = basic_recommendation.get(
                                    "recommended_browser"
                                )
                                if recommended_browser:
                                    hardware_preferences["browser"] = recommended_browser
                                    logger.info(
                                        f"Using browser history recommended browser '{recommended_browser}' for {model_type}/{model_name}"
                                    )
                            ...
                            if (
                                ...
                                recommended_platform = basic_recommendation.get(
        ...
        if self.enable_circuit_breaker and self.circuit_breaker_manager:
            try:
                ...
                if browser_health:
                    ...
                    requested_browser = (
                        hardware_preferences.get("browser") if hardware_preferences else None
                    )
                    ...
                    if requested_browser:
                        ...
                        for (
                            browser_id,
                            circuit,
                        ) in self.circuit_breaker_manager.circuit_breakers.items():
                            if (
                                circuit.name.startswith(f"{requested_browser}_")
                        ...
                        if open_circuits:
                            ...
                            logger.warning(
                                f"All {requested_browser} browser circuits are OPEN. Looking for alternative browser."
                            )

                            # Get browser recommendations
                            if self.performance_analyzer:
                                recommendations = (
                                    self.performance_analyzer.get_browser_recommendations(
                                        force_refresh=True
                                    )
                                )
                                ...
                                model_type_key = (
                                    model_type if model_type in recommendations else "any"
                                )
                                if model_type_key in recommendations:
                                    alternative_browsers = [
                                        b["browser_type"]
                                        for b in recommendations[model_type_key].get(
                                            "all_browsers", []
                                        )
                                    ...
                                    if alternative_browsers:
                                        ...
                                        logger.info(
                                            f"Circuit breaker selects alternative browser: {browser_to_use}"
                                        )
        ...
        if model is not None:
                ...
                try:
                    ...
                    if browser and platform:
                        try:
                            ...
                            logger.warning(
                                f"Error recording model instantiation in browser history: {e}"
                            )
                except Exception as e:
            ...
            if self.enable_performance_trend_analysis and self.performance_analyzer:
                try:
                    ...
                    logger.warning(
                        f"Error recording model instantiation in performance analyzer: {e}"
                    )
    ...
    def execute_concurrent(
        ...
        if (
            ...
            and self.circuit_breaker_manager
            and self.enable_performance_trend_analysis
            and self.performance_analyzer
        ):
            try:
                ...
                for i, (model, inputs) in enumerate(model_and_inputs_list):
                    ...
                    circuit = self.circuit_breaker_manager.get_or_create_circuit(
                    ...
                    if circuit.state == "open":
                        logger.warning(
                            f"Circuit {circuit.name} is OPEN. Model {i} will likely fail."
                        )
                        ...
                        logger.info(
                            f"Circuit {circuit.name} is HALF-OPEN. Model {i} will be tested carefully."
                        )
                        ...
                        try:
                            ...
                            if model_trends and "latency" in model_trends:
                                ...
                                if latency_trend.direction == TrendDirection.DEGRADING:
                                    logger.warning(
                                        f"Model {model_name} on {browser_type} shows {latency_trend.percent_change:.1f}% "
                ...
                status["performance_analyzer"] = {
                    ...
                    "summary": {
                        ...
                        "regression_count": sum(
                            len(v) for v in performance_report.get("regressions", {}).values()
                        ),
                        "critical_regressions": len(
                            performance_report.get("regressions", {}).get("critical", [])
                        ),
                        "severe_regressions": len(
                            performance_report.get("regressions", {}).get("severe", [])
                        ),
                    },
                }
    ...
    def detect_performance_regressions(
        self, time_window_days: float = 7.0, threshold_pct: float = 10.0
        ...
        try:
            # Get regressions
            regressions = self.performance_analyzer.detect_regressions(
                time_window_days=time_window_days, threshold_pct=threshold_pct
            )
            return regressions
        except Exception as e:
    ...
    def get_browser_recommendations(
        self, time_window_days: float = 7.0
        ...
        try:
            # Get recommendations
            recommendations = self.performance_analyzer.get_browser_recommendations(
                time_window_days=time_window_days, force_refresh=True
            )
    ...
    def close(self) -> bool:
        ...
        self.initialized = False
        logger.info(
            f"ResourcePoolBridgeIntegrationEnhanced closed (success={'yes' if success else 'no'}, "
...
def run_example():
    ...
    try:
        ...
        if pool.performance_analyzer:
            ...
            logging.info(
                f"Detected {critical_count} critical, {severe_count} severe, "
            ...
            for model_type, recommendation in recommendations.items():
                logging.info(
                    f"Recommended browser for {model_type}: {recommendation.get('recommended_browser', 'unknown')} "
                    f"(confidence: {recommendation.get('confidence', 0):.2f})"
                )
        ...
        if "circuit_breaker" in health:
            ...
            logging.info(
                f"Circuit breaker health score: {circuit_health.get('overall_health_score', 0):.1f}"
            )
        ...
        if "performance_analyzer" in health:
            ...
            if "summary" in pa_status:
                ...
                logging.info(
                    f"Performance analyzer: {summary.get('record_count', 0)} records, "
                    ...
                    if critical:
                        ...
                        for reg in critical[:2]:  # Show first 2
                            if "model_name" in reg:
                                logging.warning(
                                    f"  - Model {reg['model_name']}: {reg['percent_change']:.1f}% degradation"
                                )
                            elif "browser_type" in reg:
                                logging.warning(
                                    f"  - Browser {reg['browser_type']}: {reg['percent_change']:.1f}% degradation"
                                )
