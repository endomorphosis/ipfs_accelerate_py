...
class PerformanceTrendAnalyzer:
    ...
    def __init__(
        ...
        self.logger.info(
            f"PerformanceTrendAnalyzer initialized (using {'DuckDB' if db_path else 'memory'} storage)"
        )
    ...
    def _initialize_db(self, db_path: str):
        ...
        try:
            ...
            self.db_conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_performance_records_browser_id ON performance_records(browser_id)"
            )
            self.db_conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_performance_records_model_name ON performance_records(model_name)"
            )
        ...
        if self.db_conn:
            try:
                self.db_conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (browser_id, record.browser_type, json.dumps(stats), stats["last_updated"]),
                )
            except Exception as e:
        ...
        if self.db_conn:
            try:
                self.db_conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (model_name, record.model_type, json.dumps(stats), stats["last_updated"]),
                )
            except Exception as e:
    ...
    def analyze_model_trends(
        ...
        if not records:
            self.logger.info(
                f"No records found for model {model_name} in the specified time window"
            )
            return {}
        ...
        if len(x) > 1:  # Need at least 2 points for regression
            ...
            if all_records:
                ...
                error_daily_stats = (
                    ...
                    .agg(success_rate=("success_int", "mean"), count=("success_int", "count"))
                    .reset_index()
                )
                ...
                if len(xe) > 1:
                    ...
                    # Calculate percent change
                    e_percent_change = (
                        (e_current_value - e_baseline_value) / max(0.001, e_baseline_value)
                    ) * 100
    ...
    def analyze_browser_trends(
        ...
        if not records:
            self.logger.info(
                f"No records found for browser {browser_type} in the specified time window"
            )
            return {}
        ...
        if len(x) > 1:  # Need at least 2 points for regression
            ...
            # Calculate percent change
            l_percent_change = (
                ((l_current_value - l_baseline_value) / l_baseline_value) * 100
                if l_baseline_value > 0
                else 0
            )
            ...
            if len(daily_stats) > 1:
                ...
                t_slope, t_intercept, t_r_value, t_p_value, t_std_err = stats.linregress(
                    x, y_throughput
                )
                ...
                # Calculate percent change
                t_percent_change = (
                    ((t_current_value - t_baseline_value) / t_baseline_value) * 100
                    if t_baseline_value > 0
                    else 0
                )
    ...
    def get_browser_recommendations(
        self, time_window_days: float = 7.0, min_sample_size: int = 5, force_refresh: bool = False
        ...
        if (
            not force_refresh
            and self._recommendations_cache
            and time.time() - self._recommendations_timestamp < cache_max_age
        ):
        ...
        if self.db_conn:
            try:
                ...
                # Fall back to in-memory records
                records = [
                    r for r in self.records if r.success and start_time <= r.timestamp <= end_time
                ]
        ...
        model_type_browser_stats = (
            df.groupby(["model_type", "browser_type"])
            .agg(
        ...
        model_type_browser_stats = model_type_browser_stats[
            model_type_browser_stats["count"] >= min_sample_size
        ]

        # Find the best browser for each model type
        recommendations = {}
        for model_type in model_type_browser_stats["model_type"].unique():
            model_stats = model_type_browser_stats[
                model_type_browser_stats["model_type"] == model_type
            ]
            ...
            if len(model_stats) > 1:
                ...
                performance_diff = (
                    second_best["avg_duration"] - best_browser["avg_duration"]
                ) / second_best["avg_duration"]
                performance_factor = min(
                    1.0, performance_diff * 5
                )  # Scale up for small differences
            else:
        ...
        if len(recommendations) > 0:
            ...
            browser_stats = (
                ...
                .agg(avg_duration=("duration_ms", "mean"), count=("duration_ms", "count"))
                .reset_index()
            )
            ...
            if not browser_stats.empty:
                ...
                if len(browser_stats) > 1:
                    ...
                    performance_diff = (
                        second_best["avg_duration"] - best_overall["avg_duration"]
                    ) / second_best["avg_duration"]
                ...
                percent_better = (
                    (avg_duration - best_overall["avg_duration"]) / avg_duration
                ) * 100
    ...
    def get_model_type_overview(self, time_window_days: float = 7.0) -> Dict[str, Dict[str, Any]]:
        ...
        if self.db_conn:
            ...
            df = pd.DataFrame(
                [
                    {"model_type": r.model_type, "duration_ms": r.duration_ms, "success": r.success}
                    for r in records
                ]
            )
    ...
    def detect_regressions(
        ...
        regressions = {"critical": [], "severe": [], "moderate": [], "minor": []}

        for model_name in model_names:
            model_trends = self.analyze_model_trends(
                model_name=model_name, time_window_days=time_window_days
            )

            for metric_type, trend in model_trends.items():
                if (
                    trend.direction == TrendDirection.DEGRADING
                    and trend.regression_severity != RegressionSeverity.NONE
                    and abs(trend.percent_change) >= threshold_pct
                    and trend.sample_count >= 3
                ):
                    regression_data = {
                        "model_name": model_name,
        ...
        for browser_type in browser_types:
            browser_trends = self.analyze_browser_trends(
                browser_type=browser_type, time_window_days=time_window_days
            )
