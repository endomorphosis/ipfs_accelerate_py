...
class ResultFormatter:
    ...
    def add_performance_metrics(
        ...
        if all(
            key in result["performance"]
            for key in ["inference_time_ms", "preprocessing_time_ms", "postprocessing_time_ms"]
        ):
    ...
    def update_progressive_result(
        self, result: Dict[str, Any], update: Dict[str, Any], progress: float
    ) -> Dict[str, Any]:
        """
                    ...
                    for key, value in result["performance"].items():
                        if key in merged["performance"]:
                            ...
                            if isinstance(value, (int, float)) and isinstance(
                                merged["performance"][key], (int, float)
                            ):
                                merged["performance"][key] = (
...
def format_inference_result(
    ...
    formatter = ResultFormatter(
        model_type=model_type, browser=browser, include_metadata=include_metadata
    )
...
def format_error_response(
    ...
    Returns:
        Formatted error response
    """
    formatter = ResultFormatter(model_type=model_type, browser=browser)
