...
class UnifiedWebPlatform:
    ...
    def __init__(
        ...
        **kwargs,
            ...
            try:
                ...
                if browser_info and browser_info.get("name", "").lower() == "safari":
                    ...
                    self.webgpu_handler = SafariWebGPUHandler(
                        model_path=self.model_name, config=self.config
                    )
                else:
                    self.webgpu_handler = WebPlatformHandler(
                        model_path=self.model_name, model_type=self.model_type, config=self.config
                    )
        ...
        self._perf_metrics["feature_usage"] = {
            ...
            "safari_handler": hasattr(self, "webgpu_handler")
            and "SafariWebGPUHandler" in self.webgpu_handler.__class__.__name__,
        }

        logger.info(
            f"Unified Web Platform initialized for {model_name} on {platform} in {self._perf_metrics['initialization_time_ms']:.2f}ms"
        )
        ...
        try:
            ...
            if self.model_sharding and self.config.get("use_model_sharding", False):
                ...
                if (
                    ...
                    and hasattr(self, "webgpu_handler")
                    and self.webgpu_handler
                ):
                    try:
                        raw_output = self.webgpu_handler.run_inference(processed_input)
                    except Exception as e:
                        logger.warning(
                            f"WebGPU inference failed: {str(e)}, falling back to WebAssembly"
                        )
                        if self.wasm_fallback:
                            raw_output = self.wasm_fallback.run_inference(processed_input)
                        else:
                            raise RuntimeError(
                                f"WebGPU inference failed and no fallback available: {str(e)}"
                            )
                elif (
                    self.platform == "webnn"
                    and hasattr(self, "webnn_handler")
                    and self.webnn_handler
                ):
                    try:
                        raw_output = self.webnn_handler.run_inference(processed_input)
                    except Exception as e:
                        logger.warning(
                            f"WebNN inference failed: {str(e)}, falling back to WebAssembly"
                        )
                        if self.wasm_fallback:
                            raw_output = self.wasm_fallback.run_inference(processed_input)
                        else:
                            raise RuntimeError(
                                f"WebNN inference failed and no fallback available: {str(e)}"
                            )
            ...
            if self.result_formatter:
                ...
                result = {"success": True, "output": raw_output, "error": None}
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        if self.config_manager:
            ...
            return {"valid": True, "errors": [], "auto_corrected": False, "config": self.config}
    ...
    def get_browser_compatibility(self) -> Dict[str, Any]:
        ...
        if self.platform_detector:
            ...
            return {"browser": "unknown", "compatibility": {}}
