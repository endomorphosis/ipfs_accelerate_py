...
class FallbackManager:
    ...
    def __init__(
        ...
        if MODULES_AVAILABLE:
            self.wasm_fallback = WebAssemblyFallback(
                enable_simd=True, enable_threading=True, memory_optimization=True
            )
        ...
        logger.info(
            f"FallbackManager initialized for {self.browser_info.get('name', 'unknown browser')}"
        )
        if self.is_safari:
    ...
    def needs_fallback(self, operation_name: str) -> bool:
        ...
        if operation_info.get("memory_intensive", False):
            ...
            if current_memory > self.memory_threshold:
                logger.info(
                    f"Memory threshold exceeded ({current_memory:.2f}), using fallback for {operation_name}"
                )
                return True
        ...
        try:
            ...
            if self.needs_fallback(operation_name):
                ...
                # Use WASM fallback for other browsers or if Safari fallback fails
                    ...
                    result = self.wasm_fallback.execute_operation(operation_name, inputs, context)
                else:
                    ...
                    if callable(operation):
                        ...
                        raise ValueError(
                            f"Operation {operation_name} requires fallback, but none available"
                        )
            else:
            ...
            if self.wasm_fallback:
                try:
                    logger.warning(
                        f"Operation {operation_name} failed, using emergency WASM fallback"
                    )
    ...
    def _get_current_memory_usage(self) -> float:
        ...
        operations_factor = min(
            0.3, 0.01 * (self.metrics["fallback_activations"] + self.metrics["native_operations"])
        )
...
class SafariWebGPUFallback:
    ...
    def __init__(
        ...
        try:
            ...
            self.safari_handler = SafariWebGPUHandler(fallback_to_wasm=True, enable_metal_api=True)
        except ImportError:
    ...
    def needs_fallback(self, operation_name: str) -> bool:
        ...
        if operation_name == "matmul_4bit" and not self.metal_features.get(
            "partial_4bit_support", False
        ):
            return True

        if operation_name == "kv_cache_update" and not self.metal_features.get(
            "partial_kv_cache_optimization", False
        ):
            return True
    ...
    def _layer_decomposition_strategy(
        ...
        if self.enable_layer_processing:
            ...
            if self.wasm_fallback:
                ...
                raise ValueError(
                    "Layer processing is disabled and no WebAssembly fallback available"
                )

    def _chunked_attention_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _partitioned_cache_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _head_partitioning_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _progressive_quantization_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _simplified_shader_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _chunked_embedding_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
    ...
    def _tiled_extraction_strategy(
        self, inputs: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Any:
        """
...
def create_optimal_fallback_strategy(
    ...
    if model_type == "text":
        strategy.update(
            {"chunk_size": 256, "use_token_pruning": True, "enable_cache_optimization": True}
        )
    elif model_type == "vision":
        strategy.update(
            {"use_tiled_processing": True, "tile_size": 224, "enable_feature_caching": True}
        )
    ...
    if operation_type == "attention":
        ...
        strategy.update(
            {"use_blocked_matmul": True, "block_size": 256, "use_mixed_precision": True}
        )
    ...
    if is_safari:
        ...
        if safari_version < 16.0:
            strategy.update(
                {
                    "chunk_size": max(
                        64, strategy["chunk_size"] // 2
                    ),  # Reduce chunk size for older Safari
