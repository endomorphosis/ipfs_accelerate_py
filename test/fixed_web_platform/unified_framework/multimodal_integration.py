...
def optimize_model_for_browser(
    ...
    # Optimize the model
    logger.info(
        f"Optimizing {model_name} for {browser} with {memory_constraint_mb}MB memory constraint"
    )
...
def get_best_multimodal_config(
    ...
    model_preset = MODEL_FAMILY_PRESETS.get(
        model_family, {"modalities": ["vision", "text"], "recommended_optimizations": {}}
    )
...
def configure_for_low_memory(base_config: Dict[str, Any], target_memory_mb: int) -> Dict[str, Any]:
    """
...
class MultimodalWebRunner:
    ...
    def __init__(
        ...
        self.memory_constraint_mb = memory_constraint_mb or get_browser_memory_constraint(
            self.browser
        )
    ...
    async def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for modality in self.modalities:
            ...
            if modality == Modality.AUDIO or (
                isinstance(modality, str) and modality.lower() == "audio"
            ):
                has_audio = True
                break
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if self.performance_history:
            avg_time = sum(p["total_time_ms"] for p in self.performance_history) / len(
                self.performance_history
            )
            avg_memory = sum(p["memory_usage_mb"] for p in self.performance_history) / len(
                self.performance_history
            )
