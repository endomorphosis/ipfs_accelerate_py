...
class ModelShardingManager:
    ...
    def initialize_sharding(self) -> bool:
        ...
        if len(self.active_shards) < self.num_shards:
            logger.warning(
                f"Only {len(self.active_shards)} of {self.num_shards} shards initialized"
            )
    ...
    def _initialize_shard(self, shard_config: ShardConfiguration) -> bool:
        ...
        logger.info(
            f"Initializing shard {shard_config.shard_index} with {len(shard_config.layer_indices)} layers"
        )
    ...
    def _run_shard_inference(
        self, shard_config: ShardConfiguration, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
