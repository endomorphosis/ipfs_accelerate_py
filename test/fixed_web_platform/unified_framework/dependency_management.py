...
class DependencyManager:
    ...
    def check_core_dependencies(self) -> bool:
        ...
        for name, package in self.CORE_DEPENDENCIES.items():
            try:
                ...
                logger.debug(
                    f"Core dependency {name} is available (version: {self.available_dependencies[name]['version']})"
                )
            except ImportError:
                all_available = False
    ...
    def check_optional_dependency(self, name: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Optional dependency {name} is available (version: {self.available_dependencies[name]['version']})"
            )
            ...
            if "additional_imports" in dep_info:
                for additional_import in dep_info["additional_imports"]:
                    try:
                        ...
                        logger.warning(
                            f"Additional import {additional_import} for {name} is not available"
                        )
            ...
            logger.info(
                f"Optional dependency {name} is not available. To install: {dep_info['installation']}"
            )
            return False
    ...
    def get_installation_instructions(self, missing_deps: List[str] = None) -> str:
        ...
        for dep in missing_deps:
            if (
                dep in self.missing_dependencies
                and "installation" in self.missing_dependencies[dep]
            ):
        ...
        for feature, dependencies in self.FEATURE_DEPENDENCIES.items():
            ...
            status[feature] = {
                ...
                "available_dependencies": [
                    dep for dep in dependencies if dep in self.available_dependencies
                ],
            }
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if missing:
                ...
                if fallback:
                    logger.warning(
                        f"Missing dependencies for {func.__name__}: {', '.join(missing)}, continuing with fallback"
                    )
