...
class MetricsCollector:
    ...
    def __init__(
        self, storage_path: Optional[str] = None, retention_days: int = 30, auto_save: bool = True
    ):
        """
    ...
    def record_inference(
        ...
        metric = {
            ...
            "throughput_items_per_second": 1000 * batch_size / inference_time_ms
    ...
    def _apply_retention_policy(self) -> None:
        ...
        # Filter metrics
        self.inference_metrics = [
            m for m in self.inference_metrics if m["timestamp"] >= cutoff_timestamp
        ]

        self.initialization_metrics = [
            m for m in self.initialization_metrics if m["timestamp"] >= cutoff_timestamp
        ]

        self.memory_metrics = [m for m in self.memory_metrics if m["timestamp"] >= cutoff_timestamp]

        self.feature_usage_metrics = [
            m for m in self.feature_usage_metrics if m["timestamp"] >= cutoff_timestamp
        ]

        logger.info(f"Applied retention policy: {self.retention_days} days")

    def get_model_performance(
        self, model_name: str, platform: Optional[str] = None, browser: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        inference_metrics = self._filter_metrics(
            self.inference_metrics, model_name=model_name, platform=platform, browser=browser
        )

        initialization_metrics = self._filter_metrics(
            self.initialization_metrics, model_name=model_name, platform=platform, browser=browser
        )

        memory_metrics = self._filter_metrics(
            self.memory_metrics, model_name=model_name, platform=platform, browser=browser
        )

        # Calculate average metrics
        avg_inference_time = self._calculate_average(inference_metrics, "inference_time_ms")

        avg_initialization_time = self._calculate_average(
            initialization_metrics, "initialization_time_ms"
        )

        avg_memory = self._calculate_average(memory_metrics, "memory_mb")

        avg_throughput = self._calculate_average(inference_metrics, "throughput_items_per_second")
        ...
        return {
            ...
            "historical_data": {
                ...
                "initialization_times": [
                    m.get("initialization_time_ms") for m in initialization_metrics
                ],
                "memory_usage": [m.get("memory_mb") for m in memory_metrics],
                "dates": [
                    m.get("date")
                    for m in sorted(
                        inference_metrics + initialization_metrics, key=lambda x: x["timestamp"]
                    )
                ],
            },
        }
    ...
    def get_platform_comparison(self, model_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        for platform in platforms:
            ...
            platform_inference = self._filter_metrics(
                self.inference_metrics, model_name=model_name, platform=platform
            )

            platform_initialization = self._filter_metrics(
                self.initialization_metrics, model_name=model_name, platform=platform
            )

            platform_memory = self._filter_metrics(
                self.memory_metrics, model_name=model_name, platform=platform
            )
            ...
            result["memory_mb"][platform] = self._calculate_average(platform_memory, "memory_mb")
    ...
    def get_browser_comparison(
        self, model_name: Optional[str] = None, platform: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for browser in browsers:
            ...
            result["memory_mb"][browser] = self._calculate_average(browser_memory, "memory_mb")
    ...
    def get_feature_usage_statistics(self, browser: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        # Filter metrics
        feature_metrics = self._filter_metrics(self.feature_usage_metrics, browser=browser)
        ...
        for feature in all_features:
            used_count = sum(
                ...
                and m["features"].get(feature, False)
            )
        ...
        return {"features": feature_usage, "total_records": len(feature_metrics)}
...
class PerformanceDashboard:
    ...
    def generate_html_report(
        ...
        platform_comparison = self._generate_platform_comparison_section(
            model_filter, browser_filter
        )
        browser_comparison = self._generate_browser_comparison_section(
            model_filter, platform_filter
        )
    ...
    def _generate_model_comparison_section(
        self, platform_filter: Optional[str] = None, browser_filter: Optional[str] = None
    ) -> str:
        ...
        for model in models:
            performance = self.metrics.get_model_performance(
                model, platform=platform_filter, browser=browser_filter
            )
    ...
    def _generate_platform_comparison_section(
        self, model_filter: Optional[str] = None, browser_filter: Optional[str] = None
    ) -> str:
    ...
    def _generate_browser_comparison_section(
        self, model_filter: Optional[str] = None, platform_filter: Optional[str] = None
    ) -> str:
    ...
    def _generate_feature_usage_section(self, browser_filter: Optional[str] = None) -> str:
    ...
    def create_model_comparison_chart(
        ...
        for model in models:
            ...
            performance = self.metrics.get_model_performance(
                model, platform=platform, browser=browser
            )
    ...
    def _filter_metrics(
        ...
        """Filter metrics based on criteria."""
        return [m for m in metrics if self._matches_filters(m, model_name, platform, browser)]
...
def create_performance_report(
    ...
    html = dashboard.generate_html_report(
        model_filter=model_filter, platform_filter=platform_filter, browser_filter=browser_filter
    )
