        ...
        for param_name, param_value in bound_args.arguments.items():
            ...
            if param_name in type_hints:
                ...
                if hasattr(expected_type, "__origin__") and expected_type.__origin__ is Union:
                    ...
                    if not any(
                        isinstance(param_value, t) for t in allowed_types if t is not type(None)
                    ):
    ...
    def validator(value: Any) -> bool:
        ...
        if max_value is not None:
            if inclusive and value > max_value:
                ...
                raise ValidationError(
                    f"Value {value} is greater than or equal to maximum {max_value}"
                )
...
if __name__ == "__main__":
    ...
    @validate_types
    @validate_params(age=validate_range(min_value=0, max_value=120), email=validate_email)
