...
class ErrorTelemetryCollector:
    ...
    def record_error(self, error: Dict[str, Any]) -> None:
        ...
        self.component_errors[component][category] = (
            self.component_errors[component].get(category, 0) + 1
        )
        ...
        recent_window = [
            ...
            if e.get("category") == category and current_time - e.get("timestamp", 0) < 60
        ]  # 60 second window
        ...
        if len(recent_window) >= 3:  # 3+ errors of same type in 60 seconds
            ...
            logger.warning(
                f"Error peak detected: {len(recent_window)} {category} errors in {current_time - recent_window[0].get('timestamp'):.1f} seconds"
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_recovery_attempts = (
            self.recovery_attempts["success"] + self.recovery_attempts["failure"]
        )
        recovery_success_rate = (
            self.recovery_attempts["success"] / total_recovery_attempts
        ...
        return {
            ...
            "most_common_category": max(self.error_categories.items(), key=lambda x: x[1])[0]
            if self.error_categories
            else None,
            "most_affected_component": max(
                self.component_errors.items(), key=lambda x: sum(x[1].values())
            )[0]
...
class ErrorPropagationManager:
    ...
    def __init__(self, components: List[str] = None, collect_telemetry: bool = True):
        """
    ...
    def categorize_error(self, error: Union[Exception, Dict[str, Any]]) -> str:
        ...
        if (
            ...
            or "deadline" in error_message
            or "time limit" in error_message
        ):
            ...
            "browser" in error_message
            or "compatibility" in error_message
    ...
    def propagate_error(
        ...
        return {"handled": False, "error": error_record}
    ...
    def _handle_in_component(self, error_record: Dict[str, Any], component: str) -> Dict[str, Any]:
        """
    ...
    def _implement_graceful_degradation(self, error_record: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _handle_memory_degradation(self, component: str) -> Dict[str, Any]:
        ...
        if component == "streaming":
            ...
            return {"degraded": True, "action": "reduce_memory_usage"}
    ...
    def _handle_timeout_degradation(self, component: str) -> Dict[str, Any]:
        ...
        if component == "streaming":
            ...
            return {"degraded": True, "action": "extend_timeouts", "multiplier": 1.5}
...
def register_handler(manager: ErrorPropagationManager, component: str, handler: Callable) -> None:
    """
...
def streaming_error_handler(error, context):
    ...
    if category == ErrorCategory.MEMORY:
        ...
        return {"handled": True, "action": "reduced_batch_size_and_precision"}
    elif category == ErrorCategory.TIMEOUT:
        # Handle timeout in streaming component
        return {"handled": True, "action": "simplified_generation_parameters"}
...
def webgpu_error_handler(error, context):
    ...
    if category == ErrorCategory.MEMORY:
        ...
        return {"handled": True, "action": "reduced_model_complexity"}
    elif category == ErrorCategory.HARDWARE:
        # Handle hardware issues in WebGPU
        return {"handled": True, "action": "fallback_to_webnn"}
