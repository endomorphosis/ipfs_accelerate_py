...
class BrowserProfile:
    ...
    def check_compatibility(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if config.get("use_shader_precompilation", False) and not self.capabilities.get(
        ...
        if config.get("use_compute_shaders", False) and not self.capabilities.get(
        ...
        if config.get("use_model_sharding", False) and not self.capabilities.get(
...
class ConfigurationManager:
    ...
    def _initialize_default_config(self) -> None:
        ...
        if self.model_type == "text":
            self.default_config.update({"use_kv_cache": True, "enable_parallel_loading": False})
        elif self.model_type == "vision":
            self.default_config.update({"use_kv_cache": False, "enable_parallel_loading": False})
        ...
        self.validation_rules = [
            ...
            ConfigValidationRule(
                ...
                condition=lambda cfg: (
                    cfg.get("precision") in ["2bit", "3bit", "4bit", "8bit", "16bit"]
                ),
            ...
            ConfigValidationRule(
                ...
                condition=lambda cfg: (
                    ...
                    and len(cfg.get("workgroup_size", [])) == 3
                    and all(isinstance(x, int) and x > 0 for x in cfg.get("workgroup_size", []))
                ),
            ...
            ConfigValidationRule(
                ...
                condition=lambda cfg: (
                    not (self.model_type == "text" and cfg.get("enable_parallel_loading", False))
                ),
                ...
                correction_function=lambda cfg: {
                    **cfg,
                    "enable_parallel_loading": False
                    if self.model_type == "text"
                    else cfg.get("enable_parallel_loading", False),
                },
            ),
            ...
            ConfigValidationRule(
                ...
                condition=lambda cfg: (
                    not (self.model_type in ["vision", "audio"] and cfg.get("use_kv_cache", False))
                ),
                ...
                correction_function=lambda cfg: {
                    **cfg,
                    "use_kv_cache": False
                    if self.model_type in ["vision", "audio"]
                    else cfg.get("use_kv_cache", False),
                },
            ),
        ]
    ...
    def validate_configuration(
        self, config: Dict[str, Any], raise_on_error: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def get_optimized_configuration(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
