...
class EnhancedWebResourcePoolTester:
    ...
    async def test_model(self, model_type, model_name, platform):
        ...
        try:
            ...
            if (
                model_type == "audio"
                and platform == "webgpu"
                and getattr(self.args, "firefox", False)
            ):
                ...
                and platform == "webnn"
                and getattr(self.args, "edge", False)
            ):
                ...
                model_type == "vision"
                and platform == "webgpu"
                and getattr(self.args, "chrome", False)
            ):
            ...
            if hasattr(self.args, "precision") and self.args.precision != 16:
                ...
                logger.info(
                    f"Using {self.args.precision}-bit precision"
            ...
            if model:
                ...
                result = await model(
                    inputs
                )  # Directly call model assuming it's a callable with await
                ...
                if "is_real_implementation" in result:
                    logger.info(
                        f"  - Real implementation: {result.get('is_real_implementation', False)}"
                    )
                if "throughput_items_per_sec" in result.get("performance_metrics", {}):
                    logger.info(
                        f"  - Throughput: {result.get('performance_metrics', {}).get('throughput_items_per_sec', 0):.2f} items/s"
                    )
                if "memory_usage_mb" in result.get("performance_metrics", {}):
                    logger.info(
                        f"  - Memory usage: {result.get('performance_metrics', {}).get('memory_usage_mb', 0):.2f} MB"
                    )
    ...
    async def test_concurrent_models(self, models, platform):
        ...
        try:
            ...
            for model_type, model_name in models:
                # Get model with enhanced integration
                model = await self.integration.get_model(
                    model_name=model_name, model_type=model_type, platform=platform
                )
            ...
            if models_and_inputs:
                ...
                logger.info(
                    f"Concurrent inference completed in {total_time:.2f}s for {len(models_and_inputs)} models"
                )
    ...
    async def run_stress_test(self, duration, models):
        ...
        """
        logger.info(
            f"Running enhanced stress test for {duration} seconds with {len(models)} models"
        )
        ...
        try:
                ...
                try:
                    ...
                    if model:
                        ...
                        if iteration % 10 == 0:
                            ...
                            logger.info(
                                f"Stress test progress: {elapsed:.1f}s elapsed, {remaining:.1f}s remaining, {success_count} successes, {failure_count} failures"
                            )
                    else:
                        # Model load failed
                        failure_count += 1
                ...
                if iteration % 5 == 0:
                    try:
                        ...
                        if "connection_pool" in metrics:
                            ...
                            logger.info(
                                f"Connection pool: {pool_stats.get('total_connections', 0)} connections, {pool_stats.get('health_counts', {}).get('healthy', 0)} healthy"
                            )
                    except Exception as e:
            ...
            logger.info(
                f"  - Success rate: {success_count}/{iteration} ({success_count / max(iteration, 1) * 100:.1f}%)"
            )
            ...
            if "ipfs_acceleration_count" in final_stats:
                logger.info(
                    f"  - IPFS Acceleration Count: {final_stats.get('ipfs_acceleration_count', 0)}"
                )
            ...
            try:
                ...
                if "connection_pool" in metrics:
                    ...
                    logger.info(
                        f"  - Healthy: {pool_stats.get('health_counts', {}).get('healthy', 0)}"
                    )
                    logger.info(
                        f"  - Degraded: {pool_stats.get('health_counts', {}).get('degraded', 0)}"
                    )
                    logger.info(
                        f"  - Unhealthy: {pool_stats.get('health_counts', {}).get('unhealthy', 0)}"
                    )
                    ...
                    if "adaptive_stats" in pool_stats:
                        ...
                        logger.info(
                            f"  - Current utilization: {adaptive_stats.get('current_utilization', 0):.2f}"
                        )
                        logger.info(
                            f"  - Average utilization: {adaptive_stats.get('avg_utilization', 0):.2f}"
                        )
                        logger.info(
                            f"  - Peak utilization: {adaptive_stats.get('peak_utilization', 0):.2f}"
                        )
                        logger.info(
                            f"  - Scale up threshold: {adaptive_stats.get('scale_up_threshold', 0):.2f}"
                        )
                        logger.info(
                            f"  - Scale down threshold: {adaptive_stats.get('scale_down_threshold', 0):.2f}"
                        )
                        logger.info(
                            f"  - Connection startup time: {adaptive_stats.get('avg_connection_startup_time', 0):.2f}s"
                        )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    async def test_main():
        ...
        parser.add_argument(
            "--models",
            type=str,
            default="bert-base-uncased,vit-base-patch16-224,whisper-tiny",
            help="Comma-separated list of models to test",
        )
        parser.add_argument(
            "--platform",
            type=str,
            choices=["webnn", "webgpu"],
            default="webgpu",
            help="Platform to test",
        )
        parser.add_argument("--concurrent", action="store_true", help="Test models concurrently")
        parser.add_argument(
            "--min-connections", type=int, default=1, help="Minimum number of connections"
        )
        parser.add_argument(
            "--max-connections", type=int, default=4, help="Maximum number of connections"
        )
        parser.add_argument(
            "--adaptive-scaling", action="store_true", help="Enable adaptive scaling"
        )
