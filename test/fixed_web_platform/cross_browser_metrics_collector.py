...
class MetricsCollector:
    ...
    async def record_test_result(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "summary" in performance_impact and performance_impact["summary"].get(
    ...
    async def analyze_fault_tolerance_performance(
        self,
        ...
        try:
            ...
            if not self.conn:
                ...
                if models:
                    filtered_results = [
                        r for r in filtered_results if r.get("model_name") in models
                    ]

                if strategies:
                    filtered_results = [
                        r for r in filtered_results if r.get("strategy") in strategies
                    ]
                ...
                if fault_levels:
                    filtered_results = [
                        ...
                        if r.get("fault_tolerance_level") in fault_levels
                    ]
                ...
                if recovery_strategies:
                    filtered_results = [
                        r
                        for r in filtered_results
                        if r.get("recovery_strategy") in recovery_strategies
                    ]
            ...
            if test_results.empty:
                return {"status": "no_data", "message": "No test results found matching criteria"}

            # Analyze results
            analysis = {"summary": {}, "comparisons": {}, "insights": [], "recommendations": []}
            ...
            if "initialization_time" in test_results.columns:
                analysis["summary"]["avg_initialization_time"] = test_results[
                    "initialization_time"
                ].mean()
            ...
            if "strategy" in test_results.columns and len(test_results["strategy"].unique()) > 1:
                ...
                for strategy in test_results["strategy"].unique():
                    ...
                    if "initialization_time" in strategy_results.columns:
                        strategy_comparison[strategy]["avg_initialization_time"] = strategy_results[
                            "initialization_time"
                        ].mean()

                    if "validation_time" in strategy_results.columns:
                        strategy_comparison[strategy]["avg_validation_time"] = strategy_results[
                            "validation_time"
                        ].mean()
                ...
                analysis["insights"].append(
                    f"The {best_strategy[0]} strategy has the highest pass rate at {best_strategy[1]['pass_rate']:.1%}"
                )

            # Compare fault tolerance levels if multiple are present
            if (
                "fault_tolerance_level" in test_results.columns
                and len(test_results["fault_tolerance_level"].unique()) > 1
            ):
                ft_comparison = {}
                ...
                for ft_level in test_results["fault_tolerance_level"].unique():
                    ...
                    if "validation_time" in ft_results.columns:
                        ft_comparison[ft_level]["avg_validation_time"] = ft_results[
                            "validation_time"
                        ].mean()
                ...
                if high_ft and medium_ft:
                    high_pass_rate = sum(
                        ft_comparison[level]["pass_count"] for level in high_ft
                    ) / sum(ft_comparison[level]["total_tests"] for level in high_ft)
                    medium_pass_rate = ft_comparison["medium"]["pass_rate"]

                    if high_pass_rate > medium_pass_rate:
                        analysis["insights"].append(
                            f"Higher fault tolerance levels show better recovery performance ({high_pass_rate:.1%} vs {medium_pass_rate:.1%} pass rate)"
                        )
                    else:
                        analysis["insights"].append(
                            f"Medium fault tolerance level provides adequate recovery ({medium_pass_rate:.1%} pass rate) with potentially lower overhead"
                        )

            # Compare recovery strategies if multiple are present
            if (
                "recovery_strategy" in test_results.columns
                and len(test_results["recovery_strategy"].unique()) > 1
            ):
                rs_comparison = {}
                ...
                for rs in test_results["recovery_strategy"].unique():
                    ...
                    if "validation_time" in rs_results.columns:
                        rs_comparison[rs]["avg_validation_time"] = rs_results[
                            "validation_time"
                        ].mean()
                ...
                analysis["insights"].append(
                    f"The {best_rs[0]} recovery strategy has the highest pass rate at {best_rs[1]['pass_rate']:.1%}"
                )
            ...
            if self.conn:
                try:
                    ...
                    if not recovery_metrics.empty:
                        ...
                        recovery_metrics["success_rate"] = (
                            recovery_metrics["successful_recoveries"]
                            / recovery_metrics["total_tests"]
                        )
                        ...
                        low_success_scenarios = recovery_metrics[
                            recovery_metrics["success_rate"] < 0.8
                        ]
                        if not low_success_scenarios.empty:
                            scenarios = ", ".join(low_success_scenarios["scenario"])
                            analysis["recommendations"].append(
                                f"Improve recovery mechanisms for: {scenarios}"
                            )
                except Exception as e:
            ...
            if analysis["comparisons"].get("strategies"):
                worst_strategy = min(
                    analysis["comparisons"]["strategies"].items(), key=lambda x: x[1]["pass_rate"]
                )
                if worst_strategy[1]["pass_rate"] < 0.7:
                    analysis["recommendations"].append(
                        f"Consider alternatives to the {worst_strategy[0]} strategy which has only {worst_strategy[1]['pass_rate']:.1%} pass rate"
                    )
            ...
            if analysis["comparisons"].get("fault_tolerance_levels"):
                ...
                if "high" in ft_levels and "medium" in ft_levels:
                    ...
                    if high_pass_rate > medium_pass_rate * 1.1:  # 10% better
                        analysis["recommendations"].append(
                            "Consider using high fault tolerance level for critical workloads"
                        )
                    else:
                        analysis["recommendations"].append(
                            "Medium fault tolerance level may be sufficient for most use cases"
                        )
    ...
    async def generate_fault_tolerance_visualization(
        self,
        output_path: str,
        models: Optional[List[str]] = None,
        strategies: Optional[List[str]] = None,
...
async def main():
    ...
    # Analysis options
    parser.add_argument("--analyze", action="store_true", help="Run analysis on collected metrics")
    parser.add_argument(
        "--models", type=str, help="Comma-separated list of models to include in analysis"
    )
    parser.add_argument(
        "--strategies", type=str, help="Comma-separated list of strategies to include in analysis"
    )

    # Visualization options
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualizations of metrics"
    )
    parser.add_argument(
        "--metric",
        type=str,
        choices=["recovery_time", "success_rate", "performance_impact"],
        default="recovery_time",
    ...
    try:
        ...
        if args.visualize:
            ...
            success = await collector.generate_fault_tolerance_visualization(
                output_path=args.output, models=models, strategies=strategies, metric=args.metric
            )
