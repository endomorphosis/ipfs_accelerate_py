...
from .web_platform_handler import process_for_web, init_webnn, init_webgpu, create_mock_processors

# Import Safari WebGPU handler
try:
    from .safari_webgpu_handler import SafariWebGPUHandler, optimize_for_safari
...
try:
    from .webgpu_shader_registry import WebGPUShaderRegistry, get_shader_registry
...
__all__ = ["process_for_web", "init_webnn", "init_webgpu", "create_mock_processors"]

# Add browser automation if available
if BROWSER_AUTOMATION_AVAILABLE:
    __all__.extend(["setup_browser_automation", "run_browser_test", "find_browser_executable"])

# Add Safari support if available
if SAFARI_WEBGPU_AVAILABLE:
    __all__.extend(["SafariWebGPUHandler", "optimize_for_safari"])

# Add ultra-low precision if available
if ULTRA_LOW_PRECISION_AVAILABLE:
    __all__.extend(
        ["setup_ultra_low_precision", "create_2bit_compute_shaders", "create_3bit_compute_shaders"]
    )

# Add progressive loading if available
if PROGRESSIVE_LOADING_AVAILABLE:
    __all__.extend(
        ["ProgressiveModelLoader", "MultimodalComponentManager", "load_model_progressively"]
    )
...
if SHADER_PRECOMPILATION_AVAILABLE:
    __all__.extend(["ShaderPrecompiler", "setup_shader_precompilation", "precompile_model_shaders"])

# Add shader registry if available (March 2025)
if SHADER_REGISTRY_AVAILABLE:
    __all__.extend(["WebGPUShaderRegistry", "get_shader_registry"])
