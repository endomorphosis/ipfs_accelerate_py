...
class BrowserLatencyOptimizer:
    ...
    def __init__(self, browser: str = None, device_profile: str = None):
        ...
        logger.info(
            f"Initialized latency optimizer for {self.browser} browser with {self.device_profile} profile"
        )
    ...
    def optimize_for_low_latency(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        optimized_config["max_batch_size"] = min(
            optimized_config.get("max_batch_size", 8), device_characteristics["batch_size_max"]
        )
        ...
        if re.search(pattern, shader_code):
            ...
            if match:
                ...
                modified_code = shader_code.replace(
                    compute_line, f"@compute {replacement}\nfn {match.group(1)}"
                )
            else:
...
class TokenBufferManager:
    ...
    def _adjust_buffer_size(self):
        ...
        recent_times = (
            self.generation_times[-5:] if len(self.generation_times) >= 5 else self.generation_times
        )
        ...
        if len(self.timing_history) >= 3:
            ...
            avg_flush_time = sum(item["flush_time_ms"] for item in recent_flushes) / (
                3 * 1000
            )  # Convert to seconds
            ...
            if avg_gen_time < avg_flush_time * 0.5 and self.buffer_size < 8:
                self.buffer_size += 1
                logger.debug(
                    f"Increased buffer size to {self.buffer_size} (gen time: {avg_gen_time:.4f}s, flush time: {avg_flush_time:.4f}s)"
                )
            # If generation is slow, decrease buffer for lower latency
            elif avg_gen_time > avg_flush_time * 1.5 and self.buffer_size > 1:
                self.buffer_size -= 1
                logger.debug(
                    f"Decreased buffer size to {self.buffer_size} (gen time: {avg_gen_time:.4f}s, flush time: {avg_flush_time:.4f}s)"
                )
    ...
    def _adjust_for_network_latency(self):
        ...
        recent_latencies = (
            self.network_latencies[-5:]
            if len(self.network_latencies) >= 5
            else self.network_latencies
        )
        ...
        if avg_latency_ms > 50 and self.buffer_size < 8:
            self.buffer_size += 1
            logger.debug(
                f"Increased buffer size to {self.buffer_size} due to high network latency ({avg_latency_ms:.2f}ms)"
            )
        # If network is very responsive, decrease buffer size for lower latency
        elif avg_latency_ms < 10 and self.buffer_size > 1:
            self.buffer_size -= 1
            logger.debug(
                f"Decreased buffer size to {self.buffer_size} due to low network latency ({avg_latency_ms:.2f}ms)"
            )
...
class PrefillDecodeOptimizer:
    ...
    def __init__(self, prefill_strategy: str = "parallel", decode_strategy: str = "eager"):
        ...
        logger.info(
            f"Initialized prefill/decode optimizer with strategies: prefill={prefill_strategy}, decode={decode_strategy}"
        )
    ...
    def optimize_decode(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.decode_strategy == "eager":
            ...
            decode_config["decode_max_batch_size"] = (
                1  # Start with minimal batch size for lowest latency
            )
    ...
    def optimize_transition(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        optimized_config["prefill"] = {
            key: value
            for key, value in prefill_config.items()
    ...
    def record_decode_start(self, time_ms: float, batch_size: int):
        ...
        self.decode_stats.append(
            {"time_ms": time_ms, "batch_size": batch_size, "timestamp": time.time()}
        )
        ...
        if self.prefill_stats and self.decode_stats:
            ...
            if abs(last_prefill["timestamp"] - last_decode["timestamp"]) < 10:  # Within 10 seconds
                transition_time = (
                    last_decode["timestamp"] - last_prefill["timestamp"]
                ) * 1000  # ms
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.prefill_stats:
            avg_prefill_time = sum(stat["time_ms"] for stat in self.prefill_stats) / len(
                self.prefill_stats
            )

        avg_decode_time = 0
        if self.decode_stats:
            avg_decode_time = sum(stat["time_ms"] for stat in self.decode_stats) / len(
                self.decode_stats
            )
        ...
        return {
            ...
            "transition_efficiency": 1.0
            if avg_prefill_time == 0
            else (avg_decode_time / avg_prefill_time),
        }


def optimize_for_low_latency(
    config: Dict[str, Any], browser: str = None, device_profile: str = None
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Applied low-latency optimizations for {browser_optimizer.browser} browser on {browser_optimizer.device_profile} device"
    )
    return optimized_config
...
if __name__ == "__main__":
    # Example usage
    config = {"quantization": "int4", "latency_optimized": True, "max_batch_size": 8}

    # Apply low-latency optimizations
    optimized_config = optimize_for_low_latency(config, browser="chrome", device_profile="high_end")
