...
class EnhancedParallelModelExecutor:
    ...
    def __init__(
        ...
        logger.info(
            f"EnhancedParallelModelExecutor created with {max_workers} workers (min: {min_workers})"
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Enhanced parallel model executor initialized with {len(self.workers)} workers"
            )
            return True
        ...
        try:
            ...
            if hasattr(self.resource_pool_integration, "create_connection"):
                ...
                if connection:
                    ...
                    worker["connection_id"] = getattr(
                        connection, "connection_id", str(id(connection))
                    )
                    ...
                    logger.warning(
                        f"Failed to create real connection for worker {worker_id}, using simulation mode"
                    )
            else:
                # Mark as simulation mode
                worker["status"] = "ready"
        ...
        try:
            ...
            for worker_id, stats in self.worker_stats.items():
                if "execution_times" in stats and stats["execution_times"]:
                    recent_execution_times.extend(
                        stats["execution_times"][-5:]
                    )  # Only use recent executions

            # Calculate average execution time
            avg_execution_time = (
                sum(recent_execution_times) / len(recent_execution_times)
            ...
            estimated_latency = (
                pending_tasks * avg_execution_time / max(1, current_workers - active_workers)
            )
            ...
            scale_up = (
                current_load > 0.8 or estimated_latency > 5.0
            ...
            scale_down = (
                ...
                and self.available_workers.qsize() > 0
            )
            ...
            if scale_up:
                ...
                workers_to_add = min(
                    pending_tasks // self.max_models_per_worker
                    + 1,  # At least enough for pending tasks
                    self.max_workers - current_workers,  # Don't exceed max_workers
                )

                if workers_to_add > 0:
                    logger.info(
                        f"Scaling up: adding {workers_to_add} workers (current: {current_workers}, active: {active_workers}, load: {current_load:.2f}, pending tasks: {pending_tasks})"
                    )
                ...
                if workers_to_remove > 0:
                    logger.info(
                        f"Scaling down: removing {workers_to_remove} workers (current: {current_workers}, active: {active_workers}, load: {current_load:.2f}, idle: {idle_workers})"
                    )
            ...
            for worker_id, worker in self.workers.items():
                ...
                if "connection" in worker:
                    ...
                    if hasattr(connection, "browser_info"):
                        hardware_info["browser_version"] = getattr(
                            connection, "browser_info", {}
                        ).get("version", "unknown")
                    if hasattr(connection, "adapter_info"):
                        hardware_info["platform_version"] = getattr(
                            connection, "adapter_info", {}
                        ).get("version", "unknown")
                ...
                """,
                    [
                        ...
                        / max(1, stats.get("models_executed", 1)),
    ...
    async def execute_models(
        ...
        if not self.initialized:
            ...
            if not await self.initialize():
                ...
                return [
                    {"success": False, "error": "Executor not initialized"}
                    for _ in models_and_inputs
                ]
        ...
        self.execution_metrics["max_concurrent_models"] = max(
            self.execution_metrics["max_concurrent_models"], len(models_and_inputs)
        )
        ...
        try:
            async with anyio.create_task_group() as task_group:
                ...
                try:
                    await wait_for(
                        gather(*[future.wait() for future in futures]), timeout=execution_timeout
                    )
                except TimeoutError:
                    logger.warning(
                        f"Timeout waiting for models execution after {execution_timeout}s"
                    )
            ...
            if self.db_connection:
                self._store_execution_metrics(
                    execution_id, models_and_inputs, results, execution_time
                )

            logger.info(
                f"Executed {len(models_and_inputs)} models in {execution_time:.2f}s ({throughput:.2f} models/s), {successful} successful, {failed} failed"
            )
    ...
    async def _share_text_embedding_tensors(self, model_group):
        ...
        if (
            "text_embedding"
            not in self.execution_metrics["tensor_sharing_stats"]["shared_tensor_types"]
        ):
            self.execution_metrics["tensor_sharing_stats"]["shared_tensor_types"][
                "text_embedding"
            ] = 0

        self.execution_metrics["tensor_sharing_stats"]["shared_tensor_types"]["text_embedding"] += (
            shared_count
        )
    ...
    def _store_tensor_sharing_metrics(
        self, execution_id, tensor_type, source_model, target_model, size_mb
    ):
        """
    ...
    def _store_execution_metrics(self, execution_id, models_and_inputs, results, execution_time):
        ...
        try:
            ...
            memory_usage = sum(
                r.get("memory_usage_mb", 0) for r in results if r.get("success", False)
            )
            ...
            for i, (model, _) in enumerate(models_and_inputs):
                ...
                shared_tensors = (
                    getattr(model, "shared_tensors", {}) if hasattr(model, "shared_tensors") else {}
                )
            ...
            for worker_id, stats in self.worker_stats.items():
                worker_details.append(
                    {
                        "worker_id": worker_id,
                        "browser": self.workers[worker_id]["browser"]
                        if worker_id in self.workers
                        else "unknown",
                        "platform": self.workers[worker_id]["platform"]
                        if worker_id in self.workers
                        else "unknown",
    ...
    async def _execute_model_with_worker(
        self, model, inputs, model_index, future, execution_id, pending_token=None
    ):
        """
        ...
        try:
            ...
            try:
                ...
                if worker_id in self.worker_stats:
                    ...
                    self.worker_stats[worker_id]["avg_execution_time"] = sum(execution_times) / len(
                        execution_times
                    )
                ...
                if len(self.execution_metrics["model_execution_times"][model_name]) > 100:
                    self.execution_metrics["model_execution_times"][model_name] = (
                        self.execution_metrics["model_execution_times"][model_name][-100:]
                    )
                ...
                # Update worker stats
                if worker_id in self.worker_stats:
                    self.worker_stats[worker_id]["failed_executions"] = (
                        self.worker_stats[worker_id].get("failed_executions", 0) + 1
                    )
                ...
                if self.recovery_attempts > 0:
                    ...
                    recovery_result = await self._attempt_recovery(
                        model, inputs, error_context, execution_id, model_index
                    )
        ...
        try:
            ...
            try:
                ...
                if recovery_worker_id == failed_worker_id:
                    ...
                    if recovery_worker_id == failed_worker_id:
                        logger.warning(
                            f"Still got same worker {recovery_worker_id}, using it anyway"
                        )
            ...
            if recovery_worker_id in self.worker_stats:
                ...
                self.worker_stats[recovery_worker_id]["recovery_count"] = (
                    self.worker_stats[recovery_worker_id].get("recovery_count", 0) + 1
                )
            ...
            try:
                ...
                if recovery_worker_id in self.worker_stats:
                    ...
                    self.worker_stats[recovery_worker_id]["avg_execution_time"] = sum(
                        execution_times
                    ) / len(execution_times)
                ...
                # Update worker stats
                if recovery_worker_id in self.worker_stats:
                    self.worker_stats[recovery_worker_id]["failed_executions"] = (
                        self.worker_stats[recovery_worker_id].get("failed_executions", 0) + 1
                    )
                ...
                if recovery_worker_id and recovery_worker_id in self.workers:
                    try:
                        ...
                        logger.error(
                            f"Error returning recovery worker {recovery_worker_id} to pool: {e}"
                        )
...
async def test_enhanced_parallel_executor():
    ...
    try:
        ...
        model3 = EnhancedWebModel(
            "whisper-tiny", "audio", "webgpu", "firefox", compute_shaders=True
        )
        ...
        results = await executor.execute_models(
            [(model1, inputs1), (model2, inputs2), (model3, inputs3)]
        )
        ...
        results2 = await executor.execute_models([(model1, inputs1), (model4, inputs1)])
