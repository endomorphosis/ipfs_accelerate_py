...
class PipelineMetrics:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_processing_time = sum(self.request_processing_times) / max(
            1, len(self.request_processing_times)
        )
        ...
        avg_websocket_latency = sum(self.websocket_latencies) / max(
            1, len(self.websocket_latencies)
        )
        avg_concurrent_clients = sum(self.concurrent_clients_history) / max(
            1, len(self.concurrent_clients_history)
        )
        ...
        return {
            ...
            "clients": {
                ...
                "max_concurrent_clients": max(self.concurrent_clients_history)
                if self.concurrent_clients_history
                else 0,
            },
            "errors": {
...
class WebGPUStreamingPipeline:
    ...
    def __init__(self, model_path: str, config: Dict[str, Any] = None):
        ...
        # Success message
        logger.info(
            f"WebGPU Streaming Pipeline initialized with {self.config['quantization']} quantization"
        )
        logger.info(
            f"Memory limit: {self.config['memory_limit_mb']}MB, Max clients: {self.config['max_clients']}"
        )
    ...
    def _initialize_pipeline(self):
        ...
        self.inference_engine = WebGPUStreamingInference(self.model_path, config=inference_config)
            ...
            def update_metrics_periodically():
                while self.is_running and not self.shutdown_event.is_set():
                    if self.config["metrics_enabled"]:
                        ...
                        logger.debug(
                            f"Updated dashboard metrics: {len(active_clients)} clients, "
    ...
    def _check_auto_tune_parameters(self):
        ...
        if memory_pressure_rate > 0.2:  # More than 20% of requests experience memory pressure
            ...
            if new_max_clients != self.config["max_clients"]:
                logger.info(
                    f"Auto-tuning: Reducing max clients from {self.config['max_clients']} to {new_max_clients} "
            ...
            logger.info(
                f"Auto-tuning: Increasing max clients from {self.config['max_clients']} to {new_max_clients} "
        ...
        if "batch_size_history" in dir(self.inference_engine):
            ...
            actual_max_used = (
                max(self.inference_engine._batch_size_history)
                if self.inference_engine._batch_size_history
                else 1
            )
            ...
            if actual_max_used < current_max_batch - 2:
                ...
                logger.info(
                    f"Auto-tuning: Reducing max batch size from {current_max_batch} to {new_max_batch} "
        ...
        if avg_processing_time > 0:
            ...
            if (
                abs(new_timeout - self.config["request_timeout_sec"]) > 30
            ):  # Only change if difference is significant
                logger.info(
                    f"Auto-tuning: Adjusting request timeout from {self.config['request_timeout_sec']}s "
                    ...
                    for request_id in timeout_ids:
                        ...
                        for i, request in enumerate(self.request_queue):
                            if request.id == request_id:
                                ...
                                logger.warning(
                                    f"Request {request_id} timed out after {self.config['request_timeout_sec']}s"
                                )
                ...
                try:
                    ...
                    if request.client and not request.client.closed:
                        ...
                        self.metrics.record_completion(
                            processing_time, self.inference_engine._tokens_generated
                        )

                        # Record batch size history
                        if hasattr(self.inference_engine, "_current_batch_size"):
                            self.metrics.record_batch_size(
                                self.inference_engine._current_batch_size
                            )

                        # Record memory pressure events
                        if hasattr(self.inference_engine, "_token_generation_stats"):
                            self.metrics.record_memory_pressure(
                                self.inference_engine._token_generation_stats.get(
                                    "memory_pressure_events", 0
                                )
                            )

                        logger.info(
                            f"Completed request {request.id} in {processing_time:.2f}s, "
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        with self.queue_lock:
            ...
            status = {
                ...
                "estimated_wait_time": len(self.request_queue)
                * 5,  # Rough estimate: 5 seconds per request
            }
            ...
            if self.config["metrics_enabled"]:
                ...
                if metrics["request_counts"]["total"] > 0:
                    status["avg_processing_time"] = metrics["performance"][
                        "avg_processing_time_sec"
                    ]
                    status["avg_wait_time"] = metrics["performance"]["avg_wait_time_sec"]
                    status["avg_tokens_per_second"] = metrics["performance"][
                        "avg_tokens_per_second"
                    ]
                    status["estimated_wait_time"] = (
                        len(self.request_queue) * metrics["performance"]["avg_processing_time_sec"]
                    )
        ...
        try:
            ...
            if not prompt:
                await websocket.send(
                    json.dumps(
                        {"type": "error", "message": "Prompt is required", "request_id": request_id}
                    )
                )
                return
            ...
            while True:
                ...
                try:
                    ...
                    logger.info(
                        f"WebSocket connection closed by client, cancelling request {request_id}"
                    )
            ...
            await websocket.send(json.dumps({"type": "error", "message": "Invalid JSON request"}))
        except Exception as e:
            ...
            try:
                await websocket.send(
                    json.dumps({"type": "error", "message": f"Internal server error: {str(e)}"})
                )
            except:
                pass
    ...
    async def start_server_async(self, host: str = "localhost", port: int = 8765):
        ...
        try:
            ...
            self.server = await websockets.serve(
                self.handle_websocket, host, port, ping_interval=30, ping_timeout=10
            )
...
async def start_streaming_server(
    model_path: str, host: str = "localhost", port: int = 8765, config: Dict[str, Any] = None
):
    """
...
def create_streaming_pipeline(
    model_path: str, config: Dict[str, Any] = None
) -> WebGPUStreamingPipeline:
    """
