...
class EnhancedWebSocketBridge:
    ...
    async def attempt_reconnection(self):
        ...
        logger.info(
            f"Attempting reconnection in {total_delay:.2f} seconds (attempt {self.connection_attempts}/{self.max_reconnect_attempts})"
        )
        ...
        try:
            while not self.shutdown_event.is_set():
                try:
                    ...
                    priority, message = await wait_for(self.message_receive.receive(), timeout=1.0)
            ...
            while not self.shutdown_event.is_set():
                ...
                if self.is_connected and self.connection:
                    try:
                        ...
                        await wait_for(self.connection.send(json.dumps(heartbeat_msg)), timeout=5.0)
        ...
        try:
            while not self.shutdown_event.is_set():
                ...
                if self.is_connected:
                    ...
                    receive_timeout = (
                        current_time - self.last_receive_time > self.heartbeat_interval * 3
                    )

                    # Check if heartbeat response was received (if heartbeat was sent)
                    heartbeat_timeout = (
                        self.stats["heartbeats_sent"] > 0 and self.stats["heartbeats_received"] == 0
                    ) or (
                        self.last_heartbeat_time > 0
                        and current_time - self.last_heartbeat_time > self.heartbeat_interval * 2
                    )

                    if receive_timeout or heartbeat_timeout:
                        logger.warning(
                            f"Connection appears unhealthy: received={not receive_timeout}, heartbeat={not heartbeat_timeout}"
                        )
                        ...
                        if self.connection:
                            try:
                                await self.connection.close(
                                    code=1001, reason="Connection health check failed"
                                )
                            except Exception as e:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    logger.warning(
                        f"Timeout sending message, retrying (attempt {attempt + 1}/{max_retries + 1})"
                    )
                else:
                    logger.error(
                        f"Failed to send message after {max_retries + 1} attempts: timeout"
                    )
                ...
                if attempt < max_retries and self.is_connected:
                    logger.warning(
                        f"Error sending message, retrying (attempt {attempt + 1}/{max_retries + 1}): {e}"
                    )
                else:
    ...
    async def get_browser_capabilities(self):
        ...
        def validate_capabilities(response):
            return response and response.get("status") == "success" and "data" in response

        # Send and wait for response with validation
        response = await self.send_and_wait(
            request, timeout=self.message_timeout, response_validator=validate_capabilities
        )
    ...
    async def initialize_model(self, model_name, model_type, platform, options=None):
        ...
        def validate_init_response(response):
            return (
                response
                and response.get("status") in ["success", "error"]
                and "model_name" in response
            )
    ...
    async def run_inference(self, model_name, input_data, platform, options=None):
        ...
        def validate_inference_response(response):
            return (
                response
                and response.get("status") in ["success", "error"]
                and (response.get("status") == "error" or "result" in response)
            )
    ...
    async def ping(self, timeout=5.0):
        ...
        return {"status": "success", "rtt": rtt, "timestamp": time.time()}
...
async def create_enhanced_websocket_bridge(port=8765, host="127.0.0.1", enable_heartbeat=True):
    ...
    """
    bridge = EnhancedWebSocketBridge(port=port, host=host, enable_heartbeat=enable_heartbeat)
