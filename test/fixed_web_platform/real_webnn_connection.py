...
class RealWebNNConnection:
    ...
    def __init__(
        self, browser_name="chrome", headless=True, browser_path=None, device_preference="gpu"
    ):
        ...
        if WebPlatformImplementation is None or RealWebPlatformIntegration is None:
            raise ImportError(
                "WebPlatformImplementation or RealWebPlatformIntegration not available"
            )
    ...
    async def initialize(self):
        ...
        try:
            ...
            success = await self.integration.initialize_platform(
                platform="webnn", browser_name=self.browser_name, headless=self.headless
            )
            ...
            if self.feature_detection:
                ...
                if webnn_supported:
                    ...
                    if self.device_preference not in webnn_backends:
                        ...
                        if webnn_backends:
                            logger.warning(
                                f"Preferred device '{self.device_preference}' not available. Using '{webnn_backends[0]}' instead."
                            )
...
def create_webnn_implementation(browser_name="chrome", headless=True, device_preference="gpu"):
    ...
    if RealWebNNImplementation is not None:
        return RealWebNNImplementation(
            browser_name=browser_name, headless=headless, device_preference=device_preference
        )

    # Otherwise, use the new implementation
    return RealWebNNConnection(
        browser_name=browser_name, headless=headless, device_preference=device_preference
    )
...
async def test_connection():
    ...
    try:
        ...
        result = await connection.run_inference(
            "bert-base-uncased", "This is a test input for BERT model."
        )
        if not result:
