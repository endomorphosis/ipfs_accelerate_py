...
class EnhancedWebResourcePoolTester:
    ...
    async def run_stress_test(self, duration, models):
        ...
        logger.info(
            f"Stress test progress: 1.0s elapsed, {duration - 1.0:.1f}s remaining, 5 successes, 0 failures"
        )
        await anyio.sleep(1.0)
        logger.info(
            f"Stress test progress: 2.0s elapsed, {duration - 2.0:.1f}s remaining, 10 successes, 1 failures"
        )
        await anyio.sleep(1.0)
        logger.info(
            f"Stress test progress: 3.0s elapsed, {duration - 3.0:.1f}s remaining, 15 successes, 1 failures"
        )

        # Update mock metrics to simulate stress test results
        self.mock_metrics["browser_distribution"] = {
            "chrome": 10,
            "firefox": 6,
            "edge": 4,
            "safari": 0,
        ...
        self.mock_metrics["connection_pool"]["health_counts"] = {
            "healthy": 3,
            "degraded": 1,
            "unhealthy": 0,
        }
        ...
        logger.info(
            f"  - Healthy: {self.mock_metrics['connection_pool']['health_counts']['healthy']}"
        )
        logger.info(
            f"  - Degraded: {self.mock_metrics['connection_pool']['health_counts']['degraded']}"
        )
        logger.info(
            f"  - Unhealthy: {self.mock_metrics['connection_pool']['health_counts']['unhealthy']}"
        )
        ...
        logger.info(
            f"  - Connection startup time: {adaptive_stats['avg_connection_startup_time']:.2f}s"
        )
