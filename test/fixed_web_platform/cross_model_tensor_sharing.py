...
class SharedTensor:
    ...
    def acquire(self, model_name: str) -> bool:
        ...
        self.last_accessed = time.time()
        logger.debug(
            f"Model {model_name} acquired tensor {self.name}, reference count: {self.reference_count}"
        )
        return True
    ...
    def release(self, model_name: str) -> bool:
        ...
        if model_name in self.consumer_models:
            ...
            logger.debug(
                f"Model {model_name} released tensor {self.name}, reference count: {self.reference_count}"
            )
            return True
        return False
    ...
    def can_be_freed(self) -> bool:
        ...
        return (
            ...
            and not self.consumer_models
            and time.time() - self.last_accessed > 30
    ...
    def __repr__(self) -> str:
        return (
            f"SharedTensor(name={self.name}, shape={self.shape}, "
...
class SharedTensorView:
    ...
    def __init__(self, parent: SharedTensor, name: str, offset: List[int], size: List[int]):
        """
        ...
        logger.debug(
            f"Created tensor view {name} into {parent.name} with offset {offset} and size {size}"
        )
    ...
    def acquire(self, model_name: str) -> bool:
        ...
        logger.debug(
            f"Model {model_name} acquired tensor view {self.name}, reference count: {self.reference_count}"
        )
        return True
    ...
    def release(self, model_name: str) -> bool:
        ...
        if model_name in self.consumer_models:
            ...
            logger.debug(
                f"Model {model_name} released tensor view {self.name}, reference count: {self.reference_count}"
            )
            return True
        return False
    ...
    def __repr__(self) -> str:
        return (
            f"SharedTensorView(name={self.name}, parent={self.parent.name}, "
...
class TensorSharingManager:
    ...
    def register_shared_tensor(
        ...
        logger.info(
            f"Registered shared tensor {name} with shape {shape} and storage type {storage_type}"
        )
        return tensor

    def get_shared_tensor(
        self, name: str, model_name: Optional[str] = None
    ) -> Optional[SharedTensor]:
        """
    ...
    def share_tensor_between_models(
        self, tensor_name: str, from_model: str, to_models: List[str]
    ) -> bool:
        """
    ...
    def optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        for name, tensor in list(self.tensors.items()):
            if tensor.can_be_freed():
                ...
                logger.info(
                    f"Freed unused tensor {name}, saved {tensor_memory / (1024 * 1024):.2f} MB"
                )
        ...
        result = {
            ...
            "memory_reduction_percent": (freed_memory / initial_memory * 100)
            if initial_memory > 0
            else 0,
        ...
        logger.info(
            f"Memory optimization complete: freed {freed_memory / (1024 * 1024):.2f} MB ({result['memory_reduction_percent']:.1f}%)"
        )
        return result
...
def create_tensor_sharing_demo():
    ...
    manager.share_tensor_between_models(
        tensor_name="vit_embedding", from_model="vit", to_models=["llava", "xclip"]
    )
