...
class WebGPUQuantizer:
    ...
    def __init__(self, bits=4, group_size=128, scheme="symmetric"):
        ...
        logger.info(
            f"Initialized WebGPU quantizer with {bits}-bit precision, group_size={group_size}, scheme={scheme}"
        )
    ...
    def quantize_tensor(self, tensor: np.ndarray) -> Dict[str, Any]:
        ...
        if self.group_size <= 0 or tensor.size <= self.group_size:
            ...
            if self.scheme == "symmetric":
                ...
                scale = (
                    (tensor_max - tensor_min) / (max_val - min_val)
            ...
            zero_points = (
                np.array([zero_point], dtype=np.float32) if self.zero_point_enabled else None
            )
            ...
            zero_points = (
                np.zeros((num_groups, num_cols), dtype=np.float32)
            ...
            for g in range(num_groups):
                ...
                for c in range(num_cols):
                    if self.zero_point_enabled:
                        ...
                        quantized_groups[g, :, c] = np.clip(
                            np.round(group_data[:, c] / group_scales[c]), min_val, max_val
                        )
            ...
            if len(quantized.shape) > 1:
                ...
                if quantized.shape[-1] % 2 == 1:
                    ...
                    quantized = np.concatenate(
                        [quantized, np.zeros(pad_shape, dtype=quantized.dtype)], axis=-1
                    )
        ...
        if len(scales) == 1:  # Per-tensor quantization
            ...
            for g in range(num_groups):
                ...
                if self.zero_point_enabled:
                    ...
                    for c in range(num_cols):
                        dequantized_groups[g, :, c] = (
                            group_data[:, c] - group_zero_points[c]
                        ) * group_scales[c]
                else:
                    for c in range(num_cols):
...
def quantize_model_weights(
    model, quantizer: WebGPUQuantizer = None, model_type: str = "llm"
) -> Dict[str, Any]:
    """
    ...
    metadata = {
        ...
        "num_parameters": sum(
            w.data.size if not w["quantized"] else w["data"].size * (8 / w["bits"])
    ...
    return {"weights": quantized_weights, "metadata": metadata}
        ...
        if self.quantized_weights is None:
            ...
            try:
                ...
                self.quantized_weights = {
                    "metadata": {"model_type": self.model_type, "quantization_bits": 4}
                }
...
def setup_4bit_inference(model, model_type=None, config=None, device="webgpu"):
    ...
    final_config = {"bits": 4, "group_size": 128, "scheme": "symmetric", "model_type": "llm"}
    ...
    handler = WebGPU4BitInferenceHandler(
        model_path=None, quantized_weights=quantized_model, model_type=model_type
    )
...
def compare_quantization_accuracy(model, test_inputs, bits_options=None):
    ...
    for bits_key, result in results.items():
        if bits_key == "fp16":
            ...
            result["relative_memory"] = (
                result["memory_usage_mb"] / results["fp16"]["memory_usage_mb"]
            )
...
if __name__ == "__main__":
    ...
    memory_reduction = quantizer.estimate_memory_reduction(
        sample_tensor.size * sample_tensor.itemsize
    )
