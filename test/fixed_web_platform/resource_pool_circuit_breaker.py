...
class BrowserHealthMetrics:
    ...
    def record_resource_usage(
        self, memory_mb: float, cpu_percent: float, gpu_percent: Optional[float] = None
    ):
        """
    ...
    def record_model_performance(self, model_name: str, inference_time_ms: float, success: bool):
        ...
        if len(self.model_performance[model_name]["inference_times"]) > 100:
            self.model_performance[model_name]["inference_times"] = self.model_performance[
                model_name
            ]["inference_times"][-100:]
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reconnection_success_rate": self.reconnection_successes
            / max(1, self.reconnection_attempts),
            ...
            "last_error_seconds_ago": time.time() - self.last_error_time
...
class ResourcePoolCircuitBreaker:
        ...
        async with self.circuit_locks[connection_id]:
            ...
            if circuit["state"] == CircuitState.HALF_OPEN:
                ...
                if circuit["successes"] >= self.success_threshold:
                    ...
                    logger.info(
                        f"Circuit for connection {connection_id} closed after {circuit['successes']} consecutive successes"
                    )
    ...
    async def record_failure(self, connection_id: str, error_type: str):
        ...
        async with self.circuit_locks[connection_id]:
            ...
            if (
                ...
                circuit["last_state_change_time"] = time.time()
                logger.warning(
                    f"Circuit for connection {connection_id} opened after {circuit['failures']} consecutive failures"
                )
            ...
            # If circuit is half open, any failure opens it
                ...
                logger.warning(
                    f"Circuit for connection {connection_id} reopened after failure in half-open state"
                )
    ...
    async def record_resource_usage(
        ...
        """
        if connection_id in self.health_metrics:
            self.health_metrics[connection_id].record_resource_usage(
                memory_mb, cpu_percent, gpu_percent
            )
    ...
    async def record_model_performance(
        self, connection_id: str, model_name: str, inference_time_ms: float, success: bool
    ):
        """
        ...
        """
        if connection_id in self.health_metrics:
            self.health_metrics[connection_id].record_model_performance(
                model_name, inference_time_ms, success
            )
        ...
        async with self.circuit_locks[connection_id]:
            ...
            # If circuit is open, check if reset timeout has elapsed
                ...
                if time_since_last_state_change >= self.reset_timeout_seconds:
                    ...
                    circuit["failures"] = 0
                    logger.info(
                        f"Circuit for connection {connection_id} transitioned to half-open state for testing"
                    )
        ...
        for connection_id in list(self.circuits.keys()):
            try:
                ...
                if circuit["state"] == CircuitState.OPEN:
                    ...
                    if time_since_last_state_change < self.reset_timeout_seconds:
                        logger.debug(
                            f"Skipping health check for connection {connection_id} (circuit open)"
                        )
                        continue
...
class ConnectionHealthChecker:
    ...
    def __init__(
        self, circuit_breaker: ResourcePoolCircuitBreaker, browser_connections: Dict[str, Any]
    ):
        """
        ...
        try:
            ...
            if "resource_usage" in response:
                ...
                if memory_mb > 1000:  # 1GB threshold
                    logger.warning(
                        f"Connection {connection_id} high memory usage: {memory_mb:.1f} MB"
                    )
    ...
    async def get_connection_health_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for connection_id in self.browser_connections.keys():
            ...
            results[connection_id] = {
                ...
                "health_score": circuit_state["health_metrics"]["health_score"]
                if circuit_state and "health_metrics" in circuit_state
                else 0,
                "connection_drops": circuit_state["health_metrics"]["connection_drops"]
                if circuit_state and "health_metrics" in circuit_state
                else 0,
                "reconnection_attempts": circuit_state["health_metrics"]["reconnection_attempts"]
                if circuit_state and "health_metrics" in circuit_state
                else 0,
                "last_error_seconds_ago": circuit_state["health_metrics"]["last_error_seconds_ago"]
                if circuit_state and "health_metrics" in circuit_state
                else None,
...
class ConnectionRecoveryStrategy:
    ...
    async def recover_connection(
        ...
        error_category: ConnectionErrorCategory,
    ) -> bool:
        """
        ...
        """
        logger.info(
            f"Attempting to recover connection {connection_id} from {error_category.value} error"
        )
        ...
        if error_category == ConnectionErrorCategory.TIMEOUT:
            ...
            return await self._recover_from_connection_closed(
                connection_id, connection, circuit_state
            )
            ...
            ConnectionErrorCategory.INFERENCE,
        ]:
            return await self._recover_from_operation_error(
                connection_id, connection, circuit_state, error_category
            )
    ...
    async def _recover_from_timeout(
        self, connection_id: str, connection: Dict[str, Any], circuit_state: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if ping_success:
                logger.info(
                    f"Connection {connection_id} recovered from timeout error (ping successful)"
                )
            ...
            logger.warning(
                f"Error during timeout recovery ping for connection {connection_id}: {e}"
            )
    ...
    async def _recover_from_connection_closed(
        self, connection_id: str, connection: Dict[str, Any], circuit_state: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _recover_from_websocket_error(
        self, connection_id: str, connection: Dict[str, Any], circuit_state: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _recover_from_resource_error(
        self, connection_id: str, connection: Dict[str, Any], circuit_state: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _recover_from_operation_error(
        self,
        connection_id: str,
        connection: Dict[str, Any],
...
class ResourcePoolCircuitBreakerManager:
    ...
    async def initialize(self):
        ...
        # Start health check task
        await self.circuit_breaker.start_health_check_task(
            self.health_checker.check_connection_health
        )

        logger.info(
            f"Circuit breaker manager initialized with {len(self.browser_connections)} connections"
        )
    ...
    async def handle_error(
        ...
        # Attempt recovery
        recovery_success = await self.recovery_strategy.recover_connection(
            connection_id, connection, error_category
        )

        if recovery_success:
            logger.info(
                f"Successfully recovered connection {connection_id} from {error_category.value} error"
            )
        else:
            logger.warning(
                f"Failed to recover connection {connection_id} from {error_category.value} error"
            )
    ...
    async def get_health_summary(self) -> Dict[str, Any]:
        ...
        open_circuit_count = sum(
            1 for health in connection_health.values() if health["circuit_state"] == "OPEN"
        )
        half_open_circuit_count = sum(
            1 for health in connection_health.values() if health["circuit_state"] == "HALF_OPEN"
        )

        # Calculate overall health score
        if connection_count > 0:
            overall_health_score = (
                sum(health["health_score"] for health in connection_health.values())
                / connection_count
            )
        else:
    ...
    try:
        ...
        for i in range(10):
            ...
            if allowed:
                ...
                if success:
                    logger.info(
                        f"Request {i + 1} successful (response time: {response_time:.1f}ms)"
                    )
                    await circuit_breaker_manager.record_request_result(
                        connection_id, True, response_time_ms=response_time
                    )
                else:
                    ...
                    await circuit_breaker_manager.record_request_result(
                        connection_id, False, error_type=error_type
                    )
                    ...
                    recovery_success = await circuit_breaker_manager.handle_error(
                        connection_id, error, error_context
                    )
                    logger.info(
                        f"Recovery {'successful' if recovery_success else 'failed'} for connection {connection_id}"
                    )
            else:
                logger.warning(
                    f"Request {i + 1} not allowed for connection {connection_id}: {reason}"
                )
