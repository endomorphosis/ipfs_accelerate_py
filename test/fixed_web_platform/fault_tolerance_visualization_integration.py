...
class FaultToleranceValidationSystem:
    ...
    async def run_validation_with_visualization(
        self,
        ...
        # Create validator
        self.logger.info(
            f"Creating validator with level {fault_tolerance_level} and strategy {recovery_strategy}"
        )
        ...
        self.logger.info(
            f"Validation completed in {validation_time:.2f}s with status: {validation_results.get('validation_status', 'unknown')}"
        )
        ...
        if generate_report:
            visualizations = await self._generate_visualizations(
                validation_results, report_name=report_name, ci_compatible=ci_compatible
            )
    ...
    async def run_comparative_validation(
        ...
        for level in levels:
            for strategy in strategies:
                ...
                self.logger.info(
                    f"Configuration {config_key} completed in {validation_time:.2f}s with status: {validation_results.get('validation_status', 'unknown')}"
                )
        ...
        if self.output_dir:
            await self._generate_comparative_visualizations(
                results_by_config, report_prefix=report_prefix
            )
    ...
    async def run_stress_test_validation(
        ...
        test_scenarios: Optional[List[str]] = None,
        report_name: str = "stress_test_report.html",
    ) -> Dict[str, Any]:
        """
        ...
        # Configure test scenarios
        if test_scenarios is None:
            test_scenarios = ["connection_lost", "browser_crash", "multi_browser_failure"]
        ...
        for i in range(iterations):
            ...
            self.logger.info(
                f"Iteration {i + 1} completed in {validation_time:.2f}s with status: {validation_results.get('validation_status', 'unknown')}"
            )
    ...
    async def _generate_visualizations(
        ...
        try:
            ...
            recovery_vis = await anyio.to_thread.run_sync(
                visualizer.generate_recovery_time_comparison, recovery_time_path
            )
            if recovery_vis:
            ...
            success_vis = await anyio.to_thread.run_sync(
                visualizer.generate_success_rate_dashboard, success_rate_path
            )
            if success_vis:
            ...
            perf_vis = await anyio.to_thread.run_sync(
                visualizer.generate_performance_impact_visualization, perf_impact_path
            )
            if perf_vis:
            ...
            if ci_compatible:
                report = await anyio.to_thread.run_sync(
                    visualizer.generate_ci_compatible_report, report_path
                )
            else:
                report = await anyio.to_thread.run_sync(
                    visualizer.generate_comprehensive_report, report_path
                )
    ...
    async def _generate_comparative_visualizations(
        self, results_by_config: Dict[str, Dict[str, Any]], report_prefix: str = "comparative"
    ) -> Dict[str, str]:
        """
    ...
    async def _generate_stress_test_report(
        self, stress_test_results: Dict[str, Any], report_name: str = "stress_test_report.html"
    ) -> Optional[str]:
        """
...
async def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./reports", help="Directory for output files"
    )
    ...
    parser.add_argument("--test-scenarios", type=str, help="Comma-separated list of test scenarios")
    ...
    parser.add_argument(
        "--report-name",
        type=str,
        default="fault_tolerance_report.html",
    ...
    try:
        ...
        validation_system = FaultToleranceValidationSystem(
            model_manager=manager, output_dir=args.output_dir
        )
        ...
        if args.comparative:
            ...
            validation_status = results.get("validation_results", {}).get(
