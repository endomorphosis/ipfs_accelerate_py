...
class WebSocketBridge:
    ...
    async def handle_message(self, message_data: str):
        ...
        if HAS_ERROR_FRAMEWORK:
            context = {
                ...
                "message_preview": message_data[:50] + "..."
        ...
        try:
            ...
            error_context = {
                ...
                "line": e.lineno,
                "column": e.colno,
                "preview": message_data[max(0, e.pos - 20) : min(len(message_data), e.pos + 20)]
                if e.pos
                else message_data[:100],
            }
            ...
            if HAS_ERROR_FRAMEWORK:
                ...
                logger.error(
                    f"Failed to parse message as JSON at position {e.pos}: {message_data[:100]}"
                )
        ...
        while attempt <= retry_attempts:
            try:
                if attempt > 0:
                    ...
                    logger.info(
                        f"Retry {attempt}/{retry_attempts} waiting for WebSocket connection (backoff: {backoff_delay}s)"
                    )
                    ...
                    logger.info(
                        f"Connection attempt {attempt + 1}: {elapsed:.1f}s elapsed, {retry_attempts - attempt} attempts remaining"
                    )

                # Wait for connection with timeout
                await wait_for(self.connection_event.wait(), timeout=timeout)
                logger.info(
                    f"WebSocket connection established successfully after {attempt} retries ({time.time() - connection_start:.1f}s)"
                )
                ...
                if attempt > retry_attempts:
                    logger.warning(
                        f"Timeout waiting for WebSocket connection after {retry_attempts} retries (timeout={timeout}s, elapsed={time.time() - connection_start:.1f}s)"
                    )
                ...
                logger.warning(
                    f"Timeout waiting for WebSocket connection, retrying ({attempt}/{retry_attempts}, timeout={timeout:.1f}s)"
                )
        ...
        async def send_message(self, message, timeout=None, retry_attempts=2):
            ...
            if not self.is_connected or not self.connection:
                ...
                if not self.connection_event.is_set():
                    ...
                    connection_success = await self.wait_for_connection(
            ...
            try:
                ...
                await wait_for(self.connection.send(message_json), timeout=timeout)
                return True
        ...
        async def send_message(self, message, timeout=None, retry_attempts=2):
            ...
            while attempt <= retry_attempts:
                try:
                    ...
                    timeout = timeout * 1.2
                    await anyio.sleep(0.1)  # Brief pause before retry
                    ...
                    reconnected = await self.wait_for_connection(
                        timeout=self.connection_timeout / 2
                    )
                    if not reconnected:
                    ...
                    await anyio.sleep(0.2)  # Slightly longer pause for general errors
    ...
    async def send_and_wait(self, message, timeout=None, retry_attempts=1, response_timeout=None):
        ...
        send_success = await self.send_message(
            message, timeout=timeout, retry_attempts=retry_attempts
        )
        if not send_success:
        ...
        try:
            ...
            logger.error(
                f"Timeout waiting for response to message {msg_id} (timeout={response_timeout}s)"
            )
            return None
    ...
    async def get_browser_capabilities(self, retry_attempts=2):
        ...
        if capabilities:
            ...
            logger.info(
                f"Detected capabilities - WebGPU: {webgpu_support}, WebNN: {webnn_support}, Compute Shaders: {compute_shaders}"
            )
            ...
            if adapter:
                logger.info(
                    f"WebGPU Adapter: {adapter.get('vendor', 'Unknown')} - {adapter.get('device', 'Unknown')}"
                )
    ...
    async def initialize_model(
        self, model_name, model_type, platform, options=None, retry_attempts=2
    ):
        """
        ...
        if not self.is_connected:
            logger.info(
                f"Not connected, attempting to establish connection for model initialization: {model_name}"
            )
            if not await self.wait_for_connection(retry_attempts=retry_attempts):
        ...
        response = await self.send_and_wait(
            request, retry_attempts=retry_attempts, response_timeout=120.0
        )  # Longer timeout for model init
        ...
        if init_status == "success":
            ...
            if "adapter_info" in response:
                logger.info(
                    f"Using adapter: {response['adapter_info'].get('vendor', 'Unknown')} - {response['adapter_info'].get('device', 'Unknown')}"
                )
    ...
    async def run_inference(
        self, model_name, input_data, platform, options=None, retry_attempts=1, timeout_multiplier=2
    ):
        """
        ...
        if not self.is_connected:
            logger.info(
                f"Not connected, attempting to establish connection for inference: {model_name}"
            )
            if not await self.wait_for_connection(retry_attempts=retry_attempts):
        ...
        if options:
            if isinstance(options, dict):
                ...
                logger.warning(
                    f"Options for inference with {model_name} are not a dictionary: {options}"
                )
        ...
        logger.info(
            f"Running inference with model {model_name} on {platform} (input size: {input_size})"
        )
        ...
        response = await self.send_and_wait(
            ...
            response_timeout=inference_timeout
            * 1.5,  # Even longer timeout for waiting for response
        )
        ...
        if not response:
            logger.error(
                f"Failed to run inference with model {model_name} after {inference_time:.2f}s"
            )
        ...
        if inference_status == "success":
            ...
            if (
                "performance_metrics" in response
                and "throughput_items_per_sec" in response["performance_metrics"]
            ):
    ...
    def _preprocess_input_data(self, model_name, input_data):
        ...
        try:
            ...
            if isinstance(input_data, dict):
                ...
                logger.warning(
                    f"Unknown input data type for model {model_name}: {type(input_data)}"
                )
    ...
    def _get_input_metadata(self, input_data):
        ...
        """
        metadata = {"type": type(input_data).__name__, "estimated_size": "unknown"}
