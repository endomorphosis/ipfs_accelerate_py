...
class BrowserCPUDetector:
    ...
    def __init__(self, simulate_browser: bool = True):
        ...
        logger.info(
            f"Browser CPU detection initialized with {self.capabilities['detected_cores']} cores "
    ...
    def _detect_cpu_capabilities(self) -> Dict[str, Any]:
        ...
        if browser_name == "safari":
            ...
            capabilities["browser_limits"] = {
                "max_workers": capabilities["detected_cores"]
                + 2,  # Firefox can handle more workers
        ...
        capabilities["effective_cores"] = min(
            capabilities["effective_cores"], capabilities["browser_limits"]["max_workers"]
        )
    ...
    def _create_thread_pool_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "min_threads": 1,
            "scheduler_type": "priority"
            if self.capabilities["thread_priorities_supported"]
            else "round-robin",
    ...
    def _calculate_worker_distribution(self, core_count: int) -> Dict[str, int]:
        ...
        if core_count <= 2:
            ...
            return {"compute": 1, "io": 1, "utility": 0}
        elif core_count <= 4:
            # Distribution for 3-4 cores
            return {"compute": core_count - 1, "io": 1, "utility": 0}
        else:
            ...
            return {"compute": compute, "io": io, "utility": utility}
    ...
    def update_capabilities(self, **kwargs) -> None:
        ...
        if updated:
            ...
            logger.info(
                f"CPU capabilities updated. Effective cores: {self.capabilities['effective_cores']}, "
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.capabilities["effective_cores"] < self.capabilities["detected_cores"]:
            recommendations.append(
                f"Consider increasing thread count from {self.capabilities['effective_cores']} "
        ...
        if browser_name == "safari" and self.capabilities["effective_cores"] > 2:
            recommendations.append(
                "Safari has limited worker performance, consider reducing worker count"
            )
        ...
        if workload_type == "inference":
            ...
            config["worker_distribution"] = {
                ...
                "utility": max(
                    0,
                    effective_cores
                    - int(effective_cores * 0.7)
                    - max(1, int(effective_cores * 0.2)),
                ),
            }
            ...
            config["worker_distribution"] = {
                ...
                "utility": max(
                    0,
                    effective_cores
                    - int(effective_cores * 0.6)
                    - max(1, int(effective_cores * 0.3)),
                ),
            }
            ...
            config["worker_distribution"] = {
                ...
                "utility": max(
                    0,
                    effective_cores
                    - int(effective_cores * 0.3)
                    - max(1, int(effective_cores * 0.4)),
                ),
            }
    ...
    def estimate_threading_benefit(self, core_count: int, model_size: str) -> Dict[str, Any]:
        ...
        if core_count <= max_useful_cores:
            ...
            efficiency = parallel_efficiency * (
                1 - ((max_useful_cores - 1) * 0.05) - ((core_count - max_useful_cores) * 0.15)
            )
        ...
        estimation["theoretical_max_speedup"] = (
            1 / sequential_portion
        )  # Theoretical maximum with infinite cores
...
class ThreadPoolManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize workers
        self.workers = {"compute": [], "io": [], "utility": []}

        # Task queues with priorities
        self.task_queues = {"high": [], "normal": [], "low": [], "background": []}
        ...
        logger.info(
            f"Thread pool created with {sum(len(workers) for workers in self.workers.values())} workers"
        )
    ...
    def complete_task(self, task_id: str, result: Any = None) -> None:
        ...
        if self.stats["tasks_completed"] == 1:
            ...
            self.stats["avg_wait_time_ms"] = (
                self.stats["avg_wait_time_ms"] * (self.stats["tasks_completed"] - 1) + wait_time_ms
            ) / self.stats["tasks_completed"]
...
def create_thread_pool(core_count: int, scheduler_type: str = "priority") -> ThreadPoolManager:
    ...
    if core_count <= 2:
        distribution = {"compute": max(1, core_count - 1), "io": 1, "utility": 0}
    elif core_count <= 4:
        distribution = {"compute": core_count - 2, "io": 1, "utility": 1}
    else:
        utility = max(1, int(core_count * 0.2))  # 20% for utility
        io = max(1, int(core_count * 0.2))  # 20% for I/O
        compute = core_count - utility - io  # Rest for compute

        distribution = {"compute": compute, "io": io, "utility": utility}
...
def measure_threading_overhead(core_count: int) -> Dict[str, float]:
    ...
    return {
        ...
        "overhead_percent": (overhead_per_task_ms / (10 + overhead_per_task_ms))
        * 100,  # Assume 10ms task time
    }
...
if __name__ == "__main__":
    ...
    print(
        f"Background mode - Threads: {bg_config['max_threads']}, "
    ...
    print(
        f"Foreground mode - Threads: {fg_config['max_threads']}, "
    ...
    print(
        f"High load - Threads: {hl_config['max_threads']}, "
    ...
    for size in ["small", "medium", "large"]:
        ...
        print(
            f"{size} model - Batch size: {workload['batch_size']}, "
    ...
    for cores in [2, 4, 8]:
        for size in ["small", "large"]:
            ...
            print(
                f"{cores} cores, {size} model - Speedup: {benefit['speedup_factor']:.2f}x, "
    ...
    print(
        f"Thread pool created with {sum(len(workers) for workers in pool.workers.values())} workers"
    )
    ...
    print(
        f"Pool stats - Completed: {stats['tasks_completed']}, "
