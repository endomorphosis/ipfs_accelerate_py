...
'''
        ...
        self.converter = APIBackendConverter(
            python_file=self.sample_python_file, output_dir=self.ts_dir, dry_run=True
        )
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.converter.main_file,
            os.path.join(self.ts_dir, "sample_backend", "sample_backend.ts"),
        )
        self.assertEqual(
            self.converter.index_file, os.path.join(self.ts_dir, "sample_backend", "index.ts")
        )
        self.assertEqual(
            self.converter.types_file, os.path.join(self.ts_dir, "sample_backend", "types.ts")
        )
    ...
    def test_to_camel_case_lower(self):
        ...
        self.assertEqual(
            self.converter._to_camel_case_lower("is_compatible_model"), "isCompatibleModel"
        )
...
"""
    ...
    def test_backend_specific_customizations(self):
        ...
        claude_request = next(
            (t for t in claude_converter.api_specific_types if t["name"] == "ClaudeRequest"), None
        )
...
class TestMainFunctionality(unittest.TestCase):
    ...
    @patch(
        "sys.argv",
        ["convert_api_backends.py", "--backend", "sample_backend", "--fix-source", "--dry-run"],
    )
    @patch("convert_api_backends.APIBackendConverter")
    def test_main_with_fix_source(self, mock_converter):
        ...
        # Run main function
        with patch("os.path.exists", return_value=True), patch("builtins.open", mock_open_obj):
            main()
