...
def verify_syntax(file_path):
    ...
    try:
        result = subprocess.run(
            ["python", "-m", "py_compile", file_path], capture_output=True, text=True, check=False
        )
...
def update_architecture_types(model_name, architecture_type):
    ...
    try:
        ...
        if arch_type_start == -1:
            logger.warning(
                f"Architecture type '{architecture_type}' not found in ARCHITECTURE_TYPES"
            )
            return False
        ...
        if (
            ...
            logger.info(
                f"Model '{model_name}' already exists in architecture type '{architecture_type}'"
            )
            return True
