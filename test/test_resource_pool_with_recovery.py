...
def test_resource_pool_initialization():
    ...
    logger.info(
        f"ResourcePool initialized with WebNN/WebGPU support: {pool.web_resource_pool_initialized}"
    )
    ...
    if stats["web_resource_pool"]["initialized"]:
        if "metrics" in stats["web_resource_pool"]:
            logger.info(
                f"Web resource pool metrics: {json.dumps(stats['web_resource_pool']['metrics'], indent=2)}"
            )
        else:
...
def test_get_model_with_web_preference():
    ...
    model = pool.get_model(
        "text", "bert-test", constructor=mock_constructor, hardware_preferences=hardware_preferences
    )
