...
def update_roadmap_status(implemented_models: Dict[str, Set[str]]) -> bool:
    ...
    for line in lines:
        if line.strip().startswith("- ["):
            ...
            if match:
                ...
                is_implemented = (
                    model_name in normalized_implemented
                    or model_name_norm in normalized_implemented
                )
        ...
        if "## Current Status" in line:
            ...
            for j in range(i + 1, min(i + 15, len(updated_lines))):
                if "**Total Models Tracked:**" in updated_lines[j]:
                    ...
                    updated_lines[j] = (
                        f"- **Implemented Models:** {implemented_count} ({percentage}%)\n"
                    )
    ...
    print(
        f"Updated roadmap status: {implemented_count}/{total_models} models implemented ({round(implemented_count / total_models * 100, 1)}%)"
    )
    return current_status_updated


def generate_coverage_report(
    implemented_models: Dict[str, Set[str]], roadmap_models: Dict[str, Dict[str, bool]]
) -> str:
    ...
    implemented_in_roadmap = sum(
        1
        for model in roadmap_models
        if model in normalized_implemented or model.replace("_", "-") in normalized_implemented
    )
    ...
    for priority, (implemented, total) in priority_counts.items():
        ...
        if implemented > 0:
            implemented_models = sorted(
                [
                    model
                    for model in priority_models[priority]
                    if model in normalized_implemented
                    or model.replace("_", "-") in normalized_implemented
                ]
            )
        ...
        missing_models = sorted(
            [
                model
                for model in priority_models[priority]
                if model not in normalized_implemented
                and model.replace("_", "-") not in normalized_implemented
            ]
        )
        if missing_models:
