...
try:
    ...
    from test.integration.bridge_config import (
        load_config,
        get_bridge_config,
        save_config,
...
class BridgeService:
    ...
    def initialize_bridges(self):
        ...
        if bp_config.get("enabled", True):
            try:
                self.bridges["benchmark_predictive_performance"] = (
                    BenchmarkPredictivePerformanceBridge(
                        benchmark_db_path=bp_config.get("benchmark_db_path", "benchmark_db.duckdb"),
                        predictive_api_url=bp_config.get(
    ...
    def run_sync_cycle(self) -> Dict[str, Any]:
        ...
        if "benchmark_predictive_performance" in self.bridges:
            ...
            try:
                ...
                recent_results = bridge.sync_recent_results(limit=bp_config.get("sync_limit", 100))
        ...
        try:
            ...
            while self.running:
                ...
                if auto_sync:
                    ...
                    logger.info(
                        "Auto-sync is disabled. Service will remain running but won't auto-sync"
                    )
...
def main():
    ...
    parser.add_argument(
        "--run-once", action="store_true", help="Run one synchronization cycle and exit"
    )
    parser.add_argument(
        "--report-only", action="store_true", help="Generate synchronization report and exit"
    )
