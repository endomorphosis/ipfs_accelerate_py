...
class MockCoordinator:
    ...
    def _process_message(self, message, addr):
        ...
        if msg_type == "register_worker":
            ...
            return {"type": "register_response", "worker_id": worker_id, "success": True}
    ...
    def add_task(self, test_file, parameters=None):
        ...
        task = {"task_id": task_id, "test_file": test_file, "parameters": parameters or {}}
...
class MockWorker:
    ...
    def __init__(self, coordinator_address, capabilities=None):
        ...
        self.capabilities = capabilities or {"hardware": ["cpu"], "browsers": []}
        self.worker_id = None
        self.running = False
    ...
    def connect(self):
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
                ...
                message = {"type": "register_worker", "capabilities": self.capabilities}
    ...
    def _process_task(self, task):
        ...
        self.task_results.append({"task_id": task["task_id"], "result": result})
...
def multi_workers(coordinator_address):
    ...
    workers.append(
        MockWorker(coordinator_address, capabilities={"hardware": ["cpu"], "browsers": []})
    )

    workers.append(
        MockWorker(coordinator_address, capabilities={"hardware": ["cpu", "cuda"], "browsers": []})
    )

    workers.append(
        MockWorker(coordinator_address, capabilities={"hardware": ["cpu"], "browsers": ["chrome"]})
    )
...
class TestDistributedCoordinator:
    ...
    def test_task_distribution(self, coordinator, worker):
        ...
        while worker.tasks_processed < 5:
            if time.time() - start_time > max_wait:
                pytest.fail(
                    f"Worker processed only {worker.tasks_processed} tasks in {max_wait} seconds"
                )
            time.sleep(0.1)
    ...
    def test_capability_based_task_distribution(self, coordinator):
        ...
        # Create workers with different capabilities
        cpu_worker = MockWorker(
            f"localhost:{coordinator.port}", capabilities={"hardware": ["cpu"], "browsers": []}
        )
        gpu_worker = MockWorker(
            f"localhost:{coordinator.port}",
            capabilities={"hardware": ["cpu", "cuda"], "browsers": []},
        )
        ...
        cpu_tasks = []
        for i in range(3):
            task_id = coordinator.add_task(f"cpu_test_{i}.py", {"required_hardware": "cpu"})
            cpu_tasks.append(task_id)

        gpu_tasks = []
        for i in range(3):
            task_id = coordinator.add_task(f"gpu_test_{i}.py", {"required_hardware": "cuda"})
