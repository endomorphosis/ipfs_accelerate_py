...
class BenchmarkManager:
    ...
    async def start_benchmark_run(
        self, request: BenchmarkRunRequest, background_tasks: BackgroundTasks
        ...
        # Log start of run
        logger.info(
            f"Starting benchmark run {run_id} with configuration: {json.dumps(request.dict())}"
        )
    ...
    async def _run_benchmark(
        ...
        try:
            ...
            pipeline = BenchmarkPipeline(
                ...
                on_progress_callback=lambda progress, step, completed, total: anyio.from_thread.run(
                    self._update_progress, run_id, progress, step, completed, total
                ),
            )
    ...
    async def _update_progress(
        self, run_id: str, progress: float, step: str, completed_models: int, total_models: int
    ):
        """
        ...
        self.active_runs[run_id]["total_models"] = (
            total_models or self.active_runs[run_id]["total_models"]
        )
    ...
    def get_run_results(self, run_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            if "completed_at" not in run_data
            else run_data["completed_at"].isoformat(),
        }
    ...
    async def handle_websocket_connection(self, websocket: WebSocket, run_id: str):
        ...
        if run_id not in self.active_runs:
            await websocket.send_json({"error": f"Benchmark run {run_id} not found"})
        ...
        try:
            ...
            with BenchmarkDBContext(self.db_path) as db:
                ...
                metrics = db.get_performance_metrics(
                    model_name=model, hardware_type=hardware, batch_size=batch_size, limit=limit
                )
...
async def startup_event():
    ...
    if __name__ == "__main__":
        ...
        manager = BenchmarkManager({"db_path": args.db_path, "results_dir": args.results_dir})
    else:
...
async def start_benchmark_run(request: BenchmarkRunRequest, background_tasks: BackgroundTasks):
    """
...
def main():
    ...
    uvicorn.run("benchmark_api_server:app", host=args.host, port=args.port, reload=False)
