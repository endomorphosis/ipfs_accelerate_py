...
try:
    ...
    print(
        "Warning: websocket-client package not installed. WebSocket monitoring will not be available."
    )
...
def main():
    ...
    parser.add_argument(
        "--server", default="http://localhost:8000", help="Benchmark API server URL"
    )
    ...
    start_parser.add_argument(
        "--hardware", nargs="+", default=["cpu"], help="Hardware backends to use"
    )
    start_parser.add_argument("--models", nargs="+", help="Specific models to benchmark")
    start_parser.add_argument(
        "--batch-sizes", nargs="+", type=int, default=[1, 8], help="Batch sizes to benchmark"
    )
    start_parser.add_argument(
        "--precision", default="fp32", help="Precision to use (fp32, fp16, etc.)"
    )
    start_parser.add_argument(
        "--progressive-mode", action="store_true", help="Use progressive complexity mode"
    )
    start_parser.add_argument(
        "--incremental", action="store_true", help="Run only missing or outdated benchmarks"
    )
    start_parser.add_argument(
        "--monitor", action="store_true", help="Monitor progress after starting"
    )
    ...
    results_parser = subparsers.add_parser(
        "results", help="Get the results of a completed benchmark run"
    )
    results_parser.add_argument("run_id", help="ID of the benchmark run")

    # Monitor command
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor the progress of a benchmark run"
    )
