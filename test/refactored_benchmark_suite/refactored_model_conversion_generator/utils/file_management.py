...
class ModelFileManager:
    ...
    @staticmethod
    def find_models(
        directory: str, formats: Optional[List[str]] = None, recursive: bool = True
    ) -> Dict[str, List[str]]:
        """
    ...
    @staticmethod
    def organize_models(
        source_dir: str, target_dir: str, structure: str = "format/model", copy: bool = True
    ) -> Dict[str, str]:
        """
            ...
            for model_path in model_paths:
                ...
                if structure == "format/model":
                    target_path = os.path.join(
                        target_dir, fmt, model_name, os.path.basename(model_path)
                    )
                else:  # model/format
                    target_path = os.path.join(
                        target_dir, model_name, fmt, os.path.basename(model_path)
                    )
    ...
    @staticmethod
    def get_cache_path(
        model_path: str, source_format: str, target_format: str, cache_dir: Optional[str] = None
    ) -> str:
        """
        ...
        cache_path = os.path.join(
            cache_dir, f"{model_name}_{source_format}_to_{target_format}_{file_hash[:8]}"
        )
    ...
    def check_cached_model(
        ...
        cache_path = ModelFileManager.get_cache_path(
            model_path, source_format, target_format, cache_dir
        )
