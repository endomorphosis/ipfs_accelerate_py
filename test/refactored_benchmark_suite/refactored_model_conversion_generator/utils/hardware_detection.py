...
class HardwareDetector:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if any(x in device_name for x in ["amd", "radeon"]):
                    info = {"device_count": torch.cuda.device_count(), "devices": []}
    ...
    def detect_openvino() -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                core = Core()
                info = {"version": get_version(), "devices": core.available_devices}
                return True, info
            except ImportError:
                ...
                try:
                    ...
                    ie = IECore()
                    info = {"devices": ie.available_devices}
                    return True, info
                except ImportError:
                    return False, {}
        except Exception:
    ...
    def detect_mps() -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if platform.system() == "Darwin" and platform.machine() in ["arm64", "x86_64"]:
                ...
                return True, {"system": platform.system(), "machine": platform.machine()}
            else:
                return False, {}
    ...
    def detect_webnn() -> Tuple[bool, Dict[str, Any]]:
        ...
        has_capable_cpu = (
            cpu_info.get("vendor") in ["GenuineIntel", "AuthenticAMD"]
            and cpu_info.get("features")
            and any(f in cpu_info["features"] for f in ["avx2", "avx512", "vnni"])
        )
        ...
        try:
            ...
            for path in qnn_paths:
                if os.path.exists(path):
                    return True, {"sdk_path": path, "platform": platform.system()}
            ...
            if platform.system() == "Linux" and "android" in platform.version().lower():
                try:
                    ...
                    result = subprocess.run(
                        ["getprop", "ro.board.platform"], capture_output=True, text=True
                    )
                    if result.returncode == 0 and any(
                        x in result.stdout.lower() for x in ["snapdragon", "msm", "sm"]
                    ):
                        return True, {"platform": "Android", "board": result.stdout.strip()}
                except:
                    pass
    ...
    def get_available_hardware() -> Dict[str, Dict[str, Any]]:
        ...
        hardware_info["cpu"] = {"available": True, "info": HardwareDetector.get_cpu_info()}

        # Check CUDA (NVIDIA GPU)
        cuda_available, cuda_info = HardwareDetector.detect_cuda()
        hardware_info["cuda"] = {"available": cuda_available, "info": cuda_info}

        # Check ROCm (AMD GPU)
        rocm_available, rocm_info = HardwareDetector.detect_rocm()
        hardware_info["rocm"] = {"available": rocm_available, "info": rocm_info}

        # Check OpenVINO
        openvino_available, openvino_info = HardwareDetector.detect_openvino()
        hardware_info["openvino"] = {"available": openvino_available, "info": openvino_info}

        # Check MPS (Apple Metal)
        mps_available, mps_info = HardwareDetector.detect_mps()
        hardware_info["mps"] = {"available": mps_available, "info": mps_info}

        # Check Qualcomm
        qualcomm_available, qualcomm_info = HardwareDetector.detect_qualcomm()
        hardware_info["qnn"] = {"available": qualcomm_available, "info": qualcomm_info}

        # Check WebGPU and WebNN
        webgpu_capable, webgpu_info = HardwareDetector.detect_webgpu()
        hardware_info["webgpu"] = {"available": webgpu_capable, "info": webgpu_info}

        webnn_capable, webnn_info = HardwareDetector.detect_webnn()
        hardware_info["webnn"] = {"available": webnn_capable, "info": webnn_info}
