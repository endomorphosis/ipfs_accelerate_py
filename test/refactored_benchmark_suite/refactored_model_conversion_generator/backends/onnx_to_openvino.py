    ...
    def _execute_conversion(
        ...
        try:
            ...
            return ConversionResult(
                success=False, error=f"Error converting ONNX model to OpenVINO: {e}"
            )

    def _convert_with_mo_api(
        self,
        model_path: str,
        output_path: str,
        ...
        try:
            ...
            if new_api:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if exitcode != 0:
                        return ConversionResult(
                            success=False, error=f"Model Optimizer failed with exit code {exitcode}"
                        )

                    # Check if output files exist
                    if not os.path.exists(output_path):
                        return ConversionResult(
                            success=False, error=f"Output file not found: {output_path}"
                        )
                    ...
                    try:
                        ...
                        input_info = {
                            name: str(info.shape) for name, info in network.input_info.items()
                        }
                        output_info = {
                            name: str(data.shape) for name, data in network.outputs.items()
                        }
            ...
            return ConversionResult(
                success=False, error=f"Error in Model Optimizer API conversion: {e}"
            )

    def _convert_with_mo_cli(
        self,
        model_path: str,
        output_path: str,
        model_type: Optional[str],
        ...
        try:
            ...
            if mo_path is None:
                ...
                for openvino_dir in openvino_dirs:
                    ...
                    for mo_candidate in glob.glob(
                        os.path.join(openvino_dir, "**", mo_script), recursive=True
                    ):
            ...
            if result.returncode != 0:
                ...
                return ConversionResult(
                    success=False, error=f"Model Optimizer failed: {result.stderr}"
                )
            ...
            return ConversionResult(
                success=False, error=f"Error in Model Optimizer CLI conversion: {e}"
            )
