...
def main():
    ...
    convert_parser.add_argument(
        "--source-format", required=True, help="Source model format (e.g., onnx, pytorch)"
    )
    convert_parser.add_argument(
        "--target-format", required=True, help="Target model format (e.g., onnx, openvino)"
    )
    ...
    convert_parser.add_argument(
        "--force", action="store_true", help="Force conversion even if output exists"
    )
    convert_parser.add_argument(
        "--precision",
        default="default",
    ...
    convert_parser.add_argument(
        "--mixed-precision-config", help="Path to mixed precision configuration JSON file"
    )
    ...
    quantize_parser.add_argument(
        "--output", required=True, help="Path to save quantization config file"
    )
    quantize_parser.add_argument(
        "--precision",
        default="mixed",
    ...
    if args.mixed_precision_config:
        if os.path.exists(args.mixed_precision_config):
            try:
                ...
                logger.info(
                    f"Loaded mixed precision configuration from {args.mixed_precision_config}"
                )
            except Exception as e:
                logger.warning(f"Error loading mixed precision configuration: {e}")
        else:
            logger.warning(
                f"Mixed precision configuration file not found: {args.mixed_precision_config}"
            )
...
def run_list_command(args):
    ...
    if args.format == "json":
        ...
        for converter in converters:
            ...
            print(
                f"{source_format:<13} | {target_format:<14} | {model_type:<10} | {converter_class}"
            )
