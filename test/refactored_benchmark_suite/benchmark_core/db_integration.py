...
class BenchmarkDBManager:
    ...
    def _create_indexes(self):
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_hardware_type ON hardware_platforms(hardware_type)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_perf_model_hardware ON performance_results(model_id, hardware_id)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_perf_created_at ON performance_results(created_at)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_hc_model_hardware ON hardware_compatibility(model_id, hardware_id)"
        )
    ...
    def store_hardware_compatibility(
        self,
        model_name: str,
        hardware_type: str,
        is_compatible: bool,
    ...
    def _get_or_create_model(
        ...
        RETURNING model_id
        """,
            (model_name, model_family, model_type, modality, parameters_million, now),
        ).fetchone()

        return result[0]

    def _get_or_create_hardware(
        self,
        hardware_type: str,
        simulation_mode: bool = False,
        ...
        """,
            (hardware_type, simulation_mode, device_name, platform, memory_gb, now),
        ).fetchone()
    ...
    def get_hardware_compatibility(
        ...
        model_name: Optional[str] = None,
        hardware_type: Optional[str] = None,
        compatibility_filter: Optional[bool] = None,
    ) -> List[Dict[str, Any]]:
        """
    ...
    def generate_compatibility_matrix(
        self, exclude_simulation: bool = True
        ...
        if self.conn is None:
            logger.warning(
                "Database connection not available. Cannot generate compatibility matrix."
            )
            return {}
