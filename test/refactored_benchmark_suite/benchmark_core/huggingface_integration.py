...
class ModelArchitectureRegistry:
    ...
    DIFFUSION_MODELS = {"controlnet", "stable-diffusion", "stable-diffusion-xl", "vq-diffusion"}
    ...
    def get_all_model_architectures() -> List[str]:
        ...
        if not TRANSFORMERS_AVAILABLE:
            logger.warning(
                "HuggingFace Transformers not available. Cannot list model architectures."
            )
            return []
    ...
    def group_models_by_type() -> Dict[str, List[str]]:
        ...
        if not TRANSFORMERS_AVAILABLE:
            logger.warning(
                "HuggingFace Transformers not available. Cannot group model architectures."
            )
            return {}
...
class ModelLoader:
        ...
        **kwargs,
        ...
        try:
            if model_type == "encoder":
                model, metadata = self._load_encoder_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "decoder":
                model, metadata = self._load_decoder_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "encoder-decoder":
                model, metadata = self._load_encoder_decoder_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "vision":
                model, metadata = self._load_vision_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "audio":
                model, metadata = self._load_audio_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "multimodal":
                model, metadata = self._load_multimodal_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            elif model_type == "diffusion":
                model, metadata = self._load_diffusion_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
            else:
                model, metadata = self._load_generic_model(
                    model_name, device, dtype, batch_size, **kwargs
                )
    ...
    def _load_encoder_model(
        self, model_name: str, device: str, dtype: Any, batch_size: int, **kwargs
    ) -> Tuple[Any, Dict[str, Any]]:
        """
        ...
        model = model_class.from_pretrained(
            model_name, cache_dir=self.cache_dir, **config_kwargs, **kwargs
        )
    ...
    def _load_encoder_decoder_model(
        self, model_name: str, device: str, dtype: Any, batch_size: int, **kwargs
    ) -> Tuple[Any, Dict[str, Any]]:
        """
    ...
    def _load_audio_model(
        ...
        if processor is not None:
            try:
                ...
                sample_rate = 16000
                dummy_audio = [
                    np.random.rand(sample_rate).astype(np.float32) for _ in range(batch_size)
                ]
