...
try:
    ...
    print(
        "Error: Dashboard dependencies not installed. Run: "
        "pip install dash dash_bootstrap_components plotly pandas"
    )
    print(
        "For real-time updates, also install: pip install Flask-SocketIO requests websocket-client"
    )
    sys.exit(1)
...
class BenchmarkDataManager:
    ...
    def get_model_families(self) -> List[str]:
        ...
        if not self.db_available:
            ...
            return sorted(
                list(set(model.get("family", "") for model in models if model.get("family")))
            )

        try:
            result = self.conn.execute(
                "SELECT DISTINCT model_family FROM models WHERE model_family IS NOT NULL"
            ).fetchdf()
    ...
    def get_hardware_types(self) -> List[str]:
        ...
        try:
            result = self.conn.execute(
                "SELECT DISTINCT hardware_type FROM hardware_platforms"
            ).fetchdf()
    ...
    def get_batch_sizes(self) -> List[int]:
        ...
        try:
            result = self.conn.execute(
                "SELECT DISTINCT batch_size FROM performance_results WHERE batch_size IS NOT NULL"
            ).fetchdf()
        ...
        if not self.db_available:
            ...
            for model_family in model_families:
                for hardware_type in hardware_types:
                    models = [
                        m.get("name") for m in self.get_models() if m.get("family") == model_family
                    ]
    ...
    def get_batch_size_scaling(
        self, model_family: str, hardware_type: str, metric: str = "throughput_items_per_second"
    ) -> pd.DataFrame:
        """
        ...
        if not self.db_available:
            ...
            for batch_size in batch_sizes:
                ...
                for model in models:
                    model_results = self.query_results(
                        model=model, hardware=hardware_type, batch_size=batch_size
                    )
    ...
    def get_top_models(
        self, hardware_type: str, metric: str = "throughput_items_per_second", limit: int = 10
    ) -> pd.DataFrame:
        """
    ...
    def get_hardware_comparison(
        self, model_name: str, metric: str = "throughput_items_per_second"
    ) -> pd.DataFrame:
        """
...
class BenchmarkDashboard:
                        ...
                        dbc.Tab(
                            [
                                html.Div(
                                    [
                                        html.H3(
                                            "Performance Comparison", className="text-center mb-4"
                                        ),
                                        dbc.Card(
                                            [
                                                dbc.CardHeader(
                                                    "Hardware/Model Family Performance Heatmap"
                                                ),
                                                dbc.CardBody([dcc.Graph(id="performance-heatmap")]),
                                            ],
                                            className="mb-4",
                                        ),
                                        dbc.Card(
                                    ...
                                    [
                                        ...
                                        dbc.Card(
                                            [
                                                ...
                                                dbc.CardBody([html.Div(id="reports-container")]),
                                            ],
                                            className="mb-4",
                                        ),
                                        dbc.Card(
                        ...
                        dbc.Tab(
                            [
                                html.Div(
                                    [
                                        html.H3(
                                            "Help & Documentation", className="text-center mb-4"
                                        ),
                                        dbc.Card(
                                            [
                                ...
                                """),
                                                        ...
                                                        html.Ul(
                                                            [
                                                                html.Li(
                                                                    html.B("Overview"),
                                                                    " - High-level overview of performance across different dimensions",
                                                                ),
                                                                html.Li(
                                                                    html.B("Comparison"),
                                                                    " - Detailed comparison of performance metrics with heatmap visualization",
                                                                ),
                                                                html.Li(
                                                                    html.B("Live Runs"),
                                                                    " - Monitor active benchmark runs and start new benchmarks",
                                                                ),
                                                                html.Li(
                                                                    html.B("Reports"),
                                                                    " - View benchmark reports and run custom SQL queries",
                                                                ),
                                                            ]
                                                        ),
                                                        html.H4("Metrics"),
                                                        html.Ul(
                                                            [
                                                                html.Li(
                                                                    html.B("Throughput"),
                                                                    " - Items processed per second (higher is better)",
                                                                ),
                                                                html.Li(
                                                                    html.B("Latency"),
                                                                    " - Average processing time in milliseconds (lower is better)",
                                                                ),
                                                                html.Li(
                                                                    html.B("Memory Usage"),
                                                                    " - Peak memory usage in megabytes (lower is better)",
                                                                ),
                                                            ]
                                                        ),
                                                    ]
                                                    ...
                                                    [
                                                        ...
                                                        html.Ul(
                                                            [
                                                                ...
                                                                html.Li(
                                                                    html.Code(
                                                                        "GET /api/benchmark/query"
                                                                    ),
                                                                    " - Query benchmark results",
                                                                ),
                                                                html.Li(
                                                                    html.Code(
                                                                        "WebSocket /api/benchmark/ws/{run_id}"
                                                                    ),
                                                        ...
                                                        html.P(
                                                            "The benchmark results are stored in a DuckDB database with the following tables:"
                                                        ),
                                                        html.Ul(
                                                            [
                                                                html.Li(
        ...
        layout = dbc.Container(
            [
                ...
                dcc.Interval(
                    ...
                    interval=DEFAULT_REFRESH_INTERVAL * 1000,
                    n_intervals=0,
                ),
    ...
    def _register_callbacks(self) -> None:
        ...
        def initialize_filters(trigger):
            ...
            model_options = [
                {"label": model.get("name", ""), "value": model.get("name", "")} for model in models
            ]
        ...
        def update_performance_heatmap(n_clicks, model_families, hardware_types, metric):
            ...
            # Get data
            df = self.data_manager.get_performance_comparison(
                model_families, hardware_types, metric
            )
            ...
            if "model_family" in df.columns and "hardware_type" in df.columns:
                pivot_df = df.pivot(
                    index="model_family", columns="hardware_type", values=metric_col
                )
            else:
        ...
        def update_results_table(n_clicks, model_families, hardware_types, batch_sizes):
            ...
            if (not model_families or len(model_families) == 0) and (
                not hardware_types or len(hardware_types) == 0
            ):
                ...
                for model_family in model_families or []:
                    models = [
                        m.get("name")
                        for m in self.data_manager.get_models()
                        if m.get("family") == model_family
                    ]

                    for model in models:
                        for hardware_type in (
                            hardware_types or self.data_manager.get_hardware_types()
                        ):
                            ...
                            if batch_sizes and len(batch_sizes) > 0:
                                ...
                                model_results = self.data_manager.query_results(
                                    model=model, hardware=hardware_type
                                )
                                results.extend(model_results)
        ...
        @self.app.callback(
            ...
            [Input("overview-model-selector", "value"), Input("metric-selector", "value")],
        )
        ...
        @self.app.callback(
            ...
            [Input("top-models-hardware-selector", "value"), Input("metric-selector", "value")],
        )
        ...
        @self.app.callback(
            Output("active-runs-container", "children"), [Input("refresh-trigger", "children")]
        )
                        ...
                        [
                            dbc.CardHeader(
                                [
                                    ...
                                    html.Span(
                                        f"Status: {run_data.get('status', 'unknown').upper()}",
                            ...
                            dbc.CardBody(
                                [
                                    html.P(
                                        f"Current Step: {run_data.get('current_step', 'Unknown')}"
                                    ),
                                    html.P(
                                        f"Models: {run_data.get('completed_models', 0)}/{run_data.get('total_models', 0)}"
                                    ),
                                    dbc.Progress(
                                        value=progress,
                                    ...
                                    html.P(
                                        f"Elapsed: {run_data.get('elapsed_time', 0):.1f} seconds"
                                    ),
                                ]
                            ),
                            dbc.CardFooter(
        ...
        @self.app.callback(
            Output("reports-container", "children"), [Input("refresh-trigger", "children")]
        )
        ...
        def start_new_benchmark(
            n_clicks, priority, hardware, models, batch_sizes, precision, options
        ):
            if not n_clicks:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        def ws_thread():
                            ws_url = (
                                f"ws://{self.api_url.split('://', 1)[1]}/api/benchmark/ws/{run_id}"
                            )
                        ...
                        logger.warning(
                            "websocket-client package not installed. Will use polling for updates."
                        )
        ...
        def run_sql_query(n_clicks, query):
            ...
            try:
                ...
                return html.Div(
                    [html.Div(f"Error executing query: {str(e)}", className="alert alert-danger")]
                )
...
def main():
    ...
    parser.add_argument(
        "--api-url", default=DEFAULT_API_URL, help="URL of the benchmark API server"
    )
    parser.add_argument(
        "--db-path", default=DEFAULT_DB_PATH, help="Path to the DuckDB database file"
    )
    parser.add_argument(
        "--port", type=int, default=DEFAULT_PORT, help="Port to run the dashboard server on"
    )
    ...
    dashboard = BenchmarkDashboard(
        api_url=args.api_url, db_path=args.db_path, port=args.port, debug=args.debug
    )
