...
def generate_benchmark_files(
    skillset_dir: str, output_dir: str, models: List[str] = None
) -> List[str]:
    ...
    try:
        ...
        cmd = [
            sys.executable,
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "generate_skillset_benchmarks.py"
            ),
            "--skillset-dir",
            skillset_dir,
            "--output-dir",
...
def run_benchmarks(
    ...
    """
    logger.info(
        f"Running {benchmark_type} benchmarks on {hardware} hardware for {len(benchmark_files)} models"
    )
    ...
    try:
        ...
        for benchmark_file in benchmark_files:
            try:
                ...
                model_name = (
                    os.path.basename(benchmark_file).replace("benchmark_", "").replace(".py", "")
                )
                ...
                result_file = os.path.join(
                    output_dir, f"{model_name}_{benchmark_type}_{hardware}_{timestamp}.json"
                )
                ...
                if report:
                    try:
                        ...
                        generate_html_report(
                            result, model_name, benchmark_type, hardware, report_file
                        )
...
def generate_html_report(
    result: Dict[str, Any], model_name: str, benchmark_type: str, hardware: str, output_file: str
) -> None:
    ...
    if benchmark_type == "inference":
        ...
        if "batch_results" in result:
            ...
            if batch_sizes and init_times:
                ...
                html.append("            <table>")
                html.append(
                    "                <tr><th>Batch Size</th><th>Mean Init Time (ms)</th><th>Std Dev (ms)</th></tr>"
                )

                for i in range(len(batch_sizes)):
                    html.append(
                        f"                <tr><td>{batch_sizes[i]}</td><td>{init_times[i]:.2f}</td><td>{std_devs[i]:.2f}</td></tr>"
                    )
        ...
        if "speedup_over_sequential" in throughput:
            html.append(
                f"                <tr><td>Speedup</td><td>{throughput.get('speedup_over_sequential', 0):.2f}x</td></tr>"
            )
...
def generate_combined_report(
    results: Dict[str, Dict[str, Any]], benchmark_type: str, hardware: str, output_file: str
) -> None:
    ...
    if benchmark_type == "inference":
        ...
        if model_names and throughputs:
            # Sort by throughput
            sorted_data = sorted(
                zip(model_names, throughputs, speedups), key=lambda x: x[1], reverse=True
            )
            ...
            # Add charts to HTML
            throughput_rel_path = os.path.relpath(
                throughput_chart_path, os.path.dirname(output_file)
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="benchmark_results", help="Directory to write results to"
    )
    parser.add_argument(
        "--benchmark-dir",
        type=str,
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate HTML reports for benchmark results"
    )
    ...
    benchmark_files = generate_benchmark_files(args.skillset_dir, args.benchmark_dir, args.model)
    ...
    if args.type in ["inference", "both"]:
        ...
        if not inference_results.get("success", False):
            logger.error(
                f"Inference benchmarks failed: {inference_results.get('error', 'Unknown error')}"
            )
        else:
    ...
    if args.type in ["throughput", "both"]:
        ...
        if not throughput_results.get("success", False):
            logger.error(
                f"Throughput benchmarks failed: {throughput_results.get('error', 'Unknown error')}"
            )
        else:
    ...
    if all_results:
        ...
        combined_file = os.path.join(
            args.output_dir, f"combined_benchmark_results_{args.hardware}_{timestamp}.json"
        )
