...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("benchmark_pipeline.log")],
)
...
class BenchmarkPipeline:
    ...
    def __init__(
        ...
        self.refactored_generator_path = Path(
            refactored_generator_path
            or str(Path(__file__).resolve().parent.parent / "refactored_generator_suite")
        )
        ...
        self.benchmark_output_dir = Path(
            benchmark_output_dir or str(self.output_dir / "benchmarks")
        )
        ...
        self.progress = {
            "skillset_generation": {"total": 0, "completed": 0, "failed": 0},
            "benchmark_generation": {"total": 0, "completed": 0, "failed": 0},
            "benchmark_execution": {"total": 0, "completed": 0, "failed": 0},
            "report_generation": {"total": 0, "completed": 0, "failed": 0},
        }

    def generate_model_skillsets(
        self, priority: str = "high", model_filter: Optional[List[str]] = None
    ) -> bool:
        """
        ...
        if not self.refactored_generator_path.exists():
            logger.error(
                f"Refactored generator suite not found at {self.refactored_generator_path}"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"Successfully generated {self.progress['skillset_generation']['completed']} skillset files"
            )
            return True
    ...
    def generate_benchmark_files(
        ...
        if not benchmark_script.exists():
            benchmark_script = (
                Path(__file__).resolve().parent / "benchmarks" / "generate_skillset_benchmarks.py"
            )
            if not benchmark_script.exists():
        ...
        try:
            ...
            logger.info(
                f"Successfully generated {self.progress['benchmark_generation']['completed']} benchmark files"
            )
            return True
        except subprocess.CalledProcessError as e:
    ...
    def _generate_benchmark_configs(
        self,
        priority: str,
        hardware: List[str],
                            ...
                            if incremental:
                                ...
                                if key in existing_results:
                                    ...
                                    if days_since < update_threshold:
                                        logger.debug(
                                            f"Skipping benchmark for {key} (updated {days_since} days ago)"
                                        )
                                        continue
    ...
    def _run_benchmarks_sequential(self, configs: List[Dict[str, Any]]) -> bool:
        ...
        for i, config in enumerate(configs):
            ...
            try:
                ...
                if self.interactive:
                    sys.stdout.write(
                        f"\rCompleted: {self.progress['benchmark_execution']['completed']}/{len(configs)} benchmarks..."
                    )
                    sys.stdout.flush()
            except Exception as e:
        ...
        success_rate = (
            self.progress["benchmark_execution"]["completed"] / len(configs) if configs else 1.0
        )
        logger.info(
            f"Completed {self.progress['benchmark_execution']['completed']}/{len(configs)} benchmarks ({success_rate:.2%})"
        )
    ...
    def _run_benchmarks_parallel(self, configs: List[Dict[str, Any]]) -> bool:
        ...
        """
        logger.info(
            f"Running {len(configs)} benchmarks in parallel with {self.parallel_jobs} workers..."
        )
        ...
        for hw, hw_configs in hw_groups.items():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.parallel_jobs) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_config):
                    ...
                    if self.interactive:
                        sys.stdout.write(
                            f"\rCompleted: {completed}/{len(hw_configs)} benchmarks for {hw}..."
                        )
                        sys.stdout.flush()
    ...
    def generate_reports(
        ...
        success_rate = (
            self.progress["report_generation"]["completed"]
            / self.progress["report_generation"]["total"]
            if self.progress["report_generation"]["total"]
            else 1.0
        )
        logger.info(
            f"Completed {self.progress['report_generation']['completed']}/{self.progress['report_generation']['total']} reports ({success_rate:.2%})"
        )
    ...
    def _generate_compatibility_matrix(self, output_path: Path, fmt: str) -> bool:
        ...
        try:
            ...
            hardware_types = [
                row[0] for row in self.db_manager.conn.execute(hardware_query).fetchall()
            ]
            ...
            model_info = [
                (row[0], row[1]) for row in self.db_manager.conn.execute(model_query).fetchall()
            ]
            ...
            if fmt == "html":
                ...
                for model_name, model_type in model_info:
                    ...
                    if model_type != current_type:
                        current_type = model_type
                        md_report += (
                            f"| **{model_type or 'Unknown'}** |{' | ' * len(hardware_types)}\n"
                        )
    ...
    def run_complete_pipeline(
        ...
        incremental: bool = True,
        report_formats: List[str] = ["html", "markdown"],
    ) -> bool:
        """
        ...
        logger.info(
            f"Skillset generation: {self.progress['skillset_generation']['completed']} completed, {self.progress['skillset_generation']['failed']} failed"
        )
        logger.info(
            f"Benchmark generation: {self.progress['benchmark_generation']['completed']} completed, {self.progress['benchmark_generation']['failed']} failed"
        )
        logger.info(
            f"Benchmark execution: {self.progress['benchmark_execution']['completed']} completed, {self.progress['benchmark_execution']['failed']} failed"
        )
        logger.info(
            f"Report generation: {self.progress['report_generation']['completed']} completed, {self.progress['report_generation']['failed']} failed"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Complete Benchmark Pipeline for HuggingFace Models"
    )
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        help="Path to DuckDB database (default: $BENCHMARK_DB_PATH or ./benchmark_db.duckdb)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        help="Root directory for output files (default: ./benchmark_output)",
    )
    ...
    parser.add_argument(
        ...
        default="high",
        help="Priority level for models to benchmark",
    )
    parser.add_argument(
        "--model",
        action="append",
        dest="models",
        help="Specific model to benchmark (can be used multiple times)",
    )
    parser.add_argument(
        "--model-type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="cpu",
        help="Hardware backends to benchmark on (comma-separated)",
    )
    parser.add_argument(
        "--progressive-mode",
        action="store_true",
        help="Use progressive mode (CPU first, then GPU, etc.)",
    )

    # Benchmark configuration
    parser.add_argument(
        "--batch-sizes", type=str, default="1,8", help="Batch sizes to benchmark (comma-separated)"
    )
    parser.add_argument(
        "--precision",
        type=str,
        default="fp32",
        help="Precision formats to benchmark (comma-separated)",
    )
    parser.add_argument(
        "--incremental",
        action="store_true",
        help="Only run benchmarks for missing or outdated results",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of parallel jobs for benchmark execution",
    )
    parser.add_argument(
        "--no-interactive", action="store_true", help="Disable interactive progress reporting"
    )

    # Pipeline control
    parser.add_argument(
        "--skip-skillset-generation", action="store_true", help="Skip model skillset generation"
    )
    parser.add_argument(
        "--skip-benchmark-generation", action="store_true", help="Skip benchmark file generation"
    )
    parser.add_argument(
        "--skip-benchmark-execution", action="store_true", help="Skip benchmark execution"
    )
    parser.add_argument(
        "--skip-report-generation", action="store_true", help="Skip report generation"
    )
    parser.add_argument(
        "--report-formats",
        type=str,
        default="html,markdown",
        help="Report output formats (comma-separated)",
    )
    ...
    parser.add_argument(
        "--ci-mode",
        action="store_true",
        help="Run in CI mode (non-interactive, fixed output paths)",
    )
    ...
    try:
        ...
        if not args.skip_skillset_generation:
            if not pipeline.generate_model_skillsets(
                priority=args.priority, model_filter=args.models
            ):
                logger.error("Failed to generate model skillsets")
                return 1
        ...
        if not args.skip_benchmark_generation:
            if not pipeline.generate_benchmark_files(
                priority=args.priority, model_type=args.model_type, model_filter=args.models
            ):
                logger.error("Failed to generate benchmark files")
                return 1
