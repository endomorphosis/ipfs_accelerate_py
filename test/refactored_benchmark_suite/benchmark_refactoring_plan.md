...
class BenchmarkRegistry:
    ...
    def register(cls, name=None, **kwargs):
        ...
        def decorator(benchmark_class):
            ...
            cls._registry[registry_name] = {"class": benchmark_class, "metadata": kwargs}
...
class BenchmarkRunner:
    ...
    def execute(self, benchmark_name, params=None):
        ...
        benchmark = benchmark_class(
            hardware=self.hardware_manager.get_hardware(params.get("hardware")), **params
        )
...
def ci_entrypoint():
    ...
    parser.add_argument(
        "--format", type=str, choices=["json", "csv", "md"], default="json", help="Output format"
    )
    ...
    runner = BenchmarkRunner(config={"output_dir": args.output, "storage": storage})
...
class BenchmarkASTAnalyzer:
    ...
    def _is_benchmark_file(self, file_path: Path) -> bool:
        ...
        if (
            "benchmark" in name
            or "test_" in name
            and any(hw in name for hw in ["gpu", "cpu", "webnn", "webgpu"])
        ):
            return True
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                content = f.read()
                if "benchmark" in content.lower() and any(
                    kw in content.lower() for kw in ["performance", "throughput", "latency"]
                ):
                    return True
        except Exception:
            pass
    ...
    def generate_class_hierarchy_report(self):
        ...
        for file_path, classes in self.classes.items():
            for cls in classes:
                class_name = cls["name"]
                all_classes[class_name] = {"file": file_path, "info": cls}
    ...
    def generate_dependency_report(self):
        ...
        for file_path, imports in self.imports.items():
            dependency_report[file_path] = {"imports": imports, "imported_by": []}
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="ast_analysis", help="Output directory for reports"
    )
