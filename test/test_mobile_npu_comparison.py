...
try:
    ...
    try:
        ...
        logger.info(
            f"MediaTek NPU support loaded: Available={MEDIATEK_NPU_AVAILABLE}, Simulation={MEDIATEK_NPU_SIMULATION_MODE}"
        )
    except ImportError:
        HAS_MEDIATEK = False
    ...
    try:
        ...
        logger.info(
            f"Qualcomm NPU support loaded: Available={QNN_AVAILABLE}, Simulation={QNN_SIMULATION_MODE}"
        )
    except ImportError:
        HAS_QUALCOMM = False
    ...
    try:
        ...
        logger.info(
            f"Samsung NPU support loaded: Available={SAMSUNG_NPU_AVAILABLE}, Simulation={SAMSUNG_NPU_SIMULATION_MODE}"
        )
    except ImportError:
        HAS_SAMSUNG = False
        ...
        if USING_CENTRALIZED:
            ...
            if HAS_MEDIATEK:
                ...
                if mediatek_detector.is_available():
                    ...
                    compatibility_results[model_id]["compatibility"]["mediatek"] = (
                        mediatek_compat.get("compatible", False)
                    )
            ...
            if HAS_QUALCOMM:
                ...
                if qnn_detector.is_available():
                    ...
                    compatibility_results[model_id]["compatibility"]["qualcomm"] = qnn_compat.get(
                        "compatible", False
                    )
            ...
            if HAS_SAMSUNG:
                ...
                if chipset:
                    ...
                    analysis = samsung_converter.analyze_model_compatibility(
                        model_path=model_info["path"], target_chipset=target_chipset
                    )
...
def compare_power_efficiency():
    ...
    if not USING_CENTRALIZED:
        ...
        if HAS_MEDIATEK:
            ...
            if mediatek_power.detector.is_available():
                ...
                battery_info = mediatek_power.estimate_battery_life(
                    results.get("average_power_watts", 1.0), battery_capacity_mah=5000
                )
        ...
        if HAS_SAMSUNG:
            ...
            if chipset:
                ...
                power_efficiency_score = (
                    chipset.npu_tops / average_power
                ) * 10  # Simplified efficiency score
                battery_impact_percent = (
                    average_power / 5.0
                ) * 100  # Simplified battery impact calculation
        ...
        power_results["mediatek"] = {
            ...
            "battery_life": {"estimated_runtime_hours": 22.8, "battery_percent_per_hour": 4.4},
        }
        ...
        power_results["qualcomm"] = {
            ...
            "battery_life": {"estimated_runtime_hours": 20.4, "battery_percent_per_hour": 4.9},
        }
        ...
        power_results["samsung"] = {
            ...
            "tops_per_watt": 9.8,
            "battery_life": {"estimated_runtime_hours": 21.4, "battery_percent_per_hour": 4.7},
        }
...
def compare_optimization_recommendations():
    ...
    if not USING_CENTRALIZED:
            ...
            if HAS_SAMSUNG:
                ...
                if chipset:
                    ...
                    if "optimization_opportunities" in compatibility:
                        samsung_rec["recommended_optimizations"] = compatibility[
        ...
        for model_id in ["mobilenet_v3", "mobilebert", "whisper_tiny"]:
            ...
            if model_info["type"] == "vision":
                optimization_results[model_id]["recommendations"]["samsung"][
                    "recommended_optimizations"
                ].append("Optimize for throughput with multi-image batch processing")
            elif model_info["type"] == "nlp":
                optimization_results[model_id]["recommendations"]["samsung"][
                    "recommended_optimizations"
                ].append("INT4 weight-only quantization")
            elif model_info["type"] == "audio":
                optimization_results[model_id]["recommendations"]["samsung"][
                    "recommended_optimizations"
                ].append("Game Booster integration for sustained performance")

    return optimization_results


def print_comparison_report(
    hardware_results, compatibility_results, power_results, optimization_results
):
    ...
    for platform, info in hardware_results.items():
        ...
        if (
            info["available"]
            and platform in ["qualcomm", "mediatek"]
            and info.get("simulation", False)
        ):
    ...
    print(
        f"{'Model':<20} {'Type':<10} {'MediaTek':<12} {'Qualcomm':<12} {'Samsung':<12} {'Mobile-Opt':<10}"
    )
    print("-" * 80)
    for model_id, info in compatibility_results.items():
        ...
        print(
            f"{info['name']:<20} {info['type']:<10} {mediatek_compat:<12} {qualcomm_compat:<12} {samsung_compat:<12} {mobile_opt:<10}"
        )
    print("\n")
    ...
    has_power_data = all(
        platform in power_results for platform in ["mediatek", "qualcomm", "samsung"]
    )
    if has_power_data:
        ...
        print(
            f"{'Average Power (Watts)':<30} {mediatek_avg:<16.2f} {qualcomm_avg:<16.2f} {samsung_avg:<16.2f}"
        )
        ...
        print(
            f"{'Peak Power (Watts)':<30} {mediatek_peak:<16.2f} {qualcomm_peak:<16.2f} {samsung_peak:<16.2f}"
        )
        ...
        print(
            f"{'Battery Impact (%)':<30} {mediatek_impact:<16.2f} {qualcomm_impact:<16.2f} {samsung_impact:<16.2f}"
        )
        ...
        print(
            f"{'Power Efficiency Score':<30} {mediatek_score:<16.2f} {qualcomm_score:<16.2f} {samsung_score:<16.2f}"
        )
        ...
        if "tops_per_watt" in power_results["samsung"]:
            ...
            print(
                f"{'TOPS per Watt':<30} {mediatek_tpw:<16.2f} {qualcomm_tpw:<16.2f} {samsung_tpw:<16.2f}"
            )

        # Battery life estimates
        if all(
            "battery_life" in power_results[platform]
            for platform in ["mediatek", "qualcomm", "samsung"]
        ):
            mediatek_runtime = power_results["mediatek"]["battery_life"].get(
                "estimated_runtime_hours", 0
            )
            qualcomm_runtime = power_results["qualcomm"]["battery_life"].get(
                "estimated_runtime_hours", 0
            )
            samsung_runtime = power_results["samsung"]["battery_life"].get(
                "estimated_runtime_hours", 0
            )
            print(
                f"{'Est. Runtime (hours)':<30} {mediatek_runtime:<16.2f} {qualcomm_runtime:<16.2f} {samsung_runtime:<16.2f}"
            )

            mediatek_percent = power_results["mediatek"]["battery_life"].get(
                "battery_percent_per_hour", 0
            )
            qualcomm_percent = power_results["qualcomm"]["battery_life"].get(
                "battery_percent_per_hour", 0
            )
            samsung_percent = power_results["samsung"]["battery_life"].get(
                "battery_percent_per_hour", 0
            )
            print(
                f"{'Battery % Per Hour':<30} {mediatek_percent:<16.2f} {qualcomm_percent:<16.2f} {samsung_percent:<16.2f}"
            )
    else:
            ...
            if "mediatek" in info["recommendations"]:
                ...
                if mediatek_rec.get("compatible", False):
                    for opt in mediatek_rec.get("recommended_optimizations", []):
                        print(f"    - {opt}")
                    print(
                        f"    Memory Reduction: {mediatek_rec.get('estimated_memory_reduction', 'N/A')}"
                    )
                    print(
                        f"    Power Efficiency Score: {mediatek_rec.get('estimated_power_efficiency_score', 0)}"
                    )
                else:
            ...
            if "qualcomm" in info["recommendations"]:
                ...
                if qualcomm_rec.get("compatible", False):
                    for opt in qualcomm_rec.get("recommended_optimizations", []):
                        print(f"    - {opt}")
                    print(
                        f"    Memory Reduction: {qualcomm_rec.get('estimated_memory_reduction', 'N/A')}"
                    )
                    print(
                        f"    Power Efficiency Score: {qualcomm_rec.get('estimated_power_efficiency_score', 0)}"
                    )
                else:
    ...
    mediatek_count = sum(
        1 for info in compatibility_results.values() if info["compatibility"].get("mediatek", False)
    )
    qualcomm_count = sum(
        1 for info in compatibility_results.values() if info["compatibility"].get("qualcomm", False)
    )
    samsung_count = sum(
        1 for info in compatibility_results.values() if info["compatibility"].get("samsung", False)
    )
    ...
    print(
        f"MediaTek compatibility: {mediatek_count}/{total_models} models ({mediatek_percent:.1f}%)"
    )
    print(
        f"Qualcomm compatibility: {qualcomm_count}/{total_models} models ({qualcomm_percent:.1f}%)"
    )
    ...
    if all(platform in power_results for platform in ["mediatek", "qualcomm", "samsung"]):
        ...
        scores = {"MediaTek": mediatek_score, "Qualcomm": qualcomm_score, "Samsung": samsung_score}
        winner = max(scores.items(), key=lambda x: x[1])

        print(f"Power efficiency winner: {winner[0]} (score: {winner[1]:.1f})")
        print(
            f"  MediaTek: {mediatek_score:.1f}, Qualcomm: {qualcomm_score:.1f}, Samsung: {samsung_score:.1f}"
        )
    ...
    mediatek_mobile_count = sum(
        1 for info in mobile_models if info["compatibility"].get("mediatek", False)
    )
    qualcomm_mobile_count = sum(
        1 for info in mobile_models if info["compatibility"].get("qualcomm", False)
    )
    samsung_mobile_count = sum(
        1 for info in mobile_models if info["compatibility"].get("samsung", False)
    )
    ...
    vision_models = [
        info for model_id, info in compatibility_results.items() if info["type"] == "vision"
    ]
    nlp_models = [info for model_id, info in compatibility_results.items() if info["type"] == "nlp"]
    audio_models = [
        info for model_id, info in compatibility_results.items() if info["type"] == "audio"
    ]

    # Vision model recommendations
    mediatek_vision_count = sum(
        1 for info in vision_models if info["compatibility"].get("mediatek", False)
    )
    qualcomm_vision_count = sum(
        1 for info in vision_models if info["compatibility"].get("qualcomm", False)
    )
    samsung_vision_count = sum(
        1 for info in vision_models if info["compatibility"].get("samsung", False)
    )
    ...
    mediatek_nlp_count = sum(
        1 for info in nlp_models if info["compatibility"].get("mediatek", False)
    )
    qualcomm_nlp_count = sum(
        1 for info in nlp_models if info["compatibility"].get("qualcomm", False)
    )
    samsung_nlp_count = sum(1 for info in nlp_models if info["compatibility"].get("samsung", False))
    ...
    mediatek_audio_count = sum(
        1 for info in audio_models if info["compatibility"].get("mediatek", False)
    )
    qualcomm_audio_count = sum(
        1 for info in audio_models if info["compatibility"].get("qualcomm", False)
    )
    samsung_audio_count = sum(
        1 for info in audio_models if info["compatibility"].get("samsung", False)
    )
    ...
    print(
        f"- Overall, {best_compatibility[0]} supports more tested models ({best_compatibility[1]}/{total_models})"
    )
    ...
    if all(platform in power_results for platform in ["mediatek", "qualcomm", "samsung"]):
        ...
        if samsung_score > mediatek_score and samsung_score > qualcomm_score:
            print(
                "- Samsung Exynos provides the best power efficiency for battery-sensitive applications"
            )
        elif mediatek_score > qualcomm_score:
            print(
                "- MediaTek provides the best power efficiency for battery-sensitive applications"
            )
        else:
            print(
                "- Qualcomm provides the best power efficiency for battery-sensitive applications"
            )

    # Final notes
    print("\nNOTE: ")
    if (
        hardware_results["mediatek"].get("simulation", False)
        or hardware_results["qualcomm"].get("simulation", False)
        or hardware_results["samsung"].get("simulation", False)
    ):
