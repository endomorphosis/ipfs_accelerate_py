...
class SimulatedWorker:
    ...
    def __init__(
        self, worker_id: str, coordinator: SimulatedCoordinator, supported_apis: List[str]
    ):
        """
        ...
        while self.running:
            ...
            if task:
                ...
                self.coordinator.submit_result(
                    worker_id=self.worker_id, task_id=task.task_id, result=result
                )
    ...
    def _run_latency_test(
        self, provider: SimulatedAPIProvider, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Run a latency test.
...
class EndToEndAPITest:
    ...
    def _get_test_parameters(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "basic": {
                "latency": {"messages": messages, "iterations": 5},
                "throughput": {"messages": messages, "duration": 3, "concurrent_requests": 3},
                "reliability": {"messages": messages, "iterations": 20},
                "cost_efficiency": {"messages": messages, "iterations": 3},
            },
            "comprehensive": {
                "latency": {"messages": messages, "iterations": 10},
                "throughput": {"messages": messages, "duration": 5, "concurrent_requests": 5},
                "reliability": {"messages": messages, "iterations": 50},
                "cost_efficiency": {"messages": messages, "iterations": 5},
            },
            "stress": {
                "latency": {"messages": messages, "iterations": 20},
                "throughput": {"messages": messages, "duration": 10, "concurrent_requests": 10},
                "reliability": {"messages": messages, "iterations": 100},
                "cost_efficiency": {"messages": messages, "iterations": 10},
            },
        }
                ...
                for test_type in test_types:
                    ...
                    if self.mode == "simulation":
                        task_id = self.coordinator.create_task(
                            api_type=api, test_type=test_type, parameters=test_params
                        )
    ...
    def _analyze_results(self, results: Dict[str, Any]):
        ...
        for api, api_results in results.items():
            ...
            if "cost_efficiency" in api_results:
                ...
                if "cost_efficiency_metrics" in cost_result:
                    summary["cost_efficiency"] = cost_result["cost_efficiency_metrics"].get(
                        "tokens_per_dollar"
                    )
        ...
        rankings = {
            ...
            "throughput": self._rank_apis_by_metric(
                results, "throughput_rps", lower_is_better=False
            ),
            "reliability": self._rank_apis_by_metric(results, "reliability", lower_is_better=False),
            "cost_efficiency": self._rank_apis_by_metric(
                results, "cost_efficiency", lower_is_better=False
            ),
        }
    ...
    def _rank_apis_by_metric(
        self, results: Dict[str, Any], metric: str, lower_is_better: bool
    ) -> List[Dict[str, Any]]:
        """
        ...
        for api, api_results in results.items():
            if "summary" in api_results and api_results["summary"].get(metric) is not None:
                ranking.append({"provider": api, "score": api_results["summary"][metric]})
...
def run_simulation():
    ...
    test = EndToEndAPITest(
        mode="simulation", num_workers=3, apis=["openai", "claude", "groq"], test_suite="basic"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="End-to-End API Distributed Testing Example")
    ...
    parser.add_argument(
        "--num-workers", type=int, default=DEFAULT_NUM_WORKERS, help="Number of worker nodes"
    )
