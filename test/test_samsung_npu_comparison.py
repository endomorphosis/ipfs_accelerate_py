...
try:
    from hardware_detection.qnn_support import QNNWrapper, QNNCapabilityDetector
...
class HardwareComparisonTool:
    ...
    def _initialize_benchmark_runners(self):
        ...
        if self.available_hardware.get("samsung", False) and SAMSUNG_SUPPORT_AVAILABLE:
            try:
                self.benchmark_runners["samsung"] = SamsungBenchmarkRunner(db_path=self.db_path)
    ...
    def get_hardware_capability_comparison(self) -> Dict[str, Any]:
        ...
        if self.available_hardware["samsung"] and self.samsung_detector is not None:
            ...
            if samsung_chipset is not None:
                ...
                comparison["model_compatibility"]["samsung"] = capability_analysis[
                    "model_capabilities"
                ]
                ...
                comparison["optimization_recommendations"]["samsung"] = capability_analysis[
        ...
        if len(comparison["hardware_details"]) > 1:
            comparison["comparative_recommendations"] = self._generate_comparative_recommendations(
                comparison
            )
    ...
    def _generate_comparative_recommendations(self, comparison_data: Dict[str, Any]) -> List[str]:
        ...
        if samsung_available and qualcomm_available:
            ...
            samsung_precisions = comparison_data["hardware_details"]["samsung"][
            ...
            if (
                "power_efficiency" in comparison_data
                and "samsung" in comparison_data["power_efficiency"]
            ):
            ...
            if (
                "model_compatibility" in comparison_data
                and "samsung" in comparison_data["model_compatibility"]
            ):
        ...
        # Samsung-specific recommendations
            ...
            if (
                "optimization_recommendations" in comparison_data
                and "samsung" in comparison_data["optimization_recommendations"]
            ):
        ...
        if self.available_hardware.get("samsung", False) and "samsung" in self.benchmark_runners:
            try:
                ...
                samsung_compat = converter.analyze_model_compatibility(
                    model_path=model_path, target_chipset=target_chipset
                )
    ...
    def _generate_comparative_benchmark_results(
        ...
        for batch_size in batch_sizes:
            ...
            ref_throughput = hardware_results[ref_hw]["batch_results"][batch_size][
                "throughput_items_per_second"
            ]
            ref_latency = hardware_results[ref_hw]["batch_results"][batch_size]["latency_ms"]["avg"]
            ref_power = hardware_results[ref_hw]["batch_results"][batch_size]["power_metrics"][
                "power_consumption_mw"
            ]

            # Calculate relative performance for each hardware
            for hw_type in hardware_types:
                hw_throughput = hardware_results[hw_type]["batch_results"][batch_size][
                    "throughput_items_per_second"
                ]
                hw_latency = hardware_results[hw_type]["batch_results"][batch_size]["latency_ms"][
                    "avg"
                ]
                hw_power = hardware_results[hw_type]["batch_results"][batch_size]["power_metrics"][
                    "power_consumption_mw"
                ]
                ...
                comparative["performance_comparison"][batch_size][hw_type] = {
                    ...
                    "throughput_vs_reference": hw_throughput / ref_throughput
                    if ref_throughput > 0
                    else 0,
                ...
                comparative["power_efficiency_comparison"][batch_size][hw_type] = {
                    ...
                    "efficiency_vs_reference": power_efficiency / ref_power_efficiency
                    if ref_power_efficiency > 0
                    else 0,
                }
        ...
        for hw_type in hardware_types:
            ...
            comparative["optimal_batch_size"][hw_type] = {
                ...
                "throughput_items_per_second": hardware_results[hw_type]["batch_results"][
                    optimal_batch
                ]["throughput_items_per_second"],
                "latency_ms": hardware_results[hw_type]["batch_results"][optimal_batch][
                    "latency_ms"
                ]["avg"],
                "power_consumption_mw": hardware_results[hw_type]["batch_results"][optimal_batch][
                    "power_metrics"
                ]["power_consumption_mw"],
        ...
        if "samsung" in hardware_types and "qualcomm" in hardware_types:
            ...
            if (
                samsung_opt["throughput_items_per_second"]
                > qualcomm_opt["throughput_items_per_second"]
            ):
    ...
    def run_thermal_impact_analysis(
        self,
        model_path: str,
        hardware_types: Optional[List[str]] = None,
        batch_size: int = 1,
        ...
        for hw_type in hardware_types:
            if hw_type in self.benchmark_runners and hasattr(
                self.benchmark_runners[hw_type], "run_thermal_analysis"
            ):
        ...
        if len(results["hardware_results"]) > 1:
            ...
            results["comparative_results"] = {"thermal_comparison": {}, "recommendations": []}
        ...
        if comparison["hardware_details"]:
            ...
            for hw_type, details in comparison["hardware_details"].items():
                ...
                if hw_type == "samsung":
                    ...
                    lines.append(
                        f"  Supported Prec.:  {', '.join(details['supported_precisions'])}"
                    )
...
def main():
    ...
    # General options
    parser.add_argument(
        "--use-centralized", action="store_true", help="Use centralized hardware detection"
    )
    parser.add_argument(
        "--samsung-simulation", action="store_true", help="Force Samsung simulation mode"
    )
    parser.add_argument(
        "--qualcomm-simulation", action="store_true", help="Force Qualcomm simulation mode"
    )
    ...
    report_parser.add_argument(
        "--format", type=str, choices=["json", "text"], default="text", help="Report format"
    )
    ...
    compat_parser.add_argument(
        "--format", type=str, choices=["json", "text"], default="json", help="Report format"
    )
    ...
    benchmark_parser.add_argument(
        "--hardware", type=str, nargs="+", help="Hardware types to benchmark"
    )
    benchmark_parser.add_argument(
    ...
    benchmark_parser.add_argument(
        "--no-thermals", action="store_true", help="Disable thermal monitoring"
    )
    ...
    thermal_parser.add_argument(
        "--output", type=str, help="Output path for thermal analysis report"
    )
    thermal_parser.add_argument(
        "--format", type=str, choices=["json", "text"], default="json", help="Report format"
    )
    ...
    if args.command == "report":
        result = tool.generate_hardware_report(output_path=args.output, format=args.format)
        if not args.output:
            print(result)

    elif args.command == "compatibility":
        result = tool.run_model_compatibility_assessment(model_path=args.model)
