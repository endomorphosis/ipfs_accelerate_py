...
def save_results(calibrated_parameters, evaluation, output_path):
    ...
    logger.info(
        f"Results saved to {output_path}_calibrated_parameters.json and {output_path}_evaluation.json"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", default="./simulation_calibration_results", help="Path prefix for output files"
    )
    ...
    parser.add_argument(
        "--method",
        choices=["linear_scaling", "additive_adjustment", "regression"],
    ...
    try:
        ...
        evaluation = test_calibration_evaluation(
            validation_results, calibrated_results, original_data[1]
        )

        # Print results to console
        logger.info(
            f"Overall calibration improvement: {evaluation['overall']['relative_improvement']:.2f}%"
        )
        logger.info(
            f"MAPE before: {evaluation['overall']['before_mape']:.2f}%, after: {evaluation['overall']['after_mape']:.2f}%"
        )
