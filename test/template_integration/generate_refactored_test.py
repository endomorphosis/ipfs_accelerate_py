...
def get_template_path(architecture, refactored=True):
    ...
    if not os.path.exists(template_path):
        skills_template_dir = os.path.join(
        ...
        if not os.path.exists(skills_template_path) and refactored:
            logger.warning(
                f"Refactored template {template_name} not found, falling back to standard template"
            )
...
def generate_output_path(model_name, architecture, refactored=True):
    ...
    if refactored:
        ...
        refactored_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "refactored_test_suite"
        )
        output_path = os.path.join(refactored_dir, subdir, test_filename)
    else:
        # For standard tests, use the normal output directory
        output_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "skills", "fixed_tests"
        )
...
def generate_test_file(model_name, architecture=None, refactored=True, output_path=None):
    ...
    logger.info(
        f"Generating {'refactored' if refactored else 'standard'} test file for {model_name} with architecture {architecture}"
    )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Model name/ID to generate test for"
    )
    parser.add_argument(
        "--architecture",
        type=str,
        help="Model architecture (encoder_only, decoder_only, vision, etc.)",
    )
    parser.add_argument("--output", type=str, help="Output path for the generated test file")
    parser.add_argument(
        "--no-refactor",
        action="store_true",
        help="Generate standard test file instead of refactored",
    )
