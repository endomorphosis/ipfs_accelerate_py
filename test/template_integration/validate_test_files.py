...
class TestFileValidator:
    ...
    def validate_file(self, file_path: str) -> Dict[str, Any]:
            ...
            if isinstance(node, ast.FunctionDef) and node.name == "setUp":
                for sub_node in ast.walk(node):
                    if (
                        ...
                        and isinstance(sub_node.targets[0], ast.Attribute)
                    ):
        ...
        test_methods = [
            m for m in method_names if m.startswith("test_") and m not in REQUIRED_METHODS
        ]
        ...
        result["overall_valid"] = (
            result["valid_syntax"] and result["valid_inheritance"] and result["valid_methods"]
        )

        return result

    def validate_directory(
        self, directory: str, recursive: bool = True, pattern: str = "test_*.py"
    ) -> Dict[str, Any]:
        """
        ...
        # Log summary
        logger.info(
            f"Validation complete: {valid_files}/{total_files} valid files "
...
def main():
    ...
    target_group.add_argument(
        "--directory", type=str, help="Validate all test files in a directory"
    )
    ...
    if args.file:
        ...
        results = validator.validate_directory(
            args.directory, recursive=recursive, pattern=args.pattern
        )
