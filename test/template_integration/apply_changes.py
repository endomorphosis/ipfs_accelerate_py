...
def copy_file(source, destination, dry_run=False, backup=True, force=False):
    ...
    try:
        ...
        if os.path.exists(destination) and not force:
            logger.warning(
                f"Destination file already exists (use --force to overwrite): {destination}"
            )
...
def update_architecture_types(model_name, architecture, dry_run=False, backup=True):
    ...
    try:
        ...
        if re.search(model_pattern, architecture_list):
            logger.info(
                f"Model '{model_name}' is already in the list for architecture '{architecture}'"
            )
        ...
        if not dry_run:
            with open(generator_path, "w") as f:
                f.write(new_content)
            logger.info(
                f"Updated ARCHITECTURE_TYPES with model '{model_name}' in architecture '{architecture}'"
            )
        else:
            logger.info(
                f"Would update ARCHITECTURE_TYPES with model '{model_name}' in architecture '{architecture}'"
            )
...
def apply_all_changes(dry_run=False, backup=True, force=False):
    ...
    results = {"copied_files": [], "updated_architectures": [], "failed_operations": []}
    ...
    for model_name, destination in MODEL_DESTINATIONS.items():
        ...
        if not os.path.exists(source):
            ...
            results["failed_operations"].append(
                (f"copy_{model_name}", f"Source file not found: {source}")
            )
            continue
    ...
    for model_name, destination in results["copied_files"]:
        ...
        if not success:
            results["failed_operations"].append(
                (f"permissions_{model_name}", f"Failed to set permissions on {destination}")
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apply template-generated tests to the main codebase"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force", action="store_true", help="Overwrite existing files without confirmation"
    )
    ...
    # Apply all changes
    results = apply_all_changes(dry_run=args.dry_run, backup=args.backup, force=args.force)
