...
def fix_indentation(content, model_name, architecture):
    ...
    while i < len(lines):
        ...
        if in_method and (
            line.strip().startswith("# Create dummy image")
            or line.strip().startswith("# Create dummy audio")
            or line.strip().startswith('if not os.path.exists("test.jpg")')
            or line.strip().startswith('if not os.path.exists("test.wav")')
            or line.strip().startswith("if not os.path.exists(self.test_audio_path)")
            or line.strip().startswith("# Generate a simple sine wave")
            or line.strip().startswith('if "whisper" in self.model_id.lower():')
            or line.strip().startswith('if self.model_type == "clip":')
            or line.strip().startswith('elif "wav2vec2" in self.model_id.lower():')
            or line.strip().startswith('elif self.model_type == "blip":')
            or line.strip().startswith("try:")
            or line.strip().startswith("import openvino")
            or line.strip().startswith("from optimum.intel import")
            or line.strip().startswith("except ImportError:")
        ):
...
def generate_test_file(model_name, architecture=None, debug=False):
    ...
    logger.info(
        f"Generating refactored test file for {model_name} with architecture {architecture}"
    )
    ...
    try:
        ...
        fixed_content = fixed_content.replace(
            f"Test{sanitized_model_name.capitalize()}{architecture.capitalize()}",
        ...
        fixed_content = fixed_content.replace(
            'self.model_id = "MODEL_ID"', f'self.model_id = "{model_name}"'
        )
...
def main():
    ...
    parser.add_argument(
        "--architecture", type=str, help="Model architecture (vision, text, speech, etc.)"
    )
    parser.add_argument(
        "--list-models", action="store_true", help="List available model architectures"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Print debug information for failed tests"
    )
