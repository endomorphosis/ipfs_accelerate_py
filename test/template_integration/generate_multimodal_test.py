...
def generate_multimodal_test(model_id, output_dir=None):
    ...
    if output_dir is None:
        output_dir = os.path.join(
            os.path.dirname(script_dir), "refactored_test_suite", "models", "multimodal"
        )
    ...
    reference_file = None
    if model_type == "clip" and os.path.exists(
        os.path.join(output_dir, "test_clip_vit_base_patch32.py")
    ):
        reference_file = os.path.join(output_dir, "test_clip_vit_base_patch32.py")
    elif model_type == "blip" and os.path.exists(
        os.path.join(output_dir, "test_blip_image_captioning_base.py")
    ):
        reference_file = os.path.join(output_dir, "test_blip_image_captioning_base.py")
    # For FLAVA, use CLIP as the reference
    elif model_type == "flava" and os.path.exists(
        os.path.join(output_dir, "test_clip_vit_base_patch32.py")
    ):
        reference_file = os.path.join(output_dir, "test_clip_vit_base_patch32.py")
