...
ARCHITECTURE_CATEGORIES = {
    ...
    "decoder_only": ["GPT2", "OPT", "GPTNeo", "GPTJ", "LLaMA", "Qwen", "TransfoXL"],
    "encoder_decoder": ["T5", "BART", "FlanT5", "MT5", "mBART", "LED", "PEGASUS", "Marian"],
    "speech": [
        "Whisper",
        "Wav2Vec2",
        "HuBERT",
...
def discover_transformers_classes() -> Dict[str, Dict[str, Any]]:
    ...
    for attr_name in dir(transformers):
        ...
        if (
            attr_name.endswith("Model")
            or attr_name.endswith("ForSequenceClassification")
            or attr_name.endswith("ForQuestionAnswering")
            or attr_name.endswith("ForMaskedLM")
            or attr_name.endswith("ForCausalLM")
            or attr_name.endswith("ForTokenClassification")
            or attr_name.endswith("ForImageClassification")
            or attr_name.endswith("ForConditionalGeneration")
        ):
            try:
    ...
    logger.info(
        f"Discovered {len(transformers_classes)} Transformers classes with from_pretrained support"
    )
...
def generate_test_for_class(
    class_name: str, class_info: Dict[str, Any], output_dir: str, overwrite: bool = False
) -> bool:
    """
    ...
    file_name = (
        "test_"
        + "".join(["_" + c.lower() if c.isupper() else c.lower() for c in class_name])
        .lstrip("_")
        .replace("_model", "")
        + ".py"
    )
    ...
    try:
        ...
        if result:
            ...
            model_short_name = "".join([c for c in class_name if c.isupper()]).lower()

            # Replace model specifics
            content = content.replace(
                os.path.basename(ref_path).replace(".py", ""), file_name.replace(".py", "")
            )
            ...
            content = content.replace(
                f"Test{ref_file.replace('test_', '').replace('.py', '').capitalize()}",
                f"Test{test_class_name}",
            )

            # Replace model ID
            content = content.replace(
                f'self.model_id = "{model_id}"',
            ...
            if class_info.get("pipeline_task"):
                content = content.replace(
                    f'self.task = "image-classification"',
                    f'self.task = "{class_info["pipeline_task"]}"',
                )
                content = content.replace(
                    f'self.task = "text-generation"', f'self.task = "{class_info["pipeline_task"]}"'
                )
                content = content.replace(
                    f'self.task = "fill-mask"', f'self.task = "{class_info["pipeline_task"]}"'
                )
            ...
            logger.info(
                f"Generated test file for {class_name} at {output_path} using reference file"
            )
            return True
...
def batch_generate_tests(
    ...
    if class_filter:
        filtered_classes = {
            class_name: info
            for class_name, info in filtered_classes.items()
    ...
    if dry_run:
        ...
        for class_name, info in classes_with_models.items():
            ...
            logger.info(
                f"Would generate test for {class_name} using model {info['recommended_model']} at {output_path}"
            )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_class = {
            executor.submit(
                generate_test_for_class, class_name, info, output_dir, overwrite
            ): class_name
            for class_name, info in classes_with_models.items()
        }
...
def generate_report(
    ...
    with open(output_file, "w") as f:
        ...
        testable_classes = sum(
            1 for info in class_info_dict.values() if info.get("recommended_model")
        )
        ...
        for category in sorted(ARCHITECTURE_CATEGORIES.keys()):
            cat_classes = {
                name: info for name, info in class_info_dict.items() if info["category"] == category
            }
            cat_total = len(cat_classes)
            cat_testable = sum(1 for info in cat_classes.values() if info.get("recommended_model"))
            cat_success = sum(
                1 for name, success in results.items() if name in cat_classes and success
            )
            cat_coverage = (cat_success / cat_testable) * 100 if cat_testable > 0 else 0

            f.write(
                f"| {category} | {cat_total} | {cat_testable} | {cat_success} | {cat_coverage:.1f}% |\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive test generator for HuggingFace models"
    )

    # Discovery options
    discovery_group = parser.add_argument_group("Discovery Options")
    discovery_group.add_argument(
        "--discover-only", action="store_true", help="Only discover classes, don't generate tests"
    )
    discovery_group.add_argument(
        "--discovery-output", type=str, help="Save discovered classes to JSON file"
    )
    ...
    generation_group.add_argument(
        "--classes", type=str, nargs="+", help="Specific class prefixes to generate tests for"
    )
    generation_group.add_argument("--output-dir", type=str, help="Output directory for test files")
    generation_group.add_argument(
        "--max-workers", type=int, default=4, help="Maximum number of parallel workers"
    )
    generation_group.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing test files"
    )
    generation_group.add_argument(
        "--results-file", type=str, help="Save results to specified JSON file"
    )
    generation_group.add_argument(
        "--report-file",
        type=str,
        default="hf_test_coverage_report.md",
        help="Generate coverage report to specified file",
    )

    # Other options
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't generate files, just list what would be done"
    )
    ...
    if total > 0:
        ...
        logger.info(
            f"Test generation complete: {successful}/{total} successful ({success_rate:.1f}%)"
        )
