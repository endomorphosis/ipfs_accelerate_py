...
class TestEncoderModel(ModelTest):
        ...
        try:
            ...
            if isinstance(outputs, list) and len(outputs) > 0:
                if self.task == "fill-mask":
                    top_prediction = (
                        outputs[0]["token_str"]
                        if "token_str" in outputs[0]
                        else outputs[0].get("token", "N/A")
                    )
    ...
    def test_openvino_compatibility(self):
        ...
        try:
            ...
            model = ov_model_class.from_pretrained(self.model_id, export=True, provider="CPU")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test encoder-only models with refactored test suite"
    )
