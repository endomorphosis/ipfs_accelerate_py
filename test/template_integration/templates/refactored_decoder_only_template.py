    ...
    def load_model(self, model_id=None):
        ...
        try:
            ...
            tokenizer_class = getattr(
                transformers, self.tokenizer_class, transformers.AutoTokenizer
            )

            # Load the tokenizer
            tokenizer = tokenizer_class.from_pretrained(model_id, use_fast=self.use_fast_tokenizer)
    ...
    def test_pipeline_inference(self):
        ...
        try:
            ...
            pipe = transformers.pipeline(
                self.task, model=self.model_id, device=self.device if self.device != "cpu" else -1
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test decoder-only models with refactored test suite"
    )
    ...
    parser.add_argument(
        "--max-tokens", type=int, default=20, help="Maximum number of tokens to generate"
    )
