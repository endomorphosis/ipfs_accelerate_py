...
class TestEncoderDecoderModel(ModelTest):
        ...
        if self.model_info["model_type"] == "t5" and not input_text.startswith(self.task):
            if "translation" in self.task:
                ...
                decoder_input_ids = tokenizer(
                    task_prefix + input_text, return_tensors="pt"
    ...
    def test_generation(self):
        ...
        if (
            self.model_info["model_type"] == "t5"
            and hasattr(self, "decoder_prefix")
            and self.decoder_prefix
        ):
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs, max_length=50, num_beams=4, early_stopping=True, num_return_sequences=1
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test encoder-decoder models with refactored test suite"
    )
