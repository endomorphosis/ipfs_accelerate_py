...
MODEL_CONFIG = {
    "layoutlmv2": {
        ...
        "test_inputs": {"image": "test.jpg", "text": "What is the title of this document?"},
        "processor_class": "LayoutLMv2Processor",
        "source_file": os.path.join(FINAL_MODELS_DIR, "test_layoutlmv2.py"),
        "custom_imports": ["from PIL import Image", "import numpy as np"],
...
if not os.path.exists(test_image_path):
    ...
    "layoutlmv3": {
        ...
        "task": "document-question-answering",
        "test_inputs": {"image": "test.jpg", "text": "What is the content of this document?"},
...
if not os.path.exists(test_image_path):
    ...
    "clvp": {
        ...
        "custom_imports": ["import numpy as np", "import librosa"],
...
def customize_template(template_content, model_name, model_config):
    ...
    for i, line in enumerate(lines):
        if (
            'test_input = "The quick brown fox jumps over the lazy dog."' in line
            and "text" in test_inputs
        ):
        ...
        if 'test_image_path = "test.jpg"' in line and "image" in test_inputs:
            lines[i] = line.replace(
                'test_image_path = "test.jpg"', f'test_image_path = "{test_inputs["image"]}"'
            )

        if 'test_audio_path = "test.wav"' in line and "audio" in test_inputs:
            lines[i] = line.replace(
                'test_audio_path = "test.wav"', f'test_audio_path = "{test_inputs["audio"]}"'
            )
...
def verify_test_file(file_path):
    ...
    try:
        ...
        context = "\n".join(
            [f"{i + start_line + 1}: {line}" for i, line in enumerate(lines[start_line:end_line])]
            if "lines" in locals()
            else []
        )
...
def main():
    ...
    parser.add_argument(
        "--generate-model", type=str, help="Generate a test file for a specific model"
    )
    parser.add_argument(
        "--generate-all", action="store_true", help="Generate test files for all models"
    )
    parser.add_argument(
        "--generate-specific",
        action="store_true",
        help="Generate test files for specific problematic models",
    )
