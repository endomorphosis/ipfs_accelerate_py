...
class TestBertSimple(ModelTest):
    ...
    def test_bert_pipeline(self):
        ...
        try:
            ...
            nlp = pipeline(
                "fill-mask", model=self.model_id, device=0 if self.device == "cuda" else -1
            )
    ...
    def test_bert_direct(self):
        ...
        try:
            ...
            self.assertTrue(
                hasattr(outputs, "last_hidden_state"), "Output should have last_hidden_state"
            )
    ...
    def print_results(self):
        ...
        if self.results.get("pipeline_success", False) or self.results.get("direct_success", False):
            ...
            if self.results.get("pipeline_success", False):
                ...
                print(
                    f"  Inference time: {self.results.get('pipeline_inference_time', 'N/A'):.2f}s"
                )
