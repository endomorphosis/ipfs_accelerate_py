...
async def test_adaptive_scaling():
    ...
    for model, model_type, model_name in models:
        ...
        result = model(inputs)
        logger.info(
            f"Model {model_name} ({model_type}) result: {result.get('inference_time'):.2f}s using {result.get('browser')} browser"
        )
    ...
    for i, result in enumerate(concurrent_results):
        ...
        logger.info(
            f"Concurrent model {model_name} ({model_type}) result: {result.get('inference_time'):.2f}s using {result.get('browser')} browser"
        )

    # Get metrics after concurrent execution
    after_concurrent_metrics = integration.get_metrics()
    logger.info(
        f"Metrics after concurrent execution: {json.dumps(after_concurrent_metrics, indent=2)}"
    )
