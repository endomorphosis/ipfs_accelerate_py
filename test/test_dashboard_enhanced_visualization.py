...
# Import dependencies
try:
    from data.duckdb.visualization.dashboard_enhanced_visualization import (
        DashboardEnhancedVisualizationSystem,
    )
...
def create_test_visualizations(vis_system, num_visualizations=3):
    ...
    if num_visualizations > 1:
        viz_path = vis_system.create_hardware_comparison_heatmap(
            metric="throughput", batch_size=1, title="Test Hardware Comparison Heatmap"
        )
        if viz_path:
            viz_paths.append(viz_path)
...
def test_dashboard_enhanced_visualization(
    db_path,
    output_dir,
    dashboard_url=None,
    api_key=None,
    ...
    vis_system = DashboardEnhancedVisualizationSystem(
        db_api=db_api, output_dir=output_dir, dashboard_url=dashboard_url, api_key=api_key
    )
...
def test_snapshot_export_import(db_path, output_dir, dashboard_url=None, api_key=None):
    ...
    import_success = vis_system.import_dashboard_snapshot(
        snapshot_path=snapshot_path, target_dashboard_id="test_import", merge_strategy="replace"
    )
...
def test_regression_detection_integration(db_path, output_dir, dashboard_url=None, api_key=None):
    ...
    try:
        from data.duckdb.distributed_testing.dashboard.regression_detection import (
            RegressionDetector,
        )
    ...
    try:
        ...
        logger.info(
            "Dashboard-enhanced visualization system with regression detection initialized."
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="./dashboard_visualization_tests",
    ...
    parser.add_argument(
        "--no-create", action="store_true", help="Skip creating test visualizations"
    )
    parser.add_argument("--no-sync", action="store_true", help="Skip synchronizing with dashboard")
    parser.add_argument("--no-panel", action="store_true", help="Skip creating dashboard panel")
    parser.add_argument(
        "--test",
        choices=["basic", "combined", "snapshot", "regression", "all"],
        default="all",
        help="Test to run",
    )
