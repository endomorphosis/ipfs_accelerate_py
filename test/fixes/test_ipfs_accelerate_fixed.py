...
try:
    ...
    if DEPRECATE_JSON_OUTPUT:
        print(
            "Warning: DuckDB not installed but DEPRECATE_JSON_OUTPUT=1. Will still save JSON as fallback."
        )
...
class IPFSAccelerateTest:
    ...
    async def test_api_endpoint(self, model, endpoint_list=None):
        ...
        try:
            ...
            if (
                not hasattr(self.ipfs_accelerate_py, "resources")
                or "tei_endpoints" not in self.ipfs_accelerate_py.resources
            ):
                return {"error": "Missing tei_endpoints in resources"}

            # Check if model exists in endpoints
            if (
                not hasattr(self.ipfs_accelerate_py, "endpoints")
                or "tei_endpoints" not in self.ipfs_accelerate_py.endpoints
            ):
            ...
            if endpoint_list:
                filtered_list = {
                    k: v for k, v in local_endpoints_by_model.items() if k in endpoint_list
                }
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hardware", type=str, default="cpu", help="Hardware to use (cpu, cuda, rocm, etc.)"
    )
    parser.add_argument("--batch-size", type=int, default=1, help="Batch size for inference")
    parser.add_argument(
        "--sequence-length", type=int, default=128, help="Sequence length for text models"
    )
    ...
    parser.add_argument(
        "--db-only", action="store_true", help="Only save results to database (no JSON)"
    )
