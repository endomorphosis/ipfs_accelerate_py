                        ...
                        if "setup_hardware" not in init_body:
                            ...
                            for i in range(init_end_line - 1, init_start_line, -1):
                                if lines[i].strip():
                                    ...
                                    lines.insert(
                                        i + 1, f"{' ' * (indent + 4)}self.setup_hardware()"
                                    )
                                    break
...
def fix_templates_in_directory(directory_path: str) -> Dict[str, Any]:
    ...
    """
    results = {"success": True, "total": 0, "fixed": 0, "failed": 0, "details": {}}
    ...
    try:
        ...
        for file_path in template_files:
            ...
            results["details"][file_path] = {"success": success, "message": message}
    ...
    if not HAS_DUCKDB or db_path.endswith(".json"):
        ...
        try:
            ...
            for template_id, template_data in templates.items():
                ...
                if valid:
                    logger.info(
                        f"Template {template_id} has valid syntax, checking for improvements"
                    )
                else:
