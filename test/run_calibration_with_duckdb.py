...
from data.duckdb.simulation_validation.calibration.uncertainty_quantification import (
    UncertaintyQuantifier,
)
from data.duckdb.simulation_validation.calibration.calibration_repository import (
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("calibration_duckdb.log")],
)
...
def load_data(sim_file: str, hw_file: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(simulation_results)} simulation results and {len(hardware_results)} hardware results"
        )
...
def create_calibrator(
    ...
    if calibrator_type == "basic":
        ...
        calibrator = EnsembleCalibrator(learning_rate=learning_rate, max_iterations=max_iterations)
    else:
        logger.warning(
            f"Unknown calibrator type: {calibrator_type}, falling back to BasicCalibrator"
        )
        calibrator = BasicCalibrator(learning_rate=learning_rate, max_iterations=max_iterations)
...
def run_calibration(
    ...
    calibrator = create_calibrator(
        calibrator_type, learning_rate, max_iterations, repository, calibration_id, metadata
    )
    ...
    if isinstance(calibrator, CalibratorDuckDBAdapter):
        ...
        error_before = calibrator._calculate_error(
            simulation_results, hardware_results, initial_parameters
        )
        error_after = calibrator._calculate_error(
            simulation_results, hardware_results, calibrated_parameters
        )
...
def run_cross_validation(
    ...
    if repository is not None:
        ...
        result = cross_validator.cross_validate(
            simulation_results, hardware_results, initial_parameters, None, group_by
        )
...
def run_parameter_discovery(
    ...
    logger.info(
        f"Parameter discovery completed with {len(result.get('sensitive_parameters', []))} sensitive parameters"
    )
    return result
...
def run_uncertainty_quantification(
    ...
    if repository is not None:
        ...
        param_uncertainty = adapter.quantify_parameter_uncertainty(parameter_sets, calibration_id)
        ...
        param_uncertainty = uncertainty_quantifier.quantify_parameter_uncertainty(parameter_sets)
...
def main():
    ...
    # Common arguments
    parser.add_argument(
        "--sim-file", type=str, required=True, help="Path to simulation result file (JSON)"
    )
    parser.add_argument(
        "--hw-file", type=str, required=True, help="Path to hardware result file (JSON)"
    )
    parser.add_argument(
        "--param-file", type=str, required=True, help="Path to parameter file (JSON)"
    )
    parser.add_argument(
        "--output-file",
        type=str,
        default="calibrated_parameters.json",
        help="Path to output file for calibrated parameters (JSON)",
    )
    parser.add_argument(
        "--db-path", type=str, default="calibration.duckdb", help="Path to DuckDB database file"
    )
    parser.add_argument(
        "--hardware-id", type=str, default=None, help="Hardware identifier (e.g., 'cuda', 'webgpu')"
    )
    parser.add_argument("--dataset-id", type=str, default=None, help="Dataset identifier")
    parser.add_argument(
        "--simulation-id", type=str, default=None, help="Simulation configuration identifier"
    )
    parser.add_argument(
        "--description", type=str, default=None, help="Description of this calibration/analysis"
    )
    ...
    cal_parser.add_argument(
        "--learning-rate", type=float, default=0.1, help="Learning rate for calibration"
    )
    cal_parser.add_argument(
        "--max-iterations", type=int, default=100, help="Maximum iterations for calibration"
    )
    cal_parser.add_argument(
        "--calibration-id",
        type=str,
    ...
    cal_parser.add_argument(
        "--bounds-file", type=str, default=None, help="File containing parameter bounds (JSON)"
    )
    cal_parser.add_argument("--no-db", action="store_true", help="Do not use DuckDB for storage")
    ...
    cv_parser.add_argument(
        "--test-size", type=float, default=0.2, help="Proportion of data to use for validation"
    )
    cv_parser.add_argument(
    ...
    cv_parser.add_argument(
        "--validation-id", type=str, default=None, help="Validation ID (generated if not provided)"
    )
    cv_parser.add_argument(
        "--calibration-id", type=str, default=None, help="Calibration ID to associate with"
    )
    cv_parser.add_argument("--group-by", type=str, default=None, help="Field to group results by")
    cv_parser.add_argument("--no-db", action="store_true", help="Do not use DuckDB for storage")
    ...
    disc_parser.add_argument(
        "--exploration-range", type=float, default=0.5, help="Range for parameter exploration"
    )
    disc_parser.add_argument(
        "--analysis-id", type=str, default=None, help="Analysis ID (generated if not provided)"
    )
    disc_parser.add_argument(
        "--calibration-id", type=str, default=None, help="Calibration ID to associate with"
    )
    disc_parser.add_argument("--no-db", action="store_true", help="Do not use DuckDB for storage")
    ...
    uq_parser.add_argument(
        "--n-samples", type=int, default=1000, help="Number of Monte Carlo samples"
    )
    uq_parser.add_argument(
        "--perturbation-factor", type=float, default=0.1, help="Factor for parameter perturbation"
    )
    uq_parser.add_argument(
        "--analysis-id", type=str, default=None, help="Analysis ID (generated if not provided)"
    )
    ...
    uq_parser.add_argument("--no-db", action="store_true", help="Do not use DuckDB for storage")
    ...
    sample_parser.add_argument(
        "--num-calibrations", type=int, default=5, help="Number of sample calibrations to generate"
    )
