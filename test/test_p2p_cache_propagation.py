...
class TestP2PCachePropagation:
    ...
    def test_broadcast_called_on_cache_put(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assert_equal(
                len(broadcast_calls), 1, "Broadcast should be called once when data is cached"
            )

            cache_key, entry = broadcast_calls[0]
            self.assert_true(
                "list_repos" in cache_key, f"Cache key should contain operation name: {cache_key}"
            )
            self.assert_equal(entry.data, test_data, "Broadcast should include the cached data")
    ...
    def test_broadcast_sends_to_connected_peers(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cache = configure_cache(cache_dir=tmpdir, enable_p2p=True, enable_persistence=False)
            ...
            cache._p2p_connected_peers = {"peer1": peer1, "peer2": peer2}
            ...
            async def mock_new_stream(peer_id, protocols):
                ...
                stream.write = AsyncMock(
                    side_effect=lambda data: sent_messages.append((peer_id, data))
                )
            ...
            entry = CacheEntry(
                data={"test": "data"}, timestamp=time.time(), ttl=300, content_hash="hash123"
            )
    ...
    def test_received_cache_entry_stored_locally(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assert_equal(
                cached_data, [{"name": "repo1"}], "Should retrieve the data received from peer"
            )
    ...
    def test_cache_entry_encryption(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            test_message = {
                "key": "test_key",
                "entry": {"data": {"secret": "value"}, "timestamp": time.time(), "ttl": 300},
            }
            ...
            self.assert_true(isinstance(encrypted, bytes), "Encrypted message should be bytes")

            # Verify we can decrypt it back
            decrypted = cache._decrypt_message(encrypted)
            self.assert_equal(
                decrypted["key"], test_message["key"], "Decrypted message should match original"
            )
    ...
    def test_broadcast_only_when_p2p_enabled(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_no_p2p = configure_cache(
                cache_dir=tmpdir, enable_p2p=False, enable_persistence=False
            )

            broadcast_calls_disabled = []
            original = cache_no_p2p._broadcast_in_background
            cache_no_p2p._broadcast_in_background = lambda k, e: broadcast_calls_disabled.append(
                (k, e)
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_with_p2p = configure_cache(
                cache_dir=tmpdir, enable_p2p=True, enable_persistence=False
            )

            broadcast_calls_enabled = []
            cache_with_p2p._broadcast_in_background = lambda k, e: broadcast_calls_enabled.append(
                (k, e)
            )
            ...
            self.assert_equal(
                len(broadcast_calls_enabled), 1, "Broadcast should be called when P2P is enabled"
            )
    ...
    def test_peer_hits_tracked_separately(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assert_true("hits" in stats, "Stats should include local hits")
            self.assert_true(
    ...
    def run_all_tests(self):
        ...
        # Run all tests
        self.run_test("Broadcast triggered on cache.put()", self.test_broadcast_called_on_cache_put)

        self.run_test(
            "Broadcast sends to all connected peers", self.test_broadcast_sends_to_connected_peers
        )

        self.run_test(
            "Received entries stored in local cache", self.test_received_cache_entry_stored_locally
        )

        self.run_test("Cache entries are encrypted", self.test_cache_entry_encryption)

        self.run_test("End-to-end P2P propagation flow", self.test_p2p_propagation_end_to_end)

        self.run_test("Broadcast only when P2P enabled", self.test_broadcast_only_when_p2p_enabled)

        self.run_test("Peer hits tracked separately", self.test_peer_hits_tracked_separately)
