...
def parse_args():
    ...
    parser.add_argument(
        "--model", "-m", type=str, required=True, help="Model name (e.g. bert-base-uncased)"
    )
    parser.add_argument(
        "--template-type",
        "-t",
    ...
    parser.add_argument(
        "--output", "-o", type=str, help="Output file path (if not specified, output to console)"
    )
    parser.add_argument(
        "--db-path",
        type=str,
    ...
    parser.add_argument(
        "--detect-hardware", action="store_true", help="Detect available hardware on the system"
    )
...
def prepare_template_context(
    model_name: str, hardware_platform: Optional[str] = None
) -> Dict[str, Any]:
...
def main():
    ...
    if not template:
        logger.error(
            f"Template not found for {model_type}/{args.template_type}/{args.hardware or 'generic'}"
        )
        return 1
