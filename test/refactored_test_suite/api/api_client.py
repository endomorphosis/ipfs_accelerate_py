...
class ApiClient:
    ...
    def run_test(
        ...
        # Send request
        try:
            response = self.session.post(f"{self.base_url}/api/test/run", json=request_data)
    ...
    def monitor_test(
        self, run_id: str, poll_interval: float = 1.0, timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        while True:
            ...
            logger.info(
                f"Test {run_id} - Status: {status['status']}, Progress: {status['progress']:.1%}, "
...
class AsyncApiClient:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/api/test/run", json=request_data, headers=headers
                ) as response:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.base_url}/api/test/status/{run_id}", headers=headers
                ) as response:
                    if response.status >= 400:
                        error_text = await response.text()
                        raise Exception(f"API error: {response.status} - {error_text}")
    ...
    async def get_test_results(self, run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.base_url}/api/test/results/{run_id}", headers=headers
                ) as response:
                    if response.status >= 400:
                        error_text = await response.text()
                        raise Exception(f"API error: {response.status} - {error_text}")
    ...
    async def connect_websocket(self, run_id: str) -> None:
        ...
        try:
            ...
            self._websocket = await session.ws_connect(
                f"{self.base_url}/api/test/ws/{run_id}", headers=headers
            )
