...
class TestAPIServer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, db_path: Optional[str] = None):
        ...
        self.runner = TestRunner(config={"results_dir": self.results_dir})
...
async def startup_event():
    ...
    if __name__ == "__main__":
        ...
        parser.add_argument(
            "--results-dir", type=str, default="./test_results", help="Directory for test results"
        )
        parser.add_argument("--db-path", type=str, help="Path to the DuckDB database file")
        args = parser.parse_args()

        # Create the test API manager with arguments
        manager = TestAPIServer(config={"results_dir": args.results_dir}, db_path=args.db_path)
    else:
        # Default configuration when imported as a module
        db_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "test_runs.duckdb"
        )
...
async def list_test_runs(limit: int = Query(100, gt=0, le=1000), status: Optional[str] = None):
    """
    List recent test runs.
...
def main():
    ...
    uvicorn.run("test_api_server:app", host=args.host, port=args.port, reload=False)
