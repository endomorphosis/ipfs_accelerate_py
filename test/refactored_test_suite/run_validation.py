...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"validation_run_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
try:
    from refactored_test_suite.validation.test_validator import (
        validate_test_files,
...
def parse_args():
    ...
    parser.add_argument(
        "--test-dir", default="./generated_tests", help="Directory containing generated test files"
    )

    parser.add_argument(
        "--report-dir", default="./reports", help="Directory to save validation reports"
    )

    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix common issues in invalid tests"
    )
...
def summarize_validation_results(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "validity_percentage": (results["valid"] / results["total"]) * 100
        if results["total"] > 0
        else 0,
        ...
        if not file_result["valid"]:
            if not file_result["syntax_valid"]:
                ...
                if not pattern_details.get("imports_model_test", False):
                    issues["missing_model_test_import"] = (
                        issues.get("missing_model_test_import", 0) + 1
                    )

                if not pattern_details.get("inherits_model_test", False):
                    issues["not_inheriting_model_test"] = (
                        issues.get("not_inheriting_model_test", 0) + 1
                    )
...
def generate_summary_report(summary: Dict[str, Any], output_file: str) -> None:
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"- **Valid files**: {summary['valid_files']} ({summary['validity_percentage']:.1f}%)\n"
        )
        ...
        for arch, count in sorted(
            summary["architecture_breakdown"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        if summary["common_issues"]:
            ...
            for issue, count in sorted(
                ...
                pct = (
                    (count / summary["invalid_files"]) * 100 if summary["invalid_files"] > 0 else 0
                )
...
def run_validation(args):
    ...
    detailed_report_path = os.path.join(
        args.report_dir,
        f"validation_details_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
    )
    ...
    summary_report_path = os.path.join(
        args.report_dir,
        f"validation_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
    )
