...
class TestIPFSAccelerateWebNNWebGPU(ModelTest):
    ...
    def test_firefox_optimizations(self):
        ...
        for result in self.results:
            self.assertTrue(
                result.get("p2p_optimized"), "Firefox optimizations should enable P2P optimization"
            )
    ...
    def _run_acceleration_tests(self, configs):
        ...
        for config in configs:
            ...
            self.logger.info(
                f"Testing {model} with {platform} on {browser} "
            ...
            try:
                ...
                self.logger.info(
                    f"  Throughput: {result['throughput_items_per_sec']:.2f} items/sec"
                )
