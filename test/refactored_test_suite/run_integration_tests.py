...
REPRESENTATIVE_MODELS = {
    ...
    "decoder-only": ["gpt2", "mistralai/Mistral-7B-v0.1"],
    "encoder-decoder": ["t5-small", "facebook/bart-base"],
    "vision": ["google/vit-base-patch16-224", "microsoft/swin-base-patch4-window7-224-in22k"],
    "vision-encoder-text-decoder": [
        "openai/clip-vit-base-patch32",
        "Salesforce/blip-image-captioning-base",
    ],
    "speech": ["openai/whisper-tiny", "facebook/wav2vec2-base-960h"],
    "multimodal": ["llava-hf/llava-1.5-7b-hf", "facebook/flava-full"],
}
...
def run_test_for_model(
    test_file: str, model_id: str = None, device: str = None, save_results: bool = True
) -> Dict[str, Any]:
    """
...
def run_integration_tests(args):
    ...
    for arch in architectures:
        ...
        for model in models:
            ...
            if not test_file:
                ...
                skipped.append(
                    {"model": model, "architecture": arch, "reason": "No test file found"}
                )
                continue
    ...
    report_file = os.path.join(
        args.output_dir,
        f"integration_test_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
    )
...
def generate_report(
    ...
    with open(report_file, "w") as f:
        ...
        for arch, stats in sorted(arch_results.items()):
            ...
            f.write(
                f"| {arch} | {stats['total']} | {stats['passed']} | {stats['total'] - stats['passed']} | {pass_rate:.1f}% |\n"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--test-dir", default="./generated_tests", help="Directory containing test files"
    )

    parser.add_argument(
        "--output-dir", default="./reports", help="Directory to save test results and reports"
    )
    ...
    parser.add_argument(
        "--device", choices=["cpu", "cuda", "mps"], help="Device to test on (default: auto-detect)"
    )
    ...
    parser.add_argument(
        "--mock", action="store_true", help="Use mocked dependencies (useful for CI/CD)"
    )
