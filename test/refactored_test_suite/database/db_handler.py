...
class TestDatabaseHandler:
    ...
    def store_test_run(self, run_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = self.connection.execute(
                "SELECT run_id FROM test_runs WHERE run_id = ?", [values["run_id"]]
            ).fetchone()
        ...
        try:
            ...
            if result:
                ...
                self.connection.execute(
                    ...
                    WHERE run_id = ?
                """,
                    [tests_passed, tests_failed, tests_skipped, result_data, run_id],
                )
            else:
                # Insert new results
                self.connection.execute(
    ...
    def store_test_metric(
        self, run_id: str, metric_name: str, metric_value: float, metric_unit: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            """,
                [run_id, metric_name, float(metric_value), metric_unit],
            )
    ...
    def store_batch_test_run(self, batch_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = self.connection.execute(
                "SELECT batch_id FROM batch_test_runs WHERE batch_id = ?", [values["batch_id"]]
            ).fetchone()
    ...
    def get_test_run(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if tags:
                run_data["tags"] = {tag["tag_name"]: tag["tag_value"] for tag in tags}
    ...
    def get_test_run_count(
        self, status: Optional[str] = None, model_name: Optional[str] = None
    ) -> int:
        """
