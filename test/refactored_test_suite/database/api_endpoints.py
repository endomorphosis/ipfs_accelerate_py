...
try:
    ...
    try:
        ...
        logging.error(
            "Error: TestDatabaseIntegration not found. Make sure db_integration.py is installed."
        )
        sys.exit(1)
...
async def get_run_history(
    ...
    return db.get_run_history(
        limit=limit, model_name=model_name, test_type=test_type, status=status
    )


@router.get("/batches")
async def get_batch_history(limit: int = Query(100, gt=0, le=1000), status: Optional[str] = None):
    """
...
async def get_performance_report(
    days: int = Query(30, gt=0, le=365), model_name: Optional[str] = None
):
    """
...
@router.post("/search")
async def search_runs(request: SearchRunsQuery):
    """
...
@router.post("/export")
async def export_database(request: ExportDatabaseRequest, background_tasks: BackgroundTasks):
    """
    ...
    def do_export():
        try:
            ...
            logger.info(
                f"Database export to {request.filename} {'succeeded' if success else 'failed'}"
            )
...
async def delete_run(run_id: str = Path(..., description="The ID of the test run to delete")):
    ...
    return {
        ...
        if success
        else f"Failed to delete test run {run_id}",
        "timestamp": datetime.datetime.now().isoformat(),
    }
...
async def delete_batch(
    ...
    return {
        ...
        if success
        else f"Failed to delete batch {batch_id}",
        "timestamp": datetime.datetime.now().isoformat(),
    }
