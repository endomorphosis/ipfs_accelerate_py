...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"batch_benchmark_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
class BatchBenchmarker:
    ...
    def __init__(self, db_path: Optional[str] = None, output_dir: str = "benchmark_results"):
        ...
        logger.info(
            f"Available hardware: {', '.join([hw for hw, available in self.available_hardware.items() if available])}"
        )
    ...
    def store_in_database(self, results: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            """,
                [raw_id, run_id, json.dumps(results)],
            )

            logger.info(
                f"Benchmark results for {results['model_id']} on {results['device']} stored in database"
            )
            return True
    ...
    def run_model_benchmark(
        ...
        try:
            ...
            results = benchmark.benchmark_model(
                batch_sizes=batch_sizes, sequence_lengths=sequence_lengths, iterations=iterations
            )
            ...
            return {"model_id": model_id, "device": device, "error": str(e)}
    ...
    def run_batch(
        ...
        logger.info(
            f"Running batch benchmark for {len(models)} models on {len(devices)} devices with {len(precisions)} precision modes"
        )
        ...
        if not DUCKDB_AVAILABLE or not self.db_connection:
            logger.warning(
                "DuckDB not available or no database connection. Results will not be stored in database."
            )
        ...
        for model_id, device, precision in itertools.product(models, devices, precisions):
            ...
            remaining = (
                (elapsed / completed) * (total_benchmarks - completed) if completed > 0 else 0
            )

            logger.info(
                f"Progress: {completed}/{total_benchmarks} ({completed / total_benchmarks * 100:.1f}%)"
            )
            logger.info(
                f"Elapsed: {elapsed / 60:.1f} minutes, Est. remaining: {remaining / 60:.1f} minutes"
            )
    ...
    def generate_report(self, results: Dict[str, Dict[str, Any]], output_file: str):
        ...
        try:
            with open(output_file, "w") as f:
                ...
                f.write(
                    f"| Model | Architecture | Device | Precision | Latency (ms) | Throughput (samples/s) | Memory (MB) |\n"
                )
                f.write(
                    f"|-------|--------------|--------|-----------|--------------|------------------------|------------|\n"
                )
                    ...
                    for res in model_results:
                        ...
                        if batch_key in res.get("batch_results", {}):
                            ...
                            if batch_result.get("success", False):
                                ...
                                f.write(
                                    f"| {model_id} | {arch_type} | {device} | {precision} | {latency:.2f} | {throughput:.2f} | {memory:.2f} |\n"
                                )
                ...
                f.write(
                    f"| Model | Best Device | Batch Size | Seq Length | Throughput (samples/s) |\n"
                )
                f.write(
                    f"|-------|-------------|------------|------------|------------------------|\n"
                )
                ...
                for model_id, model_results in models.items():
                    ...
                    if best_config:
                        f.write(
                            f"| {model_id} | {best_config['device']} | {best_config['batch_size']} | {best_config['seq_len']} | {best_config['throughput']:.2f} |\n"
                        )
                    else:
...
def main():
    ...
    parser.add_argument(
        "--model-list", type=str, help="File containing list of models to benchmark (one per line)"
    )

    parser.add_argument("--models", type=str, help="Comma-separated list of models to benchmark")

    parser.add_argument(
        "--devices", type=str, default="cpu", help="Comma-separated list of devices to benchmark on"
    )

    parser.add_argument(
        "--precisions", type=str, default="float32", help="Comma-separated list of precision modes"
    )
    ...
    parser.add_argument(
        "--report",
        type=str,
        default="batch_benchmark_report.md",
