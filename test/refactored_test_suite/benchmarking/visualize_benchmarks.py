...
try:
    ...
    logger.warning(
        "Matplotlib/Pandas not available. Visualizations will be limited to text output."
    )
...
class BenchmarkVisualizer:
    ...
    def visualize_json_result(
        self, data: Dict[str, Any], output_dir: str = "benchmark_visualizations"
    ):
        """
        ...
        with open(summary_path, "w") as f:
            ...
            if "summary" in data:
                ...
                f.write(
                    f"- Average Latency: {data['summary'].get('average_latency_ms', 0):.2f} ms\n"
                )
                f.write(
                    f"- Best Throughput: {data['summary'].get('best_throughput_samples_per_sec', 0):.2f} samples/sec\n"
                )
            ...
            f.write(
                f"{'Batch Size':<10} {'Seq Length':<10} {'Latency (ms)':<15} {'Throughput (samples/s)':<25} {'Memory (MB)':<15}\n"
            )
            f.write("-" * 80 + "\n")

            for _, row in df.sort_values(["sequence_length", "batch_size"]).iterrows():
                f.write(
                    f"{row['batch_size']:<10} {row['sequence_length']:<10} {row['latency_ms']:<15.2f} {row['throughput']:<25.2f} {row['memory_mb']:<15.2f}\n"
                )

        logger.info(f"Summary saved to {summary_path}")

    def visualize_hardware_comparison(
        self, model_id: str, output_dir: str = "benchmark_visualizations"
    ):
        """
        ...
        try:
            ...
            for device in df["device"].unique():
                ...
                if not subset.empty:
                    plt.plot(
                        subset["sequence_length"],
                        subset["throughput_samples_per_sec"],
                        marker="o",
                        label=device,
                        color=HARDWARE_COLORS.get(device, None),
                    )
            ...
            with open(summary_path, "w") as f:
                ...
                f.write("-" * 80 + "\n")
                f.write(
                    f"{'Device':<10} {'Precision':<10} {'Latency (ms)':<15} {'Throughput (samples/s)':<25} {'Memory (MB)':<15}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                for device in df["device"].unique():
                    ...
                    if not subset.empty:
                        row = subset.iloc[0]
                        f.write(
                            f"{device:<10} {row['precision']:<10} {row['latency_mean_ms']:<15.2f} {row['throughput_samples_per_sec']:<25.2f} {row['memory_usage_mb']:<15.2f}\n"
                        )
                ...
                f.write("-" * 80 + "\n")
                f.write(
                    f"{'Device':<10} {'Batch Size':<10} {'Seq Length':<10} {'Throughput (samples/s)':<25} {'Latency (ms)':<15}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                for device in df["device"].unique():
                    ...
                    if not subset.empty:
                        ...
                        f.write(
                            f"{device:<10} {row['batch_size']:<10} {row['sequence_length']:<10} {row['throughput_samples_per_sec']:<25.2f} {row['latency_mean_ms']:<15.2f}\n"
                        )
    ...
    def visualize_architecture_comparison(self, output_dir: str = "benchmark_visualizations"):
        ...
        try:
            ...
            with open(summary_path, "w") as f:
                ...
                f.write("-" * 80 + "\n")
                f.write(
                    f"{'Architecture':<20} {'Device':<10} {'Avg Throughput':<20} {'Model Count':<15}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                for arch in arch_types:
                    ...
                    for device in devices:
                        ...
                        if not subset.empty:
                            ...
                            if first:
                                f.write(
                                    f"{arch:<20} {device:<10} {row['avg_throughput']:<20.2f} {row['model_count']:<15}\n"
                                )
                                first = False
                            else:
                                f.write(
                                    f"{'':<20} {device:<10} {row['avg_throughput']:<20.2f} {'':<15}\n"
                                )
                    f.write("\n")
    ...
    def _text_summary(self, data: Dict[str, Any]):
        ...
        print(
            f"{'Batch Size':<10} {'Seq Length':<12} {'Latency (ms)':<15} {'Throughput (samples/s)':<25} {'Memory (MB)':<15}"
        )
        print("-" * 80)
        ...
        for batch_key, result in sorted(data.get("batch_results", {}).items()):
            ...
            print(
                f"{batch_size:<10} {seq_len:<12} {result.get('latency_mean_ms', 0):<15.2f} {result.get('throughput_samples_per_sec', 0):<25.2f} {result.get('memory_usage_mb', 0):<15.2f}"
            )
...
def main():
    ...
    parser.add_argument("--json-path", type=str, help="Path to JSON benchmark result file")

    parser.add_argument("--db-path", type=str, help="Path to benchmark database")

    parser.add_argument(
        "--model-id", type=str, help="Model ID for hardware comparison visualization"
    )
