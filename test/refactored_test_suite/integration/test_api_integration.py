...
class TestAPIServer:
    ...
    async def run_test(
        self, request: TestRunRequest, background_tasks: BackgroundTasks
    ) -> TestRunResponse:
        """
    ...
    async def _run_test(
        ...
        try:
            ...
            self.active_tests[run_id]["results"] = {
                ...
                "success": True,
                "error": None,
                "details": {"tests_passed": 10, "tests_failed": 0, "tests_skipped": 0},
    ...
    async def handle_websocket_connection(self, websocket: WebSocket, run_id: str):
        ...
        if run_id not in self.active_tests:
            await websocket.send_json({"error": f"Test run {run_id} not found"})
...
if FASTAPI_AVAILABLE:
    app = FastAPI(
        title="Test Suite API", description="API for running and monitoring tests", version="1.0.0"
    )
...
@unittest.skipIf(
    not TEST_SUITE_AVAILABLE or not BENCHMARK_AVAILABLE or not GENERATOR_AVAILABLE,
