...
class ModelTestGenerator:
    ...
    def get_template_for_architecture(self, arch_type: str) -> str:
        ...
        template_file = os.path.join(
            self.template_dir, template_map.get(arch_type, "encoder_only_template.py")
        )
...
"""
    ...
    def generate_test_file(
        ...
        try:
            ...
            if arch_type in ["diffusion", "mixture-of-experts", "state-space", "rag"]:
                ...
                "-" in model_name
                or "_" in model_name
                or any(x in model_name.lower() for x in ["1", "2", "3", "7b", "8b", "13b", "70b"])
            ):
    ...
    def generate_models_by_priority(
        self, priority: str = "high", verify: bool = True, force: bool = False
    ) -> Tuple[int, int, int]:
        """
    ...
    def generate_coverage_report(self, output_file: str = "model_test_coverage.md") -> bool:
        ...
        try:
            ...
            for model in all_models:
                ...
                potential_files = [f"test_hf_{model_snake}.py", f"test_hf_{model}.py"]
            ...
            for arch_type in ARCHITECTURE_TYPES:
                models_in_arch = [
                    m for m, info in all_models.items() if info["architecture"] == arch_type
                ]
                ...
                arch_stats[arch_type] = {
                    ...
                    "coverage_pct": (len(models_with_tests_in_arch) / len(models_in_arch)) * 100
                    if len(models_in_arch) > 0
                    else 0,
                }
            ...
            for priority in PRIORITY_MODELS:
                models_in_priority = [
                    m for m, info in all_models.items() if info["priority"] == priority
                ]
                models_with_tests_in_priority = [
                    m for m in models_in_priority if all_models[m]["has_test"]
                ]

                priority_stats[priority] = {
                    "total": len(models_in_priority),
                    "with_tests": len(models_with_tests_in_priority),
                    "coverage_pct": (len(models_with_tests_in_priority) / len(models_in_priority))
                    * 100
                    if len(models_in_priority) > 0
                    else 0,
                }
            ...
            with open(output_file, "w") as f:
                ...
                for arch_type, stats in arch_stats.items():
                    f.write(
                        f"| {arch_type} | {stats['total']} | {stats['with_tests']} | {stats['coverage_pct']:.1f}% |\n"
                    )
                ...
                for priority, stats in priority_stats.items():
                    f.write(
                        f"| {priority} | {stats['total']} | {stats['with_tests']} | {stats['coverage_pct']:.1f}% |\n"
                    )
                ...
                high_missing = [
                    ...
                    if info["priority"] == "high" and not info["has_test"]
                ]
                if high_missing:
                ...
                medium_missing = [
                    m
                    for m, info in all_models.items()
                    if info["priority"] == "medium" and not info["has_test"]
                ]
                if medium_missing:
...
if __name__ == "__main__":
    ...
    if args.model:
        success, file_path = generator.generate_test_file(
            args.model, force=args.force, verify=args.verify
        )
        if success:
