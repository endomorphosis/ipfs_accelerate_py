...
class TestClapModels(ModelTest):
    ...
    def setUp(self):
        ...
        self.test_audio, _ = create_test_audio(
            self.test_audio_path, self.audio_duration, self.sample_rate
        )
    ...
    def test_audio_text_similarity(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                (len(text_prompts), len(text_prompts)),
            ...
            for i, text in enumerate(text_prompts):
                similarities = [
                    f"{similarity_matrix[i, j].item():.4f}" for j in range(len(text_prompts))
                ]
    ...
    def test_hardware_compatibility(self):
        ...
        for device in devices_to_test:
            ...
            try:
                ...
                self.assertIn(
                    "audio_embeds", outputs, f"Model on {device} should output audio embeddings"
                )
                self.assertIn(
                    "text_embeds", outputs, f"Model on {device} should output text embeddings"
                )
