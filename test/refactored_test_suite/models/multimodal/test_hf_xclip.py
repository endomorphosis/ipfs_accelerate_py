...
class TestXCLIPModels(ModelTest):
    ...
    def setUp(self):
        ...
        if self.model_id not in XCLIP_MODELS_REGISTRY:
            self.logger.warning(
                f"Model {self.model_id} not in registry, using default configuration"
            )
    ...
    def test_basic_inference(self):
        ...
        try:
            ...
            # Process inputs
            inputs = processor(
                text=self.test_text_candidates, images=image, return_tensors="pt", padding=True
            )
    ...
    def test_pipeline_api(self):
        ...
        try:
            ...
            pipeline = transformers.pipeline(
                "zero-shot-image-classification", model=self.model_id, device=self.device
            )
            ...
            # Run inference
            start_time = time.time()
            outputs = pipeline(image, candidate_labels=self.test_text_candidates)
            inference_time = time.time() - start_time
    ...
    def test_image_text_similarity(self):
        ...
        try:
            ...
            # Process inputs
            inputs = processor(
                text=self.test_text_candidates, images=images, return_tensors="pt", padding=True
            )
            ...
            self.assertEqual(
                probs_per_image.shape,
                (len(images), len(self.test_text_candidates)),
    ...
    def test_hardware_compatibility(self):
        ...
        for device in devices_to_test:
            ...
            try:
                ...
                self.assertIn(
                    "logits_per_image", outputs, f"Model on {device} should output logits_per_image"
                )

                # Store performance results
                self.results[f"performance_{device}"] = {"inference_time": inference_time}
    ...
    def test_model_loading(self):
        ...
        try:
            ...
            if (
                "bert" in self.model_id.lower()
                or "gpt" in self.model_id.lower()
                or "t5" in self.model_id.lower()
            ):
                import transformers
