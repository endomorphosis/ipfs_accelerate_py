...
class TestSingleModelHardware(ModelTest):
    ...
    def find_test_class(self, module):
        ...
        for attr_name in dir(module):
            ...
            if isinstance(attr, type) and any(
                pattern in attr_name for pattern in test_class_patterns
            ):
                return attr
    ...
    def run_tests_on_all_platforms(self):
        ...
        for platform in self.hardware_platforms:
            ...
            if result["success"]:
                ...
                self.logger.error(
                    f"❌ {platform} test failed: {result.get('error', 'Unknown error')}"
                )
    ...
    def generate_report(self, results):
        ...
        with open(report_file, "w") as f:
            ...
            failures = [
                (platform, result) for platform, result in results.items() if not result["success"]
            ]
    ...
    def test_model_on_all_platforms(self):
        ...
        self.assertEqual(
            len(failures), 0, f"Some tests failed: {', '.join(p for p, _ in failures)}"
        )
