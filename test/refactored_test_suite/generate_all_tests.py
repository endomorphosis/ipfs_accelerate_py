...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"generate_all_tests_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
try:
    from scripts.generators.test_generator import ModelTestGenerator, PRIORITY_MODELS
    from scripts.generators.architecture_detector import ARCHITECTURE_TYPES
except ImportError:
    # Try with full path
    try:
        from refactored_test_suite.generators.test_generator import (
            ModelTestGenerator,
            PRIORITY_MODELS,
        )
        ...
        logger.error(
            "Failed to import required modules. Please ensure you're running from the correct directory."
        )
        sys.exit(1)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generate tests for multiple models across architectures"
    )
    ...
    parser.add_argument(
        "--model", help="Generate test for a specific model instead of by priority/architecture"
    )

    parser.add_argument(
        "--output-dir", default="./generated_tests", help="Directory to save generated test files"
    )
    ...
    parser.add_argument(
        "--coverage-file", default="model_test_coverage.md", help="Path to save the coverage report"
    )
...
def get_models_to_generate(priority: str, architecture: str) -> List[str]:
    ...
    return [
        model for model in priority_models if any(arch_model in model for arch_model in arch_models)
    ]
...
def generate_tests(args) -> Tuple[int, int, int]:
    ...
    if not models_to_generate:
        logger.warning(
            f"No models found for priority={args.priority}, architecture={args.architecture}"
        )
        return (0, 0, 0)
    ...
    for model_type in models_to_generate:
        ...
        success, file_path = generator.generate_test_file(
            model_type, force=args.force, verify=args.verify
        )
