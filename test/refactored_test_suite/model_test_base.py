...
class ModelTest:
    ...
    def load_model(self):
        ...
        try:
            if self.is_mock:
                ...
                self.tokenizer.__call__ = MagicMock(
                    return_value={"input_ids": [101, 2054, 2003, 2009, 102]}
                )

                # Set up model mock behavior
                self.model.__call__ = MagicMock(
                    return_value=MagicMock(logits=torch.randn(1, 5, 30522))
                )

                # Set up processor mock behavior
                self.processor.__call__ = MagicMock(
                ...
                if self.device in ["cuda", "cpu", "mps"]:
                    ...
                    if self.task == "text-generation":
                        self.model = transformers.AutoModelForCausalLM.from_pretrained(
                            self.model_id,
                            device_map="cuda",  # ROCm uses "cuda" as device identifier
                        )
                    elif self.task == "text2text-generation":
                        self.model = transformers.AutoModelForSeq2SeqLM.from_pretrained(
                    ...
                    try:
                        ...
                        if self.task == "text-generation":
                            ...
                            logger.warning(
                                f"Task {self.task} not directly supported in OpenVINO, using PyTorch"
                            )
                            self.model = transformers.AutoModel.from_pretrained(
    ...
    def run_text_inference(self, model_data):
        ...
        try:
            ...
            if self.is_mock:
                ...
                if self.task == "text-generation":
                    ...
                    output = tokenizer.decode(
                        outputs.logits.argmax(-1)[0], skip_special_tokens=True
                    )
                else:
                    outputs = model(**inputs)
                    output = str(outputs)
            ...
            return {"success": True, "time_seconds": end_time - start_time, "output": output}

        except Exception as e:
            end_time = time.time()
            logger.error(f"Error running text inference: {e}")

            return {"success": False, "time_seconds": end_time - start_time, "error": str(e)}
    ...
    def save_results(self, output_dir):
        ...
        try:
            ...
            output_path = os.path.join(
                output_dir, f"model_test_{self.model_type}_{int(time.time())}.json"
            )
