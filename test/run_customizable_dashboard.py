...
def create_dashboard(args):
    ...
    dashboard = CustomizableDashboard(
        db_connection=None, theme=args.theme, debug=args.debug, output_dir=args.output_dir
    )
...
def list_dashboards(args):
    ...
    try:
        ...
        for name, details in dashboards.items():
            created = (
                details.get("created_at", "").split("T")[0] if details.get("created_at") else ""
            )
            print(
                f"{name:<30} {details.get('title', ''):<30} {details.get('components', 0):<10} {created:<20}"
            )
...
def export_dashboard(args):
    ...
    try:
        ...
        export_path = dashboard.export_dashboard(
            dashboard_name=args.name, format=args.format, output_path=args.output_path
        )
...
def main():
    ...
    create_parser.add_argument("--name", type=str, required=True, help="Dashboard name")
    ...
    create_parser.add_argument("--title", type=str, help="Dashboard title")
    create_parser.add_argument("--description", type=str, help="Dashboard description")
    create_parser.add_argument(
        "--columns", type=int, default=2, help="Number of columns in the grid layout"
    )
    create_parser.add_argument(
        "--row-height", type=int, default=500, help="Height of each row in pixels"
    )
    create_parser.add_argument(
        "--theme", type=str, choices=["light", "dark"], default="light", help="Dashboard theme"
    )
    create_parser.add_argument(
        "--include-3d", action="store_true", help="Include 3D visualization component"
    )
    create_parser.add_argument(
        "--include-heatmap", action="store_true", help="Include heatmap visualization components"
    )
    create_parser.add_argument(
        "--include-timeseries",
        action="store_true",
    ...
    create_parser.add_argument(
        "--include-all", action="store_true", help="Include all visualization components"
    )
    create_parser.add_argument(
        "--open", action="store_true", help="Open dashboard in browser after creation"
    )
    ...
    templates_parser = subparsers.add_parser(
        "templates", help="List all available templates and components"
    )

    # Add component command
    add_parser = subparsers.add_parser("add", help="Add a component to a dashboard")
    add_parser.add_argument("--name", type=str, required=True, help="Dashboard name")
    ...
    add_parser.add_argument(
        "--config-file", type=str, help="Path to component configuration JSON file"
    )
    add_parser.add_argument(
        "--width", type=int, default=1, help="Component width (number of columns)"
    )
    add_parser.add_argument(
        "--height", type=int, default=1, help="Component height (number of rows)"
    )
    add_parser.add_argument(
        "--open", action="store_true", help="Open dashboard in browser after adding component"
    )

    # Update dashboard command
    update_parser = subparsers.add_parser("update", help="Update an existing dashboard")
    update_parser.add_argument("--name", type=str, required=True, help="Dashboard name")
    update_parser.add_argument("--title", type=str, help="Dashboard title")
    update_parser.add_argument("--description", type=str, help="Dashboard description")
    update_parser.add_argument("--columns", type=int, help="Number of columns in the grid layout")
    update_parser.add_argument("--row-height", type=int, help="Height of each row in pixels")
    update_parser.add_argument(
        "--open", action="store_true", help="Open dashboard in browser after update"
    )

    # Export dashboard command
    export_parser = subparsers.add_parser("export", help="Export a dashboard to different formats")
    export_parser.add_argument("--name", type=str, required=True, help="Dashboard name")
    export_parser.add_argument(
        "--format", type=str, choices=["html", "png", "pdf"], default="html", help="Export format"
    )
    export_parser.add_argument("--output-path", type=str, help="Output file path")
    export_parser.add_argument(
        "--open", action="store_true", help="Open exported dashboard in browser (HTML only)"
    )

    # Delete dashboard command
    delete_parser = subparsers.add_parser("delete", help="Delete a dashboard")
    delete_parser.add_argument("--name", type=str, required=True, help="Dashboard name")
