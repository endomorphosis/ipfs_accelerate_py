...
def run_find_models(hyphenated_only=True, output_file=None):
    ...
    if not output_file:
        output_file = (
            CURRENT_DIR / f"model_data_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def update_generator(model_data_file):
    ...
    for model_type, entry in registry_entries.items():
        ...
        if (
            f'"{model_type}"' in generator_content
            and f'model_type": "{model_type}"' in generator_content
        ):
...
def main():
    ...
    parser.add_argument(
        "--hyphenated-only", action="store_true", help="Only process hyphenated model names"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(FIXED_TESTS_DIR),
    ...
    parser.add_argument(
        "--skip-find-models",
        action="store_true",
        help="Skip finding models step (use existing data)",
    )
    parser.add_argument(
        "--model-data", type=str, help="Path to existing model data file (if skipping find-models)"
    )
    ...
    if not run_generator(
        output_dir=args.output_dir, hyphenated_only=args.hyphenated_only, verify=args.verify
    ):
