...
"""
            content = (
                content[: next_line_index + 1] + color_definitions + content[next_line_index + 1 :]
            )
...
def main():
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only check for colorized output without modifying",
    )
    ...
    if args.template:
        ...
        success_count, failure_count, total_count = process_all_templates(
