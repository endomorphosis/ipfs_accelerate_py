...
"""
Class-based test file for all Orca3-family models.
...
class TestOrca3Models:
    """Base test class for all Orca3-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "microsoft/Orca-3-7B"
        
        # Verify model exists in registry
        if self.model_id not in ORCA3_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = ORCA3_MODELS_REGISTRY["microsoft/Orca-3-7B"]
        else:
            self.model_info = ORCA3_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Explain how nuclear fusion works in simple terms"
        self.test_texts = [
            "Explain how nuclear fusion works in simple terms",
            "Explain how nuclear fusion works in simple terms (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "Orca3ForCausalLM":
            model_class = transformers.Orca3ForCausalLM
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_orca3_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Orca3 models in the registry."""
    return list(ORCA3_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Orca3 models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestOrca3Models(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_orca3_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Orca3-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Orca3-family models:")
        for model in models:
            info = ORCA3_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "microsoft/Orca-3-7B"
    logger.info(f"Testing model: {model_id}")
