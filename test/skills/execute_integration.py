...
def update_model_families():
    ...
    updated_content = content.replace(
        match.group(0), "MODEL_FAMILIES = {" + fixed_match.group(1) + "}"
    )
...
"""
    ...
    write_pattern = re.compile(
        r"with open\(output_path, \"w\"\) as f:\s+f\.write\(file_content\)\s+print\(f\"Test file generated: {output_path}\"\)"
    )
    ...
    parser_pattern = re.compile(
        r"parser = argparse\.ArgumentParser\(description=\"Generate model test files\"\)"
    )
    if parser_pattern.search(updated_content):
        add_arg_pattern = re.compile(
            r"parser\.add_argument\(\"--output\", type=str, help=\"Output directory\"\)"
        )
        if add_arg_pattern.search(updated_content):
            updated_content = add_arg_pattern.sub(
                r"\g<0>\n    parser.add_argument(\"--validate-syntax\", action=\"store_true\", help=\"Validate syntax of generated files\")",
                updated_content,
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=TEMP_OUTPUT_DIR, help="Directory for test output files"
    )
