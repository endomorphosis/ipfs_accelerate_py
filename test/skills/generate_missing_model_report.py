...
class MissingModelAnalyzer:
    ...
    def __init__(self, directory: str, report_file: Optional[str] = None):
        ...
        self.stats["by_architecture"]["unknown"] = {"implemented": 0, "missing": 0, "total": 0}

    def run(self):
        """Run the analysis."""
        # Find all existing test files - both test_hf_*.py and plain test_*.py
        test_files = list(self.directory.glob("test_hf_*.py")) + list(
            self.directory.glob("test_*.py")
        )
        ...
        for architecture, models in ALL_MODELS.items():
            ...
            for model in models:
                if model not in self.implemented_models:
                    ...
                    missing_in_arch.append({"name": model, "priority": priority})
        ...
        for architecture in self.missing_models:
            self.missing_models[architecture].sort(
                key=lambda m: (
                    0 if m["priority"] == "critical" else (1 if m["priority"] == "high" else 2)
                )
            )
    ...
    def _get_model_architecture(self, model_name: str) -> str:
        ...
        for architecture, models in ARCHITECTURE_TYPES.items():
            if any(
                model_name == model
                or model_name.startswith(f"{model}-")
                or model_name.startswith(f"{model}_")
                for model in models
            ):
                return architecture
    ...
    def _generate_report(self):
        ...
        report.extend(
            ["", "### High Priority Models", "", "These models should be implemented next:"]
        )
        ...
        for arch in sorted(self.missing_models.keys()):
            medium_models = [
                model["name"]
                for model in self.missing_models[arch]
                if model["priority"] == "medium"
            ]

            if medium_models:
                report.extend(["", f"#### {arch.capitalize()} Models", ""])
        ...
        report.extend(
            ["", "## Implemented Models", "", "These models already have test implementations:"]
        )
...
def main():
    ...
    parser.add_argument(
        "--report", "-r", type=str, default="reports/missing_models.md", help="Path to write report"
    )
    ...
    print(
        f"Implemented models: {analyzer.stats['total_implemented']} ({analyzer.stats['total_implemented'] / total_models * 100:.1f}%)"
    )
    print(
        f"Missing models: {analyzer.stats['total_missing']} ({analyzer.stats['total_missing'] / total_models * 100:.1f}%)"
    )
    print("\nPriority breakdown:")
