...
def fix_class_methods(content):
    ...
    content = re.sub(
        r"self\.performance_stats = {}\s+(\s+)def", r"self.performance_stats = {}\n\n\1def", content
    )

    # Fix method declarations to have 4 spaces
    content = re.sub(
        r"^(\s*)def (test_\w+|run_tests)\(", r"    def \2(", content, flags=re.MULTILINE
    )
...
def fix_try_except_blocks(content):
    ...
    fixed_content = re.sub(r"(\s+)try:\n\s+(\w)", r"\1try:\n\1    \2", content)

    fixed_content = re.sub(
        r"(\s+)except Exception:\n\s+(\w)", r"\1except Exception:\n\1    \2", fixed_content
    )

    fixed_content = re.sub(
        r"(\s+)except ImportError:\n\s+(\w)", r"\1except ImportError:\n\1    \2", fixed_content
    )

    # Fix try/except within methods (deeper indentation)
    fixed_content = re.sub(r"(\s{8})try:\n\s+(\w)", r"\1try:\n\1    \2", fixed_content)
...
def fix_if_blocks(content):
    ...
    fixed_content = re.sub(
        r'(\s+)if device == "cuda":\n\s+(\w)', r'\1if device == "cuda":\n\1    \2', content
    )

    fixed_content = re.sub(
        r'(\s+)if device != "cpu":\n\s+(\w)', r'\1if device != "cpu":\n\1    \2', fixed_content
    )
