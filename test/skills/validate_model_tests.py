...
class ModelTestValidator:
    ...
    def _validate_structure(self, content: str, model_name: str) -> Dict[str, Any]:
        ...
        if class_match:
            ...
            if expected_class_name.lower() != class_name.lower():
                errors.append(
                    f"Test class name mismatch: expected Test{expected_class_name}Models, got Test{class_name}Models"
                )
        else:
        ...
        result["structure_valid"] = (
            result["has_test_class"] and result["has_test_pipeline"] and result["has_run_tests"]
        )
    ...
    def _validate_pipeline(
        self, content: str, model_name: str, architecture: str
    ) -> Dict[str, Any]:
        """Validate the pipeline configuration of a test file."""
        result = {"pipeline_valid": False, "has_pipeline": False, "task_valid": False}
        ...
        if pipeline_match:
            ...
            if current_task == recommended_task:
                ...
                errors.append(
                    f"Task mismatch: using '{current_task}', recommended '{recommended_task}'"
                )
    ...
    def _generate_report(self):
        ...
        for file_path, result in self.validation_results.items():
            if (
                result["syntax_valid"]
                and result.get("structure_valid", False)
                and result.get("pipeline_valid", False)
            ):
        ...
        # Add valid files
        report.extend(["### Valid Files", "", "These files passed all validation checks:", ""])

        for file_path, result in sorted(files_by_status["all_valid"]):
            report.append(
                f"- `{os.path.basename(file_path)}` - {result['architecture']} - Task: {result.get('current_task', 'N/A')}"
            )
        ...
        for status, title, description in [
            (
                "syntax_invalid",
                "Syntax Errors",
                "These files have syntax errors that need to be fixed:",
            ),
            (
                "structure_invalid",
                "Structure Issues",
                "These files have structural issues (missing class or methods):",
            ),
            (
                "pipeline_missing",
                "Missing Pipeline",
                "These files are missing pipeline configuration:",
            ),
            (
                "pipeline_invalid",
                "Incorrect Pipeline",
                "These files have pipeline configuration issues:",
            ),
        ]:
            if files_by_status[status]:
                report.extend(["", f"### {title}", "", description, ""])
...
def main():
    ...
    parser.add_argument(
        "--directory", "-d", type=str, default="fixed_tests", help="Directory containing test files"
    )
    ...
    return (
        0 if syntax_valid == total and structure_valid == total and pipeline_valid == total else 1
    )
