        ...
        if is_valid:
            ...
            if fix:
                ...
                if is_valid:
                    ...
                    logger.error(
                        f"❌ {relative_path}: Failed to fix - {error_message} (line {line_number}, offset {offset})"
                    )
            else:
                failed_files.append((str(relative_path), error_message, line_number, offset))
                logger.error(
                    f"❌ {relative_path}: Syntax error - {error_message} (line {line_number}, offset {offset})"
                )
...
def main():
    ...
    parser.add_argument(
        "--directory", type=str, default="fixed_tests", help="Directory containing Python files"
    )
    ...
    # Verify files
    passed_files, failed_files = verify_files_in_directory(
        args.directory, fix=args.fix, verbose=args.verbose
    )
