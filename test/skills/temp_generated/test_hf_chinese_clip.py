...
# Models registry
CHINESE_CLIP_MODELS_REGISTRY = {
    "chinese_clip-base": {
        "description": "chinese_clip base model",
        "class": "Chinese_clipModel",
    }
}


class TestChineseClipModels:
    """Test class for chinese_clip models."""

    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "chinese_clip-base"

        # Use registry information
        if self.model_id not in CHINESE_CLIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = CHINESE_CLIP_MODELS_REGISTRY["chinese_clip-base"]
        else:
            self.model_info = CHINESE_CLIP_MODELS_REGISTRY[self.model_id]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            if HAS_TORCH:
                ...
                self.test_input = {"text": self.test_text, "image": torch.randn(1, 3, 224, 224)}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test chinese_clip models")
    ...
    # Run test
    model_id = args.model or "chinese_clip-base"
    tester = TestChineseClipModels(model_id)
