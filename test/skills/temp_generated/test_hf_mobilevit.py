...
# Models registry
MOBILEVIT_MODELS_REGISTRY = {
    "google/mobilevit-base-patch16-224": {
        "description": "mobilevit base model",
        "class": "MobilevitForImageClassification",
    }
}


class TestMobilevitModels:
    """Test class for mobilevit models."""

    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "google/mobilevit-base-patch16-224"

        # Use registry information
        if self.model_id not in MOBILEVIT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = MOBILEVIT_MODELS_REGISTRY["google/mobilevit-base-patch16-224"]
        else:
            self.model_info = MOBILEVIT_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test mobilevit models")
    ...
    # Run test
    model_id = args.model or "google/mobilevit-base-patch16-224"
    tester = TestMobilevitModels(model_id)
