...
# Models registry
PERCEIVER_MODELS_REGISTRY = {
    "google/perceiver-base-patch16-224": {
        "description": "perceiver base model",
        "class": "PerceiverForImageClassification",
    }
}


class TestPerceiverModels:
    """Test class for perceiver models."""

    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "google/perceiver-base-patch16-224"

        # Use registry information
        if self.model_id not in PERCEIVER_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = PERCEIVER_MODELS_REGISTRY["google/perceiver-base-patch16-224"]
        else:
            self.model_info = PERCEIVER_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test perceiver models")
    ...
    # Run test
    model_id = args.model or "google/perceiver-base-patch16-224"
    tester = TestPerceiverModels(model_id)
