...
class TestPhiModels:
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in PHI_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = PHI_MODELS_REGISTRY["microsoft/phi-2"]
        else:
            self.model_info = PHI_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test phi models")
    ...
    # Run test
    model_id = args.model or "microsoft/phi-2"
    tester = TestPhiModels(model_id)
