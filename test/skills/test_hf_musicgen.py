#!/usr/bin/env python3
"""
Class-based test file for all MusicGen-family models.
...
class TestMusicgenModels:
    """Base test class for all MusicGen-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "facebook/musicgen-small"
        
        # Verify model exists in registry
        if self.model_id not in MUSICGEN_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MUSICGEN_MODELS_REGISTRY["facebook/musicgen-small"]
        else:
            self.model_info = MUSICGEN_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Electronic dance music with a strong beat and synth melody"
        self.test_texts = [
            "Electronic dance music with a strong beat and synth melody",
            "Electronic dance music with a strong beat and synth melody (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "MusicgenForConditionalGeneration":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_musicgen_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MusicGen models in the registry."""
    return list(MUSICGEN_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MusicGen models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestMusicgenModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_musicgen_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MusicGen-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable MusicGen-family models:")
        for model in models:
            info = MUSICGEN_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nMusicGen Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "facebook/musicgen-small"
    logger.info(f"Testing model: {model_id}")
