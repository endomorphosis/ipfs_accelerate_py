...
try:
    from find_models import (
        get_recommended_default_model,
        query_huggingface_api as find_models_query,
    )
...
def query_huggingface_api(model_type, limit=10, task=None, size_mb=None, framework=None):
    ...
    try:
        logger.info(
            f"Querying HuggingFace API for {model_type} models (task: {task}, size: {size_mb}MB)"
        )
            ...
            try:
                ...
                for model in models:
                    ...
                    if size_mb and model_size and model_size > size_mb:
                        logger.info(
                            f"Skipping {model['id']} (size: ~{model_size}MB, limit: {size_mb}MB)"
                        )
                        continue
                ...
                logger.warning(
                    f"Error using find_models query: {e}, falling back to direct API query"
                )
...
def model_matches_framework(model_info, framework):
    ...
    if framework == "pytorch" and not any(
        indicator in model_id.lower() for indicator in ["tensorflow", "tf", "jax", "flax", "onnx"]
    ):
        return True
...
def get_models_for_task(task, size_mb=None, framework=None):
    ...
    for model_type in model_types:
        ...
        models = query_huggingface_api(
            model_type, limit=5, task=task, size_mb=size_mb, framework=framework
        )
...
def select_model_advanced(
    ...
    logger.info(
        f"Advanced model selection for {model_type} (task: {task}, hardware: {hardware_profile})"
    )
    ...
    if task:
        ...
        if task in TASK_TO_MODEL_TYPES:
            ...
            if model_type in TASK_TO_MODEL_TYPES[task]:
                ...
                models = query_huggingface_api(
                    model_type, limit=10, task=task, size_mb=size_limit, framework=framework
                )
                ...
                if models:
                    ...
                    if models_with_benchmark:
                        logger.info(
                            f"Found {len(models_with_benchmark)} models with benchmark data"
                        )
    ...
    if model_type in registry_data:
        ...
        if "models" in reg_entry:
            ...
            if size_filtered_models:
                # Sort by downloads
                size_filtered_models.sort(key=lambda m: downloads.get(m, 0), reverse=True)

                selected_model = size_filtered_models[0]
                logger.info(
                    f"Selected model from registry by size and popularity: {selected_model}"
                )
                return selected_model
...
def main():
    ...
    parser.add_argument(
        "--hardware",
        type=str,
        choices=HARDWARE_PROFILES.keys(),
    ...
    parser.add_argument(
        "--framework",
        type=str,
        choices=list(FRAMEWORK_COMPATIBILITY.keys()),
        help="Framework compatibility",
    )
    parser.add_argument("--list-tasks", action="store_true", help="List available tasks")
    parser.add_argument(
        "--list-hardware", action="store_true", help="List available hardware profiles"
    )
    ...
    if args.list_tasks:
        ...
        for task, model_types in sorted(TASK_TO_MODEL_TYPES.items()):
            print(
                f"  - {task}: {', '.join(model_types[:3])}{'...' if len(model_types) > 3 else ''}"
            )
        return 0
    ...
    if args.task:
        ...
        task_models = get_models_for_task(
            args.task,
            size_mb=args.max_size
            or (HARDWARE_PROFILES[args.hardware]["max_size_mb"] if args.hardware else None),
            framework=args.framework,
        )
