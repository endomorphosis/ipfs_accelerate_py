...
def fix_test_file(file_path):
    ...
    try:
        cmd = ["python", "fix_template_imports.py", "--file", file_path]
...
def main():
    ...
    parser.add_argument(
        "--hyphenated-only", action="store_true", help="Process only hyphenated model names"
    )
    ...
    logger.info(
        f"Process summary: {len(successful)} models successful, {len(failed)} models failed"
    )
