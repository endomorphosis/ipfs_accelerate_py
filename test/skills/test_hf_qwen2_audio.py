...
"""
Class-based test file for all Qwen2-Audio-family models.
...
class TestQwen2AudioModels:
    """Base test class for all Qwen2-Audio-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "Qwen/Qwen2-Audio-7B"
        
        # Verify model exists in registry
        if self.model_id not in QWEN2-AUDIO_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = QWEN2-AUDIO_MODELS_REGISTRY["Qwen/Qwen2-Audio-7B"]
        else:
            self.model_info = QWEN2-AUDIO_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = "audio-to-text"
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "Qwen2AudioForCausalLM":
            model_class = transformers.Qwen2AudioForCausalLM
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_qwen2_audio_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Qwen2-Audio models in the registry."""
    return list(QWEN2-AUDIO_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Qwen2-Audio models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestQwen2AudioModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_qwen2_audio_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Qwen2-Audio-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Qwen2-Audio-family models:")
        for model in models:
            info = QWEN2-AUDIO_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nQwen2-Audio Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "Qwen/Qwen2-Audio-7B"
    logger.info(f"Testing model: {model_id}")
