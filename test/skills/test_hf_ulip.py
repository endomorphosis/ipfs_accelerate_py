...
"""
Class-based test file for all ULIP-family models.
...
class TestUlipModels:
    """Base test class for all ULIP-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "salesforce/ulip-pointbert-base"
        
        # Verify model exists in registry
        if self.model_id not in ULIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = ULIP_MODELS_REGISTRY["salesforce/ulip-pointbert-base"]
        else:
            self.model_info = ULIP_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = "point-cloud-classification"
        self.class_name = self.model_info["class"]
        self.description = self.model_info["description"]
        
        # Define test inputs
        self.test_text = "a 3D model of a chair"
        self.test_texts = [
            "a 3D model of a chair",
            "a 3D model of a chair (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "UlipModel":
            model_class = transformers.UlipModel
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_ulip_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available ULIP models in the registry."""
    return list(ULIP_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered ULIP models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestUlipModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_ulip_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ULIP-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable ULIP-family models:")
        for model in models:
            info = ULIP_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "salesforce/ulip-pointbert-base"
    logger.info(f"Testing model: {model_id}")
