...
def extract_mock_detection_code(template_path):
    ...
    metadata_pattern = (
        r"# Determine if real inference or mock objects were used.*?return \{.*?\"test_type\":.*?\}"
    )
...
def update_template_file(file_path, metadata_code, summary_code):
    ...
    if not metadata_matched:
        ...
        if (
            "return {" in content
            and '"hardware": HW_CAPABILITIES,' in content
    ...
    if not summary_matched:
        ...
        if (
            'print("\\nTEST RESULTS SUMMARY:")' in content
...
def main():
    ...
    template_files = [
        f for f in TEMPLATES_DIR.glob("*_template.py") if f.name != "encoder_only_template.py"
    ]
