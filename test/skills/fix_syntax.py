...
def fix_unterminated_strings(file_path):
    ...
    try:
        ...
        content = re.sub(
            r"from generators\.hardware\.hardware_detection import \(+\)+",
            "from scripts.generators.hardware.hardware_detection import (",
            content,
        )
        content = re.sub(
            r"HAS_CUDA, HAS_ROCM, HAS_OPENVINO, HAS_MPS, HAS_WEBNN, HAS_WEBGPU,",
        ...
        if 1 <= line_number <= len(lines):
            ...
            if '""""' in line:
                ...
                if stripped.startswith(('"""', "'''")):
                    ...
                    logger.info(
                        f"Line doesn't seem to have quote issues, replacing with empty string"
                    )
                    lines[line_number - 1] = '        ""\n'
        else:
