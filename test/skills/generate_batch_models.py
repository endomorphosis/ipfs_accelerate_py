...
def generate_model_test(
    model_name, architecture, template=None, task=None, original_name=None, output_dir=None
):
...
def update_roadmap_mark_implemented(model_name, section_name):
    ...
    try:
        ...
        if model_entry_format1 in roadmap_content:
            updated_content = roadmap_content.replace(model_entry_format1, f"- [x] {model_name}")
        ...
        # Try with the second format (hyphenated)
            ...
            updated_content = roadmap_content.replace(
                model_entry_format2, f"- [x] {model_name.replace('_', '-')}"
            )
            # Write back the updated content
            with open(ROADMAP_PATH, "w") as f:
                f.write(updated_content)
...
def generate_batch_report(batch_name, successful_models, failed_models):
    ...
    try:
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Successfully Implemented:** {len(successful_models)} ({success_rate:.1f}%)\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--batch-file", required=True, help="JSON file containing batch model definitions"
    )
