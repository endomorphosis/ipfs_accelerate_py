#!/usr/bin/env python3
"""
Class-based test file for all ERNIE-family models.
...
class TestErnieModels:
    """Base test class for all ERNIE-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "ernie-base"
        
        # Verify model exists in registry
        if self.model_id not in ernie_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = ernie_MODELS_REGISTRY["ernie-base"]
        else:
            self.model_info = ernie_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_ernie_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available ERNIE models in the registry."""
    return list(ernie_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered ERNIE models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestErnieModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_ernie_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ERNIE-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable ERNIE-family models:")
        for model in models:
            info = ernie_MODELS_REGISTRY[model]
