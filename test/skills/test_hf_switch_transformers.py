...
"""
Class-based test file for all SWITCH_TRANSFORMERS-family models.
...
switch_transformers_MODELS_REGISTRY = {
    "switch-transformers-base": {
        "description": "SWITCH_TRANSFORMERS models",
        "class": "SwitchTransformersModel"
    }

}

class TestSwitch_TransformersModels:
    """Base test class for all SWITCH_TRANSFORMERS-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "switch-transformers-base"
        
        # Verify model exists in registry
        if self.model_id not in switch_transformers_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = switch_transformers_MODELS_REGISTRY["switch-transformers-base"]
        else:
            self.model_info = switch_transformers_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_switch_transformers_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available SWITCH_TRANSFORMERS models in the registry."""
    return list(switch_transformers_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered SWITCH_TRANSFORMERS models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestSwitch_TransformersModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_switch_transformers_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SWITCH_TRANSFORMERS-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable SWITCH_TRANSFORMERS-family models:")
        for model in models:
            info = switch_transformers_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nSWITCH_TRANSFORMERS Models Testing Summary:")
        total = len(results)
