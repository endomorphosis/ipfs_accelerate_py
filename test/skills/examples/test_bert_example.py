...
def save_results(model_id, results, output_dir="results"):
    ...
    with open(output_file, "w") as f:
        ...
        if "pipeline" in results:
            ...
            f.write(
                f"Pipeline test: {'Success' if pipeline.get('pipeline_success') else 'Failed'}\n"
            )
        ...
        if "from_pretrained" in results:
            ...
            f.write(
                f"Tokenizer loading: {'Success' if pretrained.get('tokenizer_loaded') else 'Failed'}\n"
            )
            f.write(
                f"Inference: {'Success' if pretrained.get('inference_success') else 'Failed'}\n"
            )
