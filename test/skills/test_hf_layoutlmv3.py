#!/usr/bin/env python3
"""
Class-based test file for all LayoutLMv3-family models.
...
class TestLayoutLMv3Models:
    """Base test class for all LayoutLMv3-family models."""
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in LAYOUTLMV3_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = LAYOUTLMV3_MODELS_REGISTRY["microsoft/layoutlmv3-base"]
        else:
            self.model_info = LAYOUTLMV3_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = "document-question-answering"
        self.class_name = self.model_info["class"]
        self.description = self.model_info["description"]
        
        # Define test inputs
        self.test_image_url = "https://huggingface.co/datasets/hf-internal-testing/fixtures_docvqa/resolve/main/document.png"
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "LayoutLMv3ForTokenClassification":
            model_class = transformers.LayoutLMv3ForTokenClassification
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_layoutlmv3_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available LayoutLMv3 models in the registry."""
    return list(LAYOUTLMV3_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered LayoutLMv3 models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestLayoutLMv3Models(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_layoutlmv3_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test LayoutLMv3-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable LayoutLMv3-family models:")
        for model in models:
            info = LAYOUTLMV3_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nLayoutLMv3 Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "microsoft/layoutlmv3-base"
    logger.info(f"Testing model: {model_id}")
