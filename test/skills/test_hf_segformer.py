...
"""
Class-based test file for all SegFormer-family models.
...
class TestSegformerModels:
    """Base test class for all SegFormer-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "nvidia/segformer-b0-finetuned-ade-512-512"
        
        # Verify model exists in registry
        if self.model_id not in SEGFORMER_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = SEGFORMER_MODELS_REGISTRY["nvidia/segformer-b0-finetuned-ade-512-512"]
        else:
            self.model_info = SEGFORMER_MODELS_REGISTRY[self.model_id]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "SegformerForSemanticSegmentation":
            model_class = transformers.SegformerForSemanticSegmentation
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_segformer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available SegFormer models in the registry."""
    return list(SEGFORMER_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered SegFormer models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestSegformerModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_segformer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SegFormer-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable SegFormer-family models:")
        for model in models:
            info = SEGFORMER_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nSegFormer Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "nvidia/segformer-b0-finetuned-ade-512-512"
    logger.info(f"Testing model: {model_id}")
