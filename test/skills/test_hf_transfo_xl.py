...
TRANSFO_XL_MODELS_REGISTRY = {
    "transfo-xl": {
        "description": "TRANSFO-XL small model",
        "class": "TransfoXLLMHeadModel",
    },
    "transfo-xl-medium": {
        "description": "TRANSFO-XL medium model",
        "class": "TransfoXLLMHeadModel",
    },
    "distiltransfo-xl": {
        "description": "DistilTRANSFO-XL model",
        "class": "TransfoXLLMHeadModel",
    }
}
...
class TestTransfoXlModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "transfo-xl"
        
        # Verify model exists in registry
        if self.model_id not in TRANSFO_XL_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = TRANSFO_XL_MODELS_REGISTRY["transfo-xl"]
        else:
            self.model_info = TRANSFO_XL_MODELS_REGISTRY[self.model_id]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            if tokenizer.pad_token is None:
                ...
                logger.info("Set pad_token to eos_token for TRANSFO-XL tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for TRANSFO-XL padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for TRANSFO-XL tokenizer")
            ...
            model_class = None
            if self.class_name == "TransfoXLLMHeadModel":
                model_class = transformers.TransfoXLLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_transfo_xl_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(TRANSFO_XL_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_transfo_xl_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = TRANSFO_XL_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "transfo-xl"
    logger.info(f"Testing model: {model_id}")
