...
"""
        ...
        # Insert the code
        updated_content = (
            content[:import_section_end] + lookup_integration_code + content[import_section_end:]
        )
            ...
            try:
                ...
                if hasattr(test_generator, "generate_test_file"):
                    ...
                    if result.returncode == 0:
                        ...
                        logger.error(
                            f"❌ Failed to generate test for {model_type}: {result.stderr}"
                        )
...
def main():
    ...
    parser.add_argument(
        "--registry", action="store_true", help="Update MODEL_REGISTRY with registry values"
    )
