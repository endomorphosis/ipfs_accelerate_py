...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"high_priority_generation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def generate_batch(missing_models, max_models=None, num_workers=4, output_dir=OUTPUT_DIR):
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_model = {
            executor.submit(generate_test_for_model, model, output_dir): model for model in batch
        }
        ...
        for line in lines:
            if (
                ...
                updated_line = line.replace(
                    f"(🔄 IN PROGRESS - 40%)", f"(🔄 IN PROGRESS - {percentage}%)"
                )
...
def main():
    ...
    parser.add_argument(
        "--list-missing",
        action="store_true",
        help="List missing high-priority models without generating tests",
    )
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel workers")
    parser.add_argument(
        "--specific-model", type=str, help="Generate test for a specific high-priority model"
    )
    parser.add_argument(
    ...
    if args.list_missing:
        ...
        for i, model in enumerate(missing_models, 1):
            ...
            logger.info(
                f"{i}. {model} (architecture: {config['architecture']}, template: {config['template']})"
            )
        return
