#!/usr/bin/env python3
"""
Class-based test file for all SigLIP-family models.
...
class TestSiglipModels:
    """Base test class for all SigLIP-family models."""
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in SIGLIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = SIGLIP_MODELS_REGISTRY["google/siglip-base-patch16-224"]
        else:
            self.model_info = SIGLIP_MODELS_REGISTRY[self.model_id]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "SiglipModel":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_siglip_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available SigLIP models in the registry."""
    return list(SIGLIP_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered SigLIP models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestSiglipModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_siglip_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SigLIP-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable SigLIP-family models:")
        for model in models:
            info = SIGLIP_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "google/siglip-base-patch16-224"
    logger.info(f"Testing model: {model_id}")
