#!/usr/bin/env python3
"""
Class-based test file for all GraphSAGE-family models.
This file provides a unified testing interface for:
- GraphSageForNodeClassification
"""
...
class TestGraphSageModels:
    """Base test class for all GraphSAGE-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "deepgnn/graphsage-base"
        
        # Verify model exists in registry
        if self.model_id not in GRAPHSAGE_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = GRAPHSAGE_MODELS_REGISTRY["deepgnn/graphsage-base"]
        else:
            self.model_info = GRAPHSAGE_MODELS_REGISTRY[self.model_id]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "GraphSageForNodeClassification":
            model_class = transformers.GraphSageForNodeClassification
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_graphsage_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available GraphSAGE models in the registry."""
    return list(GRAPHSAGE_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered GraphSAGE models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestGraphSageModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_graphsage_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test GraphSAGE-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable GraphSAGE-family models:")
        for model in models:
            info = GRAPHSAGE_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nGraphSAGE Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "deepgnn/graphsage-base"
    logger.info(f"Testing model: {model_id}")
