#!/usr/bin/env python3
"""
Class-based test file for all PEGASUS_X-family models.
...
pegasus_x_MODELS_REGISTRY = {
    "pegasus-x-base": {
        "description": "PEGASUS_X models",
        "class": "PegasusXModel"
    }

}

class TestPegasus_XModels:
    """Base test class for all PEGASUS_X-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "pegasus-x-base"
        
        # Verify model exists in registry
        if self.model_id not in pegasus_x_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = pegasus_x_MODELS_REGISTRY["pegasus-x-base"]
        else:
            self.model_info = pegasus_x_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_pegasus_x_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available PEGASUS_X models in the registry."""
    return list(pegasus_x_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered PEGASUS_X models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestPegasus_XModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_pegasus_x_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test PEGASUS_X-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable PEGASUS_X-family models:")
        for model in models:
            info = pegasus_x_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nPEGASUS_X Models Testing Summary:")
        total = len(results)
