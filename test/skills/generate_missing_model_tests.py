...
try:
    from regenerate_fixed_tests import (
        regenerate_test_file,
        get_architecture_type,
        get_template_for_architecture,
    )
except ImportError:
    # Try alternative modules that might contain these functions
    try:
        from regenerate_tests_with_fixes import (
            regenerate_test_file,
            get_architecture_type,
            get_template_for_architecture,
        )
        ...
        logger.error(
            "Failed to import required functions. Make sure regenerate_fixed_tests.py or regenerate_tests_with_fixes.py exists in the same directory."
        )
        sys.exit(1)
...
def extract_models_from_roadmap():
    ...
    try:
        ...
        high_priority_section = re.search(
            r"## Phase 2: High-Priority Models.*?## Phase 3:", content, re.DOTALL
        )
        ...
        medium_priority_section = re.search(
            r"## Phase 3: Architecture Expansion.*?## (?!Phase 3)", content, re.DOTALL
        )
        ...
        logger.info(
            f"Extracted priorities from roadmap: {len(PRIORITY_MODELS['high'])} high, "
    ...
    for model_type in models_to_generate:
        ...
        try:
            success, output_path = regenerate_test_file(
                model_type, output_dir=output_dir, verify=verify
            )
...
"""
        ...
        new_content = re.sub(
            r"## Testing Progress.*?testing is underway.*?\)\.\s*\n",
            testing_progress,
            content,
            flags=re.DOTALL,
        )
...
"""
        ...
        if "## Recent Updates" in new_content:
            ...
            if available_resources != -1:
                new_content = (
                    new_content[:available_resources]
                    + recent_updates
                    + new_content[available_resources:]
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--no-update-readme", action="store_true", help="Skip updating the README with progress"
    )
    parser.add_argument(
        "--direct-regenerate",
        action="store_true",
    ...
    if args.direct_regenerate:
        ...
        models_to_generate = mapped_models
        logger.info(
            f"Will generate {len(models_to_generate)} {args.priority}-priority models directly"
        )
    ...
    successes, failures, total = generate_missing_models(
        priority=args.priority, output_dir=args.output_dir, verify=args.verify
    )
