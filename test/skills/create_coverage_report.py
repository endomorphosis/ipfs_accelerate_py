...
def check_file_validity(file_path):
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "py_compile", file_path], check=True, capture_output=True
        )
        return True
...
def run_test(test_file):
    ...
    try:
        result = subprocess.run(
            [sys.executable, test_file, "--list-models"], check=True, capture_output=True, text=True
        )
...
def create_report(test_dir="fixed_tests", output_file=None):
    ...
    for arch_type in by_architecture:
        by_architecture[arch_type] = sorted(
            by_architecture[arch_type], key=lambda x: x["model_type"]
        )
    ...
    valid_tests = sum(
        sum(1 for test in tests if test["valid"]) for tests in by_architecture.values()
    )
    runnable_tests = sum(
        sum(1 for test in tests if test["runnable"]) for tests in by_architecture.values()
    )

    report.append(f"- Total test files: {total_tests}")
    report.append(
        f"- Syntactically valid files: {valid_tests} ({valid_tests / total_tests * 100:.1f}%)"
    )
    ...
    tested_models = len(
        {test["model_type"] for tests in by_architecture.values() for test in tests}
    )
    report.append(f"- Known model types: {potential_models}")
    report.append(
        f"- Tested model types: {tested_models} ({tested_models / potential_models * 100:.1f}%)"
    )
    report.append("")
    ...
    hyphenated_models = [
        test["model_type"]
        for tests in by_architecture.values()
        for test in tests
        if "-" in test["model_type"]
    ]
    ...
    for test in sorted(all_tests, key=lambda x: x["file"]):
        ...
        report.append(
            f"| {test['file']} | {test['model_type']} | {test['architecture']} | {valid_mark} | {runnable_mark} |"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create coverage report for HuggingFace model tests"
    )
    parser.add_argument(
