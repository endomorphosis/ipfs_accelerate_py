...
"""
Class-based test file for all REFORMER-family models.
...
reformer_MODELS_REGISTRY = {
    "reformer-base": {
        "description": "REFORMER models",
        "class": "ReformerModel"
    }

}

class TestReformerModels:
    """Base test class for all REFORMER-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "reformer-base"
        
        # Verify model exists in registry
        if self.model_id not in reformer_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = reformer_MODELS_REGISTRY["reformer-base"]
        else:
            self.model_info = reformer_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_reformer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available REFORMER models in the registry."""
    return list(reformer_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered REFORMER models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestReformerModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_reformer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test REFORMER-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable REFORMER-family models:")
        for model in models:
            info = reformer_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nREFORMER Models Testing Summary:")
        total = len(results)
