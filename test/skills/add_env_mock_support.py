...
def check_env_mock_support(content):
    ...
    has_mock_transformers = (
        "MOCK_TRANSFORMERS = os.environ.get('MOCK_TRANSFORMERS', 'False').lower() == 'true'"
        in content
    )
    has_mock_torch = (
        "MOCK_TORCH = os.environ.get('MOCK_TORCH', 'False').lower() == 'true'" in content
    )
...
"""
            content = (
                content[:torch_import_index] + env_vars_definition + content[torch_import_index:]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Add environment variable-based mocking support to template files"
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only check for env mock support without modifying",
    )
