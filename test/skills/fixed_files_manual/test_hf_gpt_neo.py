...
class TestGptNeoModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "gpt_neo"

        # Verify model exists in registry
        if self.model_id not in GPT_NEO_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = GPT_NEO_MODELS_REGISTRY["gpt_neo"]
        else:
            self.model_info = GPT_NEO_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "GptNeoLMHeadModel":
                model_class = transformers.GptNeoLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_gpt_neo_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_gpt_neo_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    model_id = args.model or "gpt_neo"
    logger.info(f"Testing model: {model_id}")
