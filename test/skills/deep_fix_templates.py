...
def fix_specific_print_errors(content):
    ...
    content = re.sub(
        r'print\(f"\s*Available [^"]*:")+")', r'print(f"Available MODEL-family models:")', content
    )

    # Fix the specific pattern in decoder_only_template.py
    content = re.sub(
        r'print\(f"\s*Available GPT-2-family models:")+")',
        r'print(f"Available MODEL-family models:")',
        content,
    )

    # Fix the pattern in speech_template.py
    content = re.sub(
        r'print\(f"\s*Available SPEECH-family models:"\nBERT Models Testing Summary:',
    ...
    content = re.sub(
        r'print\(f"\s*Available MULTIMODAL-family models:")+")',
        r'print(f"Available MULTIMODAL-family models:")',
        content,
    )
    ...
    try:
        ...
        if "decoder_only" in template_name:
            ...
            content = content.replace(
                ...
                'print("Available GPT2-family models:")',
            )
            content = content.replace(
                'print(f"\nAvailable GPT-2-family models:")',
                'print("Available GPT-2-family models:")',
            )
            ...
            content = content.replace(
                'print(f"\nAvailable VISION TEXT-family models:")',
                'print("Available VISION-TEXT-family models:")',
            )
            ...
            content = content.replace(
                ...
                'print("Available SPEECH-family models:")',
            )
            ...
            content = content.replace(
                ...
                'print("Available MULTIMODAL-family models:")',
            )
                ...
                if line_no < len(lines):
                    if "Available" in lines[line_no]:
                        ...
                        if model_type:
                            ...
                            lines[line_no] = (
                                f'        print("Available {model_name}-family models:")'
                            )
                        else:
...
def main():
    ...
    parser.add_argument(
        "--template", type=str, help="Specific template file to fix (relative to templates/)"
    )
    parser.add_argument("--all", action="store_true", help="Fix all template files")
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory for fixed templates"
    )
