...
"""
Class-based test file for all MLP-Mixer models.
This file provides a unified testing interface for:
    - MlpMixerForImageClassification
    - Other mixer model variants
"""
...
MLP_MIXER_MODELS_REGISTRY = {
    "google/mlp-mixer-base-patch16-224": {
        "description": "MLP-Mixer Base model (patch size 16, image size 224)",
        "class": "MlpMixerForImageClassification",
    },
    "google/mlp-mixer-large-patch16-224": {
        "description": "MLP-Mixer Large model (patch size 16, image size 224)",
        "class": "MlpMixerForImageClassification",
    },
}

class TestMlpMixerModels:
    """Base test class for all MLP-Mixer models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "google/mlp-mixer-base-patch16-224"
        
        # Verify model exists in registry
        if self.model_id not in MLP_MIXER_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MLP_MIXER_MODELS_REGISTRY["google/mlp-mixer-base-patch16-224"]
        else:
            self.model_info = MLP_MIXER_MODELS_REGISTRY[self.model_id]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            logger.info("Created mock image for MLP-Mixer pipeline testing - Avoiding URL download")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "MlpMixerForImageClassification":
                model_class = transformers.MlpMixerForImageClassification
            else:
            ...
            # Prepare test input - MLP-Mixer requires proper image input
            ...
            # Default MLP-Mixer input size is 224x224 pixels with 3 color channels
            batch_size = 1
            num_channels = 3  # RGB
            height = 224
            width = 224
            ...
            logger.info("Created proper image input tensor for MLP-Mixer model")
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_mlp_mixer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MLP-Mixer models in the registry."""
    return list(MLP_MIXER_MODELS_REGISTRY.keys())


def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MLP-Mixer models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestMlpMixerModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_mlp_mixer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MLP-Mixer models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable MLP-Mixer models:")
        for model in models:
            info = MLP_MIXER_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print(f"\nMLP-Mixer Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "google/mlp-mixer-base-patch16-224"
    logger.info(f"Testing model: {model_id}")
