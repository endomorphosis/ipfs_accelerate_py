...
MODEL_TYPE_MODELS_REGISTRY = {
    "model_type-base-uncased": {
        "full_name": "MODEL_TYPE Base Uncased",
        "architecture": "encoder-only",
        "description": "MODEL_TYPE Base model with uncased vocabulary",
    ...
    "model_type-large-uncased": {
        "full_name": "MODEL_TYPE Large Uncased",
        "architecture": "encoder-only",
        "description": "MODEL_TYPE Large model with uncased vocabulary",
        "model_type": "model_type",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "model_type-base-cased": {
        "full_name": "MODEL_TYPE Base Cased",
        "architecture": "encoder-only",
        "description": "MODEL_TYPE Base model with cased vocabulary",
        "model_type": "model_type",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="model_type-base-uncased", device=None):
        """Initialize the test class for MODEL_TYPE models.

        Args:
            model_id: The model ID to test (default: "model_type-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing MODEL_TYPE model {self.model_id} with pipeline API on {self.device}"
            )
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing MODEL_TYPE model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing MODEL_TYPE model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_model_type_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="model_type-base-uncased", help="Model ID to test"
    )
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="List available MODEL_TYPE models"
    )
    ...
    if args.list_models:
        ...
        for model_id, info in MODEL_TYPE_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested MODEL_TYPE model: {args.model}")
