    ...
    def test_xclip_video(self, device="auto"):
        ...
        try:
            ...
            if "cuda" in error_str or "cuda" in traceback_str:
                ...
                "xclipprocessor" in error_str.lower()
                or "xclipforvideoclassification" in error_str.lower()
            ):
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_{xclip}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Vision-Text models (BLIP, CLIP, XCLIP, etc.)"
    )
    ...
    if success:
        ...
        for test_name, result in results["results"].items():
            if (
                model_type == "xclip"
                and "xclip_video" in test_name
                and "xclip_video_error" in result
            ):
                print(
                    f"  - Error in {test_name}: {result.get('xclip_video_error_type', 'unknown')}"
                )
