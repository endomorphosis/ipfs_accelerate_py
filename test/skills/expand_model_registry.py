...
def verify_integration():
    ...
    try:
        ...
        has_model_lookup = hasattr(test_generator, "HAS_MODEL_LOOKUP") and getattr(
            test_generator, "HAS_MODEL_LOOKUP", False
        )
        ...
        if has_model_lookup and has_get_model:
            ...
            try:
                ...
                logger.info(
                    f"✅ Successfully retrieved default model for {model_type}: {default_model}"
                )
                return True
            except Exception as e:
            ...
            logger.error(
                f"HAS_MODEL_LOOKUP: {has_model_lookup}, has get_model_from_registry: {has_get_model}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify the integration with test_generator_fixed.py"
    )
