...
def update_model_tracking_file():
    ...
    model_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "model_tracking.json"
    )
            ...
            for priority in ["critical", "high", "medium"]:
                if hyphenated in tracking_data["priorities"][priority]:
                    ...
                    if underscore not in tracking_data["priorities"][priority]:
                        ...
                        logger.info(
                            f"Updated {priority} priority list: {hyphenated} -> {underscore}"
                        )
...
def update_report_generator():
    ...
    report_generator_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "generate_missing_model_report.py"
    )
