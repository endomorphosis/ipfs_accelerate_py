...
"""
Class-based test file for all StarCoder2-family models.
...
class TestStarcoder2Models:
    """Base test class for all StarCoder2-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "bigcode/starcoder2-3b"
        
        # Verify model exists in registry
        if self.model_id not in STARCODER2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = STARCODER2_MODELS_REGISTRY["bigcode/starcoder2-3b"]
        else:
            self.model_info = STARCODER2_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "def quicksort(arr):"
        self.test_texts = [
            "def quicksort(arr):",
            "def quicksort(arr): (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "StarCoder2ForCausalLM":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_starcoder2_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available StarCoder2 models in the registry."""
    return list(STARCODER2_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered StarCoder2 models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestStarcoder2Models(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_starcoder2_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test StarCoder2-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable StarCoder2-family models:")
        for model in models:
            info = STARCODER2_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nStarCoder2 Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "bigcode/starcoder2-3b"
    logger.info(f"Testing model: {model_id}")
