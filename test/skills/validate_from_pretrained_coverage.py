    ...
    for category, data in inventory["categories"].items():
        ...
        for cls in data["classes"]:
            ...
            if has_proper_test:
                ...
                if test_details.get("has_from_pretrained_method") or test_details.get(
                    "calls_from_pretrained"
                ):
                    # Partial test - has method or calls but doesn't fully test
                    results["categories"][category]["partial"].append(cls["name"])
                    results["partial_tests"].append(
                        (category, cls["name"], test_file, test_details)
                    )
                else:
                    # Missing test - no method or calls
                    results["categories"][category]["missing"].append(cls["name"])
                    results["missing_tests"].append(
                        (category, cls["name"], "Test file exists but no from_pretrained test")
                    )

    # Calculate percentages
    results["percent_tested"] = (
        round(results["total_tested"] / results["total_with_from_pretrained"] * 100, 2)
        if results["total_with_from_pretrained"] > 0
        else 0
    )
...
def create_coverage_report(
    results: Dict[str, Any], output_file: str = "from_pretrained_validation_report.md"
):
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"- **Classes with proper tests:** {results['total_tested']} ({results['percent_tested']}%)\n"
        )
        ...
        for category, data in sorted(results["categories"].items()):
            percent = (
                round(data["tested"] / data["with_from_pretrained"] * 100, 2)
                if data["with_from_pretrained"] > 0
                else 0
            )
            f.write(
                f"| {category} | {data['total']} | {data['with_from_pretrained']} | {data['tested']} | {percent}% |\n"
            )
        ...
        if results["missing_tests"]:
            ...
            f.write(
                f"The following model classes have a from_pretrained method but no proper test:\n\n"
            )

            f.write(f"| Category | Model Class | Issue |\n")
            f.write(f"|----------|-------------|-------|\n")

            for category, cls_name, issue in sorted(
                results["missing_tests"], key=lambda x: (x[0], x[1])
            ):
        ...
        if results["partial_tests"]:
            ...
            for category, cls_name, test_file, details in sorted(
                results["partial_tests"], key=lambda x: (x[0], x[1])
            ):
                issues = []
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="from_pretrained_validation_report.md",
