...
class TestBertModels:
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            # Time model loading
            model_load_start = time.time()
            model = OVModelForMaskedLM.from_pretrained(self.model_id, export=True, provider="CPU")
            model_load_time = time.time() - model_load_start
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_bert_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
