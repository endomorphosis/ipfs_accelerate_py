...
def get_model_details_from_docs(model_name):
    ...
    for file in doc_files:
        ...
        if (
            "bert" in file_lower
            or "roberta" in file_lower
            or "albert" in file_lower
            or "encoder-only" in file_lower
        ):
            architecture_type = "encoder-only"
        elif (
            "gpt" in file_lower
            or "llama" in file_lower
            or "bloom" in file_lower
            or "decoder-only" in file_lower
        ):
...
def manually_classify_remaining_models(uncategorized_models):
    ...
    manual_classifications = {
        "omdet_turbo": {
            "architecture_type": "vision",
            "description": "Omnivore Detection Turbo model - a vision model for object detection.",
        }
    }
...
def main():
    ...
    classified_models, still_uncategorized = manually_classify_remaining_models(
        uncategorized_models
    )
    ...
    if classified_models:
        ...
        for model in classified_models:
            print(
                f"- {model['model_name']}: {model['architecture_type']} (Source: {model['source']})"
            )
