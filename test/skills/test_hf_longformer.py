...
try:
    from ipfs_accelerate_py.worker.skillset.hf_longformer import hf_longformer
except ImportError:
...
def init_cuda(self, model_name, model_type, device_label="cuda:0", **kwargs):
    ...
    try:
        ...
        try:
            ...
            print(f"Attempting to load real Longformer model {model_name} with CUDA support")
            ...
            try:
                ...
                return model, tokenizer, real_handler, None, 4  # Lower batch size for Longformer due to memory
        ...
        print(f"Successfully loaded simulated Longformer model on {device}")
        return endpoint, tokenizer, simulated_handler, None, 4  # Lower batch size for Longformer
...
def init_openvino(self, model_name, model_type, device="CPU", openvino_label="openvino:0", **kwargs):
    ...
    print(f"Initializing Longformer model {model_name} with OpenVINO for {device}")
...
class test_hf_longformer:
    def __init__(self, resources=None, metadata=None):
        ...
        self.metadata = metadata if metadata else {}
        self.longformer = hf_longformer(resources=self.resources, metadata=self.metadata)
        
        # Use a smaller accessible Longformer model by default
        self.model_name = "allenai/longformer-base-4096"  # From mapped_models.json
                ...
                try:
                    ...
                    if self.model_name == self.alternative_models[0]:
                        ...
                        if os.path.exists(cache_dir):
                            ...
                            longformer_models = [name for name in os.listdir(cache_dir) if "longformer" in name.lower()]
                            if longformer_models:
                                # Use the first model found
                                longformer_model_name = longformer_models[0].replace("--", "/")
                                print(f"Found local Longformer model: {longformer_model_name}")
                                self.model_name = longformer_model_name
                            else:
    ...
    def _create_test_model(self):
        ...
        """
        try:
            print("Creating local test model for Longformer testing...")
            ...
            if hasattr(torch, "save") and not isinstance(torch, MagicMock):
                ...
                # Add a couple of attention layers with both local and global attention
                for i in range(2):  # Just 2 layers to keep it small
                    model_state[f"longformer.encoder.layer.{i}.attention.self.query.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.query.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.key.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.key.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.value.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.value.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.output.dense.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.output.dense.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.output.LayerNorm.weight"] = torch.ones(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.output.LayerNorm.bias"] = torch.zeros(768)
                    
                    # Longformer specific attention weights
                    model_state[f"longformer.encoder.layer.{i}.attention.self.query_global.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.query_global.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.key_global.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.key_global.bias"] = torch.zeros(768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.value_global.weight"] = torch.randn(768, 768)
                    model_state[f"longformer.encoder.layer.{i}.attention.self.value_global.bias"] = torch.zeros(768)
    ...
    def test(self):
        ...
        try:
            results["init"] = "Success" if self.longformer is not None else "Failed initialization"
        except Exception as e:
            results["init"] = f"Error: {str(e)}"

        # ====== CPU TESTS ======
        try:
            print("Testing Longformer on CPU...")
            # Initialize for CPU without mocks
            endpoint, tokenizer, handler, queue, batch_size = self.longformer.init_cpu(
                self.model_name,
                "cpu", 
                "cpu"
            )
        ...
        if torch.cuda.is_available():
            try:
                print("Testing Longformer on CUDA...")
                # Initialize for CUDA without mocks
                endpoint, tokenizer, handler, queue, batch_size = self.longformer.init_cuda(
                    self.model_name,
                    "cuda",
                    "cuda:0"
                )
        ...
        try:
            ...
            if has_openvino:
                ...
                try:
                    ...
                    endpoint, tokenizer, handler, queue, batch_size = self.longformer.init_openvino(
...
if __name__ == "__main__":
    try:
        ...
        print("\nLONGFORMER TEST RESULTS SUMMARY")
