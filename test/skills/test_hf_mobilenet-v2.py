...
MOBILENET-V2_MODELS_REGISTRY = {
    "mobilenet-v2-base-uncased": {
        "full_name": "MOBILENET-V2 Base Uncased",
        "architecture": "encoder-only",
        "description": "MOBILENET-V2 Base model with uncased vocabulary",
    ...
    "mobilenet-v2-large-uncased": {
        "full_name": "MOBILENET-V2 Large Uncased",
        "architecture": "encoder-only",
        "description": "MOBILENET-V2 Large model with uncased vocabulary",
        "mobilenet-v2": "mobilenet-v2",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "mobilenet-v2-base-cased": {
        "full_name": "MOBILENET-V2 Base Cased",
        "architecture": "encoder-only",
        "description": "MOBILENET-V2 Base model with cased vocabulary",
        "mobilenet-v2": "mobilenet-v2",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="mobilenet-v2-base-uncased", device=None):
        """Initialize the test class for MOBILENET-V2 models.
        
        Args:
            model_id: The model ID to test (default: "mobilenet-v2-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing MOBILENET-V2 model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing MOBILENET-V2 model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing MOBILENET-V2 model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_mobilenet-v2_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MOBILENET-V2 HuggingFace models")
    parser.add_argument("--model", type=str, default="mobilenet-v2-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available MOBILENET-V2 models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable MOBILENET-V2 models:")
        for model_id, info in MOBILENET-V2_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested MOBILENET-V2 model: {args.model}")
