...
KOSMOS-2_MODELS_REGISTRY = {
    "kosmos-2-base-uncased": {
        "full_name": "KOSMOS-2 Base Uncased",
        "architecture": "encoder-only",
        "description": "KOSMOS-2 Base model with uncased vocabulary",
        "kosmos-2": "kosmos-2",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "kosmos-2-large-uncased": {
        "full_name": "KOSMOS-2 Large Uncased",
        "architecture": "encoder-only",
        "description": "KOSMOS-2 Large model with uncased vocabulary",
        "kosmos-2": "kosmos-2",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "kosmos-2-base-cased": {
        "full_name": "KOSMOS-2 Base Cased",
        "architecture": "encoder-only",
        "description": "KOSMOS-2 Base model with cased vocabulary",
        "kosmos-2": "kosmos-2",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="kosmos-2-base-uncased", device=None):
        """Initialize the test class for KOSMOS-2 models.
        
        Args:
            model_id: The model ID to test (default: "kosmos-2-base-uncased")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing KOSMOS-2 model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing KOSMOS-2 model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_kosmos-2_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="kosmos-2-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available KOSMOS-2 models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable KOSMOS-2 models:")
        for model_id, info in KOSMOS-2_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested KOSMOS-2 model: {args.model}")
