...
try:
    ...
    logger.error(
        "Could not import fix_hyphenated_models.py. Make sure it exists in the same directory."
    )
    sys.exit(1)
...
def validate_test_file_content(model_name: str) -> Tuple[bool, List[str]]:
    ...
    if not re.search(registry_pattern, content):
        issues.append(
            f"Missing or incorrect model registry: expected {valid_id.upper()}_MODELS_REGISTRY"
        )
...
def validate_all_hyphenated_models() -> Dict[str, Dict]:
    ...
    for model_name in hyphenated_models:
        ...
        results[model_name] = {"exists": file_exists, "valid": valid, "issues": issues}
...
def main():
    ...
    parser.add_argument(
        "--fix-issues", action="store_true", help="Attempt to fix validation issues"
    )
    parser.add_argument(
        "--output-file",
        type=str,
        default="hyphenated_model_validation.md",
