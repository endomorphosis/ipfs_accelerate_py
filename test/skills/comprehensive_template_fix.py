...
def find_problematic_lines(file_path):
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "py_compile", file_path], capture_output=True, check=True
        )
...
def fix_template_file(template_name):
    ...
    for i in range(len(lines)):
        ...
        if (
            "print(" in line
            and ('print("' in line or 'print(f"' in line)
            and line.strip().endswith("print(")
        ):
            # This is likely an unterminated print statement
            if (
                i + 1 < len(lines)
                and not lines[i + 1].strip().startswith('"')
                and not lines[i + 1].strip().startswith('f"')
            ):
