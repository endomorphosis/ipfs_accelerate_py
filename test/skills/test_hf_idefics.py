...
class TestIdeficsModels:
    """Base test class for all IDEFICS-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "idefics-base"
        
        # Verify model exists in registry
        if self.model_id not in idefics_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = idefics_MODELS_REGISTRY["idefics-base"]
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_idefics_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(idefics_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestIdeficsModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_idefics_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test IDEFICS-family models")
    ...
    if args.list_models:
        ...
        for model in models:
            info = idefics_MODELS_REGISTRY[model]
