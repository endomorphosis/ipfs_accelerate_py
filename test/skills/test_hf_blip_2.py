#!/usr/bin/env python3
"""
Class-based test file for all BLIP-2-family models.
...
class TestBlip2Models:
    """Base test class for all BLIP-2-family models."""
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in BLIP-2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = BLIP-2_MODELS_REGISTRY["Salesforce/blip2-opt-2.7b"]
        else:
            self.model_info = BLIP-2_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Question: What is shown in the image? Answer:"
        self.test_texts = [
            "Question: What is shown in the image? Answer:",
            "Question: What is shown in the image? Answer: (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "Blip2ForConditionalGeneration":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_blip2_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available BLIP-2 models in the registry."""
    return list(BLIP-2_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered BLIP-2 models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestBlip2Models(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_blip2_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test BLIP-2-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable BLIP-2-family models:")
        for model in models:
            info = BLIP-2_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "Salesforce/blip2-opt-2.7b"
    logger.info(f"Testing model: {model_id}")
