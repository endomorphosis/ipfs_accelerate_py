...
class TestSeamlessM4TModels:
    """Base test class for all Seamless-M4T-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "facebook/seamless-m4t-large"
        
        # Verify model exists in registry
        if self.model_id not in SEAMLESS-M4T_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = SEAMLESS-M4T_MODELS_REGISTRY["facebook/seamless-m4t-large"]
        else:
            self.model_info = SEAMLESS-M4T_MODELS_REGISTRY[self.model_id]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "SeamlessM4TModel":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_seamless_m4t_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(SEAMLESS-M4T_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestSeamlessM4TModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_seamless_m4t_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Seamless-M4T-family models")
    ...
    if args.list_models:
        ...
        for model in models:
            info = SEAMLESS-M4T_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "facebook/seamless-m4t-large"
    logger.info(f"Testing model: {model_id}")
