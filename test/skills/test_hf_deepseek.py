#!/usr/bin/env python3
"""
Class-based test file for all DeepSeek-family models.
...
class TestDeepSeekModels:
    """Base test class for all DeepSeek-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "deepseek-ai/deepseek-llm-7b-base"
        
        # Verify model exists in registry
        if self.model_id not in DEEPSEEK_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = DEEPSEEK_MODELS_REGISTRY["deepseek-ai/deepseek-llm-7b-base"]
        else:
            self.model_info = DEEPSEEK_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "DeepSeek is a model that can"
        self.test_texts = [
            "DeepSeek is a model that can",
            "DeepSeek is a model that can (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "DeepSeekForCausalLM":
            model_class = transformers.DeepSeekForCausalLM
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_deepseek_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available DeepSeek models in the registry."""
    return list(DEEPSEEK_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered DeepSeek models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestDeepSeekModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_deepseek_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test DeepSeek-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable DeepSeek-family models:")
        for model in models:
            info = DEEPSEEK_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nDeepSeek Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "deepseek-ai/deepseek-llm-7b-base"
    logger.info(f"Testing model: {model_id}")
