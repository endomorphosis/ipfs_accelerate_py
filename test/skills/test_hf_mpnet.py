...
def init_cuda(self, model_name, model_type, device_label="cuda:0"):
    ...
    try:
            ...
            def handler(text, **kwargs):
                ...
                try:
                    ...
                    if isinstance(model, mock.MagicMock) or isinstance(tokenizer, mock.MagicMock):
                        print("Using mock handler for CUDA MPNet")
        ...
        print(f"Error in MPNet init_cuda: {e}")
        import traceback
        traceback.print_exc()
...
class test_hf_mpnet:
    def __init__(self, resources=None, metadata=None):
        ...
        self.primary_model = "sentence-transformers/all-mpnet-base-v2"  # Primary model for testing
                ...
                try:
                    ...
                    if self.model_name == self.primary_model:
                        ...
                        if os.path.exists(cache_dir):
                            # Look for any MPNet or sentence-transformer model in cache
                            mpnet_models = [name for name in os.listdir(cache_dir) if any(
                                x in name.lower() for x in ["mpnet", "sentence", "minilm", "bert"])]
                            
                            if mpnet_models:
                                # Use the first model found
                                mpnet_model_name = mpnet_models[0].replace("--", "/")
                                print(f"Found local MPNet-like model: {mpnet_model_name}")
                                self.model_name = mpnet_model_name
                            else:
        ...
        self.test_inputs = ["This is a test sentence for MPNet embeddings.", 
    ...
    def _create_test_model(self):
        ...
        """
        try:
            print("Creating local test model for MPNet testing...")
            ...
            if hasattr(torch, "save") and not isinstance(torch, MagicMock):
                ...
                for layer_idx in range(num_layers):
                    ...
                    model_state[f"{layer_prefix}.attention.self.query.bias"] = torch.zeros(hidden_size)
                    model_state[f"{layer_prefix}.attention.self.key.weight"] = torch.randn(hidden_size, hidden_size)
                    model_state[f"{layer_prefix}.attention.self.key.bias"] = torch.zeros(hidden_size)
                    model_state[f"{layer_prefix}.attention.self.value.weight"] = torch.randn(hidden_size, hidden_size)
                    model_state[f"{layer_prefix}.attention.self.value.bias"] = torch.zeros(hidden_size)
                    model_state[f"{layer_prefix}.attention.output.dense.weight"] = torch.randn(hidden_size, hidden_size)
                    model_state[f"{layer_prefix}.attention.output.dense.bias"] = torch.zeros(hidden_size)
    ...
    def test(self):
        ...
        # ====== OPENVINO TESTS ======
        try:
            print("Testing MPNet on OpenVINO...")
...
if __name__ == "__main__":
    try:
        print("Starting MPNet test...")
        mpnet_test = test_hf_mpnet()
        results = mpnet_test.__test__()
        print("MPNet test completed")
        ...
        print("MPNET TEST RESULTS SUMMARY")
