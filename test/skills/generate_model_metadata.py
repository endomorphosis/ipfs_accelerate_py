...
def generate_markdown_report(metadata: Dict) -> str:
    ...
    report += (
        "These models require special handling in Python code due to hyphens in their names.\n\n"
    )
    report += (
        "| Original Name | Valid Identifier | Class Name | Test File Name | Test Class Name |\n"
    )
    report += "|---------------|-----------------|------------|----------------|----------------|\n"
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="model_naming_conventions.md",
        help="Output file for the markdown report",
    )
    parser.add_argument(
        "--json", type=str, default="model_metadata.json", help="Output file for the JSON metadata"
    )
