...
MOSAIC_MPT_MODELS_REGISTRY = {
    ...
    "mosaicml/mpt-30b": {
        "description": "MPT-30B base model",
        "class": "AutoModelForCausalLM",
    },
...
class TestMosaicMptModels:
    """Base test class for all MosaicML MPT-family models."""

    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "mosaicml/mpt-7b-instruct"

        # Verify model exists in registry
        if self.model_id not in MOSAIC_MPT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MOSAIC_MPT_MODELS_REGISTRY["mosaicml/mpt-7b-instruct"]
        else:
        ...
        # Define test inputs
        self.test_text = "Write a short poem about AI"
        self.test_texts = [
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            # Check pad token setup for MPT models
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for MPT tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for MPT padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for MPT tokenizer")
    ...
    def test_batch_inference(self, device="auto"):
        ...
        try:
            ...
            encoded_inputs = tokenizer(
                self.test_texts, padding=True, truncation=True, return_tensors="pt"
            )
                ...
                for i in range(batch_size):
                    ...
                    if hasattr(tokenizer, "decode"):
                        ...
                        batch_results.append(
                            {"input": self.test_texts[i], "next_token": next_token}
                        )
                    else:
                        batch_results.append(
                            {"input": self.test_texts[i], "next_token": "<mock_token>"}
                        )
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_mosaic_mpt_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def get_available_models():
    ...
    return list(MOSAIC_MPT_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir,
        f"hf_mosaic_mpt_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MosaicML MPT models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable MosaicML MPT models:")
        for model in models:
            info = MOSAIC_MPT_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "mosaicml/mpt-7b-instruct"
    logger.info(f"Testing model: {model_id}")
