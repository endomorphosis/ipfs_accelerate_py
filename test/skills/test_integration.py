    ...
    try:
        ...
        summary = {
            ...
            "successful_models": sum(
                1 for model_results in results.values() if any(r["success"] for r in model_results)
            ),
            "total_tests": sum(len(model_results) for model_results in results.values()),
            "successful_tests": sum(
                sum(1 for r in model_results if r["success"]) for model_results in results.values()
            ),
            "models": {
                model: {"success": any(r["success"] for r in model_results)}
        ...
        if summary["total_models"] > 0:
            summary["coverage_percentage"] = (
                summary["successful_models"] / summary["total_models"]
            ) * 100
        else:
        ...
        report_path = os.path.join(
            ROOT_DIR, f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Successful Models**: {summary['successful_models']} ({summary['coverage_percentage']:.1f}%)\n"
            )
            f.write(f"- **Total Tests**: {summary['total_tests']}\n")
            f.write(
                f"- **Successful Tests**: {summary['successful_tests']} ({summary['successful_tests'] / summary['total_tests'] * 100 if summary['total_tests'] else 0:.1f}%)\n\n"
            )
...
def full_integration(models=None, verify=True, force=False, run_tests=False, report=True):
    ...
    # Track results
    results = {"generated": [], "fixed": [], "verified": [], "tested": [], "failed": []}
...
def main():
    ...
    model_group.add_argument(
        "--core", action="store_true", help="Use core models (bert, gpt2, t5, vit)"
    )
    model_group.add_argument(
        "--arch",
        type=str,
        choices=ARCHITECTURE_TYPES.keys(),
    ...
    if args.all:
        ...
        success = full_integration(
            models=models, verify=True, force=args.force, run_tests=True, report=True
        )
