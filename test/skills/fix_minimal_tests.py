...
def fix_file(file_path):
    ...
    content = content.replace(
        "self.performance_stats = {}    def", "self.performance_stats = {}\n\n    def"
    )
    content = content.replace(
        "self.examples = []    self.performance_stats",
    ...
    content = content.replace(
        "        tokenizers.Tokenizer = MockTokenizer", "    tokenizers.Tokenizer = MockTokenizer"
    )
    content = content.replace(
    ...
    content = content.replace(
        "        Image.open = MockImage.open", "    Image.open = MockImage.open"
    )
    content = content.replace(
        "        requests.get = MockRequests.get", "    requests.get = MockRequests.get"
    )
    ...
    content = content.replace(
        'logger.warning("tokenizers not available, using mock")        if not HAS_SENTENCEPIECE:',
