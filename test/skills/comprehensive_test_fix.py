...
def check_mock_detection(content):
    ...
    results = {
        ...
        "has_visual_indicators": "🚀 Using REAL INFERENCE" in content
        and "🔷 Using MOCK OBJECTS" in content,
        "has_metadata": '"using_real_inference":' in content and '"using_mocks":' in content,
    }

    # Derive overall results
    results["complete"] = (
        all(results["has_env_vars"].values())
        and all(results["has_detection"].values())
        and all(results["has_mock_control"].values())
        and results["has_inference_detection"]
        and results["has_mock_detection"]
        and results["has_visual_indicators"]
        and results["has_metadata"]
    )
...
def add_env_vars(content):
    ...
    logger.info(
        f"Added environment variables: {', '.join([var.split(' =')[0] for var in env_vars_to_add])}"
    )
...
def add_mock_imports(content):
    ...
    needs_sentencepiece = (
        "import sentencepiece" not in content or "HAS_SENTENCEPIECE" not in content
    )
    ...
    transformers_import = re.search(
        r'# Try to import transformers.*?logger\.warning\("transformers not available, using mock"\)',
        content,
        re.DOTALL,
    )
...
def add_visual_indicators(content):
    ...
    summary_patterns = ["TEST RESULTS SUMMARY", 'print(f"Successfully tested', "print(results)"]
...
def verify_test_file(file_path):
    ...
    for config in configurations:
        ...
        try:
            ...
            process = subprocess.run(command, env=env, timeout=60, capture_output=True, text=True)
            ...
            detection_match = (expected_mock and mock_detected) or (
                not expected_mock and real_detected
            )
            ...
            logger.info(
                f"Configuration {config_desc}: {'✅ Passed' if success and detection_match else '❌ Failed'}"
            )
            ...
            results[config_desc] = {"success": False, "output": str(e), "detection_match": False}
            all_passed = False
...
def main():
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Verify tests with different mock configurations after fixing",
    )
        ...
        if args.check_only:
            ...
            if success and args.verify:
                ...
                for config, result in verify_results.items():
                    config_status = (
                        f"{GREEN}✅ Passed"
                        if result["success"] and result["detection_match"]
                        else f"{RED}❌ Failed"
                    )
    ...
    for file_path in test_files:
        ...
        results[file_path] = {"success": success, "message": message}
