...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"critical_models_generation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def generate_model_test(
    model_name, architecture, template=None, default_model=None, task=None, original_name=None
):
