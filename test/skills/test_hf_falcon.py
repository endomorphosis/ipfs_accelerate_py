...
# Import the module to test - Falcon will likely use the hf_lm module
from ipfs_accelerate_py.worker.skillset.hf_lm import hf_lm

# Add CUDA support to the Falcon class
def init_cuda(self, model_name, model_type, device_label="cuda:0"):
    """Initialize Falcon model with CUDA support.
    ...
    try:
        ...
        try:
            ...
            # Load model
            try:
                # Try with bfloat16 first (better precision than float16)
                try:
                    # First try with bfloat16 if available
                    if torch.cuda.is_bf16_supported():
                        model = AutoModelForCausalLM.from_pretrained(
                            model_name,
                            torch_dtype=torch.bfloat16,
                            device_map={"": device}  # Map all to specified device
                        )
                        print(f"Successfully loaded model {model_name} with bfloat16")
                    else:
                        ...
                        print(f"Successfully loaded model {model_name} with float16")
                except Exception as half_err:
                    print(f"Half precision loading failed: {half_err}, trying with defaults")
                    # Try with defaults
                    model = AutoModelForCausalLM.from_pretrained(model_name)
                    model = test_utils.optimize_cuda_memory(model, device, use_half_precision=True)
                    print(f"Successfully loaded model {model_name} with default settings")
            ...
            def handler(prompt, max_new_tokens=100, temperature=0.7, top_p=0.9, top_k=50, **kwargs):
                ...
                try:
                    ...
                    if isinstance(model, mock.MagicMock) or isinstance(tokenizer, mock.MagicMock):
                        print("Using mock handler for CUDA Falcon")
                    ...
                    try:
                        ...
                        if isinstance(prompt, list):
                            # Batch processing
                            batch_inputs = tokenizer(prompt, return_tensors="pt", padding=True, truncation=True)
                            
                            # Check if we need to manually move to device
                            # For device_map="auto" or similar, tensors might already be on the right device
                            ...
                            # Process each output to remove prompts if needed
                            processed_texts = []
                            for i, text in enumerate(batch_texts):
                                if prompt[i] in text:
                                    text = text[len(prompt[i]):].strip()
                            ...
                            if "falcon" in model_name.lower():
                                # Falcon models may benefit from these settings
        ...
        print(f"Error in Falcon init_cuda: {e}")
        import traceback
        traceback.print_exc()
...
def create_cuda_falcon_endpoint_handler(self, tokenizer, model_name, cuda_label, endpoint=None):
    """Create handler function for CUDA-accelerated Falcon.
    ...
    def handler(prompt, max_new_tokens=100, temperature=0.7, top_p=0.9, **kwargs):
        ...
        if is_mock:
            ...
            # Handle batch input
            if isinstance(prompt, list):
                return [f"(MOCK CUDA) Generated text for: {p[:20]}..." for p in prompt]
        ...
        try:
            ...
            if isinstance(prompt, list):
                ...
                # Check if we need to manually move to device
                if not hasattr(endpoint, 'hf_device_map') or not endpoint.hf_device_map:
                    batch_inputs = {k: v.to(device) for k, v in batch_inputs.items()}
                ...
                with torch.no_grad():
                    batch_outputs = endpoint.generate(**batch_inputs, **generation_kwargs)
...
hf_lm.create_cuda_falcon_endpoint_handler = create_cuda_falcon_endpoint_handler

class test_hf_falcon:
    def __init__(self, resources=None, metadata=None):
        """
        Initialize the Falcon test class.
        ...
        # Try multiple Falcon models in order of preference
        # Starting with smaller models to improve test reliability
        self.primary_model = "tiiuae/falcon-7b"
        
        # Alternative models in order of preference
        self.alternative_models = [
            "tiiuae/falcon-7b-instruct",      # Instruction-tuned 7B
            "tiiuae/falcon-rw-1b",            # Smallest Falcon model (1B)
            "tiiuae/falcon-instruct",         # Falcon 40B instruct (avoid if possible, too large)
            "tiiuae/falcon-40b",              # Falcon 40B base (avoid if possible, too large)
            "tiiuae/falcon-180b",             # Falcon 180B (too large, included for completeness)
            "tiiuae/falcon-180b-chat",        # Falcon 180B chat (too large, included for completeness)
            # Non-falcon alternatives
            "databricks/dolly-v2-3b",         # Dolly 3B (alternative smaller model)
            "EleutherAI/pythia-1.4b",         # Pythia 1.4B (very small alternative)
            "EleutherAI/pythia-410m",         # Pythia 410M (tiny alternative)
            "facebook/opt-125m"               # OPT 125M (smallest fallback)
        ]
                ...
                try:
                    ...
                    if self.model_name == self.primary_model:
                        ...
                        if os.path.exists(cache_dir):
                            # Look for any Falcon model in cache
                            falcon_models = [name for name in os.listdir(cache_dir) if any(
                                x in name.lower() for x in ["falcon", "tiiuae--falcon"])]
                            
                            if falcon_models:
                                # Use the first model found
                                falcon_model_name = falcon_models[0].replace("--", "/")
                                print(f"Found local Falcon model: {falcon_model_name}")
                                self.model_name = falcon_model_name
                            else:
        ...
        self.test_prompt = "Explain the key characteristics of the Falcon LLM architecture."
    ...
    def _create_test_model(self):
        ...
        """
        try:
            print("Creating local test model for Falcon testing...")
            
            # Create model directory in /tmp for tests
            test_model_dir = os.path.join("/tmp", "falcon_test_model")
            os.makedirs(test_model_dir, exist_ok=True)
            
            # Create a minimal config file for a tiny Falcon-style model
            config = {
                ...
                "num_hidden_layers": 2,
                "num_kv_heads": 4,  # Falcon-specific parameter for multi-query attention
                "bias": False,      # Falcon-specific parameter
                "new_decoder_architecture": True,  # Falcon-specific parameter
                "pad_token_id": 0,
            ...
            if hasattr(torch, "save") and not isinstance(torch, MagicMock):
                ...
                model_state["transformer.word_embeddings.weight"] = torch.randn(vocab_size, hidden_size)
                ...
                for layer_idx in range(num_layers):
                    ...
                    # Self-attention
                    # Falcon uses multi-query attention with separate KV heads
                    head_dim = hidden_size // num_heads
                    model_state[f"{layer_prefix}.self_attention.query_key_value.weight"] = torch.randn(
                        hidden_size + 2 * num_kv_heads * head_dim, hidden_size
                    )
                    model_state[f"{layer_prefix}.self_attention.dense.weight"] = torch.randn(hidden_size, hidden_size)
    ...
    def test(self):
        """
        Run all tests for the Falcon language model, organized by hardware platform.
        ...
        try:
            ...
            try:
                ...
                with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                    ...
                    mock_tokenizer.return_value.batch_decode = MagicMock(return_value=["The Falcon LLM architecture..."])
                    ...
                    # Record example
                    mock_text = "The Falcon LLM architecture has several key characteristics: 1) It uses multi-query attention, a variant of multi-head attention that reduces memory requirements; 2) It incorporates RoPE (Rotary Position Embeddings) for better handling of positional information; 3) It was trained on a massive, high-quality dataset called RefinedWeb; 4) It features a decoder-only transformer architecture optimized for efficient inference; 5) It's trained with FlashAttention for faster training and inference; and 6) It comes in multiple sizes (1B, 7B, 40B, and 180B parameters) with instructed-tuned variants."
                    self.examples.append({
                        "input": self.test_prompt,
                        "output": {
            ...
            try:
                ...
                try:
                        ...
                        if valid_init:
                            ...
                            try:
                                ...
                                if hasattr(test_utils, 'enhance_cuda_implementation_detection'):
                                    ...
                                    print("Enhancing Falcon CUDA handler with implementation markers")
                            ...
                            if output is not None:
                                ...
                                try:
                                    ...
                                    batch_prompts = [self.test_prompt, "What are the advantages of multi-query attention in LLMs?"]
                                ...
                                try:
                                    ...
                                    generation_config = {
                                        ...
                                        "temperature": 0.8,
                                        "top_p": 0.95,
                                        "repetition_penalty": 1.1  # Falcon-specific param
                                    }
                    ...
                    with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                        ...
                        mock_model.return_value = MagicMock()
                        mock_model.return_value.generate.return_value = torch.tensor([[1, 2, 3]])
                        mock_tokenizer.batch_decode.return_value = ["The Falcon LLM architecture..."]
                        ...
                        test_handler = self.lm.create_cuda_falcon_endpoint_handler(
                            tokenizer,
                            self.model_name,
                            "cuda:0",
                            endpoint,
                        ...
                        if isinstance(output, dict) and "text" in output:
                            ...
                            mock_text = "The Falcon LLM architecture features several key characteristics: 1) Multi-query attention mechanism that reduces memory requirements while maintaining model quality; 2) Rotary Position Embeddings (RoPE) for better handling of position information; 3) Trained on the RefinedWeb dataset, a high-quality filtered subset of web data; 4) FlashAttention implementation for faster training and inference; 5) Available in various sizes (1B, 7B, 40B, 180B) with instruct-tuned variants; 6) Decoder-only architecture optimized for text generation tasks."
                            implementation_type = "MOCK"
                        ...
                        try:
                            ...
                            if batch_output is not None and isinstance(batch_output, list):
                                ...
                                self.examples.append({
                                    "input": f"Batch of {len(batch_prompts)} prompts",
                                    "output": {
                                        "first_result": "The Falcon LLM architecture features several key characteristics...",
        ...
        # ====== OPENVINO TESTS ======
        try:
            print("Testing Falcon on OpenVINO...")
    ...
    def __test__(self):
        ...
        collected_file = os.path.join(collected_dir, 'hf_falcon_test_results.json')
        with open(collected_file, 'w') as f:
            json.dump(test_results, f, indent=2)
            print(f"Saved results to {collected_file}")
            
        # Compare with expected results if they exist
        expected_file = os.path.join(expected_dir, 'hf_falcon_test_results.json')
...
if __name__ == "__main__":
    try:
        print("Starting Falcon test...")
        this_falcon = test_hf_falcon()
        results = this_falcon.__test__()
        print("Falcon test completed")
        ...
        print("FALCON TEST RESULTS SUMMARY")
