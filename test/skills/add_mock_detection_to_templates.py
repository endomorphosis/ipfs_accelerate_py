...
def check_mock_detection(content):
    ...
    has_visual_indicators = (
        "🚀 Using REAL INFERENCE with actual models" in content
        and "🔷 Using MOCK OBJECTS for CI/CD testing only" in content
    )
    has_metadata = (
        '"using_real_inference": using_real_inference,' in content
        and '"using_mocks": using_mocks,' in content
    )
...
"""
    ...
    if "def main():" in content and 'print(f"Successfully tested' in content:
        success_print_index = content.find(
            'print(f"Successfully tested', content.find("def main():")
        )
        ...
        if success_print_index != -1:
            ...
            summary_index = content.find(
                "TEST RESULTS SUMMARY", success_print_index - 200, success_print_index
            )
            if summary_index != -1:
...
def main():
    ...
    parser.add_argument(
        "--check-only", action="store_true", help="Only check for mock detection without modifying"
    )
