...
XGLM_MODELS_REGISTRY = {
    "facebook/xglm-564M": {
        "description": "XGLM 564M multilingual model",
        "class": "XGLMForCausalLM",
    },
    "facebook/xglm-1.7B": {
        "description": "XGLM 1.7B multilingual model",
        "class": "XGLMForCausalLM",
    },
    "facebook/xglm-2.9B": {
        "description": "XGLM 2.9B multilingual model",
        "class": "XGLMForCausalLM",
    },
    "facebook/xglm-4.5B": {
        "description": "XGLM 4.5B multilingual model",
        "class": "XGLMForCausalLM",
    },
    "facebook/xglm-7.5B": {
        "description": "XGLM 7.5B multilingual model",
        "class": "XGLMForCausalLM",
    }
}
...
class TestXglmModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "facebook/xglm-1.7B"
        
        # Verify model exists in registry
        if self.model_id not in XGLM_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = XGLM_MODELS_REGISTRY["facebook/xglm-1.7B"]
        else:
        ...
        # Define test inputs
        self.test_text = "XGLM is a multilingual language model that"
        self.test_texts = [
            "XGLM is a multilingual language model that",
            "XGLM can be used for different languages such as"
        ]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            # Check pad token setup for XGLM models
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for XGLM tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for XGLM padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for XGLM tokenizer")
            ...
            if self.class_name == "XGLMForCausalLM":
    ...
    def test_multilingual_generation(self, device="auto"):
        """Test the model's multilingual generation capabilities."""
        if device == "auto":
            device = self.preferred_device
        ...
        if not HAS_TRANSFORMERS or not HAS_TORCH:
            results["multilingual_error_type"] = "missing_dependency"
            results["multilingual_missing_core"] = ["transformers", "torch"]
            results["multilingual_success"] = False
            return results
        
        try:
            logger.info(f"Testing {self.model_id} with multilingual generation on {device}...")
            
            # Sample text prompts in various languages
            multilingual_prompts = {
                "english": "XGLM can generate text in English: ",
                "spanish": "XGLM puede generar texto en español: ",
                "french": "XGLM peut générer du texte en français: ",
                "german": "XGLM kann Text auf Deutsch generieren: "
            }
            ...
            model_load_start = time.time()
            if self.class_name == "XGLMForCausalLM":
                model = transformers.XGLMForCausalLM.from_pretrained(self.model_id)
            else:
            ...
            for lang, prompt in multilingual_prompts.items():
                ...
                start_time = time.time()
                with torch.no_grad():
                    # Check if we're using mock objects
                    using_mocks = not HAS_TRANSFORMERS or not HAS_TORCH or not HAS_TOKENIZERS or not HAS_SENTENCEPIECE
                    ...
                    if not using_mocks:
                        ...
                        # Mock generation for CI/CD testing
                        generated_text = f"Mock generated text in {lang}"
                end_time = time.time()
                
                generation_time = end_time - start_time
                generation_times.append(generation_time)
                
                # Store language-specific result
            ...
            # Store results
            results["multilingual_success"] = True
            results["avg_generation_time"] = avg_generation_time
            results["language_results"] = language_results
            results["multilingual_error_type"] = "none"
            
            # Store in performance stats
            self.performance_stats[f"multilingual_{device}"] = {
                "avg_generation_time": avg_generation_time,
                "tokenizer_load_time": tokenizer_load_time,
                "model_load_time": model_load_time,
                "languages_tested": list(multilingual_prompts.keys())
            }
            
            # Add to examples
            self.examples.append({
                "method": f"multilingual_generation on {device}",
            ...
            # Store error information
            results["multilingual_success"] = False
            results["multilingual_error"] = str(e)
            results["multilingual_traceback"] = traceback.format_exc()
            logger.error(f"Error testing multilingual generation on {device}: {e}")
            ...
            if "cuda" in error_str or "cuda" in traceback_str:
                results["multilingual_error_type"] = "cuda_error"
            elif "memory" in error_str:
                results["multilingual_error_type"] = "out_of_memory"
            elif "no module named" in error_str:
                results["multilingual_error_type"] = "missing_dependency"
            else:
                results["multilingual_error_type"] = "other"
        
        # Add to overall results
        self.results[f"multilingual_{device}"] = results
        return results
    ...
    def run_tests(self, all_hardware=False):
        ...
        self.test_multilingual_generation()
        ...
        if all_hardware:
            ...
            if self.preferred_device != "cpu":
                ...
                self.test_multilingual_generation(device="cpu")
            ...
            if HW_CAPABILITIES["cuda"] and self.preferred_device != "cuda":
                ...
                self.test_multilingual_generation(device="cuda")
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_xglm_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(XGLM_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_xglm_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Facebook XGLM models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable Facebook XGLM models:")
        for model in models:
            info = XGLM_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "facebook/xglm-1.7B"
    logger.info(f"Testing model: {model_id}")
