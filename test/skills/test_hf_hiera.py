#!/usr/bin/env python3
"""
Class-based test file for all HIERA-family models.
...
hiera_MODELS_REGISTRY = {
    "hiera-base": {
        "description": "HIERA models",
        "class": "HieraModel"
    }

}

class TestHieraModels:
    """Base test class for all HIERA-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "hiera-base"
        
        # Verify model exists in registry
        if self.model_id not in hiera_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = hiera_MODELS_REGISTRY["hiera-base"]
        else:
            self.model_info = hiera_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_hiera_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available HIERA models in the registry."""
    return list(hiera_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered HIERA models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestHieraModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_hiera_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test HIERA-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable HIERA-family models:")
        for model in models:
            info = hiera_MODELS_REGISTRY[model]
