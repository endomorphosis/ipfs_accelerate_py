...
def fix_model_ids(content):
    ...
    for model_type, model_id in model_id_map.items():
        ...
        if re.search(rf"Testing {pattern} with", content):
            fixed_content = re.sub(rf"{model_type}(-small)?", model_id, content)
        ...
        try:
            ...
            if "unterminated string literal" in str(e):
                ...
                if line_num <= len(lines):
                    ...
                    logger.info(
                        f"Attempting to fix unterminated string on line {line_num}: {problem_line}"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"❌ {file_path}: Still has syntax error after additional fixes: {e2}"
                        )
                        # Restore from backup
