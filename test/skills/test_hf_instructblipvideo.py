...
"""
Class-based test file for all INSTRUCTBLIPVIDEO-family models.
...
instructblipvideo_MODELS_REGISTRY = {
    "instructblipvideo-base": {
        "description": "INSTRUCTBLIPVIDEO models",
        "class": "InstructblipvideoModel"
    }

}

class TestInstructblipvideoModels:
    """Base test class for all INSTRUCTBLIPVIDEO-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "instructblipvideo-base"
        
        # Verify model exists in registry
        if self.model_id not in instructblipvideo_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = instructblipvideo_MODELS_REGISTRY["instructblipvideo-base"]
        else:
            self.model_info = instructblipvideo_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_instructblipvideo_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available INSTRUCTBLIPVIDEO models in the registry."""
    return list(instructblipvideo_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered INSTRUCTBLIPVIDEO models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestInstructblipvideoModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_instructblipvideo_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test INSTRUCTBLIPVIDEO-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable INSTRUCTBLIPVIDEO-family models:")
        for model in models:
            info = instructblipvideo_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nINSTRUCTBLIPVIDEO Models Testing Summary:")
        total = len(results)
