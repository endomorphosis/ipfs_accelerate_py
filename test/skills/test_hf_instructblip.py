...
class TestInstructBlipModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "Salesforce/instructblip-flan-t5-xl"
        
        # Verify model exists in registry
        if self.model_id not in INSTRUCTBLIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = INSTRUCTBLIP_MODELS_REGISTRY["Salesforce/instructblip-flan-t5-xl"]
        else:
            self.model_info = INSTRUCTBLIP_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "What is unusual about this scene?"
        self.test_texts = [
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "InstructBlipForConditionalGeneration":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_instructblip_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(INSTRUCTBLIP_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_instructblip_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = INSTRUCTBLIP_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "Salesforce/instructblip-flan-t5-xl"
    logger.info(f"Testing model: {model_id}")
