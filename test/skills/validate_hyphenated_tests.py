    ...
    try:
        ...
        if match:
            ...
            result["errors"].append(
                f"Could not extract model name from file name: {file_path.name}"
            )
...
def validate_all_files() -> List[Dict]:
    ...
    for file_path in files:
        ...
        if (
            result["syntax_valid"]
            and result["model_class_found"]
            and result["registry_variable_found"]
        ):
...
def generate_validation_report(results: List[Dict]) -> str:
    ...
    valid_files = sum(
        1
        for r in results
        if r["syntax_valid"] and r["model_class_found"] and r["registry_variable_found"]
    )
    ...
    for result in results:
        ...
        report.append(
            f"- Registry Variable Found: {'✅' if result['registry_variable_found'] else '❌'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--report-path", type=Path, default=VALIDATION_REPORT_PATH, help="Path for the report file"
    )
