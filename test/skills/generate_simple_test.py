...
def get_task_for_model(model_type):
    ...
    for task, model_types in TASK_TO_MODEL_TYPES.items():
        if any(
            model_type_lower == model.lower() or model_type_lower == model.lower().replace("-", "_")
...
def select_best_model(
    ...
    if hardware_profile and hardware_profile in size_variants:
        ...
        if model_type.lower() in hw_models:
            logger.info(
                f"Selected model based on hardware profile {hardware_profile}: {hw_models[model_type.lower()]}"
            )
    ...
    try:
        ...
        if not template_path:
            template_path = get_template_for_architecture(
                model_type, os.path.join(os.path.dirname(__file__), "templates")
            )
        ...
        replacements = {
            ...
            "bert-base-uncased": default_model.split("/")[-1]
            if "/" in default_model
            else default_model,
            "[MASK]": mask_token,
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Model type (e.g., bert, gpt2, t5)"
    )
    ...
    parser.add_argument(
        "--list-architectures", action="store_true", help="List available architecture types"
    )
    ...
    if args.list_architectures:
        ...
        for arch, model_types in sorted(ARCHITECTURE_TYPES.items()):
            print(
                f"  - {arch}: {', '.join(model_types[:3])}{'...' if len(model_types) > 3 else ''}"
            )
        return 0
