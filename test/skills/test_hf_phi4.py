#!/usr/bin/env python3
"""
Class-based test file for all Phi-4-family models.
...
class TestPhi4Models:
    """Base test class for all Phi-4-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "microsoft/phi-4-mini-instruct"
        
        # Verify model exists in registry
        if self.model_id not in PHI-4_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = PHI-4_MODELS_REGISTRY["microsoft/phi-4-mini-instruct"]
        else:
            self.model_info = PHI-4_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "In a world where AI continues to evolve,"
        self.test_texts = [
            "In a world where AI continues to evolve,",
            "In a world where AI continues to evolve, (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "Phi4ForCausalLM":
            model_class = transformers.Phi4ForCausalLM
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_phi4_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Phi-4 models in the registry."""
    return list(PHI-4_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Phi-4 models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestPhi4Models(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_phi4_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Phi-4-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Phi-4-family models:")
        for model in models:
            info = PHI-4_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "microsoft/phi-4-mini-instruct"
    logger.info(f"Testing model: {model_id}")
