...
"""
Class-based test file for all IMAGEGPT-family models.
...
imagegpt_MODELS_REGISTRY = {
    "imagegpt-base": {
        "description": "IMAGEGPT models",
        "class": "ImagegptModel"
    }

}

class TestImagegptModels:
    """Base test class for all IMAGEGPT-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "imagegpt-base"
        
        # Verify model exists in registry
        if self.model_id not in imagegpt_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = imagegpt_MODELS_REGISTRY["imagegpt-base"]
        else:
            self.model_info = imagegpt_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_imagegpt_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available IMAGEGPT models in the registry."""
    return list(imagegpt_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered IMAGEGPT models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestImagegptModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_imagegpt_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test IMAGEGPT-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable IMAGEGPT-family models:")
        for model in models:
            info = imagegpt_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nIMAGEGPT Models Testing Summary:")
        total = len(results)
