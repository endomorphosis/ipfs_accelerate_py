...
def add_missing_imports(content):
    ...
    if "MOCK_TOKENIZERS" not in content:
        ...
        env_vars = re.search(
            r"MOCK_TORCH.*?MOCK_TRANSFORMERS.*?(?=\n# Try|import)", content, re.DOTALL
        )
    ...
    if "import tokenizers" not in content or "HAS_TOKENIZERS" not in content:
        ...
        transformers_import_end = re.search(
            r'logger\.warning\("transformers not available, using mock"\)', content
        )
...
try:
    ...
    if "import sentencepiece" not in content or "HAS_SENTENCEPIECE" not in content:
        ...
        tokenizers_import_end = re.search(
            r'logger\.warning\("tokenizers not available, using mock"\)', content
        )
...
def fix_file(file_path):
    ...
    try:
        ...
        success = (
            has_torch_check
            and has_transformers_check
            and has_tokenizers_check
            and has_sentencepiece_check
        )
...
def main():
    ...
    parser.add_argument(
        "--dir", type=str, default="fixed_tests", help="Directory containing files to fix"
    )
