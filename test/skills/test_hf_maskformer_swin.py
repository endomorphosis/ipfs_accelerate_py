...
"""
Class-based test file for all MASKFORMER-SWIN-family models.
This file provides a unified testing interface for:
- Maskformer-SwinModel
"""
...
maskformer_swin_MODELS_REGISTRY = {
    "maskformer-swin-base": {
        "description": "MASKFORMER-SWIN models",
        "class": "Maskformer-SwinModel"
    }

}

class TestMaskformerSwinModels:
    """Base test class for all MASKFORMER-SWIN-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "maskformer-swin-base"
        
        # Verify model exists in registry
        if self.model_id not in maskformer_swin_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = maskformer_swin_MODELS_REGISTRY["maskformer-swin-base"]
        else:
            self.model_info = maskformer_swin_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_maskformer_swin_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MASKFORMER-SWIN models in the registry."""
    return list(maskformer_swin_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MASKFORMER-SWIN models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestMaskformerSwinModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_maskformer_swin_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MASKFORMER-SWIN-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable MASKFORMER-SWIN-family models:")
        for model in models:
            info = maskformer_swin_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nMASKFORMER-SWIN Models Testing Summary:")
        total = len(results)
