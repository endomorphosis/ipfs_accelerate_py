...
from ipfs_accelerate_py.worker.skillset.hf_gemma import hf_gemma

# Add CUDA support to the Gemma class
def init_cuda(self, model_name, model_type, device_label="cuda:0"):
    """Initialize Gemma model with CUDA support.
    ...
    try:
            ...
            def handler(prompt, max_new_tokens=100, temperature=0.7, top_p=0.9, top_k=50, **kwargs):
                ...
                try:
                    ...
                    if isinstance(model, mock.MagicMock) or isinstance(tokenizer, mock.MagicMock):
                        print("Using mock handler for CUDA Gemma")
        ...
        print(f"Error in Gemma init_cuda: {e}")
        import traceback
        traceback.print_exc()
...
# Add the CUDA initialization method to the Gemma class
hf_gemma.init_cuda = init_cuda

# Add CUDA handler creator
def create_cuda_gemma_endpoint_handler(self, tokenizer, model_name, cuda_label, endpoint=None):
    """Create handler function for CUDA-accelerated Gemma.
...
# Add the handler creator method to the Gemma class
hf_gemma.create_cuda_gemma_endpoint_handler = create_cuda_gemma_endpoint_handler

class test_hf_gemma:
    def __init__(self, resources=None, metadata=None):
        """
        Initialize the Gemma test class.
        ...
        self.gemma = hf_gemma(resources=self.resources, metadata=self.metadata)
        
        # Try multiple Gemma and similar models in order of preference and size
        # Start with the smallest, most reliable options first
        self.primary_model = "google/gemma-2b"
        
        # Alternative models in increasing size order
        self.alternative_models = [
            "google/gemma-2b-it",     # Smaller instruction-tuned Gemma
            "google/gemma-7b",        # Medium Gemma
            "google/gemma-7b-it",     # Medium instruction-tuned Gemma
            "google/gemma2-2b",       # Gemma 2 (2B)
            "google/gemma2-9b",       # Gemma 2 (9B)
            "google/gemma2-27b",      # Gemma 2 (27B)
            "google/gemma3-2b-it",    # Gemma 3 (2B)
            "google/gemma3-8b-it",    # Gemma 3 (8B)
            "google/gemma3-27b-it",   # Gemma 3 (27B)
            # Fall back to non-Gemma small models if needed
            "facebook/opt-350m",      # Very small open model
            "TinyLlama/TinyLlama-1.1B-Chat-v1.0"  # Small open model
        ]
                ...
                try:
                    ...
                    if self.model_name == self.primary_model:
                        ...
                        if os.path.exists(cache_dir):
                            # Look for any Gemma model in cache
                            gemma_models = [name for name in os.listdir(cache_dir) if any(
                                x in name.lower() for x in ["gemma", "google--gemma"])]
                            
                            if gemma_models:
                                # Use the first model found
                                gemma_model_name = gemma_models[0].replace("--", "/")
                                print(f"Found local Gemma model: {gemma_model_name}")
    ...
    def _create_test_model(self):
        ...
        """
        try:
            print("Creating local test model for Gemma testing...")
            
            # Create model directory in /tmp for tests
            test_model_dir = os.path.join("/tmp", "gemma_test_model")
    ...
    def test(self):
        """
        Run all tests for the Gemma language model, organized by hardware platform.
        ...
        try:
            results["init"] = "Success" if self.gemma is not None else "Failed initialization"
        except Exception as e:
            results["init"] = f"Error: {str(e)}"

        # ====== CPU TESTS ======
        try:
            print("Testing Gemma on CPU...")
            ...
            try:
                ...
                if transformers_available:
                    ...
                    endpoint, tokenizer, handler, queue, batch_size = self.gemma.init_cpu(
                        self.model_name,
                        "cpu",
                        "cpu"
                    )
                ...
                with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                    ...
                    test_handler = self.gemma.create_cpu_gemma_endpoint_handler(
                        tokenizer,
                        self.model_name,
                        "cpu",
                        endpoint
                    ...
                    # Record example
                    mock_text = "Once upon a time, there was a robot named Pixel who desperately wanted to learn how to paint. Every day, Pixel would watch human artists create beautiful works of art in the park."
                    self.examples.append({
                        "input": self.test_prompt,
                        "output": {
            ...
            try:
                ...
                try:
                    ...
                    if transformers_available:
                        ...
                        # Real model initialization
                        endpoint, tokenizer, handler, queue, batch_size = self.gemma.init_cuda(
                            self.model_name,
                            "cuda",
                            "cuda:0"
                        )
                        ...
                        if valid_init:
                            ...
                            try:
                                ...
                                if hasattr(test_utils, 'enhance_cuda_implementation_detection'):
                                    ...
                                    print("Enhancing Gemma CUDA handler with implementation markers")
                            ...
                            if output is not None:
                                ...
                                try:
                                    ...
                                    batch_prompts = [self.test_prompt, "Tell a short story about a cat who becomes a chef"]
                    ...
                    with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                        ...
                        mock_model.return_value = MagicMock()
                        mock_model.return_value.generate.return_value = torch.tensor([[1, 2, 3]])
                        mock_tokenizer.batch_decode.return_value = ["A robot named Bolt decided to learn painting..."]
                        ...
                        test_handler = self.gemma.create_cuda_gemma_endpoint_handler(
                            tokenizer,
                            self.model_name,
                            "cuda:0",
                            endpoint,
                        ...
                        if isinstance(output, dict) and "text" in output:
                            ...
                            mock_text = "A robot named Bolt decided to learn painting. Every day after finishing its factory tasks, Bolt would study the great painters and attempt to replicate their techniques."
                            implementation_type = "MOCK"
                        ...
                        try:
                            ...
                            if batch_output is not None and isinstance(batch_output, list):
                                ...
                                self.examples.append({
                                    "input": f"Batch of {len(batch_prompts)} prompts",
                                    "output": {
                                        "first_result": "A robot named Bolt decided to learn painting...",
        ...
        # ====== OPENVINO TESTS ======
        try:
            print("Testing Gemma on OpenVINO...")
            ...
            if has_openvino:
                ...
                with patch('openvino.runtime.Core' if hasattr(openvino, 'runtime') and hasattr(openvino.runtime, 'Core') else 'openvino.Core'):
                    ...
                    endpoint, tokenizer, handler, queue, batch_size = self.gemma.init_openvino(
                        self.model_name,
                        "text-generation",
                        "CPU",
                        "openvino:0",
                    ...
                    test_handler = self.gemma.create_openvino_gemma_endpoint_handler(
                        tokenizer,
                        self.model_name,
                        "openvino:0",
                        endpoint
    ...
    def __test__(self):
        ...
        collected_file = os.path.join(collected_dir, 'hf_gemma_test_results.json')
        with open(collected_file, 'w') as f:
            json.dump(test_results, f, indent=2)
            print(f"Saved results to {collected_file}")
            
        # Compare with expected results if they exist
        expected_file = os.path.join(expected_dir, 'hf_gemma_test_results.json')
...
if __name__ == "__main__":
    try:
        print("Starting Gemma test...")
        this_gemma = test_hf_gemma()
        results = this_gemma.__test__()
        print("Gemma test completed")
        ...
        print("GEMMA TEST RESULTS SUMMARY")
