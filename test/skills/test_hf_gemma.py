...
class TestGemmaModels:
    """Base test class for all Gemma-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "google/gemma-2b"
        ...
        # Define test inputs
        self.test_text = "Write a poem about artificial intelligence"
        self.test_texts = [
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "GemmaForCausalLM":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_gemma_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(GEMMA_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestGemmaModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_gemma_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Gemma-family models")
    ...
    if args.list_models:
        ...
        for model in models:
            info = GEMMA_MODELS_REGISTRY[model]
