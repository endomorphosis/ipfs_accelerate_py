...
def find_test_file_for_model(
    model_class_name: str, test_dir: str = "./fixed_tests"
) -> Optional[str]:
...
def create_model_inventory(
    output_file: str = "model_class_inventory.json", test_dir: str = "./fixed_tests"
):
    ...
    for category, classes in categorized.items():
        inventory["categories"][category] = {"count": len(classes), "classes": []}
    ...
    inventory["coverage"] = {
        ...
        "with_from_pretrained_pct": round(total_with_from_pretrained / len(model_classes) * 100, 2)
        if model_classes
        else 0,
        "with_test_file": total_with_test_file,
        "with_test_file_pct": round(total_with_test_file / len(model_classes) * 100, 2)
        if model_classes
        else 0,
        "with_from_pretrained_test": total_with_from_pretrained_test,
        "with_from_pretrained_test_pct": round(
            total_with_from_pretrained_test / total_with_from_pretrained * 100, 2
        )
        if total_with_from_pretrained
        else 0,
    }
    ...
    logger.info(
        f"  - Classes with from_pretrained: {total_with_from_pretrained}/{len(model_classes)} ({inventory['coverage']['with_from_pretrained_pct']}%)"
    )
    logger.info(
        f"  - Classes with test file: {total_with_test_file}/{len(model_classes)} ({inventory['coverage']['with_test_file_pct']}%)"
    )
    logger.info(
        f"  - Classes with from_pretrained test: {total_with_from_pretrained_test}/{total_with_from_pretrained} ({inventory['coverage']['with_from_pretrained_test_pct']}%)"
    )
...
def create_coverage_report(
    inventory: Dict[str, Any], output_file: str = "from_pretrained_coverage.md"
):
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"- **Classes with from_pretrained:** {inventory['coverage']['with_from_pretrained']} ({inventory['coverage']['with_from_pretrained_pct']}%)\n"
        )
        f.write(
            f"- **Classes with test file:** {inventory['coverage']['with_test_file']} ({inventory['coverage']['with_test_file_pct']}%)\n"
        )
        f.write(
            f"- **Classes with from_pretrained test:** {inventory['coverage']['with_from_pretrained_test']} ({inventory['coverage']['with_from_pretrained_test_pct']}%)\n\n"
        )
        ...
        for category, data in inventory["categories"].items():
            ...
            # Count coverage
            classes_with_from_pretrained = sum(
                1 for c in data["classes"] if c["has_from_pretrained"]
            )
            classes_with_test = sum(1 for c in data["classes"] if c["test_file"])
            classes_with_from_pretrained_test = sum(
                1 for c in data["classes"] if c["has_from_pretrained_test"]
            )

            # Write category stats
            f.write(
                f"- **Classes with from_pretrained:** {classes_with_from_pretrained}/{data['count']} "
                f"({round(classes_with_from_pretrained / data['count'] * 100, 2) if data['count'] else 0}%)\n"
            )
            f.write(
                f"- **Classes with test file:** {classes_with_test}/{data['count']} "
                f"({round(classes_with_test / data['count'] * 100, 2) if data['count'] else 0}%)\n"
            )
            f.write(
                f"- **Classes with from_pretrained test:** {classes_with_from_pretrained_test}/{classes_with_from_pretrained} "
            ...
            f.write(
                f"| Model Class | Has from_pretrained | Has Test File | Tests from_pretrained |\n"
            )
            f.write(f"|-------------|-------------------|---------------|----------------------|\n")
        ...
        f.write(
            f"The following model classes have a from_pretrained method but no test for it:\n\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate an inventory of all HuggingFace model classes with from_pretrained"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="model_class_inventory.json",
    ...
    parser.add_argument(
        "-r",
        "--report",
        default="from_pretrained_coverage.md",
        help="Output markdown report file path (default: from_pretrained_coverage.md)",
    )
    parser.add_argument(
        "--update-report", action="store_true", help="Update the report without being prompted"
    )
