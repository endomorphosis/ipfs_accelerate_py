...
COMPREHENSIVE_REPORT_FILE = (
    ...
    / f"comprehensive_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
)
...
def run_syntax_validation():
    ...
    try:
        result = subprocess.run(
            [
                sys.executable,
                "validate_hyphenated_model_solution.py",
                "--all",
                "--report",
                "--output-dir",
                str(VALIDATION_REPORTS_DIR),
            ],
            capture_output=True,
            text=True,
            cwd=CURRENT_DIR,
        ...
        reports = sorted(
            VALIDATION_REPORTS_DIR.glob("validation_report_*.md"),
...
def run_parallel_inference_validation(models, use_small=True, max_workers=4):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_model = {
            executor.submit(run_inference_validation, model, use_small): model for model in models
        }
...
def collect_validation_reports():
    ...
    reports = {"syntax": [], "inference": []}

    # Collect syntax validation reports
    syntax_reports = sorted(
        VALIDATION_REPORTS_DIR.glob("validation_report_*.md"), key=os.path.getmtime, reverse=True
    )
    for report in syntax_reports[:5]:  # Get the 5 most recent
        reports["syntax"].append(str(report))

    # Collect inference validation reports
    inference_reports = sorted(
        VALIDATION_REPORTS_DIR.glob("inference_validation_*.json"),
        key=os.path.getmtime,
        reverse=True,
    )
...
def generate_comprehensive_report(model_results, inference_results, output_file=None):
    ...
    syntax_passed = sum(
        1 for model, data in model_results.items() if data.get("syntax") == "passed"
    )
    inference_passed = sum(
        1 for model, data in model_results.items() if data.get("inference") == "passed"
    )
    ...
    for model, data in sorted(model_results.items()):
        ...
        syntax_icon = (
            "✅" if syntax_status == "passed" else "❌" if syntax_status == "failed" else "❓"
        )
        inference_icon = (
            "✅" if inference_status == "passed" else "❌" if inference_status == "failed" else "❓"
        )
    ...
    report += (
        "- Verify capitalization in model class names (e.g., GPTJForCausalLM, XLMRoBERTaModel)\n"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Complete validation suite for hyphenated model solution"
    )
    parser.add_argument(
        "--regenerate", action="store_true", help="Regenerate test files before validation"
    )
    ...
    parser.add_argument(
        "--models", type=str, help="Comma-separated list of models to validate (defaults to all)"
    )
    ...
    if args.inference:
        inference_results = run_parallel_inference_validation(
            models_to_validate, use_small=True, max_workers=args.max_workers
        )
    ...
    all_inference_passed = all(
        data.get("inference") == "passed"
