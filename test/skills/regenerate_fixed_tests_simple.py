...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"regenerate_tests_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def run_test_generator(model_type, output_dir=None):
    ...
    generator_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "test_generator_fixed.py"
    )
...
def main():
    ...
    output_dir = ensure_output_dir(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "fixed_tests")
    )
