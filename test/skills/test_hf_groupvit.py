#!/usr/bin/env python3
"""
Class-based test file for all GROUPVIT-family models.
...
groupvit_MODELS_REGISTRY = {
    "groupvit-base": {
        "description": "GROUPVIT models",
        "class": "GroupvitModel"
    }

}

class TestGroupvitModels:
    """Base test class for all GROUPVIT-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "groupvit-base"
        
        # Verify model exists in registry
        if self.model_id not in groupvit_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = groupvit_MODELS_REGISTRY["groupvit-base"]
        else:
            self.model_info = groupvit_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_groupvit_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available GROUPVIT models in the registry."""
    return list(groupvit_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered GROUPVIT models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestGroupvitModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_groupvit_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test GROUPVIT-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable GROUPVIT-family models:")
        for model in models:
            info = groupvit_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nGROUPVIT Models Testing Summary:")
        total = len(results)
