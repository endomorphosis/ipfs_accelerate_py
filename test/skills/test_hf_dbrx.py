#!/usr/bin/env python3
"""
Class-based test file for all DBRX-family models.
...
dbrx_MODELS_REGISTRY = {
    "dbrx-base": {
        "description": "DBRX models",
        "class": "DbrxModel"
    }

}

class TestDbrxModels:
    """Base test class for all DBRX-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "dbrx-base"
        
        # Verify model exists in registry
        if self.model_id not in dbrx_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = dbrx_MODELS_REGISTRY["dbrx-base"]
        else:
            self.model_info = dbrx_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_dbrx_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available DBRX models in the registry."""
    return list(dbrx_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered DBRX models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestDbrxModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_dbrx_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test DBRX-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable DBRX-family models:")
        for model in models:
            info = dbrx_MODELS_REGISTRY[model]
