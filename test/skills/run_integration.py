...
def run_integration(args):
    ...
    if args.deploy:
        ...
        if not args.test_only and args.deploy:
            ...
            post_deployment_results = test_functionality(
                deployed_test_files, cpu_only=args.cpu_only
            )
...
def print_summary(results):
    ...
    overall_success = (
        syntax_all_passed and func_all_passed and deployment_passed and post_all_passed
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Automate HuggingFace test file integration process"
    )
    ...
    parser.add_argument(
        "--backup-dir", type=str, default="backups", help="Directory for backups (default: backups)"
    )

    # Test options
    parser.add_argument(
        "--cpu-only", action="store_true", help="Use CPU only for testing (default: true)"
    )
    parser.add_argument(
        "--test-only", action="store_true", help="Test integration without copying files"
    )

    # Integration options
    parser.add_argument(
        "--deploy", action="store_true", help="Actually deploy the files (default: false)"
    )
    parser.add_argument("--force", action="store_true", help="Force integration even if tests fail")
    parser.add_argument(
        "--save-results", action="store_true", help="Save integration results to JSON file"
    )
