...
def check_template(template_path, detailed=False):
    ...
    results = {
        ...
        "components": {
            ...
            "detection_logic": {"using_real_inference": False, "using_mocks": False},
            "visual_indicators": {"real_inference": False, "mock_objects": False},
            "color_codes": {"GREEN": False, "BLUE": False, "RESET": False},
            "metadata": {
    ...
    try:
        ...
        is_vision_template = (
            "vision_template.py" in template_path or "speech_template.py" in template_path
        )
        ...
        if is_vision_template:
            ...
            vision_components_complete = all(
                [
                    all(
                        results["components"]["env_vars"].values()[:2]
                    ),  # Only MOCK_TORCH and MOCK_TRANSFORMERS
                    all(
                        results["components"]["mock_checks"].values()[:2]
                    ...
                    all(
                        [
                            v
                            for k, v in results["components"]["metadata"].items()
                            if not k.startswith("has_t")
                            or k == "has_transformers"
                            or k == "has_torch"
                        ]
                    ),
                ]
        ...
        return {"file": os.path.basename(template_path), "error": str(e), "complete": False}


def main():
    parser = argparse.ArgumentParser(
        description="Check template files for mock detection implementation status"
    )
    parser.add_argument(
        "--detailed", action="store_true", help="Print detailed report for each template"
    )
    parser.add_argument("--json", action="store_true", help="Output results as JSON")
    parser.add_argument(
        "--directory", type=str, default="templates", help="Directory containing template files"
    )
