...
def get_template_for_architecture(arch_type):
    ...
    template_map = {
        ...
        "vision": os.path.join(
            template_dir, "minimal_vision_template.py"
        ),  # Using our minimal vision template
    ...
    try:
        ...
        if "-" in model_type:
            ...
            if model_type.lower() in ["gpt-j", "gpt-neo", "gpt-neox"]:
                ...
                model_capitalized = "".join(
                    part.upper() if i == 0 else part.capitalize() for i, part in enumerate(parts)
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix indentation and apply template for a model type"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Model type to generate (e.g., bert, gpt2, t5)"
    )
    parser.add_argument(
        "--output-dir", type=str, default="fixed_tests", help="Directory to save fixed file"
    )
