...
def check_cuda():
    ...
    try:
        ...
        if available:
            ...
            device_name = (
                torch.cuda.get_device_name(0) if device_count > 0 else "Unknown CUDA Device"
            )
            return {
                "available": True,
                "name": device_name,
    ...
    try:
        ...
        try:
            ...
            if model_type in ["encoder-only", "decoder-only", "encoder-decoder"]:
                ...
                inputs = processor(
                    "Testing the model performance across different hardware platforms.",
                    return_tensors="pt",
                )
            ...
            result.update(
                {
                    ...
                    "output_shape": str(tuple(output[0].shape))
...
def generate_compatibility_report(results, hardware_info, output_dir="compatibility_reports"):
    ...
    with open(os.path.join(output_dir, "hardware_compatibility_summary.md"), "w") as f:
        ...
        f.write(
            f"- **Platform**: {sys_info['platform']} {sys_info['release']} {sys_info['version']}\n"
        )
        ...
        for arch_type, arch_data in arch_results.items():
            ...
            # Create table
            f.write(
                "| Model | Hardware | Status | Load Time (s) | Inference Time (s) | Memory (MB) |\n"
            )
            f.write(
                "|-------|----------|--------|--------------|-------------------|------------|\n"
            )
            ...
            for result in arch_data:
                ...
                inference_time = (
                    f"{result['inference_time']:.3f}" if result["inference_time"] else "N/A"
                )
                memory = f"{result['memory_usage']:.1f}" if result["memory_usage"] else "N/A"

                f.write(
                    f"| {result['model_id']} | {result['hardware']} | {status} | {load_time} | {inference_time} | {memory} |\n"
                )
    ...
    with open(os.path.join(output_dir, "performance_analysis.md"), "w") as f:
        ...
        for result in memory_results[:20]:  # Show top 20
            f.write(
                f"| {result['model_id']} | {result['hardware']} | {result['memory_usage']:.1f} |\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Hardware Compatibility Matrix Generator for HuggingFace Models"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        choices=REPRESENTATIVE_MODELS.keys(),
    ...
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of worker threads for parallel testing"
    )
    parser.add_argument(
        "--timeout", type=int, default=300, help="Timeout in seconds for each model test"
    )
    parser.add_argument(
        "--detect-only", action="store_true", help="Only detect hardware without running tests"
    )
    ...
    test_hardware = (
        ...
        if args.hardware
            ...
            hw
            for hw, data in hardware_info.items()
        ...
        for model_id, model_type in test_models:
            for hw in test_hardware:
                ...
                future = executor.submit(
                    test_model_on_hardware, model_id, model_type, hw, args.timeout
                )
