...
class TestEncoderDecoderModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "encoder-decoder-base"
        
        # Verify model exists in registry
        if self.model_id not in encoder_decoder_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = encoder_decoder_MODELS_REGISTRY["encoder-decoder-base"]
        else:
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_encoder_decoder_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
