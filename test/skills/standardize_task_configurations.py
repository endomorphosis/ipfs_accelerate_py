...
class TaskStandardizer:
    ...
    def _process_file(self, file_path: Path):
        ...
        if pipeline_match:
            ...
            if current_task != recommended_task:
                ...
                if self.verbose:
                    logger.info(
                        f"Updating task for {model_name} from '{current_task}' to '{recommended_task}'"
                    )
                ...
                if recommended_task in TEST_INPUTS:
                    ...
                    if re.search(test_input_pattern, updated_content):
                        updated_content = re.sub(
                            test_input_pattern, test_input_replacement, updated_content
                        )
    ...
    def _print_summary(self):
        ...
        logger.info(
            f"Files already correct: {already_correct} ({already_correct / total * 100:.1f}%)"
        )
        ...
        for arch, stats in sorted(self.stats["by_architecture"].items()):
            ...
            logger.info(
                f"    - Already correct: {arch_correct} ({arch_correct / arch_total * 100:.1f}%)"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Standardize task configurations in HuggingFace model test files"
    )
    parser.add_argument(
        "--directory", "-d", type=str, default="fixed_tests", help="Directory containing test files"
    )
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="Don't actually write changes to files"
    )
