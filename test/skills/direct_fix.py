...
try:
    ...
    if "# Import hardware detection capabilities if available" in content:
        ...
        pattern = re.compile(
            r"# Import hardware detection capabilities if available.*?# We\'ll detect hardware manually as fallback",
            re.DOTALL,
        )
...
def main():
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Apply the fixes (default is just to show them)"
    )
