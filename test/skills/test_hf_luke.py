...
"""
Class-based test file for all LUKE-family models.
...
luke_MODELS_REGISTRY = {
    "luke-base": {
        "description": "LUKE models",
        "class": "LukeModel"
    }

}

class TestLukeModels:
    """Base test class for all LUKE-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "luke-base"
        
        # Verify model exists in registry
        if self.model_id not in luke_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = luke_MODELS_REGISTRY["luke-base"]
        else:
            self.model_info = luke_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_luke_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available LUKE models in the registry."""
    return list(luke_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered LUKE models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestLukeModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_luke_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test LUKE-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable LUKE-family models:")
        for model in models:
            info = luke_MODELS_REGISTRY[model]
