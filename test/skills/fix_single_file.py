...
def fix_mock_import_controls(content):
    ...
    if "import transformers" in content and "if MOCK_TRANSFORMERS:" not in content:
        transformers_import_pattern = (
            r"try:\s+import transformers\s+HAS_TRANSFORMERS = True\s+except ImportError:"
        )
    ...
    if "import tokenizers" in content and "if MOCK_TOKENIZERS:" not in content:
        tokenizers_import_pattern = (
            r"try:\s+import tokenizers\s+HAS_TOKENIZERS = True\s+except ImportError:"
        )
    ...
    if "import sentencepiece" in content and "if MOCK_SENTENCEPIECE:" not in content:
        sentencepiece_import_pattern = (
            r"try:\s+import sentencepiece\s+HAS_SENTENCEPIECE = True\s+except ImportError:"
        )
...
def add_missing_imports(content):
    ...
    needs_sentencepiece = (
        "import sentencepiece" not in content and "HAS_SENTENCEPIECE" not in content
    )
