            ...
            if success:
                ...
                for key, result in results.get("results", {}).items():
                    if key.startswith("pipeline_") and not result.get("pipeline_success"):
                        errors.append(
                            f"Pipeline error: {result.get('pipeline_error', 'Unknown error')}"
                        )
                    elif key.startswith("from_pretrained_") and not result.get(
                        "from_pretrained_success"
                    ):
                        errors.append(
                            f"from_pretrained error: {result.get('from_pretrained_error', 'Unknown error')}"
                        )
...
def validate_model_inference(model_name, test_file=None, use_small=False):
    ...
    if not transformers_success:
        ...
        results["recommendations"].append(
            "Install transformers and torch to enable model inference validation"
        )
    ...
    if test_file:
        ...
        results["test_file_execution"] = {"success": test_success, "details": test_result}
        ...
        if os.path.exists(default_test_file):
            ...
            test_success, test_result = load_and_run_test_file(
                default_test_file, model_name, use_small
            )
            ...
            results["issues"].append(
                f"No test file provided and default file not found: {default_test_file}"
            )
            results["recommendations"].append(
                f"Generate test file using integrate_generator_fixes.py"
            )

    # Determine overall success
    transformers_ok = (
        results["direct_transformers_test"]["success"]
        if results["direct_transformers_test"]
        else False
    )
    test_file_ok = (
        results["test_file_execution"]["success"] if results["test_file_execution"] else False
    )
    ...
    if not results["success"]:
        if not transformers_ok and not test_file_ok:
            results["recommendations"].append(
                "Install transformers, torch, and tokenizers for full validation"
            )

        if not test_file_ok and test_file:
            results["recommendations"].append(
                "Check test file for compatibility with model architecture"
            )
...
def main():
    ...
    parser.add_argument(
        "--use-small", action="store_true", help="Use smaller model variants for testing"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
