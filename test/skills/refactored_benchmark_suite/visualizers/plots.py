...
def plot_latency_comparison(
    benchmark_results, output_path: Optional[str] = None, include_percentiles: bool = True
):
    """
    ...
    try:
        ...
        has_percentiles = (
            include_percentiles
            and None not in p90_latencies
            and len(p90_latencies) == len(latencies)
        )
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_latency_comparison.png"
            )
        ...
        for i, hw in enumerate(unique_hardware):
            ...
            for bs in unique_batch_sizes:
                ...
                if matches:
                    ...
                    if has_percentiles:
                        hw_p90_latencies.append(
                            sum(p90_latencies[j] for j in matches_indices) / len(matches_indices)
                        )
                        hw_p95_latencies.append(
                            sum(p95_latencies[j] for j in matches_indices) / len(matches_indices)
                        )
                        hw_p99_latencies.append(
                            sum(p99_latencies[j] for j in matches_indices) / len(matches_indices)
                        )
                else:
            ...
            if has_percentiles:
                plt.plot(
                    ...
                    linewidth=1.5,
                    markersize=4,
                    label=f"{hw.upper()} (p90)" if i == 0 else "",
                )
                plt.plot(
                    bar_positions,
                    hw_p95_latencies,
                    ...
                    linewidth=1.5,
                    markersize=4,
                    label=f"{hw.upper()} (p95)" if i == 0 else "",
                )
                plt.plot(
                    bar_positions,
                    hw_p99_latencies,
                    ...
                    linewidth=1.5,
                    markersize=4,
                    label=f"{hw.upper()} (p99)" if i == 0 else "",
                )
...
def plot_throughput_scaling(benchmark_results, output_path: Optional[str] = None):
    ...
    try:
        ...
        if output_path is None:
            ...
            safe_model_id = benchmark_results.config.model_id.replace("/", "__")
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_throughput_scaling.png"
            )
...
def plot_memory_usage(benchmark_results, output_path: Optional[str] = None, detailed: bool = True):
    ...
    try:
        ...
        has_allocated = (
            detailed
            and None not in memory_allocated
            and len(memory_allocated) == len(memory_usages)
        )
        has_reserved = (
            detailed and None not in memory_reserved and len(memory_reserved) == len(memory_usages)
        )
        ...
        if output_path is None:
            ...
            safe_model_id = benchmark_results.config.model_id.replace("/", "__")
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_memory_usage.png"
            )
        ...
        if detailed and (has_allocated or has_reserved or has_peak or has_cpu):
            ...
            plot_memory_bars(
                ax,
                memory_usages,
                f"Total Memory Usage for {benchmark_results.config.model_id}",
                "Memory Usage (MB)",
            )
            current_ax += 1
            ...
            for i, hw in enumerate(unique_hardware):
                ...
                for bs in unique_batch_sizes:
                    ...
                    matches = [
                        memory_usages[j]
                        for j in range(len(memory_usages))
                        if hardware_platforms[j] == hw and batch_sizes[j] == bs
                    ]
...
def plot_flops_comparison(
    benchmark_results, output_path: Optional[str] = None, detailed: bool = True
):
    """
    ...
    try:
        ...
        if output_path is None:
            ...
            safe_model_id = benchmark_results.config.model_id.replace("/", "__")
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_flops_comparison.png"
            )
        ...
        for i, hw in enumerate(unique_hardware):
            ...
            for bs in unique_batch_sizes:
                ...
                matches = [
                    flops_values[j]
                    for j in range(len(flops_values))
                    if hardware_platforms[j] == hw and batch_sizes[j] == bs
                ]
        ...
        if has_detailed:
            ...
            if sample_breakdown:
                ...
                for i, hw in enumerate(unique_hardware):
                    ...
                    hw_indices = [
                        j for j in range(len(hardware_platforms)) if hardware_platforms[j] == hw
                    ]
                    if not hw_indices:
                        continue
                    ...
                    pie_ax = (
                        ...
                        else fig.add_subplot(2, len(unique_hardware), len(unique_hardware) + i + 1)
                    )
                    pie_ax.pie(
                        breakdown_values,
                        labels=breakdown_keys,
                        autopct="%1.1f%%",
                        shadow=True,
                        startangle=90,
                    )
...
def plot_power_efficiency(benchmark_results, output_path: Optional[str] = None):
    ...
    try:
        ...
        if output_path is None:
            ...
            safe_model_id = benchmark_results.config.model_id.replace("/", "__")
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_power_efficiency.png"
            )

        # Create figure with subplots based on available metrics
        num_plots = (
            1 + (1 if has_gflops_efficiency else 0) + (1 if has_throughput_efficiency else 0)
        )
        ...
        if has_gflops_efficiency:
            ...
            for i, hw in enumerate(unique_hardware):
                ...
                for bs in unique_batch_sizes:
                    ...
                    matches = [
                        gflops_per_watt[j]
                        for j in range(len(gflops_per_watt))
                        if hardware_platforms[j] == hw and batch_sizes[j] == bs
                    ]
        ...
        if has_throughput_efficiency:
            ...
            for i, hw in enumerate(unique_hardware):
                ...
                for bs in unique_batch_sizes:
                    # Find matching results
                    matches = [
                        throughput_per_watt[j]
                        for j in range(len(throughput_per_watt))
                        if hardware_platforms[j] == hw and batch_sizes[j] == bs
                    ]
    ...
    try:
        ...
        for result in benchmark_results.results:
            ...
            if "avg_bandwidth_gbps" in result.metrics:
                ...
                arithmetic_intensity.append(
                    result.metrics.get("arithmetic_intensity_flops_per_byte", 0)
                )
        ...
        if output_path is None:
            ...
            safe_model_id = benchmark_results.config.model_id.replace("/", "__")
            output_path = os.path.join(
                benchmark_results.config.output_dir, f"{safe_model_id}_bandwidth_utilization.png"
            )
        ...
        for i, hw in enumerate(unique_hardware):
            ...
            for bs in unique_batch_sizes:
                ...
                indices = [
                    j
                    for j in range(len(utilization_percent))
                    if hardware_platforms[j] == hw and batch_sizes[j] == bs
                ]

                # Use average if multiple matches
                if indices:
                    hw_utilization.append(
                        sum(utilization_percent[j] for j in indices) / len(indices)
                    )
                else:
