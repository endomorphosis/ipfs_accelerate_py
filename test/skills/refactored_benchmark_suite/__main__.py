...
def parse_args():
    ...
    model_group.add_argument(
        "--model", type=str, nargs="+", help="HuggingFace model ID(s) to benchmark"
    )
    model_group.add_argument(
        "--task", type=str, help="Model task (e.g., text-generation, image-classification)"
    )
    model_group.add_argument(
        "--suite",
        type=str,
    ...
    model_group.add_argument("--config", type=str, help="Path to benchmark configuration file")
    ...
    hardware_group.add_argument(
        "--all-hardware", action="store_true", help="Benchmark on all available hardware platforms"
    )

    # Benchmark parameters
    param_group = parser.add_argument_group("Benchmark Parameters")
    param_group.add_argument(
        "--batch-sizes", type=int, nargs="+", default=[1, 2, 4], help="Batch sizes to benchmark"
    )
    param_group.add_argument(
        "--sequence-lengths",
        type=int,
    ...
    param_group.add_argument(
        ...
        type=str,
        nargs="+",
        default=["latency", "throughput", "memory"],
        help="Metrics to collect",
    )
    param_group.add_argument("--warmup", type=int, default=5, help="Number of warmup iterations")
    param_group.add_argument(
    ...
    output_group.add_argument(
        "--output-dir", type=str, default="benchmark_results", help="Directory to save results"
    )
    output_group.add_argument(
        "--export-formats",
        type=str,
    ...
    output_group.add_argument("--no-save", action="store_true", help="Don't save results to disk")
    ...
    hub_group.add_argument(
        "--publish-to-hub", action="store_true", help="Publish results to HuggingFace Hub"
    )
    hub_group.add_argument("--token", type=str, help="HuggingFace Hub token for publishing")
    ...
    misc_group.add_argument("--quiet", "-q", action="store_true", help="Suppress non-error output")
            ...
            if args.dashboard:
                try:
                    ...
                    dashboard_path = generate_dashboard(
                        list(suite_results.values()), output_dir=args.output_dir
                    )
                    logger.info(f"Generated dashboard: {dashboard_path}")
                except ImportError:
                    logger.error(
                        "Dashboard generation requires dash and plotly. Install with 'pip install dash plotly'"
                    )
            ...
            try:
                ...
                if args.dashboard and results:
                    try:
                        ...
                        dashboard_path = generate_dashboard(
                            list(results.values()), output_dir=args.output_dir
                        )
                        logger.info(f"Generated dashboard: {dashboard_path}")
                    except ImportError:
                        logger.error(
