...
from hardware import get_available_hardware, get_hardware_info, initialize_hardware
    ...
    def __post_init__(self):
        ...
        for hw in self.hardware:
            if hw not in available_hw:
                logger.warning(
                    f"Hardware '{hw}' is not available on this system. Available: {available_hw}"
                )
        ...
        for metric in self.metrics:
            if metric not in available_metrics:
                logger.warning(
                    f"Metric '{metric}' is not available. Available: {available_metrics}"
                )
...
class BenchmarkResults:
    ...
    def export_to_json(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                self.config.output_dir, f"benchmark_{safe_model_id}_{timestamp}.json"
            )
    ...
    def export_to_csv(self, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            if output_path is None:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_path = os.path.join(
                    self.config.output_dir, f"benchmark_{safe_model_id}_{timestamp}.csv"
                )
    ...
    def export_to_markdown(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = os.path.join(
                self.config.output_dir, f"benchmark_{safe_model_id}_{timestamp}.md"
            )
    ...
    def publish_to_hub(self, token: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully published benchmark results to HuggingFace Hub for {self.config.model_id}"
                )
            else:
                logger.error(
                    f"Failed to publish benchmark results to HuggingFace Hub for {self.config.model_id}"
                )
            ...
            logger.error(
                "huggingface_hub is required for Hub export. Install with 'pip install huggingface_hub'"
            )
            return False

    def plot_latency_comparison(
        self, output_path: Optional[str] = None, include_percentiles: bool = True
    ):
        """
...
class ModelBenchmark:
    ...
    def __init__(self, model_id: str, **kwargs):
        ...
        if not TRANSFORMERS_AVAILABLE:
            raise ImportError(
                "transformers is required for benchmarking. Install with 'pip install transformers'"
            )

        if not TORCH_AVAILABLE:
            raise ImportError(
                "torch is required for benchmarking. Install with 'pip install torch'"
            )
    ...
    def _initialize_task(self):
        ...
        try:
            ...
            logger.warning(
                f"Could not auto-detect task for model {self.config.model_id}. Defaulting to {self.config.task}"
            )
    ...
    def _prepare_model_and_inputs(
        self, hardware: str, batch_size: int, sequence_length: int
    ) -> Tuple:
        """
        ...
        try:
            ...
            model = model_adapter.load_model(device=device, use_4bit=use_4bit, use_8bit=use_8bit)
            ...
            try:
                ...
                if self.config.task in [
                    ...
                    if tokenizer.mask_token is None:
                        ...
                        text = f"The quick {tokenizer.mask_token} fox jumps over the lazy dog. " * (
                            sequence_length // 10 + 1
                        )
                        text = text[:sequence_length]
                        inputs = tokenizer(
                ...
                inputs = {
                    "input_ids": torch.randint(
                        0, 1000, (batch_size, sequence_length), dtype=torch.long
                    ).to(device)
                }
                ...
                model = nn.Sequential(
                    nn.Embedding(1000, 768), nn.Linear(768, 768), nn.LayerNorm(768)
                ).to(device)
    ...
    def _benchmark_on_hardware(
        ...
        """
        logger.info(
            f"Running benchmark on {hardware} with batch_size={batch_size}, sequence_length={sequence_length}"
        )

        try:
            # Prepare model and inputs
            model, inputs, input_shape = self._prepare_model_and_inputs(
                hardware, batch_size, sequence_length
            )
                ...
                for i in range(self.config.test_iterations):
                    ...
                    for metric in metric_instances:
                        ...
                        # For memory metrics, record at various points
                            ...
                            isinstance(metric, MemoryMetric) and i % 5 == 0
                        ):  # Record every 5 iterations
                ...
                if hasattr(metric, "set_memory_transfers") and model_parameters > 0:
                    ...
                    if hasattr(metric, "estimate_memory_transfers"):
                        metric.estimate_memory_transfers(
                            model_parameters, batch_size, self.config.test_iterations
                        )
...
class BenchmarkSuite:
    ...
    def from_predefined_suite(cls, suite_name: str, **kwargs) -> "BenchmarkSuite":
        ...
        suites = {
            ...
            "text2text-generation": ["t5-small", "facebook/bart-base", "google/flan-t5-small"],
        ...
        if suite_name not in suites:
            raise ValueError(
                f"Unknown suite: {suite_name}. Available suites: {list(suites.keys())}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["json"],
        help="Export formats (json, csv, markdown)",
    )
