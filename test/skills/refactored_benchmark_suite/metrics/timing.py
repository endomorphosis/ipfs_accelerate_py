...
class LatencyMetric:
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        p90_latency = (
            sorted_latencies[p90_index] if p90_index < len(sorted_latencies) else max_latency
        )
        p95_latency = (
            sorted_latencies[p95_index] if p95_index < len(sorted_latencies) else max_latency
        )
        p99_latency = (
            sorted_latencies[p99_index] if p99_index < len(sorted_latencies) else max_latency
        )
...
class ThroughputMetric:
    ...
    def __init__(
        self, batch_size: int = 1, items_per_batch: Optional[int] = None, device_type: str = "cpu"
    ):
        """
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        if len(self.batch_times) > 1:
            ...
            time_per_batch_ms = (
                (elapsed_time / self.num_iterations) * 1000 if self.num_iterations > 0 else 0.0
            )
...
class TimingMetricFactory:
    ...
    def create_throughput_metric(
        device: Any, batch_size: int = 1, items_per_batch: Optional[int] = None
    ) -> ThroughputMetric:
        """
