...
class PowerMetric:
    ...
    def _check_powermetrics(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["powermetrics", "-h"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
    ...
    def start(self):
        ...
        if (
            self.has_nvidia_smi
            or self.has_intel_rapl
            or self.has_powermetrics
            or self.has_amd_rocm_smi
        ):
            self._sample_power()
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if not self.power_samples:
            return {"power_supported": False, "power_avg_watts": 0.0}
