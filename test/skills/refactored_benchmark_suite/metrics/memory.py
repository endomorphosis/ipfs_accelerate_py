...
class MemoryMetric:
    ...
    def start(self):
        ...
        if self.device_type == "cuda" and self.can_track_device_memory:
            ...
            self.start_reserved = (
                torch.mps.memory_reserved() if hasattr(torch.mps, "memory_reserved") else 0
            )
        ...
        self.memory_timeline = [
            (time.time(), self.start_allocated, self.start_reserved, self.start_cpu_memory)
        ]
    ...
    def record_memory(self):
        ...
        self.memory_timeline.append(
            (time.time(), current_allocated, current_reserved, current_cpu_memory)
        )
    ...
    def stop(self):
        ...
        self.memory_timeline.append(
            (time.time(), self.end_allocated, self.end_reserved, self.end_cpu_memory)
        )
    ...
    def get_metrics(self) -> Dict[str, float]:
            ...
            metrics.update(
                {
                    ...
                    "cpu_memory_growth_mb": bytes_to_mb(
                        self.end_cpu_memory - self.start_cpu_memory
                    ),
                }
            )
        ...
        metrics["memory_usage_mb"] = (
            ...
            if self.can_track_device_memory
            else bytes_to_mb(self.end_cpu_memory)
        )
    ...
    def get_memory_timeline(self) -> List[Dict[str, Any]]:
        ...
        for timestamp, allocated, reserved, cpu_memory in self.memory_timeline:
            ...
            if self.can_track_device_memory:
                entry.update(
                    {"allocated_mb": bytes_to_mb(allocated), "reserved_mb": bytes_to_mb(reserved)}
                )
