...
class FLOPsMetric:
    ...
    def _check_hardware_profilers(self) -> Dict[str, bool]:
        ...
        """
        profilers = {"cuda_profiler": False, "xla_profiler": False, "openvino_profiler": False}
    ...
    def stop(self):
        ...
        try:
            ...
            if self.hardware_profilers.get("cuda_profiler") and self.device_type == "cuda":
                ...
                self.hardware_profilers.get("openvino_profiler") and self.device_type == "openvino"
            ):
    ...
    def _estimate_flops_simple(self):
        ...
        if model_type == "transformer":
            ...
            if seq_length > 0 and hidden_size > 0:
                ...
                feed_forward_flops = self._estimate_feed_forward_flops(
                    seq_length, hidden_size, num_layers
                )
                embedding_flops = self._estimate_embedding_flops(seq_length, hidden_size)
                layer_norm_flops = self._estimate_layer_norm_flops(
                    seq_length, hidden_size, num_layers
                )

                # Hardware-specific multiplier to account for optimizations
                total_flops = (
                    attention_flops + feed_forward_flops + embedding_flops + layer_norm_flops
                ) * batch_size
        ...
        if hidden_size == 0:
            for module in self.model.modules():
                ...
                if any(
                    ...
                    for name in ["embedding", "embeddings"]
                ):
    ...
    def _get_image_dimensions(self) -> tuple:
        ...
        if self.inputs is not None:
            if isinstance(self.inputs, dict) and any(
                k in self.inputs for k in ["pixel_values", "images", "pixel_inputs"]
            ):
    ...
    def _estimate_attention_flops(self, seq_length: int, hidden_size: int, num_heads: int) -> int:
        ...
        if multi_query_attn:
            ...
            qkv_flops = (
                hidden_size**2 * seq_length  # Q projection
                + 2 * (hidden_size * head_dim) * seq_length
            )  # K,V projections (only 1 per model)
            attention_flops = (
                2 * hidden_size * seq_length**2  # QK and Attention*V
                + seq_length**2 * num_heads
            )  # Softmax
            ...
            qkv_flops = (
                hidden_size**2 * seq_length  # Q projection
                + 2 * (hidden_size * head_dim * num_kv_heads) * seq_length
            )  # K,V projections
            attention_flops = (
                2 * hidden_size * seq_length**2  # QK and Attention*V
                + seq_length**2 * num_heads
    ...
    def _estimate_feed_forward_flops(
        self, seq_length: int, hidden_size: int, num_layers: int
    ) -> int:
        """
        ...
        ff_flops_per_layer = (
            hidden_size * (hidden_size * expansion_factor) * seq_length
    ...
    def _estimate_image_encoder_flops(self) -> int:
        ...
        if image_encoder is not None:
            ...
            if height > 0 and width > 0:
                return (
                    self._estimate_conv_flops(height, width, channels)
                    + self._estimate_dense_flops()
                )
        ...
        if self.model and batch_size > 0:
            ...
            if self.device_type == "cuda" and hasattr(torch.cuda, "get_device_properties"):
                ...
                try:
                    ...
                    gpu_flops = (
                        props.multi_processor_count * props.max_threads_per_multi_processor * 2
                    )  # FMA = 2 ops
                    ...
                    metrics["flops_utilization_pct"] = (
                        min(100.0, (tflops / theoretical_tflops) * 100)
            ...
            metrics.update(
                {
                    ...
                    "flops_per_parameter": self.flops_count / total_params
                    if total_params > 0
                    else 0,
                }
            )
    ...
    def _get_model_precision_info(self) -> Dict[str, Any]:
        ...
        for dtype, count in dtype_counts.items():
            precision_info[f"precision_{dtype}_pct"] = (
        ...
        if dtype_counts:
            ...
            precision_info["precision_float"] = (
                "float16" in dtype_counts or "float32" in dtype_counts
            )
    ...
    def get_detailed_metrics(self) -> Dict[str, Any]:
        ...
        if self.device_type == "cuda":
            ...
            try:
                ...
                metrics["cuda_capability"] = (
                    f"{torch.cuda.get_device_capability(0)[0]}.{torch.cuda.get_device_capability(0)[1]}"
                )
                metrics["cuda_memory_total"] = torch.cuda.get_device_properties(0).total_memory / (
                    1024**3
                )  # GB
            ...
            if self.model and self.inputs:
                ...
                if hidden_size > 0 and seq_length > 0:
                    ...
                    attention_scaling = {
                        ...
                        "sequence_length": seq_length,
                        "attention_scaling_factor": seq_length
                        * seq_length
                        / 1000000,  # n² scaling factor
                    }

                    # Check for optimized attention mechanisms
                    if any(
                        name.lower() in str(self.model.__class__).lower()
                        for name in ["flash", "triton", "xformers"]
                    ):
                        attention_scaling["attention_optimized"] = True
                        attention_scaling["attention_complexity"] = (
                            "O(n) to O(n log n)"  # Optimized attention can be sub-quadratic
                        )
                    ...
                    transformer_details = {
                        ...
                        "flops_per_token": self.flops_count / (seq_length * self._get_batch_size())
                        if seq_length > 0
                        else 0,
                    }
            ...
            if self.model and self.inputs:
                ...
                if height > 0 and width > 0:
                    cnn_details = {
                        ...
                        "flops_per_pixel": self.flops_count
                        / (height * width * self._get_batch_size())
                        if height > 0 and width > 0
                        else 0,
                    }
