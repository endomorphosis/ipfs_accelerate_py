...
class BandwidthMetric:
    ...
    def _get_cuda_peak_bandwidth(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "nvidia-smi",
                    "--query-gpu=memory.total,memory.clock",
    ...
    def estimate_memory_transfers(
        self, model_size_bytes: int, batch_size: int, inference_count: int
    ) -> int:
        """
        ...
        estimated_transfers = (
            base_transfers * (1 + batch_size * activation_multiplier) * inference_count
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if not self.bandwidth_samples and self.memory_transfers_bytes == 0:
            return {"bandwidth_supported": False, "avg_bandwidth_gbps": 0.0}
        ...
        return {
            ...
            "memory_transfers_gb": self.memory_transfers_bytes / 1e9
            if self.memory_transfers_bytes > 0
            else 0.0,
