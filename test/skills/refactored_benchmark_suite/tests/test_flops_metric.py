...
class MultimodalModel(nn.Module):
    ...
    def __init__(self):
        ...
        # Fusion module
        self.fusion = nn.Sequential(nn.Linear(256 * 2, 512), nn.ReLU(), nn.Linear(512, 128))
...
class TestFLOPsMetric(unittest.TestCase):
    ...
    def test_sequence_length_detection(self):
        ...
        inputs_with_mask = {"input_ids": torch.ones(2, 10), "attention_mask": torch.ones(2, 10)}
        metric.inputs = inputs_with_mask
        self.assertEqual(metric._get_sequence_length(), 10)

        # Test with input_ids only
        inputs_with_ids = {"input_ids": torch.ones(2, 15)}
    ...
    def test_attention_type_detection(self):
        ...
        hidden_size = 256
        num_heads = 8
        attention_flops = mha_metric._estimate_attention_flops(seq_length, hidden_size, num_heads)
