...
class TestBandwidthMetric(unittest.TestCase):
    ...
    def test_memory_transfer_estimation(self):
        ...
        expected_min = (
            model_size * inference_count
        )  # At minimum, each parameter is read once per inference
        self.assertGreaterEqual(estimated, expected_min)

        # Should scale with batch size
        estimated_larger_batch = metric.estimate_memory_transfers(
            model_size, batch_size * 2, inference_count
        )
        ...
        with patch.object(BandwidthMetric, "_get_theoretical_peak_bandwidth") as mock_peak:
            ...
            with patch.object(BandwidthMetric, "_get_peak_compute") as mock_compute:
                ...
                self.assertAlmostEqual(
                    roofline_data["ridge_point_flops_per_byte"], expected_ridge_point, places=2
                )

                # Arithmetic intensity = compute_operations / memory_transfers
                expected_intensity = 1e12 / (1024 * 1024 * 1024)
                self.assertAlmostEqual(
                    roofline_data["arithmetic_intensity_flops_per_byte"],
                    expected_intensity,
                    places=2,
                )
    ...
    def test_metrics_calculation(self):
        ...
        with patch.object(BandwidthMetric, "_get_theoretical_peak_bandwidth") as mock_peak:
            ...
            self.assertAlmostEqual(
                metrics["arithmetic_intensity_flops_per_byte"], expected_intensity, places=2
            )
    ...
    def test_integration_with_benchmark(self):
        ...
        try:
            ...
            self.skipTest(
                "ModelBenchmark import failed - this is expected if BandwidthMetric integration is not complete"
            )
