...
class TestPowerMetric(unittest.TestCase):
    ...
    def test_metrics_calculation(self):
        ...
        self.assertAlmostEqual(
            metrics["gflops_per_watt"], 1000000000 / 100.0 / 1e9, places=5
        )  # 10 GFLOPs/W
        self.assertAlmostEqual(
            metrics["throughput_per_watt"], 100.0 / 100.0, places=5
        )  # 1 item/s/W
    ...
    def test_integration_with_benchmark(self):
        ...
        try:
            ...
            self.skipTest(
                "ModelBenchmark import failed - this is expected if PowerMetric integration is not complete"
            )
