...
def get_model_adapter(model_id: str, task: Optional[str] = None) -> ModelAdapter:
    ...
    if task in [
        ...
        logger.warning(
            f"Unknown task '{task}' for model {model_id}, using default text model adapter"
        )
        return TextModelAdapter(model_id, task)
...
def _infer_task_from_model_id(model_id: str) -> str:
    ...
    # Vision models
    if any(
        name in model_id_lower
        for name in ["vit", "resnet", "efficientnet", "convnext", "swin", "beit", "deit"]
    ):
    ...
    if any(
        name in model_id_lower
        for name in ["gpt", "llama", "bloom", "opt", "falcon", "mistral", "llm"]
    ):
