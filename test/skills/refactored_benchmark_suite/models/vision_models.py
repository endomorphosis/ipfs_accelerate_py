...
def apply_hardware_optimizations(
    model, device_type: str, use_flash_attention: bool = False, use_torch_compile: bool = False
) -> torch.nn.Module:
    """
        ...
        if self.processor is not None:
            if hasattr(self.processor, "size"):
                if isinstance(self.processor.size, dict):
                    self.image_size = (
                        self.processor.size.get("height", 224),
                        self.processor.size.get("width", 224),
                    )
                elif (
                    isinstance(self.processor.size, (list, tuple)) and len(self.processor.size) >= 2
                ):
                ...
                if isinstance(self.processor.crop_size, dict):
                    self.image_size = (
                        self.processor.crop_size.get("height", 224),
                        self.processor.crop_size.get("width", 224),
                    )
                elif (
                    isinstance(self.processor.crop_size, (list, tuple))
                    and len(self.processor.crop_size) >= 2
                ):
        ...
        try:
            if self.is_sam:
                ...
                if self.task == "image-classification":
                    ...
                    logger.warning(
                        f"Unknown task '{self.task}' for vision model, using image classification"
                    )
                    model = AutoModelForImageClassification.from_pretrained(
                        self.model_id, **model_kwargs
                    )
