...
def apply_multimodal_hardware_optimizations(
    ...
    if use_flash_attention and device_type == "cuda":
        try:
            ...
            if (
                ...
                or hasattr(model, "text_model")
            ):
                logger.info(
                    "Applying Flash Attention optimization to multimodal transformer layers"
                )
    ...
    if device_type == "cuda":
        ...
        try:
            ...
            if hasattr(torch, "set_num_threads"):
                ...
                logger.info(
                    f"Set CPU threads to {num_threads} for optimal multimodal model performance"
                )
...
class MultimodalModelAdapter(ModelAdapter):
        ...
        if hasattr(self.processor, "feature_extractor"):
            ...
            for attr_name in ["size", "image_size", "size_dict", "crop_size"]:
                if hasattr(feature_extractor, attr_name):
                    ...
                    if isinstance(size_attr, dict):
                        self.image_size = (
                            size_attr.get("height", 224),
                            size_attr.get("width", 224),
                        )
                        return
                    elif isinstance(size_attr, (list, tuple)) and len(size_attr) >= 2:
                        self.image_size = (size_attr[0], size_attr[1])
    ...
    def prepare_inputs(
        self, batch_size: int, sequence_length: Optional[int] = None
                ...
                try:
                    ...
                    if hasattr(self.processor, "image_processor"):
                        image_inputs = self.processor.image_processor(
                            random_images, return_tensors="pt"
                        )
                    else:
                    ...
                    if hasattr(self.processor, "tokenizer"):
                        ...
                        text_inputs = {
                            ...
                            "attention_mask": torch.ones(
                                batch_size, self.max_length, dtype=torch.long
                            ),
                        }
        ...
        try:
            ...
            try:
                ...
                if self.processor:
                    ...
                    inputs = {
                        "pixel_values": image_inputs.get(
                            "pixel_values", image_inputs.get("image", None)
                        )
                    }
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    inputs = self.processor(images=random_images, return_tensors="pt")
    ...
    def _prepare_document_inputs(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            try:
                # Try to process with the processor
                inputs = self.processor(
                    images=random_images, questions=questions, return_tensors="pt", padding=True
                )
            except:
                try:
                    # Try alternative processing
                    inputs = self.processor(
                        images=random_images, text=questions, return_tensors="pt", padding=True
                    )
                except:
