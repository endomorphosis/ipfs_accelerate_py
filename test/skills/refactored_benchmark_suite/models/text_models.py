    ...
    def load_model(
        ...
        if is_large_model and (use_4bit or use_8bit):
            try:
                ...
                logger.warning(
                    "BitsAndBytes not available. Install with 'pip install bitsandbytes transformers>=4.30.0'"
                )
    ...
    def prepare_inputs(self, batch_size: int, sequence_length: int) -> Dict[str, torch.Tensor]:
        ...
        if self.task == "fill-mask":
            ...
            if self.tokenizer.mask_token:
                text = f"The quick {self.tokenizer.mask_token} fox jumps over the lazy dog. " * (
