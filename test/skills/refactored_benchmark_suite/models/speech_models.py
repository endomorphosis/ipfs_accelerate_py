...
def apply_speech_hardware_optimizations(
    ...
    if device_type == "cuda":
        ...
        try:
            ...
            if hasattr(torch, "set_num_threads"):
                ...
                logger.info(
                    f"Set CPU threads to {num_threads} for optimal speech model performance"
                )
...
class SpeechModelAdapter(ModelAdapter):
    ...
    def _extract_sampling_rate(self):
        ...
        if self.processor is not None:
            ...
            elif hasattr(self.processor, "feature_extractor") and hasattr(
    ...
    def _load_model_by_task(self, **model_kwargs):
        ...
        if self.task == "automatic-speech-recognition":
            ...
            logger.warning(
                f"Unknown task '{self.task}' for speech model, using automatic-speech-recognition"
            )
            try:
                return AutoModelForSpeechSeq2Seq.from_pretrained(self.model_id, **model_kwargs)
            except:
    ...
    def prepare_inputs(
        ...
        if sequence_length is not None:
            self.max_length = (
                sequence_length * self.sampling_rate // 16
            )  # Convert from tokens to samples
    ...
    def _prepare_whisper_inputs(self, sample_audios, batch_size):
        ...
        try:
            ...
            inputs = self.processor(
                sample_audios, sampling_rate=self.sampling_rate, return_tensors="pt", padding=True
            )
