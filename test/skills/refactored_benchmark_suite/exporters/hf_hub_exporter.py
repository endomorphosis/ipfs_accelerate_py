...
class ModelCardExporter:
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        try:
            from huggingface_hub import HfApi, ModelCard, ModelCardData, CardData, login
    ...
    def format_benchmark_data(self, benchmark_results) -> Dict[str, Any]:
        ...
        for hardware in sorted_hardware:
            ...
            results = sorted(
                hardware_results[hardware], key=lambda r: (r.batch_size, r.sequence_length)
            )
            ...
            # Add to table
            markdown_table += (
                f"| {hw_display} | {inference_str} | {memory_str} | {batch_str} | {seq_str} |\n"
            )
            ...
            if inference_ms is not None:
                ...
                badge_url = BENCHMARK_BADGE_TEMPLATE.format(
                    hardware=hw_display, time_ms=f"{inference_ms:.1f}", color=color
                )
...
"""
        ...
        return {"markdown": markdown_table, "metadata": metadata}

    def update_model_card(
        self,
        model_id: str,
        benchmark_data: Dict[str, Any],
        commit_message: str = "Update performance benchmarks",
    ) -> bool:
        """
    ...
    def publish_results(
        self,
        benchmark_results,
        commit_message: str = "Update performance benchmarks from IPFS Accelerate Testing Framework",
    ) -> bool:
        """
        ...
        return self.update_model_card(
            model_id=model_id, benchmark_data=benchmark_data, commit_message=commit_message
        )
