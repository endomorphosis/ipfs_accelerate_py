...
try:
    ...
    print(
        "Warning: Hugging Face Hub not available. Please install with 'pip install huggingface_hub'"
    )
        ...
        metadata["benchmarks"].append(
            {
                ...
                "memory_usage_mb": float(f"{row['memory_usage']:.1f}")
                if pd.notna(row["memory_usage"])
                else None,
                "load_time_s": float(f"{row['load_time']:.2f}")
                if pd.notna(row["load_time"])
                else None,
...
def update_model_card(
    model_id, benchmark_data, token=None, commit_message="Update performance benchmarks"
):
    """
...
def publish_benchmarks_to_hub(benchmarks_df, token=None, dry_run=False, limit=None):
    ...
    logger.info(
        f"Publishing benchmarks for {len(model_ids)} models" + (" (DRY RUN)" if dry_run else "")
    )

    results = {"success": 0, "failed": 0, "skipped": 0, "total": len(model_ids), "models": {}}
    ...
    # Log summary
    logger.info(
        f"Benchmark publishing complete: {results['success']} succeeded, {results['failed']} failed, {results['skipped']} skipped"
    )
...
def main():
    ...
    parser.add_argument("--token", type=str, help="HuggingFace API token (or use HF_TOKEN env var)")
    parser.add_argument(
        "--dry-run", action="store_true", help="Generate benchmark data but don't publish to Hub"
    )
    parser.add_argument(
        "--local",
        action="store_true",
    ...
    parser.add_argument("--model", type=str, help="Process only the specified model ID")
    ...
    if not token and not args.dry_run and not args.local:
        logger.warning(
            "No HuggingFace API token provided. Using anonymous access (may fail for private repos)."
        )
    ...
    if args.local:
        ...
        results = publish_benchmarks_to_hub(
            benchmarks_df=benchmarks_df, token=token, dry_run=args.dry_run, limit=args.limit
        )
