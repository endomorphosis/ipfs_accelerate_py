#!/usr/bin/env python3
"""
Class-based test file for all MVP-family models.
...
class TestMvpModels:
    """Base test class for all MVP-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "mvp-base"
        
        # Verify model exists in registry
        if self.model_id not in mvp_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = mvp_MODELS_REGISTRY["mvp-base"]
        else:
            self.model_info = mvp_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_mvp_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MVP models in the registry."""
    return list(mvp_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MVP models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestMvpModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_mvp_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MVP-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable MVP-family models:")
        for model in models:
            info = mvp_MODELS_REGISTRY[model]
