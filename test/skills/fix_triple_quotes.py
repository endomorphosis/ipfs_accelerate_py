        ...
        if open_positions:
            for line_num, pos, indent in open_positions:
                ...
                if next_code_line is not None:
                    ...
                    lines.insert(next_code_line, f"{indent_str}'''")
                    logger.info(
                        f"Added closing single triple quotes before line {next_code_line + 1}"
                    )
                    changes += 1
                else:
                    # Add to the end of the file
...
def main():
    ...
    parser.add_argument(
        "--pattern", type=str, default="*.py", help="File pattern to match (default: *.py)"
    )
    ...
    logger.info(
        f"Processed {success_count + fail_count} files: {success_count} fixed successfully, {fail_count} with errors"
    )
