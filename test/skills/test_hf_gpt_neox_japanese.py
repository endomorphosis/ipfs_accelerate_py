...
"""
Class-based test file for all GPT_NEOX_JAPANESE-family models.
...
gpt_neox_japanese_MODELS_REGISTRY = {
    "gpt-neox-japanese-base": {
        "description": "GPT_NEOX_JAPANESE models",
        "class": "GptNeoxJapaneseModel"
    }

}

class TestGpt_Neox_JapaneseModels:
    """Base test class for all GPT_NEOX_JAPANESE-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "gpt-neox-japanese-base"
        
        # Verify model exists in registry
        if self.model_id not in gpt_neox_japanese_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = gpt_neox_japanese_MODELS_REGISTRY["gpt-neox-japanese-base"]
        else:
            self.model_info = gpt_neox_japanese_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_gpt_neox_japanese_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available GPT_NEOX_JAPANESE models in the registry."""
    return list(gpt_neox_japanese_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered GPT_NEOX_JAPANESE models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestGpt_Neox_JapaneseModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_gpt_neox_japanese_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test GPT_NEOX_JAPANESE-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable GPT_NEOX_JAPANESE-family models:")
        for model in models:
            info = gpt_neox_japanese_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nGPT_NEOX_JAPANESE Models Testing Summary:")
        total = len(results)
