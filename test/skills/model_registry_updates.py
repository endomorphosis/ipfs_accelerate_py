...
def update_model_registry(generator_file, apply_changes=False):
    ...
    try:
        ...
        registry_end = (
            registry_start + registry_end_match.start() + 1
        ...
        if new_entries:
            ...
            updated_content = (
                content[:registry_start] + new_registry_content + content[registry_end:]
            )
        else:
        ...
        if arch_types_match:
            for arch_type, models in ARCHITECTURE_UPDATES.items():
                ...
                arch_pattern = re.compile(rf'(\s*"{arch_type}":\s*\[)([^\]]+)(\],)', re.MULTILINE)
                ...
                if arch_match:
                    ...
                    if models_to_add:
                        ...
                        if current_models.strip():
                            new_models_str = (
                                current_models.rstrip() + ', "' + '", "'.join(models_to_add) + '"'
                            )
                        else:
                        ...
                        updated_content = updated_content.replace(
                            arch_match.group(0), new_arch_line
                        )
