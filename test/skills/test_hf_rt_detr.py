...
"""
Class-based test file for all RT_DETR-family models.
...
rt_detr_MODELS_REGISTRY = {
    "rt-detr-base": {
        "description": "RT_DETR models",
        "class": "RtDetrModel"
    }

}

class TestRt_DetrModels:
    """Base test class for all RT_DETR-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "rt-detr-base"
        
        # Verify model exists in registry
        if self.model_id not in rt_detr_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = rt_detr_MODELS_REGISTRY["rt-detr-base"]
        else:
            self.model_info = rt_detr_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_rt_detr_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available RT_DETR models in the registry."""
    return list(rt_detr_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered RT_DETR models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestRt_DetrModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_rt_detr_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test RT_DETR-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable RT_DETR-family models:")
        for model in models:
            info = rt_detr_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nRT_DETR Models Testing Summary:")
        total = len(results)
