...
def validate_class_name(content, model_name):
    ...
    if actual_class_name != expected_class_name:
        return (
            False,
            f"Incorrect test class name: found '{actual_class_name}', expected '{expected_class_name}'",
        )
...
def validate_file(file_path, run_inference=False):
    ...
    try:
        ...
        if file_name.startswith("test_hf_"):
            ...
            matching_models = [
                m
                for m in KNOWN_HYPHENATED_MODELS
                if to_valid_identifier(m) == model_name.replace("-", "_")
            ]
            if matching_models:
        ...
        return {
            ...
            "issues": [str(e)],
            "raw_model_name": os.path.basename(file_path)[8:-3]
            if os.path.basename(file_path).startswith("test_hf_")
            else None,
        }
    ...
    rules = ARCHITECTURE_VALIDATION_RULES.get(
        arch_type, ARCHITECTURE_VALIDATION_RULES["encoder-only"]
    )
    ...
    model_class_valid, model_class_error = validate_model_class_imports(
        content, model_name, arch_type
    )
    ...
    if validation_results["issues"]:
        validation_results["status"] = (
            "warning"
            if run_inference and validation_results["inference_results"]["success"]
            else "failed"
        )
        validation_results["message"] = (
            f"Validation completed with {len(validation_results['issues'])} issues"
        )
    else:
...
def generate_actionable_recommendations(validation_results):
        ...
        if "Missing required variable" in issue:
            ...
            if match:
                ...
                recommendations.append(
                    f"📝 Rename class from {found} to {expected} to follow naming convention"
                )

        elif "Missing expected model class" in issue:
            model_class = issue.split(": ")[1]
            recommendations.append(
                f"📝 Use {model_class} for consistency with HuggingFace's class naming"
            )
            ...
            recommendations.append(
                "🔍 Test the model with the actual HuggingFace dependency to verify functionality"
            )
    ...
    if validation_results.get("inference_results", {}).get("success") is False:
        recommendations.append(
            "📦 Verify that the transformers, torch, and tokenizers packages are installed"
        )
...
def generate_markdown_report(validation_results_list, file_path=None):
    ...
    failed = sum(
        1 for r in validation_results_list if r["status"] == "error" or r["status"] == "failed"
    )
...
def validate_files(file_patterns, run_inference=False, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(validate_file, file_path, run_inference): file_path
...
def main():
    ...
    file_group.add_argument(
        "--all", action="store_true", help="Validate all test files in fixed_tests directory"
    )
