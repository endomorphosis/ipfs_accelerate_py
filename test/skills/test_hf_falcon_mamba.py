#!/usr/bin/env python3
"""
Class-based test file for all FALCON_MAMBA-family models.
...
falcon_mamba_MODELS_REGISTRY = {
    "falcon-mamba-base": {
        "description": "FALCON_MAMBA models",
        "class": "FalconMambaModel"
    }

}

class TestFalcon_MambaModels:
    """Base test class for all FALCON_MAMBA-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "falcon-mamba-base"
        
        # Verify model exists in registry
        if self.model_id not in falcon_mamba_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = falcon_mamba_MODELS_REGISTRY["falcon-mamba-base"]
        else:
            self.model_info = falcon_mamba_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_falcon_mamba_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available FALCON_MAMBA models in the registry."""
    return list(falcon_mamba_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered FALCON_MAMBA models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestFalcon_MambaModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_falcon_mamba_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test FALCON_MAMBA-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable FALCON_MAMBA-family models:")
        for model in models:
            info = falcon_mamba_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nFALCON_MAMBA Models Testing Summary:")
        total = len(results)
