...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"medium_priority_generation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def generate_test_for_model(model_name, model_config, output_dir=OUTPUT_DIR):
    ...
    try:
        ...
        syntax_check = subprocess.run(
            [sys.executable, "-m", "py_compile", test_file], capture_output=True, text=True
        )
...
def generate_batch(missing_models, max_models=None, num_workers=4, output_dir=OUTPUT_DIR):
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_model = {
            executor.submit(
                generate_test_for_model, model_name, model_config, output_dir
            ): model_name
    ...
    try:
        ...
        if "## Phase 3: Architecture Expansion" in content:
            ...
            if "##" in phase3_part:
                ...
                insert_position = (
                    len(parts[0]) + len("## Phase 3: Architecture Expansion") + phase3_end
                )
            else:
                insert_position = (
                    len(parts[0]) + len("## Phase 3: Architecture Expansion") + len(phase3_part)
                )
    ...
    try:
        ...
        for model_name in successful_models.keys():
            ...
            if model_name.lower() != model_name:
                for pattern in patterns:
                    capitalized_pattern = pattern.replace(
...
def update_coverage_statistic(successful_count):
    ...
    try:
        ...
        for line in lines:
            if (
                "**Priority 2: Comprehensive HuggingFace Model Testing (300+ classes)**" in line
                and "IN PROGRESS" in line
            ):
                # Update the progress percentage
                updated_line = line.replace(
                    f"(🔄 IN PROGRESS - 45%)", f"(🔄 IN PROGRESS - {percentage}%)"
                )
...
def generate_coverage_report():
    ...
    try:
        ...
        subprocess.run([sys.executable, coverage_tool, "--output-dir", COVERAGE_DIR])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate tests for medium-priority HuggingFace models"
    )
    parser.add_argument("--max-models", type=int, help="Maximum number of models to generate")
    parser.add_argument(
        "--output-dir", type=str, default=OUTPUT_DIR, help="Output directory for generated tests"
    )
    parser.add_argument(
        "--list-missing",
        action="store_true",
        help="List missing medium-priority models without generating tests",
    )
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel workers")
    parser.add_argument(
        "--specific-model", type=str, help="Generate test for a specific medium-priority model"
    )
    parser.add_argument(
        "--skip-update", action="store_true", help="Skip updating the roadmap and coverage files"
    )
    parser.add_argument(
        "--init-roadmap",
        action="store_true",
        help="Initialize Phase 4 section in roadmap without generating tests",
    )
    parser.add_argument(
        "--category",
        type=str,
    ...
    if args.category:
        ...
        logger.info(
            f"Filtered to {len(medium_priority_models)} models in category: {args.category}"
        )
    ...
    if args.list_missing:
        ...
        for i, (model_name, config) in enumerate(missing_models.items(), 1):
            logger.info(
                f"{i}. {model_name} (architecture: {config['architecture']}, template: {config['template']})"
            )
        return
    ...
    if args.specific_model:
        if args.specific_model in medium_priority_models:
            success = generate_test_for_model(
                args.specific_model, medium_priority_models[args.specific_model], output_dir
            )
            if success and not args.skip_update:
                update_model_coverage_roadmap(
                    {args.specific_model: medium_priority_models[args.specific_model]}
                )
    ...
    results, successful, failed = generate_batch(
        missing_models, max_models, args.workers, output_dir
    )
