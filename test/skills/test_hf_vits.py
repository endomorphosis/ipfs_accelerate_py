...
"""
Class-based test file for all VITS-family models.
...
class TestVitsModels:
    """Base test class for all VITS-family models."""
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in vits_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = vits_MODELS_REGISTRY["MIT/ast-finetuned-audioset-10-10-0.4593"]
        else:
            self.model_info = vits_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_vits_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available VITS models in the registry."""
    return list(vits_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered VITS models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestVitsModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_vits_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test VITS-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable VITS-family models:")
        for model in models:
            info = vits_MODELS_REGISTRY[model]
