#!/usr/bin/env python3
"""
Class-based test file for all QWEN3_VL-family models.
...
qwen3_vl_MODELS_REGISTRY = {
    "qwen3-vl-base": {
        "description": "QWEN3_VL models",
        "class": "Qwen3VlModel"
    }

}

class TestQwen3_VlModels:
    """Base test class for all QWEN3_VL-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "qwen3-vl-base"
        
        # Verify model exists in registry
        if self.model_id not in qwen3_vl_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = qwen3_vl_MODELS_REGISTRY["MIT/ast-finetuned-audioset-10-10-0.4593"]
        else:
            self.model_info = qwen3_vl_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_qwen3_vl_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available QWEN3_VL models in the registry."""
    return list(qwen3_vl_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered QWEN3_VL models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestQwen3_VlModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_qwen3_vl_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test QWEN3_VL-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable QWEN3_VL-family models:")
        for model in models:
            info = qwen3_vl_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nQWEN3_VL Models Testing Summary:")
        total = len(results)
