...
def fix_missing_imports(content, file_path):
    ...
    if not has_tokenizers_var:
        ...
        if "Try to import tokenizers" not in content:
            ...
            transformers_import_match = re.search(
                r'(# Try to import transformers.*?HAS_TRANSFORMERS = False.*?logger\.warning\("transformers not available, using mock"\))',
                content,
                re.DOTALL,
            )
...
"""
    ...
    if not has_sentencepiece_var:
        ...
        if "Try to import sentencepiece" not in content:
            ...
            tokenizers_import_match = re.search(
                r'(# Try to import tokenizers.*?HAS_TOKENIZERS = False.*?logger\.warning\("tokenizers not available, using mock"\))',
                content,
                re.DOTALL,
            )
    ...
    for model_prefix, (incorrect, correct) in model_class_corrections.items():
        if model_type.startswith(model_prefix):
            ...
            if old_line in content:
                logger.info(
                    f"❌ {file_path}: Incorrect model class name '{incorrect}', fixing to '{correct}'"
                )
...
def fix_run_tests_function(content, file_path):
    ...
    run_tests_match = re.search(
        r"(def run_tests\(self, all_hardware=False\):.*?return results\s*$)",
        content,
        re.DOTALL | re.MULTILINE,
    )
    if not run_tests_match:
        logger.warning(
            f"❗ {file_path}: Could not find run_tests function, skipping mock detection fix"
        )
        return content
    ...
    if not has_detection or not has_mocks:
        ...
        metadata_match = re.search(
            r'(\s+# Add metadata\s+results\["metadata"\] = {)', run_tests_content
        )
        if metadata_match:
    ...
    if missing_keys:
        ...
        if metadata_dict_match:
            ...
            if dict_closing_match:
                ...
                new_dict = (
                    metadata_dict[: dict_closing_match.start()]
                    + ",\n"
                    + missing_keys_str
                    + metadata_dict[dict_closing_match.start() :]
                )
...
def fix_main_function(content, file_path):
    ...
    summary_section_match = re.search(
        r'(\s+# Print a summary\s+print\("\n" \+ "="\*50\)\s+print\("TEST RESULTS SUMMARY"\)\s+print\("="\*50\)\s+)',
        main_function,
    )
...
def fix_registry_variables(content, file_path):
    ...
    for registry_var in registry_vars:
        ...
        if registry_var != new_registry_var:
            logger.info(
                f"❌ {file_path}: Incorrect registry variable '{registry_var}', fixing to '{new_registry_var}'"
            )
...
def fix_test_class_name(content, file_path):
    ...
    if current_class_name != correct_class_name:
        logger.info(
            f"❌ {file_path}: Incorrect test class name '{current_class_name}', fixing to '{correct_class_name}'"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix hyphenated model names and ensure consistent mock detection"
    )
    parser.add_argument("--file", type=str, help="Path to specific file to fix")
    parser.add_argument(
        "--dir", type=str, default="fixed_tests", help="Directory containing test files to fix"
    )
    parser.add_argument(
        "--scan-models", action="store_true", help="Scan HuggingFace for hyphenated model names"
    )
    parser.add_argument(
        "--generate-all", action="store_true", help="Generate test files for all hyphenated models"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify syntax of generated test files"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    if args.file:
        ...
        logger.info(
            f"Models: {', '.join(hyphenated_models[:10])}..."
            if len(hyphenated_models) > 10
            else f"Models: {', '.join(hyphenated_models)}"
        )
        ...
        if args.generate_all:
            ...
            with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                future_to_model = {
                    executor.submit(process_model, model, output_dir): model
