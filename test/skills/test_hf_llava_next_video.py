...
class TestLlavaNextVideoModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "llava-hf/llava-v1.6-vicuna-7b-video"
        
        # Verify model exists in registry
        if self.model_id not in LLAVA-NEXT-VIDEO_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = LLAVA-NEXT-VIDEO_MODELS_REGISTRY["llava-hf/llava-v1.6-vicuna-7b-video"]
        else:
            self.model_info = LLAVA-NEXT-VIDEO_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Describe what's happening in this video"
        self.test_texts = [
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "LlavaNextVideoForConditionalGeneration":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_llava_next_video_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(LLAVA-NEXT-VIDEO_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_llava_next_video_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = LLAVA-NEXT-VIDEO_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "llava-hf/llava-v1.6-vicuna-7b-video"
    logger.info(f"Testing model: {model_id}")
