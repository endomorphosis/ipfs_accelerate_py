...
class TestCodeLlamaModels:
    """Base test class for all CodeLLama models."""

    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "codellama/CodeLlama-7b-hf"

        # Verify model exists in registry
        if self.model_id not in CODELLAMA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = CODELLAMA_MODELS_REGISTRY["codellama/CodeLlama-7b-hf"]
        else:
            self.model_info = CODELLAMA_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "def fibonacci(n):"
        self.test_texts = ["def fibonacci(n):", "def quicksort(arr):"]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_codellama_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def get_available_models():
    """Get a list of all available CodeLLama models in the registry."""
    return list(CODELLAMA_MODELS_REGISTRY.keys())


def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered CodeLLama models."""
    models = get_available_models()
    results = {}

    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestCodeLlamaModels(model_id)
    ...
    summary_path = os.path.join(
        output_dir, f"hf_codellama_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test CodeLLama models")
    ...
    if args.list_models:
        ...
        for model in models:
            info = CODELLAMA_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print(f"\nCodeLLama Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "codellama/CodeLlama-7b-hf"
    logger.info(f"Testing model: {model_id}")
