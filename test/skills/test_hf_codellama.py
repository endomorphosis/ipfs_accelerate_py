#!/usr/bin/env python3
"""
Class-based test file for all CodeLlama-family models.
...
class TestCodeLlamaModels:
    """Base test class for all CodeLlama-family models."""
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "LlamaForCausalLM":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_codellama_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_codellama_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test CodeLlama-family models")
