...
"""
Class-based test file for all MIMI-family models.
...
mimi_MODELS_REGISTRY = {
    "mimi-base": {
        "description": "MIMI models",
        "class": "MimiModel"
    }

}

class TestMimiModels:
    """Base test class for all MIMI-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "mimi-base"
        
        # Verify model exists in registry
        if self.model_id not in mimi_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = mimi_MODELS_REGISTRY["mimi-base"]
        else:
            self.model_info = mimi_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_mimi_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MIMI models in the registry."""
    return list(mimi_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MIMI models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestMimiModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_mimi_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MIMI-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable MIMI-family models:")
        for model in models:
            info = mimi_MODELS_REGISTRY[model]
