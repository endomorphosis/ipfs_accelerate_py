...
"""
Class-based test file for all Audio-Spectrogram-Transformer-family models.
...
AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY = {
...
class TestAudioSpectrogramTransformerModels:
    """Base test class for all Audio-Spectrogram-Transformer-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "MIT/ast-finetuned-audioset-10-10-0.4593"
        
        # Verify model exists in registry
        if self.model_id not in AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY["MIT/ast-finetuned-audioset-10-10-0.4593"]
        else:
            self.model_info = AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_audio_spectrogram_transformer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Audio-Spectrogram-Transformer models in the registry."""
    return list(AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Audio-Spectrogram-Transformer models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestAudioSpectrogramTransformerModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_audio_spectrogram_transformer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Audio-Spectrogram-Transformer-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Audio-Spectrogram-Transformer-family models:")
        for model in models:
            info = AUDIO_SPECTROGRAM_TRANSFORMER_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nAudio-Spectrogram-Transformer Models Testing Summary:")
        total = len(results)
