...
def generate_batch(missing_models, batch_size, num_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_model = {
            executor.submit(generate_test_for_model, model): model for model in batch
        }
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Number of tests to generate in a batch"
    )
    parser.add_argument(
        "--output-dir", type=str, default=OUTPUT_DIR, help="Output directory for generated tests"
    )
    parser.add_argument(
        "--temp-dir", type=str, default=TEMP_DIR, help="Temporary directory for generated files"
    )
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel workers")
    parser.add_argument("--all", action="store_true", help="Generate tests for all missing models")
    parser.add_argument(
        "--list-missing", action="store_true", help="List missing models without generating tests"
    )
    ...
    if args.all:
        ...
        for i in range(0, len(missing_models), args.batch_size):
            ...
            logger.info(
                f"Batch {i // args.batch_size + 1} results: {len(successful)} successful, {len(failed)} failed"
            )
