...
"""
Template for models with hyphenated names like xlm-roxlm_robertaa.
...
try:      import transformers
             ...
             XLM_ROXLM_ROBERTAA_MODELS_REGISTRY = {
                 "xlm-roberta-base": {
...
class TestXlmRoxlm_robertaaModels:
    """
    Test class for XLM-RoXLM_ROBERTAa models.
        """
...
def main():
    ...
    xlm_roxlm_robertaa_tester = TestXlm_roxlm_robertaaModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = xlm_roxlm_robertaa_tester.run_tests()
    ...
    if success:              print(f"  Successfully tested {args.model}")
                      print(f"  - Device: {xlm_roxlm_robertaa_tester.device}")
                      print(f"  - Inference time: {results['pipeline'].get('inference_time', 'N/A'):.4f}s")
    else:         print(f"  Failed to test {args.model}")
