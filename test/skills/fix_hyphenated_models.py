...
def fix_registry_references(content: str, original_name: str) -> str:
    ...
    content = re.sub(
        rf"{original_name.upper()}_MODELS_REGISTRY", f"{upper_case}_MODELS_REGISTRY", content
    )

    # Fix lowercase references
    content = re.sub(rf"{original_name.lower()}\.", f"{valid_id}.", content)

    # Fix lowercase references in strings
    content = re.sub(rf"hf_{original_name.lower()}_", f"hf_{valid_id}_", content)
...
def fix_method_references(content: str, original_name: str) -> str:
    ...
    content = re.sub(rf"test_{original_name.lower()}", f"test_{valid_id}", content)

    # Fix variable references
    content = re.sub(rf"{original_name.lower()}_tester", f"{valid_id}_tester", content)
...
def create_test_file(model_name: str, output_dir: Path) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.info(
            f"Processing {model_name} -> id: {valid_id}, class: {class_name}, upper: {upper_name}"
        )
        ...
        while not syntax_valid and attempts < max_attempts:
            ...
            if not syntax_valid:
                ...
                if "unterminated string literal" in error.lower():
                    ...
                    content = re.sub(
                        r'print\(["\'](.*?)$', r'print(f"\1")', content, flags=re.MULTILINE
                    )
        ...
        if not syntax_valid:
            logger.error(
                f"Failed to fix syntax errors after {max_attempts} attempts for {model_name}"
            )
...
def main():
    ...
    group.add_argument(
        "--all-hyphenated", action="store_true", help="Process all hyphenated model names"
    )
    group.add_argument("--list", action="store_true", help="List all known hyphenated model names")

    parser.add_argument(
        "--output-dir", type=Path, default=OUTPUT_DIR, help="Output directory for fixed test files"
    )
