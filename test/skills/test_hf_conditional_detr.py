...
class TestConditional_DetrModels:
    """Base test class for all CONDITIONAL_DETR-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "conditional-detr-base"
        
        # Verify model exists in registry
        if self.model_id not in conditional_detr_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = conditional_detr_MODELS_REGISTRY["conditional-detr-base"]
        else:
            self.model_info = conditional_detr_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_conditional_detr_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(conditional_detr_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_conditional_detr_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = conditional_detr_MODELS_REGISTRY[model]
