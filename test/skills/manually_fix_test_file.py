...
def fix_file(file_path, model_type=None):
    ...
    try:
        ...
        if re.search(constructor_pattern, content):
            content = re.sub(
                constructor_pattern,
                f"{model_valid}_tester = Test{model_capitalized}Models(",
                content,
            )
            logger.info(
                f"Fixed constructor to: {model_valid}_tester = Test{model_capitalized}Models("
            )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, help="Model type (extracted from filename if not provided)"
    )
