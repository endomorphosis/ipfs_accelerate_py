#!/usr/bin/env python3
"""
Class-based test file for all OPTIMIZED_MODEL-family models.
...
optimized_model_MODELS_REGISTRY = {
    "optimized-model-base": {
        "description": "OPTIMIZED_MODEL models",
        "class": "OptimizedModelModel"
    }

}

class TestOptimized_ModelModels:
    """Base test class for all OPTIMIZED_MODEL-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "optimized-model-base"
        
        # Verify model exists in registry
        if self.model_id not in optimized_model_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = optimized_model_MODELS_REGISTRY["optimized-model-base"]
        else:
            self.model_info = optimized_model_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_optimized_model_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available OPTIMIZED_MODEL models in the registry."""
    return list(optimized_model_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered OPTIMIZED_MODEL models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestOptimized_ModelModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_optimized_model_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test OPTIMIZED_MODEL-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable OPTIMIZED_MODEL-family models:")
        for model in models:
            info = optimized_model_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nOPTIMIZED_MODEL Models Testing Summary:")
        total = len(results)
