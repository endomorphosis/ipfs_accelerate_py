...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_mlp_mixer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_mlp_mixer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
