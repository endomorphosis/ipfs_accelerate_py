...
"""
Class-based test file for all ViLT-family models.
...
class TestViltModels:
    """Base test class for all ViLT-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "dandelin/vilt-b32-mlm"
        
        # Verify model exists in registry
        if self.model_id not in VILT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = VILT_MODELS_REGISTRY["dandelin/vilt-b32-mlm"]
        else:
            self.model_info = VILT_MODELS_REGISTRY[self.model_id]
        ...
        self.test_texts = [
            ...
            "What is shown in the image? (alternative)"
        ]
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "ViltForMaskedLM":
            model_class = transformers.ViltForMaskedLM
        else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_vilt_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available ViLT models in the registry."""
    return list(VILT_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered ViLT models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestViltModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_vilt_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ViLT-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable ViLT-family models:")
        for model in models:
            info = VILT_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "dandelin/vilt-b32-mlm"
    logger.info(f"Testing model: {model_id}")
