...
def create_hyphenated_test_file(model_name, output_dir=None):
    ...
    try:
        ...
        logger.info(
            f"Creating test file for {model_name} -> id: {model_id}, class: {class_name}, type: {arch_type}, task: {default_task}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create test files for models with hyphenated names"
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory for test files"
    )
