...
def run_generator(family, output_dir=PARENT_DIR, template=None):
    ...
    command = [sys.executable, TOOLS["generator"], "--family", family, "--output", output_dir]
    ...
    try:
        ...
        syntax_check = subprocess.run(
            [sys.executable, "-m", "py_compile", output_path], capture_output=True, text=True
        )
...
def run_test_suite():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, TOOLS["test_suite"]], capture_output=True, text=True
        )
...
def install_pre_commit():
    ...
    try:
        ...
        result = subprocess.run([TOOLS["install_script"]], capture_output=True, text=True)
...
def run_all_tests(families=None, cpu_only=True):
    ...
    logger.info(
        f"Total: {len(families)}, Successful: {success_count}, Failed: {len(families) - success_count}"
    )
