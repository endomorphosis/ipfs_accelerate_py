...
def generate_hardware_compatibility_data(vision_text_models):
    ...
    hardware_types = ["CUDA GPU", "Apple Silicon", "WebGPU", "CPU", "TPU", "AMD ROCm"]
    ...
    for model in vision_text_models:
        model_data = {"name": model, "hardware_compatibility": {}, "task_compatibility": {}}
        ...
        for hardware in hardware_types:
            ...
            compatibility = (
                "Full" if len(model) % 3 == 0 else "Partial" if len(model) % 3 == 1 else "Limited"
            )
        ...
        for task in tasks:
            ...
            compatibility = (
                "Supported"
                if (len(model) + len(task)) % 3 == 0
                else "Partial"
                if (len(model) + len(task)) % 3 == 1
                else "Unsupported"
            )
