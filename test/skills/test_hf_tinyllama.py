...
"""
Class-based test file for all TinyLlama-family models.
...
class TestTinyLlamaModels:
    """Base test class for all TinyLlama-family models."""
    ...
    def __init__(self, model_id=None):
        ...
        if self.model_id not in TINYLLAMA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = TINYLLAMA_MODELS_REGISTRY["TinyLlama/TinyLlama-1.1B-Chat-v1.0"]
        else:
            self.model_info = TINYLLAMA_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "What are the benefits of using smaller language models?"
        self.test_texts = [
            "What are the benefits of using smaller language models?",
            "What are the benefits of using smaller language models? (alternative)"
        ]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_tinyllama_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available TinyLlama models in the registry."""
    return list(TINYLLAMA_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered TinyLlama models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestTinyLlamaModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_tinyllama_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test TinyLlama-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable TinyLlama-family models:")
        for model in models:
            info = TINYLLAMA_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nTinyLlama Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
    logger.info(f"Testing model: {model_id}")
