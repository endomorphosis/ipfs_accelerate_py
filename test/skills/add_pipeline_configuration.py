...
class PipelineConfigurator:
    ...
    def _process_file(self, file_path: Path):
        ...
        test_pipeline_match = re.search(
            r'def\s+test_pipeline\s*\([^)]*\):\s*(?:""".*?""")?\s*(.*?)(?=\s*def|\s*$)',
            content,
            re.DOTALL,
        )
...
''',
    ...
    def _print_summary(self):
        ...
        for arch, stats in sorted(self.stats["by_architecture"].items()):
            ...
            logger.info(
                f"    - Already had pipeline: {arch_skipped} ({arch_skipped / arch_total * 100:.1f}%)"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add pipeline configurations to HuggingFace model test files"
    )
    parser.add_argument(
