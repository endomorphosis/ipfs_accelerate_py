...
def get_model_from_registry(
    model_type, task=None, hardware_profile=None, max_size_mb=None, framework=None
):
...
def fix_unterminated_triple_quotes(content):
    ...
    if triple_quote_count % 2 != 0:
        ...
        if in_docstring and docstring_start_line is not None:
            ...
            logger.info(
                f"Added missing closing triple quotes with matching indentation from line {docstring_start_line + 1}"
            )

    # Handle single triple quotes '''
    if single_triple_quote_count % 2 != 0:
        logger.info(
            f"Odd number of single triple quotes found: {single_triple_quote_count}, fixing..."
        )
        in_docstring = False
        docstring_start_line = None
        docstring_start_indent = 0
        ...
        if in_docstring and docstring_start_line is not None:
            ...
            logger.info(
                f"Added missing closing single triple quotes with matching indentation from line {docstring_start_line + 1}"
            )
    ...
    for line in lines:
        ...
        while i < len(line):
            ...
            if (
                ...
                and i + len(string_delimiter) <= len(line)
                and line[i : i + len(string_delimiter)] == string_delimiter
            ):
                in_string = False
            ...
            for token, replacement in replacements.items():
                if i + len(token) <= len(line) and line[i : i + len(token)] == token:
                    ...
                    next_char = (
                        line[i + len(token) : i + len(token) + 1]
                    ...
                    ) and (not prev_char or not (prev_char.isalnum() or prev_char == "_"))
    ...
    while i < len(lines):
        ...
        if line.strip() == "try:":
            ...
            if j < len(lines):
                next_line = lines[j]
                if (
                    not next_line.strip().startswith(("except", "finally"))
        ...
        # Handle except blocks immediately after try blocks
            ...
            if j < len(lines):
                next_line = lines[j]
                if (
                    not next_line.strip().startswith(("except", "finally", "try:"))
                    and len(next_line) - len(next_line.lstrip()) < 4
                ):
...
def final_cleanup(content):
    ...
    for i, line in enumerate(lines):
        ...
        if (
            i > 0
            and lines[i - 1].strip() == 'if __name__ == "__main__":'
            and line.strip() == "sys.exit(main())"
        ):
        ...
        if (
            line.strip().startswith("return 0 if success else 1")
            and i + 1 < len(lines)
    ...
    for match in model_tester_pattern.finditer(content):
        ...
        pascal_case = (
            get_pascal_case_identifier(model_type)
            if "get_pascal_case_identifier" in globals()
            else model_type.capitalize()
        )

        if model_type != referenced_type.lower():
            logger.info(
                f"Fixing model tester reference: {model_type}_tester = Test{referenced_type}Models -> Test{pascal_case}Models"
            )
            content = content.replace(
    ...
    for match in tester_ref_pattern.finditer(content):
        ...
        if model_type_ref and referenced_tester != model_type_ref.group(1):
            ...
            logger.info(
                f"Fixing tester device reference: {referenced_tester}_tester -> {model_type}_tester"
            )
            content = content.replace(
...
def fix_indentation_issues(content):
    ...
    while i < len(lines):
        ...
        # End of block detection
            ...
            in_except_block
            and indent <= try_indent
            and stripped
            and not stripped.startswith(("except ", "finally:"))
        ):
            in_except_block = False
        ...
        # Fix indentation in try/except blocks
            ...
            and indent < try_indent + 4
            and stripped
            and not stripped.startswith(("try:", "except ", "finally:"))
        ):
...
MODEL_REGISTRY = {
    ...
    "canine": {
        ...
        "inputs": {"text": "CANINE is a [MASK] language model for character-level understanding."},
    },
    "roformer": {
        ...
        "tasks": ["fill-mask", "token-classification", "text-classification"],
        "inputs": {"text": "RoFormer is a [MASK] language model with rotary position embeddings."},
    },
    "bart": {
        "family_name": "BART",
            ...
            if i < len(lines) and lines[i].strip().startswith('"""'):
                ...
                while i < len(lines) and in_docstring:
                    ...
                    if '"""' in current_line and not current_line.strip().startswith('"""'):
                        ...
                        in_docstring = False
                        logger.info(
                            f"Added missing closing quotes for class docstring before line {i + 1}"
                        )
                        ...
                        fixed_lines.append(
                            " " * (class_indent + 4)
                            + 'def __init__(self, model_id="default-model", device=None):'
                        )
    ...
    while i < len(lines):
        ...
        if i < len(lines) - 1:
            ...
            if current_line.endswith('"""') and next_line.startswith("def "):
                ...
                logger.info(
                    f"Inserted blank line between docstring and method definition at line {i + 1}"
                )
                ...
                if not lines[docstring_start].strip().endswith('"""'):
                    ...
                    if not found_end:
                        ...
                        if docstring_start + 1 < len(lines) and lines[docstring_start + 1].strip():
                            ...
                            logger.info(
                                f"Added missing closing quotes for method docstring after line {docstring_start + 1}"
                            )
                            ...
                            logger.info(
                                f"Added missing closing quotes for method docstring on line {docstring_start + 1}"
                            )
...
def get_template_for_architecture(model_type, templates_dir="templates"):
    ...
    if "-" in model_type:
        ...
        if os.path.exists(hyphenated_template):
            ...
            logger.warning(
                f"Hyphenated name template not found, will use architecture-specific template for {model_type}"
            )
    ...
    # For models with underscores, use the specialized underscore template
        ...
        if os.path.exists(underscore_template):
            ...
            logger.warning(
                f"Underscore name template not found, will use architecture-specific template for {model_type}"
            )
...
def generate_test_file(model_family, output_dir="."):
    ...
    try:
        ...
        model_pascal_case = get_pascal_case_identifier(
            model_family
        )  # PascalCase for class names (e.g., XlmRoberta)
        ...
        if "-" in model_family:
            ...
            for i, line in enumerate(lines):
                ...
                if (
                    "test_input =" in line
                    and "The quick brown fox jumps over the [MASK] dog." in line
                ):
                    if model_config.get("inputs") and "text" in model_config["inputs"]:
                        custom_input = model_config["inputs"]["text"]
                        lines[i] = line.replace(
                            "The quick brown fox jumps over the [MASK] dog.", custom_input
                        )
            ...
            if init_start is not None and init_end is not None:
                ...
                lines[init_start] = lines[init_start].replace(
                    'model_id="google-bert/bert-base-uncased"', f'model_id="{default_model}"'
                )
            ...
            for i, line in enumerate(lines):
                if 'parser.add_argument("--model"' in line and "default=" in line:
                    lines[i] = line.replace(
                        'default="google-bert/bert-base-uncased"', f'default="{default_model}"'
                    )

                # Fix description
                if "parser.add_argument" in line and "description=" in line:
                    if "Test BERT" in line:
                        lines[i] = line.replace(f"Test BERT", f"Test {model_pascal_case}")
                ...
                if "-" in model_family:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if f"{model_family.upper()}_MODELS_REGISTRY" in line:
                            lines[i] = line.replace(
                                f"{model_family.upper()}_MODELS_REGISTRY",
                                f"{model_upper}_MODELS_REGISTRY",
                            )
                        ...
                        if original_class_pattern in line:
                            lines[i] = line.replace(
                                original_class_pattern, f"Test{model_pascal_case}Models"
                            )
                ...
                try:
                    ...
                    if hasattr(final_e, "lineno") and final_e.lineno is not None:
                        ...
                        if 0 <= line_no < len(lines):
                            ...
                            logger.error(
                                f"Final problematic line {final_e.lineno}: {problematic_line}"
                            )
                            ...
                            if "unterminated" in str(final_e) and "string" in str(final_e):
                                ...
                                if '"""' in content:
                                    ...
                                    if len(triple_quote_lines) % 2 != 0:
                                        ...
                                        indent = len(lines[last_quote_line]) - len(
                                            lines[last_quote_line].lstrip()
                                        )
                                        lines.insert(last_quote_line + 1, " " * indent + '"""')
                                        logger.info(
                                            f"Added missing triple quotes after line {last_quote_line + 1}"
                                        )
                                elif "'''" in content:
                                    ...
                                    if len(single_triple_lines) % 2 != 0:
                                        ...
                                        lines.insert(last_quote_line + 1, " " * indent + "'''")
                                        logger.info(
                                            f"Added missing single triple quotes after line {last_quote_line + 1}"
                                        )
                                ...
                                if line_no < len(lines):
                                    ...
                                    logger.info(
                                        f"Last resort: Added triple quotes after line {line_no + 1}"
                                    )

                            elif "unexpected EOF" in str(final_e):
                                # Try to fix unterminated strings or missing closing brackets
                                for char in ")}]":
                                    if problematic_line.count(char) < problematic_line.count(
                                        {"(": ")", "{": "}", "[": "]"}[char]
                                    ):
                                        lines[line_no] = problematic_line + char
                                        logger.info(
                                            f"Added missing '{char}' to line {final_e.lineno}"
                                        )
                            ...
                            # Fix syntax error in docstring-method definition run-on
                                ...
                                if '"""def ' in problematic_line:
                                    ...
                                    if len(parts) >= 2:
                                        ...
                                        lines.insert(
                                            line_no + 1, "    " + method_def
                                        )  # Use 4 spaces for class method

                                        logger.info(
                                            f"Fixed docstring-method definition run-on on line {final_e.lineno}"
                                        )
                                elif problematic_line.strip().startswith(
                                    "class "
                                ) and line_no + 1 < len(lines):
                                    # Check if the class needs docstring closure
                                    if '"""' in lines[line_no + 1] and not lines[
                                        line_no + 1
                                    ].strip().endswith('"""'):
                                        # Close the docstring
                                        indent = len(lines[line_no + 1]) - len(
                                            lines[line_no + 1].lstrip()
                                        )
                                        lines.insert(line_no + 2, " " * indent + '"""')
                                        logger.info(
                                            f"Fixed unclosed class docstring after line {line_no + 2}"
                                        )
                            ...
                            try:
                                compile(fixed_content, output_file, "exec")
                                logger.info(
                                    f"✅ Syntax is valid after final line-specific fixes for {output_file}"
                                )
                            except Exception:
...
def list_model_families():
    ...
    for family in families:
        ...
        print(
            f"  - {family} ({config['family_name']}): {config['description']} [Architecture: {arch_type}]"
        )
    print()
    return families
...
def test_hyphenated_model_support(output_dir="fixed_tests", verify=True):
    ...
    logger.info(
        f"Hyphenated model test summary: {success_count}/{total} successful ({success_count / total * 100:.1f}%)"
    )
...
def main():
    ...
    group.add_argument(
        "--hyphenated-only",
        action="store_true",
        help="Generate tests only for models with hyphenated names",
    )
    group.add_argument(
        "--test-hyphenated", action="store_true", help="Test hyphenated model name handling"
    )
...
def fix_method_boundaries(content):
    ...
    content = re.sub(r"}\s*return results", r"}\n        return results", content)
...
def fix_mock_definitions(content):
    ...
    for indent, class_name in mock_classes:
        ...
        content = content.replace(
            f"{indent}class Mock{class_name}:", f"    class Mock{class_name}:"
        )
...
def fix_try_except_blocks(content):
    ...
    except_pattern = re.compile(
        r"(\s+)except.*?:(.*?)(?=\s+(?:try:|except|else:|finally:|def|$))", re.DOTALL
    )
    ...
    try_if_pattern = re.compile(r"try:\s*\n\s*if not HAS_\w+:\s*\n\s*if not HAS_\w+:", re.MULTILINE)
    content = re.sub(
        r"try:\s*\n\s*if not HAS_(\w+):\s*\n\s*if not HAS_\1:",
        r"try:\n            if not HAS_\1:",
        content,
...
def fix_if_blocks(content):
    ...
    if_pattern = re.compile(
        r"(\s+)if\s+.*?:(.*?)(?=\s+(?:elif|else:|try:|except|def|$))", re.DOTALL
    )
...
def fix_class_method_indentation(content):
    ...
    for class_match in class_matches:
        ...
        class_content_pattern = re.compile(
            r"class\s+" + class_name + r":(.*?)(?=class\s+\w+:|$)", re.DOTALL
        )
        ...
        content = (
            content[:class_start_pos]
            + "class "
            + class_name
            + ":"
            + class_content
            + content[class_content_match.end() :]
        )
    ...
    for func_match in re.finditer(r"def\s+(\w+)\s*\(", content):
        ...
        if func_name not in ["__init__", "test_pipeline", "test_from_pretrained", "run_tests"]:
            func_pattern = re.compile(
                r"def\s+" + func_name + r"\s*\(.*?\):(.*?)(?=def\s+\w+\s*\(|$)", re.DOTALL
            )
            func_match = func_pattern.search(content, func_match.start())
            if func_match:
                func_text = func_match.group(0)
                fixed_func = apply_indentation(
                    func_text, 0
                )  # Top-level functions have 0 indentation
