                ...
                try:
                    ...
                    try:
                        ...
                        return {
                            ...
                            if not prompt in generated_text
                            else generated_text,
...
class test_hf_stablelm:
    def __init__(self, resources=None, metadata=None):
        ...
        self.resources = (
            resources
            if resources
            else {"torch": torch, "numpy": np, "transformers": transformers_module}
        )
        self.metadata = metadata if metadata else {}
                ...
                try:
                    ...
                    if self.model_name == self.primary_model:
                        ...
                        if os.path.exists(cache_dir):
                            ...
                            stablelm_models = [
                                name for name in os.listdir(cache_dir) if "stablelm" in name.lower()
                            ]
                            ...
                            if stablelm_models:
                                ...
                                print(
                                    "No suitable StableLM models found in cache, using test model"
                                )
    ...
    def _create_test_model(self):
        ...
        try:
            ...
            tokenizer_json = {
                ...
                "normalizer": {
                    "type": "Sequence",
                    "normalizers": [{"type": "Lowercase", "lowercase": []}],
                },
                "pre_tokenizer": {
                    "type": "Sequence",
            ...
            if not isinstance(torch, MagicMock) and hasattr(torch, "save"):
                ...
                for layer_idx in range(num_layers):
                    ...
                    # Self-attention
                    model_state[f"{layer_prefix}.attention.query_key_value.weight"] = torch.randn(
                        3 * hidden_size, hidden_size
                    )
                    model_state[f"{layer_prefix}.attention.query_key_value.bias"] = torch.randn(
                        3 * hidden_size
                    )
                    model_state[f"{layer_prefix}.attention.dense.weight"] = torch.randn(
                    ...
                    model_state[f"{layer_prefix}.mlp.dense_h_to_4h.weight"] = torch.randn(
                        intermediate_size, hidden_size
                    )
                    model_state[f"{layer_prefix}.mlp.dense_h_to_4h.bias"] = torch.randn(
                        intermediate_size
                    )
                    model_state[f"{layer_prefix}.mlp.dense_4h_to_h.weight"] = torch.randn(
                        hidden_size, intermediate_size
                    )
                    model_state[f"{layer_prefix}.mlp.dense_4h_to_h.bias"] = torch.randn(hidden_size)

                    # Post-attention layernorm
                    model_state[f"{layer_prefix}.post_attention_layernorm.weight"] = torch.ones(
                        hidden_size
                    )
                    model_state[f"{layer_prefix}.post_attention_layernorm.bias"] = torch.zeros(
                        hidden_size
                    )
        ...
        try:
            ...
            try:
                ...
                if transformers_available:
                    ...
                    valid_init = (
                        endpoint is not None and tokenizer is not None and handler is not None
                    )
                    results["cpu_init"] = (
                        "Success (REAL)" if valid_init else "Failed CPU initialization"
                    )
                    ...
                    if valid_init:
                        ...
                        results["cpu_handler"] = (
                            "Success (REAL)" if output is not None else "Failed CPU handler"
                        )
                ...
                with (
                    ...
                    mock_tokenizer.return_value.decode = MagicMock(
                        return_value="Once upon a time in a land far away, there lived a brave knight who protected the kingdom from dragons and other mythical creatures."
                    )
                    ...
                    results["cpu_init"] = (
                        "Success (MOCK)" if valid_init else "Failed CPU initialization"
                    )
                    ...
                    results["cpu_handler"] = (
                        "Success (MOCK)" if output is not None else "Failed CPU handler"
                    )
                    ...
                    self.examples.append(
                        {
                            "input": self.test_prompt,
                            "output": {"generated_text": mock_text},
                            "timestamp": datetime.datetime.now().isoformat(),
                            "elapsed_time": elapsed_time,
                            "implementation_type": "MOCK",
                            "platform": "CPU",
        ...
        if cuda_available:
            try:
                ...
                endpoint, tokenizer, handler, queue, batch_size = self.lm.init_cuda_stablelm(
                    self.model_name, "text-generation", "cuda:0"
                )
                ...
                if valid_init:
                    ...
                    results["cuda_handler"] = (
                        "Success" if output is not None else "Failed CUDA handler"
                    )
                ...
                if valid_init:
                    try:
                        ...
                        params_output = handler(
                            self.test_prompt, max_new_tokens=50, temperature=0.9, top_p=0.95
                        )

                        if (
                            params_output is not None
                            and isinstance(params_output, dict)
                            and "generated_text" in params_output
                        ):
                            ...
                            self.examples.append(
                                {
                                    ...
                                    "output": {
                                        "generated_text": params_output["generated_text"][:300]
                                        + "..."
                                        if len(params_output["generated_text"]) > 300
                                        else params_output["generated_text"],
                                        "parameters": {
                                            "max_new_tokens": 50,
                                            "temperature": 0.9,
                                            "top_p": 0.95,
                                    ...
                                    "timestamp": datetime.datetime.now().isoformat(),
                                    "implementation_type": params_output.get(
                        ...
                        if isinstance(batch_output, list):
                            ...
                            if batch_output and len(batch_output) > 0:
                                ...
                                if (
                                    ...
                                    and "generated_text" in first_result
                                ):
                                ...
                                self.examples.append(
                                    {
                                        ...
                                        "output": {
                                            "first_result": generated_text[:150] + "..."
                                            if len(generated_text) > 150
                                            else generated_text,
        ...
        try:
            ...
            if not has_openvino:
                ...
                try:
                    ...
                    results["openvino_init"] = (
                        "Success" if valid_init else "Failed OpenVINO initialization"
                    )
                    ...
                    if valid_init:
                        ...
                        results["openvino_handler"] = (
                            "Success" if output is not None else "Failed OpenVINO handler"
                        )

                        # Check output structure and store sample output
                        if output is not None and isinstance(output, dict):
                            results["openvino_output"] = (
                                "Valid" if "generated_text" in output else "Missing generated_text"
                            )
                                ...
                                self.examples.append(
                                    {
                                        ...
                                        "timestamp": datetime.datetime.now().isoformat(),
                                        "elapsed_time": elapsed_time,
                                        "implementation_type": output.get(
                                            "implementation_type", "Unknown"
                                        ),
                                        "platform": "OpenVINO",
                                    }
