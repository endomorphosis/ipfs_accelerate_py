...
def run_tests_parallel(test_files, workers=4, timeout=600):
    ...
    def run_test(test_file):
        ...
        try:
            ...
            return {
                "test_name": test_name,
                "model_name": os.path.basename(test_file)
                .replace("test_hf_", "")
                .replace(".py", ""),
                "success": False,
                "error": str(e),
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if result["success"]:
                    print(
                        f"✅ Completed: {result['test_name']} in {result.get('execution_time', 0):.2f}s"
                    )
                else:
                    print(
                        f"❌ Failed: {result['test_name']} - {result.get('error', 'Unknown error')}"
                    )
...
def generate_dashboard_data(results, output_dir="distributed_results"):
    ...
    with open(summary_file, "w") as f:
        ...
        if successful_tests:
            execution_times = [
                r.get("execution_time", 0) for r in successful_tests if "execution_time" in r
            ]
            if execution_times:
                ...
                f.write(
                    f"**Slowest Test**: {slowest_test['test_name']} ({slowest_test.get('execution_time', 0):.2f}s)\n"
                )
                f.write(
                    f"**Fastest Test**: {fastest_test['test_name']} ({fastest_test.get('execution_time', 0):.2f}s)\n\n"
                )
        ...
        sorted_tests = sorted(
            successful_tests, key=lambda r: r.get("execution_time", 0), reverse=True
        )
...
def run_distributed_tests(
    model_family=None, workers=4, timeout=600, all_models=False, test_dir="fixed_tests"
):
...
def main():
    ...
    model_group.add_argument(
        "--model-family", type=str, help="Model family to test (e.g., bert, gpt2)"
    )
    model_group.add_argument("--all", action="store_true", help="Test all available models")

    # Distributed testing args
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of workers for distributed testing"
    )
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds for each test")
    parser.add_argument(
        "--test-dir", type=str, default="fixed_tests", help="Directory containing test files"
    )

    # Framework management
    parser.add_argument(
        "--update-tests", action="store_true", help="Update test files for distributed testing"
    )
