...
# Import the module to test - Mamba will likely use the hf_lm module or a specific mamba module
from ipfs_accelerate_py.worker.skillset.hf_lm import hf_lm

# Add CUDA support to the Mamba class
def init_cuda(self, model_name, model_type, device_label="cuda:0"):
    """Initialize Mamba model with CUDA support.
    ...
    try:
        ...
        try:
            ...
            # Load model
            try:
                # Mamba models may require different loading strategies
                try:
                    # Try loading with standard approach first
                    model = AutoModelForCausalLM.from_pretrained(model_name)
                    print(f"Successfully loaded model {model_name}")
                except Exception as standard_err:
                    print(f"Standard model loading failed: {standard_err}")
                    try:
                        # Special handling for specific Mamba models
                        if "mamba" in model_name.lower():
                            # For state-space models, use half precision by default to save memory
                            model = AutoModelForCausalLM.from_pretrained(
                                model_name,
                                torch_dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
                            )
                            print(f"Successfully loaded model {model_name} with half precision")
                        else:
                            raise
                    except Exception as special_err:
                        print(f"Special loading also failed: {special_err}")
                        # Last resort - fall back to default loading
                        model = AutoModelForCausalLM.from_pretrained(model_name)
            ...
            def handler(prompt, max_new_tokens=100, temperature=0.7, top_p=0.9, top_k=50, **kwargs):
                ...
                try:
                    ...
                    if isinstance(model, mock.MagicMock) or isinstance(tokenizer, mock.MagicMock):
                        print("Using mock handler for CUDA Mamba")
        ...
        print(f"Error in Mamba init_cuda: {e}")
        import traceback
        traceback.print_exc()
...
def create_cuda_mamba_endpoint_handler(self, tokenizer, model_name, cuda_label, endpoint=None):
    """Create handler function for CUDA-accelerated Mamba.
...
hf_lm.create_cuda_mamba_endpoint_handler = create_cuda_mamba_endpoint_handler

class test_hf_mamba:
    def __init__(self, resources=None, metadata=None):
        """
        Initialize the Mamba test class.
        ...
        # Try various Mamba models from smallest to largest
        self.primary_model = "state-spaces/mamba-2.8b-slimpj"
        
        # Alternative models in increasing size order
        self.alternative_models = [
            "state-spaces/mamba-2.8b",               # Standard Mamba 2.8B
            "state-spaces/mamba-1.4b",               # Smaller Mamba 1.4B
            "state-spaces/mamba-790m",               # Very small Mamba model
            "state-spaces/mamba-370m",               # Tiny Mamba model
            "state-spaces/mamba-130m",               # Extra-tiny Mamba 
            "HuggingFaceM4/idefics2-8b-mamba",       # Multimodal Mamba version
            "EleutherAI/gpt-neo-125m",               # Non-Mamba fallback, small model
            "facebook/opt-125m",                     # Non-Mamba tiny fallback
            "TinyLlama/TinyLlama-1.1B-Chat-v1.0"     # Non-Mamba chat fallback
        ]
                ...
                try:
                    ...
                    if self.model_name == self.primary_model:
                        ...
                        if os.path.exists(cache_dir):
                            # Look for any Mamba model in cache
                            mamba_models = [name for name in os.listdir(cache_dir) if any(
                                x in name.lower() for x in ["mamba", "state--spaces"])]
                            
                            if mamba_models:
                                # Use the first model found
                                mamba_model_name = mamba_models[0].replace("--", "/")
                                print(f"Found local Mamba model: {mamba_model_name}")
                                self.model_name = mamba_model_name
                            else:
        ...
        self.test_prompt = "Explain the advantages of state space models like Mamba compared to Transformers."
    ...
    def _create_test_model(self):
        ...
        """
        try:
            print("Creating local test model for Mamba testing...")
            
            # Create model directory in /tmp for tests
            test_model_dir = os.path.join("/tmp", "mamba_test_model")
            os.makedirs(test_model_dir, exist_ok=True)
            
            # Create a minimal config file for a tiny Mamba-style model
            config = {
                ...
                "pad_token_id": 0,
                "rms_norm_eps": 1e-05,
                "state_size": 16,           # Mamba-specific parameter
                "conv_kernel": 4,           # Mamba-specific parameter
                "expand": 2,                # Mamba-specific parameter
            ...
            if hasattr(torch, "save") and not isinstance(torch, MagicMock):
                ...
                for layer_idx in range(num_layers):
                    ...
                    # SSM parameters specific to Mamba
                    model_state[f"{layer_prefix}.mixer.A_log"] = torch.randn(hidden_size * expand, state_size)
                    model_state[f"{layer_prefix}.mixer.D"] = torch.randn(hidden_size * expand)
                    model_state[f"{layer_prefix}.mixer.in_proj.weight"] = torch.randn(hidden_size * expand, hidden_size)
                    model_state[f"{layer_prefix}.mixer.out_proj.weight"] = torch.randn(hidden_size, hidden_size * expand)
                    model_state[f"{layer_prefix}.mixer.conv1d.weight"] = torch.randn(hidden_size * expand, 1, config["conv_kernel"])
                    
                    # Additional SSM specific parameters
                    model_state[f"{layer_prefix}.mixer.dt_proj.weight"] = torch.randn(hidden_size * expand, hidden_size)
    ...
    def test(self):
        """
        Run all tests for the Mamba language model, organized by hardware platform.
        ...
        try:
            ...
            try:
                ...
                with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                    ...
                    mock_tokenizer.return_value.batch_decode = MagicMock(return_value=["State space models like Mamba..."])
                    ...
                    # Record example
                    mock_text = "State space models like Mamba offer several advantages over Transformers. First, they have linear scaling in sequence length (O(n)) compared to the quadratic scaling (O(n²)) of Transformers, allowing them to process much longer sequences efficiently. Second, they use a recurrent mechanism that maintains state through the sequence, which can better capture long-range dependencies while using less memory. Third, Mamba models often achieve faster inference time while maintaining competitive accuracy. Finally, their selective state space mechanisms allow them to dynamically adjust which information to retain, enabling more efficient processing of diverse inputs."
                    self.examples.append({
                        "input": self.test_prompt,
                        "output": {
            ...
            try:
                ...
                try:
                        ...
                        if valid_init:
                            ...
                            try:
                                ...
                                if hasattr(test_utils, 'enhance_cuda_implementation_detection'):
                                    ...
                                    print("Enhancing Mamba CUDA handler with implementation markers")
                            ...
                            if output is not None:
                                ...
                                try:
                                    ...
                                    batch_prompts = [self.test_prompt, "What makes Mamba models more efficient for longer sequences?"]
                    ...
                    with patch('transformers.AutoConfig.from_pretrained') as mock_config, \
                        ...
                        mock_model.return_value = MagicMock()
                        mock_model.return_value.generate.return_value = torch.tensor([[1, 2, 3]])
                        mock_tokenizer.batch_decode.return_value = ["State space models like Mamba have several advantages over Transformers..."]
                        ...
                        test_handler = self.lm.create_cuda_mamba_endpoint_handler(
                            tokenizer,
                            self.model_name,
                            "cuda:0",
                            endpoint,
                        ...
                        if isinstance(output, dict) and "text" in output:
                            ...
                            mock_text = "State space models like Mamba have several advantages over Transformers: 1) Linear scaling with sequence length (O(n)) instead of quadratic (O(n²)) allowing processing of much longer sequences; 2) Recurrent state mechanism that can efficiently model long-range dependencies; 3) Much faster inference speed while maintaining comparable accuracy; 4) Lower memory requirements for the same performance level; 5) Selective state space mechanism that dynamically focuses on important information."
                            implementation_type = "MOCK"
                        ...
                        try:
                            ...
                            if batch_output is not None and isinstance(batch_output, list):
                                ...
                                self.examples.append({
                                    "input": f"Batch of {len(batch_prompts)} prompts",
                                    "output": {
                                        "first_result": "State space models like Mamba have several advantages...",
        ...
        # ====== OPENVINO TESTS ======
        try:
            print("Testing Mamba on OpenVINO...")
    ...
    def __test__(self):
        ...
        collected_file = os.path.join(collected_dir, 'hf_mamba_test_results.json')
        with open(collected_file, 'w') as f:
            json.dump(test_results, f, indent=2)
            print(f"Saved results to {collected_file}")
            
        # Compare with expected results if they exist
        expected_file = os.path.join(expected_dir, 'hf_mamba_test_results.json')
...
if __name__ == "__main__":
    try:
        print("Starting Mamba test...")
        this_mamba = test_hf_mamba()
        results = this_mamba.__test__()
        print("Mamba test completed")
        ...
        print("MAMBA TEST RESULTS SUMMARY")
