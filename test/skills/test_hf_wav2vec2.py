#!/usr/bin/env python3
"""
Class-based test file for all Wav2Vec2-family models.
...
class TestWav2Vec2Models:
    """Base test class for all Wav2Vec2-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "facebook/wav2vec2-base"
        
        # Verify model exists in registry
        if self.model_id not in WAV2VEC2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = WAV2VEC2_MODELS_REGISTRY["facebook/wav2vec2-base"]
        else:
...
def test_from_pretrained(self, device="auto"):
    ...
    try:
        ...
        model_class = None
        if self.class_name == "Wav2Vec2ForCTC":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_wav2vec2_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Wav2Vec2 models in the registry."""
    return list(WAV2VEC2_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Wav2Vec2 models."""
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_wav2vec2_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Wav2Vec2-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Wav2Vec2-family models:")
        for model in models:
            info = WAV2VEC2_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nWav2Vec2 Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "facebook/wav2vec2-base"
    logger.info(f"Testing model: {model_id}")
