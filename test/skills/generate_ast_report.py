...
class TestFileAnalyzer:
        ...
        for file_path in test_files:
            ...
            if tree:
                ...
                self.test_structure_patterns.append(
                    {"file": file_path, "fingerprint": structure_fingerprint}
                )
        ...
        for fingerprint, files in fingerprint_map.items():
            if len(files) > 1:
                self.duplicate_structure_candidates.append(
                    {"fingerprint": fingerprint, "files": files, "count": len(files)}
                )
    ...
    def generate_report(self, file_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        common_patterns = sorted(self.method_patterns.items(), key=lambda x: x[1], reverse=True)[
            :20
        ]
        ...
        complex_files = sorted(self.file_complexities.items(), key=lambda x: x[1], reverse=True)[
            :20
        ]
                ...
                for cls in analysis["classes"]:
                    for method in cls["methods"]:
                        if (
                            ...
                            and "from_pretrained" in method["signature"]
                        ):
        ...
        report = {
            ...
            "complexity": {"complex_files": complex_files},
    ...
    def run_analysis(
        self,
        output_file: str = "ast_analysis_report.json",
        markdown_file: str = "ast_analysis_report.md",
    ):
    ...
    def generate_markdown_report(self, report: Dict[str, Any], output_file: str):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            for impl_type, count in impl_types.items():
                ...
                f.write(
                    f"| {impl_type.replace('_', ' ').title()} | {count} | {percentage:.1f}% |\n"
                )
            ...
            if report["potential_duplicates"]:
                f.write(
                    f"Found {len(report['potential_duplicates'])} groups of potentially similar files:\n\n"
                )
            ...
            if report["common_patterns"]["method_patterns"]:
                ...
                f.write(
                    f"1. **Standardize Method Implementations**: The pattern `{pattern}` appears in {count} methods ({coverage:.1f}% of all methods). Consider standardizing this pattern across the codebase.\n\n"
                )
            ...
            if report["potential_duplicates"]:
                ...
                f.write(
                    f"2. **Refactor Similar Files**: The largest group contains {top_dup['count']} similar files. Consider creating a common base class or utility functions.\n\n"
                )

            # from_pretrained standardization
            f.write(
                f"3. **Standardize from_pretrained Testing**: Currently using {len([t for t, c in impl_types.items() if c > 0])} different approaches for testing from_pretrained(). Consider standardizing to the explicit method pattern.\n\n"
            )
            ...
            if report["complexity"]["complex_files"]:
                ...
                f.write(
                    f"4. **Simplify Complex Files**: The most complex file `{path}` has a complexity score of {complexity}, which is {complexity / summary['avg_complexity']:.1f}x the average. Consider breaking it into smaller components.\n\n"
                )

            # Template-based generation
            f.write(
                f"5. **Template-Based Generation**: Given the structural similarities, use template-based generation for all test files to ensure consistency.\n\n"
            )

            # Conclusion
            f.write(f"## Conclusion\n\n")
            f.write(
                f"The test codebase shows clear patterns that can be leveraged for standardization. While there is some duplication, it appears to be structured and intentional based on the test generator approach. The main opportunity is to standardize the from_pretrained() testing methodology across all model types.\n\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="ast_analysis_report.json",
        help="Output JSON file (default: ast_analysis_report.json)",
    )
    parser.add_argument(
        "--markdown",
        default="ast_analysis_report.md",
        help="Output Markdown file (default: ast_analysis_report.md)",
    )
    parser.add_argument(
        "--exclude", nargs="+", default=None, help="Patterns to exclude from analysis"
    )
