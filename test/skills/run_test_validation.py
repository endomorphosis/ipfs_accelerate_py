...
class TestRunner:
    ...
    def __init__(
        self, directory: str, max_tests: int = 10, use_mocks: bool = True, verbose: bool = False
    ):
    ...
    def _get_model_architecture(self, model_name: str) -> str:
        ...
        if (
            "bert" in model_name_lower
            or "roberta" in model_name_lower
            or "electra" in model_name_lower
        ):
            return "encoder-only"
        elif (
            "gpt" in model_name_lower or "llama" in model_name_lower or "bloom" in model_name_lower
        ):
            return "decoder-only"
        elif (
            "t5" in model_name_lower or "bart" in model_name_lower or "pegasus" in model_name_lower
        ):
    ...
    def _generate_report(self):
        ...
        failed_tests = [
            (path, result)
            for path, result in self.stats["test_results"].items()
            if not result["success"]
        ]

        if failed_tests:
            report.extend(["## Failed Tests", "", "The following tests failed:", ""])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run functional tests on HuggingFace model test files"
    )
    parser.add_argument(
        "--directory", "-d", type=str, default="fixed_tests", help="Directory containing test files"
    )
    parser.add_argument(
        "--max-tests", "-m", type=int, default=10, help="Maximum number of tests to run (0 for all)"
    )
    parser.add_argument(
        "--use-mocks",
        "-M",
