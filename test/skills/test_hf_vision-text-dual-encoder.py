...
"""
Test file for VisionTextDualEncoder models.
This file tests the VisionTextDualEncoder model type from HuggingFace Transformers.
"""
...
# Registry for VisionTextDualEncoder models
VISION_TEXT_DUAL_ENCODER_MODELS_REGISTRY = {
    "openai/clip-vit-base-patch32": {
        "full_name": "VisionTextDualEncoder Base",
        "architecture": "vision-text",
        "description": "VisionTextDualEncoder model for text generation",
...
class TestVisionTextDualEncoderModels:
    """
    Test class for VisionTextDualEncoder models.
    """

    def __init__(self, model_id="openai/clip-vit-base-patch32", device=None):
        """Initialize the test class for VisionTextDualEncoder models.

        Args:
            model_id: The model ID to test (default: "openai/clip-vit-base-patch32")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing VisionTextDualEncoder model {self.model_id} with pipeline API on {self.device}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test VisionTextDualEncoder HuggingFace models")
    parser.add_argument(
        "--model", type=str, default="openai/clip-vit-base-patch32", help="Model ID to test"
    )
    ...
    vision_text_dual_encoder_tester = TestVisionTextDualEncoderModels(
        model_id=args.model, device=args.device
    )

    # Run the tests
    results = vision_text_dual_encoder_tester.run_tests()
    ...
    if success:
        ...
        print(f"  - Device: {vision_text_dual_encoder_tester.device}")
