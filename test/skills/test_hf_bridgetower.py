...
"""
Class-based test file for all BRIDGETOWER-family models.
...
bridgetower_MODELS_REGISTRY = {
    "bridgetower-base": {
        "description": "BRIDGETOWER models",
        "class": "BridgetowerModel"
    }

}

class TestBridgetowerModels:
    """Base test class for all BRIDGETOWER-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "bridgetower-base"
        
        # Verify model exists in registry
        if self.model_id not in bridgetower_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = bridgetower_MODELS_REGISTRY["bridgetower-base"]
        else:
            self.model_info = bridgetower_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_bridgetower_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available BRIDGETOWER models in the registry."""
    return list(bridgetower_MODELS_REGISTRY.keys())

def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered BRIDGETOWER models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestBridgetowerModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_bridgetower_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test BRIDGETOWER-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable BRIDGETOWER-family models:")
        for model in models:
            info = bridgetower_MODELS_REGISTRY[model]
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nBRIDGETOWER Models Testing Summary:")
        total = len(results)
