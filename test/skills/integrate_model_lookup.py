...
"""
        ...
        import_section_end = generator_code.find(
...
def update_model_registry_from_json(registry_file=None, generator_file=None, dry_run=False):
    ...
    try:
        ...
        for model_type, data in registry_data.items():
            if model_type in updated_registry:
                ...
                if old_default != new_default and new_default:
                    logger.info(
                        f"Updating {model_type} default model: {old_default} -> {new_default}"
                    )
        ...
        if not dry_run:
            ...
            updated_code = (
                generator_code[:registry_start]
                + updated_registry_str
                + generator_code[registry_end + 1 :]
            )
...
def update_all_models_in_registry(dry_run=False):
    ...
    try:
        ...
        test_generator = load_module_from_path(
            "test_generator_fixed", CURRENT_DIR / "test_generator_fixed.py"
        )
        if not test_generator:
        ...
        for model_type in model_types:
            ...
            try:
                ...
                if popular_models:
                    ...
                    logger.info(
                        f"Updated registry for {model_type} with default model: {default_model}"
                    )
                    success_count += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Integrate HuggingFace model lookup with test generator"
    )
    parser.add_argument(
        "--update-all", action="store_true", help="Update all model types in the registry"
    )
    ...
    if args.update_all:
        ...
        try:
            ...
            if popular_models:
                ...
                logger.info(
                    f"Updated registry for {args.model} with default model: {default_model}"
                )
