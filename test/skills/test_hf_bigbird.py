...
BIGBIRD_MODELS_REGISTRY = {
    "bigbird-base-uncased": {
        "full_name": "BIGBIRD Base Uncased",
        "architecture": "encoder-only",
        "description": "BIGBIRD Base model with uncased vocabulary",
        "bigbird": "bigbird",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "bigbird-large-uncased": {
        "full_name": "BIGBIRD Large Uncased",
        "architecture": "encoder-only",
        "description": "BIGBIRD Large model with uncased vocabulary",
        "bigbird": "bigbird",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "bigbird-base-cased": {
        "full_name": "BIGBIRD Base Cased",
        "architecture": "encoder-only",
        "description": "BIGBIRD Base model with cased vocabulary",
        "bigbird": "bigbird",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="bigbird-base-uncased", device=None):
        """Initialize the test class for BIGBIRD models.

        Args:
            model_id: The model ID to test (default: "bigbird-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing BIGBIRD model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing BIGBIRD model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing BIGBIRD model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_bigbird_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test BIGBIRD HuggingFace models")
    parser.add_argument(
        "--model", type=str, default="bigbird-base-uncased", help="Model ID to test"
    )
    ...
    parser.add_argument("--list-models", action="store_true", help="List available BIGBIRD models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable BIGBIRD models:")
        for model_id, info in BIGBIRD_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested BIGBIRD model: {args.model}")
