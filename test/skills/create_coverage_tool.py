...
class ModelCoverageTool:
    ...
    def _load_model_registry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Model registry not found at {MODEL_REGISTRY_PATH}. Using empty registry."
            )
            return {}
    ...
    def get_model_defaults(self, model_type: str) -> Dict[str, Any]:
        ...
        return {"default_model": f"{model_type}-base", "models": [f"{model_type}-base"]}
    ...
    def get_architecture_type(self, model_type: str) -> str:
        ...
        logger.warning(
            f"Could not determine architecture type for {model_type}. Defaulting to encoder-only."
        )
        return "encoder-only"
            ...
            if (
                ...
                if '"""def ' in lines[i]:
                    ...
                    if len(parts) >= 2:
                        ...
                        logger.info(
                            f"Fixed docstring and method definition combined on line {i + 1}"
                        )
    ...
    def generate_test_file(self, model_type: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            if 'if __name__ == "__main__":' in content:
                ...
                content = content.replace("        sys.exit(main())", "    sys.exit(main())")
...
def main():
    ...
    # Optional arguments
    parser.add_argument(
        "--output-dir", type=str, default=str(OUTPUT_DIR), help="Output directory for test files"
    )
    parser.add_argument(
        "--temp-dir",
        type=str,
        default="/tmp/model_tests",
