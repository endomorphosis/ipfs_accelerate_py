...
XLM-ROBERTA_MODELS_REGISTRY = {
    "xlm-roberta-base-uncased": {
...
class TestXlm-roxlm-robertaaModels:
    def __init__(self, model_id="xlm-roberta-base-uncased", device=None):
        """Initialize the test class for BERT models.
        
        Args:
            model_id: The model ID to test (default: "xlm-roberta-base-uncased")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_xlm-roberta_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="xlm-roberta-base-uncased", help="Model ID to test")
    ...
    if args.list_models:
        ...
        for model_id, info in XLM-ROBERTA_MODELS_REGISTRY.items():
    ...
    xlm-roberta_tester = TestXlm-roxlm-robertaaModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = xlm-roberta_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {xlm-roberta_tester.device}")
