...
class TestT5Models:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "t5"

        # Define model parameters
        self.task = "text2text-generation"
        self.class_name = "T5ForConditionalGeneration"
        self.description = "Test for T5 models"
    ...
    def get_model_class(self):
        ...
        if self.class_name == "T5ForConditionalGeneration":
            return transformers.T5ForConditionalGeneration
        else:
    ...
    def process_model_output(self, output, tokenizer):
        ...
        if self.task == "text-generation":
            ...
            mask_token_index = torch.where(
                torch.tensor(tokenizer.encode(self.test_text)) == tokenizer.mask_token_id
            )[0]
            if len(mask_token_index) > 0:
                ...
                return [
                    {"token": tokenizer.decode([token_id.item()]), "score": float(score.item())}
            ...
            return [
                {
                    "generated_text": tokenizer.decode(
                        output.logits.argmax(dim=-1)[0], skip_special_tokens=True
                    )
                }
            ]
