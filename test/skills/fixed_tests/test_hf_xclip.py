...
class TestVisionTextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.xclip = self.model_info.get("type", "blip")  # Default to blip if not specified
    ...
    def _create_video_frames(self, num_frames=8):
    ...
    def test_xclip_video(self, device="auto"):
        """Test the XClip model specifically for video classification."""
        if device == "auto":
        ...
        if not HAS_TRANSFORMERS:
            results["xclip_video_error_type"] = "missing_dependency"
            results["xclip_video_missing_core"] = ["transformers"]
            results["xclip_video_success"] = False
            return results
            
        if not HAS_TORCH:
            results["xclip_video_error_type"] = "missing_dependency"
            results["xclip_video_missing_core"] = ["torch"]
            results["xclip_video_success"] = False
            return results
            
        if not HAS_PIL:
            results["xclip_video_error_type"] = "missing_dependency"
            results["xclip_video_missing_deps"] = ["Pillow"]
            results["xclip_video_success"] = False
            return results
        
        try:
            logger.info(f"Testing {self.model_id} (XClip) with video input on {device}...")
            
            # Create video frames
            video_frames = self._create_video_frames(num_frames=16)
            if not video_frames:
                raise ValueError("Failed to create video frames")
            
            # For XClip, use XClipProcessor and XClipForVideoClassification
            processor_load_start = time.time()
            processor = transformers.XClipProcessor.from_pretrained(self.model_id)
            processor_load_time = time.time() - processor_load_start
            
            # Load model
            model_load_start = time.time()
            model = transformers.XClipForVideoClassification.from_pretrained(self.model_id)
            ...
            if hasattr(model.config, "id2label"):
                ...
                class_label = model.config.id2label.get(predicted_class_idx, f"Class {predicted_class_idx}")
                predictions = {
            ...
            # Store results
            results["xclip_video_success"] = True
            results["xclip_video_avg_time"] = avg_time
            results["xclip_video_min_time"] = min_time
            results["xclip_video_max_time"] = max_time
            results["xclip_video_load_time"] = model_load_time
            results["processor_load_time"] = processor_load_time
            results["model_size_mb"] = model_size_mb
            results["predictions"] = predictions
            results["xclip_video_error_type"] = "none"
            
            # Add to examples
            self.examples.append({
                "method": f"XClip video classification on {device}",
                "input": f"Video with {len(video_frames)} frames",
                "prediction": predictions
            })
            
            # Store performance stats
            self.performance_stats[f"xclip_video_{device}"] = {
                "avg_time": avg_time,
                "min_time": min_time,
                "max_time": max_time,
            ...
            results["xclip_video_success"] = False
            results["xclip_video_error"] = str(e)
            results["xclip_video_traceback"] = traceback.format_exc()
            logger.error(f"Error testing XClip video on {device}: {e}")
            ...
            if "cuda" in error_str or "cuda" in traceback_str:
                results["xclip_video_error_type"] = "cuda_error"
            elif "memory" in error_str:
                results["xclip_video_error_type"] = "out_of_memory"
            elif "no module named" in error_str:
                results["xclip_video_error_type"] = "missing_dependency"
            elif "xclipprocessor" in error_str.lower() or "xclipforvideoclassification" in error_str.lower():
                results["xclip_video_error_type"] = "model_class_not_found"
            else:
                results["xclip_video_error_type"] = "other"
        
        # Add to overall results
        self.results[f"xclip_video_{device}"] = results
        return results
    ...
    def run_tests(self, all_hardware=False):
        ...
        """
        # Check if this is an XClip model and run special test
        if self.xclip == "xclip":
            logger.info(f"Running XClip-specific test for {self.model_id}")
            # Run XClip-specific test on default device
            self.test_xclip_video()
            
            # Also run on additional devices if requested
            if all_hardware:
                if self.preferred_device != "cpu":
                    self.test_xclip_video(device="cpu")
                
                if HW_CAPABILITIES["cuda"] and self.preferred_device != "cuda":
                    self.test_xclip_video(device="cuda")
        
        # Always run standard tests for all models
        self.test_pipeline()
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    xclip = results.get("metadata", {}).get("xclip", "vision_text")
    filename = f"hf_{xclip}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Vision-Text models (BLIP, CLIP, XCLIP, etc.)")
    ...
    model_group.add_argument("--xclip-only", action="store_true", help="Test only XClip models")
    ...
    if args.list_models:
        ...
        for model in models:
            model_type = VISION_TEXT_MODELS_REGISTRY[model].get("type", "unknown")
            if model_type == "blip":
                blip_models.append(model)
            elif model_type == "clip":
                clip_models.append(model)
            elif model_type == "xclip":
                xclip_models.append(model)
            else:
        ...
        if xclip_models:
            print("\nXClip Models (Video Classification):")
            for model in xclip_models:
                info = VISION_TEXT_MODELS_REGISTRY[model]
                print(f"  - {model} ({info['class']}): {info['description']}")
    ...
    if args.blip_only or args.clip_only or args.xclip_only:
        ...
        elif args.xclip_only:
            model_type = "xclip"
            default_model = "microsoft/xclip-base-patch32"
            
        for model in all_models:
            if VISION_TEXT_MODELS_REGISTRY[model].get("type") == model_type:
        ...
        if args.all_models:
            ...
            for model_id in filtered_models:
                ...
                # Determine success based on model type
                if model_type == "xclip":
                    # For XClip models, check the xclip_video test results
                    success = any(
                        r.get("xclip_video_success", False) 
                        for r in model_results["results"].values()
                        if "xclip_video" in r
                    )
                else:
                    # For other models, check the pipeline test results
                    success = any(
            ...
            # Print summary
            model_type_display = "XCLIP" if model_type == "xclip" else model_type.upper()
            print(f"\n{model_type_display} Models Testing Summary:")
            total = len(results)
    ...
    model_type = results.get("metadata", {}).get("xclip", "unknown")
    ...
    if success:
        print(f"✅ Successfully tested {model_id} ({model_type_display} model)")
        ...
        if results.get("examples") and len(results["examples"]) > 0:
            print("\nExample output:")
            # Try to find a specific example for xclip models
            xclip_example = None
            if model_type == "xclip":
                for example in results["examples"]:
                    if "XClip video classification" in example.get("method", ""):
                        xclip_example = example
                        break
            
            # Use xclip-specific example if found, otherwise use the first example
            example = xclip_example if xclip_example else results["examples"][0]
        ...
        print(f"❌ Failed to test {model_id} ({model_type_display} model)")
        
        # Print error information for appropriate test type
        for test_name, result in results["results"].items():
            if model_type == "xclip" and "xclip_video" in test_name and "xclip_video_error" in result:
                print(f"  - Error in {test_name}: {result.get('xclip_video_error_type', 'unknown')}")
                print(f"    {result.get('xclip_video_error', 'Unknown error')}")
