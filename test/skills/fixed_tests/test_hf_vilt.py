...
class TestVisionTextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.vilt = self.model_info.get("type", "blip")  # Default to blip if not specified
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    vilt = results.get("metadata", {}).get("vilt", "vision_text")
    filename = f"hf_{vilt}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            vilt = VISION_TEXT_MODELS_REGISTRY[model].get("type", "unknown")
            if vilt == "blip":
    ...
    if args.blip_only or args.clip_only:
        ...
        for model in all_models:
            if VISION_TEXT_MODELS_REGISTRY[model].get("type") == vilt:
        ...
        if args.all_models:
            ...
            # Print summary
            print(f"\n{vilt.upper()} Models Testing Summary:")
            total = len(results)
    ...
    vilt = results.get("metadata", {}).get("vilt", "unknown")

    if success:
        print(f"✅ Successfully tested {model_id} ({vilt.upper()} model)")
        ...
        print(f"❌ Failed to test {model_id} ({vilt.upper()} model)")
