...
ELECTRA_MODELS_REGISTRY = {
    "electra-base-uncased": {
        "full_name": "ELECTRA Base Uncased",
        "architecture": "encoder-only",
        "description": "ELECTRA Base model with uncased vocabulary",
        "electra": "electra",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "electra-large-uncased": {
        "full_name": "ELECTRA Large Uncased",
        "architecture": "encoder-only",
        "description": "ELECTRA Large model with uncased vocabulary",
        "electra": "electra",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "electra-base-cased": {
        "full_name": "ELECTRA Base Cased",
        "architecture": "encoder-only",
        "description": "ELECTRA Base model with cased vocabulary",
        "electra": "electra",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="electra-base-uncased", device=None):
        """Initialize the test class for ELECTRA models.

        Args:
            model_id: The model ID to test (default: "electra-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing ELECTRA model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing ELECTRA model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing ELECTRA model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_electra_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ELECTRA HuggingFace models")
    parser.add_argument(
        "--model", type=str, default="electra-base-uncased", help="Model ID to test"
    )
    ...
    parser.add_argument("--list-models", action="store_true", help="List available ELECTRA models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable ELECTRA models:")
        for model_id, info in ELECTRA_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested ELECTRA model: {args.model}")
