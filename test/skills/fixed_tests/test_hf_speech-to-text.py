...
try:
    ...
    sf = MagicMock()
    HAS_SOUNDFILE = False
    logger.warning("soundfile not available, using mock")
...
class TestWhisperModels:
    ...
    def __init__(self, model_id=None):
        ...
        # Define test audio file path - prefer local file if it exists
        self.test_file = None
        test_file_candidates = ["test.wav", "test.mp3", "test_audio.wav", "test_audio.mp3"]

        for file in test_file_candidates:
            if os.path.exists(file):
                self.test_file = file
                break

        # If no file exists, create a simple audio signal
        if not self.test_file:
            self.test_audio = np.sin(2 * np.pi * 440 * np.arange(0, 3, 1 / self.sampling_rate))
    ...
    def test_pipeline(self, device="auto"):
        ...
        if not HAS_LIBROSA:
            results["pipeline_error_type"] = "missing_dependency"
            results["pipeline_missing_deps"] = ["librosa"]
            results["pipeline_success"] = False
            return results
        ...
        try:
            ...
            # Prepare test input - either use test file or generated audio
            if self.test_file:
                pipeline_input = self.test_file
            else:
                # Create a temporary file for the audio
                tmp_file = "tmp_test_audio.wav"
                sf.write(tmp_file, self.test_audio, self.sampling_rate)
                pipeline_input = tmp_file
            ...
            if not self.test_file and os.path.exists("tmp_test_audio.wav"):
                os.remove("tmp_test_audio.wav")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        if not HAS_LIBROSA:
            results["from_pretrained_error_type"] = "missing_dependency"
            results["from_pretrained_missing_deps"] = ["librosa"]
            results["from_pretrained_success"] = False
            return results
        ...
        try:
            ...
            if self.test_file:
                # Load audio file
                if HAS_LIBROSA:
                    audio, sample_rate = librosa.load(self.test_file, sr=self.sampling_rate)
                else:
                    # Fallback to mock data if librosa not available
                    audio = np.sin(2 * np.pi * 440 * np.arange(0, 3, 1 / self.sampling_rate))
            ...
            predictions = []
            if hasattr(outputs[0], "logits") and hasattr(processor, "batch_decode"):
                logits = outputs[0].logits
                predicted_ids = torch.argmax(logits, dim=-1)
                transcription = processor.batch_decode(predicted_ids, skip_special_tokens=True)
                predictions = [{"text": text} for text in transcription]
            ...
            example_data = {
                ...
                "input": self.test_file if self.test_file else "Generated sine wave audio",
            }
    ...
    def test_with_openvino(self):
        ...
        try:
            from optimum.intel import OVModelForSpeechToTextSeq2Seq

            logger.info(f"Testing {self.model_id} with OpenVINO...")

            # Time processor loading
            processor_load_start = time.time()
            processor = transformers.WhisperProcessor.from_pretrained(self.model_id)
            processor_load_time = time.time() - processor_load_start

            # Time model loading
            model_load_start = time.time()
            model = OVModelForSpeechToTextSeq2Seq.from_pretrained(
                self.model_id, export=True, provider="CPU"
            )
            model_load_time = time.time() - model_load_start

            # Prepare audio input - either use test file or generated audio
            if self.test_file:
                # Load audio file
                if HAS_LIBROSA:
                    audio, sample_rate = librosa.load(self.test_file, sr=self.sampling_rate)
            ...
            # Add predictions
            results["openvino_predictions"] = transcription
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_whisper_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_whisper_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    if args.list_models:
        models = get_available_models()
        print(f"Available SPEECH-family models:")
        for model in models:
            print(f"  - {model}")
        return
