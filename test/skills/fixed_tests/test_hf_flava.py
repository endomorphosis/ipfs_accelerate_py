...
FLAVA_MODELS_REGISTRY = {
    "flava-base-uncased": {
        "full_name": "FLAVA Base Uncased",
        "architecture": "encoder-only",
        "description": "FLAVA Base model with uncased vocabulary",
        "flava": "flava",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "flava-large-uncased": {
        "full_name": "FLAVA Large Uncased",
        "architecture": "encoder-only",
        "description": "FLAVA Large model with uncased vocabulary",
        "flava": "flava",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "flava-base-cased": {
        "full_name": "FLAVA Base Cased",
        "architecture": "encoder-only",
        "description": "FLAVA Base model with cased vocabulary",
        "flava": "flava",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="flava-base-uncased", device=None):
        """Initialize the test class for FLAVA models.

        Args:
            model_id: The model ID to test (default: "flava-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing FLAVA model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing FLAVA model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing FLAVA model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_flava_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test FLAVA HuggingFace models")
    parser.add_argument("--model", type=str, default="flava-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available FLAVA models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable FLAVA models:")
        for model_id, info in FLAVA_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested FLAVA model: {args.model}")
