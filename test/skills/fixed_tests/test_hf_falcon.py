...
FALCON_MODELS_REGISTRY = {
    "falcon": {
        "description": "FALCON small model",
        "class": "FALCONLMHeadModel",
    },
    "falcon-medium": {
        "description": "FALCON medium model",
        "class": "FALCONLMHeadModel",
    },
    "distilfalcon": {
        "description": "DistilFALCON model",
        "class": "FALCONLMHeadModel",
    }
}

class TestFalconModels:
    """Base test class for all FALCON-family models."""
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for FALCON padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for FALCON tokenizer")
            ...
            model_class = None
            if self.class_name == "FALCONLMHeadModel":
                model_class = transformers.FALCONLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_falcon_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available FALCON models in the registry."""
    return list(FALCON_MODELS_REGISTRY.keys())
    
def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered FALCON models."""
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_falcon_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test FALCON-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable FALCON-family models:")
        for model in models:
            info = FALCON_MODELS_REGISTRY[model]
