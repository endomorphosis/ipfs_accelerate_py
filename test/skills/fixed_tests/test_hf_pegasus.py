...
PEGASUS_MODELS_REGISTRY = {
    ...
    "google/flan-pegasus-small": {
        "description": "Flan-T5 small model",
        "class": "PegasusForConditionalGeneration",
    }
}

class TestPegasusModels:
    """Base test class for all T5-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "pegasus-small"
        
        # Verify model exists in registry
        if self.model_id not in PEGASUS_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = PEGASUS_MODELS_REGISTRY["pegasus-small"]
        else:
            self.model_info = PEGASUS_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "PegasusForConditionalGeneration":
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            if has_optimum:
                ...
                pt_model = transformers.PegasusForConditionalGeneration.from_pretrained(self.model_id)
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_pegasus_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(PEGASUS_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestPegasusModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_pegasus_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = PEGASUS_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "pegasus-small"
    logger.info(f"Testing model: {model_id}")
