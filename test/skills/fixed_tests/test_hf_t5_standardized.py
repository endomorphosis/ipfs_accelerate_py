...
"""
Standardized test for the T5 model type from HuggingFace Transformers.
...
T5_MODELS_REGISTRY = {
    "t5-small": {
        ...
        "description": "T5 model for text2text-generation",
        "model_type": "t5",
        "parameters": "110M",
...
class TestT5Models:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "t5-small")
        ...
        if self.model_id not in T5_MODELS_REGISTRY:
            ...
            self.model_config = T5_MODELS_REGISTRY["t5-small"]
        else:
            self.model_config = T5_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "text2text-generation")
        self.class_name = "T5ForConditionalGeneration"
        self.architecture = self.model_config.get("architecture", "encoder-decoder")

        # Test inputs - model type specific
        self.test_text = "translate English to German: The house is wonderful."
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_t5_t5_2025-03-22T23:33:48.962578.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test T5 HuggingFace models")
    ...
    t5_tester = TestT5Models(model_id=args.model, device=args.device)

    # Run the tests
    results = t5_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {t5_tester.preferred_device}")

        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{t5_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{t5_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{t5_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{t5_tester.preferred_device}"]
