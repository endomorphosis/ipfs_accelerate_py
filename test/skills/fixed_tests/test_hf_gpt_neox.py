...
class TestGpt_neoxModels:
    """Base test class for all GPT-2-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "gpt_neox"
        
        # Verify model exists in registry
        if self.model_id not in GPT_NEOX_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = GPT_NEOX_MODELS_REGISTRY["gpt_neox"]
        else:
            self.model_info = GPT_NEOX_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "Gpt_neoxLMHeadModel":
                model_class = transformers.Gpt_neoxLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_gpt_neox_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(GPT_NEOX_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestGpt_neoxModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_gpt_neox_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = GPT_NEOX_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "gpt_neox"
    logger.info(f"Testing model: {model_id}")
