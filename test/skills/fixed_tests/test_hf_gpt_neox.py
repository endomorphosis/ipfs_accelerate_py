...
class TestGPTNeoXModels:
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing GPTNeoX model {self.model_id} with from_pretrained on {self.device}"
            )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="EleutherAI/gpt-neox-20b", help="Model ID to test"
    )
    ...
    if overall_success:
        ...
        print(
            f"  - Pipeline inference time: {results['pipeline'].get('inference_time', 'N/A'):.4f}s"
        )
        print(
            f"  - From_pretrained inference time: {results['from_pretrained'].get('inference_time', 'N/A'):.4f}s"
        )
    else:
        ...
        if not pretrained_success:
            print(
                f"  - From_pretrained error: {results['from_pretrained'].get('error', 'Unknown error')}"
            )
