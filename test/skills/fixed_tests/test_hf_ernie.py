...
ERNIE_MODELS_REGISTRY = {
    "ernie-base-uncased": {
        "full_name": "ERNIE Base Uncased",
        "architecture": "encoder-only",
        "description": "ERNIE Base model with uncased vocabulary",
        "ernie": "ernie",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "ernie-large-uncased": {
        "full_name": "ERNIE Large Uncased",
        "architecture": "encoder-only",
        "description": "ERNIE Large model with uncased vocabulary",
        "ernie": "ernie",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "ernie-base-cased": {
        "full_name": "ERNIE Base Cased",
        "architecture": "encoder-only",
        "description": "ERNIE Base model with cased vocabulary",
        "ernie": "ernie",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="ernie-base-uncased", device=None):
        """Initialize the test class for ERNIE models.

        Args:
            model_id: The model ID to test (default: "ernie-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing ERNIE model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing ERNIE model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing ERNIE model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_ernie_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ERNIE HuggingFace models")
    parser.add_argument("--model", type=str, default="ernie-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available ERNIE models")
    ...
    if args.list_models:
        ...
        for model_id, info in ERNIE_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested ERNIE model: {args.model}")
