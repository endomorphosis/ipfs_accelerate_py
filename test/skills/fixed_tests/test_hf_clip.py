        ...
        try:
            ...
            if isinstance(image, str):
                ...
                if os.path.exists(image):
                    ...
                    image = Image.new(
                        "RGB", (self.image_size, self.image_size), color=(73, 109, 137)
                    )
            ...
            # Get top prediction
            predictions = {"labels": texts, "scores": probs[0].tolist()}

            # Verify output
            self.assertIsNotNone(outputs, "Model output should not be None")
            self.assertIn(
                "logits_per_image", outputs, "CLIP output should contain logits_per_image"
            )
    ...
    def run_tests(self, all_hardware=False):
        ...
        if model_components:
            try:
                ...
                test_input = {"image": test_image, "texts": self.test_texts}
...
def main():
    ...
    if args.unittest or "unittest" in sys.argv:
        ...
        using_mocks = (
            not using_real_inference
            or not results["metadata"]["has_tokenizers"]
            or not results["metadata"]["has_pil"]
        )
        ...
        if using_real_inference and not using_mocks:
            ...
            print(
                f"   Dependencies: transformers={results['metadata']['has_transformers']}, torch={results['metadata']['has_torch']}, tokenizers={results['metadata']['has_tokenizers']}, PIL={results['metadata']['has_pil']}"
            )
