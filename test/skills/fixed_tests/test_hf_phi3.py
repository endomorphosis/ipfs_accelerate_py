...
MOCK_ACCELERATE = os.environ.get("MOCK_ACCELERATE", "False").lower() == "true"
...
try:
    if MOCK_ACCELERATE:
        raise ImportError("Mocked accelerate import failure")
    import accelerate
...
class TestPhi3Models:
    """Base test class for all Phi-3-family models."""

    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "microsoft/phi-3-mini-4k-instruct"

        # Verify model exists in registry
        if self.model_id not in PHI3_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = PHI3_MODELS_REGISTRY["microsoft/phi-3-mini-4k-instruct"]
        else:
            self.model_info = PHI3_MODELS_REGISTRY[self.model_id]
        ...
        # Define test inputs
        self.test_text = "Explain the theory of relativity in simple terms"
        self.test_texts = [
            "Explain the theory of relativity in simple terms",
            "Explain the theory of relativity in simple terms (alternative)",
        ]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "Phi3ForCausalLM":
                model_class = (
                    transformers.AutoModelForCausalLM
                )  # Fallback to Auto since Phi3 might not be explicitly supported
            else:
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            # Time model loading
            model_load_start = time.time()
            model = OVModelForCausalLM.from_pretrained(self.model_id, export=True, provider="CPU")
            model_load_time = time.time() - model_load_start
            ...
            # Add to examples
            example_data = {"method": "OpenVINO inference", "input": str(test_input)}
    ...
    def run_tests(self, all_hardware=False):
        ...
        using_real_inference = HAS_TRANSFORMERS and HAS_TORCH and HAS_ACCELERATE
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_phi3_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available Phi-3 models in the registry."""
    return list(PHI3_MODELS_REGISTRY.keys())


def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered Phi-3 models."""
    models = get_available_models()
    results = {}

    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestPhi3Models(model_id)
    ...
    summary_path = os.path.join(
        output_dir, f"hf_phi3_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Phi-3-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print("\nAvailable Phi-3-family models:")
        for model in models:
            info = PHI3_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "microsoft/phi-3-mini-4k-instruct"
    logger.info(f"Testing model: {model_id}")
    ...
    if using_real_inference and not using_mocks:
        ...
        print(
            f"   Dependencies: transformers={HAS_TRANSFORMERS}, torch={HAS_TORCH}, tokenizers={HAS_TOKENIZERS}, accelerate={HAS_ACCELERATE}"
        )
