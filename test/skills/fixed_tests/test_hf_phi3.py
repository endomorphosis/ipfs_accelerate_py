    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            if self.class_name == "Phi3ForCausalLM":
                model_class = transformers.AutoModelForCausalLM  # Fallback to Auto since Phi3 might not be explicitly supported
            else:
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_phi3_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if using_real_inference and not using_mocks:
        ...
        print(f"   Dependencies: transformers={HAS_TRANSFORMERS}, torch={HAS_TORCH}, tokenizers={HAS_TOKENIZERS}, accelerate={HAS_ACCELERATE}")
