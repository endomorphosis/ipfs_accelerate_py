...
REMBERT_MODELS_REGISTRY = {
    "rembert-base-uncased": {
        "full_name": "REMBERT Base Uncased",
        "architecture": "encoder-only",
        "description": "REMBERT Base model with uncased vocabulary",
        "rembert": "rembert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "rembert-large-uncased": {
        "full_name": "REMBERT Large Uncased",
        "architecture": "encoder-only",
        "description": "REMBERT Large model with uncased vocabulary",
        "rembert": "rembert",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "rembert-base-cased": {
        "full_name": "REMBERT Base Cased",
        "architecture": "encoder-only",
        "description": "REMBERT Base model with cased vocabulary",
        "rembert": "rembert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="rembert-base-uncased", device=None):
        """Initialize the test class for REMBERT models.
        
        Args:
            model_id: The model ID to test (default: "rembert-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing REMBERT model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing REMBERT model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing REMBERT model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_rembert_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test REMBERT HuggingFace models")
    parser.add_argument("--model", type=str, default="rembert-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available REMBERT models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable REMBERT models:")
        for model_id, info in REMBERT_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested REMBERT model: {args.model}")
