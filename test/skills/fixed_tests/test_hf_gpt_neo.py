...
class TestGpt_neoModels:
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "Gpt_neoLMHeadModel":
                model_class = transformers.Gpt_neoLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_gpt_neo_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestGpt_neoModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_gpt_neo_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
