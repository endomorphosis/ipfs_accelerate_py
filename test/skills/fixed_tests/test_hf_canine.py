...
CANINE_MODELS_REGISTRY = {
    "canine-base-uncased": {
        "full_name": "CANINE Base Uncased",
        "architecture": "encoder-only",
        "description": "CANINE Base model with uncased vocabulary",
        "canine": "canine",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "canine-large-uncased": {
        "full_name": "CANINE Large Uncased",
        "architecture": "encoder-only",
        "description": "CANINE Large model with uncased vocabulary",
        "canine": "canine",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "canine-base-cased": {
        "full_name": "CANINE Base Cased",
        "architecture": "encoder-only",
        "description": "CANINE Base model with cased vocabulary",
        "canine": "canine",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestCanineModels:
    def __init__(self, model_id="canine-base-uncased", device=None):
        """Initialize the test class for CANINE models.
        
        Args:
            model_id: The model ID to test (default: "canine-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing CANINE model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing CANINE model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.CanineForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing CANINE model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.CanineForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_canine_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test CANINE HuggingFace models")
    parser.add_argument("--model", type=str, default="canine-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available CANINE models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable CANINE models:")
        for model_id, info in CANINE_MODELS_REGISTRY.items():
    ...
    canine_tester = TestCanineModels(model_id=args.model, device=args.device)

    # Run the tests
    results = canine_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {canine_tester.device}")
    ...
    print(f"\nSuccessfully tested CANINE model: {args.model}")
