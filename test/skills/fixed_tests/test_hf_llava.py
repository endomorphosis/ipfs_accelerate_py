...
class TestLlavaModels(ModelTest):
    ...
    def verify_model_output(self, model, input_data, expected_output=None):
        ...
        try:
            ...
            # Run generation
            with torch.no_grad():
                output_ids = llava_model.generate(**inputs, max_new_tokens=256, do_sample=False)
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            pipeline = transformers.pipeline(
                "image-to-text", model=self.model_id, device=device if device != "cpu" else -1
            )
    ...
    def run_tests(self, all_hardware=False):
        ...
        if model_components:
            try:
                ...
                self.examples.append(
                    {"method": "verify_model_output", "input": str(test_input), "output": output}
                )
            except Exception as e:
