...
ALBERT_MODELS_REGISTRY = {
    "albert-base-uncased": {
        "full_name": "ALBERT Base Uncased",
        "architecture": "encoder-only",
        "description": "ALBERT Base model with uncased vocabulary",
        "albert": "albert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "albert-large-uncased": {
        "full_name": "ALBERT Large Uncased",
        "architecture": "encoder-only",
        "description": "ALBERT Large model with uncased vocabulary",
        "albert": "albert",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "albert-base-cased": {
        "full_name": "ALBERT Base Cased",
        "architecture": "encoder-only",
        "description": "ALBERT Base model with cased vocabulary",
        "albert": "albert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="albert-base-uncased", device=None):
        """Initialize the test class for ALBERT models.

        Args:
            model_id: The model ID to test (default: "albert-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing ALBERT model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing ALBERT model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing ALBERT model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_albert_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ALBERT HuggingFace models")
    parser.add_argument("--model", type=str, default="albert-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available ALBERT models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable ALBERT models:")
        for model_id, info in ALBERT_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested ALBERT model: {args.model}")
