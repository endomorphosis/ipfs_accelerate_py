...
ROFORMER_MODELS_REGISTRY = {
    "roformer-base-uncased": {
        "full_name": "ROFORMER Base Uncased",
        "architecture": "encoder-only",
        "description": "ROFORMER Base model with uncased vocabulary",
        "roformer": "roformer",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "roformer-large-uncased": {
        "full_name": "ROFORMER Large Uncased",
        "architecture": "encoder-only",
        "description": "ROFORMER Large model with uncased vocabulary",
        "roformer": "roformer",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "roformer-base-cased": {
        "full_name": "ROFORMER Base Cased",
        "architecture": "encoder-only",
        "description": "ROFORMER Base model with cased vocabulary",
        "roformer": "roformer",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestRoformerModels:
    def __init__(self, model_id="roformer-base-uncased", device=None):
        """Initialize the test class for ROFORMER models.
        
        Args:
            model_id: The model ID to test (default: "roformer-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing ROFORMER model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing ROFORMER model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.RoformerForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing ROFORMER model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.RoformerForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_roformer_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ROFORMER HuggingFace models")
    parser.add_argument("--model", type=str, default="roformer-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available ROFORMER models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable ROFORMER models:")
        for model_id, info in ROFORMER_MODELS_REGISTRY.items():
    ...
    roformer_tester = TestRoformerModels(model_id=args.model, device=args.device)

    # Run the tests
    results = roformer_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {roformer_tester.device}")
    ...
    print(f"\nSuccessfully tested ROFORMER model: {args.model}")
