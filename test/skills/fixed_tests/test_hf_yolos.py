...
YOLOS_MODELS_REGISTRY = {
    "yolos-base-uncased": {
        "full_name": "YOLOS Base Uncased",
        "architecture": "encoder-only",
        "description": "YOLOS Base model with uncased vocabulary",
        "yolos": "yolos",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "yolos-large-uncased": {
        "full_name": "YOLOS Large Uncased",
        "architecture": "encoder-only",
        "description": "YOLOS Large model with uncased vocabulary",
        "yolos": "yolos",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "yolos-base-cased": {
        "full_name": "YOLOS Base Cased",
        "architecture": "encoder-only",
        "description": "YOLOS Base model with cased vocabulary",
        "yolos": "yolos",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="yolos-base-uncased", device=None):
        """Initialize the test class for YOLOS models.

        Args:
            model_id: The model ID to test (default: "yolos-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing YOLOS model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing YOLOS model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing YOLOS model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_yolos_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test YOLOS HuggingFace models")
    parser.add_argument("--model", type=str, default="yolos-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available YOLOS models")
    ...
    if args.list_models:
        ...
        for model_id, info in YOLOS_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested YOLOS model: {args.model}")
