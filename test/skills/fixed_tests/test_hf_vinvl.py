...
class TestVisionTextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.vinvl = self.model_info.get("type", "blip")  # Default to blip if not specified
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            if self.vinvl == "clip":
                # Load CLIP model
                model = transformers.CLIPModel.from_pretrained(self.model_id, **pretrained_kwargs)
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    vinvl = results.get("metadata", {}).get("vinvl", "vision_text")
    filename = (
        f"hf_{vinvl}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir,
        f"hf_vision_text_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            vinvl = VISION_TEXT_MODELS_REGISTRY[model].get("type", "unknown")
            if vinvl == "blip":
    ...
    if args.blip_only or args.clip_only:
        ...
        for model in all_models:
            if VISION_TEXT_MODELS_REGISTRY[model].get("type") == vinvl:
        ...
        if args.all_models:
            ...
            # Print summary
            print(f"\n{vinvl.upper()} Models Testing Summary:")
            total = len(results)
    ...
    vinvl = results.get("metadata", {}).get("vinvl", "unknown")

    if success:
        print(f"✅ Successfully tested {model_id} ({vinvl.upper()} model)")
        ...
        print(f"❌ Failed to test {model_id} ({vinvl.upper()} model)")
