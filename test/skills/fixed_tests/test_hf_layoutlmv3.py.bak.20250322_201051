...
class TestVisionTextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.layoutlmv3 = self.model_info.get("type", "blip")  # Default to blip if not specified
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    layoutlmv3 = results.get("metadata", {}).get("layoutlmv3", "vision_text")
    filename = f"hf_{layoutlmv3}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            layoutlmv3 = VISION_TEXT_MODELS_REGISTRY[model].get("type", "unknown")
            if layoutlmv3 == "blip":
                blip_models.append(model)
            elif layoutlmv3 == "clip":
    ...
    if args.blip_only or args.clip_only:
        ...
        for model in all_models:
            if VISION_TEXT_MODELS_REGISTRY[model].get("type") == layoutlmv3:
        ...
        if args.all_models:
            ...
            # Print summary
            print(f"\n{layoutlmv3.upper()} Models Testing Summary:")
            total = len(results)
    ...
    layoutlmv3 = results.get("metadata", {}).get("layoutlmv3", "unknown")
    
    if success:
        print(f"✅ Successfully tested {model_id} ({layoutlmv3.upper()} model)")
        ...
        print(f"❌ Failed to test {model_id} ({layoutlmv3.upper()} model)")
