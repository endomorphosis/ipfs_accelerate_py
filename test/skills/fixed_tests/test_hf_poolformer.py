...
class TestPoolFormerModels:
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            mock_image = Image.fromarray(np.random.randint(0, 255, (height, width, 3), dtype=np.uint8))
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            logger.info(f"Created proper image input tensor ({height}x{width}) for PoolFormer model")
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            # Create a random test image
            test_image = Image.fromarray(np.random.randint(0, 255, (height, width, 3), dtype=np.uint8))
    ...
    def test_pooling_mechanism(self, device="auto"):
        ...
        if not HAS_TRANSFORMERS or not HAS_TORCH:
            ...
            results["pooling_test_missing"] = ["transformers", "torch"] if not HAS_TRANSFORMERS and not HAS_TORCH else (["transformers"] if not HAS_TRANSFORMERS else ["torch"])
        ...
        try:
            ...
            for name, module in model.named_modules():
                if 'pool' in name.lower() or any(pool_type in module.__class__.__name__.lower() 
            ...
            if len(pooling_activations) > 0:
                results["pooling_activations"] = pooling_activations[:3]  # Store first 3 to avoid too much data
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_poolformer_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_poolformer_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            ...
            print(f"  - {model} ({info.get('parameters', 'Unknown parameters')}): {info['description']}")
        return
