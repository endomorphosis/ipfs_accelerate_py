...
CONVNEXT_MODELS_REGISTRY = {
    "google/convnext-base-patch16-224": {
...
class TestConvnextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "google/convnext-base-patch16-224"
        
        # Verify model exists in registry
        if self.model_id not in CONVNEXT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = CONVNEXT_MODELS_REGISTRY["google/convnext-base-patch16-224"]
        else:
...
def main():
    ...
    model_id = args.model or "google/convnext-base-patch16-224"
    logger.info(f"Testing model: {model_id}")
