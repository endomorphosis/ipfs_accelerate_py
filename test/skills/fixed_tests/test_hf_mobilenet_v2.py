...
"""
Class-based test file for MobileNet-v2 models.
This file provides a unified testing interface for MobileNetV2 vision models from HuggingFace.

MobileNetV2 is an efficient CNN architecture designed for mobile and edge devices, with:
...
- Low computational complexity while maintaining accuracy

This test file supports both pipeline API and direct model usage.
"""
...
MOBILENET_V2_MODELS_REGISTRY = {
    "google/mobilenet_v2_1.0_224": {
        "description": "MobileNetV2 model with width multiplier 1.0 and input size 224x224",
    ...
    "google/mobilenet_v2_1.4_224": {
        "description": "MobileNetV2 model with width multiplier 1.4 and input size 224x224",
        "class": "MobileNetV2ForImageClassification", 
        "input_size": (224, 224),
    ...
    "microsoft/mobilenet-v2": {
        "description": "MobileNetV2 model from Microsoft, trained on ImageNet",
        "class": "MobileNetV2ForImageClassification",
        "input_size": (224, 224),
        "width_multiplier": 1.0,
        "paper": "https://arxiv.org/abs/1801.04381",
...
class TestMobileNetV2Models:
    """Base test class for all MobileNetV2 models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "google/mobilenet_v2_1.0_224"
        
        # Verify model exists in registry
        if self.model_id not in MOBILENET_V2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MOBILENET_V2_MODELS_REGISTRY["google/mobilenet_v2_1.0_224"]
        else:
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            logger.info(f"Created mock image {height}x{width}x3 for MobileNetV2 pipeline testing")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # MobileNetV2 uses the generic AutoModelForImageClassification
            ...
            # Prepare test input - MobileNetV2 requires proper image input
            
            # Get the input size for this model
            height, width = self.input_size
            
            # Create random image tensor and process through processor
            random_image = Image.fromarray(np.random.randint(0, 255, (height, width, 3), dtype=np.uint8))
            ...
            logger.info(f"Created proper image input tensor ({height}x{width}) for MobileNetV2 model")
    ...
    def test_mobile_performance(self, device="auto"):
        """
        Test the model performance characteristics relevant to mobile deployment.
        
        This tests inference speed, memory usage and other metrics important for mobile usage.
        """
        if device == "auto":
            device = self.preferred_device
        ...
        if not HAS_TRANSFORMERS or not HAS_TORCH:
            results["mobile_perf_error_type"] = "missing_dependency"
            results["mobile_perf_missing"] = ["transformers", "torch"] if not HAS_TRANSFORMERS and not HAS_TORCH else (["transformers"] if not HAS_TRANSFORMERS else ["torch"])
            results["mobile_perf_success"] = False
            return results
            
        try:
            logger.info(f"Testing mobile performance metrics for {self.model_id} on {device}...")
            ...
            # Get model size characteristics
            param_count = sum(p.numel() for p in model.parameters())
            param_size_mb = (param_count * 4) / (1024 * 1024)  # Size in MB (32-bit floats)
            
            # For mobile, we care about inference with batch size 1
            height, width = self.input_size
            test_image = Image.fromarray(np.random.randint(0, 255, (height, width, 3), dtype=np.uint8))
            inputs = processor(images=test_image, return_tensors="pt")
            ...
            # Run multiple inferences for more stable timing
            times = []
            for _ in range(10):  # More runs for mobile perf testing
            ...
            # Get peak memory if using CUDA
            peak_memory_mb = 0
            if device == "cuda" and torch.cuda.is_available():
                peak_memory = torch.cuda.max_memory_allocated()
                peak_memory_mb = peak_memory / (1024 * 1024)
            
            # Calculate metrics relevant to mobile
            fps = 1.0 / avg_time
            
            # Store results
            results["mobile_perf_success"] = True
            results["mobile_perf_avg_time"] = avg_time
            results["mobile_perf_min_time"] = min_time 
            results["mobile_perf_max_time"] = max_time
            results["mobile_perf_fps"] = fps
            results["mobile_perf_param_count"] = param_count
            results["mobile_perf_param_size_mb"] = param_size_mb
            results["mobile_perf_peak_memory_mb"] = peak_memory_mb if device == "cuda" else "N/A"
            
            # Add to performance stats
            self.performance_stats["mobile_performance"] = {
                "avg_time": avg_time,
                "min_time": min_time,
                "max_time": max_time,
                "fps": fps,
                ...
                "peak_memory_mb": peak_memory_mb if device == "cuda" else "N/A"
            }
            
        except Exception as e:
            results["mobile_perf_success"] = False
            results["mobile_perf_error"] = str(e)
            results["mobile_perf_traceback"] = traceback.format_exc()
            logger.error(f"Error testing mobile performance: {e}")
            
        # Add to overall results
        self.results["mobile_performance"] = results
        return results
    ...
    def run_tests(self, all_hardware=False):
        ...
        # Always test mobile performance (important for MobileNetV2)
        ...
        return {
            ...
            "metadata": {
                ...
                "width_multiplier": self.model_info.get("width_multiplier", "Unknown"),
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_mobilenetv2_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available MobileNetV2 models in the registry."""
    return list(MOBILENET_V2_MODELS_REGISTRY.keys())


def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered MobileNetV2 models."""
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_mobilenetv2_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MobileNetV2 models")
    ...
    if args.all_models:
        ...
        # Print summary
        print("\nMobileNetV2 Models Testing Summary:")
        total = len(results)
    ...
    model_id = args.model or "google/mobilenet_v2_1.0_224"
    logger.info(f"Testing model: {model_id}")
    ...
    if success:
        ...
        # Print mobile performance if available
        if "mobile_performance" in results["performance"]:
            mobile_stats = results["performance"]["mobile_performance"]
            print(f"\nMobile Performance Metrics:")
            print(f"  - FPS: {mobile_stats.get('fps', 'N/A'):.2f}")
            print(f"  - Model Size: {mobile_stats.get('param_size_mb', 'N/A'):.2f} MB")
            print(f"  - Parameters: {mobile_stats.get('param_count', 'N/A'):,}")
