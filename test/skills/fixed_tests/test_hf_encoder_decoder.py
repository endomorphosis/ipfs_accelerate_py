    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            if has_optimum:
                ...
                pt_model = transformers.EncoderDecoderForConditionalGeneration.from_pretrained(
                    self.model_id
                )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_encoder_decoder_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
