...
class TestVisionTextModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.align = self.model_info.get("type", "blip")  # Default to blip if not specified
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    align = results.get("metadata", {}).get("align", "vision_text")
    filename = f"hf_{align}_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            align = VISION_TEXT_MODELS_REGISTRY[model].get("type", "unknown")
            if align == "blip":
    ...
    if args.blip_only or args.clip_only:
        ...
        for model in all_models:
            if VISION_TEXT_MODELS_REGISTRY[model].get("type") == align:
        ...
        if args.all_models:
            ...
            # Print summary
            print(f"\n{align.upper()} Models Testing Summary:")
            total = len(results)
    ...
    align = results.get("metadata", {}).get("align", "unknown")
    
    if success:
        print(f"✅ Successfully tested {model_id} ({align.upper()} model)")
        ...
        print(f"❌ Failed to test {model_id} ({align.upper()} model)")
