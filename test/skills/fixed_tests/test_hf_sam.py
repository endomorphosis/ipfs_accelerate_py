...
SAM_MODELS_REGISTRY = {
    "sam-base-uncased": {
        "full_name": "SAM Base Uncased",
        "architecture": "encoder-only",
        "description": "SAM Base model with uncased vocabulary",
        "sam": "sam",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "sam-large-uncased": {
        "full_name": "SAM Large Uncased",
        "architecture": "encoder-only",
        "description": "SAM Large model with uncased vocabulary",
        "sam": "sam",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "sam-base-cased": {
        "full_name": "SAM Base Cased",
        "architecture": "encoder-only",
        "description": "SAM Base model with cased vocabulary",
        "sam": "sam",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="sam-base-uncased", device=None):
        """Initialize the test class for SAM models.

        Args:
            model_id: The model ID to test (default: "sam-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing SAM model {self.model_id} with pipeline API on {self.device}")
            ...
            test_input = "An image for segmentation."
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing SAM model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing SAM model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_sam_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SAM HuggingFace models")
    parser.add_argument("--model", type=str, default="sam-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available SAM models")
    ...
    if args.list_models:
        ...
        for model_id, info in SAM_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested SAM model: {args.model}")
