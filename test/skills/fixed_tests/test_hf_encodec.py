...
ENCODEC_MODELS_REGISTRY = {
    "encodec-base-uncased": {
        "full_name": "ENCODEC Base Uncased",
        "architecture": "encoder-only",
        "description": "ENCODEC Base model with uncased vocabulary",
        "encodec": "encodec",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "encodec-large-uncased": {
        "full_name": "ENCODEC Large Uncased",
        "architecture": "encoder-only",
        "description": "ENCODEC Large model with uncased vocabulary",
        "encodec": "encodec",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "encodec-base-cased": {
        "full_name": "ENCODEC Base Cased",
        "architecture": "encoder-only",
        "description": "ENCODEC Base model with cased vocabulary",
        "encodec": "encodec",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="encodec-base-uncased", device=None):
        """Initialize the test class for ENCODEC models.
        
        Args:
            model_id: The model ID to test (default: "encodec-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing ENCODEC model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing ENCODEC model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing ENCODEC model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_encodec_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test ENCODEC HuggingFace models")
    parser.add_argument("--model", type=str, default="encodec-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available ENCODEC models")
    ...
    if args.list_models:
        ...
        for model_id, info in ENCODEC_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested ENCODEC model: {args.model}")
