...
class TestKosmos2Models:
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Kosmos-2 model {self.model_id} with pipeline API on {self.device}"
            )
    ...
    def test_grounding_capabilities(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Kosmos-2 model {self.model_id} with grounding capabilities on {self.device}"
            )
            ...
            for i, prompt in enumerate(test_prompts):
                ...
                with torch.no_grad():
                    outputs = model.generate(
                        **inputs, max_new_tokens=128, num_beams=3, early_stopping=True
                    )
                ...
                processed_outputs = processor.post_process_generation(
                    decoded_outputs, cleanup_and_extract=True
                )
                ...
                results[f"prompt_{i + 1}"] = {
                    ...
                    "entities": processed_outputs[0]["entities"]
                    if "entities" in processed_outputs[0]
                    else [],
                }
    ...
    def test_direct_inference(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Kosmos-2 model {self.model_id} with direct inference on {self.device}"
            )
            ...
            for i, question in enumerate(test_questions):
                # Process inputs
                inputs = processor(text=question, images=test_image, return_tensors="pt").to(
                    self.device
                )
                ...
                # Store result
                results[f"question_{i + 1}"] = {"question": question, "answer": decoded_text}
