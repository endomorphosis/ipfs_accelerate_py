...
if HAS_TORCH:
    ...
    if HAS_CUDA:
        ...
        for i in range(num_devices):
            ...
            logger.info(
                f"CUDA Device {i}: {device_props.name} with {device_props.total_memory / 1024**3:.2f} GB memory"
            )
    else:
...
# Main Data2vecVision model registry
DATA2DATA2DATA2VEC_VISION_MODELS_REGISTRY = {
    "data2vec-vision": {
        "full_name": "Data2vecVision Base Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecVision Base model with uncased vocabulary",
        "model_type": "data2vec_vision",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
        "attention_heads": 12,
    ...
    "data2vec_vision-large-uncased": {
        "full_name": "Data2vecVision Large Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecVision Large model with uncased vocabulary",
        "model_type": "data2vec_vision",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "data2vec_vision-base-cased": {
        "full_name": "Data2vecVision Base Cased",
        "architecture": "encoder-only",
        "description": "Data2vecVision Base model with cased vocabulary",
        "model_type": "data2vec_vision",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class MockTokenizer:
    ...
    def __call__(self, text, return_tensors=None, padding=False, truncation=False, max_length=None):
        ...
        input_ids = [
            [self.cls_token_id] + [i % 100 + 999 for i in range(10)] + [self.sep_token_id]
...
class TestData2vec_visionModels:
    def __init__(self, model_id="data2vec-vision", device=None):
        """Initialize the test class for Data2vecVision models.

        Args:
            model_id: The model ID to test (default: "data2vec-vision")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Data2vecVision model {self.model_id} with pipeline API on {self.device}"
            )
            ...
            pipe = transformers.pipeline(
                "fill-mask", model=self.model_id, device=self.device if self.device != "cpu" else -1
            )
            ...
            if isinstance(outputs, list) and len(outputs) > 0:
                top_prediction = (
                    outputs[0]["token_str"]
                    if isinstance(outputs[0], dict) and "token_str" in outputs[0]
                    else "N/A"
                )
                ...
                return {
                    ...
                    "input": test_input,
                    "top_prediction": top_prediction,
                    "performance": {"load_time": load_time, "inference_time": inference_time},
                }
            else:
    ...
    def test_from_pretrained(self):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH:
                logger.warning(
                    "Transformers or torch library not available, skipping from_pretrained test"
                )
                return {"success": False, "error": "Required libraries not available"}

            logger.info(
                f"Testing Data2vecVision model {self.model_id} with from_pretrained API on {self.device}"
            )
            ...
            model = transformers.Data2vecVisionForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecVision model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.Data2vecVisionForMaskedLM.from_pretrained(self.model_id)
                ...
                try:
                    ...
                    torch.onnx.export(
                        ...
                        output_names=["logits"],
                        dynamic_axes={
                            key: {0: "batch_size", 1: "sequence_length"} for key in inputs.keys()
                        },
                    )
    ...
    def run_tests(self, all_hardware=False):
        ...
        results["metadata"] = {
            ...
            or from_pretrained_result.get("success", False),
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_data2vec_vision_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Data2vecVision HuggingFace models")
    parser.add_argument("--model", type=str, default="data2vec-vision", help="Model ID to test")
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="List available Data2vecVision models"
    )
    ...
    if args.list_models:
        print(f"\nAvailable Data2vecVision models:")
        for model_id, info in DATA2DATA2DATA2VEC_VISION_MODELS_REGISTRY.items():
            print(f"  - {model_id} ({info['full_name']})")
            print(
                f"      Parameters: {info['parameters']}, Embedding: {info['embedding_dim']}, Context: {info['context_length']}"
            )
    ...
    data2vec_vision_tester = TestData2vec_visionModels(model_id=args.model, device=args.device)

    # Run the tests
    results = data2vec_vision_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {data2vec_vision_tester.device}")
    ...
    if pipeline_success:
        ...
        print(
            ...
            else f"  - Inference time: {inference_time}"
        )
    else:
    ...
    if from_pretrained_success:
        ...
        inference_time = (
            results["from_pretrained"].get("performance", {}).get("inference_time", "N/A")
        )
    ...
    print(f"\nSuccessfully tested Data2vecVision model: {args.model}")
