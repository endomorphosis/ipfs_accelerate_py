...
try:
    ...
    try:
        if (
            ...
            and hasattr(torch._C, "_rocm_version")
        ):
            HAS_ROCM = True
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            try:
                ...
                class OVT5Wrapper:
                    def __init__(self, encoder_model, pt_model, tokenizer):
                        ...
                        self.pt_model = (
                            pt_model  # Keep PyTorch model for decoder (simplified approach)
                        )
            ...
            try:
                ...
                results["openvino_backend"] = (
                    "optimum.intel" if has_optimum else "direct_conversion"
                )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_t5_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
