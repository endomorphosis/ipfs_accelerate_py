...
class TestLlavaNextModels:
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing LLaVA-Next model {self.model_id} with pipeline API on {self.device}"
            )
    ...
    def test_direct_model_inference(self):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH:
                logger.warning(
                    "Required libraries not available, skipping direct model inference test"
                )
                return {"success": False, "error": "Required libraries not available"}

            logger.info(
                f"Testing LLaVA-Next model {self.model_id} with direct model inference on {self.device}"
            )
    ...
    def test_conversation_capabilities(self):
        ...
        try:
            ...
            logger.info(
                f"Testing LLaVA-Next model {self.model_id} conversation capabilities on {self.device}"
            )
            ...
            for i, query in enumerate(conversation):
                ...
                # Store result
                results[f"turn_{i + 1}"] = {"query": query, "response": new_response}
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="llava-hf/llava-v1.6-mistral-7b-hf", help="Model ID to test"
    )
