...
MIXTRAL_MODELS_REGISTRY = {
    "mixtral": {
        "description": "GPT-2 small model",
        "class": "MIXTRALLMHeadModel",
    },
    "mixtral-medium": {
...
class TestMixtralModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "mixtral"

        # Verify model exists in registry
        if self.model_id not in MIXTRAL_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MIXTRAL_MODELS_REGISTRY["mixtral"]
        else:
            self.model_info = MIXTRAL_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "MIXTRALLMHeadModel":
                model_class = transformers.MIXTRALLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_mixtral_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def get_available_models():
    ...
    return list(MIXTRAL_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestMixtralModels(model_id)
    ...
    summary_path = os.path.join(
        output_dir, f"hf_mixtral_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = MIXTRAL_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "mixtral"
    logger.info(f"Testing model: {model_id}")
