...
# Main Wav2Vec2BERT model registry
WAV2VEC2_Wav2Vec2WAV2VEC2_WAV2VEC2_BERT_MODELS_REGISTRY = {
    "wav2vec2-wav2vec2_bert": {
        "full_name": "Wav2Vec2BERT Base Uncased",
        "architecture": "encoder-only",
        "description": "Wav2Vec2BERT Base model with uncased vocabulary",
    ...
    "wav2vec2_bert-large-uncased": {
        "full_name": "Wav2Vec2BERT Large Uncased",
        "architecture": "encoder-only",
        "description": "Wav2Vec2BERT Large model with uncased vocabulary",
        "model_type": "wav2vec2_bert",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "wav2vec2_bert-base-cased": {
        "full_name": "Wav2Vec2BERT Base Cased",
        "architecture": "encoder-only",
        "description": "Wav2Vec2BERT Base model with cased vocabulary",
        "model_type": "wav2vec2_bert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestWav2vec2_bertModels:
    def __init__(self, model_id="wav2vec2-wav2vec2_bert", device=None):
        """Initialize the test class for Wav2Vec2BERT models.
        
        Args:
            model_id: The model ID to test (default: "wav2vec2-wav2vec2_bert")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing Wav2Vec2BERT model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing Wav2Vec2BERT model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.Wav2Vec2BERTForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing Wav2Vec2BERT model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.Wav2Vec2BERTForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_wav2vec2_bert_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Wav2Vec2BERT HuggingFace models")
    parser.add_argument("--model", type=str, default="wav2vec2-wav2vec2_bert", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available Wav2Vec2BERT models")
    
    args = parser.parse_args()
    
    # List models if requested
    if args.list_models:
        print(f"\nAvailable Wav2Vec2BERT models:")
        for model_id, info in WAV2VEC2_Wav2Vec2WAV2VEC2_WAV2VEC2_BERT_MODELS_REGISTRY.items():
    ...
    wav2vec2_bert_tester = TestWav2vec2_bertModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = wav2vec2_bert_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {wav2vec2_bert_tester.device}")
    ...
    print(f"\nSuccessfully tested Wav2Vec2BERT model: {args.model}")
