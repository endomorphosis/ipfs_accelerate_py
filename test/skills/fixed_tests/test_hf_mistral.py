...
MISTRAL_MODELS_REGISTRY = {
    "mistral": {
        "description": "MISTRAL small model",
        "class": "MISTRALLMHeadModel",
    },
    "mistral-medium": {
    ...
    "distilmistral": {
        "description": "DistilMISTRAL model",
        "class": "MISTRALLMHeadModel",
    },
}
...
class TestMistralModels:
    """Base test class for all MISTRAL-family models."""

    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "mistral"

        # Verify model exists in registry
        if self.model_id not in MISTRAL_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = MISTRAL_MODELS_REGISTRY["mistral"]
        else:
            self.model_info = MISTRAL_MODELS_REGISTRY[self.model_id]
        ...
        self.test_texts = ["Once upon a time", "Once upon a time (alternative)"]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            if tokenizer.pad_token is None:
                ...
                logger.info("Set pad_token to eos_token for MISTRAL tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for MISTRAL padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for MISTRAL tokenizer")
            ...
            model_class = None
            if self.class_name == "MISTRALLMHeadModel":
                model_class = transformers.MISTRALLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_mistral_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def get_available_models():
    ...
    return list(MISTRAL_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestMistralModels(model_id)
    ...
    summary_path = os.path.join(
        output_dir, f"hf_mistral_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MISTRAL-family models")
    ...
    if args.list_models:
        ...
        for model in models:
            info = MISTRAL_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "mistral"
    logger.info(f"Testing model: {model_id}")
