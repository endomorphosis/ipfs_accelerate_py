...
IMAGEBIND_MODELS_REGISTRY = {
    "imagebind-base-uncased": {
        "full_name": "IMAGEBIND Base Uncased",
        "architecture": "encoder-only",
        "description": "IMAGEBIND Base model with uncased vocabulary",
        "imagebind": "imagebind",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "imagebind-large-uncased": {
        "full_name": "IMAGEBIND Large Uncased",
        "architecture": "encoder-only",
        "description": "IMAGEBIND Large model with uncased vocabulary",
        "imagebind": "imagebind",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "imagebind-base-cased": {
        "full_name": "IMAGEBIND Base Cased",
        "architecture": "encoder-only",
        "description": "IMAGEBIND Base model with cased vocabulary",
        "imagebind": "imagebind",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="imagebind-base-uncased", device=None):
        """Initialize the test class for IMAGEBIND models.
        
        Args:
            model_id: The model ID to test (default: "imagebind-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing IMAGEBIND model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing IMAGEBIND model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing IMAGEBIND model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_imagebind_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test IMAGEBIND HuggingFace models")
    parser.add_argument("--model", type=str, default="imagebind-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available IMAGEBIND models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable IMAGEBIND models:")
        for model_id, info in IMAGEBIND_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested IMAGEBIND model: {args.model}")
