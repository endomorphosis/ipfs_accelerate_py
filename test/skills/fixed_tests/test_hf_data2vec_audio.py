...
# Main Data2vecAudio model registry
DATA2DATA2DATA2VEC_AUDIO_MODELS_REGISTRY = {
    "data2vec-audio": {
        "full_name": "Data2vecAudio Base Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecAudio Base model with uncased vocabulary",
    ...
    "data2vec_audio-large-uncased": {
        "full_name": "Data2vecAudio Large Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecAudio Large model with uncased vocabulary",
        "model_type": "data2vec_audio",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "data2vec_audio-base-cased": {
        "full_name": "Data2vecAudio Base Cased",
        "architecture": "encoder-only",
        "description": "Data2vecAudio Base model with cased vocabulary",
        "model_type": "data2vec_audio",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestData2vec_audioModels:
    def __init__(self, model_id="data2vec-audio", device=None):
        """Initialize the test class for Data2vecAudio models.
        
        Args:
            model_id: The model ID to test (default: "data2vec-audio")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecAudio model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecAudio model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.Data2vecAudioForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecAudio model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.Data2vecAudioForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_data2vec_audio_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Data2vecAudio HuggingFace models")
    parser.add_argument("--model", type=str, default="data2vec-audio", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available Data2vecAudio models")
    
    args = parser.parse_args()
    
    # List models if requested
    if args.list_models:
        print(f"\nAvailable Data2vecAudio models:")
        for model_id, info in DATA2DATA2DATA2VEC_AUDIO_MODELS_REGISTRY.items():
    ...
    data2vec_audio_tester = TestData2vec_audioModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = data2vec_audio_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {data2vec_audio_tester.device}")
    ...
    print(f"\nSuccessfully tested Data2vecAudio model: {args.model}")
