...
IDEFICS_MODELS_REGISTRY = {
    "idefics-base-uncased": {
        "full_name": "IDEFICS Base Uncased",
        "architecture": "encoder-only",
        "description": "IDEFICS Base model with uncased vocabulary",
        "idefics": "idefics",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "idefics-large-uncased": {
        "full_name": "IDEFICS Large Uncased",
        "architecture": "encoder-only",
        "description": "IDEFICS Large model with uncased vocabulary",
        "idefics": "idefics",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "idefics-base-cased": {
        "full_name": "IDEFICS Base Cased",
        "architecture": "encoder-only",
        "description": "IDEFICS Base model with cased vocabulary",
        "idefics": "idefics",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="idefics-base-uncased", device=None):
        """Initialize the test class for IDEFICS models.
        
        Args:
            model_id: The model ID to test (default: "idefics-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing IDEFICS model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing IDEFICS model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing IDEFICS model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_idefics_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test IDEFICS HuggingFace models")
    parser.add_argument("--model", type=str, default="idefics-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available IDEFICS models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable IDEFICS models:")
        for model_id, info in IDEFICS_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested IDEFICS model: {args.model}")
