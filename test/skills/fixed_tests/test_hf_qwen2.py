...
"""
Test file for Qwen2 HuggingFace models.
"""
...
# Model registry for Qwen2 models
QWEN2_MODELS_REGISTRY = {
    "Qwen/Qwen2-7B": {
        "full_name": "Qwen2 7B",
        "architecture": "decoder-only",
        "description": "Qwen2 7B large language model developed by Alibaba",
        "model_type": "qwen2",
        "parameters": "7B",
    ...
    "Qwen/Qwen2-7B-Instruct": {
        ...
        "architecture": "decoder-only",
        "description": "Instruction-tuned Qwen2 7B model",
        "model_type": "qwen2",
        "parameters": "7B",
        "context_length": 32768,
        "embedding_dim": 4096,
...
class TestQwen2Models:
    ...
    def __init__(self, model_id="Qwen/Qwen2-7B", device=None):
        """Initialize the test class for Qwen2 models.

        Args:
            model_id: The model ID to test (default: "Qwen/Qwen2-7B")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing Qwen2 model {self.model_id} with pipeline API on {self.device}")
            ...
            test_input = "Qwen2 is a large language model that"
    ...
    def test_direct_model_inference(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Qwen2 model {self.model_id} with direct model API on {self.device}"
            )
    ...
    def test_multiple_prompts(self):
        ...
        try:
            ...
            logger.info(
                f"Testing Qwen2 model {self.model_id} with multiple prompts on {self.device}"
            )
    ...
    def test_hardware_compatibility(self):
        ...
        # Test CPU
        logger.info("Testing Qwen2 model on CPU")
        cpu_tester = TestQwen2Models(model_id=self.model_id, device="cpu")
        ...
        if HAS_TORCH and torch.cuda.is_available():
            logger.info("Testing Qwen2 model on CUDA")
            cuda_tester = TestQwen2Models(model_id=self.model_id, device="cuda:0")
        ...
        if HAS_TORCH and hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            logger.info("Testing Qwen2 model on MPS (Apple Silicon)")
            mps_tester = TestQwen2Models(model_id=self.model_id, device="mps")
        ...
        results["openvino"] = {
            "success": False,
            "error": "OpenVINO support not yet implemented for Qwen2 models",
        }
    ...
    def test_openvino_inference(self):
        ...
        logger.warning("OpenVINO support not yet implemented for Qwen2 models")
        return {"success": False, "error": "OpenVINO support not yet implemented for Qwen2 models"}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Qwen2 HuggingFace models")
    parser.add_argument("--model", type=str, default="Qwen/Qwen2-7B", help="Model ID to test")
    ...
    qwen2_tester = TestQwen2Models(model_id=args.model, device=args.device)

    # Run the tests
    results = qwen2_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {qwen2_tester.device}")
