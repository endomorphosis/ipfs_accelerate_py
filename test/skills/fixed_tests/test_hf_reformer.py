...
REFORMER_MODELS_REGISTRY = {
    "reformer-base-uncased": {
        "full_name": "REFORMER Base Uncased",
        "architecture": "encoder-only",
        "description": "REFORMER Base model with uncased vocabulary",
        "reformer": "reformer",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "reformer-large-uncased": {
        "full_name": "REFORMER Large Uncased",
        "architecture": "encoder-only",
        "description": "REFORMER Large model with uncased vocabulary",
        "reformer": "reformer",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "reformer-base-cased": {
        "full_name": "REFORMER Base Cased",
        "architecture": "encoder-only",
        "description": "REFORMER Base model with cased vocabulary",
        "reformer": "reformer",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="reformer-base-uncased", device=None):
        """Initialize the test class for REFORMER models.
        
        Args:
            model_id: The model ID to test (default: "reformer-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing REFORMER model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing REFORMER model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing REFORMER model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_reformer_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test REFORMER HuggingFace models")
    parser.add_argument("--model", type=str, default="reformer-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available REFORMER models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable REFORMER models:")
        for model_id, info in REFORMER_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested REFORMER model: {args.model}")
