...
class TestSwinModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "google/swin-base-patch16-224"
        
        # Verify model exists in registry
        if self.model_id not in SWIN_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = SWIN_MODELS_REGISTRY["google/swin-base-patch16-224"]
        else:
...
def main():
    ...
    model_id = args.model or "google/swin-base-patch16-224"
    logger.info(f"Testing model: {model_id}")
