...
GEMMA_MODELS_REGISTRY = {
    "gemma-base-uncased": {
        "full_name": "GEMMA Base Uncased",
        "architecture": "encoder-only",
        "description": "GEMMA Base model with uncased vocabulary",
        "gemma": "gemma",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "gemma-large-uncased": {
        "full_name": "GEMMA Large Uncased",
        "architecture": "encoder-only",
        "description": "GEMMA Large model with uncased vocabulary",
        "gemma": "gemma",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "gemma-base-cased": {
        "full_name": "GEMMA Base Cased",
        "architecture": "encoder-only",
        "description": "GEMMA Base model with cased vocabulary",
        "gemma": "gemma",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="gemma-base-uncased", device=None):
        """Initialize the test class for GEMMA models.

        Args:
            model_id: The model ID to test (default: "gemma-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing GEMMA model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing GEMMA model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing GEMMA model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_gemma_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test GEMMA HuggingFace models")
    parser.add_argument("--model", type=str, default="gemma-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available GEMMA models")
    ...
    if args.list_models:
        ...
        for model_id, info in GEMMA_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested GEMMA model: {args.model}")
