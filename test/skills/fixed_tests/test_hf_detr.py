...
DETR_MODELS_REGISTRY = {
    "detr-base-uncased": {
        "full_name": "DETR Base Uncased",
        "architecture": "encoder-only",
        "description": "DETR Base model with uncased vocabulary",
        "detr": "detr",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "detr-large-uncased": {
        "full_name": "DETR Large Uncased",
        "architecture": "encoder-only",
        "description": "DETR Large model with uncased vocabulary",
        "detr": "detr",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "detr-base-cased": {
        "full_name": "DETR Base Cased",
        "architecture": "encoder-only",
        "description": "DETR Base model with cased vocabulary",
        "detr": "detr",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="detr-base-uncased", device=None):
        """Initialize the test class for DETR models.

        Args:
            model_id: The model ID to test (default: "detr-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing DETR model {self.model_id} with pipeline API on {self.device}")
            ...
            test_input = "An image of a street scene."
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing DETR model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing DETR model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_detr_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test DETR HuggingFace models")
    parser.add_argument("--model", type=str, default="detr-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available DETR models")
    ...
    if args.list_models:
        ...
        for model_id, info in DETR_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested DETR model: {args.model}")
