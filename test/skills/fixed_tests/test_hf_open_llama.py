...
class TestOpenLlamaModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.test_texts = ["Open-LLaMA is a transformer model that", "Open-LLaMA can be used for"]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = (
        f"hf_open_llama_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir,
        f"hf_open_llama_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
