...
OPEN_LLAMA_MODELS_REGISTRY = {
    "openlm-research/open_llama_3b": {
        "description": "Open-LLaMA 3B model (decoder-only)",
        "class": "LlamaForCausalLM",
        "parameters": "3B"
    },
    "openlm-research/open_llama_7b": {
        "description": "Open-LLaMA 7B model (decoder-only)",
        "class": "LlamaForCausalLM",
        "parameters": "7B"
    },
    "openlm-research/open_llama_13b": {
        "description": "Open-LLaMA 13B model (decoder-only)",
        "class": "LlamaForCausalLM",
        "parameters": "13B"
    }
}
...
class TestOpenLlamaModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "openlm-research/open_llama_3b"
        
        # Verify model exists in registry
        if self.model_id not in OPEN_LLAMA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = OPEN_LLAMA_MODELS_REGISTRY["openlm-research/open_llama_3b"]
        else:
        ...
        # Define test inputs
        self.test_text = "Open-LLaMA is a transformer model that"
        self.test_texts = [
            "Open-LLaMA is a transformer model that",
            "Open-LLaMA can be used for"
        ]
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_open_llama_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(OPEN_LLAMA_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_open_llama_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = OPEN_LLAMA_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "openlm-research/open_llama_3b"
    logger.info(f"Testing model: {model_id}")
