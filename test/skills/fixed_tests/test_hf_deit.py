...
DEIT_MODELS_REGISTRY = {
    "google/deit-base-patch16-224": {
...
class TestDeitModels:
    """Base test class for all ViT-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "google/deit-base-patch16-224"
        
        # Verify model exists in registry
        if self.model_id not in DEIT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = DEIT_MODELS_REGISTRY["google/deit-base-patch16-224"]
        else:
            self.model_info = DEIT_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "DeitForImageClassification":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_deit_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(DEIT_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestDeitModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_deit_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = DEIT_MODELS_REGISTRY[model]
    ...
    model_id = args.model or "google/deit-base-patch16-224"
    logger.info(f"Testing model: {model_id}")
