...
DISTILBERT_MODELS_REGISTRY = {
    "distilbert-base-uncased": {
        "full_name": "DISTILBERT Base Uncased",
        "architecture": "encoder-only",
        "description": "DISTILBERT Base model with uncased vocabulary",
    ...
    "distilbert-large-uncased": {
        "full_name": "DISTILBERT Large Uncased",
        "architecture": "encoder-only",
        "description": "DISTILBERT Large model with uncased vocabulary",
        "distilbert": "distilbert",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "distilbert-base-cased": {
        "full_name": "DISTILBERT Base Cased",
        "architecture": "encoder-only",
        "description": "DISTILBERT Base model with cased vocabulary",
        "distilbert": "distilbert",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="distilbert-base-uncased", device=None):
        """Initialize the test class for DISTILBERT models.

        Args:
            model_id: The model ID to test (default: "distilbert-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"Testing DISTILBERT model {self.model_id} with pipeline API on {self.device}"
            )
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing DISTILBERT model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing DISTILBERT model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_distilbert_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test DISTILBERT HuggingFace models")
    parser.add_argument(
        "--model", type=str, default="distilbert-base-uncased", help="Model ID to test"
    )
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="List available DISTILBERT models"
    )
    ...
    if args.list_models:
        ...
        for model_id, info in DISTILBERT_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested DISTILBERT model: {args.model}")
