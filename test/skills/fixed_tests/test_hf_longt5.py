    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            if "generation_time" in locals():
                self.performance_stats[f"from_pretrained_{device}"]["generation_time"] = (
                    generation_time
                )
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    summary_path = os.path.join(
        output_dir, f"hf_longt5_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            ...
            print(
                f"      Parameters: {info['parameters']}, Context Length: {info['context_length']}"
            )
        return
    ...
    if success:
        ...
        if results["examples"]:
            ...
            print(f"\nExample:")
            print(
                f"  Input: {example['input'][:100]}..."
                if len(example["input"]) > 100
                else f"  Input: {example['input']}"
            )
        ...
        for test_name, result in results["results"].items():
            if isinstance(result, dict):
                if "pipeline_error" in result:
                    print(
                        f"  - Error in {test_name}: {result.get('pipeline_error_type', 'unknown')}"
                    )
