...
"""
Standardized test for the Bert model type from HuggingFace Transformers.
...
BERT_MODELS_REGISTRY = {
    "google-bert/bert-base-uncased": {
        ...
        "description": "Bert model for fill-mask",
        "model_type": "bert",
        "parameters": "110M",
...
class TestBertModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "google-bert/bert-base-uncased")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "google-bert/bert-base-uncased"

        # Verify model exists in registry
        if self.model_id not in BERT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = BERT_MODELS_REGISTRY["google-bert/bert-base-uncased"]
        else:
            self.model_config = BERT_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "BertForMaskedLM"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_bert_bert_2025-03-22T23:33:43.312900.json"
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="google-bert/bert-base-uncased", help="Model ID to test"
    )
    ...
    if success:
        ...
        print(f"  - Device: {bert_tester.preferred_device}")

        # Print pipeline performance stats if available
        if (
            pipeline_success
            and f"pipeline_{bert_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"pipeline_{bert_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{bert_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{bert_tester.preferred_device}"]
