...
MPT_MODELS_REGISTRY = {
    "mpt-base-uncased": {
        "full_name": "MPT Base Uncased",
        "architecture": "encoder-only",
        "description": "MPT Base model with uncased vocabulary",
        "mpt": "mpt",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "mpt-large-uncased": {
        "full_name": "MPT Large Uncased",
        "architecture": "encoder-only",
        "description": "MPT Large model with uncased vocabulary",
        "mpt": "mpt",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "mpt-base-cased": {
        "full_name": "MPT Base Cased",
        "architecture": "encoder-only",
        "description": "MPT Base model with cased vocabulary",
        "mpt": "mpt",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="mpt-base-uncased", device=None):
        """Initialize the test class for MPT models.

        Args:
            model_id: The model ID to test (default: "mpt-base-uncased")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing MPT model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing MPT model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_mpt_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="mpt-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available MPT models")
    ...
    if args.list_models:
        ...
        for model_id, info in MPT_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested MPT model: {args.model}")
