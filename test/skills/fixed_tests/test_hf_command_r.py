...
class TestHFCommandR(unittest.TestCase):
    """Test for the HuggingFace command-r model"""
    ...
    def test_model_loading(self):
        """Test loading command-r model with proper hardware detection"""
        try:
    ...
    def test_model_with_mock(self, mock_tokenizer, mock_model):
        """Test command-r model functionality with mocks"""
        # Setup mocks
        mock_tokenizer_instance = MagicMock()
        mock_tokenizer_instance.decode.return_value = "This is a mock response from command-r"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test command-r model")
    parser.add_argument("--model", type=str, default="CohereForAI/c4ai-command-r-v01", help="Model ID to test")
