...
FUNNEL_MODELS_REGISTRY = {
    "funnel-base-uncased": {
        "full_name": "FUNNEL Base Uncased",
        "architecture": "encoder-only",
        "description": "FUNNEL Base model with uncased vocabulary",
        "funnel": "funnel",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "funnel-large-uncased": {
        "full_name": "FUNNEL Large Uncased",
        "architecture": "encoder-only",
        "description": "FUNNEL Large model with uncased vocabulary",
        "funnel": "funnel",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "funnel-base-cased": {
        "full_name": "FUNNEL Base Cased",
        "architecture": "encoder-only",
        "description": "FUNNEL Base model with cased vocabulary",
        "funnel": "funnel",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="funnel-base-uncased", device=None):
        """Initialize the test class for FUNNEL models.

        Args:
            model_id: The model ID to test (default: "funnel-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing FUNNEL model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing FUNNEL model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing FUNNEL model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_funnel_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test FUNNEL HuggingFace models")
    parser.add_argument("--model", type=str, default="funnel-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available FUNNEL models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable FUNNEL models:")
        for model_id, info in FUNNEL_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested FUNNEL model: {args.model}")
