...
class TestStablelmModels:
    ...
    def test_pipeline(self, device=None):
        ...
        try:
            ...
            # Initialize pipeline
            generator = transformers.pipeline(self.task, model=self.model_id, device=device_idx)
            ...
            outputs = generator(self.test_text, max_length=50, num_return_sequences=1)
            ...
            self.results["pipeline"] = {"success": False, "device": device, "error": str(e)}
    ...
    def test_model_tokenizer(self, device=None):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH:
                logger.warning(
                    "Transformers or torch library not available, skipping model/tokenizer test"
                )
            ...
            logger.info(
                f"Testing StableLM model {self.model_id} with model/tokenizer API on {device}"
            )
            ...
            self.results["model_tokenizer"] = {"success": False, "device": device, "error": str(e)}
    ...
    def test_batch_inference(self, device=None):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH:
                logger.warning(
                    "Transformers or torch library not available, skipping batch inference test"
                )
                return {"success": False, "error": "Required libraries not available"}
            ...
            batch_inputs = tokenizer(
                self.test_texts, return_tensors="pt", padding=True, truncation=True
            )
            ...
            self.results["batch_inference"] = {"success": False, "device": device, "error": str(e)}
    ...
    def run_tests(self, device=None, batch=True):
        ...
        overall_results["metadata"] = {
            ...
            "test_success": any(
                [r.get("success", False) for r in overall_results.values() if isinstance(r, dict)]
            ),
            "dependencies": {
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="stabilityai/stablelm-base-alpha-7b",
