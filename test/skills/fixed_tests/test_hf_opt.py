...
class TestOptModels:
    ...
    def __init__(self, model_id=None):
        ...
        self.model_id = model_id or "opt"
        
        # Verify model exists in registry
        if self.model_id not in OPT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = OPT_MODELS_REGISTRY["opt"]
        else:
            self.model_info = OPT_MODELS_REGISTRY[self.model_id]
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "OptLMHeadModel":
                model_class = transformers.OptLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_opt_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(OPT_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_opt_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = OPT_MODELS_REGISTRY[model]
