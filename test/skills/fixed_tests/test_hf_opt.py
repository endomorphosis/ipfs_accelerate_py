...
OPT_MODELS_REGISTRY = {
    "opt-base-uncased": {
        "full_name": "OPT Base Uncased",
        "architecture": "encoder-only",
        "description": "OPT Base model with uncased vocabulary",
        "opt": "opt",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "opt-large-uncased": {
        "full_name": "OPT Large Uncased",
        "architecture": "encoder-only",
        "description": "OPT Large model with uncased vocabulary",
        "opt": "opt",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "opt-base-cased": {
        "full_name": "OPT Base Cased",
        "architecture": "encoder-only",
        "description": "OPT Base model with cased vocabulary",
        "opt": "opt",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="opt-base-uncased", device=None):
        """Initialize the test class for OPT models.

        Args:
            model_id: The model ID to test (default: "opt-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing OPT model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing OPT model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing OPT model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_opt_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test OPT HuggingFace models")
    parser.add_argument("--model", type=str, default="opt-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available OPT models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable OPT models:")
        for model_id, info in OPT_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested OPT model: {args.model}")
