...
RESNET_MODELS_REGISTRY = {
    "resnet-base-uncased": {
        "full_name": "RESNET Base Uncased",
        "architecture": "encoder-only",
        "description": "RESNET Base model with uncased vocabulary",
        "resnet": "resnet",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "resnet-large-uncased": {
        "full_name": "RESNET Large Uncased",
        "architecture": "encoder-only",
        "description": "RESNET Large model with uncased vocabulary",
        "resnet": "resnet",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "resnet-base-cased": {
        "full_name": "RESNET Base Cased",
        "architecture": "encoder-only",
        "description": "RESNET Base model with cased vocabulary",
        "resnet": "resnet",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="resnet-base-uncased", device=None):
        """Initialize the test class for RESNET models.

        Args:
            model_id: The model ID to test (default: "resnet-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing RESNET model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(
                f"Testing RESNET model {self.model_id} with from_pretrained API on {self.device}"
            )
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing RESNET model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_resnet_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test RESNET HuggingFace models")
    parser.add_argument("--model", type=str, default="resnet-base-uncased", help="Model ID to test")
    ...
    parser.add_argument("--list-models", action="store_true", help="List available RESNET models")
    ...
    if args.list_models:
        ...
        for model_id, info in RESNET_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested RESNET model: {args.model}")
