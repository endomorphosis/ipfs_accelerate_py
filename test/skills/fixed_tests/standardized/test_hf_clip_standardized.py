...
"""
Standardized test for the Clip model type from HuggingFace Transformers.
...
CLIP_MODELS_REGISTRY = {
    "clip-base": {
        ...
        "description": "Clip model for fill-mask",
        "model_type": "clip",
        "parameters": "110M",
...
class TestClipModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "clip-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "clip-base"
        
        # Verify model exists in registry
        if self.model_id not in CLIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = CLIP_MODELS_REGISTRY["clip-base"]
        else:
            self.model_config = CLIP_MODELS_REGISTRY[self.model_id]
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_clip_clip_2025-03-22T23:36:32.036483.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Clip HuggingFace models")
    parser.add_argument("--model", type=str, default="clip-base", help="Model ID to test")
    ...
    clip_tester = TestClipModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = clip_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {clip_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{clip_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{clip_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{clip_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{clip_tester.preferred_device}"]
