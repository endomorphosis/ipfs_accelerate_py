...
class TestIdeficsModels(ModelTest):
    ...
    def verify_model_output(self, model, input_data, expected_output=None):
        ...
        try:
            ...
            inputs = processor(prompt_with_image, images=[image], return_tensors="pt")
            ...
            # Generate text
            with torch.no_grad():
                outputs = idefics_model.generate(**inputs, max_new_tokens=100, do_sample=False)
    ...
    def test_multimodal_conversation(self):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH or not HAS_PIL:
                logger.warning(
                    "Required libraries not available, skipping multimodal conversation test"
                )
                return {
                    "success": False,
            ...
            inputs = processor(continuation_prompt, images=[image], return_tensors="pt")
...
def main():
    ...
    if args.unittest or "unittest" in sys.argv:
        ...
        conversation_success = (
            results["results"].get("multimodal_conversation", {}).get("success", False)
        )
