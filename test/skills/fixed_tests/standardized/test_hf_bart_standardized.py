...
"""
Standardized test for the Bart model type from HuggingFace Transformers.
...
# Model registry
BART_MODELS_REGISTRY = {
    "bart-base": {
        "full_name": "Bart Base",
        "architecture": "encoder-only",
        "description": "Bart model for fill-mask",
        "model_type": "bart",
        "parameters": "110M",
...
class TestBartModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "bart-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "bart-base"

        # Verify model exists in registry
        if self.model_id not in BART_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = BART_MODELS_REGISTRY["bart-base"]
        else:
            self.model_config = BART_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "BartModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_bart_bart_2025-03-22T23:36:31.833413.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Bart HuggingFace models")
    parser.add_argument("--model", type=str, default="bart-base", help="Model ID to test")
    ...
    bart_tester = TestBartModels(model_id=args.model, device=args.device)

    # Run the tests
    results = bart_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {bart_tester.preferred_device}")

        # Print pipeline performance stats if available
        if (
            pipeline_success
            and f"pipeline_{bart_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"pipeline_{bart_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{bart_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{bart_tester.preferred_device}"]
