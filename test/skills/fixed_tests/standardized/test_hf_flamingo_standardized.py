...
class TestFlamingoModels(ModelTest):
    ...
    def load_model(self, model_name):
        ...
        try:
            ...
            if HAS_TRANSFORMERS and HAS_TORCH:
                ...
                return {"model": MagicMock(), "processor": MagicMock()}
    ...
    def verify_model_output(self, model, input_data, expected_output=None):
        ...
        try:
            ...
            # Generate text
            with torch.no_grad():
                outputs = flamingo_model.generate(**inputs, max_new_tokens=100, do_sample=False)
    ...
    def test_model_loading(self):
        ...
        try:
            ...
            self.assertIn(
                "processor", model_components, "Model dict should contain 'processor' key"
            )
    ...
    def test_multi_image_sequence(self):
        ...
        try:
            if not HAS_TRANSFORMERS or not HAS_TORCH or not HAS_PIL:
                logger.warning(
                    "Required libraries not available, skipping multi-image sequence test"
                )
                return {
                    "success": False,
            ...
            for i in range(3):  # Create 3 test images with different colors
                color = (73 + i * 60, 109 + i * 30, 137 - i * 30)
    ...
    def run_tests(self, all_hardware=False):
        ...
        if model_components:
            try:
                ...
                test_input = {"image": test_image, "prompt": test_prompt}
...
def main():
    ...
    if args.unittest or "unittest" in sys.argv:
        ...
        multi_image_success = (
            results["results"].get("multi_image_sequence", {}).get("success", False)
        )

        # Determine if real inference or mock objects were used
        using_real_inference = (
            results["metadata"]["has_transformers"]
            and results["metadata"]["has_torch"]
            and results["metadata"]["has_pil"]
        )
        ...
        if using_real_inference:
            ...
            print(
                f"   Dependencies: transformers={results['metadata']['has_transformers']}, torch={results['metadata']['has_torch']}, PIL={results['metadata']['has_pil']}"
            )
