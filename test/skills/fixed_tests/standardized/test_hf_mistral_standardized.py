...
"""
Standardized test for the Mistral model type from HuggingFace Transformers.
...
MISTRAL_MODELS_REGISTRY = {
    "mistral-base": {
        "full_name": "Mistral Base",
        "architecture": "encoder-only",
        "description": "Mistral model for fill-mask",
        "model_type": "mistral",
        "parameters": "110M",
...
class TestMistralModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "mistral-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "mistral-base"
        
        # Verify model exists in registry
        if self.model_id not in MISTRAL_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = MISTRAL_MODELS_REGISTRY["mistral-base"]
        else:
            self.model_config = MISTRAL_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "MistralModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_mistral_mistral_2025-03-22T23:36:32.178602.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Mistral HuggingFace models")
    parser.add_argument("--model", type=str, default="mistral-base", help="Model ID to test")
    ...
    mistral_tester = TestMistralModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = mistral_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {mistral_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{mistral_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{mistral_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{mistral_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{mistral_tester.preferred_device}"]
