...
"""
Standardized test for the Electra model type from HuggingFace Transformers.
...
ELECTRA_MODELS_REGISTRY = {
    "electra-base": {
        "full_name": "Electra Base",
        "architecture": "encoder-only",
        "description": "Electra model for fill-mask",
        "model_type": "electra",
        "parameters": "110M",
...
class TestElectraModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "electra-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "electra-base"
        
        # Verify model exists in registry
        if self.model_id not in ELECTRA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = ELECTRA_MODELS_REGISTRY["electra-base"]
        else:
            self.model_config = ELECTRA_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "ElectraModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_electra_electra_2025-03-22T23:36:31.871228.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="electra-base", help="Model ID to test")
    ...
    if success:
        ...
        print(f"  - Device: {electra_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{electra_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{electra_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{electra_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{electra_tester.preferred_device}"]
