...
"""
Standardized test for the Wav2vec2 model type from HuggingFace Transformers.
...
WAV2VEC2_MODELS_REGISTRY = {
    "wav2vec2-base": {
        ...
        "description": "Wav2vec2 model for fill-mask",
        "model_type": "wav2vec2",
        "parameters": "110M",
...
class TestWav2vec2Models:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "wav2vec2-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "wav2vec2-base"

        # Verify model exists in registry
        if self.model_id not in WAV2VEC2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = WAV2VEC2_MODELS_REGISTRY["wav2vec2-base"]
        else:
            self.model_config = WAV2VEC2_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "Wav2vec2Model"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_wav2vec2_wav2vec2_2025-03-22T23:36:31.974457.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Wav2vec2 HuggingFace models")
    parser.add_argument("--model", type=str, default="wav2vec2-base", help="Model ID to test")
    ...
    wav2vec2_tester = TestWav2vec2Models(model_id=args.model, device=args.device)

    # Run the tests
    results = wav2vec2_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {wav2vec2_tester.preferred_device}")

        # Print pipeline performance stats if available
        if (
            pipeline_success
            and f"pipeline_{wav2vec2_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"pipeline_{wav2vec2_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{wav2vec2_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{wav2vec2_tester.preferred_device}"]
