    ...
    def verify_model_output(self, model, input_data, expected_output=None):
        ...
        try:
            ...
            if hasattr(xclip_model.config, "id2label"):
                ...
                class_label = xclip_model.config.id2label.get(
            ...
            result = {"predictions": predictions, "inference_time": inference_time}

            # Verify output
            if HAS_TRANSFORMERS and not isinstance(xclip_model, MagicMock):
                self.assertIsNotNone(logits, "Model output should not be None")
                self.assertGreater(
                    len(list(logits.shape)) if hasattr(logits, "shape") else 0,
                    0,
                    "Logits should not be empty",
                )
        ...
        try:
            ...
            for frame_count in frame_counts:
                ...
                try:
                    ...
                    results[test_key] = {"success": False, "error": str(e)}

        except Exception as e:
            logger.error(f"Error in variable frame count test: {e}")
            for frame_count in frame_counts:
                results[f"frames_{frame_count}"] = {"success": False, "error": str(e)}
