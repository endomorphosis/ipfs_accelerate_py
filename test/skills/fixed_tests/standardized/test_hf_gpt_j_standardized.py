...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_gpt-j_gpt-j_2025-03-22T23:36:31.677717.json"
...
def main():
    ...
    if success:
        ...
        if (
            pipeline_success
            and f"pipeline_{gpt_j_tester.preferred_device}" in results["performance"]
        ):
        ...
        if (
            pretrained_success
            and f"from_pretrained_{gpt_j_tester.preferred_device}" in results["performance"]
        ):
