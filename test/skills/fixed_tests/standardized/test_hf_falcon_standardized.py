...
if not HAS_TRANSFORMERS:
    ...
    transformers.pipeline = lambda *args, **kwargs: (
        lambda x: [{"generated_text": "Mock generated text"}]
    )
...
class TestFalcon(ModelTest):
    ...
    def setUp(self):
        ...
        # Define test inputs
        self.test_prompts = ["Once upon a time", "The quick brown fox", "In the beginning"]
    ...
    def load_model(self, model_name):
        ...
        if not HAS_TRANSFORMERS or not HAS_TORCH:
            ...
            return {"model": MagicMock(), "tokenizer": MagicMock(), "is_mock": True}
        ...
        try:
            ...
            model_info = next(
                (info for _, info in MODEL_REGISTRY.items() if info["model_id"] == model_name), None
            )
            ...
            return {"model": model, "tokenizer": tokenizer, "is_mock": False}
    ...
    def verify_model_output(self, model_components, input_data, expected_output=None):
        ...
        try:
            ...
            self.assertGreater(
                len(generated_text), len(prompt), "Generated text should be longer than prompt"
            )
    ...
    def run_all_tests(self):
        ...
        results = {"model_info": self.model_info, "device": self.preferred_device, "tests": {}}
        ...
        try:
            ...
            results["tests"]["model_loading"] = {"status": "failed", "error": str(e)}
        ...
        try:
            ...
            results["tests"]["text_generation"] = {"status": "failed", "error": str(e)}
        ...
        if self.model_info.get("is_instruct", False):
            try:
                ...
                results["tests"]["instruction_following"] = {"status": "failed", "error": str(e)}
