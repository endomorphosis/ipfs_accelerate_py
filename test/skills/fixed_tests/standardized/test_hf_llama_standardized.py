...
"""
Standardized test for the Llama model type from HuggingFace Transformers.
...
LLAMA_MODELS_REGISTRY = {
    "llama-base": {
        "full_name": "Llama Base",
        "architecture": "encoder-only",
        "description": "Llama model for fill-mask",
        "model_type": "llama",
        "parameters": "110M",
...
class TestLlamaModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "llama-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "llama-base"

        # Verify model exists in registry
        if self.model_id not in LLAMA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = LLAMA_MODELS_REGISTRY["llama-base"]
        else:
            self.model_config = LLAMA_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "LlamaModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_llama_llama_2025-03-22T23:36:32.130216.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Llama HuggingFace models")
    parser.add_argument("--model", type=str, default="llama-base", help="Model ID to test")
    ...
    llama_tester = TestLlamaModels(model_id=args.model, device=args.device)

    # Run the tests
    results = llama_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {llama_tester.preferred_device}")

        # Print pipeline performance stats if available
        if (
            pipeline_success
            and f"pipeline_{llama_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"pipeline_{llama_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{llama_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{llama_tester.preferred_device}"]
