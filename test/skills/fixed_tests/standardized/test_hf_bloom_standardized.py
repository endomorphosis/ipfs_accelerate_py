...
class TestBloomModels(ModelTest):
    ...
    def setUp(self):
        ...
        # Define test inputs
        self.test_prompts = ["Once upon a time", "The quick brown fox", "In a world where"]
    ...
    def load_model(self, model_name):
        ...
        try:
            ...
            # Load the model
            model = model_class.from_pretrained(model_name, torch_dtype=dtype)
            ...
            if HAS_TRANSFORMERS and HAS_TORCH:
                ...
                return {"model": MagicMock(), "tokenizer": MagicMock()}
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if self.task == "text-generation":
                    outputs = bloom_model.generate(
                        **inputs, max_new_tokens=30, do_sample=True, top_p=0.95, temperature=0.8
                    )
            ...
            if HAS_TRANSFORMERS and HAS_TORCH:
                ...
                return {
                    ...
                    else input_data.get("prompt", "mock prompt"),
    ...
    def test_model_loading(self):
        ...
        try:
            ...
            self.assertIn(
                "tokenizer", model_components, "Model dict should contain 'tokenizer' key"
            )
            ...
            if isinstance(bloom_model, MagicMock):
                ...
                if "bloom" in self.model_id.lower():
                    self.assertIn(
                        "Bloom",
                        bloom_model.__class__.__name__,
    ...
    def test_multiple_prompts(self):
        ...
        try:
            ...
            # Add to examples
            self.examples.append({"method": "multiple_prompts", "results": results})

            return {"success": True, "results": results, "device": self.preferred_device}

        except Exception as e:
            logger.error(f"Error in multiple prompts test: {e}")
            return {"success": False, "error": str(e), "device": self.preferred_device}
    ...
    def test_with_pipeline(self):
        ...
        try:
            ...
            # Add to examples
            self.examples.append({"method": "pipeline", "input": prompt, "output": outputs})
    ...
    def test_with_openvino(self):
        ...
        try:
            ...
            # Add to examples
            self.examples.append({"method": "openvino", "input": prompt, "output": generated_text})
        ...
        if model_components:
            try:
                ...
                self.examples.append(
                    {"method": "verify_model_output", "input": test_prompt, "output": output}
                )

                # Store in results
                self.results["basic_inference"] = {"success": True, "output": output}
            except Exception as e:
                logger.error(f"Error running model verification: {e}")
                self.results["basic_inference"] = {"success": False, "error": str(e)}
...
def main():
    ...
    if args.unittest or "unittest" in sys.argv:
        ...
        # Determine if real inference or mock objects were used
        using_real_inference = (
            results["metadata"]["has_transformers"] and results["metadata"]["has_torch"]
        )
        ...
        if using_real_inference:
            ...
            print(
                f"   Dependencies: transformers={results['metadata']['has_transformers']}, torch={results['metadata']['has_torch']}"
            )
        ...
        if basic_success or pipeline_success:
            ...
            if results["examples"]:
                ...
                if (
                    "output" in example
                    and isinstance(example["output"], dict)
                    and "generated_text" in example["output"]
                ):
                    print(f"\nSample output:")
