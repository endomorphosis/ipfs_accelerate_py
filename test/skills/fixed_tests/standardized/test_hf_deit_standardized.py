...
"""
Standardized test for the Deit model type from HuggingFace Transformers.
...
DEIT_MODELS_REGISTRY = {
    "deit-base": {
        ...
        "description": "Deit model for fill-mask",
        "model_type": "deit",
        "parameters": "110M",
...
class TestDeitModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "deit-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "deit-base"
        
        # Verify model exists in registry
        if self.model_id not in DEIT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = DEIT_MODELS_REGISTRY["deit-base"]
        else:
            self.model_config = DEIT_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "DeitModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_deit_deit_2025-03-22T23:36:31.923056.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Deit HuggingFace models")
    parser.add_argument("--model", type=str, default="deit-base", help="Model ID to test")
    ...
    deit_tester = TestDeitModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = deit_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {deit_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{deit_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{deit_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{deit_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{deit_tester.preferred_device}"]
