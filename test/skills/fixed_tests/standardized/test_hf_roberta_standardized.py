...
"""
Standardized test for the Roberta model type from HuggingFace Transformers.
...
# Model registry
ROBERTA_MODELS_REGISTRY = {
    "roberta-base": {
        "full_name": "Roberta Base",
        "architecture": "encoder-only",
        "description": "Roberta model for fill-mask",
        "model_type": "roberta",
        "parameters": "110M",
...
class TestRobertaModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "roberta-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "roberta-base"

        # Verify model exists in registry
        if self.model_id not in ROBERTA_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = ROBERTA_MODELS_REGISTRY["roberta-base"]
        else:
            self.model_config = ROBERTA_MODELS_REGISTRY[self.model_id]

        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "RobertaModel"
        self.architecture = self.model_config.get("architecture", "encoder-only")
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_roberta_roberta_2025-03-22T23:36:31.720625.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Roberta HuggingFace models")
    parser.add_argument("--model", type=str, default="roberta-base", help="Model ID to test")
    ...
    roberta_tester = TestRobertaModels(model_id=args.model, device=args.device)

    # Run the tests
    results = roberta_tester.run_tests(all_hardware=args.all_hardware)
    ...
    pipeline_success = any(
        r.get("pipeline_success", False) for r in results["results"].values() if "pipeline" in r
    )
    pretrained_success = any(
        r.get("from_pretrained_success", False)
        for r in results["results"].values()
        if "from_pretrained" in r
    )
    ...
    if success:
        ...
        print(f"  - Device: {roberta_tester.preferred_device}")

        # Print pipeline performance stats if available
        if (
            pipeline_success
            and f"pipeline_{roberta_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"pipeline_{roberta_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")

        # Print from_pretrained performance stats if available
        if (
            pretrained_success
            and f"from_pretrained_{roberta_tester.preferred_device}" in results["performance"]
        ):
            stats = results["performance"][f"from_pretrained_{roberta_tester.preferred_device}"]
        ...
        for test_name, result in results["results"].items():
            if "pipeline_error" in result:
                ...
                print(
                    f"  - Error in {test_name}: {result.get('from_pretrained_error_type', 'unknown')}"
                )
