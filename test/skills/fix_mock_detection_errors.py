    ...
    for prefix, (wrong, correct) in model_class_fixes.items():
        if model_type.startswith(prefix):
            ...
            content = content.replace(
                f'self.class_name == "{wrong}"', f'self.class_name == "{correct}"'
            )
...
try:
    ...
    if "import sentencepiece" not in content or "HAS_SENTENCEPIECE" not in content:
        ...
        tokenizers_import = re.search(
            r'# Try to import tokenizers.*?logger\.warning\("tokenizers not available, using mock"\)',
            content,
            re.DOTALL,
        )
