...
class FaultToleranceTestRunner:
    ...
    async def run_tests(self):
        ...
        logger.info(
            f"All tests completed in {total_duration:.2f}s. Summary saved to {summary_file}"
        )
    ...
    async def _run_cross_browser_sharding_tests(self):
        ...
        if self.args.comprehensive:
                ...
                for recovery_strategy in recovery_strategies:
                    ...
                    cmd.extend(
                        [
                            ...
                            os.path.join(
                                self.output_dir,
                                f"sharding_{strategy}_{recovery_strategy}_{self.timestamp}.json",
                            ),
                        ]
                    )
                    ...
                    # Log the command
                    logger.info(
                        f"Running cross-browser sharding test with {strategy} strategy and {recovery_strategy} recovery"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Sharding test with {strategy} strategy and {recovery_strategy} recovery completed with exit code: {process.returncode}"
                        )
            ...
            if self.args.fault_tolerance:
                cmd.extend(
                    [
                        "--fault-tolerance",
                        "--fault-tolerance-level",
                        self.args.fault_tolerance_level or "medium",
                    ]
                )
            ...
            try:
                ...
                results.append(result)
                logger.info(
                    f"Sharding test with {strategy} strategy completed with exit code: {process.returncode}"
                )
                            ...
                            results.append(
                                {
                                    ...
                                    "status": "success"
                                    if strategy_data.get("success_rate", 0) > 0.9
                                    else "failure",
                                }
                            )
                except Exception as e:
    ...
    def _generate_summary(self, results, total_duration):
        ...
        for result in results:
            ...
            summary["by_test_type"].setdefault(
                test_type, {"total": 0, "success": 0, "failure": 0, "error": 0}
            )
        ...
        for result in results:
            if "strategy" in result:
                ...
                summary["by_strategy"].setdefault(
                    strategy, {"total": 0, "success": 0, "failure": 0, "error": 0}
                )
        ...
        for result in results:
            if "recovery_strategy" in result:
                ...
                summary["by_recovery_strategy"].setdefault(
                    recovery, {"total": 0, "success": 0, "failure": 0, "error": 0}
                )
    ...
    def _print_summary(self, summary):
        ...
        print(
            f"Successful:      {summary['successful_tests']} ({summary['success_rate'] * 100:.1f}%)"
        )
        ...
        for test_type, data in summary["by_test_type"].items():
            print(
                f"  {test_type.ljust(20)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
            )
        ...
        if summary["by_strategy"]:
            ...
            for strategy, data in summary["by_strategy"].items():
                print(
                    f"  {strategy.ljust(20)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
                )
        ...
        if summary["by_recovery_strategy"]:
            ...
            for strategy, data in summary["by_recovery_strategy"].items():
                print(
                    f"  {strategy.ljust(15)}: {data['success_rate'] * 100:.1f}% success ({data['success']}/{data['total']})"
                )
...
def main():
    ...
    mode_group.add_argument(
        "--cross-browser-sharding",
        action="store_true",
        help="Run cross-browser model sharding tests",
    )
    mode_group.add_argument("--e2e", action="store_true", help="Run comprehensive end-to-end tests")

    # Basic options (original behavior)
    basic_group = parser.add_argument_group("Basic Options")
    basic_group.add_argument(
        "--model", type=str, default="bert-base-uncased", help="Model name to test"
    )
    basic_group.add_argument(
        "--browsers",
        type=str,
    ...
    basic_group.add_argument(
        "--mock", action="store_true", help="Use mock implementation for testing without browsers"
    )
    basic_group.add_argument("--basic", action="store_true", help="Run basic integration test")
    basic_group.add_argument(
        "--comparative", action="store_true", help="Run comparative integration test"
    )
    basic_group.add_argument(
        "--stress-test", action="store_true", help="Run stress test integration"
    )
    basic_group.add_argument(
        "--resource-pool", action="store_true", help="Test integration with resource pool"
    )
    ...
    sharding_group.add_argument(
        "--all-strategies", action="store_true", help="Test all sharding strategies"
    )

    # Fault tolerance options
    fault_group = parser.add_argument_group("Fault Tolerance Options")
    fault_group.add_argument(
        "--fault-tolerance", action="store_true", help="Enable fault tolerance testing"
    )
    fault_group.add_argument(
        "--fault-tolerance-level",
        type=str,
    ...
    fault_group.add_argument(
        "--failure-type", type=str, choices=FAILURE_MODES, help="Specific failure mode to test"
    )
    fault_group.add_argument(
        "--simulate-failure", action="store_true", help="Simulate browser failure during tests"
    )
    ...
    perf_group.add_argument("--performance-test", action="store_true", help="Run performance tests")
    ...
    output_group.add_argument("--output-dir", type=str, help="Directory for output files")
    output_group.add_argument("--verbose", action="store_true", help="Enable verbose logging")

    # Advanced options
    advanced_group = parser.add_argument_group("Advanced Options")
    advanced_group.add_argument(
        "--comprehensive", action="store_true", help="Run comprehensive tests with all options"
    )
    advanced_group.add_argument(
        "--concurrent-tests",
        type=int,
