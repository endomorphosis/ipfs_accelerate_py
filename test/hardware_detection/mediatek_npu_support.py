...
class MediaTekNPUWrapper:
    ...
    def __init__(self, version: str = "3.1", simulation_mode: bool = False):
        ...
        if simulation_mode:
            logger.warning(
                "MediaTek NPU running in SIMULATION mode. No real hardware will be used."
            )
        ...
        try:
            ...
            if sdk_root and os.path.exists(sdk_root):
                ...
                if mediatek_detected:
                    ...
                    logger.warning(
                        "MediaTek NPU SDK not available. Set MEDIATEK_APU_SDK_ROOT for real hardware."
                    )
                    self.available = False
...
MEDIATEK_NPU_SIMULATION_MODE = os.environ.get("MEDIATEK_NPU_SIMULATION_MODE", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
try:
    ...
    try:
        ...
        try:
            ...
            if MEDIATEK_NPU_SIMULATION_MODE:
                ...
                logger.warning(
                    "MediaTek NPU SDK not found. Using SIMULATION mode as requested by environment variable."
                )
                ...
                logger.warning(
                    "MediaTek NPU SDK not available. Set MEDIATEK_NPU_SIMULATION_MODE=1 for simulation."
                )
    ...
    if MEDIATEK_NPU_SIMULATION_MODE:
        mediatek_sdk = MediaTekNPUWrapper(simulation_mode=True)
        logger.warning(
            "MediaTek NPU SDK initialization failed. Using SIMULATION mode as requested."
        )
        MEDIATEK_NPU_AVAILABLE = True  # Simulation is "available"
    else:
        mediatek_sdk = MediaTekNPUWrapper(simulation_mode=False)
...
class MediaTekNPUCapabilityDetector:
    ...
    def get_capability_summary(self) -> Dict[str, Any]:
        ...
        if self.is_simulation or self.selected_device.get("simulated", False):
            summary["simulation_warning"] = (
                "This is a SIMULATED device. Results do not reflect real hardware performance."
            )
    ...
    def _get_recommended_models(self) -> List[str]:
        ...
        if "int4" in precision:
            recommendations.extend(["mobilevit-int4", "efficientnet-int4", "mobilebert-int4"])
        ...
        for metric, base_value in base_performance.items():
            if "latency" in metric:
                ...
                scaled_value = base_value / (
                    cu_factor * compute_units / 6 + mem_factor * memory_mb / 2048
                )
            else:
        ...
        if os.path.exists(model_path):
            ...
            if is_simulated:
                result["simulation_warning"] = (
                    "This compatibility assessment is SIMULATED and may not reflect actual hardware compatibility."
                )
            ...
            if (
                "mobilenet" in model_path_lower
                or "mobile" in model_path_lower
                or "efficient" in model_path_lower
            ):
                compatibility = True
                ...
                compatibility = True
                reason = (
                    "Compatibility assessed based on filename pattern; actual testing recommended"
                )
            ...
            result["simulation_warning"] = (
                "This compatibility assessment is based on filename pattern only and should not be used for production decisions."
            )
...
class MediaTekPowerMonitor:
    ...
    def stop_monitoring(self) -> Dict[str, Any]:
        ...
        for i in range(sample_count):
            ...
            power_watts = base_power * power_factor + random.uniform(
            ...
            self.monitoring_data.append(
                {
                    ...
                    "battery_temp_celsius": max(
                        20, temp_celsius - 3 + random.uniform(-1, 1)
                    ),  # Battery temp follows SOC
        ...
        avg_soc_temp = sum(d["soc_temp_celsius"] for d in self.monitoring_data) / len(
            self.monitoring_data
        )
        ...
        summary = {
            ...
            "thermal_throttling_duration_seconds": throttling_points
            / 10,  # Assuming 10 samples per second
        ...
        logger.info(
            f"Completed power monitoring: avg={avg_power:.2f}W, max={max_power:.2f}W, impact={battery_impact_percent:.2f}%"
        )
        return summary
    ...
    def estimate_battery_life(
        ...
        incremental_percent = (
            (incremental_power / avg_power_watts) * 100 if avg_power_watts > 0 else 0
        )
...
class MediaTekModelOptimizer:
    ...
    def simulate_optimization(self, model_path: str, optimizations: List[str]) -> Dict[str, Any]:
        ...
        # Check if we're in simulation mode
        is_simulated = self.detector.is_simulation or self.detector.selected_device.get(
            "simulated", False
        )
        ...
        model_filename = os.path.basename(model_path)
        original_size = (
            os.path.getsize(model_path) if os.path.exists(model_path) else 50 * 1024 * 1024
        )  # 50MB default
        ...
        result = {
            ...
            "device": self.detector.selected_device["name"]
            if self.detector.selected_device
            else "Unknown",
            "simulation_mode": is_simulated
            or True,  # Always mark optimizations as simulated for now
        }

        # Add simulation warning
        result["simulation_warning"] = (
            "These optimization results are SIMULATED and do not reflect actual measurements on real hardware."
        )
...
def main():
    ...
    if args.command == "detect":
        ...
        if args.json:
            ...
            print(
                f"Thermal Throttling: {'Yes' if results['thermal_throttling_detected'] else 'No'}"
            )
            if results["thermal_throttling_detected"]:
                print(
                    f"Throttling Duration: {results['thermal_throttling_duration_seconds']:.2f} seconds"
                )
        ...
        if args.json:
            ...
            if recommendations["compatible"]:
                ...
                print(
                    f"Estimated Power Efficiency Score: {recommendations['estimated_power_efficiency_score']}/100"
                )
