...
try:
    from ipfs_accelerate_py.kit.docker_kit import DockerKit, DockerResult, get_docker_kit
...
class TestDockerKit(unittest.TestCase):
    ...
    def test_verify_installation_success(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="Docker version 24.0.0, build abc123", stderr=""
        )
    ...
    def test_run_container_success(self, mock_run):
        ...
        kit = DockerKit()
        result = kit.run_container(image="python:3.9", command="python --version")
    ...
    def test_execute_code_in_container(self, mock_run):
        ...
        kit = DockerKit()
        result = kit.execute_code_in_container(image="python:3.9", code="print('hello')")
    ...
    def test_build_image(self, mock_run):
        ...
        if hasattr(kit, "build_image"):
            result = kit.build_image(context_path="/tmp/test", tag="test:latest")
            self.assertIsInstance(result, DockerResult)
        else:
