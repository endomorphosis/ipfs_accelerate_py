...
def setup_args():
    ...
    parser.add_argument(
        "--sample", help="Path to sample Python file", default="sample_webgpu_backend.py"
    )
...
def run_original_converter(input_file: str, output_file: str) -> bool:
    ...
    try:
        ...
        converter_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "setup_ipfs_accelerate_js_py_converter.py"
        )
...
def compare_converters(input_file: str) -> Dict[str, any]:
    ...
    original_output = os.path.join(
        Config.OUTPUT_DIRECTORY, os.path.basename(input_file).replace(".py", "_original.ts")
    )
    improved_output = os.path.join(
        Config.OUTPUT_DIRECTORY, os.path.basename(input_file).replace(".py", "_improved.ts")
    )
    ...
    metrics = {
        "original": {
            ...
            "typed_methods": len(
                re.findall(r"[a-zA-Z0-9_]+\([^)]*\)\s*:\s*[a-zA-Z0-9_<>|]+", original_content)
            ),
            "typed_props": len(
                re.findall(r"[a-zA-Z0-9_]+\s*:\s*[a-zA-Z0-9_<>|]+\s*[;=]", original_content)
            ),
        ...
        "improved": {
            ...
            "typed_methods": len(
                re.findall(r"[a-zA-Z0-9_]+\([^)]*\)\s*:\s*[a-zA-Z0-9_<>|]+", improved_content)
            ),
            "typed_props": len(
                re.findall(r"[a-zA-Z0-9_]+\s*:\s*[a-zA-Z0-9_<>|]+\s*[;=]", improved_content)
            ),
...
def test_sample_file():
    ...
    if Config.COMPARISON:
        ...
        if result["comparison"]:
            ...
            print("\nSummary:")
            print(
                f"- Improved converter generated {metrics['improved']['interfaces'] - metrics['original']['interfaces']} more interfaces"
            )
            print(
                f"- Improved converter has {metrics['improved']['typescript_errors'] - metrics['original']['typescript_errors']} more TypeScript errors"
            )
            print(
                f"- Improved converter has {metrics['improved']['typed_methods'] - metrics['original']['typed_methods']} more typed methods"
            )
            print(
                f"- Improved converter has {metrics['improved']['typed_props'] - metrics['original']['typed_props']} more typed properties"
            )
            ...
            if metrics["improved"]["typescript_errors"] < metrics["original"]["typescript_errors"]:
                ...
                metrics["improved"]["typescript_errors"] > metrics["original"]["typescript_errors"]
            ):
                print("\nRecommendation: The original converter has fewer TypeScript errors")
            else:
                better_typing = (
                    metrics["improved"]["typed_methods"] + metrics["improved"]["typed_props"]
                ) > (metrics["original"]["typed_methods"] + metrics["original"]["typed_props"])
                if better_typing:
        ...
        output_file = os.path.join(
            Config.OUTPUT_DIRECTORY,
            os.path.basename(Config.SAMPLE_FILE).replace(".py", "_improved.ts"),
        )
...
def test_directory():
    ...
    if Config.COMPARISON:
        ...
        if all_metrics:
            avg_metrics = {"original": {}, "improved": {}}

            for key in all_metrics[0]["original"]:
                avg_metrics["original"][key] = sum(m["original"][key] for m in all_metrics) / len(
                    all_metrics
                )
                avg_metrics["improved"][key] = sum(m["improved"][key] for m in all_metrics) / len(
                    all_metrics
                )
            ...
            print("\nSummary:")
            print(
                f"- Improved converter generates {avg_metrics['improved']['interfaces'] - avg_metrics['original']['interfaces']:.1f} more interfaces on average"
            )
            print(
                f"- Improved converter has {avg_metrics['improved']['typescript_errors'] - avg_metrics['original']['typescript_errors']:.1f} more TypeScript errors on average"
            )
            print(
                f"- Improved converter has {avg_metrics['improved']['typed_methods'] - avg_metrics['original']['typed_methods']:.1f} more typed methods on average"
            )
            print(
                f"- Improved converter has {avg_metrics['improved']['typed_props'] - avg_metrics['original']['typed_props']:.1f} more typed properties on average"
            )

            # Overall recommendation
            if (
                avg_metrics["improved"]["typescript_errors"]
                < avg_metrics["original"]["typescript_errors"]
            ):
                ...
                better_typing = (
                    avg_metrics["improved"]["typed_methods"]
                    + avg_metrics["improved"]["typed_props"]
                ) > (
                    avg_metrics["original"]["typed_methods"]
                    + avg_metrics["original"]["typed_props"]
                )
                if better_typing:
                    print(
        ...
        print(
            f"Improved Converter: {success_count} successes, {failure_count} failures ({success_count / len(results) * 100:.1f}% success rate)"
        )
