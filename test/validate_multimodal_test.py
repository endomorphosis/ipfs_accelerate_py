    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                classes[node.name] = {"name": node.name, "bases": bases, "methods": methods}
        ...
        return {
            ...
            "results": results,
            "overall_compliant": all(r["has_all_required_methods"] for r in results)
            if results
            else False,
        }
    except Exception as e:
        logger.error(f"Error analyzing file {file_path}: {e}")
        return {"file_path": file_path, "error": str(e), "overall_compliant": False}
...
def validate_test_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"file_path": file_path, "exists": True, "compliant": False, "error": str(e)}
            ...
            if "analysis" in result and "results" in result["analysis"]:
                for class_result in result["analysis"]["results"]:
                    if not class_result["has_all_required_methods"]:
                        logger.info(
                            f"   Class {class_result['class_name']} is missing methods: {class_result['missing_methods']}"
                        )
