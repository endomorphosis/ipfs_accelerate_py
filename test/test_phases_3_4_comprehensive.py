...
def test_response_format_validation():
    ...
    try:
        ...
        with patch.object(claude, "_get_sdk_client") as mock_client:
            ...
            assert response["mode"] in ["CLI", "SDK"], (
                f"'mode' should be CLI or SDK, got {response['mode']}"
            )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                ...
                assert claude.api_key == "sk-ant-test-from-secrets", (
                    "API key should be from secrets manager"
                )
