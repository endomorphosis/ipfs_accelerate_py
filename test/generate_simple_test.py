#!/usr/bin/env python3
"""
Simple script to generate a test file for a HuggingFace model.
"""
...
def generate_test_file(model, tasks):
    """Generate a basic test file for a model."""
    normalized_name = normalize_model_name(model)
    primary_task = tasks[0] if tasks else "feature-extraction"
...
class test_hf_{normalized_name}:
    def __init__(self, resources=None, metadata=None):
        ...
        elif "{primary_task}" in ["image-classification", "object-detection", "image-segmentation"]:
            self.test_input = "test.jpg"  # Path to a test image file
        elif "{primary_task}" in ["automatic-speech-recognition", "audio-classification"]:
    ...
    def test(self):
        ...
        try:
            ...
            endpoint, processor, handler, queue, batch_size = self.model.init_cpu(
                self.model_name, "{primary_task}", "cpu"
            )
        ...
        if torch.cuda.is_available():
            try:
                ...
                endpoint, processor, handler, queue, batch_size = self.model.init_cuda(
                    self.model_name, "{primary_task}", "cuda:0"
                )
...
if __name__ == "__main__":
    try:
        ...
        # Print summary
        print(f"\\n{normalized_name.upper()} TEST RESULTS:")
...
"""
    ...
    # Save the file
    with open(f"skills/test_hf_{normalized_name}.py", "w") as f:
        f.write(template)
    os.chmod(f"skills/test_hf_{normalized_name}.py", 0o755)
    
    print(f"Generated test file for {model} at skills/test_hf_{normalized_name}.py")
    return True

def main():
    # Check if model name was provided as argument
    if len(sys.argv) < 2:
        print("Usage: python generate_simple_test.py <model_name> [<task1> <task2> ...]")
        sys.exit(1)
    
    model = sys.argv[1]
    tasks = sys.argv[2:] if len(sys.argv) > 2 else ["feature-extraction"]
    
    # Generate the test file
    generate_test_file(model, tasks)
