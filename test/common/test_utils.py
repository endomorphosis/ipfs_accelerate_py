...
class ModelTestUtils:
    ...
    def assert_output_shape(
        output: torch.Tensor, expected_shape: Tuple, allow_batch_dim: bool = True
    ) -> None:
    ...
    def measure_inference_time(
        model: Any, inputs: Dict, warmup_runs: int = 3, test_runs: int = 10
    ) -> Dict[str, float]:
    ...
    @staticmethod
    def measure_memory_usage(model: Any, inputs: Dict, device: str = "cuda") -> Dict[str, float]:
    ...
    @staticmethod
    def create_sample_text_inputs(
        tokenizer: Any, texts: Optional[List[str]] = None, max_length: int = 128
    ...
    @staticmethod
    def create_sample_image_inputs(
        processor: Any, size: Tuple[int, int] = (224, 224), batch_size: int = 1
    ) -> Dict[str, torch.Tensor]:
...
class HardwareTestUtils:
    ...
    @staticmethod
    def assert_model_works_on_device(model: Any, inputs: Dict, device: str) -> None:
...
class PerformanceTestUtils:
    ...
    @staticmethod
    def assert_inference_time_within_threshold(
        actual_time: float, baseline_time: float, threshold_factor: float = 1.2
    ) -> None:
    ...
    @staticmethod
    def assert_memory_within_threshold(
        actual_memory: float, baseline_memory: float, threshold_factor: float = 1.2
    ) -> None:
    ...
    def create_performance_report(
        model_name: str, timing_stats: Dict, memory_stats: Optional[Dict] = None
    ) -> str:
    ...
    def check_performance_regression(
        ...
        try:
            ...
            return {"error": "Performance baseline manager not available", "has_baseline": False}
        ...
        if update_baseline:
            ...
            return {"updated": True, "message": f"✅ Updated baseline for {model_name} on {device}"}
        else:
