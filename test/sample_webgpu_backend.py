...
class WebGPUBackend:
    ...
    def create_buffer(self, size: int, usage: int, label: Optional[str] = None) -> Optional[Any]:
        ...
        try:
            buffer = self.device.create_buffer(
                {"size": size, "usage": usage, "mappedAtCreation": False, "label": label}
            )
    ...
    def write_buffer(
        self, buffer: Any, data: Union[List[float], List[int], bytes], offset: int = 0
    ) -> bool:
        """
    ...
    async def create_compute_pipeline(
        self, shader: str, entry_point: str = "main"
    ) -> Optional[Any]:
        """
        ...
        try:
            ...
            shader_module = self.device.create_shader_module({"code": shader})

            # Create pipeline
            pipeline = await self.device.create_compute_pipeline(
                {"layout": "auto", "compute": {"module": shader_module, "entryPoint": entry_point}}
            )
    ...
    def create_bind_group(self, layout: Any, entries: List[Dict[str, Any]]) -> Optional[Any]:
        ...
        try:
            bind_group = self.device.create_bind_group({"layout": layout, "entries": entries})
    ...
    async def run_compute(
        self, pipeline: Any, bind_groups: List[Any], workgroups: Tuple[int, int, int] = (1, 1, 1)
    ) -> bool:
        """
