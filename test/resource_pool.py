...
class ResourcePool:
    ...
    def __init__(self):
        ...
        self.low_memory_mode = os.environ.get("RESOURCE_POOL_LOW_MEMORY", "0").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        if self.available_memory_mb < 4096 and not self.low_memory_mode:
            self.logger.warning(
                f"Low memory detected ({self.available_memory_mb:.2f} MB). Enabling low memory mode."
            )
        ...
        if WEBNN_WEBGPU_RESOURCE_POOL_AVAILABLE:
            ...
            init_web_pool = os.environ.get("INIT_WEB_RESOURCE_POOL", "1").lower() in (
                "1",
                "true",
                "yes",
            )
            if init_web_pool:
                ...
                self.logger.info(
                    "WebNN/WebGPU Resource Pool available but not auto-initialized (set INIT_WEB_RESOURCE_POOL=1 to enable)"
                )

        self.logger.info(
            f"ResourcePool initialized (low memory mode: {self.low_memory_mode}, available memory: {self.available_memory_mb} MB, WebNN/WebGPU: {'available' if self.web_resource_pool_initialized else 'not available'})"
        )
    ...
    def get_model(self, model_type, model_name, constructor=None, hardware_preferences=None):
        ...
        with self._lock:
            ...
            should_use_web_pool = self._should_use_web_resource_pool(
                model_type, model_name, hardware_preferences
            )
            ...
            if should_use_web_pool and self.web_resource_pool_initialized:
                ...
                try:
                    ...
                    if model:
                        ...
                        platform = (
                            hardware_preferences.get("priority_list", ["unknown"])[0]
                            if hardware_preferences
                            else "unknown"
                        )
                        self.logger.info(
                            f"Model {key} loaded via WebNN/WebGPU Resource Pool ({platform}) in {load_time:.2f} seconds"
                        )

                        return self.models[key]
                    else:
                        self.logger.warning(
                            f"Failed to load model {key} via WebNN/WebGPU Resource Pool"
                        )
                        # Continue to regular loading if web pool failed
                except Exception as e:
                    self.logger.error(
                        f"Error loading model {key} via WebNN/WebGPU Resource Pool: {e}"
                    )
                    # Continue to regular loading if web pool failed
            ...
            if constructor:
                ...
                try:
                    ...
                    try:
                        ...
                        elif torch.is_tensor(self.models[key]) or hasattr(
                        ...
                        self._stats["memory_usage"] += memory_usage
                        self.logger.info(
                            f"Model {key} uses approximately {memory_usage / 1024 / 1024:.2f} MB"
                        )
                        ...
                        if (
                            ...
                            and memory_usage > (500 * 1024 * 1024)
                        ):  # Over 500MB
                            if (
                                hasattr(torch, "cuda")
                                ...
                                self.logger.info(
                                    f"Low memory mode active - moving {key} to CPU after initialization"
                                )
                                model.to("cpu")
    ...
    def _should_use_web_resource_pool(
        self, model_type: str, model_name: str, hardware_preferences: Optional[Dict[str, Any]]
    ) -> bool:
        """
        ...
        force_web_pool = os.environ.get("FORCE_WEB_RESOURCE_POOL", "0").lower() in (
            "1",
            "true",
            "yes",
        )
        if force_web_pool:
            self.logger.debug(
                f"Using WebNN/WebGPU Resource Pool for {model_type}:{model_name} due to FORCE_WEB_RESOURCE_POOL"
            )
            return True
        ...
        if hardware_preferences:
            ...
            if "priority_list" in hardware_preferences:
                ...
                if any(p in ["webgpu", "webnn"] for p in priorities):
                    self.logger.debug(
                        f"Using WebNN/WebGPU Resource Pool for {model_type}:{model_name} due to hardware priority list"
                    )
                    return True
            ...
            if "device" in hardware_preferences:
                ...
                if device in ["webgpu", "webnn"]:
                    self.logger.debug(
                        f"Using WebNN/WebGPU Resource Pool for {model_type}:{model_name} due to device preference"
                    )
                    return True
            ...
            if "platform" in hardware_preferences:
                ...
                if platform in ["webgpu", "webnn"]:
                    self.logger.debug(
                        f"Using WebNN/WebGPU Resource Pool for {model_type}:{model_name} due to platform preference"
                    )
                    return True
            ...
            if "browser" in hardware_preferences:
                self.logger.debug(
                    f"Using WebNN/WebGPU Resource Pool for {model_type}:{model_name} due to browser preference"
                )
                return True
    ...
    def _get_optimal_device(self, model_type, model_name, hardware_preferences=None):
        ...
        try:
            ...
            model_classifier_path = os.path.join(
                os.path.dirname(__file__), "model_family_classifier.py"
            )
            ...
            if has_model_classifier:
                ...
                self.logger.debug(
                    f"Using model_type '{model_type}' as family (model_family_classifier not available)"
                )

            # Special case handling based on model family
            if model_family == "multimodal" and best_device == "mps":
                self.logger.warning(
                    f"Model {model_name} is multimodal and may not work well on MPS. Using CPU instead."
                )
                return "cpu"
            ...
            if model_family == "text_generation" and best_device == "cuda":
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        if (
                            is_large_model and free_gpu_memory < 7.5
                        ):  # Need at least 8GB for 7B models
                            self.logger.warning(
                                f"Insufficient GPU memory for large model {model_name}. Available: {free_gpu_memory:.2f}GB. Using CPU instead."
                            )
                            return "cpu"
                except (ImportError, AttributeError, Exception) as e:
            ...
            self.logger.debug(
                f"Could not determine optimal device using hardware_detection: {str(e)}"
            )
    ...
    def cleanup_unused_resources(self, max_age_minutes=30):
        ...
        with self._lock:
            ...
            if self.low_memory_mode:
                ...
                self.logger.info(
                    f"Using aggressive cleanup timeout of {max_age_minutes} minutes (low memory mode)"
                )
            ...
            try:
                ...
                if available_percent < 20:
                    ...
                    self.logger.warning(
                        f"Memory pressure detected: {available_percent:.1f}% available. Using aggressive cleanup."
                    )
            ...
            for key, model in self.models.items():
                if key in self._stats["last_accessed"]:
                    ...
                    if age_minutes > max_age_minutes:
                        ...
                        self.low_memory_mode or memory_pressure
                    ) and age_minutes > max_age_minutes / 2:
                        ...
                        try:
                            ...
                            if model_size_mb > 100:  # If larger than 100MB
                                models_to_remove.append(key)
                                self.logger.info(
                                    f"Removing large model {key} ({model_size_mb:.1f} MB) due to memory pressure"
                                )
                        except:
                            pass
            ...
            removed_count = (
                len(resources_to_remove) + len(models_to_remove) + len(tokenizers_to_remove)
            )
            ...
            if (self.low_memory_mode or memory_pressure) and removed_count == 0:
                self.logger.warning(
                    "No resources removed but memory pressure exists. Consider manual clearing."
                )
        ...
        with self._lock:
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    cuda_memory = {"device_count": device_count, "devices": []}
            ...
            if web_resource_pool_metrics:
                ...
                if "recovery_stats" in web_resource_pool_metrics:
                    stats["web_resource_pool"]["recovery_stats"] = web_resource_pool_metrics[
                        "recovery_stats"
                    ]

                # Extract browser connections if available
                if (
                    "base_metrics" in web_resource_pool_metrics
                    and "connections" in web_resource_pool_metrics["base_metrics"]
                ):
                    stats["web_resource_pool"]["connections"] = web_resource_pool_metrics[
                        "base_metrics"
                    ]["connections"]
    ...
    def execute_concurrent(self, models_and_inputs):
        ...
        if self.web_resource_pool_initialized and hasattr(
            self.web_resource_pool, "execute_concurrent"
        ):
            try:
                ...
                if web_models:
                    self.logger.info(
                        f"Executing {len(web_models)} models concurrently via WebNN/WebGPU Resource Pool"
                    )
                    return self.web_resource_pool.execute_concurrent(web_models)
            except Exception as e:
                self.logger.error(
                    f"Error executing models concurrently via WebNN/WebGPU Resource Pool: {e}"
                )
        ...
        for model, inputs in models_and_inputs:
            try:
                ...
                results.append({"success": False, "error": str(e), "error_type": type(e).__name__})
    ...
    def generate_error_report(
        self, model_name: str, hardware_type: str, error_message: str, stack_trace: str = None
    ) -> dict:
        """
            ...
            try:
                ...
                if "hardware_priorities" in model_info:
                    ...
                    if hardware_type in priorities:
                        ...
                        report["alternatives"] = (
                            priorities[idx + 1 :] if idx + 1 < len(priorities) else []
                        )
                    else:
                        report["alternatives"] = priorities

                self.logger.debug(
                    f"Added model family information to error report: {report['model_family']}"
                )
            except (ImportError, Exception) as e:
        ...
        report["recommendations"] = self._generate_recommendations(
            model_name, hardware_type, error_message
        )

        return report

    def _generate_recommendations(
        self, model_name: str, hardware_type: str, error_message: str
    ) -> list:
        """
        ...
        if "out of memory" in error_lower or "oom" in error_lower:
            recommendations.append(
                f"The model {model_name} requires more memory than available on {hardware_type}."
            )
            recommendations.append("Consider using a smaller model variant if available.")
            recommendations.append(
                "Reduce batch size or sequence length to decrease memory requirements."
            )
        ...
        # Handle unsupported operation errors
            ...
            "not implemented" in error_lower
            or "not supported" in error_lower
            or "unsupported" in error_lower
            or "operation" in error_lower
        ):
            recommendations.append(
                f"The model {model_name} contains operations not supported on {hardware_type} platform."
            )
            recommendations.append(
                "This is typically due to hardware-specific limitations or missing driver functionality."
            )

            alternatives = self._suggest_alternative_hardware(hardware_type, model_name)
            if alternatives:
                recommendations.append(
                    f"Try running on {alternatives[0]} with 'device={alternatives[0]}'."
                )
            else:
                recommendations.append(
                    "Consider using a different model that's compatible with your hardware."
                )
        ...
        # Handle driver version mismatches
            ...
            if hardware_type == "cuda":
                ...
                recommendations.append(
                    "Update your AMD drivers to the latest version compatible with your ROCm toolkit."
                )
            else:
                recommendations.append(
                    f"Update your {hardware_type} drivers to the latest version."
                )
        ...
        # General recommendations
            ...
            if alternatives:
                recommendations.append(
                    f"Recommended alternative hardware: {', '.join(alternatives)}"
                )
        ...
        if os.path.exists(model_classifier_path):
            try:
                ...
                if "hardware_priorities" in model_info:
                    ...
                    priorities = model_info.get("hardware_priorities")
                    self.logger.debug(
                        f"Using model family specific hardware priorities: {priorities}"
                    )
                    ...
                    alternatives = [
                        ...
                        if hw != current_hardware and hw in available_hardware
                    ]
                ...
                self.logger.debug(
                    f"Error getting model family specific hardware suggestions: {str(e)}"
                )

        # Fallback to default priorities if model classification fails
        alternatives = [
            hw for hw in default_priority if hw != current_hardware and hw in available_hardware
        ]
        return alternatives
    ...
    def save_error_report(self, report: dict, output_dir: str = "./hardware_reports") -> str:
        ...
        filename = (
            f"{output_dir}/hardware_error_{model_name}_{report['hardware_type']}_{timestamp}.json"
        )
