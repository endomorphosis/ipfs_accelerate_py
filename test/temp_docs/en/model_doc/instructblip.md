...
InstructBLIP leverages the [BLIP-2](blip-2.md) architecture for visual instruction tuning.
...
InstructBLIP uses the same architecture as [BLIP-2](blip-2.md) with a tiny but important difference: it also feeds the text prompt (instruction) to the Q-Former.
