...
def find_test_files(directory: str) -> Dict[str, List[str]]:
    ...
    test_files = {"model": [], "hardware": [], "api": [], "other": []}
...
def calculate_migration_progress(
    analysis_results: List[Dict[str, Any]], migrated_tests: Dict[str, List[str]]
) -> Dict[str, Any]:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-report", default="migration_progress.md", help="Path to save the report"
    )
    parser.add_argument(
        "--ascii", action="store_true", help="Generate ASCII report instead of Markdown"
    )
