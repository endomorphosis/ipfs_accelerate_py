...
def store_hardware_compatibility(
    run_id: int, model_id: int, hardware_id: int, compatibility_type: str, metadata: Dict = None
) -> Optional[int]:
    """
    ...
    try:
        ...
        conn.execute(
            ...
            [
                ...
                browser_version,
                platform,
                optimization_flags_json,
                initialization_time,
                first_inference_time,
                subsequent_inference_time,
                memory,
                metadata_json,
            ],
        )
...
def store_integration_test_result(
    run_id: int,
    test_module: str,
    test_name: str,
    status: str,
    ...
    try:
        ...
        test_result_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        logger.debug(
            f"Stored integration test result for {test_module}.{test_name} (ID: {test_result_id})"
        )
        return test_result_id
    except Exception as e:
...
def store_implementation_metadata(
    model_type: str,
    file_path: str,
    ...
    try:
        ...
        generation_date_str = (
            generation_date.isoformat() if generation_date else datetime.datetime.now().isoformat()
        )

        # Check if the record already exists
        result = conn.execute(
            "SELECT implementation_id FROM model_implementations WHERE model_type = ?", [model_type]
        ).fetchone()
        ...
        if result:
            ...
            logger.debug(
                f"Updated implementation metadata for {model_type} (ID: {implementation_id})"
            )
...
def query_to_dataframe(
    query: str, params: List = None, db_path: str = None
