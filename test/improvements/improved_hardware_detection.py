...
def detect_all_hardware() -> Dict[str, Any]:
    ...
    try:
        ...
        webnn_available = (
            importlib.util.find_spec("webnn") is not None
            or importlib.util.find_spec("webnn_js") is not None
            or "WEBNN_AVAILABLE" in os.environ
            or "WEBNN_SIMULATION" in os.environ
        )
        ...
        webgpu_available = (
            importlib.util.find_spec("webgpu") is not None
            or importlib.util.find_spec("wgpu") is not None
            or "WEBGPU_AVAILABLE" in os.environ
            or "WEBGPU_SIMULATION" in os.environ
        )
...
def apply_web_platform_optimizations(model_type: str, platform: str = "webgpu") -> Dict[str, bool]:
    ...
    parallel_loading_enabled = (
        ...
        or os.environ.get("WEB_PARALLEL_LOADING", "0") == "1"
    )
    ...
    is_audio_model = any(
        x in model_type for x in ["audio", "whisper", "wav2vec", "clap", "encodec", "speech"]
    )
    is_multimodal_model = any(
        x in model_type for x in ["llava", "clip", "xclip", "visual", "vision-text", "multimodal"]
    )
    is_vision_model = any(
        x in model_type for x in ["vision", "image", "vit", "cnn", "convnext", "resnet"]
    )
