...
def test_standalone_detection(simulation=False):
    ...
    if chipset:
        ...
        for model_type, capability in analysis["model_capabilities"].items():
            print(
                f"  {model_type}: {'Suitable' if capability['suitable'] else 'Not suitable'}, "
...
def test_centralized_detection(simulation=False):
    ...
    if capabilities.get("samsung_npu", False):
        ...
        for model_type in model_types:
            ...
            print(
                f"  {model_type}: Samsung NPU compatible = {compatibility.get('samsung_npu', False)}"
            )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--centralized", action="store_true", help="Test centralized detection only"
    )
    parser.add_argument(
        "--compatibility", action="store_true", help="Test model compatibility only"
    )
