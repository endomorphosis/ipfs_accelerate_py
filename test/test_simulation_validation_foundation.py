...
def test_core_classes():
    ...
    assert (
        sim_result2.metrics["throughput_items_per_second"]
    ...
    assert (
        hw_result2.metrics["throughput_items_per_second"]
    ...
    assert (
        val_result2.metrics_comparison["throughput_items_per_second"]["mape"]
        == val_result.metrics_comparison["throughput_items_per_second"]["mape"]
    )
...
def test_database_schema(db_path):
    ...
    try:
        ...
        # Verify insertion
        result = conn.execute(
            f"SELECT * FROM simulation_results WHERE id = '{sim_record['id']}'"
        ).fetchone()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test the Simulation Accuracy and Validation Framework"
    )
    parser.add_argument(
        "--db-path",
        default=":memory:",
    ...
    parser.add_argument("--skip-db", action="store_true", help="Skip database schema test")
