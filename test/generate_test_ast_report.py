...
class TestASTVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        func_info = {
            ...
            or any("pytest" in self._get_name(d).lower() for d in node.decorator_list),
        }
        ...
        return {
            ...
            "metrics": {
                ...
                "num_test_methods": sum(
                    1 for c in visitor.classes for m in c["methods"] if m.get("is_test", False)
                )
    ...
    for file_info in report["files"]:
        for class_info in file_info["classes"]:
            ...
            class_dict[full_name] = {
                ...
                "num_test_methods": sum(
                    1 for m in class_info["methods"] if m.get("is_test", False)
                ),
            }
        ...
        for base in class_info["bases"]:
            ...
            for potential_base in class_dict.keys():
                if class_dict[potential_base]["name"] == base:
                    class_relationships.append({"child": class_name, "parent": potential_base})
...
def main():
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Include all Python files, not just test files"
    )
