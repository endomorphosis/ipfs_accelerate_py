...
try:
    ...
    from api_unified_testing_interface import APIBackendFactory, APITester, APIProvider, APITestType
...
class APIWorkerNode(WorkerNode):
    ...
    def __init__(
        ...
        self.add_tags(
            ["api_worker"]
            + [f"api_{provider.lower()}" for provider in self.capabilities["providers"]]
        )
...
def main():
    ...
    parser.add_argument(
        "--coordinator", type=str, required=True, help="URL of the coordinator server"
    )
    ...
    parser.add_argument(
        "--tags", type=str, help="Comma-separated list of tags for worker selection"
    )
    ...
    # Running options
    parser.add_argument(
        "--poll-interval", type=int, default=5, help="Time in seconds between polling for new tasks"
    )
