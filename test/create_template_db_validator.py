...
HARDWARE_CHECKS = {
    "cuda": [r"HAS_CUDA", r"torch\.cuda\.is_available", r'device\s*=\s*[\'"]cuda[\'"]'],
    "cpu": [r'device\s*=\s*[\'"]cpu[\'"]'],
    "mps": [r"HAS_MPS", r"torch\.mps\.is_available", r'device\s*=\s*[\'"]mps[\'"]'],
    "rocm": [r"HAS_ROCM", r"_rocm_version", r"ROCM_HOME"],
    "openvino": [r"HAS_OPENVINO", r"openvino", r"import\s+openvino"],
    "qualcomm": [r"HAS_QUALCOMM", r"QUALCOMM_SDK", r"qnn_wrapper", r"import\s+qti"],
    "webnn": [r"HAS_WEBNN", r"WEBNN_AVAILABLE", r"WEBNN_SIMULATION"],
    "webgpu": [r"HAS_WEBGPU", r"WEBGPU_AVAILABLE", r"WEBGPU_SIMULATION"],
}
...
def validate_class_structure(content: str) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        for cls in test_classes:
            ...
            if not test_methods:
                errors.append(
                    f"Class {cls.name} has no test methods (should start with 'test_' or include 'run' method)"
                )
...
def validate_duckdb_templates(db_path: str = "template_db.duckdb") -> Dict[str, Any]:
    ...
    if not HAS_DUCKDB:
        ...
        if not os.path.exists(json_db_path):
            return {"valid": False, "error": f"JSON database file not found: {json_db_path}"}
        ...
        try:
            ...
            if "templates" not in template_db:
                return {"valid": False, "error": "No templates found in JSON database"}
            ...
            return {"valid": False, "error": f"Error validating JSON templates: {str(e)}"}

    try:
        import duckdb
    except ImportError:
        return {"valid": False, "error": "Failed to import DuckDB"}
    ...
    try:
        ...
        if not table_check:
            return {"valid": False, "error": "No 'templates' table found in database"}
        ...
        if not templates:
            return {"valid": False, "error": "No templates found in database"}
        ...
        return {"valid": False, "error": f"Error validating database templates: {str(e)}"}
    ...
    if not HAS_DUCKDB:
        ...
        template_db = {
            ...
            "model_types": {
                "text_embedding": {
                    "id": 1,
                    "description": "Text embedding models like BERT, Sentence Transformers",
                },
                "text_generation": {
                    "id": 2,
                    "description": "Text generation models like GPT, LLAMA, T5",
                },
                ...
                "text_to_image": {
                    "id": 8,
                    "description": "Text-to-image models like Stable Diffusion",
                },
                "text_to_audio": {"id": 9, "description": "Text-to-audio models like MusicGen"},
                "text_to_video": {
                    "id": 10,
                    "description": "Text-to-video models like Video Diffusion",
                },
            },
...
def migrate_template_files_to_db(source_dir: str, db_path: str) -> Dict[str, Any]:
    ...
    if not HAS_DUCKDB:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to load JSON database: {str(e)}"}
        ...
        try:
            ...
            return {"success": False, "error": f"Error saving JSON database: {str(e)}"}
    ...
    if not os.path.exists(db_path):
        ...
        if not create_template_database(db_path):
            return {"success": False, "error": f"Failed to create database at {db_path}"}
    ...
    try:
        ...
        for file_path in template_files:
            try:
                ...
                """,
                    [model_type, template_type, platform, platform],
                ).fetchone()[0]

                if exists:
                    logger.info(
                        f"Template already exists for {model_type}/{template_type}/{platform}, updating..."
                    )
                    conn.execute(
                        """
                    UPDATE templates 
        ...
        return {"success": False, "error": f"Error migrating templates: {str(e)}"}
...
def generate_hardware_compatibility_report(db_path: str, output_file: str = None) -> Dict[str, Any]:
    ...
    if not HAS_DUCKDB:
        ...
        if not os.path.exists(json_db_path):
            return {"success": False, "error": f"JSON database file not found: {json_db_path}"}
        ...
        try:
            ...
            if "templates" not in template_db:
                return {"success": False, "error": "No templates found in JSON database"}
            ...
            platform_percentages = {
                hw: (count / total_templates) * 100 for hw, count in platform_support.items()
            }
            ...
            for model_type, hw_counts in compatibility_matrix.items():
                ...
                for hw, count in hw_counts.items():
                    model_compatibility[model_type][hw] = (
                        (count / type_count) * 100 if type_count > 0 else 0
                    )
            ...
            if output_file:
                ...
                report += (
                    "| Model Type | Count | "
                    + " | ".join([hw for hw, _ in HARDWARE_PLATFORMS])
                    + " |\n"
                )
                report += (
                    "|------------|-------|" + "|".join(["---" for _ in HARDWARE_PLATFORMS]) + "|\n"
                )
    ...
    try:
        ...
        if not templates:
            return {"success": False, "error": "No templates found in database"}
...
def main():
    ...
    parser.add_argument(
        "--validate-db", action="store_true", help="Validate templates in the database"
    )
    parser.add_argument(
        "--migrate-templates", action="store_true", help="Migrate template files to database"
    )
    parser.add_argument(
        "--check-hardware", action="store_true", help="Check hardware compatibility of templates"
    )
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        default="../generators/templates/template_db.duckdb",
