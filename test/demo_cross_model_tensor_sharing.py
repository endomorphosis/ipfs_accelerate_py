...
async def run_tensor_sharing_demo(
    ...
    print(f"\n{'=' * 80}")
    print(
        f"Cross-Model Tensor Sharing Demo {'(ENABLED)' if enable_tensor_sharing else '(DISABLED)'}"
    )
    print(f"{'=' * 80}\n")
    ...
    if enable_tensor_sharing and hasattr(pool, "tensor_sharing_manager"):
        ...
        for model_name, memory_info in model_memory.items():
            print(
                f"  {model_name}: {memory_info['total_memory_mb']:.2f} MB ({memory_info['tensor_count']} tensors)"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--compare", action="store_true", help="Run comparison with and without tensor sharing"
    )
    parser.add_argument("--no-sharing", action="store_true", help="Disable tensor sharing")
    parser.add_argument(
        "--max-memory", type=int, default=2048, help="Maximum memory for tensor sharing (MB)"
    )
    parser.add_argument(
        "--hardware",
        type=str,
