...
from integrate_documentation_system import (
...
def validate_documentation(doc_path: str, model_family: str, hardware: str) -> Dict[str, Any]:
    ...
    is_valid = (
        ...
        and len(missing_family_keywords) == 0
        and len(missing_hardware_keywords) == 0
        and len(unreplaced_vars) == 0
    )
...
def run_integration_test(
    specific_model: Optional[str] = None, specific_hardware: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    models_to_test = (
        {
            specific_model: next(
                (fam for fam, model in TEST_MODELS.items() if model == specific_model), "unknown"
            )
        }
        if specific_model
        else TEST_MODELS
    ...
    successful_tests = sum(
        1 for r in results if r["success"] and r.get("validation", {}).get("valid", False)
    )
...
def print_summary(summary: Dict[str, Any]):
    ...
    if summary["success_rate"] == 1.0:
        ...
        print(
            f"⚠️ Some documentation tests failed. Check {os.path.join(OUTPUT_DIR, 'integration_test_results.json')} for details."
        )
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    if args.model or args.hardware:
        print(
            f"Testing specific configuration: Model={args.model or 'all'}, Hardware={args.hardware or 'all'}"
        )
