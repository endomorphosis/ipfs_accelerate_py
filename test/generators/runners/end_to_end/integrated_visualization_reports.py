...
DEFAULT_DB_PATH = os.environ.get(
    "BENCHMARK_DB_PATH", os.path.join(test_dir, "test_template_db.duckdb")
)
...
class IntegratedSystem:
    ...
    def __init__(self, args):
        ...
        self.do_both = (args.dashboard and args.reports) or (
            args.dashboard and args.dashboard_export
        )
    ...
    def start_dashboard(self, wait_for_startup: bool = True) -> Optional[subprocess.Popen]:
        ...
        """
        logger.info(
            f"Starting visualization dashboard at http://{self.dashboard_host}:{self.dashboard_port}"
        )
...
<body>
    ...
    <p>This export was generated on """
    ...
    def run(self):
        ...
        try:
            ...
            if self.args.reports:
                ...
                if self.dashboard_process and reports:
                    for report_name, report_path in reports.items():
                        if os.path.isfile(report_path) and report_path.endswith(
                            (".html", ".svg", ".png")
                        ):
            ...
            if self.args.dashboard_export:
                ...
                if self.dashboard_process and export_results:
                    for export_name, export_path in export_results.items():
                        if os.path.isfile(export_path) and export_path.endswith(
                            (".html", ".svg", ".png")
                        ):
                            # The actual URL would need a web server; this is just for reference
            ...
            if (
                ...
                and self.dashboard_process
                and self.dashboard_process.poll() is None
            ):
...
def parse_arguments():
    ...
    parser.add_argument(
        "--dashboard", action="store_true", help="Start the visualization dashboard"
    )
    parser.add_argument(
        "--reports",
        action="store_true",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Run the dashboard in debug mode with hot reloading"
    )

    # Report options
    parser.add_argument(
        "--format", choices=["html", "markdown"], help="Report format (default: html)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--days", type=int, help="Number of days to include in historical data (default: 30)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tolerance", type=float, help="Tolerance for simulation validation (as percentage)"
    )
    parser.add_argument(
        "--include-visualizations", action="store_true", help="Include visualizations in reports"
    )
    parser.add_argument(
    ...
    # Common options
    parser.add_argument(
        "--db-path", help=f"Path to DuckDB database file (default: {DEFAULT_DB_PATH})"
    )
    parser.add_argument(
        "--output-dir", help=f"Directory to save reports (default: {DEFAULT_OUTPUT_DIR})"
    )
