...
DEFAULT_DB_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "template_database.duckdb"
)
MODEL_FAMILIES = [
    "text_embedding",
    "text_generation",
...
class TemplateDatabase:
    ...
    def __init__(self, db_path: str = DEFAULT_DB_PATH, verbose: bool = False):
        ...
        if not HAS_DUCKDB:
            raise ImportError(
                "DuckDB is required for template database functionality. Install with: pip install duckdb"
            )
    ...
    def _initialize_db(self):
        ...
        try:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_templates_hardware ON templates(hardware_platform)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_templates_type ON templates(template_type)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_mappings_model ON model_mappings(model_name)"
            )
    ...
    def add_template(
        ...
        if template_type not in TEMPLATE_TYPES:
            raise ValueError(
                f"Invalid template type: {template_type}. Must be one of: {TEMPLATE_TYPES}"
            )

        if model_family not in MODEL_FAMILIES:
            raise ValueError(
                f"Invalid model family: {model_family}. Must be one of: {MODEL_FAMILIES}"
            )

        if hardware_platform and hardware_platform not in HARDWARE_PLATFORMS:
            raise ValueError(
                f"Invalid hardware platform: {hardware_platform}. Must be one of: {HARDWARE_PLATFORMS}"
            )
    ...
    def get_template(
        ...
        try:
            ...
            if not result:
                logger.warning(
                    f"No template found for model_family={model_family}, template_type={template_type}, hardware={hardware_platform}"
                )
                return None
            ...
            logger.debug(
                f"Retrieved template: {template['template_name']} ({template['template_id']})"
            )
            return template
    ...
    def add_model_mapping(
        self, model_name: str, model_family: str, description: Optional[str] = None
    ) -> str:
        """
    ...
    def _infer_model_family(self, model_name: str) -> Optional[str]:
        ...
        if any(
            x in model_name
            for x in ["bert", "roberta", "distilbert", "sentence", "bge", "all-mpnet", "e5-"]
        ):
        ...
        if any(
            x in model_name
            for x in ["gpt", "llama", "t5", "bart", "palm", "bloom", "mixtral", "qwen", "mistral"]
        ):
    ...
    def add_hardware_compatibility(
        self,
        model_family: str,
        ...
        if compatibility_level not in ["full", "limited", "none"]:
            raise ValueError(
                f"Invalid compatibility level: {compatibility_level}. Must be one of: full, limited, none"
            )
        ...
        try:
            ...
            """,
                [model_family, hardware_platform],
            ).fetchone()[0]
            ...
            if exists:
                ...
                logger.info(
                    f"Updated hardware compatibility: {model_family} on {hardware_platform} -> {compatibility_level}"
                )
            else:
                # Insert new entry
                conn.execute(
                ...
                logger.info(
                    f"Added hardware compatibility: {model_family} on {hardware_platform} -> {compatibility_level}"
                )
    ...
    def get_hardware_compatibility(
        self, model_family: str, hardware_platform: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result:
                return {"compatibility_level": result[0], "description": result[1]}

            # Default to limited compatibility if not specified
            logger.warning(
                f"No hardware compatibility information found for {model_family} on {hardware_platform}"
            )
            return {
    ...
    def _extract_variables(self, template_content: str) -> Dict[str, Dict[str, Any]]:
        ...
        for var_name in matches:
            if var_name not in variables:
                variables[var_name] = {"description": f"Variable for {var_name}", "required": True}

        return variables

    def render_template(
        self, template_id: str, variables: Dict[str, Any], render_dependencies: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            if parent_template_id and render_dependencies:
                parent_content = self.render_template(
                    parent_template_id, variables, render_dependencies
                )
        ...
        for template_type in template_types:
            try:
                ...
                if template_type == "skill":
                    filename = (
                        f"{model_name.replace('/', '_')}_{hardware_platform or 'cpu'}_skill.py"
                    )
                elif template_type == "test":
                    filename = (
                        f"test_{model_name.replace('/', '_')}_{hardware_platform or 'cpu'}.py"
                    )
                elif template_type == "benchmark":
                    filename = (
                        f"benchmark_{model_name.replace('/', '_')}_{hardware_platform or 'cpu'}.py"
                    )
                elif template_type == "documentation":
                    filename = (
                        f"{model_name.replace('/', '_')}_{hardware_platform or 'cpu'}_docs.md"
                    )
                else:
...
def add_text_embedding_templates(db: TemplateDatabase):
    ...
    try:
        ...
        test_template = template_content.replace(
            "test_${model_family}", "test_${model_name.replace('-', '_').replace('/', '_')}"
        )
        db.add_template(
...
"""
    ...
    for hardware, optimizations in hardware_optimizations.items():
        ...
        hardware_doc_template = doc_template.replace(
            "${hardware_specific_optimizations}", optimizations
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--init", action="store_true", help="Initialize the database with default templates"
    )
    parser.add_argument("--list", action="store_true", help="List templates in the database")
    parser.add_argument(
        "--add-template", action="store_true", help="Add a template to the database"
    )
    parser.add_argument("--template-name", type=str, help="Name of the template")
    parser.add_argument(
        "--template-type", type=str, choices=TEMPLATE_TYPES, help="Type of template"
    )
    parser.add_argument("--model-family", type=str, choices=MODEL_FAMILIES, help="Model family")
    parser.add_argument(
        "--hardware", type=str, choices=HARDWARE_PLATFORMS, help="Hardware platform"
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./generated", help="Output directory for generated files"
    )
    ...
    if args.init:
        ...
        for i, template in enumerate(templates, 1):
            print(
                f"{i}. {template['template_name']} ({template['template_type']}, {template['model_family']})"
            )
        ...
        if (
            not args.template_name
            or not args.template_type
            or not args.model_family
            or not args.template_file
        ):
            parser.error(
                "--add-template requires --template-name, --template-type, --model-family, and --template-file"
            )
        ...
        generated_files = db.generate_template_files(
            model_name=args.model, output_dir=args.output_dir, hardware_platform=args.hardware
        )
