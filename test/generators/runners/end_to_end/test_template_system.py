        ...
        try:
            ...
            for model_name, expected_family in test_cases.items():
                ...
                if inferred_family != expected_family:
                    logger.error(
                        f"Model family inference failed for {model_name}. Expected: {expected_family}, Got: {inferred_family}"
                    )
                    return False
    ...
    def test_template_renderer(self) -> bool:
        ...
        try:
            ...
            for template_type in template_types:
                ...
                try:
                    logger.debug(
                        f"Testing {template_type} template rendering for {model_name} on {hardware}"
                    )
                    ...
                    if not rendered or len(rendered) < 100:
                        logger.error(
                            f"Rendering failed or produced too little content for {model_name} ({template_type}) on {hardware}"
                        )
                        continue
                    ...
                    if "${" in rendered:
                        logger.error(
                            f"Variable substitution failed for {model_name} ({template_type}) on {hardware}"
                        )
                        continue
                    ...
                    if template_type == "skill":
                        if "class" not in rendered.lower() or "def" not in rendered.lower():
                            logger.error(
                                f"Skill template missing expected content (class or methods)"
                            )
                            continue
                        ...
                        if "benchmark" not in rendered.lower():
                            logger.error(
                                f"Benchmark template missing expected content (benchmark function)"
                            )
                            continue

                    logger.debug(
                        f"Successfully rendered {template_type} template for {model_name} on {hardware}"
                    )
                    success_count += 1

                except Exception as e:
                    logger.error(
                        f"Error rendering {template_type} for {model_name} on {hardware}: {e}"
                    )
                    continue
            ...
            try:
                logger.debug(
                    f"Testing documentation template rendering for {model_name} on {hardware}"
                )
                ...
                if not rendered or len(rendered) < 100:
                    logger.error(
                        f"Documentation rendering failed or produced too little content for {model_name} on {hardware}"
                    )
                else:
                    # Check that variable substitution worked
                    if "${" in rendered:
                        logger.error(
                            f"Variable substitution failed for {model_name} (documentation) on {hardware}"
                        )
                    else:
                        logger.debug(
                            f"Successfully rendered documentation template for {model_name} on {hardware}"
                        )
                        success_count += 1
            except Exception as e:
            ...
            if success_count >= 3:
                logger.info(
                    f"Template renderer test passed ({success_count}/{attempts} template types rendered successfully)"
                )
                return True
            else:
                logger.error(
                    f"Template renderer test failed (only {success_count}/{attempts} template types rendered successfully)"
                )
                return False
    ...
    def test_component_generation(self) -> bool:
        ...
        try:
            ...
            rendered_content = renderer.render_component_set(
                model_name=model_name, hardware_platform=hardware, output_dir=temp_dir
            )
...
""",
            ...
            variables = {"model_name": "test-model", "hardware_type": "test-hardware"}

            # Render the template
            rendered = db.render_template(
                template_id=child_template_id, variables=variables, render_dependencies=True
            )
        ...
        try:
            ...
            for model_family, model_name in model_family_samples.items():
                ...
                for hardware in hardware_platforms:
                    try:
                        logger.debug(
                            f"Testing documentation for {model_name} ({model_family}) on {hardware}"
                        )
                        ...
                        # Check that the rendering produced content
                        if (
                            not rendered or len(rendered) < 200
                        ...
                        if not found_specific_content:
                            logger.error(
                                f"Documentation for {model_name} missing model-family specific content"
                            )
                            results[model_family][hardware] = False
                            continue
                        ...
                        if not found_hw_specific_content:
                            logger.error(
                                f"Documentation for {model_name} missing hardware-specific content for {hardware}"
                            )
                            results[model_family][hardware] = False
                            continue
                        ...
                        doc_path = os.path.join(
                            temp_dir, f"{model_name.replace('/', '_')}_{hardware}_docs.md"
                        )
                        with open(doc_path, "w") as f:
                            f.write(rendered)

                        logger.debug(
                            f"Successfully rendered documentation for {model_name} on {hardware} to {doc_path}"
                        )
                        results[model_family][hardware] = True

                    except Exception as e:
                        logger.error(
                            f"Error rendering documentation for {model_name} on {hardware}: {e}"
                        )
                        results[model_family][hardware] = False
            ...
            for model_family, hw_results in results.items():
                for hw, result in hw_results.items():
                    logger.debug(
                        f"Documentation test for {model_family} on {hw}: {'PASS' if result else 'FAIL'}"
                    )
            ...
            if all_passed:
                ...
                if failed_tests:
                    logger.error(
                        f"The following documentation template tests failed: {', '.join(failed_tests)}"
                    )
                else:
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        for model_type, model_name in TEST_MODELS.items():
            ...
            for hardware in HARDWARE_PLATFORMS:
                try:
                    ...
                    # Generate components
                    rendered_content = renderer.render_component_set(
                        model_name=model_name, hardware_platform=hardware, output_dir=model_dir
                    )

                    # Check that essential components were generated
                    if (
                        "skill" in rendered_content
                        and "test" in rendered_content
                        and "benchmark" in rendered_content
                    ):
        ...
        successful_combinations = sum(
            sum(1 for h in hw_results.values() if h) for hw_results in results.values()
        )
        ...
        # Print summary
        logger.info(
            f"Comprehensive Test Results: {successful_combinations}/{total_combinations} combinations successful ({success_rate:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument("--hardware", type=str, default="cpu", help="Hardware platform to test on")
    parser.add_argument(
        "--comprehensive",
        action="store_true",
    ...
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Do not clean up temporary files after tests"
    )
    ...
    if args.comprehensive:
        ...
        try:
            # Generate components
            rendered_content = renderer.render_component_set(
                model_name=args.model, hardware_platform=args.hardware, output_dir=temp_dir
            )
