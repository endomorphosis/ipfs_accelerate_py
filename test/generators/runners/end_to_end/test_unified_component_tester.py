...
class TestUnifiedComponentTester(unittest.TestCase):
    ...
    def test_determine_model_family(self):
        ...
        tester = UnifiedComponentTester(model_name="gpt2", hardware=TEST_HARDWARE, quick_test=True)
        self.assertEqual(tester._determine_model_family(), "text-generation")

        tester = UnifiedComponentTester(
            model_name="google/vit-base-patch16-224", hardware=TEST_HARDWARE, quick_test=True
        )
        self.assertEqual(tester._determine_model_family(), "vision")

        tester = UnifiedComponentTester(
            model_name="openai/whisper-tiny", hardware=TEST_HARDWARE, quick_test=True
        )
        self.assertEqual(tester._determine_model_family(), "audio")

        tester = UnifiedComponentTester(
            model_name="openai/clip-vit-base-patch32", hardware=TEST_HARDWARE, quick_test=True
        )
    ...
    def test_run_test(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Ran 4 tests in 2.345s\n\nOK (successes=4)", stderr=""
        )
    ...
    def test_run_benchmark(self, mock_run):
        ...
        benchmark_result_file = os.path.join(
            self.temp_dir,
            f"benchmark_{TEST_MODEL_NAME.replace('/', '_')}_{TEST_HARDWARE}_results.json",
        )
        ...
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Benchmark results saved to benchmark_results.json", stderr=""
        )
...
def run_selected_tests(
    model_family: Optional[str] = None, hardware: Optional[str] = None, comprehensive: bool = False
) -> None:
    """
...
def parse_args():
    ...
    parser.add_argument(
        "--model-family", choices=MODEL_FAMILIES.keys(), help="Test specific model family"
    )
    parser.add_argument(
        "--hardware", choices=SUPPORTED_HARDWARE, help="Test specific hardware platform"
    )
    parser.add_argument(
        "--comprehensive", action="store_true", help="Run comprehensive tests (all combinations)"
    )
...
if __name__ == "__main__":
    ...
    if args.model_family or args.hardware or args.comprehensive:
        run_selected_tests(
            model_family=args.model_family, hardware=args.hardware, comprehensive=args.comprehensive
        )
    else:
        unittest.main()
