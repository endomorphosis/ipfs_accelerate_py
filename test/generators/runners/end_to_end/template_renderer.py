    ...
    def render_component_set(
        ...
        for template_type in template_types:
            try:
                ...
                if not template:
                    logger.warning(
                        f"No {template_type} template found for {model_family} on {hardware_platform}"
                    )
                    continue
...
"""
                ...
                if output_dir:
                    ...
                    if template_type == "skill":
                        ...
                        filename = (
                            f"benchmark_{model_name.replace('/', '_')}_{hardware_platform}.py"
                        )
                    elif template_type == "documentation":
                        filename = f"{model_name.replace('/', '_')}_{hardware_platform}_docs.md"
                    else:
                        filename = (
                            f"{template_type}_{model_name.replace('/', '_')}_{hardware_platform}.py"
                        )
                ...
                logger.error(
                    f"Error rendering {template_type} template for {model_name} on {hardware_platform}: {e}"
                )
    ...
    def _add_hardware_specific_variables(
        self, hardware_platform: str, variables: Dict[str, Any]
    ) -> None:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, required=False, help="Model name to render templates for"
    )
    parser.add_argument(
        "--hardware", type=str, default="cpu", help="Hardware platform to render templates for"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--template-type",
        type=str,
        choices=["skill", "test", "benchmark", "documentation"],
        ...
        try:
            if args.template_type:
                ...
                if args.template_type == "skill":
                    ...
                    filename = (
                        f"{args.template_type}_{args.model.replace('/', '_')}_{args.hardware}.py"
                    )
                ...
                print(
                    f"Rendered {args.template_type} template for {args.model} on {args.hardware} to {file_path}"
                )
                ...
                print(
                    f"Rendered templates for {args.model} on {args.hardware} to {args.output_dir}"
                )
