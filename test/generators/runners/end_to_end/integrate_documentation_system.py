...
from enhance_documentation_templates import (
...
def integrate_enhanced_doc_generator():
        ...
        if (
            ...
            if "## Known Limitations" in base_markdown:
                base_markdown = base_markdown.replace(
                    "## Known Limitations", f"{visualization_section}\n\n## Known Limitations"
                )
            else:
    ...
    def _create_benchmark_visualizations(self, benchmark_results):
        ...
        # Get throughputs
        throughputs = [
            batch_results[str(b)].get("average_throughput_items_per_second", 0) for b in batch_sizes
        ]
        ...
        for i, batch_size in enumerate(batch_sizes):
            ...
            bar_length = (
                int((throughput / max_throughput) * chart_width) if max_throughput > 0 else 0
            )
            visualization_md += (
                f"Batch {batch_size:2d}: {'█' * bar_length} {throughput:.2f} items/s\n"
            )
        ...
        if throughputs:
            ...
            visualization_md += (
                f"- **Optimal Batch Size**: {optimal_batch} (for maximum throughput)\n"
            )
            ...
            if self.hardware in ["cuda", "rocm", "webgpu"]:
                visualization_md += (
                    "GPU acceleration shows best performance with larger batch sizes.\n"
                )
            elif self.hardware == "cpu":
                visualization_md += (
                    "CPU processing shows balanced performance across batch sizes.\n"
                )
            else:
...
def modify_integrated_component_tester():
    ...
    def enhanced_run_test_with_docs(
        self, temp_dir: str, test_results: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Running enhanced documentation generation for {self.model_name} on {self.hardware}"
        )
        ...
        docs_output_dir = os.path.join(
            self.output_dir, "enhanced_docs_test", self.model_name.replace("/", "_")
        )
...
def test_integration():
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                f"Integration test successful! Documentation generated at: {result['documentation_path']}"
            )
            ...
            with open(result["documentation_path"], "r") as f:
                content = f.read()
                if (
                    "bert-base-uncased" in content
                    and "CUDA" in content
                    and "Performance Visualizations" in content
                ):
...
def main():
    ...
    logger.info(
        "You can now run the Integrated Component Test Runner with --generate-docs to create enhanced documentation"
    )
