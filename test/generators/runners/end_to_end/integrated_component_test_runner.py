...
- **Tests Run**: {test_count}"""
    ...
    def _format_benchmark_results(self, benchmark_results):
        ...
        if (
            not benchmark_results
            or not isinstance(benchmark_results, dict)
            or "results_by_batch" not in benchmark_results
        ):
    ...
    def _format_api_documentation(self, api_info):
        ...
        for method_name, method_info in api_info.get("methods", {}).items():
            ...
            if "returns" in method_info:
                ...
                returns_desc = method_info["returns"].get(
...
The implementation uses HuggingFace's `AutoModel` and appropriate tokenizer/processor classes for loading and running the model."""
    ...
    def extract_huggingface_info(self, model_name):
        ...
        if "bert" in model_name.lower():
            ...
            model_description = (
                "This is a text embedding model that produces vector representations of text."
            )
            ...
            model_description = (
                "This is a text generation model for natural language generation tasks."
            )
    ...
    def extract_api_details(self, file_path):
        ...
        try:
            ...
            return {"class_name": class_name, "methods": methods}
        except Exception as e:
            return {"class_name": "Unknown", "methods": {}, "error": str(e)}
    ...
    def get_hardware_info(self, hardware):
        ...
        if hardware == "cpu":
            return {
                ...
                "requirements": ["PyTorch 2.0 or later", "Transformers 4.30 or later"],
            ...
            return {
                ...
                "optimizations": ["Standard optimizations for this hardware platform"],
                "requirements": ["Appropriate hardware and software support"],
...
"""
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    str(warmup),
                    "--output-dir",
                    os.path.dirname(benchmark_file),
                ],
                capture_output=True,
                text=True,
                check=False,
            ...
            if success:
                ...
                json_files = [
                    ...
                    if f.startswith(
                        f"benchmark_{self.model_name.replace('/', '_')}_{self.hardware}"
                    )
                    and f.endswith(".json")
                ]
                if json_files:
    ...
    def save_results(
        ...
        if (
            ...
            and "results_by_batch" in benchmark_results
        ):
            ...
            json.dump(
                {
                    ...
                    "timestamp": datetime.datetime.now().isoformat(),
                    "git_hash": self.git_hash,
                    "test": {"success": test_success, "results": test_results},
                    "benchmark": {
        ...
        if self.update_expected:
            expected_model_dir = os.path.join(
                EXPECTED_RESULTS_DIR, self.model_name.replace("/", "_")
            )
    ...
    def compare_with_expected(
        self, test_results: Dict[str, Any], benchmark_results: Dict[str, Any]
        ...
        comparison_results = result_comparer.compare(
            actual={
                "test": {"success": test_results.get("success", False), "results": test_results},
                "benchmark": {
    ...
    def generate_documentation(
        ...
        if (
            hasattr(self, "template_db_path")
            and self.template_db_path
            and os.path.exists(self.template_db_path)
        ):
            try:
                ...
                if "template_db_path" in sig.parameters:
                    ...
                    logger.info(
                        f"Using template system for documentation from {self.template_db_path}"
                    )
                    ...
                    for ext in [".json", ".jsonl", ".npz", ".npy"]:
                        expected_path = os.path.join(
                            EXPECTED_RESULTS_DIR, model_dir, f"{self.hardware}{ext}"
                        )
                    ...
                    logger.warning(
                        "Documentation generator doesn't support template system parameters"
                    )
            except Exception as e:
        ...
        doc_content = doc_generator.generate_markdown(
            test_results=test_results, benchmark_results=benchmark_results, git_hash=self.git_hash
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results_dir = self.save_results(
                test_success, test_results, benchmark_success, benchmark_results, components
            )
            ...
            if self.generate_docs:
                doc_file = self.generate_documentation(
                    skill_file, test_file, benchmark_file, test_results, benchmark_results
                )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Integrated Component Test Runner for IPFS Accelerate"
    )
    ...
    model_group.add_argument(
        "--model-family", type=str, help="Model family to test (e.g., text-embedding, vision)"
    )
    ...
    hardware_group.add_argument(
        "--all-hardware", action="store_true", help="Test on all supported hardware platforms"
    )

    # Test options
    parser.add_argument(
        "--quick-test", action="store_true", help="Run a quick test with minimal validation"
    )
    parser.add_argument(
        "--update-expected",
        action="store_true",
    ...
    parser.add_argument(
        "--generate-docs", action="store_true", help="Generate markdown documentation for models"
    )
    parser.add_argument(
    ...
    # Database options
    parser.add_argument(
        "--db-path", type=str, default=None, help="Path to the DuckDB database for storing results"
    )
    parser.add_argument("--no-db", action="store_true", help="Disable database storage of results")

    # Template system options
    parser.add_argument(
        "--template-db-path", type=str, default=None, help="Path to the template database"
    )
    parser.add_argument("--no-templates", action="store_true", help="Disable template system usage")
    ...
    parser.add_argument(
        "--workers", type=int, default=None, help="Number of worker threads for parallel testing"
    )
    parser.add_argument(
...
def run_parallel_tests(
    models: List[str], hardware_platforms: List[str], args, max_workers: int = None
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_combo = {
            executor.submit(run_test, model, hw): (model, hw) for model, hw in combinations
        }
...
def main():
    ...
    for model, hw_results in results.items():
        ...
        for hardware, result in hw_results.items():
            ...
            print(
                f"  {hardware}: {status} (Test: {'✅' if result.get('test_success', False) else '❌'}, "
