...
class ModelDocGenerator:
        ...
        if HAS_TEMPLATE_SYSTEM and self.template_db_path and os.path.exists(self.template_db_path):
            try:
                ...
                self.template_renderer = TemplateRenderer(
                    db_path=self.template_db_path, verbose=self.verbose
                )
    ...
    def extract_docstrings(self, file_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            class_matches = re.finditer(
                r'class\s+(\w+).*?:(?:\s+"""(.*?)""")?', file_content, re.DOTALL
            )
            ...
            method_matches = re.finditer(
                r'def\s+(\w+).*?:(?:\s+"""(.*?)""")?', file_content, re.DOTALL
            )
        ...
        try:
            ...
            if "skill" in os.path.basename(file_path):
                ...
                main_match = re.search(
                    r'if\s+__name__\s*==\s*"__main__".*', file_content, re.DOTALL
                )
                if main_match:
    ...
    def generate_documentation(self) -> str:
        ...
        doc_path = os.path.join(
            model_doc_dir, f"{self.model_name.replace('/', '_')}_{self.hardware}_docs.md"
        )
        ...
        if self.use_templates:
            try:
                ...
                if not model_family:
                    logger.warning(
                        f"Could not determine model family for {self.model_name}, falling back to manual generation"
                    )
                ...
                variables = {
                    ...
                    # Code snippets
                    "class_definition": skill_snippets.get(
                        "class_definition", "# No class definition found"
                    ),
                    ...
                    "benchmark_function": benchmark_snippets.get(
                        "benchmark_function", "# No benchmark function found"
                    ),
                    "main_execution": benchmark_snippets.get(
                        "main_execution", "# No main execution block found"
                    ),
                    # Expected results
    ...
    def _extract_api_details(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"class_name": "Unknown", "methods": {}}
...
```python
# Import the skill class
from {self.model_name.replace("-", "_").replace("/", "_")}_{self.hardware}_skill import {
                model_class_name
            }
...
- **Performance Characteristics**: Varies based on specific {hardware} hardware capabilities"""
    ...
    def _generate_documentation_manual(self, doc_path: str) -> str:
        ...
        with open(doc_path, "w") as f:
            ...
            f.write("## Overview\n\n")
            f.write(
                f"This document describes the implementation and testing of the {self.model_name} model "
            )
            f.write(
                f"on {self.hardware} hardware. It includes details about the skill implementation, "
            )
            ...
            f.write(
                "The skill implementation is responsible for loading and running the model.\n\n"
            )
            ...
            f.write(
                "The test implementation validates that the model produces correct outputs.\n\n"
            )
            ...
            if expected_results:
                ...
                f.write(
                    "No expected results are available yet. Run the tests and update the expected results.\n\n"
                )
            ...
            f.write(
                f"- Last updated: {os.environ.get('USER', 'unknown')}, {os.environ.get('DATE', 'auto-generated')}\n\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-templates", action="store_true", help="Use templates for documentation generation"
    )
