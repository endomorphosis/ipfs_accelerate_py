...
class ModelValidator:
    ...
    def validate_skill(self, skill_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results = {"valid": True, "warnings": [], "missing_elements": []}
            ...
            if expected_class_name not in skill_content:
                validation_results["missing_elements"].append(
                    f"Missing expected class: {expected_class_name}"
                )
            ...
            if self.hardware not in skill_content:
                validation_results["warnings"].append(
                    f"No hardware-specific code found for {self.hardware}"
                )
    ...
    def validate_test(self, test_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if expected_class_name not in test_content:
                validation_results["missing_elements"].append(
                    f"Missing expected test class: {expected_class_name}"
                )
                validation_results["valid"] = False
            ...
            if "test_" not in test_content:
                validation_results["missing_elements"].append(
                    "No test methods found (should start with 'test_')"
                )
                validation_results["valid"] = False
    ...
    def validate_all(self, skill_path: str, test_path: str, benchmark_path: str) -> Dict[str, Any]:
        ...
        all_valid = (
            ...
            and test_validation.get("valid", False)
            and benchmark_validation.get("valid", False)
        )
...
class ResultComparer:
    ...
    def compare(self, expected: Dict[str, Any], actual: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in expected:
            ...
            if isinstance(expected[key], dict) and isinstance(actual[key], dict):
                ...
                if (
                    not nested_comparison.get("matches", False)
                    and "differences" in nested_comparison
                ):
                ...
                if len(expected[key]) != len(actual[key]):
                    ...
                    try:
                        ...
                        if not np.allclose(
                            ...
                            actual_arr,
                            rtol=self.tolerance,
                            atol=self.tolerance * np.abs(expected_arr).mean(),
                        ):
                        ...
                        if list_diffs:
                            differences[key] = {
                                ...
                                "examples": {
                                    ...
                                    for idx, exp, act in list_diffs[:5]
                                },  # Show first 5 differences
                            }
    ...
    def statistical_tensor_compare(
        self, expected_data: List[float], actual_data: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        if not self.has_numpy:
            return {"matches": False, "reason": "statistical comparison requires numpy"}
        ...
        try:
            ...
            mean_rel_diff = abs(exp_mean - act_mean) / (
                abs(exp_mean) if abs(exp_mean) > 1e-10 else 1.0
            )
            ...
            if len(exp_arr) > 100:
                ...
                p50_rel_diff = abs(exp_p50 - act_p50) / (
                    abs(exp_p50) if abs(exp_p50) > 1e-10 else 1.0
                )
                p95_rel_diff = abs(exp_p95 - act_p95) / (
                    abs(exp_p95) if abs(exp_p95) > 1e-10 else 1.0
                )

                # Update match status with percentile comparison
                stats_match = (
                    stats_match
                    and p50_rel_diff <= self.tolerance
                    and p95_rel_diff <= self.tolerance
                )
            ...
            return {"matches": False, "reason": f"statistical comparison error: {str(e)}"}

    def deep_compare_tensors(
        self,
            ...
            if isinstance(obj, dict):
                for k, v in obj.items():
                    ...
                    if isinstance(v, dict) and any(
                        tensor_key in v for tensor_key in ["data", "shape", "dtype"]
                    ):
                ...
                if isinstance(exp_data, list) and isinstance(act_data, list):
                    try:
                        ...
                        if not np.allclose(exp_arr, act_arr, rtol=rtol, atol=atol):
                            ...
                            diff_indices = np.where(
                                ~np.isclose(exp_arr, act_arr, rtol=rtol, atol=atol)
                            )
...
if __name__ == "__main__":
    ...
    expected = {
        "output": {"value": 10.0},
        "metrics": {"latency_ms": 15.5, "throughput": 80.0, "memory_mb": 512},
    }
