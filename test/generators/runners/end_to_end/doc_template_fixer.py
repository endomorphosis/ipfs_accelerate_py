...
def monkey_patch_model_doc_generator():
    ...
    def patched_generate_documentation(self) -> str:
        ...
        logger.info(
            f"Using patched generate_documentation for {self.model_name} on {self.hardware}"
        )
        ...
        if self.use_templates:
            try:
                ...
                formatted_features = "\n".join(
                    [f"- {feature}" for feature in model_specific_features]
                )
                formatted_use_cases = "\n".join(
                    [f"- {use_case}" for use_case in model_common_use_cases]
                )
...
def monkey_patch_template_renderer():
    ...
    def patched_process_variable_transforms(self, content: str, variables: Dict[str, Any]) -> str:
        ...
        def replace_var(match):
            ...
            if var_name not in variables:
                logger.warning(
                    f"Variable '{var_name}' not found in variables dictionary, using placeholder"
                )
        ...
        def replace_with_transform(match):
            ...
            if var_name not in variables:
                logger.warning(
                    f"Variable '{var_name}' not found in variables dictionary for transform"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Error processing transformation '{transform}' for variable '{var_name}': {e}"
                )
                return f"PLACEHOLDER_{var_name}_{transform}"
    ...
    def patched_render_template(
        ...
        if not template:
            raise ValueError(
                f"No template found for {model_family} {template_type} on {hardware_platform}"
            )
        ...
        rendered_content = self.db.render_template(
            template_id=template["template_id"], variables=base_variables, render_dependencies=True
        )
