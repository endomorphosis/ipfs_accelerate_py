...
SUPPORTED_HARDWARE = ["cpu", "cuda", "rocm", "mps", "openvino", "qnn", "webnn", "webgpu", "samsung"]
...
HARDWARE_DETECTION_MAP = {
    ...
    "rocm": lambda: (
        hasattr(torch, "hip") and torch.hip.is_available() if HAS_HARDWARE_DETECTION else False
    ),
    "mps": lambda: (
        hasattr(torch, "mps") and torch.mps.is_available() if HAS_HARDWARE_DETECTION else False
    ),
...
class E2ETester:
    ...
    def _determine_hardware_to_test(self) -> List[str]:
        ...
        logger.error(
            "No hardware specified. Use --hardware, --priority-hardware, or --all-hardware"
        )
        return []
        ...
        for model in self.models_to_test:
            ...
            for hardware in self.hardware_to_test:
                ...
                try:
                    ...
                    skill_path, test_path, benchmark_path = self._generate_components(
                        model, hardware, temp_dir
                    )
                    ...
                    if self.args.generate_docs:
                        self._generate_documentation(
                            model, hardware, skill_path, test_path, benchmark_path
                        )
                    ...
                    logger.info(
                        f"Testing {model} on {hardware} - {'SUCCESS' if comparison['matches'] else 'FAILURE'}"
                    )

                except Exception as e:
                    logger.error(f"Error testing {model} on {hardware}: {str(e)}")
                    self.test_results[model][hardware] = {"status": "error", "error": str(e)}
    ...
    def _run_distributed_tests(self):
        ...
        for i in range(self.workers):
            worker = threading.Thread(target=self._worker_function, args=(i,), daemon=True)
            worker.start()
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Worker {worker_id}: {model} on {hardware} - {'SUCCESS' if comparison['matches'] else 'FAILURE'}"
                    )

                except Exception as e:
                    logger.error(
                        f"Worker {worker_id} error testing {model} on {hardware}: {str(e)}"
                    )
                    self.result_queue.put((model, hardware, {"status": "error", "error": str(e)}))
    ...
    def _generate_components(
        self, model: str, hardware: str, temp_dir: str
    ) -> Tuple[str, str, str]:
...
if __name__ == "__main__":
    ...
    def _mock_generate_benchmark(
        self, model: str, hardware: str, benchmark_path: str, skill_path: str
    ):
    ...
    def _run_test(self, model: str, hardware: str, temp_dir: str, test_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["python", modified_test_path], capture_output=True, text=True, timeout=TEST_TIMEOUT
            )
            ...
            if not os.path.exists(results_json):
                logger.warning(
                    f"Test executed successfully but no results file was found at {results_json}"
                )
                ...
                test_results.update(
                    {
                        ...
                        "memory_mb": memory_diff,
                        "console_output": result.stdout,
                        "hardware_details": test_results.get(
                            "hardware_details",
                            {
                                "platform": hardware,
                                "device_name": self._get_hardware_device_name(hardware),
            ...
            return {
                ...
                "metrics": {"latency_ms": 12.5, "throughput": 80.0, "memory_mb": 512},
                "hardware_details": {
                    "platform": hardware,
                    "device_name": self._get_hardware_device_name(hardware),
                },
...
"""
            ...
            if 'if __name__ == "__main__"' in content:
                ...
                content = content.replace(
                    'if __name__ == "__main__"', result_output + '\nif __name__ == "__main__"'
                )
            else:
    ...
    def _get_hardware_device_name(self, hardware: str) -> str:
        ...
        if hardware == "cpu":
            ...
            return f"CPU: {platform.processor()} ({cores} cores)" + (
            ...
            try:
                ...
                if hasattr(torch, "mps") and torch.mps.is_available():
                    return f"MPS: Apple Silicon ({torch.mps.get_device_name()})" + (
                        " [SIMULATED]" if is_sim else ""
                    )
                else:
    ...
    def _compare_with_expected(
        self, model: str, hardware: str, result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not comparison_result["match"]:
                ...
                for key, diff in comparison_result.get("differences", {}).items():
                    logger.warning(
                        f"  - {key}: expected {diff.get('expected')}, got {diff.get('actual')}"
                    )
            else:
    ...
    def _store_results(
        self, model: str, hardware: str, result: Dict[str, Any], comparison: Dict[str, Any]
    ):
        ...
        if self.use_db and HAS_DB_API:
            try:
                ...
                db_result = {
                    ...
                    "error_message": str(comparison.get("differences", {}))
                ...
                try:
                    ...
                    logger.info(
                        f"Stored results for {model} on {hardware} in database at {self.db_path}"
                    )
                except Exception as db_conn_error:
                    # If connection manager fails, try direct approach
                    logger.warning(
                        f"DB connection manager failed: {str(db_conn_error)}, trying direct approach"
                    )
    ...
    def _generate_documentation(
        self, model: str, hardware: str, skill_path: str, test_path: str, benchmark_path: str
    ):
        ...
        expected_results_path = os.path.join(
            EXPECTED_RESULTS_DIR, model, hardware, "expected_result.json"
        )
...
""")
            ...
            logger.info(
                f"Generated fallback documentation for {model} on {hardware} at {fallback_doc_path}"
            )
    ...
    def _generate_summary_report(self):
        ...
        summary = {
            "timestamp": self.timestamp,
            "summary": {"total": 0, "success": 0, "failure": 0, "error": 0},
        ...
        for model, hw_results in self.test_results.items():
            for hw, result in hw_results.items():
                ...
                summary["summary"][result["status"]] = (
                    summary["summary"].get(result["status"], 0) + 1
                )
            ...
            for model, hw_results in self.test_results.items():
                ...
                for hw, result in hw_results.items():
                    ...
                    if (
                        ...
                        and "comparison" in result
                        and "differences" in result["comparison"]
                    ):
                ...
                for result_dir in os.listdir(hw_path):
                    ...
                    try:
                        dir_time = datetime.datetime.strptime(
                            result_dir, "%Y%m%d_%H%M%S"
                        ).timestamp()
                        if dir_time < cutoff_time:
                            # Check if it's a failed test that we want to keep
                            if (
                                os.path.exists(os.path.join(result_path, "failure.status"))
                                and not self.args.clean_failures
                            ):
                                continue
...
def parse_args():
    ...
    model_group.add_argument(
        "--model-family", help="Model family to test (e.g., text-embedding, vision)"
    )
    ...
    hardware_group.add_argument(
        "--hardware", help="Hardware platforms to test, comma-separated (e.g., cpu,cuda,webgpu)"
    )
    hardware_group.add_argument(
        "--priority-hardware",
        action="store_true",
    ...
    parser.add_argument(
        "--days", type=int, help="Number of days to keep results when cleaning (default: 14)"
    )
    parser.add_argument(
        "--clean-failures", action="store_true", help="Clean failed test results too"
    )
    ...
    parser.add_argument(
        "--db-path",
        help="Path to the database file (default: $BENCHMARK_DB_PATH or ./benchmark_db.duckdb)",
    )
    parser.add_argument(
        "--db-only", action="store_true", help="Store results only in the database, not in files"
    )

    # Distributed testing options
    parser.add_argument(
        "--distributed", action="store_true", help="Run tests in parallel using worker threads"
    )
    parser.add_argument(
        "--workers",
        type=int,
        help=f"Number of worker threads for distributed testing (default: {WORKER_COUNT})",
    )
    parser.add_argument(
        "--simulation-aware",
        action="store_true",
        help="Be explicit about real vs simulated hardware testing",
    )
    ...
    parser.add_argument(
        "--github-actions", action="store_true", help="Optimize output for GitHub Actions"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Tolerance for tensor comparison (default: 0.1)",
    )
    parser.add_argument(
        "--parallel-docs", action="store_true", help="Generate documentation in parallel"
    )
...
def generate_ci_report(ci_info, test_results, timestamp):
    ...
    for model, hw_results in test_results.items():
        ...
        for hw, result in hw_results.items():
            ...
            report["results_by_hardware"][hw][result["status"]] = (
                report["results_by_hardware"][hw].get(result["status"], 0) + 1
            )
    ...
    with open(md_path, "w") as f:
        ...
        overall_status = (
            "SUCCESS"
            if report["summary"].get("failure", 0) == 0 and report["summary"].get("error", 0) == 0
            else "FAILURE"
        )
...
def main():
    ...
    success = sum(
        sum(1 for result in hw_results.values() if result["status"] == "success")
