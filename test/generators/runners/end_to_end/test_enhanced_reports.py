...
class TestSimulationValidator(unittest.TestCase):
    ...
    def test_is_simulation(self):
        ...
        self.assertTrue(
            self.validator.is_simulation(self.sample_results["bert-base-uncased"]["webgpu"])
        )

        # False for result without simulation flag
        self.assertFalse(
            self.validator.is_simulation(self.sample_results["bert-base-uncased"]["cpu"])
        )
...
class TestCrossHardwareComparison(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_results = {
            "bert-base-uncased": {
                "cpu": {
                    "status": "success",
                    "metrics": {"throughput": 100.0, "latency": 10.0, "memory": 1000.0},
                },
                "cuda": {
                    "status": "success",
                    "metrics": {"throughput": 350.0, "latency": 2.85, "memory": 2000.0},
                },
                "webgpu": {
                    "status": "success",
                    "metrics": {"throughput": 200.0, "latency": 5.0, "memory": 1500.0},
                },
            },
            "t5-small": {
                "cpu": {
                    "status": "success",
                    "metrics": {"throughput": 50.0, "latency": 20.0, "memory": 2000.0},
                },
                "cuda": {
                    "status": "success",
                    "metrics": {"throughput": 175.0, "latency": 5.7, "memory": 3000.0},
                },
            },
        }
    ...
    def test_determine_model_family(self):
        ...
        self.assertEqual(
            "text-embedding", self.comparator._determine_model_family("bert-base-uncased")
        )
    ...
    def test_extract_performance_metrics(self):
        ...
        metrics = self.comparator._extract_performance_metrics(
            self.sample_results["bert-base-uncased"]["cuda"]
        )
