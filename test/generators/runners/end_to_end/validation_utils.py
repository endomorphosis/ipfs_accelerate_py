...
class SimulationValidator:
    ...
    def validate_performance(
        ...
        if (hw1, hw2) not in self.reference_ratios and (hw2, hw1) not in self.reference_ratios:
            return {"valid": None, "reason": f"No reference ratio available for {hw1} vs {hw2}"}
        ...
        if not perf1 or not perf2:
            return {"valid": None, "reason": "Missing performance metrics"}
        ...
        if not actual_ratios:
            return {"valid": None, "reason": "Could not calculate performance ratios"}
                ...
                if "1" in batch_results:  # Use batch size 1 if available
                    if "average_throughput_items_per_second" in batch_results["1"]:
                        metrics["throughput"] = float(
                            batch_results["1"]["average_throughput_items_per_second"]
                        )
    ...
    def validate_results(self, results: Dict[str, Dict[str, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        for model, hw_results in simulations.items():
            ...
            for sim_hw, sim_result in hw_results.items():
                ...
                real_results = {
                    hw: result
                    for hw, result in results.get(model, {}).items()
        ...
        return {"validations": validations, "simulations": simulations}

    def generate_validation_report(
        self, results: Dict[str, Dict[str, Dict[str, Any]]], output_dir: str
    ) -> Dict[str, str]:
        """
        ...
        return {"html": html_report, "markdown": md_report, "visualization": viz_file}

    def _generate_html_report(self, validation_result: Dict[str, Any], output_path: str) -> None:
        """
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                "<style>body{font-family:sans-serif;line-height:1.6;max-width:1200px;margin:0 auto;padding:1em}"
            )
            f.write(
                "h1,h2,h3{color:#333}table{border-collapse:collapse;width:100%}th,td{text-align:left;border:1px solid #ddd;padding:8px}"
            )
            f.write(
                "th{background:#f2f2f2}.success{color:green}.failure{color:red}.unknown{color:orange}</style></head><body>"
            )

            # Report header
            f.write("<h1>Simulation Validation Report</h1>")
            f.write(
                "<p>This report validates simulation accuracy against real hardware performance expectations.</p>"
            )
            ...
            if total_validations > 0:
                ...
                f.write(
                    f"<p>Validity Rate: <span class='{'success' if validity_rate >= 80 else 'failure'}'>{validity_rate:.1f}%</span></p>"
                )

            # Visualization reference
            f.write("<h2>Validation Visualization</h2>")
            f.write(
                "<img src='simulation_validation_visualization.png' alt='Validation Visualization' width='100%'>"
            )
            ...
            for model, hw_validations in validations.items():
                ...
                for sim_hw, real_validations in hw_validations.items():
                    ...
                    for real_hw, validation in real_validations.items():
                        ...
                        status_text = (
                            "VALID" if valid is True else "INVALID" if valid is False else "UNKNOWN"
                        )

                        f.write(
                            f"<h5>Compared to {real_hw}: <span class='{status_class}'>{status_text}</span></h5>"
                        )
                        ...
                        if metrics:
                            f.write(
                                "<table><tr><th>Metric</th><th>Expected Ratio</th><th>Actual Ratio</th><th>Deviation</th><th>Status</th></tr>"
                            )
                            ...
                            for metric, metric_validation in metrics.items():
                                ...
                                f.write(
                                    f"<tr><td>{metric.capitalize()}</td><td>{expected_ratio:.2f}x</td>"
                                )
    ...
    def _generate_markdown_report(
        self, validation_result: Dict[str, Any], output_path: str
    ) -> None:
        """
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                "This report validates simulation accuracy against real hardware performance expectations.\n\n"
            )
            ...
            f.write(
                "![Simulation Validation Visualization](simulation_validation_visualization.png)\n\n"
            )
            ...
            for model, hw_validations in validations.items():
                ...
                for sim_hw, real_validations in hw_validations.items():
                    ...
                    for real_hw, validation in real_validations.items():
                        ...
                        if metrics:
                            f.write(
                                "| Metric | Expected Ratio | Actual Ratio | Deviation | Status |\n"
                            )
                            f.write(
                                "|--------|---------------|--------------|-----------|--------|\n"
                            )
                            ...
                            for metric, metric_validation in metrics.items():
                                ...
                                f.write(
                                    f"| {metric.capitalize()} | {expected_ratio:.2f}x | {actual_ratio:.2f}x | {deviation:.1f}% | {status_text} |\n"
                                )

                            f.write("\n")

    def _generate_visualization(self, validation_result: Dict[str, Any], output_path: str) -> None:
        """
        ...
        if not models or not simulated_hw or not real_hw:
            ...
            plt.text(
                0.5,
                0.5,
                "No simulation validation data available",
        ...
        for i, valid in enumerate(validity):
            if valid is True:
                plt.plot(
                    unique_models.index(models[i]),
                    unique_sim_hw.index(simulated_hw[i]),
                    "o",
                    color="none",
...
class CrossHardwareComparison:
    ...
    def generate_comparison(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for model, hw_metrics in metrics.items():
            ...
            best_hw_throughput = max(
                hw_metrics.items(),
                key=lambda x: x[1].get("throughput", 0) if "throughput" in x[1] else 0,
                default=(None, {}),
            )
            ...
            best_hw_latency = min(
                hw_metrics.items(),
                key=lambda x: (
                    x[1].get("latency", float("inf")) if "latency" in x[1] else float("inf")
                ),
                default=(None, {}),
            )
    ...
    def _determine_model_family(self, model_name: str) -> str:
        ...
        if any(
            ...
            keyword in model_name.lower() for keyword in ["gpt", "llama", "opt", "t5", "falcon"]
        ):
            return "text-generation"
        elif any(keyword in model_name.lower() for keyword in ["vit", "resnet", "detr", "yolo"]):
            return "vision"
        elif any(
            keyword in model_name.lower() for keyword in ["whisper", "wav2vec", "clap", "audio"]
        ):
            return "audio"
            ...
            if "results_by_batch" in benchmark:
                ...
                if "4" in batch_results:
                    if "average_throughput_items_per_second" in batch_results["4"]:
                        metrics["throughput_batch4"] = float(
                            batch_results["4"]["average_throughput_items_per_second"]
                        )
    ...
    def _compare_metrics(
        ...
        if "throughput" in metrics1 and "throughput" in metrics2 and metrics2["throughput"] > 0:
            ...
            comparison["throughput_percentage"] = (
                throughput_ratio - 1
        ...
        if (
            "throughput_batch4" in metrics1
            and "throughput_batch4" in metrics2
            and metrics2["throughput_batch4"] > 0
        ):
        ...
        if "throughput_ratio" in comparison and "latency_ratio" in comparison:
            comparison["overall_score"] = (
                comparison["throughput_ratio"] + comparison["latency_ratio"]
            ) / 2
    ...
    def _find_best_hardware_for_family(
        self, family_hw_metrics: Dict[str, Dict[str, Dict[str, Any]]]
    ) -> Dict[str, str]:
        """
        ...
        for hw, scores in hw_scores.items():
            if scores["total_comparisons"] > 0:
                scores["win_rate"] = scores["win_count"] / (
                    scores["total_comparisons"] * 2
                )  # Multiplied by 2 because we count throughput and latency separately
        ...
        if hw_scores:
            best_overall = max(
                hw_scores.items(), key=lambda x: x[1]["win_rate"], default=(None, {})
            )[0]
        ...
        if hw_scores:
            best_throughput = max(
                hw_scores.items(), key=lambda x: x[1]["throughput_score"], default=(None, {})
            )[0]
        ...
        if hw_scores:
            best_latency = max(
                hw_scores.items(), key=lambda x: x[1]["latency_score"], default=(None, {})
            )[0]
    ...
    def generate_comparison_report(
        self, results: Dict[str, Dict[str, Any]], output_dir: Optional[str] = None
    ) -> Dict[str, str]:
        """
    ...
    def _generate_html_report(self, comparison_data: Dict[str, Any], output_path: str) -> None:
        """
        ...
        with open(output_path, "w") as f:
            f.write(
                "<!DOCTYPE html><html><head><title>Cross-Hardware Performance Comparison</title>"
            )
            f.write(
                "<style>body{font-family:sans-serif;line-height:1.6;max-width:1200px;margin:0 auto;padding:1em}"
            )
            ...
            f.write(
                "th{background:#f2f2f2}.success{color:green}.failure{color:red}.chart{width:100%;max-width:800px;margin:1em 0}</style></head><body>"
            )

            # Report header
            f.write("<h1>Cross-Hardware Performance Comparison</h1>")
            f.write(
                "<p>This report compares performance metrics across different hardware platforms to identify optimal hardware for each model type.</p>"
            )

            # Visualizations
            f.write("<h2>Performance Visualization</h2>")
            f.write(
                "<div><img src='visualizations/performance_heatmap.png' alt='Performance Heatmap' class='chart'>"
            )
            f.write(
                "<img src='visualizations/family_comparison.png' alt='Family Comparison Chart' class='chart'></div>"
            )

            # Recommended hardware by model family
            f.write("<h2>Recommended Hardware by Model Family</h2>")
            f.write(
                "<table><tr><th>Model Family</th><th>Best Overall</th><th>Best for Throughput</th><th>Best for Latency</th></tr>"
            )
            ...
            for family, optimal in family_optimal_hardware.items():
                ...
                f.write(
                    f"<tr><td>{family}</td><td>{best_overall}</td><td>{best_throughput}</td><td>{best_latency}</td></tr>"
                )
                    ...
                    for hw1 in hardware_list:
                        ...
                        for hw2 in hardware_list:
                            if hw1 == hw2:
                                ...
                                cell_class = (
                                    "success"
                                    if throughput_pct > 0 and latency_pct > 0
                                    else "failure"
                                    if throughput_pct < 0 and latency_pct < 0
                                    else ""
                                )
                                cell_content = (
                                    f"T: {throughput_pct:+.1f}%<br>L: {latency_pct:+.1f}%"
                                )
    ...
    def _generate_markdown_report(self, comparison_data: Dict[str, Any], output_path: str) -> None:
        """
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                "This report compares performance metrics across different hardware platforms to identify optimal hardware for each model type.\n\n"
            )
    ...
    def _generate_family_comparison_chart(
        self, comparison_data: Dict[str, Any], output_path: str
    ) -> None:
        """
        ...
        if not family_comparison or not hardware_list:
            ...
            plt.text(
                0.5,
                0.5,
                "No family comparison data available",
                horizontalalignment="center",
                verticalalignment="center",
                fontsize=14,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir", default="collected_results", help="Directory containing test results"
    )

    parser.add_argument("--output-dir", default="reports", help="Directory to save reports")

    parser.add_argument(
        "--simulation-validation", action="store_true", help="Generate simulation validation report"
    )
    ...
    parser.add_argument("--combined-report", action="store_true", help="Generate both reports")
    ...
    if args.simulation_validation or args.combined_report:
        ...
        validator.generate_validation_report(
            results, os.path.join(args.output_dir, "simulation_validation")
        )

    if args.cross_hardware_comparison or args.combined_report:
        comparator = CrossHardwareComparison(
            output_dir=os.path.join(args.output_dir, "cross_hardware")
        )
