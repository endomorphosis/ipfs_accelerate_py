...
class StatusBadgeGenerator:
  ...
  <g fill="#fff" text-anchor="middle" font-family="DejaVu Sans,Verdana,Geneva,sans-serif" font-size="11">
    ...
    @staticmethod
    def generate_model_badges(
        model_results: Dict[str, Dict[str, Any]], output_dir: str
    ) -> Dict[str, str]:
        """
...
class ReportGenerator:
            ...
            for summary_file in summary_files:
                try:
                    ...
                    timestamp_match = re.search(
                        r"summary_(\d{8}_\d{6})\.json", os.path.basename(summary_file)
                    )
    ...
    def _process_historical_data(self, collected_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for data in collected_data:
            ...
            for model, hw_results in results.items():
                ...
                result["historical"]["model_success_rates"][model]["success_rates"].append(
                    model_success_rate
                )
            ...
            for hw, hw_results in hw_results_dict.items():
                ...
                result["historical"]["hardware_success_rates"][hw]["success_rates"].append(
                    hw_success_rate
                )
    ...
    def _is_simulation(self, result: Dict[str, Any]) -> bool:
        ...
        if "result_path" in result:
            ...
            if os.path.exists(result_file):
                try:
                    ...
                    if (
                        env.get("simulation", False)
                        or "simulator" in env.get("platform", "").lower()
                    ):
                        return True
        ...
        if "comparison" in result:
            ...
            if (
                comparison.get("matches", True) == False
                and len(comparison.get("differences", {})) > 3
            ):
    ...
    def _generate_combined_html_report(
        ...
        hardware_comparison_report: str,
    ) -> str:
        """
        ...
        try:
            with open(simulation_validation_report, "r") as f:
                ...
                sim_validation_body = re.search(
                    r"<body>(.*?)</body>", simulation_validation_content, re.DOTALL
                )
        ...
        try:
            with open(hardware_comparison_report, "r") as f:
                ...
                # Extract body content
                hw_comparison_body = re.search(
                    r"<body>(.*?)</body>", hardware_comparison_content, re.DOTALL
                )
        ...
        with open(combined_report_path, "w") as f:
            ...
            f.write(
                "  <title>Combined Report - Simulation Validation and Hardware Comparison</title>\n"
            )
            f.write(HTML_STYLE)
            f.write("</head>\n")
            f.write("<body>\n")
            ...
            f.write(
                "<p>This report combines the simulation validation and cross-hardware comparison results.</p>\n"
            )
    ...
    def _generate_combined_markdown_report(
        self,
        results: Dict[str, Dict[str, Any]],
        simulation_validation_report: str,
        hardware_comparison_report: str,
        ...
        with open(combined_report_path, "w") as f:
            ...
            f.write(
                "This report combines the simulation validation and cross-hardware comparison results.\n\n"
            )
    ...
    def _generate_html_reports(self) -> Dict[str, str]:
        ...
        with open(main_report, "w") as f:
            ...
            f.write(
                f'<p><strong>Success Rate:</strong> <span class="success">{success_rate:.1f}%</span></p>\n'
            )
            f.write("</div>\n")
            ...
            if historical and "success_rates" in historical and historical["success_rates"]:
                ...
                for i, timestamp in enumerate(historical["timestamps"]):
                    ...
                    f.write(
                        f"  <tr><td>{date_str}</td><td>{success_rate:.1f}%</td><td>{total_tests}</td></tr>\n"
                    )
            ...
            if results:
                ...
                for model, hw_results in results.items():
                    ...
                    for hw, result in hw_results.items():
                        ...
                        is_simulation = self._is_simulation(result)
                        simulation_tag = (
                            " (SIMULATION)" if is_simulation and self.highlight_simulation else ""
                        )
            ...
            if results:
                ...
                for model, hw_results in results.items():
                    ...
                    for hw, result in hw_results.items():
                        status = result.get("status", "unknown")
                        status_icon = (
                            "✅" if status == "success" else "❌" if status == "failure" else "⚠️"
                        )
    ...
    def generate_report(self) -> Dict[str, str]:
        ...
        if self.badge_only:
            ...
            hw_reports = comparator.generate_comparison_report(self.test_results.get("results", {}))
            ...
            reports = comparator.generate_comparison_report(self.test_results.get("results", {}))
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Generate enhanced CI/CD reports for end-to-end testing"
    )
    ...
    parser.add_argument(
        "--historical", action="store_true", help="Include historical trend data in the report"
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    parser.add_argument(
        "--badge-only", action="store_true", help="Generate only status badges (no full reports)"
    )
    parser.add_argument("--ci", action="store_true", help="Generate reports for CI/CD integration")
    parser.add_argument(
        "--github-pages", action="store_true", help="Generate reports for GitHub Pages"
    )
