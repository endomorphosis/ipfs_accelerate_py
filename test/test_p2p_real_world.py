...
async def test_two_peers_communication():
    ...
    try:
        ...
        host1 = await new_libp2p_host()
        host1_id = peer_id_text(host1.get_id())
        ...
        host2 = await new_libp2p_host()
        host2_id = peer_id_text(host2.get_id())
        ...
        if host1_addrs:
            # Use first address
            target_addr = str(host1_addrs[0])
            if "/p2p/" not in target_addr:
                target_addr = f"{target_addr}/p2p/{host1_id}"
