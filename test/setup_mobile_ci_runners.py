...
class MobileCIRunnerSetup:
    ...
    def __init__(
        self, platform_type: str = "all", device_id: Optional[str] = None, verbose: bool = False
    ):
        """
    ...
    def _check_android_environment(self) -> Dict[str, Any]:
        ...
        try:
            adb_output = subprocess.run(
                ["adb", "version"], capture_output=True, text=True, check=False
            )
        ...
        try:
            ...
            if android_status["emulator_installed"]:
                avds = [
                    avd.strip() for avd in emulator_output.stdout.strip().split("\n") if avd.strip()
                ]
        ...
        if (
            android_status["java_installed"]
            and android_status["android_sdk_installed"]
            and android_status["adb_installed"]
        ):
    ...
    def _check_ios_environment(self) -> Dict[str, Any]:
        ...
        try:
            xcode_output = subprocess.run(
                ["xcode-select", "--print-path"], capture_output=True, text=True, check=False
            )
        ...
        try:
            xcrun_output = subprocess.run(
                ["xcrun", "--version"], capture_output=True, text=True, check=False
            )
            ...
            if ios_status["xcrun_installed"]:
                ...
                simctl_output = subprocess.run(
                    ["xcrun", "simctl", "list"], capture_output=True, text=True, check=False
                )
        ...
        # Determine overall status
        if ios_status["macos"] and ios_status["xcode_installed"] and ios_status["xcrun_installed"]:
        ...
        try:
            ...
            for line in lines[1:]:  # Skip header line
                if line.strip():
                    ...
                    if len(parts) >= 2:
                        device_id = parts[0]
                        device_info = {"id": device_id, "status": parts[1]}
    ...
    def _get_ios_devices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            output = subprocess.run(
                ["xcrun", "xctrace", "list", "devices"], capture_output=True, text=True, check=True
            )
        ...
        if self.platform_type == "all":
            ...
            for platform_type, platform_status in platforms.items():
                if platform_status.get("status") != "ready":
                    logger.error(
                        f"Platform {platform_type} is not ready: {platform_status.get('status')}"
                    )
                    all_ready = False
            ...
            if platform_status.get("status") != "ready":
                logger.error(
                    f"Platform {self.platform_type} is not ready: {platform_status.get('status')}"
                )
                return False
    ...
    def _configure_android_environment(self) -> bool:
            ...
            if missing_deps:
                ...
                if self.system == "Linux":
                    ...
                    try:
                        ...
                        # Install on macOS
                        subprocess.run(
                            ["brew", "install", "openjdk@11", "android-platform-tools"], check=True
                        )
                        logger.info("Dependencies installed successfully")
                    except (subprocess.SubprocessError, FileNotFoundError):
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "install",
                    "-r",
                    "test/android_test_harness/requirements.txt",
                ],
                check=True,
            )
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "test/android_test_harness/download_test_models.py"], check=True
            )
    ...
    def _configure_ios_environment(self) -> bool:
            ...
            for simulator in simulators:
                if simulator.get("id") == self.device_id:
                    ...
                    if simulator.get("state") != "Booted":
                        ...
                        try:
                            subprocess.run(["xcrun", "simctl", "boot", self.device_id], check=True)
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "install",
                    "-r",
                    "test/ios_test_harness/requirements.txt",
                ],
                check=True,
            )
            logger.info("Test dependencies installed successfully")
        ...
        try:
            ...
            subprocess.run([sys.executable, "-m", "pip", "install", "coremltools>=6.0"], check=True)
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    device_id,
                    "--output-db",
                    "benchmark_results/verify_test.duckdb",
                    "--timeout",
                    "60",
                    "--verbose",
                ],
    ...
    def _verify_ios_connectivity(self) -> bool:
        ...
        if devices:
            ...
            try:
                subprocess.run(["xcrun", "simctl", "boot", simulator.get("id")], check=True)
                logger.info("Simulator booted successfully")
                return self._test_ios_device(simulator.get("id"), True)
    ...
    def _test_ios_device(self, device_id: str, is_simulator: bool) -> bool:
        ...
        try:
            ...
            if not is_simulator:
                subprocess.run(
                    ["xcrun", "xctrace", "list", "devices"], capture_output=True, check=True
                )
            ...
            logger.info(
                f"{'Simulator' if is_simulator else 'Device'} {device_id} connectivity verified successfully"
            )
            return True

        except subprocess.SubprocessError as e:
            logger.error(
                f"Error testing {'simulator' if is_simulator else 'device'} {device_id}: {e}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--platform", choices=["android", "ios", "all"], default="all", help="Platform to target"
    )
    parser.add_argument("--device-id", help="Device ID for specific device operations")
    ...
    try:
        ...
        setup = MobileCIRunnerSetup(
            platform_type=args.platform, device_id=args.device_id, verbose=args.verbose
        )
