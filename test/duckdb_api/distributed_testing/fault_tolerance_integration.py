...
class CircuitBreakerIntegration:
    ...
    def get_worker_circuit(self, worker_id: str) -> CircuitBreaker:
        ...
        return self.circuit_registry.get_or_create(circuit_name, **self.worker_circuit_config)
    ...
    def get_hardware_circuit(self, hardware_class: str) -> CircuitBreaker:
        ...
        return self.circuit_registry.get_or_create(circuit_name, **self.hardware_circuit_config)
    ...
    def get_task_type_circuit(self, task_type: str) -> CircuitBreaker:
        ...
        return self.circuit_registry.get_or_create(circuit_name, **self.task_type_circuit_config)
            ...
            if "worker" in open_circuits[0]:
                ...
                return RecoveryAction(
                    ...
                    hardware_requirements={
                        "hardware": [
                            self.fault_tolerance_manager._determine_fallback_hardware_class(
                                hardware_class
                            ).lower()
                        ]
                    },
    ...
    def _track_failure(self, failure_context: FailureContext):
        ...
        if circuit_type == "worker":
            ...
            logger.debug(
                f"Updated worker circuit for {worker_id} due to {failure_context.error_type.name}"
            )
        elif circuit_type == "hardware":
            # Update hardware circuit
            hardware_circuit._on_failure()
            logger.debug(
                f"Updated hardware circuit for {hardware_class} due to {failure_context.error_type.name}"
            )
        else:
            # Update task type circuit for other errors
            task_type_circuit._on_failure()
            logger.debug(
                f"Updated task type circuit for {task_type} due to {failure_context.error_type.name}"
            )

    def track_success(
        self, task_id: str, worker_id: str, hardware_class: str = None, task_type: str = None
    ):
        """
...
def create_fault_tolerance_integration(
...
def apply_recovery_with_circuit_breaker(
    task_id: str,
    ...
    if coordinator:
        try:
            ...
            if recovery_action.strategy == RecoveryStrategy.IMMEDIATE_RETRY:
                ...
                if recovery_action.delay > 0:
                    ...
                    threading.Timer(
                        ...
                        lambda: coordinator.retry_task(
                            task_id, worker_id=recovery_action.worker_id
                        ),
                    ).start()
                else:
                    coordinator.retry_task(task_id, worker_id=recovery_action.worker_id)
                ...
                if recovery_action.hardware_requirements:
                    coordinator.update_task_requirements(
                        task_id, recovery_action.hardware_requirements
                    )
                ...
                if recovery_action.modified_task:
                    coordinator.update_task_config(
                        task_id, recovery_action.modified_task.get("config", {})
                    )
                coordinator.retry_task(task_id)
            ...
            logger.info(
                f"Applied recovery action for task {task_id}: {recovery_action.strategy.name}"
            )
            return True
