...
class ComprehensiveMonitoringDashboard:
    ...
    def _initialize_visualization_components(self):
        ...
        if fault_tolerance_system:
            self.fault_tolerance_viz = FaultToleranceVisualization(
                fault_tolerance_system=fault_tolerance_system, dashboard_integration=self
            )
    ...
    def register_page(self, page_id, page_title, page_generator):
        ...
        """
        self.registered_pages[page_id] = {"title": page_title, "generator": page_generator}
    ...
    def register_visualization(self, vis_id, vis_title, vis_generator):
        ...
        """
        self.registered_visualizations[vis_id] = {"title": vis_title, "generator": vis_generator}
    ...
    def _get_worker_metrics(self):
        ...
        if self.coordinator and hasattr(self.coordinator, "worker_manager"):
            ...
            worker_metrics = {
                ...
                "disconnected": sum(
                    1 for w in workers.values() if w.get("status") == "disconnected"
                ),
        ...
        return {
            ...
            "inactive": 1,
            "disconnected": 1,
            "by_hardware": {"cpu": 10, "cuda": 6, "rocm": 2, "webgpu": 3},
        }
    ...
    def _get_task_metrics(self):
        ...
        return {
            ...
            "failed": 1,
            "cancelled": 1,
            "by_type": {"benchmark": 15, "test": 8, "command": 2},
        }
    ...
    def _get_error_rate_metrics(self):
        ...
        if self.coordinator and hasattr(self.coordinator, "fault_tolerance_system"):
            ...
            return {
                "current": len(fault_tolerance_system.error_history)
                / fault_tolerance_system.error_window_size
                if fault_tolerance_system.error_history
                else 0,
                "threshold": fault_tolerance_system.error_rate_threshold,
                "severity_distribution": {
                    "low": sum(
                        1
                        for e in fault_tolerance_system.error_history
                        if e["severity"].value == "low"
                    ),
                    "medium": sum(
                        1
                        for e in fault_tolerance_system.error_history
        ...
        return {
            "current": 0.05,
            "threshold": 0.5,
            "severity_distribution": {"low": 3, "medium": 2, "high": 0, "critical": 0},
        }
    ...
    def generate_system_overview(self):
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("Worker Status", "Task Status", "Resource Utilization", "Error Rate"),
            specs=[
                [{"type": "pie"}, {"type": "pie"}],
        ...
        fig.add_trace(
            go.Pie(
                ...
                values=[
                    worker_metrics["active"],
                    worker_metrics["inactive"],
                    worker_metrics["disconnected"],
                ],
        ...
        fig.add_trace(
            go.Pie(
                ...
                values=[
                    ...
                    task_metrics["running"],
                    task_metrics["completed"],
                    task_metrics["failed"],
                    task_metrics["cancelled"],
                ],
        ...
        # Update layout
        fig.update_layout(title="System Overview", height=800, width=1200, showlegend=True)
        ...
        return {"figure": fig, "html_path": filepath, "metrics": metrics}
    ...
    def generate_worker_performance_visualization(self):
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "bar"}, {"type": "bar"}], [{"type": "box"}, {"type": "pie"}]],
        )
        ...
        execution_times = [[random.uniform(0.5, 2.0) for _ in range(10)] for _ in range(5)]

        for i, worker_id in enumerate(worker_ids):
            fig.add_trace(go.Box(y=execution_times[i], name=worker_id, boxmean=True), row=2, col=1)
        ...
        # Update layout
        fig.update_layout(
            title="Worker Performance Analysis", height=800, width=1200, showlegend=True
        )
    ...
    def generate_task_performance_visualization(self):
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "box"}, {"type": "pie"}], [{"type": "scatter"}, {"type": "pie"}]],
        )
        ...
        for task_type in task_types:
            fig.add_trace(
                go.Box(y=execution_times[task_type], name=task_type, boxmean=True), row=1, col=1
            )
        ...
        # Update layout
        fig.update_layout(
            title="Task Performance Analysis", height=800, width=1200, showlegend=True
        )
    ...
    def generate_dashboard_index(self):
        ...
        visualizations = [
            (vis_id, info["title"]) for vis_id, info in self.registered_visualizations.items()
        ]
    ...
    <div class="dashboard-grid">
        <div class="sidebar">
            ...
            <ul class="nav-list">
                ...
                {
            chr(10).join(
                [f'<li><a href="/pages/{page_id}">{title}</a></li>' for page_id, title in pages]
            )
        }
            ...
            chr(10).join(
                [
                    f'<li><a href="/visualizations/{vis_id}">{title}</a></li>'
                    for vis_id, title in visualizations
                ]
            )
        }
            </ul>
...
class PageHandler(tornado.web.RequestHandler):
    ...
    def get(self, page_id):
        ...
        if page_id in self.dashboard.registered_pages:
            ...
            self.render("page.html", title=page_info["title"], page_id=page_id, page_data=page_data)
        else:
            # Page not found
            self.set_status(404)
...
def run_dashboard(coordinator=None, port=8888, coordinator_url=None, db_path=None):
    ...
    dashboard = ComprehensiveMonitoringDashboard(
        coordinator=coordinator, port=port, coordinator_url=coordinator_url, db_path=db_path
    )
...
if __name__ == "__main__":
    ...
    dashboard = run_dashboard(
        port=args.port, coordinator_url=args.coordinator_url, db_path=args.db_path
    )
