...
class CoordinatorDuckDBIntegration:
        ...
        try:
            ...
            app.layout = html.Div(
                [
                    ...
                    html.Div([html.H2("Recent Test Results"), html.Table(id="results-table")]),
                ]
            )
            ...
            def update_dashboard(n):
                ...
                if summary_data:
                    ...
                    df_summary["success_rate"] = (
                        df_summary["success_count"] / df_summary["test_count"] * 100
                    )
            ...
            def update_hardware_comparison(selected_model):
                ...
                if results:
                    df = pd.DataFrame(
                        results, columns=["hardware_type", "avg_time", "avg_memory", "test_count"]
                    )
    ...
    def _flush_cache(self):
        ...
        with self.cache_lock:
            ...
            if success and not failed:
                ...
                logger.warning(
                    f"Partially flushed cache: {len(results) - len(failed)} succeeded, {len(failed)} failed"
                )
                return False
            else:
    ...
    def generate_report(self, format="markdown", output_file=None, filters=None):
        ...
        if format == "markdown":
            return self._generate_markdown_report(
                results, summary, model_hardware_summary, output_file
            )
        elif format == "html":
    ...
    def _generate_markdown_report(self, results, summary, model_hardware_summary, output_file):
        ...
        report.append(
            f"- Success rate: {summary[1] / summary[0] * 100:.2f}% ({summary[1]} / {summary[0]})"
        )
        ...
        report.append("")
        report.append(
            "| Model | Hardware | Tests | Success Rate | Avg Time (ms) | Avg Memory (MB) | Avg Power (W) |"
        )
        report.append(
            "|-------|----------|-------|--------------|---------------|-----------------|---------------|"
        )
        ...
        for row in model_hardware_summary:
            ...
            report.append(
                f"| {model_name} | {hardware_type} | {test_count} | {success_rate:.2f}% | {avg_time:.2f} | {avg_memory:.2f} | {avg_power:.2f} |"
            )
        ...
        report.append("")
        report.append(
            "| Test ID | Model | Hardware | Worker | Success | Time (ms) | Memory (MB) | Timestamp |"
        )
        report.append(
            "|---------|-------|----------|--------|---------|-----------|-------------|-----------|"
        )
        ...
        for row in results[:20]:  # Show only first 20 for brevity
            ...
            report.append(
                f"| {test_id} | {model_name} | {hardware_type} | {worker_id} | {success} | {execution_time:.2f} | {memory_usage:.2f} | {timestamp} |"
            )
    ...
    def _generate_html_report(self, results, summary, model_hardware_summary, output_file):
        ...
        try:
            ...
            fig3 = px.bar(
                ...
                labels={
                    "avg_memory_usage": "Avg Memory (MB)",
                    "model_name": "Model",
                    "hardware_type": "Hardware",
                },
                barmode="group",
            ...
            html_parts.append(
                "        .summary-item { background-color: #f5f5f5; padding: 10px; border-radius: 5px; }"
            )
            ...
            html_parts.append(
                f"        <div class='summary-item'><strong>Total Tests:</strong> {summary[0]}</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Success Rate:</strong> {summary[1] / summary[0] * 100:.2f}% ({summary[1]} / {summary[0]})</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Avg Execution Time:</strong> {summary[2]:.2f} ms</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Avg Memory Usage:</strong> {summary[3]:.2f} MB</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Unique Models:</strong> {summary[4]}</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Unique Hardware:</strong> {summary[5]}</div>"
            )
            html_parts.append(
                f"        <div class='summary-item'><strong>Unique Workers:</strong> {summary[6]}</div>"
            )
            ...
            html_parts.append(
                f"        <li><strong>Success Rate:</strong> {summary[1] / summary[0] * 100:.2f}% ({summary[1]} / {summary[0]})</li>"
            )
            html_parts.append(
                f"        <li><strong>Avg Execution Time:</strong> {summary[2]:.2f} ms</li>"
            )
            html_parts.append(
                f"        <li><strong>Avg Memory Usage:</strong> {summary[3]:.2f} MB</li>"
            )
            ...
            html_parts.append("    <p>Report generated by Distributed Testing Framework</p>")
            html_parts.append(
                "    <p><em>Note: Install pandas and plotly for enhanced visualizations</em></p>"
            )
            html_parts.append("</body>")
            html_parts.append("</html>")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch-size", type=int, default=20, help="Batch size for result insertion"
    )
    ...
    parser.add_argument(
        "--vacuum-interval", type=int, default=24, help="Vacuum interval in hours (0 to disable)"
    )
    ...
    if args.generate_report:
        ...
        report = integration.generate_report(
            format=args.report_format, output_file=args.report_output, filters=filters
        )
