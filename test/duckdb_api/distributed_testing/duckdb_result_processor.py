...
class DuckDBResultProcessor:
        ...
        try:
            ...
            """,
                [
                    ...
                    db_record["hardware_type"],
                    db_record["execution_time"],
                    db_record["success"],
                    db_record["error_message"],
                    db_record["timestamp"],
                    db_record["memory_usage"],
                    json.dumps(db_record["details"]),
                    db_record["power_consumption"],
                    db_record["test_type"],
                ],
            )
            conn.execute("COMMIT")
        ...
        try:
            ...
            for result in results:
                try:
                    ...
                    logger.warning(
                        f"Error processing result {result.get('test_id', 'unknown')}: {e}"
                    )
                    failed_results.append(result)

            conn.execute("COMMIT")
            logger.info(
                f"Stored {len(results) - len(failed_results)} of {len(results)} results in database"
            )
    ...
    def _validate_result(self, result):
        ...
        if "execution_time" in result and result["execution_time"] is not None:
            if (
                not isinstance(result["execution_time"], (int, float))
                or result["execution_time"] < 0
            ):
    ...
    def _ensure_schema_exists(self):
        ...
        try:
            ...
            if not table_exists:
                ...
                conn.execute(
                    "CREATE INDEX idx_test_results_hardware_type ON test_results(hardware_type)"
                )
    ...
    def get_result_by_id(self, test_id):
        ...
        """
        results = self.query_results("SELECT * FROM test_results WHERE test_id = ?", [test_id])
