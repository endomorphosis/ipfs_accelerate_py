...
class CoordinatorOrchestratorIntegration:
    ...
    def _register_api_endpoints(self):
        ...
        if not hasattr(self.coordinator, "api_handlers"):
            logger.warning(
                "Coordinator does not support API handlers, skipping endpoint registration"
            )
            return
    ...
    def _setup_result_handling(self):
        ...
        if hasattr(self.coordinator, "register_callback_handler"):
            ...
            logger.warning(
                "Coordinator does not support callback registration, subtask results will not be processed"
            )
    ...
    async def _handle_orchestrated_task_request(self, request_data):
        ...
        try:
            ...
            return {"success": False, "error": f"Error getting task status: {str(e)}"}
            ...
            tasks.append(
                {
                    ...
                    "creation_time": task_info.get("creation_time", "").isoformat()
                    if task_info.get("creation_time")
                    else None,
    ...
    async def _handle_cancel_orchestrated_task(self, request_data):
        ...
        try:
            ...
            return {"success": True, "message": f"Task canceled: {task_id}"}
        except Exception as e:
            logger.exception(f"Error canceling task: {e}")
            return {"success": False, "error": f"Error canceling task: {str(e)}"}
