...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler(f"stress_test_{int(time.time())}.log")],
)
...
class StressTest:
    ...
    def __init__(
        ...
        if scenario not in SCENARIOS:
            raise ValueError(
                f"Unknown scenario: {scenario}. Available scenarios: {', '.join(SCENARIOS.keys())}"
            )
    ...
    def start_coordinator(self) -> None:
        ...
        cmd = [
            ...
            "INFO",
            "--demo-tasks",
            "100",  # More tasks for stress test
        ]
        ...
        self.coordinator_process = subprocess.Popen(
            cmd, stdout=coordinator_log, stderr=subprocess.STDOUT, text=True
        )
        ...
        if self.coordinator_process.poll() is not None:
            raise RuntimeError(
                f"Coordinator failed to start, exit code: {self.coordinator_process.returncode}"
            )
    ...
    def start_workers_thundering_herd(self) -> None:
        ...
        for i in range(worker_count):
            ...
            worker_process = subprocess.Popen(
                cmd, stdout=worker_log, stderr=subprocess.STDOUT, text=True
            )
    ...
    def start_workers_steady_load(self) -> None:
        ...
        for i in range(worker_count):
            ...
            cmd = [
                ...
                "INFO",
                "--reconnect-delay",
                str(reconnect_interval / delay_factor),
                "--max-reconnect-delay",
                str(reconnect_interval * 2),
            ]
    ...
    def start_workers_message_flood(self) -> None:
        ...
        for i in range(worker_count):
            ...
            cmd = [
                ...
                "INFO",
                "--compression-level",
                "1",  # Fast compression for high throughput
    ...
    def start_workers_checkpoint_heavy(self) -> None:
        ...
        for i in range(worker_count):
            ...
            cmd = [
                ...
                "INFO",
                "--heartbeat-interval",
                "2",  # More frequent heartbeat to create more messages
            ]
    ...
    def start_workers(self) -> None:
        ...
        if self.scenario in scenario_map:
            ...
            logger.warning(
                f"No specific worker starter for scenario {self.scenario}, using thundering herd"
            )
    ...
    def run_scenario_disruptions(self) -> None:
        ...
        if self.scenario == "thundering_herd":
            ...
            disruption_count = max(
                1, self.config["test_duration"] // 120
            )  # At least once, then every 2 minutes
                ...
                for process in workers_to_disrupt:
                    if process.poll() is None:  # Check if process is still running
                        try:
                            ...
                            threading.Thread(
                                target=resume_later, args=(process.pid, delay), daemon=True
                            ).start()
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        total_tasks_completed = sum(
            w.get("tasks_completed", 0) for w in metrics["workers"].values()
        )
        ...
        total_messages_received = sum(
            w.get("messages_received", 0) for w in metrics["workers"].values()
        )
        total_checkpoints_created = sum(
            w.get("checkpoints_created", 0) for w in metrics["workers"].values()
        )
        total_checkpoints_restored = sum(
            w.get("checkpoints_restored", 0) for w in metrics["workers"].values()
        )
        ...
        metrics["aggregate"] = {
            ...
            else total_tasks_completed / (total_tasks_completed + total_tasks_failed),
            "reconnections_per_worker": total_reconnections / self.config["worker_count"]
    ...
    def generate_report(self, metrics: Dict[str, Any]) -> str:
        ...
        if self.scenario == "thundering_herd":
            ...
            success_message = (
                f"Message queue handled high volume with {agg['total_messages_sent']} messages sent"
            )
        elif self.scenario == "checkpoint_heavy":
            success_message = (
                f"Checkpoint system handled {agg['total_checkpoints_created']} checkpoints"
            )
        else:
        ...
        if self.scenario == "thundering_herd":
            ...
            report.append(
                f"  Reconnection storm handling: {'Good' if agg['task_success_rate'] >= 0.95 else 'Needs improvement'}"
            )
            ...
            for w_metrics in metrics["workers"].values():
                if (
                    "connection_stats" in w_metrics
                    and "avg_reconnect_time" in w_metrics["connection_stats"]
                ):
            ...
            for w_metrics in metrics["workers"].values():
                if (
                    "performance_metrics" in w_metrics
                    and "compression_ratio" in w_metrics["performance_metrics"]
                ):
        ...
        if self.scenario == "thundering_herd" and agg["reconnections_per_worker"] > 2:
            report.append(
                f"  • Optimize exponential backoff strategy to reduce excessive reconnection attempts"
            )

        if (
            self.scenario == "message_flood"
            and "message_throughput" in locals()
            and message_throughput < self.config.get("message_rate", 100) * 0.75
        ):
    ...
    def cleanup(self) -> None:
        ...
        for worker_process in self.worker_processes:
            if worker_process.poll() is None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Worker process {worker_process.pid} did not terminate, killing"
                    )
                    worker_process.kill()
        ...
        if self.coordinator_process and self.coordinator_process.poll() is None:
            ...
            try:
                ...
                logger.warning(
                    f"Coordinator process {self.coordinator_process.pid} did not terminate, killing"
                )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run stress tests for Enhanced Worker Reconnection System"
    )

    parser.add_argument(
        "--scenario",
        type=str,
        default="thundering_herd",
    ...
    parser.add_argument(
        "--port", type=int, default=8765, help="Port for the coordinator server (default: 8765)"
    )
