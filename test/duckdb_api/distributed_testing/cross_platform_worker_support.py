...
class LinuxPlatformHandler(PlatformHandler):
    ...
    def get_worker_command(self, config):
        ...
        cmd = [
            ...
            "--coordinator",
            config.get("coordinator_url", "http://localhost:8080"),
            "--api-key",
            config.get("api_key", "default_key"),
        ]
...
""".format(
    ...
    def _detect_linux_cpu(self):
        ...
        """
        cpu_info = {"cores": os.cpu_count(), "model": "Unknown"}
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    for i, name in enumerate(gpu_names):
                        gpu_info["devices"].append({"id": i, "name": name, "type": "rocm"})
                    gpu_info["count"] = len(gpu_info["devices"])
            except Exception as e:
    ...
    def _detect_windows_cpu(self):
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    "wmic cpu get name", shell=True, capture_output=True, text=True
                )
                lines = result.stdout.strip().split("\n")
                if len(lines) >= 2:
        ...
        if gpu_info["count"] == 0:
            try:
                ...
                for gpu in c.Win32_VideoController():
                    gpu_info["devices"].append(
                        {"id": len(gpu_info["devices"]), "name": gpu.Name, "type": "generic"}
                    )
                gpu_info["count"] = len(gpu_info["devices"])
            except Exception as e:
...
"""
    ...
    def get_platform_info(self):
        ...
        info["edition"] = (
            platform.win32_edition() if hasattr(platform, "win32_edition") else "Unknown"
        )
...
"""
    ...
    def get_platform_info(self):
        ...
        try:
            result = subprocess.run(
                ["sw_vers"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False
            )
...
class ContainerPlatformHandler(PlatformHandler):
    ...
    def get_worker_command(self, config):
        ...
        cmd = [
            ...
            "run_worker_client.py",
            "--coordinator",
            config.get("coordinator_url", "http://coordinator:8080"),
            "--api-key",
            config.get("api_key", "default_key"),
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--coordinator-url", default="http://localhost:8080", help="Coordinator URL"
    )
