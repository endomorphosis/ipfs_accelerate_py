...
class ResourcePerformancePredictor:
    ...
    def __init__(self, database_path: Optional[str] = None):
        ...
        logger.info(
            f"Resource Performance Predictor initialized with {self.data_count} historical records"
        )
    ...
    def record_task_execution(self, task_id: str, execution_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Recorded execution data for task {task_id} ({model_type}: {model_name}, batch_size: {batch_size})"
            )
            return True
        ...
        try:
            ...
            for field in ["cpu_cores", "memory_mb", "gpu_memory_mb"]:
                if field not in prediction:
                    ...
                    if field == "cpu_cores":
                        prediction[field] = DEFAULT_CPU_CORES.get(
                            model_type, DEFAULT_CPU_CORES["default"]
                        )
                    elif field == "memory_mb":
                        prediction[field] = DEFAULT_MEMORY_MB.get(
                            model_type, DEFAULT_MEMORY_MB["default"]
                        )
                    elif field == "gpu_memory_mb":
                        prediction[field] = DEFAULT_GPU_MEMORY_MB.get(
                            model_type, DEFAULT_GPU_MEMORY_MB["default"]
                        )
            ...
            logger.debug(
                f"Predicted resources for {model_type}:{model_name} (batch={batch_size}): "
            ...
            return {
                ...
                "gpu_memory_mb": DEFAULT_GPU_MEMORY_MB.get(
                    model_type, DEFAULT_GPU_MEMORY_MB["default"]
                ),
                "confidence": 0.5,
    ...
    def get_resource_scaling_factor(self, scaling_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            if batch_size_source == batch_size_target:
                return {"cpu_cores": 1.0, "memory_mb": 1.0, "gpu_memory_mb": 1.0, "confidence": 1.0}
            ...
            if self.data_count >= 10:
                scaling_factors = self._get_scaling_factors_from_data(
                    model_type, model_name, batch_size_source, batch_size_target
                )
                if scaling_factors:
            ...
            logger.debug(
                f"Calculated scaling factors for {model_type} from batch {batch_size_source} "
            ...
            batch_ratio = scaling_data.get("batch_size_target", 2) / max(
                1, scaling_data.get("batch_size", 1)
            )
            return {
                "cpu_cores": max(1.0, batch_ratio ** DEFAULT_BATCH_SCALING["cpu_cores"]),
                "memory_mb": max(1.0, batch_ratio ** DEFAULT_BATCH_SCALING["memory_mb"]),
    ...
    def train_models(self) -> bool:
        ...
        with self.training_lock:
            try:
                ...
                if len(data) < 10:
                    logger.warning(
                        f"Not enough successful executions to train models ({len(data)} available)"
                    )
                    return False
                ...
                for target in targets:
                    ...
                    if (data[target] > 0).sum() < 5:
                        logger.warning(
                            f"Not enough non-zero data for {target}. Skipping model training."
                        )
                        continue
        ...
        for target in ["cpu_cores", "memory_mb", "gpu_memory_mb"]:
            if target in self.models:
                try:
                    ...
                    predictions[target] = stats_prediction.get(
                        target,
                        DEFAULT_CPU_CORES.get(model_type, DEFAULT_CPU_CORES["default"])
                        if target == "cpu_cores"
                        else DEFAULT_MEMORY_MB.get(model_type, DEFAULT_MEMORY_MB["default"])
                        if target == "memory_mb"
                        else DEFAULT_GPU_MEMORY_MB.get(
                            model_type, DEFAULT_GPU_MEMORY_MB["default"]
                        ),
                    )
            else:
                ...
                predictions[target] = stats_prediction.get(
                    ...
                    if target == "memory_mb"
                    else DEFAULT_GPU_MEMORY_MB.get(model_type, DEFAULT_GPU_MEMORY_MB["default"]),
                )
                ...
                if model_match:
                    ...
                    return {
                        ...
                        "execution_time_ms": max(
                            1, int(row[3] * scaling["cpu_cores"])
                        ),  # Execution time scales with CPU
                        "confidence": 0.7,
    ...
    def _get_scaling_factors_from_data(
        self,
        model_type: str,
        ...
        try:
            ...
            if model_name:
                ...
                query = query.replace("a.model_type = ?", "a.model_type = ? AND a.model_name = ?")
            ...
            with self.conn:
                ...
                if row and row[3] >= 3 and row[0] is not None:
                    ...
                    return {
                        ...
                        "memory_mb": max(
                            1.0,
                            row[1] if row[1] is not None else batch_size_target / batch_size_source,
                        ),
                        "gpu_memory_mb": max(
                            1.0,
                            row[2] if row[2] is not None else batch_size_target / batch_size_source,
                        ),
                        "confidence": min(
                            0.9, 0.6 + (row[3] / 20)
                        ),  # Higher confidence with more data pairs
                    }
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    or current_count >= self.data_count + 10
                ):  # At least 10 new data points
...
if __name__ == "__main__":
    ...
    prediction = predictor.predict_resource_requirements(
        {"model_type": "text_embedding", "model_name": "bert-base-uncased", "batch_size": 64}
    )
    ...
    # Get scaling factor
    scaling = predictor.get_resource_scaling_factor(
        {"model_type": "text_embedding", "batch_size": 32, "batch_size_target": 64}
    )
