...
class TestGeneratorIntegration:
    ...
    def _ensure_schema_exists(self):
        ...
        try:
            ...
            if not table_exists:
                ...
                self.template_db.execute(
                    "CREATE INDEX idx_templates_model_family ON templates(model_family)"
                )
                self.template_db.execute(
                    "CREATE INDEX idx_templates_hardware_type ON templates(hardware_type)"
                )
            ...
            if not mapping_exists:
                ...
                self.template_db.execute(
                    "CREATE INDEX idx_model_mapping_model_name ON model_mapping(model_name)"
                )
    ...
    def generate_and_submit_tests(self, model_name, hardware_types=None, batch_sizes=None):
        ...
        if self.coordinator_client:
            ...
            logger.info(
                f"Generated {len(tests_with_dependencies)} tests (coordinator not available, not submitted)"
            )
    ...
    def _infer_model_family(self, model_name):
        ...
        # Vision models
        if (
            "vit" in model_name_lower
            or "resnet" in model_name_lower
            or "efficientnet" in model_name_lower
        ):
            return "vision"
    ...
    def add_template(
        self, template_name, model_family, content, hardware_type=None, description=None
    ):
        ...
        try:
            ...
            """,
                [template_name, model_family, created_at],
            ).fetchone()
    ...
    try:
        if args.add_template:
            if not args.template_name or not args.model_family or not args.template_file:
                logger.error(
                    "Template name, model family, and template file are required for adding a template"
                )
                return 1
            ...
            template_id = integration.add_template(
                args.template_name, args.model_family, content, args.hardware, args.description
            )
            ...
            success = integration.add_model_mapping(
                args.model, args.model_family, description=args.description
            )
            ...
            success, tests = integration.generate_and_submit_tests(
                args.model, hardware_types, batch_sizes
            )
            ...
            if success:
                ...
                for i, test in enumerate(tests):
                    print(
                        f"Test {i + 1}: {test['model_name']} on {test['hardware_type']} with batch size {test['batch_size']}"
                    )
            else:
            ...
            if templates:
                ...
                for template in templates:
                    print(
                        f"ID: {template[0]}, Name: {template[1]}, Family: {template[2]}, Hardware: {template[3]}"
                    )
            else:
                print("No templates found")
        else:
            print(
                "No action specified. Use --add-template, --add-mapping, --generate, or --list-templates"
            )
