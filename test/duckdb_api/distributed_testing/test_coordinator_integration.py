...
def test_basic_coordinator_integration():
    ...
    try:
        ...
        with load_balancer.lock:
            ...
            all_tests = set(load_balancer.pending_tests.keys()) | set(
                load_balancer.test_assignments.keys()
            )
            ...
            with bridge.lock:
                submitted_test_ids = [
                    bridge.coordinator_to_lb_test_map.get(test_id)
        ...
        coordinator.update_worker_load(
            "coord-worker1",
            {
        ...
        with load_balancer.lock:
            ...
            if worker_load:
                print(
                    f"Worker load: CPU={worker_load.cpu_utilization}%, Memory={worker_load.memory_utilization}%"
                )
        ...
        with coordinator.lock:
            for test_id, test_data in coordinator.tests.items():
                print(
                    f"Test {test_id}: status={test_data['status']}, worker={test_data['worker_id']}"
                )
...
def test_bidirectional_sync():
    ...
    try:
        ...
        with load_balancer.lock:
            ...
            assert "coord-worker3" in load_balancer.workers, (
                "Coordinator worker not registered in load balancer"
            )
        ...
        with load_balancer.lock:
            ...
            if coordinator_test_id in coordinator_to_lb:
                ...
                assert lb_test_id in all_tests, (
                    f"Coordinator test {coordinator_test_id} not in load balancer"
                )
            ...
            if "coord-test3" in bridge.coordinator_to_lb_test_map:
                ...
                for attempt in range(5):  # Try up to 5 times with 2-second intervals
                    ...
                    with coordinator.lock:
                        ...
                        if test_data:
                            print(
                                f"Attempt {attempt + 1}: Status in coordinator: {test_data['status']}"
                            )
