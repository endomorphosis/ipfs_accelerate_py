...
class TaskScheduler:
    ...
    def add_task_batch(self, tasks: List[Dict[str, Any]]) -> List[str]:
        ...
        with self.task_lock:
            for task_config in tasks:
                ...
                priority = task_config.get(
                    "priority", self.config["type_priorities"].get(task_type, 5)
                )
                ...
                task_ids.append(
                    self.add_task(task_id, task_type, priority, config, requirements, dependencies)
                )
    ...
    def get_next_task(
        self, worker_id: str, worker_capabilities: Dict[str, Any]
        ...
        with self.task_lock:
            ...
            worker_running_tasks = sum(
                1 for w_id in self.running_tasks.values() if w_id == worker_id
            )
            ...
            if best_task_index is not None and best_score > 0:
                ...
                if self.db_manager:
                    self.db_manager.update_task_status(
                        best_task["task_id"], TASK_STATUS_ASSIGNED, worker_id
                    )

                logger.info(
                    f"Assigned task {best_task['task_id']} to worker {worker_id} (score: {best_score:.2f})"
                )
                return best_task

            return None

    def _calculate_worker_task_match(
        self, worker_id: str, worker_capabilities: Dict[str, Any], task: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_base_suitability(
        self, worker_capabilities: Dict[str, Any], task: Dict[str, Any]
    ) -> float:
        ...
        if self.match_algorithm == MATCH_ALGORITHM_EXACT:
            ...
            return (
                1.0 if self._worker_meets_requirements(worker_capabilities, requirements) else 0.0
            )
    ...
    def _worker_meets_requirements(
        self, worker_capabilities: Dict[str, Any], task_requirements: Dict[str, Any]
    ) -> bool:
    ...
    def complete_task(
        ...
        with self.task_lock:
            ...
            if self.running_tasks[task_id] != worker_id:
                logger.warning(
                    f"Task {task_id} is assigned to {self.running_tasks[task_id]}, not {worker_id}"
                )
                return False
                            ...
                            if task:
                                with self.task_lock:
                                    ...
                                    self.task_queue.append(
                                        (priority, create_time, dep_task_id, task)
                                    )
                                    self.task_queue.sort()

                                logger.info(
                                    f"Task {dep_task_id} added to queue after dependencies satisfied"
                                )

            # Remove completed task from reverse dependencies
            del self.reverse_dependencies[completed_task_id]

    def fail_task(self, task_id: str, worker_id: str, error: str, metadata: Dict[str, Any]) -> bool:
    ...
    def _update_performance_stats(
        self, worker_id: str, task_id: str, success: bool, metadata: Dict[str, Any]
    ):
        ...
        if type_perf["task_count"] > 0:
            type_perf["avg_execution_time"] = (
                type_perf["total_execution_time"] / type_perf["task_count"]
            )
        ...
        if global_stats["task_count"] > 0:
            global_stats["success_rate"] = (
                global_stats["success_count"] / global_stats["task_count"]
            )
            global_stats["avg_execution_time"] = (
                global_stats["total_execution_time"] / global_stats["task_count"]
            )
                ...
                if task and "start_time" in task:
                    ...
                    if (current_time - start_time).total_seconds() > timeout_seconds:
                        ...
                        if task["attempts"] < max_retries:
                            ...
                            logger.info(
                                f"Requeued timed out task {task_id} (attempt {task['attempts']})"
                            )
                            ...
                            logger.warning(
                                f"Task {task_id} failed after timing out {max_retries} times"
                            )
                            if self.db_manager:
                                self.db_manager.update_task_status(task_id, TASK_STATUS_FAILED)
