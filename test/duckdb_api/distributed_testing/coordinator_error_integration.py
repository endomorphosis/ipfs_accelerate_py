...
def integrate_error_handler(coordinator):
    ...
    def enhanced_handle_worker_error(worker_id, error):
        ...
        worker_error = {
            ...
            "hardware_context": {"hardware_type": context["hardware_type"]},
        }
    ...
    coordinator.get_worker_hardware_type = lambda worker_id: (
        coordinator.workers.get(worker_id, {})
        .get("capabilities", {})
        .get("hardware_types", ["unknown"])[0]
...
def reschedule_task(coordinator, task_id, delay_seconds=60):
    ...
    logger.info(
        f"Task {task_id} rescheduled with attempt {task['attempt_count']} "
...
def request_resource_cleanup(coordinator, worker_id):
    ...
    try:
        coordinator.send_message_to_worker(
            worker_id,
            {
                "type": "command",
...
def increase_timeout(coordinator, task_id):
    ...
    try:
        ...
        coordinator.tasks[task_id]["last_updated"] = coordinator.get_current_time()
        logger.info(
            f"Timeout for task {task_id} increased from {current_timeout} to {new_timeout} seconds"
        )
        return True
    except Exception as e:
...
def mark_hardware_unavailable(coordinator, worker_id, hardware_type):
    ...
    try:
        ...
        logger.error(
            f"Failed to mark hardware {hardware_type} unavailable for worker {worker_id}: {e}"
        )
        return False
...
def reallocate_to_alternative_hardware(coordinator, task_id):
    ...
    try:
        ...
        if current_hardware and alternatives:
            ...
            coordinator.tasks[task_id]["requirements"] = {**requirements, "hardware": all_hardware}
            coordinator.tasks[task_id]["last_updated"] = coordinator.get_current_time()
...
def update_hardware_requirements(coordinator, task_id):
    ...
    try:
        ...
        if "min_cuda_compute" in requirements:
            ...
            logger.info(
                f"Reduced min_cuda_compute for task {task_id} from {current_min} to {new_min}"
            )
...
def mark_worker_unavailable(coordinator, worker_id):
    ...
    try:
        ...
        logger.info(
            f"Worker {worker_id} marked as unavailable, {len(active_tasks)} tasks reassigned"
        )
        return True
    except Exception as e:
...
def mark_worker_slow(coordinator, worker_id):
    ...
    try:
        ...
        logger.info(
            f"Worker {worker_id} marked as slow with priority {coordinator.workers[worker_id]['priority']}"
        )
        return True
    except Exception as e:
...
def reassign_task_with_increased_timeout(coordinator, task_id):
    ...
    try:
        ...
        logger.info(
            f"Task {task_id} marked for reassignment with timeout increased to {new_timeout} seconds"
        )
        return True
    except Exception as e:
...
def reassign_task_to_different_worker(coordinator, task_id, current_worker_id):
    ...
    try:
        coordinator.tasks[task_id]["status"] = "pending"
        coordinator.tasks[task_id]["needs_reassignment"] = True
        coordinator.tasks[task_id]["exclude_workers"] = coordinator.tasks[task_id].get(
            "exclude_workers", []
        )
        ...
        if (
            current_worker_id
            and current_worker_id not in coordinator.tasks[task_id]["exclude_workers"]
        ):
        ...
        logger.info(
            f"Task {task_id} marked for reassignment to a different worker (excluding {current_worker_id})"
        )
        return True
    except Exception as e:
