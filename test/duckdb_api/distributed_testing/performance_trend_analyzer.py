...
class PerformanceTrendAnalyzer:
    ...
    def _load_historical_data(self):
        ...
        if not self.db_manager or not self.config["database_enabled"]:
            logger.info(
                "No database manager available or database disabled, skipping historical data loading"
            )
            return
    ...
    def _update_time_series(self):
        ...
        for worker_id, perf in worker_performance.items():
            ...
            metrics = {
                ...
                "avg_execution_time": perf.get("total_execution_time", 0)
                / max(1, perf.get("task_count", 1)),
            ...
            try:
                ...
                for worker_id, metrics in worker_performance.items():
                    ...
                    stored_metrics = {
                        ...
                        / max(1, metrics.get("task_count", 1)),
                    ...
                    if "gpu_memory_available_mb" in metrics:
                        stored_metrics["gpu_memory_available_mb"] = metrics.get(
    ...
    def _prune_time_series(self):
        ...
        for worker_id in list(self.time_series.keys()):
            for metric in list(self.time_series[worker_id].keys()):
                ...
                self.time_series[worker_id][metric] = [
                    (timestamp, value)
                    for timestamp, value in self.time_series[worker_id][metric]
        ...
        for task_type in list(self.task_time_series.keys()):
            for metric in list(self.task_time_series[task_type].keys()):
                ...
                self.task_time_series[task_type][metric] = [
                    (timestamp, value)
                    for timestamp, value in self.task_time_series[task_type][metric]
                    if timestamp >= cutoff_time
                ]
        ...
        for worker_id, metrics in self.time_series.items():
            ...
            for metric, data_points in metrics.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"Error computing baseline for worker {worker_id}, metric {metric}: {e}"
                    )
        ...
        for task_type, metrics in self.task_time_series.items():
            ...
            for metric, data_points in metrics.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"Error computing baseline for task type {task_type}, metric {metric}: {e}"
                    )
                ...
                for timestamp, value in recent_points:
                    ...
                    if abs(z_score) > threshold:
                        ...
                        for existing in self.anomalies[worker_id]:
                            ...
                            if existing["timestamp"] == timestamp and existing["metric"] == metric:
                                is_new = False
                                break
    ...
    def _analyze_trends(self):
        ...
        for worker_id, metrics in self.time_series.items():
            ...
            for metric, data_points in metrics.items():
                ...
                timestamps = np.array(
                    [(ts - datetime(1970, 1, 1)).total_seconds() for ts, _ in data_points]
                )
                ...
                try:
                        ...
                        if self.db_manager and self.config["database_enabled"]:
                            try:
                                self.db_manager.add_performance_trend(
                                    ...
                                    < self.config["trend_significance_threshold"],
                                    direction=direction,
                    ...
                    logger.warning(
                        f"Error analyzing trend for worker {worker_id}, metric {metric}: {e}"
                    )
        ...
        for task_type, metrics in self.task_time_series.items():
            ...
            for metric, data_points in metrics.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"Error analyzing trend for task type {task_type}, metric {metric}: {e}"
                    )
    ...
    def get_worker_trends(
        self, worker_id: str = None, metric: str = None, significant_only: bool = False
    ) -> Dict[str, Any]:
        ...
        if worker_id:
            ...
            if metric:
                ...
                if significant_only:
                    return {
                        m: t
                        for m, t in self.trends[worker_id].items()
            ...
            if significant_only:
                ...
                for w_id, metrics in self.trends.items():
                    result[w_id] = {
                        m: t for m, t in metrics.items() if t.get("is_significant", False)
                    }
    ...
    def get_task_trends(
        self, task_type: str = None, metric: str = None, significant_only: bool = False
    ) -> Dict[str, Any]:
        ...
        if task_type:
            ...
            if metric:
                ...
                if significant_only:
                    return {
                        m: t
                        for m, t in self.task_trends[task_type].items()
                        if t.get("is_significant", False)
                    }
                else:
    ...
    def get_worker_anomalies(
        self, worker_id: str = None, metric: str = None, limit: int = 10
        ...
        if worker_id:
            ...
            if metric:
                ...
                sorted_filtered = sorted(filtered, key=lambda a: a["timestamp"], reverse=True)[
                    :limit
                ]
                ...
                for m in result:
                    result[m] = sorted(result[m], key=lambda a: a["timestamp"], reverse=True)[
                        :limit
                    ]
                ...
                if metric:
                    ...
                    for m in worker_result:
                        worker_result[m] = sorted(
                            worker_result[m], key=lambda a: a["timestamp"], reverse=True
                        )[:limit]
    ...
    def get_task_anomalies(
        self, task_type: str = None, metric: str = None, limit: int = 10
    ) -> Dict[str, List[Dict[str, Any]]]:
                ...
                if metric:
                    ...
                    for m in task_result:
                        task_result[m] = sorted(
                            task_result[m], key=lambda a: a["timestamp"], reverse=True
                        )[:limit]
    ...
    def get_time_series(
        self, entity_id: str, entity_type: str = "worker", metric: str = None, days: int = None
    ...
    def get_performance_report(
        self, entity_type: str = "worker", significant_only: bool = True
    ) -> Dict[str, Any]:
                ...
                if worker_anomalies:
                    ...
                    for metric in grouped:
                        grouped[metric] = sorted(
                            grouped[metric], key=lambda a: a["timestamp"], reverse=True
                        )
    ...
    def generate_visualization(
        self, entity_id: str, entity_type: str = "worker", metric: str = None, days: int = 30
    ) -> Optional[str]:
