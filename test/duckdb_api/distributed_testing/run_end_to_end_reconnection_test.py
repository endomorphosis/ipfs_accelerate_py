...
try:
    ...
    print(
        "Failed to import required modules. Make sure you're running this script from the correct directory."
    )
    sys.exit(1)
...
class EndToEndTest:
    ...
    def start_workers(self) -> None:
        ...
        for i in range(self.worker_count):
            ...
            if i % 3 == 0:
                ...
                cmd.extend(["--api-key", f"test-key-{i}", "--compression-level", "6"])
            elif i % 3 == 1:
                # Worker with adaptive parameters
                cmd.extend(["--reconnect-delay", "1.0", "--max-reconnect-delay", "30.0"])
            else:
    ...
    def start_network_disruptor(self) -> None:
        ...
        self.network_disruptor = NetworkDisruptor(
            processes=self.worker_processes, disruption_interval=self.disruption_interval
        )
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        metrics["aggregate"] = {
            ...
            "reconnections_per_worker": total_reconnections / self.worker_count
    ...
    def generate_report(self, metrics: Dict[str, Any]) -> str:
        ...
        for worker_id, worker_metrics in metrics["workers"].items():
            ...
            if worker_metrics["connection_stats"]:
                ...
                report.append(
                    f"      Successful connections: {cs.get('successful_connections', 'N/A')}"
                )
                report.append(
                    f"      Average reconnect time: {cs.get('avg_reconnect_time', 'N/A')}s"
                )
            ...
            if worker_metrics["performance_metrics"]:
                ...
                report.append(
                    f"      Message latency (avg): {pm.get('avg_message_latency_ms', 'N/A')}ms"
                )
                report.append(
                    f"      Message size (avg): {pm.get('avg_message_size_bytes', 'N/A')} bytes"
                )
                report.append(f"      Compression ratio: {pm.get('compression_ratio', 'N/A')}")
                report.append(
                    f"      Task execution time (avg): {pm.get('avg_task_execution_time_ms', 'N/A')}ms"
                )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run end-to-end tests for Enhanced Worker Reconnection System"
    )

    parser.add_argument(
        "--workers", type=int, default=5, help="Number of worker clients to spawn (default: 5)"
    )
