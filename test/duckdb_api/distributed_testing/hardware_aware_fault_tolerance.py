...
class HardwareAwareFaultToleranceManager:
    ...
    def __init__(self, db_manager=None, scheduler=None, coordinator=None, enable_ml=False):
        ...
        if self.enable_ml:
            try:
                ...
                ml_module = importlib.import_module(
                    "duckdb_api.distributed_testing.ml_pattern_detection"
                )
    ...
    def handle_failure(
        self, task_id: str, worker_id: str, error_info: Dict[str, Any]
    ) -> RecoveryAction:
        """
        ...
        if retry_count >= self.config["max_retries"]:
            logger.warning(
                f"Task {task_id} exceeded maximum retry count ({self.config['max_retries']})"
            )
            return RecoveryAction(
                strategy=RecoveryStrategy.ESCALATION,
        ...
        logger.info(
            f"Task {task_id} failed on worker {worker_id}, recovery strategy: {recovery_action.strategy.name}"
        )
    ...
    def _check_pattern_group(
        self, grouped_failures: Dict[str, List[FailureContext]], group_type: str
    ):
        """
        ...
        for key, failures in grouped_failures.items():
            if len(failures) >= threshold:
                ...
                self.failure_patterns[pattern_id] = {
                    ...
                    "recommended_action": self._recommend_action_for_pattern(
                        group_type, key, failures
                    ),
                }
    ...
    def _recommend_action_for_pattern(
        self, pattern_type: str, pattern_key: str, failures: List[FailureContext]
    ) -> str:
        """
    ...
    def _check_ml_patterns(self, failure_context: FailureContext) -> Optional[RecoveryAction]:
        ...
        try:
            ...
            if recommended_strategy:
                logger.info(
                    f"ML pattern detection recommended strategy: {recommended_strategy.name}"
                )
                return RecoveryAction(
    ...
    def _get_pattern_recovery_action(
        self, pattern: Dict[str, Any], failure_context: FailureContext
    ) -> RecoveryAction:
        """
    ...
    def _determine_recovery_strategy(self, failure_context: FailureContext) -> RecoveryAction:
        ...
        if strategy == RecoveryStrategy.IMMEDIATE_RETRY:
            ...
            return RecoveryAction(
                strategy=strategy, message="Retrying on different worker with similar capabilities"
            )
            ...
            return RecoveryAction(strategy=strategy, message="Restarting browser instance")

        # Default to escalation if no strategy matched
        return RecoveryAction(
            strategy=RecoveryStrategy.ESCALATION, message="No suitable recovery strategy found"
        )
        ...
        for task_id in running_tasks:
            ...
            if last_checkpoint:
                ...
                checkpoint_time = datetime.fromisoformat(
                    last_checkpoint.get("timestamp", "2000-01-01T00:00:00")
                )
            ...
            try:
                ...
                if task_data:
                    ...
                    self.create_checkpoint(
                        task_id, {"task_data": task_data, "state": self.get_task_state(task_id)}
                    )
            except Exception as e:
                ...
                if "recovery_history" in state:
                    with self.recovery_history_lock:
                        for task_id, actions in state["recovery_history"].items():
                            self.recovery_history[task_id] = [
                                self._dict_to_recovery_action(action) for action in actions
                            ]
...
def create_recovery_manager(
    coordinator, db_manager=None, scheduler=None, enable_ml=False
    ...
    manager = HardwareAwareFaultToleranceManager(
        db_manager=db_manager, scheduler=scheduler, enable_ml=enable_ml, coordinator=coordinator
    )
...
def visualize_fault_tolerance(manager, output_dir="./visualizations"):
    ...
    try:
        ...
        visualization = importlib.import_module(
            "duckdb_api.distributed_testing.fault_tolerance_visualization"
        )

        # Create visualizer and generate report
        report_path = visualization.visualize_fault_tolerance_system(
            fault_tolerance_manager=manager, output_dir=output_dir
        )
...
def apply_recovery_action(
    task_id: str, action: RecoveryAction, coordinator=None, scheduler=None
) -> bool:
    """
