...
class MockWorker:
    ...
    def execute_task(self, task):
        ...
        self.resources_over_time.append(
            {"timestamp": time.time(), "resources": self.resources.copy()}
        )
        ...
        for i, task in enumerate(self.task_queue):
            ...
            if worker_resources:
                resources_check = self.dynamic_resource_manager.check_resource_availability(
                    worker_id=worker_id, resource_requirements=task.get("requirements", {})
                )
...
def run_demo(args):
    ...
    worker_result = coordinator.cloud_provider_manager.create_worker(
        provider="test-provider", worker_type="default"
    )
    ...
    def monitoring_thread():
        ...
        while coordinator.running:
            ...
            if (
                num_completed_tasks >= args.num_tasks
                and num_active_tasks == 0
                and num_queued_tasks == 0
            ):
