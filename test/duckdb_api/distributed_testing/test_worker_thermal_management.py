...
def test_thermal_state_transitions():
    ...
    # Initial state
    print(
        f"Initial state: warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
    ...
    print(
        f"After start_warming: warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
        f"performance={worker_load.performance_level:.2f}"
    )
    ...
    for i in range(3):
        ...
        print(
            f"During warming ({i + 1}s): warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
            f"performance={worker_load.performance_level:.2f}"
        )
    ...
    worker_load.update_thermal_state()
    print(
        f"After warming: warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
        f"performance={worker_load.performance_level:.2f}"
    )
    ...
    print(
        f"After start_cooling: warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
        f"performance={worker_load.performance_level:.2f}"
    )
    ...
    for i in range(3):
        ...
        worker_load.update_thermal_state()
        print(
            f"During cooling ({i + 1}s): warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
            f"performance={worker_load.performance_level:.2f}"
        )
    ...
    worker_load.update_thermal_state()
    print(
        f"After cooling: warming={worker_load.warming_state}, cooling={worker_load.cooling_state}, "
        f"performance={worker_load.performance_level:.2f}"
    )
...
def test_effective_load_calculation():
    ...
    worker_load = WorkerLoad(worker_id="test_worker", cpu_utilization=50.0, memory_utilization=40.0)
    ...
    print(
        f"Warming state: base_score={warming_base_score:.2f}, effective_score={warming_effective_score:.2f}"
    )
    ...
    print(
        f"Cooling state: base_score={cooling_base_score:.2f}, effective_score={cooling_effective_score:.2f}"
    )
...
def test_thermal_management_in_load_balancer():
    ...
    try:
        ...
        with load_balancer.lock:
            ...
            if load1:
                print(
                    f"Worker1: warming={load1.warming_state}, cooling={load1.cooling_state}, "
                    f"performance={load1.performance_level:.2f}"
                )

            if load2:
                print(
                    f"Worker2: warming={load2.warming_state}, cooling={load2.cooling_state}, "
        ...
        test_req = TestRequirements(
            test_id="test1", model_id="test-model", minimum_memory=2.0, expected_duration=5.0
        )
