...
class AWSCloudProvider(CloudProviderBase):
        ...
        try:
            ...
            if worker_config["use_spot"]:
                ...
                if worker_config["max_price"]:
                    spot_params["InstanceMarketOptions"]["SpotOptions"]["MaxPrice"] = str(
                        worker_config["max_price"]
                    )

                launch_params.update(spot_params)

                logger.info(
                    f"Launching spot instance {worker_config['worker_name']} with type {instance_type}"
                )
            else:
                logger.info(
                    f"Launching on-demand instance {worker_config['worker_name']} with type {instance_type}"
                )
                ...
                for reservation in response["Reservations"]:
                    for instance in reservation["Instances"]:
                        ...
                        if instance_id in self.workers:
                            ...
                            self.workers[instance_id]["private_ip"] = instance.get(
                                "PrivateIpAddress"
                            )
    ...
    def _select_instance_type(
        self, cpu_cores: int, memory_gb: int, gpu_type: Optional[str] = None, gpu_count: int = 0
    ) -> str:
        """
...
class GCPCloudProvider(CloudProviderBase):
    ...
    def __init__(self, region: str, **kwargs):
        ...
        logger.info(
            f"GCP cloud provider initialized for project {self.project_id}, region {self.region}, zone {self.zone}"
        )
    ...
    def create_worker(self, config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            machine_type = self._select_machine_type(
                cpu_cores=worker_config["cpu_cores"], memory_gb=worker_config["memory_gb"]
            )
            ...
            instance = {
                ...
                "labels": {"managed_by": "distributed_testing_framework"},
                "scheduling": {
            ...
            if worker_config.get("network") or worker_config.get("subnetwork"):
                ...
                if worker_config.get("network"):
                    network_interface["network"] = (
                        f"projects/{self.project_id}/global/networks/{worker_config['network']}"
                    )

                if worker_config.get("subnetwork"):
                    network_interface["subnetwork"] = (
                        f"projects/{self.project_id}/regions/{self.region}/subnetworks/{worker_config['subnetwork']}"
                    )
            ...
            if worker_config["gpu_count"] > 0 and worker_config["gpu_type"]:
                ...
                gcp_gpu_type = gpu_type_map.get(
                    worker_config["gpu_type"], worker_config["gpu_type"]
                )
            ...
            while not operation.status == compute_v1.Operation.Status.DONE:
                operation = self.instance_client.get_global_operation(
                    project=self.project_id, operation=operation.name
                )
                time.sleep(1)
    ...
    def terminate_worker(self, worker_id: str) -> bool:
        ...
        try:
            operation = self.instance_client.delete(
                project=self.project_id, zone=self.zone, instance=worker_id
            )
    ...
    def _get_machine_types(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # List machine types
            response = self.machine_types_client.list(project=self.project_id, zone=self.zone)
    ...
    def _get_accelerator_types(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.accelerator_types_client.list(project=self.project_id, zone=self.zone)
...
class DockerLocalProvider(CloudProviderBase):
    ...
    def __init__(self, region: str = "local", **kwargs):
        ...
        self.client = (
            docker.from_env() if not docker_host else docker.DockerClient(base_url=docker_host)
        )
    ...
    def list_workers(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            containers = self.client.containers.list(
                all=True, filters={"label": "managed_by=distributed_testing_framework"}
            )
...
class CloudProviderManager:
    ...
    def __init__(self, provider: str, region: str, **kwargs):
        ...
        if self.provider_type == "aws":
            ...
            if not GCP_AVAILABLE:
                raise ImportError(
                    "google.cloud.compute_v1 not available. GCP integration disabled."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        choices=["create", "status", "list", "terminate"],
        required=True,
