...
class EnhancedHardwareDetector:
            ...
            for browser_name, browser_info in self._browser_info.items():
                try:
                    if browser_info.get("available", False):
                        ...
                        if self._gpu_info:
                            gpu_profile = (
                                self._hardware_profiles[1]
                                if len(self._hardware_profiles) > 1
                                else None
                            )
    ...
    def _detect_cpu(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if platform.system() == "Linux":
                ...
                try:
                    feature_output = (
                        subprocess.check_output(["sysctl", "-n", "machdep.cpu.features"])
                        .decode()
                        .strip()
                    )
                        ...
                        gpu_info.append(
                            {
                                ...
                                * 1024,  # Convert from MB to bytes
                                ...
                                "vendor": "nvidia",
                                "has_tensor_cores": "RTX" in gpu.name
                                or "A100" in gpu.name
                                or "H100" in gpu.name,
                ...
                try:
                    ...
                    if os.path.exists(rocm_path):
                        rocm_output = subprocess.check_output(
                            [rocm_path, "--showproductname", "--showmeminfo"]
                        ).decode()
                        for line in rocm_output.split("\n"):
                            if "GPU" in line and ":" in line:
                                ...
                                rocm_gpu = {
                                    ...
                                    "has_ray_tracing": "RX 6000" in gpu_name
                                    or "RX 7000" in gpu_name,
                                    "memory_bandwidth_gbps": None,
                                    "tdp_w": None,
                                }
            ...
            if not gpu_info and platform.system() in ["Linux", "Windows"]:
                try:
                    nvidia_smi_output = subprocess.check_output(
                        [
                            "nvidia-smi",
                            "--query-gpu=name,memory.total,memory.free,clocks.sm",
                            "--format=csv,noheader",
                        ]
                    ).decode()
                    for line in nvidia_smi_output.split("\n"):
                        if line.strip():
                            ...
                            if len(parts) >= 3:
                                name = parts[0]
                                memory_total = (
                                    int(parts[1].split()[0]) * 1024 * 1024
                                )  # Convert from MiB to bytes
                                memory_free = (
                                    int(parts[2].split()[0]) * 1024 * 1024
                                )  # Convert from MiB to bytes
                                ...
                                has_tensor_cores = (
                                    ...
                                    or any(
                                        arch in name
                                        for arch in ["Volta", "Turing", "Ampere", "Ada", "Hopper"]
                                    )
                                )
    ...
    def _detect_browsers(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            # Get Chrome version
            version = driver.capabilities.get("browserVersion") or driver.capabilities.get(
                "version"
            )
            ...
            try:
                ...
                if version:
                    ...
                    if major_version >= 17:
                        ...
                        minor_version = (
                            int(version.split(".")[1]) if len(version.split(".")) > 1 else 0
                        )
                        if minor_version >= 4:
                            browser_info["safari"]["webgpu"] = True
    ...
    def _detect_specialized_hardware(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if platform.system() == "Linux" and (
            os.path.exists("/usr/lib/libhexagon.so")
            or os.path.exists("/vendor/lib/libhexagon_nn_skel.so")
            or os.path.exists("/system/lib/libhexagon_nn_skel.so")
        ):
            try:
                dsp_info = {
                    "type": "dsp",
    ...
    def find_optimal_hardware_for_workload(
        self, workload_type: str, min_effectiveness: float = 0.5
    ) -> Dict:
        """
    ...
    def get_performance_ranking(self, operation_type: str, precision: str) -> List[Dict]:
        ...
        precision_type = next(
            (p for p in PrecisionType if p.value == precision), PrecisionType.OTHER
        )
        ...
        serialized_profiles.append(
            {
                ...
                "supported_precisions": [
                    precision.value for precision in profile.supported_precisions
                ],
                "features": [feature.value for feature in profile.features],
                "memory_total_gb": profile.memory.total_bytes / (1024 * 1024 * 1024),
