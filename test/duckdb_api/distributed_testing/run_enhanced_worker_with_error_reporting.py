...
class EnhancedWorkerClientWithErrorReporting:
    ...
    def _generate_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                # Get CUDA details
                cuda_info = {"count": torch.cuda.device_count(), "devices": []}

                for i in range(torch.cuda.device_count()):
                    device_info = {"name": torch.cuda.get_device_name(i), "index": i}
        ...
        try:
            ...
            capabilities["browsers"] = {"selenium_available": True}
        except ImportError:
            capabilities["browsers"] = {"selenium_available": False}
    ...
    def start(self):
        ...
        threading.Thread(target=self._status_reporting_loop, daemon=True).start()
    ...
    def execute_task(self, task_id: str, task_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        thread = threading.Thread(
            target=self._task_thread, args=(task_id, task_config), daemon=True
        )
    ...
    def _task_thread(self, task_id: str, task_config: Dict[str, Any]):
        ...
        try:
            ...
            if checkpoint_data and "iteration" in checkpoint_data:
                ...
                logger.info(
                    f"Resuming task {task_id} from checkpoint (iteration {start_iteration})"
                )
            ...
            for i in range(start_iteration, iterations):
                ...
                if self.reconnection_plugin and (i + 1) % 2 == 0:
                    ...
                    logger.debug(
                        f"Created checkpoint {checkpoint_id} for task {task_id} (iteration {i + 1})"
                    )

                # Do simulated work
                logger.info(
                    f"Task {task_id} ({task_name}): iteration {i + 1}/{iterations} - {progress:.1f}%"
                )
            ...
            enhanced_error = self.error_reporter.create_enhanced_error_report(
                error_type="TaskExecutionError", message=str(e), task_id=task_id
            )
            ...
            if self.reconnection_plugin:
                ...
                # Log error details
                logger.error(
                    f"Enhanced error report for task {task_id}: "
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.reconnection_plugin:
                    ...
                    logger.info(
                        f"Average message size: {metrics.get('average_message_size', 0):.1f} bytes"
                    )
                    logger.info(
                        f"Average message latency: {metrics.get('average_message_latency', 0):.1f} ms"
                    )
                    logger.info(f"Message errors: {metrics.get('message_errors', 0)}")
                    logger.info(f"Reconnections: {metrics.get('reconnections', 0)}")
                    logger.info(
                        f"Average reconnection duration: {metrics.get('average_reconnection_duration', 0):.1f} s"
                    )
                    ...
                    logger.info(
                        f"Average task duration: {metrics.get('average_task_duration', 0):.1f} s"
                    )
                    ...
                    if self.error_reporter.error_history:
                        ...
                        if recent_errors:
                            ...
                            for error in recent_errors:
                                logger.info(
                                    f"  - {error.get('timestamp')}: {error.get('type')}: {error.get('message')[:50]}..."
                                )
...
def parse_args():
    ...
    parser.add_argument(
        "--worker-id", default="", help="Worker ID (leave empty for auto-generated ID)"
    )

    parser.add_argument("--coordinator-host", default="localhost", help="Coordinator hostname")

    parser.add_argument("--coordinator-port", type=int, default=8765, help="Coordinator port")

    parser.add_argument("--api-key", default="test-api-key", help="API key for authentication")

    parser.add_argument(
        "--heartbeat-interval", type=float, default=5.0, help="Heartbeat interval in seconds"
    )

    parser.add_argument(
        "--reconnect-delay", type=float, default=1.0, help="Initial reconnection delay in seconds"
    )
    ...
    parser.add_argument("--no-compression", action="store_true", help="Disable message compression")
    ...
    parser.add_argument(
        "--no-priority-queue", action="store_true", help="Disable priority-based message queue"
    )
    ...
    parser.add_argument(
        "--simulate-error", action="store_true", help="Simulate errors for testing error reporting"
    )

    parser.add_argument(
        "--error-type", default="TaskExecutionError", help="Type of error to simulate"
    )
...
def main():
    ...
    coordinator_url = (
        f"ws://{args.coordinator_host}:{args.coordinator_port}/api/v1/worker/{{worker_id}}/ws"
    )
    ...
    try:
        ...
        # Wait for shutdown
        logger.info(
            f"Enhanced worker {worker_id} with error reporting started, press Ctrl+C to stop"
        )
        while True:
            try:
                time.sleep(1)
