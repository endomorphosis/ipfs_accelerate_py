...
class CircuitState(Enum):
    ...
    HALF_OPEN = (
        auto()
    )  # Testing state, limited requests are allowed to check if the issue is resolved
...
class CircuitBreaker:
    ...
    def __init__(
        ...
        reset_timeout_factor: float = 2.0,
        ...
        with self.state_lock:
            if self.state == CircuitState.OPEN:
                ...
                time_since_last_failure = (
                    (current_time - self.last_failure_time).total_seconds()
            ...
            if self.state == CircuitState.HALF_OPEN:
                if self.half_open_calls >= self.half_open_max_calls:
                    ...
                    if fallback:
                        logger.debug(
                            f"Circuit '{self.name}' is HALF_OPEN with too many calls, using fallback"
                        )
                        return fallback()
                    else:
                        logger.warning(
                            f"Circuit '{self.name}' is HALF_OPEN with too many calls, no fallback provided"
                        )
                        raise CircuitOpenError(
                            f"Circuit '{self.name}' is half-open with too many calls"
                        )
    ...
    def _transition_to_open(self):
        ...
        self.current_reset_timeout = min(
            self.current_reset_timeout * self.reset_timeout_factor, self.max_reset_timeout
        )
...
class CircuitBreakerRegistry:
    ...
    def get_aggregate_metrics(self) -> Dict[str, Any]:
        ...
        with self.registry_lock:
            ...
            open_circuits = sum(
                1 for c in self.circuits.values() if c.get_state() == CircuitState.OPEN
            )
            half_open_circuits = sum(
                1 for c in self.circuits.values() if c.get_state() == CircuitState.HALF_OPEN
            )
            closed_circuits = sum(
                1 for c in self.circuits.values() if c.get_state() == CircuitState.CLOSED
            )
            ...
            return {
                ...
                "overall_health": self._calculate_overall_health(
                    open_circuits, half_open_circuits, total_circuits
                ),
            }

    def _calculate_overall_health(
        self, open_circuits: int, half_open_circuits: int, total_circuits: int
    ) -> float:
        """
        ...
        health_reduction = (
            ((open_circuits * open_weight) + (half_open_circuits * half_open_weight))
            / total_circuits
            * 100.0
        )
...
def circuit_breaker(
    circuit_name: str, failure_threshold: int = 3, recovery_timeout: float = 60.0, **kwargs
):
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            return circuit.execute(lambda: func(*args, **kwargs), fallback=lambda: None)
...
def create_worker_circuit_breaker(
    worker_id: str, failure_threshold: int = 3, recovery_timeout: float = 60.0
) -> CircuitBreaker:
    """
...
def create_endpoint_circuit_breaker(
    endpoint_name: str, failure_threshold: int = 3, recovery_timeout: float = 60.0
) -> CircuitBreaker:
    """
