...
try:
    ...
    logger.warning(
        "Circuit Breaker integration not available. Advanced fault tolerance features disabled."
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--demo-tasks", type=int, default=0, help="Number of demo tasks to submit (0 to disable)"
    )
...
async def run_coordinator_with_demo(
    host: str, port: int, num_demo_tasks: int, enable_circuit_breaker: bool = False
):
    """
    ...
    try:
        ...
        if enable_circuit_breaker and CIRCUIT_BREAKER_AVAILABLE:
            ...
            logger.warning(
                "Circuit breaker pattern requested but not available. Advanced fault tolerance features disabled."
            )
...
async def submit_demo_tasks_after_delay(
    server: CoordinatorWebSocketServer, num_tasks: int, delay: float = 5.0
):
    """
