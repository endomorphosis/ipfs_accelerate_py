...
class HealthMonitor:
    ...
    def _check_worker_health(self, worker_id: str, worker: Dict[str, Any]):
        ...
        self.health_history[worker_id].append(
            {"timestamp": datetime.now(), "status": new_status, "issues": issues.copy()}
        )
        ...
        if new_status != previous_status:
            if new_status == HEALTH_STATUS_HEALTHY:
                ...
                logger.log(
                    level,
                    f"Worker {worker_id} health status degraded to {new_status}: {', '.join(issues)}",
                )
    ...
    def _check_task_timeouts(self):
        ...
        for task_id, worker_id in running_tasks.items():
            ...
            if time_running > timeout_seconds:
                logger.warning(
                    f"Task {task_id} on worker {worker_id} has timed out after {time_running:.1f}s"
                )
            ...
            for task_type, type_perf in performance["task_types"].items():
                        ...
                        if stdev > 0:
                            ...
                            if outliers:
                                ...
                                for i in outliers:
                                    ...
                                    logger.warning(
                                        f"Performance anomaly detected for worker {worker_id}, "
                ...
                if "success_rates" in type_perf and len(type_perf["success_rates"]) >= 2:
                    ...
                    if previous_rate > 0 and current_rate < previous_rate * 0.8:
                        ...
                        logger.warning(
                            f"Success rate anomaly detected for worker {worker_id}, "
    ...
    def _process_auto_recovery(self):
        ...
        for worker_id, health_status in self.worker_health.items():
            ...
            if self.config["worker_quarantine_enabled"]:
                failed_attempts = sum(
                    1 for a in self.recovery_attempts[worker_id] if not a["success"]
                )
    ...
    def _recover_worker(self, worker_id: str, issues: List[str]) -> Dict[str, Any]:
        ...
        result = {"action": "none", "success": False, "details": "No recovery action taken"}
        ...
        if result["success"]:
            logger.info(
                f"Recovery of worker {worker_id} successful with action '{result['action']}'"
            )
        else:
            logger.warning(
                f"Recovery of worker {worker_id} failed with action '{result['action']}': {result['details']}"
            )
    ...
    def _quarantine_worker(self, worker_id: str):
        ...
        if self.db_manager and hasattr(self.db_manager, "quarantine_worker"):
            probation_end = datetime.now() + timedelta(
                seconds=self.config["worker_probation_period"]
            )
    ...
    def get_worker_anomalies(self, worker_id: str = None, limit: int = 10) -> Dict[str, Any]:
        ...
        """
        if worker_id:
            return {worker_id: self.anomaly_history.get(worker_id, [])[-limit:]}
        else:
            return {w_id: anomalies[-limit:] for w_id, anomalies in self.anomaly_history.items()}

    def get_recovery_history(self, worker_id: str = None, limit: int = 10) -> Dict[str, Any]:
        ...
        """
        if worker_id:
            return {worker_id: self.recovery_attempts.get(worker_id, [])[-limit:]}
        else:
            return {w_id: attempts[-limit:] for w_id, attempts in self.recovery_attempts.items()}
    ...
    def unsubscribe_from_alerts(self, callback: Callable[[Dict[str, Any]], None]):
        ...
        if callback in self.alert_subscribers:
            ...
            logger.info(
                f"Removed alert subscriber, remaining subscribers: {len(self.alert_subscribers)}"
            )
    ...
    def generate_health_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "system": {
                ...
                "healthy_workers": sum(
                    1
                    for status in self.worker_health.values()
            ...
            "recovery": {
                ...
                "successful_attempts": sum(
                    self.recovery_success.get(w_id, 0) for w_id in self.recovery_attempts
                ),
        ...
        if self.db_manager and hasattr(self.db_manager, "get_quarantined_workers_count"):
            report["system"]["quarantined_workers"] = (
                self.db_manager.get_quarantined_workers_count()
            )
