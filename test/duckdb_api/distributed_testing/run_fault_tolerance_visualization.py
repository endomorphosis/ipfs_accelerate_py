...
from data.duckdb.distributed_testing.ml_pattern_detection import MLPatternDetector
...
def create_simulation_data(coordinator=None):
    ...
    manager = HardwareAwareFaultToleranceManager(
        db_manager=None, coordinator=coordinator, enable_ml=True
    )
    ...
    cpu_profile = HardwareCapabilityProfile(
        ...
        memory=MemoryProfile(
            total_bytes=8 * 1024 * 1024 * 1024, available_bytes=6 * 1024 * 1024 * 1024
        ),
        compute_units=8,
    )
    ...
    gpu_profile = HardwareCapabilityProfile(
        ...
        memory=MemoryProfile(
            total_bytes=16 * 1024 * 1024 * 1024, available_bytes=14 * 1024 * 1024 * 1024
        ),
        compute_units=128,
    )
    ...
    webgpu_profile = HardwareCapabilityProfile(
        ...
        memory=MemoryProfile(
            total_bytes=4 * 1024 * 1024 * 1024, available_bytes=3 * 1024 * 1024 * 1024
        ),
        compute_units=32,
    )
...
def _add_gpu_failures(manager, gpu_profile):
    ...
    for i in range(15):
        error_type = random.choice([FailureType.HARDWARE_ERROR, FailureType.RESOURCE_EXHAUSTION])
...
def _add_webgpu_failures(manager, webgpu_profile):
    ...
    for i in range(10):
        error_type = random.choice([FailureType.BROWSER_FAILURE, FailureType.RESOURCE_EXHAUSTION])

        error_message = "Generic WebGPU error"
        if error_type == FailureType.BROWSER_FAILURE:
            error_message = random.choice(
                ["WebGPU context lost", "Browser crashed unexpectedly", "WebGPU validation error"]
            )
        ...
        if manager.ml_detector:
            for action in actions:
                ...
                manager.ml_detector.update_recovery_result(
                    task_id=task_id, strategy=action.strategy, success=success
                )
...
def main():
    ...
    parser.add_argument(
        "--open-browser", action="store_true", help="Open the report in a web browser"
    )
