...
class DashboardMetricsCollector:
    ...
    def update_last_minute_metrics(self) -> None:
        ...
        with self.lock:
            ...
            last_minute_points = [
                value for timestamp, value in self.throughput_history if timestamp >= one_minute_ago
            ]

            self.last_minute_throughput = (
                sum(last_minute_points) / len(last_minute_points) if last_minute_points else 0
            )

            # Filter latency history for last minute
            last_minute_latency = [
                value for timestamp, value in self.latency_history if timestamp >= one_minute_ago
            ]

            self.last_minute_latency = (
                sum(last_minute_latency) / len(last_minute_latency) if last_minute_latency else 0
            )
...
class TerminalDashboard:
    ...
    def _print_key_metrics(self, summary: Dict[str, Any]) -> None:
        ...
        # First row
        print(
            f"Throughput: {current_throughput:.2f} tests/s (Avg: {avg_throughput:.2f})", end="  |  "
        )
    ...
    def _print_throughput_graph(self, summary: Dict[str, Any]) -> None:
        ...
        if len(throughput_history) >= 2:
            first_time = (
                throughput_history[-graph_width][0]
                if len(throughput_history) >= graph_width
                else throughput_history[0][0]
            )
    ...
    def _print_worker_utilization(self, summary: Dict[str, Any]) -> None:
        ...
        sorted_workers = sorted(
            worker_loads.items(), key=lambda x: x[1].cpu_utilization, reverse=True
        )
        ...
        for worker_id, load in sorted_workers[:5]:
            ...
            print(
                f"{short_id:<20} {cpu_util:<8.1f} {mem_util:<10.1f} {gpu_util:<8.1f} {status:<10}"
            )
        ...
        while not self.stop_event.is_set():
            try:
                ...
                with self.load_balancer.lock:
                    ...
                    worker_loads = {
                        worker_id: load
                        for worker_id, load in self.load_balancer.worker_loads.items()
                    }
...
def run_stress_test_with_monitor(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        stress_thread = threading.Thread(target=stress_test.run, daemon=True)
        stress_thread.start()
...
def run_scenario_with_monitor(args: argparse.Namespace) -> None:
    ...
    from data.duckdb.distributed_testing.test_load_balancer_stress import (
        LoadBalancerStressTest,
        load_config,
...
def main():
    ...
    stress_parser.add_argument(
        "--workers", type=int, default=20, help="Number of workers (default: 20)"
    )
    stress_parser.add_argument(
        "--tests", type=int, default=100, help="Number of tests (default: 100)"
    )
    stress_parser.add_argument(
    ...
    scenario_parser = subparsers.add_parser(
        "scenario", help="Run a specific scenario with monitoring"
    )
    scenario_parser.add_argument("scenario", type=str, help="Name of the scenario to run")
