...
def main():
    ...
    aggregator.configure(
        {"model_family_grouping": False, "cache_ttl_seconds": 600, "anomaly_threshold": 2.0}
    )
    ...
    for model_id, stats in basic_stats.items():
        ...
        if "average_latency_ms" in stats:
            ...
            print(
                f"Latency (ms): mean={latency_stats.get('mean', 0):.2f}, "
        ...
        if "throughput_items_per_second" in stats:
            ...
            print(
                f"Throughput (items/s): mean={throughput_stats.get('mean', 0):.2f}, "
    ...
    for anomaly in anomalies.get("anomalies", []):
        ...
        print(
            f"Direction: {anomaly.get('direction', 'unknown')}, Severity: {anomaly.get('severity', 'unknown')}"
        )
