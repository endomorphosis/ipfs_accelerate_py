...
from data.duckdb.distributed_testing.load_balancer.service import (
    LoadBalancerService,
    create_load_balancer,
)
from data.duckdb.distributed_testing.load_balancer.coordinator_integration import (
...
class CoordinatorLoadBalancerIntegration:
    ...
    def __init__(
        ...
        self.coordinator.register_event_handler(
            "worker_status_changed", self._on_worker_status_changed
        )
    ...
    def start(self):
        ...
        self.sync_thread = threading.Thread(
            target=self._sync_loop, daemon=True, name="load-balancer-sync"
        )
            ...
            worker_load = WorkerLoad(
                ...
                queue_depth=len(
                    [
                        t
                        for t in tasks
                        if self.coordinator.tasks.get(t, {}).get("status") == "queued"
                    ]
                ),
                reserved_memory=metrics.get("memory_used", 0.0),
                reserved_accelerators={"gpu": metrics.get("gpu_used", 0)},
            )
    ...
    def _register_task_with_load_balancer(self, task_id, task_data):
        ...
        try:
            ...
            self._last_sync[task_id] = {"time": datetime.now(), "status": "pending"}
    ...
    def _handle_assignment_update(self, notification):
        ...
        try:
            ...
            if task_id in self._last_sync:
                self._last_sync[task_id] = {"time": datetime.now(), "status": status}
            ...
            if task_id in self.coordinator.tasks:
                ...
                if (
                    ...
                    and status in valid_transitions[current_status]
                ):
                    ...
                    if status in status_map:
                        ...
                        self.coordinator.update_task_status(
                            task_id, coordinator_status, worker_id=worker_id, result=result
                        )

                        logger.info(
                            f"Updated task {task_id} status to {coordinator_status} in coordinator"
                        )
        except Exception as e:
            ...
            if worker["status"] in ["registered", "active"]:
                ...
                worker_data[worker_id] = {
                    ...
                    "load": {
                        ...
                        "memory_utilization": worker.get("metrics", {}).get(
    ...
    def get_next_worker_assignment(self, worker_id):
        ...
        try:
            ...
            if assignment:
                ...
                return {"task_id": assignment["test_id"], "config": assignment["requirements"]}
    ...
    def get_worker_capabilities_score(self, worker_id, task_id):
        ...
        try:
            ...
            if (
                requirements.get("required_backend")
                and requirements.get("required_backend") not in capabilities.supported_backends
            ):
                return -1
