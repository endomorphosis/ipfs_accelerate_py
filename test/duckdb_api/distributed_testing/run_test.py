...
try:
    ...
    logger.warning(
        "Could not import directly from distributed_testing modules, will use subprocess"
    )
    DIRECT_IMPORT = False
...
def run_coordinator(
    host: str, port: int, db_path: Optional[str] = None, security_config: Optional[str] = None
) -> subprocess.Popen:
...
def run_worker(
    ...
    if DIRECT_IMPORT:
        ...
        def worker_thread():
            # Create worker
            worker = WorkerClient(
                coordinator_url=coordinator_url, api_key=api_key, worker_id=worker_id
            )
...
def run_dashboard(
    host: str, port: int, coordinator_url: str, auto_open: bool = False
) -> subprocess.Popen:
    ...
    if DIRECT_IMPORT:
        ...
        def dashboard_thread():
            ...
            dashboard = DashboardServer(
                host=host, port=port, coordinator_url=coordinator_url, auto_open=auto_open
            )
...
def submit_test_task(
    coordinator_url: str, test_file: str, test_args: List[str] = None, priority: int = 5
) -> str:
    ...
    try:
        ...
        task_data = {
            "type": "test",
            "priority": priority,
            "config": {"test_file": test_file, "test_args": test_args or []},
            "requirements": {},
        }
...
def wait_for_task_completion(
    coordinator_url: str, task_id: str, timeout: int = DEFAULT_TEST_TIMEOUT
) -> Dict[str, Any]:
...
def generate_security_config(file_path: str = DEFAULT_SECURITY_CONFIG) -> Dict[str, Any]:
    ...
    config = {"token_secret": token_secret, "api_keys": {"worker": worker_api_key}}
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=[MODE_COORDINATOR, MODE_WORKER, MODE_CLIENT, MODE_DASHBOARD, MODE_ALL],
        default=MODE_ALL,
        help="Mode to run in",
    )

    # Coordinator options
    parser.add_argument("--host", default=DEFAULT_HOST, help="Host to bind servers to")
    parser.add_argument(
        "--port",
        type=int,
    ...
    parser.add_argument(
        "--api-key", default=None, help="API key for authentication (for worker mode)"
    )
    parser.add_argument(
        "--worker-id", default=None, help="Worker ID (for worker mode, generated if not provided)"
    )
    parser.add_argument(
        "--work-dir", default=None, help="Working directory for tasks (for worker mode)"
    )
    ...
    # Client options
    parser.add_argument("--test-file", default=None, help="Test file to submit (for client mode)")
    parser.add_argument(
        "--test-args", default=None, help="Arguments for the test (for client mode)"
    )
    parser.add_argument(
        "--priority",
        type=int,
    ...
    try:
        ...
        if args.mode == MODE_COORDINATOR:
            ...
            # Run dashboard
            run_dashboard(
                args.host, args.dashboard_port, args.coordinator, args.dashboard_auto_open
            )
            ...
            processes = run_all_mode(
                args.host, args.port, args.dashboard_port, args.db_path, args.worker_count
            )
