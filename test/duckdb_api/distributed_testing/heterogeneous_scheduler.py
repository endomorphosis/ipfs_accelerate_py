...
class WorkloadProfile:
    ...
    required_capabilities: Set[str] = field(
        default_factory=set
    )  # Required capabilities from Enhanced Hardware Taxonomy
    preferred_capabilities: Set[str] = field(
        default_factory=set
    )  # Preferred capabilities from Enhanced Hardware Taxonomy
...
class WorkerState:
    ...
    def __post_init__(self):
        ...
        for profile in self.hardware_profiles:
            ...
            if "memory_available_gb" in profile:
                self.available_memory_gb = max(
                    self.available_memory_gb, profile["memory_available_gb"]
                )
    ...
    def update_thermal_state(self):
        ...
        cooling = (
            self.thermal_state["cooling_rate"] * elapsed_seconds if not self.active_tasks else 0.0
        )
    ...
    def complete_task(
        self, task_id: str, result: Any, hardware_class: str, hardware_model: str
    ) -> Optional[TestTask]:
        """
...
class HeterogeneousScheduler:
    ...
    def register_worker(self, worker_id: str, capabilities: Dict[str, Any]) -> WorkerState:
        ...
        with self._lock:
            ...
            logger.info(
                f"Registered worker {worker_id} with {len(worker.hardware_profiles)} hardware profiles"
            )
            return worker
    ...
    def submit_task(self, task: TestTask) -> str:
        ...
        with self._lock:
            ...
            logger.debug(
                f"Submitted task {task.task_id} of type {task.workload_profile.workload_type}"
            )
            return task.task_id
    ...
    def schedule_tasks(self):
        ...
        with self._lock:
            ...
            logger.debug(
                f"Scheduled {len(scheduled_tasks)} tasks, {len(self.pending_tasks)} pending"
            )
    ...
    def _schedule_adaptive(self, tasks: List[TestTask]) -> List[TestTask]:
        ...
        scheduled_tasks = []
        available_workers = [
            w for w in self.workers.values() if w.status != "offline" and w.status != "cooling"
        ]
            ...
            for task in workload_tasks:
                ...
                for worker, _ in workers_with_scores:
                    if worker.has_capacity_for(task):
                        ...
                        self.stats["worker_utilization"][worker.worker_id] = len(
                            worker.active_tasks
                        )
    ...
    def _schedule_performance_aware(self, tasks: List[TestTask]) -> List[TestTask]:
        ...
        for task in tasks:
            ...
            if workers_with_scores:
                ...
                # Update worker utilization stats
                self.stats["worker_utilization"][best_worker.worker_id] = len(
                    best_worker.active_tasks
                )
            else:
    ...
    def report_task_completion(
        self, worker_id: str, task_id: str, result: Any, hardware_info: Dict[str, str]
    ):
        """
        ...
        with self._lock:
            ...
            task = self.workers[worker_id].complete_task(
                task_id, result, hardware_class, hardware_model
            )
            ...
            if task:
                ...
                if task.execution_time_ms:
                    ...
                    self.stats["avg_execution_time_ms"] = (
                        old_avg * (self.stats["tasks_completed"] - 1) + task.execution_time_ms
                    ) / self.stats["tasks_completed"]

                queue_time_ms = task.get_queue_time() * 1000
                old_avg = self.stats["avg_queue_time_ms"]
                self.stats["avg_queue_time_ms"] = (
                    old_avg * (self.stats["tasks_completed"] - 1) + queue_time_ms
                ) / self.stats["tasks_completed"]

                logger.debug(
                    f"Task {task_id} completed on {worker_id} ({hardware_class}/{hardware_model}) in {task.execution_time_ms:.2f}ms"
                )
            else:
                logger.warning(
                    f"Completion reported for unknown task {task_id} on worker {worker_id}"
                )
    ...
    def get_scheduler_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats["active_workers"] = len(
                [w for w in self.workers.values() if w.status != "offline"]
            )
                ...
                active_task_details.append(
                    {
                        ...
                        "queue_time_ms": task.get_queue_time() * 1000,
                        "running_time_ms": (time.time() - task.start_time) * 1000
                        if task.start_time
                        else None,
                    }
                )
    ...
    def get_workload_stats(self, workload_type: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            pending = [
                t for t in self.pending_tasks if t.workload_profile.workload_type == workload_type
            ]
            scheduled = [
                t
                for t in self.scheduled_tasks.values()
                if t.workload_profile.workload_type == workload_type
            ]
            completed = [
                t for t in self.completed_tasks if t.workload_profile.workload_type == workload_type
            ]
            failed = [
                t for t in self.failed_tasks if t.workload_profile.workload_type == workload_type
            ]

            # Performance statistics
            execution_times = [
                t.execution_time_ms for t in completed if t.execution_time_ms is not None
            ]
            avg_execution_time = (
            ...
            stats = {
                ...
                "profile": {
                    ...
                    "hardware_class_affinity": self.workload_profiles[
                        workload_type
                    ].hardware_class_affinity,
                },
            }
        ...
        with self._lock:
            ...
            for worker_id, worker in list(self.workers.items()):
                if now - worker.last_heartbeat > timeout_seconds and worker.status != "offline":
                    logger.warning(
                        f"Worker {worker_id} hasn't reported in {timeout_seconds} seconds, marking as offline"
                    )
    ...
    def perform_load_balancing(self):
        ...
        with self._lock:
            ...
            for worker_id, load, worker in worker_loads:
                if (
                    load > avg_load * 1.5 and load > 1
                ):  # At least 50% above average and more than 1 task
            ...
            for over_id, over_load, over_worker in overloaded:
                ...
                tasks_to_move = sorted(
                    over_worker.active_tasks, key=lambda t: t.scheduled_time or 0, reverse=True
                )
                ...
                for task in tasks_to_move:
                    ...
                    for under_id, under_load, under_worker in underloaded:
                        if under_worker.has_capacity_for(task):
                            ...
                            logger.info(
                                f"Moving task {task.task_id} from {over_id} to {under_id} for load balancing"
                            )
        ...
        with self._lock:
            try:
                ...
                for workload_type, profile_data in state.get("workload_profiles", {}).items():
                    ...
                    profile.hardware_class_affinity = profile_data.get(
                        "hardware_class_affinity", {}
                    )
                ...
                logger.info(
                    f"Imported {len(state.get('workload_profiles', {}))} workload profiles from {file_path}"
                )
                return True
            except Exception as e:
