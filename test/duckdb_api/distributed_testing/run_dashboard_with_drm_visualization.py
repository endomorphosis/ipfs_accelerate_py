...
async def main():
    ...
    try:
        ...
        logger.error(
            "Make sure the duckdb_api.distributed_testing package is installed and in your PYTHONPATH."
        )
        return 1
    ...
    if not args.no_mock:
            ...
            scaling_history.append(
                {
                    ...
                    else "workers_removed": random.randint(1, 3),
        ...
        mock_resource_manager.get_resource_efficiency.return_value = {
            worker_id: random.uniform(0.5, 0.95) for worker_id in worker_resources.keys()
        }
    ...
    logger.info(
        "Navigate to http://{args.host}:{args.port}/drm-dashboard to view the DRM visualizations"
    )
