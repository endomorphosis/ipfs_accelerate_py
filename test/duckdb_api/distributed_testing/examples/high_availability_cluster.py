...
def start_node_process(node_id, port, other_ports, base_dir):
    ...
    cmd = [
        ...
        ",".join(coordinator_addresses),
        "--auto-leader-election",
        "--mock-workers",
        "2",  # Start 2 mock workers
...
def run_cluster_example(num_nodes=3, base_port=8080, fault_injection=False, runtime=60):
    ...
    for i in range(num_nodes):
        ...
        process = start_node_process(
            node_id=node_ids[i], port=node_ports[i], other_ports=other_ports, base_dir=base_dir
        )

        node_processes.append(
            {"index": i, "node_id": node_ids[i], "port": node_ports[i], "process": process}
        )
        ...
        while time.time() - start_time < runtime:
            ...
            for node in node_processes:
                if node["process"].poll() is not None:
                    print(
                        f"Node {node['index']} (ID: {node['node_id']}) has crashed, restarting..."
                    )
