...
def detect_platform_and_hardware():
    ...
    print(
        f"Disk space: {disk_info.get('total_gb', 'Unknown')} GB total, {disk_info.get('free_gb', 'Unknown')} GB free"
    )
...
def create_deployment_scripts(output_dir, coordinator_url, api_key, worker_id):
    ...
    config = {"coordinator_url": coordinator_url, "api_key": api_key, "worker_id": worker_id}
...
def main():
    ...
    if args.scripts or args.all:
        ...
        create_deployment_scripts(
            args.output_dir, args.coordinator_url, args.api_key, f"worker_example"
        )
    ...
    if args.startup or args.all:
        ...
        print("=" * 50)
        generate_startup_scripts(args.output_dir, args.coordinator_url, args.api_key)
