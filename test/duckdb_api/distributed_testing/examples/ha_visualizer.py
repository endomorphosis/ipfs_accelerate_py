...
def create_cluster_visualization(cluster_dir, output_dir):
    ...
    legend_elements = [
        plt.Line2D(
            [0], [0], marker="o", color="w", markerfacecolor=color, markersize=10, label=role
        )
...
def create_health_metrics_visualization(cluster_dir, output_dir):
    ...
    metrics = {
        "CPU Usage": {
            node_dir.replace("node_", ""): np.random.uniform(20, 80) for node_dir in node_dirs
        },
        "Memory Usage": {
            node_dir.replace("node_", ""): np.random.uniform(30, 70) for node_dir in node_dirs
        },
        "Disk Usage": {
            node_dir.replace("node_", ""): np.random.uniform(40, 60) for node_dir in node_dirs
        },
        "Network Usage": {
            node_dir.replace("node_", ""): np.random.uniform(10, 50) for node_dir in node_dirs
        },
    }
    ...
    fig.suptitle(
        f"High Availability Cluster Health Metrics\n{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    with open(leader_transition_md, "w") as f:
        ...
        f.write(f"|------|----------------|------------|--------|\n")
        f.write(
            f"| {datetime.now().strftime('%H:%M:%S')} | Initial state | {node_dirs[0].replace('node_', '')} | Initial election |\n"
        )
        ...
        for i in range(3):
            ...
            reason = np.random.choice(
                ["Timeout", "Failure detected", "Manual failover", "Resource exhaustion"]
            )
...
def main():
    ...
    parser.add_argument(
        "--cluster-dir",
        type=str,
        default="/home/barberb/ipfs_accelerate_py/test/ha_cluster_example",
        help="Directory containing cluster data",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/barberb/ipfs_accelerate_py/test/ha_cluster_example/visualizations",
