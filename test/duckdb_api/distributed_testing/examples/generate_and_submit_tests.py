...
try:
    ...
    logger.error(
        "Cannot import TestGeneratorIntegration. Make sure the module is in your Python path."
    )
    sys.exit(1)
        ...
        for family, models in MODEL_FAMILIES.items():
            for model_name in models:
                ...
                success, tests = generator.generate_and_submit_tests(
                    model_name=model_name, hardware_types=HARDWARE_TYPES, batch_sizes=BATCH_SIZES
                )
...
def main():
    ...
    parser.add_argument(
        "--generate-tests", action="store_true", help="Generate tests from templates"
    )
