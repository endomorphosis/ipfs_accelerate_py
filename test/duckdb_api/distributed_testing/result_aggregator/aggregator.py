...
class ResultAggregator:
    ...
    def __init__(self, db_manager=None, task_scheduler=None):
        ...
        self.config = {
            ...
            "anomaly_detection": {"enabled": True, "z_score_threshold": 3.0, "min_samples": 5},
        }
    ...
    def _load_latest_results(self):
        ...
        try:
            ...
            last_update_time = datetime.now() - timedelta(
                seconds=self.config["update_interval"] * 1.1
            )
    ...
    def _calculate_aggregates(self, results):
        ...
        for key, values in metrics.items():
            ...
            if aggregates[f"{key}_mean"] > 0:
                aggregates[f"{key}_cv"] = (
                    aggregates[f"{key}_stdev"] / aggregates[f"{key}_mean"]
                ) * 100
            else:
    ...
    def _analyze_failure_reasons(self, results):
        ...
        if total_failures > 0:
            reason_percentages = {
                reason: (count / total_failures) * 100 for reason, count in reason_counts.items()
            }
        else:
            reason_percentages = {}
    ...
    def _analyze_task_type_distribution(self, results):
        ...
        total = len(results)
        type_percentages = {
            task_type: (count / total) * 100 for task_type, count in type_counts.items()
        }
    ...
    def _analyze_worker_distribution(self, results):
        ...
        total = len(results)
        worker_percentages = {
            worker_id: (count / total) * 100 for worker_id, count in worker_counts.items()
        }
    ...
    def _analyze_performance_trend(self, results):
        ...
        timestamps = np.array(
            [
                (r.get("timestamp", datetime.now()) - datetime(1970, 1, 1)).total_seconds()
    ...
    def _analyze_dimension_correlations(self, results):
        ...
        for dimension in self.config["aggregate_dimensions"]:
            ...
            # Extract durations
            duration_by_value = {
                value: [r.get("duration", 0) for r in group] for value, group in grouped.items()
            }
    ...
    def _analyze_hardware_performance(self, results):
        ...
        for hardware_id, hw_results in grouped.items():
            ...
            hardware_stats[hardware_id] = {
                ...
                "success_rate": sum(1 for r in hw_results if r.get("status") == "success")
                / len(hw_results),
            }
    ...
    def _analyze_model_performance(self, results):
        ...
        for model, model_results in grouped.items():
            ...
            model_stats[model] = {
                ...
                "max_duration": max(durations),
                "std_duration": statistics.stdev(durations) if len(durations) > 1 else 0,
                "success_rate": sum(1 for r in model_results if r.get("status") == "success")
                / len(model_results),
            }
    ...
    def _analyze_performance_variance(self, results):
        ...
        stats = {
            ...
            "coefficient_of_variation": statistics.stdev(durations) / statistics.mean(durations)
        ...
        for dimension in self.config["aggregate_dimensions"]:
            ...
            for value, value_durations in grouped.items():
                ...
                value_stats[value] = {
                    ...
                    "coefficient_of_variation": statistics.stdev(value_durations)
                    / statistics.mean(value_durations)
                    if statistics.mean(value_durations) > 0
                    else 0,
                }
            ...
            if len(value_stats) >= 2:
                try:
                    ...
                    within_variance = sum(
                        count * var for count, var in zip(group_counts, group_variances)
                    ) / sum(group_counts)
    ...
    def _analyze_optimal_configurations(self, results):
        ...
        for config, config_results in configs.items():
            ...
            config_stats[config] = {
                ...
                "max_duration": max(durations),
                "std_duration": statistics.stdev(durations) if len(durations) > 1 else 0,
                "success_rate": sum(1 for r in config_results if r.get("status") == "success")
                / len(config_results),
            }
    ...
    def _generate_visualizations(self):
        ...
        for test_id, history in self.historical_performance.items():
            ...
            for i, metric in enumerate(sorted(metrics_to_plot)):
                ...
                if (
                    test_id in self.regression_results
                    and metric in self.regression_results[test_id]["metrics"]
                ):
                    ...
                    if regression_info["is_regression"]:
                        ...
                        if regression_info["is_significant"]:
                            ...
                            plt.axhspan(
                                min(
                                    regression_info["current_mean"],
                                    regression_info["baseline_mean"],
                                ),
                                max(
                                    regression_info["current_mean"],
                                    regression_info["baseline_mean"],
    ...
    def get_regressions(self, significant_only=False):
        ...
        if significant_only:
            return {
                test_id: info
                for test_id, info in self.regression_results.items()
