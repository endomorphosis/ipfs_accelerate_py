...
class ResultAggregatorService:
        ...
        for result in results:
            ...
            if context["result_type"] == RESULT_TYPE_PERFORMANCE:
                ...
                if is_valid and (
                    ...
                    or result.get("average_latency_ms", 0) < 0
                    or result.get("throughput_items_per_second", 0) < 0
                ):
                    is_valid = False
                ...
                # Check for logical constraints
                if is_valid and (
                    result.get("load_time_ms", 0) < 0
                    or result.get("inference_time_ms", 0) < 0
                    or result.get("total_time_ms", 0) < 0
                ):
                    is_valid = False
                        ...
                        if metric in result and (model_id, metric) in reference_values:
                            ...
                            if ref_value > 0:  # Avoid division by zero
                                ...
                                if metric == "throughput_items_per_second":
                                    normalized_result[f"normalized_{metric}"] = (
                                        result[metric] / ref_value
                                    )
                                else:
                                    normalized_result[f"normalized_{metric}"] = (
                                        ref_value / result[metric]
                                    )
    ...
    def _aggregate_basic_statistics(
        self, results: List[Dict[str, Any]], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if result_type == RESULT_TYPE_PERFORMANCE:
            ...
            metrics_to_aggregate = ["execution_time_seconds"]
    ...
    def _aggregate_percentiles(
        ...
        if result_type == RESULT_TYPE_PERFORMANCE:
            ...
            metrics_to_aggregate = ["compatibility_score"]
    ...
    def _aggregate_distributions(
        ...
        if result_type == RESULT_TYPE_PERFORMANCE:
            categorical_metrics = ["precision", "batch_size", "is_simulated"]
            ...
            categorical_metrics = ["status", "test_module", "test_class"]

        elif result_type == RESULT_TYPE_WEB_PLATFORM:
            categorical_metrics = ["platform", "browser", "success"]
        ...
        for group_key, group_results in grouped_results.items():
            ...
            for metric in categorical_metrics:
                ...
                distribution = {
                    value: {"count": count, "percentage": (count / valid_values) * 100}
    ...
    def _detect_anomalies(
        self, aggregated_results: Dict[str, Any], context: Dict[str, Any]
    ) -> None:
        ...
        if result_type == RESULT_TYPE_PERFORMANCE:
            ...
            anomaly_metrics = ["compatibility_score"]

        elif result_type == RESULT_TYPE_INTEGRATION:
            anomaly_metrics = ["execution_time_seconds", "passed"]
    ...
    def _comparative_analysis(
        ...
        if result_type == RESULT_TYPE_PERFORMANCE:
            ...
            comparison_metrics = ["compatibility_score"]
    ...
    def _add_context_metadata(
        ...
        if time_range and all(ts is not None for ts in time_range if time_range):
            aggregated_results["context"]["time_range"] = [
                ts.isoformat() if ts else None for ts in time_range
            ]
    ...
    def _group_results_by_level(
        self, results: List[Dict[str, Any]], aggregation_level: str
    ...
    def get_result_anomalies(
        ...
        # Generate cache key
        cache_key = (
            self._generate_cache_key(result_type, aggregation_level, filter_params, time_range)
            + "_anomalies"
        )
        ...
        formatted_anomalies = {
            ...
            "aggregation_level": aggregation_level,
            "filter_params": filter_params or {},
            "time_range": [ts.isoformat() if ts else None for ts in time_range]
            if time_range
            else None,
            "anomalies": [],
            "anomaly_count": 0,
