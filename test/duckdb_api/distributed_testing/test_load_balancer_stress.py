...
def generate_worker_capabilities(
...
class TestMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            success_rate = (
                (self.successfully_scheduled / total_tests * 100) if total_tests > 0 else 0
            )
            ...
            worker_assignment_stddev = (
                statistics.stdev(worker_assignments) if len(worker_assignments) > 1 else 0
            )
            worker_assignment_range = (
                max(worker_assignments) - min(worker_assignments) if worker_assignments else 0
            )
            worker_utilization = (
                len(self.assignments_per_worker) / len(self.schedule_attempts)
                if self.schedule_attempts
                else 0
            )

            # Calculate average scheduling attempts
            avg_attempts = (
                statistics.mean(list(self.schedule_attempts.values()))
                if self.schedule_attempts
                else 0
            )
...
class LoadBalancerStressTest:
    ...
    def assignment_callback(self, assignment: WorkerAssignment) -> None:
        ...
        if assignment.status == "completed":
            ...
            if assignment.worker_id == "none":
                logger.debug(
                    f"❌ Test {assignment.test_id} failed to schedule - {assignment.result.get('error', 'Unknown error')}"
                )
            else:
    ...
    def register_worker(self) -> str:
        ...
        logger.debug(
            f"Registered worker {worker_id} with {memory_gb}GB memory, {cuda_devices} CUDA devices"
        )
    ...
    def submit_test(self) -> str:
        ...
        requirements = create_test_requirements(test_id, memory_required, cuda_required, priority)

        self.load_balancer.submit_test(requirements)
        self.metrics.record_submission(test_id)

        logger.debug(
            f"Submitted test {test_id} with {memory_required}GB memory, {cuda_required} CUDA devices, priority {priority}"
        )
                ...
                with self.worker_lock:
                    ...
                    for worker_id in self.active_workers:
                        ...
                        load.gpu_utilization = (
                            random.uniform(0, 100) if random.random() < 0.7 else 0.0
                        )
                    ...
                    if self.dynamic_workers:
                        ...
                        if (
                            random.random() < 0.1
                            and len(self.active_workers) < self.num_workers * 1.5
                        ):
                        ...
                        if (
                            random.random() < 0.05
                            and len(self.active_workers) > self.num_workers * 0.5
                        ):
        ...
        while tests_submitted < self.num_tests and not self.stop_event.is_set():
            try:
                ...
                if self.burst_mode:
                    ...
                    next_burst_time = (
                        start_time + (tests_submitted / self.num_tests) * self.duration
                    )
    ...
    def run(self) -> Dict[str, Any]:
        """Run the stress test."""
        logger.info(
            f"Starting stress test with {self.num_workers} workers, {self.num_tests} tests over {self.duration}s"
        )
        ...
        try:
            ...
            self.worker_update_thread = threading.Thread(
                target=self.worker_update_loop, daemon=True
            )
            self.worker_update_thread.start()

            # Start test submission loop
            self.test_submit_thread = threading.Thread(target=self.test_submit_loop, daemon=True)
...
def run_benchmark_suite(args: argparse.Namespace) -> None:
    ...
    for workers in worker_counts:
        for tests in test_counts:
            ...
            logger.info(
                f"Running benchmark with {workers} workers, {tests} tests, {duration}s duration"
            )
            ...
            result = {"workers": workers, "tests": tests, "duration": duration, "metrics": metrics}
            ...
            print(
                f"  Throughput: {metrics['peak_throughput']} tests/s, Latency: {metrics['avg_latency']:.2f}s"
            )
            print(f"  Success Rate: {metrics['success_rate']:.2f}%")
...
def simulate_load_spike(args: argparse.Namespace) -> None:
    ...
    if args.output:
        ...
        result_data = {
            ...
            "metrics": metrics,
            "time_series": {"throughput": throughput_data, "latency": latency_data},
        }
...
def load_config(config_file_path: str = None) -> Dict[str, Any]:
    ...
    if not config_file_path:
        ...
        config_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "load_balancer_stress_config.json"
        )
...
def run_scenario(scenario_name: str, output_file: str, config: Dict[str, Any]) -> None:
    ...
    # Run the test
    logger.info(
        f"Starting scenario test with {scenario_config.get('workers')} workers, "
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="", help="Output file for results (JSON format)"
    )
    parser.add_argument(
        "--config", type=str, default="", help="Path to configuration file (JSON format)"
    )
    parser.add_argument(
        "--list-scenarios",
        action="store_true",
    ...
    parser.add_argument(
        "--scenario", type=str, default="", help="Run a specific test scenario from configuration"
    )
    ...
    stress_parser.add_argument(
        "--burst", action="store_true", help="Submit tests in bursts rather than evenly distributed"
    )
    stress_parser.add_argument(
        "--dynamic", action="store_true", help="Add/remove workers during the test"
    )

    # Benchmark suite
    benchmark_parser = subparsers.add_parser(
        "benchmark", help="Run benchmark suite with varying configurations"
    )
    benchmark_parser.add_argument(
        "--full", action="store_true", help="Run the full benchmark suite (can take a long time)"
    )
