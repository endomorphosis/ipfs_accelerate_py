        ...
        try:
            ...
            for result_id, result_data in cached_results:
                try:
                    ...
                    cursor.execute(
                        "UPDATE cached_results SET submitted = 1 WHERE id = ?", (result_id,)
                    )
            ...
            if batch:
                ...
                if success and not failed:
                    ...
                    cursor.execute(
                        f"UPDATE cached_results SET submitted = 1 WHERE id IN ({placeholders})", ids
                    )
                elif failed:
                ...
                self.local_cache.commit()
                logger.debug(
                    f"Processed {len(batch)} cached results, {len(failed) if failed else 0} failed"
                )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Batch size for result submission"
    )
    parser.add_argument(
        "--interval", type=int, default=60, help="Interval for automatic batch submission"
    )
