...
def run_all_tests(verbosity=2, generate_report=False, report_format="html"):
    ...
    if generate_report:
        if report_format == "html":
            ...
            runner = xmlrunner.XMLTestRunner(output="test_reports", verbosity=verbosity)
    else:
...
def run_specific_tests(test_type, verbosity=2):
    ...
    if test_type == "sound":
        ...
        if os.path.exists(test_notification_script):
            ...
            os.system(
                f"python {test_notification_script} --system-critical-only --url http://localhost:8080"
            )
...
def main():
    ...
    parser.add_argument(
        "--verbosity", type=int, default=2, choices=[1, 2, 3], help="Verbosity level (1-3)"
    )
    parser.add_argument(
        "--type",
        choices=["sound", "severity", "websocket", "dashboard", "html", "system-critical"],
        help="Run specific type of tests",
    )
    parser.add_argument("--report", action="store_true", help="Generate test report")
    parser.add_argument(
        "--report-format", choices=["html", "xml"], default="html", help="Format for test report"
    )
    parser.add_argument(
        "--test-system-critical",
        action="store_true",
