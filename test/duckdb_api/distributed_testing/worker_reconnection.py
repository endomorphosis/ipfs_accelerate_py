...
class WorkerReconnectionManager:
    ...
    def start(self):
        ...
        self.message_sender_thread = threading.Thread(target=self._message_sender_loop, daemon=True)
    ...
    def submit_task_result(
        self, task_id: str, result: Dict[str, Any], error: Optional[Dict] = None
    ):
        """
    ...
    def get_latest_checkpoint(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        task_checkpoints = {
            cp_id: cp for cp_id, cp in self.checkpoints.items() if cp["task_id"] == task_id
        }
        ...
        latest_checkpoint_id = max(
            task_checkpoints.keys(), key=lambda cp_id: task_checkpoints[cp_id]["timestamp"]
        )
    ...
    def _start_connection(self):
        ...
        self.ws_thread = threading.Thread(target=self._ws_connection_loop, daemon=True)
    ...
    def _handle_connection_failure(self):
        ...
        logger.info(
            f"Will attempt reconnection in {delay:.1f} seconds (attempt {self.reconnect_attempts})"
        )
    ...
    def _handle_task_assignment(self, data: Dict[str, Any]):
        ...
        if self.task_executor:
            try:
                ...
                threading.Thread(
                    target=self._execute_task, args=(task_id, task_config), daemon=True
                ).start()
            except Exception as e:
                logger.error(f"Error starting task {task_id}: {e}")
                error_info = {"message": str(e), "traceback": traceback.format_exc()}
                self.submit_task_result(task_id, {}, error_info)
        else:
            logger.warning(f"No task executor available for task {task_id}")
            self.submit_task_result(task_id, {}, {"message": "No task executor available"})
    ...
    def _execute_task(self, task_id: str, task_config: Dict[str, Any]):
        ...
        try:
            ...
            self.update_task_state(
                task_id, {"status": "running", "started_at": datetime.now().isoformat()}
            )
...
def create_worker_reconnection_plugin(worker) -> WorkerReconnectionPlugin:
    ...
    logger.info(
        f"Created worker reconnection plugin for worker {getattr(worker, 'worker_id', 'unknown')}"
    )
    return plugin
