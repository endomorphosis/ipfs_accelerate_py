...
class EnhancedHardwareTaxonomy(HardwareTaxonomy):
    ...
    def __init__(self):
        ...
        self.hardware_hierarchies: Dict[
            Union[HardwareClass, str], List[Tuple[Union[HardwareClass, str], float]]
        ] = {}
    ...
    def _initialize_default_hierarchies(self):
        ...
        self.define_hardware_hierarchy(
            HardwareArchitecture.X86_64, HardwareArchitecture.GPU_CUDA, 0.8
        )
        self.define_hardware_hierarchy(
            HardwareArchitecture.X86_64, HardwareArchitecture.GPU_ROCM, 0.8
        )
        self.define_hardware_hierarchy(
            HardwareArchitecture.ARM64, HardwareArchitecture.GPU_METAL, 0.9
        )
        self.define_hardware_hierarchy(
            HardwareArchitecture.ARM64, HardwareArchitecture.NPU_QUALCOMM, 0.9
        )
    ...
    def _initialize_default_capabilities(self):
        ...
        self.register_capability(
            ...
            properties={"min_precision": "int4", "optimal_precision": "int8"},
            supported_hardware_classes={HardwareClass.GPU, HardwareClass.TPU, HardwareClass.NPU},
        ...
        self.register_capability(
            ...
            properties={"workgroup_size": 256, "max_compute_invocations": 16384},
    ...
    def define_hardware_hierarchy(
        self,
        parent_hardware: Union[HardwareClass, HardwareArchitecture, str],
        ...
        if child_hardware is not None:
            ...
            relationship_id = (
                f"{parent_type}:{parent_hardware.value if hasattr(parent_hardware, 'value') else parent_hardware}_"
    ...
    def register_hardware_relationship(
        self,
        source_hardware: Union[HardwareClass, HardwareArchitecture, str],
        source_type: str,
        target_hardware: Union[HardwareClass, HardwareArchitecture, str],
        ...
        """
        relationship_id = (
            f"{source_type}:{source_hardware.value if hasattr(source_hardware, 'value') else source_hardware}_"
    ...
    def get_hardware_relationships(
        self,
        hardware: Union[HardwareClass, HardwareArchitecture, str],
        hardware_type: str = "class",
        relationship_type: Optional[str] = None,
    ) -> List[HardwareRelationship]:
        """
        ...
        for rel_id, relationship in self.hardware_relationships.items():
            if rel_id.startswith(prefix) and (
                relationship_type is None or relationship.relationship_type == relationship_type
            ):
                relationships.append(relationship)

        return relationships

    def assign_capability_to_hardware(
        self,
        ...
        if (
            capability.supported_hardware_classes
            and hardware_profile.hardware_class not in capability.supported_hardware_classes
        ):
            raise ValueError(
                f"Hardware class {hardware_profile.hardware_class} is not compatible with capability '{capability_id}'"
            )

        if (
            capability.supported_architectures
            and hardware_profile.architecture not in capability.supported_architectures
        ):
            raise ValueError(
                f"Architecture {hardware_profile.architecture} is not compatible with capability '{capability_id}'"
            )

        if (
            capability.supported_vendors
            and hardware_profile.vendor not in capability.supported_vendors
        ):
            raise ValueError(
                f"Vendor {hardware_profile.vendor} is not compatible with capability '{capability_id}'"
            )

        if (
            capability.supported_models
            and hardware_profile.model_name not in capability.supported_models
        ):
            raise ValueError(
                f"Model {hardware_profile.model_name} is not compatible with capability '{capability_id}'"
            )
        ...
        for required_cap in capability.requires_capabilities:
            if required_cap not in current_capabilities:
                raise ValueError(
                    f"Capability '{capability_id}' requires capability '{required_cap}' which is not present"
                )
        ...
        for incompatible_cap in capability.incompatible_capabilities:
            if incompatible_cap in current_capabilities:
                raise ValueError(
                    f"Capability '{capability_id}' is incompatible with capability '{incompatible_cap}' which is present"
                )
    ...
    def has_capability(
        self, hardware_profile: HardwareCapabilityProfile, capability_id: str
    ) -> bool:
        """
    ...
    def get_hardware_capabilities(
        self, hardware_profile: HardwareCapabilityProfile, include_inherited: bool = True
    ...
    def _add_inherited_capabilities_from_class(
        self,
    ...
    def _add_inherited_capabilities_from_architecture(
        self,
    ...
    def discover_capabilities(self, hardware_profile: HardwareCapabilityProfile) -> Set[str]:
        ...
        if (
            hardware_profile.hardware_class == HardwareClass.GPU
            and hardware_profile.compute_units >= 30
        ):
        ...
        if (
            hardware_profile.architecture == HardwareArchitecture.GPU_CUDA
            and hardware_profile.memory.has_unified_memory
        ):
    ...
    def register_hardware_profile(
        self, profile: HardwareCapabilityProfile, auto_discover: bool = True
    ):
        """
    ...
    def calculate_workload_capability_match(
        self,
        workload_type: str,
