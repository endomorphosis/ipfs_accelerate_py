...
class MatchingEngine:
    ...
    def find_best_worker(
        ...
        for worker_id in worker_ids:
            ...
            if not worker_load.has_capacity_for(task_requirements, worker_capabilities):
                logger.debug(
                    f"Worker {worker_id} lacks capacity for task {task_requirements.test_id}"
                )
                continue

            # Calculate scores for this worker
            capability_score = self._calculate_capability_score(
                task_requirements, worker_capabilities
            )

            # Skip if capability score is below minimum threshold
            if capability_score < self.config["minimum_capability_score"]:
                logger.debug(
                    f"Worker {worker_id} capability score {capability_score:.2f} below threshold"
                )
                continue

            # Calculate remaining scores
            perf_score = self._calculate_performance_score(
                task_requirements, worker_id, worker_performances
            )

            load_score = self._calculate_load_score(task_requirements, worker_id, worker_load)
            ...
            if (
                self.config["enable_penalty_for_migrations"]
                and previous_assignments
                and task_requirements.test_id in previous_assignments
                and previous_assignments[task_requirements.test_id] != worker_id
            ):
        ...
        logger.info(
            f"Selected worker {best_worker.worker_id} for task {task_requirements.test_id} "
    ...
    def find_best_task_worker_pairs(
        ...
        for task_id, requirements in tasks.items():
            for worker_id, capabilities in workers.items():
                ...
                # Calculate remaining scores
                perf_score = self._calculate_performance_score(
                    requirements, worker_id, worker_performances
                )

                load_score = self._calculate_load_score(requirements, worker_id, worker_load)
        ...
        for score, task_id, worker_id, worker_load in all_scores:
            ...
            updated_load.reserve_resources(task_id, task_requirements, workers.get(worker_id))
    ...
    def _calculate_capability_score(
        self, requirements: TestRequirements, capabilities: WorkerCapabilities
    ) -> float:
    ...
    def _calculate_performance_score(
        ...
        if (
            ...
            and requirements.test_type
            and requirements.model_id in worker_perf
            and requirements.test_type in worker_perf[requirements.model_id]
        ):
            ...
            if perf.average_execution_time > 0:
                expected_time = (
                    requirements.expected_duration if requirements.expected_duration > 0 else 60.0
                )
    ...
    def _calculate_load_score(
        self, requirements: TestRequirements, worker_id: str, worker_load: WorkerLoad
    ) -> float:
