...
from .models import WorkerCapabilities, WorkerLoad, TestRequirements, WorkerAssignment
...
class WorkStealer:
    ...
    def identify_stealing_opportunities(
        ...
        logger.debug(
            f"Found {len(overloaded_workers)} overloaded and {len(underloaded_workers)} underloaded workers"
        )
        ...
        for task_id, (source_worker_id, task_requirements, start_time) in assigned_tasks.items():
            ...
            if estimated_total_time > 0:
                ...
                if estimated_remaining < self.config["min_remaining_time"]:
                    logger.debug(
                        f"Task {task_id} on {source_worker_id} likely to finish soon (est. {estimated_remaining:.1f}s remaining)"
                    )
                    continue
    ...
    def select_tasks_to_steal(
        self, opportunities: List[StealingOpportunity], max_steals: Optional[int] = None
    ) -> List[StealingOpportunity]:
    ...
    def execute_stealing(self, opportunity: StealingOpportunity, task_executor) -> bool:
    ...
    def _is_compatible(
        self, requirements: TestRequirements, capabilities: WorkerCapabilities, load: WorkerLoad
    ) -> bool:
    ...
    def _evaluate_stealing_opportunity(
        ...
        if self.config["enable_specialization_aware_stealing"] and performance_data:
            ...
            if target_worker_id in performance_data:
                ...
                if (
                    task_requirements.model_id
                    and task_requirements.model_id in worker_perf
                    and task_requirements.test_type in worker_perf[task_requirements.model_id]
                ):
                    target_perf = worker_perf[task_requirements.model_id][
                        task_requirements.test_type
                    ]
                ...
                # Check model family performance as fallback
                    ...
                    task_requirements.model_family
                    and "model_family" in worker_perf
                    and task_requirements.model_family in worker_perf["model_family"]
                ):
                ...
                # Check test type performance as fallback
                    ...
                    task_requirements.test_type
                    and "test_type" in worker_perf
                    and task_requirements.test_type in worker_perf["test_type"]
                ):
        ...
        migration_cost = self._estimate_migration_cost(
            task_requirements=task_requirements, elapsed_time=elapsed_time
        )
    ...
    def _estimate_execution_benefit(
        ...
        if performance_data:
            ...
            if target_worker_id in performance_data and source_worker_id in performance_data:
                ...
                if (not target_perf or not source_perf) and task_requirements.model_family:
                    ...
                    if "model_family" in target_data and "model_family" in source_data:
                        target_family = target_data["model_family"].get(
                            task_requirements.model_family
                        )
                        source_family = source_data["model_family"].get(
                            task_requirements.model_family
                        )
                ...
                if target_perf and source_perf and source_perf.average_execution_time > 0:
                    perf_ratio = (
                        source_perf.average_execution_time / target_perf.average_execution_time
                    )
        ...
        remaining_time_target = (
            max(0, total_expected_time - elapsed_time) * target_slowdown / perf_factor
        )
    ...
    def _estimate_migration_cost(
        self, task_requirements: TestRequirements, elapsed_time: float
    ) -> float:
        ...
        if hasattr(task_requirements, "memory_gb") and task_requirements.memory_gb > 0:
            ...
            complexity_factor = max(
                1.0, task_requirements.memory_gb / 2.0
            )  # Scale with memory (GB)
