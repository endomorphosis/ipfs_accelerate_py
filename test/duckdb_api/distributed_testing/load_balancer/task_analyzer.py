...
class TaskRequirementsAnalyzer:
    ...
    def analyze_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        final_requirements = self._adjust_for_task_parameters(
            adjusted_requirements, test_parameters
        )

        # Determine hardware preferences
        hardware_preferences = self._determine_hardware_preferences(
            model_family, test_type, test_parameters
        )

        # Determine browser preferences if this is a web test
        browser_preferences = self._determine_browser_preferences(
            model_family, test_type, test_parameters
        )

        # Predict execution time
        predicted_execution_time = self._predict_execution_time(
            model_id, model_family, test_type, test_parameters
        )
        ...
        analysis = {
            ...
            "specialized_hardware_required": self._requires_specialized_hardware(
                model_id, test_type, test_parameters
            ),
    ...
    def _determine_model_family(self, model_id: str, parameters: Dict[str, Any]) -> str:
        ...
        # Audio models
            ...
            x in model_id_lower
            for x in ["wav2vec", "whisper", "hubert", "clap", "audio", "speech", "mms"]
        ):
        ...
        # Multimodal models
            ...
            x in model_id_lower
            for x in ["blip", "flava", "flamingo", "pali", "llava", "qwen-vl", "fuyu"]
        ):
    ...
    def _determine_model_size(self, model_id: str, parameters: Dict[str, Any]) -> str:
        ...
        if any(
            x in model_id_lower for x in ["large", "-l", "-xl", "-xxl", "13b", "33b", "70b", "7b"]
        ):
            return "large"
        ...
        if "bert-base" in model_id_lower or "distilbert" in model_id_lower:
            ...
            return "medium"
        elif "gpt2" in model_id_lower and not any(
            x in model_id_lower for x in ["medium", "large", "xl"]
        ):
            return "small"
    ...
    def _get_base_requirements(
        self, model_family: str, model_size: str, test_type: str
    ) -> Dict[str, Any]:
        ...
        if (
            model_family in DEFAULT_REQUIREMENTS
            and model_size in DEFAULT_REQUIREMENTS[model_family]
        ):
    ...
    def _adjust_for_test_type(
        self, base_requirements: Dict[str, Any], test_type: str
    ) -> Dict[str, Any]:
    ...
    def _adjust_for_task_parameters(
        self, base_requirements: Dict[str, Any], parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _determine_hardware_preferences(
        self, model_family: str, test_type: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _determine_browser_preferences(
        self, model_family: str, test_type: str, parameters: Dict[str, Any]
        ...
        is_web_test = parameters.get("platform", "").lower() in [
            "web",
            "browser",
            "webnn",
            "webgpu",
        ...
        if "required_browser" in parameters:
            ...
            preferences = {
                k: (0.9 if k in required_browser else 0.1) for k, v in preferences.items()
            }
    ...
    def _predict_execution_time(
        self, model_id: str, model_family: str, test_type: str, parameters: Dict[str, Any]
    ) -> float:
        ...
        size_multipliers = {"small": 0.5, "medium": 1.0, "large": 3.0}
    ...
    def _requires_specialized_hardware(
        self, model_id: str, test_type: str, parameters: Dict[str, Any]
    ) -> bool:
    ...
    def _suggest_batch_size(
        ...
        family_defaults = default_batch_sizes.get(
            model_family, default_batch_sizes[MODEL_FAMILY_TEXT]
        )
        batch_size = family_defaults.get(test_type, family_defaults.get("default", 8))

        # Adjust for model size
        model_size = self._determine_model_size(model_id, parameters)
        size_multipliers = {"small": 2.0, "medium": 1.0, "large": 0.25}
...
def analyze_task_cli() -> None:
    ...
    parser.add_argument(
        "--parameters", type=str, default="{}", help="Task parameters in JSON format"
    )
    ...
    task = {"model_id": args.model, "test_type": args.test_type, "parameters": parameters}
