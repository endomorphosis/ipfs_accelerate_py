...
class LoadBalancerCoordinatorBridge:
    ...
    def register_worker(self, coordinator_worker_id: str, capabilities: Dict[str, Any]) -> str:
        ...
        with self.lock:
            ...
            self.load_balancer.update_worker_load(
                coordinator_worker_id, WorkerLoad(worker_id=coordinator_worker_id)
            )
    ...
    def get_test_status(self, coordinator_test_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            ...
            if not assignment:
                # Still pending
                return {"status": "pending", "worker_id": None, "assigned_at": None, "result": None}
    ...
    def update_assignment_status(
        self, coordinator_test_id: str, status: str, result: Optional[Dict[str, Any]] = None
    ) -> bool:
        ...
        with self.lock:
            try:
                ...
                for test_id, data in tests.items():
                    if (
                        test_id not in self.coordinator_to_lb_test_map
...
class CoordinatorClient:
    ...
    def notify_assignment_update(self, notification: Dict[str, Any]) -> None:
        ...
        with self.lock:
            ...
            logger.info(
                f"Notification sent to coordinator: Test {test_id} status {notification['status']}"
            )
