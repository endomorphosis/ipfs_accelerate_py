...
class PerformanceTracker:
        ...
        with self.lock:
            ...
            try:
                ...
                performance = WorkerPerformance(
                    ...
                    last_execution_time=datetime.fromisoformat(row[10])
                    if row[10]
                    else datetime.now(),
                    total_failures=int(row[6] * (1.0 - row[5]))
                    if row[6] and row[5] is not None
                    else 0,
                )
    ...
    def _cleanup_cache(self) -> None:
        ...
        if len(self.performance_cache) > self.max_cache_size:
            ...
            sorted_items = sorted(self.performance_cache.items(), key=lambda item: item[1][1])
