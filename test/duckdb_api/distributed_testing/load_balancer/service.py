...
class LoadBalancerService:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.active_assignments: Dict[
            str, Dict[str, WorkerAssignment]
        ] = {}  # worker_id -> test_id -> assignment
    ...
    def register_worker(self, worker_id: str, capabilities: WorkerCapabilities) -> None:
        ...
        with self.lock:
            ...
            logger.info(
                f"Registered worker {worker_id} with {len(capabilities.supported_backends)} backends"
            )
    ...
    def update_worker_load(self, worker_id: str, load: WorkerLoad) -> None:
        ...
        with self.lock:
            if worker_id in self.workers:
                ...
                logger.debug(
                    f"Updated load for worker {worker_id}: {load.calculate_load_score():.2f}"
                )
    ...
    def submit_test(self, test_requirements: TestRequirements) -> str:
        ...
        with self.lock:
            ...
            logger.info(
                f"Submitted test {test_requirements.test_id} with priority {test_requirements.priority}"
            )
    ...
    def update_assignment_status(
        self, test_id: str, status: str, result: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def rebalance(self) -> None:
        ...
        with self.lock:
            ...
            assignments_to_rebalance.sort(key=lambda a: a.test_requirements.priority, reverse=True)
    ...
    def _transfer_assignment(self, test_id: str, from_worker: str, to_worker: str) -> None:
        ...
        if to_worker in self.worker_loads:
            ...
            self.worker_loads[to_worker].reserve_resources(
                test_id, requirements, worker_capabilities
            )

        # Update active assignments
        if (
            from_worker in self.active_assignments
            and test_id in self.active_assignments[from_worker]
        ):
                ...
                try:
                    ...
                    if worker_id:
                        ...
                        assignment = WorkerAssignment(
                            worker_id=worker_id, test_id=test_id, test_requirements=requirements
                        )
                        ...
                        if worker_id in self.worker_loads:
                            ...
                            self.worker_loads[worker_id].reserve_resources(
                                test_id, requirements, worker_capabilities
                            )
                        ...
                        if requeue_count < self.max_requeue_attempts:
                            ...
                            if (
                                requirements.concurrency_key
                                and requirements.concurrency_key in self.concurrency_locks
                            ):
                                try:
                            ...
                            logger.warning(
                                f"No suitable worker for test {test_id}, requeued with priority {new_priority} (attempt {requeue_count}/{self.max_requeue_attempts})"
                            )
                        else:
                            ...
                            assignment.mark_completed(
                                False,
                                {"error": "Failed to find suitable worker after maximum attempts"},
                            )
                            ...
                            logger.error(
                                f"Test {test_id} failed after {requeue_count} scheduling attempts, no suitable worker found"
                            )
        ...
        with self.lock:
            ...
            for worker_id, capabilities in active_workers.items():
                ...
                if hasattr(worker_load, "browser_metrics"):
                    ...
                    worker_browser_metrics[worker_id] = getattr(
                        worker_load, "browser_capacities", {}
                    )
            ...
            logger.info(
                f"Work stealing: {len(idle_workers)} idle workers, {len(busy_workers)} busy workers"
            )
            ...
            if browser_aware_stealing:
                ...
                if overloaded_browsers and underutilized_browsers:
                    logger.info(
                        f"Browser-aware work stealing: overloaded={overloaded_browsers}, "
                        ...
                        if busy_worker in worker_browser_metrics:
                            ...
                            for browser in overloaded_browsers:
                                if browser in metrics:
                                    if (
                                        isinstance(metrics[browser], dict)
                                        and "utilization" in metrics[browser]
                                    ):
                ...
                busy_workers.sort(
                    key=lambda wid: self.worker_loads[wid].calculate_load_score(), reverse=True
                )

                # Sort idle workers by load (lowest first) when browser metrics not available
                idle_workers.sort(key=lambda wid: self.worker_loads[wid].calculate_load_score())
            ...
            for busy_worker in busy_workers:
                ...
                if browser_aware_stealing:
                    ...
                    for test_id, assignment in stealable_tests:
                        ...
                        model_type = (
                            test_req.model_type if hasattr(test_req, "model_type") else None
                        )
                    ...
                    stealable_tests = [
                        (test_id, assignment) for test_id, assignment, _ in enhanced_stealable_tests
                    ]
                else:
                    # Default sorting by priority (lowest priority first for stealing)
                    stealable_tests.sort(
                        key=lambda x: x[1].test_requirements.priority, reverse=True
                    )
                ...
                for test_id, assignment in stealable_tests:
                    ...
                    for idle_worker in idle_workers:
                        ...
                        if (
                            browser_aware_stealing
                            and model_type
                            and model_type in model_browser_affinity
                        ):
                            preferred_browser = model_browser_affinity[model_type]
                            ...
                            if (
                                ...
                                metrics = worker_browser_metrics[idle_worker]
                                browser_util = (
                                    metrics[preferred_browser]["utilization"]
                                ...
                                if (
                                    browser_util > 0.7
                                    and preferred_browser not in underutilized_browsers
                                ):
                        ...
                        if browser_aware_stealing and model_type:
                            logger.info(
                                f"Browser-aware work stealing: transferred {model_type} test {test_id} "
                                f"from {busy_worker} to {idle_worker}"
                            )
                        else:
                            logger.info(
                                f"Work stealing: transferred test {test_id} from {busy_worker} to {idle_worker}"
                            )
    ...
    def _calculate_adaptive_batch_size(self) -> int:
        ...
        active_worker_count = sum(
            1 for worker_id, status in self.worker_status.items() if status == "active"
        )
        ...
        logger.debug(
            f"Adaptive batch size: {batch_size} (load: {average_load:.2f}, workers: {active_worker_count})"
        )
