...
class WorkerCapabilityDetector:
    ...
    def _detect_supported_backends(self) -> List[str]:
        ...
        if HAS_ORT:
            backends.extend(
                [
                    provider.split("ExecutionProvider")[0].lower()
        ...
        if (
            ...
            try:
                ...
                result = subprocess.run(
                    ["rocm-smi", "--showcount"], capture_output=True, text=True, check=False
                )
                if result.returncode == 0:
                    count_line = [
                        line for line in result.stdout.splitlines() if "GPU count" in line
                    ]
                    if count_line:
