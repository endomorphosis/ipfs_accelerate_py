...
try:
    ...
    logger.warning(
        "Flask or SocketIO not available. Install with 'pip install flask flask-cors flask-socketio'"
    )
...
class DashboardServer:
    ...
    def _setup_routes(self):
        ...
        def get_system_metrics_history():
            ...
            for metric, values in metrics_data.items():
                formatted_data[metric] = [
                    {"timestamp": ts.isoformat(), "value": val} for ts, val in values
                ]
        ...
        def get_worker_performance(worker_id):
            ...
            return jsonify({"worker_id": worker_id, "performance_score": score})
        ...
        def get_task_stats():
            ...
            status_counts = {"queued": 0, "assigned": 0, "running": 0, "completed": 0, "failed": 0}
    ...
    def _setup_socketio(self):
        ...
        def handle_unsubscribe(data):
            ...
            if (
                ...
                and metric_type in self.client_subscriptions[client_id]
            ):
    ...
    def _register_callbacks(self):
        ...
        def system_metrics_callback(timestamp, metrics):
            self._emit_to_subscribers(
                "system", {"timestamp": timestamp.isoformat(), "metrics": metrics}
            )

        # Worker metrics callback
        def worker_metrics_callback(timestamp, metrics):
            self._emit_to_subscribers(
                "worker", {"timestamp": timestamp.isoformat(), "metrics": metrics}
            )

        # Task metrics callback
        def task_metrics_callback(timestamp, metrics):
            self._emit_to_subscribers(
                "task", {"timestamp": timestamp.isoformat(), "metrics": metrics}
            )
    ...
    def start(self):
        ...
        if (
            not getattr(self.metrics_collector, "collection_thread", None)
            or not self.metrics_collector.collection_thread.is_alive()
        ):
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, default="metrics.duckdb", help="Path to metrics database"
    )
    parser.add_argument(
        "--static-folder", type=str, help="Folder containing static dashboard files"
    )
    parser.add_argument(
        "--collection-interval",
        type=float,
    ...
    metrics_collector = MetricsCollector(
        db_path=args.db_path, collection_interval=args.collection_interval
    )
