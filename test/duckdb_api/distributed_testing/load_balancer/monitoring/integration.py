...
from data.duckdb.distributed_testing.load_balancer.monitoring.dashboard_server import (
    DashboardServer,
)
...
class MonitoringIntegration:
    ...
    def __init__(
        ...
        self.metrics_collector = MetricsCollector(
            db_path=db_path, collection_interval=collection_interval
        )

        # Set metrics sources
        self.metrics_collector.set_sources(load_balancer=load_balancer, coordinator=coordinator)
    ...
    def set_sources(self, load_balancer=None, coordinator=None):
        ...
        self.metrics_collector.set_sources(
            load_balancer=self.load_balancer, coordinator=self.coordinator
        )
...
def integrate_monitoring(
    load_balancer=None, coordinator=None, dashboard_port: int = 5000, start: bool = True
    ...
    integration = MonitoringIntegration(
        load_balancer=load_balancer, coordinator=coordinator, dashboard_port=dashboard_port
    )
...
def integrate_with_coordinator_server(
    coordinator_server, dashboard_port: int = 5000, start: bool = True
) -> MonitoringIntegration:
    """
...
def main():
    ...
    parser.add_argument(
        "--dashboard-port", type=int, default=5000, help="Port to bind the dashboard server to"
    )
    parser.add_argument(
