...
class MetricsCollector:
    ...
    def __init__(
        ...
        self._callbacks = {MetricType.SYSTEM: [], MetricType.WORKER: [], MetricType.TASK: []}
    ...
    def _collect_worker_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        if self._coordinator:
            ...
            for worker_id, worker in coordinator.workers.items():
                ...
                worker_metrics["memory_utilization"] = float(
                    metrics_data.get("memory_utilization", 0.0)
                )
                worker_metrics["gpu_utilization"] = float(metrics_data.get("gpu_utilization", 0.0))
                worker_metrics["disk_utilization"] = float(
                    metrics_data.get("disk_utilization", 0.0)
                )
    ...
    def get_historical_worker_metrics(
        self,
        worker_id: str,
        metric_names: List[str],
        start_time: Optional[datetime.datetime] = None,
    ...
    def get_worker_performance_score(self, worker_id: str) -> float:
        ...
        try:
            ...
            AND timestamp > ?
            """,
                [worker_id, datetime.datetime.now() - datetime.timedelta(hours=24)],
            ).fetchone()
            ...
            if results and results[1] > 0:
                ...
                if system_avg and system_avg[0]:
                    ...
                    time_score = min(
                        100, max(0, 100 * (system_avg_time / max(0.1, avg_execution_time)))
                    )
                    ...
                    if avg_tasks and avg_tasks[0]:
                        ...
                        task_count_score = min(
                            100, max(0, 100 * (task_count / max(1, avg_tasks_per_worker)))
                        )
                    else:
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Check throughput
            if (
                system_metrics.get("throughput", 0) < 1
                and system_metrics.get("queued_tasks", 0) > 5
            ):
                anomalies.append(
                    {
