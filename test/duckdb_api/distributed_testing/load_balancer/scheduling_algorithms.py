    ...
    def select_worker(
        ...
        for worker_id in self.worker_ids:
            if self._is_compatible(
                test_requirements, available_workers[worker_id], worker_loads.get(worker_id)
            ):
...
class PerformanceBasedScheduler(SchedulingAlgorithm):
    ...
    def _calculate_performance_score(
        ...
        if perf:
            ...
            time_score = max(
                0.0, min(1.0, 1.0 - (time_ratio - 1.0) / 3.0)
...
class AffinityBasedScheduler(SchedulingAlgorithm):
    ...
    def select_worker(
        ...
        if (
            test_requirements.model_family
            and test_requirements.model_family in self.family_affinities
        ):
