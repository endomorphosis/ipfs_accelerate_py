...
class MockCoordinator:
    ...
    def connect_worker(self, worker_id, worker_ws):
        ...
        self.send_message(
            worker_id,
            {"type": "welcome", "worker_id": worker_id, "timestamp": datetime.now().isoformat()},
        )
...
class TestWorkerReconnectionManager(unittest.TestCase):
    ...
    def test_configure(self):
        ...
        config_updates = {"heartbeat_interval": 10.0, "max_reconnect_delay": 120.0}
    ...
    def test_checkpoint_creation(self):
        ...
        self.assertEqual(
            self.manager.task_states[task_id]["last_checkpoint"]["checkpoint_id"], checkpoint_id
        )
...
class TestWorkerReconnectionIntegration(unittest.TestCase):
    ...
    def test_state_synchronization(self, mock_ws_app_class):
        ...
        self.manager.task_states[task_id] = {"status": "running", "progress": 50}
    ...
    def test_task_resumption_from_checkpoint(self, mock_ws_app_class):
        ...
        for call in mock_ws.send.call_args_list:
            ...
            message = json.loads(args[0])
            if (
                message.get("type") == "checkpoint_data"
                and message.get("checkpoint_id") == checkpoint_id
            ):
    ...
    def test_message_delivery_reliability(self, mock_ws_app_class):
        ...
        message1 = {"type": "test_message", "content": "Message 1"}
        ...
        message2 = {"type": "test_message", "content": "Message 2"}
    ...
    def test_interaction_with_hardware_fault_tolerance(self, mock_ws_app_class):
        ...
        mock_hardware_recovery.get_recovery_actions.return_value = [RecoveryAction.RESTART_PROCESS]
        ...
        self.manager.update_task_state(
            task_id, {"status": "failed", "hardware_error": hardware_error}
        )
        ...
        for call in mock_ws.send.call_args_list:
            ...
            try:
                ...
                if (
                    ...
                    and "hardware_error" in message.get("state", {})
                ):
...
class TestWorkerReconnectionPlugin(unittest.TestCase):
    ...
    def test_plugin_api_passthrough(self):
        ...
        # Create plugin with mocked reconnection manager
        with patch(
            "duckdb_api.distributed_testing.worker_reconnection.WorkerReconnectionManager",
    ...
    def test_create_worker_reconnection_plugin(self):
        ...
        with patch(
            "duckdb_api.distributed_testing.worker_reconnection.WorkerReconnectionPlugin",
