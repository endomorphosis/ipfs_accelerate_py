...
class TestPerformanceTrendAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.analyzer = PerformanceTrendAnalyzer(
            db_manager=self.db_manager, task_scheduler=self.task_scheduler
        )
    ...
    def test_update_time_series(self):
        ...
        self.task_scheduler.worker_performance = {
            "worker1": {
                ...
                "task_types": {
                    "benchmark": {"success_rate": 0.8, "avg_execution_time": 5.0, "task_count": 100}
                },
            }
        }
    ...
    def test_prune_time_series(self):
        ...
        self.analyzer.time_series["worker1"] = {
            ...
            "avg_execution_time": [(current_time, 5.0)],
        }
        ...
        self.assertEqual(
            self.analyzer.task_time_series["benchmark"]["success_rate"][0][0], current_time
        )
    ...
    def test_compute_baselines(self):
        ...
        self.analyzer.time_series["worker1"] = {
            "success_rate": [(current_time - timedelta(minutes=5 * i), 0.8) for i in range(6)],
            "avg_execution_time": [
        ...
        self.analyzer.time_series["worker2"] = {"success_rate": [(current_time, 0.9)]}
        ...
        self.assertEqual(
            self.analyzer.worker_baselines["worker1"]["avg_execution_time"]["mean"], 5.0
        )
    ...
    def test_detect_anomalies(self):
        ...
        self.analyzer.time_series["worker1"] = {
            "success_rate": [(current_time - timedelta(minutes=5 * i), 0.8) for i in range(5)]
            + [
                (current_time, 0.5)  # Anomaly: 6 std devs below mean
            ],
            "avg_execution_time": [(current_time - timedelta(minutes=5 * i), 5.0) for i in range(5)]
            + [
        ...
        anomaly = next(
            a for a in self.analyzer.anomalies["worker1"] if a["metric"] == "success_rate"
        )
        ...
        anomaly = next(
            a for a in self.analyzer.anomalies["worker1"] if a["metric"] == "avg_execution_time"
        )
    ...
    def test_export_data(self):
        ...
        # Add time series data
        self.analyzer.time_series["worker1"] = {
            "success_rate": [(current_time - timedelta(days=i), 0.8 - 0.02 * i) for i in range(3)]
        }
