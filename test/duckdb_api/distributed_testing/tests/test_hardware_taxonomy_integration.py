...
from data.duckdb.distributed_testing.hardware_taxonomy_integrator import HardwareTaxonomyIntegrator
...
class TestHardwareTaxonomyIntegration(unittest.TestCase):
    ...
    def create_test_worker(
        self, worker_id: str, hardware_class: str, capabilities: Set[str] = None
    ) -> Dict[str, Any]:
    ...
    def create_test_task(
        ...
        # Create task
        return TestTask(task_id=task_id, workload_profile=profile, priority=2, batch_size=8)
    ...
    def test_register_worker_with_capabilities(self):
        ...
        worker = self.scheduler.register_worker(
            worker_data["worker_id"], worker_data["capabilities"]
        )

        # Check that worker has been enhanced
        self.assertTrue(
            hasattr(worker, "capability_profiles"),
        ...
        self.assertTrue(has_matrix_mult, "GPU worker should have matrix_multiplication capability")
    ...
    def test_submit_task_with_capabilities(self):
        ...
        self.assertTrue(
            len(task.workload_profile.required_capabilities) > 0
            or len(task.workload_profile.preferred_capabilities) > 0,
    ...
    def test_capability_based_scheduling(self):
        ...
        gpu_worker_state = self.scheduler.register_worker(
            gpu_worker["worker_id"], gpu_worker["capabilities"]
        )

        cpu_worker_state = self.scheduler.register_worker(
            cpu_worker["worker_id"], cpu_worker["capabilities"]
        )
    ...
    def test_capability_breakdown(self):
        ...
        self.assertGreaterEqual(
            nlp_impacts["tensor_core_acceleration"],
            0.5,
