...
class TestResourcePerformancePredictor(unittest.TestCase):
    ...
    def test_create_database(self):
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='prediction_models'"
        )
        self.assertIsNotNone(cursor.fetchone())

        # Check if task_execution_features table exists
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='task_execution_features'"
        )
        self.assertIsNotNone(cursor.fetchone())
    ...
    def test_record_task_execution(self):
        ...
        self.predictor.record_task_execution(
            task_data=self.task_data, resource_usage=self.resource_usage, success=True
        )
        ...
        cursor.execute(
            "SELECT cpu_cores, memory_mb, gpu_memory_mb, execution_time_seconds FROM task_executions"
        )
        row = cursor.fetchone()
    ...
    @patch(
        "resource_performance_predictor.ResourcePerformancePredictor._get_statistical_prediction"
    )
    ...
    def test_calculate_batch_scaling_factor(self):
        ...
        for batch_size in [1, 2, 4, 8, 16]:
            ...
            self.predictor.record_task_execution(
                task_data=task_data, resource_usage=resource_usage, success=True
            )
    ...
    def test_extract_task_features(self):
        ...
        different_task = {
            ...
            "requirements": {"hardware": ["cpu"], "min_memory_gb": 16},
        }
