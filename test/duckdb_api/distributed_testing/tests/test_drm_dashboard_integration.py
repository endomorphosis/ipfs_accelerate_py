...
try:
    from data.duckdb.distributed_testing.dynamic_resource_management_visualization import (
        DRMVisualization,
    )
...
class TestDRMDashboardIntegration(AioHTTPTestCase):
    ...
    async def get_application(self):
        ...
        self.drm_integration = DRMVisualizationIntegration(
            output_dir=self.viz_dir, update_interval=1, resource_manager=self.mock_resource_manager
        )
        ...
        self.drm_integration.drm_visualization.create_resource_allocation_visualization.return_value = os.path.join(
            self.viz_dir, "resource_allocation.png"
        )
        self.drm_integration.drm_visualization.create_resource_efficiency_visualization.return_value = os.path.join(
            self.viz_dir, "resource_efficiency.png"
        )
