...
def run_tests(test_pattern=None, verbose=False):
    ...
    if test_pattern:
        ...
        # Add all tests
        suite.addTests(
            unittest.TestLoader().loadTestsFromTestCase(TestVisualizationDashboardIntegration)
        )
        suite.addTests(
            unittest.TestLoader().loadTestsFromTestCase(TestMonitoringDashboardWithVisualization)
        )
...
def main():
    ...
    parser.add_argument(
        "--create-dummy-data", action="store_true", help="Create dummy data for tests"
    )
