...
from data.duckdb.distributed_testing.coordinator_load_balancer_integration import (
...
class LoadBalancerFaultToleranceTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.coordinator_thread = threading.Thread(
            target=cls._run_coordinator_with_load_balancer, daemon=True
        )
    ...
    def _run_coordinator_with_load_balancer(cls):
        ...
        try:
            ...
            async def coordinator_task():
                ...
                from data.duckdb.distributed_testing.coordinator_patch import (
                    apply_patches,
                    remove_patches,
                )
    ...
    def _start_workers(cls, count=6):
        ...
        default_capabilities = [
            ...
            {"hardware_types": ["cpu"], "cpu_cores": 64, "memory_gb": 512},
        ]
        ...
        for i in range(count):
            ...
            print(
                f"Starting worker {worker_id} with capabilities: {default_capabilities[capability_index]}"
            )
    ...
    def _kill_worker(self, worker_index):
        ...
        if worker_index >= len(self.worker_processes):
            raise ValueError(
                f"Worker index {worker_index} out of range (0-{len(self.worker_processes) - 1})"
            )
    ...
    def _restart_worker(self, worker_id, capabilities=None):
        ...
        if capabilities is None:
            capabilities = {"hardware_types": ["cpu"], "memory_gb": 4}
        ...
        for task_id, worker_id in task_assignments.items():
            ...
            if "requirements" in task and "hardware" in task["requirements"]:
                if "cuda" in task["requirements"]["hardware"]:
                    ...
                    self.assertIn(
                        "cuda",
                        worker["capabilities"]["hardware_types"],
    ...
    def test_06_worker_recovery(self):
        ...
        worker_index, worker_id, affected_tasks = worker_to_kill
        capabilities = next(
            (w["capabilities"] for w in self.worker_processes if w["worker_id"] == worker_id), None
        )
        print(f"Worker {worker_id} has {len(affected_tasks)} tasks: {affected_tasks}")
        ...
        worker = self.coordinator.worker_manager.get_worker(worker_id)
        self.assertEqual(
            worker["status"], "active", f"Worker {worker_id} should be active after reconnection"
        )
        ...
        self.assertGreater(
            ...
            0,
            "No affected tasks are running or completed after worker recovery",
        )
    ...
    def test_07_simultaneous_worker_failures(self):
        ...
        self.assertGreater(
            ...
            0,
            "No tasks are running or completed after multiple worker failures",
        )
    ...
    def test_08_work_stealing_after_recovery(self):
        ...
        most_loaded_worker = max(
            initial_distribution.items(), key=lambda x: len(x[1]), default=(None, [])
        )
        ...
        worker_cmd = [
            ...
            "3",
            "--capabilities",
            json.dumps(high_capacity_capabilities),
        ]

        print(
            f"Starting high-capacity worker {new_worker_id} with capabilities: {high_capacity_capabilities}"
        )
        process = subprocess.Popen(worker_cmd)
        self.worker_processes.append(
            {
        ...
        self.assertGreaterEqual(
            len(new_distribution.get(new_worker_id, [])),
            1,
    ...
    def test_09_fault_tolerant_task_completion(self):
        ...
        self.assertGreaterEqual(
            completed_count, 7, f"Only {completed_count} out of 10 quick tasks completed"
        )
    ...
    def test_10_resiliency_to_repeated_failures(self):
        ...
        self.assertGreater(completed_count, 0, f"No tasks completed after repeated worker failures")
