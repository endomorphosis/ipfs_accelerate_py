...
def cleanup_containers():
    ...
    for container in [PROMETHEUS_CONTAINER, GRAFANA_CONTAINER]:
        try:
            subprocess.run(
                ["docker", "rm", "-f", container], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
...
def create_prometheus_config(metrics_port, config_path):
    ...
    prometheus_config = {
        ...
        "scrape_configs": [
            {
                "job_name": "drm",
                "static_configs": [{"targets": [f"host.docker.internal:{metrics_port}"]}],
            }
        ],
    }
...
def start_prometheus_container(config_path, port):
    ...
    cmd = [
        ...
        f"{port}:9090",
        "-v",
        f"{config_path}:/etc/prometheus/prometheus.yml",
        "--add-host",
...
def configure_grafana_prometheus_datasource(grafana_port, prometheus_port):
    ...
    try:
        response = requests.post(
            f"http://localhost:{grafana_port}/api/datasources", json=datasource
        )
...
def verify_grafana_dashboard_import(grafana_port, dashboard_path):
    ...
    try:
        ...
        # Import dashboard
        response = requests.post(
            f"http://localhost:{grafana_port}/api/dashboards/import", json=dashboard_import
        )
...
"""
        ...
        subprocess.run(
            [
                "docker",
                "cp",
                alert_rule_file.name,
                f"{PROMETHEUS_CONTAINER}:/etc/prometheus/alert.yml",
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
...
"""
        ...
        subprocess.run(
            [
                ...
                "cp",
                config_file.name,
                f"{PROMETHEUS_CONTAINER}:/etc/prometheus/prometheus.yml",
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
...
class TestDRMExternalMonitoringE2E(unittest.TestCase):
    ...
    def test_04_metrics_validation(self):
        ...
        for metric in metrics_to_check:
            response = requests.get(
                f"http://localhost:{TEST_PROMETHEUS_PORT}/api/v1/query", params={"query": metric}
            )
