...
def create_summary_visualization(
    report: Dict, output_dir: str, test_id: str, theme: str = "light"
) -> str:
    ...
    fig.add_trace(
        go.Pie(
            ...
            values=values,
            marker_colors=[
                theme_config["success_color"] if v else theme_config["failure_color"]
                for v in values
            ],
...
def create_component_visualization(
    ...
    component_status = {
        "Result Aggregator": validation_results.get("result_aggregation", {}).get(
            "summary_accessible", False
        ),
        "Coordinator": True,  # Assuming coordinator is working if the test ran
        "Monitoring Dashboard": validation_results.get("dashboard", {}).get(
            "dashboard_accessible", False
        ),
        "Worker Nodes": True,  # Assuming workers are functioning if the test ran
        "Integration System": validation_results.get("integration", {}).get(
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker_color=[
                theme_config["success_color"] if status else theme_config["failure_color"]
    ...
    for i, (component, status) in enumerate(component_status.items()):
        if component == "Result Aggregator":
            has_data = validation_results.get("result_aggregation", {}).get(
            ...
            dashboard_content = validation_results.get("dashboard", {}).get(
                "dashboard_content_length", 0
            )
            fig.add_annotation(
                x=1.1,
                y=i,
...
def create_timing_visualization(
    ...
    timing_data = {
        ...
        "inject_failures": report.get("configuration", {}).get("include_failures", False)
        and 15
        or None,
        "failures_injected": report.get("configuration", {}).get("include_failures", False)
        and 20
        or None,
        "test_execution": 12,
    ...
    phases = [
        {
            "Task": "Start Services",
            "Start": timing_data["start_services"],
            "Finish": timing_data["services_started"],
            "Resource": "Infrastructure",
        },
        {
            "Task": "Start Workers",
            "Start": timing_data["start_workers"],
            "Finish": timing_data["workers_started"],
            "Resource": "Infrastructure",
        },
        {
            "Task": "Submit Workloads",
            "Start": timing_data["submit_workloads"],
            "Finish": timing_data["workloads_submitted"],
            "Resource": "Workload",
        },
    ]
        ...
        phases.append(
            {
                ...
                "Start": timing_data["inject_failures"],
                "Finish": timing_data["failures_injected"],
    ...
    phases.extend(
        [
            {
                "Task": "Test Execution",
                "Start": timing_data["test_execution"],
                "Finish": timing_data["test_completed"],
                "Resource": "Execution",
            },
            {
                "Task": "Validation",
                "Start": timing_data["validation_started"],
                "Finish": timing_data["validation_completed"],
                "Resource": "Validation",
            },
            {
                "Task": "Cleanup",
                "Start": timing_data["cleanup_started"],
                "Finish": timing_data["cleanup_completed"],
                "Resource": "Infrastructure",
            },
        ]
    )
...
def create_failure_visualization(
    ...
    if not include_failures:
        ...
        failures = [
            ...
            {
                "type": "Malformed Request (Coordinator)",
                "time": 20,
                "duration": 1,
                "recovery": "Automatic",
                "impact": "Low",
            ...
            {
                "type": "Malformed Request (Result Aggregator)",
                "time": 22,
                "duration": 1,
                "recovery": "Automatic",
                "impact": "Low",
        ...
        fig = make_subplots(
            rows=2,
            cols=1,
            subplot_titles=("Injected Failures", "Recovery Timeline"),
        ...
        for f in failures:
            ...
            sizes.append(15)
            hover_texts.append(
                f"Failure: {f['type']}<br>Time: {f['time']}s<br>Impact: {f['impact']}"
            )
            ...
            sizes.append(15)
            hover_texts.append(
                f"Recovery: {f['type']}<br>Time: {recovery_time}s<br>Method: {f['recovery']}"
            )
        ...
        fig.update_yaxes(
            showticklabels=False, showgrid=False, zeroline=False, range=[-0.1, 1.1], row=2, col=1
        )
...
def integrate_with_dashboard(
    ...
    try:
        ...
        response = requests.post(f"{dashboard_url}/api/e2e-test-results", json=data)
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to integrate with dashboard: HTTP {response.status_code} - {response.text}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--report-dir",
        type=str,
        default="./e2e_test_reports",
        help="Directory containing test reports",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./e2e_visualizations",
        help="Directory for generated visualizations",
    )
    parser.add_argument(
        "--test-id", type=str, default="latest", help="Specific test ID to visualize"
    )
    parser.add_argument(
        "--visualization-types",
        type=str,
    ...
    parser.add_argument(
        "--generate-standalone", action="store_true", help="Generate standalone HTML visualizations"
    )
    parser.add_argument(
        "--theme",
        type=str,
    ...
    parser.add_argument(
        "--open-browser", action="store_true", help="Open the dashboard in a web browser when done"
    )
    ...
    if "component" in visualization_types:
        ...
        component_file = create_component_visualization(
            report, args.output_dir, test_id, args.theme
        )
    ...
    if (
        args.generate_standalone
        and "summary" in generated_files
        and "component" in generated_files
        and "timing" in generated_files
        and "failures" in generated_files
    ):
