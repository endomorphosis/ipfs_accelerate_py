...
class MockCoordinator:
    ...
    def __init__(self):
        ...
        self.tasks["task1"] = {
            ...
            "type": "test",
            "timeout_seconds": 600,
            "requirements": {"hardware": ["cuda"], "min_cuda_compute": 7.0, "min_memory_gb": 4.0},
        }
...
class TestCoordinatorErrorIntegration(unittest.TestCase):
    ...
    def test_integration_adds_error_handler(self):
        ...
        self.assertIsInstance(
            self.coordinator_with_error_handler.error_handler, DistributedErrorHandler
        )
    ...
    def test_execute_recovery_action_resource(self):
        ...
        # Mock implementation
        with patch(
            "duckdb_api.distributed_testing.coordinator_error_integration.request_resource_cleanup"
        ) as mock_action:
            mock_action.return_value = True

            # Test resource cleanup action
            result = execute_recovery_action(
                self.coordinator, "request_resource_cleanup", None, "worker1"
            )
    ...
    def test_execute_recovery_action_hardware(self):
        ...
        # Mock implementation
        with patch(
            "duckdb_api.distributed_testing.coordinator_error_integration.mark_hardware_unavailable"
        ) as mock_action:
            mock_action.return_value = True

            # Test hardware action
            result = execute_recovery_action(
                self.coordinator, "mark_hardware_unavailable:cuda:worker1", None, "worker1"
            )
    ...
    def test_mark_hardware_unavailable(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.coordinator.workers["worker1"]["hardware_status"]["cuda"], "unavailable"
        )
    ...
    def test_reallocate_to_alternative_hardware(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.coordinator.tasks["task1"]["requirements"]["hardware"],
            ["rocm", "mps", "webgpu", "cpu", "cuda"],
        )
