...
def run_e2e_tests(quick=False, include_failures=False, debug=False):
    ...
    cmd = [sys.executable, "-m", "duckdb_api.distributed_testing.tests.run_e2e_tests"]
...
def main():
    ...
    parser.add_argument(
        "--dashboard-only", action="store_true", help="Start only the dashboard, not the tests"
    )
    parser.add_argument(
        "--monitoring-only", action="store_true", help="Start only the real-time monitoring"
    )
    parser.add_argument(
        "--visualization-only", action="store_true", help="Start only the visualization"
    )
    ...
    parser.add_argument(
        "--include-failures", action="store_true", help="Include fault tolerance tests"
    )
