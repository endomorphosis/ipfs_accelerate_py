...
class MockCoordinator:
    ...
    def retry_task(self, task_id, worker_id=None, exclude_workers=None):
        ...
        self.retried_tasks.append(
            {"task_id": task_id, "worker_id": worker_id, "exclude_workers": exclude_workers}
        )
        return True
    ...
    def escalate_task(self, task_id, message):
        ...
        self.escalated_tasks.append({"task_id": task_id, "message": message})
        return True
...
class HardwareAwareFaultToleranceTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.task_cpu = self.coordinator.add_task(
            "task_cpu_1", {"batch_size": 4, "precision": "fp32"}, {"hardware": ["cpu"]}
        )

        self.task_gpu = self.coordinator.add_task(
            "task_gpu_1", {"batch_size": 8, "precision": "fp16"}, {"hardware": ["cuda"]}
        )
        ...
        self.worker_cpu = self.coordinator.add_worker(
            "worker_cpu_1", create_hardware_profile("CPU")
        )

        self.worker_gpu = self.coordinator.add_worker(
            "worker_gpu_1", create_hardware_profile("GPU")
        )

        self.worker_webgpu = self.coordinator.add_worker(
            "worker_webgpu_1", create_hardware_profile("WEBGPU")
        )

        # Create fault tolerance manager for testing
        self.manager = HardwareAwareFaultToleranceManager(
            db_manager=self.db_manager, coordinator=self.coordinator
        )
    ...
    def test_handle_failure_cpu(self):
        ...
        # Handle the failure
        recovery_action = self.manager.handle_failure(self.task_cpu, self.worker_cpu, error_info)
    ...
    def test_handle_failure_gpu(self):
        ...
        # Handle the failure
        recovery_action = self.manager.handle_failure(self.task_gpu, self.worker_gpu, error_info)
    ...
    def test_reduced_precision(self):
        ...
        task_id = self.coordinator.add_task("fp32_task", {"precision": "fp32", "batch_size": 4})
        ...
        task_id = self.coordinator.add_task("fp16_task", {"precision": "fp16", "batch_size": 4})
        modified_task = self.manager._create_reduced_precision_task(task_id)
    ...
    def test_reduced_batch_size(self):
        ...
        task_id = self.coordinator.add_task("batch_task", {"batch_size": 16})
        ...
        task_id = self.coordinator.add_task("batch_sizes_task", {"batch_sizes": [16, 32, 64]})
        modified_task = self.manager._create_reduced_batch_task(task_id)
    ...
    def test_create_checkpoint(self):
        ...
        # Create a checkpoint
        checkpoint_data = {"progress": 50, "completed_steps": 10, "results": [1, 2, 3, 4, 5]}
    ...
    def test_failure_pattern_detection(self):
        ...
        error_info = {"message": "CUDA error: device-side assert triggered", "type": "cuda_error"}

        # Create a second GPU task
        task_gpu_2 = self.coordinator.add_task(
            "task_gpu_2", {"batch_size": 8, "precision": "fp16"}, {"hardware": ["cuda"]}
        )
        ...
        task_gpu_3 = self.coordinator.add_task(
            "task_gpu_3", {"batch_size": 8, "precision": "fp16"}, {"hardware": ["cuda"]}
        )
    ...
    def test_apply_recovery_action(self):
        ...
        action = RecoveryAction(
            strategy=RecoveryStrategy.DIFFERENT_WORKER, message="Test recovery action"
        )

        # Apply the action
        success = apply_recovery_action(self.task_cpu, action, coordinator=self.coordinator)
        ...
        success = apply_recovery_action(self.task_webgpu, action, coordinator=self.coordinator)
        ...
        # Test escalation
        action = RecoveryAction(
            strategy=RecoveryStrategy.ESCALATION, message="Escalate to human operator"
        )
    ...
    def test_max_retries(self):
        ...
        task_id = self.coordinator.add_task(
            "max_retry_task", {"batch_size": 4}, retry_count=self.manager.config["max_retries"]
        )
    ...
    def test_helper_functions(self):
        ...
        manager = create_recovery_manager(coordinator=self.coordinator, db_manager=self.db_manager)
    ...
    def test_state_persistence(self):
        ...
        # Create some state
        self.manager.handle_failure(self.task_cpu, self.worker_cpu, {"message": "Test error 1"})

        self.manager.handle_failure(self.task_gpu, self.worker_gpu, {"message": "Test error 2"})
