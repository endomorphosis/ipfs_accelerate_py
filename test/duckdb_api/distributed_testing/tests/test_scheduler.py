...
class TaskSchedulerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rocm_worker = {
            ...
            "capabilities": {"hardware_types": ["cpu", "rocm"], "memory_gb": 16},
        }
        ...
        for worker in [self.cpu_worker, self.cuda_worker, self.rocm_worker, self.browser_worker]:
            self.assertTrue(
                self.task_scheduler._worker_meets_requirements(
                    worker["capabilities"], cpu_task["requirements"]
                ),
    ...
    def test_cuda_task_matching(self):
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.cpu_worker["capabilities"], cuda_task["requirements"]
            ),
        ...
        self.assertTrue(
            self.task_scheduler._worker_meets_requirements(
                self.cuda_worker["capabilities"], cuda_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.rocm_worker["capabilities"], cuda_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.browser_worker["capabilities"], cuda_task["requirements"]
            ),
    ...
    def test_browser_task_matching(self):
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.cpu_worker["capabilities"], browser_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.cuda_worker["capabilities"], browser_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.rocm_worker["capabilities"], browser_task["requirements"]
            ),
        ...
        self.assertTrue(
            self.task_scheduler._worker_meets_requirements(
                self.browser_worker["capabilities"], browser_task["requirements"]
            ),
    ...
    def test_memory_requirement_matching(self):
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.cpu_worker["capabilities"], high_memory_task["requirements"]
            ),
        ...
        self.assertTrue(
            self.task_scheduler._worker_meets_requirements(
                self.cuda_worker["capabilities"], high_memory_task["requirements"]
            ),
        ...
        self.assertTrue(
            self.task_scheduler._worker_meets_requirements(
                self.rocm_worker["capabilities"], high_memory_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.browser_worker["capabilities"], high_memory_task["requirements"]
            ),
    ...
    def test_cuda_compute_requirement_matching(self):
        ...
        self.assertTrue(
            self.task_scheduler._worker_meets_requirements(
                self.cuda_worker["capabilities"], cuda_compute_task["requirements"]
            ),
        ...
        self.assertFalse(
            self.task_scheduler._worker_meets_requirements(
                self.cuda_worker["capabilities"], high_cuda_compute_task["requirements"]
            ),
    ...
    def test_priority_based_scheduling(self):
        ...
        self.task_scheduler.add_task(
            "low_priority", "test", 10, {"test_type": "low_priority_test"}, {"hardware": ["cpu"]}
        )
        ...
        self.task_scheduler.add_task(
            "high_priority", "test", 1, {"test_type": "high_priority_test"}, {"hardware": ["cpu"]}
        )

        # Get next task (should be high priority)
        next_task = self.task_scheduler.get_next_task("cpu_worker", self.cpu_worker["capabilities"])

        self.assertIsNotNone(next_task, "Should have a task to assign")
        self.assertEqual(
            next_task["task_id"], "high_priority", "High priority task should be assigned first"
        )
        ...
        self.assertIsNotNone(next_task, "Should have a task to assign")
        self.assertEqual(
            next_task["task_id"], "low_priority", "Low priority task should be assigned last"
        )
