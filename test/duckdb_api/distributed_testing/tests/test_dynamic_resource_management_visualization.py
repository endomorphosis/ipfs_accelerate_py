...
class TestDRMVisualization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.drm.last_scaling_decision = ScalingDecision(
            action="scale_up", reason="Test scaling decision", count=2, utilization=0.85
        )
    ...
    def _generate_test_data(self):
        ...
        self.drm.reserve_resources(
            task_id="task-1",
            resource_requirements={"cpu_cores": 2, "memory_mb": 4096, "gpu_memory_mb": 2048},
        )

        self.drm.reserve_resources(
            task_id="task-2", resource_requirements={"cpu_cores": 1, "memory_mb": 2048}
        )

        # Add a scaling history entry
        scaling_history_entry = {
            "timestamp": datetime.now(),
            "decision": {"action": "scale_up", "reason": "Testing visualization", "count": 2},
        }
        ...
        self.visualization.cloud_usage_history = {
            "aws": {
                ...
                "cost": [
                    {"timestamp": datetime.now() - timedelta(minutes=10), "cost": 1.50},
                    {"timestamp": datetime.now(), "cost": 2.25},
                ],
            },
            "gcp": {
                "workers": [
                    ...
                    {"timestamp": datetime.now(), "count": 1, "workers": ["worker-4"]},
                ],
                "cost": [
                    {"timestamp": datetime.now() - timedelta(minutes=10), "cost": 0.75},
                    {"timestamp": datetime.now(), "cost": 0.75},
                ],
            },
        }
        ...
        with patch("os.path.join", return_value="mock_path.html"):
            with patch("os.path.basename", return_value="mock_file.html"):
                ...
                test_viz = DRMVisualization(
                    dynamic_resource_manager=self.drm, output_dir=self.temp_dir, interactive=False
                )
                ...
                with patch.object(
                    test_viz, "create_resource_utilization_heatmap", return_value="heatmap.png"
                ):
                    with patch.object(
                        test_viz, "create_scaling_history_visualization", return_value="scaling.png"
                    ):
                        with patch.object(
                            test_viz,
                            "create_resource_allocation_visualization",
                            return_value="allocation.png",
                        ):
                            with patch.object(
                                test_viz,
                                "create_resource_efficiency_visualization",
                                return_value="efficiency.png",
                            ):
                                with patch.object(
                                    test_viz,
                                    "create_cloud_resource_visualization",
                                    return_value="cloud.png",
                                ):
                                    # Mock file writing
                                    with patch(
    ...
    def test_data_collection_and_pruning(self):
        ...
        self.visualization.scaling_history.append(
            {"timestamp": old_time, "decision": {"action": "scale_down", "reason": "Old test data"}}
        )
    ...
    def test_interactive_mode(self):
        ...
        if plotly_available:
            interactive_viz = DRMVisualization(
                dynamic_resource_manager=self.drm, output_dir=self.temp_dir, interactive=True
            )
    ...
    def test_no_drm_available(self):
        ...
        no_drm_viz = DRMVisualization(
            dynamic_resource_manager=None, output_dir=self.temp_dir, interactive=False
        )
