...
def generate_time_series_with_regression(
    length=100, change_point=50, before_mean=100, after_mean=120, noise=5.0
):
    ...
    time_series_data = {"timestamps": timestamp_strings, "values": values.tolist()}
...
def generate_multi_metric_data(num_metrics=3, length=100):
    ...
    for i in range(num_metrics):
        ...
        after = np.random.normal(
            loc=after_mean, scale=after_mean * 0.05, size=length - change_point
        )
        values = np.concatenate([before, after])

        # Create time series data
        metrics_data[metric_name] = {"timestamps": timestamp_strings, "values": values.tolist()}
        ...
        regression = {
            ...
            "is_significant": True,
            "severity": "high"
            if abs(((after_mean - before_mean) / before_mean) * 100) > 20
            else "medium",
            "is_regression": (after_mean > before_mean)
            if i in [0, 2]
...
class TestRegressionVisualization(unittest.TestCase):
    ...
    def test_create_comparative_regression_visualization(self):
        ...
        figure_dict = self.visualizer.create_comparative_regression_visualization(
            self.metrics_data, self.regressions_by_metric, title="Comparative Regression Analysis"
        )
        ...
        expected_traces = (
            len(self.metrics_data) * 2
        )  # Each metric has a line trace and regression markers
    ...
    def test_create_regression_heatmap(self):
        ...
        figure_dict = self.visualizer.create_regression_heatmap(
            time_ranges, metrics, regression_data, title="Regression Heatmap"
        )
    ...
    def test_create_regression_summary_report(self):
        ...
        report_path = self.visualizer.create_regression_summary_report(
            self.metrics_data, self.regressions_by_metric, include_plots=True
        )
    ...
    def test_export_regression_visualization(self):
        ...
        html_path = self.visualizer.export_regression_visualization(figure_dict, format="html")
        ...
        json_path = self.visualizer.export_regression_visualization(figure_dict, format="json")
