...
def run_dashboard(db_path, output_dir, port, debug=False, open_browser=True):
    ...
    dashboard = EnhancedVisualizationDashboard(
        db_conn=db_api, output_dir=output_dir, enable_regression_detection=True, debug=debug
    )
    ...
    logger.info(
        "2. Select 'latency_ms' from the dropdown and filter for 'bert-base' model and 'webgpu' hardware"
    )
    logger.info(
        "3. Run regression analysis and observe the detected regression around Jan 31, 2025"
    )
    ...
    logger.info(
        "6. Test gradual regression detection by selecting 'memory_usage_mb' with 'gpt2-medium' model and 'cuda' hardware"
    )
...
def verify_regression_detection(db_path, output_dir):
    ...
    for scenario in scenarios:
        ...
        regression_results = detector.detect_regressions(
            data, timestamp_col="timestamp", value_col="value"
        )
        ...
        if detected:
            ...
            logger.warning(
                f"❌ Failed to detect regression near day {expected_cp}. Closest point found at day {closest_cp}"
            )
    ...
    logger.info(
        f"Successfully detected {success_count} out of {len(scenarios)} injected regressions"
    )
    ...
    for desc, result in results.items():
        ...
        logger.info(
            f"  - Closest detected change at day: {result['closest_detected_change_point']}"
        )
...
def main():
    ...
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="Verify regression detection without running dashboard",
    )
    parser.add_argument(
        "--output-dir", help="Output directory for visualizations (default: temporary directory)"
    )
