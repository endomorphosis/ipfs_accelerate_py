...
class MockCloudProvider:
    ...
    def get_available_resources(self):
        ...
        return {
            "max_workers": 5,
            "available_workers": 5
            - len([w for w in self.workers.values() if w["status"] == "running"]),
        }
...
class IntegrationTestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cloud_config = {"test_provider": {"enabled": True, "max_workers": 5}}
        ...
        self.small_task = {
            "task_id": "small_task",
            "type": "benchmark",
            "config": {"model": "bert-tiny", "batch_size": 1},
            "requirements": {"cpu_cores": 2, "memory_mb": 4096},
        }

        self.large_task = {
            "task_id": "large_task",
            "type": "benchmark",
            "config": {"model": "llama-7b", "batch_size": 4},
            "requirements": {"cpu_cores": 8, "memory_mb": 16384, "gpu_memory_mb": 8192},
        }
...
class TestDRMCoordinatorIntegration(IntegrationTestBase):
    ...
    def test_coordinator_initialization_with_drm(self):
        ...
        self.assertIsInstance(
            self.coordinator.resource_performance_predictor, ResourcePerformancePredictor
        )
    ...
    def test_resource_aware_task_scheduling(self):
        ...
        def mock_send_message(message, websocket):
            if message.get("type") == "task":
                scheduled_tasks.append(
                    {"task": message.get("task"), "worker_id": message.get("worker_id")}
                )
        ...
        with patch.object(self.coordinator, "_send_message", side_effect=mock_send_message):
            ...
            next_cpu_task = self.coordinator.task_manager.get_next_task(
                worker_id=cpu_worker_id, capabilities=["cpu", "memory"]
            )
            ...
            next_gpu_task = self.coordinator.task_manager.get_next_task(
                worker_id=gpu_worker_id, capabilities=["cpu", "memory", "gpu"]
            )
    ...
    def test_resource_reservation_and_release(self):
        ...
        next_task = self.coordinator.task_manager.get_next_task(
            worker_id=worker_id, capabilities=["cpu", "memory", "gpu"]
        )
        ...
        self.assertIn(
            reservation_id, self.coordinator.dynamic_resource_manager.resource_reservations
        )

        # Verify worker's available resources are reduced
        worker_resources = self.coordinator.dynamic_resource_manager.worker_resources[worker_id]
        self.assertGreaterEqual(
            worker_resources["cpu"].get("reserved_cores", 0),
            self.large_task["requirements"]["cpu_cores"],
        )
        self.assertGreaterEqual(
            worker_resources["memory"].get("reserved_mb", 0),
            self.large_task["requirements"]["memory_mb"],
        )
        self.assertGreaterEqual(
            worker_resources["gpu"].get("reserved_memory_mb", 0),
            self.large_task["requirements"]["gpu_memory_mb"],
        )
    ...
    def test_resource_recording_for_prediction(self):
        ...
        # Get next task
        next_task = self.coordinator.task_manager.get_next_task(
            worker_id=worker_id, capabilities=["cpu", "memory"]
        )
        ...
        with patch.object(
            self.coordinator.resource_performance_predictor, "record_task_execution"
        ) as mock_record:
            ...
            self.coordinator.task_manager.add_task(
                {
                    ...
                    "type": "benchmark",
                    "config": {"model": "bert-base", "batch_size": 4},
                    "requirements": {"cpu_cores": 4, "memory_mb": 8192, "gpu_memory_mb": 4096},
                }
            )
    ...
    def test_cloud_provider_integration(self):
        ...
        status = self.coordinator.cloud_provider_manager.get_worker_status(
            provider="test_provider", worker_id=new_worker["worker_id"]
        )
...
class TestEndToEndDRMWorkflow(IntegrationTestBase):
    ...
    def test_complete_workflow(self, mock_start_websocket, mock_connect):
        ...
        try:
            ...
            # Mock registration
            with patch.object(
                self.coordinator.dynamic_resource_manager, "register_worker"
            ) as mock_register:
            ...
            # Mock heartbeat
            with patch.object(
                self.coordinator.dynamic_resource_manager, "update_worker_resources"
            ) as mock_update:
            ...
            self.assertEqual(
                len(self.coordinator.dynamic_resource_manager.resource_reservations), 0
            )
