...
class TestErrorVisualizationE2E(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.coordinator_process = cls._start_coordinator(
            cls.coordinator_port, cls.log_dir, cls.db_path
        )
        ...
        cls.dashboard_process = cls._start_dashboard(
            cls.dashboard_port, cls.coordinator_url, cls.db_path
        )
        ...
        for i in range(cls.worker_count):
            ...
            worker_process = cls._start_worker(worker_id, cls.coordinator_url, cls.log_dir)
    ...
    def _start_worker(cls, worker_id: str, coordinator_url: str, log_dir: str) -> subprocess.Popen:
        ...
        cmd = [
            ...
            "0.3",  # Inject errors at a moderate rate
            "--error-types",
            "resource,network,hardware,worker,test",
        ]
        ...
        async with aiohttp.ClientSession() as session:
            for i in range(task_count):
                ...
                async with session.post(
                    f"{self.coordinator_url}/api/tasks", json=task_data
                ) as response:
    ...
    async def _wait_for_task_completion(
        self, task_ids: List[str], timeout_seconds: int = 30
    ) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            while (time.time() - start_time) < timeout_seconds and len(completed_tasks) < len(
                task_ids
            ):
                for task_id in task_ids:
                    ...
                    async with session.get(
                        f"{self.coordinator_url}/api/tasks/{task_id}"
                    ) as response:
                        if response.status == 200:
    ...
    async def _test_07_error_classification(self):
        ...
        logger.info(
            f"Test 07: Found {recurring_count} recurring errors and {critical_count} critical errors"
        )
