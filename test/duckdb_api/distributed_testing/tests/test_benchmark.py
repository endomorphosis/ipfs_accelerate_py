...
class DistributedFrameworkBenchmark(unittest.TestCase):
            ...
            test_workers.append(
                {
                    ...
                    "capabilities": {"hardware_types": ["cpu"], "memory_gb": 8 + (i % 8)},
                }
            )
            ...
            with BenchmarkTimer(f"Assigning {num_tasks} tasks to {num_workers} workers"):
                for _ in range(10):  # Run multiple iterations for more stable timing
                    ...
                    for task in pending_tasks:
                        worker = scheduler.get_next_task(
                            f"bench_worker_{random.randint(0, num_workers - 1)}",
                            {"hardware_types": ["cpu"]},
                        )
                    end = time.time()
    ...
    def test_05_benchmark_health_monitoring(self):
        ...
        for count in worker_counts:
            ...
            monitor = HealthMonitor(db_manager=test_db, recovery_action=mock_recovery)
