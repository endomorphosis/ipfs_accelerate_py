...
def create_test_drm():
    ...
    drm.last_scaling_decision = ScalingDecision(
        ...
        utilization=0.85,
        worker_type="gpu",
        resource_requirements={"cpu_cores": 8, "memory_mb": 16384, "gpu_memory_mb": 8192},
    )
...
def simulate_workload(drm, duration_seconds=30, task_interval=2):
    ...
    while time.time() - start_time < duration_seconds:
        ...
        if (
            random.random() < 0.7 and len(active_tasks) < 20
        ):  # 70% chance to create task if < 20 active
            # Create a new task
...
def create_simulated_cloud_data(visualization):
    ...
    for i in range(10):
        ...
        aws_workers.append({"timestamp": timestamp, "count": count, "workers": workers})

        # Simulate increasing cost
        aws_costs.append({"timestamp": timestamp, "cost": 0.10 * count})
    ...
    for i in range(10):
        ...
        gcp_workers.append({"timestamp": timestamp, "count": count, "workers": workers})

        # Simulate stable cost
        gcp_costs.append({"timestamp": timestamp, "cost": 0.15 * count})

    # Add to visualization
    cloud_usage_history["aws"] = {"workers": aws_workers, "cost": aws_costs}

    cloud_usage_history["gcp"] = {"workers": gcp_workers, "cost": gcp_costs}
    ...
    for i in range(count):
        ...
        if i % 3 == 0:  # Scale up
            ...
            decision = {
                ...
                "worker_ids": [
                    f"worker-{random.randint(1, 10)}" for _ in range(random.randint(1, 2))
                ],
        ...
        scaling_history.append({"timestamp": timestamp, "decision": decision})
...
def main():
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Use interactive Plotly visualizations"
    )
    parser.add_argument("--dashboard", action="store_true", help="Start the web dashboard server")
    parser.add_argument(
        "--output-dir", default="./visualization_output", help="Output directory for visualizations"
    )
    parser.add_argument(
        "--duration", type=int, default=30, help="Duration of workload simulation in seconds"
    )
