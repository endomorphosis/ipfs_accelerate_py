...
class LoadBalancerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.worker_data = [
            {
                "worker_id": "worker_1",
                "hostname": "host_1",
                "capabilities": {"hardware_types": ["cpu"], "memory_gb": 8},
                "performance": {"avg_task_time": 2.5, "completed_tasks": 10, "failed_tasks": 1},
            },
            {
                "worker_id": "worker_2",
                "hostname": "host_2",
                ...
                "performance": {"avg_task_time": 1.5, "completed_tasks": 15, "failed_tasks": 0},
            },
            {
                "worker_id": "worker_3",
                "hostname": "host_3",
                ...
                "performance": {"avg_task_time": 3.0, "completed_tasks": 8, "failed_tasks": 2},
            },
        ]
        ...
        for worker in self.worker_data:
            self.db_manager.add_worker(
                worker["worker_id"], worker["hostname"], worker["capabilities"]
            )
    ...
    def test_worker_scoring(self):
        ...
        self.assertEqual(
            sorted_workers[0][0], "worker_2", "Worker 2 should have highest score for CPU task"
        )
        ...
        self.assertEqual(len(worker_scores), 1, "Only 1 worker should have a score for GPU task")
        self.assertIn(
            "worker_2",
            worker_scores,
        ...
        self.assertEqual(
            len(worker_scores), 1, "Only 1 worker should have a score for browser task"
        )
        self.assertIn(
            "worker_3",
            worker_scores,
    ...
    def test_task_assignment(self):
        ...
        self.assertEqual(assigned_worker, "worker_2", "CPU task should be assigned to worker 2")
        ...
        self.assertIsNone(
            assigned_worker, "No worker should be assigned to GPU task when worker 2 is busy"
        )
        ...
        self.assertEqual(
            assigned_worker, "worker_2", "GPU task should be assigned to worker 2 when available"
        )
    ...
    def test_load_balancing(self):
        ...
        self.load_balancer_thread = threading.Thread(
            target=self.load_balancer.start_balancing, daemon=True
        )
        ...
        self.assertIn("cpu_task_1", self.task_assignments, "CPU task 1 should be assigned")
        self.assertIn("cpu_task_2", self.task_assignments, "CPU task 2 should be assigned")
        self.assertIn("gpu_task", self.task_assignments, "GPU task should be assigned")
        self.assertIn("browser_task", self.task_assignments, "Browser task should be assigned")

        # CPU tasks should be distributed across workers
        cpu_task_workers = set(
            [self.task_assignments.get("cpu_task_1"), self.task_assignments.get("cpu_task_2")]
        )
        self.assertGreaterEqual(len(cpu_task_workers), 1, "CPU tasks should be distributed")

        # GPU task should be assigned to worker 2
        self.assertEqual(
            self.task_assignments.get("gpu_task"),
            "worker_2",
            "GPU task should be assigned to worker 2",
        )
        ...
        self.assertEqual(
            self.task_assignments.get("browser_task"),
            "worker_3",
            "Browser task should be assigned to worker 3",
        )
        ...
        self.assertEqual(
            self.task_assignments.get("high_memory_task"),
            "worker_2",
            "High memory task should be assigned to worker 2",
        )
    ...
    def test_workload_balancing(self):
        ...
        self.assertEqual(workload.get("worker_1", 0), 2, "Worker 1 should have 2 tasks")
        self.assertEqual(workload.get("worker_2", 0), 1, "Worker 2 should have 1 task")
        self.assertEqual(workload.get("worker_3", 0), 0, "Worker 3 should have 0 tasks")
        ...
        self.assertIn("worker_1", overloaded_workers, "Worker 1 should be detected as overloaded")
        self.assertIn(
            "worker_3", underutilized_workers, "Worker 3 should be detected as underutilized"
        )
    ...
    def test_task_migration(self):
        ...
        def mock_migrate_task(task_id, from_worker_id, to_worker_id):
            migrations.append(
                {"task_id": task_id, "from_worker": from_worker_id, "to_worker": to_worker_id}
            )
            return True
        ...
        self.assertGreaterEqual(len(migrations), 1, "At least one task should be migrated")

        # The migration should be from worker_1 to worker_3
        for migration in migrations:
            self.assertEqual(
                migration["from_worker"], "worker_1", "Migration should be from worker_1"
            )
            self.assertEqual(migration["to_worker"], "worker_3", "Migration should be to worker_3")
    ...
    def test_performance_based_balancing(self):
        ...
        for worker in self.worker_data:
            ...
            for i in range(performance["completed_tasks"]):
                self.db_manager.add_execution_history(
                    ...
                    datetime.now()
                    - timedelta(hours=1)
                    + timedelta(seconds=performance["avg_task_time"]),
        ...
        self.assertEqual(
            highest_score_worker, "worker_2", "Worker 2 should have highest performance score"
        )

        # Worker 3 should have lowest score (slowest, most failures)
        lowest_score_worker = min(scores.items(), key=lambda x: x[1])[0]
        self.assertEqual(
            lowest_score_worker, "worker_3", "Worker 3 should have lowest performance score"
        )
