...
class PerformanceAnalytics:
    ...
    def load_performance_data_from_db(
        self,
    ...
    def analyze_performance_trends(
        self, df: Optional[pd.DataFrame] = None, metrics: List[str] = None
    ) -> Dict[str, Any]:
        ...
        if "test_timestamp" in df.columns and not pd.api.types.is_datetime64_dtype(
            df["test_timestamp"]
        ):
        ...
        for (model, hw), group_df in grouped:
            ...
            trends[model_hw_key] = {"model": model, "hardware_type": hw, "metrics": {}}
            ...
            for metric in metrics:
                ...
                is_good = (
                    metric == "throughput_items_sec" and trend_direction == "increasing"
                ) or (
                    (metric in ["latency_ms", "memory_usage_mb", "cpu_usage_percent"])
                    and trend_direction == "decreasing"
                )
    ...
    def detect_performance_regressions(
        ...
        baseline_df = df[
            (df["test_timestamp"] >= baseline_start) & (df["test_timestamp"] < baseline_end)
        ]
        comparison_df = df[
            (df["test_timestamp"] >= comparison_start) & (df["test_timestamp"] <= comparison_end)
        ]
        ...
        for (model, hw), baseline_group in baseline_grouped:
            ...
            # Analyze each metric
            model_hw_key = f"{model}_{hw}"
            regressions[model_hw_key] = {"model": model, "hardware_type": hw, "metrics": {}}
            ...
            for metric in metrics:
                ...
                if (
                    ...
                    or np.isnan(baseline_values).all()
                    or len(comparison_values) == 0
                    or np.isnan(comparison_values).all()
                ):
                    continue
                ...
                is_regression = (
                    metric == "throughput_items_sec" and change_ratio < -regression_threshold
                ) or (
                    (metric in ["latency_ms", "memory_usage_mb", "cpu_usage_percent"])
                    and change_ratio > regression_threshold
                )
        ...
        total_regressions = sum(
            1
            for model_data in regressions.values()
            for metric_data in model_data["metrics"].values()
            if metric_data["is_regression"]
        )

        logger.info(
            f"Detected {total_regressions} regressions across {len(regressions)} model-hardware combinations"
        )
        return regressions

    def create_trend_visualization(
        self, trends: Dict[str, Any] = None, top_n: int = 5, metrics: List[str] = None
    ) -> Dict[str, Any]:
        ...
        for metric in metrics:
            ...
            fig.add_trace(
                go.Bar(
                    y=[
                        f"{row['model']} ({row['hardware_type']})"
                        for _, row in top_models.iterrows()
                    ],
                    x=top_models["slope"],
                    orientation="h",
                    marker_color=[
                        "green" if is_good else "red" for is_good in top_models["is_good_trend"]
                    ],
                    name="Trend Slope",
                    text=[
                        f"{slope:.4f} ({direction})"
                        for slope, direction in zip(
                            top_models["slope"], top_models["trend_direction"]
                        )
                    ],
                    textposition="auto",
                ),
            ...
            trending_metrics[metric] = {
                ...
                "trend_distribution": {k: int(v) for k, v in trend_counts.items()},
            }
    ...
    def create_regression_visualization(
        self, regressions: Dict[str, Any] = None, metrics: List[str] = None
    ) -> Dict[str, Any]:
        ...
        if regressions is None:
            if self.regression_analysis is None:
                ...
                self.regression_analysis = self.detect_performance_regressions(
        ...
        logger.info(
            f"Created regression visualizations for {len(regression_visualizations)} metrics"
        )
    ...
    def create_html_report(
        ...
        total_regressions = sum(
            v.get("regression_count", 0) for v in regression_visualizations.values()
        )
        ...
        good_trends = sum(
            1
            for v in trend_visualizations.values()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{dashboard_url}/api/performance-analytics", json=upload_data
                ) as response:
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Failed to upload to dashboard: HTTP {response.status} - {await response.text()}"
                        )
                        return False
        except Exception as e:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Performance Analytics for End-to-End Testing Framework"
    )

    # Basic options
    parser.add_argument(
        "--report-dir", default="./e2e_test_reports", help="Directory containing test reports"
    )
    parser.add_argument(
        "--output-dir", default="./e2e_performance_reports", help="Directory for generated reports"
    )
    parser.add_argument(
    ...
    # Analysis options
    parser.add_argument(
        "--time-range", type=int, default=30, help="Time range in days for analysis"
    )
    parser.add_argument("--baseline-days", type=int, default=7, help="Days to use for baseline")
    parser.add_argument("--comparison-days", type=int, default=1, help="Days to use for comparison")
    parser.add_argument(
        "--metrics", default="all", help="Metrics to analyze [latency,throughput,memory,cpu,all]"
    )
    parser.add_argument("--models", default="all", help="Models to include in analysis")
    parser.add_argument(
        "--hardware-types",
        default="all",
    ...
    parser.add_argument(
        "--upload-to-dashboard", action="store_true", help="Upload results to dashboard"
    )
    parser.add_argument(
        "--dashboard-url", default="http://localhost:8082", help="URL of monitoring dashboard"
    )
    ...
    performance_data = analytics.load_performance_data_from_db(
        time_range_days=args.time_range, models=models, hardware_types=hardware_types
    )
    ...
    regressions = analytics.detect_performance_regressions(
        performance_data, regression_threshold=args.regression_threshold, metrics=metrics
    )

    # Create visualizations
    trend_visualizations = analytics.create_trend_visualization(trends, metrics=metrics)
    regression_visualizations = analytics.create_regression_visualization(
        regressions, metrics=metrics
    )

    # Generate HTML report if requested
    if args.generate_report:
        report_path = analytics.create_html_report(trend_visualizations, regression_visualizations)
    ...
    if args.upload_to_dashboard:
        success = await analytics.upload_to_dashboard(
            args.dashboard_url, trend_visualizations, regression_visualizations
        )
