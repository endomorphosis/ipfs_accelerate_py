...
try:
    ...
    from data.duckdb.distributed_testing.tests.test_dashboard_regression_integration import (
        generate_performance_data_with_regressions,
    )
...
def run_dashboard(db_path, output_dir, port, debug=False, open_browser=True):
    ...
    logger.info(
        "5. Select different export formats (HTML, PNG, SVG, JSON, PDF) and test the export buttons"
    )
    logger.info("6. Generate a regression report and verify it contains the visualization options")
    logger.info(
        "7. Try changing the theme (light/dark) in the settings panel and verify the visualization updates"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run end-to-end test for Enhanced Visualization UI features"
    )
