...
async def run_test(args):
    ...
    test_cmd = ["python", "-m", "duckdb_api.distributed_testing.tests.run_e2e_tests"]
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{args.dashboard_url}/api/e2e-test-results", json=data
            ) as response:
                if response.status == 200:
                    print(
                        f"Successfully integrated visualizations with dashboard at {args.dashboard_url}"
                    )
                    ...
                    print(
                        f"Failed to integrate with dashboard: HTTP {response.status} - {await response.text()}"
                    )
                    return False
...
async def main():
    ...
    parser.add_argument(
        "--fault-tolerance", action="store_true", help="Include fault tolerance tests"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report-dir", default="./e2e_test_reports", help="Directory for test reports"
    )
    parser.add_argument(
        "--visualization-dir", default="./e2e_visualizations", help="Directory for visualizations"
    )
    parser.add_argument(
        "--theme", choices=["light", "dark"], default="dark", help="Visualization theme"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip integration with monitoring dashboard",
    )
    parser.add_argument(
        "--open-browser", action="store_true", help="Open visualization in browser when done"
    )
