...
class MockCustomizableDashboard:
    ...
    def create_dashboard(
        self, dashboard_name, template="overview", title=None, description=None, components=None
    ):
...
class TestVisualizationDashboardIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            if os.path.exists(os.path.join(cls.static_dir, "dashboards")):
                os.symlink(
                    cls.dashboard_dir,
                    os.path.join(cls.static_dir, "dashboards"),
        ...
        if cls.mock_dashboard:
            ...
            if (
                "duckdb_api.visualization.advanced_visualization.viz_customizable_dashboard"
                in sys.modules
            ):
                cls.original_viz_customizable_dashboard = sys.modules[
                    "duckdb_api.visualization.advanced_visualization.viz_customizable_dashboard"
                ]
            else:
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, "mock_dashboard") and cls.mock_dashboard:
            if cls.original_viz_customizable_dashboard is not None:
                sys.modules[
                    "duckdb_api.visualization.advanced_visualization.viz_customizable_dashboard"
                ] = cls.original_viz_customizable_dashboard
            else:
                del sys.modules[
                    "duckdb_api.visualization.advanced_visualization.viz_customizable_dashboard"
                ]
    ...
    def setUp(self):
        ...
        self.integration = VisualizationDashboardIntegration(
            dashboard_dir=self.dashboard_dir, integration_dir=self.integration_dir
        )
    ...
    def test_create_dashboard_with_components(self):
        ...
        components = [
            ...
            {
                "type": "heatmap",
                "config": {"metric": "throughput", "title": "Throughput Comparison"},
                "width": 1,
                "height": 1,
            },
        ]
    ...
    def test_get_dashboard_iframe_html(self):
        ...
        custom_iframe_html = self.integration.get_dashboard_iframe_html(
            name="iframe_test_dashboard", width="800px", height="400px"
        )
    ...
    def test_generate_dashboard_from_performance_data(self):
        ...
        performance_data = {
            "metrics": {
                "throughput_items_per_second": {"mean": 120.5, "min": 100.2, "max": 140.8},
                "average_latency_ms": {"mean": 25.3, "min": 20.1, "max": 30.5},
                "memory_peak_mb": {"mean": 1500.0, "min": 1200.0, "max": 1800.0},
            },
            "dimensions": {
                "model_family": ["bert", "t5", "vit"],
                "hardware_type": ["gpu", "cpu", "webgpu"],
            },
            "time_series": {"available": True, "time_points": 90},
        }
    ...
    def test_export_embedded_dashboard(self):
        ...
        html_path = self.integration.export_embedded_dashboard(
            name="export_test_dashboard", format="html"
        )
        ...
        pdf_path = self.integration.export_embedded_dashboard(
            name="export_test_dashboard", format="pdf"
        )
...
class TestMonitoringDashboardWithVisualization(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            from data.duckdb.distributed_testing.dashboard.monitoring_dashboard import (
                MonitoringDashboard,
            )
            from data.duckdb.visualization.advanced_visualization.viz_customizable_dashboard import (
                CustomizableDashboard,
        ...
        if cls.use_mocks:
            ...
            if (
                ...
                cls.original_modules["viz_customizable_dashboard"] = sys.modules[
                    "duckdb_api.visualization.advanced_visualization.viz_customizable_dashboard"
                ]
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, "use_mocks") and cls.use_mocks:
            for module_name, original_module in cls.original_modules.items():
                sys.modules[f"duckdb_api.visualization.advanced_visualization.{module_name}"] = (
                    original_module
                )
    ...
    def test_create_default_dashboards(self):
        ...
        performance_dashboard = (
            self.monitoring_dashboard.visualization_integration.get_embedded_dashboard(
