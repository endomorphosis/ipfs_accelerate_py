...
class RealTimeMonitor:
    ...
    async def send_update(self) -> bool:
        ...
        try:
            ...
            # Send update
            await self.ws_connection.send(
                json.dumps({"type": "e2e_test_monitoring_update", "data": self.test_state})
            )
    ...
    async def monitor_test_process(self, process_id: int = None):
        ...
        self.update_test_state(
            {"status": "completed", "end_time": datetime.now().isoformat(), "progress": 100.0}
        )
    ...
    async def simulate_services_startup_phase(self):
        ...
        # Update test state
        self.update_test_state({"components": components, "progress": 10.0})
        await self.send_update()
        ...
        for component, delay in [
            ...
            self.update_test_state(
                {"components": components, "progress": self.test_state["progress"] + 5.0}
            )
            await self.send_update()
    ...
    async def simulate_worker_startup_phase(self):
        ...
        for i in range(num_workers):
            ...
            workers[f"worker_{i}"] = {"status": "starting", "hardware_type": hardware_type}

        # Update test state
        self.update_test_state({"components": {"workers": workers}, "progress": 30.0})
        await self.send_update()
    ...
    async def simulate_workload_submission_phase(self):
        ...
        self.update_test_state(
            {
                "resources": {
                    "tasks": {task_type: {"submitted": 0, "total": 10} for task_type in task_types}
                },
                "progress": 45.0,
            }
        )
        ...
        for task_type in task_types:
            for i in range(10):  # 10 tasks per type
                ...
                self.update_test_state(
                    {
                        "progress": 45.0
                        + (task_type.index(task_type) * 10 + (i + 1)) / (len(task_types) * 10) * 5.0
                    }
                )
                await self.send_update()
    ...
    async def simulate_test_execution_phase(self):
        ...
        # Update test state
        self.update_test_state({"resources": {"task_execution": task_tracking}, "progress": 55.0})
        await self.send_update()
        ...
        for task_type in task_types:
            for i in range(10):  # 10 tasks per type
                ...
                if success:
                    task_tracking[task_type]["succeeded"] = (
                        task_tracking[task_type]["succeeded"] + 1
                    )
                else:
                            ...
                            "errors": [
                                {
                                    ...
                                    "details": {"task_type": task_type, "task_id": i},
                                }
                            ]
                        }
                    )
                ...
                self.update_test_state(
                    {
                        "resources": {"task_execution": task_tracking},
                        "progress": 55.0
                        + (task_type.index(task_type) * 10 + (i + 1))
                        / (len(task_types) * 10)
                        * 20.0,
                    }
                )
        ...
        self.update_test_state(
            {
                "phases": {
                    ...
                    "validation": {"status": "running", "start_time": datetime.now().isoformat()},
                },
                "progress": 75.0,
            }
        )
    ...
    async def simulate_validation_phase(self):
        ...
        # Update test state
        self.update_test_state({"resources": {"validation": validation_status}, "progress": 80.0})
        await self.send_update()
        ...
        for i, area in enumerate(validation_areas):
            ...
            self.update_test_state(
                {
                    "resources": {"validation": validation_status},
        ...
        self.update_test_state(
            {
                "phases": {
                    "validation": {"status": "completed", "end_time": datetime.now().isoformat()},
                    "cleanup": {"status": "running", "start_time": datetime.now().isoformat()},
                },
                "progress": 90.0,
            }
        )
    ...
    async def simulate_cleanup_phase(self):
        ...
        # Update test state
        self.update_test_state({"components": component_status, "progress": 92.0})
        await self.send_update()
        ...
        self.update_test_state({"components": component_status, "progress": 95.0})
        await self.send_update()
            ...
            self.update_test_state(
                {
                    ...
                    "progress": 95.0
                    + ["coordinator", "result_aggregator", "monitoring_dashboard"].index(component)
                    / 3
                    * 5.0,
                }
            )
        ...
        self.update_test_state(
            {
                "phases": {
                    "cleanup": {"status": "completed", "end_time": datetime.now().isoformat()}
                },
                "progress": 100.0,
            }
        )
