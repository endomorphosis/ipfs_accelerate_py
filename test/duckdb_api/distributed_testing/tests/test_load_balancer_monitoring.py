...
class LoadBalancerMonitoringIntegrationTest(unittest.TestCase):
    ...
    def _start_workers(cls, count=3):
        ...
        for i in range(count):
            ...
            if i == 0:
                # Basic CPU worker
                capabilities = {"hardware_types": ["cpu"]}
            elif i == 1:
                # CPU + GPU worker
                capabilities = {
    ...
    def test_04_websocket_real_time_updates(self):
        ...
        ws.send(json.dumps({"action": "subscribe", "channel": "system_metrics"}))
    ...
    def test_05_anomaly_detection(self):
        ...
        for i in range(20):
            self.coordinator.add_task(
                "command", {"command": ["sleep", "10"]}, {"hardware": ["cpu"]}, priority=1
            )
        ...
        self.assertTrue(
            any(
                atype in anomaly_types
                for atype in ["system_high_queue_time", "worker_overload", "worker_queue_depth"]
            ),
    ...
    def test_09_load_balancer_monitoring_integration(self):
        ...
        cpu_task = self.coordinator.add_task(
            "command", {"command": ["echo", "CPU task"]}, {"hardware": ["cpu"]}, priority=1
        )

        gpu_task = self.coordinator.add_task(
            "command", {"command": ["echo", "GPU task"]}, {"hardware": ["cuda"]}, priority=1
        )

        browser_task = self.coordinator.add_task(
            "command", {"command": ["echo", "Browser task"]}, {"hardware": ["webgpu"]}, priority=1
        )
    ...
    def _add_test_tasks(self):
        ...
        # Add WebGPU task
        task_id = self.coordinator.add_task(
            "command", {"command": ["echo", "WebGPU task"]}, {"hardware": ["webgpu"]}, priority=1
        )
        task_ids.append(task_id)
