...
def run_specific_test_case(test_path: str, verbose: bool = False) -> bool:
    ...
    module = __import__(
        f"duckdb_api.distributed_testing.tests.{module_name}", fromlist=[test_class_name]
    )
        ...
        with open(file_path, "r") as f:
            ...
            for class_line in class_lines:
                ...
                test_methods = [
                    ...
                    and class_name in content.split(line)[0].split("class ")[-1]
                ]
...
def main():
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available tests without running them"
    )
