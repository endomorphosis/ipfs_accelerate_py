...
class TestResourceOptimizer(unittest.TestCase):
    ...
    def test_predict_task_requirements(self):
        ...
        self.performance_predictor.predict_resource_requirements.assert_called_once_with(
            self.text_embedding_task
        )
    ...
    def test_predict_task_requirements_fallback(self):
        ...
        self.performance_predictor.predict_resource_requirements.side_effect = Exception(
            "Prediction error"
        )
    ...
    def test_allocate_resources(self):
        ...
        self.resource_manager.worker_resources = {
            "worker-1": {"resources": self.cpu_resources},
            "worker-2": {"resources": self.gpu_resources},
        }

        # Configure reserve_resources to return reservation IDs
        self.resource_manager.reserve_resources.side_effect = lambda **kwargs: (
            f"res-{kwargs['task_id']}"
        )
        ...
        def mock_predict_resources(task_data):
            if task_data.get("model_type") == "text_embedding":
                return {"cpu_cores": 2, "memory_mb": 4096, "gpu_memory_mb": 0, "confidence": 0.9}
            else:  # text_generation
                return {"cpu_cores": 4, "memory_mb": 8192, "gpu_memory_mb": 4096, "confidence": 0.9}

        self.performance_predictor.predict_resource_requirements.side_effect = (
            mock_predict_resources
        )
    ...
    def test_allocate_resources_insufficient(self):
        ...
        self.resource_manager.worker_resources = {
            "worker-1": {
                "resources": {
                    "cpu": {"cores": 2, "physical_cores": 1, "available_cores": 1.5},
                    "memory": {"total_mb": 4096, "available_mb": 2048},
                }
            }
        }
        ...
        task_batch = [
            {
                ...
                "type": "benchmark",
                "priority": 1,
                "config": {"model_type": "text_generation", "model": "llama-65b", "batch_size": 1},
            }
        ]
    ...
    def test_recommend_worker_types(self):
        ...
        def mock_predict_resources(task_data):
            if task_data.get("model_type") == "text_embedding":
                ...
                return {"cpu_cores": 2, "memory_mb": 4096, "gpu_memory_mb": 2048, "confidence": 0.9}
...
class TestResourceOptimizerIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.optimizer = ResourceOptimizer(
            resource_manager=self.resource_manager, performance_predictor=self.performance_predictor
        )
    ...
    def test_end_to_end_task_lifecycle(self):
        ...
        future_tasks = [
            ...
            },
            {
                ...
                "type": "benchmark",
                "config": {"model_type": "text_generation", "model": "llama-7b", "batch_size": 2},
            },
        ]
...
class TestResourceOptimizerPerformance(unittest.TestCase):
    ...
    def test_allocation_performance(self):
        ...
        allocations = self.optimizer.allocate_resources(
            task_batch, [f"worker-{i}" for i in range(20)]
        )
        end_time = time.time()
