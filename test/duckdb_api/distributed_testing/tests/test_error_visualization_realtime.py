...
def generate_mock_error() -> Dict[str, Any]:
    ...
    error_type = next(
        (t for t in ERROR_TYPES if category_prefix.lower() in t.lower()), "UnknownError"
    )
...
async def run_error_generator(
    dashboard_url: str, count: int, interval: float, critical_percent: float
) -> None:
    ...
    for i in range(count):
        ...
        if is_critical:
            ...
            if random.random() < 0.5:
                ...
                # High memory usage
                error["system_context"]["metrics"]["memory"]["used_percent"] = random.randint(
                    95, 100
                )
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8080", help="URL of the dashboard server"
    )
    parser.add_argument("--count", type=int, default=10, help="Number of errors to generate")
    parser.add_argument(
        "--interval", type=float, default=2.0, help="Interval between error reports in seconds"
    )
    parser.add_argument(
        "--critical-percent",
        type=float,
        default=20.0,
        help="Percentage of errors that should be critical",
    )
