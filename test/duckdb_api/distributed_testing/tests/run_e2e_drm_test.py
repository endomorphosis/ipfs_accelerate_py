...
class DRMTestEnvironment:
    ...
    def start_coordinator(self, host="localhost", port=8080):
        ...
        try:
            ...
            cloud_config = {
                "aws": {"enabled": True, "max_workers": 5, "api_key": "test-key"},
                "gcp": {"enabled": True, "max_workers": 3, "api_key": "test-key"},
                "docker": {"enabled": True, "max_workers": 10},
            }
            ...
            self.coordinator_process = multiprocessing.Process(
                target=run_coordinator, name="CoordinatorProcess"
            )
    ...
    def start_worker(self, worker_id, resources, capabilities=None):
        ...
        try:
            ...
            worker_process = multiprocessing.Process(
                target=run_worker, name=f"WorkerProcess-{worker_id}"
            )
    ...
    def submit_tasks(self, task_batch, submit_interval=0.5):
        ...
        try:
            ...
            for task in task_batch:
                ...
                # Log task submission
                logger.info(
                    f"Submitted task {task['task_id']} of type {task.get('type', 'unknown')}"
                )
    ...
    def wait_for_tasks(self, task_ids, timeout=300):
        ...
        try:
            ...
            if completed_tasks == set(task_ids):
                ...
                logger.warning(
                    f"Timeout waiting for tasks. {len(completed_tasks)}/{len(task_ids)} completed"
                )
                return False
    ...
    def _sample_utilization_metrics(self):
        ...
        for worker_id in self.worker_processes:
            ...
            gpu_util = (
                ...
                if "gpu" in self.test_metrics["worker_metrics"][worker_id]["capabilities"]
                else 0.0
            )
    ...
    def _generate_test_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            completion_rate = (
                0
                if self.test_metrics["tasks_submitted"] == 0
                else self.test_metrics["tasks_completed"]
                / self.test_metrics["tasks_submitted"]
                * 100
            )
            ...
            if self.test_metrics["utilization_samples"]:
                ...
                # Calculate averages
                cpu_util = sum(
                    s["overall_cpu_utilization"] for s in self.test_metrics["utilization_samples"]
                ) / len(self.test_metrics["utilization_samples"])
                memory_util = sum(
                    s["overall_memory_utilization"]
                    for s in self.test_metrics["utilization_samples"]
                ) / len(self.test_metrics["utilization_samples"])
                gpu_util = sum(
                    s["overall_gpu_utilization"] for s in self.test_metrics["utilization_samples"]
                ) / len(self.test_metrics["utilization_samples"])
...
def run_e2e_test(args):
    ...
    try:
        ...
        # CPU worker
        cpu_resources = {
            "cpu": {"cores": 8, "physical_cores": 4, "available_cores": 7.5},
        ...
        memory_resources = {
            "cpu": {"cores": 8, "physical_cores": 4, "available_cores": 7.5},
            "memory": {"total_mb": 65536, "available_mb": 58000},
        }
        env.start_worker("memory-worker-1", memory_resources)

        # GPU worker
        gpu_resources = {
            "cpu": {"cores": 16, "physical_cores": 8, "available_cores": 14.0},
