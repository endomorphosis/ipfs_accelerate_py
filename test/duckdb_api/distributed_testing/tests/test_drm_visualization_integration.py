...
class TestDRMVisualizationIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_resource_manager.get_worker_statistics.return_value = {
            ...
            "workers": {
                "worker-1": {
                    "utilization": {"cpu": 0.7, "memory": 0.6, "gpu": 0.5, "overall": 0.6}
                },
    ...
    def test_initialization(self, mock_drm_vis_class):
        ...
        integration = DRMVisualizationIntegration(
            output_dir=self.temp_dir, resource_manager=self.mock_resource_manager
        )
    ...
    def test_update_visualizations(self, mock_drm_vis_class):
        ...
        mock_drm_vis_instance.create_resource_utilization_heatmap.return_value = self.paths[
            "resource_heatmap"
        ]
        mock_drm_vis_instance.create_scaling_history_visualization.return_value = self.paths[
            "scaling_history"
        ]
        mock_drm_vis_instance.create_resource_allocation_visualization.return_value = self.paths[
            "resource_allocation"
        ]
        mock_drm_vis_instance.create_resource_efficiency_visualization.return_value = self.paths[
            "resource_efficiency"
        ]
        ...
        for viz_type in [
            ...
            self.assertEqual(
                integration.visualization_registry[viz_type]["path"], self.paths[viz_type]
            )
    ...
    def test_registry_persistence(self, mock_drm_vis_class):
        ...
        new_integration = DRMVisualizationIntegration(output_dir=self.temp_dir)

        # Check that registry was loaded
        self.assertIn("test_viz", new_integration.visualization_registry)
        self.assertEqual(
            new_integration.visualization_registry["test_viz"]["title"], "Test Visualization"
        )
