...
try:
    from data.duckdb.distributed_testing.dashboard.enhanced_visualization_dashboard import (
    ...
    from data.duckdb.distributed_testing.dashboard.regression_visualization import (
...
def generate_test_data():
    ...
    time_series_data = {"timestamps": dates.tolist(), "values": values}
...
class TestEnhancedVisualizationUI(unittest.TestCase):
    ...
    def test_theme_integration(self):
        ...
        fig_dark = self.regression_visualization.create_interactive_regression_figure(
            self.time_series_data, self.regressions, "test_metric", title="Dark Theme Test"
        )

        self.assertIsNotNone(fig_dark)
        self.assertEqual(
            fig_dark["layout"]["template"]["layout"]["template"]["data"], "plotly_dark"
        )
        ...
        fig_light = self.regression_visualization.create_interactive_regression_figure(
            self.time_series_data, self.regressions, "test_metric", title="Light Theme Test"
        )

        self.assertIsNotNone(fig_light)
        self.assertEqual(
            fig_light["layout"]["template"]["layout"]["template"]["data"], "plotly_white"
        )
    ...
    def test_export_format_support(self):
        ...
        fig_dict = self.regression_visualization.create_interactive_regression_figure(
            self.time_series_data, self.regressions, "test_metric", title="Export Format Test"
        )
    ...
    def test_dashboard_visualization_options_panel(self):
        ...
        with mock.patch(
            "duckdb_api.distributed_testing.dashboard.enhanced_visualization_dashboard.BenchmarkDBAPI"
        ):
            ...
            with mock.patch.object(
                dashboard, "_get_available_metrics", return_value=["test_metric"]
            ):
    ...
    def test_dashboard_visualization_callbacks(self):
        ...
        with mock.patch(
            ...
            dashboard.data_cache["regression_analysis"]["last_time_series_data"] = (
                self.time_series_data
            )
            ...
            with mock.patch.object(
                dashboard.regression_visualization, "create_interactive_regression_figure"
            ) as mock_create_fig:
    ...
    def test_export_regression_visualization_callback(self):
        ...
        with mock.patch(
            ...
            fig_dict = self.regression_visualization.create_interactive_regression_figure(
                self.time_series_data, self.regressions, "test_metric"
            )
            ...
            with mock.patch.object(
                ...
                with mock.patch("plotly.graph_objects.Figure.write_html") as mock_write_html:
                    with mock.patch("plotly.graph_objects.Figure.write_image") as mock_write_image:
                        with mock.patch(
                            "plotly.graph_objects.Figure.write_json"
                        ) as mock_write_json:
                            ...
                            with mock.patch("dash.callback_context") as mock_context:
                                ...
                                mock_context.triggered = [
                                    {"prop_id": "export-regression-viz-btn.n_clicks"}
                                ]

                                # Call the export callback
                                result = dashboard._export_regression_visualization_callback(
                                    1, 0, "html"
                                )
                                ...
                                # Test PNG export
                                dashboard.data_cache["regression_analysis"][
                                ...
                                mock_context.triggered = [
                                    {"prop_id": "export-regression-viz-btn-inline.n_clicks"}
                                ]
                                dashboard.data_cache["regression_analysis"][
                                    "visualization_options"
                                ]["export_format"] = "json"
    ...
    def test_generate_regression_report_with_options(self):
        ...
        with mock.patch(
            ...
            with mock.patch.object(
                dashboard.regression_detector, "generate_regression_report"
