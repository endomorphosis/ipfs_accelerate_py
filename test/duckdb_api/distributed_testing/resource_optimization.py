...
try:
    ...
    logger.warning(
        "Resource Performance Predictor not available. Using default resource predictions."
    )
...
class ResourceOptimizer:
    ...
    def allocate_resources(
        self, task_batch: List[Dict[str, Any]], available_workers: List[str]
        ...
        try:
            ...
            sorted_tasks = sorted(task_batch, key=lambda t: t.get("priority", 5), reverse=True)
            ...
            for task in sorted_tasks:
                ...
                for worker_id, resources in remaining_resources.items():
                    ...
                    if (
                        resources["cpu"]["available_cores"] >= requirements.cpu_cores
                        and resources["memory"]["available_mb"] >= requirements.memory_mb
                        and (
                            requirements.gpu_memory_mb == 0
                            or (
                                "gpu" in resources
                                and resources["gpu"]["available_memory_mb"]
                ...
                if best_worker_id:
                    ...
                    try:
                        ...
                        if reservation_id:
                            ...
                            remaining_resources[best_worker_id]["cpu"]["available_cores"] -= (
                                requirements.cpu_cores
                            )
                            remaining_resources[best_worker_id]["memory"]["available_mb"] -= (
                                requirements.memory_mb
                            )

                            if (
                                requirements.gpu_memory_mb > 0
                                and "gpu" in remaining_resources[best_worker_id]
                            ):
                                remaining_resources[best_worker_id]["gpu"][
                                    "available_memory_mb"
                                ] -= requirements.gpu_memory_mb
                            ...
                            allocation = ResourceAllocation(
                                ...
                                estimated_completion_time=(
                                    time.time() + requirements.execution_time_ms / 1000
                                )
                                if requirements.execution_time_ms > 0
                                else None,
                            ...
                            logger.debug(
                                f"Allocated task {task_id} to worker {best_worker_id} (reservation {reservation_id})"
                            )
                        else:
                            # Reservation failed
                            allocation = ResourceAllocation(
                            ...
                            logger.warning(
                                f"Failed to create resource reservation for task {task_id} on worker {best_worker_id}"
                            )
    ...
    def recommend_worker_types(
        self, pending_tasks: List[Dict[str, Any]]
    ) -> List[WorkerTypeRecommendation]:
        """
        ...
        try:
            ...
            for model_type, tasks in tasks_by_type.items():
                ...
                avg_gpu_memory_mb = sum(r.gpu_memory_mb for r in requirements_list) / len(
                    requirements_list
                )
                ...
                recommendation = WorkerTypeRecommendation(
                    ...
                    estimated_utilization=min(1.0, task_count / (capacity * min_workers))
                    if capacity > 0
                    else 1.0,
    ...
    def get_scaling_recommendations(self) -> ScalingDecision:
        ...
        try:
            if not DRM_AVAILABLE or not self.resource_manager:
                logger.warning(
                    "Dynamic Resource Manager not available. Cannot provide scaling recommendations."
                )
            ...
            if base_decision.action == "scale_up":
                ...
                if self.resource_manager.worker_resources and task_queue:
                    ...
                    workers_to_keep = [
                        w[0] for w in sorted_workers[: len(sorted_workers) - base_decision.count]
                    ]
                    workers_to_remove = [
                        w
                        for w in self.resource_manager.worker_resources
                        if w not in workers_to_keep
                    ]
    ...
    def _determine_worker_type(
        self, cpu_cores: float, memory_mb: float, gpu_memory_mb: float
    ) -> Tuple[str, str]:
    ...
    def _calculate_worker_capacity(
        self, worker_type: str, cpu_cores: float, memory_mb: float, gpu_memory_mb: float
    ) -> int:
        ...
        if not task_requirements:
            ...
            for worker_id, worker_data in self.resource_manager.worker_resources.items():
                ...
                worker_scores[worker_id] = (
                    1.0 - utilization
                )  # Lower utilization = higher score (more likely to be removed)
...
if __name__ == "__main__":
    ...
    # Create optimizer
    optimizer = ResourceOptimizer(
        resource_manager=resource_manager, performance_predictor=performance_predictor
    )
    ...
    tasks = [
        ...
        },
        {
            ...
            "type": "benchmark",
            "priority": 2,
            "config": {"model_type": "text_generation", "model": "llama-7b", "batch_size": 1},
        },
        {
            "task_id": "task-3",
            "type": "benchmark",
            "priority": 3,
            "config": {"model_type": "vision", "model": "vit-base-patch16-224", "batch_size": 16},
        },
    ]
    ...
    for task in tasks:
        ...
        print(
            f"  {task['task_id']} ({task['config']['model_type']}): "
    ...
    for allocation in allocations:
        print(
            f"  {allocation.task_id}: {'Success' if allocation.allocated else 'Failed'} - "
    ...
    for recommendation in recommendations:
        print(
            f"  {recommendation.recommended_type}: {recommendation.reason}, "
