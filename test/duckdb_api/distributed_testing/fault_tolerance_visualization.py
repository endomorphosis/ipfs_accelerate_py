...
class FaultToleranceVisualizer:
    ...
    def visualize_failure_distribution(
        ...
        hardware_classes = [
            f.get("hardware_profile", {}).get("hardware_class", "UNKNOWN") for f in failure_history
        ]
        ...
        # Plot failure types
        failure_types, ft_counts = (
            zip(*failure_type_counts.items()) if failure_type_counts else ([], [])
        )
        ...
        hw_classes, hw_counts = (
            zip(*hardware_class_counts.items()) if hardware_class_counts else ([], [])
        )
    ...
    def visualize_recovery_effectiveness(
        self,
    ...
    def visualize_failure_timeline(
        ...
        df = pd.DataFrame({"timestamp": timestamps, "error_type": error_types})
    ...
    def visualize_recovery_success_rates(
        self,
        ml_detector,
        ...
        for hw_class, strategies in ml_detector.strategy_success_rates.items():
            for strategy, counts in strategies.items():
                ...
                if total > 0:
                    ...
                    success_rates[hw_class][strategy] = {"rate": rate, "total": total}
    ...
    def visualize_ml_patterns(
        self, ml_patterns, title="Machine Learning-Detected Patterns", filename="ml_patterns.png"
    ):
        """
    ...
    def create_hardware_failure_heatmap(
        self,
        failure_history: List[Dict[str, Any]],
        ...
        try:
                    ...
                    failure_history.append(
                        {
                            ...
                            "hardware_profile": {
                                ...
                                if f_contexts.hardware_profile
                                else "UNKNOWN"
                            },
                        }
                    )
            ...
            if recovery_history:
                ...
                if recovery_eff_path:
                    vis_paths.append(
                        ("Recovery Effectiveness", os.path.basename(recovery_eff_path))
                    )
            ...
            if ml_detector:
                ...
                if success_rates_path:
                    vis_paths.append(
                        ("Recovery Success Rates", os.path.basename(success_rates_path))
                    )
            ...
            html_content = self._generate_html_report(
                vis_paths, fault_tolerance_manager, ml_detector
            )
    ...
    def _generate_html_report(self, vis_paths, fault_tolerance_manager, ml_detector):
        ...
        # Get statistics
        total_failures = (
            len(fault_tolerance_manager.failure_history) if fault_tolerance_manager else 0
        )
        total_patterns = (
            len(fault_tolerance_manager.failure_patterns) if fault_tolerance_manager else 0
        )
...
def visualize_fault_tolerance_system(fault_tolerance_manager, output_dir="./visualizations"):
    ...
    return visualizer.create_comprehensive_report(
        fault_tolerance_manager=fault_tolerance_manager, ml_detector=ml_detector
    )
