...
class HardwareCapabilityProfile:
    ...
    def estimate_performance(
        self, operation_type: str, size: int, precision: PrecisionType
    ) -> Optional[float]:
        """
...
class HardwareSpecialization:
    ...
    @classmethod
    def create_specialization_map(
        cls, hardware_profiles: List[HardwareCapabilityProfile]
    ) -> Dict[str, List["HardwareSpecialization"]]:
        """
        ...
        for profile in hardware_profiles:
            ...
            if (
                profile.hardware_class == HardwareClass.CPU
                and AcceleratorFeature.AVX2 in profile.features
            ):
                effectiveness = 0.85
                specializations.append(
                    cls(
...
class HardwareTaxonomy:
    ...
    def __init__(self):
        ...
        self.performance_index: Dict[
            Tuple[str, PrecisionType], List[Tuple[float, HardwareCapabilityProfile]]
        ] = {}
    ...
    def _initialize_compatibility_matrix(self):
        ...
        self.compatibility_matrix[(HardwareClass.GPU, HardwareClass.GPU)] = (
            0.8  # Different GPU architectures
        )
    ...
    def register_hardware_profile(self, profile: HardwareCapabilityProfile):
        ...
        for operation_type, performance in profile.performance_profile.items():
            ...
            precision = next(
                (p for p in PrecisionType if p.value == precision_str), PrecisionType.OTHER
            )
    ...
    def find_best_hardware_for_workload(
        self, workload_type: str, worker_ids: List[str] = None, min_effectiveness: float = 0.0
        ...
        result = []
        valid_worker_ids = (
            worker_ids if worker_ids is not None else list(self.worker_hardware_map.keys())
        )
            ...
            for worker_id in valid_worker_ids:
                if worker_id in self.worker_hardware_map:
                    ...
                    for worker_profile in worker_profiles:
                        if (
                            worker_profile.hardware_class == profile.hardware_class
                            and worker_profile.architecture == profile.architecture
                            and worker_profile.vendor == profile.vendor
                            and worker_profile.model_name == profile.model_name
                        ):
    ...
    def calculate_compatibility_score(
        self, profile1: HardwareCapabilityProfile, profile2: HardwareCapabilityProfile
    ) -> float:
        """
        ...
        backend_score = backend_overlap / max(
            1, len(profile1.supported_backends.union(profile2.supported_backends))
        )

        # Adjust for shared precision types
        precision_overlap = len(
            profile1.supported_precisions.intersection(profile2.supported_precisions)
        )
        precision_score = precision_overlap / max(
            1, len(profile1.supported_precisions.union(profile2.supported_precisions))
        )
    ...
    def find_most_compatible_workers(
        self, source_worker_id: str, candidate_worker_ids: List[str]
        ...
        for candidate_id in candidate_worker_ids:
            ...
            for source_profile in source_profiles:
                for candidate_profile in candidate_profiles:
                    total_score += self.calculate_compatibility_score(
    ...
    def get_performance_ranking(
        self, operation_type: str, precision: PrecisionType
    ) -> List[Tuple[HardwareCapabilityProfile, float]]:
        """
...
def create_cpu_profile(
    ...
    supported_precisions = {PrecisionType.FP32, PrecisionType.INT8}
...
def create_browser_profile(
    ...
    performance_profile = {
        ...
        "fp32_conv": base_performance * 0.8 * performance_factor["webgpu"]
        if supports_webgpu
        else 0.0,
        "fp16_matmul": base_performance * 1.5 * performance_factor["webgpu"]
        if supports_webgpu
        else 0.0,
        "fp16_conv": base_performance * 1.2 * performance_factor["webgpu"]
        if supports_webgpu
        else 0.0,
        "int8_matmul": base_performance * 2.0 * performance_factor["webnn"]
        if supports_webnn
        else 0.0,
        "int8_conv": base_performance * 1.5 * performance_factor["webnn"]
        if supports_webnn
        else 0.0,
        "fp32_audio": base_performance * 1.2 * performance_factor["audio"]
        if supports_webgpu
        else 0.0,
    }
    ...
    return HardwareCapabilityProfile(
        ...
        if supports_webgpu
        else HardwareArchitecture.OTHER,
        vendor=vendor,
