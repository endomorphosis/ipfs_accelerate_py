...
class PerformanceMetrics:
    ...
    reconnection_durations: List[float] = field(
        default_factory=list
    )  # Reconnection durations in seconds
...
class SecurityEnhancement:
    ...
    def sign_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        hmac_obj = hmac.new(self.hmac_key.encode(), message_json.encode(), hashlib.sha256)
    ...
    def generate_auth_headers(self) -> Dict[str, str]:
        ...
        hmac_obj = hmac.new(self.hmac_key.encode(), message.encode(), hashlib.sha256)
        signature = base64.b64encode(hmac_obj.digest()).decode()
...
class EnhancedWorkerReconnectionManager(WorkerReconnectionManager):
    ...
    def _queue_message(
        self, message: Dict[str, Any], priority: MessagePriority = MessagePriority.NORMAL
    ):
        """
        ...
        if self.enable_compression:
            compressed_message, orig_size, compressed_size = self.compression.compress_message(
                message_json
            )
            ...
            try:
                ...
                if self.connection_state == ConnectionState.CONNECTED and self.ws_conn:
                    try:
                        ...
                        logger.debug(
                            f"Sent message: {message[:100] if isinstance(message, str) else message.get('type', 'unknown')}"
                        )
    ...
    def _adapt_connection_parameters(self):
        ...
        if (
            now - self.adaptive_stats["last_adjustment_time"]
            < self.adaptive_stats["adjustment_interval"]
        ):
            return
        ...
        if len(self.adaptive_stats["connection_quality_history"]) > 10:
            self.adaptive_stats["connection_quality_history"] = self.adaptive_stats[
                "connection_quality_history"
            ][-10:]

        # Calculate average connection quality
        avg_stability = sum(s for s, _ in self.adaptive_stats["connection_quality_history"]) / len(
            self.adaptive_stats["connection_quality_history"]
        )
        avg_success_rate = sum(
            r for _, r in self.adaptive_stats["connection_quality_history"]
        ) / len(self.adaptive_stats["connection_quality_history"])
        ...
        if avg_stability < 0.5:
            ...
            logger.info(
                f"Adapting heartbeat interval: {self.config['heartbeat_interval']:.1f}s -> {new_heartbeat:.1f}s (low stability)"
            )
            ...
            logger.info(
                f"Adapting heartbeat interval: {self.config['heartbeat_interval']:.1f}s -> {new_heartbeat:.1f}s (high stability)"
            )
            self.config["heartbeat_interval"] = new_heartbeat
        ...
        if avg_success_rate < 0.5:
            ...
            logger.info(
                f"Adapting reconnect delay: {self.config['initial_reconnect_delay']:.1f}s -> {new_delay:.1f}s (low success rate)"
            )
            ...
            logger.info(
                f"Adapting reconnect delay: {self.config['initial_reconnect_delay']:.1f}s -> {new_delay:.1f}s (high success rate)"
            )
            self.config["initial_reconnect_delay"] = new_delay
    ...
    def execute_task_with_metrics(
        ...
        if (
            task_executor
            and task_executor.__qualname__
            != "EnhancedWorkerReconnectionPlugin._task_executor_wrapper"
        ):
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        if self.enable_priority_queue:
            ...
            return {
                ...
                "priority_queues": {
                    str(priority.name): size for priority, size in queue_sizes.items()
                },
            }
        else:
            return {
                "total_messages": self.message_queue.qsize()
                if hasattr(self.message_queue, "qsize")
                else 0
            }
...
def create_enhanced_worker_reconnection_plugin(
    ...
    logger.info(
        f"Created enhanced worker reconnection plugin for worker {getattr(worker, 'worker_id', 'unknown')}"
    )
    return plugin
