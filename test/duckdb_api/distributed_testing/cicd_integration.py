...
class CICDIntegration:
    ...
    def __init__(
        ...
        if self.provider not in valid_providers:
            raise ValueError(
                f"Provider '{provider}' not supported. Use one of: {', '.join(valid_providers)}"
            )
    ...
    def wait_for_results(self, task_ids: List[str]) -> Dict[str, Dict]:
        ...
        if pending_tasks:
            if self.verbose:
                print(
                    f"Warning: {len(pending_tasks)} tasks did not complete within timeout ({self.timeout}s)"
                )
    ...
    def generate_report(
        ...
        status_counts = {"completed": 0, "failed": 0, "cancelled": 0, "timeout": 0}
            ...
            if fmt.lower() == "json":
                    ...
                    json.dump(
                        {
                            ...
                            "branch": self.branch,
                            "commit": self.commit_sha,
                            "summary": {"total": total_tasks, "status_counts": status_counts},
                            "results": results,
                        },
                        f,
                        indent=2,
                ...
                with open(report_file, "w") as f:
                    ...
                    for task_id, result in results.items():
                        ...
                        f.write(
                            f"| {task_id} | {test_file} | {status.upper()} | {duration} | {hardware} | {details} |\n"
                        )
                ...
                with open(report_file, "w") as f:
                    ...
                    f.write(
                        f"<li><strong>Completed:</strong> {status_counts.get('completed', 0)}</li>\n"
                    )
                    f.write(f"<li><strong>Failed:</strong> {status_counts.get('failed', 0)}</li>\n")
                    f.write(
                        f"<li><strong>Cancelled:</strong> {status_counts.get('cancelled', 0)}</li>\n"
                    )
                    f.write(
                        f"<li><strong>Timeout:</strong> {status_counts.get('timeout', 0)}</li>\n"
                    )
                    f.write("</ul>\n")
                    ...
                    f.write("<table>\n")
                    f.write(
                        "<tr><th>Task ID</th><th>Test File</th><th>Status</th><th>Duration</th>"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CI/CD Integration for Distributed Testing Framework"
    )
