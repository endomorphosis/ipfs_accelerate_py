...
class MockDynamicResourceManager:
    ...
    def __init__(self):
        ...
        # Worker data
        self.workers = {
            f"worker-{i}": self._create_mock_worker(i) for i in range(self.total_workers)
        }
        ...
        return {
            ...
            "resources": {
                ...
                "gpu": {
                    ...
                    "available_memory_mb": math.ceil(8192 * (1 - base_gpu))
                    if worker_index % 2 == 0
                    else 0,
                },
            },
    ...
    def _update_simulation(self):
        ...
        self.pending_tasks = max(
            0, min(10, int(load_factor * 5 - self.total_workers + random.randint(-1, 1)))
        )
        ...
        for worker_id, worker in self.workers.items():
            ...
            overall_util = (
                (cpu_util + memory_util + (gpu_util if gpu_util > 0 else 0)) / 3
                if gpu_util > 0
                else (cpu_util + memory_util) / 2
            )
        ...
        self.performance_metrics["task_throughput"] = max(
            1.0, min(30.0, 15.0 * load_factor + random.uniform(-2.0, 2.0))
        )
        self.performance_metrics["allocation_time"] = max(
            10.0, min(300.0, 100.0 + 100.0 * load_factor + random.uniform(-20.0, 20.0))
        )
        self.performance_metrics["resource_efficiency"] = max(
            30.0, min(95.0, 70.0 + 20.0 * math.sin(phase + math.pi / 4) + random.uniform(-5.0, 5.0))
        )
    ...
    def _generate_scaling_decision(self, load_factor):
        ...
        if overall_util > self.scale_up_threshold and random.random() < 0.7:
            ...
            overall_util < self.scale_down_threshold
            and self.total_workers > 3
    ...
    def get_worker_statistics(self) -> Dict[str, Any]:
        ...
        gpu_values = [
            w["utilization"]["gpu"] for w in self.workers.values() if w["utilization"]["gpu"] > 0
        ]
        ...
        overall_utilization = {
            ...
            "overall": sum([w["utilization"]["overall"] for w in self.workers.values()])
            / len(self.workers)
            if self.workers
            else 0,
        }
