...
class FaultToleranceSystem:
    ...
    def __init__(
        ...
        self.circuit_breaker_monitor_thread = threading.Thread(
        ...
        while retry_count <= max_retries_count:
            try:
                ...
                recovery_action = self.handle_error(
                    error=e, context=context, operation_id=operation_id
                )

                # If not retry action or reached max retries, break
                if (
                    recovery_action["action"] != RecoveryAction.RETRY
                    or retry_count >= max_retries_count
                ):
                    break
                ...
                logger.info(
                    f"Retrying operation after {backoff_seconds}s (attempt {retry_count + 1}/{max_retries_count})"
                )
    ...
    def register_recovery_strategy(
        self, error_category: Union[str, ErrorCategory], strategy: Callable
    ) -> None:
        """
    ...
    def _init_recovery_strategies(self) -> None:
        ...
        self.recovery_strategies[ErrorCategory.NETWORK] = lambda severity, retry_count, context: {
            ...
            else RecoveryAction.CIRCUIT_BREAK,
        ...
        self.recovery_strategies[ErrorCategory.RESOURCE] = lambda severity, retry_count, context: {
            ...
            if retry_count < 1
            else (RecoveryAction.RETRY if retry_count < self.max_retries else RecoveryAction.ABORT),
        ...
        self.recovery_strategies[ErrorCategory.HARDWARE] = lambda severity, retry_count, context: {
            "action": RecoveryAction.FALLBACK
            if context.get("alternative_hardware")
            else RecoveryAction.ABORT,
        ...
        self.recovery_strategies[ErrorCategory.AUTHENTICATION] = (
            lambda severity, retry_count, context: {
                "action": RecoveryAction.RETRY
                if context.get("refresh_credentials") and retry_count < 1
                else RecoveryAction.ABORT,
        ...
        self.recovery_strategies[ErrorCategory.AUTHORIZATION] = (
            lambda severity, retry_count, context: {
                "action": RecoveryAction.ABORT,
    ...
    def _categorize_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(
            ...
            for s in ["connection", "network", "socket", "timeout", "unreachable"]
        ):
        ...
        # Resource errors
            ...
            s in error_str.lower() for s in ["resource", "memory", "capacity", "overflow", "full"]
        ):
        ...
        # Data errors
            ...
            s in error_str.lower() for s in ["data", "invalid", "corrupt", "format", "parsing"]
        ):
        ...
        # Authentication errors
            ...
            s in error_str.lower() for s in ["authentication", "login", "credentials", "password"]
        ):
        ...
        # Authorization errors
            ...
            s in error_str.lower() for s in ["authorization", "permission", "access", "forbidden"]
        ):
    ...
    def _record_error(
        ...
        logger.info(
            f"Recorded {error_info['category'].value} error with {error_info['severity'].value} severity "
    ...
    def _determine_recovery_action(
        ...
        if error_severity == ErrorSeverity.CRITICAL:
            return {"action": RecoveryAction.ABORT, "reason": "Critical error"}
        ...
        if (
            error_severity in [ErrorSeverity.LOW, ErrorSeverity.MEDIUM]
            and retry_count < self.max_retries
        ):
            return {"action": RecoveryAction.RETRY, "reason": "Transient error"}
        else:
            return {"action": RecoveryAction.ABORT, "reason": "Persistent error"}
    ...
    def _increment_circuit_error(self, service_key: str) -> None:
        ...
        with self.circuit_breakers_lock:
            ...
            if (
                ...
                and circuit["error_count"] >= self.circuit_break_threshold
            ):
                circuit["state"] = "open"
                logger.warning(
                    f"Circuit breaker opened for {service_key} after {circuit['error_count']} errors"
                )
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if current_time - last_check_time >= 10:
                    # Calculate error rate
                    error_rate = (
                        len(self.error_history) / self.error_window_size
                        if self.error_history
                        else 0
                    )
                    ...
                    if error_rate > self.error_rate_threshold:
                        logger.warning(
                            f"Error rate alert: {error_rate:.2f} (threshold: {self.error_rate_threshold})"
                        )
                ...
                with self.circuit_breakers_lock:
                    for service_key, circuit in list(self.circuit_breakers.items()):
                        ...
                        if circuit["last_error_time"]:
                            elapsed_seconds = (
                                current_time - circuit["last_error_time"]
                            ).total_seconds()
                            ...
                            if elapsed_seconds >= self.circuit_break_timeout:
                                ...
                                logger.info(
                                    f"Circuit breaker for {service_key} reset to half-open after {elapsed_seconds:.1f}s"
                                )
