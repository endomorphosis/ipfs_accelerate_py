...
try:
    from cloud_provider_integration import AWSCloudProvider, GCPCloudProvider, DockerLocalProvider
...
DEFAULT_WORKER_PARAMS = {
    "cpu": {"cpu_cores": 4, "memory_gb": 16, "gpu_count": 0},
    "gpu": {"cpu_cores": 8, "memory_gb": 32, "gpu_count": 1, "gpu_type": "nvidia-t4"},
    "memory": {"cpu_cores": 8, "memory_gb": 64, "gpu_count": 0},
    "default": {"cpu_cores": 2, "memory_gb": 8, "gpu_count": 0},
}
...
class CloudProviderManager:
            ...
            if "aws" in self.config and self.config["aws"].get("enabled", True):
                ...
                try:
                    aws_provider = AWSCloudProvider(
                        ...
                        secret_access_key=aws_config.get("credentials", {}).get(
                            "secret_access_key"
                        ),
                    )
            ...
            if "gcp" in self.config and self.config["gcp"].get("enabled", True):
                ...
                if project_id:
                    try:
                        ...
                        logger.info(
                            f"Initialized GCP provider for project {project_id}, zone {zone}"
                        )
                    except Exception as e:
    ...
    def create_worker(
        ...
        try:
            ...
            logger.info(
                f"Created worker on {provider}: {result.get('worker_id') if result else 'failed'}"
            )
            return result
    ...
    def terminate_worker(self, provider: str, worker_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Terminated worker {worker_id} on {provider}: {'success' if result else 'failed'}"
            )
            return result
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        choices=["create", "terminate", "status", "resources"],
        default="resources",
