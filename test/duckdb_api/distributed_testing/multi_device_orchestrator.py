...
class MultiDeviceOrchestrator:
    ...
    def orchestrate_task(
        self, task_data: Dict[str, Any], strategy: Union[str, SplitStrategy]
    ) -> str:
        """
        ...
        threading.Thread(target=self._execute_orchestration, args=(task_id,), daemon=True).start()
    ...
    def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        with self.tasks_lock:
            ...
            completed_subtasks = sum(
                1
                for s in subtasks_info
                if s.get("status")
                in [SubtaskStatus.COMPLETED, SubtaskStatus.FAILED, SubtaskStatus.CANCELLED]
            )
            if subtask_ids:
                task_info["completion_percentage"] = int(
                    completed_subtasks * 100 / len(subtask_ids)
                )
            else:
    ...
    def _schedule_subtasks(self, subtasks: List[Dict[str, Any]]) -> None:
        ...
        for subtask in subtasks:
            ...
            with self.tasks_lock:
                if (
                    task_id in self.orchestrated_tasks
                    and self.orchestrated_tasks[task_id]["status"] == TaskStatus.CANCELLED
                ):
            ...
            if self.task_manager:
                new_task_id = self.task_manager.add_task(
                    task_request, priority=task_request.get("priority", 5)
                )
                ...
                with self.subtasks_lock:
                    for subtask_id, subtask_info in list(self.subtasks.items()):
                        if subtask_info["status"] == SubtaskStatus.RUNNING:
                            ...
                            if (
                                subtask_info["start_time"]
                                and (current_time - subtask_info["start_time"]).total_seconds()
                                > timeout
                            ):
    ...
    def _merge_data_parallel(
        self, task_id: str, subtask_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if all("predictions" in subtask["result"] for subtask in subtask_results):
            ...
            for key, values in predictions.items():
                if isinstance(values[0], (int, float)):
                    ...
                    isinstance(v, (int, float)) for v in values[0]
                ):
