...
class DRMVisualization:
    ...
    def _collect_resource_data(self):
            ...
            for provider in self.cpm.providers:
                ...
                try:
                    ...
                    self.cloud_usage_history[provider]["resources"].append(
                        {"timestamp": timestamp, "resources": resources}
                    )
        ...
        if hasattr(self.drm, "last_scaling_decision"):
            ...
            if scaling_decision:
                self.scaling_history.append({"timestamp": timestamp, "decision": scaling_decision})
    ...
    def _prune_old_data(self):
        ...
        self.scaling_history = [
            entry for entry in self.scaling_history if entry["timestamp"] >= cutoff_time
        ]
        ...
        for worker_id in list(self.worker_history.keys()):
            self.worker_history[worker_id] = [
                entry
                for entry in self.worker_history[worker_id]
        ...
        for provider in list(self.cloud_usage_history.keys()):
            for data_type in list(self.cloud_usage_history[provider].keys()):
                self.cloud_usage_history[provider][data_type] = [
                    entry
                    for entry in self.cloud_usage_history[provider][data_type]
                    if entry["timestamp"] >= cutoff_time
                ]
    ...
    def create_resource_utilization_heatmap(
        self, output_path=None, show_plot=False, interactive=None
    ):
        """
        ...
        if interactive and PLOTLY_AVAILABLE:
            ...
            # Update layout
            fig.update_layout(
                title="Resource Utilization Heatmap", height=800, width=1200, showlegend=False
            )
            ...
            # Plot CPU heatmap
            im0 = axes[0].imshow(cpu_data, aspect="auto", cmap="viridis", vmin=0, vmax=100)
            ...
            # Plot Memory heatmap
            im1 = axes[1].imshow(memory_data, aspect="auto", cmap="viridis", vmin=0, vmax=100)
            ...
            # Plot GPU heatmap
            im2 = axes[2].imshow(gpu_data, aspect="auto", cmap="viridis", vmin=0, vmax=100)
            ...
            if len(timestamps) > 10:
                ...
                axes[2].set_xticklabels(
                    [timestamps[i].strftime("%H:%M:%S") for i in idx], rotation=45
                )
            else:
    ...
    def create_scaling_history_visualization(
        self, output_path=None, show_plot=False, interactive=None
    ):
        """
        ...
        utilizations = [
            snapshot["overall_utilization"].get("overall", 0) * 100
        ...
        if interactive and PLOTLY_AVAILABLE:
            ...
            for timestamp in scale_up_times:
                fig.add_vline(
                    x=timestamp, line=dict(color="green", width=2, dash="dash"), row=1, col=1
                )
                fig.add_annotation(
                    x=timestamp,
            ...
            for timestamp in scale_down_times:
                fig.add_vline(
                    x=timestamp, line=dict(color="red", width=2, dash="dash"), row=1, col=1
                )
                fig.add_annotation(
                    x=timestamp,
                    y=max(worker_counts) * 1.1,
            ...
            # Plot worker count
            ax1.plot(
                timestamps, worker_counts, "bo-", linewidth=2, markersize=6, label="Worker Count"
            )
    ...
    def create_cloud_resource_visualization(
        self, output_path=None, show_plot=False, interactive=None
    ):
        """
    ...
    def create_resource_allocation_visualization(
        self, output_path=None, show_plot=False, interactive=None
    ):
        """
        ...
        if interactive and PLOTLY_AVAILABLE:
            ...
            # Add CPU allocation
            fig.add_trace(
                go.Bar(x=worker_ids, y=cpu_allocations, name="CPU Cores", marker_color="blue")
            )
            ...
            mem_bars = ax.bar(
                x,
                [mem / 1024 for mem in memory_allocations],
                width,
                label="Memory (GB)",
                color="green",
            )
            gpu_bars = ax.bar(
                x + width,
                [gpu / 1024 for gpu in gpu_allocations],
                width,
                label="GPU Memory (GB)",
                color="red",
            )
    ...
    def create_resource_efficiency_visualization(
        self, output_path=None, show_plot=False, interactive=None
    ):
        """
        ...
        if interactive and PLOTLY_AVAILABLE:
            ...
            # Add CPU efficiency
            fig.add_trace(
                go.Bar(x=worker_ids, y=cpu_efficiency, name="CPU Efficiency", marker_color="blue")
            )
            ...
            # Add GPU efficiency
            fig.add_trace(
                go.Bar(x=worker_ids, y=gpu_efficiency, name="GPU Efficiency", marker_color="red")
            )
            ...
            # Plot bars
            cpu_bars = ax.bar(
                x - width, cpu_efficiency, width, label="CPU Efficiency", color="blue"
            )
    ...
    def create_resource_dashboard(self, output_dir=None):
        ...
        if not output_dir:
            output_dir = os.path.join(
                self.output_dir, f"dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        heatmap_path = self.create_resource_utilization_heatmap(
            output_path=os.path.join(output_dir, "resource_heatmap.html"), interactive=True
        )

        scaling_path = self.create_scaling_history_visualization(
            output_path=os.path.join(output_dir, "scaling_history.html"), interactive=True
        )

        allocation_path = self.create_resource_allocation_visualization(
            output_path=os.path.join(output_dir, "resource_allocation.html"), interactive=True
        )

        efficiency_path = self.create_resource_efficiency_visualization(
            output_path=os.path.join(output_dir, "resource_efficiency.html"), interactive=True
        )

        cloud_path = None
        if self.cloud_usage_history:
            cloud_path = self.create_cloud_resource_visualization(
                output_path=os.path.join(output_dir, "cloud_resources.html"), interactive=True
            )
...
<body>
    ...
    <div class="container">
        <div class="dashboard-section">
            ...
            <iframe src="{
            os.path.basename(heatmap_path) if heatmap_path else ""
        ...
        <div class="dashboard-section">
            ...
            <iframe src="{
            os.path.basename(scaling_path) if scaling_path else ""
        }" class="dashboard-iframe"></iframe>
        </div>
        ...
        <div class="dashboard-row">
            <div class="dashboard-section half-width">
                ...
                <iframe src="{
            os.path.basename(allocation_path) if allocation_path else ""
        }" class="dashboard-iframe"></iframe>
            </div>
            ...
            <div class="dashboard-section half-width">
                ...
                <iframe src="{
            os.path.basename(efficiency_path) if efficiency_path else ""
        }" class="dashboard-iframe"></iframe>
            </div>
        </div>
        ...
        try:
            ...
            if background:
                ...
                self.dashboard_thread = threading.Thread(
                    target=self.dashboard_app.start, daemon=True
                )
...
class DRMDashboardApp:
        ...
        self.app = tornado.web.Application(
            [
                ...
                (
                    ...
                    {"path": self.visualization.output_dir},
                ),
    ...
    def broadcast_update(self, data):
        ...
        for handler in tornado.web.Application.handlers[0][1]:
            if isinstance(handler[0], tornado.web.URLSpec) and handler[0].name == "websocket":
...
class UpdatesHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        self.write(
            json.dumps({"visualizations": visualizations, "timestamp": datetime.now().isoformat()})
        )
...
class DataHandler(tornado.web.RequestHandler):
    ...
    def _get_summary_data(self):
        ...
        summary = {
            ...
            "total_workers": 0,
            "active_tasks": 0,
            "utilization": {"cpu": 0, "memory": 0, "gpu": 0, "overall": 0},
        }
    ...
    def _get_resource_data(self):
        ...
        return {"resource_history": filtered_history, "timestamp": datetime.now().isoformat()}
    ...
    def _get_scaling_data(self):
        ...
        return {"scaling_history": filtered_history, "timestamp": datetime.now().isoformat()}
    ...
    def _get_worker_data(self):
        ...
        return {"worker_history": result, "timestamp": datetime.now().isoformat()}
    ...
    def _get_cloud_data(self):
        ...
        return {"cloud_history": result, "timestamp": datetime.now().isoformat()}
...
if __name__ == "__main__":
    ...
    visualization = DRMVisualization(
        dynamic_resource_manager=drm, output_dir=args.output_dir, dashboard_port=args.port
    )
