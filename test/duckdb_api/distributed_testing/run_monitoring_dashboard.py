...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitoring Dashboard for Distributed Testing Framework"
    )
    parser.add_argument(
        "--host", type=str, default="localhost", help="Host to bind the dashboard server"
    )
    ...
    parser.add_argument(
        "--auto-open", action="store_true", help="Automatically open the dashboard in a browser"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument(
        "--generate-sample-data", action="store_true", help="Generate sample data for demonstration"
    )
    ...
    for i in range(1, 6):
        ...
        metrics.record_entity(
            ...
            entity_data={
                ...
                "version": "1.0.0",
                "os": "Linux",
                "resource_usage": {"cpu": 20 + i * 10, "memory": 30 + i * 5, "gpu": 10 + i * 15},
                "task_count": i * 2,
            },
            status=status,
        )
