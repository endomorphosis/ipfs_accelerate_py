        ...
        if gpu_info["count"] == 0:
            if platform.system() == "Linux":
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            for i, name in enumerate(gpu_names):
                                device_info = {"id": i, "brand": name, "type": "rocm"}
                                gpu_info["devices"].append(device_info)
                ...
                if platform.processor() == "arm":
                    device_info = {"id": 0, "brand": "Apple Silicon", "type": "mps"}
                    gpu_info["devices"].append(device_info)
                    gpu_info["count"] = 1
                    ...
                    for addr in addrs:
                        if addr.family == socket.AF_INET:  # IPv4
                            interface_info["addresses"].append(
                                {"address": addr.address, "netmask": addr.netmask, "type": "ipv4"}
                            )
                        elif addr.family == socket.AF_INET6:  # IPv6
                            interface_info["addresses"].append(
                                {"address": addr.address, "netmask": addr.netmask, "type": "ipv6"}
                            )
...
class TaskRunner:
                ...
                for name in dir(module):
                    if name.startswith("test_"):
                        ...
                        if callable(func):
                            ...
                            try:
                                result = func()
                                test_results[name] = {"success": True, "result": result}
                            except Exception as e:
    ...
    def _run_command(self, command: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"command": " ".join(command), "error": str(e), "success": False}

    def _simulate_benchmark(
        self, model_name: str, batch_size: int, precision: str, iterations: int
    ) -> Dict[str, Any]:
    ...
    def _get_hardware_metrics(self) -> Dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE:
            try:
                ...
                cpu_load = (
                    [x / 100.0 for x in psutil.getloadavg()]
                ...
                resources["cpu"]["available_cores"] = max(
                    0.1, cpu_count - cpu_load[0]
        ...
        if TORCH_AVAILABLE and torch.cuda.is_available():
            try:
                ...
                for i in range(cuda_device_count):
                    torch_gpu_metrics = {"id": i, "name": torch.cuda.get_device_name(i)}
                    ...
                    if f"device_{i}" not in resources["gpu"] and total_memory_bytes > 0:
                        ...
                        resources["gpu"][f"device_{i}"] = {
                            ...
                            "available_memory_mb": available_memory_mb,
                            "utilization_percent": (allocated_bytes / total_memory_bytes * 100)
...
class WorkerClient:
            ...
            heartbeat_request = {
                ...
                "hardware_metrics": {
                    ...
                    "gpu_utilization": next(
                        (
                            gpu.get("load_percent", 0)
                            for gpu in hardware_metrics.get("gpu_metrics", [])
                        ),
                        0,
                    )
                    if hardware_metrics.get("gpu_metrics")
                    else 0,
                },
            }
    ...
    async def _handle_task_assignment(self, data: Dict[str, Any]):
        ...
        task_thread = threading.Thread(target=self._run_task_thread, args=(task,), daemon=True)
        task_thread.start()
    ...
    async def _update_status(self, status: str) -> bool:
        ...
        try:
            # Send status update
            status_update = {"type": "status_update", "worker_id": self.worker_id, "status": status}
...
def main():
    ...
    parser.add_argument("--coordinator", required=True, help="URL of the coordinator server")
    parser.add_argument("--api-key", required=True, help="API key for authentication")
    parser.add_argument("--worker-id", default=None, help="Worker ID (generated if not provided)")
    parser.add_argument("--work-dir", default=None, help="Working directory for tasks")
    parser.add_argument(
        "--reconnect-interval",
        type=int,
        default=5,
    ...
    parser.add_argument(
        "--heartbeat-interval", type=int, default=30, help="Interval in seconds between heartbeats"
    )
