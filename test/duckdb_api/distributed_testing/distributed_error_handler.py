...
class RecoveryStrategy(abc.ABC):
    ...
    @abc.abstractmethod
    def recover(
        self, task_id: str, error: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    @abc.abstractmethod
    def is_applicable(
        self, error_category: ErrorCategory, hardware_type: Optional[str] = None
    ) -> bool:
    ...
    def recover(
        ...
        if error_category == ErrorCategory.HARDWARE_NOT_AVAILABLE:
            ...
            recovery_result["actions_taken"].append(
                f"mark_hardware_unavailable:{hardware_type}:{worker_id}"
            )
...
class DistributedErrorHandler:
    ...
    def _initialize_retry_policies(self) -> Dict[ErrorCategory, RetryPolicy]:
        ...
        default_policies = {
            ...
            ErrorCategory.RESOURCE_EXHAUSTED: RetryPolicy(
                max_retries=5, retry_delay_seconds=30, retry_backoff_factor=2
            ),
            ErrorCategory.RESOURCE_UNAVAILABLE: RetryPolicy(
                max_retries=3, retry_delay_seconds=60, retry_backoff_factor=2
            ),
            # Network errors - retry with quick backoff
            ErrorCategory.NETWORK_TIMEOUT: RetryPolicy(
                max_retries=5, retry_delay_seconds=10, retry_backoff_factor=1.5
            ),
            ErrorCategory.NETWORK_CONNECTION_ERROR: RetryPolicy(
                max_retries=5, retry_delay_seconds=15, retry_backoff_factor=1.5
            ),
            ErrorCategory.NETWORK_SERVER_ERROR: RetryPolicy(
                max_retries=3, retry_delay_seconds=30, retry_backoff_factor=2
            ),
            # Worker errors - retry on different worker
            ErrorCategory.WORKER_DISCONNECTED: RetryPolicy(
                max_retries=3, retry_delay_seconds=5, retry_backoff_factor=1.5
            ),
            ErrorCategory.WORKER_TIMEOUT: RetryPolicy(
                max_retries=3, retry_delay_seconds=10, retry_backoff_factor=2
            ),
            ErrorCategory.WORKER_CRASHED: RetryPolicy(
                max_retries=3, retry_delay_seconds=15, retry_backoff_factor=2
            ),
            # Hardware errors - retry with different hardware
            ErrorCategory.HARDWARE_NOT_AVAILABLE: RetryPolicy(
                max_retries=3, retry_delay_seconds=10, retry_backoff_factor=1
            ),
            ErrorCategory.HARDWARE_MISMATCH: RetryPolicy(
                max_retries=2, retry_delay_seconds=5, retry_backoff_factor=1
            ),
            ...
            ErrorCategory.TEST_ASSERTION_ERROR: RetryPolicy(max_retries=0),
            ErrorCategory.TEST_SYNTAX_ERROR: RetryPolicy(max_retries=0),
            # Default policy for unknown errors
            ErrorCategory.UNKNOWN: RetryPolicy(
                max_retries=1, retry_delay_seconds=30, retry_backoff_factor=2
            ),
        }
    ...
    def should_retry(
        self, task_id: str, error: Dict[str, Any], attempt_count: int
    ...
    def log_and_report_error(
        self, task_id: str, error: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> None:
