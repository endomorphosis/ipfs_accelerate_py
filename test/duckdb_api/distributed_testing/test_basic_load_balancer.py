...
def assignment_callback(assignment: WorkerAssignment) -> None:
    ...
    if assignment.status == "completed":
        logger.info(
            f"Test {assignment.test_id} completed successfully on worker {assignment.worker_id} "
...
def worker_simulation(
    worker_id: str, load_balancer: LoadBalancerService, stop_event: threading.Event
) -> None:
    ...
    while not stop_event.is_set():
        ...
        if assignment:
            ...
            load.cpu_utilization = max(
                0.0, min(100.0, load.cpu_utilization + random.uniform(-5.0, 5.0))
            )
            load.memory_utilization = max(
                0.0, min(100.0, load.memory_utilization + random.uniform(-3.0, 3.0))
            )
            if "cuda" in capabilities.supported_backends:
                load.gpu_utilization = max(
                    0.0, min(100.0, load.gpu_utilization + random.uniform(-10.0, 10.0))
                )
            load_balancer.update_worker_load(worker_id, load)
...
def main():
    ...
    for i in range(1, 4):  # 3 workers
        ...
        thread = threading.Thread(
            target=worker_simulation, args=(worker_id, load_balancer, stop_event), daemon=True
        )
        thread.start()
    ...
    try:
        ...
        with load_balancer.lock:
            ...
            completed_tests = sum(
                1 for a in load_balancer.test_assignments.values() if a.status == "completed"
            )
            failed_tests = sum(
                1 for a in load_balancer.test_assignments.values() if a.status == "failed"
            )
