...
def generate_test_worker_capabilities(
    worker_id: str, memory_gb: float = 8.0, cuda_devices: int = 1
) -> WorkerCapabilities:
    ...
    return WorkerCapabilities(
        ...
        software_versions={"python": "3.10.0", "torch": "2.0.1"},
...
def create_test_requirements(
    test_id: str, memory_required: float = 2.0, cuda_required: int = 0, priority: int = 3
) -> TestRequirements:
...
def test_assignment_callback(assignment: WorkerAssignment) -> None:
    ...
    if assignment.status == "completed":
        logger.info(
            f"✅ Test {assignment.test_id} completed successfully on {assignment.worker_id}"
        )
    elif assignment.status == "failed":
        if assignment.worker_id == "none":
            logger.warning(
                f"❌ Test {assignment.test_id} could not be scheduled - {assignment.result.get('error', 'Unknown error')}"
            )
        else:
...
def test_requeue_limit():
    ...
    try:
        ...
        test1 = create_test_requirements(
            "gpu_test1", memory_required=2.0, cuda_required=1, priority=1
        )
        test2 = create_test_requirements(
            "gpu_test2", memory_required=2.0, cuda_required=1, priority=2
        )
        test3 = create_test_requirements(
            "memory_test", memory_required=8.0, cuda_required=0, priority=3
        )
