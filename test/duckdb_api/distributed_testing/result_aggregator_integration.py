...
def setup_result_aggregator(db_path: str = None) -> Tuple[ResultAggregatorService, BenchmarkDBAPI]:
    ...
    result_aggregator = ResultAggregatorService(db_manager=db_api, trend_analyzer=trend_analyzer)
...
def generate_performance_report(
    aggregator: ResultAggregatorService, output_dir: str = "reports", format: str = "json"
) -> Dict[str, str]:
    ...
    try:
        ...
        for result_type in [RESULT_TYPE_PERFORMANCE, RESULT_TYPE_WEB_PLATFORM]:
            anomalies = aggregator.get_result_anomalies(
                result_type=result_type, aggregation_level=AGGREGATION_LEVEL_MODEL_HARDWARE
            )
...
def analyze_test_results(aggregator: ResultAggregatorService, test_run_id: str) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument("--test-run", default=None, help="Test run ID to analyze")
    ...
    try:
        if args.test_run:
            ...
            print(
                f"Integration Test Pass Rate: {results['summary']['integration_test_pass_rate']:.1f}%"
            )
            print(
                f"Web Platform Success Rate: {results['summary']['web_platform_success_rate']:.1f}%"
            )
            ...
            report_files = generate_performance_report(
                aggregator, output_dir=args.report_dir, format=args.format
            )
