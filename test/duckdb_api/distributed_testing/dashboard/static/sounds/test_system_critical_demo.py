...
def generate_system_critical_error(error_category, message, worker_id="test-worker"):
    ...
    return {
        ...
        "system_context": {
            ...
            "platform": "linux",
            "metrics": {"cpu": {"percent": 95, "temperature": 82}, "memory": {"used_percent": 98}},
        },
        "hardware_context": {
            "hardware_type": "test",
            "hardware_status": {"overheating": True, "memory_pressure": True},
        },
    }
...
def send_error_event(dashboard_url, error_data):
    ...
    try:
        ...
        response = requests.post(
            endpoint, json=error_data, headers={"Content-Type": "application/json"}, timeout=5
        )
...
def run_error_demo(dashboard_url, interval=5, count=3):
    ...
    for i in range(count):
        ...
        error_data = generate_system_critical_error(
            error_category=error_category, message=message, worker_id=f"test-worker-{i + 1}"
        )
...
def main():
    ...
    parser.add_argument(
        "--count", type=int, default=3, help="Number of error events to send (default: 3)"
    )
