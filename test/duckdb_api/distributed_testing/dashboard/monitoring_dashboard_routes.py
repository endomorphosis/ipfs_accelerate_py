...
try:
    ...
    logger.warning(
        "Circuit Breaker Visualization not available. Circuit breaker monitoring features disabled."
    )
...
async def handle_index(request: web.Request) -> web.Response:
    ...
    if dashboard.enable_visualization_integration and dashboard.visualization_integration:
        try:
            embedded_dashboards = (
                dashboard.visualization_integration.get_embedded_dashboards_for_page("index")
            )
        ...
        for name, dash_details in embedded_dashboards.items():
            try:
                iframe_html = dashboard.visualization_integration.get_dashboard_iframe_html(
                    name=name, width="100%", height="600px"
                )
...
async def handle_results_view(request: web.Request) -> web.Response:
    ...
    if not dashboard.enable_result_aggregator_integration:
        return web.Response(
            text=dashboard.env.get_template("results_disabled.html").render(theme=theme),
            content_type="text/html",
        )
    ...
    if dashboard.enable_visualization_integration and dashboard.visualization_integration:
        try:
            embedded_dashboards = (
                dashboard.visualization_integration.get_embedded_dashboards_for_page("results")
            )
        except Exception as e:
            logger.error(f"Error getting embedded dashboards: {e}")
    ...
    if (
        dashboard.enable_visualization_integration
        and dashboard.visualization_integration
        and not embedded_dashboard_html
        and dashboard.visualization_integration.visualization_available
    ):
        try:
            # Convert result summary to a format suitable for visualization
            performance_data = {
                "metrics": {
                    "throughput_items_per_second": result_summary.get("overall_stats", {}).get(
                        "avg_throughput", 0
                    ),
                    "average_latency_ms": result_summary.get("overall_stats", {}).get(
                        "avg_latency", 0
                    ),
            ...
            dashboard_path = (
                dashboard.visualization_integration.generate_dashboard_from_performance_data(
            ...
            if dashboard_path:
                ...
                if dashboard_details:
                    # Get the iframe HTML
                    iframe_html = dashboard.visualization_integration.get_dashboard_iframe_html(
                        name=dashboard_name, width="100%", height="600px"
                    )
...
async def handle_e2e_test_results(request: web.Request) -> web.Response:
    ...
    if not dashboard.enable_e2e_test_integration:
        return web.Response(
            text=dashboard.env.get_template("e2e_test_results_disabled.html").render(theme=theme),
            content_type="text/html",
        )
...
async def handle_e2e_test_monitoring(request: web.Request) -> web.Response:
    ...
    # Render template
    return web.Response(
        text=dashboard.env.get_template("e2e_test_monitoring.html").render(theme=theme),
        content_type="text/html",
    )
...
async def handle_performance_analytics(request: web.Request) -> web.Response:
    ...
    if dashboard.enable_visualization_integration and dashboard.visualization_integration:
        try:
            embedded_dashboards = (
                dashboard.visualization_integration.get_embedded_dashboards_for_page(
    ...
    # Handle update action
        ...
        try:
            result = dashboard.visualization_integration.update_embedded_dashboard(
                name=name, title=title, description=description, position=position, page=page
            )
...
async def handle_error_visualization(request: web.Request) -> web.Response:
    ...
    if (
        hasattr(dashboard, "error_visualization_integration")
        and dashboard.error_visualization_integration
    ):
        ...
        # Get error data
        error_data = await dashboard.error_visualization_integration.get_error_data(
    ...
    return web.Response(
        text=dashboard.env.get_template("error_visualization.html").render(
            theme=theme, error_data=error_data, time_range=time_range
        ),
        content_type="text/html",
    )
...
async def handle_api_report_error(request: web.Request) -> web.Response:
    ...
    # Check if error visualization integration is available
    if (
        not hasattr(dashboard, "error_visualization_integration")
        or not dashboard.error_visualization_integration
    ):
        return web.json_response(
            {"status": "error", "message": "Error visualization integration not available"},
            status=400,
        )
    ...
    try:
        ...
        if success:
            return web.json_response(
                {"status": "success", "message": "Error reported successfully"}
            )
        else:
            return web.json_response(
                {"status": "error", "message": "Failed to report error"}, status=500
            )
...
async def handle_circuit_breakers(request: web.Request) -> web.Response:
    ...
    if not CIRCUIT_BREAKER_VISUALIZATION_AVAILABLE:
        return web.Response(
            text=dashboard.env.get_template("feature_unavailable.html").render(
                feature="Circuit Breaker Visualization", theme=theme
            ),
            content_type="text/html",
        )
    ...
    if circuit_breaker_dashboard is None:
        try:
            ...
            circuit_breaker_dashboard = CircuitBreakerDashboardIntegration(
                coordinator=coordinator, output_dir=output_dir
            )
            ...
            return web.Response(
                text=dashboard.env.get_template("error.html").render(
                    error=f"Error initializing Circuit Breaker Dashboard: {e}", theme=theme
                ),
                content_type="text/html",
            )
    ...
    try:
        ...
        return web.Response(
            text=dashboard.env.get_template("error.html").render(
                error=f"Error generating Circuit Breaker Dashboard: {e}", theme=theme
            ),
            content_type="text/html",
        )
...
async def handle_api_circuit_breakers(request: web.Request) -> web.Response:
    ...
    if circuit_breaker_dashboard is None:
        return web.json_response({"error": "Circuit Breaker Dashboard Integration not available"})
    ...
    try:
        ...
        return web.json_response(
            {"error": f"Error getting circuit breaker metrics: {e}"}, status=500
        )
...
async def handle_drm_dashboard(request: web.Request) -> web.Response:
    ...
    try:
        from data.duckdb.distributed_testing.dashboard.drm_visualization_integration import (
    ...
    if action == "start-server":
        ...
        try:
            if (
                hasattr(drm_integration.drm_visualization, "dashboard_app")
                and drm_integration.drm_visualization.dashboard_app is not None
            ):
                dashboard_server_running = True
    ...
    resource_heatmap_html = drm_integration.get_iframe_html(
        "resource_heatmap", width="100%", height="450px"
    )
    scaling_history_html = drm_integration.get_iframe_html(
        "scaling_history", width="100%", height="450px"
    )
    resource_allocation_html = drm_integration.get_iframe_html(
        "resource_allocation", width="100%", height="450px"
    )
    resource_efficiency_html = drm_integration.get_iframe_html(
        "resource_efficiency", width="100%", height="450px"
    )
    cloud_resources_html = drm_integration.get_iframe_html(
        "cloud_resources", width="100%", height="450px"
    )
