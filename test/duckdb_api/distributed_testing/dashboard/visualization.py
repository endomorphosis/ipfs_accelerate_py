...
class VisualizationEngine:
    ...
    def create_performance_dashboard(
        ...
        if PLOTLY_AVAILABLE and self.config["interactive"]:
            return self._create_interactive_dashboard(
                data, output_path, dimensions, metrics, time_range, include_regressions
            )

        # Fall back to static dashboard with Matplotlib
        elif MATPLOTLIB_AVAILABLE:
            return self._create_static_dashboard(
                data, output_path, dimensions, metrics, time_range, include_regressions
            )
    ...
    def _create_interactive_dashboard(
        self, data, output_path, dimensions, metrics, time_range, include_regressions
    ):
        ...
        try:
            ...
            <head>
                ...
                + """;
                    ...
                    .dashboard-header {
                        ...
                        padding: 10px;
                        background-color: """
                + ("#f0f0f0" if self.config["theme"] == "light" else "#333333")
                + """;
                        border-radius: 5px;
                    }
                    .dashboard-title {
                    ...
                    .dashboard-timestamp {
                        font-size: 14px;
                        color: """
                + ("#666666" if self.config["theme"] == "light" else "#aaaaaa")
                + """;
                    }
                    .dashboard-summary {
                        display: flex;
                    ...
                    .summary-card {
                        ...
                        padding: 15px;
                        background-color: """
                + ("#ffffff" if self.config["theme"] == "light" else "#333333")
                + """;
                        border-radius: 5px;
                ...
                + ("#dddddd" if self.config["theme"] == "light" else "#444444")
                + """;
                        padding-bottom: 10px;
                    }
                    .chart-container {
            ...
            <body>
                <div class="dashboard-container">
                    <div class="dashboard-header">
                        ...
                        <div class="dashboard-timestamp">Generated: """
            ...
            dashboard_html += (
                ...
                <div class="dashboard-summary">
                    <div class="summary-card">
                        <h3>Test Summary</h3>
                        <p><strong>Test Count:</strong> """
                + str(overall_status.get("test_count", 0))
                + """</p>
                        <p><strong>Total Executions:</strong> """
                + str(overall_status.get("total_executions", 0))
                + """</p>
                        <p><strong>Worker Count:</strong> """
                + str(overall_status.get("worker_count", 0))
                + """</p>
                        <p><strong>Task Type Count:</strong> """
                + str(overall_status.get("task_type_count", 0))
                + """</p>
                    </div>
            ...
            dashboard_html += (
                """
                    ...
                    <div class="summary-card">
                        <h3>Regressions</h3>
                        <p><strong>Regression Count:</strong> """
                + str(overall_status.get("regression_count", 0))
                + """</p>
                        <p><strong>Significant Regressions:</strong> """
                + str(overall_status.get("significant_regression_count", 0))
                + """</p>
            """
            )
                ...
                for test_id, regression_info in regressions.items():
                    ...
                    if regression_info.get("has_significant_regression", False):
                        for metric, info in regression_info.get("metrics", {}).items():
                            if info.get("is_regression", False) and info.get(
                ...
                for test_id, dates in history.items():
                    for date, metrics_data in dates.items():
                        for metric_name in metrics:
                            if metric_name in metrics_data.get("metrics", {}) and isinstance(
                                metrics_data["metrics"][metric_name], dict
                            ):
    ...
    def _create_static_dashboard(
        ...
        try:
            ...
            summary_ax.set_title(
                "Distributed Testing Performance Dashboard", fontsize=16, fontweight="bold"
            )
            ...
            self._plot_dimension_comparison(
                dimension_ax, data, dimensions, metrics[0] if metrics else None
            )
            ...
            plt.savefig(
                output_path,
                format=self.config["static_format"],
                dpi=self.config["dpi"],
                bbox_inches="tight",
    ...
    def _plot_time_series(self, ax, data, metrics, time_range):
        ...
        if not plot_data:
            ax.text(
                0.5, 0.5, f"No historical data found for metric: {metric}", ha="center", va="center"
            )
            return
    ...
    def _plot_regression_analysis(self, ax, data, include_regressions):
            ...
            for metric, info in regression_info.get("metrics", {}).items():
                if info.get("is_regression", False):
                    ...
                    if significant and percent_change > max_percent_change:
                        ...
                        not significant
                        and percent_change > max_percent_change
                        and not max_significant
                    ):
                        max_percent_change = percent_change
                        max_significant = False
        ...
        # Sort by percent change
        sorted_data = sorted(
            zip(test_ids, percent_changes, is_significant), key=lambda x: x[1], reverse=True
        )
        ...
        colors = [
            self.colors["danger"] if sig else self.colors["warning"] for sig in is_significant
        ]
    ...
    def create_regression_report(
        ...
        if data is None and self.result_aggregator:
            data = {
                "regressions": self.result_aggregator.get_regressions(
                    significant_only=not include_non_significant
                ),
                "overall_status": self.result_aggregator.get_overall_status(),
            }
        ...
        try:
            ...
            <head>
                ...
                + """;
                    ...
                    .regression-table tr:hover {
                        background-color: """
                + ("#f5f5f5" if self.config["theme"] == "light" else "#3a3a3a")
                + """;
                    }
                    .significant {
                        color: """
                ...
                + """;
                    ...
                    .regression-details {
                        ...
                        padding: 10px;
                        background-color: """
                + ("#f9f9f9" if self.config["theme"] == "light" else "#2a2a2a")
                + """;
                        border-radius: 5px;
                        display: none;
                    }
            ...
            <body>
                <div class="report-container">
                    <div class="report-header">
                        ...
                        <div class="report-timestamp">Generated: """
                + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                + """</div>
                    </div>
            ...
            significant_count = sum(
                1 for r in regressions.values() if r.get("has_significant_regression", False)
            )
            ...
            for test_id, regression_info in sorted(regressions.items()):
                ...
                for metric, info in metrics.items():
                    if info.get("is_regression", False):
                        ...
                        css_class = (
                            "significant"
                            if info.get("is_significant", False)
    ...
    def create_visualization(
        self, visualization_type: str, data: Dict[str, Any], output_path: Optional[str] = None
    ) -> Optional[str]:
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                self.output_dir, f"{visualization_type}_{timestamp}.{self.config['static_format']}"
            )
    ...
    def _create_time_series_visualization(self, data, output_path):
        ...
        if all(
            ...
            for timestamp, _ in next(iter(time_series_data.values()))
        ):
        ...
        try:
            ...
            ax.annotate(
                ...
                bbox=dict(
                    ...
                    fc="white" if self.config["theme"] == "light" else "#333333",
                    alpha=0.8,
                ),
            )
        except ImportError:
