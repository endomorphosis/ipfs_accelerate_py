...
class CircuitBreakerVisualization:
    ...
    def __init__(self, output_dir: str = "./dashboards/circuit_breakers"):
        ...
        logger.info(
            f"Circuit breaker visualization initialized with output directory: {output_dir}"
        )

    def _generate_circuit_state_indicators(
        ...
        """
        indicators = {"workers": [], "tasks": [], "endpoints": [], "browsers": []}
    ...
    def _generate_global_health_gauge(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"figure": gauge_json, "value": global_health}
    ...
    def _generate_state_distribution_chart(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"figure": chart_json, "counts": state_counts}
    ...
    def _generate_failure_rate_chart(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        failure_rates = {"workers": [], "tasks": [], "endpoints": [], "browsers": []}
        ...
        for browser_id, browser_metrics in metrics.get("browser_circuits", {}).items():
            total_successes = browser_metrics.get(
                "success_count", 0
            )  # different field name in browser metrics
            total_failures = browser_metrics.get(
                "failure_count", 0
            )  # different field name in browser metrics
            total_calls = total_successes + total_failures
        ...
        if worker_ids:
            fig.add_trace(
                go.Bar(
                    x=worker_ids, y=worker_rates, name="Worker Failure Rates", marker_color="blue"
                ),
                row=1,
                col=1,
            )
        ...
        return {"figure": chart_json, "failure_rates": failure_rates}
    ...
    def _update_history(self, metrics: Dict[str, Any]) -> None:
        ...
        worker_healths = [
            w.get("health_percentage", 100.0) for w in metrics.get("worker_circuits", {}).values()
        ]
        if worker_healths:
        ...
        task_healths = [
            t.get("health_percentage", 100.0) for t in metrics.get("task_circuits", {}).values()
        ]
        if task_healths:
        ...
        endpoint_healths = [
            e.get("health_percentage", 100.0) for e in metrics.get("endpoint_circuits", {}).values()
        ]
        if endpoint_healths:
    ...
    def _generate_history_chart(self) -> Dict[str, Any]:
        ...
        fig.add_trace(
            go.Scatter(
                x=timestamps, y=half_open_counts, name="Half-Open", line=dict(color="yellow")
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=timestamps, y=global_health, name="Global Health", line=dict(color="blue")
            ),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=timestamps, y=worker_health, name="Worker Health", line=dict(color="purple")
            ),
            row=2,
            col=1,
        )
