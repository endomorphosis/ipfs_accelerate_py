...
class RegressionDetector:
    ...
    def detect_regressions(
        ...
        if (
            ...
            or "timestamps" not in time_series_data
            or "values" not in time_series_data
        ):
        ...
        if len(values) < self.config["min_samples"]:
            logger.warning(
                f"Insufficient data points for regression detection. Need at least {self.config['min_samples']}."
            )
            return []

        # Get metric configuration
        metric_config = self.config["metrics_config"].get(
            metric,
            {
        ...
        if not change_points or len(change_points) == 0:
            ...
            if len(values) > 2 * self.config["window_size"]:
                ...
                regression = self._analyze_segment_pair(
                    start_segment, end_segment, timestamps[0], timestamps[-1], metric, metric_config
                )
    ...
    def _detect_change_points(self, values: List[float]) -> List[int]:
        ...
        if RUPTURES_AVAILABLE:
            try:
                ...
                algo = rpt.Pelt(
                    model=self.config["change_point_model"],
    ...
    def _analyze_segment_pair(
        self,
        before_segment: List[float],
        after_segment: List[float],
        before_time,
        after_time,
        metric: str,
        ...
        if not is_regression and not (
            self.config["allow_positive_regressions"]
            and abs(percentage_change) > self.config["regression_threshold"]
        ):
            return None
    ...
    def create_regression_visualization(
        ...
        # Get metric configuration
        metric_config = self.config["metrics_config"].get(
            metric, {"display_name": metric, "unit": "", "higher_is_better": False}
        )

        # Apply smoothing if needed
        smoothed_values = (
            self._apply_smoothing(values) if self.config["smoothing_factor"] > 0 else None
        )
    ...
    def generate_regression_report(
        ...
        for metric, regressions in regressions_by_metric.items():
            ...
            significant_regressions = [
                ...
                if r.get("is_significant", False) and r.get("is_regression", False)
            ]
    ...
    def _generate_correlation_insights(self, corr_matrix) -> List[str]:
        ...
        if (
            "latency_ms" in corr_matrix.columns
            and "throughput_items_per_second" in corr_matrix.columns
        ):
            corr = corr_matrix.loc["latency_ms", "throughput_items_per_second"]
            if corr < -0.5:
                insights.append(
                    f"Expected negative correlation ({corr:.2f}) between latency and throughput: lower latency correlates with higher throughput"
                )
