...
class WebSocketManager:
        ...
        try:
            async for msg in ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    logger.error(
                        f"WebSocket connection {conn_id} closed with error: {ws.exception()}"
                    )
        finally:
    ...
    async def handle_message(self, conn_id: int, message: str):
        ...
        try:
            ...
            if msg_type == "subscribe":
                ...
                logger.info(
                    f"Error visualization monitoring initialized for time range {time_range}"
                )

                # Return confirmation
                await self.send_to_connection(
                    conn_id,
                    {"type": "error_visualization_init_confirmed", "time_range": time_range},
                )
    ...
    async def subscribe(self, conn_id: int, topic: str):
        ...
        # Send confirmation
        await self.send_to_connection(conn_id, {"type": "subscription_confirmed", "topic": topic})
    ...
    async def unsubscribe(self, conn_id: int, topic: str):
        ...
        # Send confirmation
        await self.send_to_connection(conn_id, {"type": "unsubscription_confirmed", "topic": topic})
