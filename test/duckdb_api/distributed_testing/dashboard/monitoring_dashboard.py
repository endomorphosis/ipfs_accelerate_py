...
from data.duckdb.distributed_testing.dashboard.websocket_handlers import (
    WebSocketManager,
...
class MonitoringDashboard:
    ...
    def __init__(
                ...
                if not os.path.exists(dashboards_static_dir):
                    try:
                        ...
                        target_path = os.path.relpath(
                            self.dashboard_dir, os.path.dirname(dashboards_static_dir)
                        )
        ...
        logger.info(
            f"Result Aggregator Integration: {'Enabled' if enable_result_aggregator_integration else 'Disabled'}"
        )
        logger.info(
            f"E2E Test Integration: {'Enabled' if enable_e2e_test_integration else 'Disabled'}"
        )
        logger.info(
            f"Visualization Dashboard Integration: {'Enabled' if enable_visualization_integration else 'Disabled'}"
        )
    ...
    async def get_coordinator_status(self) -> Dict[str, Any]:
        ...
        if not self.coordinator_url:
            return {"status": "Unknown", "error": "Coordinator URL not configured"}
    ...
    async def get_workers_status(self) -> Dict[str, Any]:
        ...
        """
        if not self.coordinator_url:
            return {"status": "Unknown", "error": "Coordinator URL not configured", "workers": []}
    ...
    async def get_tasks_status(self) -> Dict[str, Any]:
        ...
        """
        if not self.coordinator_url:
            return {"status": "Unknown", "error": "Coordinator URL not configured", "tasks": []}
    ...
    async def get_task_history(self) -> Dict[str, Any]:
        ...
        """
        if not self.coordinator_url:
            return {"status": "Unknown", "error": "Coordinator URL not configured", "history": []}
