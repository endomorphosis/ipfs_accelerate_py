...
class ErrorVisualizationIntegration:
    ...
    def __init__(
        ...
        logger.info(
            f"Error Visualization Integration initialized with output directory: {output_dir}"
        )
    ...
    async def get_error_data(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"summary": None, "timestamp": datetime.now().isoformat(), "error": str(e)}

    async def _get_error_data_from_db(
    ...
    async def _get_error_data_from_coordinator(
        self, start_time: datetime, end_time: datetime
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    f"{self.coordinator_url}/api/errors", params=params
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        logger.info(
                            f"Retrieved {len(data['errors'])} error records from coordinator API"
                        )
                        return data["errors"]
                    else:
    ...
    async def _get_error_data_from_files(
        self, start_time: datetime, end_time: datetime
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for log_dir in log_dirs:
                ...
                worker_logs = [
                    ...
                    if f.startswith("worker-") and f.endswith(".log")
                ]
        ...
        try:
            ...
            if self.websocket_manager:
                for time_range in time_ranges:
                    ...
                    await self.websocket_manager.broadcast(
                        ...
                        message={
                            ...
                            "data": {"error": display_error, "time_range": time_range},
                        },
                    )
                ...
                await self.websocket_manager.broadcast(
                    ...
                    message={
                        "type": "error_visualization_update",
                        "data": {"error": display_error},
                    },
                )

                logger.info(
                    f"Error broadcast to WebSocket clients: {error_data.get('error_type', 'Unknown error')}"
                )
    ...
    async def _store_error_in_db(self, error_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Error stored in database: {error_data.get('error_type', 'Unknown error')}"
            )
            return True
        except Exception as e:
    ...
    async def _process_error_data(
        self, error_data: List[Dict[str, Any]], time_range_hours: int
    ) -> Dict[str, Any]:
        ...
        """
        if not error_data:
            return {"summary": None, "timestamp": datetime.now().isoformat()}
        ...
        resource_errors = sum(
            error_categories.get(cat, 0)
            for cat in ["RESOURCE_EXHAUSTED", "RESOURCE_UNAVAILABLE", "RESOURCE_NOT_FOUND"]
        )
        network_errors = sum(
            error_categories.get(cat, 0)
            for cat in ["NETWORK_CONNECTION_ERROR", "NETWORK_TIMEOUT", "NETWORK_SERVER_ERROR"]
        )
        hardware_errors = sum(
            error_categories.get(cat, 0)
            for cat in [
        ...
        critical_hardware_errors = sum(
            1
            for e in error_data
            if e.get("error_category")
            in ["HARDWARE_NOT_AVAILABLE", "HARDWARE_MISMATCH", "HARDWARE_COMPATIBILITY_ERROR"]
        ...
        result["recent_errors"] = [
            self._prepare_error_for_display(e)
            for e in sorted(error_data, key=lambda x: x.get("timestamp", ""), reverse=True)[:100]
        ]
    ...
    def _is_critical_error(self, error: Dict[str, Any]) -> bool:
        ...
        critical_categories = ["HARDWARE_NOT_AVAILABLE", "RESOURCE_EXHAUSTED", "WORKER_CRASHED"]
    ...
    async def _generate_error_distribution(
        ...
        try:
            ...
            if not has_plotly:
                return {"chart_data": {"data": [], "layout": {}}, "categories": categories_dict}
            ...
            chart_json = json.dumps(
                {"data": fig.data, "layout": fig.layout}, cls=plotly.utils.PlotlyJSONEncoder
            )

            return {"chart_data": json.loads(chart_json), "categories": categories_dict}
        except Exception as e:
            logger.error(f"Error generating error distribution: {e}")
            logger.error(traceback.format_exc())
            return {"chart_data": {"data": [], "layout": {}}, "categories": []}
    ...
    async def _generate_error_patterns(self, error_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern, indices in message_groups.items():
                ...
                first_seen = (
                    min(pattern_timestamps).strftime("%Y-%m-%d %H:%M:%S")
                    if pattern_timestamps
                    else "Unknown"
                )
                last_seen = (
                    max(pattern_timestamps).strftime("%Y-%m-%d %H:%M:%S")
                    if pattern_timestamps
                    else "Unknown"
                )
            ...
            if has_pandas:
                df_patterns = pd.DataFrame(
                    {"Category": category_names, "Patterns": category_values}
                )
                ...
                fig = go.Figure(
                    data=[go.Pie(labels=category_names, values=category_values, hole=0.3)]
                )

            fig.update_layout(
                title="Error Pattern Distribution by Category", template="plotly_white"
            )
            ...
            logger.error(traceback.format_exc())
            return {"chart_data": {"data": [], "layout": {}}, "top_patterns": []}
    ...
    async def _generate_worker_error_analysis(
        self, error_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Generate worker error analysis.
        ...
        try:
            ...
            for worker_id, errors in worker_errors.items():
                ...
                try:
                    timestamps = [
                        datetime.fromisoformat(e["timestamp"]) for e in errors if "timestamp" in e
                    ]
                    if timestamps:
            ...
            # If plotly is not available, return simplified data
            if not has_plotly:
                return {"chart_data": {"data": [], "layout": {}}, "worker_stats": worker_stats}
            ...
            if has_pandas:
                ...
                fig.add_trace(
                    go.Bar(
                        x=worker_ids, y=total_errors, name="Total Errors", marker_color="lightblue"
                    )
                )
                ...
                fig.add_trace(
                    go.Bar(
                        x=worker_ids, y=critical_errors, name="Critical Errors", marker_color="red"
                    )
                )
            ...
            return {"chart_data": json.loads(chart_json), "worker_stats": worker_stats}
        except Exception as e:
            logger.error(f"Error generating worker error analysis: {e}")
            logger.error(traceback.format_exc())
            return {"chart_data": {"data": [], "layout": {}}, "worker_stats": []}

    async def _generate_hardware_error_analysis(
        self, error_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Generate hardware error analysis.
        ...
        try:
            ...
            for error in sorted(
                hardware_errors, key=lambda x: x.get("timestamp", ""), reverse=True
            )[:50]:
                # Format for display
            ...
            color_map = {"Critical": "red", "Warning": "orange", "Stable": "green"}
