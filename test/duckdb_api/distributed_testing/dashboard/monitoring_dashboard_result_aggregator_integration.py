...
class ResultAggregatorIntegration:
    ...
    def get_performance_summary(self, time_range_days=7) -> Dict[str, Any]:
        ...
        try:
            ...
            total_tests = sum(group_stats.get("result_count", 0) for group_stats in stats.values())
        ...
        try:
            ...
            for group_key, group_data in distributions.items():
                ...
                if "is_compatible" in group_data:
                    ...
                    true_count = (
                        is_compatible_data.get("distribution", {}).get("True", {}).get("count", 0)
                    )
        ...
        try:
            ...
            if test_stats["total_tests_run"] > 0:
                test_stats["pass_rate"] = (
                    test_stats["tests_passed"] / test_stats["total_tests_run"]
                ) * 100
        ...
        try:
            ...
            for platform_browser, platform_data in distributions.items():
                ...
                if "success" in platform_data:
                    ...
                    success_count = (
                        success_data.get("distribution", {}).get("True", {}).get("count", 0)
                    )
                    ...
                    web_stats["platform_browser_pairs"][platform_browser] = {
                        ...
                        "failed": fail_count,
                        "success_rate": (success_count / total_values) * 100
                        if total_values > 0
                        else 0,
                        "platform": platform,
                        "browser": browser,
            ...
            if web_stats["total_tests_run"] > 0:
                web_stats["success_rate"] = (
                    web_stats["successful_tests"] / web_stats["total_tests_run"]
                ) * 100
    ...
    def create_performance_trend_chart(
        self, time_range_days=7, metric="average_latency_ms", top_n=8
    ) -> Optional[str]:
        """
        ...
        output_path = os.path.join(
            self.visualizations_dir, f"performance_trend_{metric}_{timestamp}.html"
        )
        ...
        try:
            ...
            for pair, _ in top_pairs:
                ...
                if ":" in pair:
                    ...
                    filter_params = {"model_id": model_id, "hardware_id": hardware_id}
            ...
            fig.update_layout(
                ...
                font=dict(color=color_scheme["text_color"]),
                xaxis=dict(gridcolor=color_scheme["grid_color"], tickformat="%Y-%m-%d"),
                yaxis=dict(gridcolor=color_scheme["grid_color"]),
                hovermode="closest",
            )
            ...
            if "latency" in metric or "time" in metric:
                fig.update_layout(
                    yaxis=dict(autorange="reversed", gridcolor=color_scheme["grid_color"])
                )
    ...
    def create_compatibility_matrix_chart(self, time_range_days=7) -> Optional[str]:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(
            self.visualizations_dir, f"compatibility_matrix_{timestamp}.html"
        )
        ...
        try:
            ...
            if len(models) > max_items:
                ...
                sorted_models = sorted(
                    models,
                    key=lambda m: compatibility_stats["model_counts"][m].get(
            ...
            if len(hardware) > max_items:
                ...
                sorted_hardware = sorted(
                    hardware,
                    key=lambda h: compatibility_stats["hardware_counts"][h].get(
                        "compatibility_rate", 0
                    ),
                    reverse=True,
                )
            ...
            for h_idx, hw in enumerate(hardware):
                ...
                for m_idx, model in enumerate(models):
                    ...
                    filter_params = {"model_id": model, "hardware_id": hw}
            ...
            fig.update_layout(
                ...
                plot_bgcolor=color_scheme["background"],
                font=dict(color=color_scheme["text_color"]),
                xaxis=dict(title="Models", gridcolor=color_scheme["grid_color"], tickangle=-45),
                yaxis=dict(title="Hardware", gridcolor=color_scheme["grid_color"]),
            )
    ...
    def create_test_pass_rate_chart(self, time_range_days=7) -> Optional[str]:
        ...
        # Define output file path
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(self.visualizations_dir, f"test_pass_rate_{timestamp}.html")
        ...
        try:
            ...
            sorted_modules = sorted(
                module_results.items(), key=lambda x: x[1]["pass_rate"], reverse=True
            )
            ...
            fig.update_layout(
                ...
                xaxis=dict(
                    title="Test Modules", tickangle=-45, gridcolor=color_scheme["grid_color"]
                ),
                yaxis=dict(
                    title="Pass Rate (%)",
                    range=[0, 100],
    ...
    def create_web_platform_browser_chart(self, time_range_days=7) -> Optional[str]:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(
            self.visualizations_dir, f"web_platform_browsers_{timestamp}.html"
        )
        ...
        try:
            ...
            fig = make_subplots(
                rows=1,
                cols=2,
                subplot_titles=("Browser Success Rates", "Platform Success Rates"),
            ...
            # Update x-axis
            fig.update_xaxes(
                title="Browsers", tickangle=-45, gridcolor=color_scheme["grid_color"], row=1, col=1
            )

            fig.update_xaxes(
                title="Platforms", tickangle=-45, gridcolor=color_scheme["grid_color"], row=1, col=2
            )
    ...
    def create_anomaly_chart(self, time_range_days=7) -> Optional[str]:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(
            self.visualizations_dir, f"performance_anomalies_{timestamp}.html"
        )
        ...
        try:
            ...
            for i, metric in enumerate(metrics):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                ...
                                size=12,
                                symbol="circle",
                                line=dict(color="red" if "bad" in severities else "green", width=2),
                            ),
                            hovertext=hover_text,
                            hoverinfo="text",
                        ),
                ...
                fig.update_yaxes(
                    title=metric, gridcolor=color_scheme["grid_color"], row=i + 1, col=1
                )
    ...
    def create_performance_comparison_chart(self, time_range_days=7) -> Optional[str]:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(
            self.visualizations_dir, f"performance_comparison_{timestamp}.html"
        )
        ...
        try:
            ...
            for i, metric in enumerate(metrics):
                ...
                sorted_data = sorted(
                    metric_data, key=lambda x: abs(x.get("pct_change_mean", 0)), reverse=True
                )
    ...
    def create_dashboard_result_summary(self, time_range_days=7) -> Dict[str, Any]:
        ...
        try:
            ...
            perf_trend_chart = self.create_performance_trend_chart(
                time_range_days=time_range_days, metric="average_latency_ms"
            )
            ...
            test_pass_chart = self.create_test_pass_rate_chart(time_range_days=time_range_days)

            web_platform_chart = self.create_web_platform_browser_chart(
                time_range_days=time_range_days
            )

            anomaly_chart = self.create_anomaly_chart(time_range_days=time_range_days)
            ...
            dashboard_summary = {
                ...
                "overall_stats": {
                    ...
                    "total_model_hardware_pairs": performance_summary.get(
                        "total_model_hardware_pairs", 0
                    ),
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="./monitoring_dashboard", help="Output directory for visualizations"
    )
    parser.add_argument(
        "--db-path", default="./benchmark_db.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument(
        "--time-range", type=int, default=7, help="Time range in days for visualizations"
    )
    parser.add_argument(
        "--theme", choices=["light", "dark"], default="dark", help="Theme for visualizations"
    )
    ...
    integration = ResultAggregatorIntegration(
        result_aggregator=result_aggregator, output_dir=args.output_dir
    )

    # Configure integration
    integration.configure({"theme": args.theme})
    ...
    if "error" in summary:
        ...
        print(
            f"Integration test pass rate: {summary['overall_stats']['integration_pass_rate']:.1f}%"
        )
        print(
            f"Web platform success rate: {summary['overall_stats']['web_platform_success_rate']:.1f}%"
        )
