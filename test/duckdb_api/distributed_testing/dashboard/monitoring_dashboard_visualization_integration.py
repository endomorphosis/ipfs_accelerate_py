...
class VisualizationDashboardIntegration:
        ...
        if self.visualization_available:
            try:
                self.dashboard_instance = CustomizableDashboard(
                    db_connection=None, output_dir=self.dashboard_dir
                )
    ...
    def _load_registry(self):
        ...
        if os.path.exists(self.registry_file):
            try:
                ...
                logger.info(
                    f"Loaded {len(self.embedded_dashboards)} embedded dashboards from registry"
                )
            except Exception as e:
    ...
    def get_embedded_dashboards_for_page(self, page: str) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: details
            for name, details in self.embedded_dashboards.items()
        ...
        try:
            ...
            if title is not None or description is not None:
                dashboard_path = self.dashboard_instance.update_dashboard(
                    dashboard_name=name, title=title, description=description
                )
                ...
                if dashboard_path:
                    ...
                    current_details["updated_at"] = self.dashboard_instance.dashboard_config.get(
                        "updated_at", ""
                    )
    ...
    def get_dashboard_iframe_html(
        self, name: str, width: str = "100%", height: str = "600px"
    ) -> str:
    ...
    def generate_dashboard_from_performance_data(
        self,
