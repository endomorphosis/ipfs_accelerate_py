...
class DRMRealTimeDashboard:
    ...
    def __init__(
        ...
        self.output_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "static", "drm_visualizations"
        )
    ...
    def start(self):
        ...
        try:
            ...
            self.dashboard_app.run_server(debug=self.debug, port=self.port, use_reloader=False)
    ...
    def start_in_background(self):
        ...
        logger.info(
            f"DRM Real-Time Dashboard running in background at http://localhost:{self.port}"
        )
        return True
    ...
    def _collect_drm_metrics(self):
        ...
        try:
            ...
            self.resource_metrics["memory_utilization"].append(
                overall_utilization.get("memory", 0) * 100
            )
            ...
            for worker_id, worker_data in worker_stats.get("workers", {}).items():
                ...
                self.worker_metrics[worker_id]["cpu_utilization"].append(
                    utilization.get("cpu", 0) * 100
                )
                self.worker_metrics[worker_id]["memory_utilization"].append(
                    utilization.get("memory", 0) * 100
                )
                self.worker_metrics[worker_id]["gpu_utilization"].append(
                    utilization.get("gpu", 0) * 100
                )
            ...
            if hasattr(self.drm, "get_performance_metrics"):
                ...
                if "task_throughput" in perf_metrics:
                    self.performance_metrics["task_throughput"].append(
                        {"timestamp": current_time, "value": perf_metrics["task_throughput"]}
                    )

                if "allocation_time" in perf_metrics:
                    self.performance_metrics["allocation_time"].append(
                        {"timestamp": current_time, "value": perf_metrics["allocation_time"]}
                    )

                if "resource_efficiency" in perf_metrics:
                    self.performance_metrics["resource_efficiency"].append(
                        {"timestamp": current_time, "value": perf_metrics["resource_efficiency"]}
                    )
            ...
            if hasattr(self.drm, "last_scaling_decision") and self.drm.last_scaling_decision:
                ...
                if (
                    not self.scaling_decisions
                    or last_decision != self.scaling_decisions[-1]["decision"]
                ):
                    self.scaling_decisions.append(
                        {
                            ...
                            if hasattr(last_decision, "action")
                            else last_decision.get("action", "unknown"),
                            "reason": last_decision.reason
                            if hasattr(last_decision, "reason")
                            else last_decision.get("reason", ""),
                            "count": last_decision.count
                            if hasattr(last_decision, "count")
                            else last_decision.get("count", 0),
                        }
                    )
        ...
        if keep_idx > 0:
            ...
            for worker_id in self.worker_metrics:
                for key in self.worker_metrics[worker_id]:
                    ...
                    self.worker_metrics[worker_id][key] = self.worker_metrics[worker_id][key][
                        w_keep_idx:
                    ]
            ...
            self.scaling_decisions = [
                entry for entry in self.scaling_decisions if entry["timestamp"] >= cutoff_time
            ]
    ...
    def _detect_performance_regressions(self):
        ...
        if (
            not REGRESSION_AVAILABLE
            or not self.regression_detector
            or not self.resource_metrics["timestamps"]
        ):
            return
        ...
        try:
            ...
            if len(self.performance_metrics["allocation_time"]) >= 10:
                allocation_timestamps = [
                    entry["timestamp"] for entry in self.performance_metrics["allocation_time"]
                ]
                allocation_values = [
                    entry["value"] for entry in self.performance_metrics["allocation_time"]
                ]

                regressions = self.regression_detector.detect_regressions(
                    {"timestamps": allocation_timestamps, "values": allocation_values},
                    "allocation_time_ms",
                )

                # Process significant regressions
                for regression in regressions:
                    if regression.get("is_significant", False) and regression.get(
                        "is_regression", False
                    ):
                        # Create an alert for significant regressions
                        self._create_alert(
                    ...
                    html.A(
                        dbc.Row(
                            [
                                ...
                                dbc.Col(
                                    dbc.NavbarBrand("DRM Real-Time Dashboard", className="ms-2")
                                ),
                            ],
                            align="center",
                            className="g-0",
                            ...
                            [
                                ...
                                dbc.Card(
                                    [
                                        ...
                                        dbc.CardBody([html.Div(id="regression-analysis-content")]),
                                    ]
                                ),
                            ]
                        ),
                            ...
                            [
                                ...
                                dbc.Card(
                                    [
                                        ...
                                        dbc.CardBody([html.Div(id="scaling-decisions-table")]),
                                    ]
                                ),
                            ]
                        ),
                                                    ...
                                                    options=[
                                                        {
                                                            ...
                                                            "value": "scaling_decision",
                                                        },
                                                        {
                                                            "label": "Regression Detection",
                                                            "value": "regression_detection",
                                                        },
                            ...
                            [
                                ...
                                html.P(
                                    [
                                        ...
                                        html.Span(
                                            f"Data retention: {self.retention_window} minutes • "
                                        ),
                                        html.Span(
    ...
    def _register_dashboard_callbacks(self):
        ...
        def update_data_store(n_intervals):
            ...
            serialized_data = {
                "resource_metrics": {
                    key: [
                        ts.isoformat() if isinstance(ts, datetime.datetime) else ts
                        for ts in self.resource_metrics["timestamps"]
                    ]
                    if key == "timestamps"
        ...
        def update_utilization(data):
            ...
            if len(data["resource_metrics"]["cpu_utilization"]) > 1:
                ...
                if diff > 1:
                    ...
                    trend_class = (
                        "card-text text-center mt-2 text-danger"  # Higher utilization is a concern
                    )
                elif diff < -1:
                    trend = f"↓ Decreased by {-diff:.1f}%"
                    trend_class = (
                        "card-text text-center mt-2 text-success"  # Lower utilization is good
                    )
                else:
                    trend = "→ Stable"
                    trend_class = "card-text text-center mt-2 text-muted"
        ...
        def update_alert_count(data):
            ...
            # Count by severity
            warning_count = sum(
                1 for a in data["alerts"] if a["level"] in ["warning", "error", "critical"]
            )
        ...
        @app.callback(
            Output("resource-utilization-graph", "figure"), Input("drm-data-store", "data")
        )
        def update_resource_utilization_graph(data):
            ...
            # Parse timestamps
            timestamps = [
                datetime.datetime.fromisoformat(ts) for ts in data["resource_metrics"]["timestamps"]
            ]
        ...
        @app.callback(Output("worker-task-graph", "figure"), Input("drm-data-store", "data"))
        ...
        @app.callback(Output("worker-selector", "options"), Input("drm-data-store", "data"))
        def update_worker_selector(data):
            ...
            options = [
                {"label": f"Worker {worker_id}", "value": worker_id}
        ...
        @app.callback(
            Output("worker-graph-title", "children"), Input("worker-metric-selector", "value")
        )
        ...
        def update_worker_metrics_graph(data, selected_workers, metric):
            ...
            if (
                not data
                or not data["worker_metrics"]
                or not selected_workers
                or not selected_workers
            ):
            ...
            for worker_id in selected_workers:
                if worker_id in data["worker_metrics"]:
                    ...
                    # Parse timestamps
                    timestamps = [
                        datetime.datetime.fromisoformat(ts) for ts in worker_data["timestamps"]
                    ]
        ...
        def update_worker_heatmap(data, metric):
            ...
            timestamp_labels = [
                datetime.datetime.fromisoformat(ts).strftime("%H:%M:%S") for ts in all_timestamps
            ]
            ...
            fig = go.Figure(
                data=go.Heatmap(
                    ...
                    colorscale="Viridis",
                    colorbar=dict(title="% Utilization" if metric != "tasks" else "Task Count"),
        ...
        def update_performance_metrics_graph(data, selected_metrics, time_range):
            ...
            for i, metric in enumerate(selected_metrics):
                if metric in data["performance_metrics"] and data["performance_metrics"][metric]:
                    ...
                    filtered_data = [
                        entry
                        for entry in data["performance_metrics"][metric]
                    ...
                    if i == 0:  # Primary axis
                        fig.update_yaxes(
                            title_text=display_names.get(metric, metric), secondary_y=False
                        )
                    elif i == 1:  # Secondary axis
                        fig.update_yaxes(
                            title_text=display_names.get(metric, metric), secondary_y=True
                        )
        ...
        @app.callback(
            Output("regression-analysis-content", "children"), Input("drm-data-store", "data")
        )
        def update_regression_analysis(data):
            ...
            if not REGRESSION_AVAILABLE:
                return html.Div(
                    [
                        html.P(
                            "Regression detection module not available. Install required dependencies to enable this feature."
                        ),
            ...
            if not data or not data["resource_metrics"]["timestamps"]:
                return html.Div(
                    "No data available for regression analysis.", className="text-muted p-3"
                )

            # Check if we have enough data for analysis
            if len(data["resource_metrics"]["timestamps"]) < 10:
                return html.Div(
                    "Collecting data for regression analysis (minimum 10 data points required)...",
                    className="text-muted p-3",
                )
            ...
            return html.Div(
                [
                    html.P(
                        "Performance metrics are being analyzed for statistical regressions.",
                        className="mb-3",
                    ),
                    dbc.Row(
                        [
                            ...
                            dbc.Col(
                                [
                                    dbc.Card(
                                        [
                                            dbc.CardHeader(
                                                "Memory Utilization", className="fw-bold"
                                            ),
                                            dbc.CardBody(
                                                [
        ...
        @app.callback(Output("scaling-timeline-graph", "figure"), Input("drm-data-store", "data"))
        def update_scaling_timeline(data):
            ...
            # Parse timestamps
            timestamps = [
                datetime.datetime.fromisoformat(d["timestamp"]) for d in data["scaling_decisions"]
            ]
            ...
            if data["resource_metrics"]["timestamps"]:
                ...
                resource_ts = [
                    ...
                    for ts in data["resource_metrics"]["timestamps"]
                ]
            ...
            fig.update_layout(
                ...
                xaxis_title="Time",
                yaxis=dict(title="Worker Count Change", side="left"),
                yaxis2=dict(title="Utilization (%)", side="right", overlaying="y", range=[0, 100]),
                template="plotly_dark" if self.theme == "dark" else "plotly_white",
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                margin=dict(l=20, r=20, t=40, b=20),
                height=400,
        ...
        @app.callback(
            Output("scaling-decisions-table", "children"), Input("drm-data-store", "data")
        )
        def update_scaling_decisions_table(data):
            ...
            sorted_decisions = sorted(
                data["scaling_decisions"], key=lambda x: x["timestamp"], reverse=True
            )

            # Create rows for each decision
            rows = []
            for decision in sorted_decisions:
                timestamp = datetime.datetime.fromisoformat(decision["timestamp"]).strftime(
                    "%H:%M:%S"
                )
                action = decision["action"]
                count = decision["count"]
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Div(
                                                [
                                                    html.Small(
                                                        f"Time: {timestamp}", className="me-3"
                                                    ),
                                                    html.Small(
...
def main():
    ...
    parser.add_argument(
        "--retention", type=int, default=60, help="Data retention window in minutes"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument(
        "--theme", choices=["light", "dark"], default="dark", help="Dashboard theme"
    )
