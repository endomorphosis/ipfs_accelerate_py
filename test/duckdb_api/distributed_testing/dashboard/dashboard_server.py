...
class DashboardServer:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_static(
            "/static", os.path.join(self.output_dir, "static"), name="static"
        )
        self.app.router.add_static(
            "/visualizations",
...
"""
        ...
        try:
            ...
            if self.result_aggregator:
                ...
                if viz_type == "time_series":
                    ...
                    time_series = self.result_aggregator.get_time_series(
                        entity_id, entity_type, metric, days
                    )
    ...
    async def handle_api_regressions(self, request):
        ...
        # Get regression data
        if self.result_aggregator:
            regression_data = self.result_aggregator.get_regressions(
                significant_only=significant_only
            )
        else:
            # Use static data for testing
            ...
            if significant_only:
                regression_data = {
                    k: v
                    for k, v in regression_data.items()
    ...
    async def handle_api_dimensions(self, request):
        ...
        if self.result_aggregator:
            ...
            dimension_data = (
                all_dimensions.get(dimension, all_dimensions) if dimension else all_dimensions
            )
    ...
    async def handle_api_performance(self, request):
        ...
        for t_id, dates in performance_data.items():
            ...
            filtered_dates = {d: v for d, v in dates.items() if d >= cutoff_date}
    ...
    async def handle_websocket(self, request):
        ...
        try:
            ...
            await ws.send_json(
                {"type": "status", "data": status_data, "timestamp": datetime.now().isoformat()}
            )
        ...
        return {
            ...
            "historical_performance": {
                "test1": {
                    "2025-03-01": {
                        "metrics": {
                            "throughput": {"mean": 165.2, "std": 5.1, "count": 10},
                            "latency": {"mean": 28.3, "std": 2.1, "count": 10},
                        },
                        "count": 10,
                    },
                    "2025-03-02": {
                        "metrics": {
                            "throughput": {"mean": 166.5, "std": 4.8, "count": 10},
                            "latency": {"mean": 27.9, "std": 1.9, "count": 10},
                        },
                        "count": 10,
                    },
                    "2025-03-10": {
                        "metrics": {
                            "throughput": {"mean": 142.1, "std": 5.3, "count": 10},
                            "latency": {"mean": 32.5, "std": 2.4, "count": 10},
                        },
                        "count": 10,
                    },
                    "2025-03-11": {
                        "metrics": {
                            "throughput": {"mean": 138.7, "std": 5.7, "count": 10},
                            "latency": {"mean": 33.1, "std": 2.6, "count": 10},
                        },
                        "count": 10,
                    },
                }
    ...
    def get_static_viz_data(self, viz_type: str) -> Dict[str, Any]:
        ...
        if viz_type == "time_series":
            ...
            return {
                "time_series": {"test1": list(zip(dates, values))},
                "metric": "throughput",
            ...
            return {
                "dimension": "hardware",
                "metric": "throughput",
                "values": {"gpu1": 156.7, "gpu2": 142.3, "cpu": 72.5},
            ...
            return {
                ...
                "metric": "throughput",
                "values": [[156.7, 142.3, 135.6], [142.3, 135.6, 120.8], [135.6, 120.8, 110.3]],
...
def main():
    parser = argparse.ArgumentParser(
        description="Dashboard Server for Distributed Testing Framework"
    )
    ...
    # Configure server
    server.configure({"theme": args.theme})
