...
class PrometheusExporter:
    ...
    def _initialize_metrics(self):
        ...
        self.metrics["cpu_utilization"] = Gauge(
            "drm_cpu_utilization_percent", "CPU utilization percentage across all workers"
        )

        self.metrics["memory_utilization"] = Gauge(
            "drm_memory_utilization_percent", "Memory utilization percentage across all workers"
        )

        self.metrics["gpu_utilization"] = Gauge(
            "drm_gpu_utilization_percent", "GPU utilization percentage across all workers"
        )

        # Worker metrics
        self.metrics["worker_count"] = Gauge("drm_worker_count", "Total number of workers")

        self.metrics["active_workers"] = Gauge("drm_active_workers", "Number of active workers")

        # Task metrics
        self.metrics["active_tasks"] = Gauge("drm_active_tasks", "Number of active tasks")

        self.metrics["pending_tasks"] = Gauge("drm_pending_tasks", "Number of pending tasks")
        ...
        self.metrics["task_throughput"] = Gauge(
            "drm_task_throughput_per_second", "Task throughput in tasks per second"
        )

        self.metrics["allocation_time"] = Gauge(
            "drm_allocation_time_milliseconds", "Resource allocation time in milliseconds"
        )

        self.metrics["resource_efficiency"] = Gauge(
            "drm_resource_efficiency_percent", "Resource efficiency percentage"
        )
        ...
        self.metrics["alerts"] = Counter(
            "drm_alerts_total", "Total number of alerts", ["level", "source"]
        )

        # Worker-specific metrics using labels
        self.metrics["worker_cpu_utilization"] = Gauge(
            "drm_worker_cpu_utilization_percent", "Worker CPU utilization percentage", ["worker_id"]
        )
        ...
        self.metrics["worker_gpu_utilization"] = Gauge(
            "drm_worker_gpu_utilization_percent", "Worker GPU utilization percentage", ["worker_id"]
        )

        self.metrics["worker_tasks"] = Gauge(
            "drm_worker_tasks", "Number of tasks on worker", ["worker_id"]
        )
        ...
        try:
            ...
            if "resource_metrics" in drm_metrics:
                ...
                if resource_metrics.get("memory_utilization"):
                    self.metrics["memory_utilization"].set(
                        resource_metrics["memory_utilization"][-1]
                    )
            ...
            if "performance_metrics" in drm_metrics:
                ...
                if (
                    performance_metrics.get("task_throughput")
                    and performance_metrics["task_throughput"]
                ):
                ...
                if (
                    performance_metrics.get("allocation_time")
                    and performance_metrics["allocation_time"]
                ):
                ...
                if (
                    performance_metrics.get("resource_efficiency")
                    and performance_metrics["resource_efficiency"]
                ):
            ...
            if "scaling_decisions" in drm_metrics and drm_metrics["scaling_decisions"]:
                ...
                self.metrics["scaling_operations"].labels(
                    direction=latest_decision["action"], reason=latest_decision["reason"]
                ).inc()
            ...
            if "alerts" in drm_metrics and drm_metrics["alerts"]:
                ...
                self.metrics["alerts"].labels(
                    level=latest_alert["level"], source=latest_alert["source"]
                ).inc()
...
class GrafanaDashboardGenerator:
        ...
        dashboard["panels"].append(
            {
                ...
                "renderer": "flot",
                "seriesOverrides": [
                    {"alias": "Task Throughput", "yaxis": 1},
                    {"alias": "Allocation Time", "yaxis": 2},
                ],
                "spaceLength": 10,
                "stack": False,
                "steppedLine": False,
    ...
    def save_dashboard(
        self, dashboard: Dict[str, Any], filename: str = "drm_dashboard.json"
    ) -> str:
        """
    ...
    def upload_dashboard(
        self, dashboard: Dict[str, Any], grafana_url: str, api_key: str, folder_id: int = 0
    ) -> bool:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"Dashboard uploaded to Grafana successfully: {response.json().get('url')}"
                )
                return True
            else:
                logger.error(
        ...
        try:
            ...
            if self.drm_dashboard:
                ...
                self.update_thread = threading.Thread(
                    target=self._update_metrics_loop, args=(update_interval,), daemon=True
                )
        ...
        try:
            ...
            if self.grafana_api_key:
                self.grafana_generator.upload_dashboard(
                    dashboard=dashboard, grafana_url=self.grafana_url, api_key=self.grafana_api_key
                )
                ...
                external_links = dbc.DropdownMenu(
                    children=[
                        dbc.DropdownMenuItem(
                            "Prometheus Metrics", href=prometheus_url, target="_blank"
                        ),
                        dbc.DropdownMenuItem(
                            "Grafana Dashboard", href=grafana_url, target="_blank"
                        ),
                    ],
                    nav=True,
                    in_navbar=True,
...
def main():
    ...
    parser.add_argument(
        "--metrics-port", type=int, default=9100, help="Port to expose Prometheus metrics on"
    )
    parser.add_argument(
        "--prometheus-url", default="http://localhost:9090", help="URL of the Prometheus server"
    )
    parser.add_argument(
        "--grafana-url", default="http://localhost:3000", help="URL of the Grafana server"
    )
    ...
    parser.add_argument(
        "--export-only",
        action="store_true",
        help="Only export dashboard, don't start metrics server",
    )
    parser.add_argument(
        "--update-interval", type=int, default=5, help="Metrics update interval in seconds"
    )
