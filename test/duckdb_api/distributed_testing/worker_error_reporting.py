...
class EnhancedErrorReporter:
    ...
    def categorize_error(self, error: Dict[str, Any]) -> ErrorCategory:
        ...
        if any(
            term in error_message.lower()
            for term in ["timeout", "timed out", "deadline exceeded", "request timed out"]
        ):
                ...
                if torch.cuda.is_available():
                    ...
                    for i in range(gpu_count):
                        ...
                        try:
                            ...
                            free_mem = torch.cuda.memory_reserved(i) - torch.cuda.memory_allocated(
                                i
                            )
                            ...
                            device_metrics["memory_utilization"] = round(
                                (used_mem / mem_info) * 100, 2
                            )
                        except Exception as e:
    ...
    def create_enhanced_error_report(
        self, error_type: str, message: str, task_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Created enhanced error report for {error_type} error (category: {error_category})"
        )
        return error_info
                ...
                try:
                    ...
                    for gpu in gpus:
                        if gpu.temperature > 85:
                            logger.warning(
                                f"GPU overheating detected: {gpu.name} at {gpu.temperature}°C"
                            )
                            return True
                except ImportError:
                    pass
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        for i in range(torch.cuda.device_count()):
                            ...
                            if usage_percent > 90:
                                logger.warning(
                                    f"GPU memory pressure detected: Device {i} at {usage_percent:.1f}% used"
                                )
                                return True
                except ImportError:
                    pass
            ...
            if hasattr(psutil, "cpu_freq"):
                ...
                if freq and hasattr(freq, "current") and hasattr(freq, "max"):
                    ...
                    if freq.current < (freq.max * 0.7):
                        logger.warning(
                            f"CPU throttling detected: {freq.current}MHz vs {freq.max}MHz max"
                        )
                        return True
...
def integrate_error_reporting(worker):
    ...
    worker.error_reporter = EnhancedErrorReporter(
        worker_id=worker.worker_id, capabilities=worker.capabilities
    )
    ...
    async def enhanced_report_task_error(task_id, error):
        ...
        if isinstance(error, dict):
            ...
            enhanced_error = worker.error_reporter.create_enhanced_error_report(
                error_type="TaskError", message=str(error), task_id=task_id
            )
