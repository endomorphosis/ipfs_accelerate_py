...
class MockDynamicResourceManager:
    ...
    def __init__(self):
        ...
        self.cloud_resources = {
            "aws": {"instances": random.randint(5, 15), "cost_per_hour": 0.5},
            "gcp": {"instances": random.randint(3, 10), "cost_per_hour": 0.6},
            "azure": {"instances": random.randint(2, 8), "cost_per_hour": 0.55},
        }
    ...
    def get_worker_resources(self):
        ...
        """
        return {
            worker_id: worker_data["resources"] for worker_id, worker_data in self.workers.items()
        }
    ...
    def get_worker_statistics(self):
        ...
        if total_workers > 0:
            cpu_util = (
                sum(worker["utilization"]["cpu"] for worker in self.workers.values())
                / total_workers
            )
            memory_util = (
                sum(worker["utilization"]["memory"] for worker in self.workers.values())
                / total_workers
            )
            ...
            gpu_util = (
                sum(worker["utilization"]["gpu"] for worker in gpu_workers) / len(gpu_workers)
                if gpu_workers
                else 0
            )

            disk_util = (
                sum(worker["utilization"]["disk"] for worker in self.workers.values())
                / total_workers
            )
    ...
    def get_resource_utilization(self):
        ...
        """
        return {
            worker_id: worker_data["utilization"] for worker_id, worker_data in self.workers.items()
        }
    ...
    def get_resource_efficiency(self):
        ...
        """
        return {worker_id: random.uniform(0.5, 0.95) for worker_id in self.workers.keys()}
    ...
    def update_worker_utilization(self):
        ...
        for worker_id, worker_data in self.workers.items():
            ...
            # Add randomness
            cpu_change = direction * random.uniform(0, 0.05) + random.uniform(
                -self.utilization_variability, self.utilization_variability
            )
            memory_change = direction * random.uniform(0, 0.04) + random.uniform(
                -self.utilization_variability, self.utilization_variability
            )
            gpu_change = direction * random.uniform(0, 0.06) + random.uniform(
                -self.utilization_variability, self.utilization_variability
            )
            disk_change = direction * random.uniform(0, 0.02) + random.uniform(
                -self.utilization_variability, self.utilization_variability
            )

            # Update utilization
            worker_data["utilization"]["cpu"] = max(
                0.1, min(0.95, worker_data["utilization"]["cpu"] + cpu_change)
            )
            worker_data["utilization"]["memory"] = max(
                0.1, min(0.95, worker_data["utilization"]["memory"] + memory_change)
            )

            # Only update GPU if the worker has GPU
            if worker_data["resources"].get("gpu", 0) > 0:
                worker_data["utilization"]["gpu"] = max(
                    0.1, min(0.95, worker_data["utilization"]["gpu"] + gpu_change)
                )

            worker_data["utilization"]["disk"] = max(
                0.05, min(0.9, worker_data["utilization"]["disk"] + disk_change)
            )
    ...
    def evaluate_scaling(self):
        ...
        # Record decision
        self.scaling_history.append({"timestamp": datetime.now(), "decision": decision})
...
def main():
    ...
    integration = DRMVisualizationIntegration(
        output_dir=output_dir, update_interval=args.update_interval, resource_manager=drm
    )
    ...
    try:
        ...
        while time.time() < end_time:
            ...
            logger.info(
                f"Update {update_count + 1} (Elapsed: {elapsed_minutes:.1f} min, Remaining: {remaining_minutes:.1f} min)"
            )
