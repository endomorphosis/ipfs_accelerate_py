...
def plot_single_test_results(results: Dict[str, Any], output_dir: str) -> None:
    ...
    if "time_series" in results and "throughput" in results["time_series"]:
        ...
        if throughput_data:
            ...
            fig.tight_layout()
            fig.savefig(
                os.path.join(output_dir, f"throughput_time_series_{timestamp}.png"), dpi=300
            )
            plt.close(fig)
    ...
    ax3 = axs[1, 0]
    metrics_to_plot = [
        "worker_assignment_stddev",
        "worker_assignment_range",
    ...
    bars3 = ax3.bar(
        ["Assignment StdDev", "Assignment Range", "Avg Scheduling Attempts"], values, color=colors
    )
    ...
    workers_data = [
        worker_count,
        final_worker_count,
        worker_count * metrics.get("worker_utilization", 0),
    ]

    bars4 = ax4.bar(
        ["Initial Workers", "Final Workers", "Effective Workers"],
        workers_data,
        color=["#009688", "#607D8B", "#8BC34A"],
    )
...
def plot_benchmark_results(results: Dict[str, Any], output_dir: str) -> None:
    ...
    for i, workers in enumerate(worker_counts):
        for j, tests in enumerate(test_counts):
            ...
            if mask.any():
                worker_util[j, i] = (
                    df.loc[mask, "worker_utilization"].values[0] * 100
    ...
    for i in range(len(test_counts)):
        for j in range(len(worker_counts)):
            text = ax.text(
                j,
                i,
                f"{worker_util[i, j]:.1f}%",
                ha="center",
                va="center",
                color="black" if worker_util[i, j] < 70 else "white",
            )
    ...
    for test_count in test_counts:
        ...
        if not test_data.empty:
            ...
            ax.plot(
                test_data["workers"],
                test_data["avg_latency"],
                marker="o",
                linewidth=2,
                label=f"{test_count} tests",
            )
    ...
    for test_count in test_counts:
        ...
        if not test_data.empty:
            ...
            ax.plot(
                test_data["workers"],
                test_data["throughput_per_worker"],
                marker="o",
                linewidth=2,
                label=f"{test_count} tests",
            )
...
def plot_spike_simulation_results(results: Dict[str, Any], output_dir: str) -> None:
    ...
    if "throughput" in time_series and time_series["throughput"]:
        ...
        ax.axhline(
            y=avg_throughput, color="r", linestyle="--", label=f"Avg: {avg_throughput:.1f} tests/s"
        )
    ...
    if (
        "throughput" in time_series
        and time_series["throughput"]
        and "latency" in time_series
        and time_series["latency"]
    ):
        throughput_data = time_series["throughput"]
        latency_data = time_series["latency"]
        ...
        if common_times:
            ...
            fig.tight_layout()
            fig.suptitle(
                f"Throughput vs Latency During Load Spike\n{config.get('initial_workers', 'N/A')} initial workers, {config.get('tests', 'N/A')} tests"
            )

            # Add vertical lines for correlation points (where throughput increases and latency also changes)
            if len(times) > 5:
                throughput_changes = [
                    throughputs[i + 1] - throughputs[i] for i in range(len(throughputs) - 1)
                ]
                latency_changes = [
                    latencies[i + 1] - latencies[i] for i in range(len(latencies) - 1)
                ]
    ...
    if (
        "throughput" in time_series
        and len(time_series["throughput"]) > 10
        and "latency" in time_series
        and len(time_series["latency"]) > 10
    ):
        # Extract data
        ...
        if len(common_times) > 10:
            ...
            # Add title
            ax.set_title(
                f"Performance Heat Map Over Time\n{config.get('initial_workers', 'N/A')} initial workers, {config.get('tests', 'N/A')} tests"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "-t",
        type=str,
        choices=["stress", "benchmark", "spike"],
