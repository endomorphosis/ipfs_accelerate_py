...
class BenchmarkDBConverter:
            ...
            for test in data["tests"]:
                ...
                for hw_type, hw_data in test.get("compatibility", {}).items():
                    entry = {
                        ...
                        "is_compatible": hw_data.get(
                            "is_compatible", hw_data.get("compatible", False)
                        ),
    ...
    def convert_directory(
        self, input_dir: str, categories: List[str] = None
    ...
    def save_to_parquet(
        self, dataframes: Dict[str, pd.DataFrame], output_dir: str = "./benchmark_parquet"
    ) -> bool:
        """
    ...
    def consolidate_directories(
        self, directories: List[str], categories: List[str] = None
    ) -> Dict[str, pd.DataFrame]:
        """
    ...
    def deduplicate_data(self, dataframes: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for category, df in dataframes.items():
            ...
            logger.info(
                f"Deduplicated {category} data from {len(dataframes[category])} to {len(df)} rows"
            )
            result_dfs[category] = df
...
def main():
    ...
    parser.add_argument("--input-dir", help="Directory containing JSON benchmark files")
    parser.add_argument(
        "--output-db", default="./benchmark_db.duckdb", help="Output DuckDB database path"
    )
    parser.add_argument(
        "--output-parquet-dir",
        default="./benchmark_parquet",
    ...
    parser.add_argument(
        "--consolidate", action="store_true", help="Consolidate data from multiple directories"
    )
    parser.add_argument(
        "--deduplicate", action="store_true", help="Deduplicate data, keeping the latest version"
    )
    parser.add_argument(
        "--directories", nargs="+", help="Directories to consolidate when using --consolidate"
    )
