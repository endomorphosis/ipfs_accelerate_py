...
class StaleReportCleaner:
    ...
    def __init__(
        self,
        search_dir: str = ".",
        archive_dir: str = "./archived_reports",
        debug: bool = False,
        dry_run: bool = False,
    ):
        """
                ...
                if file_type == "markdown":
                    ...
                    try:
                        ...
                        return (
                            ...
                            and data.get("warning") == self.warning_templates["json"]["warning"]
                        )
            ...
            if file_type == "markdown":
                ...
                if "<body" in content:
                    ...
                    if body_end_match:
                        ...
                        logger.warning(
                            f"Could not find where body tag ends in HTML file: {file_path}"
                        )
                        return False
                else:
                    # Just add at the top
...
"""
            ...
            if "class " in content:
                ...
                if class_matches:
                    ...
                    # Find all 'def ' lines
                    method_matches = list(
                        re.finditer(r"    def\s+\w+", content[last_class_match.start() :])
                    )
                    ...
                    if method_matches:
                        ...
                        next_method_match = re.search(
                            r"    def\s+\w+", content[last_method_start + 1 :]
                        )
                        next_class_match = re.search(
                            r"class\s+\w+", content[last_method_start + 1 :]
                        )

                        if next_method_match and (
                            not next_class_match
                            or next_method_match.start() < next_class_match.start()
                        ):
                        ...
                        new_content = (
                            content[:insert_pos] + validation_function + content[insert_pos:]
                        )
                        ...
                        if "generate_report" in content:
                            ...
                            if report_match:
                                ...
                                data_processing_match = re.search(
                                    r"(data|df|results)\s*=", content[report_method_start:]
                                )
                                if data_processing_match:
                                    data_processing_line_end = (
                                        content[report_method_start:].find(
                                            "\n", data_processing_match.end()
                                        )
                                        + report_method_start
                                    )
                                    ...
                                    new_content = (
                                        new_content[:data_processing_line_end]
                                        + validation_call
                                        + new_content[data_processing_line_end:]
                                    )
        ...
        for script_path in report_scripts:
            try:
                with open(script_path, "r", encoding="utf-8") as f:
                    ...
                    if re.search(
                        r"(generate|create|build)_(report|visualization|chart)", content
                    ) or re.search(r"(to_(html|markdown|md|csv|json)|savefig)", content):
...
def main():
    ...
    parser.add_argument("--search-dir", default=".", help="Directory to search for reports")
    parser.add_argument(
        "--archive-dir", default="./archived_reports", help="Directory to move archived reports to"
    )
    parser.add_argument(
        "--scan", action="store_true", help="Scan for reports but don't modify them"
    )
    parser.add_argument("--mark", action="store_true", help="Mark reports as potentially stale")
    parser.add_argument("--archive", action="store_true", help="Archive reports after marking them")
    parser.add_argument(
        "--fix-report-py",
        action="store_true",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't actually modify or move files"
    )
    ...
    if args.scan:
        ...
        logger.info(
            f"Found {total_count} total reports, marked {marked_count} as potentially stale"
        )
