...
class IncrementalBenchmarkRunner:
    ...
    def identify_missing_benchmarks(
        self, models: List[str] = None, hardware: List[str] = None, batch_sizes: List[int] = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if models:
                ...
                for _, row in model_df.iterrows():
                    result = conn.execute(
                        "SELECT COUNT(*) FROM models WHERE model_name = ?", [row["model_name"]]
                    ).fetchone()[0]
                ...
                model_df = conn.execute("SELECT model_id, model_name FROM models").fetch_df()
            ...
            if hardware:
                ...
                for _, row in hardware_df.iterrows():
                    ...
                    if result == 0:
                        ...
                        max_id = conn.execute(
                            "SELECT COALESCE(MAX(hardware_id), 0) FROM hardware_platforms"
                        ).fetchone()[0]
                        next_id = max_id + 1
            ...
            if existing_df.empty:
                logger.info(
                    f"No existing benchmarks found. All {len(all_df)} combinations are missing."
                )
                return all_df
            ...
            logger.info(
                f"Found {len(missing_df)} missing benchmark configurations out of {len(all_df)} total."
            )
            return missing_df
    ...
    def identify_outdated_benchmarks(
        self,
        ...
        try:
            ...
            logger.info(
                f"Found {len(outdated_df)} outdated benchmark configurations (older than {older_than_days} days)."
            )
            return outdated_df
    ...
    def identify_priority_benchmarks(
        self,
        ...
        if not priority_hardware:
            priority_hardware = ["cpu", "cuda", "rocm", "openvino", "webgpu"]
        ...
        missing_df = self.identify_missing_benchmarks(
            models=priority_models, hardware=priority_hardware, batch_sizes=batch_sizes
        )
        ...
        priority_df = combined_df.drop_duplicates(
            subset=["model_id", "model_name", "hardware_id", "hardware_type", "batch_size"]
        )
    ...
    def run_benchmarks(self, benchmarks_df: pd.DataFrame) -> bool:
        ...
        for (model, hardware), batch_sizes in grouped_benchmarks.items():
            ...
            logger.info(
                f"Running benchmarks for {model} on {hardware} with batch sizes {batch_sizes_str}"
            )
...
def main():
    ...
    parser.add_argument(
        "--models", type=str, help="Comma-separated list of model names to benchmark"
    )
    parser.add_argument(
        "--hardware", type=str, help="Comma-separated list of hardware types to benchmark"
    )
    parser.add_argument(
        "--batch-sizes",
        type=str,
    ...
    parser.add_argument(
        "--missing-only", action="store_true", help="Only run benchmarks for missing configurations"
    )
    parser.add_argument(
        "--refresh-older-than",
        type=int,
    ...
    parser.add_argument(
        "--output", type=str, help="Output file for benchmark configurations (CSV format)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.priority_only:
        benchmarks_df = runner.identify_priority_benchmarks(
            priority_models=models, priority_hardware=hardware, batch_sizes=batch_sizes
        )
    elif args.missing_only:
        benchmarks_df = runner.identify_missing_benchmarks(
            models=models, hardware=hardware, batch_sizes=batch_sizes
        )
    else:
        # Combine missing and outdated benchmarks
        ...
        # Remove duplicates if any
        benchmarks_df = benchmarks_df.drop_duplicates(
            subset=["model_id", "model_name", "hardware_id", "hardware_type", "batch_size"]
        )
    ...
    if not args.dry_run:
        ...
        for _, row in benchmarks_df.iterrows():
            logger.info(
                f"Model: {row['model_name']}, Hardware: {row['hardware_type']}, Batch Size: {row['batch_size']}"
            )
