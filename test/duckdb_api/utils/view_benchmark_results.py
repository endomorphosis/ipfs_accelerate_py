...
class BenchmarkResultViewer:
    ...
    def generate_markdown_report(
        ...
        results = self.get_benchmark_results(
            model_name=model_name, hardware_type=hardware_type, latest_only=True
        )
        ...
        if not results.empty:
            ...
            results_for_md["throughput_items_per_second"] = results_for_md[
...
def main():
    ...
    parser.add_argument("--hardware", type=str, help="Filter by hardware type")
    parser.add_argument("--batch-size", type=int, help="Filter by batch size")
    parser.add_argument(
        "--simulated-only", action="store_true", help="Only include simulated results"
    )
    parser.add_argument("--real-only", action="store_true", help="Only include real results")
    parser.add_argument(
        "--latest-only",
        action="store_true",
        default=True,
    ...
    if args.check_simulation:
        ...
        if not exists:
            print(
                "Run 'python -m duckdb_api.schema.update_db_schema_for_simulation' to add simulation tracking."
            )
        return
