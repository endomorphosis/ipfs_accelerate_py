...
class BenchmarkDBMaintenance:
    ...
    def clean_json_files(
        self, json_dir: str = "./benchmark_results", older_than_days: int = 30
    ) -> int:
        """
        ...
        logger.info(
            f"Cleaned up {deleted_count} old JSON files (older than {older_than_days} days)"
        )
        return deleted_count
    ...
    def get_database_stats(self) -> Dict:
        ...
        """
        stats = {"file_size_mb": 0, "table_counts": {}, "total_rows": 0, "last_modified": None}
...
def main():
    ...
    parser.add_argument(
        "--optimize-db", action="store_true", help="Optimize the database (vacuum and analyze)"
    )
    ...
    parser.add_argument("--backup-compress", action="store_true", help="Compress database backups")
    parser.add_argument("--purge-backups", action="store_true", help="Purge old database backups")
    parser.add_argument(
        "--backup-retention", type=int, default=30, help="Number of days to retain backups"
    )
    parser.add_argument(
        "--clean-json", action="store_true", help="Clean up old JSON benchmark files"
    )
    parser.add_argument(
        "--json-dir", default="./benchmark_results", help="Directory containing JSON files"
    )
    parser.add_argument(
        "--older-than", type=int, default=30, help="Only clean files older than this many days"
    )
    parser.add_argument(
        "--migration-stats", action="store_true", help="Generate migration statistics report"
    )
    parser.add_argument("--output", default=None, help="Output file for migration statistics")
    ...
    if args.backup:
        backup_path = maintenance.backup_db(
            backup_dir=args.backup_dir, compress=args.backup_compress
        )
        if not backup_path:
            sys.exit(1)
    ...
    if args.purge_backups:
        maintenance.purge_old_backups(
            backup_dir=args.backup_dir, retention_days=args.backup_retention
        )

    if args.clean_json:
        maintenance.clean_json_files(json_dir=args.json_dir, older_than_days=args.older_than)
