...
class SimulationDetection:
    ...
    def add_simulation_columns(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Successfully added simulation columns and hardware_availability_log table."
            )
            return True
    ...
    def mark_hardware_as_simulated(self, hardware_type: str, reason: str) -> int:
        ...
        try:
            ...
            logger.info(
                f"Marked {count_result} benchmark results for {hardware_type} as simulated: {reason}"
            )
            return count_result
            ...
            if runner:
                ...
                try:
                    ...
                    summary = runner.run_benchmarks(
                        model_names=[model], hardware_types=[hardware], batch_sizes=batch_sizes_list
                    )
                    ...
                    logger.info(
                        f"Total benchmarks: {total}, Successful: {summary.get('successful', 0)}, Failed: {failed}"
                    )
                ...
                cmd = [
                    sys.executable,
                    "-m",
                    "duckdb_api.core.run_benchmark_with_db",
                    "--model",
                    model,
                    "--hardware",
                    hardware,
    ...
    def generate_simulation_report(self, output_file: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            # Get total counts
            total_results = conn.execute("SELECT COUNT(*) FROM performance_results").fetchone()[0]
            ...
            report = {
                ...
                "simulation_percentage": (total_simulated / total_results * 100)
                if total_results > 0
                else 0,
...
def main():
    ...
    parser.add_argument(
        "--get-simulated", action="store_true", help="Get all simulated benchmark results"
    )
    parser.add_argument(
        "--get-availability", action="store_true", help="Get hardware availability log"
    )
    parser.add_argument(
        "--mark-result", type=int, help="Mark a benchmark result as simulated by result_id"
    )
    parser.add_argument(
        "--mark-hardware",
        type=str,
    ...
    parser.add_argument("--mark-available", type=str, help="Mark a hardware type as available")
    parser.add_argument("--reason", type=str, help="Reason for simulation")
    parser.add_argument(
        "--report", action="store_true", help="Generate a report on simulated benchmark results"
    )
    parser.add_argument(
        "--run-benchmarks", type=str, help="Path to CSV file with benchmark configurations to run"
    )
    parser.add_argument("--output", type=str, help="Output file for report or simulated results")
    parser.add_argument(
        "--format",
    ...
    if args.update_schema:
        ...
        if not availability_df.empty:
            if args.output:
                ...
                logger.info(
                    f"Wrote {len(availability_df)} availability log entries to {args.output}"
                )
            else:
        ...
        if not report and not args.output:
            ...
            print(
                f"Simulated Results: {report['total_simulated']} ({report['simulation_percentage']:.2f}%)"
            )
        ...
        try:
            ...
            if missing_columns:
                logger.error(
                    f"Missing required columns in benchmark configuration file: {', '.join(missing_columns)}"
                )
                sys.exit(1)
