...
class Visualization3D(BaseVisualization):
                    ...
                    for hw in hw_list:
                        ...
                        for batch in batch_sizes:
                            for precision in precision_options:
                                ...
                                model_size_base = (
                                    1000 if model in ["LLAMA", "Stable-Diffusion"] else 300
                                )
                                ...
                                rows.append(
                                    {
                                        ...
                                        / (power + 0.1),  # Throughput per watt
                                        "is_simulated": np.random.random()
                                        > 0.7,  # Random simulation flag
                                    }
                                )
    ...
    def _create_interactive_3d(
        ...
        try:
            ...
            if (
                viz_3d_config.get("enable_animation")
                and viz_3d_config.get("animation_frame") in df.columns
            ):
                ...
                return self._create_3d_surface_plot(
                    df, x_column, y_column, z_column, color_by, output_path, title, viz_3d_config
                )
    ...
    def _create_3d_scatter_plot(
        ...
        if color_by and color_by in df.columns:
            ...
            if unique_values <= 10 or pd.api.types.is_string_dtype(df[color_by]):
                ...
                if len(categories) <= 10:
                    ...
                    color_palette = plotly.colors.sample_colorscale(colorscale, len(categories))
                    ...
                    fig.add_trace(
                        go.Scatter3d(
                            ...
                            marker=dict(
                                ...
                                color=color,
                                line=dict(width=viz_3d_config["marker_line_width"], color="white"),
                ...
                fig.add_trace(
                    go.Scatter3d(
                        ...
                        marker=dict(
                            ...
                            colorbar=dict(title=self._get_metric_label(color_by)),
                            line=dict(width=viz_3d_config["marker_line_width"], color="white"),
                            opacity=viz_3d_config["marker_opacity"],
                        ),
                        name=color_by,
        ...
        if viz_3d_config.get("auto_rotate"):
            ...
            for t in np.linspace(0, 2 * np.pi, 60):
                ...
                eye_z = 0.8
                frames.append(
                    go.Frame(layout=dict(scene_camera_eye=dict(x=eye_x, y=eye_y, z=eye_z)))
                )
            fig.frames = frames
    ...
    def _create_3d_surface_plot(
        self, df, x_column, y_column, z_column, color_by, output_path, title, viz_3d_config
    ):
        """
        ...
        try:
            ...
            fig = go.Figure(
                data=[
                    go.Surface(
                        ...
                        colorbar=dict(title=self._get_metric_label(z_column), thickness=20),
                        hoverinfo="x+y+z",
            ...
            logger.warning(
                "SciPy is required for surface plotting. Install with: pip install scipy"
            )
    ...
    def _create_animated_3d_plot(
        ...
        try:
            ...
            fig.layout.updatemenus[0].buttons[0].args[1]["frame"]["duration"] = (
                viz_3d_config.get("animation_speed", 1) * 1000
            )
