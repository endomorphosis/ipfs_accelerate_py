...
class HardwareHeatmapVisualization(BaseVisualization):
        ...
        **kwargs,
        ...
        if PLOTLY_AVAILABLE:
            return self._create_interactive_heatmap(
                df, metric, metric_column, metric_title, output_path, title, heatmap_config
            )
    ...
    def _create_interactive_heatmap(
        self, df, metric, metric_column, metric_title, output_path, title, heatmap_config
    ):
        """
        ...
        try:
            ...
            for i, family in enumerate(model_families):
                ...
                if len(models) > heatmap_config["max_rows_per_family"]:
                    logger.warning(
                        f"Too many models in family {family}, showing top {heatmap_config['max_rows_per_family']}"
                    )
                ...
                for j, model in enumerate(models):
                    ...
                    for k, hw in enumerate(hardware_types):
                        ...
                        if len(hw_df) > 0:
                            ...
                            if (
                                heatmap_config["mark_simulated"]
                                and "is_simulated" in hw_df.columns
                                and hw_df["is_simulated"].values[0]
                            ):
                ...
                fig.add_trace(
                    go.Heatmap(
                        ...
                        colorbar=dict(
                            title=metric_title
                            if heatmap_config["colorbar_title"] == ""
                            else heatmap_config["colorbar_title"],
            ...
            total_rows = sum(
                len(df[df["model_family"] == family]["model_name"].unique())
                            ...
                            if len(hw_df) > 0:
                                ...
                                if not np.isnan(value):
                                    fig.add_annotation(
                                        ...
                                        font=dict(
                                            color="black"
                                            if value < 0.7 * np.nanmax(family_df[metric_column])
                                            else "white"
                                        ),
                                        xref=f"x{i + 1}" if i > 0 else "x",
                                        yref=f"y{i + 1}" if i > 0 else "y",
            ...
            fig.update_xaxes(title="Hardware Platform", row=len(model_families), col=1)
    ...
    def _create_static_heatmap(
        ...
        try:
            ...
            fig_height = max(
                6, total_rows * 0.4 + len(model_families) * 1.0
            )  # Base height + rows + space for titles

            # Create figure with subplots
            fig, axes = plt.subplots(
                len(model_families), 1, figsize=(10, fig_height), squeeze=False
            )
                        ...
                        for k in range(len(hardware_types)):
                            if simulated_mask[j, k]:
                                ax.text(
                                    k,
                                    j,
                                    heatmap_config["simulated_marker"],
                                    ha="right",
                                    va="top",
                                    color="white",
                                    fontsize=8,
                                    bbox=dict(
                                        boxstyle="round,pad=0.1", fc="rgba(0,0,0,0.5)", ec="none"
                                    ),
                                )
            ...
            if heatmap_config["mark_simulated"] and np.any(simulated_mask):
                fig.text(
                    0.95,
                    0.02,
                    f"{heatmap_config['simulated_marker']} = Simulated Result",
                    ha="right",
                    va="bottom",
                    fontsize=8,
