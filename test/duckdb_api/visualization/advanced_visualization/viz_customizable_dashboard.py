...
class CustomizableDashboard(BaseVisualization):
    ...
    def list_available_components(self):
        ...
        return {
            comp_type: info["description"] for comp_type, info in self.component_registry.items()
        }
    ...
    def create_dashboard(
        ...
        if template is not None:
            if template not in self.dashboard_templates:
                raise ValueError(
                    f"Template '{template}' not found. Available templates: {list(self.dashboard_templates.keys())}"
                )

            template_config = self.dashboard_templates[template]
            self.title = template_config["title"] if title is None else title
            self.description = (
                template_config["description"] if description is None else description
            )
            self.layout["columns"] = template_config["columns"] if columns is None else columns
            self.layout["row_height"] = (
                template_config["row_height"] if row_height is None else row_height
            )
            self.components = (
                template_config["components"].copy() if components is None else components
            )
        else:
            # Use custom configuration
            self.title = title if title is not None else "Custom Dashboard"
            self.description = (
                description if description is not None else "Custom dashboard configuration"
            )
        ...
        for idx, component_config in enumerate(self.components):
            ...
            if component_type == "3d":
                ...
                if creation_method:
                    ...
                    logger.warning(
                        f"Component {component_type} does not have create_3d_visualization method"
                    )
                    continue
                ...
                if creation_method:
                    ...
                    logger.warning(
                        f"Component {component_type} does not have create_hardware_heatmap method"
                    )
                    continue
                ...
                if creation_method:
                    ...
                    logger.warning(
                        f"Component {component_type} does not have create_time_series_visualization method"
                    )
                    continue
                ...
                if creation_method:
                    ...
                    logger.warning(
                        f"Component {component_type} does not have create_animated_time_series method"
                    )
                    continue
            else:
    ...
    def update_dashboard(
        self, dashboard_name, title=None, description=None, columns=None, row_height=None
    ):
        ...
        self.dashboard_name = dashboard_name
        self.title = (
            title if title is not None else current_config.get("title", "Untitled Dashboard")
        )
        self.description = (
            description if description is not None else current_config.get("description", "")
        )
    ...
    def add_component_to_dashboard(
        self, dashboard_name, component_type, component_config, width=1, height=1
    ):
        """Add a new component to an existing dashboard."""
        # Check if component type is valid
        if component_type not in self.component_registry:
            raise ValueError(
                f"Component type '{component_type}' not available. Available types: {list(self.component_registry.keys())}"
            )
        ...
        self.dashboard_config = current_config
        self.dashboard_config.update(
            {"components": self.components, "updated_at": datetime.now().isoformat()}
        )
    ...
    def remove_component_from_dashboard(self, dashboard_name, component_index):
        ...
        if component_index < 0 or component_index >= len(components):
            raise ValueError(
                f"Component index {component_index} is out of range (0-{len(components) - 1})"
            )
