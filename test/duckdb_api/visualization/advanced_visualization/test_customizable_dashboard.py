...
def generate_sample_data(days=90, model_families=None, hardware_types=None):
    ...
    for model_family in model_families:
        for hardware_type in hardware_types:
            ...
            base_memory = {"nvidia_a100": 40, "amd_mi250": 50, "intel_arc": 35, "apple_m2": 30}.get(
                hardware_type, 40
            )
...
def test_dashboard_creation(output_dir="./dashboards", template=None, open_browser=True):
    ...
    if template:
        ...
        dashboard_path = dashboard.create_dashboard(
            dashboard_name=dashboard_name, template=template
        )
    else:
...
def test_dashboard_management(output_dir="./dashboards", open_browser=True):
    ...
    dashboard_path = dashboard.update_dashboard(
        dashboard_name=dashboard_name, title="Updated Managed Dashboard", columns=3
    )
...
def main():
    ...
    parser.add_argument(
        "--template",
        type=str,
        choices=["overview", "hardware_comparison", "model_analysis", "empty", "all"],
    ...
    parser.add_argument(
        "--management-test", action="store_true", help="Run dashboard management tests"
    )
    parser.add_argument("--no-open", action="store_true", help="Don't open dashboards in browser")
