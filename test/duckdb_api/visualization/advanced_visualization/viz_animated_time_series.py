...
class AnimatedTimeSeriesVisualization(BaseVisualization):
    ...
    def create_animated_time_series(
        ...
        **kwargs,
        ...
        if title is None:
            ...
            if len(dimensions) > 0:
                title_parts.append(
                    f"by {', '.join(dim.replace('_', ' ').title() for dim in dimensions)}"
                )
        ...
        if data is None:
            ...
            date_range = pd.date_range(
                start=start_dt, end=end_dt, freq=interval_config["pandas_freq"]
            )
            ...
            hw_trend_factors = {"CPU": 1.0, "GPU": 1.2, "WebGPU": 1.15, "WebNN": 1.1}

            family_trend_factors = {"Text": 1.0, "Vision": 1.05, "Audio": 0.95}
                ...
                for family in model_families:
                    for hw in hardware_types:
                        ...
                        row = {
                            ...
                            if metric == "throughput"
                            else base_value * (1 + np.random.normal(0, 0.15)),
                            "average_latency_ms": value
                            if metric == "latency"
                            else base_value * (1 + np.random.normal(0, 0.15)),
                            "memory_peak_mb": value
                            if metric == "memory"
                            else base_value * (1 + np.random.normal(0, 0.1)),
                            "power_consumption_w": value
                            if metric == "power"
                            else base_value * (1 + np.random.normal(0, 0.1)),
                            "efficiency": value
    ...
    def _create_interactive_animated_time_series(
        self,
        df,
        time_column,
        value_column,
        ...
        try:
            ...
            for group in groups:
                ...
                if (
                    animated_ts_config["show_trend"]
                    and len(group_df) >= animated_ts_config["trend_window"]
                ):
            ...
            fig = self._create_animated_figure(
                group_dfs, groups, all_dates, value_column, metric, title, animated_ts_config
            )
    ...
    def _create_animated_figure(
        self, group_dfs, groups, all_dates, value_column, metric, title, animated_ts_config
    ):
        """
        ...
        for i, group in enumerate(groups):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines+markers",
                    name=group,
                    line=dict(width=animated_ts_config["line_width"], color=color),
                    marker=dict(size=animated_ts_config["marker_size"], color=color),
            ...
            if animated_ts_config["show_trend"]:
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="lines",
                        name=f"{group} (Trend)",
                        line=dict(width=animated_ts_config["line_width"], color=color, dash="dash"),
                        opacity=0.7,
        ...
        for i, date in enumerate(all_dates):
            ...
            for group in groups:
                ...
                # Add main trace data
                frame_data.append(go.Scatter(x=visible_df["date"], y=visible_df[value_column]))
                ...
                if animated_ts_config["show_anomalies"]:
                    ...
                    frame_data.append(go.Scatter(x=anomalies["date"], y=anomalies[value_column]))
                    ...
                    "buttons": [
                        {
                            ...
                            "args": [
                                ...
                                {
                                    "frame": {
                                        "duration": animated_ts_config["animation_speed"],
                                        "redraw": True,
                                    },
                                    "fromcurrent": True,
                                    "transition": {
                                        "duration": animated_ts_config["transition_duration"]
                                    },
                                },
                            ],
                        },
        ...
        fig.update_layout(
            ...
            xaxis=dict(type="date", tickformat=animated_ts_config["date_format"]),
    ...
    def _create_static_animated_time_series(
        self,
        df,
        time_column,
        value_column,
        ...
        try:
            ...
            def update(frame_idx):
                ...
                ax.xaxis.set_major_formatter(
                    mdates.DateFormatter(animated_ts_config["date_format"])
                )
            ...
            if output_path:
                ...
                if ext.lower() in [".mp4", ".gif"]:
                    ...
                    logger.info(
                        f"Matplotlib doesn't support HTML output. Saved static image as {png_path}"
                    )
                    return png_path
                else:
        ...
        try:
            ...
            if output_format.lower() == "mp4":
                ...
                self.figure.write_html(
                    output_path.replace(".mp4", ".html"), include_plotlyjs="cdn", auto_play=True
                )
                ...
                logger.info(
                    f"MP4 export not directly supported. HTML with auto-play saved to {output_path.replace('.mp4', '.html')}"
                )
                ...
                try:
                    ...
                    for i, frame in enumerate(self.figure.frames):
                        frame_fig = go.Figure(data=frame.data, layout=self.figure.layout)
                    ...
                    logger.error(
                        "Kaleido is required for frame export. Install with: pip install kaleido"
                    )
                    return None
            else:
                # HTML export (default)
                self.figure.write_html(
                    output_path.replace(f".{output_format}", ".html"), include_plotlyjs="cdn"
                )
