...
from data.duckdb.visualization.advanced_visualization.base import (
    BaseVisualization,
    PLOTLY_AVAILABLE,
...
class TimeSeriesVisualization(BaseVisualization):
                    ...
                    rows.append(
                        {
                            ...
                            "throughput_items_per_second": value
                            if metric == "throughput"
    ...
    def _create_interactive_time_series(
        ...
        try:
            ...
            metric_config = self.metric_config.get(
                metric.lower(),
                {
                    "color": "#1f77b4",
                    "line_style": "solid",
            ...
            if group_by:
                ...
                for i, group in enumerate(sorted(groups)):
                    ...
                    if (
                        time_series_config["show_trend"]
                        and len(group_df) >= time_series_config["trend_window"]
                    ):
                        ...
                        group_df["trend"] = (
                            group_df[value_column].rolling(window=window, center=True).mean()
                        )
                    ...
                    if time_series_config["show_anomalies"] and len(group_df) >= 4:
                        try:
                            ...
                            z_scores = np.abs(
                                (group_df[value_column] - group_df[value_column].mean())
                                / group_df[value_column].std()
                            )
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        line=dict(
                            width=time_series_config["line_width"], color=metric_config["color"]
                        ),
                        marker=dict(
                            size=time_series_config["marker_size"], color=metric_config["color"]
                        ),
                    )
                )
                    ...
                    try:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="lines",
                                name="Regression",
                                line=dict(width=1, color=metric_config["color"], dash="dot"),
                                opacity=0.5,
                            )
                        )
                        ...
                        direction = (
                            "improving"
                            if (
                                (slope > 0 and metric_config["better_direction"] == "up")
                                or (slope < 0 and metric_config["better_direction"] == "down")
                            )
                            else "degrading"
                        )
                        ...
                        time_diff = (
                            df_reg[time_column].max() - df_reg[time_column].min()
                        ).total_seconds()
