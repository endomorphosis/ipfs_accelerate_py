    ...
    for i in range(num_visualizations):
        ...
        if viz_type == "3d":
            ...
            viz_path = vis.create_hardware_comparison_heatmap(
                metric="throughput", batch_size=1, title=f"Test Heatmap Visualization {i + 1}"
            )
        else:  # time-series
...
def test_visualization_sync(
    dashboard_url=None, api_key=None, db_path="./benchmark_db.duckdb", output_dir="./test_output"
):
    ...
    num_synced = integration.synchronize_visualizations(
        visualization_dir=output_dir, file_pattern="*.html", recursive=True
    )
...
def test_dashboard_panels(
    ...
    layout = {"type": "grid", "columns": columns, "rows": rows, "items": []}
    ...
    panel_id = integration.create_dashboard_panel(
        panel_title="Test Visualization Panel", visualization_ids=visualization_ids, layout=layout
    )
...
def test_dashboard_snapshot(dashboard_url=None, api_key=None, output_dir="./test_output"):
    ...
    success = integration.import_dashboard_snapshot(
        snapshot_path=snapshot_path, target_dashboard_id="test_import"
    )
...
def main():
    ...
    parser.add_argument(
        "--dashboard-url", default="http://localhost:8082", help="URL of the monitoring dashboard"
    )
    ...
    parser.add_argument(
        "--output-dir", default="./test_dashboard_output", help="Directory to save test outputs"
    )
    parser.add_argument(
        "--test",
        choices=["connection", "registration", "sync", "panels", "snapshot", "all"],
        default="all",
        help="Test to run",
    )
    ...
    if args.test == "sync" or args.test == "all":
        num_synced = test_visualization_sync(
            args.dashboard_url, args.api_key, args.db_path, args.output_dir
        )
        if num_synced > 0:
            logger.info(
                f"✅ Visualization sync test passed: {num_synced} visualizations synchronized"
            )
        else:
