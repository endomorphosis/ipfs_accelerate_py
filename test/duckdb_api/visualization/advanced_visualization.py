...
try:
    ...
    logger.warning(
        "Scikit-learn not available. Some dimensionality reduction features will be limited."
    )
...
class AdvancedVisualizationSystem(*AdvancedVisualizationBaseClasses):
    ...
    def __init__(
        ...
        if HAS_DASHBOARD_INTEGRATION and issubclass(
            self.__class__, MonitorDashboardIntegrationMixin
        ):
            MonitorDashboardIntegrationMixin.__init__(
                self, dashboard_url=dashboard_url, api_key=api_key
            )
        self.db_api = db_api
    ...
    def create_3d_performance_visualization(
        self,
                ...
                for value in df[color_dim].unique():
                    ...
                    fig.add_trace(
                        go.Scatter3d(
                            ...
                            text=[
                                self._create_hover_text(row, metrics, dimensions)
    ...
    def create_hardware_comparison_heatmap(
        ...
        batch_size: int = 1,
        output_path: Optional[str] = None,
        title: str = "Hardware Comparison by Model Family",
    ) -> Optional[str]:
        ...
        try:
            ...
            for i in range(1, len(model_families) + 1):
                fig.update_yaxes(title="Model", autorange="reversed", row=i, col=1)

            # Only add x-axis title to the bottom subplot
            fig.update_xaxes(title="Hardware Type", row=len(model_families), col=1)
    ...
    def create_power_efficiency_visualization(
        ...
        output_path: Optional[str] = None,
        title: str = "Power Efficiency Visualization",
    ) -> Optional[str]:
        ...
        try:
            ...
            if (
                "throughput_items_per_second" in df.columns
                and "energy_consumption_joules" in df.columns
            ):
                df["efficiency"] = (
                    df["throughput_items_per_second"] / df["energy_consumption_joules"]
                )
            else:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        size=df["average_latency_ms"].apply(
                            lambda x: max(10, min(50, 200 / (x + 1)))
                        ),  # Scale marker size
                        color=df["efficiency"],
                        colorscale=self.config["color_palette"],
                        showscale=True,
            ...
            if len(df) > 0:
                ...
                for efficiency in efficiency_levels:
                    ...
                    if (
                        efficiency * max_energy > df["throughput_items_per_second"].max() * 3
                        or efficiency * max_energy < df["throughput_items_per_second"].min() / 3
                    ):
                        continue
                    ...
                    for family in sorted(df["model_family"].unique()):
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="markers",
                                marker=dict(
                                    size=family_df["average_latency_ms"].apply(
                                        lambda x: max(10, min(50, 200 / (x + 1)))
                                    ),
                        ...
                        button_visibility = (
                            [False]
                            + [True] * (len(fig.data) - 2)
                            + [False] * (len(fig.data) - len(visibility) - 1)
                        )
                    ...
                    for hw_type in sorted(df["hardware_type"].unique()):
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="markers",
                                marker=dict(
                                    size=hw_df["average_latency_ms"].apply(
                                        lambda x: max(10, min(50, 200 / (x + 1)))
                                    ),
    ...
    def create_animated_time_series_visualization(
        self,
        data: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if data is None and self.db_api:
                ...
                # Convert results to the required format
                data = {"data_points": results.to_dict(orient="records")}
            ...
            for time_bucket in time_buckets:
                ...
                latest_df = (
                    time_df.sort_values("created_at").groupby(group_cols).last().reset_index()
                )
                ...
                for _, group in latest_df.groupby(dimensions):
                    ...
                    if dim_key not in dimension_combinations:
                        dimension_combinations[dim_key] = {
                            ...
                            "label": " / ".join(
                                [f"{dim}={group[dim].iloc[0]}" for dim in dimensions]
                            ),
                        }
                        color_idx += 1
                    ...
                    if include_trend and len(group) >= window_size:
                        ...
                        group[f"{metric_col}_trend"] = (
                            group[metric_col].rolling(window=window_size, min_periods=1).mean()
                        )
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            text=[
                                ...
                                + (
                                    f"<b>Trend:</b> {row[f'{metric_col}_trend']:.2f}"
            ...
            for i, time_bucket in enumerate(time_buckets):
                step = dict(
                    ...
                    args=[
                        ...
                        {
                            "title": f"{title} - {time_bucket.strftime('%Y-%m-%d')}"
    ...
    def create_dashboard(
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                self.dashboards_dir, f"dashboard_{dashboard_name}_{timestamp}.html"
            )
    ...
    def add_component_to_dashboard(
        ...
        component = {"type": component_type, "width": width, "height": height, **component_config}
    ...
    def remove_component_from_dashboard(
        self, dashboard_name: str, component_index: int, output_path: Optional[str] = None
    ) -> Optional[str]:
    ...
    def export_dashboard(
        self, dashboard_name: str, format: str = "html", output_path: Optional[str] = None
    ) -> Optional[str]:
        ...
        if output_path is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = os.path.join(
                self.dashboards_dir, f"dashboard_{dashboard_name}_{timestamp}.{format}"
            )
            ...
            try:
                ...
                if component_type == "3d":
                    ...
                    viz_path = self.create_3d_performance_visualization(
                        ...
                        dimensions=dimensions,
                        output_path=os.path.join(
                            self.output_dir, f"dashboard_component_{i}_3d.html"
                        ),
                    ...
                    viz_path = self.create_hardware_comparison_heatmap(
                        ...
                        output_path=os.path.join(
                            self.output_dir, f"dashboard_component_{i}_heatmap.html"
                        ),
                    ...
                    viz_path = self.create_power_efficiency_visualization(
                        ...
                        output_path=os.path.join(
                            self.output_dir, f"dashboard_component_{i}_power.html"
                        ),
                    ...
                    viz_path = self.create_animated_time_series_visualization(
                        ...
                        output_path=os.path.join(
                            self.output_dir, f"dashboard_component_{i}_time_series.html"
                        ),
                ...
                if viz_path:
                    ...
                    plotly_script_match = re.search(
                        r'<script type="text/javascript">.*?</script>', viz_content, re.DOTALL
                    )
                        ...
                        viz_components.append(
                            {
                                ...
                                "title": component.get(
                                    "title", f"{component_type.title()} Visualization"
                                ),
                                "div": plotly_div,
    ...
    def _generate_grid_layout(
        self, components: List[Dict[str, Any]], columns: int, row_height: int
    ) -> Dict[str, str]:
