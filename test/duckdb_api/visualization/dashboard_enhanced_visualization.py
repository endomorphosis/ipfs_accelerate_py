...
class DashboardEnhancedVisualizationSystem(
    AdvancedVisualizationSystem, MonitorDashboardIntegrationMixin
):
    """
    ...
    def synchronize_with_dashboard(
        ...
        if not HAS_DASHBOARD_INTEGRATION:
            logger.error(
                "Dashboard integration not available. Install with: pip install requests websocket-client"
            )
            return 0
        ...
        if create_panel and num_synced > 0:
            ...
            visualization_files = glob.glob(
                os.path.join(self.output_dir, "*.html"), recursive=False
            )

            if visualization_files:
                # Extract visualization IDs
                visualization_ids = [
                    os.path.splitext(os.path.basename(path))[0] for path in visualization_files
                ]

                # Create panel
                panel_id = self.create_dashboard_panel_from_visualizations(
                    panel_title=panel_title, visualization_ids=visualization_ids, layout_columns=2
                )
    ...
    def setup_auto_dashboard_sync(
        ...
        self.setup_auto_dashboard_sync(
            interval_seconds=interval_seconds, output_dir=self.output_dir
        )
        ...
        if max_runtime_minutes is not None:
            ...
            def stop_after_timeout():
                ...
                logger.info(
                    f"Automatic synchronization completed after {max_runtime_minutes} minutes"
                )
        ...
        if sync_with_dashboard and HAS_DASHBOARD_INTEGRATION:
            ...
            if not self.connect_to_dashboard():
                if dashboard_url or api_key:
                    ...
                    if not self.connect_to_dashboard():
                        logger.warning(
                            "Failed to connect to dashboard, visualization not synchronized"
                        )
                else:
    ...
    def export_dashboard_snapshot(
        self, output_path: Optional[str] = None, include_visualizations: bool = True
    ) -> Optional[str]:
        """
        ...
        return self._dashboard_integration.export_dashboard_snapshot(
            output_path=output_path, include_visualizations=include_visualizations
        )
