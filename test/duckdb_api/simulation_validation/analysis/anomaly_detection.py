...
class AnomalyDetection(AnalysisMethod):
    ...
    def analyze(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        analysis_results = {
            ...
            "analysis_methods": {},
            "anomalies_detected": [],
            "anomaly_insights": {"summary": {}, "key_findings": [], "recommendations": []},
        }
        ...
        if self.config["deviation_detection"]["enabled"]:
            try:
                ...
                has_timestamps = all(
                    hasattr(result, "validation_timestamp") for result in validation_results
                )
        ...
        if self.config["ensemble_detection"]["enabled"]:
            try:
                ...
                for idx, count_data in anomaly_counts.items():
                    ...
                    if (
                        methods_count >= min_methods
                        and methods_count / total_methods >= voting_threshold
                    ):
        ...
        if self.config["context_analysis"]["enabled"] and analysis_results["anomalies_detected"]:
            try:
        ...
        analysis_results["anomaly_insights"]["recommendations"] = self._generate_recommendations(
            analysis_results["anomalies_detected"], analysis_results["anomaly_insights"]
        )
        ...
        for i, result in enumerate(validation_results):
            ...
            for metric in metrics_to_analyze:
                if metric in result.metrics_comparison:
                    ...
                    for error_metric in ["mape", "absolute_error", "relative_error"]:
                        if error_metric in comparison:
                            # Skip NaN values
                            if comparison[error_metric] is None or np.isnan(
                                comparison[error_metric]
                            ):
            ...
            if row and len(row) > 0:
                ...
                if not features_list:
                    ...
                    logger.warning(
                        f"Skipping result with mismatched feature count: {len(row)} vs {len(features_list[0])}"
                    )
    ...
    def _detect_statistical_anomalies(
        self, features: np.ndarray, feature_names: List[str], metadata: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        results = {"methods_used": [], "anomalies": []}
            ...
            try:
                ...
                for i in range(features.shape[0]):
                    ...
                    if len(anomalous_features) > 0:
                        ...
                        anomaly_feature_details.sort(key=lambda x: abs(x["z_score"]), reverse=True)
                ...
                results["methods_used"].append(
                    {
                        ...
                        "threshold": threshold,
                        "anomalies_found": sum(
                            1 for a in results["anomalies"] if a["method"] == "z_score"
                        ),
                    }
                )
                ...
                results["methods_used"].append(
                    {
                        ...
                        "threshold": threshold,
                        "anomalies_found": sum(
                            1 for a in results["anomalies"] if a["method"] == "modified_z_score"
                        ),
                    }
                )
                ...
                results["methods_used"].append(
                    {
                        ...
                        "scale": self.config["statistical_detection"]["iqr_scale"],
                        "anomalies_found": sum(
                            1 for a in results["anomalies"] if a["method"] == "iqr"
                        ),
                    }
                )
        ...
        if "dbscan" in methods:
            try:
                ...
                if len(core_sample_indices) > 0 and len(anomaly_indices) > 0:
                    ...
                    confidence_scores = np.minimum(
                        min_distances / max_dist, np.ones_like(min_distances)
                    )
    ...
    def _detect_deviation_anomalies(
        ...
        if not all(hasattr(result, "validation_timestamp") for result in validation_results):
            return {"status": "skipped", "message": "Missing timestamps for deviation detection"}
        ...
        if "moving_average" in methods:
            try:
                ...
                if sorted_features.shape[0] < window_size + 1:
                    logger.warning(
                        f"Insufficient data points for moving average: "
                    ...
                    for j in range(sorted_features.shape[1]):
                        ...
                        residuals[window_size - 1 : window_size - 1 + len(ma_values)] = (
                            values[window_size - 1 : window_size - 1 + len(ma_values)] - ma_values
                        )
        ...
        if "ewma" in methods:
            try:
                ...
                if sorted_features.shape[0] < 3:
                    logger.warning(
                        f"Insufficient data points for EWMA: {sorted_features.shape[0]} < 3"
                    )
                else:
    ...
    def _ensemble_detection(
        self, method_results: Dict[str, Dict[str, Any]], validation_results: List[ValidationResult]
        ...
        for method_name, result in method_results.items():
            ...
            if (
                "anomalies" not in result
                or not isinstance(result["anomalies"], list)
                or not result["anomalies"]
            ):
                continue
            ...
            for anomaly in result["anomalies"]:
                ...
                if idx not in anomaly_counts:
                    anomaly_counts[idx] = {"count": 0, "total": total_methods, "methods": []}

                # Update count and add method
                anomaly_counts[idx]["count"] += 1
                anomaly_counts[idx]["methods"].append(
                    {"method": method_name, "confidence": anomaly.get("confidence", 0.5)}
                )
    ...
    def _analyze_anomaly_context(
        ...
        context_analysis = {"summary": {}, "patterns": [], "contexts": {}}
                ...
                if anomaly_timestamps:
                    ...
                    if time_diffs:
                        context_analysis["temporal_context"] = {
                            ...
                            "anomalies_per_day": 24 / np.mean(time_diffs)
        ...
        for anomaly in anomalies:
            ...
            for field, pattern in metadata_patterns.items():
                if (
                    idx < len(metadata)
                    and field in metadata[idx]
                    and str(metadata[idx][field]) == pattern["most_common"]
                ):
    ...
    def _generate_anomaly_summary(
        self, anomalies: List[Dict[str, Any]], validation_results: List[ValidationResult]
    ) -> Dict[str, Any]:
        """
        ...
        summary = {
            ...
            "anomaly_percentage": len(anomalies) / len(validation_results) * 100
        ...
        for field in metadata_fields:
            ...
            for anomaly in anomalies:
                if (
                    "metadata" in anomaly
                    and isinstance(anomaly["metadata"], dict)
                    and field in anomaly["metadata"]
                ):
            ...
            if total_with_field > 0:
                ...
                # Add to patterns
                metadata_patterns[field] = {"distribution": frequencies, "total": total_with_field}
    ...
    def _generate_key_findings(
        ...
        findings.append(
            f"Detected {len(anomalies)} anomalies ({anomaly_percentage:.1f}% of validation results)"
        )
        ...
        if "confidence" in summary and "high_confidence_count" in summary["confidence"]:
            ...
            if high_conf_count > 0:
                findings.append(
                    f"Found {high_conf_count} high-confidence anomalies that require attention"
                )
        ...
        if "metadata_patterns" in context_analysis:
            ...
            for field, pattern in patterns.items():
                ...
                if frequency >= 0.7:  # Only add if at least 70% have this value
                    findings.append(
                        f"{frequency * 100:.0f}% of anomalies occurred with {field}={value}"
                    )
        ...
        if "feature_correlations" in context_analysis:
            ...
            if correlations:
                ...
                findings.append(
                    f"Strong {direction} correlation ({abs(coefficient):.2f}) "
        ...
        if "features" in summary:
            ...
            if feature_counts:
                ...
                findings.append(
                    f"{count} anomalies ({count / len(anomalies) * 100:.0f}%) "
        ...
        if "cluster_analysis" in context_analysis:
            ...
            if "clusters" in cluster_analysis:
                ...
                if len(clusters) > 1:
                    findings.append(
                        f"Anomalies form {len(clusters)} distinct clusters, "
                ...
                if largest_cluster:
                    ...
                    if "common_metadata" in cluster and cluster["common_metadata"]:
                        ...
                        findings.append(
                            f"Cluster {cluster_id} ({cluster['size']} anomalies) "
        ...
        if "temporal_context" in context_analysis:
            ...
            if "anomalies_per_day" in temporal and temporal["anomalies_per_day"]:
                ...
                findings.append(f"Anomalies occur at a rate of approximately {rate:.1f} per day")
    ...
    def _generate_recommendations(
        self, anomalies: List[Dict[str, Any]], anomaly_insights: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        if "confidence" in summary and "high_confidence_count" in summary["confidence"]:
            ...
            if high_conf_count > 0:
                recommendations.append(
                    f"Investigate the {high_conf_count} high-confidence anomalies as a priority"
                )
        ...
        if "metadata_patterns" in context_analysis:
            ...
            for field, pattern in patterns.items():
                ...
                if frequency >= 0.7:  # Only add if at least 70% have this value
                    recommendations.append(
                        f"Review simulation configuration for {field}={value} "
        ...
        if "cluster_analysis" in context_analysis:
            ...
            if "clusters" in cluster_analysis:
                ...
                if len(clusters) > 1:
                    recommendations.append(
                        f"Address each of the {len(clusters)} anomaly clusters "
                ...
                if largest_cluster:
                    ...
                    if "common_metadata" in cluster and cluster["common_metadata"]:
                        ...
                        if metadata_items:
                            ...
                            recommendations.append(
                                f"Prioritize investigating issues with "
        ...
        if "features" in summary:
            ...
            if feature_counts:
                ...
                recommendations.append(
                    f"Focus on improving simulation accuracy for the "
        ...
        if "temporal_context" in context_analysis:
            ...
            if "avg_hours_between_anomalies" in temporal:
                ...
                if hours < 24:
                    recommendations.append(
                        f"Set up more frequent monitoring with alerts, "
        ...
        if len(anomalies) / summary.get("total_results", 1) > 0.2:
            # High anomaly rate
            recommendations.append(
                "Review and recalibrate the simulation model as the anomaly rate is unusually high"
            )

        # Ensure at least one recommendation
        if not recommendations:
            recommendations.append(
                "Continue monitoring validation results and collect more "
