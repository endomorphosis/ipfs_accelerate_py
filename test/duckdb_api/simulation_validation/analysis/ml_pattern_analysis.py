...
class MLPatternAnalysis(AnalysisMethod):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        default_config = {
            ...
            "parameter_sensitivity": {
                "enabled": True,
                "parameters_to_analyze": ["batch_size", "precision"],
                "metrics_to_analyze": ["throughput_items_per_second", "average_latency_ms"],
    ...
    def analyze(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        analysis_results = {
            ...
            "analysis_methods": {},
            "insights": {"key_findings": [], "patterns_identified": [], "recommendations": []},
        }
        ...
        if data_matrix is None or data_matrix.shape[0] < 3:
            return {"status": "error", "message": "Insufficient data for ML pattern analysis"}
            ...
            try:
                ...
                if "clusters" in clustering_results and clustering_results["clusters"]:
                    if len(clustering_results["clusters"]) > 1:
                        finding = (
                            f"Identified {len(clustering_results['clusters'])} distinct "
                    ...
                    for cluster_id, cluster_info in clustering_results["clusters"].items():
                        if "distinctive_features" in cluster_info:
                            ...
                            finding = (
                                f"Cluster {cluster_id} ({cluster_info['size']} samples) "
        ...
        if self.config["dimensional_reduction"]["enabled"]:
            try:
                ...
                if data_matrix.shape[0] >= min_samples:
                    ...
                    if (
                        "pca" in reduction_results
                        and "explained_variance" in reduction_results["pca"]
                    ):
                        var = reduction_results["pca"]["explained_variance"][0] * 100
                        finding = (
                            f"First principal component explains {var:.1f}% of variation "
            ...
            try:
                ...
                if "frequent_patterns" in pattern_results:
                    for pattern in pattern_results["frequent_patterns"][:2]:
                        finding = (
                            f"Discovered frequent pattern: {pattern['description']} "
                    ...
                    for param, impacts in sensitivity_results["parameter_impacts"].items():
                        if impacts:
                            ...
                            sorted_impacts = sorted(
                                impacts, key=lambda x: abs(x.get("impact", 0)), reverse=True
                            )
                            if sorted_impacts:
                                impact = sorted_impacts[0]
                                if (
                                    abs(impact.get("impact", 0)) > 0.2
                                ):  # Only include significant impacts
                                    direction = (
                                        "increases" if impact.get("impact", 0) > 0 else "decreases"
                                    )
                                    finding = (
                                        f"Changing {param} significantly {direction} "
    ...
    def get_requirements(self) -> Dict[str, Any]:
        ...
        requirements = {
            ...
            "parameter_sensitivity_requirements": {
                "min_samples_per_parameter": self.config["parameter_sensitivity"][
    ...
    def _perform_clustering(
        self, data_matrix: np.ndarray, feature_names: List[str], metadata: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        clustering_results = {"methods_used": [], "clusters": {}, "visualization_data": {}}
        ...
        if "kmeans" in self.config["clustering"]["methods"]:
            try:
                ...
                max_clusters = min(
                    self.config["clustering"]["max_clusters"], clustering_data.shape[0] // 2
                )
                ...
                for label in range(optimal_clusters):
                    ...
                    for i, (feat_name, center_val, var_val) in enumerate(
                        ...
                        if other_centers:
                            ...
                            distinctiveness = abs(center_val - np.mean(other_centers)) / (
                                np.std(other_centers) + 1e-10
                            )
    ...
    def _generate_pca_for_visualization(self, data: np.ndarray) -> List[List[float]]:
        """
    ...
    def _analyze_feature_importance(
        ...
        """
        importance_results = {"methods_used": [], "important_features": []}
        ...
        if "permutation" in self.config["feature_importance"]["methods"]:
            try:
                ...
                for i, (name, importance) in enumerate(
                    zip(X_feature_names, result.importances_mean)
                ):
    ...
    def _perform_dimensional_reduction(
        ...
        """
        reduction_results = {"methods_used": []}
    ...
    def _mine_patterns(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        """
        ...
        """
        pattern_results = {"frequent_patterns": [], "association_rules": []}
        ...
        frequent_attributes = {
            attr: support for attr, support in attribute_support.items() if support >= min_support
        }
        ...
        for i, (attr1, support1) in enumerate(sorted_frequent):
            ...
            for attr2, support2 in sorted_frequent[i + 1 :]:
                ...
                for record in discretized_data:
                    ...
                    if (
                        attr1_val is not None
                        and attr2_val is not None
                        and f"{attr1_name}={attr1_val}" == attr1
                        and f"{attr2_name}={attr2_val}" == attr2
                    ):
                        co_occurrences += 1

                # Calculate confidence in both directions
                conf1 = (
                    co_occurrences / attribute_counts[attr1] if attribute_counts[attr1] > 0 else 0
                )
                conf2 = (
                    co_occurrences / attribute_counts[attr2] if attribute_counts[attr2] > 0 else 0
                )

                # Calculate lift
                expected_co_occurrences = (
                    attribute_counts[attr1] * attribute_counts[attr2]
                ) / total_records
                lift = (
                    co_occurrences / expected_co_occurrences if expected_co_occurrences > 0 else 0
                )
    ...
    def _analyze_parameter_sensitivity(
        ...
        """
        sensitivity_results = {"parameter_impacts": {}, "sensitivity_scores": {}}
        ...
        for param in parameters:
            ...
            for result in validation_results:
                ...
                if param == "batch_size":
                    ...
                    hasattr(result.simulation_result, "additional_metadata")
                    and result.simulation_result.additional_metadata
                    and param in result.simulation_result.additional_metadata
                ):
        ...
        for param, groups in parameter_groups.items():
            ...
            valid_groups = {
                value: results for value, results in groups.items() if len(results) >= min_samples
            }
            ...
            for metric in metrics:
                ...
                try:
                    ...
                    impact = (
                        correlation * (error_range / min_error) if min_error > 0 else correlation
                    )
    ...
    def _generate_recommendations(self, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        if "parameter_sensitivity" in analysis_results["analysis_methods"]:
            ...
            if "sensitivity_scores" in sensitivity:
                ...
                if (
                    most_sensitive and max_score > 0.3
                ):  # Only recommend if sensitivity is significant
                    recommendations.append(
            ...
            if "parameter_impacts" in sensitivity:
                for param, impacts in sensitivity["parameter_impacts"].items():
                    if (
                        impacts and abs(impacts[0]["impact"]) > 0.5
