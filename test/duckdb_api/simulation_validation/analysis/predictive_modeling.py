...
class PredictiveModeling(AnalysisMethod):
    ...
    def analyze(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        if not prediction_data:
            return {"status": "error", "message": "Failed to extract data for predictive modeling"}

        # Split data into features (X) and targets (y)
        X, y, timestamps, metadata = self._prepare_model_data(prediction_data)

        if X is None or y is None:
            return {"status": "error", "message": "Insufficient data for predictive modeling"}
        ...
        if self.config["time_series_forecasting"]["enabled"]:
            try:
                ...
                if len(timestamps) >= min_periods:
                    ts_forecasts = self._forecast_time_series(y, timestamps, prediction_data)
        ...
        if self.config["model_evaluation"]["enabled"]:
            try:
                ...
                analysis_results["model_evaluation"] = {"status": "error", "message": str(e)}

        # Generate key findings
        analysis_results["insights"]["key_findings"] = self._generate_key_findings(
            analysis_results["forecasts"], analysis_results["model_evaluation"], prediction_data
        )
    ...
    def _extract_prediction_data(
        ...
        for result in validation_results:
            ...
            for metric in metrics_to_predict:
                if (
                    metric in result.simulation_result.metrics
                    and metric in result.hardware_result.metrics
                ):
    ...
    def _prepare_model_data(
        ...
        Optional[np.ndarray], Optional[np.ndarray], List[datetime.datetime], List[Dict[str, Any]]
    ]:
        """
        ...
        if (
            not prediction_data
            or "features" not in prediction_data
            or "targets" not in prediction_data
            or not prediction_data["features"]
            or not prediction_data["targets"]
        ):
            return None, None, [], []
        ...
        if min(feature_lengths) != max(feature_lengths):
            logger.warning(
                f"Inconsistent feature lengths: min={min(feature_lengths)}, max={max(feature_lengths)}"
            )
        ...
        if min(target_lengths) != max(target_lengths):
            logger.warning(
                f"Inconsistent target lengths: min={min(target_lengths)}, max={max(target_lengths)}"
            )
            return None, None, timestamps, metadata
    ...
    def _forecast_time_series(
        self, y: np.ndarray, timestamps: List[datetime.datetime], prediction_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Initialize results
        results = {"methods_used": [], "forecasts": {}}
    ...
    def _forecast_arima(
        self, df: Union[Dict[str, List], Any], target_names: List[str], forecast_horizon: int
        ...
        for target in target_names:
            try:
                ...
                lower = (
                    pred_intervals.iloc[:, 0].tolist()
                    if is_dataframe
                    else pred_intervals[:, 0].tolist()
                )
                upper = (
                    pred_intervals.iloc[:, 1].tolist()
                    if is_dataframe
                    else pred_intervals[:, 1].tolist()
                )
                ...
                if is_dataframe:
                    forecasts[target]["forecast_dates"] = [
                        d.isoformat() if hasattr(d, "isoformat") else str(d) for d in forecast_dates
                    ]
        ...
        for target in target_names:
            try:
                ...
                forecasts[target] = {
                    ...
                    "forecast_horizon": forecast_horizon,
                    "model_params": {"interval_width": interval_width},
                }
    ...
    def _build_regression_models(
        self, X: np.ndarray, y: np.ndarray, metadata: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Initialize results
        results = {"methods_used": [], "models": {}, "feature_importance": {}, "predictions": {}}
        ...
        for j in range(y.shape[1]):
            ...
            if "random_forest" in methods:
                try:
                    ...
                    results["feature_importance"][target_name]["random_forest"] = (
            ...
            if "gradient_boosting" in methods:
                try:
                    ...
                    results["feature_importance"][target_name]["gradient_boosting"] = (
                        feature_importance.tolist()
                    )
        ...
        for j in range(y.shape[1]):
            ...
            if "random_forest" in methods:
                try:
                    ...
                    logger.warning(
                        f"Error building Random Forest classifier for {target_name}: {e}"
                    )
            ...
            if "gradient_boosting" in methods:
                try:
                    ...
                    logger.warning(
                        f"Error building Gradient Boosting classifier for {target_name}: {e}"
                    )
    ...
    def _build_ensemble_prediction(self, forecasts: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
            ...
            if "voting" in methods:
                try:
                    ...
                    if "time_series" in target_forecasts:
                        ts_methods = [
                            m for m in target_forecasts["time_series"] if m != "forecast_dates"
                        ]
                        for method in ts_methods:
                            available_methods.append(("time_series", method))
                            method_weights[("time_series", method)] = weights.get(
                                "time_series", 0.4
                            ) / len(ts_methods)
                    ...
                    if "regression" in target_forecasts:
                        ...
                        method_weights[("regression", "regression")] = weights.get(
                            "regression", 0.4
                        )
                    ...
                    if "classification" in target_forecasts:
                        ...
                        method_weights[("classification", "classification")] = weights.get(
                            "classification", 0.2
                        )
                    ...
                    for method_type, method_name in available_methods:
                        ...
                        if weighted_sum is None:
                            ...
                            weighted_sum = (
                                weighted_sum[:min_length]
                                + np.array(forecast_values)[:min_length] * weight
                            )
                    ...
                    if ensemble_values is not None and weighted_sum is not None:
                        ...
                        if (
                            "time_series" in target_forecasts
                            and "forecast_dates" in target_forecasts["time_series"]
                        ):
                            ensemble_forecast["forecast_dates"] = target_forecasts["time_series"][
                                "forecast_dates"
                            ]
    ...
    def _evaluate_models(
        ...
        """
        # Initialize results
        evaluation_results = {"best_models": {}, "comparison": {}}
                    ...
                    if method != "forecast_dates":
                        ...
                        evaluation_results["comparison"][target][method] = {
                            "forecast_values": forecast_values[
                                :5
                            ]  # Include first 5 forecast values
                        }
    ...
    def _generate_key_findings(
        ...
        for target, methods in forecasts.items():
            ...
            try:
                ...
                target_name = (
                    target_names[target_index] if target_index < len(target_names) else target
                )
            except:
                target_name = target
        ...
        if "best_models" in model_evaluation:
            for target, model_info in model_evaluation["best_models"].items():
                ...
                findings.append(f"Best predictive model for {target_name} is {best_model}")
        ...
        for method_name, method_results in {
            ...
            if method_name in feature_importance:
                for target, importance_dict in (
                    feature_importance[method_name].get("feature_importance", {}).items()
                ):
                    ...
                    if best_importance is not None:
                        ...
                        if len(feature_names) == len(best_importance):
                            ...
                            # Add finding
                            features_str = ", ".join(
                                [f"{name} ({importance:.2f})" for name, importance in top_features]
                            )
