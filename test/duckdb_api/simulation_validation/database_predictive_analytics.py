...
class DatabasePredictiveAnalytics:
    ...
    def __init__(self, automated_optimization_manager, config: Optional[Dict[str, Any]] = None):
        """
    ...
    def forecast_database_metrics(
        self, horizon: str = "medium_term", specific_metrics: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def generate_forecast_visualizations(
        self, forecast_results: Dict[str, Any], output_format: str = "base64"
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if metric_name in forecast_results.get("anomalies", {}):
                    ...
                    if "forecast_anomalies" in anomalies:
                        ...
                        forecast_anomaly_values = [
                            forecast_values[i] for i in forecast_anomaly_indices
                        ]
                        forecast_anomaly_dates = [
                            forecast_dates[i] for i in forecast_anomaly_indices
                        ]
                ...
                if "predicted_warning_threshold" in forecast:
                    ax.axhline(
                        y=forecast["predicted_warning_threshold"],
                        color="orange",
                        linestyle="--",
                        label="Predicted Warning",
                    )

                if "predicted_error_threshold" in forecast:
                    ax.axhline(
                        y=forecast["predicted_error_threshold"],
                        color="red",
                        linestyle="--",
                ...
                if output_format == "base64":
                    ...
                    try:
                        ...
                        if os.path.exists(filename):
                            print(
                                f"DEBUG: File {filename} exists, size: {os.path.getsize(filename)}"
                            )
                        else:
                    ...
                    results["visualizations"][metric_name] = {"format": "object", "figure": fig}
                ...
                results["visualizations"][metric_name] = {"status": "error", "message": str(e)}

        return results

    def check_predicted_thresholds(self, forecast_results: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _get_parameter_storage_path(self) -> Path:
        ...
        storage_path = (
            ...
            .get("parameter_persistence", {})
            .get("storage_path", "./hyperparameters")
        )
    ...
    def _get_parameter_key(
        self, metric_name: str, model_type: str, data_signature: Optional[str] = None
    ) -> str:
        """
    ...
    def _validate_parameters(self, parameters: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if age.days > max_age_days:
                logger.debug(
                    f"Parameters too old ({age.days} days) - max age is {max_age_days} days"
                )
                return False
    ...
    def recommend_proactive_actions(
        self, forecast_results: Dict[str, Any], threshold_alerts: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        urgency_order = {"immediate": 0, "this_week": 1, "this_month": 2, "future": 3}
        ...
        if results["recommendations"]:
            ...
            for urgency, recs in sorted(
                urgency_groups.items(), key=lambda x: urgency_order.get(x[0], 99)
            ):
    ...
    def _generate_forecasts(
        ...
        results = {
            ...
            "forecasting_config": {
                "auto_model_selection": self.config.get("forecasting", {}).get(
        ...
        if "arima" in forecast_methods and PANDAS_AVAILABLE and SCIPY_AVAILABLE:
            try:
                arima_forecast = self._forecast_arima(
                    values, timestamps, forecast_dates, metric_name
                )
        ...
        if "exponential_smoothing" in forecast_methods and PANDAS_AVAILABLE:
            try:
                es_forecast = self._forecast_exponential_smoothing(
                    values, timestamps, forecast_dates, metric_name
                )
        ...
        if "linear_regression" in forecast_methods and SCIPY_AVAILABLE:
            try:
                lr_forecast = self._forecast_linear_regression(
                    values, timestamps, forecast_dates, metric_name
                )
    ...
    def _use_best_individual_forecast(
        ...
        if self.config.get("forecasting", {}).get("auto_model_selection", True):
            best_method, validation_metrics = self._select_best_forecast_method(
                method_results, historical_values
            )
            if best_method:
    ...
    def _tune_arima_hyperparameters(
        ...
        early_stopping_min_iterations = tuning_config.get("early_stopping", {}).get(
            "min_iterations", 3
        )
        early_stopping_patience = tuning_config.get("early_stopping", {}).get("patience", 3)
        early_stopping_tolerance = tuning_config.get("early_stopping", {}).get(
            "tolerance", 0.05
        ...
        if persistence_enabled:
            ...
            if (
                saved_params
                and "parameters" in saved_params
                and not saved_params.get("needs_revalidation", False)
            ):
                ...
                logger.info(
                    f"Using saved ARIMA parameters for {metric_name}: {saved_order}, MAPE: {saved_mape}"
                )
                ...
                logger.info(
                    f"Saved ARIMA parameters for {metric_name} need revalidation, retuning..."
                )
        ...
        if validation_data is not None:
            ...
            for idx, order in enumerate(param_combinations):
                try:
                    ...
                    # Record result
                    results.append({"order": order, "mape": mape})

                    # Update best model if this one is better
                    if mape < best_mape:
                        # Calculate improvement percentage
                        improvement = (
                            (best_mape - mape) / best_mape if best_mape != float("inf") else 1.0
                        )
                    ...
                    if (
                        ...
                        logger.info(
                            f"Early stopping triggered after {idx + 1} iterations without significant improvement"
                        )
                        break
            ...
            for idx, order in enumerate(param_combinations):
                try:
                    ...
                    if cv_mape_values:
                        ...
                        # Record result
                        results.append(
                            {"order": order, "mape": cv_mape, "cv_mape_values": cv_mape_values}
                        )
                        ...
                        if (
                            ...
                            logger.info(
                                f"Early stopping triggered after {idx + 1} iterations with {iterations_without_improvement} iterations without significant improvement"
                            )
                            break
        ...
        if best_mape == float("inf"):
            logger.warning(
                "Hyperparameter tuning failed to find optimal ARIMA parameters. Using default (1,1,1)."
            )
            best_order = (1, 1, 1)
            best_mape = None
        ...
        if best_mape is not None:
            logger.info(
                f"ARIMA hyperparameter tuning complete. Best order: {best_order}, MAPE: {best_mape:.2f}%"
            )
        else:
            logger.info(
                f"ARIMA hyperparameter tuning complete. Best order: {best_order}, MAPE: N/A"
            )
    ...
    def _forecast_arima(
        ...
        hyperparameter_tuning = (
            self.config["forecasting"].get("hyperparameter_tuning", {}).get("enabled", False)
        )

        if hyperparameter_tuning:
            try:
                # Determine validation size for hyperparameter tuning
                validation_size_percent = min(
                    max(1, self.config["forecasting"].get("validation_size_percent", 20)), 50
                )
                ...
                if len(train_data) >= self.config["forecasting"]["min_data_points"]:
                    ...
                    best_order, best_mape = self._tune_arima_hyperparameters(
                        ...
                        cv_folds=self.config["forecasting"]["hyperparameter_tuning"].get(
                            "cv_folds", 3
                        ),
                        metric_name=metric_name,
                    )
                ...
                logger.warning(
                    f"ARIMA hyperparameter tuning failed: {e}. Using default parameters."
                )
                model = ARIMA(df, order=(1, 1, 1))
                model_info = {
                    "type": "ARIMA",
    ...
    def _tune_exponential_smoothing_hyperparameters(
        ...
        # Check if we have saved parameters to reuse
        if persistence_enabled:
            saved_params = self._load_parameters(
                metric_name, "exponential_smoothing", data_signature
            )
            if (
                saved_params
                and "parameters" in saved_params
                ...
                saved_mape = saved_params.get("performance_metric")
                logger.info(
                    f"Using saved Exponential Smoothing parameters for {metric_name}: {saved_params_dict}, MAPE: {saved_mape}"
                )
                return saved_params_dict, saved_mape
            elif saved_params and saved_params.get("needs_revalidation", False):
                # Parameters found but need revalidation
                logger.info(
                    f"Saved Exponential Smoothing parameters for {metric_name} need revalidation, retuning..."
                )
        ...
        if search_method == "grid":
            ...
            for _ in range(max_iterations):
                ...
                seasonal = np.random.choice(
                    seasonals + [seasonals[0]]
                )  # Add extra weight to first option
        ...
        if validation_data is not None:
            ...
            for idx, params in enumerate(params_list):
                try:
                    ...
                    if (
                        params["seasonal"] is not None
                        and len(train_data) < params["seasonal_periods"] * 2
                    ):
                        continue
                    ...
                    # Record result
                    results.append({"params": params, "mape": mape})
            ...
            for idx, params in enumerate(params_list):
                try:
                    ...
                    for i in range(cv_folds):
                        ...
                        if (
                            params["seasonal"] is not None
                            and len(cv_train) < params["seasonal_periods"] * 2
                        ):
                            continue
                    ...
                    if cv_mape_values:
                        ...
                        # Record result
                        results.append(
                            {"params": params, "mape": cv_mape, "cv_mape_values": cv_mape_values}
                        )
                    ...
                    logger.warning(
                        f"Error in CV for Exponential Smoothing with params {params}: {e}"
                    )
                    continue
        ...
        if best_mape == float("inf"):
            logger.warning(
                "Hyperparameter tuning failed to find optimal Exponential Smoothing parameters. Using defaults."
            )
            best_params = {
                "trend": "add",
                "seasonal": "add" if len(train_data) >= 14 else None,
        ...
        if best_mape is not None:
            logger.info(
                f"Exponential Smoothing hyperparameter tuning complete. Best params: {param_str}, MAPE: {best_mape:.2f}%"
            )
        else:
            logger.info(
                f"Exponential Smoothing hyperparameter tuning complete. Best params: {param_str}, MAPE: N/A"
            )
        ...
        if persistence_enabled and best_params is not None:
            ...
            is_default = (
                best_params.get("trend") == "add"
                and best_params.get("seasonal") == "add"
                and best_params.get("seasonal_periods") == 7
                and best_params.get("damped_trend") == False
            )
    ...
    def _forecast_exponential_smoothing(
        ...
        if hyperparameter_tuning:
            try:
                ...
                # If tuning fails, fall back to default parameters
                logger.warning(
                    f"Exponential Smoothing hyperparameter tuning failed: {e}. Using default parameters."
                )
    ...
    def _tune_linear_regression_hyperparameters(
        ...
        if persistence_enabled:
            ...
            combined_data = (
                ...
                else np.hstack([X_train, y_train])
            )
        ...
        if persistence_enabled:
            ...
            if (
                ...
                saved_mape = saved_params.get("performance_metric")
                logger.info(
                    f"Using saved Linear Regression parameters for {metric_name}: {saved_params_dict}, MAPE: {saved_mape}"
                )
                return saved_params_dict, saved_mape
            elif saved_params and saved_params.get("needs_revalidation", False):
                # Parameters found but need revalidation
                logger.info(
                    f"Saved Linear Regression parameters for {metric_name} need revalidation, retuning..."
                )
        ...
        early_stopping_min_iterations = tuning_config.get("early_stopping", {}).get(
            "min_iterations", 2
        )  # Linear regression has fewer params
        early_stopping_patience = tuning_config.get("early_stopping", {}).get(
            "patience", 2
        )  # Fewer combos for linear regression
        early_stopping_tolerance = tuning_config.get("early_stopping", {}).get(
            "tolerance", 0.05
        )  # 5% improvement threshold
        ...
        if SKLEARN_AVAILABLE:
            fit_intercept_values = tuning_config["linear_regression_params"].get(
                "fit_intercept", [True, False]
            )
            positive_values = tuning_config["linear_regression_params"].get(
                "positive", [True, False]
            )
            ...
            # If using cross-validation or validation failed
                ...
                for idx, params in enumerate(param_combinations):
                    try:
                        ...
                        logger.warning(
                            f"Error in CV for Linear Regression with params {params}: {e}"
                        )
                        continue
            ...
            if best_mape == float("inf"):
                logger.warning(
                    "Hyperparameter tuning failed to find optimal Linear Regression parameters. Using defaults."
                )
                best_params = {"fit_intercept": True, "positive": False}
                best_mape = None
        else:
            ...
            best_mape = None
            logger.warning(
                "scikit-learn not available for advanced Linear Regression tuning. Using simple regression."
            )

        # Log results
        logger.info(
            f"Linear Regression hyperparameter tuning complete. Best params: {best_params}, MAPE: {best_mape if best_mape is not None else 'N/A'}"
        )
        ...
        if persistence_enabled and best_params is not None:
            # Don't save default parameters (which are used as a fallback when tuning fails)
            is_default = (
                best_params.get("fit_intercept") == True and best_params.get("positive") == False
            )
    ...
    def _forecast_linear_regression(
        ...
        forecast_days = np.array(
            [(date - first_timestamp).total_seconds() / (24 * 3600) for date in forecast_dates]
        )
        ...
        model_info = {"type": "LinearRegression", "tuned": False, "reason": "default_settings"}
        ...
        try:
            ...
            SKLEARN_AVAILABLE = False
            logger.warning(
                "scikit-learn not available. Using scipy.stats.linregress for linear regression."
            )
        ...
        if SKLEARN_AVAILABLE and hyperparameter_tuning:
            try:
                ...
                if len(X_train) >= self.config["forecasting"]["min_data_points"]:
                    ...
                    # Use the full dataset with the optimal parameters
                    model = LinearRegression(
                        fit_intercept=best_params["fit_intercept"], positive=best_params["positive"]
                    )
                    ...
                    for x in forecast_days:
                        ...
                        if X_diff_squared == 0:  # Avoid division by zero
                            ...
                            margin = (
                                t_value
                                * std_err_pred
                                * np.sqrt(1 + 1 / len(X) + ((x - X_mean) ** 2) / X_diff_squared)
                            )
                    ...
                    sum_squared_error = sum(
                        (intercept + slope * x - y) ** 2 for x, y in zip(days_since_start, values)
                    )
                    ...
                    for x in forecast_days:
                        ...
                        if X_diff_squared == 0:  # Avoid division by zero
                            ...
                            margin = (
                                ...
                                * std_err_pred
                                * np.sqrt(
                                    1 + 1 / len(values) + ((x - X_mean) ** 2) / X_diff_squared
                                )
                            )
                ...
                logger.warning(
                    f"Linear Regression hyperparameter tuning failed: {e}. Using default parameters."
                )
                ...
                for x in forecast_days:
                    ...
                    if X_diff_squared == 0:  # Avoid division by zero
                        ...
                        margin = (
                            t_value
                            * std_err_pred
                            * np.sqrt(1 + 1 / len(values) + ((x - X_mean) ** 2) / X_diff_squared)
                        )
    ...
    def _create_ensemble_forecast(
        self, method_results: Dict[str, Dict[str, Any]], forecast_dates: List[datetime.datetime]
    ) -> Dict[str, Any]:
        """
        ...
        ensemble_forecast = {
            ...
            "model_info": {"type": "Ensemble", "methods": list(method_results.keys())},
        }
    ...
    def _select_best_forecast_method(
        self, method_results: Dict[str, Dict[str, Any]], historical_values: List[float]
        ...
        try:
            ...
            if len(historical_values) < min_data_points:
                logger.warning(
                    f"Insufficient data points for model validation: {len(historical_values)} < {min_data_points}"
                )
                return None, None

            # Determine how much historical data to use for validation
            validation_percent = self.config.get("forecasting", {}).get(
                "validation_size_percent", 25
            )
            ...
            for method_name, method_forecast in method_results.items():
                ...
                errors = [
                    ...
                    for predicted, actual in zip(predicted_values, validation_data)
                ]
                ...
                mape = (
                    sum(mape_values) / len(mape_values) if mape_values else float("inf")
                )  # Mean Absolute Percentage Error

                # Store metrics
                validation_metrics[method_name] = {"mae": mae, "mape": mape, "errors": errors}
            ...
            logger.info(
                f"Automated model selection chose '{best_method}' with MAPE: {validation_metrics[best_method]['mape']:.2f}%"
            )
    ...
    def clean_parameter_storage(self, older_than_days: int = None) -> Dict[str, Any]:
        ...
        if older_than_days is not None:
            logger.info(
                f"Cleaned parameter storage: removed {removed_files} files older than {older_than_days} days"
            )
        else:
    ...
    def test_parameter_persistence(self, metric_name: str = "test_metric") -> Dict[str, Any]:
        ...
        try:
            ...
            arima_order1, mape1 = self._tune_arima_hyperparameters(
                train_data=df, metric_name=metric_name
            )
            ...
            logger.info(
                f"ARIMA parameter persistence test: {'Success' if arima_test_success else 'Failed'}"
            )
        ...
        try:
            ...
            es_params1, es_mape1 = self._tune_exponential_smoothing_hyperparameters(
                train_data=df, metric_name=f"{metric_name}_es"
            )
            ...
            # Parameters should be the same
            es_test_success = (
                es_params1.get("trend") == es_params2.get("trend")
                and es_params1.get("seasonal") == es_params2.get("seasonal")
                and es_params1.get("seasonal_periods") == es_params2.get("seasonal_periods")
                and es_params1.get("damped_trend") == es_params2.get("damped_trend")
            )
            logger.info(
                f"Exponential Smoothing parameter persistence test: {'Success' if es_test_success else 'Failed'}"
            )
        ...
        try:
            ...
            lr_params1, lr_mape1 = self._tune_linear_regression_hyperparameters(
                X_train=X, y_train=y, metric_name=f"{metric_name}_lr"
            )
            ...
            # Parameters should be the same
            lr_test_success = lr_params1.get("fit_intercept") == lr_params2.get(
                "fit_intercept"
            ) and lr_params1.get("positive") == lr_params2.get("positive")
            logger.info(
                f"Linear Regression parameter persistence test: {'Success' if lr_test_success else 'Failed'}"
            )
        ...
        return {
            ...
            "arima_params": {"order1": arima_order1, "order2": arima_order2}
            if arima_test_success
            else None,
            "es_params": {"params1": es_params1, "params2": es_params2}
            if es_test_success
            else None,
            "lr_params": {"params1": lr_params1, "params2": lr_params2}
            if lr_test_success
            else None,
        }

    def _analyze_forecast_trend(
        self, forecast_values: List[float], forecast_dates: List[datetime.datetime]
    ) -> Dict[str, Any]:
        """
        ...
        if len(forecast_values) < 2:
            return {"direction": "stable", "magnitude": "stable", "percent_change": 0.0}
        ...
        return {"direction": direction, "magnitude": magnitude, "percent_change": percent_change}
        ...
        if forecast_values and len(forecast_values) >= 1:
            ...
            if anomaly_indices:
                results["forecast_anomalies"] = {
                    ...
                    "z_scores": anomaly_z_scores,
                    "forecast_dates": [
                        forecast_result["forecast_dates"][i] for i in anomaly_indices
                    ],
                }
