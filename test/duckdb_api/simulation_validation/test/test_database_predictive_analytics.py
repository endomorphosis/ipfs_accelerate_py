...
class TestDatabasePredictiveAnalytics(unittest.TestCase):
    ...
    def test_parameter_validation(self):
        ...
        is_valid = self.analyzer._validate_parameters(
            valid_params, data_signature, force_revalidation=False
        )
        ...
        is_valid = self.analyzer._validate_parameters(
            old_params, data_signature, force_revalidation=False
        )
        ...
        is_valid = self.analyzer._validate_parameters(
            diff_signature_params, data_signature, force_revalidation=False
        )
        self.assertFalse(is_valid)

        # Test force revalidation
        is_valid = self.analyzer._validate_parameters(
            valid_params, data_signature, force_revalidation=True
        )
        self.assertFalse(is_valid)
    ...
    def test_parameter_persistence_end_to_end_arima(self):
        ...
        result1 = self.analyzer.forecast_time_series(
            test_df, metric_name=metric_name, model_type=model_type, forecast_days=7
        )
    ...
    def test_clean_parameter_storage(self):
        ...
        for metric in metrics:
            params = {"p": 2, "d": 1, "q": 2, "performance": 0.95, "metric_name": metric}
