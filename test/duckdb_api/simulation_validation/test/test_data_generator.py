...
class TestDataGenerator:
    ...
    BASE_METRICS = {
        "gpu": {
            "throughput_items_per_second": {"rtx3080": 90.0, "rtx4090": 150.0, "a100": 180.0},
            "average_latency_ms": {"rtx3080": 17.0, "rtx4090": 10.0, "a100": 8.5},
            "peak_memory_mb": {"rtx3080": 2200, "rtx4090": 2800, "a100": 3500},
        },
        "cpu": {
            ...
            "average_latency_ms": {"intel_xeon": 35.0, "amd_epyc": 32.0, "intel_core_i9": 38.0},
            "peak_memory_mb": {"intel_xeon": 1800, "amd_epyc": 1700, "intel_core_i9": 1900},
        },
        "webgpu": {
            "throughput_items_per_second": {"chrome": 60.0, "firefox": 55.0, "safari": 50.0},
            "average_latency_ms": {"chrome": 25.0, "firefox": 27.0, "safari": 29.0},
            "peak_memory_mb": {"chrome": 1500, "firefox": 1600, "safari": 1700},
        },
    }
    ...
    HARDWARE_DETAILS = {
        ...
        "cpu_intel_xeon": {
            ...
            "cores": 24,
            "threads": 48,
            "test_environment": {"os": "Linux", "memory_gb": 64, "compiler": "GCC 11.2"},
        },
        "cpu_amd_epyc": {
            "name": "AMD EPYC",
            "cores": 32,
            "threads": 64,
            "test_environment": {"os": "Linux", "memory_gb": 128, "compiler": "GCC 11.2"},
        },
        "cpu_intel_core_i9": {
            "name": "Intel Core i9",
            "cores": 16,
            "threads": 24,
            "test_environment": {"os": "Linux", "memory_gb": 32, "compiler": "GCC 11.2"},
        },
        "webgpu_chrome": {
            "name": "Chrome WebGPU",
            "browser_version": "120.0.6099.109",
            "test_environment": {"os": "Windows 10", "js_engine": "V8"},
        },
        "webgpu_firefox": {
            "name": "Firefox WebGPU",
            "browser_version": "113.0",
            "test_environment": {"os": "Windows 10", "js_engine": "SpiderMonkey"},
        },
        "webgpu_safari": {
            "name": "Safari WebGPU",
            "browser_version": "17.0",
            "test_environment": {"os": "macOS Sonoma", "js_engine": "JavaScriptCore"},
        },
    }
    ...
    def _add_trend(
        self, base_value: float, day_idx: int, trend_type: str, strength: float
    ) -> float:
    ...
    def _add_seasonality(
        self, base_value: float, day_idx: int, pattern: str, amplitude: float
    ) -> float:
    ...
    def _add_outliers(
        self, base_value: float, outlier_prob: float, outlier_magnitude: float
    ) -> float:
    ...
    def generate_hardware_result(
        ...
        for metric_name in ["throughput_items_per_second", "average_latency_ms", "peak_memory_mb"]:
            base_value = (
                self.BASE_METRICS.get(hardware_type, {}).get(metric_name, {}).get(hardware_model, 0)
            )
    ...
    def generate_simulation_result(
        ...
        simulation_params = {"model_params": self.MODEL_CONFIGS.get(model_id, {})}

        # Add hardware-specific parameters
        if hardware_type == "gpu":
            simulation_params["hardware_params"] = {
                "gpu_compute_capability": self.HARDWARE_DETAILS.get(hardware_id, {}).get(
                    "compute_capability", "8.0"
                ),
                "gpu_memory": self.HARDWARE_DETAILS.get(hardware_id, {}).get("vram_gb", 8)
                * 1024,  # Convert to MB
            }
            ...
            simulation_params["hardware_params"] = {
                ...
                "js_engine": self.HARDWARE_DETAILS.get(hardware_id, {})
                .get("test_environment", {})
                .get("js_engine", "V8"),
            }
    ...
    def generate_validation_result(
        ...
        if simulation_result is None and hardware_result is None:
            ...
            hardware_result = self.generate_hardware_result(
                model_id=model_id, hardware_id=hardware_id
            )

            # Generate matching simulation result
            simulation_result = self.generate_simulation_result(
                model_id=model_id, hardware_id=hardware_id, matching_hardware_result=hardware_result
            )
    ...
    def generate_calibration_record(
        ...
        if validation_results_before is None:
            ...
            for i in range(3):
                hw_result = self.generate_hardware_result(
                    model_id=model_type, hardware_id=hardware_type, time_index=i
                )
                ...
                val_result = self.generate_validation_result(
                    simulation_result=sim_result, hardware_result=hw_result
                )
            ...
            for i in range(3):
                ...
                for metric, value in hw_result.metrics.items():
                    if metric in correction_factors:
                        ...
                        sim_metrics[metric] = (
                            value / correction_factors[metric] * random.uniform(0.97, 1.03)
                        )
        ...
        result = CalibrationRecord(
            ...
            previous_parameters={"correction_factors": {k: 1.0 for k in correction_factors}},
            updated_parameters={"correction_factors": correction_factors},
    ...
    def generate_drift_detection_result(
        ...
        result = DriftDetectionResult(
            ...
            thresholds_used={"p_value": 0.05, "mean_change_pct": 10.0},
        )
    ...
    def generate_drift_scenario(
        ...
        List[HardwareResult], List[SimulationResult], List[ValidationResult], DriftDetectionResult
    ]:
    ...
    def generate_calibration_scenario(
        ...
        List[HardwareResult], List[SimulationResult], List[ValidationResult], CalibrationRecord
    ]:
    ...
    def generate_complete_dataset(
        ...
        selected_hardware = random.sample(
            available_hardware, min(num_hardware_types, len(available_hardware))
        )
        ...
        if include_calibrations:
            for hardware_id in selected_hardware:
                ...
                self.generate_calibration_scenario(
                    model_id=model_id, hardware_id=hardware_id, start_date=start_date
                )
    ...
    def save_dataset_to_json(self, dataset: Dict[str, List], output_path: str) -> str:
        ...
        serializable_dataset = {
            ...
            "data": {
                "simulation_results": [
                    result.to_dict() for result in dataset["simulation_results"]
                ],
                "hardware_results": [result.to_dict() for result in dataset["hardware_results"]],
                "validation_results": [
                    result.to_dict() for result in dataset["validation_results"]
                ],
                "calibration_records": [
                    record.to_dict() for record in dataset["calibration_records"]
                ],
                "drift_detection_results": [
                    result.to_dict() for result in dataset["drift_detection_results"]
                ],
            },
        }
