...
def create_test_data(
    ...
    for i in range(num_samples):
        ...
        base_throughput = (
            100 * (1 + 0.1 * model_idx) * (1 + 0.3 * batch_size) * (1 + 0.2 * (precision == "fp16"))
        )
        base_latency = (
            10 * (1 + 0.2 * model_idx) * (1 - 0.05 * batch_size) * (1 - 0.1 * (precision == "fp16"))
        )
        base_memory = (
            1000
            * (1 + 0.3 * model_idx)
            * (1 + 0.1 * batch_size)
            * (1 - 0.2 * (precision == "fp16"))
        )
        ...
        sim_result = SimulationResult(
            ...
            additional_metadata={"test_case": f"test_{i}"},
        )
        ...
        hw_result = HardwareResult(
            ...
            precision=precision,
            timestamp=timestamp,
            hardware_info={"type": "gpu" if "gpu" in hardware_id else "cpu", "name": hardware_id},
            test_environment={
        ...
        val_result = ValidationResult(
            ...
            validation_metrics={
                "throughput_items_per_second": {
                    ...
                    "relative_error": abs(sim_throughput - hw_throughput) / hw_throughput
                    if hw_throughput > 0
                    else None,
                    "percentage_error": abs(sim_throughput - hw_throughput) / hw_throughput * 100
                    if hw_throughput > 0
                    else None,
                },
                "average_latency_ms": {
                    "absolute_error": abs(sim_latency - hw_latency),
                    "relative_error": abs(sim_latency - hw_latency) / hw_latency
                    if hw_latency > 0
                    else None,
                    "percentage_error": abs(sim_latency - hw_latency) / hw_latency * 100
                    if hw_latency > 0
                    else None,
                },
                "memory_peak_mb": {
                    "absolute_error": abs(sim_memory - hw_memory),
                    "relative_error": abs(sim_memory - hw_memory) / hw_memory
                    if hw_memory > 0
                    else None,
                    "percentage_error": abs(sim_memory - hw_memory) / hw_memory * 100
                    if hw_memory > 0
                    else None,
                },
            },
...
class TestAdvancedCalibrator(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            calibrator.config["metrics_to_calibrate"],
            [
                "throughput_items_per_second",
                "average_latency_ms",
                "memory_peak_mb",
    ...
    def test_basic_calibration(self):
        ...
        for metric in metrics:
            self.assertIn(
                metric, updated_parameters["correction_factors"]["gpu_rtx3080"]["bert-base-uncased"]
            )

            # For linear scaling, factor should be a scalar close to expected values
            factor = updated_parameters["correction_factors"]["gpu_rtx3080"]["bert-base-uncased"][
                metric
            ]
    ...
    def test_ensemble_calibration(self):
        ...
        config = {
            ...
            "min_samples_per_model": 1,
            "learning_rate": 1.0,  # Use full update for testing
            "ensemble_weights": {"linear_scaling": 0.5, "regression": 0.5},
        }
        calibrator = AdvancedSimulationCalibrator(config=config)

        # Create more test data
        _, _, validation_results = create_test_data(
            num_samples=30, hardware_ids=["gpu_rtx3080"], model_ids=["bert-base-uncased"]
        )

        # Calibrate
        updated_parameters = calibrator.calibrate(
            validation_results=validation_results, simulation_parameters=self.simulation_parameters
        )
    ...
    def test_apply_calibration(self):
        ...
        calibrated_result = calibrator.apply_calibration(
            simulation_result=simulation_result, simulation_parameters=simulation_parameters
        )
        ...
        self.assertEqual(
            calibrated_result.additional_metadata["calibration_method"], "linear_scaling"
        )
    ...
    def test_apply_ensemble_calibration(self):
                ...
                "gpu_rtx3080": {
                    "bert-base-uncased": {
                        "throughput_items_per_second": {
                            ...
                            "ensemble_weights": {"linear_scaling": 0.7, "regression": 0.3},
                        },
                        "average_latency_ms": {
                            "ensemble_factors": {
                                "linear_scaling": 1.2,
                            ...
                            "ensemble_weights": {"linear_scaling": 0.6, "regression": 0.4},
                        },
                    }
                }
            },
        ...
        self.assertAlmostEqual(
            calibrated_result.metrics["throughput_items_per_second"], expected_throughput, places=5
        )
        self.assertAlmostEqual(
            calibrated_result.metrics["average_latency_ms"], expected_latency, places=5
        )
    ...
    def test_evaluate_calibration(self):
        ...
        for i in range(10):
            ...
            hw_result = HardwareResult(
                ...
                metrics={
                    ...
                    * (1 + 0.1 * np.random.randn()),  # 20% lower than sim
                    "average_latency_ms": 12.0
                    * (1 + 0.1 * np.random.randn()),  # 20% higher than sim
                ...
                validation_metrics={
                    "throughput_items_per_second": {
                        "percentage_error": abs(
                            sim_result.metrics["throughput_items_per_second"]
                            - hw_result.metrics["throughput_items_per_second"]
                        )
                        / hw_result.metrics["throughput_items_per_second"]
                        * 100
                    },
                    "average_latency_ms": {
                        "percentage_error": abs(
                            sim_result.metrics["average_latency_ms"]
                            - hw_result.metrics["average_latency_ms"]
                        )
                        / hw_result.metrics["average_latency_ms"]
                        * 100
                    },
                    "memory_peak_mb": {
                        "percentage_error": abs(
                            sim_result.metrics["memory_peak_mb"]
                            - hw_result.metrics["memory_peak_mb"]
                        )
                        / hw_result.metrics["memory_peak_mb"]
                        * 100
                    },
        ...
        for i in range(10):
            sim_result = SimulationResult(
                ...
                metrics={
                    "throughput_items_per_second": 80.0
                    * (1 + 0.05 * np.random.randn()),  # Calibrated
            ...
            hw_result = HardwareResult(
                ...
                metrics={
                    "throughput_items_per_second": 80.0
                    * (1 + 0.1 * np.random.randn()),  # Same as before
        ...
        evaluation = calibrator.evaluate_calibration(
            before_calibration=before_validation, after_calibration=after_validation
        )
    ...
    def test_hardware_profiles(self):
        ...
        calibrator = AdvancedSimulationCalibrator(config={"use_hardware_profiles": True})
    ...
    def test_learning_rate_adaptation(self):
        ...
        # Calibrate
        updated_parameters = calibrator.calibrate(
            validation_results=self.validation_results, simulation_parameters=simulation_parameters
        )

        # Verify that calibration history was updated
        self.assertEqual(
            len(updated_parameters["calibration_history"]), 3 + 1
        )  # Previous 3 + new one
        ...
        simulation_result = SimulationResult(
            model_id="bert-base-uncased",
            hardware_id="gpu_rtx3080",
            metrics={"throughput_items_per_second": 100.0},
            batch_size=1,
            precision="fp32",
            timestamp=datetime.now().isoformat(),
            simulation_version="sim_v1.0",
        ...
        calibrated_result = calibrator.apply_calibration(
            simulation_result=simulation_result, simulation_parameters=updated_parameters
        )
    ...
    def test_cross_validation_for_calibration(self):
        ...
        _, _, validation_results = create_test_data(
            num_samples=50, hardware_ids=["gpu_rtx3080"], model_ids=["bert-base-uncased"]
        )
    ...
    def test_uncertainty_quantification(self):
        ...
        _, _, validation_results = create_test_data(
            num_samples=20, hardware_ids=["gpu_rtx3080"], model_ids=["bert-base-uncased"]
        )
