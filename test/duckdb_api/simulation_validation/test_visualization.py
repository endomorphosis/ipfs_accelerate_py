...
from core.base import SimulationResult, HardwareResult, ValidationResult
from simulation_validation_framework import SimulationValidationFramework, get_framework_instance
...
def test_visualizer_directly():
    ...
    drift_viz_path = visualizer.create_drift_detection_visualization(
        drift_results, output_path=str(output_dir / "drift_detection.html"), interactive=True
    )
    ...
    dashboard_path = visualizer.create_comprehensive_dashboard(
        validation_results, output_path=str(output_dir / "dashboard.html")
    )
...
def test_framework_visualization():
    ...
    dashboard_path = framework.create_comprehensive_dashboard(
        validation_results, output_path=str(output_dir / "framework_dashboard.html")
    )
...
class TestVisualizationEnhancements(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            ...
            timestamp = (datetime.datetime(2025, 3, 1) + datetime.timedelta(days=i)).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
            ...
            metrics_comparison = {
                "throughput_items_per_second": {
                    ...
                    "error": hw_result.metrics["throughput_items_per_second"]
                ...
                "average_latency_ms": {
                    ...
                    "error": hw_result.metrics["average_latency_ms"]
                ...
                "peak_memory_mb": {
                    ...
                    "error": hw_result.metrics["peak_memory_mb"]
                    - sim_result.metrics["peak_memory_mb"],
                    "mape": abs(
                        hw_result.metrics["peak_memory_mb"] - sim_result.metrics["peak_memory_mb"]
                    )
                    / hw_result.metrics["peak_memory_mb"]
                    * 100,
                },
            }
