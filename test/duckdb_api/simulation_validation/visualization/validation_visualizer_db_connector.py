...
class ValidationVisualizerDBConnector:
    ...
    def _connect_to_dashboard(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                auth_endpoint, headers=headers, json={"client": "simulation_validation_framework"}
            )
            ...
            if response.status_code == 200:
                ...
                self.dashboard_session_expires = datetime.datetime.now() + datetime.timedelta(
                    seconds=expires_in
                )
    ...
    def _ensure_dashboard_connection(self) -> bool:
        ...
        if (
            self.dashboard_session_expires
            and datetime.datetime.now()
            > self.dashboard_session_expires - datetime.timedelta(minutes=5)
        ):
    ...
    def create_dashboard_panel_from_db(
        ...
        **additional_params,
        ...
        try:
            ...
            response = requests.post(panel_endpoint, headers=headers, json=panel_config)
            ...
            if response.status_code in [200, 201]:
                ...
                logger.error(
                    f"Failed to create dashboard panel. Status code: {response.status_code}"
                )
                logger.error(f"Response: {response.text}")
    ...
    def create_comprehensive_monitoring_dashboard(
        ...
        **additional_params,
        ...
        try:
            ...
            response = requests.post(dashboard_endpoint, headers=headers, json=dashboard_config)
            ...
            if response.status_code in [200, 201]:
                ...
                return {
                    ...
                    "dashboard_id": dashboard_id,
                    "url": urljoin(
                        self.dashboard_url.replace("/api", ""), f"/dashboard/{dashboard_id}"
                    ),
    ...
    def set_up_real_time_monitoring(
        ...
        try:
            ...
            response = requests.post(monitoring_endpoint, headers=headers, json=monitoring_config)

            if response.status_code in [200, 201]:
                monitoring_data = response.json()
                logger.info(
                    f"Successfully set up real-time monitoring for {model_type} on {hardware_type}"
                )
    ...
    def create_hardware_comparison_heatmap_from_db(
        ...
        try:
            # Get validation results from the database
            validation_results = self._get_validation_results_from_db(
                model_ids=model_ids, start_date=start_date, end_date=end_date
            )
            ...
            for hw in hardware_models:
                ...
                for model in model_types:
                    ...
                    filtered_results = [
                        ...
                        if vr.hardware_result.hardware_id == hw
                        and vr.simulation_result.model_id == model
                    ]
    ...
    def create_time_series_chart_from_db(
        ...
        try:
            ...
            validation_results = sorted(validation_results, key=lambda vr: vr.validation_timestamp)
        ...
        try:
            ...
            if metrics:
                ...
                for dr in drift_results:
                    filtered_metrics = {
                        metric: data
                        for metric, data in dr.drift_metrics.items()
                        if metric in metrics
                    }
                    dr.drift_metrics = filtered_metrics
                    if (
                        filtered_metrics
                    ):  # Only include if it has at least one of the requested metrics
    ...
    def create_calibration_improvement_chart_from_db(
        ...
        try:
            ...
            if start_date:
                calibration_records = [
                    cr for cr in calibration_records if cr.timestamp >= start_date
                ]
            if end_date:
    ...
    def create_comprehensive_dashboard_from_db(
        ...
        try:
            ...
            mape_chart_path = self.create_mape_comparison_chart_from_db(
                ...
                end_date=end_date,
                interactive=interactive,
                output_path=os.path.join(os.path.dirname(output_path), "mape_comparison.html")
                if output_path
                else None,
            ...
            for metric in metrics:
                ts_chart_path = self.create_time_series_chart_from_db(
                    ...
                    output_path=os.path.join(
                        os.path.dirname(output_path), f"time_series_{metric}.html"
                    )
                    if output_path
                    else None,
            ...
            return self.visualizer.create_comprehensive_dashboard(
                visualizations=visualizations, title=dashboard_title, output_path=output_path
            )
    ...
    def _get_validation_results_from_db(
        ...
        if hardware_ids and len(hardware_ids) == 1 and model_ids and len(model_ids) == 1:
            ...
            results = self.db_integration.get_validation_results_by_model_and_hardware(
                model_id=model_ids[0], hardware_id=hardware_ids[0]
            )
            ...
            for model_id in model_ids:
                results = self.db_integration.get_validation_results_by_model_and_hardware(
                    model_id=model_id, hardware_id=hardware_ids[0]
                )
                validation_results.extend(results)
            ...
            for hardware_id in hardware_ids:
                results = self.db_integration.get_validation_results_by_model_and_hardware(
                    model_id=model_ids[0], hardware_id=hardware_id
                )
                validation_results.extend(results)
        else:
        ...
        if hardware_ids:
            validation_results = [
                vr for vr in validation_results if vr.hardware_result.hardware_id in hardware_ids
            ]

        # Filter by model IDs if specified
        if model_ids:
            validation_results = [
                vr for vr in validation_results if vr.simulation_result.model_id in model_ids
            ]

        # Filter by date if specified
        if start_date:
            validation_results = [
                vr for vr in validation_results if vr.validation_timestamp >= start_date
            ]

        if end_date:
            validation_results = [
                vr for vr in validation_results if vr.validation_timestamp <= end_date
            ]
