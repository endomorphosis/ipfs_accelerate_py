...
class ValidationVisualizer:
    ...
    def export_visualization(
        self, fig: Any, output_path: Optional[str] = None, formats: Optional[List[str]] = None
    ) -> Dict[str, str]:
        """
            ...
            try:
                if is_plotly:
                    ...
                    if matplotlib_available and hasattr(fig, "savefig"):
                        ...
                        logger.warning(
                            f"Cannot export to {fmt}: matplotlib not available or invalid figure object"
                        )
            except Exception as e:
    ...
    def create_animated_time_series(
        ...
        for val_result in validation_results:
            if (
                val_result.hardware_result.hardware_id == hardware_id
                and val_result.simulation_result.model_id == model_id
            ):
                filtered_results.append(val_result)

        if not filtered_results:
            logger.warning(
                f"No validation results found for hardware '{hardware_id}' and model '{model_id}'"
            )
            return None
                ...
                data.append(
                    {
                        ...
                        "mape": mape,
                        "error": hw_value - sim_value
                        if hw_value is not None and sim_value is not None
                        else None,
                    }
                )
        ...
        try:
            ...
            updatemenus = [
                {
                    "buttons": [
                        {"args": [None, animation_settings], "label": "Play", "method": "animate"},
                        {
                            "args": [
                                [None],
                                {
    ...
    def create_3d_error_visualization(
        ...
        if hardware_ids:
            filtered_results = [
                r for r in filtered_results if r.hardware_result.hardware_id in hardware_ids
            ]

        if model_ids:
            filtered_results = [
                r for r in filtered_results if r.simulation_result.model_id in model_ids
            ]
        ...
        try:
            ...
            if len(metrics) == 3:
                ...
                fig = go.Figure(
                    data=[
                        go.Scatter3d(
                            ...
                            text=[
                                f"Hardware: {h}<br>Model: {m}<br>Batch: {b}<br>Precision: {p}<br>"
                ...
                # Add annotations
                fig.add_annotation(
                    x=2.5, y=2.5, text="Excellent", showarrow=False, font=dict(color="green")
                )

                fig.add_annotation(
                    x=7.5, y=7.5, text="Good", showarrow=False, font=dict(color="orange")
                )

                fig.add_annotation(x=15, y=15, text="Poor", showarrow=False, font=dict(color="red"))
    ...
    def create_mape_comparison_chart(
        ...
        for val_result in validation_results:
            ...
            if metric_name == "all":
                ...
                if (
                    metric_name in val_result.metrics_comparison
                    and "mape" in val_result.metrics_comparison[metric_name]
                ):
    ...
    def create_metric_comparison_chart(
        ...
        for val_result in validation_results:
            if (
                val_result.hardware_result.hardware_id == hardware_id
                and val_result.hardware_result.model_id == model_id
            ):
                filtered_results.append(val_result)
        ...
        if metrics is None:
            ...
            for metric in common_metrics:
                if (
                    metric in val_result.simulation_result.metrics
                    and metric in val_result.hardware_result.metrics
                ):
        ...
        for metric in metrics:
            if (
                ...
                data.append({"metric": metric, "simulation": sim_value, "hardware": hw_value})
        ...
        if pandas_available:
            ...
            if use_interactive and plotly_available:
                return self._create_interactive_metric_comparison(
                    df, title, show_absolute_values, output_path
                )
            else:
                return self._create_static_metric_comparison(
                    df, title, show_absolute_values, output_path
                )
        else:
            # Basic implementation without pandas
            if matplotlib_available:
                return self._create_basic_metric_comparison(
                    data, title, show_absolute_values, output_path
                )
            else:
                logger.error(
                    "Cannot create metric comparison chart: required dependencies not available"
                )
                return None
    ...
    def create_error_distribution_chart(
        ...
        if pandas_available and numpy_available:
            ...
            if use_interactive and plotly_available:
                return self._create_interactive_error_distribution(
                    df, title, metric_name, output_path
                )
            else:
            ...
            if matplotlib_available:
                ...
                logger.error(
                    "Cannot create error distribution chart: required dependencies not available"
                )
                return None
        ...
        if pandas_available:
            ...
            if use_interactive and plotly_available:
                return self._create_interactive_time_series(
                    df, title, metric_name, show_trend, output_path
                )
            else:
            ...
            # Basic implementation without pandas
            if matplotlib_available:
                return self._create_basic_time_series(
                    data, title, metric_name, show_trend, output_path
                )
            else:
    ...
    def create_hardware_comparison_heatmap(
        ...
        for i, hw_id in enumerate(hw_list):
            for j, mdl_id in enumerate(mdl_list):
                ...
                for val_result in validation_results:
                    if (
                        val_result.hardware_result.hardware_id == hw_id
                        and val_result.hardware_result.model_id == mdl_id
                    ):
        ...
        if pandas_available and numpy_available:
            ...
            if matplotlib_available:
                ...
                logger.error(
                    "Cannot create hardware comparison heatmap: required dependencies not available"
                )
                return None
    ...
    def create_metric_importance_chart(
        ...
        if pandas_available:
            ...
            if matplotlib_available:
                ...
                logger.error(
                    "Cannot create metric importance chart: required dependencies not available"
                )
                return None
    ...
    def create_comprehensive_dashboard(
        ...
        try:
            ...
            for i, section in enumerate(sections, 1):
                try:
                    self._add_dashboard_section(
                        fig, i, 1, section, validation_results, hardware_id, model_id
                    )
                except Exception as e:
            ...
            # Update layout
            fig.update_layout(
                title=title, height=400 * rows, width=self.config["default_width"], showlegend=True
            )
    ...
    def create_drift_detection_visualization(
        ...
        if (
            not drift_results
            or "status" not in drift_results
            or drift_results["status"] != "success"
        ):
        ...
        if pandas_available:
            ...
            if use_interactive and plotly_available:
                return self._create_interactive_drift_visualization(
                    df, drift_results, title, output_path
                )
            else:
                return self._create_static_drift_visualization(
                    df, drift_results, title, output_path
                )
        else:
            # Basic implementation without pandas
            if matplotlib_available:
                return self._create_basic_drift_visualization(
                    drift_metrics, drift_results, title, output_path
                )
            else:
                logger.error(
                    "Cannot create drift visualization: required dependencies not available"
                )
                return None
    ...
    def create_calibration_improvement_chart(
        ...
        for i in range(len(before_calibration)):
            ...
            if (
                before.hardware_result.hardware_id != after.hardware_result.hardware_id
                or before.hardware_result.model_id != after.hardware_result.model_id
            ):
            ...
            for metric in common_metrics:
                if (
                    metric in before.metrics_comparison
                    and "mape" in before.metrics_comparison[metric]
                    and metric in after.metrics_comparison
                    and "mape" in after.metrics_comparison[metric]
                ):
        ...
        if pandas_available:
            ...
            if matplotlib_available:
                ...
                logger.error(
                    "Cannot create calibration improvement chart: required dependencies not available"
                )
                return None
    ...
    def create_3d_error_visualization(
        ...
        for val_result in validation_results:
            ...
            for metric in metrics:
                if (
                    metric in val_result.metrics_comparison
                    and "mape" in val_result.metrics_comparison[metric]
                ):
    ...
    def _create_static_mape_chart(self, df, title, output_path):
        ...
        try:
            ...
            df.pivot(index="hardware_id", columns="model_id", values="mape").plot(
                kind="bar", ax=ax, colormap=self.config["color_scheme"]
            )
    ...
    def _create_basic_mape_chart(self, data, title, output_path):
        ...
        try:
            ...
            model_ids = sorted(
                set([model_id for item in data for model_id in hw_model_mape[item["hardware_id"]]])
            )
            ...
            for i, model_id in enumerate(model_ids):
                bar_positions[model_id] = [
                    j - 0.4 + (i + 0.5) * bar_width for j in range(len(hardware_ids))
                ]
            ...
            for model_id in model_ids:
                ...
                plt.bar(bar_positions[model_id], values, width=bar_width, label=model_id)
    ...
    def _add_dashboard_section(
        self, fig, row, col, section, validation_results, hardware_id, model_id
    ):
        ...
        try:
            ...
            if section == "summary":
                ...
                if summary_data:
                    ...
                    # Format the table
                    summary_df["average_mape"] = summary_df["average_mape"].apply(
                        lambda x: f"{x:.2f}%"
                    )
                    summary_df["timestamp"] = pd.to_datetime(summary_df["timestamp"]).dt.strftime(
                        "%Y-%m-%d %H:%M"
                    )
                ...
                for (hw_id, mdl_id), val_result in latest_results.items():
                    ...
                    if mape_values:
                        ...
                        data.append({"hardware_id": hw_id, "model_id": mdl_id, "mape": avg_mape})
                ...
                if data:
                    ...
                    bar = go.Bar(
                        ...
                        marker=dict(
                            ...
                            colorbar=dict(title="MAPE (%)", thickness=15, len=0.7, y=0.5),
                            cmin=0,
                ...
                if sorted_metrics:
                    # Create DataFrame
                    metric_data = [
                        {"metric": metric, "avg_mape": avg_mape}
                        for metric, avg_mape in sorted_metrics
                    ]
                ...
                if pandas_available and numpy_available:
                    ...
                    heatmap = go.Heatmap(
                        ...
                        hoverongaps=False,
                        text=[
                            [f"{val:.2f}%" if not pd.isna(val) else "N/A" for val in row]
                ...
                if pandas_available and numpy_available:
                    ...
                    if df.shape[1] >= 2:
                        ...
                        heatmap = go.Heatmap(
                            ...
                            zmin=-1,
                            zmax=1,
                            colorbar=dict(title="Correlation", thickness=15, len=0.7, y=0.5),
                ...
                ts_data = {
                    "timestamp": [
                        val_result.validation_timestamp for val_result in time_series_results
                    ]
                }
    ...
    def _create_interactive_heatmap(self, df, title, output_path):
        ...
        try:
            ...
            fig.update_layout(
                ...
                height=self.config["default_height"],
                xaxis=dict(
                    title="Model", tickangle=-45, tickfont=dict(size=12), titlefont=dict(size=14)
                ),
                yaxis=dict(title="Hardware", tickfont=dict(size=12), titlefont=dict(size=14)),
                font=dict(size=self.config["font_size"]),
