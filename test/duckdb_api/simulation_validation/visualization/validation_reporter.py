...
# Import base classes
from data.duckdb.simulation_validation.core.base import ValidationResult, ValidationReporter
...
try:
    ...
    logger.warning(
        "NumPy and/or SciPy not available. Statistical analysis features will be limited."
    )
...
class ValidationReporterImpl(ValidationReporter):
    ...
    def _create_error_distribution_chart(
        ...
        if interactive and PLOTLY_AVAILABLE:
            ...
            logger.warning(
                "Cannot create error distribution chart: No visualization library available"
            )
            return ""

    def _create_interactive_error_distribution(
        self, validation_results: List[ValidationResult], metric_name: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            fig = px.histogram(
                ...
                if self.config["report_theme"] == "light"
                else "plotly_dark",
            )
    ...
    def _create_static_error_distribution(
        ...
        try:
            ...
            if n_metrics > 1:
                ...
                ax.hist(values, alpha=0.7, bins=min(20, len(values)), edgecolor="black")
    ...
    def _generate_executive_summary(
        ...
        for (hw_id, model_id), results in hardware_model_results.items():
            ...
            if result_mape_values:
                hw_model_mapes[(hw_id, model_id)] = sum(result_mape_values) / len(
                    result_mape_values
                )
    ...
    def _create_static_statistical_analysis(
        ...
        try:
            ...
            fig.suptitle(
                f"Statistical Analysis - {'All Metrics' if metric_name is None else metric_name}"
            )
            ...
            table_data = [
                ...
                [
                    f"{self.config['statistical_confidence_level'] * 100:.0f}% CI",
            ...
            # Create table
            table = ax3.table(
                cellText=table_data, colWidths=[0.3, 0.3], cellLoc="center", loc="center"
            )
            ...
            # Cache the visualization
            if cache_key:
                self._cache_visualization(
                    cache_key, image_base64, self.config["visualization_format"]
                )
    ...
    def generate_report(
        ...
        if hardware_filter:
            filtered_results = [
                vr for vr in filtered_results if vr.hardware_result.hardware_id == hardware_filter
            ]

        if model_filter:
            filtered_results = [
                vr for vr in filtered_results if vr.hardware_result.model_id == model_filter
            ]
        ...
        if date_range:
            ...
            if start_date:
                filtered_results = [
                    vr for vr in filtered_results if vr.validation_timestamp >= start_date
                ]
            if end_date:
                filtered_results = [
                    vr for vr in filtered_results if vr.validation_timestamp <= end_date
                ]
        ...
        if include_visualizations:
            ...
            if "error_distribution" in self.config["visualization_types"]:
                visualizations["error_distribution"] = self._create_error_distribution_chart(
                    filtered_results, interactive=(format == "html")
                )

            if (
                include_statistical_analysis
                and "statistical_analysis" in self.config["visualization_types"]
            ):
                visualizations["statistical_analysis"] = (
                    self._create_statistical_analysis_visualization(
                        filtered_results, interactive=(format == "html")
                    )
                )
    ...
    def _generate_enhanced_html_report(self, report_data: Dict[str, Any]) -> str:
        ...
        include_exec_summary = "executive_summary" in include_sections and report_data.get(
            "executive_summary"
        )
        ...
        if "hardware_comparison" in include_sections:
            ...
            if include_visualizations and "error_distribution" in visualizations:
                ...
                if error_dist:
                    ...
                    if (
                        error_dist.startswith("<!DOCTYPE html")
                        or error_dist.startswith("<html")
                        or error_dist.startswith("<div")
                    ):
                        hw_visualization = f"""
        ...
        if include_statistical_analysis and "statistical_analysis" in include_sections:
            ...
            if include_visualizations and "statistical_analysis" in visualizations:
                ...
                if stat_analysis:
                    # Check if it's an HTML or an image
                    if (
                        stat_analysis.startswith("<!DOCTYPE html")
                        or stat_analysis.startswith("<html")
                        or stat_analysis.startswith("<div")
                    ):
        ...
        if "detailed_results" in include_sections:
            ...
            for i, val_result in enumerate(report_data["validation_results"]):
                ...
                throughput_mape = val_result.metrics_comparison.get(
                    "throughput_items_per_second", {}
                ).get("mape", "N/A")
                latency_mape = val_result.metrics_comparison.get("average_latency_ms", {}).get(
                    "mape", "N/A"
                )
                memory_mape = val_result.metrics_comparison.get("memory_peak_mb", {}).get(
                    "mape", "N/A"
                )
                power_mape = val_result.metrics_comparison.get("power_consumption_w", {}).get(
                    "mape", "N/A"
                )
        ...
        if include_recommendations and "recommendations" in include_sections:
            ...
            if report_data.get("executive_summary") and report_data["executive_summary"].get(
                "recommendations"
            ):
        ...
        if include_exec_summary and "executive_summary" in include_sections:
            ...
            if "statistical_metrics" in exec_summary and exec_summary["statistical_metrics"]:
                ...
                if "confidence_interval" in sm and "confidence_level" in sm:
                    ...
                    cl = sm["confidence_level"]
                    markdown += (
                        f"- **{cl * 100:.0f}% Confidence Interval:** {ci[0]:.2f}% - {ci[1]:.2f}%\n"
                    )
            ...
            if exec_summary.get("best_metric") and exec_summary.get("worst_metric"):
                ...
                markdown += "\n### Best and Worst Metrics\n\n"
                markdown += (
                    f"- **Best performing metric:** {best_metric} ({best_value:.2f}% MAPE)\n"
                )
                markdown += (
                    f"- **Worst performing metric:** {worst_metric} ({worst_value:.2f}% MAPE)\n"
                )
            ...
            if include_recommendations and exec_summary.get("recommendations"):
                ...
                for rec in exec_summary["recommendations"]:
                    markdown += (
                        f"- **{rec['type'].replace('_', ' ').title()}:** {rec['recommendation']}\n"
                    )
    ...
    def _generate_enhanced_json_report(self, report_data: Dict[str, Any]) -> str:
        ...
        json_data = {
            "title": report_data.get("custom_title")
            or self.config["report_title_template"].format(
                timestamp=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ),
            "timestamp": datetime.datetime.now().isoformat(),
            "total_results": report_data["total_results"],
    ...
    def _generate_enhanced_text_report(self, report_data: Dict[str, Any]) -> str:
        ...
        if "hardware_comparison" in include_sections:
            ...
            col_widths = {"hardware": 25, "count": 10, "mape": 10, "status": 15}
        ...
        if "model_comparison" in include_sections:
            ...
            col_widths = {"model": 30, "count": 10, "mape": 10, "status": 15}
        ...
        if format == "pdf" and not report_content.startswith("<!DOCTYPE html"):
            ...
            if report_content != output_path:
                try:
                    ...
                    logger.error(
                        f"Error copying PDF report from {report_content} to {output_path}: {e}"
                    )
    ...
    def _generate_html_report(
        self, report_data: Dict[str, Any], include_visualizations: bool
    ) -> str:
        """
        ...
        hardware_model_summary += (
            "<tr><th>Hardware</th><th>Model</th><th>Count</th><th>MAPE</th><th>Status</th></tr>"
        )
        ...
        for (hw_id, model_id), stats in report_data["hardware_model_stats"].items():
            if "mean_mape" in stats:
                ...
                hardware_model_summary += (
                    f"<td class='status-{status_class}'>{stats['status']}</td>"
                )
                hardware_model_summary += f"</tr>"
        ...
        for i, val_result in enumerate(report_data["validation_results"]):
            ...
            detailed_results += (
                f"<td>{latency_mape if latency_mape == 'N/A' else f'{latency_mape:.2f}%'}</td>"
            )
            detailed_results += (
                f"<td>{memory_mape if memory_mape == 'N/A' else f'{memory_mape:.2f}%'}</td>"
            )
            detailed_results += (
                f"<td>{power_mape if power_mape == 'N/A' else f'{power_mape:.2f}%'}</td>"
            )
            detailed_results += "</tr>"
    ...
    def _generate_markdown_report(
        ...
        for i, val_result in enumerate(report_data["validation_results"]):
            ...
            markdown += (
                f"| {throughput_mape if throughput_mape == 'N/A' else f'{throughput_mape:.2f}%'} "
            )
