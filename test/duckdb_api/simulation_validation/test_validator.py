            ...
            for batch_size in batch_sizes:
                for precision in precisions:
                    ...
                    for i in range(num_samples):
                        ...
                        hw_throughput = (
                            throughput_base * throughput_scale * (1 + np.random.normal(0, 0.05))
                        )
                        ...
                        sim_throughput = (
                            hw_throughput
                            * (1.2 if "gpu" in hardware_id else 1.1)
                            * (1 + np.random.normal(0, 0.1))
                        )
                        sim_latency = (
                            hw_latency
                            * (0.9 if "gpu" in hardware_id else 0.95)
                            * (1 + np.random.normal(0, 0.1))
                        )
...
def test_validation_basic(framework, output_dir):
    ...
    try:
        ...
        from data.duckdb.simulation_validation.comparison.comparison_pipeline import (
            ComparisonPipeline,
        )
        from data.duckdb.simulation_validation.statistical.statistical_validator import (
...
def test_validation_plan(framework, output_dir):
    ...
    try:
        ...
        # Print summary
        logger.info(
            f"Standard plan - Current confidence: {standard_plan['current_confidence']:.2f}"
        )
...
def test_check_calibration(framework, output_dir):
    ...
    try:
        ...
        for key, result in calibration_results.items():
            if "calibration_recommended" in result and hasattr(
                result["calibration_recommended"], "item"
            ):
...
def main():
    ...
    parser.add_argument(
        "--hardware", action="store_true", help="Run validation test by hardware type"
    )
    ...
    parser.add_argument(
        "--calibration", action="store_true", help="Run calibration need check test"
    )
    ...
    run_all = args.all or not any(
        [args.basic, args.hardware, args.confidence, args.plan, args.calibration]
    )
