...
try:
    ...
    logger.error(
        "Flask or related packages not available. Install required packages with: pip install flask flask-wtf apscheduler"
    )
...
class NotificationManager:
    ...
    def get_notifications(
        ...
        filtered = [
            ...
            and (notification_type is None or n["type"] == notification_type)
        ]
...
class JobScheduler:
    ...
    def __init__(
        self, app=None, max_jobs: int = 5, poll_interval: int = 10, db_path: Optional[str] = None
    ):
        """
        ...
        self.scheduler = BackgroundScheduler(
            jobstores={"default": SQLAlchemyJobStore(url=job_db_path)}
        )
    ...
    def add_job(
        ...
        active_jobs = [j for j in self.jobs.values() if j["status"] in ["pending", "running"]]
        ...
        if scheduled_time:
            scheduler_job = self.scheduler.add_job(
                job_wrapper, "date", run_date=scheduled_time, id=job_id
            )
        else:
            scheduler_job = self.scheduler.add_job(
                job_wrapper, "date", run_date=datetime.datetime.now(), id=job_id
            )
...
class WebApp:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.app.config["UPLOAD_FOLDER"] = os.path.join(
            tempfile.gettempdir(), "simulation_validation_uploads"
        )
    ...
    def _load_configuration(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "theme": "light",
            "pagination_size": 20,
            "database": {"enabled": True, "db_path": "benchmark_db.duckdb"},
            "authentication": {
                "enabled": False,
                "users": {
            ...
            "notifications": {"enabled": True, "max_notifications": 50},
        }
    ...
    def _setup_routes(self):
            ...
            if "user_id" in session:
                try:
                    notifications = self.notification_manager.get_notifications(
                        user_id=session["user_id"], include_read=False
                    )
        ...
        def index():
            ...
            if "user_id" in session:
                try:
                    recent_jobs = self.job_scheduler.get_jobs(user_id=session["user_id"], limit=5)
                except Exception as e:
        ...
        def generate_report():
            ...
            if request.method == "POST":
                ...
                report_filename = (
                    f"validation_report_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
                )
                ...
                if self.db_available:
                    try:
                        ...
                        validation_results = self.db_integration.convert_to_validation_results(
                            db_results
                        )
                    except Exception as e:
                        logger.error(f"Error fetching validation results: {e}")
        ...
        def dashboard():
            ...
            dashboard_filename = (
                f"dashboard_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.html"
            )
        ...
        def validate():
            ...
            if request.method == "POST":
                ...
                simulation_metrics = {
                    "throughput_items_per_second": request.form.get(
                        "sim_throughput", 0, type=float
                    ),
                ...
                try:
                    validation_results = self.framework.validate(
                        [simulation_result], [hardware_result], protocol=protocol
                    )
                    ...
                    session["validation_id"] = (
                        validation_results[0].id if hasattr(validation_results[0], "id") else None
                    )
                    ...
                    if session.get("validation_id"):
                        return redirect(
                            url_for("validation_details", validation_id=session["validation_id"])
                        )
                    else:
                        return redirect(url_for("validation_results"))
        ...
        def calibrate():
            ...
            if request.method == "POST":
                ...
                if self.db_available:
                    try:
                        filters = {"hardware_id": hardware_id, "model_id": model_id}
            ...
            if request.method == "POST":
                ...
                if self.db_available:
                    try:
                        ...
                        # Convert to validation result objects
                        all_validation_results = self.db_integration.convert_to_validation_results(
                            db_results
                        )
                    except Exception as e:
                        logger.error(f"Error fetching validation results: {e}")
                ...
                if not historical_results:
                    flash(
                        f"No historical validation results found (older than {historical_days} days)",
                        "warning",
                    )
                    return redirect(url_for("drift_detection"))
                ...
                # Run drift detection
                try:
                    drift_results = self.framework.detect_drift(historical_results, new_results)
        ...
        def drift_results():
            ...
            try:
                ...
                visualization_path = self.framework.visualize_drift_detection(
                    drift_results, interactive=True, output_path=viz_path
                )

                # Get URL
                if visualization_path:
                    visualization_url = url_for(
                        "static_dashboard", filename=os.path.basename(visualization_path)
                    )
                else:
    ...
    def _setup_auth_routes(self):
        ...
        def login():
            ...
            return render_template(
                "login.html", title="Login", theme=self.config["theme"], form=form
            )
        ...
        def register():
            ...
            if form.validate_on_submit():
                ...
                if self._get_user_by_username(username):
                    ...
                    return render_template(
                        "register.html", title="Register", theme=self.config["theme"], form=form
                    )
        ...
        def profile():
            ...
            # Get user jobs
            jobs = self.job_scheduler.get_jobs(user_id=session["user_id"], limit=10)

            # Get user notifications
            notifications = self.notification_manager.get_notifications(
                user_id=session["user_id"], limit=10, include_read=True
            )
        ...
        def reset_password():
            ...
            return render_template(
                "reset_password.html", title="Reset Password", theme=self.config["theme"], form=form
            )
        ...
        def manage_users():
            ...
            return render_template(
                "manage_users.html", title="Manage Users", theme=self.config["theme"], users=users
            )
    ...
    def _setup_job_routes(self):
        ...
        def jobs():
            ...
            jobs = self.job_scheduler.get_jobs(user_id=user_id, job_type=job_type, status=status)
            ...
            if request.method == "POST":
                ...
                if "notifications_enabled" in request.form:
                    user_preferences["notifications_enabled"] = (
                        request.form["notifications_enabled"] == "on"
                    )
            ...
            if self.db_available:
                try:
                    ...
                    if last_ci_run and "run_id" in last_ci_run:
                        ...
                        validation_results = self.db_integration.get_validation_results_by_ci_run(
                            run_id
                        )
        ...
        def webhook_validation():
            ...
            if self.db_available:
                try:
                    ...
                    return jsonify(
                        {"status": "success", "message": "Validation result stored successfully"}
                    )
                except Exception as e:
