...
from data.duckdb.simulation_validation.visualization.validation_visualizer_db_connector import (
...
from data.duckdb.simulation_validation.visualization.validation_visualizer import (
...
class TestValidationVisualizerDBConnector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.connector = ValidationVisualizerDBConnector(
            db_integration=self.mock_db_integration, visualizer=self.mock_visualizer
        )
        ...
        self.sample_db_results = [
            {
                ...
                "hardware_details": json.dumps(
                    {"name": "NVIDIA RTX 3080", "compute_capability": "8.6"}
                ),
    ...
    def test_init(self):
        ...
        with patch(
            "duckdb_api.simulation_validation.visualization.validation_visualizer_db_connector.SimulationValidationDBIntegration"
        ) as mock_db_class:
            with patch(
                "duckdb_api.simulation_validation.visualization.validation_visualizer_db_connector.ValidationVisualizer"
            ) as mock_vis_class:
    ...
    def test_create_mape_comparison_chart_from_db(self):
        ...
        self.mock_db_integration.get_validation_results.assert_called_once_with(
            hardware_id="gpu_rtx3080", model_id="bert-base-uncased", start_date=None, end_date=None
        )
    ...
    def test_create_hardware_comparison_heatmap_from_db(self):
        ...
        # Set up mock return values
        self.mock_db_integration.get_validation_results.return_value = self.sample_db_results
        self.mock_visualizer.create_hardware_comparison_heatmap.return_value = (
            "/path/to/heatmap.html"
        )
        ...
        # Check that the database method was called with correct parameters
        self.mock_db_integration.get_validation_results.assert_called_once_with(
            model_id="bert-base-uncased", start_date=None, end_date=None
        )
    ...
    def test_create_drift_visualization_from_db(self):
        ...
        # Set up mock return values
        self.mock_db_integration.get_drift_detection_results.return_value = (
            self.sample_drift_results
        )
        self.mock_visualizer.create_drift_detection_visualization.return_value = (
            "/path/to/drift.html"
        )
    ...
    def test_create_calibration_improvement_chart_from_db(self):
        ...
        # Set up mock return values
        self.mock_db_integration.get_calibration_history.return_value = (
            self.sample_calibration_history
        )
        self.mock_visualizer.create_calibration_improvement_chart.return_value = (
            "/path/to/calibration.html"
        )
    ...
    def test_dashboard_integration(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "duckdb_api.simulation_validation.visualization.validation_visualizer_db_connector.requests"
                ) as mock_requests:
                    mock_db_instance = MagicMock()
                    mock_vis_instance = MagicMock()
                    mock_db_class.return_value = mock_db_instance
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "duckdb_api.simulation_validation.visualization.validation_visualizer_db_connector.time"
                    ) as mock_time:
                        mock_db_instance = MagicMock()
                        mock_vis_instance = MagicMock()
                        mock_db_class.return_value = mock_db_instance
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        connector.upload_visualization_to_dashboard = MagicMock(
                            return_value={
                                "status": "success",
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        connector.create_dashboard_panel_from_db = MagicMock(
                            return_value={
                                "status": "success",
                                "panel_id": "panel_12345",
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "duckdb_api.simulation_validation.visualization.validation_visualizer_db_connector.datetime"
                        ) as mock_datetime:
                            mock_db_instance = MagicMock()
                            mock_vis_instance = MagicMock()
                            mock_db_class.return_value = mock_db_instance
                            ...
                            self.assertEqual(
                                result["monitoring_config"]["hardware_type"], "gpu_rtx3080"
                            )
                            self.assertEqual(
                                result["monitoring_config"]["model_type"], "bert-base-uncased"
                            )
                            self.assertEqual(
                                result["monitoring_config"]["metrics"],
                                ["throughput_mape", "latency_mape"],
                            )
                            self.assertEqual(
                                result["monitoring_config"]["monitoring_interval"], 300
                            )
                            self.assertEqual(
                                result["monitoring_config"]["alert_thresholds"]["throughput_mape"],
                                10.0,
                            )
                            self.assertEqual(
                                result["monitoring_config"]["alert_thresholds"]["latency_mape"],
                                12.0,
                            )
    ...
    def test_create_simulation_vs_hardware_chart_from_db(self):
        ...
        # Set up mock return values
        self.mock_db_integration.get_simulation_vs_hardware_values.return_value = (
            self.sample_sim_vs_hw_data
        )
                    ...
                    with patch("matplotlib.pyplot.plot") as mock_plot:
                        with patch("matplotlib.pyplot.savefig") as mock_savefig:
                            with patch("base64.b64encode") as mock_b64encode:
                                ...
                                self.assertTrue(
                                    result.startswith('<img src="data:image/png;base64,')
                                )

    def test_create_metrics_over_time_chart_from_db(self):
        """Test creation of metrics over time chart from database."""
        # Set up mock return values
        self.mock_db_integration.get_validation_metrics_over_time.return_value = (
            self.sample_time_series_data
        )
    ...
    def test_visualize_calibration_effectiveness_from_db(self):
        ...
        # Set up mock return values
        self.mock_db_integration.analyze_calibration_effectiveness.return_value = (
            self.sample_calibration_analysis
        )
