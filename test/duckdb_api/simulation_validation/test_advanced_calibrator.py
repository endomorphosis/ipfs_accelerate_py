...
def test_basic_vs_advanced_calibration():
    ...
    for i in range(len(simulation_results)):
        ...
        for metric in [
            ...
            if metric in simulation_results[i].metrics and metric in hardware_results[i].metrics:
    ...
    for metric in [
        ...
        if metric in test_sim_result.metrics:
            ...
            logger.info(
                f"  Basic Calibration: {basic_val} (change: {(basic_val - original) / original * 100 if original and basic_val != 'N/A' else 'N/A'}%)"
            )
            logger.info(
                f"  Advanced Calibration: {advanced_val} (change: {(advanced_val - original) / original * 100 if original and advanced_val != 'N/A' else 'N/A'}%)"
            )
...
def test_multiple_calibration_methods():
    ...
    methods = ["linear_scaling", "regression", "ensemble", "incremental"]
    ...
    for method in methods:
        ...
        calibrator = AdvancedSimulationCalibrator(
            {"calibration_method": method, "learning_rate": 0.8}
        )
...
def test_hardware_specific_profiles():
    ...
    for hardware_type in hardware_types:
        ...
        for i in range(len(simulation_results)):
            ...
            for metric in [
                ...
                if (
                    metric in simulation_results[i].metrics
                    and metric in hardware_results[i].metrics
                ):
                    sim_value = simulation_results[i].metrics[metric]
                    hw_value = hardware_results[i].metrics[metric]
    ...
    profile_calibrator = AdvancedSimulationCalibrator(
        {"calibration_method": "ensemble", "use_hardware_profiles": True}
    )

    # Create advanced calibrator without profiles
    no_profile_calibrator = AdvancedSimulationCalibrator(
        {"calibration_method": "ensemble", "use_hardware_profiles": False}
    )
    ...
    for hardware_type in hardware_types:
        ...
        # Apply calibration with profiles
        with_profile_result = profile_calibrator.apply_calibration(
            test_sim_result, with_profile_params
        )

        # Apply calibration without profiles
        without_profile_result = no_profile_calibrator.apply_calibration(
            test_sim_result, without_profile_params
        )
        ...
        for metric in [
            ...
            if metric in test_sim_result.metrics:
                ...
                comparison[metric] = {
                    ...
                    "with_profile_change_pct": (with_profile - original) / original * 100
                    if isinstance(with_profile, (int, float))
                    else "N/A",
                    "without_profile_change_pct": (without_profile - original) / original * 100
                    if isinstance(without_profile, (int, float))
                    else "N/A",
                }
                ...
                logger.info(f"  Original: {original}")
                logger.info(
                    f"  With Profile: {with_profile} (change: {comparison[metric]['with_profile_change_pct'] if comparison[metric]['with_profile_change_pct'] != 'N/A' else 'N/A'}%)"
                )
                logger.info(
                    f"  Without Profile: {without_profile} (change: {comparison[metric]['without_profile_change_pct'] if comparison[metric]['without_profile_change_pct'] != 'N/A' else 'N/A'}%)"
                )
...
def test_incremental_learning():
    ...
    for i in range(3):
        ...
        for j in range(len(simulation_results)):
            ...
            for metric in [
                ...
                if (
                    metric in simulation_results[j].metrics
                    and metric in hardware_results[j].metrics
                ):
    ...
    incremental_calibrator = AdvancedSimulationCalibrator(
        {"calibration_method": "incremental", "learning_rate": 0.8}
    )
    ...
    for i, batch in enumerate(batches):
        ...
        test_sim_result = SimulationResult(
            ...
            metrics={
                "throughput_items_per_second": 100.0
                * (1.0 + (i * 0.1)),  # Introduce progressive bias
        ...
        incremental_result = incremental_calibrator.apply_calibration(
            test_sim_result, incremental_params
        )
        ...
        for metric in [
            ...
            if metric in test_sim_result.metrics:
                ...
                metrics_comparison[metric] = {
                    ...
                    "basic_change_pct": (basic_val - original) / original * 100
                    if isinstance(basic_val, (int, float))
                    else "N/A",
                    "incremental_change_pct": (incremental_val - original) / original * 100
                    if isinstance(incremental_val, (int, float))
                    else "N/A",
                }
                ...
                logger.info(f"  Original: {original}")
                logger.info(
                    f"  Basic: {basic_val} (change: {metrics_comparison[metric]['basic_change_pct'] if metrics_comparison[metric]['basic_change_pct'] != 'N/A' else 'N/A'}%)"
                )
                logger.info(
                    f"  Incremental: {incremental_val} (change: {metrics_comparison[metric]['incremental_change_pct'] if metrics_comparison[metric]['incremental_change_pct'] != 'N/A' else 'N/A'}%)"
                )

        # Store calibration history
        basic_history.append(
            {"batch": i + 1, "params": basic_params, "test_result": metrics_comparison}
        )

        incremental_history.append(
            {"batch": i + 1, "params": incremental_params, "test_result": metrics_comparison}
        )
    ...
    return {"basic_history": basic_history, "incremental_history": incremental_history}
