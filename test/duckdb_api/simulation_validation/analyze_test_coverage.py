...
class TestCoverageAnalyzer:
    ...
    def analyze_coverage(self, coverage_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not coverage_data:
            return {"status": "error", "message": "No coverage data available for analysis"}
        ...
        lowest_coverage = (
            modules_by_coverage[:5] if len(modules_by_coverage) > 5 else modules_by_coverage
        )
        ...
        for module in modules:
            ...
            if not component_found:
                ...
                component = "other"
                if component not in components:
                    components[component] = {"modules": [], "total_lines": 0, "covered_lines": 0}
    ...
    def generate_report(
        self, analysis: Dict[str, Any], format: str = "text", output_file: Optional[str] = None
    ) -> str:
        """
        ...
        if format == "json":
            ...
            lines.append(
                f"- **Lines Covered**: {analysis['lines_covered']} of {analysis['lines_total']}"
            )
            ...
            if total_modules > 0:
                lines.append(
                    f"| Excellent (90-100%) | {levels['excellent']} | {round(levels['excellent'] / total_modules * 100, 1)}% |"
                )
                lines.append(
                    f"| Good (75-90%) | {levels['good']} | {round(levels['good'] / total_modules * 100, 1)}% |"
                )
                lines.append(
                    f"| Moderate (50-75%) | {levels['moderate']} | {round(levels['moderate'] / total_modules * 100, 1)}% |"
                )
                lines.append(
                    f"| Low (25-50%) | {levels['low']} | {round(levels['low'] / total_modules * 100, 1)}% |"
                )
                lines.append(
                    f"| Poor (0-25%) | {levels['poor']} | {round(levels['poor'] / total_modules * 100, 1)}% |"
                )
            else:
            ...
            for component in sorted_components:
                lines.append(
                    f"| {component['name']} | {component['coverage_percent']}% | {component['lines_covered']} | {component['lines_total']} |"
                )
            ...
            if analysis["lowest_coverage_modules"]:
                ...
                for module in analysis["lowest_coverage_modules"]:
                    ...
                    lines.append(
                        f"| {name} | {module['coverage_percent']}% | {module['lines_covered']} | {module['lines_total']} |"
                    )
            ...
            overall_class = (
                "good"
                if analysis["overall_coverage"] >= 75
                else "moderate"
                if analysis["overall_coverage"] >= 50
                else "poor"
            )
            ...
            for component in sorted_components:
                lines.append(
                    f"  {component['name']}: {component['coverage_percent']}% ({component['lines_covered']}/{component['lines_total']} lines)"
                )
            lines.append("")
            ...
            if analysis["lowest_coverage_modules"]:
                ...
                for module in analysis["lowest_coverage_modules"]:
                    name = module["name"].split(".")[-1]  # Use just the filename without path
                    lines.append(
                        f"  {name}: {module['coverage_percent']}% ({module['lines_covered']}/{module['lines_total']} lines)"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze test coverage for Simulation Validation Framework"
    )
    parser.add_argument(
        "--coverage-file", type=str, required=True, help="Path to coverage.xml file"
    )
    parser.add_argument(
        "--output-format",
        type=str,
    ...
    report = analyzer.generate_report(
        analysis, format=args.output_format, output_file=args.output_file
    )
