...
class ValidationIssueDetector:
    ...
    def generate_report(
        self, issues: List[Dict[str, Any]], format: str = "json", output_file: Optional[str] = None
    ) -> str:
        """
        ...
        if format == "json":
            ...
            if severity_counts["high"] > 0:
                ...
                for i, issue in enumerate([i for i in issues if i.get("severity") == "high"]):
                    ...
                    if "metric" in issue:
                        html_lines.append(
                            f"        <p><strong>Metric</strong>: {issue['metric']}</p>"
                        )
            ...
            if severity_counts["low"] > 0:
                ...
                for issue in [i for i in issues if i.get("severity") == "low"]:
                    html_lines.append(
                        f"            <li><strong>{issue['issue_type']}</strong>: {issue['description']}"
                    )
                    if "recommendation" in issue:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Detect validation issues in simulation validation results"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="MAPE threshold for flagging issues (default: 0.1)",
    )
    parser.add_argument(
        "--output-format",
        type=str,
    ...
    parser.add_argument(
        "--output-file", type=str, help="Output file (default: validation_issues.<format>)"
    )
