...
class StatisticalValidator(SimulationValidator):
    ...
    def validate(
        self, simulation_result: SimulationResult, hardware_result: HardwareResult
    ) -> ValidationResult:
        """
        ...
        if simulation_result.model_id != hardware_result.model_id:
            logger.warning(
                f"Model ID mismatch: {simulation_result.model_id} vs {hardware_result.model_id}"
            )

        if simulation_result.hardware_id != hardware_result.hardware_id:
            logger.warning(
                f"Hardware ID mismatch: {simulation_result.hardware_id} vs {hardware_result.hardware_id}"
            )

        # Check that batch size and precision match
        if simulation_result.batch_size != hardware_result.batch_size:
            logger.warning(
                f"Batch size mismatch: {simulation_result.batch_size} vs {hardware_result.batch_size}"
            )

        if simulation_result.precision != hardware_result.precision:
            logger.warning(
                f"Precision mismatch: {simulation_result.precision} vs {hardware_result.precision}"
            )

        # Calculate metrics comparison
        metrics_comparison = self._calculate_error_metrics(
            simulation_result.metrics, hardware_result.metrics
        )
        ...
        additional_metrics = self._calculate_advanced_metrics(
            simulation_result, hardware_result, metrics_comparison
        )
    ...
    def validate_batch(
        self, simulation_results: List[SimulationResult], hardware_results: List[HardwareResult]
    ...
    def summarize_validation(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        for metric in all_metrics:
            metric_values = {"mape": [], "rmse": [], "absolute_error": [], "relative_error": []}
        ...
        all_error_metrics = {"mape": [], "rmse": [], "absolute_error": [], "relative_error": []}
        ...
        if self.config["advanced_metrics"]["enabled"]:
            ...
            for metric in advanced_metrics:
                ...
                for val_result in validation_results:
                    if val_result.additional_metrics and metric in val_result.additional_metrics:
    ...
    def calculate_confidence_score(
        self, validation_results: List[ValidationResult], hardware_id: str, model_id: str
    ) -> Dict[str, Any]:
        """
        ...
        filtered_results = [
            ...
            and result.hardware_result.model_id == model_id
        ]
        ...
        if len(filtered_results) >= 2:
            ...
            for metric in self.config["metrics_to_validate"]:
                ...
                for result in filtered_results:
                    if (
                        metric in result.metrics_comparison
                        and "mape" in result.metrics_comparison[metric]
                        and not np.isnan(result.metrics_comparison[metric]["mape"])
                    ):
        ...
        overall_confidence = sum(
            component * component_weights[name] for name, component in components.items()
        )
    ...
    def _calculate_error_metrics(
        self, simulation_metrics: Dict[str, float], hardware_metrics: Dict[str, float]
        ...
        for metric_name in metrics_to_validate:
            ...
            if "relative_error" in error_metrics:
                if hw_value != 0:
                    metrics_comparison[metric_name]["relative_error"] = abs(
                        sim_value - hw_value
                    ) / abs(hw_value)
                else:
            ...
            if "mape" in error_metrics:
                if hw_value != 0:
                    metrics_comparison[metric_name]["mape"] = (
                        abs(sim_value - hw_value) / abs(hw_value) * 100.0
                    )
                else:
                    # Handle division by zero
            ...
            if "percent_error" in error_metrics:
                if hw_value != 0:
                    metrics_comparison[metric_name]["percent_error"] = (
                        (sim_value - hw_value) / abs(hw_value) * 100.0
                    )
                else:
                    # Handle division by zero
            ...
            if "normalized_rmse" in error_metrics:
                ...
                if hw_value != 0:
                    metrics_comparison[metric_name]["normalized_rmse"] = abs(
                        sim_value - hw_value
                    ) / abs(hw_value)
                else:
    ...
    def _run_statistical_tests(
        ...
        for metric_name in metrics_to_validate:
            if (
                metric_name in simulation_metrics
                and metric_name in hardware_metrics
                and simulation_metrics[metric_name] is not None
                and hardware_metrics[metric_name] is not None
            ):
    ...
    def _calculate_advanced_metrics(
        ...
        if "precision_score" in self.config["advanced_metrics"]["metrics"]:
            ...
            if "mape_std_dev" in advanced_metrics:
                ...
                normalized_precision = max(
                    0.0, 1.0 - (std_dev / 50.0)
                )  # 50% std dev or more = 0 precision
        ...
        if "reliability_score" in self.config["advanced_metrics"]["metrics"]:
            ...
            if "bias_score" in advanced_metrics and "precision_score" in advanced_metrics:
                reliability = (
                    advanced_metrics["bias_score"] * 0.5 + advanced_metrics["precision_score"] * 0.5
                )
        ...
        if "fidelity_score" in self.config["advanced_metrics"]["metrics"]:
            ...
            if (
                "overall" in metrics_comparison
                and "pearson_correlation" in metrics_comparison["overall"]
            ):
                ...
                for metric in metrics_comparison.keys():
                    if (
                        metric != "overall"
                        and metric in simulation_result.metrics
                        and metric in hardware_result.metrics
                    ):
    ...
    def _group_results(
        self, results: List[Union[SimulationResult, HardwareResult]]
        ...
        for result in results:
            group_key = (
                f"{result.model_id}_{result.hardware_id}_{result.batch_size}_{result.precision}"
            )
    ...
    def _validate_result_groups(
        self, simulation_group: List[SimulationResult], hardware_group: List[HardwareResult]
        ...
        for metric in self.config["metrics_to_validate"]:
            ...
            if sim_values and hw_values:
                metrics_data[metric] = {"simulation": sim_values, "hardware": hw_values}
        ...
        for metric, data in metrics_data.items():
            ...
            if "rmse" in self.config["error_metrics"] and absolute_errors:
                metrics_comparison[metric]["rmse"] = np.sqrt(
                    np.mean(np.array(absolute_errors) ** 2)
                )
            ...
            # Run statistical tests if enabled
            if self.config["statistical_tests"]["enabled"]:
                self._add_group_statistical_tests(metrics_comparison[metric], sim_values, hw_values)

        # Generate additional metrics
        additional_metrics = self._calculate_group_advanced_metrics(
            simulation_group, hardware_group, metrics_comparison
        )
    ...
    def _add_group_statistical_tests(
        self, metrics_comparison: Dict[str, float], sim_values: List[float], hw_values: List[float]
    ) -> None:
        """
    ...
    def _calculate_group_advanced_metrics(
        ...
        for metric in self.config["metrics_to_validate"]:
            ...
            if sim_values and hw_values:
                distribution_metrics[metric] = self._calculate_distribution_metrics(
                    sim_values, hw_values
                )
        ...
        if "confidence_score" in self.config["advanced_metrics"]["metrics"]:
            ...
            confidence_components["sample_size"] = min(
                1.0, sample_size / 10.0
            )  # 10+ samples for full confidence
            ...
            for metric, comparison in metrics_comparison.items():
                ...
                if (
                    "pearson_correlation" in comparison
                    and "coefficient" in comparison["pearson_correlation"]
                ):
            ...
            if confidence_components:
                weights = {"mape": 0.4, "sample_size": 0.3, "statistical_tests": 0.3}
    ...
    def _calculate_distribution_metrics(
        self, simulation_values: List[float], hardware_values: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        metrics["mean_diff"] = {
            ...
            "percent": float(
                ...
                / abs(metrics["hardware"]["mean"])
                * 100.0
                if metrics["hardware"]["mean"] != 0
                else float("nan")
            ),
        }
        ...
        metrics["median_diff"] = {
            ...
            "percent": float(
                ...
                / abs(metrics["hardware"]["median"])
                * 100.0
                if metrics["hardware"]["median"] != 0
                else float("nan")
            ),
        }
        ...
        metrics["std_dev_diff"] = {
            ...
            "percent": float(
                abs(metrics["simulation"]["std_dev"] - metrics["hardware"]["std_dev"])
                / abs(metrics["hardware"]["std_dev"])
                * 100.0
                if metrics["hardware"]["std_dev"] != 0
                else float("nan")
            ),
        }
        ...
        metrics["range_diff"] = {
            ...
            "percent": float(
                abs(sim_range - hw_range) / abs(hw_range) * 100.0 if hw_range != 0 else float("nan")
            ),
        }
