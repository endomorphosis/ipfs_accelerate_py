...
class EnhancedStatisticalValidator(StatisticalValidator):
    ...
    def validate(
        ...
        enhanced_metrics = self._calculate_enhanced_metrics(
            simulation_result, hardware_result, validation_result.metrics_comparison
        )
        ...
        if self.config["bland_altman"]["enabled"]:
            ba_metrics = self._perform_bland_altman_analysis(simulation_result, hardware_result)
    ...
    def validate_batch(
        ...
        if (
            self.config["distribution_comparison"]["enabled"]
            and len(simulation_results) > 1
            and len(hardware_results) > 1
        ):
            ...
            for i, result in enumerate(validation_results):
                if hasattr(result.simulation_result, "model_id") and hasattr(
                    result.simulation_result, "hardware_id"
                ):
                    ...
                    if (
                        group_key in grouped_sim_results
                        and group_key in grouped_hw_results
                        and len(grouped_sim_results[group_key]) > 1
                        and len(grouped_hw_results[group_key]) > 1
                    ):
                        distribution_comparison = self._compare_distributions(
                            grouped_sim_results[group_key], grouped_hw_results[group_key]
                        )
                        ...
                        if distribution_comparison:
                            ...
                            result.additional_metrics["distribution_comparison"] = (
                                distribution_comparison
                            )
    ...
    def summarize_validation(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        for val_result in validation_results:
            ...
            if "confidence_intervals" in val_result.additional_metrics:
                for metric, ci_data in val_result.additional_metrics[
                    "confidence_intervals"
                ].items():
                    ...
                    if "lower_bound" in ci_data:
                        enhanced_summary["confidence_intervals"][metric]["lower_bounds"].append(
                            ci_data["lower_bound"]
                        )

                    if "upper_bound" in ci_data:
                        enhanced_summary["confidence_intervals"][metric]["upper_bounds"].append(
                            ci_data["upper_bound"]
                        )
            ...
            if "bland_altman" in val_result.additional_metrics:
                for metric, ba_data in val_result.additional_metrics["bland_altman"].items():
                    ...
                    if "lower_loa" in ba_data:
                        enhanced_summary["bland_altman"][metric]["lower_loa"].append(
                            ba_data["lower_loa"]
                        )

                    if "upper_loa" in ba_data:
                        enhanced_summary["bland_altman"][metric]["upper_loa"].append(
                            ba_data["upper_loa"]
                        )

                    if (
                        "proportional_bias" in ba_data
                        and "is_significant" in ba_data["proportional_bias"]
                    ):
            ...
            if "power_analysis" in val_result.additional_metrics:
                for effect_size, power_data in val_result.additional_metrics[
                    "power_analysis"
                ].items():
        ...
        for effect_size, values in enhanced_summary["power_analysis"].items():
            if values:
                enhanced_summary["power_analysis"][effect_size] = {
                    ...
                    "std_dev": np.std(values),
                    "count": len(values),
                    "proportion_sufficient": np.mean(
                        [
                            1 if v >= self.config["power_analysis"]["power_threshold"] else 0
                            for v in values
                        ]
                    ),
                }
    ...
    def _calculate_enhanced_metrics(
        ...
        for metric_name in metrics_to_validate:
            ...
            if (
                metric_name not in simulation_result.metrics
                or metric_name not in hardware_result.metrics
            ):
                continue
    ...
    def _compare_distributions(
        ...
        for metric in self.config["metrics_to_validate"]:
            ...
            if (
                len(sim_values) >= 3 and len(hw_values) >= 3
            ):  # Most tests need at least 3 data points
                metrics_data[metric] = {
        ...
        for metric, data in metrics_data.items():
            ...
            if "levene" in distribution_tests:
                try:
                    ...
                    distribution_comparison[metric]["tests"]["levene"] = {
                        ...
                        "interpretation": (
                            "Equal variances" if levene_p > alpha else "Unequal variances"
                        ),
                    }
                except Exception as e:
            ...
            if "chi_squared" in distribution_tests:
                try:
                    ...
                    distribution_comparison[metric]["tests"]["chi_squared"] = {
                        ...
                        "interpretation": (
                            "Same distribution" if chi2_p > alpha else "Different distributions"
                        ),
                    }
                except Exception as e:
            ...
            distribution_comparison[metric]["differences"] = {
                "mean": {
                    ...
                    "percent": float(abs(sim_mean - hw_mean) / hw_mean * 100.0)
                    if hw_mean != 0
                    else float("nan"),
                },
                "std_dev": {
                    "absolute": float(abs(sim_std - hw_std)),
                    "percent": float(abs(sim_std - hw_std) / hw_std * 100.0)
                    if hw_std != 0
                    else float("nan"),
                },
                "cv": {
                    "absolute": float(abs(sim_cv - hw_cv))
                    if not np.isnan(sim_cv) and not np.isnan(hw_cv)
                    else float("nan"),
                    "percent": float(abs(sim_cv - hw_cv) / hw_cv * 100.0)
                    if hw_cv != 0 and not np.isnan(sim_cv) and not np.isnan(hw_cv)
                    else float("nan"),
                },
                "median": {
                    "absolute": float(abs(sim_percentiles[1] - hw_percentiles[1])),
                    "percent": float(
                        abs(sim_percentiles[1] - hw_percentiles[1]) / hw_percentiles[1] * 100.0
                    )
                    if hw_percentiles[1] != 0
                    else float("nan"),
                },
                "iqr": {
                    "absolute": float(abs(sim_iqr - hw_iqr)),
                    "percent": float(abs(sim_iqr - hw_iqr) / hw_iqr * 100.0)
                    if hw_iqr != 0
                    else float("nan"),
                },
            }
        ...
        for effect_size in effect_sizes:
            ...
            if not is_sufficient:
                ...
                while True:
                    ...
                    power_required = stats.nct.sf(
                        t_crit_required, df_required, nc_required
                    ) + stats.nct.cdf(-t_crit_required, df_required, nc_required)

                    if (
                        power_required >= self.config["power_analysis"]["power_threshold"]
                        or required_n > 1000
                    ):
                        break
...
def get_enhanced_statistical_validator_instance(
