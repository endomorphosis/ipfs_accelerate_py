...
class TestQueryCache(unittest.TestCase):
    ...
    def test_cache_max_size(self):
        ...
        self.assertIsNone(
            self.cache.get("SELECT * FROM test_table WHERE id = 0"),
        ...
        for i in range(1, 6):
            query = f"SELECT * FROM test_table WHERE id = {i}"
            self.assertIsNotNone(self.cache.get(query), f"Entry {i} should still be in cache")
    ...
    def test_cache_invalidation(self):
        ...
        self.assertIsNone(
            self.cache.get("SELECT * FROM table1"), "table1 entry should have been invalidated"
        )
        self.assertIsNotNone(
            self.cache.get("SELECT * FROM table2"), "table2 entry should still be in cache"
        )
        ...
        self.assertIsNone(
            self.cache.get("SELECT * FROM table2"), "Cache should be empty after clear"
        )
...
class TestDatabaseOptimization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.backup_manager = DatabaseBackupManager(
            self.db_integration, backup_dir=str(self.backup_dir)
        )
    ...
    def test_batch_operations(self):
        ...
        for i in range(10):
            ...
            record = {
                ...
                "metrics": json.dumps(
                    {"throughput_items_per_second": 100 + i, "average_latency_ms": 10 - i * 0.1}
                ),
        ...
        self.assertEqual(
            count, len(test_records), f"Should have inserted {len(test_records)} records"
        )
        ...
        self.assertEqual(
            result[0], len(test_records), "Database should contain the batch-inserted records"
        )
        ...
        self.assertEqual(
            update_count, len(update_records), f"Should have updated {len(update_records)} records"
        )

        # Verify the updates
        for record in update_records:
            result = conn.execute(
                f"SELECT precision FROM simulation_results WHERE id = ?", [record["id"]]
            ).fetchone()
            self.assertEqual(
                result[0],
                record["precision"],
        ...
        delete_count = self.batch_operations.batch_delete(
            table_name="simulation_results", id_column="id", id_values=delete_ids
        )

        self.assertEqual(
            delete_count, len(delete_ids), f"Should have deleted {len(delete_ids)} records"
        )

        # Verify the deletes
        for id_val in delete_ids:
            result = conn.execute(
                f"SELECT COUNT(*) FROM simulation_results WHERE id = ?", [id_val]
            ).fetchone()
    ...
    def test_database_maintenance(self):
        ...
        self.assertNotIn(
            "error", monitor_result, "Performance monitoring should complete without errors"
        )
        self.assertIn(
            "database_size_bytes", monitor_result, "Performance report should include database size"
        )
        self.assertIn(
            "table_counts", monitor_result, "Performance report should include table counts"
        )
    ...
    def test_database_backup(self):
        ...
        schedule_result = self.backup_manager.schedule_backup(
            interval_hours=24, keep_backups=5, compress=True
        )
        self.assertTrue(schedule_result["success"], "Scheduling backups should succeed")
        self.assertTrue(
            os.path.exists(schedule_result["script_path"]), "Backup script should exist"
        )
        self.assertTrue(
            os.path.exists(schedule_result["readme_path"]), "Backup README should exist"
        )
    ...
    def test_optimized_db_integration(self):
        ...
        self.assertEqual(
            first_result, third_result, "Result after invalidation should match original"
        )
        logger.info(
            f"First query: {first_time:.6f}s, Third query (after invalidation): {third_time:.6f}s"
        )
        ...
        self.assertIn(
            "database_size_bytes", stats, "Performance stats should include database size"
        )
