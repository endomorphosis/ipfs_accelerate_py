...
class SimulationValidationDBIntegration:
    ...
    def store_validation_result(self, result: ValidationResult) -> str:
        ...
        try:
            ...
            record = SimulationValidationSchema.validation_result_to_db_dict(result, sim_id, hw_id)
    ...
    def get_current_simulation_parameters(
        self, hardware_type: str, model_type: str
        ...
        try:
            ...
            if not row:
                logger.warning(
                    f"No current simulation parameters found for {hardware_type}/{model_type}"
                )
                return None
    ...
    def get_model_hardware_accuracy(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved accuracy metrics for {len(accuracy_metrics)} model-hardware combinations"
            )
    ...
    def analyze_calibration_effectiveness(
        ...
        try:
            ...
            if overall_stats["mean_relative_improvement"]:
                overall_stats["mean_relative_improvement"] = np.mean(
                    overall_stats["mean_relative_improvement"]
                )
            else:
        ...
        try:
            ...
            if query_type == "sim_vs_hw":
                ...
                data = self.get_simulation_vs_hardware_values(
                    model_id=model_id, hardware_id=hardware_type, metric=metric, limit=limit
                )
                result["data"] = data
...
def get_db_integration_instance(
    db_path: str = "./benchmark_db.duckdb", debug: bool = False
