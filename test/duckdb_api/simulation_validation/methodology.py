...
class ValidationMethodology:
    ...
    def get_hardware_validation_requirements(self, hardware_id: str) -> Dict[str, Any]:
        ...
        requirements = self.config["hardware_validation_requirements"].get(
            hardware_id, self.config["hardware_validation_requirements"]["default"]
        )
        return requirements
    ...
    def get_model_validation_requirements(self, model_id: str) -> Dict[str, Any]:
        ...
        requirements = self.config["model_validation_requirements"].get(
            model_id, self.config["model_validation_requirements"]["default"]
        )
        return requirements
    ...
    def validate_requirements(
        ...
        combined_requirements = {
            "required_metrics": list(
                set(
                    hardware_requirements["required_metrics"]
                    + model_requirements["required_metrics"]
                )
            ),
            "batch_size_validation": hardware_requirements["batch_size_validation"]
            or model_requirements["batch_size_validation"],
            "precision_validation": hardware_requirements["precision_validation"]
            or model_requirements["precision_validation"],
        }
        ...
        return {"passes_requirements": len(issues) == 0, "issues": issues}
    ...
    def calculate_confidence_score(
        ...
        timestamps = [
            datetime.datetime.fromisoformat(result.validation_timestamp)
    ...
    def evaluate_validation_result(self, validation_result: ValidationResult) -> Dict[str, Any]:
        ...
        for metric in self.config["primary_metrics"]:
            if (
                metric in validation_result.metrics_comparison
                and "mape" in validation_result.metrics_comparison[metric]
            ):
                ...
                evaluation["metrics"][metric] = {"mape": mape, "status": status}
        ...
        if mape_values:
            ...
            evaluation["overall"] = {
                ...
                "calibration_recommended": overall_mape
                > self.config["calibration_integration"]["auto_calibrate_threshold"],
            }
        else:
            evaluation["overall"] = {
                "mape": float("nan"),
    ...
    def aggregate_validation_results(
        self, validation_results: List[ValidationResult], grouping: str = "hardware_model"
        ...
        for val_result in validation_results:
            ...
            if key not in aggregated:
                aggregated[key] = {"metrics": {}, "overall": {"mape_values": [], "count": 0}}
            ...
            for metric in self.config["primary_metrics"]:
                if (
                    ...
                    if metric not in aggregated[key]["metrics"]:
                        aggregated[key]["metrics"][metric] = {"mape_values": [], "count": 0}
        ...
        for key, group in aggregated.items():
            ...
            if group["overall"]["mape_values"]:
                ...
                if group["overall"]["mean_mape"] <= self.config["accuracy_thresholds"]["excellent"]:
                    ...
                    group["overall"]["mean_mape"]
                    <= self.config["accuracy_thresholds"]["acceptable"]
                ):
    ...
    def check_calibration_needed(
        ...
        """
        if not validation_results:
            return {"calibration_recommended": False, "reason": "No validation results available"}
        ...
        if (
            ...
            < self.config["calibration_integration"]["min_samples_before_calibration"]
        ):
            return {
                "calibration_recommended": False,
        ...
        if all_mape_values:
            ...
            calibration_needed = (
                overall_mape > self.config["calibration_integration"]["auto_calibrate_threshold"]
            )
            ...
            return {"calibration_recommended": False, "reason": "No MAPE values available"}
    ...
    def check_drift_detection_needed(
        ...
        if (
            len(filtered_results)
            < self.config["drift_detection_integration"]["min_samples_before_drift_check"]
        ):
            return {
                "drift_detection_recommended": False,
    ...
    def generate_validation_report(
        ...
        if model_id:
            filtered_results = [
                result for result in filtered_results if result.hardware_result.model_id == model_id
            ]
        ...
        if report_format == "json":
            ...
            report.append(
                "| Group | Validation Count | Overall MAPE | Status | Calibration Recommended |"
            )
            report.append(
                "|-------|-----------------|-------------|--------|-------------------------|"
            )
            ...
            for key, group in aggregated.items():
                ...
                report.append(
                    "| Metric | Mean MAPE | Median MAPE | Min MAPE | Max MAPE | Std Dev | Status |"
                )
                report.append(
                    "|--------|-----------|-------------|---------|---------|---------|--------|"
                )
                ...
                if "mean_mape" in overall:
                    ...
                    report.append(
                        f"- **Calibration Recommended:** {'Yes' if overall.get('calibration_recommended', False) else 'No'}"
                    )

                    if (
                        "mean_mape" in overall
                        and overall["mean_mape"]
                        > self.config["calibration_integration"]["auto_calibrate_threshold"]
                    ):
                        report.append(
