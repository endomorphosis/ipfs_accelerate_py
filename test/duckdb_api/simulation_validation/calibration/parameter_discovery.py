...
class AutomaticParameterDiscovery:
    ...
    def discover_parameters(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        """
        ...
        if len(analysis_data) < self.config["min_samples_for_analysis"]:
            logger.warning(
                f"Not enough samples for analysis (need at least {self.config['min_samples_for_analysis']})"
            )
            return {}
    ...
    def analyze_parameter_sensitivity(
        self, validation_results: List[ValidationResult], parameter_name: str
    ) -> Dict[str, Any]:
        """
    ...
    def _prepare_analysis_data(self, validation_results: List[ValidationResult]) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.error(
                "pandas is required for parameter discovery. Please install it with 'pip install pandas'."
            )
            return pd.DataFrame()
        ...
        for val_result in validation_results:
            ...
            for metric in self.config["metrics_to_analyze"]:
                ...
                if (
                    metric not in sim_result.metrics
                    or metric not in hw_result.metrics
                    or sim_result.metrics[metric] is None
                    or hw_result.metrics[metric] is None
                    or hw_result.metrics[metric] == 0
                ):
                    continue
    ...
    def _discover_significant_parameters(self, data: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        feature_cols = [
            ...
            if not any(col.startswith(prefix) for prefix in metric_prefixes)
        ]
    ...
    def _analyze_parameter_sensitivity(
        ...
        for metric, parameters in self.discovered_parameters.items():
            ...
            for parameter in parameters:
                ...
                if parameter not in data.columns or not pd.api.types.is_numeric_dtype(
                    data[parameter]
                ):
                    continue
    ...
    def _analyze_single_parameter_sensitivity(
        self, data: pd.DataFrame, parameter: str, error_col: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for metric, error_col in metrics_to_analyze:
            ...
            try:
                ...
                bin_stats = (
                    data.groupby("parameter_bin")[error_col]
                ...
                if self.config["precision_analysis"]:
                    ...
                    if precision_cols:
                        ...
                        for col in precision_cols:
                            precision_value = col.split("_")[
                                1
                            ]  # Extract precision value from column name
                            precision_data = data[data[col] == 1]  # Filter data for this precision
                            if len(precision_data) >= self.config["min_samples_for_analysis"]:
                                precision_correlation = precision_data[parameter].corr(
                                    precision_data[error_col]
                                )
    ...
    def _analyze_parameter_by_factor(
        self, data: pd.DataFrame, parameter: str, error_col: str, factor_col: str
    ) -> Dict[str, float]:
        """
        ...
        for metric in self.config["metrics_to_analyze"]:
            ...
            if method == "correlation":
                ...
                try:
                    ...
                    logger.warning(
                        "scikit-learn is required for permutation importance. Falling back to correlation."
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "scikit-learn is required for mutual information. Falling back to correlation."
                    )
                    # Fallback to correlation
                    scores = {}
                    for param in numeric_params:
                ...
                try:
                    ...
                    mi_scores = mutual_info_regression(
                        X, y, random_state=self.config["random_state"]
                    )
            ...
            for param, sensitivity_data in sensitivity.items():
                ...
                if batch_sensitivity:
                    ...
                    if batch_values:
                        ...
                        if all(
                            batch_sensitivity[str(batch_values[i])]
                            <= batch_sensitivity[str(batch_values[i + 1])]
        ...
        for metric, importance in self.parameter_importance.items():
            ...
            for param, score in top_params:
                ...
                if correlation is not None:
                    if correlation > 0:
                        param_recommendations.append(
                            f"Consider reducing {param} to improve {metric}"
                        )
                    else:
                        param_recommendations.append(
                            f"Consider increasing {param} to improve {metric}"
                        )
                ...
                if batch_sensitivity:
                    ...
                    param_recommendations.append(
                        f"Focus on calibrating for batch size {highest_batch[0]}"
                    )
                ...
                if precision_sensitivity:
                    ...
                    highest_precision = max(
                        precision_sensitivity.items(), key=lambda x: abs(float(x[1]))
                    )
                    param_recommendations.append(
                        f"Focus on calibrating for {highest_precision[0]} precision"
                    )
    ...
    def _generate_key_findings(self) -> List[str]:
        ...
        for metric, importance in self.parameter_importance.items():
            ...
            if top_params:
                ...
                if score > 0.25:
                    findings.append(
                        f"For {metric}, the most important parameter is {param} with {score:.2f} importance score"
                    )
                ...
                if batch_sensitivity:
                    ...
                    if len(batch_values) >= 2:
                        ...
                        if max_sens > min_sens * 2:  # At least double the sensitivity
                            batch_size_findings.append(
                                f"{param} sensitivity to {metric} varies significantly with batch size"
                            )
            ...
            for param, sensitivity_data in sensitivity.items():
                ...
                if precision_sensitivity:
                    ...
                    if len(precision_sensitivity) >= 2:
                        ...
                        if max_sens > min_sens * 2:  # At least double the sensitivity
                            precision_findings.append(
                                f"{param} sensitivity to {metric} varies significantly with precision type"
                            )
    ...
    def _identify_optimization_opportunities(self) -> List[Dict[str, Any]]:
        ...
        for metric, sensitivity in self.parameter_sensitivity.items():
            ...
            for param, sensitivity_data in sensitivity.items():
                ...
                if batch_sensitivity:
                    # Find batch sizes with highest sensitivity
                    high_sensitivity_batches = [
                        (k, v)
                        for k, v in batch_sensitivity.items()
        ...
        if batch_size_opportunities:
            ...
            opportunities.extend(
                batch_size_opportunities[:3]
            )  # Add up to 3 batch size opportunities
        ...
        for metric, sensitivity in self.parameter_sensitivity.items():
            ...
            for param, sensitivity_data in sensitivity.items():
                ...
                if precision_sensitivity:
                    ...
                    high_sensitivity_precisions = [
                        (k, v)
                        for k, v in precision_sensitivity.items()
                        if abs(v) > self.config["sensitivity_threshold"]
                    ]

                    if high_sensitivity_precisions:
                        # Sort by sensitivity
                        high_sensitivity_precisions.sort(
    ...
    def _identify_parameter_relationships(self) -> List[Dict[str, Any]]:
        ...
        numeric_params = [
            p
            for p in all_params
            if p in analysis_data.columns and pd.api.types.is_numeric_dtype(analysis_data[p])
        ]
        ...
        if len(self.parameter_importance) >= 2:
            for metric1, metric2 in combinations(self.parameter_importance.keys(), 2):
                ...
                top_params1 = set(
                    sorted(
                        self.parameter_importance[metric1].items(), key=lambda x: x[1], reverse=True
                    )[:3]
                )
                top_params2 = set(
                    sorted(
                        self.parameter_importance[metric2].items(), key=lambda x: x[1], reverse=True
                    )[:3]
                )

                # Find parameters unique to each metric
                unique_to_metric1 = {
                    p[0] for p in top_params1 if p[0] not in {p[0] for p in top_params2}
                }
                unique_to_metric2 = {
                    p[0] for p in top_params2 if p[0] not in {p[0] for p in top_params1}
                }
