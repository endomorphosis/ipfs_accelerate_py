    ...
    def calibrate(
        ...
        for (hardware_id, model_id), results in validation_by_hardware_model.items():
            ...
            for metric, factor in correction_factors.items():
                ...
                logger.info(
                    f"Updated correction factor for {hardware_id}/{model_id}/{metric}: {current_factor:.4f} -> {updated_factor:.4f}"
                )
            ...
            if calibration_method == "linear_scaling":
                ...
                if isinstance(factor, (list, tuple)):
                    ...
                    if len(factor) == 2:
                        ...
                        logger.warning(
                            f"Unexpected list format for factor: {factor}, using first element"
                        )
                    ...
                    if "base_factor" in factor:
                        ...
                        calibrated_metrics[metric] = (
                            calibrated_metrics[metric] * factor["base_factor"]
                        )
                    elif (
                ...
                if isinstance(factor, (list, tuple, dict)):
                    logger.warning(
                        f"Complex factor not supported for additive_adjustment: {factor}, using linear_scaling instead"
                    )
                ...
                if isinstance(factor, (list, tuple)) and len(factor) == 2:
                    ...
                    if "ensemble_factors" in factor and "regression" in factor["ensemble_factors"]:
                        ...
                        if (
                            ...
                            calibrated_metrics[metric] = (
                                calibrated_metrics[metric] * slope + intercept
                            )
                        else:
                            logger.warning(
                                f"Invalid regression factor in ensemble: {regression_factor}, using 1.0"
                            )
                            calibrated_metrics[metric] = calibrated_metrics[metric] * 1.0
                    else:
                        logger.warning(
                            f"Unsupported dict format for regression: {factor}, using 1.0"
                        )
                        calibrated_metrics[metric] = calibrated_metrics[metric] * 1.0
                else:
                    logger.warning(
                        f"Invalid regression factor: {factor}, using as multiplicative factor"
                    )
                    calibrated_metrics[metric] = calibrated_metrics[metric] * factor
        ...
        for metric in self.config["metrics_to_calibrate"]:
            ...
            if self.config["calibration_method"] == "linear_scaling":
                ...
                try:
                    ...
                    logger.warning(
                        "sklearn not available for regression, falling back to linear scaling"
                    )
                    ratios = [hw / sim for hw, sim in zip(hw_values, sim_values)]
                    correction_factor = np.mean(ratios)
            else:
