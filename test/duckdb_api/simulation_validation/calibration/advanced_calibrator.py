...
class AdvancedSimulationCalibrator(SimulationCalibrator):
    ...
    def calibrate(
        self, validation_results: List[ValidationResult], simulation_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        updated_parameters["calibration_history"] = updated_parameters["calibration_history"][
            -self.config["calibration_history_size"] + 1 :
        ] + [calibration_event]
        ...
        for (hardware_id, model_id), results in validation_by_hardware_model.items():
            if len(results) < self.config["min_samples_per_hardware"]:
                logger.info(
                    f"Skipping calibration for {hardware_id}/{model_id}: insufficient samples ({len(results)})"
                )
                continue
            ...
            for metric, factor in correction_factors.items():
                current_factor = updated_parameters["correction_factors"][hardware_id][
                    model_id
                ].get(metric, 1.0)
                ...
                if isinstance(factor, (list, tuple)) and isinstance(current_factor, (list, tuple)):
                    ...
                    for k, v in factor.items():
                        if k in current_factor:
                            if isinstance(current_factor[k], dict) or isinstance(v, dict):
                                ...
                                try:
                                    updated_factor[k] = (
                                        float(current_factor[k]) * (1 - lr) + float(v) * lr
                                    )
                    ...
                    if isinstance(factor, (list, tuple)):
                        if isinstance(current_factor, (list, tuple)) and len(current_factor) == len(
                            factor
                        ):
                            updated_factor = [
                                curr * (1 - lr) + new * lr
                ...
                if not isinstance(updated_factor, (dict, list, tuple)):
                    updated_factor = max(
                        ...
                        min(
                            self.config["parameter_constraints"]["max_scale_factor"], updated_factor
                        ),
                    )

                updated_parameters["correction_factors"][hardware_id][model_id][metric] = (
                    updated_factor
                )

                logger.info(
                    f"Updated correction factor for {hardware_id}/{model_id}/{metric}: {current_factor} -> {updated_factor}"
                )
    ...
    def evaluate_calibration(
        self, before_calibration: List[ValidationResult], after_calibration: List[ValidationResult]
    ) -> Dict[str, Any]:
        """
        ...
        before_stats = self._calculate_enhanced_error_statistics(
            before_calibration, metrics_to_evaluate
        )

        # Calculate statistics after calibration
        after_stats = self._calculate_enhanced_error_statistics(
            after_calibration, metrics_to_evaluate
        )
        ...
        if overall_before_mape > 0:
            overall_mape_relative_improvement = (
                (overall_before_mape - overall_after_mape) / overall_before_mape * 100.0
            )
        else:
            overall_mape_relative_improvement = 0.0

        if overall_before_rmse > 0:
            overall_rmse_relative_improvement = (
                (overall_before_rmse - overall_after_rmse) / overall_before_rmse * 100.0
            )
        else:
    ...
    def apply_calibration(
        self, simulation_result: SimulationResult, simulation_parameters: Dict[str, Any]
    ) -> SimulationResult:
        """
        ...
        calibration_method = simulation_parameters.get(
            "calibration_method", self.config["calibration_method"]
        )
            ...
            if hardware_specific_profile and metric in profile.get("metric_adjustments", {}):
                ...
                if "batch_size_factors" in metric_profile:
                    ...
                    if str(batch_size) in batch_factors:
                        ...
                        logger.info(
                            f"Applying batch-specific factor for {hardware_id}/{metric}/batch={batch_size}: {batch_factor}"
                        )
                        ...
                        if isinstance(factor, (list, tuple)):
                            ...
                            if "base_factor" in factor:
                                ...
                                "ensemble_factors" in factor
                                and "linear_scaling" in factor["ensemble_factors"]
                            ):
                                factor["ensemble_factors"]["linear_scaling"] = (
                                    factor["ensemble_factors"]["linear_scaling"] * batch_factor
                                )
                            else:
                                # Cannot apply batch factor to dictionary, skip adjustment
                                logger.warning(
                                    f"Cannot apply batch factor to dictionary factor: {factor}"
                                )
                        else:
                ...
                if "precision_factors" in metric_profile:
                    ...
                    if precision in precision_factors:
                        ...
                        logger.info(
                            f"Applying precision-specific factor for {hardware_id}/{metric}/precision={precision}: {precision_factor}"
                        )
                        ...
                        if isinstance(factor, (list, tuple)):
                            ...
                            if "base_factor" in factor:
                                ...
                                factor["ensemble_factors"]["linear_scaling"] = (
                                    factor["ensemble_factors"]["linear_scaling"] * precision_factor
                                )
                            else:
                                # Cannot apply precision factor to dictionary, skip adjustment
                                logger.warning(
                                    f"Cannot apply precision factor to dictionary factor: {factor}"
                                )
                        else:
            ...
            if calibration_method == "ensemble":
                ...
                if isinstance(factor, dict) and "ensemble_factors" in factor:
                            ...
                            if method == "linear_scaling":
                                ...
                                if isinstance(method_factor, (list, tuple)):
                                    if len(method_factor) == 2:
                                        ...
                                        method_value = calibrated_metrics[metric] * float(
                                            slope
                                        ) + float(intercept)
                                    else:
                                        # Use the first element as a multiplier
                                        logger.warning(
                                            f"Unexpected list length for linear_scaling: {method_factor}"
                                        )
                                        method_value = (
                                            calibrated_metrics[metric] * float(method_factor[0])
                                            if method_factor
                                            else calibrated_metrics[metric]
                                        )
                                elif isinstance(method_factor, dict):
                                    # Try to extract a usable value
                                    if "base_factor" in method_factor:
                                        method_value = calibrated_metrics[metric] * float(
                                            method_factor["base_factor"]
                                        )
                                    else:
                                        logger.warning(
                                            f"Unsupported dict format for linear_scaling: {method_factor}"
                                        )
                                ...
                                if (
                                    isinstance(method_factor, (list, tuple))
                                    and len(method_factor) == 2
                                ):
                                    slope, intercept = method_factor
                                    method_value = calibrated_metrics[metric] * float(
                                        slope
                                    ...
                                    logger.warning(
                                        f"Unexpected list format for {method}: {method_factor}"
                                    )
                                    method_value = (
                                        calibrated_metrics[metric] * float(method_factor[0])
                                        if method_factor
                                    ...
                                    logger.warning(
                                        f"Unsupported dict format for {method}: {method_factor}"
                                    )
                                    method_value = calibrated_metrics[metric]
                                else:
                                    # Assume it's a numeric value
                    ...
                    if weight_sum > 0:
                        ...
                        logger.warning(
                            f"Zero weight sum for ensemble methods on {metric}, using original value"
                        )
                else:
                    ...
                    if isinstance(factor, (list, tuple)):
                        if len(factor) == 2:
                            ...
                            calibrated_metrics[metric] = calibrated_metrics[metric] * float(
                                slope
                            ) + float(intercept)
                        elif len(factor) > 0:
                        ...
                        if "base_factor" in factor:
                            ...
                            if isinstance(linear_factor, (list, tuple)):
                                if len(linear_factor) == 2:
                                    ...
                                    logger.warning(
                                        f"Empty linear factor list, using original value"
                                    )
                            else:
                                calibrated_metrics[metric] = calibrated_metrics[metric] * float(
                                    linear_factor
                            ...
                            logger.warning(
                                f"Unsupported dict factor format: {factor}, using original value"
                            )
                    else:
                        # Assume it's a numeric value
                ...
                if isinstance(factor, (list, tuple)) and len(factor) == 2:
                    slope, intercept = factor
                    calibrated_metrics[metric] = calibrated_metrics[metric] * float(slope) + float(
                        intercept
                    )
                elif isinstance(factor, (list, tuple)) and len(factor) > 0:
                    # Use first element as a scalar if not the right format
                    logger.warning(
                        f"Unexpected list length for {calibration_method}: {factor}, using first element"
                    )
                    ...
                    if "base_factor" in factor:
                        ...
                        if isinstance(linear_factor, (list, tuple)):
                            if len(linear_factor) == 2:
                                ...
                                logger.warning(
                                    f"Empty linear factor list in ensemble, using original value"
                                )
                        else:
                        ...
                        logger.warning(
                            f"Unsupported dict format for {calibration_method}: {factor}, using original value"
                        )
                else:
                    try:
                        ...
                        logger.warning(
                            f"Could not convert {factor} to float for {calibration_method}, using original value"
                        )
                ...
                if (
                    ...
                    and "base_factor" in factor
                    and "trend_factor" in factor
                ):
                    ...
                    if "linear_scaling" in factor["ensemble_factors"]:
                        ...
                        logger.warning(
                            f"No usable factor in ensemble for incremental method: {factor}, using original value"
                        )
                elif isinstance(factor, (list, tuple)) and len(factor) > 0:
                    # Use first element as a scalar if not a dict
                    logger.warning(
                        f"Expected dict for incremental method, got: {factor}, using first element"
                    )
                    calibrated_metrics[metric] = calibrated_metrics[metric] * float(factor[0])
                else:
                    try:
                        ...
                        logger.warning(
                            f"Could not convert {factor} to float for incremental method, using original value"
                        )
                ...
                if isinstance(factor, (list, tuple)) and len(factor) > 0:
                    ...
                    if "base_factor" in factor:
                        ...
                        logger.warning(
                            f"Unsupported dict format for linear_scaling: {factor}, using original value"
                        )
                else:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not convert {factor} to float for linear_scaling, using original value"
                        )
                ...
                if isinstance(factor, (list, tuple)) and len(factor) > 0:
                    ...
                    if "base_factor" in factor:
                        # Use base_factor if available
                        calibrated_metrics[metric] = calibrated_metrics[metric] * (
                            2 - float(factor["base_factor"])
                        )
                    elif (
                        "ensemble_factors" in factor
                        and "linear_scaling" in factor["ensemble_factors"]
                        ...
                        if isinstance(linear_factor, (list, tuple)):
                            ...
                            calibrated_metrics[metric] = calibrated_metrics[metric] * (
                                2 - float(linear_factor)
                            )
                    else:
                        logger.warning(
                            f"Unsupported dict format for additive_adjustment: {factor}, using original value"
                        )
                else:
                    # Try direct conversion to float
                    try:
                        ...
                        logger.warning(
                            f"Could not convert {factor} to float for additive_adjustment, using original value"
                        )
                ...
                if isinstance(factor, (list, tuple)) and len(factor) == 2:
                    ...
                    # Use first element as a scalar if not the right format
                    logger.warning(
                        f"Unexpected list length for regression: {factor}, using first element as multiplier"
                    )
                    calibrated_metrics[metric] = calibrated_metrics[metric] * float(factor[0])
                elif isinstance(factor, dict):
                    if "ensemble_factors" in factor and "regression" in factor["ensemble_factors"]:
                        ...
                        if (
                            isinstance(regression_factor, (list, tuple))
                            and len(regression_factor) == 2
                        ):
                            ...
                            isinstance(regression_factor, (list, tuple))
                            and len(regression_factor) > 0
                        ):
                            # Use first element as a scalar if not the right format
                            logger.warning(
                                f"Unexpected list length for regression in ensemble: {regression_factor}, using first element"
                            )
                            calibrated_metrics[metric] = calibrated_metrics[metric] * float(
                                regression_factor[0]
                            )
                            ...
                            logger.warning(
                                f"Invalid regression factor in ensemble: {regression_factor}, using original value"
                            )
                    elif "base_factor" in factor:
                        # Use base_factor if available (though this isn't regression, it's a fallback)
                        logger.warning(
                            f"No regression parameters in factor dict, using base_factor as fallback"
                        )
                        calibrated_metrics[metric] = calibrated_metrics[metric] * float(
                            factor["base_factor"]
                        )
                        ...
                        logger.warning(
                            f"Invalid regression factor dict: {factor}, using original value"
                        )
                else:
                    try:
                        # Try as simple scaling as a last resort
                        logger.warning(
                            f"Invalid regression factor: {factor}, trying as multiplicative factor"
                        )
                        calibrated_metrics[metric] = calibrated_metrics[metric] * float(factor)
                    except (TypeError, ValueError):
                        logger.warning(
                            f"Could not convert {factor} to float for regression, using original value"
                        )
                ...
                if isinstance(factor, (list, tuple)) and len(factor) == 2:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Could not convert factor to float: {factor}, using original value"
                        )
                        # Keep original value
        ...
        calibrated_result = SimulationResult(
            ...
            additional_metadata={
                **(simulation_result.additional_metadata or {}),
                ...
                "calibration_version": simulation_parameters.get(
                    "calibration_version", self.config["calibration_version"]
                ),
    ...
    def _select_calibration_method(
        self, hardware_id: str, model_id: str, validation_results: List[ValidationResult]
    ) -> str:
        """
        ...
        if self.config["use_hardware_profiles"] and hardware_id in self.hardware_profiles:
            ...
            if "preferred_calibration_method" in profile:
                ...
                logger.info(
                    f"Using hardware-specific calibration method for {hardware_id}: {method}"
                )
                return method
        ...
        if recent_drift and recent_drift.get("significant_drift_detected", False):
            ...
            if len(validation_results) >= 15:
                logger.info(
                    f"Significant drift detected for {hardware_id}/{model_id}, using bayesian optimization"
                )
                return "bayesian"
            else:
                logger.info(
                    f"Significant drift detected for {hardware_id}/{model_id}, but limited data, using ensemble"
                )
                return "ensemble"
        ...
        if calibration_frequency.get("adaptive_method_selection", False):
            ...
            if suggested_method:
                logger.info(
                    f"Using adaptive method selection for {hardware_id}/{model_id}: {suggested_method}"
                )
        ...
        if linear_fit_quality > 0.9:
            ...
            if len(validation_results) >= 20:
                ...
                try:
                    ...
                    logger.info(
                        f"Complex relationship for {hardware_id}/{model_id}, using advanced methods"
                    )
                    return "network"
                except ImportError:
                    logger.info(
                        f"sklearn not available, falling back to ensemble for {hardware_id}/{model_id}"
                    )
                    return "ensemble"
            else:
                # Not enough data for advanced methods
                logger.info(
                    f"Not enough data for advanced methods for {hardware_id}/{model_id}, using ensemble"
                )
                return "ensemble"

    def _check_recent_drift_detection(
        self, hardware_id: str, model_id: str
    ...
    def _determine_calibration_frequency(
        ...
        try:
            ...
            if recent_drift and recent_drift.get("significant_drift_detected", False):
                ...
                recommendation["should_calibrate"] = True
                recommendation["recommended_frequency_days"] = max(
                    7, days_since_last_calibration // 2
                )
                ...
                if avg_improvement < 5:  # Less than 5% improvement
                    ...
                    recommendation["adaptive_method_selection"] = True
                    recommendation["suggested_method"] = last_calibration.get(
                        "method", self.config["calibration_method"]
                    )
                else:
    ...
    def _apply_calibration_method(
        ...
        current_factors = {}
        if hardware_id in current_parameters.get(
            "correction_factors", {}
        ) and model_id in current_parameters["correction_factors"].get(hardware_id, {}):
        ...
        if method == "linear_scaling":
            ...
            return self._apply_bayesian_optimization(
                hardware_id, model_id, validation_results, current_factors
            )

        elif method == "network":
            # Apply neural network for complex relationships
            return self._apply_neural_network(
                hardware_id, model_id, validation_results, current_factors
            )

        elif method == "incremental":
            # Apply incremental learning with trend analysis
            return self._apply_incremental_learning(
                hardware_id, model_id, validation_results, current_factors, current_parameters
            )

        elif method == "ensemble":
            # Apply ensemble of methods
            return self._apply_ensemble_methods(
                hardware_id, model_id, validation_results, current_factors
            )
    ...
    def _apply_bayesian_optimization(
        ...
        try:
            ...
            logger.warning(
                "scikit-optimize not available for Bayesian optimization, falling back to regression"
            )
        ...
        for metric in self.config["metrics_to_calibrate"]:
            ...
            for val_result in validation_results:
                ...
                if (
                    ...
                    if (
                        hasattr(val_result.simulation_result, "batch_size")
                        and val_result.simulation_result.batch_size is not None
                    ):
                    ...
                    if (
                        hasattr(val_result.simulation_result, "precision")
                        and val_result.simulation_result.precision is not None
                    ):
            ...
            if metric in current_factors:
                if (
                    isinstance(current_factors[metric], (list, tuple))
                    and len(current_factors[metric]) == 2
                ):
            ...
            res = gp_minimize(
                objective, space, n_calls=self.config["bayesian_iterations"], x0=x0, random_state=42
            )
            ...
            if len(batch_sizes) > 0 and not all(bs is None for bs in batch_sizes):
                ...
                if len(unique_batches) > 1:
                    batch_analysis = {"unique_batch_sizes": unique_batches, "errors_by_batch": {}}
                    ...
                    for batch in unique_batches:
                        ...
                        batch_mape = (
                            np.mean(np.abs(batch_residuals / batch_y)) * 100
                            if np.all(batch_y != 0)
                            else None
                        )
                        ...
                        if batch_mape is not None:
                            batch_analysis["errors_by_batch"][str(batch)]["mape"] = float(
                                batch_mape
                            )
            ...
            if len(precisions) > 0 and not all(p is None for p in precisions):
                ...
                if len(unique_precisions) > 1:
                    ...
                    for precision in unique_precisions:
                        ...
                        precision_mape = (
                            np.mean(np.abs(precision_residuals / precision_y)) * 100
                            if np.all(precision_y != 0)
                            else None
                        )
                        ...
                        if precision_mape is not None:
                            precision_analysis["errors_by_precision"][precision]["mape"] = float(
                                precision_mape
                            )
            ...
            if fold_indices is not None:
                result["cross_validation"] = {"folds": n_folds, "used": True}

            correction_factors[metric] = result

            logger.info(
                f"Bayesian optimization for {hardware_id}/{model_id}/{metric}: "
    ...
    def _apply_neural_network(
        ...
        try:
            ...
            logger.warning(
                "scikit-learn not available for neural network, falling back to regression"
            )
            # Use regression as a fallback
            basic_config = self.basic_calibrator.config.copy()
            basic_config["calibration_method"] = "regression"
        ...
        for metric in self.config["metrics_to_calibrate"]:
            ...
            model_uncertainty = {}
            if self.config.get("enable_cross_validation", True) and len(data) >= self.config.get(
                "cross_validation_folds", 3
            ):
                cv = KFold(
                    n_splits=self.config.get("cross_validation_folds", 3),
                    shuffle=True,
                    random_state=42,
                )
                cv_scores = cross_val_score(
                    nn, X_scaled, y, cv=cv, scoring="neg_mean_squared_error"
                )
                ...
                model_uncertainty = {
                    ...
                    "cross_val_coefficient_of_variation": float(std_mse / mean_mse)
                    if mean_mse > 0
                    else 0.0,
                }

                logger.info(
                    f"Cross-validation for {hardware_id}/{model_id}/{metric}: "
            ...
            logger.info(
                f"Neural network for {hardware_id}/{model_id}/{metric}: "
        ...
        for metric in self.config["metrics_to_calibrate"]:
            ...
            if len(calibration_history) > 1 and metric in current_factors:
                ...
                for i, event in enumerate(calibration_history[:-1]):  # Skip most recent
                    if (
                        i > 0  # Skip the first one since we need at least two for trend
                        and "hardware_models" in event
                        and (hardware_id, model_id) in event["hardware_models"]
                    ):
                        # Find corresponding factor in history
                        if (
                            hardware_id in current_parameters.get("correction_factors", {})
                            and model_id
                            in current_parameters["correction_factors"].get(hardware_id, {})
                            and metric
                            in current_parameters["correction_factors"][hardware_id][model_id]
                        ):
                            factor = current_parameters["correction_factors"][hardware_id][
                                model_id
                            ][metric]
                ...
                if len(previous_factors) >= 2:
                    ...
                    changes = [
                        previous_factors[i] - previous_factors[i - 1]
                        for i in range(1, len(previous_factors))
                    ]
                    trend_factor = sum(changes) / len(changes)

                    logger.info(
                        f"Calculated trend for {hardware_id}/{model_id}/{metric}: {trend_factor:.6f} per calibration"
                    )
        ...
        for metric in self.config["metrics_to_calibrate"]:
            ...
            for method in methods:
                if method == "linear_scaling":
                    ...
                    factors = self.basic_calibrator._calculate_correction_factors(
                        validation_results
                    )
                    ...
                    method_factors = self._apply_bayesian_optimization(
                        ...
                        model_id,
                        validation_results,
                        {metric: current_factors.get(metric, 1.0)},
                    )
            ...
            correction_factors[metric] = {
                ...
                "ensemble_weights": {
                    k: v for k, v in ensemble_weights.items() if k in ensemble_factors
                },
            }
        ...
        profiles = {
            ...
            "gpu_rtx3080": {
                ...
                "metric_adjustments": {
                    ...
                    "memory_peak_mb": {"precision_factors": {"fp32": 1.0, "fp16": 0.55}},
                },
            },
            "gpu_a100": {
                "preferred_calibration_method": "ensemble",
    ...
    def _calculate_enhanced_error_statistics(
        self, validation_results: List[ValidationResult], metrics: List[str]
