...
try:
    ...
    from data.duckdb.simulation_validation.drift_detection.advanced_detector import (
                ...
                for batch_size in batch_sizes:
                    for precision in precisions:
                        ...
                        for i in range(num_samples):
                            ...
                            hw_latency = (
                                latency_base * latency_scale * (1 + np.random.normal(0, 0.05))
                            )
                            ...
                            sim_throughput = (
                                hw_throughput * throughput_bias * (1 + np.random.normal(0, 0.1))
                            )
    ...
    def test_comparison_pipeline(self):
        ...
        for val_result in validation_results[:5]:  # Check the first few results
            ...
            self.assertIsInstance(
                val_result.simulation_result,
                SimulationResult,
                "Simulation result is not a SimulationResult",
            )
            self.assertIsInstance(
                val_result.hardware_result,
                HardwareResult,
                "Hardware result is not a HardwareResult",
            )
            self.assertTrue(len(val_result.metrics_comparison) > 0, "Metrics comparison is empty")
    ...
    def test_statistical_validator(self):
        ...
        if len(validation_results) > 0:
            ...
            filtered_results = [
                ...
                if r.hardware_result.hardware_id == hw_id and r.hardware_result.model_id == model_id
            ]

            if len(filtered_results) >= 3:  # Need enough results for confidence
                confidence = validator.calculate_confidence_score(filtered_results, hw_id, model_id)

                self.assertIsInstance(confidence, dict, "Confidence score is not a dictionary")
                self.assertIn(
                    "overall_confidence", confidence, "Overall confidence not in confidence score"
                )
                self.assertIsInstance(
...
class TestCalibration(TestFrameworkBase):
    ...
    def test_basic_calibrator(self):
        ...
        simulation_parameters = {"correction_factors": {}}
        ...
        self.assertIn(
            "correction_factors", updated_parameters, "Correction factors not in updated parameters"
        )
        ...
        if hw_id and model_id:
            ...
            for metric in [
                ...
                if metric in factors:
                    ...
                    self.assertTrue(
                        isinstance(factor, (int, float, np.number, list, tuple))
                        or (isinstance(factor, dict) and "base_factor" in factor),
    ...
    def test_apply_calibration(self):
        ...
        if len(self.biased_sim_results) > 0:
            ...
            self.assertIsInstance(
                calibrated_result, SimulationResult, "Calibrated result is not a SimulationResult"
            )
    ...
    def test_advanced_calibrator(self):
        ...
        if len(validation_results) >= 2:
            ...
            if "mape" in evaluation["overall"]:
                self.assertIn(
                    "before", evaluation["overall"]["mape"], "Before MAPE missing from evaluation"
                )
                self.assertIn(
                    "after", evaluation["overall"]["mape"], "After MAPE missing from evaluation"
                )
...
class TestDriftDetection(TestFrameworkBase):
    ...
    def test_basic_drift_detector(self):
        ...
        self.assertIn(
            "is_significant", drift_results, "is_significant flag missing from drift results"
        )
        ...
        for metric, info in drift_metrics.items():
            self.assertIn("historical_mape", info, f"historical_mape missing for {metric}")
            self.assertIn("new_mape", info, f"new_mape missing for {metric}")
            self.assertIn("absolute_change", info, f"absolute_change missing for {metric}")
            self.assertIn("is_significant", info, f"is_significant flag missing for {metric}")
    ...
    def test_advanced_drift_detector(self):
        ...
        self.assertIn(
            "basic", drift_results["analysis_types"], "basic analysis missing from analysis_types"
        )
        ...
        for analysis_type, result_key in advanced_analyses:
            if analysis_type in drift_results["analysis_types"]:
                ...
                self.assertIsInstance(analysis_result, dict, f"{result_key} is not a dictionary")
                self.assertIn(
                    "is_significant",
                    analysis_result,
    ...
    def test_drift_thresholds(self):
        ...
        for metric, threshold in new_thresholds.items():
            self.assertEqual(
                detector.drift_thresholds[metric],
                threshold,
...
class TestVisualization(TestFrameworkBase):
    ...
    def test_mape_comparison_chart(self):
        ...
        result = visualizer.create_mape_comparison_chart(
            validation_results, metric_name="all", output_path=output_path, interactive=False
        )
    ...
    def test_drift_detection_visualization(self):
        ...
        result = visualizer.create_drift_detection_visualization(
            drift_results, output_path=output_path, interactive=False
        )
    ...
    def test_comprehensive_dashboard(self):
        ...
        result = visualizer.create_comprehensive_dashboard(
            validation_results, output_path=output_path
        )
...
class TestFrameworkIntegration(TestFrameworkBase):
    ...
    def test_end_to_end_validation(self):
        ...
        validation_results = self.framework.validate(
            self.simulation_results, self.hardware_results, protocol="standard"
        )
        ...
        report = self.framework.generate_report(
            validation_results, format="markdown", output_path=report_path
        )
    ...
    def test_calibration_workflow(self):
        ...
        validation_results = self.framework.validate(
            self.biased_sim_results, self.biased_hw_results, protocol="standard"
        )
        ...
        # Calibrate the simulation
        updated_parameters = self.framework.calibrate(validation_results, simulation_parameters)
    ...
    def test_drift_detection_workflow(self):
        ...
        # Detect drift
        drift_results = self.framework.check_drift(historical_results, new_results)
    ...
    def test_visualization_workflow(self):
        ...
        self.framework.visualize_mape_comparison(
            validation_results, metric_name="all", output_path=mape_path
        )
        ...
        self.framework.visualize_hardware_comparison_heatmap(
            validation_results, metric_name="throughput_items_per_second", output_path=heatmap_path
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory for test results"
    )
