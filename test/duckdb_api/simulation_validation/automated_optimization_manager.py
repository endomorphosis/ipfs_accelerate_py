...
try:
    ...
    logger.error(
        "Failed to import DBPerformanceOptimizer. Make sure duckdb_api is properly installed."
    )
    sys.exit(1)
...
class AutomatedOptimizationManager:
    ...
    def _setup_scheduled_tasks(self) -> None:
        ...
        # Daily tasks
        self.schedule_task(
            "daily_optimization", ["create_indexes", "analyze_tables"], interval_hours=24
        )

        # Weekly tasks
        self.schedule_task(
            "weekly_optimization", ["optimize_database", "vacuum_database"], interval_hours=24 * 7
        )
    ...
    def schedule_task(
        ...
        logger.info(
            f"Scheduled task '{task_name}' to run every {interval_hours} hours starting at {start_time}"
        )
    ...
    def _check_scheduled_tasks(self) -> None:
        ...
        for task_name, task in self.scheduled_tasks.items():
            if task["enabled"] and now >= task["next_run"]:
                ...
                logger.info(
                    f"Scheduled task '{task_name}' completed. Next run at {task['next_run']}"
                )
    ...
    def start_monitoring(self) -> None:
        ...
        logger.info(
            f"Started automated optimization monitoring (interval: {self.check_interval} seconds)"
        )
    ...
    def _process_detected_issues(
        ...
        for metric_name, issue in issues.items():
            logger.info(
                f"Processing issue with {metric_name}: {issue['value']} {issue['unit']} ({issue['severity']})"
            )
            ...
            if self.auto_apply_actions:
                ...
                improvement = self._calculate_improvement(
                    metric_name, before_metrics, after_metrics
                )
                ...
                results[metric_name] = {
                    ...
                    "before_value": before_metrics[metric_name]["value"]
                    if metric_name in before_metrics
                    else None,
                    "after_value": after_metrics[metric_name]["value"]
            ...
            try:
                ...
                if action == "optimize_queries":
                    ...
                    cleanup_results = self.db_optimizer.cleanup_old_records(
                        older_than_days=self.retention_days, dry_run=False
                    )
                    ...
                    results[action] = {
                        "status": "skipped",
                        "message": "Cache settings adjustment not implemented",
                    }
                    ...
                    results[action] = {
                        "status": "skipped",
                        "message": "Batch operations is a strategy, not a direct action",
                    }
    ...
    def check_performance(
        self, specific_metrics: Optional[List[str]] = None
    ) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def optimize_now(
        self, issues: Optional[Dict[str, Dict[str, Any]]] = None
    ) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for metric_name in optimization_results:
            ...
            optimization_results[metric_name]["after_value"] = (
                after_metrics[metric_name]["value"] if metric_name in after_metrics else None
            )
    ...
    def analyze_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        for metric_name, history in self.metrics_history.items():
            ...
            filtered_history = [
                entry for entry in history if entry["timestamp"] >= threshold_timestamp
            ]
            ...
            if end_value > start_value:
                ...
                change_pct = (
                    ((start_value - end_value) / start_value) * 100 if start_value != 0 else 0
                )
            else:
                direction = "stable"
                change_pct = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        choices=["check", "optimize", "comprehensive", "monitor", "trends"],
        default="check",
    ...
    parser.add_argument("--metrics", nargs="+", help="Specific metrics to check")
    parser.add_argument(
        "--monitor-time", type=int, help="Time in seconds to run monitoring (default: indefinite)"
    )
    parser.add_argument(
        "--check-interval", type=int, default=3600, help="Interval in seconds between checks"
    )
    ...
    try:
        ...
        manager = get_optimization_manager(
            db_optimizer=db_optimizer, config_file=args.config, auto_apply=args.auto_apply
        )
