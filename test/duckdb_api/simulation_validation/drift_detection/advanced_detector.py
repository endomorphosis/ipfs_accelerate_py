...
try:
    ...
    logger.warning(
        "scikit-learn not available, some advanced drift detection methods will be limited"
    )
...
class AdvancedDriftDetector(DriftDetector):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.current_drift_status = {
            ...
            "significant_metrics": [],
            "multi_dimensional_drift": {"detected": False, "p_value": None, "method": None},
            "distribution_drift": {"detected": False, "metrics": {}},
            "correlation_drift": {"detected": False, "changes": {}},
            "time_series_drift": {"detected": False, "trends": {}},
            "anomaly_detection": {"anomalies_detected": False, "metrics": {}},
        }
    ...
    def detect_drift(
        ...
        if (
            len(historical_validation_results) < self.config["min_samples"]
            or len(new_validation_results) < self.config["min_samples"]
        ):
            return {
                "status": "error",
        ...
        self.current_drift_status = {
            ...
            "multi_dimensional_drift": {
                "detected": drift_results.get("multi_dimensional_analysis", {}).get(
                    "is_significant", False
                ),
            ...
            "distribution_drift": {
                "detected": drift_results.get("distribution_analysis", {}).get(
                    "is_significant", False
                ),
                "metrics": drift_results.get("distribution_analysis", {}).get("metrics", {}),
            },
            "correlation_drift": {
                "detected": drift_results.get("correlation_analysis", {}).get(
                    "is_significant", False
                ),
                "changes": drift_results.get("correlation_analysis", {}).get(
                    "correlation_changes", {}
                ),
            },
            "time_series_drift": {
                "detected": drift_results.get("time_series_analysis", {}).get(
                    "is_significant", False
                ),
                "trends": drift_results.get("time_series_analysis", {}).get("trends", {}),
            },
            "anomaly_detection": {
                "anomalies_detected": drift_results.get("anomaly_detection", {}).get(
                    "is_significant", False
                ),
        ...
        try:
            ...
            if historical_array.shape[1] > 2:
                ...
                if method == "pca":
                    ...
                    logger.warning(
                        f"Unknown dimension reduction method: {method}, using first two dimensions"
                    )
        ...
        try:
            ...
            for metric in self.config["metrics_to_monitor"]:
                ...
                metrics_results[metric] = {
                    ...
                    "tests": test_results,
                    "sample_counts": {"historical": len(historical_mape), "new": len(new_mape)},
                }
    ...
    def _perform_correlation_analysis(
        ...
        try:
            ...
            historical_metrics_by_result = self._extract_metrics_by_result(
            ...
            historical_correlations = self._calculate_metric_correlations(
                historical_metrics_by_result
            )
            ...
            for pair, historical_corr in historical_correlations.items():
                if pair in new_correlations:
                    ...
                    z_historical = (
                        0.5 * log((1 + historical_corr) / (1 - historical_corr))
                        if abs(historical_corr) < 1
                        else 0
                    )
                    ...
                    is_pair_significant = (
                        p_value < self.config["correlation_significance_threshold"]
                    )
                    ...
                    correlation_changes[pair] = {
                        ...
                        "sample_counts": {"historical": historical_n, "new": new_n},
                    }
    ...
    def _perform_time_series_analysis(
        ...
        try:
            ...
            historical_sorted = sorted(
                historical_validation_results, key=lambda x: x.validation_timestamp
            )
                ...
                for metric, comparison in result.metrics_comparison.items():
                    if metric in self.config["metrics_to_monitor"] and "mape" in comparison:
                        if metric not in metrics_by_time:
                            metrics_by_time[metric] = []
                        metrics_by_time[metric].append(
                            {"timestamp": timestamp, "mape": comparison["mape"], "dataset": "new"}
                        )
            ...
            for metric, time_series in metrics_by_time.items():
                ...
                linear_significant = (
                    p_value < self.config["significance_level"] and abs(slope) > 0.05
                )  # At least 0.05 MAPE change per point
                mk_significant = (
                    mk_p_value is not None
                    and mk_p_value < self.config["significance_level"]
                    and abs(tau) > 0.3
                ...
                try:
                    ...
                    if mid_point > 5:  # Need enough points for meaningful regression
                        ...
                        slope_second, _, _, p_second, _ = stats.linregress(
                            second_half_x, second_half_y
                        )
                        ...
                        if p_first < 0.1 and p_second < 0.1:  # Both slopes are significant
                            ...
                            if (
                                slope_change > 0.1
                            ):  # At least 0.1 difference in MAPE change per point
                                change_point = mid_point
                                change_point_confidence = 1.0 - max(
                                    p_first, p_second
                                )  # Higher confidence with lower p-values
                except Exception as e:
                ...
                trends[metric] = {
                    ...
                    "change_point": {"index": change_point, "confidence": change_point_confidence},
        ...
        try:
            ...
            for metric in self.config["metrics_to_monitor"]:
                ...
                if method == "local_outlier_factor":
                    ...
                    predictions = [
                        1 if c == -1 else -1 for c in new_clusters
                ...
                anomaly_significant = (
                    anomaly_ratio > self.config["anomaly_detection_contamination"] * 1.5
                )
    ...
    def _extract_mape_values(
        self, validation_results: List[ValidationResult], metric: str
    ) -> List[float]:
        """
    ...
    def _extract_metrics_by_result(
        self, validation_results: List[ValidationResult]
    ) -> List[Dict[str, float]]:
        """
    ...
    def _calculate_metric_correlations(
        self, metrics_by_result: List[Dict[str, float]]
    ) -> Dict[str, float]:
        """
        ...
        for i in range(len(metric_names)):
            for j in range(i + 1, len(metric_names)):
                ...
                if (
                    metric1.startswith("sim_")
                    and metric2.startswith("hw_")
                    and metric1[4:] == metric2[3:]
                ):
                    continue
                ...
                if len(values1) >= 5:
                    try:
                        ...
                        logger.warning(
                            f"Error calculating correlation for {metric1}/{metric2}: {e}"
                        )
