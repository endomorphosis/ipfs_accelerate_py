    ...
    def invalidate(self, table_name: Optional[str] = None) -> None:
        ...
        with self.lock:
            if table_name is None:
                ...
                logger.info(
                    f"Cleared {len(keys_to_remove)} entries from query cache for table {table_name}"
                )
...
class DBPerformanceOptimizer:
    ...
    def batch_insert_validation_results(self, results: List[ValidationResult]) -> None:
        ...
        try:
            from data.duckdb.simulation_validation.core.schema import (
                SimulationValidationSchema as schema,
            )
    ...
    def get_validation_results_optimized(
        ...
        try:
            ...
            if start_date:
                conditions.append(
                    "vr.validation_timestamp /* validation_timestamp_idx */ >= :start_date"
                )
                params["start_date"] = start_date

            if end_date:
                conditions.append(
                    "vr.validation_timestamp /* validation_timestamp_idx */ <= :end_date"
                )
        ...
        try:
            ...
            if backup_path is None:
                ...
                backup_path = os.path.join(
                    backup_dir, f"{os.path.basename(self.db_path)}.{timestamp}.bak"
                )
    ...
    def cleanup_old_records(
        self, older_than_days: int, tables: Optional[List[str]] = None, dry_run: bool = False
    ) -> Dict[str, int]:
        """
    ...
    def _get_query_time_metrics(self, conn) -> Dict[str, Any]:
        ...
        try:
            ...
            change_pct = (
                ((query_time_ms - previous_value) / previous_value * 100) if previous_value else 0
            )
    ...
    def _get_storage_metrics(self, db_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            previous_value = history[-1] if history else None
            change_pct = (
                ((current_size - previous_value) / previous_value * 100) if previous_value else 0
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        choices=["optimize", "backup", "restore", "stats", "cleanup", "create-indexes", "analyze"],
        default="optimize",
