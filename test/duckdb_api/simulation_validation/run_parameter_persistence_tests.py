...
def run_tests(args):
    ...
    if args.integrated:
        ...
        suite.addTest(
            TestComprehensiveEndToEnd("test_25_database_predictive_analytics_parameter_persistence")
        )
    ...
    if args.performance_report and args.output_dir:
        ...
        for model_type in model_types:
            ...
            config_disabled = {"parameter_persistence": {"enabled": False}}
        ...
        performance_report = {
            ...
            "summary": {
                "average_speedup": sum(
                    [performance_data[model_type]["speedup"] for model_type in model_types]
                )
                / len(model_types),
                "max_speedup": max(
                    [performance_data[model_type]["speedup"] for model_type in model_types]
                ...
                "average_overhead": sum(
                    [performance_data[model_type]["overhead_time"] for model_type in model_types]
                )
                / len(model_types),
            },
        }
        ...
        report_path = os.path.join(
            args.output_dir,
            f"parameter_persistence_performance_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        )
        ...
        md_report_path = os.path.join(
            args.output_dir,
            f"parameter_persistence_performance_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
        )
        with open(md_report_path, "w") as f:
            ...
            f.write(
                f"- Average Overhead (first run): {performance_report['summary']['average_overhead']:.2f} seconds\n\n"
            )

            f.write("## Detailed Performance\n\n")
            f.write(
                "| Model Type | First Run (s) | Second Run (s) | Without Persistence (s) | Speedup | Overhead (s) |\n"
            )
            f.write(
                "|------------|--------------|---------------|-------------------------|---------|-------------|\n"
            )
            ...
            for model_type in model_types:
                ...
                f.write(
                    f"| {model_type} | {data['first_run_time']:.2f} | {data['second_run_time']:.2f} | "
                )
                f.write(
                    f"{data['disabled_time']:.2f} | {data['speedup']:.2f}x | {data['overhead_time']:.2f} |\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for the Parameter Persistence functionality."
    )
    parser.add_argument(
        "--standalone", action="store_true", help="Run standalone parameter persistence tests"
    )
    parser.add_argument(
        "--integrated",
        action="store_true",
        help="Run the parameter persistence test from the comprehensive E2E test suite",
    )
    parser.add_argument(
        "--performance-report", action="store_true", help="Generate detailed performance report"
    )
    parser.add_argument("--html-report", action="store_true", help="Generate HTML test report")
    parser.add_argument(
        "--output-dir", type=str, default="test_output", help="Directory to save test reports"
    )
