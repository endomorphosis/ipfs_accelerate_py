...
class ComparisonPipeline:
    ...
    def preprocess_data(
        ...
        if self.config["preprocessing"]["outlier_detection"]["enabled"]:
            ...
            preprocessed_simulation_results = self._remove_outliers(
                preprocessed_simulation_results, method, threshold
            )
            preprocessed_hardware_results = self._remove_outliers(
                preprocessed_hardware_results, method, threshold
            )
                ...
                if matching_hw_results:
                    if len(matching_hw_results) == 1:
                        ...
                        if time_window > 0 and closest_time_diff > time_window:
                            logger.warning(
                                f"Closest match for {key} exceeds time window ({closest_time_diff} > {time_window} seconds)"
                            )
    ...
    def compare_results(
        self, aligned_pairs: List[Tuple[SimulationResult, HardwareResult]]
        ...
        for sim_result, hw_result in aligned_pairs:
            ...
            metrics_comparison = self._calculate_metrics_comparison(
                sim_result.metrics, hw_result.metrics
            )

            # Generate additional metrics
            additional_metrics = self._generate_additional_metrics(
                sim_result, hw_result, metrics_comparison
            )
    ...
    def analyze_rankings(
        self, simulation_values: Dict[str, float], hardware_values: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        if len(common_keys) < 2:
            return {
                "status": "error",
                "message": "Insufficient common data points for ranking analysis",
            }
        ...
        sim_ranking = {
            ...
            for rank, (k, _) in enumerate(
                sorted(common_sim_values.items(), key=lambda x: x[1], reverse=True), 1
            )
        }
        hw_ranking = {
            k: rank
            for rank, (k, _) in enumerate(
                sorted(common_hw_values.items(), key=lambda x: x[1], reverse=True), 1
            )
        }
            ...
            try:
                ...
                analysis["kendall_tau"] = {
                    ...
                    "interpretation": (
                        ...
                        if abs(tau) > 0.7
                        else "Moderate rank correlation"
                        if abs(tau) > 0.4
                        else "Weak rank correlation"
                    ),
                }
            except ImportError:
            ...
            for n in [1, 3, 5, 10]:
                if n <= len(common_keys):
                    ...
                    analysis[f"percentage_same_top_{n}"] = {
                        ...
                        "total_items": n,
                        "interpretation": (
                            "Excellent ranking preservation"
                            if percentage >= 80
                            else "Good ranking preservation"
                            if percentage >= 60
                            else "Moderate ranking preservation"
                            if percentage >= 40
                            else "Poor ranking preservation"
                        ),
                    }
    ...
    def _remove_outliers(
        self, results: List[Union[SimulationResult, HardwareResult]], method: str, threshold: float
    ...
    def _remove_outliers_iqr(
        self, results: List[Union[SimulationResult, HardwareResult]], threshold: float
    ) -> List[Union[SimulationResult, HardwareResult]]:
        """
        ...
        for result in results:
            ...
            for metric, (lower_bound, upper_bound) in metric_bounds.items():
                if (
                    ...
                    and result.metrics[metric] is not None
                    and (
    ...
    def _remove_outliers_dbscan(
        self, results: List[Union[SimulationResult, HardwareResult]], eps: float
    ) -> List[Union[SimulationResult, HardwareResult]]:
        """
