...
class ValidationResultsAnalyzer:
    ...
    def load_validation_results(
        self, results_dir: str, run_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_results(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not validation_items and not summary:
            return {"status": "error", "message": "No validation items or summary found in results"}
        ...
        if format == "json":
            ...
            if analysis["best_performing_pairs"]:
                ...
                for pair in analysis["best_performing_pairs"]:
                    ...
                    lines.append(
                        f"| {pair['hardware_id']} | {pair['model_id']} | {mean_mape:.2f}% | {median_mape:.2f}% | {min_mape:.2f}% | {max_mape:.2f}% |"
                    )
            ...
            if analysis["hardware_model_metrics"]:
                ...
                for pair in sorted_pairs:
                    ...
                    for metric, data in sorted(
                        metrics.items(), key=lambda x: x[1].get("mean_mape", float("inf"))
                    ):
                        if metric != "overall":
                    ...
                    for metric, data in sorted_metrics.items():
                        ...
                        lines.append(
                            f"| {metric} | {mean_mape:.2f}% | {median_mape:.2f}% | {min_mape:.2f}% | {max_mape:.2f}% |"
                        )
            ...
            if "summary" in analysis:
                ...
                if "overall_mape" in summary:
                    ...
                    mape_class = (
                        "good" if overall_mape < 10 else "moderate" if overall_mape < 20 else "poor"
                    )
                    html_lines.append(
                        f"        <p><strong>Overall MAPE</strong>: <span class='{mape_class}'>{overall_mape:.2f}%</span></p>"
                    )
                if "status" in summary:
                    html_lines.append(
                        f"        <p><strong>Validation Status</strong>: {summary['status']}</p>"
                    )
                if "timestamp" in summary:
                    html_lines.append(
                        f"        <p><strong>Validation Timestamp</strong>: {summary['timestamp']}</p>"
                    )
            ...
            if analysis["best_performing_pairs"]:
                ...
                for pair in analysis["best_performing_pairs"]:
                    ...
                    mean_mape = overall_metrics.get("mean_mape", 0) * 100
                    lines.append(
                        f"  {pair['hardware_id']} - {pair['model_id']}: {mean_mape:.2f}% mean MAPE"
                    )
                lines.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze validation results for Simulation Validation Framework"
    )
    parser.add_argument(
        "--results-dir", type=str, required=True, help="Directory containing validation results"
    )
