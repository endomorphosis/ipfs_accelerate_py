...
try:
    ...
    logger.warning(
        "AdvancedSimulationCalibrator not available, advanced calibration functions will be limited"
    )
...
try:
    ...
    logger.warning(
        "AutomaticParameterDiscovery not available, parameter discovery functions will be limited"
    )
...
class SimulationValidationFramework:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if enhanced_validator_available and use_enhanced_validator:
            self.statistical_validator = EnhancedStatisticalValidator(
                self.config.get("statistical_validator", {})
            )
    ...
    def calibrate(
        ...
        # Run calibration
        updated_parameters = self.calibrator.calibrate(validation_results, simulation_parameters)
        ...
        if validation_results:
            ...
            logger.info(
                f"Calibration completed with {improvement['overall']['relative_improvement']:.2f}% overall improvement"
            )

            # Store calibration in database if available
            if self.db_api:
                self._store_calibration_results(
                    validation_results, simulation_parameters, updated_parameters, improvement
                )
        else:
        ...
        if self.reporter:
            ...
            if output_path:
                ...
                return self.reporter.generate_report(
                    validation_results, format=format, include_visualizations=include_visualizations
                )
        else:
            ...
            if hardware_id or model_id:
                ...
                for result in validation_results:
                    if (
                        hardware_id is None or result.hardware_result.hardware_id == hardware_id
                    ) and (model_id is None or result.hardware_result.model_id == model_id):
                        filtered_results.append(result)

            report = self.methodology.generate_validation_report(
                filtered_results, hardware_id=hardware_id, model_id=model_id, report_format=format
            )
    ...
    def check_calibration_needed(
        ...
        """
        return self.methodology.check_calibration_needed(validation_results, hardware_id, model_id)
    ...
    def discover_parameters(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        if not self.parameter_discovery:
            ...
            return {"status": "error", "message": "Parameter discovery not available"}
    ...
    def analyze_parameter_sensitivity(
        ...
        if not self.parameter_discovery:
            logger.warning(
                "Parameter discovery not available, cannot analyze parameter sensitivity"
            )
            return {"status": "error", "message": "Parameter discovery not available"}
    ...
    def generate_parameter_insight_report(self) -> Dict[str, Any]:
        ...
        if not self.parameter_discovery:
            logger.warning(
                "Parameter discovery not available, cannot generate parameter insight report"
            )
            return {"status": "error", "message": "Parameter discovery not available"}
    ...
    def visualize_drift_detection(
        ...
        return self.visualizer.create_drift_detection_visualization(
            drift_results, interactive=interactive, output_path=output_path, title=title
        )
    ...
    def _initialize_database(self) -> None:
        ...
        try:
            ...
            self.db_api = BenchmarkDBAPI(
                db_path=db_config.get("db_path", "benchmark_db.duckdb"), create_if_missing=True
            )
    ...
    def _store_drift_detection_results(self, drift_results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.db_api.commit()
            logger.info(
                f"Stored drift detection results for {hardware_type}/{model_type} in database"
            )
    ...
    def _store_parameter_discovery_results(self, parameter_recommendations: Dict[str, Any]) -> None:
        ...
        try:
            ...
            optimization_recommendations = parameter_recommendations.get(
            ...
            key_findings = []
            if (
                "insights" in parameter_recommendations
                and "key_findings" in parameter_recommendations["insights"]
            ):
