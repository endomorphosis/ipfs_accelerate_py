...
try:
    ...
    logger.error(
        "Failed to import SimulationValidationDBIntegration. Make sure duckdb_api is properly installed."
    )
    sys.exit(1)
...
def test_baseline_performance(
    db_path: str, num_queries: int, hardware_types: List[str], model_types: List[str]
) -> Dict[str, float]:
    """
    ...
    for _ in range(num_queries):
        ...
        results = db_integration.get_validation_results(
            hardware_id=hardware_id, model_id=model_id, limit=100
        )
        end_time = time.time()
    ...
    logger.info(
        f"Baseline performance: Avg={avg_query_time:.4f}s, Min={min_query_time:.4f}s, Max={max_query_time:.4f}s"
    )
...
def test_optimized_performance(
    ...
    """
    # Create optimizer
    optimizer = get_db_optimizer(db_path=db_path, enable_caching=enable_caching)
    ...
    for i in range(num_queries):
        ...
        results = optimizer.get_validation_results_optimized(
            hardware_id=hardware_id, model_id=model_id, limit=100, use_cache=enable_caching
        )
        end_time = time.time()
    ...
    logger.info(
        f"Optimized performance: Avg={avg_query_time:.4f}s, Min={min_query_time:.4f}s, Max={max_query_time:.4f}s{cache_info}"
    )
...
def test_batch_insertion_performance(
    db_path: str, records: List[ValidationResult], batch_sizes: List[int]
) -> Dict[str, Any]:
    """
    ...
    for batch_size in batch_sizes:
        ...
        optimizer = get_db_optimizer(db_path=db_path, batch_size=batch_size)
        ...
        # Log result
        logger.info(
            f"Batch insertion (size={batch_size}): {len(test_records)} records in {insertion_time:.4f} seconds"
        )
...
def run_performance_tests(
    db_path: str, num_queries: int, num_insertion_records: int
) -> Dict[str, Any]:
    """
    ...
    batch_insertion_results = test_batch_insertion_performance(
        db_path=db_path, records=batch_test_records, batch_sizes=batch_sizes
    )
...
def print_test_summary(results: Dict[str, Any]) -> None:
    ...
    print(
        f"- Optimized (no cache): {no_cache_avg:.4f} sec/query ({no_cache_improvement:.1f}% improvement)"
    )
    print(
        f"- Optimized (with cache): {with_cache_avg:.4f} sec/query ({with_cache_improvement:.1f}% improvement)"
    )
    ...
    for batch_size, batch_data in sorted(batch_results.items()):
        print(
            f"- Batch size {batch_size}: {batch_data.get('insertion_time', 0):.4f} sec "
...
def main() -> None:
    ...
    parser.add_argument(
        "--db-path", default="./benchmark_db_test.duckdb", help="Path to the test database"
    )
    parser.add_argument("--create-db", action="store_true", help="Create a new test database")
    parser.add_argument(
        "--num-records", type=int, default=1000, help="Number of records for database creation"
    )
    parser.add_argument(
        "--num-queries", type=int, default=50, help="Number of queries to run in each test"
    )
    parser.add_argument(
        "--num-batch-records",
        type=int,
        default=500,
        help="Number of records for batch insertion test",
    )
    ...
    results_path = (
        f"db_performance_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
