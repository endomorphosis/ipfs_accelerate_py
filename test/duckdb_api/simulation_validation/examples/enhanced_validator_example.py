...
def demonstrate_basic_validation():
    ...
    print("\nEnhanced metrics:")
    if (
        validation_result.additional_metrics
        and "enhanced_metrics" in validation_result.additional_metrics
    ):
        for metric_name, metrics in validation_result.additional_metrics[
            "enhanced_metrics"
        ].items():
            print(f"  {metric_name}:")
    ...
    if (
        validation_result.additional_metrics
        and "confidence_intervals" in validation_result.additional_metrics
    ):
        for metric_name, ci_data in validation_result.additional_metrics[
            "confidence_intervals"
        ].items():
            if "mape" in ci_data:
                ci = ci_data["mape"]
                print(
                    f"  {metric_name} MAPE: {ci['value']:.2f}% (95% CI: {ci['lower_bound']:.2f}% - {ci['upper_bound']:.2f}%)"
                )
    ...
    if (
        validation_result.additional_metrics
        and "bland_altman" in validation_result.additional_metrics
    ):
        for metric_name, ba_data in validation_result.additional_metrics["bland_altman"].items():
            print(f"  {metric_name}:")
            print(f"    Bias: {ba_data['bias']:.4f}")
            print(
                f"    Limits of Agreement: {ba_data['lower_loa']:.4f} to {ba_data['upper_loa']:.4f}"
            )
...
def demonstrate_batch_validation():
    ...
    if (
        ...
        and validation_results[0].additional_metrics
        and "distribution_comparison" in validation_results[0].additional_metrics
    ):
        print("\nDistribution tests:")
        for metric_name, dist_data in (
            validation_results[0].additional_metrics["distribution_comparison"].items()
        ):
