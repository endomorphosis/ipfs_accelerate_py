    ...
    for hw_idx, hardware_type in enumerate(hardware_types):
        for model_idx, model_type in enumerate(model_types):
            for i in range(num_results_per_combination):
                ...
                # Calculate timestamp
                timestamp = timestamp_base + timestamp_increment * (
                    hw_idx * num_model_types * num_results_per_combination
                    + model_idx * num_results_per_combination
                    + i
                )
...
def test_enhanced_reporting():
    ...
    validation_results = create_test_data(
        num_hardware_types=3, num_model_types=4, num_results_per_combination=3
    )
    ...
    reporter.export_report(
        validation_results=validation_results, output_path=csv_path, format="csv"
    )
...
def test_comparative_analysis():
    ...
    for vr in baseline_results:
        ...
        for metric, comparison in vr.metrics_comparison.items():
            ...
            new_sim_value = hw_value * (
                1 + (new_mape / 100.0) * (1 if hw_value < comparison["simulation_value"] else -1)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", type=str, default="test_reports", help="Directory to save test reports"
    )
    parser.add_argument(
        "--test-type",
        type=str,
        choices=["basic", "specialized", "comparative", "all"],
        default="basic",
