    ...
    def _calculate_mape(self):
        ...
        for col in self.real_data.select_dtypes(include=["number"]).columns:
            if col in self.sim_data.columns:
                ...
                if mask.any():
                    result[col] = (
                        np.mean(np.abs((real_values[mask] - sim_values[mask]) / real_values[mask]))
                        * 100
                    )
                else:
                    result[col] = np.nan
...
class SimulationValidator:
    ...
    def __init__(
        self, db_connection=None, real_data_source=None, sim_data_source=None, debug=False
    ):
    ...
    def validate_simulation(self, model_name=None, hardware_type=None, metrics=None, tolerance=0.1):
        """
    ...
    def _generate_sample_data(self, model_name=None, hardware_type=None, metrics=None):
        ...
        for hw in hardware_types:
            ...
            for batch_size in batch_sizes:
                ...
                memory = (
                    memory_base * (batch_size**0.5) * memory_bias * (1 + np.random.normal(0, 0.1))
                )
    ...
    def _align_data(self, real_data, sim_data):
        ...
        group_cols = [
            col for col in group_cols if col in real_data.columns and col in sim_data.columns
        ]
        metric_cols = [
            col for col in metric_cols if col in real_data.columns and col in sim_data.columns
        ]
