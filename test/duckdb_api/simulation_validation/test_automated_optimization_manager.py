...
class TestAutomatedOptimizationManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_metrics = {
            "query_time": {"value": 300.0, "unit": "ms", "status": "good"},
            "storage_size": {
                "value": 419430400,  # 400 MB
                "unit": "bytes",
                "status": "good",
            },
            "index_efficiency": {"value": 85.0, "unit": "percent", "status": "good"},
            "vacuum_status": {"value": 75.0, "unit": "percent", "status": "good"},
            "compression_ratio": {"value": 3.5, "unit": "ratio", "status": "good"},
            "read_efficiency": {"value": 230.0, "unit": "records/second", "status": "good"},
            "write_efficiency": {"value": 180.0, "unit": "records/second", "status": "good"},
            "cache_performance": {"value": 65.0, "unit": "percent", "status": "good"},
        }
    ...
    def test_initialization(self):
        ...
        custom_thresholds = {"query_time": {"warning": 400.0, "error": 800.0}}
        custom_actions = {"query_time": ["create_indexes"]}

        manager = AutomatedOptimizationManager(
            db_optimizer=self.mock_optimizer, thresholds=custom_thresholds, actions=custom_actions
        )
    ...
    def test_logging_configuration(self):
        ...
        manager_with_log = AutomatedOptimizationManager(
            db_optimizer=self.mock_optimizer, log_file=self.log_file
        )
    ...
    def test_detect_performance_issues_with_issues(self):
        ...
        metrics_with_issues["storage_size"]["value"] = (
            1073741824 + 1
        )  # Exceeds error threshold (1 GB)
        ...
        self.assertEqual(
            issues["query_time"]["recommended_actions"], self.manager.actions["query_time"]
        )
        self.assertEqual(
            issues["storage_size"]["recommended_actions"], self.manager.actions["storage_size"]
        )
    ...
    def test_execute_actions(self):
        ...
        for action in actions:
            ...
            self.assertEqual(
                results[action]["status"], "success", f"Action {action} should succeed"
            )
    ...
    def test_run_comprehensive_optimization(self):
        ...
        self.assertEqual(
            result["improvements"]["query_time"]["improvement"], 50.0
        )  # 50% improvement
    ...
    def test_get_optimization_recommendations(self):
        ...
        self.assertEqual(
            result["recommendations"]["storage_size"]["recommended_actions"],
    ...
    def test_scheduled_tasks(self):
        ...
        manager = AutomatedOptimizationManager(
            db_optimizer=self.mock_optimizer, enable_scheduled_tasks=True
        )
        ...
        task = manager.schedule_task(
            task_name="custom_task", actions=["optimize_database"], interval_hours=12
        )
    ...
    def test_check_scheduled_tasks(self):
        ...
        self.assertGreater(
            self.manager.scheduled_tasks["immediate_task"]["next_run"], datetime.now()
        )
    ...
    def test_update_metrics_history(self):
        ...
        for metric_name in self.sample_metrics:
            ...
            self.assertEqual(
                history[metric_name][0]["value"], self.sample_metrics[metric_name]["value"]
            )
    ...
    def test_get_optimization_manager_utility(self):
        ...
        config = {
            "thresholds": {"query_time": {"warning": 400.0, "error": 800.0}},
            "check_interval": 300,
        ...
        manager = get_optimization_manager(
            db_optimizer=self.mock_optimizer, config_file=config_file
        )
        ...
        manager = get_optimization_manager(
            db_optimizer=self.mock_optimizer, config_file="nonexistent.json", auto_apply=True
        )
