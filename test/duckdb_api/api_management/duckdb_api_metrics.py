...
class DuckDBAPIMetricsRepository:
    ...
    def __init__(
        ...
        if not DUCKDB_AVAILABLE:
            raise ImportError(
                "DuckDB is required for DuckDBAPIMetricsRepository. Install with: pip install duckdb pandas"
            )
    ...
    def _initialize_tables(self) -> None:
        ...
        try:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_historical_metrics_api_name ON historical_metrics(api_name)"
            )
            self.conn.execute(
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_predictions_api_name ON predictions(api_name)"
            )
            self.conn.execute(
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_anomalies_api_name ON anomalies(api_name)"
            )
            self.conn.execute(
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_recommendations_api_name ON recommendations(api_name)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_recommendations_impact ON recommendations(impact)"
            )
            self.conn.execute(
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_comparative_metrics_metric_type ON comparative_metrics(metric_type)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_comparative_metrics_timestamp ON comparative_metrics(timestamp)"
            )
    ...
    def save_historical_metrics_batch(self, metrics: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def save_predictions_batch(self, predictions: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def save_anomalies_batch(self, anomalies: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def save_recommendations_batch(self, recommendations: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def update_recommendation_status(self, recommendation_id: str, new_status: str) -> bool:
        """
    ...
    def get_recommendations_formatted(
        ...
        try:
            ...
            raw_recommendations = self.get_recommendations(
                api_name=api_name, status=status, min_impact=min_impact, effort=effort, limit=limit
            )
    ...
    def save_comparative_metrics_batch(self, metrics: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def get_comparative_metrics_formatted(
        ...
        try:
            ...
            raw_metrics = self.get_comparative_metrics(
                metric_type=metric_type, start_time=start_time, end_time=end_time, limit=limit
            )
            ...
            for metric in raw_metrics:
                ...
                formatted_metrics[m_type].append(
                    {"timestamp": metric["timestamp"], "values": metric["values"]}
                )
...
if __name__ == "__main__":
    ...
    if args.generate_sample:
        ...
        timestamps = [
            now - datetime.timedelta(days=30) + datetime.timedelta(hours=i) for i in range(24 * 30)
        ]
        ...
        for api in apis:
            for metric in metrics:
                ...
                last_value = repo.get_historical_metrics(api_name=api, metric_type=metric, limit=1)[
                    0
                ]["value"]
        ...
        for api in apis:
            for metric in metrics:
                ...
                historical_data = repo.get_historical_metrics(api_name=api, metric_type=metric)
        ...
        for metric in metrics:
            ...
            for i in range(0, len(timestamps), 24):
                ...
                if values:
                    batch.append(
                        {"metric_type": metric, "timestamp": ts.isoformat(), "values": values}
                    )
    ...
    if args.export:
        ...
        print(
            f"Historical metrics: {sum(len(data['historical_data'].get(m, {}).get(a, [])) for m in metrics for a in apis)}"
        )
        print(
            f"Predictions: {sum(len(data['predictions'].get(m, {}).get(a, [])) for m in metrics for a in apis)}"
        )
        print(
            f"Anomalies: {sum(len(data['anomalies'].get(m, {}).get(a, [])) for m in metrics for a in apis)}"
        )
        print(f"Recommendations: {sum(len(data['recommendations'].get(a, [])) for a in apis)}")
        print(
            f"Comparative metrics: {sum(len(data['comparative_data'].get(m, [])) for m in metrics)}"
        )
