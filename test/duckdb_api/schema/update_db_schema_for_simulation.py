...
def update_schema(db_path: str, force: bool = False) -> bool:
    ...
    try:
        ...
        if not result:
            logger.error(
                "performance_results table not found. Please initialize the database first."
            )
            return False
        ...
        if not force:
            ...
            has_simulation_columns = (
                "is_simulated" in column_names and "simulation_reason" in column_names
            )
        ...
        if result:
            ...
            if has_simulation_columns and not force:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error adding simulation_reason column (may already exist): {e}"
                    )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force update even if columns already exist"
    )
