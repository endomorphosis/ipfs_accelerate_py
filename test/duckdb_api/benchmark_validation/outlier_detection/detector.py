...
class StatisticalOutlierDetector(OutlierDetector):
    ...
    def validate(
        ...
        if len(reference_results) < self.config["min_samples"]:
            logger.warning(
                f"Not enough reference results for outlier detection, need at least {self.config['min_samples']}"
            )
            return ValidationResult(
                benchmark_result=benchmark_result,
                status=ValidationStatus.WARNING,
        ...
        outlier_scores = self.calculate_outlier_score(
            benchmark_result=benchmark_result, reference_results=reference_results, metrics=metrics
        )
        ...
        sample_size_factor = min(
            1.0, len(reference_results) / 30
        )  # More samples = higher confidence
        outlier_magnitude = max([abs(score) for score in outlier_scores.values()], default=0)
        outlier_factor = max(
            0, 1 - (outlier_magnitude / (2 * threshold))
        )  # Higher outlier score = lower confidence
    ...
    def detect_outliers(
        ...
        for group_key, group_results in grouped_results.items():
            ...
            for metric, values in metric_values.items():
                ...
                if std > 0:
                    zscores = [
                        (result, (value - mean) / std) for result, value in zip(results, data)
                    ]
    ...
    def calculate_outlier_score(
        ...
        filtered_refs = [
            ...
            if ref.model_id == benchmark_result.model_id
            and ref.hardware_id == benchmark_result.hardware_id
        ]
