...
try:
    ...
    from data.duckdb.visualization.advanced_visualization.viz_time_series import (
...
class ValidationReporterImpl(ValidationReporter):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if ADVANCED_VIZ_AVAILABLE:
            self.viz_components["heatmap"] = HardwareHeatmapVisualization(
    ...
    def export_report(
        ...
        report_content = self.generate_report(
            validation_results, report_format, include_visualizations
        )
    ...
    def _prepare_report_data(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        for val_result in validation_results:
            ...
            benchmark_type_stats[benchmark_type]["confidence_scores"].append(
                val_result.confidence_score
            )
            ...
            validation_level_stats[validation_level]["confidence_scores"].append(
                val_result.confidence_score
            )
    ...
    def _generate_html_report(
        ...
        for benchmark_type, stats in report_data["benchmark_type_stats"].items():
            ...
            benchmark_type_summary += (
                f"<td class='status-VALID'>{stats['status_counts']['VALID']}</td>"
            )
            benchmark_type_summary += (
                f"<td class='status-WARNING'>{stats['status_counts']['WARNING']}</td>"
            )
            benchmark_type_summary += (
                f"<td class='status-INVALID'>{stats['status_counts']['INVALID']}</td>"
            )
            benchmark_type_summary += (
                f"<td class='status-ERROR'>{stats['status_counts']['ERROR']}</td>"
            )
            benchmark_type_summary += (
                f"<td class='status-PENDING'>{stats['status_counts']['PENDING']}</td>"
            )
            benchmark_type_summary += f"</tr>"
        ...
        visualizations = ""
        if include_visualizations and (
            PLOTLY_AVAILABLE or MATPLOTLIB_AVAILABLE or ADVANCED_VIZ_AVAILABLE
        ):
            ...
            if PLOTLY_AVAILABLE and PANDAS_AVAILABLE:
                ...
                confidence_scores = [
                    val_result.confidence_score for val_result in report_data["validation_results"]
                ]
                confidence_statuses = [
                    val_result.status.name for val_result in report_data["validation_results"]
                ]

                df = pd.DataFrame(
                    {"confidence_score": confidence_scores, "status": confidence_statuses}
                )
            ...
            if PLOTLY_AVAILABLE:
                ...
                valid_counts = [
                    stats["status_counts"]["VALID"]
                    for stats in report_data["benchmark_type_stats"].values()
                ]
                warning_counts = [
                    stats["status_counts"]["WARNING"]
                    for stats in report_data["benchmark_type_stats"].values()
                ]
                invalid_counts = [
                    stats["status_counts"]["INVALID"]
                    for stats in report_data["benchmark_type_stats"].values()
                ]
                error_counts = [
                    stats["status_counts"]["ERROR"]
                    for stats in report_data["benchmark_type_stats"].values()
                ]
                pending_counts = [
                ...
                fig = go.Figure(
                    data=[
                        go.Bar(
                            name="Valid", x=benchmark_types, y=valid_counts, marker_color="#27ae60"
                        ),
                        go.Bar(
                            name="Warning",
                            x=benchmark_types,
                        ...
                        go.Bar(
                            name="Error", x=benchmark_types, y=error_counts, marker_color="#c0392b"
                        ),
                        go.Bar(
                            name="Pending",
                            x=benchmark_types,
            ...
            if PLOTLY_AVAILABLE:
                ...
                mean_confidences = [
                    ...
                    for stats in report_data["validation_level_stats"].values()
                ]
        ...
        for i, val_result in enumerate(report_data["validation_results"]):
            ...
            detailed_results += (
                f"<td class='status-{val_result.status.name}'>{val_result.status.name}</td>"
            )
    ...
    def _generate_markdown_report(
        ...
        for benchmark_type, stats in report_data["benchmark_type_stats"].items():
            ...
            markdown += (
                f"{stats['status_counts']['VALID']} | {stats['status_counts']['WARNING']} | "
            )
            markdown += (
                f"{stats['status_counts']['INVALID']} | {stats['status_counts']['ERROR']} | "
            )
        ...
        for validation_level, stats in report_data["validation_level_stats"].items():
            markdown += (
                f"| {validation_level} | {stats['count']} | {stats['mean_confidence']:.2f} | "
            )
            markdown += (
                f"{stats['status_counts']['VALID']} | {stats['status_counts']['WARNING']} | "
            )
    ...
    def _generate_text_report(self, report_data: Dict[str, Any]) -> str:
        ...
        for benchmark_type, stats in report_data["benchmark_type_stats"].items():
            text += (
                f"{benchmark_type:<20} {stats['count']:<8} {stats['mean_confidence']:.2f}{'':<6} "
            )
        ...
        for validation_level, stats in report_data["validation_level_stats"].items():
            text += (
                f"{validation_level:<20} {stats['count']:<8} {stats['mean_confidence']:.2f}{'':<6} "
            )
            text += f"{stats['status_counts']['VALID']:<6} {stats['status_counts']['WARNING']:<8} "
            text += f"{stats['status_counts']['INVALID']:<8} {stats['status_counts']['ERROR']:<6} "
            text += f"{stats['status_counts']['PENDING']:<8}\n"
    ...
    def create_visualization(
        ...
        **kwargs,
        ...
        if ADVANCED_VIZ_AVAILABLE and visualization_type in [
            "validation_heatmap",
            "benchmark_comparison",
        ]:
            return self._create_advanced_visualization(
                validation_results, visualization_type, output_path, title, **kwargs
            )
        ...
        **kwargs,
        ...
        if visualization_type == "validation_heatmap":
            ...
            return self.viz_components["heatmap"].create_hardware_heatmap(
                data=data, metric=metric, output_path=output_path, title=title, **kwargs
            )
        ...
        **kwargs,
        ...
        try:
            if visualization_type == "confidence_distribution":
                ...
                confidence_scores = [
                    val_result.confidence_score for val_result in validation_results
                ]
                statuses = [val_result.status.name for val_result in validation_results]

                # Create a DataFrame for the data
                if PANDAS_AVAILABLE:
                    df = pd.DataFrame({"confidence_score": confidence_scores, "status": statuses})
                    ...
                    for status, scores in status_groups.items():
                        fig.add_trace(
                            go.Histogram(
                                x=scores, name=status, marker_color=colors.get(status, "#7f8c8d")
                            )
                        )
    ...
    def _prepare_validation_heatmap_data(
