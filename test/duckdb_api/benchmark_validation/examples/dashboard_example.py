...
def create_sample_validation_results(count: int = 50) -> List[ValidationResult]:
    ...
    for i in range(count):
        ...
        status = random.choices(
            list(status_weights.keys()), weights=list(status_weights.values()), k=1
        )[0]
...
def create_basic_dashboard():
    ...
    dashboard = ValidationDashboard(
        config={"output_directory": "output", "dashboard_directory": "dashboards", "theme": "light"}
    )
    ...
    if dashboard_path:
        ...
        dashboard_url = dashboard.get_dashboard_url(
            dashboard_name="sample_validation_dashboard", base_url="http://localhost:8080"
        )
        logger.info(f"Dashboard URL: {dashboard_url}")

        # Export dashboard to different formats
        html_path = dashboard.export_dashboard(
            dashboard_name="sample_validation_dashboard", export_format="html"
        )
        logger.info(f"Exported dashboard as HTML to {html_path}")

        markdown_path = dashboard.export_dashboard(
            dashboard_name="sample_validation_dashboard", export_format="markdown"
        )
...
def create_comparison_dashboard():
    ...
    if dashboard_path:
        ...
        # Display URL
        dashboard_url = dashboard.get_dashboard_url(
            dashboard_name="validation_comparison_dashboard", base_url="http://localhost:8080"
        )
...
def update_dashboard():
    ...
    # Create dashboard instance
    dashboard = ValidationDashboard(
        config={"output_directory": "output", "dashboard_directory": "dashboards"}
    )
...
def register_with_monitoring():
    ...
    success = dashboard.register_with_monitoring_dashboard(
        dashboard_name="sample_validation_dashboard", page="validation", position="below"
    )
    ...
    if success:
        ...
        try:
            ...
            logger.error(
                "Monitoring integration is not available. Check if distributed_testing module is installed"
            )
