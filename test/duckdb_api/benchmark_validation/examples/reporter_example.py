...
def create_sample_validation_results(num_results=20):
    ...
    benchmark_types = [
        ...
        BenchmarkType.COMPATIBILITY,
        BenchmarkType.INTEGRATION,
        BenchmarkType.WEB_PLATFORM,
    ]
    model_ids = [
        "bert-base-uncased",
...
def main():
    ...
    reporter.export_report(
        validation_results=validation_results, output_path=md_path, report_format="markdown"
    )
    ...
    reporter.export_report(
        validation_results=validation_results, output_path=json_path, report_format="json"
    )
