...
class ReproducibilityValidator(BaseReproducibilityValidator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.config.setdefault(
            "error_metrics",
            ["average_error", "std_deviation", "coefficient_of_variation", "min_max_range"],
        )
    ...
    def validate_batch(
        ...
        """
        logger.info(
            f"Batch validating {len(benchmark_results)} benchmark results for reproducibility"
        )
            ...
            if len(group_results) < self.config["min_runs"]:
                for result in group_results:
                    validation_result = ValidationResult(
                        ...
                        recommendations=[
                            f"Perform at least {self.config['min_runs']} benchmark runs"
                        ],
                        validator_id=self.validator_id,
                    )
    ...
    def validate_reproducibility(
        ...
        metrics = (
            reference_data.get("metrics", self.config["default_metrics"])
            if reference_data
        ...
        for metric, values in metric_values.items():
            ...
            metric_stats[metric] = {
                ...
                "range_percent": float((max_val - min_val) / mean * 100)
                if mean > 0
                else float("inf"),
        ...
        if is_reproducible:
            ...
            non_reproducible_metrics = [
                metric for metric, stats in metric_stats.items() if not stats["is_reproducible"]
            ]
    ...
    def calculate_reproducibility_score(self, benchmark_results: List[BenchmarkResult]) -> float:
        """
        ...
        for metric, cv in cv_scores.items():
            if cv <= cv_threshold:
                ...
                metric_scores[metric] = max(
                    0.0, 0.8 * (1 - (cv - cv_threshold) / (5 * cv_threshold))
                )
    ...
    def calculate_confidence(self, validation_result: ValidationResult) -> float:
        ...
        if (
            "reproducibility" not in validation_result.validation_metrics
            or validation_result.validation_metrics["reproducibility"].get("status") != "completed"
        ):
            return 0.5
        ...
        cv_values = [
            stats.get("coefficient_of_variation", float("inf")) for stats in metric_stats.values()
        ]
