...
from data.duckdb.benchmark_validation.framework import (
    ComprehensiveBenchmarkValidation,
...
def load_batch(input_path: str) -> List[BenchmarkResult]:
    ...
    try:
        ...
        if os.path.isdir(input_path):
            files = [
                os.path.join(input_path, f) for f in os.listdir(input_path) if f.endswith(".json")
            ]
        else:
...
def validate_cmd(args):
    ...
    if os.path.isdir(args.input) or "*" in args.input:
        ...
        validation_result = framework.validate(
            benchmark_result=benchmark_result, validation_level=validation_level
        )
...
def reproducibility_cmd(args):
    ...
    for key, results in grouped_results.items():
        if len(results) < 3:
            print(
                f"  Skipping group {key}: not enough results (have {len(results)}, need at least 3)"
            )
            continue
        ...
        reproducibility_result = framework.validate_reproducibility(
            benchmark_results=results, validation_level=validation_level
        )
...
def certify_cmd(args):
    ...
    if args.validation_results:
        if os.path.isdir(args.validation_results) or "*" in args.validation_results:
            ...
            files = (
                glob.glob(args.validation_results)
                if "*" in args.validation_results
                else [
...
def stability_cmd(args):
    ...
    stability_analysis = framework.track_benchmark_stability(
        benchmark_results=benchmark_results, metric=args.metric, time_window_days=args.time_window
    )
...
def main():
    ...
    validate_parser.add_argument(
        "--input", type=str, required=True, help="Path to benchmark result JSON or directory"
    )
    validate_parser.add_argument(
        "--level",
        type=str,
        default="STANDARD",
        help="Validation level (MINIMAL, STANDARD, STRICT, CERTIFICATION)",
    )
    validate_parser.add_argument(
        "--detect-outliers", action="store_true", help="Detect outliers in batch validation"
    )
    ...
    reproducibility_parser = subparsers.add_parser(
        "reproducibility", help="Validate reproducibility of benchmark results"
    )
    reproducibility_parser.add_argument(
        "--input", type=str, required=True, help="Path to directory of benchmark result JSONs"
    )
    reproducibility_parser.add_argument(
        "--level",
        type=str,
    ...
    reproducibility_parser.add_argument(
        "--output", type=str, help="Path to save reproducibility result"
    )
    ...
    certify_parser.add_argument(
        "--input", type=str, required=True, help="Path to benchmark result JSON"
    )
    certify_parser.add_argument(
        "--validation-results", type=str, help="Path to validation result JSON or directory"
    )
    certify_parser.add_argument(
        "--level",
        type=str,
        default="auto",
        help="Certification level (basic, standard, advanced, gold, auto)",
    )
    ...
    report_parser.add_argument(
        "--input", type=str, required=True, help="Path to validation result JSON or directory"
    )
    report_parser.add_argument(
        "--format", type=str, default="html", help="Report format (html, markdown, json, text)"
    )
    report_parser.add_argument(
        "--visualizations", action="store_true", help="Include visualizations in report"
    )
    ...
    stability_parser.add_argument(
        "--metric", type=str, default="average_latency_ms", help="Metric to track stability for"
    )
