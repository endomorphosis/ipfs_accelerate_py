...
class TestValidationDashboard(unittest.TestCase):
    ...
    def test_create_dashboard_basic(self):
        ...
        with patch(
            "duckdb_api.benchmark_validation.visualization.dashboard.ADVANCED_VIZ_AVAILABLE", False
        ):
            ...
            dashboard_path = dashboard.create_dashboard(
                validation_results=self.validation_results, dashboard_name=dashboard_name
            )
    ...
    def test_export_dashboard(self):
        ...
        for format in export_formats:
            output_path = self.dashboard.export_dashboard(
                dashboard_name=dashboard_name, export_format=format
            )
    ...
    @patch(
        "duckdb_api.benchmark_validation.visualization.dashboard.MONITORING_DASHBOARD_AVAILABLE",
        True,
    )
    @patch(
        "duckdb_api.benchmark_validation.visualization.dashboard.VisualizationDashboardIntegration"
    )
    def test_register_with_monitoring(self, mock_visualization_integration):
        ...
        dashboard = ValidationDashboard(config={**self.config, "monitoring_integration": True})

        # Create list_dashboards mock to return our test dashboard
        dashboard.list_dashboards = MagicMock(
            return_value=[
                {
                    "name": "test_register_dashboard",
        ...
        success = dashboard.register_with_monitoring_dashboard(
            dashboard_name="test_register_dashboard", page="validation", position="below"
        )
