...
class DuckDBValidationRepository(ValidationRepository):
    ...
    def save_validation_result(self, validation_result: ValidationResult) -> str:
        """
        ...
        try:
            ...
            existing = self.conn.execute(
                "SELECT id FROM validation_results WHERE id = ?", [validation_result.id]
            ).fetchone()
    ...
    def load_validation_result(self, result_id: str) -> Optional[ValidationResult]:
        """
        ...
        try:
            ...
            if not benchmark_result:
                logger.error(
                    f"Associated benchmark result {result['benchmark_result_id']} not found"
                )
                return None
            ...
            if filters:
                for key, value in filters.items():
                    ...
                    if key.startswith("benchmark_result."):
                        field = key.split(".", 1)[1]
                        conditions.append(
                            f"vr.benchmark_result_id IN (SELECT result_id FROM benchmark_results WHERE {field} = ?)"
                        )
                        params.append(value)
                    elif key == "status":
    ...
    def save_benchmark_result(self, benchmark_result: BenchmarkResult) -> str:
        """
    ...
    def load_benchmark_result(self, result_id: str) -> Optional[BenchmarkResult]:
        """
    ...
    def save_certification(self, certification: Dict[str, Any]) -> str:
        """
        ...
        try:
            # Convert JSON fields
            certification_requirements_json = json.dumps(
    ...
    def load_certification(self, certification_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            certification = {
                ...
                "certification_timestamp": result["certification_timestamp"].isoformat()
                if result["certification_timestamp"]
                else None,
            ...
            if filters:
                for key, value in filters.items():
                    ...
                    if key == "model_id" or key == "hardware_id":
                        conditions.append(
                            f"c.benchmark_id IN (SELECT result_id FROM benchmark_results WHERE {key} = ?)"
                        )
                        params.append(value)
                    elif key == "since":
