...
def main():
    ...
    reproducibility_result = framework.validate_reproducibility(
        benchmark_results=regular_results, validation_level=ValidationLevel.STANDARD
    )
    ...
    certification = framework.certify_benchmark(
        ...
        validation_results=[reproducibility_result]
        + [vr for vr in validation_results if vr.benchmark_result.result_id != "benchmark-outlier"],
    ...
    stability_analysis = framework.track_benchmark_stability(
        benchmark_results=regular_results, metric="average_latency_ms", time_window_days=30
    )
