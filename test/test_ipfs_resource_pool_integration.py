...
try:
    ...
    logger.error(
        "IPFSAccelerateWebIntegration not available. Make sure fixed_web_platform module is properly installed"
    )
...
class IPFSResourcePoolTester:
    ...
    def __init__(self, args):
        ...
        if (
            hasattr(args, "db_path")
            and args.db_path
            and not args.disable_db
            and REQUIRED_MODULES["duckdb"]
        ):
            try:
    ...
    async def initialize_resource_pool(self):
        ...
        if not REQUIRED_MODULES["resource_pool_integration"]:
            logger.error(
                "Cannot initialize resource pool: IPFSAccelerateWebIntegration not available"
            )
        ...
        try:
            ...
            self.resource_pool_integration = IPFSAccelerateWebIntegration(
                ...
                if hasattr(self.args, "db_path") and not getattr(self.args, "disable_db", False)
                else None,
                smart_fallback=True,
            )
    ...
    async def _initialize_legacy_resource_pool(self):
        ...
        if not REQUIRED_MODULES["resource_pool_bridge"]:
            logger.error(
                "Cannot initialize legacy resource pool: ResourcePoolBridgeIntegration not available"
            )
            return False
    ...
    async def test_model_enhanced(self, model_name, model_type):
        ...
        try:
            ...
            quantization = None
            if (
                hasattr(self.args, "precision")
                and self.args.precision != 16
                or hasattr(self.args, "mixed_precision")
                and self.args.mixed_precision
            ):
                quantization = {
                    "bits": self.args.precision if hasattr(self.args, "precision") else 16,
                    "mixed_precision": self.args.mixed_precision
                    if hasattr(self.args, "mixed_precision")
                    else False,
                }
            ...
            if (
                hasattr(self.args, "optimize_audio")
                and self.args.optimize_audio
                or hasattr(self.args, "all_optimizations")
                and self.args.all_optimizations
            ):
                optimizations["compute_shaders"] = True
                hardware_preferences["compute_shaders"] = True
            if (
                hasattr(self.args, "shader_precompile")
                and self.args.shader_precompile
                or hasattr(self.args, "all_optimizations")
                and self.args.all_optimizations
            ):
            ...
            if (
                hasattr(self.args, "parallel_loading")
                and self.args.parallel_loading
                or hasattr(self.args, "all_optimizations")
                and self.args.all_optimizations
            ):
            ...
            logger.debug(
                f"Final hardware_preferences for model {model_name}: {hardware_preferences}"
            )
            ...
            result = self.resource_pool_integration.run_inference(
                ...
                track_metrics=True,
                store_in_db=hasattr(self.args, "db_path")
                and self.args.db_path
                and not getattr(self.args, "disable_db", False),
            ...
            # Debug final flags
            logger.debug(
                f"Final test result flags for {model_name}: compute_shader_optimized={compute_shader_optimized}, precompile_shaders={precompile_shaders}, parallel_loading={parallel_loading}"
            )
            ...
            logger.info(
                f"Enhanced resource pool test completed in {execution_time:.2f}s: {model_name}"
            )
            return test_result
        except Exception as e:
    ...
    def _create_test_input(self, model_type):
        ...
        if model_type == "text_embedding":
            ...
            try:
                ...
                return {
                    "pixel_values": [
                        [[0.5 for _ in range(224)] for _ in range(224)] for _ in range(3)
                    ]
                }
    ...
    def _store_test_result(self, test_result):
        ...
        try:
            ...
            logger.info(
                f"Test result stored in database for {test_result.get('model_name', 'unknown')}"
            )
        except Exception as e:
        ...
        try:
            ...
            for i, result in enumerate(concurrent_results):
                if i < len(loaded_models):
                    ...
                    if hasattr(model, "compute_shader_optimized"):
                        logger.debug(
                            f"Model {model_name} optimization flags directly from model object:"
                        )
                        logger.debug(
                            f"  compute_shader_optimized: {model.compute_shader_optimized}"
                        )
                        logger.debug(f"  precompile_shaders: {model.precompile_shaders}")
                        logger.debug(f"  parallel_loading: {model.parallel_loading}")
                    ...
                    if isinstance(result, dict):
                        compute_shader_optimized = result.get(
                            "compute_shader_optimized",
                            model_info.get(
                                "compute_shader_optimized",
                                getattr(model, "compute_shader_optimized", False),
                            ),
                        )
                        precompile_shaders = result.get(
                            "precompile_shaders",
                            model_info.get(
                                "precompile_shaders", getattr(model, "precompile_shaders", False)
                            ),
                        )
                        parallel_loading = result.get(
                            "parallel_loading",
                            model_info.get(
                                "parallel_loading", getattr(model, "parallel_loading", False)
                            ),
                        )

                        logger.debug(
                            f"Result contains optimization flags: {result.get('compute_shader_optimized', None)}, {result.get('precompile_shaders', None)}, {result.get('parallel_loading', None)}"
                        )
                    ...
                    logger.debug(
                        f"Test result flags for {model_name}: {compute_shader_optimized}, {precompile_shaders}, {parallel_loading}"
                    )
    ...
    async def run_benchmark_enhanced(self):
        ...
        try:
            ...
            with open(filename, "w") as f:
                json.dump({"results": benchmark_results, "summary": summary}, f, indent=2)
    ...
    def _calculate_enhanced_benchmark_summary(self, benchmark_results):
        ...
        summary["success_rate"] = {
            "single_model": sum(
                1 for r in benchmark_results["single_model"] if r.get("success", False)
            )
            / len(benchmark_results["single_model"])
            if benchmark_results["single_model"]
            else 0,
            "concurrent_execution": sum(
                1 for r in benchmark_results["concurrent_execution"] if r.get("success", False)
            )
            / len(benchmark_results["concurrent_execution"])
            if benchmark_results["concurrent_execution"]
            else 0,
            "distributed_execution": sum(
                1 for r in benchmark_results["distributed_execution"] if r.get("success", False)
            )
            / len(benchmark_results["distributed_execution"])
            if benchmark_results["distributed_execution"]
            else 0,
        }
        ...
        summary["real_hardware_rate"] = {
            "single_model": sum(
                1
                for r in benchmark_results["single_model"]
                if r.get("is_real_implementation", False)
            )
            / len(benchmark_results["single_model"])
            if benchmark_results["single_model"]
            else 0,
            "concurrent_execution": sum(
                1
                for r in benchmark_results["concurrent_execution"]
                if r.get("is_real_implementation", False)
            )
            / len(benchmark_results["concurrent_execution"])
            if benchmark_results["concurrent_execution"]
            ...
            "distributed_execution": sum(
                1
                for r in benchmark_results["distributed_execution"]
                if r.get("is_real_implementation", False)
            )
            / len(benchmark_results["distributed_execution"])
            if benchmark_results["distributed_execution"]
        ...
        summary["optimization_usage"] = {
            "compute_shaders": sum(
                1
                for r in benchmark_results["single_model"]
                if r.get("compute_shader_optimized", False)
            )
            / len(benchmark_results["single_model"])
            if benchmark_results["single_model"]
            else 0,
            "precompile_shaders": sum(
                1 for r in benchmark_results["single_model"] if r.get("precompile_shaders", False)
            )
            / len(benchmark_results["single_model"])
            if benchmark_results["single_model"]
            else 0,
            "parallel_loading": sum(
                1 for r in benchmark_results["single_model"] if r.get("parallel_loading", False)
            )
            / len(benchmark_results["single_model"])
            if benchmark_results["single_model"]
            else 0,
        ...
        if benchmark_results["concurrent_execution"] and benchmark_results["single_model"]:
            ...
            if single_time > 0:
                ...
                concurrent_items_per_second = (
                    len(benchmark_results["concurrent_execution"]) / concurrent_time
                )
                improvement_factor = (
                    concurrent_items_per_second / single_items_per_second
                    if single_items_per_second > 0
                    else 0
                )
        ...
        if benchmark_results["distributed_execution"] and benchmark_results["concurrent_execution"]:
            ...
            if concurrent_time > 0:
                ...
                distributed_improvement = (
                    concurrent_time / distributed_time if distributed_time > 0 else 0
                )
    ...
    def _print_enhanced_benchmark_summary(self, summary):
        ...
        print(
            f"  Concurrent Execution:  {summary['avg_execution_time']['concurrent_execution']:.3f}"
        )
        if "distributed_execution" in summary["avg_execution_time"]:
            print(
                f"  Distributed Execution: {summary['avg_execution_time']['distributed_execution']:.3f}"
            )

        print("\nSuccess Rate:")
        print(f"  Single Model:          {summary['success_rate']['single_model'] * 100:.1f}%")
        print(
            f"  Concurrent Execution:  {summary['success_rate']['concurrent_execution'] * 100:.1f}%"
        )
        if "distributed_execution" in summary["success_rate"]:
            print(
                f"  Distributed Execution: {summary['success_rate']['distributed_execution'] * 100:.1f}%"
            )

        print("\nReal Hardware Rate:")
        print(
            f"  Single Model:          {summary['real_hardware_rate']['single_model'] * 100:.1f}%"
        )
        print(
            f"  Concurrent Execution:  {summary['real_hardware_rate']['concurrent_execution'] * 100:.1f}%"
        )
        if "distributed_execution" in summary["real_hardware_rate"]:
            print(
                f"  Distributed Execution: {summary['real_hardware_rate']['distributed_execution'] * 100:.1f}%"
            )

        print("\nOptimization Usage:")
        print(
            f"  Compute Shaders:       {summary['optimization_usage']['compute_shaders'] * 100:.1f}%"
        )
        print(
            f"  Shader Precompilation: {summary['optimization_usage']['precompile_shaders'] * 100:.1f}%"
        )
        print(
            f"  Parallel Loading:      {summary['optimization_usage']['parallel_loading'] * 100:.1f}%"
        )
    ...
    def _generate_markdown_report(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            for method, results in methods.items():
                ...
                f.write(
                    f"- {method.replace('_', ' ').title()}: {len(results)} tests, {method_successful} successful ({method_successful / len(results) * 100:.1f}%)\n"
                )
            ...
            for method, results in methods.items():
                ...
                f.write(
                    "| Model | Type | Platform | Browser | Success | Real HW | Execution Time (s) |\n"
                )
                f.write(
                    "|-------|------|----------|---------|---------|---------|--------------------|\n"
                )
                ...
                for result in sorted(results, key=lambda r: r.get("model_name", "")):
                    ...
                    f.write(
                        f"| {model_name} | {model_type} | {platform} | {browser} | {success} | {real_hw} | {execution_time} |\n"
                    )
            ...
            f.write(
                "| Model | Type | Compute Shaders | Shader Precompilation | Parallel Loading | Precision | Mixed Precision |\n"
            )
            f.write(
                "|-------|------|-----------------|------------------------|------------------|-----------|----------------|\n"
            )
            ...
            for result in sorted(self.results, key=lambda r: r.get("model_name", "")):
                ...
                f.write(
                    f"| {model_name} | {model_type} | {compute_shaders} | {precompile_shaders} | {parallel_loading} | {precision}-bit | {mixed_precision} |\n"
                )
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(
        description="Test IPFS Acceleration with Enhanced WebGPU/WebNN Resource Pool Integration"
    )

    # Model selection options
    model_group = parser.add_argument_group("Model Options")
    model_group.add_argument("--model", type=str, default="bert-base-uncased", help="Model to test")
    model_group.add_argument("--models", type=str, help="Comma-separated list of models to test")
    ...
    opt_group.add_argument(
        "--optimize-audio", action="store_true", help="Enable Firefox audio optimizations"
    )
    opt_group.add_argument(
    ...
    test_group.add_argument(
        "--concurrent-models", action="store_true", help="Test multiple models concurrently"
    )
    test_group.add_argument(
        "--distributed",
        action="store_true",
    ...
    test_group.add_argument(
        "--benchmark", action="store_true", help="Run comprehensive benchmark comparing all methods"
    )
    test_group.add_argument("--batch-size", type=int, default=1, help="Batch size for inference")
    test_group.add_argument(
        "--timeout", type=float, default=60.0, help="Timeout for operations in seconds"
    )
    ...
    db_group.add_argument("--disable-db", action="store_true", help="Disable database storage")

    # IPFS options
    ipfs_group = parser.add_argument_group("IPFS Options")
    ipfs_group.add_argument("--use-ipfs", action="store_true", help="Use IPFS acceleration")
