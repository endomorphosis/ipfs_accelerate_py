...
class IOSDevice:
    ...
    def _start_debug_proxy(self) -> bool:
        ...
        try:
            ...
            # Start new proxy
            cmd = ["ios_webkit_debug_proxy", "-c", f"{self.udid}:{self.debug_proxy_port}", "-d"]
    ...
    def get_battery_info(self) -> Dict[str, Any]:
        ...
        """
        battery_info = {"level": 0, "temperature": 0.0, "state": "unknown"}
            ...
            try:
                ...
                if isinstance(data, dict) and "IOPMBattery" in data:
                    ...
                    if isinstance(battery_data, list) and len(battery_data) > 0:
                        ...
                        battery_info["state"] = (
                            "charging" if battery_info["state"] else "discharging"
                        )
    ...
    def get_thermal_info(self) -> Dict[str, float]:
        ...
        try:
            ...
            result = self._run_idevice_command(
                ["idevicediagnostics", "diagnostics", "ThermalState"]
            )
...
class IOSModelRunner:
    ...
    def _convert_onnx_to_coreml(self, onnx_path: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "onnx-coreml converter not available. Install with 'pip install onnx-coreml'"
                )
                return None
    ...
    def run_model(
        self, model_path: str, iterations: int = 10, warmup_iterations: int = 2, batch_size: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        performance_results = {
            ...
            "battery_metrics": {
                ...
                "temperature_delta": post_battery.get("temperature", 0)
                - pre_battery.get("temperature", 0),
            },
...
class IOSTestHarness:
    ...
    def connect_to_device(self) -> bool:
        ...
        try:
            self.device = IOSDevice(udid=self.udid, developer_dir=self.developer_dir)
            ...
            self.model_runner = IOSModelRunner(
                device=self.device, app_bundle_id="com.ipfsaccelerate.benchmarkapp"
            )
    ...
    def _collect_metrics(self, result: Dict[str, Any], model_name: str) -> Dict[str, Any]:
        ...
        has_neural_engine = (
            result.get("device_info", {}).get("capabilities", {}).get("neural_engine", False)
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="iOS Test Harness for IPFS Accelerate Python Framework"
    )
    ...
    if args.command == "connect":
        ...
        # Create test harness
        harness = IOSTestHarness(udid=args.udid, db_path=args.db_path, output_dir=args.output_dir)
