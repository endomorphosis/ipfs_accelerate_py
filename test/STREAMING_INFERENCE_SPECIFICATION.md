...
class StreamingInferencePipeline:
    def __init__(self, model, config=None):
        ...
        if config.get("enable_websocket", False):
            self.websocket_manager = WebSocketManager(config=config.get("websocket_config"))
        ...
        try:
            ...
            while len(generated_tokens) < max_tokens:
                ...
                optimizations = self.latency_optimizer.optimize_token_generation(
                    model=self.model, inputs=input_tokens, generated_tokens=generated_tokens
                )
            ...
            if self.config.get("return_summary", True):
                yield {"is_summary": True, "session_summary": session_summary}
```
...
pipeline = StreamingInferencePipeline(
    ...
    config={
        ...
        "websocket_config": {"port": 8765, "path": "/stream"},
    },
)

# Initialize with browser and device information
pipeline.initialize(device_info=get_device_capabilities(), browser_info=get_browser_info())
