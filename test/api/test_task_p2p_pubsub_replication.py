...
def test_task_p2p_pubsub_like_replication_fanout(tmp_path):
    ...
    proc_a = ctx.Process(
        target=_run_service, args=(queue_a, port_a, announce_a, cache_dir_a), daemon=True
    )
    proc_b = ctx.Process(
        target=_run_service, args=(queue_b, port_b, announce_b, cache_dir_b), daemon=True
    )
    proc_a.start()
    proc_b.start()
    ...
    try:
        ...
        remote_a = RemoteQueue(
            peer_id=str(ann_a.get("peer_id") or ""), multiaddr=str(ann_a.get("multiaddr") or "")
        )
        remote_b = RemoteQueue(
            peer_id=str(ann_b.get("peer_id") or ""), multiaddr=str(ann_b.get("multiaddr") or "")
        )
