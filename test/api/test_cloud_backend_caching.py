...
def test_openai_backend_uses_cache_to_avoid_network_calls(monkeypatch):
    ...
    with (
        patch("ipfs_accelerate_py.backends.openai.get_llm_cache", return_value=dummy_cache),
        patch("ipfs_accelerate_py.backends.openai.requests.post") as mock_post,
    ):
        backend = OpenAIBackend(
            id="openai", api_key="test-key", engine="test-engine", use_cache=True
        )
...
def test_huggingface_backend_uses_cache_to_avoid_network_calls(monkeypatch):
    ...
    with (
        patch("ipfs_accelerate_py.backends.huggingface.get_llm_cache", return_value=dummy_cache),
        patch(
            "ipfs_accelerate_py.backends.huggingface.requests.request", return_value=fake_response
        ) as mock_req,
    ):
        backend = HuggingFaceBackend(
            id="huggingface", api_key="test-token", engine="test-engine", use_cache=True
        )
