...
def test_semantic_layout_exports_and_deprecated_aliases_match() -> None:
    ...
    assert api.AGENT_SUPERVISOR_G040_PACKAGES is api.AGENT_SUPERVISOR_TASK_SOURCES_PACKAGES
    assert api.AGENT_SUPERVISOR_G050_PACKAGES is api.AGENT_SUPERVISOR_CONTEXT_PROMPT_PACKAGES
    assert api.AGENT_SUPERVISOR_G060_PACKAGES is api.AGENT_SUPERVISOR_ANALYSIS_PROOF_PACKAGES
    assert api.AGENT_SUPERVISOR_G070_PACKAGES is api.AGENT_SUPERVISOR_OPERATIONS_PACKAGES
    assert api.AGENT_SUPERVISOR_G080_PACKAGES is api.AGENT_SUPERVISOR_INTEGRATIONS_DAEMON_PACKAGES
    assert (
    ...
    assert api.AGENT_SUPERVISOR_PACKAGE_GOAL_EVIDENCE is api.AGENT_SUPERVISOR_DOMAIN_LAYOUT_GOAL_IDS
    assert (
        api.AGENT_SUPERVISOR_PACKAGE_GOAL_TO_PACKAGES
        is api.AGENT_SUPERVISOR_LAYOUT_GOAL_TO_PACKAGES
    )
    assert (
        api.AGENT_SUPERVISOR_CUTOVER_GOAL_ID is api.AGENT_SUPERVISOR_DOMAIN_LAYOUT_CUTOVER_GOAL_ID
    )
    ...
    assert api.AGENT_SUPERVISOR_G030_CONTROL_STEMS is api.AGENT_SUPERVISOR_CONTROL_STEMS
    assert api.AGENT_SUPERVISOR_G040_TASK_SOURCES_STEMS is api.AGENT_SUPERVISOR_TASK_SOURCES_STEMS
    assert (
