"""PCPR-096: Accelerate-owned objective-to-release candidate gate."""
...
from ipfs_accelerate_py.assurance.next_bounded_pilot import (
...
def test_closed_vocabularies_match_pcpr_094_requirements() -> None:
    ...
    assert INTERFACE == "AccelerateNextBoundedPilot@1"
    assert SCHEMA == "ipfs_accelerate_py/assurance/next-bounded-pilot-receipt@1"
    assert OBJECTIVE_KIND == "declared_objective_to_release_next_bounded_pilot_recommendation"
    assert OPERATOR_BLOCKING_TASK_ID == "pcpr-096-operator-live-next-bounded-pilot"
    assert "release_candidate_qualified" in CLOSED_RELEASE_OUTCOMES
    assert "rnd_non_promoted" not in CLOSED_RELEASE_OUTCOMES
    assert HERMETIC_CANDIDATE_SUITES[-1].endswith(
        "test_agent_supervisor_next_bounded_pilot.py"
    )
    ...
    assert package["change_kind"] == "objective_to_release_next_bounded_pilot_recommendation"
    ...
    assert package["promotion_decision_cid"] == PINNED_PROMOTION_DECISION_CID
    assert document["promotion_or_non_promotion"]["decision_cid"] == PINNED_PROMOTION_DECISION_CID
    assert package["residual_report_produced"] is True
    assert package["all_residuals_named"] is True
    assert package["successor_campaign_created"] is False
    assert package["deployment_class"] == "rnd_non_promoted"
    assert package["next_pilot_deferred"] is False
    assert package["next_pilot_created"] is False
    assert package["next_pilot_recommended"] is True
    assert package["recommended_kind"] == "synthetic"
    assert package["customer_pilot_eligible"] is False
    assert package["pilot_created"] is False
    assert package["expands_pcpr"] is False
    assert package["next_task_id"] is None
    assert package["residual_gap_report_cid"] == PINNED_RESIDUAL_GAP_REPORT_CID
    assert RECOMMENDED_PILOT["kind"] == "synthetic"
    assert RECOMMENDED_PILOT["created"] is False
    assert package["residual_count"] == 8
    for residual in package["residuals"]:
        assert residual["impact"]
        assert residual["evidence"]
        assert residual["owner"]
        assert residual["precondition"]
        assert residual["bounded_action"]
        assert residual["successor_campaign"] is False
    ...
    assert package["report_cid"] == PINNED_REPORT_CID
    ...
    assert idempotency["first_report_cid"] == PINNED_REPORT_CID
    ...
    with pytest.raises(AccelerateNextBoundedPilotError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(AccelerateNextBoundedPilotError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(AccelerateNextBoundedPilotError, match="not an authorized"):
        refuse_unauthorized_path("/etc/passwd")
    with pytest.raises(AccelerateNextBoundedPilotError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="promote"):
        refuse_self_promotion(promoted=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="canonical"):
        refuse_non_canonical_objective("a different idea")
    with pytest.raises(AccelerateNextBoundedPilotError, match="closed PCPR"):
        refuse_closed_release_claim(claimed=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="bounded"):
        refuse_unbounded_gate(unbounded=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="hidden"):
        refuse_hidden_evidence(hidden=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="live pass"):
...
def test_current_head_is_rnd_non_promoted_and_not_a_release() -> None:
    ...
    assert verdict.report_cid == PINNED_REPORT_CID
...
def test_static_probes_show_declared_gate_constraints() -> None:
    ...
    assert probes["promotion_decision_cid_bound_not_minted"].present is True
    assert probes["residual_gap_report_cid_bound_not_minted"].present is True
    assert probes["synthetic_pilot_recommended_not_created"].present is True
    assert probes["residual_gap_report_identity_reminted"].present is False
    assert probes["residual_report_produced"].present is True
    assert probes["all_residuals_named"].present is True
    assert probes["successor_campaign_not_created"].present is True
    assert probes["idempotent_report_cid"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    catalog = platform_next_bounded_pilot_catalog()
    assert catalog["interface"] == "PlatformNextBoundedPilot@1"
    assert catalog["residual_gap_report_cid"] == PINNED_RESIDUAL_GAP_REPORT_CID
    ...
    assert catalog["report_cid"] == PINNED_REPORT_CID
    ...
    assert 'interface = "AccelerateNextBoundedPilot@1"' in pyproject
    plan = produce_next_bounded_pilot()
    ...
    with pytest.raises(AccelerateNextBoundedPilotError, match="cannot complete"):
        produce_next_bounded_pilot(complete_from="frontier_model")
    with pytest.raises(AccelerateNextBoundedPilotError, match="DuckDB"):
        produce_next_bounded_pilot(database_edited=True)
    with pytest.raises(AccelerateNextBoundedPilotError, match="closed PCPR"):
        produce_next_bounded_pilot(closed_release_claimed=True)


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(AccelerateNextBoundedPilotError, match="simulated"):
        qualify_next_bounded_pilot(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(AccelerateNextBoundedPilotError, match="measured_live"):
        qualify_next_bounded_pilot(
            (
                OutcomeProbe(
                    probe_id="bogus",
