...
@pytest.mark.parametrize('boundary', ['attach', 'consume', 'projection'])
...
def test_fresh_attachment_has_native_resource_limits_even_when_load_fails(monkeypatch):
    import duckdb
    ...
    class Client:
        ...
        def execute(self, sql):
            ...
            observations.append((self.raw.execute("SELECT current_setting('threads')").fetchone()[0],
                                 self.raw.execute("SELECT current_setting('memory_limit')").fetchone()[0]))
    ...
    def connect(*args, **kwargs):
        client = Client(native_connect(*args, **kwargs))
        clients.append(client)
        return client
    monkeypatch.setattr(duckdb, 'connect', connect)
    for _ in range(2):
        with pytest.raises(RuntimeError, match='load failure'):
            duckdb_state._open_quack_transport_connection_once('quack:127.0.0.1:45123')
    with native_connect(':memory:', config={'threads': 1, 'memory_limit': '256MB'}) as expected:
        memory = expected.execute("SELECT current_setting('memory_limit')").fetchone()[0]
    assert observations == [(1, memory), (1, memory)]
    assert all(client.closed for client in clients)


def test_stop_wins_over_expired_read_deadline_without_replay(monkeypatch):
    import time
    from ipfs_accelerate_py.agent_supervisor.task_sources import quack_read_continuity as reads
    calls = []
    stop = SystemExit(143)
    def open_connection(deadline):
        calls.append(deadline)
        monkeypatch.setattr(time, 'monotonic', lambda: deadline + 1)
        raise RuntimeError('Query interrupted') from stop
    with pytest.raises(SystemExit) as caught:
        reads.read_task_projection(open_connection=open_connection, read_projection=lambda _: pytest.fail('read'),
                                   store_id='store:test', endpoint='quack:127.0.0.1:45123')
    assert caught.value is stop
    assert len(calls) == 1
