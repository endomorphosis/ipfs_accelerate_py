    ...
    async def _main() -> None:
        ...
        async with background_trio_service(host.get_network()):
            await host.get_network().listen(
                make_multiaddr(f"/ip4/127.0.0.1/tcp/{int(listen_port)}")
            )
            ...
            try:
                ...
                with open(ready_file, "w", encoding="utf-8") as handle:
                    handle.write(
                        json.dumps({"peer_id": peer_id, "multiaddr": multiaddr}, ensure_ascii=False)
                    )
...
def test_task_p2p_dht_auto_discovery_allows_tools_cache_and_tasks(tmp_path, monkeypatch):
    ...
    try:
        ...
        remote = RemoteQueue(
            peer_id=str(result.get("peer_id") or "").strip(), multiaddr=discovered_ma
        )
        assert remote.peer_id

        # Cache roundtrip after discovery (no preconfigured peer multiaddr).
        set_resp = cache_set_sync(
            remote=remote, key="k", value={"v": 1}, ttl_s=30.0, timeout_s=10.0
        )
        ...
        tool_resp = call_tool_sync(
            remote=remote, tool_name="unit_test.echo", args={"x": 1}, timeout_s=10.0
        )
        ...
        task_id = submit_task_sync(
            remote=remote, task_type="text-generation", model_name="demo", payload={"p": 1}
        )
