...
def test_exhausted_restart_admission_keeps_live_peer_lanes(
    tmp_path,
    monkeypatch,
):
    """A capsule-denied idle restart must not SIGTERM remaining live lanes."""
    ...
    result = run_supervisor_tracks(
        ...
        stop_grace_seconds=0.2,
        python_executable=sys.executable,
        label="isolated restart admission",
        output=output.append,
    )
    ...
    isolated = [
        line
        for line in output
        if "isolated restart admission failure track=FAIL" in line
    ]
    assert isolated, output
    iso_at = output.index(isolated[0])
    assert any(
        "heartbeat KEEP" in line for line in output[iso_at:]
    ), output
    assert not any(
        "ended at typed supervisor blocker" in line
        for line in output[:iso_at]
    )
    assert starts.get("KEEP", 0) >= 1
    assert starts.get("FAIL", 0) >= 2
    assert result["all_trees_fenced"] is True
