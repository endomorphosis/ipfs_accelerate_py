"""PCPR-095: Accelerate-owned objective-to-release candidate gate."""
...
from ipfs_accelerate_py.assurance.residual_gap_report import (
...
def test_closed_vocabularies_match_pcpr_094_requirements() -> None:
    ...
    assert INTERFACE == "AccelerateResidualGapReport@1"
    assert SCHEMA == "ipfs_accelerate_py/assurance/residual-gap-report-receipt@1"
    assert OBJECTIVE_KIND == "declared_objective_to_release_residual_gap_report"
    assert OPERATOR_BLOCKING_TASK_ID == "pcpr-095-operator-live-residual-gap-report"
    assert "release_candidate_qualified" in CLOSED_RELEASE_OUTCOMES
    assert "rnd_non_promoted" not in CLOSED_RELEASE_OUTCOMES
    assert HERMETIC_CANDIDATE_SUITES[-1].endswith(
        "test_agent_supervisor_residual_gap_report.py"
    )
    ...
    assert package["change_kind"] == "objective_to_release_residual_gap_report"
    ...
    assert package["next_pilot_deferred"] is True
    ...
    with pytest.raises(AccelerateResidualGapReportError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(AccelerateResidualGapReportError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(AccelerateResidualGapReportError, match="not an authorized"):
        refuse_unauthorized_path("/etc/passwd")
    with pytest.raises(AccelerateResidualGapReportError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(AccelerateResidualGapReportError, match="promote"):
        refuse_self_promotion(promoted=True)
    with pytest.raises(AccelerateResidualGapReportError, match="canonical"):
        refuse_non_canonical_objective("a different idea")
    with pytest.raises(AccelerateResidualGapReportError, match="closed PCPR"):
        refuse_closed_release_claim(claimed=True)
    with pytest.raises(AccelerateResidualGapReportError, match="bounded"):
        refuse_unbounded_gate(unbounded=True)
    with pytest.raises(AccelerateResidualGapReportError, match="hidden"):
        refuse_hidden_evidence(hidden=True)
    with pytest.raises(AccelerateResidualGapReportError, match="live pass"):
...
def test_committed_files_match_generator() -> None:
    ...
    catalog = platform_residual_gap_report_catalog()
    assert catalog["interface"] == "PlatformResidualGapReport@1"
    ...
    assert 'interface = "AccelerateResidualGapReport@1"' in pyproject
    plan = produce_residual_gap_report()
    ...
    with pytest.raises(AccelerateResidualGapReportError, match="cannot complete"):
        produce_residual_gap_report(complete_from="frontier_model")
    with pytest.raises(AccelerateResidualGapReportError, match="DuckDB"):
        produce_residual_gap_report(database_edited=True)
    with pytest.raises(AccelerateResidualGapReportError, match="closed PCPR"):
        produce_residual_gap_report(closed_release_claimed=True)


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(AccelerateResidualGapReportError, match="simulated"):
        qualify_residual_gap_report(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(AccelerateResidualGapReportError, match="measured_live"):
        qualify_residual_gap_report(
            (
                OutcomeProbe(
                    probe_id="bogus",
