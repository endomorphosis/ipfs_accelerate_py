...
def test_cross_lane_completion_is_projected_before_successor_claim(
    ...
    try:
        ...
        completion_body = first_projection["logical_completions"][0]["body"]
        ...
        assert completion_body == {
            ...
            "authoritative_revision": completed.revision,
...
def test_ready_frontier_accepts_1001_dependencies_without_point_reads(
    ...
    population = (*completed_tasks, ready_task)
    ...
    class TaskSource:
        ...
        def snapshot(self) -> SimpleNamespace:
            return SimpleNamespace(
                projection_cid="projection:wide-frontier",
                task_count=len(population),
            )

        def list_tasks(self, **kwargs: object) -> SimpleNamespace:
            offset = int(str(kwargs.get("cursor") or "0"))
            limit = int(kwargs.get("limit") or 1_000)
            end = min(len(population), offset + limit)
            return SimpleNamespace(
                tasks=population[offset:end],
                next_cursor=str(end) if end < len(population) else "",
            )
    ...
    class Coordinator:
        def __init__(self) -> None:
            self.synchronized: list[dict[str, object]] = []

        def synchronize_authoritative_task(
            self,
            **kwargs: object,
        ) -> None:
            self.synchronized.append(dict(kwargs))
    ...
    assert len(coordinator.synchronized) == 1_002
    synchronized = {
        str(item["task_cid"]): item for item in coordinator.synchronized
    }
    assert set(synchronized) == {*dependencies, "task:cid:wide-successor"}
    assert all(
        synchronized[dependency]["authoritative_completed"] is True
        and synchronized[dependency]["authoritative_ready"] is False
        for dependency in dependencies
    )
    assert synchronized["task:cid:wide-successor"][
