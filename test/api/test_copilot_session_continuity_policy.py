...
def test_copilot_session_requires_sticky_worker_id() -> None:
    ...
    with pytest.raises(RuntimeError, match="sticky_worker_id"):
        _copilot_session_controls_allowed(
            payload=payload, local_session="S1", assigned_worker_id="w1"
        )
...
def test_copilot_continue_without_resume_allowed_when_opted_in(
...
def test_copilot_resume_allowed_with_resume_session_id(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv(
        "IPFS_ACCELERATE_PY_TASK_WORKER_ALLOW_COPILOT_CONTINUE_WITHOUT_RESUME", raising=False
    )
