...
def test_reconcile_rearms_blocked_portal_provider_failed_aseh(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert attempt is not None
        assert daemon.task_source.get(attempt.task_cid).status == "blocked"
        released = daemon.coordinator.get_task_claim(attempt.claim_id)
...
def test_provider_capacity_exhausted_deferrals_do_not_consume_typed_budget(
    ...
    daemon = _open_daemon(
        tmp_path,
        session="session:provider-capacity-leftover-wait-budget",
        provider_fn=provider,
        lease_ms=5_000,
        max_task_attempts=2,
        clock_ms=lambda: now["ms"],
...
def test_run_once_rearms_provider_capacity_exhausted_budget_exhaustion(
    ...
    daemon = _open_daemon(
        tmp_path,
        session="session:provider-capacity-leftover-wait-rearm",
        provider_fn=provider,
        lease_ms=5_000,
        max_task_attempts=2,
        clock_ms=lambda: now["ms"],
...
def test_quack_bootstrap_without_command_gateway_does_not_cas_landed_retrying() -> None:
    ...
    daemon = SimpleNamespace(
        ...
        _quack_command_gateway=None,
        _uses_quack_command_gateway=lambda: False,
        _task_outputs_landed_on_target=lambda _task: True,
        _task_declared_output_paths=lambda _task: (
            "artifacts/proof_carrying_platform_qualification_and_release/receipts/PCPR-001.json",
        ),
...
def test_resume_unknown_callback_completes_landed_outputs() -> None:
    ...
    daemon = SimpleNamespace(
        task_source=SimpleNamespace(get=lambda _cid: task),
        resume_attempt=lambda _attempt: (_ for _ in ()).throw(
            DatabaseProviderCallbackOutcomeUnknownError(failure_evidence={})
        ),
        _complete_landed_quarantined_task=lambda _task: {
            "task_cid": "task:pcpr-001",
            "task_alias": "PCPR-001",
        ...
        _quarantine_neutral_portal_failure=lambda *_args, **_kwargs: (
            (_ for _ in ()).throw(AssertionError("landed resume must not quarantine"))
        ),
    )
