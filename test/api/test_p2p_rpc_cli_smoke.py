...
def _run_service_cache(
    queue_path: str, listen_port: int, announce_file: str, cache_dir: str
) -> None:
...
def test_p2p_rpc_cli_status_and_cache_roundtrip(tmp_path):
    ...
    proc = ctx.Process(
        target=_run_service_cache, args=(queue_path, port, announce_file, cache_dir), daemon=True
    )
    proc.start()
    try:
        ...
        del_resp = _run_cli_json(
            ["--announce-file", announce_file, "cache-delete", "--key", "demo"]
        )
...
def test_p2p_rpc_cli_call_tool_get_server_status(tmp_path):
    ...
    ctx = mp.get_context("spawn")
    proc = ctx.Process(
        target=_run_service_with_mcp_tools, args=(queue_path, port, announce_file), daemon=True
    )
    proc.start()
    try:
        ann = _wait_for_announce(announce_file)
...
def test_p2p_rpc_cli_task_submit_and_wait_roundtrip(tmp_path):
    ...
    ctx = mp.get_context("spawn")
    proc = ctx.Process(
        target=_run_worker_with_service, args=(queue_path, port, announce_file), daemon=True
    )
    proc.start()
    try:
        ann = _wait_for_announce(announce_file)
...
def test_p2p_rpc_cli_announce_file_zero_falls_back_to_env(tmp_path):
    ...
    try:
        ...
        assert any(
            a.get("method") in {"explicit", "announce-file"} and a.get("ok") is True
            for a in attempts
        )
    finally:
        proc.terminate()
...
def test_p2p_rpc_cli_discover_peer_id_hint_uses_env_announce_file(tmp_path):
    ...
    try:
        ...
        assert any(
            a.get("method") in {"announce-file", "explicit", "announce-file:env"}
