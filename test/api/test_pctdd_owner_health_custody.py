...
def test_extra_gate_preservation_is_not_reported_as_whole_tree_fencing(
    tmp_path, monkeypatch
):
    track = _track(tmp_path)
    process = SimpleNamespace(pid=54321)
    monkeypatch.setattr(
        runner, "daemon_pid_health_fields", lambda *a, **k: {"daemon_pid": 87654}
    )
    monkeypatch.setattr(
        runner, "_restarting_track_must_preserve_extra_gate_grok", lambda *a, **k: True
    )

    def forbidden(*a, **k):
        raise AssertionError("retained tree must not be signalled or waited")
