...
def test_quack_mutation_timeout_is_unknown_outcome_without_internal_replay_aseh(
    tmp_path,
    monkeypatch,
) -> None:
    from ipfs_accelerate_py.agent_supervisor.task_sources import (
...
def test_quack_attach_retries_authentication_failed_contention_aseh(monkeypatch) -> None:
    from ipfs_accelerate_py.agent_supervisor.task_sources import duckdb_state as ds
...
def test_quack_attach_reuses_cached_connection_aseh(monkeypatch) -> None:
    from ipfs_accelerate_py.agent_supervisor.task_sources import duckdb_state as ds
...
def test_unstall_stale_in_progress_tasks_retries_dead_gate_aseh(tmp_path) -> None:
    import duckdb
    from datetime import datetime, timedelta, timezone
...
def test_unstall_drops_status_indexes_that_fatal_status_updates_aseh(tmp_path) -> None:
    import duckdb
    from datetime import datetime, timedelta, timezone
...
def test_apply_owner_command_payload_unstalls_without_client_sql_aseh(tmp_path) -> None:
    import duckdb
    from datetime import datetime, timedelta, timezone
...
def test_false_terminal_blocked_unstall_reopens_isolate_merge_queue_gate() -> None:
    import duckdb
    ...
    connection.execute(
        ...
        [
            ...
            "blocked",
            1877,
            "2026-09-10T05:36:58Z",
            "isolate_merge_queue_to_task_projection AttributeError",
        ],
    )
