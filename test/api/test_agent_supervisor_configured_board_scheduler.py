...
def _task_block(
    ...
    *,
    ...
    lines = (
        ...
        *(
            (f"- No-change completion: {no_change_completion}",)
...
def test_sealed_bootstrap_denies_missing_native_dependency_pin(
    ...
    denial = (
        "if module=='ipfs_accelerate_py.agent_supervisor.todo_daemon."
        "implementation_supervisor': raise SystemExit(78)"
    )
    assert multi_runner_module.SEALED_CONTROL_PLANE_BOOTSTRAP.index(
        denial
    ) < multi_runner_module.SEALED_CONTROL_PLANE_BOOTSTRAP.index(
        ...
        if bridge_scenario in {"no_change", "no_change_guard_denied"}:
            ...
            captured["no_change_guard"] = (
                self._validated_no_change_completion_guard(
                    ...
                    # This bridge test exercises the plan-bound lease/merge
                    # transition directly.  The canonical daemon path creates
                    # and consumes the separately tested attempt-local
                    # no-change policy gate before invoking this boundary.
