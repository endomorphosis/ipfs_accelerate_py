...
def test_worker_emits_backend_routing_failure_event_when_backend_manager_raises(
    ...
    monkeypatch.setattr(ibm, "get_backend_manager", lambda config=None: _FailingBackendManager())
    monkeypatch.setattr(
        datasets_integration, "DatasetsManager", lambda *args, **kwargs: fake_datasets
    )
    ...
    assert any(
        ...
        for event in fake_datasets.events
    )
    assert fake_datasets.provenance
    assert any(
        item["operation"] == "workflow_task_failed_backend_routing"
        for item in fake_datasets.provenance
    )


def test_worker_emits_degraded_event_when_backend_manager_optional_and_falls_back(
    monkeypatch, tmp_path
):
    import ipfs_accelerate_py.inference_backend_manager as ibm
    import ipfs_accelerate_py.datasets_integration as datasets_integration
    ...
    degraded_events = [
        e for e in fake_datasets.events if e["event_type"] == "workflow_backend_routing_degraded"
    ]
