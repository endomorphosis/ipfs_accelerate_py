...
def _run_service(queue_path: str, listen_port: int, announce_file: str) -> None:
    ...
    async def _main() -> None:
        await serve_task_queue(
            queue_path=queue_path, listen_port=listen_port, accelerate_instance=None
        )
...
def _expected_for_peer(
    tasks: list[dict], *, peer_id: str, peers: list[str], clock_hash: str
) -> dict | None:
...
def test_task_p2p_deterministic_claim_and_visibility(tmp_path):
    ...
    try:
        ...
        remote = RemoteQueue(
            peer_id=str(ann.get("peer_id") or ""), multiaddr=str(ann.get("multiaddr") or "")
        )
        ...
        queued = list_tasks_sync(
            remote=remote, status="queued", limit=100, task_types=["unit-test"]
        ).get("tasks")
        ...
        got_a = claim_next_sync(
            remote=remote, worker_id="worker-a", peer_id=peer_a, supported_task_types=["unit-test"]
        )
        got_b = claim_next_sync(
            remote=remote, worker_id="worker-b", peer_id=peer_b, supported_task_types=["unit-test"]
        )
