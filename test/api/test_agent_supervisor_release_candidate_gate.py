"""PCPR-093: Accelerate-owned objective-to-release candidate gate."""
...
from ipfs_accelerate_py.assurance.release_candidate_gate import (
...
def test_closed_vocabularies_match_pcpr_093_requirements() -> None:
    ...
    assert INTERFACE == "AccelerateReleaseCandidateGate@1"
    assert SCHEMA == "ipfs_accelerate_py/assurance/release-candidate-gate@1"
    assert OBJECTIVE_KIND == "declared_objective_to_release_candidate_gate"
    assert OPERATOR_BLOCKING_TASK_ID == "pcpr-093-operator-live-release-candidate-gate"
    assert "release_candidate_qualified" in CLOSED_RELEASE_OUTCOMES
    assert "rnd_non_promoted" not in CLOSED_RELEASE_OUTCOMES
    assert HERMETIC_CANDIDATE_SUITES[-1].endswith(
        "test_agent_supervisor_release_candidate_gate.py"
    )
    ...
    assert package["change_kind"] == "objective_to_release_candidate_gate"
    ...
    assert package["gate_run_produced"] is True
    assert package["gate_run_deferred"] is False
    ...
    assert idempotency["first_gate_run_cid"] == PINNED_GATE_RUN_CID
    ...
    with pytest.raises(AccelerateReleaseCandidateGateError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(AccelerateReleaseCandidateGateError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(AccelerateReleaseCandidateGateError, match="not an authorized"):
        refuse_unauthorized_path("/etc/passwd")
    with pytest.raises(AccelerateReleaseCandidateGateError, match="DuckDB"):
        refuse_database_edit(edited=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="promote"):
        refuse_self_promotion(promoted=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="canonical"):
        refuse_non_canonical_objective("a different idea")
    with pytest.raises(AccelerateReleaseCandidateGateError, match="PCPR-094"):
        refuse_closed_release_claim(claimed=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="bounded"):
        refuse_unbounded_gate(unbounded=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="hidden"):
        refuse_hidden_evidence(hidden=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="live pass"):
...
def test_current_head_is_rnd_non_promoted_and_not_a_release() -> None:
    ...
    assert verdict.gate_run_cid == PINNED_GATE_RUN_CID
...
def test_static_probes_show_declared_gate_constraints() -> None:
    ...
    assert probes["gate_run_produced"].present is True
    assert probes["closed_release_deferred"].present is True
    assert probes["idempotent_gate_run_cid"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    catalog = platform_release_candidate_gate_catalog()
    assert catalog["interface"] == "PlatformReleaseCandidateGate@1"
    ...
    assert catalog["gate_run_cid"] == PINNED_GATE_RUN_CID
    ...
    assert 'interface = "AccelerateReleaseCandidateGate@1"' in pyproject
    plan = produce_release_candidate_gate()
    assert plan["produced"] is True
    assert plan["live"] is False
    assert plan["closed_release_deferred"] is True
    assert tuple(plan["escalation_order"]) == ESCALATION_ORDER
    with pytest.raises(AccelerateReleaseCandidateGateError, match="cannot complete"):
        produce_release_candidate_gate(complete_from="frontier_model")
    with pytest.raises(AccelerateReleaseCandidateGateError, match="DuckDB"):
        produce_release_candidate_gate(database_edited=True)
    with pytest.raises(AccelerateReleaseCandidateGateError, match="PCPR-094"):
        produce_release_candidate_gate(closed_release_claimed=True)


def test_simulated_live_probe_is_rejected() -> None:
    dummy = "baguqeerabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    with pytest.raises(AccelerateReleaseCandidateGateError, match="simulated"):
        qualify_release_candidate_gate(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(AccelerateReleaseCandidateGateError, match="measured_live"):
        qualify_release_candidate_gate(
            (
                OutcomeProbe(
                    probe_id="bogus",
