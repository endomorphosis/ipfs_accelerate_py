        ...
        for c in candidates:
            ...
            try:
                ...
                if require_docker_worker:
                    scheduler = (
                        result.get("scheduler") if isinstance(result.get("scheduler"), dict) else {}
                    )
                    counts = (
                        scheduler.get("counts") if isinstance(scheduler.get("counts"), dict) else {}
                    )
                    try:
                    ...
                    if isinstance(local, dict):
                        if bool(local.get("enabled")) and bool(local.get("docker_enabled")):
                            ...
                            if isinstance(sup, str):
                                ...
                                sup_list = []
                            if any(t.startswith("docker.") for t in sup_list) or (
                                "docker.execute" in sup_list
                            ):
                                local_ok = True
...
def test_task_p2p_docker_execute_nvidia_smi_50x_across_network():
    ...
    p2p_mode = (
        str(os.environ.get("IPFS_ACCELERATE_PY_TEST_DOCKER_P2P_MODE") or "task").strip().lower()
        or "task"
    )
    ...
    if len(targets) < 2:
        ...
        autodiscover = _truthy(
            os.environ.get("IPFS_ACCELERATE_PY_TEST_P2P_AUTODISCOVER") or autodiscover_default
        )
        if autodiscover:
            try:
            ...
            try:
                timeout_s = float(
                    os.environ.get("IPFS_ACCELERATE_PY_TEST_P2P_AUTODISCOVER_TIMEOUT_S") or "6"
                )
                limit = int(
                    os.environ.get("IPFS_ACCELERATE_PY_TEST_P2P_AUTODISCOVER_LIMIT") or "25"
                )
            except Exception:
            ...
            try:
                discovered = discover_peers_via_mdns_sync(
                    timeout_s=float(timeout_s), limit=int(limit), exclude_self=True
                )
            finally:
            ...
            if local.get("peer_id") and local.get("multiaddr"):
                merged.append(
                    {"peer_id": str(local["peer_id"]), "multiaddr": str(local["multiaddr"])}
                )
            ...
            pytest.skip(
                "Provide at least 2 targets via IPFS_ACCELERATE_PY_TEST_P2P_TARGETS[_FILE] or enable mDNS autodiscovery"
            )
    ...
    try:
        status_timeout_s = float(
            os.environ.get("IPFS_ACCELERATE_PY_TEST_P2P_STATUS_TIMEOUT_S") or "3"
        )
    except Exception:
        status_timeout_s = 3.0
    require_docker_worker = _truthy(
        os.environ.get("IPFS_ACCELERATE_PY_TEST_P2P_REQUIRE_DOCKER_WORKER") or "1"
    )
    ...
    image = str(
        os.environ.get("IPFS_ACCELERATE_PY_TEST_DOCKER_IMAGE")
        or "nvidia/cuda:12.4.0-base-ubuntu22.04"
    ).strip()
    ...
    report_path = (
        str(os.environ.get("IPFS_ACCELERATE_PY_TEST_DOCKER_REPORT_PATH") or "").strip()
    ...
    remotes = [
        RemoteQueue(peer_id=str(t.get("peer_id") or ""), multiaddr=str(t.get("multiaddr") or ""))
        for t in targets
    ]
    ...
    for r in results:
        ...
        if r.get("error"):
            ...
            logger.error(
                "docker.execute submit/wait failed peer_id=%s multiaddr=%s error=%s",
                pid,
                r.get("multiaddr"),
                r.get("error"),
            )
        ...
        if not isinstance(task, dict):
            ...
            failures_by_peer[pid] = failures_by_peer.get(pid, 0) + 1
            logger.error(
                "docker.execute invalid task response peer_id=%s multiaddr=%s task=%r",
                pid,
                r.get("multiaddr"),
                task,
            )
    ...
    if expect_mesh:
        ...
        if gpu_uuids:
            assert len(gpu_uuids) >= 2, (
                f"Expected distribution (>=2 GPU UUIDs), saw {sorted(gpu_uuids)}"
            )
        else:
            assert len(worker_ids) >= 2, (
                f"Expected distribution (>=2 worker_ids), saw {sorted(worker_ids)}"
            )
    ...
    if expect_all:
        ...
        if gpu_uuids:
            ...
            assert len(worker_ids) >= want, (
                f"Expected >= {want} worker_ids, saw {sorted(worker_ids)}"
            )

    assert failures == 0, (
        f"docker.execute had {failures} failures (see logs for per-peer diagnostics)"
    )
