...
def test_native_birth_callback_failure_keeps_prompt_withheld_and_child_fenced(
    tmp_path, failure_type
):
    marker = tmp_path / "prompt-received"
    child = []

    def reject(process):
        child.append(process)
        raise failure_type("disposable birth receipt unavailable")

    with pytest.raises(failure_type, match="disposable birth receipt unavailable"):
        runtime.run_process_group_stream(
            [
                sys.executable,
                "-c",
                "import pathlib,sys; data=sys.stdin.read(); "
                + f"pathlib.Path({str(marker)!r}).write_text(data)",
            ],
            cwd=tmp_path,
            stdout=subprocess.DEVNULL,
...
def test_native_cancellation_keeps_its_original_typed_fence(tmp_path):
    child = []
    with pytest.raises(runtime.ProcessGroupCancelled, match="fixture cancellation"):
        runtime.run_process_group_stream(
            ...
            timeout_seconds=5,
            termination_grace_seconds=0.1,
            on_started=lambda process: child.append(process),
            cancel_requested=lambda: "fixture cancellation",
