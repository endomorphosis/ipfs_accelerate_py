...
def test_ordinary_do_not_edit_documentation_instruction_is_not_generated_marker(
    tmp_path: Path,
) -> None:
    path = "docs/architecture/decisions/0000-template.md"
    before = "Use a copy of the template.\n"
    after = "Do not edit 0000-template.md to record a real decision.\n"
    _seed(tmp_path, path=path, source=before)

    result = _admit(
        _proposal(path=path, before=before, after=after).to_dict(),
        tmp_path,
        policy=_policy(task_owned_paths=("docs/", "src/", "test/")),
    )

    assert result.accepted
    assert ProposalFindingCode.GENERATED_CHANGE_FORBIDDEN not in {
        finding.code for finding in result.findings
    }
...
def test_true_generated_file_warnings_remain_forbidden(
    tmp_path: Path,
    marker: str,
) -> None:
    ...
    result = _admit(
        _proposal(path=path, before=before, after=marker).to_dict(),
        tmp_path,
        policy=_policy(task_owned_paths=("docs/", "src/", "test/")),
    )

    assert not result.accepted
    assert ProposalFindingCode.GENERATED_CHANGE_FORBIDDEN in {
        finding.code for finding in result.findings
    }
