...
def test_caught_signal_remains_latched_through_native_cleanup(
    ...
    def run_once():
        calls.append("pass")
        assert calls.count("pass") == 1, "stop request started a second pass"
        if phase == "run_once":
            swallowed()
...
def test_latched_stop_before_loop_does_not_start_first_pass(native_main, monkeypatch):
    ...
    monkeypatch.setattr(
        daemon, "run_once", lambda: pytest.fail("started a stopped daemon")
    )
    with pytest.raises(SystemExit) as stopped:
        runtime.main(args)
    assert stopped.value.code == 128 + signal.SIGTERM
...
def _child_signal_run(path, phase):
    ...
    def run_once():
        ...
        if calls.count("pass") != 1:
            raise AssertionError("daemon resumed after SIGTERM")
        if phase == "run_once":
            try:
