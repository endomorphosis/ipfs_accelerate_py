...
def test_foreign_daemon_owned_journal_keeps_existing_7f8_refusal_without_adoption(tmp_path):
    ...
    todo_before = todo.read_bytes()
    result = supervisor._recover_retained_generated_checkout_lease()
    # 7f8 predates DOEP's separate pending classification. Preserve its exact
    # existing refusal instead of importing an unrelated owner-policy change.
    assert result["blocked"] is True and result["attempted"] is False
    assert result["adopted"] is False and result["recovered"] is False
    assert result["reason"] == "external_protected_checkout_recovery_required"
