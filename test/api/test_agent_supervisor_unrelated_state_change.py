"""PCPR-066: Accelerate-owned unrelated state change."""
...
def test_closed_vocabularies_match_pcpr_066_requirements() -> None:
    ...
    assert INTERFACE == "AccelerateUnrelatedStateChange@1"
    assert SCHEMA == "ipfs_accelerate_py/assurance/unrelated-state-change@1"
    assert OBJECTIVE_KIND == "declared_unrelated_state_change"
    ...
    assert HERMETIC_CANDIDATE_SUITES[-1].endswith(
        "test_agent_supervisor_unrelated_state_change.py"
    )
    ...
    assert change["change_kind"] == "unrelated_documentation"
    ...
    assert list(change["unrelated_change_paths"]) == list(UNRELATED_CHANGE_PATHS)
    ...
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="remints"):
        refuse_pack_cid_remint(
            "baguqeeraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        )
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="cannot complete"):
        refuse_model_completion("frontier_model")
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="not an authorized"):
        refuse_unauthorized_path("/etc/passwd")
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="relevant"):
        refuse_relevant_interface_change(relevant=True)
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="whole-plan"):
        refuse_whole_plan_regeneration(required=True)
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="PCPR-067"):
...
def test_static_probes_show_declared_change_constraints() -> None:
    ...
    assert probes["change_is_unrelated_documentation"].present is True
...
def test_committed_files_match_generator() -> None:
    ...
    catalog = platform_unrelated_state_change_catalog()
    assert catalog["interface"] == "PlatformUnrelatedStateChange@1"
    ...
    assert 'interface = "AccelerateUnrelatedStateChange@1"' in pyproject
    plan = produce_unrelated_state_change()
    assert plan["produced"] is True
    assert plan["live"] is False
    assert tuple(plan["escalation_order"]) == ESCALATION_ORDER
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="cannot complete"):
        produce_unrelated_state_change(complete_from="frontier_model")
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="relevant"):
        produce_unrelated_state_change(relevant=True)
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="PCPR-067"):
        produce_unrelated_state_change(reuse_demonstrated=True)


def test_simulated_live_probe_is_rejected() -> None:
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="simulated"):
        qualify_unrelated_state_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
    ...
    with pytest.raises(AccelerateUnrelatedStateChangeError, match="measured_live"):
        qualify_unrelated_state_change(
            (
                OutcomeProbe(
                    probe_id="bogus",
