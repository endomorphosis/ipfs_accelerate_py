...
def test_mirror_steal_fixed_point_attempt_rebalance_and_frontier(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("steal_record", "steal:1", "knowledge_graph", "record_cid"),
        ("fixed_point_record", "fixed:1", "knowledge_graph", "record_cid"),
        ("merge_attempt_record", "attempt:1", "knowledge_graph", "record_cid"),
        ("rebalance_record", "rebalance:1", "knowledge_graph", "record_cid"),
        ("causal_frontier_record", "frontier:1", "knowledge_graph", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_train_node_evidence_plan_and_recovery(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("merge_train_record", "train:1", "knowledge_graph", "record_cid"),
        ("causal_node_record", "node:1", "knowledge_graph", "record_cid"),
        ("causal_evidence_record", "evidence:1", "knowledge_graph", "record_cid"),
        ("shard_plan_record", "plan:1", "knowledge_graph", "record_cid"),
        ("federation_recovery_record", "recovery:1", "knowledge_graph", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_intervention_ducklake_test_and_capsule(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("abstraction_intervention_record", "intervention:1", "knowledge_graph", "record_cid"),
        ("ducklake_projection_record", "projection:1", "metadata", "record_cid"),
        ("ducklake_projection_recovery", "recovery:1", "metadata", "record_cid"),
        ("test_projection_record", "test:1", "proof_certificate", "record_cid"),
        ("capsule_projection_record", "capsule:1", "capsule", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_cache_seal_invalidation_relation_and_capsule_change(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("proof_cache_projection_record", "cache:1", "proof_cache", "record_cid"),
        ("seal_projection_record", "seal:1", "proof_certificate", "record_cid"),
        ("proof_invalidation_record", "invalidation:1", "proof_certificate", "record_cid"),
        ("kg_relation_record", "edge:1", "knowledge_graph", "record_cid"),
        ("capsule_invalidation_record", "invalidation:2", "capsule", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_measurement_usage_token_failure_and_guardrail(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("resource_measurement_record", "measure:1", "metadata", "record_cid"),
        ("provider_usage_record", "usage:1:admitted", "metadata", "record_cid"),
        ("end_to_end_token_compute", "input_tokens:measured", "metadata", "record_cid"),
        ("implementation_failure_context", "receipt:1", "metadata", "record_cid"),
        ("dependency_guardrail_record", "dependency:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_guardrails_shadow_cost_and_autonomy_status(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("reconciliation_guardrail_record", "reconciliation:1", "metadata", "record_cid"),
        ("retry_budget_guardrail_record", "retry-budget:1", "metadata", "record_cid"),
        ("shadow_artifact_record", "artifact:1", "metadata", "record_cid"),
        ("shadow_cost_record", "expanded", "metadata", "record_cid"),
        ("autonomy_status_record", "blocked", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_cycle_validation_retry_and_command_receipts(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("cycle_exception_record", "daemon_exception", "metadata", "record_cid"),
        ("validation_result_record", "task:1:passed", "metadata", "task_id"),
        ("remote_queue_retry_record", "task:1", "metadata", "task_id"),
        ("authorized_command_receipt", "event:1", "metadata", "record_cid"),
        ("authorized_command_quarantine", "event:2", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_remote_evidence_validation_backoff_and_digests(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("remote_queue_backoff_record", "task:1", "metadata", "task_id"),
        ("remote_evidence_record", "task:1", "metadata", "task_id"),
        ("remote_validation_record", "task:1:passed", "metadata", "task_id"),
        ("idempotency_digest", "digest:1", "metadata", "record_cid"),
        ("store_revision_advance", "generation:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_metrics_receipt_policy_effects_and_submodule(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("token_metrics_record", "metrics:1", "metadata", "record_cid"),
        ("epoch_receipt_record", "receipt:1", "metadata", "record_cid"),
        ("runtime_policy_record", "clock", "metadata", "record_cid"),
        ("parallel_effect_record", "overlap:serialized", "metadata", "record_cid"),
        ("merge_submodule_record", "digest:merged", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_runtime_trace_file_code_service_capability_and_module(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("trace_file_record", "sha256:file", "metadata", "record_cid"),
        ("trace_code_record", "sha256:code", "metadata", "record_cid"),
        ("trace_service_record", "sha256:service", "metadata", "record_cid"),
        ("trace_capability_record", "sha256:capability", "metadata", "record_cid"),
        ("trace_module_record", "python:sha256:module", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_environment_subprocess_events_and_queue_success(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("trace_environment_record", "cid:env", "metadata", "record_cid"),
        ("trace_subprocess_record", "sha256:args:admitted", "metadata", "record_cid"),
        ("trace_unsupported_event", "unsupported", "metadata", "record_cid"),
        ("trace_private_event", "private", "metadata", "record_cid"),
        ("queue_selection_success", "task:1", "metadata", "task_id"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_queue_reset_registration_creation_and_compaction(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("queue_selection_reset", "task:1:reset", "metadata", "task_id"),
        ("queue_renewal_reset", "task:1", "metadata", "task_id"),
        ("queue_registration", "cid:task", "metadata", "record_cid"),
        ("queue_entry_created", "task:1", "metadata", "task_id"),
        ("queue_compaction", "removed:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_status_cas_goal_deferral_guard_and_leftover(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("status_cas_record", "task:1", "metadata", "task_id"),
        ("goal_status_cas_record", "goal:1", "metadata", "task_id"),
        ("typed_deferral_recovery_record", "task:1", "metadata", "task_id"),
        ("guarded_status_cas_record", "task:1", "metadata", "task_id"),
        ("leftover_wait_recovery_record", "task:1", "metadata", "task_id"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_receipt_access_and_configured_finding_batches(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("receipt_access_record", "cid:1", "metadata", "record_cid"),
        ("configured_objective_scan_record", "generated:1", "metadata", "record_cid"),
        ("configured_codebase_scan_record", "exhausted:0", "metadata", "record_cid"),
        ("configured_retry_budget_record", "recorded:1", "metadata", "record_cid"),
        ("codebase_audit_record", "recorded:0", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_direct_refill_scan_and_retry_findings(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("direct_refill_scan_record", "codebase:generated:1", "metadata", "record_cid"),
        ("direct_refill_scan_record", "objective:exhausted:0", "metadata", "record_cid"),
        ("direct_retry_budget_record", "recorded:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_dependency_reconciliation_and_successor_admission(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("dependency_guardrail_record", "recorded:1", "metadata", "record_cid"),
        ("reconciliation_guardrail_record", "recorded:2", "metadata", "record_cid"),
        ("self_improvement_successor_admission", "epoch:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_equality_rewrite_replay_extract_and_plan_r2_binds(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("equality_rewrite_record", "theory:proved:equality_proved_under_declared_theory", "proof_certificate", "record_cid"),
        ("equality_replay_record", "theory:1", "proof_certificate", "record_cid"),
        ("equality_extract_record", "theory:1", "proof_certificate", "record_cid"),
        ("plan_r2_envelope_journal_binding", "capability:1", "metadata", "record_cid"),
        ("plan_r2_owner_gateway_binding", "capability:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        item = mirror_work_record(
            catalog_kind=catalog_kind,
...
def test_mirror_tactician_claim_forest_and_typed_owner_channel(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("tactician_plan_evaluation", "plan:1:admitted", "metadata", "record_cid"),
        ("plan_r2_typed_owner_channel_binding", "capability:1", "metadata", "record_cid"),
        ("logical_claim_binding", "task:1", "metadata", "task_id"),
        ("kit_source_forest_binding", "profile:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_attempt_sample_rollback_snapshot_and_overlay(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("llm_avoidance_attempt_record", "attempt:1:closed_deterministic", "metadata", "record_cid"),
        ("formal_planning_sample_record", "sample:1:cold", "metadata", "record_cid"),
        ("mutation_rollback_record", "mutation:1:verified", "metadata", "record_cid"),
        ("worktree_snapshot_record", "snapshot:1", "metadata", "record_cid"),
        ("dirty_overlay_record", "overlay:1", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_context_evidence_prover_and_lifecycle_receipt(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("program_world_context_compilation", "receipt:context", "capsule", "record_cid"),
        ("completion_evidence_result", "evidence:1", "metadata", "record_cid"),
        ("prover_outcome", "unit:1:proved", "proof_cache", "record_cid"),
        ("lifecycle_receipt_binding", "receipt:1:graph_leaf", "metadata", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_typed_status_claim_recovery_and_retry_cooldown(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("typed_status_cas_record", "task:1:retrying", "metadata", "task_id"),
        ("typed_dead_claim_recovery", "task:1", "metadata", "task_id"),
        ("typed_legacy_unstall_recovery", "task:1", "metadata", "task_id"),
        ("typed_retry_cooldown_record", "task:1", "metadata", "task_id"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
...
def test_mirror_typed_recovery_stale_claim_and_seal_publication(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("IPFS_ACCELERATE_META_INDEX_DUCKDB", str(tmp_path / "meta_index.duckdb"))
    kinds = (
        ("typed_post_merge_retry_recovery", "task:1:replay", "metadata", "task_id"),
        ("typed_leftover_wait_recovery", "task:1", "metadata", "task_id"),
        ("stale_claim_recovery", "entry:1:pending", "metadata", "record_cid"),
        ("seal_publication_recovery", "digest:replay", "proof_cache", "record_cid"),
    )
    for record_kind, record_ref, catalog_kind, subject_kind in kinds:
        subject_ref = "task:1" if subject_kind == "task_id" else record_ref
        item = mirror_work_record(
