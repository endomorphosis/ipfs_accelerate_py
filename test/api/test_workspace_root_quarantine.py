...
@pytest.mark.parametrize("operation", ["exact_delete", "exact_adopt", "exact_finalize"])
def test_native_lifecycle_mutations_preserve_frozen_custody(tmp_path, operation):
    ...
    frozen = q.freeze(repo, root, expected=before)
    expected = {
        "expected_record_id": terminal.record_id,
        "expected_fence": terminal.fence,
        "expected_lease_id": terminal.lease_id,
        "expected_task_id": terminal.task_id,
        "expected_canonical_task_cid": terminal.canonical_task_cid,
        "expected_attempt": terminal.attempt,
        "expected_branch": terminal.branch,
        "expected_merge_target": terminal.merge_target,
        "expected_repo_root": terminal.repo_root,
        "expected_state_dir": terminal.state_dir,
    }
    with pytest.raises(QuarantineDenied, match="workspace_root_quarantined"):
        ...
        elif operation == "exact_adopt":
            lifecycle.adopt_dead_owner(lease.path, lane_id="other-lane", **expected)
        else:
            lifecycle.finalize_exact_dead_owner(
                lease.path, expected_owner=terminal.owner,
                reason="retained callback custody", **expected,
            )
    assert q.census(repo, root) == before
    assert q.verify(repo, root) == frozen
...
def test_all_native_global_maintenance_paths_defer_for_sibling_root(tmp_path):
    ...
    assert daemon._cleanup_already_merged_worktrees()["reason"] == "retained_workspace_scope"
    # This native fork keeps its inert projection refusal before any repo gate.
    projection = object.__new__(PortalImplementationDaemon)
    projection.isolate_merge_queue_to_task_projection = True
    assert projection._cleanup_already_merged_worktrees() == {
        "attempted": False,
