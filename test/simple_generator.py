...
def map_model_to_architecture(model_type: str) -> str:
    ...
    for architecture, models in ARCHITECTURE_MAPPING.items():
        for model in models:
            if model_type_lower.startswith(model.lower()) or model.lower().startswith(
                model_type_lower
            ):
                return architecture
...
def simple_strip_indentation(content: str) -> str:
    ...
    for line in lines:
        ...
        # Indent lines inside blocks
            ...
            if i > 0 and i == len(processed_lines) - 1
        ):
            processed_lines.append("    " + stripped)
        else:
            processed_lines.append(stripped)
...
def fix_indentation(content: str) -> str:
    ...
    if not is_valid:
        ...
        content = re.sub(
            r"class ([a-zA-Z0-9_]+)(\([^)]*\))?:\s*$", r"class \1\2:\n    pass", content
        )
        ...
        for line in lines:
            ...
            if stripped.startswith(("}", ")", "]")) or re.match(
                r"^(else|elif|except|finally|case)(\s|:)", stripped
            ):
            ...
            prev_indented = False
            if re.match(
                r"^(if|for|while|def|class|with|try|else|elif|except|finally|match|case)\b.*:$",
                stripped,
        ...
        for key, value in context["model_info"].items():
            if isinstance(value, (str, int, float, bool)):
                ...
                rendered = rendered.replace(
                    f"{{{{ model_info.{key}|capitalize }}}}", str(value).capitalize()
                )
...
def main():
    ...
    parser.add_argument(
        "--architecture",
        choices=list(ARCHITECTURE_MAPPING.keys()),
        help="Generate tests for all models of a specific architecture",
    )
    parser.add_argument(
        "--output-dir", default="./generated_tests", help="Output directory for generated files"
    )
    parser.add_argument(
        "--limit", type=int, default=0, help="Limit the number of models to generate (0 = no limit)"
    )
