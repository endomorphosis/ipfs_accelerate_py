...
from ipfs_kit_integration import IPFSKitStorage, get_storage, reset_storage, StorageBackendConfig
...
class TestIPFSKitStorageInitialization:
    ...
    def test_init_with_fallback(self, temp_cache_dir):
        ...
        storage = IPFSKitStorage(
            enable_ipfs_kit=False, cache_dir=temp_cache_dir, force_fallback=True
        )
    ...
    def test_init_with_env_disable(self, temp_cache_dir):
        ...
        with patch.dict(os.environ, {"IPFS_KIT_DISABLE": "1"}):
            storage = IPFSKitStorage(enable_ipfs_kit=True, cache_dir=temp_cache_dir)
    ...
    def test_cache_dir_creation(self, temp_cache_dir):
        ...
        storage = IPFSKitStorage(
            enable_ipfs_kit=False, cache_dir=str(cache_path), force_fallback=True
        )
...
class TestSingletonPattern:
    ...
    def test_get_storage_singleton(self, temp_cache_dir):
        ...
        storage1 = get_storage(enable_ipfs_kit=False, cache_dir=temp_cache_dir, force_fallback=True)
...
class TestConfigurationOptions:
    ...
    def test_custom_config(self, temp_cache_dir):
        ...
        config = {"enable_ipfs": True, "enable_s3": False, "custom_option": "value"}

        storage = IPFSKitStorage(
            enable_ipfs_kit=False, cache_dir=temp_cache_dir, config=config, force_fallback=True
        )
