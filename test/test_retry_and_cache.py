...
def test_github_retry_logic():
    ...
    result = gh._run_command(["repo", "list", "--limit", "3"], max_retries=2, base_delay=0.5)
...
def test_shared_cache():
    ...
    cache = configure_cache(default_ttl=300, max_cache_size=500, enable_persistence=True)
...
def test_retry_scenarios():
    ...
    for scenario in scenarios:
        print(f"\n   {scenario['name']}:")
        print(
            f"     max_retries={scenario['max_retries']}, "
        ...
        for attempt in range(scenario["max_retries"]):
            delay = min(scenario["base_delay"] * (2**attempt), scenario["max_delay"])
            delays.append(delay)
