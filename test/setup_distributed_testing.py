...
def check_prometheus_installation():
    ...
    try:
        ...
        result = subprocess.run(
            ["which", "prometheus"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def check_grafana_installation():
    ...
    try:
        ...
        result = subprocess.run(
            ["which", "grafana-server"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", default="./distributed_testing_data", help="Base directory for framework data"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force creation of directories even if they exist"
    )
    parser.add_argument(
        "--skip-dependencies", action="store_true", help="Skip installing Python dependencies"
    )
    parser.add_argument(
