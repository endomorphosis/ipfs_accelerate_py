...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("fix_typescript_syntax.log")],
)
...
def fix_init_files(file_path: str) -> Tuple[str, int]:
    ...
    if Config.FIX_FILES:
        ...
        for file in os.listdir(dir_name):
            if (
                file.endswith(".ts")
                and file not in ["__init__.ts", "__pycache__.ts", "index.ts"]
...
def fix_python_class(content: str) -> Tuple[str, int]:
    ...
    def class_replacement(match):
        ...
        if inheritance:
            ...
            inheritance = re.sub(
                r"\([^)]*\)",
                lambda m: m.group(0).replace("(", " extends ").replace(")", ""),
                inheritance,
            )
...
def generate_report():
    ...
    report_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "typescript_syntax_fixes_report.md"
    )
