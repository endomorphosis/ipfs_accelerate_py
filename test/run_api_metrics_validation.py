...
try:
    from data.duckdb.simulation_validation.api_metrics import (
        DuckDBAPIMetricsRepository,
        APIMetricsValidator,
    )
except ImportError:
    try:
        from ipfs_accelerate_py.duckdb_api.simulation_validation.api_metrics import (
            DuckDBAPIMetricsRepository,
            APIMetricsValidator,
        )
    except ImportError:
        from test.duckdb_api.simulation_validation.api_metrics import (
            DuckDBAPIMetricsRepository,
            APIMetricsValidator,
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--db-path", type=str, default="api_metrics.duckdb", help="Path to DuckDB database file"
    )

    parser.add_argument(
        "--generate-sample", action="store_true", help="Generate sample data in the DuckDB database"
    )

    parser.add_argument(
        "--num-samples", type=int, default=1000, help="Number of sample records to generate"
    )

    parser.add_argument(
        "--days-back", type=int, default=30, help="Number of days back to generate data for"
    )

    parser.add_argument("--endpoint", type=str, help="Filter by specific API endpoint")

    parser.add_argument("--model", type=str, help="Filter by specific model")

    parser.add_argument(
        "--output", type=str, help="Path to output JSON file for validation results"
    )
...
def run_validation(args):
    ...
    try:
        ...
        repository = DuckDBAPIMetricsRepository(db_path=args.db_path, create_if_missing=True)

        # Generate sample data if requested
        if args.generate_sample:
            logger.info(f"Generating {args.num_samples} sample records over {args.days_back} days")
            repository.generate_sample_data(num_records=args.num_samples, days_back=args.days_back)
        ...
        if args.report_type == "full":
            ...
            results = validator.generate_validation_report(
                start_time=start_time, end_time=end_time, endpoint=args.endpoint, model=args.model
            )
...
def print_summary(results, report_type):
    ...
    if report_type == "full":
        ...
        print(
            f"- Prediction Accuracy: {results.get('prediction_accuracy', {}).get('accuracy', 0.0):.2f}"
        )
        print(
            f"- Anomaly Detection: {results.get('anomaly_detection', {}).get('effectiveness', 0.0):.2f}"
        )
        print(
            f"- Recommendation Quality: {results.get('recommendation_relevance', {}).get('overall_quality', 0.0):.2f}"
        )

        print("\nTop Recommendations:")
        for i, rec in enumerate(results.get("all_recommendations", [])[:5], 1):
            print(
                f"{i}. [{rec.get('category', 'General')} - {rec.get('priority', 'medium').upper()}] {rec.get('issue', '')}"
            )
        ...
        for i, rec in enumerate(results.get("recommendations", []), 1):
            print(
                f"{i}. [{rec.get('priority', 'medium').upper()}] {rec.get('issue', '')}: {rec.get('recommendation', '')}"
            )
