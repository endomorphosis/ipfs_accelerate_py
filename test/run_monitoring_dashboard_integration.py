...
def synchronize_visualizations(
    visualization_dir, dashboard_url=None, api_key=None, file_pattern="*.html", recursive=True
):
    """
    ...
    num_synced = integration.synchronize_visualizations(
        visualization_dir=visualization_dir, file_pattern=file_pattern, recursive=recursive
    )
...
def create_dashboard_panel(
    ...
    # Synchronize visualizations
    num_synced = integration.synchronize_visualizations(
        visualization_dir=visualization_dir, file_pattern="*.html", recursive=True
    )
    ...
    panel_id = integration.create_dashboard_panel(
        panel_title=panel_title, visualization_ids=viz_ids, layout=layout, dashboard_id=dashboard_id
    )
...
def run_visualization_and_sync(
    ...
    for i in range(min(2, num_visualizations)):
        viz_path = vis.create_hardware_comparison_heatmap(
            metric="throughput", batch_size=1, title=f"Hardware Comparison Heatmap {i + 1}"
        )
        if viz_path:
            viz_paths.append(viz_path)
    ...
    num_synced = synchronize_visualizations(
        visualization_dir=output_dir, dashboard_url=dashboard_url, api_key=api_key
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="./advanced_visualizations",
        help="Directory containing visualizations",
    )
    parser.add_argument(
        "--mode",
        choices=["sync", "auto-sync", "panel", "run-and-sync"],
        default="sync",
        help="Operation mode",
    )
    parser.add_argument(
        "--interval", type=int, default=60, help="Interval between synchronization runs (seconds)"
    )
