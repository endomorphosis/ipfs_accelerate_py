...
class TestCredentialManager:
    ...
    def test_store_and_get_credential(self, manager):
        ...
        manager.store_credential(
            name="TEST_TOKEN", value="secret_value_123", scope=CredentialScope.GLOBAL
        )
    ...
    def test_store_scoped_credential(self, manager):
        ...
        value = manager.get_credential("REPO_TOKEN", repo="owner/repo")
        assert value == "repo_secret"

        # Should fail with wrong scope
        value = manager.get_credential("REPO_TOKEN", repo="other/repo")
    ...
    def test_rotate_credential(self, manager):
        ...
        manager.store_credential(name="ROTATE_TEST", value="original_value")
        ...
        # Rotate to new value
        success = manager.rotate_credential("ROTATE_TEST", "new_value")
        assert success
    ...
    def test_delete_credential(self, manager):
        ...
        # Store credential
        manager.store_credential(name="TO_DELETE", value="delete_me")
    ...
    def test_persistence(self, temp_storage_dir):
        ...
        manager1 = CredentialManager(storage_dir=temp_storage_dir, use_keyring=False)
        manager1.store_credential("PERSIST", "persistent_value")

        # Create new manager with same storage
        manager2 = CredentialManager(storage_dir=temp_storage_dir, use_keyring=False)
...
class TestGlobalManager:
    ...
    def test_configure_manager(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = configure_credential_manager(storage_dir=tmpdir, use_keyring=False)
