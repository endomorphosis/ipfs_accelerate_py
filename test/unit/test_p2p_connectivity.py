...
class TestConnectivityConfig:
    ...
    def test_custom_config(self):
        ...
        config = ConnectivityConfig(
            enable_tcp=True, enable_quic=True, enable_mdns=False, dht_bucket_size=50
        )
...
class TestUniversalConnectivity:
    ...
    async def test_discover_peers_multimethod(self, connectivity):
        ...
        peers = await connectivity.discover_peers_multimethod(bootstrap_peers=bootstrap)
        ...
        class MockRegistry:
            def discover_peers(self, max_peers=10):
                return [
                    ...
                    {"peer_id": "QmTest456", "multiaddr": "/ip4/10.0.0.50/tcp/9001/p2p/QmTest456"},
                ]

        registry = MockRegistry()
        peers = await connectivity.discover_peers_multimethod(github_registry=registry)
    ...
    def test_connectivity_disabled_features(self):
        ...
        config = ConnectivityConfig(enable_mdns=False, enable_dht=False, enable_relay=False)
