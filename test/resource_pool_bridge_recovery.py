...
class ResourcePoolBridgeRecovery:
    ...
    def categorize_error(self, error: Exception, context: Dict[str, Any]) -> ErrorCategory:
        ...
        if any(
            term in error_str
            for term in ["connection", "websocket", "closed", "connect", "disconnect", "handshake"]
        ):
        ...
        if any(
            term in error_str
            for term in ["crashed", "terminated", "killed", "browser closed", "not responding"]
        ):
        ...
        if any(
            term in error_str
            for term in ["out of memory", "oom", "memory limit", "insufficient memory"]
        ):
        ...
        if any(
            term in error_str
            for term in ["unsupported", "not supported", "not implemented", "operation not allowed"]
        ):
        ...
        if any(
            term in error_str
            for term in ["webgpu not supported", "webnn not supported", "feature not available"]
        ):
        ...
        if any(
            term in error_str
            for term in ["model incompatible", "incompatible model", "not compatible"]
        ):
        ...
        if any(
            term in error_str
            for term in ["internal error", "implementation error", "assertion", "unexpected"]
        ):
    ...
    def determine_recovery_strategy(
        self, error_category: ErrorCategory, context: Dict[str, Any], attempt: int
    ) -> RecoveryStrategy:
        """
        ...
        strategies = self._error_strategy_map.get(
            error_category, self._error_strategy_map[ErrorCategory.UNKNOWN]
        )
        ...
        if platform and platform in self._platform_health:
            ...
            if platform_health < 0.3 and strategy != RecoveryStrategy.ALTERNATIVE_PLATFORM:
                logger.info(
                    f"Platform {platform} health score is low ({platform_health:.2f}), prioritizing platform switch"
                )
        ...
        if browser and browser in self._browser_health:
            ...
            if browser_health < 0.3 and strategy != RecoveryStrategy.ALTERNATIVE_BROWSER:
                logger.info(
                    f"Browser {browser} health score is low ({browser_health:.2f}), prioritizing browser switch"
                )
        ...
        if (
            ...
            and browser
            and browser != "firefox"
            and error_category in [ErrorCategory.TIMEOUT, ErrorCategory.UNSUPPORTED_OPERATION]
        ):
            logger.info(
                f"Audio model on {browser} encountered {error_category}, switching to Firefox"
            )
            return RecoveryStrategy.ALTERNATIVE_BROWSER
    ...
    def apply_recovery_strategy(
        ...
        if strategy == RecoveryStrategy.IMMEDIATE_RETRY:
            ...
            if (
                "hardware_preferences" in new_context
                and "priority_list" in new_context["hardware_preferences"]
            ):
        ...
        while attempt < self.max_retries:
            try:
                ...
                recovery_strategy = self.determine_recovery_strategy(
                    error_category, current_context, attempt
                )
                ...
                logger.info(
                    f"Categorized as {error_category.value}, applying {recovery_strategy.value}"
                )
    ...
    def _record_recovery_attempt(
        self, error_category: ErrorCategory, strategy: RecoveryStrategy, context: Dict[str, Any]
    ):
    ...
    def _update_performance_baseline(self, result: Any, context: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if latency:
                baseline["avg_latency_ms"] = (1 - alpha) * baseline[
                    "avg_latency_ms"
                ] + alpha * latency

            if throughput:
                baseline["avg_throughput"] = (1 - alpha) * baseline[
                    "avg_throughput"
                ] + alpha * throughput
    ...
    def _check_performance_degradation(self, result: Any, context: Dict[str, Any]) -> bool:
        ...
        if latency and baseline["avg_latency_ms"] > 0:
            ...
            if latency_ratio > 2.0:  # 2x slowdown
                logger.warning(
                    f"Performance degradation detected: latency {latency}ms vs baseline {baseline['avg_latency_ms']:.2f}ms"
                )
                return True
        ...
        if throughput and baseline["avg_throughput"] > 0:
            ...
            if throughput_ratio > 2.0:  # 2x slowdown
                logger.warning(
                    f"Performance degradation detected: throughput {throughput} vs baseline {baseline['avg_throughput']:.2f}"
                )
                return True
    ...
    def get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Get health scores
            browser_health = {
                b: self._browser_health[b]["health_score"] for b in self._browser_health
            }
            platform_health = {
                p: self._platform_health[p]["health_score"] for p in self._platform_health
            }
...
class ResourcePoolBridgeWithRecovery:
    ...
    def __init__(
        ...
        if self.mock_mode:
            ...
            try:
                ...
                self.integration = MockResourcePoolBridgeIntegration(
                ...
                logger.info(
                    "Created basic mock integration (MockResourcePoolBridgeIntegration not available)"
                )
        else:
        ...
        class BasicMockIntegration:
            ...
            def get_model(self, model_type, model_name, hardware_preferences=None):
                ...
                model = lambda inputs: {
                    ...
                    "metrics": {"latency_ms": 100.0, "throughput_items_per_sec": 10.0},
                }
    ...
    def get_model(self, model_type, model_name, hardware_preferences=None):
        ...
        if success and model:
            ...
            return self._wrap_model_with_recovery(
                model, model_id, model_type, model_name, final_context
            )
        else:
            return None
    ...
    def execute_concurrent(self, models_and_inputs):
        ...
        """
        # Create context for recovery
        context = {"operation": "execute_concurrent", "model_count": len(models_and_inputs)}
...
def run_example():
    ...
    bridge = ResourcePoolBridgeWithRecovery(
        max_connections=3, max_retries=3, fallback_to_simulation=True
    )
    ...
    audio_model = bridge.get_model(
        ...
        hardware_preferences={"priority_list": ["webgpu", "cpu"], "browser": "firefox"},
    )
