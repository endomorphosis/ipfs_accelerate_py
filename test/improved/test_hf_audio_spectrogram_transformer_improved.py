...
Usage:
    Replace AUDIO_SPECTROGRAM_TRANSFORMER, audio_spectrogram_transformer-base, text-classification with actual values.
"""
...
# Model configuration
MODEL_ID = "audio_spectrogram_transformer-base"  # e.g., "bert-base-uncased"
MODEL_NAME = "AUDIO_SPECTROGRAM_TRANSFORMER"  # e.g., "bert"
TASK_TYPE = "text-classification"  # e.g., "text_embedding"
...
class TestInference:
    ...
    def test_deterministic_output(self, model_and_tokenizer, sample_inputs):
        ...
        assert ModelTestUtils.compare_outputs(
            output1.last_hidden_state, output2.last_hidden_state
    ...
    def test_batch_inference(self, model_and_tokenizer):
        ...
        for batch_size in [1, 2, 4]:
            inputs = ModelTestUtils.create_sample_text_inputs(
                tokenizer, texts=["test"] * batch_size
            )
...
class TestPerformance:
    ...
    def test_inference_speed(self, model_and_tokenizer, sample_inputs):
        ...
        timing_stats = ModelTestUtils.measure_inference_time(
            model, sample_inputs, warmup_runs=3, test_runs=10
        )

        # Print performance report
        report = PerformanceTestUtils.create_performance_report(MODEL_NAME, timing_stats)
        print(report)

        # Assert reasonable performance (adjust threshold as needed)
        assert timing_stats["mean"] < 1.0, f"Inference too slow: {timing_stats['mean']:.4f}s"
