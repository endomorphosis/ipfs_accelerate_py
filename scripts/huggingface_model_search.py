...
class HuggingFaceModelSearch:
        ...
        try:
            ...
            with open(self.models_cache_file, "r", encoding="utf-8") as f:
                ...
                self.last_update = datetime.fromisoformat(
                    cache_data.get("last_update", datetime.now().isoformat())
                )
    ...
    async def _process_model(self, model) -> Optional[Dict]:
        ...
        try:
            model_data = {
                "id": model.modelId,
                "author": getattr(model, "author", "") or model.modelId.split("/")[0]
                if "/" in model.modelId
                else "",
                ...
                "created_at": getattr(model, "createdAt", "").isoformat()
                if hasattr(getattr(model, "createdAt", ""), "isoformat")
                else str(getattr(model, "createdAt", "")),
                "last_modified": getattr(model, "lastModified", "").isoformat()
                if hasattr(getattr(model, "lastModified", ""), "isoformat")
                else str(getattr(model, "lastModified", "")),
            ...
            try:
                ...
                if card and card.data:
                    model_data["description"] = (
                        getattr(card.data, "description", "") or getattr(card, "text", "")[:200]
                        if hasattr(card, "text")
                        else ""
                    )
                    model_data["languages"] = (
                        getattr(card.data, "language", [])
                        or getattr(card.data, "languages", [])
                        or []
                    )
                    model_data["card_data"] = (
                        card.data.to_dict() if hasattr(card.data, "to_dict") else {}
                    )
    ...
    async def _build_bm25_index(self):
        ...
        try:
            ...
            tokenized_texts = [model["searchable_text"].split() for model in self.models_data]
    ...
    async def search_models(
        ...
        try:
            ...
            for rank, (original_idx, score, model) in enumerate(paginated_results, 1):
                result = {**model, "search_score": round(score, 4), "rank": rank + offset}
            ...
            return {"results": [], "total": 0, "offset": offset, "limit": limit, "error": str(e)}
    ...
    def _apply_filters(self, models: List[Dict], filters: Dict) -> List[Dict]:
        ...
        if "language" in filters and filters["language"]:
            ...
            filtered = [
                m
                for m in filtered
                if any(language in lang.lower() for lang in m.get("languages", []))
            ]
    ...
    async def _perform_search(
        self, query: str, search_type: str, models: List[Dict]
    ...
    async def _hybrid_search(self, query: str, models: List[Dict]) -> List[Tuple[int, float, Dict]]:
        ...
        try:
            ...
            if vector_results:
                ...
                vector_results = [
                    (idx, score / max_vector, model) for idx, score, model in vector_results
                ]

            if bm25_results:
                max_bm25 = max(result[1] for result in bm25_results)
                bm25_results = [
                    (idx, score / max_bm25, model) for idx, score, model in bm25_results
                ]
    ...
    def _sort_results(
        self, results: List[Tuple[int, float, Dict]], sort_by: str, sort_order: str
    ) -> List[Tuple[int, float, Dict]]:
        """Sort search results."""
    ...
    def get_search_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "top_libraries": dict(
                    sorted(libraries.items(), key=lambda x: x[1], reverse=True)[:10]
                ),
