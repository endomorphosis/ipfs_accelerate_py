...
class ProductionHFScraper(EnhancedModelScraper):
            ...
            while True:
                ...
                try:
                    async with session.get(url, params=params) as response:
                        ...
                        print(
                            f"\r🔍 Fetched {len(models):,} models (page {page})", end="", flush=True
                        )
    ...
    def extract_model_metadata(self, model_data: Dict[str, Any]) -> ModelRecord:
        ...
        for tag in tags:
            if any(
                arch in tag.lower()
                for arch in ["gpt", "bert", "t5", "bart", "roberta", "distilbert"]
            ):
    ...
    async def scrape_production_models(self, limit: int = None) -> Dict[str, Any]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(self.extract_model_metadata, model_data)
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                try:
                    ...
                    if i % 1000 == 0:
                        print(
                            f"\r📊 Processed {i:,}/{len(raw_models):,} models", end="", flush=True
                        )
