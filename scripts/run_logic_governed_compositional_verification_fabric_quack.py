...
* ``bootstrap`` materializes the exact tracked candidate projection and
  atomically publishes one no-overwrite run-v31 database with provenance;
...
* ``launch`` owns run-v31 in-process, starts exactly one foreground
...
SUCCESSOR_RUN_RELATIVE: Final = PROGRAM_ROOT_RELATIVE / "run-v31"
...
SUCCESSOR_STORE_GENERATION: Final = "lgcvf-run-v31"
...
def _load_native_resume_config(root: Path) -> tuple[dict[str, Any], bytes]:
    """Load the exact tracked run-v31 profile with duplicate-key rejection."""
...
def bootstrap_native_resume(root: Path = ROOT) -> dict[str, Any]:
    """Atomically publish run-v31 from the tracked candidate projection."""
