...
* ``bootstrap`` materializes the exact tracked candidate projection and
  atomically publishes one no-overwrite run-v32 database with provenance;
...
* ``launch`` owns run-v32 in-process, starts exactly one foreground
...
SUCCESSOR_RUN_RELATIVE: Final = PROGRAM_ROOT_RELATIVE / "run-v32"
...
SUCCESSOR_STORE_GENERATION: Final = "lgcvf-run-v32"
...
def _load_native_resume_config(root: Path) -> tuple[dict[str, Any], bytes]:
    """Load the exact tracked run-v32 profile with duplicate-key rejection."""
...
def bootstrap_native_resume(root: Path = ROOT) -> dict[str, Any]:
    """Atomically publish run-v32 from the tracked candidate projection."""
