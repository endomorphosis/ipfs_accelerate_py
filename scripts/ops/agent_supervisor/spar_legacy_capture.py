...
This module never signals a native process or changes unit policy, markers,
source, or a live database. An explicit method attaches one isolated workflow
sentinel to a separately inhibited unit. A session must be armed against the
actual native owner before it exits. JSON reports cannot recreate a session.
Its copied queue is not callback settlement, signing authority, source
...
from .spar_merge_owner_handoff import configured_queue_root, _scopes
...
UNIT_FIELDS = (
    ...
    "TimeoutStopUSec", "RefuseManualStart", "NeedDaemonReload", "Delegate", "ExitType",
)
SENTINEL_CODE = "import sys;print('SPAR-CAPTURE-SENTINEL-READY',flush=True);sys.stdin.buffer.read(1)"
...
class RetainedNativeLegacySession:
    ...
    def __init__(self, *, repository_root, config_path, expected_source_commit,
        ...
        self.sentinel_close_receipt = None
        ...
        try:
            ...
            self._bootstrap = _json(self.paths["bootstrap_receipt"])
            bootstrap_body = {key: value for key, value in self._bootstrap.items() if key != "bootstrap_receipt_id"}
            _require(self._bootstrap.get("bootstrap_receipt_id") == self.operator._identity(bootstrap_body)
                     and type(self._bootstrap.get("plan_root_cid")) is str,
                     "native bootstrap namespace does not rehash")
            from types import SimpleNamespace
            self._coordinates = {
                "repository_id": _repository_id(self.root),
                "target_branch": str(self.board.payload.get("merge_target_branch") or ""),
                "store_id": str(self.queue_root / "merge_queue.duckdb"),
                "source_commit": self._source["head"], "source_tree": self._source["tree"],
                "scope_bindings": _scopes(board=self.board, paths=self.paths, amendment=SimpleNamespace(
                    launch_config_cid="sha256:" + self._source["config_sha256"],
                    bootstrap_plan_root_cid=self._bootstrap["plan_root_cid"])),
            }
    ...
    def _inhibition(self, *, require_hold=True):
        unit = _unit()
        ...
        if not require_hold:
            _require(not self.hold_path.exists() and not self.hold_path.is_symlink(),
                     "native sentinel must be retained before the hold is placed")
            return unit, None
        hold = _bytes(self.hold_path, 65536)
        _require(hold, "native capture hold is empty")
        return unit, hashlib.sha256(hold).hexdigest()

    def retain_workflow_sentinel(self):
        """Attach exactly one reviewed no-fork/no-state-access child before stop.

        Delegate=yes and ExitType=cgroup are independently applied unit policy.
        The helper starts at / with an empty credential environment and no
        inherited authority descriptors. Its only action after READY is waiting
        for its private stdin to close. This is an explicit local test/workflow
        actor, not a supervisor, provider, or daemon.
        """
        from ipfs_accelerate_py.agent_supervisor.merge.worktree_lifecycle import read_process_birth

        _require(not self._closed and self._sentinel is None, "native sentinel already retained or session closed")
        unit, _ = self._inhibition(require_hold=False)
        _require(unit["Delegate"] == "yes" and unit["ExitType"] == "cgroup"
                 and unit["MainPID"] == str(self.pid) and _alive(self.birth)
                 and not _exited(self.pidfd) and _cgroup_population(self.cgroup_fd) == ((self.pid,), "1"),
                 "native sentinel requires the inhibited original owner alone")
        process = subprocess.Popen(
            ["/usr/bin/python3", "-I", "-S", "-c", SENTINEL_CODE],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            cwd="/", env={"PATH": "/usr/bin:/bin", "LANG": "C.UTF-8"}, close_fds=True,
        )
        self._sentinel = process
        self._sentinel_pidfd = os.pidfd_open(process.pid, 0)
        self._resources.callback(os.close, self._sentinel_pidfd)
        self._resources.callback(self._close_sentinel)
        birth = read_process_birth(process.pid)
        _require(birth is not None, "workflow sentinel birth unavailable")
        self._sentinel_birth = birth.to_dict()
        poller = select.poll()
        poller.register(process.stdout.fileno(), select.POLLIN)
        _require(poller.poll(5000), "workflow sentinel readiness unavailable")
        _require(os.read(process.stdout.fileno(), 128) == b"SPAR-CAPTURE-SENTINEL-READY\n",
                 "workflow sentinel readiness differs")
        subprocess.run(
            ["busctl", "--user", "call", "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
             "org.freedesktop.systemd1.Manager", "AttachProcessesToUnit", "ssau", UNIT, "", "1", str(process.pid)],
            capture_output=True, text=True, check=True, timeout=10,
        )
        _require(self._inhibition(require_hold=False)[0] == unit
                 and self._source_binding() == self._source and _alive(self.birth)
                 and set(_cgroup_population(self.cgroup_fd)[0]) == {self.pid, process.pid}
                 and _alive(self._sentinel_birth) and not _exited(self._sentinel_pidfd),
                 "native sentinel attachment changed the admitted population")
        return {"birth": self._sentinel_birth, "code_sha256": hashlib.sha256(SENTINEL_CODE.encode()).hexdigest(),
                "role": "workflow_sentinel_without_native_state_access"}

    def _close_sentinel(self):
        process = self._sentinel
        if process is None:
            return
        if not process.stdin.closed:
            process.stdin.close()
        process.wait(timeout=10)
        _require(process.returncode == 0 and self._sentinel_pidfd is not None
                 and _exited(self._sentinel_pidfd), "workflow sentinel did not prove its own normal exit")
        self.sentinel_close_receipt = {"birth": self._sentinel_birth, "pidfd_exit_observed": True, "returncode": 0}
    ...
    def _closed_gate(self):
        ...
        population = {"cgroup_empty_observed": False, "workflow_sentinel": None}
        if self._sentinel is not None:
            _require(unit["MainPID"] == "0" and unit["ActiveState"] == "active"
                     and unit["SubState"] == "running" and unit["Delegate"] == "yes"
                     and unit["ExitType"] == "cgroup", "native unit does not retain only its workflow sentinel")
            _require(_alive(self._sentinel_birth) and not _exited(self._sentinel_pidfd)
                     and _cgroup_population(self.cgroup_fd) == ((self._sentinel.pid,), "1"),
                     "native cgroup has an unknown actor or lost its sentinel")
            population["workflow_sentinel"] = {
                "birth": self._sentinel_birth,
                "code_sha256": hashlib.sha256(SENTINEL_CODE.encode()).hexdigest(),
                "observed_cgroup_members": [self._sentinel.pid],
                "role": "workflow_sentinel_without_native_state_access",
            }
        else:
            ...
            population["cgroup_empty_observed"] = True
        ...
        _require(self._source_binding() == self._source and _json(self._broker_path) == self._broker
                 and _json(self.paths["bootstrap_receipt"]) == self._bootstrap,
    ...
    def import_context(self, *, queue_policy=None):
        return role._decode(role._json({**self._coordinates, "queue_policy": dict(
            role.DEFAULT_QUEUE_POLICY if queue_policy is None else queue_policy)}))

    def capture(self, *, destination, inspection_destination, context=None):
        ...
        context = self.import_context() if context is None else context
        _require(all(context.get(key) == value for key, value in self._coordinates.items()),
