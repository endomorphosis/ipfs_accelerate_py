...
# The published additive control-plane catalog creates this derived relation.
# It may be introduced empty; it is not owner metadata and cannot receive
# domain rows as a side effect of queue migration or acquire semantic authority.
ADDITIVE_DOMAIN_TABLES = frozenset({"hash_observations"})
...
def prepare_offline_clone(
    *, offline_root: Path, destination: Path, manifest: Mapping[str, Any]
    ...
    return PreparedQueueStore(
        ...
        tuple(receipts),
        tuple(cursors),
        cursor_normalizations=tuple(cursor_normalizations),
    )
...
def start_queue_owner(
    ...
    *,
    ...
    # start() owns its failed-start cleanup and may retain unresolved custody.
    # A second stop without an admitted identity must neither mask its error
    # nor attempt to turn that retained boundary into a fresh owner.
    server.start()
    try:
