...
CONTROL_ARTIFACT_OWNERSHIP: tuple[dict[str, Any], ...] = (
    {
        ...
        "path": "config/external_agent_autonomous_execution_fabric_casf_import.json",
...
TASK_ROWS: tuple[
    ...
    ("180", "EAAEF-G190", "Inventory host-gated launch-plan blockers into a machine receipt", "ipfs_accelerate_py", _paths("docs/architecture/external_agent_autonomous_execution_fabric/receipts/host_admission/blocker_inventory.json", "docs/architecture/external_agent_autonomous_execution_fabric/receipts/host_admission/held_board_catalog.json", "ipfs_accelerate_py/agent_supervisor/validation/eaaef_host_admission.py", "scripts/collect_eaaef_host_admission_receipts.py", "scripts/ingest_eaaef_held_board_into_duckdb.py", "scripts/run_eaaef_host_admission_supervisor.py", "test/api/test_eaaef_host_admission_unblocking.py", "test/api/test_eaaef_duckdb_board_catalog.py"), (), "Capture the current isolated launch-plan no-go, classify every blocker as auto_recoverable, host_source_commit_required or host_gated_external_authority, and publish a content-addressed inventory that later S tasks must close or restate as typed missing evidence.", "python3 -m pytest -q test/api/test_eaaef_host_admission_unblocking.py -k inventory"),
    ...
    ("184", "EAAEF-G190", "Obtain independently signed EAAEF-scoped provider authorization", "ipfs_accelerate_py", _paths("docs/architecture/external_agent_autonomous_execution_fabric/receipts/host_admission/provider_authorization.json", "scripts/issue_eaaef_provider_authorization.py"), ("EAAEF-181",), "Bind a source-addressed EAAEF provider-authorization artifact for the grok_cli/codex route that an independent signer, not the prospective supervisor, attests; unsigned or self-signed material is a typed no-go.", "python3 -m pytest -q test/api/test_eaaef_host_admission_unblocking.py -k provider_authorization"),
    ...
    ("191", "EAAEF-G190", "Sign the EAAEF-000 admission or typed no-go bundle", "ipfs_accelerate_py", _paths("docs/architecture/external_agent_autonomous_execution_fabric/receipts/host_admission/admission_bundle.json", "docs/architecture/external_agent_autonomous_execution_fabric/receipts/host_admission/admission_bundle.signatures.json", "scripts/issue_eaaef_admission_bundle.py"), ("EAAEF-184", "EAAEF-186", "EAAEF-187", "EAAEF-188", "EAAEF-190"), "Join the S-epic receipts, current materialization identity and launch-plan statement into one create-once bundle that an independent operator and security reviewer sign as admitted or no_go; the prospective supervisor cannot sign or start before this bundle verifies.", "python3 -m pytest -q test/api/test_eaaef_host_admission_unblocking.py -k admission_bundle"),
)
...
def _render_board(board: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "The per-task `Status` fields below are the sealed bootstrap projection, not a mutable runtime dashboard. Never replay DuckDB, DuckLake, a historical receipt, or a generated Markdown export into those fields.",
        "",
        "Request current authority only through the typed owner: `python3 scripts/run_eaaef_reconciliation.py --repo-root . --state-root data/agent_supervisor/external_agent_autonomous_execution_fabric/reconciliation-v2 status`. A stopped, stale, unqualified, or unavailable owner must return a typed no-go instead of overlaying this board. Preserved pre-reboot status artifacts are recovery history until a live owner binds their exact board, source forest, generation, fence, and process birth.",
        "",
        "Current-tree source reconciliation found implementation surfaces for 116/116 tasks and all 196 declared validation paths. That is source presence, not receipt-backed operational completion. After recovery of the deterministic EAAEF-001 source-reconciliation receipt, the remaining absent declared evidence outputs are EAAEF-097, EAAEF-141, EAAEF-144, and EAAEF-145; each requires genuine live, external-authority, or container evidence and must not be synthesized from historical DuckDB state.",
        "",
