...
try:
    ...
    pd = type(
        "MockPandas", (), {"DataFrame": MockDataFrame, "read_parquet": lambda x: MockDataFrame()}
    )()
    np = type(
        "MockNumpy",
        (),
        {
            ...
            "random": type(
                ...
                (),
                {
                    "normal": lambda *args: [0.0] * 384,
                    "choice": lambda *args, **kwargs: args[0][0] if args else "mock",
                    "randint": lambda *args: 1,
...
class EnhancedModelScraper:
    ...
    def create_mock_comprehensive_dataset(self, size: int = 1000) -> List[ModelRecord]:
        ...
        model_templates = [
            ...
            {
                "id": "electra-small-discriminator",
                "name": "ELECTRA Small",
                "author": "google",
                "arch": "ELECTRA",
                "task": "fill-mask",
        ...
        for i in range(size):
            ...
            compatibility_score = min(
                100, max(0, 90 - (gpu_memory * 0.001) + (10 if base_size < 1000 else 0))
            )
            ...
            model = ModelRecord(
                ...
                library_name=np.random.choice(
                    ["transformers", "sentence-transformers", "diffusers", "timm"]
                ),
                pipeline_tag=task,
                task_type=task,
                architecture=arch,
                ...
                languages=np.random.choice(
                    ["en", "multilingual", "de", "fr", "es", "zh"],
                    size=np.random.randint(1, 3),
                    replace=False,
                ).tolist(),
                datasets=np.random.choice(
                    ["common_crawl", "wikipedia", "bookcorpus", "openwebtext"],
    ...
    def build_search_index(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            text = (
                f"{row['model_name']} {row['architecture']} {row['task_type']} {row['description']}"
            )
        ...
        if embedding_matrix is not None:
            ...
            tfidf_normalized = tfidf_reduced / (
                np.linalg.norm(tfidf_reduced, axis=1, keepdims=True) + 1e-8
            )
        ...
        # Save index
        index_data = {"vectorizer": self.vectorizer, "model_index": self.model_index}
            ...
            if idx in self.model_index:
                ...
                if filters:
                    if "task_type" in filters and filters["task_type"] != result["task_type"]:
                        continue
                    if (
                        "max_size_mb" in filters
                        and result["model_size_mb"] > filters["max_size_mb"]
                    ):
                        continue
                    if (
                        "min_downloads" in filters
                        and result["downloads"] < filters["min_downloads"]
                    ):
                        continue
    ...
    def scrape_all_models(self, limit: int = None, mock_mode: bool = True) -> Dict[str, Any]:
        ...
        results = {
            ...
            "storage_size_mb": self.parquet_path.stat().st_size / (1024 * 1024)
            ...
            stats.update(
                {
                    ...
                    "size_distribution": {
                        ...
                        "medium_models_500mb_5gb": len(
                            df[(df["model_size_mb"] >= 500) & (df["model_size_mb"] < 5000)]
                        ),
                ...
                if query:
                    ...
                    if results:
                        ...
                        for i, result in enumerate(results, 1):
                            ...
                            print(
                                f"      Architecture: {result['architecture']} | Task: {result['task_type']}"
                            )
                            print(
                                f"      Size: {result['model_size_mb']:.1f}MB | Similarity: {result['similarity_score']:.3f}"
                            )
                            print(
                                f"      Downloads: {result['downloads']:,} | Likes: {result['likes']:,}"
                            )
                            print()
                    else:
