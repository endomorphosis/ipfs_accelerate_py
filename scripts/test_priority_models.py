...
def run_model_test(test_file, update_baselines=False, hardware="cpu"):
    ...
    if not test_path.exists():
        return {"status": "SKIP", "reason": "Test file not found", "output": ""}
    ...
    try:
        ...
        return {"status": "TIMEOUT", "reason": "Test timed out after 5 minutes", "output": ""}
...
def main():
    parser = argparse.ArgumentParser(description="Test priority models and establish baselines")
    parser.add_argument(
    ...
    parser.add_argument(
        "--models", nargs="+", help="Specific models to test (default: all priority models)"
    )
    ...
    for i, model in enumerate(models_to_test, 1):
        ...
        status_icon = {"PASS": "✅", "FAIL": "❌", "SKIP": "⏭️", "TIMEOUT": "⏱️", "ERROR": "⚠️"}.get(
            result["status"], "❓"
        )
