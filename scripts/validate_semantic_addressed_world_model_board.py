...
_M69_AUTHORITY_CID = (
    "sha256:8123f23a73059764286dc69c9edfbf230b7623b2d7c324a603c983c181d94a9a"
)
_M69_AUTHORITY_SIZE = 20_260
_M69_UNSEALED_AUTHORITY_CID = "sha256:PENDING_M69_FINAL_CONTROL_AUTHORITY_CID"
_M69_SUCCESSOR_KEY = (
    "post_m68_stopped_owner_missing_client_token_vault_restart_successor_materialization"
)
...
def _m58_migration_errors(
    ...
    *,
    ...
    """Validate M58's generation-43/event-328 restart authority."""

    try:
        module = _dependency_validator_module(REPO_ROOT)
        return list(
            module._m58_post_m57_stall_unblock_and_shutdown_fence_restart_errors(
                scheduler,
                seal,
                migration,
                root=REPO_ROOT,
        ...
        return [f"M58 migration validator unavailable: {type(exc).__name__}: {exc}"]


def _m59_migration_errors(
    scheduler: Mapping[str, Any],
    seal: Mapping[str, Any],
    migration: Mapping[str, Any],
    *,
...
def _m69_migration_errors(
    ...
    """Validate M69's stopped generation-46 missing-vault generation-47 restart."""

    try:
        module = _dependency_validator_module(REPO_ROOT)
        return list(
            module._m69_post_m68_stopped_owner_missing_client_token_vault_restart_errors(
                scheduler, seal, migration, root=REPO_ROOT
            )
        )
    except Exception as exc:
        return [f"M69 migration validator unavailable: {type(exc).__name__}: {exc}"]
...
def _m57_migration_errors(
    ...
    *,
    ...
    """Validate M57's token-vault generation-42 restart authority."""
...
def _active_successor_migration_errors(
    ...
    *,
    ...
    if any(m69_presence):
        errors = _m69_migration_errors(scheduler, seal, migration)
        if not all(m69_presence):
            errors.append("M69 successor authority is only partially declared")
        if errors:
            return errors
        ...
        if not all(m68_presence):
            return [
                "M69 successor does not preserve the immutable M68 controls"
            ]
        historical_scheduler = dict(scheduler)
        historical_migration = dict(migration)
        historical_seal = dict(seal)
        historical_scheduler.pop(m69_key, None)
        historical_migration.pop(m69_key, None)
        historical_seal.pop(f"{m69_key}_cid", None)
        errors.extend(
    ...
    if any(m63_presence):
        ...
        historical_migration = dict(migration)
        historical_seal = dict(seal)
        historical_scheduler.pop(m63_key, None)
        historical_migration.pop(m63_key, None)
        historical_seal.pop(f"{m63_key}_cid", None)
        return _active_successor_migration_errors(
            historical_scheduler,
            historical_seal,
            historical_migration,
    ...
    if any(m61_presence):
        ...
        historical_migration = dict(migration)
        historical_seal = dict(seal)
        historical_scheduler.pop(m61_key, None)
        historical_migration.pop(m61_key, None)
        historical_seal.pop(f"{m61_key}_cid", None)
        return _active_successor_migration_errors(
            historical_scheduler,
            historical_seal,
            historical_migration,
    ...
    if any(m59_presence):
        ...
        historical_migration = dict(migration)
        historical_seal = dict(seal)
        historical_scheduler.pop(m59_key, None)
        historical_migration.pop(m59_key, None)
        historical_seal.pop(f"{m59_key}_cid", None)
        return _active_successor_migration_errors(
            historical_scheduler,
            historical_seal,
            historical_migration,
    ...
    if any(m58_presence):
        ...
        if not all(m58_presence):
            errors.append("M58 successor authority is only partially declared")
        if errors:
            return errors
...
def validate_program(repo_root: Path | str = REPO_ROOT) -> dict[str, Any]:
    ...
    m68_key = _M68_SUCCESSOR_KEY
    m68_selected = m69_selected or any((
        m68_key in config,
        m68_key in migration,
        f"{m68_key}_cid" in seal,
    ))
    ...
    m62_key = _M62_SUCCESSOR_KEY
    # Treat M63 as an M62-or-newer sentinel for historical-selection guards;
    # the exact M62 validation block below excludes M63 explicitly.
    m62_selected = m63_selected or any((
        m62_key in config,
        m62_key in migration,
        f"{m62_key}_cid" in seal,
    ))
    ...
    m60_key = _M60_SUCCESSOR_KEY
    # Treat M61 as an M60-or-newer sentinel for all historical-selection
    # guards below; the exact M60 validation block excludes M61 explicitly.
    m60_selected = m61_selected or any((
        m60_key in config,
        m60_key in migration,
        f"{m60_key}_cid" in seal,
    ))
    ...
    if (
        ...
        config_errors.append(
            "four lanes are required for M63/M62/M61/M60/M59/M58/M48/M47/M46/M45/M44/M43/M42/M41/M40/M39/M38/M37/M36/M35/M34/M33/M32/M31/M30/M29/M28/M27/M26/M25/M24/M23"
    ...
    if m69_selected:
        ...
        try:
            ...
            reference = dict(materializer._m69_authority_reference())
            contract = materializer._validated_m69_live_preflight_contract(expected)
            ...
            if (
                ...
                config_errors.append(
                    "M69 stopped-owner token-vault restart authority differs"
                )
        except Exception as exc:
            config_errors.append(
                f"M69 stopped-owner token-vault restart authority unavailable: "
                f"{type(exc).__name__}: {exc}"
            )
    elif m68_selected:
    ...
    if m63_selected and not m64_selected:
        ...
        try:
            ...
            if (
                ...
                or _M63_AUTHORITY_CID == _M63_UNSEALED_AUTHORITY_CID
                or materializer._M63_AUTHORITY_CID != _M63_AUTHORITY_CID
                or materializer._M63_AUTHORITY_SIZE != _M63_AUTHORITY_SIZE
                or configured is not True
                or expected.get("authorized") is not True
                or expected.get("authority") != "operator_control_plane"
                or expected.get("migration_revision") != "SAWM-R2-M63"
                or expected.get("migration_kind") != m63_key
                ...
                m63_errors.append("M63 successor authority binding differs")
            config_errors.extend(m63_errors)
        except Exception as exc:
            config_errors.append(
                f"M63 successor authority unavailable: {type(exc).__name__}: {exc}"
            )
    if m62_selected and not m63_selected:
    ...
    if m61_selected and not m62_selected:
        ...
        try:
            ...
            if (
                ...
                or _M61_AUTHORITY_CID == _M61_UNSEALED_AUTHORITY_CID
                or materializer._M61_AUTHORITY_CID != _M61_AUTHORITY_CID
                or materializer._M61_AUTHORITY_SIZE != _M61_AUTHORITY_SIZE
                or configured is not True
                or expected.get("authorized") is not True
                or expected.get("authority") != "operator_control_plane"
                or expected.get("migration_revision") != "SAWM-R2-M61"
                or expected.get("migration_kind") != m61_key
                or expected.get("target_generation") != 43
                or expected.get("target_event_watermark") != 331
                or expected.get("target_projection_cid")
                != _M61_TARGET_PROJECTION_CID
                or contract.get("target_generation") != 43
                or contract.get("prior_event_watermark") != 330
                or contract.get("target_event_watermark") != 331
                or contract.get("prior_projection_cid")
                ...
                m61_errors.append("M61 successor authority binding differs")
            config_errors.extend(m61_errors)
        except Exception as exc:
            config_errors.append(
                f"M61 successor authority unavailable: {type(exc).__name__}: {exc}"
            )
    if m60_selected and not m61_selected:
    ...
    if m59_selected:
        ...
        try:
            ...
            if (
                ...
                or expected.get("migration_revision") != "SAWM-R2-M59"
                or expected.get("migration_kind") != m59_key
                or contract.get("target_generation") != 43
                or contract.get("target_event_watermark") != 329
                or contract.get("target_projection_cid")
                != _M59_TARGET_PROJECTION_CID
                or contract.get("same_live_owner_required") is not True
                or contract.get("generation_restart_authorized") is not False
            ):
                m59_errors.append("M59 successor authority binding differs")
            config_errors.extend(m59_errors)
        except Exception as exc:
            config_errors.append(
                f"M59 successor authority unavailable: {type(exc).__name__}: {exc}"
            )
    if m58_selected:
        successor = config.get(m58_key)
        try:
            module, materializer = _m26_validation_modules(root)
            expected = (
                materializer
            ...
            if (
                ...
                or expected.get("migration_revision") != "SAWM-R2-M58"
                or expected.get("migration_kind") != m58_key
                or contract.get("target_generation") != 43
                or contract.get("target_event_watermark") != 328
                or contract.get("target_projection_cid")
                != _M58_TARGET_PROJECTION_CID
            ):
                m58_errors.append("M58 successor authority binding differs")
            config_errors.extend(m58_errors)
        except Exception as exc:
            config_errors.append(
                f"M58 successor authority unavailable: {type(exc).__name__}: {exc}"
            )
    if m57_selected:
