...
class ModelManagerIntegration:
    ...
    def import_from_existing_metadata(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"Successfully imported {imported_count} models from existing metadata system"
            )
    ...
    def _convert_mapping_to_metadata(
        self, model_name: str, mapping_data: Dict[str, Any]
    ) -> ModelMetadata:
        """
    ...
    def _infer_model_type(self, architecture: str, model_name: str) -> ModelType:
        ...
        # Multimodal models
        if any(
            x in arch_lower for x in ["clip", "align", "flava", "layoutlm", "donut", "pix2struct"]
        ):
            return ModelType.MULTIMODAL

        # Encoder-decoder models
        if any(
            x in arch_lower for x in ["t5", "bart", "pegasus", "marian", "m2m", "encoder_decoder"]
        ):
    ...
    def _create_io_specs(
        self, model_type: ModelType, architecture: str
        ...
        if model_type == ModelType.AUDIO_MODEL:
            inputs.append(
                IOSpec(
                    ...
                    shape=(None, None),
                    description="Audio input values",
                )
            )
        ...
        if any(x in architecture.lower() for x in ["classification", "qa", "masked", "lm"]):
            outputs.append(
                IOSpec(
                    ...
                    shape=(None, None, None),
                    description="Classification/prediction logits",
                )
            )
    ...
    def populate_huggingface_models(self, model_list: List[str], fetch_configs: bool = True) -> int:
        """
        ...
        for model_id in model_list:
            try:
                ...
                if config:
                    metadata = create_model_from_huggingface(model_id=model_id, hf_config=config)
                else:
