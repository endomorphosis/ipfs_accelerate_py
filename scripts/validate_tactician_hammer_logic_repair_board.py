...
def _repository_git_dir(repo_root: Path) -> Path:
    """Resolve the repository-local git directory without invoking git."""
    ...
    if marker.is_file():
        try:
            ...
            raise RuntimeError("unable to read repository gitdir marker") from exc
        prefix = "gitdir:"
        if not marker_text.lower().startswith(prefix):
            raise RuntimeError("repository gitdir marker is malformed")
...
EXPECTED_RECONCILIATION_DISCOVERY_ROOT = (
    _repository_git_dir(REPO_ROOT)
...
def _expected_reconciliation_discovery_roots() -> frozenset[Path]:
    """Return default and explicitly configured supervisor-owned roots."""

    roots = {EXPECTED_RECONCILIATION_DISCOVERY_ROOT}
    configured_state_root = str(os.environ.get("LPR_STATE_ROOT") or "").strip()
    if configured_state_root:
        state_root = Path(configured_state_root).expanduser().resolve(strict=False)
...
def _validate_reconciliation_resolution_receipt(
    *,
    ...
    try:
        _require(
            not path.is_symlink(),
            f"{repair.task_id} reconciliation discovery must not be a symlink",
        )
        _require(
    ...
    _require(
        receipt_digest == _resolution_receipt_digest(receipt),
        f"{repair.task_id} resolution receipt digest mismatch",
    )
    _require(
        metadata.get("resolution receipt digest") == receipt_digest,
        f"{repair.task_id} resolution receipt anchor mismatch",
    )
...
def _validate_reconciliation_guardrail_task(
    ...
    *,
    ...
    discovery_root_path = Path(discovery_root_text)
    try:
        resolved_discovery_root = discovery_root_path.resolve(strict=False)
    except (OSError, RuntimeError) as exc:
        raise BoardValidationError(
            f"{repair.task_id} reconciliation discovery output is unavailable"
        ) from exc
    _require(
        resolved_discovery_root in _expected_reconciliation_discovery_roots()
        and not discovery_root_path.is_symlink(),
        f"{repair.task_id} reconciliation discovery output is not supervisor-owned",
    )
    _require(
    ...
    if repair.status == "completed":
        ...
        _require(
            not metadata.get("resolution receipt digest"),
            f"{repair.task_id} blocked reconciliation has a stale receipt anchor",
        )
