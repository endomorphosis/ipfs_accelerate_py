...
class HardwareDetector:
    ...
    def _detect_hardware(self):
        ...
        logger.info(
            f"Hardware detection complete. Available: {', '.join(hw for hw, available in self._hardware_info.items() if available)}"
        )
    ...
    def _get_cpu_cores(self) -> Dict[str, int]:
        ...
        cores = {"logical": multiprocessing.cpu_count(), "physical": None}
        ...
        try:
            if platform.system() == "Linux":
                ...
                try:
                    ...
                    for line in output.splitlines():
                        if "Core(s) per socket" in line:
                            sockets_line = next(
                                (l for l in output.splitlines() if "Socket(s)" in l), None
                            )
                            sockets = 1
                            if sockets_line:
                    ...
                    try:
                        ...
                        for line in cpuinfo.splitlines():
                            if "physical id" in line and "core id" in line:
                                parts = line.split()
                                physical_id = next(
                                    (p.split(":")[1].strip() for p in parts if "physical id" in p),
                                    None,
                                )
                                core_id = next(
                                    (p.split(":")[1].strip() for p in parts if "core id" in p), None
                                )
                ...
                try:
                    output = (
                        subprocess.check_output(["sysctl", "-n", "hw.physicalcpu"]).decode().strip()
                    )
        ...
        try:
            ...
            if platform.system() == "Linux":
                ...
                try:
                    ...
                    lines = output.splitlines()
                    if len(lines) >= 2:
                        memory_info["total"] = int(lines[1].strip()) / (
                            1024 * 1024
            ...
            if cuda_available:
                ...
                try:
                    ...
                    for i in range(torch.cuda.device_count()):
                        ...
                        if hasattr(torch.cuda, "get_device_properties"):
                            ...
                            if hasattr(props, "max_shared_memory_per_block"):
                                device_info["max_shared_memory_per_block"] = (
                    ...
                    if hasattr(torch.backends, "cudnn"):
                        ...
                        if torch.backends.cudnn.is_available() and hasattr(
                ...
                for device in available_devices:
                    try:
                        ...
                        try:
                            ...
                            self._details[OPENVINO]["device_info"][device] = {"name": device_name}
                    except Exception as e:
                        logger.warning(
                            f"Could not get detailed info for OpenVINO device {device}: {str(e)}"
                        )
            except ImportError:
            ...
            if onnx:
                self._details[WEBNN] = {
                    ...
                    "export_capability": {
                        ...
                        if torch and hasattr(torch, "__version__")
                        else "not_available",
                        "onnx": onnx.__version__
                        if onnx and hasattr(onnx, "__version__")
                        else "not_available",
                    },
        ...
        try:
            ...
            try:
                ...
                npm_list = subprocess.check_output(
                    ["npm", "list", "--json"], universal_newlines=True
                )
                    ...
                    if has_transformers_js:
                        try:
                            ...
                            pkg_info = subprocess.check_output(
                                ["npm", "view", "@xenova/transformers", "version"],
    ...
    def _detect_qualcomm(self):
        ...
        try:
            ...
            if "QNN_SDK" in os.environ:
                ...
                self._details[QUALCOMM] = {"qnn_sdk": os.environ["QNN_SDK"], "backend": "QNN"}
                return
            ...
            try:
                ...
                if importlib.util.find_spec("qti.aisw.dlc_utils") is not None:
                    ...
                    self._hardware_info[QUALCOMM] = True
                    self._details[QUALCOMM] = {"qnn_available": True, "backend": "QNN"}
                    return
            except ImportError:
                pass
            ...
            try:
                if importlib.util.find_spec("snpe") is not None:
                    ...
                    self._hardware_info[QUALCOMM] = True
                    self._details[QUALCOMM] = {"snpe_available": True, "backend": "SNPE"}
                    return
            except ImportError:
                pass
            ...
            try:
                ...
                snpe_version = subprocess.check_output(
                    ["snpe-net-run", "--version"], universal_newlines=True
                ).strip()
                self._hardware_info[QUALCOMM] = True
                self._details[QUALCOMM] = {
        ...
        if device == "cuda":
            try:
                ...
                if device_count > 0:
                    ...
                    logger.debug(
                        f"Selected CUDA device index {preferred_index} from {device_count} available devices"
                    )
        ...
        # For ROCm devices we also add indices (ROCm uses CUDA API)
            ...
            try:
                ...
                if device_count > 0:
                    ...
                    logger.debug(
                        f"Selected ROCm device index {preferred_index} from {device_count} available devices"
                    )
            except (ImportError, AttributeError) as e:
        ...
        for hw_type in [CUDA, ROCM, MPS, OPENVINO, WEBNN, WEBGPU, QUALCOMM]:
            ...
            if hw_type in [CUDA, ROCM, MPS]:
                ...
                if (
                    ...
                    and required_memory > available_memory
                ):
    ...
    def get_hardware_by_priority(self, priority_list: Optional[List[str]] = None) -> str:
        ...
        logger.warning(
            f"No hardware from priority list {priority_list} is available, falling back to CPU"
        )
        return CPU

    def get_torch_device_with_priority(
        self, priority_list: Optional[List[str]] = None, preferred_index: int = 0
    ) -> str:
        """
    ...
    def print_summary(self, detailed: bool = False):
        ...
        print(
            f"Available hardware: {', '.join(hw for hw, available in self._hardware_info.items() if available)}"
        )
...
def detect_available_hardware(
    ...
    if priority_list:
        ...
        torch_device = detector.get_torch_device_with_priority(
            priority_list, preferred_device_index
        )
    else:
    ...
    try:
            ...
            if has_webnn:
                ...
                try:
                    features["webnn_capabilities"] = {
                        ...
                        "preferred_backend": eval(
                            "navigator.ml.getNeuralNetworkContext().getPreferredBackend()"
                        ),
                    }
                except:
                    features["webnn_capabilities"] = {
                        "detection_error": "Could not access WebNN capabilities"
                    }
        except:
        ...
        try:
            ...
            if has_webgpu:
                ...
                try:
                    ...
                    features["webgpu_capabilities"] = {
                        "detection_error": "Could not access WebGPU capabilities"
                    }
        except:
        ...
        try:
            if eval("'performance' in window && 'memory' in performance"):
                features["available_memory_mb"] = eval(
                    "performance.memory.totalJSHeapSize / (1024*1024)"
                )
                features["memory_limit_mb"] = eval(
