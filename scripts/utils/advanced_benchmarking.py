...
class AdvancedBenchmarkSuite:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        self.cache_dir = (
            Path(cache_dir) if cache_dir else Path.home() / ".ipfs_accelerate" / "benchmarks"
        )
    ...
    def run_comprehensive_benchmark(
        ...
        if parallel and len(models) * len(hardware) > 2:
            results = self._run_parallel_benchmarks(models, hardware, benchmark_types, iterations)
        else:
            results = self._run_sequential_benchmarks(models, hardware, benchmark_types, iterations)
                ...
                for hw in hardware:
                    for benchmark_type in benchmark_types:
                        future = executor.submit(
                            self._run_single_benchmark, model, hw, benchmark_type, iterations
                        )
            ...
            for hw in hardware:
                for benchmark_type in benchmark_types:
                    try:
                        ...
                        logger.error(
                            f"Benchmark failed for {model}/{hw}/{benchmark_type.value}: {e}"
                        )
                        # Add failed result placeholder
                        results.append(
                            BenchmarkResult(
    ...
    def _run_single_benchmark(
        self, model: str, hardware: str, benchmark_type: BenchmarkType, iterations: int
    ) -> List[BenchmarkResult]:
        ...
        for precision in precisions:
            for batch_size in batch_sizes:
                ...
                for i in range(iterations):
                    try:
                        if benchmark_type == BenchmarkType.LATENCY:
                            value, unit = self._benchmark_latency(
                                model, hardware, precision, batch_size
                            )
                        elif benchmark_type == BenchmarkType.THROUGHPUT:
                            value, unit = self._benchmark_throughput(
                                model, hardware, precision, batch_size
                            )
                        elif benchmark_type == BenchmarkType.MEMORY:
                            value, unit = self._benchmark_memory(
                                model, hardware, precision, batch_size
                            )
                        elif benchmark_type == BenchmarkType.POWER:
                            value, unit = self._benchmark_power(
                                model, hardware, precision, batch_size
                            )
                        elif benchmark_type == BenchmarkType.SCALABILITY:
                            value, unit = self._benchmark_scalability(
                                model, hardware, precision, batch_size
                            )
                        else:
                ...
                if iteration_results:
                    ...
                    std_dev = (
                        statistics.stdev(iteration_results) if len(iteration_results) > 1 else 0.0
                    )
    ...
    def _benchmark_latency(
        self, model: str, hardware: str, precision: str, batch_size: int
    ) -> Tuple[float, str]:
        """Benchmark inference latency."""
        result = simulate_model_performance(
            model, hardware, batch_size=batch_size, precision=precision
        )
    ...
    def _benchmark_scalability(
        ...
        for bs in batch_sizes:
            if bs <= batch_size:  # Only test up to requested batch size
                result = simulate_model_performance(
                    model, hardware, batch_size=bs, precision=precision
                )
    ...
    def _generate_benchmark_summary(
        self, results: List[BenchmarkResult], optimization_target: OptimizationTarget
    ) -> Dict[str, Any]:
        ...
        summary["performance_rankings"] = self._calculate_performance_rankings(
        ...
        summary["best_configurations"] = self._identify_best_configurations(
            results, optimization_target
        )
    ...
    def _calculate_performance_rankings(
        ...
        rankings = {"hardware": {}, "models": {}, "configurations": {}}
    ...
    def _calculate_normalized_score(
        self, result: BenchmarkResult, optimization_target: OptimizationTarget
    ) -> float:
    ...
    def _generate_optimization_recommendations(self, results: List[BenchmarkResult]) -> List[str]:
        ...
        latency_results = [
            r for r in results if r.benchmark_type == BenchmarkType.LATENCY and r.unit != "error"
        ]
        if latency_results:
            avg_latency = statistics.mean(r.value for r in latency_results)
            if avg_latency > 100:  # ms
                recommendations.append(
                    "Consider using hardware acceleration or model optimization to reduce latency"
                )
        ...
        memory_results = [
            r for r in results if r.benchmark_type == BenchmarkType.MEMORY and r.unit != "error"
        ]
        if memory_results:
            max_memory = max(r.value for r in memory_results)
            if max_memory > 4000:  # MB
                recommendations.append(
                    "Consider using lower precision (fp16/int8) or smaller models to reduce memory usage"
                )
        ...
        if fp32_results and fp16_results:
            fp32_avg = statistics.mean(
                r.value for r in fp32_results if r.benchmark_type == BenchmarkType.LATENCY
            )
            fp16_avg = statistics.mean(
                r.value for r in fp16_results if r.benchmark_type == BenchmarkType.LATENCY
            )

            if fp16_avg < fp32_avg * 0.8:
                speedup = ((fp32_avg - fp16_avg) / fp32_avg) * 100
                recommendations.append(
                    f"Using FP16 precision can provide {speedup:.1f}% latency improvement"
                )

        # Batch size recommendations
        throughput_results = [
            r for r in results if r.benchmark_type == BenchmarkType.THROUGHPUT and r.unit != "error"
        ]
    ...
    def _calculate_benchmark_statistics(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        for benchmark_type in BenchmarkType:
            type_results = [
                r for r in results if r.benchmark_type == benchmark_type and r.unit != "error"
            ]
            if not type_results:
                continue
    ...
    def compare_benchmark_runs(self, run1_id: str, run2_id: str) -> List[PerformanceComparison]:
    ...
    def generate_benchmark_report(
        ...
        if "statistics" in summary:
            ...
            if "overall" in stats:
                ...
                report_lines.append(
                    f"  • Unique Configurations: {overall.get('unique_configurations', 0)}"
                )
...
def run_full_benchmark() -> BenchmarkRun:
    ...
    return suite.run_comprehensive_benchmark(iterations=3, parallel=True)
    ...
    try:
        if args.compare:
            ...
            for comp in comparisons:
                ...
                print(
                    f"📊 {comp.details['benchmark_type']}: {comp.improvement_percent:+.1f}% {direction} ({significance})"
                )
            ...
            benchmark_run = suite.run_comprehensive_benchmark(
                models=args.models, hardware=args.hardware
            )
