...
def _candidate_token_files(explicit_token_file: Optional[str]) -> List[Path]:
    env_token_file = os.environ.get("GITHUB_RUNNER_AUTOSCALER_TOKEN_FILE") or os.environ.get(
        "GITHUB_AUTOSCALER_TOKEN_FILE"
    )
    if env_token_file:
    ...
    state_dir = os.environ.get("GITHUB_RUNNER_AUTOSCALER_STATE_DIR") or os.environ.get(
        "GITHUB_AUTOSCALER_STATE_DIR"
    )
    if state_dir:
...
def _resolve_token_file(explicit_token_file: Optional[str]) -> str:
    ...
    return (
        str(candidates[0])
        if candidates
        else (explicit_token_file or "/var/lib/github-runner-autoscaler/runner_tokens.json")
    )
...
class DockerRunnerLauncher:
    ...
    def _resolve_network_mode(self) -> str:
        ...
        return os.environ.get(
            "RUNNER_DOCKER_NETWORK_MODE", os.environ.get("CACHE_DOCKER_NETWORK_MODE", "host")
        )
    ...
    def _resolve_cache_listen_port(self, network_mode: str) -> int:
        ...
        if env_port:
            try:
                ...
                logger.warning(
                    f"Invalid CACHE_LISTEN_PORT value: {env_port}. Falling back to default."
                )
    ...
    def _pull_runner_image(self) -> None:
        ...
        try:
            result = subprocess.run(
                ["docker", "pull", self.runner_image], capture_output=True, text=True, timeout=300
            )
            if result.returncode != 0:
    ...
    def _get_active_containers(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["docker", "ps", "--filter", "label=github-runner=true", "--format", "{{.ID}}"],
                capture_output=True,
                text=True,
                timeout=5,
            )
...
def main():
    ...
    parser.add_argument(
        "--max-containers", type=int, default=10, help="Maximum concurrent runner containers"
    )
    parser.add_argument(
        "--cleanup-interval", type=int, default=300, help="Seconds between cleanup checks"
    )
    parser.add_argument("--work-dir", default="/tmp/_work", help="Work directory for runners")
    parser.add_argument(
        "--bridge-mode", action="store_true", help="Show token bridge integration info"
    )
