...
class GitHubRunnerAutoscaler:
        ...
        if self.enable_p2p:
            try:
                ...
                self.p2p_discovery = P2PWorkflowDiscoveryService(
                    owner=self.owner, poll_interval=self.poll_interval, scheduler=p2p_scheduler
                )
    ...
    def _write_tokens_to_file(
        ...
        if tokens:
            ...
            last_path, last_error = (
                errors[-1] if errors else (Path(token_file), RuntimeError("unknown error"))
            )
            logger.error(
            ...
            if self.owner:
                ...
                for repo_name in test_repos:
                    result = self.graphql.list_workflow_runs(
                        owner=self.owner, repo=repo_name, status="in_progress", limit=50
                    )
        ...
        try:
            ...
            if self.enable_p2p and self.p2p_discovery:
                try:
                    ...
                    if p2p_pending > 0 or p2p_assigned > 0:
                        logger.info(
                            f"P2P workflows: {p2p_pending} pending, {p2p_assigned} assigned"
                        )
                except Exception as e:
            ...
            total_failed = sum(
                sum(
                    1
                    for w in workflows
                    if w.get("conclusion") in ["failure", "timed_out", "cancelled"]
                )
                for workflows in queues.values()
            )
            ...
            if p2p_runners_needed > 0:
                logger.info(
                    f"Allocating {p2p_runners_needed} runners for P2P tasks, "
            ...
            # Report results
            success_count = sum(
                1 for status in provisioning.values() if status.get("status") == "token_generated"
            )
            ...
            if self.enable_p2p and (p2p_pending > 0 or p2p_assigned > 0):
                logger.info(
                    f"P2P Summary: {p2p_pending} pending, {p2p_assigned} assigned, "
    ...
    def start(self, setup_signals=True) -> None:
        ...
        while self.running:
            iteration += 1
            logger.info(
                f"--- Check #{iteration} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---"
            )
...
Requirements:
  ...
  - Will auto-provision runners as workflows are detected
    ...
    parser.add_argument(
        "--max-runners", type=int, help="Maximum runners to provision (default: system CPU cores)"
    )
    parser.add_argument(
        "--no-arch-filter",
        action="store_true",
    ...
    parser.add_argument(
        "--no-p2p", action="store_true", help="Disable P2P workflow monitoring and autoscaling"
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Run as background daemon (not yet implemented)"
    )
