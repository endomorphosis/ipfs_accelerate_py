...
class EnhancedPerformanceModeling:
    ...
    def _calculate_performance_metrics(
        ...
        precision_multipliers = {"fp32": 1.0, "fp16": 0.6, "int8": 0.35, "int4": 0.20}
    ...
    def _generate_optimization_recommendations(
        ...
        if "gpt" in model.name.lower() or "llama" in model.name.lower():
            ...
            recommendations.append(
                f"Implement KV caching for {kv_improvement * 100:.0f}% speedup in generation"
            )
    ...
    def compare_hardware_performance(
        ...
        for hardware_type in hardware_list:
            try:
                metrics = self.simulate_inference_performance(model_name, hardware_type, batch_size)
    ...
    def get_optimal_hardware_recommendation(
        ...
        if best_hardware:
            ...
            reasons.append(
                f"High throughput: {best_metrics.throughput_samples_per_sec:.1f} samples/sec"
            )
...
def run_enhanced_performance_analysis():
    ...
    for model in test_models[:2]:  # Test first 2 models
        ...
        sorted_results = sorted(
            results.items(), key=lambda x: x[1].throughput_samples_per_sec, reverse=True
        )

        for hw, metrics in sorted_results:
            print(
                f"  {hw:12}: {metrics.inference_time_ms:6.1f}ms, "
