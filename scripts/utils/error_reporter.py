...
class ErrorReporter:
    ...
    def __init__(
        ...
        cache_base = (
            cache_dir
            or os.environ.get("ERROR_REPORTER_CACHE_DIR")
            or str(Path.home() / ".ipfs_accelerate")
        )
    ...
    def _gather_system_info(self) -> Dict[str, Any]:
        ...
        try:
            info["environment"] = {"docker": self._is_docker, "virtual_env": self._is_venv}
        except Exception:
            pass
    ...
    def _create_github_issue(self, error_info: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            labels = (
                self._determine_labels(error_info)
                if self.auto_label
                else ["bug", "automated-report"]
            )
