...
class AdvancedMonitoringSystem:
    ...
    def _add_metric(self, metric: Metric):
        ...
        try:
            ...
            self.metric_history[metric.name] = [
                m for m in self.metric_history[metric.name] if m.timestamp > cutoff_time
            ]
        ...
        return [
            {
                ...
                "condition": lambda metrics: (
                    self._get_latest_metric_value("system_cpu_usage", metrics)
                    > thresholds["cpu_usage"]
                ),
                "severity": AlertSeverity.WARNING,
                "message": "CPU usage is above {threshold}%".format(
            ...
            },
            {
                ...
                "condition": lambda metrics: (
                    self._get_latest_metric_value("system_memory_usage", metrics)
                    > thresholds["memory_usage"]
                ),
                "severity": AlertSeverity.WARNING,
                "message": "Memory usage is above {threshold}%".format(
            ...
            },
            {
                ...
                "condition": lambda metrics: (
                    self._get_latest_metric_value("system_disk_usage", metrics)
                    > thresholds["disk_usage"]
                ),
                "severity": AlertSeverity.ERROR,
                "message": "Disk usage is above {threshold}%".format(
            ...
            },
            {
                ...
                "condition": lambda metrics: (
                    self._get_latest_metric_value("app_metrics_queue_size", metrics) > 1000
                ),
                "severity": AlertSeverity.WARNING,
    ...
    def _trigger_alert(
        self, name: str, severity: AlertSeverity, message: str, labels: Dict[str, str]
    ):
        """Trigger an alert."""
        ...
        if not active_alert:
            alert = Alert(
                name=name, severity=severity, message=message, labels=labels, timestamp=time.time()
            )
    ...
    def _check_system_resources(self) -> tuple:
        ...
        try:
            ...
            if cpu_percent > 90 or memory_percent > 95:
                ...
                return (
                    "degraded",
                    f"Moderate resource usage: CPU {cpu_percent:.1f}%, Memory {memory_percent:.1f}%",
                    {},
                )
            else:
                return (
                    "healthy",
                    f"Resource usage normal: CPU {cpu_percent:.1f}%, Memory {memory_percent:.1f}%",
                    {},
                )
    ...
    def _check_disk_space(self) -> tuple:
        ...
        try:
            ...
            if usage_percent > 95:
                ...
                return (
                    "degraded",
                    f"Disk space high: {usage_percent:.1f}% used",
                    {"usage_percent": usage_percent},
                )
            else:
                return (
                    "healthy",
                    f"Disk space normal: {usage_percent:.1f}% used",
                    {"usage_percent": usage_percent},
                )
    ...
    def _check_application_components(self) -> tuple:
        ...
        try:
            ...
            return (
                "healthy",
                f"All {len(components)} components healthy",
            ...
            class MetricsHandler(http.server.SimpleHTTPRequestHandler):
                ...
                def _get_metrics_response(self):
                    return {
                        ...
                        "metrics_count": sum(
                            len(history)
                            for history in self.monitoring_system.metric_history.values()
                        ),
    ...
    def _generate_monitoring_recommendations(
        ...
        for metric in metrics:
            if metric.name == "system_cpu_usage" and metric.value > 80:
                ...
                recommendations.append(
                    "Monitor memory usage patterns and consider increasing memory"
                )
...
def run_monitoring_demo():
    ...
    try:
        ...
        print(
            f"   • System Status: {report.performance_summary.get('system_status', 'unknown').upper()}"
        )
