...
def fix_text_embedding_template(db):
    ...
    if test_cases_match:
        ...
        ).replace('"data": {}', '"data": {"input": "This is a test sentence for embedding"}')
...
def main():
    ...
    try:
        ...
        if fix_text_embedding_template(db):
            logger.info(
                "Successfully fixed text_embedding template. Saved to fixed_text_embedding.py"
            )
            ...
            logger.info(
                "NOTE: Database not actually updated. Uncomment the save_template_db call to update."
            )
