...
# Key models to test
KEY_MODELS = ["bert", "t5", "vit", "clip", "whisper", "llama"]
    ...
    for model in KEY_MODELS:
        ...
        if success:
            ...
            with open(output_file, "r") as f:
                content = f.read()
                has_platforms = all(
                    f"test_{platform.lower()}" in content
                    for platform in [
                        "cpu",
                        "cuda",
                        "rocm",
...
def validate_skill_generator(generator_path, output_dir):
    ...
    for model in KEY_MODELS:
        ...
        cmd = ["python", generator_path, "--model", model, "--output-dir", output_dir]
...
def print_results(title, results):
    ...
    print(
        f"Syntax Valid: {syntax_valid_count}/{len(results)} ({syntax_valid_count / len(results) * 100:.1f}%)"
    )
...
def main():
    ...
    try:
        ...
        skill_results = validate_skill_generator(
            "integrated_skillset_generator.py", skill_output_dir
        )
