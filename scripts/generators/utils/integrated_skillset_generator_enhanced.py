    ...
    def _load_model_data(self):
        ...
        if self.model_types_file.exists():
            with open(self.model_types_file, "r") as f:
                ...
                logger.info(
                    f"Loaded {len(self.model_types)} model types from {self.model_types_file}"
                )
        else:
        ...
        logger.info(
            f"Models grouped into {len(self.model_families)} families and {len(self.model_tasks)} primary tasks"
        )
...
class TestAnalyzer:
    ...
    def run_test(self, model_type: str) -> Dict:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(test_file)], capture_output=True, text=True, check=False
            )
    ...
    def analyze_hardware_compatibility(self, model_type: str, test_results: Dict = None) -> Dict:
        ...
        if is_key_model:
            ...
            compatibility.update(
                category_compatibility.get(model_category, category_compatibility["unknown"])
            )
            ...
            if model_category != "unknown":
                logger.info(
                    f"Applying category-based hardware compatibility for {model_type} (category: {model_category})"
                )
            ...
            if "status" in test_results:
                ...
                for platform in HARDWARE_PLATFORMS:
                    ...
                    if (
                        platform_key in status
                        and "Success" in str(status[platform_key])
                        or platform_init in status
                        and "Success" in str(status[platform_init])
                        or platform_test in status
                        and "Success" in str(status[platform_test])
                    ):
                        compatibility[platform] = True

                    # Check for simulated or mock implementation
                    elif platform_key in status and (
                        "MOCK" in str(status[platform_key]).upper()
                        or "SIMULATION" in str(status[platform_key]).upper()
                        or "ENHANCED" in str(status[platform_key]).upper()
                    ):
                ...
                for example in test_results["examples"]:
                    ...
                    if platform in compatibility:
                        if "real" in impl_type:
                            ...
                            "simulation" in impl_type
                            or "mock" in impl_type
                            or "enhanced" in impl_type
                        ):
                            compatibility[platform] = "simulation"
    ...
    def _create_template_from_reference(self, reference_code: str, model_metadata: Dict) -> str:
        ...
        if old_model_name:
            ...
            template = template.replace(
                f"{old_model_name.capitalize()}", f"{new_model_name.capitalize()}"
            )
        ...
        template = method_pattern.sub(
            lambda m: f"{m.group(1)}{m.group(2).replace('-', '_')}{m.group(3)}", template
        )

        # Also fix self assignments with hyphens
        assign_pattern = re.compile(r"(self\.create_[a-z]+_)([a-z\-]+)(_endpoint_handler)")
        template = assign_pattern.sub(
            lambda m: f"{m.group(1)}{m.group(2).replace('-', '_')}{m.group(3)}", template
        )
...
class SkillsetGenerator:
    ...
    def validate_implementation(
        self, model_type: str, implementation_path: Optional[Path] = None
    ) -> bool:
        ...
        hardware_compatibility = self.analyzer.analyze_hardware_compatibility(
        ...
        try:
            ...
            if hardware_compatibility.get("webnn", False) or hardware_compatibility.get(
                "webgpu", False
            ):
...
def main():
    ...
    model_group.add_argument(
        "--model", type=str, help="Generate implementation for a specific model"
    )
    model_group.add_argument(
        "--family", type=str, help="Generate implementations for a model family"
    )
    model_group.add_argument(
        "--task", type=str, help="Generate implementations for models with a specific task"
    )
    model_group.add_argument(
        "--all", action="store_true", help="Generate implementations for all supported models"
    )
    model_group.add_argument(
        "--validate", type=str, help="Validate implementation for a specific model"
    )
    model_group.add_argument(
        "--list-families", action="store_true", help="List all available model families"
    )
    ...
    parser.add_argument(
        "--run-tests", action="store_true", help="Run tests before generating implementations"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force overwrite of existing implementation files"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
