...
def generate_test_template(model_type):
    ...
    if modality == "text":
        template = template.replace(
            "MODEL_INPUT_PLACEHOLDER", "The quick brown fox jumps over the lazy dog."
        )
        ...
        template = template.replace(
            "MODEL_INPUT_PLACEHOLDER", '{"image": "test.jpg", "text": "What is in this image?"}'
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--modality",
        "-m",
        help="Override detected modality",
