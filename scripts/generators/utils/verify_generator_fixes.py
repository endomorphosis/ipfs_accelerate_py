    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for model in KEY_MODELS:
            ...
            success, file_or_error = run_generator_test(
                "fixed_merged_test_generator.py", model, temp_dir
            )
            ...
            if success:
                ...
                # Log results
                logger.info(
                    f"Model: {model}, Syntax: {'✅' if syntax_ok else '❌'}, Fixes: {fix_pct:.1f}%"
                )
                for fix, present in fix_results.items():
                    logger.info(f"  - {fix}: {'✅' if present else '❌'}")
            else:
                generator_results.append({"model": model, "success": False, "error": file_or_error})
        ...
        logger.info(
            f"Generator success: {succeeded}/{len(KEY_MODELS)} models ({succeeded / len(KEY_MODELS) * 100:.1f}%)"
        )
        logger.info(
            f"Syntax correctness: {syntax_ok}/{len(KEY_MODELS)} models ({syntax_ok / len(KEY_MODELS) * 100:.1f}%)"
        )
