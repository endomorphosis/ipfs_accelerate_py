...
class TemplateValidator:
    ...
    def validate_hardware_compatibility(self, content: str) -> bool:
        ...
        if not hardware_compatible:
            self.errors.append(
                f"Hardware compatibility is incomplete. Found {found_patterns}/7 hardware platforms."
            )
            ...
            for class_node in class_nodes:
                ...
                for method in required_methods:
                    if method not in method_names:
                        self.errors.append(
                            f"Required method '{method}' not found in class '{class_node.name}'"
                        )
                        return False
    ...
    def validate_indentation(self, content: str, strict: bool = False) -> bool:
        ...
        if indentation_sizes:
            ...
            indentation_debug = ", ".join(
                [f"{size} spaces ({count} lines)" for size, count in unique_indentations]
            )
            ...
            if not strict and "template" in self.generator_type.lower():
                ...
                self.errors.append(
                    f"Warning: Mixed indentation found in template: {indentation_debug}"
                )
                return True
            ...
            self.errors.append(
                f"Inconsistent indentation found in template: {indentation_debug}. Examples: {inconsistent_examples}"
            )
            return False
    ...
    def validate_all(
        ...
        if syntax_valid:
            ...
            is_valid = (
                ...
                and imports_valid
                and classes_valid
                and indentation_valid
                and hardware_valid
                and (not check_resource_pool or resource_pool_valid)
            )
        else:
            is_valid = False
    ...
    def validate_file(
        ...
        try:
            ...
            return self.validate_all(
                content, validate_hardware, check_resource_pool, strict_indentation
            )
        except Exception as e:
            return False, [f"Error reading or validating file: {str(e)}"]


def validate_template_for_generator(
    template_content: str, generator_type: str = "generic", **kwargs
) -> Tuple[bool, List[str]]:
    """
...
def validate_template_file_for_generator(
    file_path: str, generator_type: str = "generic", **kwargs
) -> Tuple[bool, List[str]]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resource-pool", action="store_true", help="Check resource pool compatibility"
    )
    parser.add_argument(
        "--strict-indentation", action="store_true", help="Enforce strict indentation rules"
    )
