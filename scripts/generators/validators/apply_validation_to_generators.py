...
def apply_args(content: str) -> str:
    ...
    arg_parser_match = re.search(
        r"parser\s*=\s*argparse\.ArgumentParser\(.*?\)", content, re.DOTALL
    )
    ...
    if parse_args_match:
        ...
        parser_args = list(
            re.finditer(r"parser\.add_argument\(.*?\)", content[:insert_position], re.DOTALL)
        )
        if parser_args:
...
def main():
    ...
    parser.add_argument(
        "--pattern", type=str, default="*_generator.py", help="Pattern for generator files"
    )
