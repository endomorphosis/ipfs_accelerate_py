...
def extract_template_strings(content: str) -> List[Tuple[str, int, int]]:
    ...
    for match in matches:
        ...
        if (
            "{{" in template_str
            or "}}" in template_str
            or "self." in template_str
            or "import" in template_str
            or "logger." in template_str
            or
            # Model input pattern
...
def fix_indentation(template: str) -> str:
    ...
    is_python_code = any(
        line.strip().startswith(("def ", "class ", "import ", "from ")) for line in lines
    )
...
def fix_file(file_path: str, dry_run: bool = False) -> Tuple[bool, int]:
    ...
    try:
        ...
        templates_sorted = sorted(
            templates, key=lambda x: x[1], reverse=True
        ...
        for template_str, start_pos, end_pos in templates_sorted:
            ...
            if fixed_template != template_str:
                if dry_run:
                    ...
                    template_snippet = (
                        template_str[:50] + "..." if len(template_str) > 50 else template_str
                    )
                    logger.info(
                        f"Would fix template at position {start_pos}-{end_pos}: {template_snippet}"
                    )
                else:
                    # Replace the template in the content
                    fixed_content = (
                        fixed_content[:start_pos] + fixed_template + fixed_content[end_pos:]
                    )
        ...
        if fixed_count > 0 and not dry_run:
            ...
            logger.info(
                f"Would fix indentation in {fixed_count} templates in {file_path} (dry run)"
            )
        else:
...
def fix_directory(
    directory: str, pattern: str = "*.py", dry_run: bool = False
) -> Tuple[bool, int, int]:
    """
    ...
    try:
        ...
        if dry_run:
            logger.info(
                f"Would fix {template_count} templates in {success_count}/{len(files)} files (dry run)"
            )
        else:
            logger.info(
                f"Successfully processed {success_count}/{len(files)} files, fixed {template_count} templates"
            )
...
def main():
    ...
    parser.add_argument(
        "--identify-templates",
        action="store_true",
        help="Only identify template variables without fixing",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.file:
        ...
        if success:
            if dry_run:
                logger.info(
                    f"Would fix {template_count} templates in {file_count} files in {args.directory} (dry run)"
                )
            else:
                logger.info(
                    f"Successfully processed {file_count} files, fixed {template_count} templates in {args.directory}"
                )
        else:
