...
from scripts.generators.validators.fix_template_indentation import (
    fix_file,
    fix_directory,
...
def validate_and_fix_file(
    ...
    validation_success, validation_errors = validate_template_file_for_generator(
        file_path, generator_type=generator_type, strict_indentation=strict_validation
    )
    ...
    return validation_success and (
        not fix_indent or report["indentation"].get("success", False)
    ), report
...
def validate_and_fix_directory(
    ...
    report = {
        ...
        "pattern": pattern,
        "files": [],
        "validation": {"success_count": 0, "fail_count": 0},
        "indentation": {"fixed_count": 0},
    }
    ...
    try:
        ...
        logger.info(
            f"Validation results: {validation_success_count} passed, {validation_fail_count} failed"
        )

        if fix_indent:
            if dry_run:
                logger.info(
                    f"Would fix {indentation_fixed_count} templates in {len(files)} files (dry run)"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="**/*.py",
        help="Glob pattern for files when using --directory",
    )
    parser.add_argument(
        "--generator-type", type=str, default="generic", help="Type of generator to validate for"
    )
    parser.add_argument("--no-fix", action="store_true", help="Skip indentation fixing")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Don't actually modify files, just report what would be changed",
    )
