...
from scripts.generators.validators.fixed_template_example import (
    STANDARD_TEMPLATE,
...
class FixedTemplateGenerator:
    ...
    def generate_test_file(
        self, model_name: str, output_file: str = None, model_type: str = None
    ) -> str:
        """
        ...
        model_class_name = "".join(
            part.capitalize() for part in model_class_name.replace("-", " ").split()
        )
        ...
        if getattr(self.args, "validate", True) and not getattr(
            self.args, "skip_validation", False
        ):
