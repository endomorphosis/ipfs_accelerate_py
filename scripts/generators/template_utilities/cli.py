...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Template utilities command line interface")
    ...
    db_parser.add_argument(
        "--create", action="store_true", help="Create database and tables if they don't exist"
    )
    db_parser.add_argument(
        "--check", action="store_true", help="Check if database exists and has proper schema"
    )
    db_parser.add_argument("--list", action="store_true", help="List all templates in the database")
    db_parser.add_argument("--model-type", type=str, help="Filter templates by model type")
    db_parser.add_argument(
        "--add-defaults", action="store_true", help="Add default parent templates to the database"
    )

    # Template commands
    template_parser = subparsers.add_parser("template", help="Template operations")
    template_parser.add_argument(
        "--get", action="store_true", help="Get a template from the database"
    )
    template_parser.add_argument(
        "--model-type", type=str, required=True, help="Model type to get template for"
    )
    template_parser.add_argument(
        "--template-type", type=str, required=True, help="Template type to get"
    )
    template_parser.add_argument(
        "--hardware-platform", type=str, help="Hardware platform to get template for"
    )
    template_parser.add_argument(
        "--output",
        type=str,
    ...
    template_parser.add_argument(
        "--store", action="store_true", help="Store a template in the database"
    )
    template_parser.add_argument(
        "--input", type=str, help="Input file containing template content to store"
    )
    template_parser.add_argument("--parent", type=str, help="Parent template name")
    template_parser.add_argument("--modality", type=str, help="Template modality")
    template_parser.add_argument(
        "--no-validate", action="store_true", help="Skip validation when storing template"
    )

    # Validation commands
    validation_parser = subparsers.add_parser("validate", help="Template validation")
    validation_parser.add_argument("--template", type=str, help="Path to template file to validate")
    validation_parser.add_argument(
        "--model-type", type=str, required=True, help="Model type the template is for"
    )
    validation_parser.add_argument(
        "--template-type", type=str, required=True, help="Template type (test, benchmark, skill)"
    )
    validation_parser.add_argument(
        "--hardware-platform", type=str, help="Hardware platform to validate for"
    )
    validation_parser.add_argument(
        "--all", action="store_true", help="Validate all templates in the database"
    )
    validation_parser.add_argument(
        "--output",
        type=str,
    ...
    inheritance_parser.add_argument(
        "--get-parent", action="store_true", help="Get parent template for a model type"
    )
    inheritance_parser.add_argument("--model-type", type=str, help="Model type to get parent for")
    inheritance_parser.add_argument(
        "--get-hierarchy", action="store_true", help="Get inheritance hierarchy for a model type"
    )
    ...
    placeholder_parser.add_argument(
        "--extract", action="store_true", help="Extract placeholders from a template"
    )
    placeholder_parser.add_argument(
        "--template", type=str, help="Path to template file to extract placeholders from"
    )
    placeholder_parser.add_argument(
        "--context", action="store_true", help="Generate default context for a model"
    )
    placeholder_parser.add_argument(
        "--model-name", type=str, help="Model name to generate context for"
    )
    placeholder_parser.add_argument(
        "--hardware-platform", type=str, help="Hardware platform to use for context generation"
    )
    placeholder_parser.add_argument(
        "--output",
        type=str,
    ...
    placeholder_parser.add_argument(
        "--render", action="store_true", help="Render a template with context"
    )
    placeholder_parser.add_argument(
        "--context-file", type=str, help="JSON file containing context for rendering"
    )

    # Hardware commands
    hardware_parser = subparsers.add_parser("hardware", help="Hardware operations")
    hardware_parser.add_argument(
        "--detect", action="store_true", help="Detect available hardware platforms"
    )
    hardware_parser.add_argument(
        "--validate", action="store_true", help="Validate hardware support in a template"
    )
    hardware_parser.add_argument(
        "--template", type=str, help="Path to template file to validate hardware support for"
    )
    hardware_parser.add_argument(
        "--hardware-platform", type=str, help="Hardware platform to validate for"
    )
...
def execute_db_command(args):
    ...
    if args.create:
        ...
        print("-" * 100)
        print(
            f"{'ID':<5} {'Model Type':<15} {'Template Type':<15} {'Hardware':<10} {'Status':<10} {'Modality':<12} {'Parent':<15} {'Validation':<10}"
        )
        print("-" * 100)

        for template in templates:
            print(
                f"{template['id']:<5} {template['model_type']:<15} {template['template_type']:<15} {template['hardware_platform']:<10} {template['status']:<10} {template['modality']:<12} {template.get('parent_template', ''):<15} {template['validation_status']:<10}"
            )
...
def execute_template_command(args):
    ...
    if args.get:
        ...
        template, parent, modality = get_template(
            args.db_path, args.model_type, args.template_type, args.hardware_platform
        )
        ...
        if template:
            ...
            logger.error(
                f"Template not found for {args.model_type}/{args.template_type}/{args.hardware_platform or 'generic'}"
            )
            return 1
        ...
        if store_template(
            ...
            logger.info(
                f"Template stored successfully for {args.model_type}/{args.template_type}/{args.hardware_platform or 'generic'}"
            )
            return 0
        else:
...
def execute_validation_command(args):
    ...
    if args.all:
        ...
        success, validation_results = validate_template(
            template_content, args.template_type, args.model_type, args.hardware_platform
        )
...
def execute_hardware_command(args):
    ...
    if args.detect:
        ...
        success, hardware_support = validate_hardware_support(
            template_content, args.hardware_platform
        )
