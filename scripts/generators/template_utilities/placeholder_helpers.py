...
def get_standard_placeholders() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "has_qualcomm": {
            "description": "Boolean indicating Qualcomm AI Engine availability",
            "default_value": "False",
            "required": False,
        },
        "has_samsung": {
            "description": "Boolean indicating Samsung NPU availability",
            "default_value": "False",
            "required": False,
        },
        "has_webnn": {
        ...
        "modality": {
            "description": "Model modality (text, vision, audio, multimodal)",
            "default_value": "text",
            "required": False,
        },
        # Template metadata
        "template_type": {
            "description": "Type of template (test, benchmark, skill)",
            "default_value": "test",
            "required": False,
        },
        "template_version": {
...
def render_template(template: str, context: Dict[str, Any], strict: bool = False) -> str:
    ...
    if all_missing:
        ...
        for placeholder in all_missing:
            if (
                placeholder in standard_placeholders
                and standard_placeholders[placeholder]["default_value"] is not None
            ):
                context[placeholder] = standard_placeholders[placeholder]["default_value"]
                logger.debug(
                    f"Using default value for {placeholder}: {standard_placeholders[placeholder]['default_value']}"
                )
