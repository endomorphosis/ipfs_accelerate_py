...
def validate_hardware_support(
    template: str, hardware_platform: Optional[str] = None
    ...
    hardware_patterns = {
        ...
        "rocm": [r"rocm", r"AMD", r"amd", r"ROCm", r"hip", r"HIP", r"AMD_GPU"],
        "mps": [
            r"mps",
            r"torch\.backends\.mps",
            r'device\s*=\s*[\'"]mps[\'"]',
            r'\.to\([\'"]mps[\'"]',
            r"Apple",
            r"apple",
...
def validate_placeholders_in_template(template: str) -> Tuple[bool, List[str], List[str]]:
    ...
    mandatory_placeholders = {
        name for name, info in standard_placeholders.items() if info.get("required", False)
    }
...
def validate_template(
    template: str, template_type: str, model_type: str, hardware_platform: Optional[str] = None
    ...
    placeholders_valid, missing_placeholders, all_placeholders = validate_placeholders_in_template(
        template
    )
