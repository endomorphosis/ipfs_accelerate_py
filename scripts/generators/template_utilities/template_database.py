...
def get_template(
    db_path: str, model_type: str, template_type: str, hardware_platform: Optional[str] = None
    ...
    try:
        ...
        conn.close()
        logger.warning(
            f"No template found for {model_type}/{template_type}/{hardware_platform or 'generic'}"
        )
...
def store_template(
    ...
    try:
        ...
        if result:
            ...
            logger.info(
                f"Updated template for {model_type}/{template_type}/{hardware_platform or 'generic'}"
            )
        else:
            # Insert new template
            conn.execute(
            ...
            logger.info(
                f"Inserted new template for {model_type}/{template_type}/{hardware_platform or 'generic'}"
            )
...
def list_templates(
    db_path: str = DEFAULT_DB_PATH, model_type: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
        ...
        for template_id, model_type, template_type, template, hardware_platform in results:
            ...
            if success:
                ...
                if not validation_results["syntax"]["success"]:
                    logger.error(
                        f"Syntax errors in {model_type}/{template_type}: {validation_results['syntax']['errors']}"
                    )

                if not validation_results["placeholders"]["success"]:
                    logger.error(
                        f"Missing placeholders in {model_type}/{template_type}: {validation_results['placeholders']['missing']}"
                    )
