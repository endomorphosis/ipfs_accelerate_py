...
try:
    ...
    try:
        from template_inheritance_system_fixed import (
            ...
            get_template_inheritance_graph,
            TemplateError,
        )
...
def validate_cross_platform_support(content: str) -> Tuple[bool, List[str], List[str]]:
    ...
    resource_pool_aware = "resource_pool" in content and (
    ...
    uses_hardware_detection_module = "hardware_detection" in content and (
    ...
    uses_template_inheritance = (
        "template_inheritance" in content
        or "BaseTemplate" in content
        or "inherit" in content
        and ("template_base" in content or "template_parent" in content)
    )
    ...
    if uses_central_detection:
        ...
        if len(supported_platforms) < 3 and uses_hardware_detection_module:
            ...
            logger.debug(
                "Template uses hardware_detection module, assuming support for all platforms"
            )
    else:
    ...
    if uses_template_inheritance and len(supported_platforms) < 3:
        ...
        warnings.append(
            "Template uses inheritance; hardware support may be defined in parent templates"
        )
    ...
    if len(supported_platforms) < 3:
        warnings.append(
            f"Limited hardware support detected, only found: {', '.join(supported_platforms)}"
        )
...
def validate_template_content(content: str) -> Dict[str, Any]:
    ...
    for validator_name, validator_func in validators:
        ...
        results_by_validator[validator_name] = {"valid": valid, "errors": errors}
    ...
    if template_system_available:
        try:
            ...
            try:
                ...
                template_inheritance_results = {"valid": valid, "errors": errors}

                all_valid = all_valid and valid
                all_errors.extend([f"template_system: {error}" for error in errors])
            except TemplateError as e:
                template_inheritance_results = {"valid": False, "errors": [str(e)]}
                all_valid = False
...
def validate_template_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"valid": False, "file": file_path, "errors": [f"Failed to read file: {str(e)}"]}
...
def validate_template_from_db(model_id: str, db_path: str = None) -> Dict[str, Any]:
    ...
    if not template_db_available:
        return {"valid": False, "model_id": model_id, "errors": ["Template database not available"]}
...
def validate_generator_compatibility(
    template_content: str, generator_type: str
) -> Tuple[bool, List[str]]:
    """
    ...
    if generator_type == "merged":
        ...
        if (
            "from scripts.generators.test_generators.merged_test_generator import"
            not in template_content
            and "import scripts.generators.test_generators.merged_test_generator as merged_test_generator"
            not in template_content
        ):
            errors.append(
                "No merged_test_generator import found - template may not be compatible with merged generator"
            )

        if "generate_test" not in template_content:
            errors.append(
                "No generate_test function found - required for merged generator compatibility"
            )

    elif generator_type == "integrated_skillset":
        # Integrated skillset generator requirements
        if (
            "from integrated_skillset_generator import" not in template_content
            and "import integrated_skillset_generator" not in template_content
        ):
            errors.append(
                "No integrated_skillset_generator import found - template may not be compatible with integrated skillset generator"
            )

        if "generate_skillset" not in template_content:
            errors.append(
                "No generate_skillset function found - required for integrated skillset generator"
            )

    elif generator_type == "fixed":
        # Fixed generator requirements
        if (
            "from fixed_merged_test_generator import" not in template_content
            and "import fixed_merged_test_generator" not in template_content
        ):
            errors.append(
                "No fixed_merged_test_generator import found - template may not be compatible with fixed generator"
            )

    elif generator_type == "simple":
        # Simple generator requirements
        if (
            "from scripts.generators.test_generators.simple_test_generator import"
            not in template_content
            and "import scripts.generators.test_generators.simple_test_generator as simple_test_generator"
            not in template_content
        ):
            errors.append(
                "No simple_test_generator import found - template may not be compatible with simple generator"
            )

    return len(errors) == 0, errors


def validate_all_db_templates(
    db_path: str = None, model_family: str = None, modality: str = None, generator_type: str = None
    ...
    for template in templates:
        ...
        if generator_type and "template_content" in result:
            ...
            result["errors"].extend(
                [f"generator_compatibility: {error}" for error in generator_errors]
            )
...
def store_validation_results(
    results: Dict[str, Any], output_file: str = None, store_in_db: bool = False, db_path: str = None
) -> bool:
    """
    ...
    if store_in_db and template_db_available:
        try:
            ...
            try:
                ...
                logger.info(
                    f"Validation results stored in database for {len(models_results)} templates"
                )
            finally:
                conn.close()
        except Exception as e:
...
def get_validation_history(
    model_id: str, limit: int = 10, db_path: str = None
) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    parser.add_argument(
        "--modality", type=str, help="Validate all templates with a specific modality"
    )
    parser.add_argument("--all-db", action="store_true", help="Validate all templates in database")
    parser.add_argument(
        "--test-compatibility", action="store_true", help="Test hardware compatibility"
    )
    parser.add_argument(
        "--generator-type",
        type=str,
        choices=["merged", "integrated_skillset", "fixed", "simple"],
    ...
    parser.add_argument(
        "--store-in-db", action="store_true", help="Store validation results in database"
    )
    parser.add_argument("--history", type=str, help="Show validation history for a model ID")
    parser.add_argument(
        "--history-limit", type=int, default=10, help="Limit for validation history"
    )
    ...
    if args.file:
        ...
        if args.validate_all_generators:
            ...
            for generator_type in generator_types:
                ...
                results = validate_all_db_templates(
                    args.db_path, args.family, args.modality, generator_type
                )
            ...
            for generator_type in generator_types:
                generator_results = {
                    k: v for k, v in combined_results.items() if k.endswith(f"_{generator_type}")
                }
                ...
                if generator_results:
                    print(
                        f"  - {generator_type} generator: {valid_gen} valid, {invalid_gen} invalid"
                    )

        else:
            # Validate with single generator type if specified
            results = validate_all_db_templates(
                args.db_path, args.family, args.modality, args.generator_type
            )
            ...
            if isinstance(results, dict) and "error" in results:
                ...
                if invalid_count > 0:
                    ...
                    for model_id, result in results.items():
                        if not result.get("valid", True):
                            print(
                                f"  - {model_id} ({result.get('model_family', 'unknown')}, {result.get('modality', 'unknown')}):"
                            )
                            for error in result.get("errors", [])[:3]:  # Show first 3 errors
                                print(f"    - {error}")
                            if len(result.get("errors", [])) > 3:
                                print(
                                    f"    - ... and {len(result.get('errors', [])) - 3} more errors"
                                )
        ...
        if not history:
            ...
            for i, entry in enumerate(history):
                status = (
                    ...
                    else f"❌ Invalid ({entry.get('error_count', 0)} errors)"
                )
