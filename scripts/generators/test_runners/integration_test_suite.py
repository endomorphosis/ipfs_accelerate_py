...
class IntegrationTestSuite:
    ...
    def add_result(
        ...
        if success:
            ...
            logger.error(
                f"❌ {category} - {name}: Failed in {execution_time:.2f}s - {error_message}"
            )
    ...
    def save_results(self):
        ...
        if not self.output:
            self.output = (
                f"integration_test_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def print_summary(self):
        ...
        for category in self.categories:
            ...
            if category_total > 0:
                logger.info(
                    f"{category}: {category_passed}/{category_total} passed "
            ...
            try:
                ...
                # PyTorch MPS (Metal Performance Shaders) detection for Apple Silicon
                    ...
                    if hasattr(torch, "mps") and hasattr(torch.mps, "is_available"):
                        ...
                        if available:
                            details = {"available": True, "platform": "Apple Silicon"}
    ...
    def test_cross_platform(self):
        ...
        modules_to_test = ["torch", "transformers", "numpy", "pandas", "duckdb"]
...
def main():
    ...
    parser.add_argument(
        "--hardware", "-h", nargs="+", default=["cpu"], help="Hardware types to test on"
    )
    ...
    suite = IntegrationTestSuite(
        categories=args.categories, hardware=args.hardware, ci_mode=args.ci_mode, output=args.output
    )
