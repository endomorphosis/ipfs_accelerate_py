...
def detect_available_hardware(
    use_cache: bool = True, cache_file: Optional[str] = None, force_refresh: bool = False
) -> Dict[str, Any]:
    """
...
def is_hardware_simulated(
    hardware_type: str, hardware_info: Optional[Dict[str, Any]] = None
) -> bool:
    """
...
def get_recommended_device(model_type: str = None, model_name: str = None) -> str:
    ...
    if model_type or model_name:
        ...
        memory_intensive_patterns = ["llama", "gpt", "t5", "opt", "falcon", "mixtral", "llava"]

        # For audio models, prioritize CPU or CUDA
        audio_patterns = ["whisper", "wav2vec2", "hubert", "clap"]
        ...
        if model_name:
            ...
            is_memory_intensive = any(
                pattern in model_name_lower for pattern in memory_intensive_patterns
            )
