...
KEY_MODELS = [
    {
        ...
        "modality": "text",
        "description": "BERT text encoder",
    },
    {
        "name": "t5",
        "full_name": "t5-small",
        "modality": "text",
        "description": "T5 text encoder/decoder",
    },
    {
        "name": "vit",
        "full_name": "vit-base-patch16-224",
        "modality": "vision",
    ...
    },
    {
        ...
        "modality": "multimodal",
        "description": "CLIP text-image model",
    },
    {
        "name": "whisper",
    ...
    {
        "name": "wav2vec2",
        "full_name": "wav2vec2-base",
        "modality": "audio",
        "description": "Wav2Vec2 audio model",
    },
    {
        "name": "clap",
        "full_name": "clap-htsat-fused",
        "modality": "audio",
        "description": "CLAP audio-text model",
    },
    {
        "name": "llama",
        "full_name": "llama-7b",
        "modality": "text",
        "description": "LLaMA text generation",
    },
    {
        "name": "llava",
        "full_name": "llava-1.5-7b",
        "modality": "multimodal",
        "description": "LLaVA vision-language model",
    },
    {
        "name": "xclip",
        "full_name": "xclip-base-patch32",
        "modality": "video",
        "description": "XCLIP video-text model",
    },
    {
        "name": "detr",
        "full_name": "detr-resnet-50",
        "modality": "vision",
        "description": "DETR object detection",
    },
    {
        "name": "qwen2",
        "full_name": "qwen2-7b",
        "modality": "text",
        "description": "Qwen2 text generation",
    },
    {
        "name": "llava-next",
        "full_name": "llava-next-7b",
        "modality": "multimodal",
        "description": "LLaVA-Next vision-language model",
    },
]
...
def generate_coverage_report(results):
    ...
    test_coverage = (
        sum(1 for model in results if results[model]["test_success"]) / total_models * 100
    )
    skill_coverage = (
        sum(1 for model in results if results[model]["skill_success"]) / total_models * 100
    )
    overall_coverage = (
        sum(
            1
            for model in results
            if results[model]["test_success"] and results[model]["skill_success"]
        )
        / total_models
        * 100
    )
    ...
    for model in KEY_MODELS:
        ...
        if model_name in results:
            json_report["models"][model_name] = {
                ...
                "test_file": results[model_name]["test_file"]
                if "test_file" in results[model_name]
                else None,
                "skill_file": results[model_name]["skill_file"]
                if "skill_file" in results[model_name]
                else None,
            }
    ...
    with open(report_path, "w") as f:
        ...
        for model in KEY_MODELS:
            ...
            f.write(
                f"| {model_name} | {modality} | {test_status} | {skill_status} | {description} |\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--skills-only", action="store_true", help="Generate only skills, not tests"
    )
    parser.add_argument(
        "--model",
        type=str,
        action="append",
        help="Generate for a specific model (can be used multiple times)",
    )
    parser.add_argument(
        "--cpu-only", action="store_true", help="Generate for CPU only, not all platforms"
    )
            ...
            for model, future in test_futures:
                ...
                if test_success:
                    test_file = os.path.join(
                        TEST_OUTPUT_DIR, f"test_hf_{model['full_name'].replace('-', '_')}.py"
                    )
            ...
            for model, future in skill_futures:
                ...
                if skill_success:
                    skill_file = os.path.join(
                        SKILL_OUTPUT_DIR, f"skill_hf_{model['full_name'].replace('-', '_')}.py"
                    )
    ...
    successful_tests = sum(
        1 for model_name in results if results[model_name].get("test_success", False)
    )
    successful_skills = sum(
        1 for model_name in results if results[model_name].get("skill_success", False)
    )

    if generate_tests:
        logger.info(
            f"Tests generated: {successful_tests}/{total_models} ({successful_tests / total_models * 100:.1f}%)"
        )

    if generate_skills:
        logger.info(
            f"Skills generated: {successful_skills}/{total_models} ({successful_skills / total_models * 100:.1f}%)"
        )

    if args.verify:
        verified_tests = sum(
            1 for model_name in results if results[model_name].get("test_verified", False)
        )
        verified_skills = sum(
            1 for model_name in results if results[model_name].get("skill_verified", False)
        )

        if generate_tests:
            logger.info(
                f"Tests verified: {verified_tests}/{successful_tests} ({verified_tests / successful_tests * 100 if successful_tests else 0:.1f}%)"
            )

        if generate_skills:
            logger.info(
                f"Skills verified: {verified_skills}/{successful_skills} ({verified_skills / successful_skills * 100 if successful_skills else 0:.1f}%)"
            )
