...
from scripts.generators.test_generators.merged_test_generator import (
    detect_model_modality,
    generate_modality_specific_template,
)
...
def generate_test_for_model(model_name, output_dir):
    ...
    class_name = "".join(
        word.capitalize() for word in model_name.replace("-", "_").replace(".", "_").split("_")
    )
    ...
    try:
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                f"import {module_name}; print('Successfully imported {module_name}')",
            ],
            capture_output=True,
            text=True,
            check=False,
...
def generate_tests(modality, output_dir, verify=True):
    ...
    if modality not in SAMPLE_MODELS:
        print(
            f"Unknown modality: {modality}. Available modalities: {', '.join(SAMPLE_MODELS.keys())}"
        )
        return []
...
def main():
    ...
    parser.add_argument(
        "--modality",
        choices=list(SAMPLE_MODELS.keys()) + ["all"],
        default="all",
        help="Which modality to generate tests for",
    )
    parser.add_argument(
        "--output-dir", default="modality_tests", help="Output directory for test files"
    )
    parser.add_argument(
