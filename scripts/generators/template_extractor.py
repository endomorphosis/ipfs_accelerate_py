    ...
    if not HAS_DUCKDB or db_path.endswith(".json"):
        ...
        try:
            ...
            templates = conn.execute(
                "SELECT id, model_type, template_type, platform, updated_at FROM templates"
            ).fetchall()
...
def extract_template(db_path: str, template_key: str) -> Optional[str]:
    ...
    if not HAS_DUCKDB or db_path.endswith(".json"):
        ...
        try:
            ...
            for template_id, template_data in templates.items():
                if (
                    template_data.get("model_type") == model_type
                    and template_data.get("template_type") == template_type
                ):
...
def main():
    ...
    parser.add_argument(
        "--extract-template",
        type=str,
        help="Extract a template by key (model_type/template_type[/platform])",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-template",
        nargs=2,
        metavar=("TEMPLATE_ID", "CONTENT_PATH"),
        help="Save a fixed template to the database",
    )
    parser.add_argument(
        "--db-path",
        type=str,
