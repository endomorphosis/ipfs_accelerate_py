...
def add_sample_data(db_path: str) -> bool:
    ...
    try:
        ...
        RETURNING id
        """,
            [model_id, mediatek_id, datetime.datetime.now().isoformat()],
        ).fetchone()[0]
        ...
        RETURNING id
        """,
            [model_id, qualcomm_id, datetime.datetime.now().isoformat()],
        ).fetchone()[0]
...
def verify_schema(db_path: str) -> bool:
    ...
    try:
        ...
        result = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='view' AND name='mobile_performance_comparison'"
        ).fetchone()
        if not result:
...
def main():
    ...
    parser.add_argument(
        "--db-path", help="Path to the benchmark database", default="./benchmark_db.duckdb"
    )
