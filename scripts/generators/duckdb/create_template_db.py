...
def validate_duckdb_templates(db_path: str):
    ...
    try:
        ...
        logger.info(
            f"Found {valid_count}/{len(templates)} templates with valid syntax ({valid_count / len(templates) * 100:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--create-only", action="store_true", help="Only create the database, don't migrate"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate templates after migration"
    )
