...
try:
    from scripts.generators.hardware.hardware_detection import (
        detect_hardware_with_comprehensive_checks,
    )
...
class ModelCompressor:
    ...
    def _detect_hardware(self) -> Dict[str, Any]:
        ...
        try:
            if "hardware_detection" in sys.modules:
                ...
                hardware_types = [
                    hw
                    for hw, available in self.hardware_info.items()
    ...
    def load_model(self, model_name: str, model_type: Optional[str] = None) -> Any:
        ...
        if model_type is None and "model_family_classifier" in sys.modules:
            try:
                ...
                logger.info(
                    f"Model classified as {model_type} (confidence: {classification.get('confidence', 0):.2f})"
                )
            except Exception as e:
    ...
    def get_recommended_compression(
        self, model_type: Optional[str] = None, target_hardware: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Getting compression recommendations for {model_type} model targeting {target_hardware}"
        )
        ...
        if target_hardware == "cuda":
            ...
            if (
                self.hardware_info
                and self.hardware_info.get("cuda_capabilities", [])
                and any(c >= 8.0 for c in self.hardware_info.get("cuda_capabilities", []))
            ):
    ...
    def apply_compression(
        ...
        for method_spec in methods:
            ...
            if len(parts) != 2:
                logger.warning(
                    f"Invalid method specification: {method_spec}. Should be 'category:method'"
                )
                continue
            ...
            try:
                ...
                if category == "quantization":
                    compressed_model = self._apply_quantization(
                        compressed_model, method, method_params, validation_data
                    )
                    ...
                    compressed_model = self._apply_graph_optimization(
                        compressed_model, method, method_params
                    )
                else:
                ...
                if compressed_model is not None:
                    ...
                    if original_size and compressed_size:
                        ...
                        logger.info(
                            f"  Compression ratio: {compression_ratio:.2f}x (from {original_size / 1e6:.2f}MB to {compressed_size / 1e6:.2f}MB)"
                        )
        ...
        if original_size and final_size:
            ...
            logger.info(
                f"Overall compression ratio: {self.compression_stats['overall_compression_ratio']:.2f}x"
            )
    ...
    def _apply_quantization(self, model, method, params, validation_data=None):
        ...
        if method == "dynamic":
            ...
            try:
                ...
                try:
                    ...
                    if HAS_OPTIMUM:
                        ...
                        config = PostTrainingQuantConfig(approach="dynamic", target_device="cpu")

                        from optimum.onnxruntime import ORTQuantizer

                        quantizer = ORTQuantizer.from_pretrained(model)
                        quantized_model = quantizer.quantize(quantization_config=config)
                        return quantized_model
            ...
            if validation_data is None:
                logger.warning(
                    "Static quantization requires calibration data. Falling back to dynamic quantization."
                )
            ...
            try:
                if HAS_OPTIMUM:
                    ...
                    config = PostTrainingQuantConfig(approach="static", target_device="cpu")
                    ...
                    quantized_model = quantizer.quantize(
                        quantization_config=config, calibration_dataset=validation_data
                    )
                    return quantized_model
                else:
                    logger.warning(
                        "Static quantization requires Optimum library. Falling back to dynamic quantization."
                    )
                    return self._apply_quantization(model, "dynamic", params)
            except Exception as e:
            ...
            if validation_data is None:
                logger.warning(
                    "Quantization-aware training requires training data. Falling back to dynamic quantization."
                )
                return self._apply_quantization(model, "dynamic", params)
            ...
            try:
                ...
                if HAS_OPTIMUM:
                    ...
                    logger.warning(
                        "Quantization-aware training requires full training setup. Falling back to static quantization."
                    )
                    return self._apply_quantization(model, "static", params, validation_data)
                else:
                    logger.warning(
                        "QAT requires Optimum library. Falling back to dynamic quantization."
                    )
                    return self._apply_quantization(model, "dynamic", params)
            except Exception as e:
                        ...
                        if hasattr(model, "config"):
                            ...
                            if HAS_OPTIMUM:
                                ...
                                qconfig = PostTrainingQuantConfig(
                                    approach="dynamic", target_device="cpu"
                                )
                                ...
                                logger.warning(
                                    "Optimum not available for ONNX export. Falling back to dynamic quantization."
                                )
                                return self._apply_quantization(model, "dynamic", params)
                    else:
                        logger.warning(
                            "Transformers not available for ONNX export. Falling back to dynamic quantization."
                        )
                        return self._apply_quantization(model, "dynamic", params)
            except Exception as e:
            ...
            try:
                ...
                if not hasattr(model, "half"):
                    logger.warning(
                        "Model doesn't support half precision. Returning original model."
                    )
                    return model
                ...
                if self.hardware_info and self.hardware_info.get("cuda", False):
                    ...
                    if "cuda_capabilities" in self.hardware_info:
                        cuda_capable = any(
                            cap >= 7.0 for cap in self.hardware_info["cuda_capabilities"]
                        )

                    if not cuda_capable:
                        logger.warning(
                            "GPU may not support efficient FP16 operations. Performance might be impacted."
                        )
            ...
            try:
                ...
                if self.hardware_info:
                    ...
                    if self.hardware_info.get("cuda", False):
                        if "cuda_capabilities" in self.hardware_info:
                            bf16_supported = any(
                                cap >= 8.0 for cap in self.hardware_info["cuda_capabilities"]
                            )
                ...
                if not hasattr(torch, "bfloat16"):
                    logger.warning(
                        "PyTorch doesn't support bfloat16 on this platform. Falling back to FP16."
                    )
                    return self._apply_quantization(model, "fp16", params)
                ...
                if hasattr(model, "to") and callable(model.to):
                    ...
                    logger.warning(
                        "Model doesn't support bfloat16 conversion. Returning original model."
                    )
                    return model
            except Exception as e:
            ...
            if HAS_OPTIMUM:
                try:
                    ...
                    config = PostTrainingQuantConfig(
                        bits=bits, approach="dynamic", target_device="cpu"
                    )
                    ...
                    with tempfile.TemporaryDirectory() as tmpdirname:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"{method} quantization requires specialized setup for this model type"
                            )
                ...
                logger.warning(
                    f"{method} quantization requires the Optimum library. Falling back to dynamic quantization."
                )
                return self._apply_quantization(model, "dynamic", params)
    ...
    def _apply_pruning(self, model, method, params, validation_data=None):
        ...
        if method == "magnitude":
            ...
            if validation_data is None:
                logger.warning(
                    "Progressive pruning requires training data. Falling back to magnitude pruning."
                )
            ...
            logger.warning(
                "Progressive pruning requires full training setup. Falling back to magnitude pruning."
            )
            return self._apply_pruning(model, "magnitude", params)
    ...
    def _apply_distillation(self, model, method, params, validation_data=None):
        ...
        if method == "standard":
            ...
            logger.warning(
                "Standard distillation requires training a student model. Not implemented in this simplified version."
            )
            return model
            ...
            logger.warning(
                "Self-distillation requires training setup. Not implemented in this simplified version."
            )
            return model
            ...
            logger.warning(
                "Token-level distillation requires specialized training. Not implemented in this simplified version."
            )
            return model
    ...
    def _apply_graph_optimization(self, model, method, params):
        ...
        if method == "fusion":
            ...
            logger.warning(
                "Operator fusion is best applied during ONNX export. No direct PyTorch implementation."
            )
            return model
            ...
            logger.warning(
                "Constant folding is best applied during ONNX export. No direct PyTorch implementation."
            )
            return model
                ...
                if HAS_OPTIMUM:
                    ...
                    with tempfile.TemporaryDirectory() as tmpdirname:
                        ...
                        try:
                            ...
                            logger.warning(
                                "ONNX graph optimization requires specialized setup and is not fully implemented here."
                            )
                    ...
                    logger.warning(
                        "ONNX graph optimization requires the Optimum library. Skipping optimization."
                    )
                    return model
            except Exception as e:
    ...
    def validate_compression(
        self, model=None, reference_model=None, validation_data=None, metrics=None
    ) -> Dict[str, Any]:
        """
        ...
        if "accuracy" in metrics and validation_data is not None:
            validation_results["metrics"]["accuracy"] = self._measure_accuracy(
                model, reference_model, validation_data
            )
        ...
        try:
            if HAS_TORCH and torch.cuda.is_available():
                ...
                peak_memory = {
                    ...
                    "peak_reduction_ratio": ref_peak / compressed_peak
    ...
    def _measure_accuracy(self, model, reference_model, validation_data):
        ...
        # For simplicity, we just log the process
        logger.warning(
            "Accuracy measurement requires model-specific implementation and validation data"
        )
        return {}
        ...
        if HAS_TRANSFORMERS and hasattr(model, "config"):
            try:
                ...
                if self.model_type == "embedding" or self.model_type == "text_generation":
                    ...
                    # Generic input
                    logger.warning(
                        f"No specific dummy input generator for model type {self.model_type}. Using generic input."
                    )
                    batch_size = 1
    ...
    def save_compressed_model(self, output_path: Optional[str] = None, format: str = "pytorch"):
        ...
        if output_path is None:
            ...
            model_dir = os.path.join(
                self.output_dir, f"{self.model_name.replace('/', '_')}_{timestamp}"
            )
        ...
        try:
            if format.lower() == "pytorch":
                ...
                if hasattr(self.compressed_model, "save_pretrained"):
                    ...
                    logger.warning(
                        f"Don't know how to save model of type {type(self.compressed_model)}"
                    )
                    return None
                ...
                with torch.no_grad():
                    if isinstance(dummy_inputs, dict):
                        ...
                        if HAS_OPTIMUM:
                            ...
                            logger.warning(
                                "ONNX export requires specialized setup for transformers models"
                            )
                            ...
                            logger.warning(
                                "Optimum not available for proper ONNX export of transformers models"
                            )
                            logger.warning("Falling back to PyTorch format")
                            return self.save_compressed_model(output_path, "pytorch")
                    else:
            ...
            if self.validation_results:
                validation_path = os.path.join(
                    os.path.dirname(output_path), "validation_results.json"
                )
    ...
    def generate_compression_report(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            ...
            report_dir = os.path.join(
                self.output_dir, f"{self.model_name.replace('/', '_')}_{timestamp}_report"
            )
        ...
        try:
            ...
            if (
                "original_size" in self.compression_stats
                and "compressed_size" in self.compression_stats
            ):
                ...
                report_lines.append(
                    f"- **Size Reduction:** {(1 - compressed_mb / original_mb) * 100:.1f}%"
                )

            # Add applied methods
            report_lines.append(
                f"- **Methods Applied:** {', '.join(self.compression_stats.get('methods_applied', []))}"
            )
            if self.compression_stats.get("methods_failed", []):
                report_lines.append(
                    f"- **Methods Failed:** {', '.join(self.compression_stats.get('methods_failed', []))}"
                )
            ...
            for i, step in enumerate(self.compression_stats.get("compression_steps", [])):
                ...
                report_lines.append("\n#### Results:\n")
                report_lines.append(
                    f"- **Success:** {'Yes' if step.get('success', False) else 'No'}"
                )

                if step.get("success", False):
                    if "compression_ratio" in step:
                        report_lines.append(
                            f"- **Compression Ratio:** {step['compression_ratio']:.2f}x"
                        )
            ...
            if self.validation_results:
                ...
                if "latency" in metrics:
                    ...
                    report_lines.append(
                        f"- **Reference Model:** {latency.get('reference_latency', 0):.4f} seconds"
                    )
                    report_lines.append(
                        f"- **Compressed Model:** {latency.get('compressed_latency', 0):.4f} seconds"
                    )
                ...
                if "memory" in metrics:
                    ...
                    report_lines.append(
                        f"- **Reference Model Size:** {memory.get('reference_size_mb', 0):.2f} MB"
                    )
                    report_lines.append(
                        f"- **Compressed Model Size:** {memory.get('compressed_size_mb', 0):.2f} MB"
                    )
                    report_lines.append(
                        f"- **Compression Ratio:** {memory.get('compression_ratio', 0):.2f}x\n"
                    )
                    ...
                    if peak_memory:
                        ...
                        report_lines.append(
                            f"- **Reference Model:** {peak_memory.get('reference_peak_mb', 0):.2f} MB"
                        )
                        report_lines.append(
                            f"- **Compressed Model:** {peak_memory.get('compressed_peak_mb', 0):.2f} MB"
                        )
                        report_lines.append(
                            f"- **Reduction Ratio:** {peak_memory.get('peak_reduction_ratio', 0):.2f}x\n"
                        )
            ...
            if self.hardware_info:
                ...
                if "cuda" in self.hardware_info and self.hardware_info["cuda"]:
                    ...
                    if "cuda_devices" in self.hardware_info:
                        ...
                        report_lines.append(
                            "- CUDA available but detailed information not available\n"
                        )
            ...
            if self.compression_stats.get("overall_compression_ratio", 0) > 3:
                ...
                report_lines.append(
                    "- ⚠️ The compression ratio is modest. Consider trying additional compression methods."
                )
            ...
            if self.validation_results and "latency" in self.validation_results.get("metrics", {}):
                ...
                if speedup > 1.5:
                    ...
                    report_lines.append(
                        "- ⚠️ Limited speedup observed. Consider exploring different compression methods."
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced model compression and optimization toolkit"
    )
    ...
    parser.add_argument(
        "--model-type",
        type=str,
        help="Model type (embedding, text_generation, vision, audio, multimodal)",
    )
    parser.add_argument(
        "--methods",
        type=str,
        nargs="+",
        help="Compression methods to apply (e.g., quantization:dynamic pruning:magnitude)",
    )
    parser.add_argument(
        "--params", type=str, help="JSON string with parameters for compression methods"
    )
    parser.add_argument(
        "--target-hardware",
        type=str,
        default="cpu",
        help="Target hardware (cpu, cuda, mps, openvino)",
    )
    parser.add_argument(
        "--list-methods", action="store_true", help="List available compression methods"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate compression with basic metrics"
    )
    parser.add_argument(
        "--format", type=str, default="pytorch", help="Output format (pytorch, onnx)"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument(
        "--recommend",
        action="store_true",
        help="Get compression recommendations without applying them",
    )
    ...
    # Create compressor
    compressor = ModelCompressor(output_dir=args.output_dir, cache_dir=args.cache_dir)
    ...
    if args.recommend:
        ...
        # Get recommendations
        recommendations = compressor.get_recommended_compression(
            args.model_type, args.target_hardware
        )
        ...
        print(
            f"python model_compression.py --model {args.model} --methods {methods_str} --params '{params_str}' --target-hardware {args.target_hardware}"
        )
    ...
    if not methods:
        ...
        recommendations = compressor.get_recommended_compression(
            compressor.model_type, args.target_hardware
        )
