...
MODEL_CATEGORIES = ["text_embedding", "text_generation", "vision", "audio", "multimodal", "video"]
...
class HardwareModelFeatureTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for idx, row in X_transformed.iterrows():
            ...
            parallelism_score = hw_caps.get("parallel_cores", 1.0) * model_chars.get(
                "parallelism_benefit", 0.5
            )
            ...
            batch_efficiency = 1.0 + log_batch_size * model_chars.get("batch_scalability", 0.5) * (
            ...
            if self.use_extended_features:
                ...
                cache_efficiency = hw_caps.get("cache_efficiency", 0.8) * model_chars.get(
                    "cache_locality", 0.7
                )
                ...
                sequence_scaling = 1.0 + (log_seq_length / 5.0) * model_chars.get(
                    "sequence_length_sensitivity", 0.5
                )
                ...
                model_scale_efficiency = 1.0 / (
                    1.0
                    + 0.1 * log_model_size * (1.0 - hw_caps.get("memory_hierarchy_efficiency", 0.8))
                )
                ...
                attention_efficiency = (
                    ...
                    * hw_caps.get("memory_bandwidth", 50.0)
                    / 400.0  # Normalize to typical bandwidth
                )
                ...
                X_transformed.at[idx, "memory_throughput_score"] = (
                    memory_throughput_score / 100.0
                )  # Normalize
                ...
                if mode == "training":
                    ...
                    X_transformed.at[idx, "inference_throughput_potential"] = (
                        inference_throughput_potential
                    )
                ...
                latency_sensitivity = (
                    1.0
                    / (hw_caps.get("memory_latency", 200.0) / 200.0)  # Normalize and invert
                ...
                X_transformed.at[idx, "seq_batch_interaction"] = sequence_scaling * batch_efficiency
    ...
    try:
        ...
        df["hardware_platform"] = df["hardware_platform"].apply(
            lambda x: (
                ...
                if isinstance(x, str) and x.startswith("rocm")
                else "mps"
                if isinstance(x, str) and x.startswith("mps")
                else "openvino"
                if isinstance(x, str) and x.startswith("openvino")
                else "qnn"
                if isinstance(x, str) and (x.startswith("qualcomm") or x.startswith("qnn"))
                else x
            )
        )

        # Add distributed training flag and strategy
        df["is_distributed"] = df["hardware"].apply(lambda x: "distributed" in str(x))
        ...
        df["gpu_count"] = df.apply(
            lambda row: row.get("total_gpus", 1) if row.get("is_distributed", False) else 1, axis=1
        )
            ...
            if df["category"].isna().any():
                ...
                def infer_category(model_name):
                    ...
                    if any(kw in model_lower for kw in ["clip", "vit", "resnet", "swin"]):
                        ...
                        kw in model_lower for kw in ["gpt", "t5", "llama", "baichuan", "falcon"]
                    ):
                        return "text_generation"
                    elif any(
                        kw in model_lower for kw in ["bert", "roberta", "electra", "distilbert"]
                    ):
                ...
                df.loc[df["category"].isna(), "category"] = df.loc[
                    df["category"].isna(), "model_name"
        ...
        for metric in PREDICTION_METRICS:
            if metric in df.columns:
                ...
                if outlier_count > 0:
                    logger.info(
                        f"Detected {outlier_count} outliers in {metric} ({outlier_count / len(df):.1%})"
                    )
        ...
        if outlier_cols:
            ...
            logger.info(
                f"Detected {df['is_outlier'].sum()} total outliers ({df['is_outlier'].sum() / len(df):.1%})"
            )
        else:
            df["is_outlier"] = False
        ...
        if len(df_filtered) < 0.7 * original_count:
            logger.warning(
                f"Outlier removal filtered too many rows ({len(df_filtered)}/{original_count}). Using original dataset."
            )
            df_filtered = df.copy()
        ...
        available_extended_derived = [
            col for col in extended_derived_cols if col in df_filtered.columns
        ]
        ...
        for col in cat_cols:
            mode_val = (
                df_filtered[col].mode().iloc[0] if not df_filtered[col].mode().empty else None
            )
        ...
        if remaining_nans > 0:
            logger.warning(
                f"There are still {remaining_nans} NaN values in feature columns after preprocessing"
            )
        ...
        for target in PREDICTION_METRICS:
            if target in df_filtered.columns and df_filtered[target].isna().any():
                logger.warning(
                    f"Target column {target} contains {df_filtered[target].isna().sum()} NaN values"
                )
                # Drop rows with NaN values in this target
                df_filtered = df_filtered.dropna(subset=[target])
                logger.info(
                    f"Dropped rows with NaN in target {target}. Remaining rows: {len(df_filtered)}"
                )
        ...
        if len(df_filtered) < 20 or len(df_filtered) < 0.2 * original_count:
            logger.warning(
                f"Too many rows were removed during preprocessing ({len(df_filtered)}/{original_count}). Using original dataset with basic NaN handling."
            )
            df_filtered = df.copy()
            # Basic NaN handling
        ...
        numeric_columns = (
            df_filtered[feature_cols].select_dtypes(include=["number"]).columns.tolist()
        )
        ...
        logger.info(
            f"Preprocessed data: {len(df_filtered)} rows (from {original_count}), {len(feature_cols)} features"
        )
...
class ModelComplexityTransformer(BaseEstimator, TransformerMixin):
    ...
    def _get_feature_names(self, X):
        ...
        self.hardware_features = [
            ...
            if any(hw_feat in col for hw_feat in feature_categories["hardware"])
        ]
        self.model_features = [
            col
            for col in X.columns
            if any(model_feat in col for model_feat in feature_categories["model"])
        ]
        self.performance_features = [
            col
            for col in X.columns
            if any(perf_feat in col for perf_feat in feature_categories["performance"])
        ]
    ...
    def fit(self, X, y=None):
        ...
        if self.feature_selection in ["k_best", "model_based"] and y is None:
            raise ValueError(
                f"Feature selection method '{self.feature_selection}' requires target values"
            )
        ...
        if self.feature_selection != "none" and y is not None:
            ...
            if self.feature_selection == "k_best":
                ...
                self.selector = SelectKBest(
                    f_regression, k=min(self.n_features_to_select, poly_X.shape[1])
                )
            ...
            if hw_features and model_features:
                for hw_col in hw_features:
                    for model_col in model_features:
                        ...
                        interaction = numeric_X[hw_col].values.reshape(-1, 1) * numeric_X[
                            model_col
                        ].values.reshape(-1, 1)
...
def train_prediction_models(
    ...
    try:
        ...
        logger.info(
            f"Training data has {len(feature_cols)} features "
        ...
        if feature_selection == "auto":
            ...
            if len(feature_cols) < 20:
                ...
                logger.info(
                    f"Using top {min(40, len(feature_cols) - 5)} features (auto feature selection)"
                )
        ...
        for target in target_cols:
            ...
            if f"{target}_outlier" in df.columns:
                ...
                logger.info(
                    f"Using preprocessing outlier detection: {len(X_filtered)}/{len(X)} samples"
                )
            else:
                ...
                if target == "throughput" or target == "latency_mean" or target == "memory_usage":
                    ...
                    if (
                        len(X_filtered) < len(X) * 0.7
                    ):  # If removing too many points, revert to full dataset
                        X_filtered = X
                        y_filtered = y
                        logger.warning(
                            f"Too many outliers detected for {target}, using full dataset."
                        )
                    else:
            ...
            if use_ensemble:
                    ...
                    if target == "throughput":
                        base_model = VotingRegressor(
                            [
                                (
                                    "gbm",
                                    HistGradientBoostingRegressor(
                                        random_state=random_state, max_iter=100, learning_rate=0.1
                                    ),
                                ),
                                (
                                    "rf",
                                    RandomForestRegressor(
                                        random_state=random_state, n_estimators=50, max_depth=None
                                    ),
                                ),
                            ]
                        )
                        ...
                        base_model = VotingRegressor(
                            [
                                ...
                                (
                                    "elastic",
                                    ElasticNet(random_state=random_state, alpha=0.1, l1_ratio=0.5),
                                ),
                            ]
                        )
                    else:  # memory_usage
                        base_model = VotingRegressor(
                            [
                                ...
                                ("elastic", ElasticNet(random_state=random_state, alpha=0.1)),
                            ]
                        )
                        ...
                        base_model = VotingRegressor(
                            [
                                ...
                                (
                                    "rf",
                                    RandomForestRegressor(
                                        random_state=random_state, n_estimators=100, max_depth=None
                                    ),
                                ),
                                (
                                    "mlp",
                        ...
                        base_model = VotingRegressor(
                            [
                                (
                                    "gbm",
                                    HistGradientBoostingRegressor(
                                        random_state=random_state, max_iter=150, learning_rate=0.1
                                    ),
                                ),
                                (
                                    "rf",
                                ...
                                (
                                    "elastic",
                                    ElasticNet(random_state=random_state, alpha=0.01, l1_ratio=0.2),
                                ),
                            ]
                        )
                    ...
                    if target == "throughput":
                        ...
                        estimators = [
                            ...
                            (
                                "rf",
                                RandomForestRegressor(
                                    random_state=random_state, n_estimators=150, min_samples_leaf=3
                                ),
                            ),
                        ...
                        base_model = StackingRegressor(
                            ...
                            final_estimator=ElasticNet(
                                random_state=random_state, alpha=0.01, l1_ratio=0.1
                            ),
                            cv=3,
                            n_jobs=n_jobs,
                        )
                        ...
                        estimators = [
                            (
                                "gbm",
                                HistGradientBoostingRegressor(
                                    random_state=random_state, max_iter=200, learning_rate=0.05
                                ),
                            ),
                            (
                                "rf",
                            ...
                            ("ridge", Ridge(random_state=random_state, alpha=0.5)),
                            (
                                "elastic",
                                ElasticNet(random_state=random_state, alpha=0.01, l1_ratio=0.2),
                            ),
                        ...
                        base_model = StackingRegressor(
                            estimators=estimators,
                            final_estimator=TheilSenRegressor(random_state=random_state),
                            cv=3,
                            n_jobs=n_jobs,
                        )
            else:
                ...
                if model_complexity == "simple":
                    base_model = HistGradientBoostingRegressor(
                        random_state=random_state, max_iter=100, learning_rate=0.1, max_depth=5
                    )
                elif model_complexity == "standard":
                    base_model = HistGradientBoostingRegressor(
                        random_state=random_state, max_iter=200, learning_rate=0.05, max_depth=7
                    )
                else:  # complex
                    base_model = HistGradientBoostingRegressor(
                        random_state=random_state,
            ...
            pipeline = Pipeline([("features", feature_engineering), ("model", final_model)])
            ...
            if hyperparameter_tuning:
                if progress_callback:
                    current_step += 1
                    progress_callback(
                        current_step / total_steps, f"Tuning hyperparameters for {target}"
                    )
                ...
                if model_complexity == "simple":
                    ...
                    if use_ensemble:
                        if isinstance(base_model, VotingRegressor):
                            if "gbm" in base_model.named_estimators_:
                                param_grid = {
                                    "features__selector__k": [10, 20, "all"]
                    ...
                    if use_ensemble:
                        if isinstance(base_model, StackingRegressor):
                            ...
                            if isinstance(base_model.final_estimator_, AdaBoostRegressor):
                                param_grid = {
                                    "features__selector__k": [20, 30, "all"]
                                    if hasattr(feature_selector, "k")
                                    else [],
                ...
                if log_transform:
                    ...
                    for param, values in param_grid.items():
                        if param.startswith("model__") and not param.startswith(
                            "model__regressor__"
                        ):
                ...
                if param_grid:
                    try:
                        ...
                        logger.warning(
                            f"Hyperparameter tuning failed for {target}: {e}. Using default model."
                        )
            ...
            if uncertainty_estimation:
                ...
                if isinstance(pipeline.named_steps["model"], RandomForestRegressor):
                    ...
                    isinstance(pipeline.named_steps["model"], VotingRegressor)
                    and "rf" in pipeline.named_steps["model"].named_estimators_
                ):
                    rf_model = pipeline.named_steps["model"].named_estimators_["rf"]
                elif isinstance(
                    pipeline.named_steps["model"], TransformedTargetRegressor
                ) and isinstance(pipeline.named_steps["model"].regressor, RandomForestRegressor):
                    rf_model = pipeline.named_steps["model"].regressor
                elif (
                    isinstance(pipeline.named_steps["model"], TransformedTargetRegressor)
                    and isinstance(pipeline.named_steps["model"].regressor, VotingRegressor)
                    and "rf" in pipeline.named_steps["model"].regressor.named_estimators_
                ):
                ...
                if rf_model is not None:
                    try:
                        ...
                        tree_preds = np.array(
                            [tree.predict(X_test_transformed) for tree in rf_model.estimators_]
                        )
                        ...
                        confidence_intervals["rf_rel_uncertainty"] = float(
                            np.mean(pred_std) / np.mean(y_test) if np.mean(y_test) > 0 else 1.0
                        )
                ...
                try:
                    ...
                    with ThreadPoolExecutor(
                        max_workers=min(n_jobs if n_jobs > 0 else os.cpu_count(), 8)
                    ) as executor:
                        futures = []
                    ...
                    confidence_intervals["bootstrap_rel_uncertainty"] = float(
                        np.mean(bootstrap_std) / np.mean(y_test) if np.mean(y_test) > 0 else 1.0
                    )
                    ...
                    if "rf_mean_std" in confidence_intervals:
                        ...
                        confidence_intervals["combined_uncertainty"] = confidence_intervals[
                            "bootstrap_rel_uncertainty"
                        ]

                except Exception as e:
                    logger.warning(f"Failed to calculate bootstrap uncertainty: {e}")
                    # Fallback uncertainty estimation based on error metrics
                    confidence_intervals["fallback_uncertainty"] = float(
                        metrics["rmse"] / metrics["target_mean"]
                        if metrics["target_mean"] > 0
                        else 0.3
                    )
            ...
            if not confidence_intervals:
                confidence_intervals["fallback_uncertainty"] = float(
                    metrics["rmse"] / metrics["target_mean"] if metrics["target_mean"] > 0 else 0.3
                )
            ...
            if hasattr(pipeline.named_steps["model"], "feature_importances_"):
                ...
                try:
                    ...
                    feature_importance["direct"] = dict(
                        zip(
                            feature_names,
            ...
            # For TransformedTargetRegressor, check the underlying regressor
            elif isinstance(pipeline.named_steps["model"], TransformedTargetRegressor) and hasattr(
                pipeline.named_steps["model"].regressor, "feature_importances_"
            ):
                ...
                try:
                    ...
                    feature_importance["regressor"] = dict(
                        zip(
                            feature_names,
                            importances.tolist() if hasattr(importances, "tolist") else importances,
                        )
                ...
                for name, estimator in pipeline.named_steps["model"].named_estimators_.items():
                    if hasattr(estimator, "feature_importances_"):
                        ...
                        try:
                            ...
                            feature_importance[f"{name}"] = dict(
                                zip(
                                    feature_names,
                                    importances.tolist()
            ...
            if not feature_importance and len(X_test) > 10:
                try:
                    ...
                    feature_importance["permutation"] = dict(
                        zip(
                            feature_names,
                            importances.tolist() if hasattr(importances, "tolist") else importances,
                        )
                    ...
                    feature_importance["permutation_std"] = dict(
                        zip(
                            feature_names,
                            importance_std.tolist()
                            if hasattr(importance_std, "tolist")
                            else importance_std,
                        )
                    )
            ...
            if hyperparameter_tuning and "search" in locals() and hasattr(search, "best_params_"):
                ...
                metrics["cv_results_summary"] = {
                    ...
                    if "mean_fit_time" in search.cv_results_
                    else None,
                    "n_evaluated_params": len(search.cv_results_["params"])
                    if "params" in search.cv_results_
                    else 0,
                }
                ...
                if "cv_results_" in dir(search):
                    ...
                    metrics["cv_results_summary"]["detailed"] = {
                        ...
                        "best_params_fit_time": float(search.cv_results_["mean_fit_time"][best_idx])
                        if "mean_fit_time" in search.cv_results_
                        else None,
                        "best_params_score_time": float(
                            search.cv_results_["mean_score_time"][best_idx]
                        )
                        if "mean_score_time" in search.cv_results_
                        else None,
                    }
            ...
            if use_cross_validation:
                try:
                    ...
                    cv_scores = cross_val_score(
                        pipeline, X_filtered, y_filtered, cv=cv_folds, scoring="r2"
                    )
                    ...
                    try:
                        ...
                        metrics["cv_detailed"] = {
                            ...
                            "test_rmse_std": float(
                                np.std(np.sqrt(-cv_results["test_neg_mean_squared_error"]))
                            ),
                            "test_mae_mean": float(
                            ...
                            "train_r2_std": float(np.std(cv_results["train_r2"]))
                            if "train_r2" in cv_results
                            else None,
            ...
            # Store model and metrics
            models[target] = {"pipeline": pipeline, "metrics": metrics}

            # Log completion and metrics
            logger.info(
                f"Model for {target}: R² = {test_r2:.4f}, MAPE = {metrics.get('mape', 'N/A'):.2%}, RMSE = {metrics['rmse']:.4f}"
            )
        ...
        logger.info(
            f"Completed training {len(models) - 1} prediction models in {time.time() - start_time:.2f} seconds"
        )
        return models
    ...
    try:
        ...
        for target in PREDICTION_METRICS:
            ...
            try:
                ...
                if calculate_uncertainty:
                    ...
                    if "confidence_intervals" in model_metrics:
                        ...
                        if "mean_pred_std" in confidence_intervals:
                            ...
                            uncertainty = confidence_intervals["mean_pred_std"] * max(
                                1.0, relative_scale
                            )
                ...
                if "target_mean" in model_metrics and "target_std" in model_metrics:
                    z_score = abs(pred_value - model_metrics["target_mean"]) / max(
                        0.1, model_metrics["target_std"]
                    )
        ...
        for target, value in predictions.items():
            if target in PREDICTION_METRICS and value is not None:
                ...
                uncertainty_value = uncertainties.get(
                    target, value * 0.15
                )  # Default to 15% if not calculated
        ...
        for target in PREDICTION_METRICS:
            if target in predictions and predictions[target] is not None:
                ...
                if target_conf < 0.5:
                    explanation.append(
                        f"Low confidence in {target} prediction due to limited training data or extrapolation."
                    )
...
def generate_prediction_matrix(
    ...
    try:
        ...
        for model_config in model_configs:
            ...
            model_results = {"name": model_name, "category": model_category, "predictions": {}}
...
def visualize_predictions(
    matrix: Dict[str, Any], metric: str = "throughput", output_dir: Optional[str] = None
) -> List[str]:
    """
                ...
                if hw in model_data["predictions"]:
                    if str(batch_size) in model_data["predictions"][hw]:
                        if precision in model_data["predictions"][hw][str(batch_size)]:
                            value = model_data["predictions"][hw][str(batch_size)][precision].get(
                                metric
                            )
                    ...
                    if precision in model_data["predictions"]["cpu"][bs_str]:
                        ...
                        if cpu_value is not None and cpu_value > 0:
                            ...
                            for hw in hardware_platforms:
                                ...
                                if (
                                    hw in model_data["predictions"]
                                    and bs_str in model_data["predictions"][hw]
                                ):
                                    if precision in model_data["predictions"][hw][bs_str]:
                                        hw_value = model_data["predictions"][hw][bs_str][
                                            precision
                                        ].get(metric)
...
def main():
    ...
    group.add_argument(
        "--predict", action="store_true", help="Predict performance for a configuration"
    )
    ...
    group.add_argument(
        "--evaluate", action="store_true", help="Evaluate model accuracy on test data"
    )
    ...
    parser.add_argument(
        "--test-size", type=float, default=0.2, help="Fraction of data to use for testing (0.0-1.0)"
    )
    parser.add_argument(
        "--ensemble", action="store_true", help="Use ensemble models for better accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--category",
        help="Model category (text_embedding, text_generation, vision, audio, multimodal, video)",
    )
    parser.add_argument("--hardware", help="Hardware platform")
    parser.add_argument("--batch-size", type=int, help="Batch size")
    parser.add_argument(
        "--precision", choices=["fp32", "fp16", "int8"], default="fp32", help="Precision"
    )
    parser.add_argument(
        "--mode", choices=["inference", "training"], default="inference", help="Mode"
    )
    parser.add_argument(
        "--seq-length", type=int, default=128, help="Sequence length for text models"
    )
    parser.add_argument(
        "--gpu-count", type=int, default=1, help="Number of GPUs for distributed training"
    )
    parser.add_argument("--distributed", action="store_true", help="Use distributed training")
    parser.add_argument(
        "--no-uncertainty", action="store_true", help="Disable uncertainty estimation"
    )
    ...
    parser.add_argument(
        "--metric",
        choices=["throughput", "latency_mean", "memory_usage"],
        default="throughput",
    ...
    parser.add_argument(
        ...
        type=str,
        default="1,8,32",
        help="Comma-separated list of batch sizes for matrix",
    )
    parser.add_argument(
        "--precisions",
        type=str,
        default="fp32,fp16",
        help="Comma-separated list of precisions for matrix",
    )
    parser.add_argument(
        "--hardware-platforms",
        type=str,
        help="Comma-separated list of hardware platforms for matrix",
    )
    parser.add_argument(
        "--custom-models", type=str, help="Path to JSON file with custom model configs"
    )

    # Visualization options
    parser.add_argument(
        "--theme", choices=["light", "dark"], default="light", help="Theme for visualizations"
    )
    parser.add_argument(
        "--output-format",
        choices=["png", "svg", "pdf"],
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Generate interactive HTML visualizations"
    )
    ...
    if args.train:
        ...
        for target in PREDICTION_METRICS:
            if target in models:
                ...
                if "feature_importance" in metrics and isinstance(
        ...
        for target in PREDICTION_METRICS:
            if target in prediction.get("predictions", {}):
                ...
                if target in prediction.get("uncertainties", {}):
                    ...
                    if target == "throughput":
                        print(
                            f"Throughput: {value:.2f} samples/sec (confidence: {confidence:.1f}%)"
                        )
        ...
        visualization_files = visualize_predictions(
            matrix=matrix, metric=args.metric, output_dir=args.output_dir
        )
        ...
        for idx, row in eval_df.iterrows():
            ...
            precision = (
                "fp32" if precision_numeric == 32 else "fp16" if precision_numeric == 16 else "int8"
            )
            mode = row.get("mode", "inference")
        ...
        for target in PREDICTION_METRICS:
            ...
            mape_val = (
                mean_absolute_percentage_error(actual[actual > 0], predicted[actual > 0])
                if (actual > 0).any()
                else float("inf")
            )
