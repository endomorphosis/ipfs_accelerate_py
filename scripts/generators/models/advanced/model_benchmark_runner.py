...
def generate_random_input(
    ...
    if model_type == "text":
        ...
        if TORCH_AVAILABLE:
            ...
            inputs["pixel_values"] = np.random.rand(
                batch_size, 3, image_size[0], image_size[1]
            ).astype(np.float32)
        ...
        if TORCH_AVAILABLE:
            ...
            inputs["input_features"] = np.random.rand(
                batch_size, 1, 80, audio_length // 160
            ).astype(np.float32)
        ...
        if "text" in inputs:
            if TORCH_AVAILABLE:
                inputs["labels"] = torch.randint(
                    0, 2, (batch_size,), device=device if device in ["cuda", "mps"] else "cpu"
                )
            else:
            ...
            if TORCH_AVAILABLE:
                inputs["labels"] = torch.randint(
                    0, 1000, (batch_size,), device=device if device in ["cuda", "mps"] else "cpu"
                )
            else:
...
def run_benchmark(args) -> Dict[str, Any]:
    ...
    try:
        ...
        if device == "mps" and not (
            TORCH_AVAILABLE and hasattr(torch.backends, "mps") and torch.backends.mps.is_available()
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Model benchmark runner for hardware performance testing"
    )
    ...
    parser.add_argument(
        "--device",
        choices=["cpu", "cuda", "mps", "openvino", "rocm", "webnn", "webgpu"],
        default="cpu",
        help="Device to benchmark on",
    )
    parser.add_argument("--batch-size", type=int, default=1, help="Batch size")
    parser.add_argument(
        "--precision", choices=["fp32", "fp16", "int8"], default="fp32", help="Precision to use"
    )
    ...
    parser.add_argument(
        "--training", action="store_true", help="Benchmark training instead of inference"
    )
    ...
    parser.add_argument(
        "--multimodal-input", action="store_true", help="Force multimodal input type"
    )

    parser.add_argument("--db-path", type=str, default=None, help="Path to the benchmark database")
    parser.add_argument(
        "--db-only", action="store_true", help="Store results only in the database, not in JSON"
    )
