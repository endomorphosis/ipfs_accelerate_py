...
class ModelRegistryIntegration:
    ...
    def _get_hardware_id(self, conn, hardware_type: str) -> Optional[int]:
        ...
        result = conn.execute(
            "SELECT hardware_id FROM hardware_platforms WHERE hardware_type = ?", [hardware_type]
        ).fetchone()
    ...
    def link_test_results(self, model_name: str, model_version: str, result_ids: List[int]) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Linked {updated}/{len(result_ids)} test results to version {model_version}"
            )
            return updated > 0
        ...
        try:
            ...
            for m_name, hardware_data in grouped_data.items():
                ...
                for h_type, data in hardware_data.items():
                    ...
                    memory_score = (
                        1.0 - ((avg_memory - min_memory) / memory_range)
                        if memory_range > 0
                        else 0.5
                    )
                    ...
                    compatibility_score = (
                        (throughput_score * 0.5) + (latency_score * 0.3) + (memory_score * 0.2)
                    )
                    ...
                    if update_db:
                        try:
                            ...
                            if result:
                                ...
                                # Create new entry
                                max_id = conn.execute(
                                    "SELECT MAX(id) FROM hardware_model_compatibility"
                                ).fetchone()[0]
    ...
    def recommend_hardware(
        ...
        try:
            ...
            if not results:
                ...
                logger.info(
                    f"No hardware compatibility scores found for {model_name}, calculating now..."
                )
            ...
            for row in results:
                ...
                if batch_size is not None and recommended_batch_size is not None:
                    batch_factor = (
                        min(1.0, batch_size / recommended_batch_size)
                ...
                for rank, rec in enumerate(recommendations):
                    try:
                        ...
                        if result:
                            ...
                            max_id = conn.execute(
                                "SELECT MAX(id) FROM task_recommendations"
                            ).fetchone()[0]
                            entry_id = 1 if max_id is None else max_id + 1
            ...
            for model_id, model_name in models:
                try:
                    ...
                    recommendations = self.recommend_hardware(
                        model_name=model_name, task_type=task_type, update_db=True
                    )
...
class VersionControlSystem:
    ...
    def create_compatibility_snapshot(self, model_name: str, version_tag: str) -> bool:
        """
        ...
        try:
            ...
            for row in results:
                ...
                if result:
                    logger.warning(
                        f"Snapshot already exists for {model_name} version {version_tag} on hardware {hardware_id}"
                    )
                    continue

                # Create snapshot
                max_id = conn.execute(
                    "SELECT MAX(id) FROM hardware_compatibility_snapshots"
                ).fetchone()[0]
            ...
            logger.info(
                f"Created {created} compatibility snapshots for {model_name} version {version_tag}"
            )
            return created > 0
    ...
    def compare_compatibility_versions(
        self, model_name: str, version_tag1: str, version_tag2: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for hardware_id, hardware_type in hardware_platforms:
                ...
                compatibility_change = (
                    ((compatibility2 - compatibility1) / compatibility1) * 100
                    if compatibility1
                    else 0
                )
                suitability_change = (
                    ((suitability2 - suitability1) / suitability1) * 100 if suitability1 else 0
                )
...
def main():
    ...
    link_parser.add_argument(
        "--result-ids", required=True, help="Comma-separated list of test result IDs"
    )
    ...
    scores_parser.add_argument(
        "--no-update", action="store_true", help="Do not update the database"
    )
    ...
    recommend_parser.add_argument(
        "--task", choices=["inference", "training"], default="inference", help="Task type"
    )
    recommend_parser.add_argument("--batch-size", type=int, help="Batch size")
    recommend_parser.add_argument(
        "--latency-sensitive", action="store_true", help="Task is latency sensitive"
    )
    recommend_parser.add_argument(
        "--memory-constrained", action="store_true", help="Environment is memory constrained"
    )
    recommend_parser.add_argument(
        "--top-k", type=int, default=3, help="Number of recommendations to return"
    )
    recommend_parser.add_argument(
        "--no-update", action="store_true", help="Do not update the database"
    )
    ...
    update_task_parser.add_argument(
        "--task", required=True, choices=["inference", "training"], help="Task type"
    )
    ...
    create_snapshot_parser = subparsers.add_parser(
        "create-snapshot", help="Create compatibility snapshot"
    )
    ...
    compare_versions_parser = subparsers.add_parser(
        "compare-versions", help="Compare compatibility versions"
    )
    ...
    if args.command == "create-schema":
        ...
        scores = integration.calculate_suitability_scores(
            model_name=args.model, hardware_type=args.hardware, update_db=not args.no_update
        )
        if scores:
            # Print summary
            ...
            for model_name, hardware_data in scores.items():
                ...
                for hardware_type, score_data in hardware_data.items():
                    print(
                        f"    {hardware_type}: compatibility={score_data['compatibility_score']:.4f}, suitability={score_data['suitability_score']:.4f}"
                    )
        else:
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations):
                print(
                    f"  {i + 1}. {rec['hardware_type']} (score: {rec['weighted_score']:.4f}, recommended batch size: {rec['recommended_batch_size']})"
                )
        else:
        ...
        changes = version_control.compare_compatibility_versions(
            args.model, args.version1, args.version2
        )
        if changes:
            print(
                f"Compatibility changes for {args.model} between {args.version1} and {args.version2}:"
            )
            for i, change in enumerate(changes):
                print(f"  {i + 1}. {change['hardware_type']}:")
                print(
                    f"     Compatibility: {change['compatibility_v1']:.4f} -> {change['compatibility_v2']:.4f} ({change['compatibility_change']:+.2f}%)"
                )
                print(
                    f"     Suitability: {change['suitability_v1']:.4f} -> {change['suitability_v2']:.4f} ({change['suitability_change']:+.2f}%)"
                )
                print(
                    f"     Batch Size: {change['batch_size_v1']} -> {change['batch_size_v2']} ({change['batch_size_change']:+d})"
                )
                print(
                    f"     Memory: {change['memory_v1']:.2f} -> {change['memory_v2']:.2f} ({change['memory_change']:+.2f}%)"
                )
