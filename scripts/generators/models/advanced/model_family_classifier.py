...
MODEL_SUBFAMILIES = {
    "embedding": [
        {
            "name": "masked_lm",
            "keywords": ["bert", "roberta", "distilbert", "albert", "masked"],
            "tasks": ["fill-mask", "masked-lm"],
        },
        {
            "name": "sentence_transformer",
            "keywords": ["sentence", "sbert", "simcse"],
            "tasks": ["sentence-similarity"],
        },
        {
            "name": "token_classifier",
            "keywords": ["ner", "token", "sequence-classification"],
    ...
    "text_generation": [
        {
            "name": "causal_lm",
            "keywords": ["gpt", "llama", "falcon", "mixtral", "phi", "gemma", "causal"],
            "tasks": ["text-generation"],
        },
        {
            "name": "seq2seq",
            "keywords": ["t5", "bart", "mbart", "pegasus", "seq2seq"],
    ...
    "vision": [
        {
            ...
            "keywords": ["classifier", "classification", "vit", "resnet"],
            "tasks": ["image-classification"],
        },
        {
            "name": "object_detector",
            "keywords": ["yolo", "detr", "detection", "detector"],
            "tasks": ["object-detection"],
        },
        {
            "name": "segmentation",
            "keywords": ["segmentation", "mask", "sam", "segment"],
            "tasks": ["image-segmentation"],
        },
        {
            "name": "depth_estimation",
            "keywords": ["depth", "estimation", "dpt", "glpn"],
    ...
    "audio": [
        {
            ...
            "keywords": ["asr", "speech", "recognition", "whisper", "wav2vec"],
            "tasks": ["automatic-speech-recognition"],
        },
        {
            "name": "audio_classifier",
            "keywords": ["audio", "classification", "clap"],
    ...
    "multimodal": [
        {
            ...
            "keywords": ["llava", "blip", "vqa", "visual-question"],
            "tasks": ["visual-question-answering"],
        },
        {
            "name": "image_text_encoder",
            "keywords": ["clip", "siglip", "contrastive"],
            "tasks": ["zero-shot-image-classification"],
        },
        {
            "name": "document_qa",
            "keywords": ["document", "ocr", "layout", "donut"],
...
class ModelFamilyClassifier:
    ...
    def load_model_db(self, db_path: str):
        ...
        try:
            with open(db_path, "r") as f:
                ...
                logger.info(
                    f"Loaded model database with {len(self.model_db)} entries from {db_path}"
                )
    ...
    def analyze_model_name(self, model_name: str) -> Dict[str, Any]:
        ...
        if "bert" in normalized_name and not any(
            x in normalized_name for x in ["visual", "blip", "llava", "vilt", "flava"]
        ):
            return {
                "model_name": model_name,
                "family": "embedding",
        ...
        if best_family[0] in self.subfamilies:
            ...
            best_subfamily = (
                max(subfamily_scores.items(), key=lambda x: x[1]) if subfamily_scores else None
            )
    ...
    def analyze_model_tasks(self, tasks: List[str]) -> Dict[str, Any]:
        ...
        """
        if not tasks:
            return {"family": None, "subfamily": None, "confidence": 0, "source": "task_analysis"}
        ...
        if best_family[0] in self.subfamilies:
            ...
            for subfam in self.subfamilies[best_family[0]]:
                ...
                # Count exact matches
                exact_matches = sum(
                    1 for task in normalized_tasks if task in normalized_subfam_tasks
                )
                ...
                subfamily_scores[subfam["name"]] = (exact_matches + partial_matches) / len(
                    subfam["tasks"]
                )
    ...
    def analyze_model_methods(self, methods: List[str]) -> Dict[str, Any]:
        ...
        """
        if not methods:
            return {"family": None, "subfamily": None, "confidence": 0, "source": "method_analysis"}
    ...
    def analyze_model_class(self, class_name: str) -> Dict[str, Any]:
        ...
        """
        if not class_name:
            return {"family": None, "subfamily": None, "confidence": 0, "source": "class_analysis"}
        ...
        if for_pattern:
            ...
            if task_suffix in task_family_map:
                family_scores[task_family_map[task_suffix]] += (
                    2  # Give extra weight to explicit task markers
                )
    ...
    def analyze_hardware_compatibility(self, hw_compatibility: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not hw_compatibility:
            return {"family": None, "confidence": 0, "source": "hardware_analysis"}
