#!/usr/bin/env python3
"""
Skill implementation for bert-tiny with hardware platform support
"""
...
HAS_ROCM = (HAS_CUDA and hasattr(torch, "_C") and hasattr(torch._C, "_rocm_version")) or (
...
class BerttinySkill:
    """Skill for bert-tiny model with hardware platform support."""
    ...
    def __init__(self, model_id="bert-base-uncased", device=None):
        ...
        self.model = None
        self.modality = "text"
        logger.info(f"Initialized bert-tiny skill with device: {self.device}")
...
def create_skill(model_id="bert-tiny", device=None):
    """Create a skill instance."""
    return BerttinySkill(model_id=model_id, device=device)
