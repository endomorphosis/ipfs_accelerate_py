...
class IPFSAccelerateTest:
    ...
    def store_test_result(self, test_result):
        ...
        try:
            ...
            result = self.db_conn.execute("SELECT last_insert_rowid()").fetchone()
        ...
        try:
            ...
            for _, row in matrix_data.iterrows():
                ...
                matrix[model_name][hardware_type] = {
                    ...
                    "success_rate": float(row["success_count"]) / float(row["test_count"])
                    if row["test_count"] > 0
                    else 0.0,
                }
        ...
        if "error" in data["acceleration_results"]:
            ...
            for model_name, model_data in data["acceleration_results"]["models"].items():
                ...
                for hw_type, hw_data in model_data["hardware"].items():
                    ...
                    lines.append(
                        "| Batch Size | Latency (ms) | Throughput (items/sec) | Memory (MB) | Simulated |"
                    )
                    lines.append(
                        "|------------|-------------|------------------------|------------|-----------|"
                    )
                    ...
                    for batch_size, batch_data in hw_data["batch_sizes"].items():
                        ...
                        lines.append(
                            f"| {batch_size} | {batch_data['latency_ms']:.2f} | "
    ...
    def _generate_html_report(self, data):
        ...
        <body>
            <h1>{title}</h1>
            <p>Generated: {date}</p>
        """.format(title=data["report_type"], date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def parse_args():
    ...
    parser.add_argument(
        "--sequence-length", type=int, default=128, help="Sequence length for testing"
    )
    parser.add_argument("--runs", type=int, default=1, help="Number of runs for each test")
    parser.add_argument(
        "--run-id", type=str, default=f"test_{int(time.time())}", help="Run ID for tracking tests"
    )
    ...
    parser.add_argument(
        "--comparison-report", action="store_true", help="Generate a comparison report"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--webgpu-analysis", action="store_true", help="Generate a WebGPU analysis report"
    )

    # Feature flags
    parser.add_argument(
        "--p2p-optimization", action="store_true", help="Enable P2P optimization tests"
    )
    parser.add_argument(
        "--browser",
        type=str,
