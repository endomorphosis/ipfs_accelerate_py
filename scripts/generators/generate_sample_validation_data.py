...
def generate_mock_performance(model_category: str, hardware_key: str, batch_size: int = 1) -> Dict:
    ...
    memory = (
        base_memory.get(model_category, 100)
        * (1 + 0.2 * (batch_size - 1))
...
def generate_sample_result(
    model_key: str, hardware_key: str, status: str, implementation_type: str, date_offset: int = 0
) -> Dict:
    """
...
def generate_realistic_dataset() -> List[Dict]:
    ...
    for model_key in KEY_MODELS:
        for hw_key in HARDWARE_PLATFORMS:
            ...
            result = generate_sample_result(model_key, hw_key, status, impl_type, date_offset)
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="./validation_samples", help="Output directory for samples"
    )
    parser.add_argument(
        "--realistic",
        action="store_true",
        help="Generate realistic dataset based on known implementation status",
    )
    parser.add_argument(
        "--count",
        type=int,
        default=None,
        help="Number of samples to generate (default: all combinations)",
    )
    ...
    for i, result in enumerate(results):
        filename = (
            f"{dataset_type}_validation_{i:03d}_{result['model_key']}_{result['hardware_key']}.json"
        )
