...
PRIORITY_MODELS = {
    "critical": ["bert", "roberta", "gpt2", "t5", "llama", "mistral", "vit", "clip", "whisper"],
    "high": [
        "albert",
        "electra",
        "deberta",
...
class ModelSkillsetGenerator:
    ...
    def fill_template(
        self, template_content: str, model_name: str, arch_type: str, device: str
    ) -> str:
        """
    ...
    def generate_skillset_file(
        self, model_name: str, device: str = "all", force: bool = False, verify: bool = True
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if device == "all":
            ...
            if is_device_compatible_with_model(device, arch_type):
                ...
                logger.warning(
                    f"Device {device} is not compatible with {arch_type}. Using CPU instead."
                )
    ...
    def generate_models_by_priority(
        ...
        logger.info(
            f"Generated {total_files} files for {len(models)} models with priority '{priority}'"
        )

        return overall_success, results

    def generate_all_models(
        self, device: str = "all", verify: bool = True, force: bool = False
        ...
        for priority in PRIORITY_MODELS.keys():
            success, priority_results = self.generate_models_by_priority(
                priority, device, verify, force
            )
        ...
        # Summarize results
        total_files = sum(
            len(files) for priority_dict in results.values() for files in priority_dict.values()
        )
        total_models = sum(len(priority_dict) for priority_dict in results.values())
        logger.info(
            f"Generated {total_files} files for {total_models} models across all priority levels"
        )
