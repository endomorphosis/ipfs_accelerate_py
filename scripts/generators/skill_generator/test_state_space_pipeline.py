...
def test_state_space_generation():
    ...
    if success:
        ...
        assert (
            "# state-space pipeline imports" in content
            or "# State-Space pipeline imports" in content
        ), "Missing State-Space pipeline imports"
...
def test_state_space_classification():
    ...
    if success:
        logger.info(
            f"Successfully generated State-Space classification implementation at {output_file}"
        )
...
def test_template_composer_integration():
    ...
    _, _, pipeline_template = composer.select_templates_for_model(
        model_name="mamba-2-7b", arch_type="state-space", hardware_types=["cpu"]
    )

    # Check that the pipeline template is the State-Space pipeline
    assert pipeline_template.pipeline_type == "state-space", (
        "Template composer did not map state-space to State-Space pipeline"
    )
