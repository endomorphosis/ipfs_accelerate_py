...
def fix_class_name(content: str) -> str:
    ...
    if match:
        ...
        if "-" in original_name:
            ...
            logger.info(
                f"Found class with hyphenated name: hf_{original_name}, replacing with hf_{fixed_name}"
            )
...
def fix_incomplete_returns(content: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if stripped == "return" and (
            i == len(lines) - 1 or not lines[i + 1].strip() or lines[i + 1].strip().startswith("#")
        ):
...
def fix_incomplete_code_blocks(content: str) -> str:
    ...
    patterns = [
        ...
        (
            r"(\s+)return\s+#\s*Code will be generated here",
            r'\1return {"success": True, "device": device, "hardware": hardware_label}',
        ),
    ...
    for line in lines:
        ...
        if "{" in line and "}" in line and not ("{{" in line or "}}" in line):
            ...
            if (
                "{" in line
                and "}" in line
                and not line.strip().startswith(('f"', "f'", 'r"', "r'", "u'", 'u"'))
...
def fix_imports(content: str) -> str:
    ...
    if imports_to_add:
        ...
        if last_import_idx >= 0:
            updated_lines = (
                lines[: last_import_idx + 1] + imports_to_add + lines[last_import_idx + 1 :]
            )
            return "\n".join(updated_lines)
