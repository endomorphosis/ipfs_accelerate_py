...
def test_rag_generative_qa():
    ...
    if success:
        ...
        assert "# rag pipeline imports" in content or "# RAG pipeline imports" in content, (
            "Missing RAG pipeline imports"
        )
        ...
        assert "evaluate_document_relevance" in content, (
            "Missing document relevance evaluation function"
        )
...
def test_rag_document_retrieval():
    ...
    if success:
        logger.info(
            f"Successfully generated RAG document retrieval implementation at {output_file}"
        )
...
def test_template_composer_integration():
    ...
    _, _, pipeline_template = composer.select_templates_for_model(
        model_name="rag-sequence", arch_type="rag", hardware_types=["cpu"]
    )

    # Check that the pipeline template is the RAG pipeline
    assert pipeline_template.pipeline_type == "rag", (
        "Template composer did not map rag to RAG pipeline"
    )
...
def test_rag_model_template():
    ...
    try:
        ...
        template_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "templates/rag_model_template.py"
        )
        ...
        assert "format_context_from_documents" in content or "format_context" in content, (
