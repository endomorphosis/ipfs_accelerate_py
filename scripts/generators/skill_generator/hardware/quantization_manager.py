...
class QuantizationManager:
    ...
    def __init__(self, hardware_type: str = "cpu", model_type: str = "encoder-only"):
        ...
        self.hardware_compatibility = {
            ...
            "mps": [QuantizationMethod.NONE, QuantizationMethod.DYNAMIC_INT8],
            "openvino": [QuantizationMethod.NONE, QuantizationMethod.OPENVINO_INT8],
            "qnn": [QuantizationMethod.NONE, QuantizationMethod.QNN_8BIT],
        }
    ...
    def _detect_available_methods(self) -> List[QuantizationMethod]:
        ...
        try:
            ...
            if self.hardware_type == "cuda":
                available.extend(
                    [QuantizationMethod.BITSANDBYTES_8BIT, QuantizationMethod.BITSANDBYTES_4BIT]
                )
    ...
    def get_recommended_method(
        self, memory_budget_gb: Optional[float] = None, speed_priority: bool = True
    ) -> QuantizationMethod:
        """
        ...
        compatible_methods = self.hardware_compatibility.get(
            self.hardware_type, [QuantizationMethod.NONE]
        )

        # Filter to only available methods
        available_compatible = [m for m in compatible_methods if m in self.available_methods]
    ...
    def get_quantization_config(self, method: QuantizationMethod) -> Dict[str, Any]:
        """
        ...
        configs = {
            ...
            QuantizationMethod.AWQ: {"bits": 4, "group_size": 128, "zero_point": True},
            QuantizationMethod.GGUF: {"n_ctx": 2048, "n_batch": 512, "n_threads": os.cpu_count()},
            QuantizationMethod.OPENVINO_INT8: {
                "quantization_level": "INT8",
                "quantization_preset": "PERFORMANCE",
            },
            QuantizationMethod.QNN_8BIT: {"quantization_overrides": "int8", "use_dsp": True},
            QuantizationMethod.DYNAMIC_INT8: {"dtype": "qint8", "qconfig": "default"},
        }

        return configs.get(method, {})

    def load_quantized_model(
        self, model_id: str, method: Optional[QuantizationMethod] = None, **kwargs
    ):
        """
        ...
        try:
            if (
                method == QuantizationMethod.BITSANDBYTES_8BIT
                or method == QuantizationMethod.BITSANDBYTES_4BIT
            ):
    ...
    def _load_gptq_model(self, model_id: str, config: Dict) -> Any:
        ...
        model = AutoGPTQForCausalLM.from_quantized(
            model_id, use_safetensors=True, device_map="auto", **config
        )
        return model
    ...
    def _load_awq_model(self, model_id: str, config: Dict) -> Any:
        ...
        model = AutoAWQForCausalLM.from_quantized(model_id, fuse_layers=True, **config)
        return model
    ...
    def _load_gguf_model(self, model_id: str, config: Dict) -> Any:
        ...
        model = Llama(model_path=model_id, **config)
        return model
    ...
    def _load_openvino_model(self, model_id: str, config: Dict) -> Any:
        ...
        model = OVModelForCausalLM.from_pretrained(model_id, export=True, **config)
        return model
...
def create_quantization_config(
    hardware_type: str, model_type: str, memory_budget_gb: Optional[float] = None
) -> Dict[str, Any]:
    """
