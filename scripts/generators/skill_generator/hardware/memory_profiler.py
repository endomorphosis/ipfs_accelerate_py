...
class MemoryProfiler:
        ...
        if self.hardware_type == "cuda" and self.has_torch:
            try:
                ...
                if torch.cuda.is_available():
                    snapshot.gpu_vram_used_mb = torch.cuda.memory_allocated(self.device_id) / (
                        1024**2
                    )
                    snapshot.gpu_vram_total_mb = torch.cuda.get_device_properties(
                        self.device_id
                    ).total_memory / (1024**2)
                    snapshot.gpu_vram_percent = (
                        snapshot.gpu_vram_used_mb / snapshot.gpu_vram_total_mb
                    ) * 100
            ...
            try:
                ...
                if hasattr(torch, "hip") and torch.hip.is_available():
                    snapshot.gpu_vram_used_mb = torch.hip.memory_allocated(self.device_id) / (
                        1024**2
                    )
    ...
    def profile_operation(self, operation_name: str, enable_monitoring: bool = False):
        ...
        try:
            ...
            peak_memory = (
                peak_snapshot.cpu_ram_used_mb if peak_snapshot else end_snapshot.cpu_ram_used_mb
            )
...
class MemoryBudgetManager:
    ...
    def can_fit_model(self, estimated_model_size_mb: float) -> bool:
        ...
        if can_fit:
            logger.info(
                f"Model ({estimated_model_size_mb:.0f} MB) can fit in available memory ({available:.0f} MB)"
            )
        else:
            logger.warning(
                f"Model ({estimated_model_size_mb:.0f} MB) may not fit in available memory ({available:.0f} MB)"
            )

        return can_fit

    def recommend_batch_size(self, per_sample_memory_mb: float, max_batch_size: int = 128) -> int:
        """
        ...
        logger.info(
            f"Recommended batch size: {recommended} (based on {available:.0f} MB available)"
        )
