...
class PrecisionManager:
    ...
    def set_precision(self, precision: PrecisionType, force: bool = False) -> bool:
        ...
        if precision == PrecisionType.FP16 and not self.capabilities.get("fp16"):
            if not force:
                logger.warning(
                    f"FP16 not supported on {self.hardware_type}, staying with {self.current_precision.value}"
                )
                return False
        ...
        if precision == PrecisionType.BF16 and not self.capabilities.get("bf16"):
            if not force:
                logger.warning(
                    f"BF16 not supported on {self.hardware_type}, staying with {self.current_precision.value}"
                )
                return False
    ...
    def handle_precision_error(self, error: Exception, operation_name: str = "unknown") -> bool:
        ...
        if self.current_precision != PrecisionType.FP32:
            logger.warning(f"Precision error detected in '{operation_name}': {error}")
            logger.warning(f"Falling back from {self.current_precision.value} to FP32")
            self.current_precision = PrecisionType.FP32
            return True
    ...
    def check_for_numerical_issues(self, tensor) -> Dict[str, bool]:
        ...
        try:
            ...
            if has_nan or has_inf:
                logger.warning(f"Numerical issues detected: NaN={has_nan}, Inf={has_inf}")
        ...
        try:
            ...
            if precision_manager.handle_precision_error(e, "inference"):
                ...
                if attempt < max_retries:
                    logger.info(
                        f"Retrying inference with FP32 (attempt {attempt + 1}/{max_retries})"
                    )
                    continue
            raise
