...
class BatchOptimizer:
    ...
    def __init__(self, hardware_type: str = "cpu", device_id: int = 0, safety_margin: float = 0.15):
        """
    ...
    def calculate_initial_batch_size(
        self, available_memory_mb: float, model_size_mb: float, per_sample_memory_mb: float
    ) -> int:
        """
    ...
    def profile_batch_size(
        self, batch_size: int, inference_func: Callable, num_iterations: int = 3
    ) -> BatchSizeProfile:
        """
    ...
    def find_optimal_batch_size(
        ...
        initial_batch = self.calculate_initial_batch_size(
            available_memory, model_size_mb, per_sample_memory_mb
        )
        ...
        while left <= right:
            ...
            if not profile.success:
                ...
                left = mid + 1
                logger.info(
                    f"Batch {mid} succeeded (throughput: {profile.throughput_samples_per_sec:.1f}), trying larger"
                )

        # Get best profile
        best_profile = next(
            (p for p in self.profiles if p.batch_size == best_batch_size and p.success), None
        )
    ...
    def recommend_for_workload(
        self, workload_type: str, available_memory_mb: Optional[float] = None
    ) -> int:
        """
...
def auto_tune_batch_size(
    ...
    optimal = optimizer.find_optimal_batch_size(
        model_inference_func, model_size_mb, per_sample_memory_mb
    )
...
if __name__ == "__main__":
    ...
    batch_size = optimizer.calculate_initial_batch_size(
        available_memory_mb=available, model_size_mb=500, per_sample_memory_mb=10
    )
