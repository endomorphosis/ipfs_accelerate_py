    ...
    try:
        with open(output_file, "w") as f:
            ...
            for arch, arch_counts in sorted(counts.items()):
                f.write(
                    f"| {arch} | {arch_counts['implemented']} | {arch_counts['total']} | {arch_counts['percentage']:.1f}% |\n"
                )
            ...
            for arch in sorted(architectures):
                ...
                arch_models = {
                    k: v for k, v in model_matrix.items() if v.get("architecture") == arch
                }

                for model_type, model_info in sorted(arch_models.items()):
                    implemented = "✅" if model_info.get("implemented", False) else "❌"
                    f.write(
                        f"| {model_type} | {model_info.get('name', model_type)} | {implemented} |\n"
                    )
    ...
    try:
        with open(output_file, "w") as f:
            ...
            for arch, arch_counts in sorted(counts.items()):
                ...
                f.write(
                    f'            <td><div class="progress-container"><div class="progress-bar" style="width: {percentage}%">{percentage:.1f}%</div></div></td>\n'
                )
                f.write("        </tr>\n")
...
def main():
    ...
    parser.add_argument(
        "--formats",
        nargs="+",
        choices=["csv", "markdown", "html", "json"],
