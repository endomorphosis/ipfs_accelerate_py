...
def load_models_from_report(report_path):
    ...
    try:
        ...
        high_priority = [
            model
            for model in data.get("high_priority_models", [])
            if not model.get("implemented", False)
        ]
        medium_priority = [
            model
            for model in data.get("medium_priority_models", [])
            if not model.get("implemented", False)
        ]
        ...
        return {"high_priority": [], "medium_priority": [], "by_architecture": {}}
...
def batch_generate_models(models, output_dir, batch_size=10, start_index=0, priority="high"):
    ...
    logger.info(
        f"Generating batch of {len(batch_models)} {priority} priority models ({start_index + 1}-{end_index} of {len(models)})"
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="generated_tests", help="Directory to output generated files"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Number of models to generate in this batch"
    )
    ...
    parser.add_argument(
        "--priority", choices=["high", "medium"], default="high", help="Priority level to generate"
    )
    ...
    if args.architecture:
        ...
        logger.info(
            f"Generating models for architecture: {args.architecture} ({len(architecture_models)} models)"
        )
        ...
        generated_files, next_index = batch_generate_models(
            priority_models, args.output_dir, args.batch_size, args.start_index, args.priority
        )
    ...
    if next_index < (
        len(models_data["by_architecture"].get(args.architecture, []))
        if args.architecture
        ...
        if args.architecture:
            logger.info(
                f"python {sys.argv[0]} --report {args.report} --output-dir {args.output_dir} --batch-size {args.batch_size} --start-index {next_index} --architecture {args.architecture}"
            )
        else:
            logger.info(
                f"python {sys.argv[0]} --report {args.report} --output-dir {args.output_dir} --batch-size {args.batch_size} --start-index {next_index} --priority {args.priority}"
            )
    else:
