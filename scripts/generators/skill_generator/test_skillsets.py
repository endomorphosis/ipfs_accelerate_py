...
def test_skillset_by_model(model_name: str, device: str = "cpu") -> bool:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            f"{model_name}_{device}_skillset", skillset_path
        )
        ...
        if hasattr(module, "test_skillset"):
            ...
            logger.info(
                f"Load result: {{'success': {load_result['success']}, 'device': {load_result['device']}}}"
            )
...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        logger.info(
            f"Test {'succeeded' if success else 'failed'} for {args.model} on {args.device}"
        )
