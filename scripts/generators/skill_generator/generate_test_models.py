    ...
    try:
        ...
        if COMPOSER_AVAILABLE:
            ...
            composer = TemplateComposer(
                hardware_templates, architecture_templates, pipeline_templates, output_dir
            )
            ...
            if success:
                logger.info(
                    f"Successfully generated vision-text model implementation at {output_file}"
                )
                return output_file
            else:
...
def generate_speech_model(output_dir, model_name="whisper"):
    ...
    try:
        ...
        if COMPOSER_AVAILABLE:
            ...
            success, output_file = composer.generate_model_implementation(
                model_name=model_name, arch_type="speech", hardware_types=["cpu"], force=True
            )
...
def generate_multimodal_model(output_dir, model_name="flava", use_cuda=False):
    ...
    try:
        ...
        if COMPOSER_AVAILABLE:
            ...
            success, output_file = composer.generate_model_implementation(
                model_name=model_name, arch_type="multimodal", hardware_types=hw_types, force=True
            )

            if success:
                logger.info(
                    f"Successfully generated multimodal model implementation at {output_file}"
                )
                return output_file
            else:
