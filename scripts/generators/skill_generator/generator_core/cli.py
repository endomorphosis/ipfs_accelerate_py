...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", "-c", dest="config_file", help="Configuration file path")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose logging")

    # Generator options
    parser.add_argument(
        "--output-dir", dest="output_dir", help="Output directory for generated files"
    )
    ...
    # Model options
    parser.add_argument("--model-name", dest="model_name", help="Specific model name/ID to use")
    parser.add_argument("--max-size", dest="max_size", type=int, help="Maximum model size in MB")
    parser.add_argument("--framework", help="Framework (pt, tf, onnx, etc.)")

    # Batch generation
    parser.add_argument("--batch", action="store_true", help="Enable batch generation mode")
    parser.add_argument(
        "--batch-file", dest="batch_file", help="File containing list of models to generate"
    )
    parser.add_argument(
        "--batch-size",
        dest="batch_size",
    ...
    parser.add_argument(
        "--priority", choices=["high", "medium", "low"], help="Priority level for batch generation"
    )
    parser.add_argument(
        "--architecture",
        choices=[
    ...
    parser.add_argument("--report-file", dest="report_file", help="Report file path")

    return parser.parse_args()


def load_batch_models(
...
def generate_model(
    generator: GeneratorCore, model_type: str, options: Dict[str, Any]
) -> Dict[str, Any]:
...
def generate_batch(
    generator: GeneratorCore, models: List[str], options: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    if result.get("error_count", 0) > 0:
        ...
        for item in result.get("results", []):
            if not item.get("success", False):
                print(
                    f"  - {item.get('model_type', 'unknown')}: {item.get('error', 'Unknown error')}"
                )
