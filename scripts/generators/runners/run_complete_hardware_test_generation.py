...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("hardware_test_coverage.log")],
)
...
def generate_key_model_tests(all_models=False):
    ...
    if all_models:
        command = (
            "python merged_test_generator.py --generate-missing --high-priority-only --limit 20"
        )
...
def run_parallel_integrations(models, template=None):
    ...
    def process_model(model):
        ...
        return {"model": model, "success": success, "output": output}
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Generate tests for all models, not just key models"
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force updates even if hardware support is already present",
    )
    parser.add_argument(
        "--skip-enhancement", action="store_true", help="Skip the hardware enhancement steps"
    )
    parser.add_argument(
        "--skip-generation", action="store_true", help="Skip the test generation steps"
    )
    parser.add_argument(
        "--skip-benchmarks", action="store_true", help="Skip the benchmark integration steps"
    )
    ...
    logger.info(
        f"Options: all={args.all}, force={args.force}, skip_enhancement={args.skip_enhancement}, skip_generation={args.skip_generation}, skip_benchmarks={args.skip_benchmarks}"
    )
    ...
    if not args.skip_generation:
        ...
        if args.parallel:
            ...
            logger.info(
                f"Successfully generated {success_count} of {len(key_models)} key model tests in parallel."
            )
        else:
            # Run sequentially
