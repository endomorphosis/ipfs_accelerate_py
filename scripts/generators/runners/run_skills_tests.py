...
MODEL_CATEGORIES = {
    ...
    "conversational": ["blenderbot", "blenderbot_small"],
    "default": ["default_embed", "default_lm"],
}
...
def print_summary(results):
    ...
    try:
        ...
        if missing_models:
            ...
            print(
                f"  {len(missing_models)}/{len(model_types)} models in huggingface_model_types.json are missing tests"
            )
...
def generate_model_coverage_report():
    ...
    try:
        ...
        for model_type in model_types:
            ...
            # Determine category
            if any(
                term in normalized for term in ["gpt", "bert", "llama", "t5", "roberta", "gemma"]
            ):
                category = "language_models"
            elif any(
                term in normalized for term in ["vision", "vit", "clip", "swin", "detr", "convnext"]
            ):
                category = "vision_models"
            elif any(
                term in normalized for term in ["audio", "wav2vec", "whisper", "clap", "speech"]
            ):
                category = "audio_models"
            elif any(
                term in normalized
                for term in ["llava", "blip", "multimodal", "instructblip", "kosmos"]
            ):
            ...
            if normalized in tested_models:
                categories[category].append(
                    {"model": model_type, "test_file": tested_models[normalized], "has_test": True}
                )
            else:
                categories[category].append({"model": model_type, "has_test": False})
                missing_models.append(model_type)
        ...
        with open(report_path, "w") as f:
            f.write(
                f"# Model Test Coverage Report - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            )

            # Write summary stats
            total_models = len(model_types) - sum(
                1
                for m in model_types
                if any(m.lower().startswith(p) for p in ["openai-", "optimized-"])
            )
            ...
            for category_name, models in categories.items():
                ...
                coverage = (
                    (tested_in_category / total_in_category * 100) if total_in_category > 0 else 0
                )
                ...
                sorted_models = sorted(
                    models, key=lambda m: (not m["has_test"], m["model"].lower())
                )
...
def main():
    ...
    parser.add_argument(
        "--implementation", type=str, choices=["real", "mock"], help="Force specific implementation"
    )
    parser.add_argument(
        "--parallel", type=int, default=4, help="Number of tests to run in parallel"
    )
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.parallel) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_file), 1):
            ...
            try:
                ...
                print(
                    f"[{i}/{len(valid_test_files)}] Testing {model_name}: {status} ({result['elapsed_time']:.2f}s)"
                )
