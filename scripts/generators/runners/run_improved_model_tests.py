...
try:
    ...
    logger.error(
        "Hardware detection module not found. Please ensure hardware_detection.py is in the current directory"
    )
    sys.exit(1)
...
def prepare_test_environment() -> Dict[str, Any]:
    ...
    if not skills_dir.exists():
        ...
        if not skills_dir.exists():
            logger.warning(
                "Neither 'skills' nor 'modality_tests' directory found. Testing may fail."
            )
            skills_dir = None
...
def run_model_test(model: str, hardware: List[str], skills_dir: Path) -> Dict[str, Any]:
    ...
    for hw in hardware:
        ...
        try:
            ...
            logger.info(
                f"    {model} on {hw}: {'SUCCESS' if exit_code == 0 else 'FAILED'} ({implementation_type})"
            )
    ...
    successful_platforms = [
        hw for hw, result in results["hardware_results"].items() if result.get("success", False)
    ]

    real_implementations = [
        hw
        for hw, result in results["hardware_results"].items()
...
def generate_report(results: List[Dict[str, Any]], report_dir: Path) -> str:
    ...
    with open(md_report_path, "w") as f:
        ...
        f.write(
            f"- Overall success rate: {overall_success_rate:.1%} ({successful_tests}/{total_tests})\n\n"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for key models on all hardware platforms"
    )
    parser.add_argument(
        "--models",
        type=str,
    ...
    logger.info("=" * 60)
    logger.info(
        f"Testing completed for {len(results)} models on {len(hardware_to_test)} hardware platforms"
    )
