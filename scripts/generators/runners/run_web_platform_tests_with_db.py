...
try:
    ...
    logger.warning(
        "web_platform_test_runner module not available, using built-in model definitions"
    )
...
class WebPlatformTestsDBIntegration:
    ...
    def __init__(
        ...
        if not self.compute_shaders:
            self.model_keys = [
                k for k in self.model_keys if HIGH_PRIORITY_MODELS[k].get("modality") != "audio"
            ]
        ...
        logger.info(
            f"Initialized web platform tests database integration with {len(self.models)} models"
        )
        ...
        try:
            ...
            if missing_tables:
                ...
                if schema_script:
                    ...
                    logger.warning(
                        f"Schema script not found. Checked paths: {possible_paths}. Creating minimal schema."
                    )
                    # Re-connect to create the minimal schema
                    conn = duckdb.connect(self.db_path, read_only=False)
                    self._create_minimal_schema(conn)
    ...
    def _create_test_run(self) -> int:
        ...
        try:
            ...
            run_id_result = conn.execute(
                "SELECT COALESCE(MAX(run_id), 0) + 1 FROM test_runs"
            ).fetchone()
    ...
    def _ensure_model_exists(self, model_key: str) -> int:
        ...
        try:
            ...
            model_id_result = conn.execute(
                "SELECT COALESCE(MAX(model_id), 0) + 1 FROM models"
            ).fetchone()
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
                [model_id, model_name, model_family, modality, "huggingface", "1.0", None],
            )
    ...
    def _ensure_hardware_exists(self, platform: str) -> int:
        ...
        try:
            ...
            hw_result = conn.execute(
                "SELECT hardware_id FROM hardware_platforms WHERE hardware_type = ?", [platform]
            ).fetchone()
            ...
            hw_id_result = conn.execute(
                "SELECT COALESCE(MAX(hardware_id), 0) + 1 FROM hardware_platforms"
            ).fetchone()
            ...
            for model_key in self.model_keys:
                ...
                for platform in self.platforms:
                    ...
                    if platform == "webnn" and "edge" not in runner.available_browsers:
                        logger.warning(
                            f"Skipping {platform} test for {model_key} - Edge browser not available"
                        )
                    ...
                    if platform == "webgpu" and "chrome" not in runner.available_browsers:
                        logger.warning(
                            f"Skipping {platform} test for {model_key} - Chrome browser not available"
                        )
                        results[model_key][platform] = {
                            "status": "skipped",
        ...
        try:
            ...
            try:
                ...
                if "Conflicting lock is held" in str(e):
                    ...
                    logger.info(
                        "Will simulate successful run but results won't be stored in database"
                    )
                    return
                else:
                    raise
    ...
    def run_all_tests(self) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        """
        logger.info(
            f"Running web platform tests for {len(self.model_keys)} models on {len(self.platforms)} platforms"
        )
    ...
    def print_summary(self, results: Dict[str, Dict[str, Dict[str, Any]]]) -> None:
        ...
        status_counts = {
            platform: {"success": 0, "error": 0, "skipped": 0, "timeout": 0, "other": 0}
            for platform in self.platforms
        }
        ...
        if "webgpu" in self.platforms:
            ...
            print(
                f"  - Shader Precompilation: {'Enabled' if self.shader_precompile else 'Disabled'}"
            )
...
def main():
    ...
    parser.add_argument(
        "--results-dir", default="./web_platform_results", help="Directory for test results"
    )

    # Model selection
    model_group = parser.add_mutually_exclusive_group()
    model_group.add_argument(
        "--models", nargs="+", help="Models to test (space-separated list of keys)"
    )
    model_group.add_argument(
        "--all-models", action="store_true", help="Test all models (except audio models)"
    )

    # Platform selection
    platform_group = parser.add_mutually_exclusive_group()
    platform_group.add_argument("--run-webnn", action="store_true", help="Run WebNN tests only")
    platform_group.add_argument("--run-webgpu", action="store_true", help="Run WebGPU tests only")
    ...
    # Other options
    parser.add_argument(
        "--small-models", action="store_true", help="Use smaller model variants when available"
    )
