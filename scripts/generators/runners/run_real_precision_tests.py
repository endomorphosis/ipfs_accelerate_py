...
async def run_all_precision_tests(args):
    ...
    summary_file = (
        RESULTS_DIR / f"{platform}_{browser}_{model}_summary_{time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    for bits, result in sorted(results.items()):
        memory_reduction = (
            f"{result.get('memory_reduction_percent', 'N/A')}%"
            if result.get("memory_reduction_percent") is not None
            else "N/A"
        )
        inference_time = (
            f"{result.get('inference_time_ms', 'N/A')}ms"
...
def main():
    ...
    parser.add_argument(
        "--bits", type=int, default=4, help="Bit precision for quantization (2, 4, 8, or 16)"
    )

    parser.add_argument("--all", action="store_true", help="Test all supported precision levels")
