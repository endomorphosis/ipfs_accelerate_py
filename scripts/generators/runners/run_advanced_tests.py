...
def run_batch_inference_tests(args):
    ...
    spec = importlib.util.spec_from_file_location(
        "test_batch_inference", os.path.join(TEST_DIR, "test_batch_inference.py")
    )
...
def run_quantization_tests(args):
    ...
    spec = importlib.util.spec_from_file_location(
        "test_quantization", os.path.join(TEST_DIR, "test_quantization.py")
    )
    ...
    with open(report_path, "w") as f:
        ...
        if batch_results and "summary" in batch_results:
            ...
            if "platforms" in summary:
                ...
                for platform, stats in summary["platforms"].items():
                    if "success_rate" in stats:
                        f.write(
                            f"| {platform} | {stats['successful']} | {stats['total']} | {stats['success_rate'] * 100:.1f}% |\n"
                        )
            ...
            if "batch_sizes" in summary:
                ...
                for batch_size, stats in sorted(
                    summary["batch_sizes"].items(), key=lambda x: int(x[0])
                ):
                    if "success_rate" in stats:
                        f.write(
                            f"| {batch_size} | {stats['successful']} | {stats['total']} | {stats['success_rate'] * 100:.1f}% |\n"
                        )
        ...
        if quant_results and "timestamp" in quant_results:
            ...
            cuda_fp16_success = sum(
                1
                for model, result in quant_results["cuda"]["fp16"].items()
                if result.get("status", "").startswith("Success")
            )
            cuda_int8_success = sum(
                1
                for model, result in quant_results["cuda"]["int8"].items()
                if result.get("status", "").startswith("Success")
            )
            openvino_int8_success = sum(
                1
                for model, result in quant_results["openvino"]["int8"].items()
                if result.get("status", "").startswith("Success")
            )
            ...
            f.write(
                "| Model | Type | Precision | Backend | Inference Time | Memory Usage | Speed Metric |\n"
            )
            f.write(
                "|-------|------|-----------|---------|----------------|--------------|-------------|\n"
            )
            ...
            for model in sorted(all_models):
                ...
                if model in quant_results["cuda"]["fp16"]:
                    ...
                    if result.get("status", "").startswith("Success"):
                        ...
                        if model_type in ["language_model", "text_to_text"]:
                            speed_metric = (
                                f"{result.get('tokens_per_second', 'N/A'):.2f} tokens/sec"
                            )
                        ...
                        f.write(
                            f"| {model} | {model_type} | FP16 | CUDA | {inference_time} | {memory_usage} | {speed_metric} |\n"
                        )
                ...
                if model in quant_results["cuda"]["int8"]:
                    ...
                    if result.get("status", "").startswith("Success"):
                        ...
                        f.write(
                            f"| {model} | {model_type} | INT8 | CUDA | {inference_time} | {memory_usage} | {speed_metric} |\n"
                        )
                ...
                if model in quant_results["openvino"]["int8"]:
                    ...
                    if result.get("status", "").startswith("Success"):
                        ...
                        f.write(
                            f"| {model} | {model_type} | INT8 | OpenVINO | {inference_time} | {memory_usage} | {speed_metric} |\n"
                        )
        ...
        if batch_results and "summary" in batch_results:
            ...
            if success_rate > 0.8:
                f.write(
                    "- **Batch Inference**: Testing shows strong support across most model types. The implementation effectively scales with batch size, although with varying efficiency between model types.\n\n"
                )
            elif success_rate > 0.5:
                f.write(
                    "- **Batch Inference**: Testing shows good support for many model types, but some limitations exist. Further optimization is needed to improve scaling efficiency and ensure consistent support across all models.\n\n"
                )
            else:
                f.write(
                    "- **Batch Inference**: Testing revealed significant limitations in batch support. Major improvements are needed before batch processing can be reliably used in production.\n\n"
                )
        ...
        if quant_results and "timestamp" in quant_results:
            ...
            if total_models > 0:
                ...
                if success_rate > 0.8:
                    f.write(
                        "- **Quantization**: Testing shows excellent support for both FP16 and INT8 precision across most model types. Significant performance gains and memory reduction were observed with minimal accuracy loss.\n\n"
                    )
                elif success_rate > 0.5:
                    f.write(
                        "- **Quantization**: Testing shows good support for quantization, with some models benefiting significantly. Further optimization and testing is required to ensure consistent behavior across all model types.\n\n"
                    )
                else:
                    f.write(
                        "- **Quantization**: Testing revealed significant challenges with quantization support. Additional development is needed to improve compatibility and ensure performance benefits.\n\n"
                    )
        ...
        if batch_success and quant_success:
            f.write(
                "1. **Production Readiness**: Both batch processing and quantization features are recommended for production use, with appropriate model-specific testing.\n"
            )
            f.write(
                "2. **Performance Optimization**: Combine batch processing with appropriate precision for maximum throughput.\n"
            )
            f.write(
                "3. **Resource Monitoring**: Implement dynamic batch size adjustment based on memory availability and monitoring.\n"
            )
        elif batch_success:
            f.write(
                "1. **Batch Processing**: Batch processing is suitable for production use with appropriate testing for specific models.\n"
            )
            f.write(
                "2. **Quantization Limitations**: Use quantization selectively with comprehensive testing for each model.\n"
            )
            f.write(
                "3. **Ongoing Development**: Further development and testing of quantization features is recommended before widespread use.\n"
            )
        elif quant_success:
            f.write(
                "1. **Quantization Benefits**: Quantization provides significant performance and memory benefits for supported models.\n"
            )
            f.write(
                "2. **Batch Processing Limitations**: Batch processing should be used with caution and thorough testing.\n"
            )
            f.write(
                "3. **Implementation Focus**: Focus on improving batch processing support for critical model types.\n"
            )
        else:
            f.write(
                "1. **Limited Production Use**: Both batch processing and quantization features require additional development before widespread production use.\n"
            )
            f.write(
                "2. **Model-Specific Testing**: Thorough testing is required for each model and feature combination.\n"
            )
            f.write(
                "3. **Prioritize Development**: Further implementation and testing of both features is needed, prioritizing critical model types.\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--tests",
        choices=["batch", "quantization", "all"],
        default="all",
    ...
    parser.add_argument(
        "--model-types",
        default="bert,t5,clip,llama,whisper,wav2vec2",
    ...
    parser.add_argument("--fp16", action="store_true", help="Use FP16 precision for CUDA tests")
