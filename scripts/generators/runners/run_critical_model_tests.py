    ...
    for model in target_models:
        ...
        try:
            ...
            process = subprocess.run(
                [sys.executable, test_file], capture_output=True, text=True, timeout=timeout
            )
            ...
            if process.returncode == 0:
                ...
                platform_status = {"cpu": "unknown", "cuda": "unknown", "openvino": "unknown"}
            ...
            results["model_results"][model] = {"status": "error", "message": str(e)}
    ...
    if failures:
        ...
        for model, result in failures.items():
            ...
            if result["status"] == "failed":
                ...
                report += (
                    f"- **Error**: Test exceeded timeout of {result.get('elapsed_time')} seconds\n"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Specific models to test (default: all critical models)",
    )
    parser.add_argument(
        "--timeout", type=int, default=300, help="Timeout in seconds for each test (default: 300)"
    )
    parser.add_argument(
        "--skip-missing", action="store_true", help="Skip models without test files"
    )
    parser.add_argument(
        "--output-dir", type=str, default="critical_model_results", help="Directory to save results"
    )
