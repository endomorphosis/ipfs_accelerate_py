...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run hardware benchmarking, optimization, and compatibility testing"
    )

    # Basic options
    parser.add_argument(
        "--output-dir", type=str, default=RESULTS_DIR, help="Output directory for test results"
    )
    parser.add_argument(
        "--optimization-dir",
        type=str,
        default=OPTIMIZATION_DIR,
        help="Directory for optimized models",
    )
    parser.add_argument(
        "--cache-dir", type=str, default=CACHE_DIR, help="Directory for hardware detection cache"
    )
    parser.add_argument(
        "--history-file", type=str, default=HISTORY_FILE, help="Path to test history JSON file"
    )
    parser.add_argument(
        "--benchmark-db",
        type=str,
        default=BENCHMARK_DATABASE,
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["compatibility", "benchmark", "optimize", "all", "schedule", "trend"],
    ...
    parser.add_argument(
        "--families", type=str, help="Comma-separated list of model families to test"
    )
    parser.add_argument(
        "--all-families",
        action="store_true",
    ...
    parser.add_argument(
        "--optimizations", type=str, help="Comma-separated list of optimization techniques to apply"
    )
    parser.add_argument(
        "--target-device", type=str, default="cpu", help="Target device for optimization"
    )

    # Scheduling options
    parser.add_argument("--interval", type=float, default=24.0, help="Hours between scheduled runs")
    parser.add_argument("--max-runs", type=int, help="Maximum number of scheduled runs")

    # CI and reporting options
    parser.add_argument(
        "--skip-report", action="store_true", help="Skip generating detailed report"
    )
    parser.add_argument(
        "--ci-mode", action="store_true", help="Run in CI mode with appropriate output format"
    )
    parser.add_argument(
        "--verify-install",
        action="store_true",
    ...
    parser.add_argument(
        "--max-workers", type=int, default=4, help="Maximum number of workers for parallel testing"
    )
...
def verify_installation():
    ...
    required_packages = ["torch", "transformers", "numpy", "psutil", "tqdm"]
            ...
            if status != previous_status:
                if (status == "incompatible" and previous_status == "compatible") or (
                    ...
                    if status == "compatible" or status == "device_mismatch":
                        ...
                        unchanged.append(
                            f"{family} on {platform}: {previous_status} → {status} (neutral change)"
                        )
            else:
    ...
    if CI_PLATFORM == "github":
        ...
        try:
            ...
            if platform_capabilities:
                ...
                total_success = sum(
                    p.get("success_count", 0) for p in platform_capabilities.values()
                )
...
def run_benchmarks(args):
    ...
    benchmark = HardwareBenchmark(output_dir=args.output_dir, database_path=args.benchmark_db)
    ...
    # Run benchmarks
    logger.info(
        f"Running benchmarks with device={device}, families={model_families}, iterations={args.iterations}"
    )

    results = benchmark.run_comprehensive_benchmark(
        device=device, families=model_families, iterations=args.iterations, batch_sizes=batch_sizes
    )
...
def run_optimizations(args, benchmark_results=None):
    ...
    compressor = ModelCompressor(output_dir=args.optimization_dir)
            ...
            if run_count > 1 and HAS_ADVANCED_COMPONENTS:
                try:
                    ...
                    benchmark = HardwareBenchmark(
                        output_dir=args.output_dir, database_path=args.benchmark_db
                    )
                    ...
                    for key, data in trends.get("by_model_device", {}).items():
                        ...
                        latency_icon = (
                            "🟢" if latency_change < 0 else "🔴" if latency_change > 0 else "⚪"
                        )
                        throughput_icon = (
                            "🟢"
...
def generate_trend_report(args):
    ...
    try:
        ...
        for key, data in trends.get("by_model_device", {}).items():
            ...
            throughput_icon = (
                "🟢" if throughput_change > 0 else "🔴" if throughput_change < 0 else "⚪"
            )
