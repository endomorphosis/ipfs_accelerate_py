...
def main():
    ...
    parser.add_argument(
        "--hardware", type=str, default="all", help="Hardware platform (cpu, cuda, openvino, all)"
    )
        ...
        try:
            ...
            if pipeline_result.get("pipeline_success", False):
                ...
                if "timings" in pipeline_result:
                    print(
                        f"  ⏱️ Average inference time: {pipeline_result['timings'].get('avg_inference_time', 0):.3f}s"
                    )
                ...
                if "implementation_type" in pipeline_result:
                    print(
                        f"  🔧 Implementation: {pipeline_result.get('implementation_type', 'UNKNOWN')}"
                    )
            else:
