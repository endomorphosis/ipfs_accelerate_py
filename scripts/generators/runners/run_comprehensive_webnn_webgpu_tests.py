...
class WebPrecisionTester:
    ...
    def verify_real_implementation(self, browser: str, platform: str) -> bool:
        ...
        self.browser_automation = BrowserAutomation(
            browser_type=browser, headless=not self.args.visible
        )

        # Detect capabilities
        capabilities = detect_web_platform_capabilities(
            browser=browser, use_browser_automation=True
        )
        ...
        if platform == "webnn":
            ...
            if not real_impl:
                logger.warning(
                    f"WebNN is not available or is simulated in {browser}. Using simulation mode."
                )
            ...
            if not real_impl:
                logger.warning(
                    f"WebGPU is not available or is simulated in {browser}. Using simulation mode."
                )
            return real_impl
    ...
    def run_precision_test(self, browser: str, platform: str, precision: int, model: str) -> Dict:
        ...
        try:
            if precision in [2, 3]:
                ...
                if platform != "webgpu":
                    logger.warning(
                        f"{precision}-bit precision is only available in WebGPU, not in {platform}"
                    )
                    return {
                        "status": "skipped",
                        "reason": f"{precision}-bit not supported in {platform}",
                    }
                ...
                if platform == "webnn" and not self.args.experimental:
                    logger.warning(
                        "4-bit precision in WebNN is experimental. Use --experimental to test."
                    )
                    return {
                        "status": "skipped",
                        "reason": "4-bit WebNN requires --experimental flag",
                    }
    ...
    def generate_markdown_report(self, results: List[Dict]):
        ...
        with open(filename, "w") as f:
            ...
            for browser in browsers:
                ...
                # Create table
                f.write(
                    "| Platform | Precision | Model | Implementation | Status | Latency (ms) | Throughput | Memory (MB) |\n"
                )
                f.write(
                    "|----------|-----------|-------|----------------|--------|-------------|-----------|------------|\n"
                )
                ...
                for result in sorted(
                    ...
                    f.write(
                        f"| {platform} | {precision}-bit | {model} | {impl} | {status} | {latency} | {throughput} | {memory} |\n"
                    )
                ...
                models = sorted(
                    set(
                        [
                            r.get("model")
                            for r in results
                            if r.get("platform") == platform and r.get("model")
                        ]
                    )
                )
                for model in models:
                    ...
                    f.write(
                        "| Precision | Browser | Implementation | Latency (ms) | Throughput | Memory (MB) | Memory Reduction |\n"
                    )
                    f.write(
                        "|-----------|---------|----------------|-------------|-----------|------------|------------------|\n"
                    )
                    ...
                    for browser in browsers:
                        fp32_results = [
                            ...
                            if r.get("browser") == browser and r.get("precision") == 32
                        ]
                        if fp32_results:
                    ...
                    # Sort by precision
                    for result in sorted(
                        model_results, key=lambda x: (x.get("precision", 0), x.get("browser", ""))
                    ):
                        precision = result.get("precision", "unknown")
                        browser = result.get("browser", "unknown")
                        impl = (
                            "REAL" if result.get("is_real_implementation", False) else "SIMULATION"
                        )
                        latency = result.get("average_latency_ms", "N/A")
                        throughput = result.get("throughput_items_per_sec", "N/A")
                        memory = result.get("memory_mb", "N/A")
                        ...
                        if (
                            browser in fp32_memory
                            and fp32_memory[browser] > 0
                        ...
                        f.write(
                            f"| {precision}-bit | {browser} | {impl} | {latency} | {throughput} | {memory} | {memory_reduction} |\n"
                        )
...
"""
        ...
        for browser in browsers:
            webnn_results = [
                r for r in results if r.get("browser") == browser and r.get("platform") == "webnn"
            ]
            webgpu_results = [
                r for r in results if r.get("browser") == browser and r.get("platform") == "webgpu"
            ]
            ...
            new_section += (
                f"| {browser.capitalize()} | {webnn_support} | {webgpu_support} | {notes} |\n"
            )
...
"""
        ...
        if "## Real Implementation Test Results" in content:
            ...
            content = re.sub(
                r"## Real Implementation Test Results.*?(?=^#|\Z)",
                new_section,
                content,
        ...
        for model in models:
            ...
            for browser in browsers:
                ...
                if webnn_support == "✅" and webgpu_support == "✅":
                    ...
                    webnn_latency = (
                        min([r.get("average_latency_ms", float("inf")) for r in webnn_results])
                        if webnn_results
                        else float("inf")
                    )
                    webgpu_latency = (
                        min([r.get("average_latency_ms", float("inf")) for r in webgpu_results])
                        if webgpu_results
                        else float("inf")
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test WebNN/WebGPU at different precision levels with real browsers"
    )
    ...
    # Model options
    parser.add_argument("--model", choices=SUPPORTED_MODELS, default="bert", help="Model to test")
    ...
    parser.add_argument(
        "--keep-browser-open", action="store_true", help="Keep browser open between tests"
    )
    ...
    parser.add_argument("--archive-docs", action="store_true", help="Archive old documentation")
    ...
    # Database options
    parser.add_argument(
        "--db-path", type=str, default=None, help="Path to benchmark database (DuckDB)"
    )
    ...
    print(
        f"Real implementations: {len([r for r in results if r.get('is_real_implementation', False)])}"
    )
