...
class ModelHardwareTester:
    ...
    def __init__(
        ...
        if models_dir:
            ...
            possible_dirs = ["./updated_models", "./key_models_hardware_fixes", "./modality_tests"]
    ...
    def get_model_files(self):
        ...
        logger.info(
            f"Found {len(model_files)} model test files out of {len(KEY_MODELS)} key models"
        )
        return model_files
    ...
    def generate_report(self):
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"({summary['mock_implementations'] / summary['successful_tests'] * 100:.1f}% of successful)\n"
            )
            f.write(f"- **Real implementations**: {summary['real_implementations']} ")
            f.write(
                f"({summary['real_implementations'] / summary['successful_tests'] * 100:.1f}% of successful)\n\n"
            )
            ...
            for platform, stats in summary["by_platform"].items():
                ...
                f.write(
                    f"| {platform} | {available} | {stats['total']} | {success_rate:.1f}% | {real_rate:.1f}% |\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--hardware", type=str, nargs="+", help="Specific hardware platforms to test"
    )
    parser.add_argument("--models-dir", type=str, help="Directory containing model test files")
