...
class TestCoverageRunner:
    ...
    def run_browser_capability_test(self, browser: str) -> Dict:
        ...
        try:
            ...
            print(
                f"[{browser}] Capability check completed: WebNN={webnn_available}, WebGPU={webgpu_available}, HW Accel={hw_acceleration}"
            )
    ...
    def run_webnn_benchmark(self, browser: str, model: str, batch_size: int = 1) -> Dict:
        ...
        output_file = os.path.join(
            self.output_dir, f"{browser}_{model.replace('/', '_')}_{batch_size}_webnn.json"
        )
        ...
        try:
            ...
            result_files = [
                ...
                if f.startswith(f"webnn_benchmark_{browser}_{model.replace('/', '_')}")
            ]
            if result_files:
                result_file = os.path.join(
                    self.output_dir, sorted(result_files)[-1]
            ...
            print(
                f"[{browser}] WebNN benchmark completed for {model}: CPU={cpu_time}ms, WebNN={webnn_time}ms, Speedup={speedup}x"
            )
    ...
    def run_webgpu_browser_comparison(self, model: str, browser: str = None) -> Dict:
        ...
        output_file = os.path.join(
            self.output_dir, f"{model.replace('/', '_')}_webgpu_comparison.json"
        )
        ...
        # Base command
        cmd = ["python", "test_webgpu_browsers_comparison.py"]
        ...
        try:
            ...
            if is_audio and "Firefox is" in output_str and "faster than Chrome" in output_str:
                ...
                try:
                    perf_line = [
                        line
                        for line in output_str.split("\n")
                        if "Firefox is" in line and "faster than Chrome" in line
                    ][0]
            ...
            error_output = e.output.decode("utf-8") if e.output else "No output"
            return {"model": model, "error": str(e), "error_output": error_output}
        except subprocess.TimeoutExpired:
            print(f"WebGPU browser comparison for {model} timed out after {self.timeout}s")
            return {"model": model, "error": f"Timeout after {self.timeout}s"}

    def run_optimization_test(
        self, optimization: str, model: str, browser: str = "firefox"
    ) -> Dict:
        ...
        cmd = [
            "python",
            "test_web_platform_optimizations.py",
            f"--browser={browser}",
            f"--model={model}",
            opt_flag,
        ]
        ...
        try:
            ...
            if "Improvement:" in output_str:
                try:
                    ...
                    improvement_pct = float(
                        improvement_line.split("Improvement:")[1].split("%")[0].strip()
                    )
                except (IndexError, ValueError):
                    pass
            ...
            if "Baseline:" in output_str:
                try:
                    ...
                    baseline_time = float(
                        baseline_line.split("Baseline:")[1].split("ms")[0].strip()
                    )
                except (IndexError, ValueError):
                    pass
            ...
            if "Optimized:" in output_str:
                try:
                    ...
                    optimized_time = float(
                        optimized_line.split("Optimized:")[1].split("ms")[0].strip()
                    )
                except (IndexError, ValueError):
                    pass
            ...
            print(
                f"[{browser}] {optimization} optimization test completed for {model}: Improvement={improvement_pct}%"
            )
            ...
            print(
                f"[{browser}] {optimization} optimization test for {model} timed out after {self.timeout}s"
            )
            return {
                "browser": browser,
                "model": model,
    ...
    def run_all_tests(self) -> Dict:
        ...
        for browser in self.browsers:
            self.results["browser_capabilities"][browser] = self.run_browser_capability_test(
                browser
            )
        ...
        if self.parallel:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
                ...
                for browser in self.browsers:
                    ...
                    if not self.results["browser_capabilities"][browser].get(
                        "webnn_available", False
                    ):
                ...
                for model in self.models:
                    ...
                    for batch_size in self.batch_sizes:
                        self.results["webnn_benchmarks"][browser][model][str(batch_size)] = (
                            self.run_webnn_benchmark(
                                browser=browser, model=model, batch_size=batch_size
                            )
                        )
        ...
        for model in self.models:
            self.results["cross_browser_comparisons"][model] = self.run_webgpu_browser_comparison(
                model=model
            )
        ...
        if self.parallel:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
                ...
                if self.enable_compute_shaders:
                    audio_models = [
                        ...
                        if any(audio in m for audio in ["whisper", "wav2vec2", "clap"])
                    ]
                ...
                if self.enable_parallel_loading:
                    multimodal_models = [
                        m for m in self.models if any(mm in m for mm in ["clip", "llava"])
                    ]
            ...
            for model in [
                ...
                self.results["optimization_tests"]["compute_shaders"][model] = (
                    self.run_optimization_test(
                        optimization="compute_shaders", model=model, browser="firefox"
                    )
                )
            ...
            for model in [m for m in self.models if any(mm in m for mm in ["clip", "llava"])]:
                self.results["optimization_tests"]["parallel_loading"][model] = (
                    self.run_optimization_test(
                        optimization="parallel_loading", model=model, browser="chrome"
                    )
                )
            ...
            for model in self.models:
                self.results["optimization_tests"]["shader_precompile"][model] = (
                    self.run_optimization_test(
                        optimization="shader_precompile", model=model, browser="chrome"
                    )
                )
        ...
        output_file = os.path.join(
            self.output_dir, f"webnn_coverage_results_{int(time.time())}.json"
        )
    ...
    def generate_coverage_summary(self) -> None:
        ...
        for model in self.models:
            ...
            for browser in self.browsers:
                if (
                    browser in self.results["webnn_benchmarks"]
                    and model in self.results["webnn_benchmarks"][browser]
                ):
                    model_coverage["browsers_tested"].append(browser)

                    # Check for best speedup
                    for batch_size, result in self.results["webnn_benchmarks"][browser][
                        model
                    ].items():
            ...
            for optimization in ["compute_shaders", "parallel_loading", "shader_precompile"]:
                if (
                    optimization in self.results["optimization_tests"]
                    and model in self.results["optimization_tests"][optimization]
                ):
                    ...
                    model_coverage["optimizations"][optimization] = {"tested": False}
    ...
    def generate_markdown_report(self) -> str:
        ...
        for browser in self.browsers:
            ...
            report += (
                "| Model | Batch Size | CPU Time (ms) | WebNN Time (ms) | Speedup | Simulation |\n"
            )
            report += (
                "|-------|------------|--------------|-----------------|---------|------------|\n"
            )
            ...
            for model in self.models:
                ...
                for batch_size in self.batch_sizes:
                    ...
                    if result.get("error"):
                        report += (
                            f"| {model} | {batch_size} | Error: {result['error']} | - | - | - |\n"
                        )
                    else:
        ...
        for model in self.models:
            ...
            if result.get("error"):
                ...
                if result.get("is_audio") and "vs_chrome_pct" in result.get(
                    "firefox_performance", {}
                ):
                    pct = result["firefox_performance"]["vs_chrome_pct"]
                    report += (
                        f"Firefox is **{pct:.1f}%** faster than Chrome for this audio model\n\n"
                    )
        ...
        report += (
            "| Model | Browsers Tested | Best Browser | Best Speedup | Optimizations Tested |\n"
        )
        report += "|-------|----------------|-------------|-------------|---------------------|\n"
        ...
        for model, coverage in summary["model_coverage"].items():
            ...
            best_speedup = (
                f"{coverage.get('best_speedup', 0):.2f}x"
                if coverage.get("best_speedup", 0) > 0
                else "N/A"
            )
            ...
            report += (
                f"| {model} | {browsers_tested} | {best_browser} | {best_speedup} | {opt_str} |\n"
            )
        ...
        firefox_webgpu = (
            summary["browser_support"].get("firefox", {}).get("webgpu_available", False)
        )
        ...
        report += "  ```bash\n"
        report += (
            "  ./run_web_platform_tests.sh --browser edge --model bert --enable-shader-precompile\n"
        )
        report += "  ```\n"
...
def parse_args():
    ...
    browser_group.add_argument(
        "--browser", type=str, help="Browser to test (chrome, edge, firefox, safari)"
    )
    browser_group.add_argument("--browsers", type=str, nargs="+", help="List of browsers to test")
    browser_group.add_argument(
        "--all-browsers", action="store_true", help="Test all supported browsers"
    )
    ...
    model_group.add_argument(
        "--audio-models-only",
        action="store_true",
        help="Test only audio models (good for testing Firefox optimization)",
    )
    model_group.add_argument(
        "--multimodal-models-only",
        action="store_true",
        help="Test only multimodal models (good for testing parallel loading)",
    )
    model_group.add_argument(
        "--batch-sizes", type=int, nargs="+", help="List of batch sizes to test"
    )

    # Optimization options
    optimization_group = parser.add_argument_group("Optimization options")
    optimization_group.add_argument(
        "--compute-shaders", action="store_true", help="Enable compute shader optimizations"
    )
    optimization_group.add_argument(
        "--parallel-loading", action="store_true", help="Enable parallel loading optimizations"
    )
    optimization_group.add_argument(
        "--shader-precompile", action="store_true", help="Enable shader precompilation"
    )
    optimization_group.add_argument(
        "--all-optimizations", action="store_true", help="Enable all optimizations"
    )

    # Test scope options
    scope_group = parser.add_argument_group("Test scope options")
    scope_group.add_argument(
        "--all", action="store_true", help="Test all browsers, models and optimizations"
    )
    scope_group.add_argument(
        "--quick", action="store_true", help="Run a quick test with minimal configuration"
    )
    scope_group.add_argument(
        "--firefox-audio-only",
        action="store_true",
        help="Test only Firefox with audio models (compute shader optimization)",
    )
    scope_group.add_argument(
        "--capabilities-only",
        action="store_true",
        help="Only check browser capabilities without benchmarks",
    )
    ...
    output_group.add_argument(
        "--output-dir",
        type=str,
        default="./webnn_coverage_results",
    ...
    output_group.add_argument(
        "--report-file",
        type=str,
        help="File to save the report to (default: webnn_coverage_report_{timestamp}.{format})",
    )

    # Execution options
    execution_group = parser.add_argument_group("Execution options")
    execution_group.add_argument(
        "--timeout", type=int, default=600, help="Timeout in seconds for each test"
    )
    execution_group.add_argument("--sequential", action="store_true", help="Run tests sequentially")
    execution_group.add_argument(
        "--max-concurrent", type=int, default=2, help="Maximum number of concurrent tests to run"
    )
...
def main():
    ...
    if args.quick:
        ...
        enable_optimizations = (
            ...
            or args.compute_shaders
            or args.parallel_loading
            or args.shader_precompile
        )
        capabilities_only = False
    ...
    if args.report_file:
        ...
        report_path = os.path.join(
            args.output_dir, f"webnn_coverage_report_{timestamp}.{extension}"
        )
