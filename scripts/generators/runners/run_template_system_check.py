...
DEPENDENCIES = {"duckdb": False, "torch": False, "transformers": False}
...
def check_database_creation():
    ...
    stdout, success = run_command(
        "python create_simple_template_db.py", "Creating template database"
    )
...
def check_template_validation():
    ...
    stdout_simple, success_simple = run_command(
    ...
    stdout_enhanced, success_enhanced = run_command(
    ...
    if success_simple and success_enhanced:
        ...
        if success_gen:
            ...
            logger.warning(
                "⚠️ Generator compatibility validation found issues (this is expected during development)"
            )
...
def check_test_generation():
    ...
    stdout, success = run_command(
        "python simple_test_generator.py -g bert -t", "Generating test for bert model"
    )
...
def check_hardware_detection():
    ...
    stdout, success = run_command(
        "python simple_test_generator.py --detect-hardware", "Detecting available hardware"
    )
...
def check_vit_generation():
    ...
    stdout, success = run_command(
        "python simple_test_generator.py -g vit -p cpu,webgpu -t -o test_vit_custom.py",
...
def check_generator_template_validation():
    ...
    if success:
        ...
        logger.warning(
            "⚠️ Some generators may not be compatible with all templates (this is expected)"
        )
...
def check_test_execution():
    ...
    stdout, success = run_command(
        "python -m py_compile test_hf_bert.py", "Compiling test_hf_bert.py to check syntax"
    )
