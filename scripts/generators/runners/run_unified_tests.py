...
def run_model_tests(test_files: List[Path], max_workers: int = 4) -> Dict[str, Any]:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(run_model_test, file_path): file_path for file_path in test_files
        }
...
def run_api_tests(test_files: List[Path], max_workers: int = 4) -> Dict[str, Any]:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(run_api_test, file_path): file_path for file_path in test_files
        }
...
def generate_report(
    model_results: Optional[Dict[str, Any]] = None, api_results: Optional[Dict[str, Any]] = None
) -> str:
    """
    ...
    if model_results:
        ...
        report.append(
            f"- Successful tests: {summary['success_count']} ({summary['success_count'] / summary['total_tests'] * 100:.1f}%)"
        )
        report.append(
            f"- Failed tests: {summary['error_count']} ({summary['error_count'] / summary['total_tests'] * 100:.1f}%)"
        )
        report.append(
            f"- Real implementations: {summary['real_implementations']} ({summary['real_implementations'] / summary['total_tests'] * 100:.1f}%)"
        )
        report.append(
            f"- Mock implementations: {summary['mock_implementations']} ({summary['mock_implementations'] / summary['total_tests'] * 100:.1f}%)"
        )
    ...
    if api_results:
        ...
        report.append(
            f"- Successful APIs: {summary['success_count']} ({summary['success_count'] / summary['total_apis'] * 100:.1f}%)"
        )
        report.append(
            f"- Failed APIs: {summary['error_count']} ({summary['error_count'] / summary['total_apis'] * 100:.1f}%)"
        )
        ...
        for result in api_results["results"]:
            ...
            report.append(
                f"| {api} | {status_str} | {tests} | {failures} | {errors} | {skipped} | {time_taken:.2f} |"
            )
...
def parse_args():
    ...
    model_group.add_argument(
        "--models", type=str, nargs="+", help="List of specific models to test"
    )
    model_group.add_argument(
        "--category",
        type=str,
    ...
    model_group.add_argument("--limit", type=int, help="Maximum number of model tests to run")

    # API test options
    api_group = parser.add_argument_group("API test options")
    api_group.add_argument("--apis", type=str, nargs="+", help="List of specific APIs to test")

    # Output options
    parser.add_argument("--output-dir", type=str, help="Custom output directory for test results")
    parser.add_argument("--report", type=str, help="Path to save test report")
