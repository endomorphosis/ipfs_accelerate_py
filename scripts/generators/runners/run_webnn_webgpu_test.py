...
async def test_webgpu_direct(browser, headless, model):
    ...
    try:
        ...
        if not success:
            ...
            return {"success": False, "error": "Failed to initialize WebGPU implementation"}
...
async def test_webnn_direct(browser, headless, model):
    ...
    try:
        ...
        if not success:
            ...
            return {"success": False, "error": "Failed to initialize WebNN implementation"}
...
def display_results(webgpu_results, webnn_results):
    ...
    if not webgpu_results.get("success", False):
        ...
        if perf_metrics:
            ...
            print(
                f"    Throughput: {perf_metrics.get('throughput_items_per_sec', 'N/A')} items/sec"
            )
    ...
    # Overall conclusion
    print("\nConclusion:")
    webgpu_real = webgpu_results.get("success", False) and not webgpu_results.get(
        "is_simulation", True
    )
    webnn_real = webnn_results.get("success", False) and not webnn_results.get(
        "is_simulation", True
    )
...
async def main_async(args):
    ...
    webgpu_real = results["webgpu"].get("success", False) and not results["webgpu"].get(
        "is_simulation", True
    )
    webnn_real = results["webnn"].get("success", False) and not results["webnn"].get(
        "is_simulation", True
    )
...
def main():
    ...
    parser.add_argument(
        "--platform", choices=["webgpu", "webnn", "both"], default="both", help="Platform to test"
    )
    parser.add_argument("--headless", action="store_true", help="Run in headless mode")
    parser.add_argument("--model", default="bert-base-uncased", help="Model to use for testing")
