...
def run_api_backoff_tests(args):
    ...
    for api in apis_to_test:
        ...
        result = run_test_script(
            "test_api_backoff_queue.py", test_args, f"Testing {api.upper()} API backoff and queue"
        )

        results[api] = {"exit_code": result, "status": "Success" if result == 0 else "Failed"}
...
def run_ollama_backoff_tests(args):
    ...
    return {"exit_code": result, "status": "Success" if result == 0 else "Failed"}
...
def run_multiplexing_tests(args):
    ...
    if args.skip_apis and all(api in args.skip_apis for api in required_apis):
        print(
            "API multiplexing tests require at least one of these APIs: " + ", ".join(required_apis)
        )
        return {"status": "Skipped"}
    ...
    # Run the test
    result = run_test_script(
        "test_api_multiplexing_enhanced.py", test_args, "Enhanced API key multiplexing tests"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "-a",
        nargs="+",
        choices=get_all_apis(),
        help="Specific APIs to test (default: all)",
    )
    parser.add_argument("--skip-apis", "-s", nargs="+", choices=get_all_apis(), help="APIs to skip")
    parser.add_argument(
        "--ollama-model",
        "-m",
        default="llama3",
    ...
    parser.add_argument(
        "--no-api-tests", action="store_true", help="Skip standard API backoff tests"
    )
    parser.add_argument(
        "--no-ollama-tests", action="store_true", help="Skip Ollama comprehensive tests"
    )
    parser.add_argument(
        "--no-multiplexing-tests", action="store_true", help="Skip API multiplexing tests"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Suppress detailed output in multiplexing tests"
    )
    ...
    total_tests = sum(1 for test in results["tests"].values() if test.get("status") != "Skipped")
    ...
    print(
        f"Success rate: {success_count}/{total_tests} tests passed ({results['success_rate']:.0%})"
    )
