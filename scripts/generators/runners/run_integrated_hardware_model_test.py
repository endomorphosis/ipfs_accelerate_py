...
def get_missing_files(include_optional=True):
    ...
    """
    # Core required files
    required_files = ["resource_pool.py", "test_resource_pool.py", "RESOURCE_POOL_GUIDE.md"]
...
def test_resource_pool_basic():
    ...
    try:
        ...
        resource1 = pool.get_resource(
            "test_resource", resource_id="1", constructor=lambda: test_obj1
        )
        resource2 = pool.get_resource(
            "test_resource", resource_id="2", constructor=lambda: test_obj2
        )
    ...
    try:
        ...
        if has_pytorch:
            ...
            if torch.cuda.is_available() and not hardware_info.get("cuda", False):
                logger.warning(
                    "⚠️ PyTorch reports CUDA available but hardware detection didn't identify it"
                )
                ...
                if torch_count != detected_count:
                    logger.warning(
                        f"⚠️ Device count mismatch: PyTorch reports {torch_count}, hardware detection reports {detected_count}"
                    )
...
def test_model_classifier():
    ...
    try:
        ...
        for model_name, expected_family in test_models.items():
            try:
                ...
                if family != expected_family:
                    logger.warning(
                        f"⚠️ Unexpected classification for {model_name}: got {family}, expected {expected_family}"
                    )
                else:
                    logger.info(
                        f"✅ Model {model_name} correctly classified as {family} (confidence: {confidence:.2f})"
                    )
        ...
        try:
            ...
            hw_analyses = [
                a for a in hw_result.get("analyses", []) if a.get("source") == "hardware_analysis"
            ]

            if hw_analyses:
                logger.info(
                    f"✅ Hardware-aware classification successfully used hardware information"
                )
                logger.info(
                    f"  Result: {hw_result.get('family')} (reason: {hw_analyses[0].get('reason', 'Unknown')})"
                )
            else:
...
def test_resource_pool_with_components(
    has_hardware_detection, hardware_info, has_model_classifier, classifier_info
):
    """
    ...
    try:
        ...
        stats = pool.get_stats()
        logger.info(
            f"Final ResourcePool stats: hits={stats.get('hits', 0)}, misses={stats.get('misses', 0)}"
        )
            ...
            if hasattr(hardware_model_integration, "integrate_hardware_and_model"):
                ...
                if has_hardware_detection and has_model_classifier:
                    try:
                        ...
                        model_family = (
                            classifier_info.get("classification_results", {})
                            .get("bert-base-uncased", {})
                            .get("family", "embedding")
                        )
                        ...
                        if isinstance(result, dict) and "device" in result:
                            ...
                            logger.warning(
                                f"⚠️ Integration function returned unexpected format: {result}"
                            )
                ...
                if known_models:
                    ...
                    model = pool.get_model(
                        ...
                        hardware_preferences={
                            "device": "cpu"
                        },  # Explicit device since no hardware detection
                    )
                    ...
                    logger.warning(
                        "⚠️ No classified models available for testing classifier-only scenario"
                    )
            except Exception as e:
            ...
            try:
                ...
                if known_models:
                    ...
                    model = pool.get_model(
                        ...
                        model_name=test_model_name,
                        constructor=create_mock_model,
                        hardware_preferences={"device": "auto", "hw_compatibility": hw_compat},
                    )
                    ...
                    logger.warning(
                        "⚠️ No classified models available for testing full integration scenario"
                    )
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test integration of ResourcePool, hardware_detection, and model_family_classifier"
    )
