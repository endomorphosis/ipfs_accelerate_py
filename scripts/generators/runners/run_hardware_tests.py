...
def get_test_class(module, model_type):
    ...
    test_classes = [
        ...
        if name.endswith("TestBase") or name.startswith("Test")
    ]
...
def main():
    ...
    parser.add_argument(
        "--models-dir", default="./updated_models", help="Directory containing model test files"
    )
    parser.add_argument(
        "--models", nargs="+", default=["bert", "t5"], help="Models to test (e.g., bert t5)"
    )
    parser.add_argument(
        "--platforms",
        nargs="+",
        default=["cpu", "cuda", "mps", "openvino", "rocm", "webnn", "webgpu"],
        help="Platforms to test on",
    )
    parser.add_argument(
        "--output-dir", default="./hardware_test_results", help="Directory to save test results"
    )
    parser.add_argument(
        "--model-names",
        nargs="+",
        default=[],
        help="Specific model names for each model (e.g., bert-base-uncased)",
    )
