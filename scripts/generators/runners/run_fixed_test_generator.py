...
def main():
    ...
    parser.add_argument(
        "--cross-platform", action="store_true", help="Enable cross-platform compatibility"
    )
    parser.add_argument(
        "--use-db-templates", action="store_true", help="Use templates from database"
    )
    parser.add_argument(
        "--enable-all",
        action="store_true",
    ...
    try:
        ...
        from fixed_merged_test_generator import (
            generate_test,
        ...
        if HAS_HARDWARE_DETECTION:
            ...
            logger.info(
                f"Detected hardware: {', '.join(k for k, v in hardware_info.items() if v.get('detected', False))}"
            )
