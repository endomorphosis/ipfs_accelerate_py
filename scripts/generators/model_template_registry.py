...
class ModelTemplateRegistry:
    ...
    def get_merged_template(self, template_name: str) -> Dict[str, Any]:
        ...
        for template in chain:
            ...
            merged["supports_hardware"] = list(
                set(merged["supports_hardware"] + template_info.get("supports_hardware", []))
            )

            # Merge compatible families (union of all compatible families)
            merged["compatible_families"] = list(
                set(merged["compatible_families"] + template_info.get("compatible_families", []))
            )
