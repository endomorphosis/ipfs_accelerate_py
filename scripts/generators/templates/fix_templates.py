...
def fix_unexpected_indent(template: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            content.startswith(("def ", "class ", "if ", "for ", "while ", "try:", "with "))
            or content == "else:"
            or content.startswith(("elif ", "except"))
            or content == "finally:"
        ):
...
def fix_all_templates(
    db_path: str = None,
    json_path: str = None,
    specific_templates: List[Tuple[str, str, Optional[str]]] = None,
) -> Dict[str, Any]:
    ...
    # Results tracking
    results = {"total": 0, "fixed": 0, "failed": 0, "skipped": 0, "details": []}
...
def add_missing_hardware_support(
    db_path: str = None, json_path: str = None, model_types: List[str] = None
) -> Dict[str, Any]:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fix template syntax errors and enhance hardware support"
    )

    parser.add_argument("--db-path", type=str, help="Path to the DuckDB database")
    parser.add_argument("--json-path", type=str, help="Path to the JSON template file")
    parser.add_argument("--fix-template", action="store_true", help="Fix a specific template")
    parser.add_argument("--model-type", type=str, help="Model type for template fix")
    parser.add_argument(
        "--template-type", type=str, default="test", help="Template type for template fix"
    )
    parser.add_argument("--hardware", type=str, help="Hardware platform for template fix")
    parser.add_argument("--fix-all", action="store_true", help="Fix all problematic templates")
    parser.add_argument(
...
def main():
    ...
    if args.fix_template:
        ...
        if template is None:
            logger.error(
                f"Template not found: {args.model_type}/{args.template_type}/{args.hardware}"
            )
            return 1
        ...
        if fixed_valid:
            ...
            success = db.add_template(
                args.model_type, args.template_type, fixed_template, args.hardware
            )
    ...
    # Add missing hardware support
        ...
        results = add_missing_hardware_support(
            args.db_path, args.json_path, [args.model_type] if args.model_type else None
        )
