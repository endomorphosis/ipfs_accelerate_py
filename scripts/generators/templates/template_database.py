...
class TemplateDatabase:
    ...
    def get_template(
        self, model_type: str, template_type: str, hardware_platform: Optional[str] = None
    ) -> Optional[str]:
    ...
    def _get_template_from_db(
        ...
        if hardware_platform:
            ...
            """,
                [model_type, template_type, hardware_platform],
            ).fetchone()
        ...
        if hardware_platform:
            ...
            ORDER BY version DESC LIMIT 1
            """,
                [template_type, hardware_platform],
            ).fetchone()
    ...
    def add_template(
        ...
        if self.conn is not None:
            try:
                ...
                ORDER BY version DESC LIMIT 1
                """,
                    [model_type, template_type, hardware_platform, hardware_platform],
                ).fetchone()
                ...
                logger.info(
                    f"Added template: {model_type}/{template_type}/{hardware_platform} (v{version})"
                )
                return True
            except Exception as e:
...
def parse_args():
    ...
    group.add_argument(
        "--list-templates", action="store_true", help="List all templates in the database"
    )
    group.add_argument("--create-template", action="store_true", help="Create a new template")
    group.add_argument(
        "--validate-templates", action="store_true", help="Validate all templates in the database"
    )
    group.add_argument(
        "--export", action="store_true", help="Export templates from database to JSON file"
    )
    group.add_argument(
        "--import",
        action="store_true",
    ...
    group.add_argument(
        "--import-directory", type=str, help="Import templates from a directory of Python files"
    )
    group.add_argument(
        "--generate-test", action="store_true", help="Generate a test file from a template"
    )

    # Template creation arguments
    group = parser.add_argument_group("Template Creation")
    group.add_argument("--model-type", type=str, help="Model type (bert, t5, llama, etc.)")
    group.add_argument(
        "--template-type",
        type=str,
        default="test",
    ...
    group.add_argument("--hardware-platform", type=str, help="Hardware platform (cuda, rocm, etc.)")
    group.add_argument("--template-file", type=str, help="File containing template content")
    group.add_argument(
        "--store-in-db",
        action="store_true",
    ...
    group.add_argument("--model-name", type=str, help="Model name for test generation")
    group.add_argument(
        "--hardware-platforms",
        type=str,
    ...
    group.add_argument(
        "--output", type=str, help="Output file for generated test (default: stdout)"
    )

    # Other arguments
    group = parser.add_argument_group("Other")
    group.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    group.add_argument(
        "--update-templates", action="store_true", help="Update all templates to latest versions"
    )
    ...
    if args.list_templates:
        ...
        if args.store_in_db:
            success = db.add_template(
                args.model_type, args.template_type, template, args.hardware_platform
            )
            ...
            logger.info(
                f"Template stored successfully: {args.model_type}/{args.template_type}/{args.hardware_platform or 'all'}"
            )
        else:
        ...
        print(
            f"Valid templates: {results['valid']} ({results['valid'] / results['total'] * 100:.1f}%)"
        )
        ...
        test_content = db.generate_test_file(
            args.model_type, args.model_name, args.template_type, hardware_platforms
        )
