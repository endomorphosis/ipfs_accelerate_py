...
MODEL_DESCRIPTIONS = {
    ...
    "llava-next": {
        "description": "LLaVA-Next (Advanced vision-language model)",
        "category": "multimodal",
    },
    ...
    "detr": {
        "description": "DETR (DEtection TRansformer for object detection)",
        "category": "vision",
    },
}
    ...
    def _generate_markdown_report(self, latest_results, output_path):
        ...
        try:
            with open(output_path, "w") as f:
                ...
                f.write(
                    "showing performance metrics including latency, throughput, and memory usage.\n\n"
                )
                ...
                for category, models in categories.items():
                    ...
                    for _, row in category_results.iterrows():
                        ...
                        f.write(
                            f"| {model} | {hardware} | {batch_size} | {latency:.2f} | {throughput:.2f} | {memory:.2f} |\n"
                        )
                ...
                f.write("## Conclusion\n\n")
                f.write(
                    "This report provides a comprehensive view of performance characteristics for 13 key model types across 8 hardware platforms. "
                )
                f.write(
                    "Use this information to guide hardware selection decisions and optimization efforts.\n"
                )
...
def main():
    ...
    # Main command groups
    parser.add_argument(
        "--generate", action="store_true", help="Generate comprehensive timing report"
    )
    ...
    parser.add_argument(
        "--format", choices=["html", "md", "markdown", "json"], default="html", help="Output format"
    )
    ...
    if args.generate:
        report_gen.generate_timing_report(
            output_format=args.format, output_path=args.output, days_lookback=args.days
        )
    else:
        parser.print_help()
