...
class TemplateDatabase:
    ...
    def _create_schema(self, conn):
        ...
        hardware_data = [
            ...
            (
                ...
                "qualcomm",
                "Qualcomm",
                "Qualcomm AI Engine/Hexagon DSP implementation",
                "supported",
            ),
        ]
        ...
        hardware_df = pd.DataFrame(
            hardware_data,
            columns=["hardware_id", "hardware_type", "display_name", "description", "status"],
        )
    ...
    def _infer_modality(self, model_id: str) -> str:
        ...
        if (
            "bert" in model_id_lower
            or "t5" in model_id_lower
            or "gpt" in model_id_lower
            or "llama" in model_id_lower
        ):
            return "text"
    ...
    def get_template(self, model_id: str) -> Optional[str]:
        ...
        try:
            result = conn.execute(
                "SELECT template_content FROM model_templates WHERE model_id = ?", [model_id]
            ).fetchone()
    ...
    def store_template(
        ...
        hardware_support: Optional[Dict[str, Any]] = None,
    ) -> bool:
        """
        ...
        try:
            ...
            exists = (
                conn.execute(
                    "SELECT COUNT(*) FROM model_templates WHERE model_id = ?", [model_id]
                ).fetchone()[0]
                > 0
            )
    ...
    def delete_template(self, model_id: str) -> bool:
        ...
        conn = self._get_connection()
        try:
            conn.execute("DELETE FROM model_templates WHERE model_id = ?", [model_id])
    ...
    def list_templates(
        self, model_family: Optional[str] = None, modality: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        for template in templates:
            print(
                f"  - {template['model_id']} ({template['model_family']}, {template['modality']})"
            )
        ...
        if template:
            ...
            print("\nTemplate content:")
            print(
                template["template_content"][:200] + "..."
                if len(template["template_content"]) > 200
                else template["template_content"]
            )
        else:
        ...
        for hw in hardware:
            print(
                f"  - {hw['hardware_type']} ({hw['display_name']}): {hw['description']} [{hw['status']}]"
            )
