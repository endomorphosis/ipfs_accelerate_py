...
def run_all_tests(
    test_dir: str, device: str = "cpu", parallel: bool = False, max_workers: int = 4
) -> List[Dict[str, Any]]:
    ...
    if parallel and len(test_files) > 1:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_file = {
                executor.submit(run_test, file, device, True): file for file in test_files
            }
...
def generate_report(
    results: List[Dict[str, Any]], output_file: str = "skillset_test_report.md"
) -> str:
...
def main():
    ...
    parser.add_argument(
        "--test-dir", type=str, default="../skillset", help="Directory containing test files"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--report-file", type=str, default="skillset_test_report.md", help="File to write report to"
    )
    parser.add_argument("--model", type=str, help="Run test for specific model only")
    parser.add_argument(
        "--sample", type=int, default=0, help="Run tests for a random sample of N models"
    )
    ...
    if args.model:
        ...
        # Run all tests
        results = run_all_tests(args.test_dir, args.device, args.parallel, args.max_workers)
