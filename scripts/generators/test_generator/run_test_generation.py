...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"generate_tests_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
def parse_args():
    ...
    model_group.add_argument("--model", help="Specific model name to generate test for")
    ...
    parser.add_argument(
        "--list-devices", action="store_true", help="List all available hardware devices and exit"
    )
    ...
    # Output options
    parser.add_argument(
        "--output-dir", default="./generated_tests", help="Directory to save generated files"
    )

    parser.add_argument(
        "--template-dir", help="Directory containing templates (default: use built-in templates)"
    )
    ...
    parser.add_argument(
        "--coverage-report", action="store_true", help="Generate coverage report after generation"
    )
...
def run_generation(args):
    ...
    if args.model:
        ...
        for model in all_models:
            success, file_path = generator.generate_test_file(
                model, force=args.force, verify=args.verify
            )
        ...
        generated, failed, total = generator.generate_models_by_priority(
            args.priority, verify=args.verify, force=args.force
        )
