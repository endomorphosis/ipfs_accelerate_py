...
def test_model_generation(
    model_name: str, force: bool = False, verify: bool = True, output_dir: str = None
) -> Tuple[bool, str, str]:
    """
    ...
    try:
        ...
        with open(report_file, "w") as f:
            ...
            for arch, stats in sorted(arch_results.items()):
                gen_rate = (
                    (stats["generation_success"] / stats["total"] * 100)
                    if stats["total"] > 0
                    else 0
                )
                test_rate = (
                    (stats["test_success"] / stats["total"] * 100) if stats["total"] > 0 else 0
                )
                f.write(
                    f"| {arch} | {stats['total']} | {stats['generation_success']} ({gen_rate:.1f}%) | {stats['test_success']} ({test_rate:.1f}%) |\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="generated_tests/new_models", help="Directory for generated files"
    )
    ...
    report_file = os.path.join(
        args.report_dir,
        f"new_models_test_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
    )
