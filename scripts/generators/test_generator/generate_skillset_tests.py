...
def generate_test_for_skillset(
    model_name: str, template_path: str, output_dir: str
) -> Tuple[bool, str]:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="../skillset", help="Directory to write test files to"
    )
    parser.add_argument(
        "--model", type=str, help="Specific model to generate test for (e.g., 'bert')"
    )
    ...
    if args.model:
        ...
        success, output_file = generate_test_for_skillset(
            args.model, args.template_path, args.output_dir
        )
        ...
        for file_path in skillset_files:
            ...
            success, _ = generate_test_for_skillset(model_name, args.template_path, args.output_dir)
