...
def main():
    ...
    group.add_argument("--validate", action="store_true", help="Validate existing test files")
    group.add_argument(
        "--report", action="store_true", help="Generate coverage report for existing tests"
    )
    ...
    # Create generator
    generator = ModelTestGenerator(output_dir=args.output_dir, template_dir=args.template_dir)
    ...
    if args.model:
        ...
        success, file_path = generator.generate_test_file(
            args.model, force=args.force, verify=not args.no_verify
        )
        ...
        generated, failed, total = generator.generate_models_by_priority(
            args.priority, verify=not args.no_verify, force=args.force
        )
        ...
        if success:
            ...
            print(
                f"Coverage report written to: {os.path.join(args.output_dir, 'model_test_coverage.md')}"
            )
            return 0
        else:
