    ...
    try:
        ...
        try:
            ...
            results["follows_pattern"] = (
                results["imports_model_test"]
                and results["inherits_model_test"]
                and results["has_required_methods"]
            )
...
def validate_test_files(directory: str, pattern: str = "test_hf_*.py") -> Dict[str, Any]:
    ...
    return {"total": total, "valid": valid, "invalid": total - valid, "files": results}


def generate_validation_report(
    results: Dict[str, Any], output_file: str = "validation_report.md"
) -> None:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory", "-d", default="./", help="Directory to search for test files"
    )
    parser.add_argument(
        "--pattern", "-p", default="test_hf_*.py", help="Glob pattern for test files"
    )
    parser.add_argument(
        "--report", "-r", default="validation_report.md", help="Path to save validation report"
    )
