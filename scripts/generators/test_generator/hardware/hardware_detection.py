            ...
            settings.update(
                {
                    ...
                    "performance_tier": "medium",
                    "pytorch_compatible": False,
                    "specific_settings": {"version": getattr(qnn, "__version__", "Unknown")},
                }
            )
        except Exception as e:
...
def initialize_device(device: str) -> Dict[str, Any]:
    ...
    """
    result = {"device": device, "success": False, "message": "", "settings": {}}
    ...
    try:
        ...
        if device == "cpu":
            ...
            result["message"] = (
                "CUDA initialized successfully" if result["success"] else "CUDA not available"
            )
            ...
            result["message"] = (
                "ROCm initialized successfully" if result["success"] else "ROCm not available"
            )
            ...
            result["message"] = (
                "MPS initialized successfully" if result["success"] else "MPS not available"
            )
