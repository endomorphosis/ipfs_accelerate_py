...
class TestOllamaBackoff(ModelTest):
    ...
    def test_concurrent_request_limit(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(
                    lambda i=i: self.client.generate(
                        model="llama3", prompt=f"Request {i}", request_id=f"test-{i}"
                    )
                )
                for i in range(3)
            ]
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    self.logger.info(
                        f"Request completed successfully: {result.get('text', '')[:50]}..."
                    )
        ...
        self.logger.info(
            f"Successfully completed {len(results)} requests with concurrency limit of 1"
        )
