...
class MockOllama(ollama):
    ...
    def make_post_request_ollama(
        ...
        if stream:
            ...
            def mock_stream():
                for i in range(3):
                    yield {"message": {"content": f"Mock response chunk {i + 1}"}, "done": i == 2}
...
class TestOllamaMock(ModelTest):
    ...
    def test_completions_method(self):
        ...
        self.assertIn(
            "chat", request["endpoint_url"], "Completions should convert to chat endpoint"
        )
        self.assertEqual("llama3", request["data"].get("model"), "Model name not correctly passed")
        self.assertEqual(
            "Complete this text",
    ...
    def test_backoff_queue_structure(self):
        ...
        self.assertTrue(
            hasattr(client, "initial_retry_delay"), "Missing initial_retry_delay attribute"
        )
        ...
        self.assertTrue(
            hasattr(client, "max_concurrent_requests"), "Missing max_concurrent_requests attribute"
        )
        ...
        self.assertTrue(
            hasattr(client, "PRIORITY_HIGH")
            and hasattr(client, "PRIORITY_NORMAL")
            and hasattr(client, "PRIORITY_LOW"),
