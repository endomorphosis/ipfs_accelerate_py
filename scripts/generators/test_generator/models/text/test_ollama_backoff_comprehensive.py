...
try:
    ...
    try:
        ...
        class ollama:
            ...
            def generate(
                self, model="llama3", prompt="", max_tokens=50, temperature=0.7, request_id=None
            ):
            ...
            def chat(
                ...
                return {
                    "message": {"content": f"Mock response to chat with {len(messages)} messages"}
                }
...
class TestOllamaBackoffComprehensive(ModelTest):
    ...
    def send_request(self, prompt, model, tag="test"):
        ...
        try:
            ...
            response = self.client.generate(
                model=model, prompt=prompt, max_tokens=30, temperature=0.7, request_id=request_id
            )
    ...
    def test_basic_queue(self):
        ...
        num_requests = 4
        prompts = [
            f"What is {i + 1} + {i + 2}? Respond with just the number." for i in range(num_requests)
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_requests) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_idx):
                ...
                try:
                    ...
                    self.logger.info(
                        f"Request {idx + 1}/{num_requests}: {status} in {result['time_taken']:.2f}s"
                    )
                except Exception as e:
                    self.logger.error(
                        f"Request {idx + 1}/{num_requests} raised exception: {str(e)}"
                    )
        ...
        self.logger.info(
            f"Basic Queue Test - {successful}/{num_requests} successful in {total_time:.2f}s"
        )
    ...
    def test_backoff_recovery(self):
        ...
        if has_backoff:
            self.logger.info(
                f"Client configuration: max_retries={self.client.max_retries}, "
        ...
        prompts = [
            ...
            * 10  # Large prompt to potentially trigger errors
    ...
    def test_queue_structure(self):
        ...
        self.assertTrue(
            hasattr(self.client, "max_concurrent_requests"),
            "Missing max_concurrent_requests attribute",
        )
        ...
        self.assertTrue(
            hasattr(self.client, "PRIORITY_HIGH")
            and hasattr(self.client, "PRIORITY_NORMAL")
            and hasattr(self.client, "PRIORITY_LOW"),
