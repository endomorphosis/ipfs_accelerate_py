...
try:
    ...
    from ipfs_accelerate_py.api_backends.groq import (
        groq,
        CHAT_MODELS,
        VISION_MODELS,
        AUDIO_MODELS,
...
class TestGroqModels(ModelTest):
    ...
    def setUp(self):
        ...
        # Test parameters
        self.test_params = {"max_tokens": 50, "temperature": 0.7, "top_p": 0.95}
    ...
    def test_usage_tracking(self):
        ...
        try:
            ...
            self.logger.info(
                f"Usage tracking working: {stats.get('total_requests', 0)} requests tracked"
            )
    ...
    def test_token_counting(self):
        ...
        try:
            token_count = self.groq_client.count_tokens(
                "This is a test sentence to count tokens.", "llama3-8b-8192"
            )
            self.assertIsNotNone(token_count)
            self.assertIn("estimated_token_count", token_count)
            self.logger.info(
                f"Token counting working: {token_count.get('estimated_token_count', 0)} tokens estimated"
            )
        except Exception as e:
        ...
        if CHAT_MODELS:
            ...
            try:
                response = self.groq_client.chat(
                    model_name=model_name, messages=self.chat_prompt, **self.test_params
                )

                self.assertIsNotNone(response)
                self.assertIn("text", response)
                self.logger.info(
                    f"Successfully got response from {model_name}: {response.get('text', '')[:50]}..."
                )
    ...
    def test_vision_models(self):
        ...
        if VISION_MODELS:
            ...
            try:
                response = self.groq_client.chat(
                    model_name=model_name, messages=self.vision_prompt, **self.test_params
                )
