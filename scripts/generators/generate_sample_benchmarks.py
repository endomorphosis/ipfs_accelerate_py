...
def add_performance_result(conn, model_id, hardware_id, batch_size, model_type, hardware_type):
    ...
    try:
        ...
        max_id_result = conn.execute(
            "SELECT COALESCE(MAX(id), 0) + 1 FROM performance_results"
        ).fetchone()
        next_id = max_id_result[0] if max_id_result else 1
        ...
        logger.info(
            f"Added performance result: {model_type} on {hardware_type}, batch_size={batch_size}"
        )
        return True
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--db", type=str, default="./benchmark_db.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument(
        "--batch-sizes", type=str, default="1,2,4,8", help="Batch sizes to simulate"
    )
    ...
    try:
        ...
        for model_key, model_info in MODEL_TYPES.items():
            ...
            model_id = ensure_model_exists(
                conn, model_info["name"], model_info["family"], model_info["modality"]
            )
            ...
            for hardware_type in HARDWARE_ENDPOINTS:
                ...
                for batch_size in batch_sizes:
                    add_performance_result(
                        conn, model_id, hardware_id, batch_size, model_key, hardware_type
                    )
