...
def fix_test_platform_method():
    ...
    try:
        ...
        test_platform_method = re.search(
            r"def test_platform\(self, input_data, platform, platform_type=None\):.*?(?=\n    def|\n\nclass|\Z)",
            content,
            re.DOTALL,
        )
        ...
        webnn_handler = re.search(
            r'elif platform_lower == "webnn":(.*?)(?=elif |else:)', original_method, re.DOTALL
        )
        webgpu_handler = re.search(
            r'elif platform_lower == "webgpu":(.*?)(?=elif |else:)', original_method, re.DOTALL
        )
...
def fix_init_web_methods():
    ...
    try:
        ...
        init_webnn_method = re.search(
            r"def init_webnn\(self.*?\):.*?(?=\n    def|\n\nclass|\Z)", content, re.DOTALL
        )
        init_webgpu_method = re.search(
            r"def init_webgpu\(self.*?\):.*?(?=\n    def|\n\nclass|\Z)", content, re.DOTALL
        )
        ...
        if "ONNX Web API" in init_webnn_method.group(
            0
        ) and "web_batch_supported" in init_webnn_method.group(0):
...
def add_mock_processors():
    ...
    try:
        ...
        mock_processor_method = re.search(
            r"def _create_mock_processor\(self.*?\):.*?(?=\n    def|\n\nclass|\Z)",
            content,
            re.DOTALL,
        )
...
def add_web_platform_cli_args():
    ...
    try:
        ...
        argparse_section = re.search(
            r"parser = argparse\.ArgumentParser\(.*?\)", content, re.DOTALL
        )
...
"""
                ...
                if init_section:
                    ...
                    init_call = content[
                        main_func.end() + init_section.start() : main_func.end()
                        + init_section.end()
                        + 200
                    ]
...
def update_hardware_mapping():
    ...
    try:
        ...
        hardware_map_end_match = re.search(
            r"}(\s*# End of hardware map|\s*})", content[hardware_map_match.end() :]
        )
        ...
        hardware_map_section = content[
            hardware_map_match.start() : hardware_map_match.end() + hardware_map_end_match.end()
        ]

        # Check if we need to update the model entries
        if (
            '"webnn": {"implementation": "REAL"' in hardware_map_section
            and '"webgpu": {"implementation": "REAL"' in hardware_map_section
        ):
        ...
        for entry in model_entries:
            ...
            if last_platform:
                ...
                web_suitable = any(
                    ...
                    for term in ["bert", "vit", "clip", "t5", "whisper", "wav2vec2", "clap"]
                )
                ...
                if any(term in model_name.lower() for term in ["bert", "vit", "clip"]):
                    ...
                    term in model_name.lower() for term in ["t5", "whisper", "wav2vec2", "clap"]
                ):
                    webnn_impl = '"ENHANCED"'
                ...
                if any(
                    term in model_name.lower() for term in ["audio", "whisper", "wav2vec2", "clap"]
                ):
                    batch = "false"
                else:
                    batch = "true"
