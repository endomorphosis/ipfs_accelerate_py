...
def generate_test_file(model_type, output_dir=OUTPUT_DIR):
    ...
    try:
        ...
        cmd = [
            ...
            str(GENERATOR_PATH),
            "--batch-generate",
            model_type,  # Use batch-generate which uses huggingface_model_types.json
            "--output-dir",
            str(output_dir),
        ]
        ...
        if success and exists:
            ...
            logger.error(
                f"Failed to generate test for {model_type}, return code: {result.returncode}"
            )
...
def run_test_file(test_file):
    ...
    try:
        ...
        if success:
            ...
            logger.error(
                f"Failed to execute test: {os.path.basename(test_file)}, return code: {result.returncode}"
            )
            logger.error(f"STDOUT: {result.stdout}")
            logger.error(f"STDERR: {result.stderr}")
...
def main():
    ...
    if missing_from_official:
        logger.warning(
            f"Found {len(missing_from_official)} model types in skillset not in official list: {missing_from_official}"
        )
