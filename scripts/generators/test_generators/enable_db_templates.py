...
"""
        ...
        if "--use-db-templates" in content and "use_db_templates=" not in content:
            ...
            if generate_match:
                ...
                # Add the flag
                content = (
                    content[:end_pos] + ", use_db_templates=use_db_templates" + content[end_pos:]
                )
        ...
        if "use_db_templates=" in content and "use_db_templates=False" not in content:
            ...
            if generate_func:
                ...
                if "use_db_templates" not in content[generate_func.start() : params_end]:
                    content = (
                        content[:params_end] + ", use_db_templates=False" + content[params_end:]
                    )
                    logger.info(
                        f"Added use_db_templates parameter to generate function in {file_path}"
                    )
                ...
                if "if use_db_templates" not in content[body_start:body_end]:
                    ...
                    if template_loading:
                        ...
                        # Get the indentation
                        indent = " " * (
                            len(content[line_start:template_pos])
                            - len(content[line_start:template_pos].lstrip())
                        )
...
{indent}"""
                        ...
                        logger.info(
                            f"Added template database support to generate function in {file_path}"
                        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Enable DB templates in test generators")
    parser.add_argument("--dir", type=str, default=".", help="Directory to search for generators")
    parser.add_argument(
        "--generators", type=str, help="Comma-separated list of generators to update (default: all)"
    )
