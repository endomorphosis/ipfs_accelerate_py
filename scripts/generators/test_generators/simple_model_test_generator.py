    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if pipeline_tag == "text-generation":
                ...
                if any(
                    arch in model_name.lower()
                    for arch in ["llama", "falcon", "mistral", "mixtral", "mamba"]
                ):
                    use_remote_code = True
                    remote_code_reason = (
                        f"LLM architecture '{model_name}' often requires remote code"
                    )

                # Check for quantized models which often need remote code
                if any(
                    qt in model_name.lower() or qt in tags
                    for qt in ["gguf", "ggml", "awq", "gptq", "quantized"]
                ):
                    use_remote_code = True
                    remote_code_reason = "Quantized model often requires special handlers"
                    special_installation_notes.append(
                        "Model may require special quantization libraries"
                    )
            ...
            if any(x in pipeline_tag for x in ["image-to-text", "visual-question-answering"]):
                if any(
                    arch in model_name.lower() for arch in ["llava", "blip", "flamingo", "idefics"]
                ):
                    use_remote_code = True
                    remote_code_reason = (
                        f"Vision-language model '{model_name}' requires remote code"
                    )
...
def update_dependencies_files(dependencies_dict):
    ...
    with open(MODEL_DEPENDENCIES_MD, "w") as f:
        ...
        f.write(
            "|-------|"
            + "|".join(["-" * (len(dep.split(">=")[0]) + 2) for dep in all_deps])
            + "|\n"
        )
        ...
        group_num = 1
        for deps, models in sorted(
            dependency_groups.items(), key=lambda x: len(x[1]), reverse=True
        ):
            if len(deps) > 0:
                ...
                for model in sorted(models):
                    remote_flag = (
                        ...
                        if existing_deps[model].get("use_remote_code", False)
                        else ""
                    )
    ...
    logger.info(
        f"Updated enhanced dependency files: {MODEL_DEPENDENCIES_JSON} and {MODEL_DEPENDENCIES_MD}"
    )
...
def generate_comprehensive_model_list():
    ...
    return [
        ...
        {
            "name": "microsoft/swinv2-base-patch4-window8-256",
            "task": "image-classification",
            "priority": "medium",
        },
        {
        ...
        {
            "name": "facebook/xclip-base-patch32",
...
def generate_models_batch(models_to_generate=None, concurrency=1, priority_filter=None):
    ...
    if priority_filter:
        models_to_generate = [
            m for m in models_to_generate if m.get("priority", "medium") in priority_filter
        ]
    ...
    if concurrency > 1 and len(models_to_generate) > 1:
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            logger.info(
                f"Analyzing dependencies for {len(models_to_generate)} models with {concurrency} workers..."
            )
            ...
            dep_futures = {
                executor.submit(analyze_model, model): model for model in models_to_generate
            }
            ...
            def generate_model_test(model):
                try:
                    ...
                    output_file = generate_test_file(
                        model["name"], normalized_name, model["task"], SKILLS_DIR, deps_info
                    )
            ...
            file_futures = {
                executor.submit(generate_model_test, model): model
                for model in models_to_generate
                if model["name"] in dependency_dict
            }
        ...
        for model in models_to_generate:
            try:
                ...
                logger.info(f"Generated test file: {output_file}")
                logger.info(
                    f"  Dependencies: {', '.join(deps_info['dependencies']) if deps_info['dependencies'] else 'None'}"
                )
    ...
    print("\nSummary:")
    print(
        f"Generated {num_successful}/{total} test files ({num_successful / total * 100:.1f}% success rate)"
    )
...
def update_test_progress(generated_files, all_models, failed_models):
    ...
    try:
        ...
        progress_data = {
            ...
            "timestamp": datetime.datetime.now().isoformat(),
            "coverage_percentage": len(generated_files) / len(all_models) * 100
            if all_models
            else 0,
            "by_task": {},
            "by_priority": {},
        ...
        remote_code_models = [
            m for m, _, info in generated_files if info.get("use_remote_code", False)
        ]
        progress_data["remote_code_models"] = len(remote_code_models)
        progress_data["remote_code_percentage"] = (
            (len(remote_code_models) / len(generated_files)) * 100 if generated_files else 0
        )
        ...
        with open(MODEL_TESTING_PROGRESS_MD, "w") as f:
            ...
            f.write(
                f"<p><b>{progress_data['successful_models']}</b> of {progress_data['total_available_models']} models</p>\n"
            )
            ...
            f.write(
                f"<p><b>{progress_data.get('remote_code_models', 0)}</b> models requiring remote code</p>\n"
            )
            f.write(
                f"<p><b>{progress_data.get('remote_code_percentage', 0):.1f}%</b> of tested models</p>\n"
            )
            f.write("</div>\n\n")
            ...
            f.write(
                f"<p><b>{progress_data.get('unique_dependencies', 0)}</b> unique dependencies</p>\n"
            )
            ...
            f.write(
                f"- **Successfully Tested**: {progress_data['successful_models']} ({progress_data['coverage_percentage']:.1f}%)\n"
            )
            ...
            for priority in ["high", "medium", "low"]:
                if priority in progress_data["by_priority"]:
                    ...
                    f.write(
                        f"| {priority.capitalize()} | {data['total']} | {data['success']} | {coverage:.1f}% |\n"
                    )
...
def main():
    ...
    parser.add_argument(
        "--task", type=str, help="Task type for the model (required if --model is specified)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of concurrent model generations for batch mode",
    )
    parser.add_argument(
        "--priority",
        type=str,
    ...
    parser.add_argument(
        "--category",
        type=str,
        choices=["language", "vision", "audio", "multimodal"],
    ...
    parser.add_argument(
        "--list-models",
        action="store_true",
        help="List all available models without generating tests",
    )
    parser.add_argument("--list-tasks", action="store_true", help="List all available tasks")
    parser.add_argument(
        "--list-dependencies", action="store_true", help="List all dependencies for selected models"
    )

    # Add arguments for output control
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Custom output directory for test files"
    )
    parser.add_argument(
        "--save-progress", action="store_true", help="Save test progress to markdown file"
    )
    ...
    parser.add_argument(
        "--add-mocks", action="store_true", help="Add comprehensive mocks to existing test files"
    )
    parser.add_argument(
        "--update-dependencies",
        action="store_true",
