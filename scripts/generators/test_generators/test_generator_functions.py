...
def test_normalize_model_name():
    ...
    for input_name, expected in test_cases:
        ...
        assert result == expected, (
            f"normalize_model_name({input_name}) returned {result}, expected {expected}"
        )
...
def test_get_pipeline_category():
    ...
    for tasks, expected in test_cases:
        ...
        assert result == expected, (
            f"get_pipeline_category({tasks}) returned {result}, expected {expected}"
        )
...
def test_get_appropriate_model_name():
    ...
    for tasks, expected_substring in test_cases:
        ...
        assert expected_substring in result, (
            f"get_appropriate_model_name({tasks}) returned {result}, expected to contain {expected_substring}"
        )
...
def test_get_specialized_test_inputs():
    ...
    assert any("self.test_text =" in inp for inp in text_inputs), (
        "Text generation inputs should include test_text"
    )

    # Test image inputs
    image_inputs = get_specialized_test_inputs("image-classification")
    assert any("self.test_image =" in inp for inp in image_inputs), (
        "Image classification inputs should include test_image"
    )

    # Test audio inputs
    audio_inputs = get_specialized_test_inputs("automatic-speech-recognition")
    assert any("self.test_audio =" in inp for inp in audio_inputs), (
        "Audio inputs should include test_audio"
    )
