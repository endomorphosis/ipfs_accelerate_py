...
def fix_unterminated_strings(file_path):
    ...
    fixed_content = re.sub(
        pattern,
        lambda m: f'template_database["{m.group(1)}"] = """{m.group(2)}"""\n\n',
        content,
        flags=re.DOTALL,
    )
...
def update_hardware_detection(file_path):
    ...
    content = re.sub(
        hardware_detection_pattern, hardware_detection_replacement, content, flags=re.DOTALL
    )
...
def update_template_generation(file_path):
    ...
    platform_code_pattern = (
        r"# Platform-specific code\s+platform_code = \{\}\s+for platform in platforms:"
    )
...
def fix_import_templates(file_path):
    ...
    if (
        "import scripts.generators.templates.template_database as template_database as db"
        not in content
        and "from hardware_test_templates import scripts.generators.templates.template_database as template_database"
        not in content
    ):
    ...
    try:
        from hardware_test_templates import scripts.generators.templates.template_database as template_database as db
...
def main():
    ...
    print(
        f"You can now run tests with the fixed generator: python {generator_path} --model bert --cross-platform --hardware all"
    )
