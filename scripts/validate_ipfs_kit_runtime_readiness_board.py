...
def _taskboard_definition_sha256(text: str) -> str:
    ...
    for match in re.finditer(r"^## (KITA-\d{3})\b", text, flags=re.MULTILINE):
        if match.group(1) in TASK_IDS:
            continue
        # Backlog-refinery repairs are an operational appendix outside the
        # sealed 48-task program.  Appending one introduces a blank separator;
        # restore the canonical board's single terminal newline before
        # calculating its original seal.
...
def _validate_operational_repair_tasks(
    repairs: Iterable[object],
    *,
    canonical_by_id: dict[str, object],
    errors: list[str],
) -> tuple[str, ...]:
    """Validate generated recovery work without adding it to the sealed DAG."""
    ...
    for offset, repair in enumerate(repair_list):
        task_id = str(getattr(repair, "task_id", "") or "")
        operational_ids.append(task_id)
        expected_id = f"KITA-{len(TASK_IDS) + offset:03d}"
        if task_id != expected_id:
            errors.append(
                "operational retry-repair IDs must be contiguous: "
                f"expected {expected_id}, got {task_id}"
            )
        raw_metadata = getattr(repair, "metadata", {}) or {}
        metadata = {
        ...
        if source is None or failure_kind not in {
            "validation",
            "implementation",
            "merge",
        }:
        ...
        if (
            ...
            or metadata.get("retry repair source") != source_task_id
            or metadata.get("retry failure kind") != failure_kind
            or metadata.get("canonical board task") != "false"
        ):
            errors.append(f"{task_id} lacks exact operational provenance")
        if getattr(repair, "completion", "") != "manual":
            errors.append(f"{task_id} must use manual completion")
        if getattr(repair, "priority", "") != "P1":
            errors.append(f"{task_id} must use P1 priority")
        if getattr(repair, "track", "") != "ops":
            errors.append(f"{task_id} must use the ops track")
        if getattr(repair, "status", "") not in PERSISTED_PROGRESS_STATES:
            errors.append(f"{task_id} has invalid operational status")
        if tuple(getattr(repair, "depends_on", ()) or ()) != tuple(
            getattr(source, "depends_on", ()) or ()
        ):
            errors.append(
                f"{task_id} dependency scope differs from {source_task_id}"
            )

        source_outputs = list(getattr(source, "outputs", ()) or ())
        ...
        if not discovery:
            errors.append(f"{task_id} has no retry discovery provenance")
        if discovery and discovery in repair_outputs:
            errors.append(
                f"{task_id} incorrectly grants write authority to discovery evidence"
            )
        if any(
            output.startswith("data/agent_supervisor/")
            and PurePosixPath(output).name == "discovery"
            for output in repair_outputs
        ):
            errors.append(
                f"{task_id} incorrectly declares runtime discovery as an output"
            )
        if not getattr(repair, "validation", ()) or len(
            getattr(repair, "validation", ()) or ()
        ) != 1:
            errors.append(f"{task_id} must have exactly one validation command")
        if not getattr(repair, "acceptance", ""):
            errors.append(f"{task_id} has empty acceptance")

        source_kind = (source_task_id, failure_kind)
        if (
            source_kind in active_source_kinds
            and getattr(repair, "status", "") != "completed"
        ):
            errors.append(
                f"{task_id} duplicates an active repair for {source_kind}"
            )
        if getattr(repair, "status", "") != "completed":
            active_source_kinds.add(source_kind)
    return tuple(operational_ids)
...
def validate(
    *,
    ...
    tasks: list[object] = []
    operational_repairs: list[object] = []
    operational_appendix_started = False
    for task in parsed_tasks:
        metadata = getattr(task, "metadata", {}) or {}
        is_operational = (
            str(metadata.get("canonical board task") or "").strip().lower()
            == "false"
        )
        if is_operational:
            operational_appendix_started = True
            operational_repairs.append(task)
            continue
        if operational_appendix_started:
            errors.append(
                f"canonical task {task.task_id} appears after the operational appendix"
            )
        tasks.append(task)
    ...
    operational_task_ids = _validate_operational_repair_tasks(
        operational_repairs,
