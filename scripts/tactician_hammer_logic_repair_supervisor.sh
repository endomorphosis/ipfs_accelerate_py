...
export LPR_STATE_ROOT="${PROGRAM_ROOT}"
export PYTHONPATH="${REPO_ROOT}:${REPO_ROOT}/ipfs_datasets_py${PYTHONPATH:+:${PYTHONPATH}}"
export PYTHONDONTWRITEBYTECODE=1
export PYTHONHASHSEED=0
export IPFS_ACCELERATE_AGENT_IMPLEMENTATION_PROVIDER="${IPFS_ACCELERATE_AGENT_IMPLEMENTATION_PROVIDER:-grok_cli}"
export IPFS_ACCELERATE_AGENT_IMPLEMENTATION_FALLBACK_PROVIDER="${IPFS_ACCELERATE_AGENT_IMPLEMENTATION_FALLBACK_PROVIDER:-codex}"
export IPFS_ACCELERATE_AGENT_IMPLEMENTATION_FALLBACK_TRIGGER="${IPFS_ACCELERATE_AGENT_IMPLEMENTATION_FALLBACK_TRIGGER:-primary_quota_exhausted}"
...
export IPFS_ACCELERATE_AGENT_CODEX_REASONING_EFFORT="${IPFS_ACCELERATE_AGENT_CODEX_REASONING_EFFORT:-medium}"
...
validate_test_mode() {
  if [[ "${TEST_MODE}" != "1" ]]; then
    if [[ "${RUNNER_MODULE}" != "${DEFAULT_RUNNER_MODULE}" ]]; then
      echo "LPR_RUNNER_MODULE overrides are allowed only in isolated test mode" >&2
      return 2
    fi
    return 0
  fi
