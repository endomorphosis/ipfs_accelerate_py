#!/usr/bin/env python3
"""
Benchmark for the speech-to-text-2 skillset implementation.

This benchmark measures the performance of the speech-to-text-2 model implementation
...
@BenchmarkRegistry.register(
    name="speech-to-text-2_inference_benchmark",
    category="inference",
    models=["speech-to-text-2"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"]
)
class SpeechToText2InferenceBenchmark(BenchmarkBase):
    """Benchmark for speech-to-text-2 inference performance."""
    
    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up speech-to-text-2 inference benchmark")
        ...
        try:
            ...
            # Import the speech-to-text-2 skillset
            self.model_name = "speech-to-text-2"
            self.model_type_safe = "speech_to_text_2"
    ...
    def execute(self):
        ...
        self.logger.info("Executing speech-to-text-2 inference benchmark")
...
@BenchmarkRegistry.register(
    name="speech-to-text-2_throughput_benchmark",
    category="throughput",
    models=["speech-to-text-2"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"]
)
class SpeechToText2ThroughputBenchmark(SpeechToText2InferenceBenchmark):
    """Benchmark for speech-to-text-2 throughput performance."""
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing speech-to-text-2 throughput benchmark")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark speech-to-text-2 skillset implementation")
    ...
    benchmark_name = "speech-to-text-2_inference_benchmark" if args.type == "inference" else "speech-to-text-2_throughput_benchmark"
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: speech-to-text-2")
