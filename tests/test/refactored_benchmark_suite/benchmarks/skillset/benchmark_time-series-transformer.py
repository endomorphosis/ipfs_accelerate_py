#!/usr/bin/env python3
"""
Benchmark for the time-series-transformer skillset implementation.

This benchmark measures the performance of the time-series-transformer model implementation
...
@BenchmarkRegistry.register(
    name="time-series-transformer_inference_benchmark",
    category="inference",
    models=["time-series-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"]
)
class TimeSeriesTransformerInferenceBenchmark(BenchmarkBase):
    """Benchmark for time-series-transformer inference performance."""
    
    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up time-series-transformer inference benchmark")
        ...
        try:
            ...
            # Import the time-series-transformer skillset
            self.model_name = "time-series-transformer"
            self.model_type_safe = "time_series_transformer"
    ...
    def execute(self):
        ...
        self.logger.info("Executing time-series-transformer inference benchmark")
...
@BenchmarkRegistry.register(
    name="time-series-transformer_throughput_benchmark",
    category="throughput",
    models=["time-series-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"]
)
class TimeSeriesTransformerThroughputBenchmark(TimeSeriesTransformerInferenceBenchmark):
    """Benchmark for time-series-transformer throughput performance."""
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing time-series-transformer throughput benchmark")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark time-series-transformer skillset implementation")
    ...
    benchmark_name = "time-series-transformer_inference_benchmark" if args.type == "inference" else "time-series-transformer_throughput_benchmark"
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: time-series-transformer")
