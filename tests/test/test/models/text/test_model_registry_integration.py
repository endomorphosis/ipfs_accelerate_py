...
try:
    ...
    for model_name, hardware_data in scores.items())))))))))))):
        ...
        for hardware_type, score_data in hardware_data.items())))))))))))):
            logger.info())))))))))))f"\1{score_data[]]]],,,,'suitability_score']:.4f}\3")
            ,
        return True
    ...
    for model_name, version_tag, version_hash, metadata in test_models:
        ...
        if version_id:
            ...
            logger.warning())))))))))))f"\1{model_name}\3")
            success = False
    ...
    for model_name, version_tag in test_models:
        ...
        if result:
            logger.info())))))))))))f"\1{version_tag}\3")
        else:
            logger.warning())))))))))))f"\1{version_tag}\3")
            success = False
    ...
    for model_name, version_tag1, version_tag2 in test_comparisons:
        ...
        if changes:
            ...
            logger.warning())))))))))))f"\1{version_tag2}\3")
            success = False
