...
MASK2FORMER_MODELS_REGISTRY = {
    "mask2former-base-uncased": {
        "full_name": "MASK2FORMER Base Uncased",
        "architecture": "encoder-only",
        "description": "MASK2FORMER Base model with uncased vocabulary",
    ...
    "mask2former-large-uncased": {
        "full_name": "MASK2FORMER Large Uncased",
        "architecture": "encoder-only",
        "description": "MASK2FORMER Large model with uncased vocabulary",
        "mask2former": "mask2former",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "mask2former-base-cased": {
        "full_name": "MASK2FORMER Base Cased",
        "architecture": "encoder-only",
        "description": "MASK2FORMER Base model with cased vocabulary",
        "mask2former": "mask2former",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="mask2former-base-uncased", device=None):
        """Initialize the test class for MASK2FORMER models.
        
        Args:
            model_id: The model ID to test (default: "mask2former-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing MASK2FORMER model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing MASK2FORMER model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing MASK2FORMER model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_mask2former_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test MASK2FORMER HuggingFace models")
    parser.add_argument("--model", type=str, default="mask2former-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available MASK2FORMER models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable MASK2FORMER models:")
        for model_id, info in MASK2FORMER_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested MASK2FORMER model: {args.model}")
