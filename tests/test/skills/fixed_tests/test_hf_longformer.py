...
LONGFORMER_MODELS_REGISTRY = {
    "longformer-base-uncased": {
        "full_name": "LONGFORMER Base Uncased",
        "architecture": "encoder-only",
        "description": "LONGFORMER Base model with uncased vocabulary",
    ...
    "longformer-large-uncased": {
        "full_name": "LONGFORMER Large Uncased",
        "architecture": "encoder-only",
        "description": "LONGFORMER Large model with uncased vocabulary",
        "longformer": "longformer",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "longformer-base-cased": {
        "full_name": "LONGFORMER Base Cased",
        "architecture": "encoder-only",
        "description": "LONGFORMER Base model with cased vocabulary",
        "longformer": "longformer",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="longformer-base-uncased", device=None):
        """Initialize the test class for LONGFORMER models.
        
        Args:
            model_id: The model ID to test (default: "longformer-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing LONGFORMER model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing LONGFORMER model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing LONGFORMER model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_longformer_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test LONGFORMER HuggingFace models")
    parser.add_argument("--model", type=str, default="longformer-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available LONGFORMER models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable LONGFORMER models:")
        for model_id, info in LONGFORMER_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested LONGFORMER model: {args.model}")
