...
DEBERTA_MODELS_REGISTRY = {
    "deberta-base-uncased": {
        "full_name": "DEBERTA Base Uncased",
        "architecture": "encoder-only",
        "description": "DEBERTA Base model with uncased vocabulary",
        "deberta": "deberta",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
    ...
    "deberta-large-uncased": {
        "full_name": "DEBERTA Large Uncased",
        "architecture": "encoder-only",
        "description": "DEBERTA Large model with uncased vocabulary",
        "deberta": "deberta",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "deberta-base-cased": {
        "full_name": "DEBERTA Base Cased",
        "architecture": "encoder-only",
        "description": "DEBERTA Base model with cased vocabulary",
        "deberta": "deberta",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="deberta-base-uncased", device=None):
        """Initialize the test class for DEBERTA models.
        
        Args:
            model_id: The model ID to test (default: "deberta-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing DEBERTA model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing DEBERTA model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing DEBERTA model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_deberta_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test DEBERTA HuggingFace models")
    parser.add_argument("--model", type=str, default="deberta-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available DEBERTA models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable DEBERTA models:")
        for model_id, info in DEBERTA_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested DEBERTA model: {args.model}")
