...
"""
Standardized test for the Blip model type from HuggingFace Transformers.
...
BLIP_MODELS_REGISTRY = {
    "blip-base": {
        ...
        "description": "Blip model for fill-mask",
        "model_type": "blip",
        "parameters": "110M",
...
class TestBlipModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "blip-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "blip-base"
        
        # Verify model exists in registry
        if self.model_id not in BLIP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = BLIP_MODELS_REGISTRY["blip-base"]
        else:
            self.model_config = BLIP_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "BlipModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_blip_blip_2025-03-22T23:36:32.079297.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Blip HuggingFace models")
    parser.add_argument("--model", type=str, default="blip-base", help="Model ID to test")
    ...
    blip_tester = TestBlipModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = blip_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {blip_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{blip_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{blip_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{blip_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{blip_tester.preferred_device}"]
