...
"""
Standardized test for the Distilbert model type from HuggingFace Transformers.
...
DISTILBERT_MODELS_REGISTRY = {
    "distilbert-base": {
        "full_name": "Distilbert Base",
        "architecture": "encoder-only",
        "description": "Distilbert model for fill-mask",
...
class TestDistilbertModels:
    """Test class for Distilbert models."""
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "distilbert-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "distilbert-base"
        
        # Verify model exists in registry
        if self.model_id not in DISTILBERT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = DISTILBERT_MODELS_REGISTRY["distilbert-base"]
        else:
            self.model_config = DISTILBERT_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "DistilbertModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_distilbert_distilbert_2025-03-22T23:36:31.796132.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="distilbert-base", help="Model ID to test")
    ...
    distilbert_tester = TestDistilbertModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = distilbert_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {distilbert_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{distilbert_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{distilbert_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{distilbert_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{distilbert_tester.preferred_device}"]
