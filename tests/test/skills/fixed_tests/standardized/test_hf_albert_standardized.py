...
"""
Standardized test for the Albert model type from HuggingFace Transformers.
...
ALBERT_MODELS_REGISTRY = {
    "albert-base": {
        "full_name": "Albert Base",
        "architecture": "encoder-only",
        "description": "Albert model for fill-mask",
        "model_type": "albert",
        "parameters": "110M",
...
class TestAlbertModels:
    """Test class for Albert models."""
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "albert-base")
            device: Device to run on (default: auto-detect)
        """
        self.model_id = model_id or "albert-base"
        
        # Verify model exists in registry
        if self.model_id not in ALBERT_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_config = ALBERT_MODELS_REGISTRY["albert-base"]
        else:
            self.model_config = ALBERT_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "fill-mask")
        self.class_name = "AlbertModel"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_albert_albert_2025-03-22T23:36:31.758729.json"
...
def main():
    ...
    parser.add_argument("--model", type=str, default="albert-base", help="Model ID to test")
    ...
    albert_tester = TestAlbertModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = albert_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {albert_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{albert_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{albert_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{albert_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{albert_tester.preferred_device}"]
