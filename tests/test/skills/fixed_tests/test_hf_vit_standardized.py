...
"""
Standardized test for the Vit model type from HuggingFace Transformers.
...
VIT_MODELS_REGISTRY = {
    "google/vit-base-patch16-224": {
        "full_name": "Vit Base",
        "architecture": "vision",
        "description": "Vit model for image-classification",
        "model_type": "vit",
        "parameters": "110M",
...
class TestVitModels:
    ...
    def __init__(self, model_id=None, device=None):
        ...
        Args:
            model_id: Model identifier (default: "google/vit-base-patch16-224")
        ...
        if self.model_id not in VIT_MODELS_REGISTRY:
            ...
            self.model_config = VIT_MODELS_REGISTRY["google/vit-base-patch16-224"]
        else:
            self.model_config = VIT_MODELS_REGISTRY[self.model_id]
        
        # Define model parameters
        self.task = self.model_config.get("default_task", "image-classification")
        self.class_name = "ViTForImageClassification"
        self.architecture = self.model_config.get("architecture", "vision")
        
        # Test inputs - model type specific
        self.test_text = "image.jpg"
...
def save_results(model_id, results, model_family, output_dir="collected_results"):
    ...
    filename = f"hf_vit_vit_2025-03-22T23:33:53.876437.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Vit HuggingFace models")
    ...
    vit_tester = TestVitModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = vit_tester.run_tests(all_hardware=args.all_hardware)
    ...
    if success:
        ...
        print(f"  - Device: {vit_tester.preferred_device}")
        
        # Print pipeline performance stats if available
        if pipeline_success and f"pipeline_{vit_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"pipeline_{vit_tester.preferred_device}"]
            print(f"  - Pipeline: {stats.get('avg_time', 0):.4f}s avg inference")
            
        # Print from_pretrained performance stats if available
        if pretrained_success and f"from_pretrained_{vit_tester.preferred_device}" in results["performance"]:
            stats = results["performance"][f"from_pretrained_{vit_tester.preferred_device}"]
