...
PROPHETNET_MODELS_REGISTRY = {
    "prophetnet-base-uncased": {
        "full_name": "PROPHETNET Base Uncased",
        "architecture": "encoder-only",
        "description": "PROPHETNET Base model with uncased vocabulary",
    ...
    "prophetnet-large-uncased": {
        "full_name": "PROPHETNET Large Uncased",
        "architecture": "encoder-only",
        "description": "PROPHETNET Large model with uncased vocabulary",
        "prophetnet": "prophetnet",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "prophetnet-base-cased": {
        "full_name": "PROPHETNET Base Cased",
        "architecture": "encoder-only",
        "description": "PROPHETNET Base model with cased vocabulary",
        "prophetnet": "prophetnet",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestModelTypeModels:
    def __init__(self, model_id="prophetnet-base-uncased", device=None):
        """Initialize the test class for PROPHETNET models.
        
        Args:
            model_id: The model ID to test (default: "prophetnet-base-uncased")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing PROPHETNET model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing PROPHETNET model {self.model_id} with from_pretrained API on {self.device}")
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing PROPHETNET model {self.model_id} with OpenVINO")
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_prophetnet_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test PROPHETNET HuggingFace models")
    parser.add_argument("--model", type=str, default="prophetnet-base-uncased", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available PROPHETNET models")

    args = parser.parse_args()

    # List models if requested
    if args.list_models:
        print(f"\nAvailable PROPHETNET models:")
        for model_id, info in PROPHETNET_MODELS_REGISTRY.items():
    ...
    print(f"\nSuccessfully tested PROPHETNET model: {args.model}")
