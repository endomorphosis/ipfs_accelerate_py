...
class TestBloomModels:
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            model_class = None
            if self.class_name == "BloomLMHeadModel":
                model_class = transformers.BloomLMHeadModel
            else:
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_bloom_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    ...
    return list(BLOOM_MODELS_REGISTRY.keys())
...
def test_all_models(output_dir="collected_results", all_hardware=False):
    ...
    for model_id in models:
        ...
        tester = TestBloomModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_bloom_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    if args.list_models:
        ...
        for model in models:
            info = BLOOM_MODELS_REGISTRY[model]
