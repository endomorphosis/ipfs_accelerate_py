...
# Main Data2vecText model registry
DATA2DATA2DATA2VEC_TEXT_MODELS_REGISTRY = {
    "data2vec-text": {
        "full_name": "Data2vecText Base Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecText Base model with uncased vocabulary",
    ...
    "data2vec_text-large-uncased": {
        "full_name": "Data2vecText Large Uncased",
        "architecture": "encoder-only",
        "description": "Data2vecText Large model with uncased vocabulary",
        "model_type": "data2vec_text",
        "parameters": "336M",
        "context_length": 512,
        "embedding_dim": 1024,
    ...
    "data2vec_text-base-cased": {
        "full_name": "Data2vecText Base Cased",
        "architecture": "encoder-only",
        "description": "Data2vecText Base model with cased vocabulary",
        "model_type": "data2vec_text",
        "parameters": "110M",
        "context_length": 512,
        "embedding_dim": 768,
...
class TestData2vec_textModels:
    def __init__(self, model_id="data2vec-text", device=None):
        """Initialize the test class for Data2vecText models.
        
        Args:
            model_id: The model ID to test (default: "data2vec-text")
    ...
    def test_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecText model {self.model_id} with pipeline API on {self.device}")
    ...
    def test_from_pretrained(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecText model {self.model_id} with from_pretrained API on {self.device}")
            
            # Record start time
            start_time = time.time()
            
            # Load the model and tokenizer
            model = transformers.Data2vecTextForMaskedLM.from_pretrained(self.model_id)
    ...
    def test_openvino(self):
        ...
        try:
            ...
            logger.info(f"Testing Data2vecText model {self.model_id} with OpenVINO")
            ...
            if not os.path.exists(model_path):
                ...
                pt_model = transformers.Data2vecTextForMaskedLM.from_pretrained(self.model_id)
...
def save_results(results, model_id=None, output_dir="collected_results"):
    ...
    try:
        ...
        filename = f"hf_data2vec_text_{model_id_safe}_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Data2vecText HuggingFace models")
    parser.add_argument("--model", type=str, default="data2vec-text", help="Model ID to test")
    parser.add_argument("--device", type=str, help="Device to run tests on (cuda, cpu, mps)")
    parser.add_argument("--all-hardware", action="store_true", help="Test on all available hardware")
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--list-models", action="store_true", help="List available Data2vecText models")
    
    args = parser.parse_args()
    
    # List models if requested
    if args.list_models:
        print(f"\nAvailable Data2vecText models:")
        for model_id, info in DATA2DATA2DATA2VEC_TEXT_MODELS_REGISTRY.items():
    ...
    data2vec_text_tester = TestData2vec_textModels(model_id=args.model, device=args.device)
    
    # Run the tests
    results = data2vec_text_tester.run_tests(all_hardware=args.all_hardware)
    ...
    print(f"\nModel: {args.model}")
    print(f"Device: {data2vec_text_tester.device}")
    ...
    print(f"\nSuccessfully tested Data2vecText model: {args.model}")
