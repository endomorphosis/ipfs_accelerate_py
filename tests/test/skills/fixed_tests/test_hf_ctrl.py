...
CTRL_MODELS_REGISTRY = {
    "ctrl": {
        "description": "CTRL small model",
        "class": "CTRLLMHeadModel",
    },
    "ctrl-medium": {
        "description": "CTRL medium model",
        "class": "CTRLLMHeadModel",
    },
    "distilctrl": {
        "description": "DistilCTRL model",
        "class": "CTRLLMHeadModel",
    }
}

class TestCtrlModels:
    """Base test class for all CTRL-family models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class for a specific model or default."""
        self.model_id = model_id or "ctrl"
        
        # Verify model exists in registry
        if self.model_id not in CTRL_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default configuration")
            self.model_info = CTRL_MODELS_REGISTRY["ctrl"]
        else:
            self.model_info = CTRL_MODELS_REGISTRY[self.model_id]
    ...
    def test_pipeline(self, device="auto"):
        ...
        try:
            ...
            if tokenizer.pad_token is None:
                ...
                logger.info("Set pad_token to eos_token for CTRL tokenizer")
    ...
    def test_from_pretrained(self, device="auto"):
        ...
        try:
            ...
            # Fix for CTRL padding token issue
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info("Set pad_token to eos_token for CTRL tokenizer")
            ...
            model_class = None
            if self.class_name == "CTRLLMHeadModel":
...
def save_results(model_id, results, output_dir="collected_results"):
    ...
    filename = f"hf_ctrl_{safe_model_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_available_models():
    """Get a list of all available CTRL models in the registry."""
    return list(CTRL_MODELS_REGISTRY.keys())
    
def test_all_models(output_dir="collected_results", all_hardware=False):
    """Test all registered CTRL models."""
    models = get_available_models()
    results = {}
    
    for model_id in models:
        logger.info(f"Testing model: {model_id}")
        tester = TestCtrlModels(model_id)
    ...
    # Save summary
    summary_path = os.path.join(output_dir, f"hf_ctrl_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test CTRL-family models")
    ...
    if args.list_models:
        models = get_available_models()
        print(f"\nAvailable CTRL-family models:")
        for model in models:
            info = CTRL_MODELS_REGISTRY[model]
