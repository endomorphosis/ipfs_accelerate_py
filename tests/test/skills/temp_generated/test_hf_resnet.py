...
RESNET_MODELS_REGISTRY = {
    "google/resnet-base-patch16-224": {
        "description": "resnet base model",
        "class": "ResnetForImageClassification",
    }
}

class TestResnetModels:
    """Test class for resnet models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "google/resnet-base-patch16-224"
        
        # Use registry information
        if self.model_id not in RESNET_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = RESNET_MODELS_REGISTRY["google/resnet-base-patch16-224"]
        else:
            self.model_info = RESNET_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test resnet models")
    ...
    # Run test
    model_id = args.model or "google/resnet-base-patch16-224"
    tester = TestResnetModels(model_id)
