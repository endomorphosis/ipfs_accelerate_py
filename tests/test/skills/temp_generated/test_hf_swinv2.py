...
# Models registry
SWINV2_MODELS_REGISTRY = {
    "google/swinv2-base-patch16-224": {
        "description": "swinv2 base model",
        "class": "Swinv2ForImageClassification",
    }
}

class TestSwinv2Models:
    """Test class for swinv2 models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "google/swinv2-base-patch16-224"
        
        # Use registry information
        if self.model_id not in SWINV2_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = SWINV2_MODELS_REGISTRY["google/swinv2-base-patch16-224"]
        else:
            self.model_info = SWINV2_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test swinv2 models")
    ...
    # Run test
    model_id = args.model or "google/swinv2-base-patch16-224"
    tester = TestSwinv2Models(model_id)
