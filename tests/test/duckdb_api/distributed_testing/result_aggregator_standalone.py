...
- Collecting and aggregating test results from multiple workers
- Normalizing results for comparable analysis
- Categorizing and grouping results by test type, hardware, model, etc.
- Statistical aggregation (mean, median, percentiles, standard deviation, etc.)
- Cross-test correlation and comparative analysis
- Result filtering and transformation
- Anomaly detection in aggregated results
- Context-aware result interpretation

The result aggregator works closely with the performance trend analyzer for long-term
trend analysis and the database manager for efficient storage and retrieval.
"""
...
class ResultAggregatorService:
    ...
    def aggregate_results(self, 
        ...
        if not raw_results:
            ...
            return {"aggregation_level": aggregation_level, "result_type": result_type, "results": {}}
    ...
    def _add_context_metadata(self, aggregated_results: Dict[str, Any], 
        ...
        aggregated_results["context"] = {
            ...
            "time_range": [ts.isoformat() if ts else None for ts in context.get("time_range", (None, None))]
        }
