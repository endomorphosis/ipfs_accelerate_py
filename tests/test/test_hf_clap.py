...
CLAP_MODELS_REGISTRY = {
    "clap-base": {
        "description": "clap base model",
        "class": "ClapForCTC",
    }
}

class TestClapModels:
    """Test class for clap models."""
    
    def __init__(self, model_id=None):
        """Initialize the test class."""
        self.model_id = model_id or "clap-base"
        
        # Use registry information
        if self.model_id not in CLAP_MODELS_REGISTRY:
            logger.warning(f"Model {self.model_id} not in registry, using default")
            self.model_info = CLAP_MODELS_REGISTRY["clap-base"]
        else:
            self.model_info = CLAP_MODELS_REGISTRY[self.model_id]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test clap models")
    ...
    # Run test
    model_id = args.model or "clap-base"
    tester = TestClapModels(model_id)
