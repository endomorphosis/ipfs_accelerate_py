...
// Define workgroup size based on Chrome's optimal parameters
@compute @workgroup_size(16, 16, 1)
fn main(@builtin(global_invocation_id) global_id: vec3<u32>,
        @builtin(workgroup_id) workgroup_id: vec3<u32>,
        @builtin(local_invocation_id) local_id: vec3<u32>) {
  
  // Declare shared memory tiles - Chrome benefits from square tiles
  ...
  for (var t: u32 = 0u; t < num_tiles; t = t + 1u) {
    ...
    // Compute dot product for this tile with loop unrolling for better performance
    // Chrome's compiler benefits from unrolled loops
    var k = 0u;
    for (; k + 4u <= 16u && tile_start + k < params.K; k = k + 4u) {
      // Process 4 values at once - faster in Chrome
