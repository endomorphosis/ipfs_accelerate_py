...
# -*- coding: utf-8 -*-
"""
iOS Test Harness CI Benchmark Runner

This script runs automated benchmarks for the iOS Test Harness as part of the CI/CD pipeline.
It automates the execution of benchmarks on iOS devices or simulators, stores results in a
...
if platform.system() != "Darwin":
    logger.error("iOS CI benchmarks can only be run on macOS")
    sys.exit(1)
...
class CIBenchmarkRunner:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            simulator: Whether to use a simulator instead of a physical device
        ...
        if self.simulator:
            try:
                ...
                if self.device_id not in result.stdout:
                    logger.warning(f"Simulator '{self.device_id}' not found in simulator list")
                    logger.info("Available simulators:")
                    for line in result.stdout.splitlines():
                        if "(" in line and ")" in line and not "unavailable" in line.lower():
        ...
        return [
            {
                ...
                "path": "models/bert-base-uncased.mlmodel",
                "type": "coreml",
                "batch_sizes": [1, 4],
                "iterations": 50,
                "priority": "high"
            ...
            {
                "name": "mobilenet-v2",
                "path": "models/mobilenet-v2.mlmodel",
                "type": "coreml",
                "batch_sizes": [1, 4],
                "iterations": 50,
                "priority": "high"
            ...
            {
                "name": "roberta-base",
                "path": "models/roberta-base.mlmodel",
                "type": "coreml",
                "batch_sizes": [1],
                "iterations": 30,
                "priority": "medium"
            ...
            {
                "name": "whisper-tiny",
                "path": "models/whisper-tiny.mlmodel",
                "type": "coreml",
                "batch_sizes": [1],
                "iterations": 20,
                "priority": "medium"
            },
            {
                ...
                "path": "models/clip-vit-base-patch32.mlmodel",
                "type": "coreml",
                "batch_sizes": [1],
                "iterations": 20,
                "priority": "medium"
    ...
    def connect_to_device(self) -> bool:
        ...
        try:
            ...
            if connected:
                ...
                logger.info(f"iOS version: {device_info.get('ios_version', 'Unknown')}")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.simulator:
            try:
                logger.info(f"Shutting down simulator: {self.device_id}")
                subprocess.run(
