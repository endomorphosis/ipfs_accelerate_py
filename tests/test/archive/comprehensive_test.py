...
class BertTest(unittest.TestCase):
    ...
    def test_webgpu_inference(self):
        """Test bert with webgpu hardware"""
        # Skip test if hardware not available
        ...
        try:
            ...
            # Create sample input
            sample_text = "This is a test for bert model on webgpu"
            inputs = tokenizer(sample_text, return_tensors="pt")
            ...
            logger.info(f"Successfully tested bert on webgpu")
            
        except Exception as e:
            logger.error(f"Error testing bert on webgpu: {str(e)}")
            raise
