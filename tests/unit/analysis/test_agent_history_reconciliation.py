...
def test_contract_version_and_schema_are_frozen() -> None:
    ...
    assert AGENT_HISTORY_RECONCILIATION_REPORT_INTERFACE == ("AgentHistoryReconciliationReport@1")
...
def test_present_file_matching_overlay() -> None:
    report = reconcile_agent_history(
        _history(
            files=(ReferencedFile(path="src/app.py", content_id=SHA_FILE, provenance=_claim()),)
        ),
        _truth(),
    )
    record = report.records[0]
...
def test_history_only_file_deleted_from_current_tree() -> None:
    report = reconcile_agent_history(
        _history(
            files=(
                ReferencedFile(path="src/legacy.py", content_id=SHA_FILE_HIST, provenance=_claim()),
            )
        ),
        _truth(
            files=(
...
def test_present_file_relocated_in_current_overlay() -> None:
    report = reconcile_agent_history(
        _history(
            files=(
                ReferencedFile(path="src/old_name.py", content_id=SHA_FILE, provenance=_claim()),
            )
        ),
        _truth(),
    )
...
def test_missing_file() -> None:
    report = reconcile_agent_history(
        _history(
            files=(ReferencedFile(path="src/absent.py", content_id=SHA_OTHER, provenance=_claim()),)
        ),
        _truth(),
    )
