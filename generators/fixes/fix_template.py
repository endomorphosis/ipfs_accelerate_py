...
def update_bert_template():
    """Update the bert template to fix syntax errors."""
    db = TemplateDatabase()
    
    # Delete old template if it exists
    db.delete_template('bert')
    
    # Define a fixed template without the shebang line
    template = """\"\"\"
Test for {model_name} model with comprehensive hardware platform support
    ...
    def run_test_on_platform(self, platform):
        ...
        try:
            ...
            if platform == "cuda" or platform == "rocm":
                device = "cuda"
            elif platform in ["openvino", "qualcomm", "webnn", "webgpu"]:
...
"""
    ...
    # Add shebang line separately to avoid escape sequence issues
