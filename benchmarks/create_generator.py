#!/usr/bin/env python3
"""
Create a properly structured test generator with hardware support
"""
...
logger = logging.getLogger("generator_creator")
...
def generate_test_file(model_name: str, platforms: List[str], output_path: Optional[str] = None) -> str:
    ...
    def test_{platform}_inference(self):
        ...
        if if_name_pos > 0:
            # Find position of last method
            last_def_pos = test_content.rfind('    def ', 0, if_name_pos)
            if last_def_pos > 0:
                # Find end of the last method
                last_method_end = test_content.find('\n\n', last_def_pos)
                if last_method_end > 0:
                    test_content = test_content[:last_method_end] + platform_test + test_content[last_method_end:]
...
def main():
    """Main function for creating test generator."""
    parser = argparse.ArgumentParser(description="Create a new test generator with hardware support")
    parser.add_argument("--output", "-o", dest="output", default="new_test_generator.py", 
                        help="Output file path for the new generator")
    args = parser.parse_args()
    ...
    print(f"Created new test generator at {output_path}")
