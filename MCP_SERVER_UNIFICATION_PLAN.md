...
72. Extended focused `logic_tools` parity coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni161_logic_tools.py` and added isolated dispatch compatibility coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni303_logic_dispatch_compat.py` (`15 passed`).
...
120. Revalidated focused `rate_limiting_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni292_rate_limiting_tools_dispatch_compat.py` and the unified bootstrap slice for alias-category `rate_limiting_tools` in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 188 deselected`).
121. Revalidated canonical `rate_limiting` contradictory failed-delegate normalization via `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni293_rate_limiting_dispatch_compat.py` and added the missing unified bootstrap dispatch regression for `check_rate_limit` and `manage_rate_limits` so canonical rate-limit flows now stay covered through `tools_dispatch` (`1 passed`; bootstrap slice `1 passed, 197 deselected`).
122. Revalidated `cache_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/cache_tools/native_cache_tools.py` and added the missing unified bootstrap dispatch regression for representative cache read, stats, optimization, and embedding lookup flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
123. Revalidated focused `cache_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni295_cache_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 189 deselected`).
124. Revalidated `analysis_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/analysis_tools/native_analysis_tools.py` and added the missing unified bootstrap dispatch regression for representative clustering, quality-assessment, and dimensionality-reduction flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
125. Revalidated focused `analysis_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni276_analysis_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 190 deselected`).
126. Revalidated `software_engineering_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/software_engineering_tools/native_software_engineering_tools.py` and added the missing unified bootstrap dispatch regression for representative repository search and workflow-log parsing flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
127. Revalidated focused `software_engineering_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni281_software_engineering_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 193 deselected`).
128. Revalidated `development_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/development_tools/native_development_tools.py` and added the missing unified bootstrap dispatch regression for representative code-search, documentation, test-runner, and VSCode CLI flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
129. Revalidated focused `development_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni298_development_tools_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 191 deselected`).
130. Revalidated `web_archive_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/web_archive_tools/native_web_archive_tools.py` and added the missing unified bootstrap dispatch regression for representative archive, Common Crawl, GitHub-provider, and unified-search/fetch flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
131. Revalidated focused `web_archive_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni299_web_archive_tools_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 192 deselected`).
132. Revalidated native `provenance_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/provenance_tools/native_provenance_tools.py` and confirmed the existing isolated dispatch regression in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni259_provenance_dispatch_compat.py` still preserves deterministic `error` status inference for `record_provenance` through canonical `tools_dispatch`.
133. Revalidated focused native `provenance_tools` direct, import-compat, and isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni115_provenance_tools.py`, `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni178_provenance_tools.py`, `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni219_provenance_import_compat.py`, and `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni259_provenance_dispatch_compat.py` (`21 passed`).
134. Revalidated `bespoke_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/bespoke_tools/native_bespoke_tools.py` and added the missing unified bootstrap dispatch regression for representative workflow execution so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
135. Revalidated focused `bespoke_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni279_bespoke_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 194 deselected`).
136. Revalidated `lizardperson_argparse_programs` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/lizardperson_argparse_programs/native_lizardperson_argparse_programs.py` and added the missing unified bootstrap dispatch regression for `municipal_bluebook_validator_invoke` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
137. Revalidated focused `lizardperson_argparse_programs` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni146_lizardperson_argparse_programs.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
138. Revalidated `geospatial_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/geospatial_tools/native_geospatial_tools.py` and added the missing unified bootstrap dispatch regression for representative geographic extraction, spatial-temporal mapping, and geographic-context flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
139. Revalidated focused `geospatial_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni277_geospatial_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 195 deselected`).
140. Revalidated `logic_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/logic_tools/native_logic_tools.py` and added the missing unified bootstrap dispatch regression for representative TDFOL parse/prove and CEC prove flows so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
141. Revalidated focused `logic_tools` isolated dispatch coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni303_logic_dispatch_compat.py` and the new unified bootstrap slice in `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`1 passed`; bootstrap slice `1 passed, 196 deselected`).
142. Revalidated `lizardpersons_function_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/lizardpersons_function_tools/native_lizardpersons_function_tools.py` and added the missing unified bootstrap dispatch regression for `get_current_time` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
143. Revalidated focused `lizardpersons_function_tools` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni144_lizardpersons_function_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
144. Revalidated `mcplusplus` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/mcplusplus/native_mcplusplus_tools.py` and added the missing unified bootstrap dispatch regression for `mcplusplus_taskqueue_get_status` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
145. Revalidated focused `mcplusplus` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni147_mcplusplus_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
146. Revalidated `dataset_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/dataset_tools/native_dataset_tools.py` and added the missing unified bootstrap dispatch regression for `load_dataset` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
147. Revalidated focused `dataset_tools` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni157_dataset_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
148. Revalidated `legacy_mcp_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/legacy_mcp_tools/native_legacy_mcp_tools.py` and added the missing unified bootstrap dispatch regression for `legacy_tools_inventory` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
149. Revalidated focused `legacy_mcp_tools` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni145_legacy_mcp_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
150. Revalidated `provenance_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/provenance_tools/native_provenance_tools.py` and added the missing unified bootstrap dispatch regression for `record_provenance` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
151. Revalidated focused `provenance_tools` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni115_provenance_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
152. Revalidated `sparse_embedding_tools` contradictory failed-delegate normalization in `ipfs_accelerate_py/mcp_server/tools/sparse_embedding_tools/native_sparse_embedding_tools.py` and added the missing unified bootstrap dispatch regression for `generate_sparse_embedding` so contradictory failed payloads now stay covered through canonical `tools_dispatch`.
153. Revalidated focused `sparse_embedding_tools` direct and unified bootstrap coverage in `ipfs_accelerate_py/mcp/tests/test_mcp_server_uni124_sparse_embedding_tools.py` and `ipfs_accelerate_py/mcp/tests/test_mcp_server_unified_bootstrap.py` (`3 passed`).
154. Revalidated `SPEC-201` profile-negotiation hardening with the current unified bootstrap module and process-level transport lane, confirming additive profile metadata and fallback semantics still hold across bootstrap/context snapshots and mounted standalone entrypoints (`204 passed`; `10 passed`).
155. Revalidated `SPEC-202` MCP-IDL stability with the focused descriptor canonicalization and repository-API corpus in `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_idl.py` and synchronized the `MCPP-016` / `MCPP-017` checklist and matrix evidence (`15 passed`).
156. Revalidated `SPEC-203` artifact durability and replay integrity with `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_artifacts.py`, confirming deterministic durable replay, chain reconstruction, and artifact-emission policy behavior still hold in the focused lane (`16 passed`).
157. Revalidated `SPEC-204` UCAN verification vectors with `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_ucan.py` and synchronized the `MCPP-018` / `MCPP-019` checklist and matrix evidence for current execution-time delegation coverage (`24 passed`).
158. Revalidated `SPEC-205` temporal policy lifecycle hardening with `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_policy.py`, confirming deterministic obligation lifecycle and decision persistence behavior still hold in the focused lane (`7 passed`).
159. Revalidated `SPEC-206` Event DAG scale/conflict coverage with `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_event_dag.py` and synchronized the `MCPP-020` / `MCPP-021` checklist and matrix evidence for current replay, rollback, and snapshot-compat behavior (`13 passed`).
160. Revalidated `SPEC-207` risk frontier hardening with `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_risk_scheduler.py`, confirming deterministic frontier execution, retry ordering, and optional consensus behavior still hold in the focused lane (`6 passed`).
161. Revalidated `SPEC-208` mcp+p2p interop and abuse-resistance coverage with the focused framing, handler, and client transport suites, and synchronized the `MCPP-022` / `MCPP-023` checklist and matrix evidence (`11 passed`; `19 passed`; `3 passed`).
162. Closed the remaining post-spec documentation drift by advancing the high-level monitoring and MCP++ convergence summaries in `mcpplusplus/SPEC_GAP_MATRIX.md` to the current `2026-03-09` bootstrap-maintenance evidence, matching the already-updated checklist and detailed matrix rows.
163. Closed another narrative-only matrix drift slice by advancing the finance-data, investigation, CLI, lizardpersons-function, and lizardperson-argparse convergence summaries in `mcpplusplus/SPEC_GAP_MATRIX.md` to the current `2026-03-09` canonical bootstrap-maintenance evidence, matching the already-current checklist bullets and detailed matrix rows.
164. Closed the remaining peer-service fallback drift in `ipfs_accelerate_py/mcp_server/mcplusplus/peer_discovery.py` so an explicit canonical `PeerServiceBundle` with `peer_registry=None` no longer silently reconstructs a registry outside the bundle boundary, and added a focused regression in `ipfs_accelerate_py/mcp/tests/test_mcp_server_mcplusplus_peer_primitives.py` (`8 passed`).
165. Converged unified peer probing in `ipfs_accelerate_py/mcp_server/server.py` so `tools_dispatch` now prefers the canonical `peer_discovery_factory` surface before falling back to `peer_registry_factory`, and added focused unified-bootstrap regressions proving both the discovery-first and registry-fallback probe paths plus bootstrap-address probing remain green (`3 passed`).
166. Revalidated the canonical HTTP facade by adding lazy import-compatible `settings` / `app` exports plus verbose passthrough in `ipfs_accelerate_py/mcp_server/fastapi_service.py`, then synchronized the checklist and matrix HTTP evidence with focused facade coverage (`5 passed`) and broader process/standalone/subprocess HTTP transport coverage (`21 passed`).
167. Closed the remaining canonical peer-probe fallback gap in `ipfs_accelerate_py/mcp_server/server.py` so `tools_dispatch` now falls back from an unavailable `peer_discovery_factory` to `peer_registry_factory`, and from an unavailable `peer_bootstrap_factory` to registry-provided bootstrap nodes where possible, with focused unified-bootstrap regressions covering both degraded-service paths (`5 passed`).
168. Converged unified bootstrap factory consumption in `ipfs_accelerate_py/mcp_server/server.py` so `_unified_risk_scheduler`, `_unified_workflow_engine`, and `_unified_workflow_dag_executor` now prefer their canonical `risk_scheduler_factory`, `workflow_engine_factory`, and `workflow_dag_executor_factory` surfaces before falling back to direct construction, with focused unified-bootstrap regressions proving factory-backed attachment stays green (`3 passed`).
169. Removed another shim-owned singleton boundary by moving `ipfs_accelerate_py/mcp_server/mcplusplus/workflow_scheduler.py` to resolve `ipfs_accelerate_py.p2p_workflow_scheduler` directly and keep its own canonical scheduler create/reset lifecycle, with focused wrapper coverage proving canonical lazy creation and reset behavior remain green (`7 passed`).
170. Converged Trio bridge resolution behind canonical compatibility helpers by routing `ipfs_accelerate_py/mcp_server/runtime_router.py` through `_resolve_trio_bridge_runner()` in `ipfs_accelerate_py/mcp_server/compatibility.py`, while preserving both the fail-closed `trio_bridge_required=True` contract and the backward-compatible standard-runtime fallback when the bridge is unavailable (`6 passed`).
171. Removed two more shim-first peer boundaries by making `ipfs_accelerate_py/mcp_server/mcplusplus/peer_registry.py` and `peer_bootstrap.py` prefer the canonical `ipfs_accelerate_py.github_cli` implementations before falling back to `mcplusplus_module`, with focused peer primitive coverage proving the canonical preference and wrapper behavior remain green (`9 passed`).
172. Converged the shared P2P registrar resolver in `ipfs_accelerate_py/mcp_server/compatibility.py` so canonical runtime and shim entrypoints now prefer the canonical `mcp_server` P2P/taskqueue and P2P-workflow registrars before falling back to `mcplusplus_module`, with focused shim adapter and Trio server coverage validating both canonical-first registration and fallback behavior (`45 passed`).
173. Removed the stale shim storage-wrapper fallback from `ipfs_accelerate_py/mcp_server/compatibility.py` after confirming `ipfs_accelerate_py.mcplusplus_module.common.storage_wrapper` no longer exists as a real module, leaving the canonical storage wrapper plus `test.common` fixture fallback with focused adapter coverage proving primary, fallback, and unavailable paths remain green (`5 passed`).
174. Removed another canonical-to-legacy runtime hop by moving Trio execution and remote-queue helpers into `ipfs_accelerate_py/mcp_server/tools/p2p/native_p2p_tools.py`, so the canonical P2P tool path no longer imports `_run_in_trio` / `_remote_queue` from `ipfs_accelerate_py.mcp.tools.p2p_taskqueue`, with focused direct and dispatch-compat coverage remaining green (`7 passed`).
175. Moved workflow-manager singleton ownership into the canonical `ipfs_accelerate_py/workflow_manager.py` accessor and repointed `ipfs_accelerate_py/mcp_server/tools/workflow/native_workflow_tools.py` plus the legacy `ipfs_accelerate_py.mcp.tools.workflows` wrapper to reuse that accessor, with focused native workflow coverage proving the canonical resolver path remains green (`7 passed`).
176. Removed the remaining Wave A category-loader dependency on legacy `ipfs_accelerate_py.mcp.tools` registrations by teaching `LegacyCollectorMCP` to capture hierarchical-style canonical registrations and switching `ipfs_accelerate_py/mcp_server/wave_a_loaders.py` to remap canonical `ipfs_tools`, `workflow_tools`, and `p2p_tools` category registrars into the Wave A `ipfs`, `workflow`, and `p2p` categories, with focused unified-bootstrap coverage remaining green (`3 passed`).
177. Moved standalone FastAPI app assembly and uvicorn runner ownership into `ipfs_accelerate_py/mcp_server/fastapi_service.py`, so the canonical FastAPI facade no longer routes through `ipfs_accelerate_py.mcp.integration`, with focused facade coverage remaining green (`6 passed`).
178. Removed the remaining canonical standalone-server dependency on `ipfs_accelerate_py.mcp.standalone` by making `ipfs_accelerate_py/mcp_server/standalone_server.py` build and run the canonical server directly, with focused unit and subprocess contract coverage remaining green (`4 passed`).
179. Removed the canonical `create_server()` dependency on the legacy `ipfs_accelerate_py.mcp.server.create_mcp_server` facade by letting `ipfs_accelerate_py/mcp_server/server.py` construct the underlying `MCPServerWrapper` directly while still synchronizing the legacy global MCP-like instance, and updated focused unified-bootstrap regressions plus the full UNI-007 cutover/rollback suite to validate both the new constructor seam and unchanged legacy facade telemetry behavior (`12 passed`, `10 passed`).
180. Removed the remaining canonical CLI-entrypoint bridges by repointing both `ipfs_accelerate_py/mcp_server/__main__.py` and `ipfs_accelerate_py/mcp_server/server.py:main()` to the canonical `standalone_server.main()` path instead of the legacy `ipfs_accelerate_py.mcp` CLI facades, with subprocess contract coverage proving both canonical entrypoints and adjacent standalone startup paths remain green (`4 passed`).
181. Removed the last direct `ipfs_accelerate_py.mcp` import from canonical `mcp_server` by making `ipfs_accelerate_py/mcp_server/registration_adapter.py` collect compatibility inventory from canonical native registrars instead of `mcp.tools.register_all_tools`, preserving captured hierarchical category metadata for canonical registrars and validating the adapter plus adjacent Wave A loader behavior with focused unified-bootstrap coverage (`5 passed`).
182. Synchronized the conformance evidence after the canonical import-boundary cleanup by revalidating the post-cutover regression bundle across UNI-007 rollback controls, canonical FastAPI facade/service behavior, standalone server startup, subprocess transport contracts, and unified bootstrap coverage (`241 passed, 1 warning`), and by recording that `ipfs_accelerate_py/mcp_server` no longer contains direct `ipfs_accelerate_py.mcp` imports.
