...
class GitHubCLIIntegration(BaseCLIWrapper):
        ...
        **kwargs,
        ...
        if HAVE_DATASETS_INTEGRATION and is_datasets_available():
            try:
                ...
                logger.info(
                    "GitHub CLI integration using datasets integration for PR/issue tracking"
                )
        ...
        super().__init__(cli_path=gh_path, cache=cache, enable_cache=enable_cache, **kwargs)

    def get_tool_name(self) -> str:
        return "GitHub CLI"

    def list_repos(self, owner: Optional[str] = None, limit: int = 30, **kwargs) -> Dict[str, Any]:
        """
        List repositories.
        ...
        return self._run_command_with_retry(args, "repo_list", owner=owner or "self", limit=limit)
    ...
    def view_repo(self, repo: str, **kwargs) -> Dict[str, Any]:
        ...
        return self._run_command_with_retry(args, "repo_view", repo=repo)

    def list_prs(self, repo: str, state: str = "open", limit: int = 30, **kwargs) -> Dict[str, Any]:
        """
        List pull requests.
        ...
        return self._run_command_with_retry(args, "pr_list", repo=repo, state=state, limit=limit)
    ...
    def view_pr(self, repo: str, pr_number: int, **kwargs) -> Dict[str, Any]:
        ...
        return self._run_command_with_retry(args, "pr_view", repo=repo, pr_number=pr_number)

    def list_issues(
        self, repo: str, state: str = "open", limit: int = 30, **kwargs
    ) -> Dict[str, Any]:
        """
        List issues.
        ...
        return self._run_command_with_retry(args, "issue_list", repo=repo, state=state, limit=limit)

    def list_workflow_runs(self, repo: str, limit: int = 20, **kwargs) -> Dict[str, Any]:
        """
        List workflow runs.
        ...
        return self._run_command_with_retry(args, "run_list", repo=repo, limit=limit)
