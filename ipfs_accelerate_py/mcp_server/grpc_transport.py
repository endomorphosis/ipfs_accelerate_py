"""Canonical compatibility facade for source gRPC transport surface."""
...
def _load_source_exports() -> Dict[str, Any]:
    """Load source grpc-transport symbols for compatibility delegation."""
    try:
        from ipfs_datasets_py.ipfs_datasets_py.mcp_server import grpc_transport as source_module  # type: ignore
    except Exception as exc:  # pragma: no cover - dependency-missing fallback
        logger.warning("Source grpc_transport import unavailable: %s", exc)
        return {}
...
if _EXPORTS:
    ...
    def __getattr__(name: str) -> Any:  # pragma: no cover - fallback path only
        raise GrpcTransportUnavailable(f"grpc_transport symbol unavailable: {name}")

    __all__ = ["GrpcTransportUnavailable"]
