...
def default_compute_tiers(amounts: Mapping[str, str]) -> dict[str, ComputeTier]:
    ...
    return {
        ...
        "gpu-reservation": ComputeTier(
            amounts["gpu-reservation"],
            units=60,
...
class CatalogSigner:
    ...
    def sign(self, document: Mapping[str, Any]) -> dict[str, Any]:
        ...
        unsigned.update(
            {"signatureAlg": "Ed25519", "publicKey": base64.b64encode(public).decode("ascii")}
        )
        return {
            **unsigned,
...
class AcceleratorExecutionStore:
    ...
    def begin(
        ...
        *,
        ...
        with self._lock, self._connect() as db:
            ...
            row = db.execute(
                "SELECT * FROM executions WHERE idempotency_key=?", (context.idempotency_key,)
            ).fetchone()
            if row:
                db.rollback()
                ...
                if record["requestCid"] != context.request_cid or record["operation"] != operation:
                    raise AcceleratorPaymentError(
                        "H_REQUEST_MISMATCH", "execution key is bound to different work"
                    )
                return record, True
    ...
    def get(self, resource_id: str) -> dict[str, Any] | None:
        with self._connect() as db:
            row = db.execute(
                "SELECT * FROM executions WHERE resource_id=?", (resource_id,)
            ).fetchone()
    ...
    def finish(
        self, resource_id: str, outcome: str, result: Any, *, units: int, allow_partial: bool
    ) -> dict[str, Any]:
        ...
        if not record:
            raise AcceleratorPaymentError(
                "H_RECONCILIATION_REQUIRED", "execution record is missing", retryable=True
            )
        usage = {
            "schema": "mcp++/profile-h/compute-usage@1.0",
            "createdAt": self.clock_ms(),
        ...
        if not changed:
            ...
            if not latest or latest["status"] != outcome:
                raise AcceleratorPaymentError(
                    "H_RECONCILIATION_REQUIRED", "execution has a conflicting terminal outcome"
                )
    ...
    def cancel(self, resource_id: str, *, reason: str = "caller-requested") -> dict[str, Any]:
        ...
        if record["status"] in self.TERMINAL:
            return record
        cancellation = {
            "schema": "mcp++/profile-h/compute-cancellation@1.0",
            "createdAt": self.clock_ms(),
            "parents": [record["entitlementCid"]],
            "resourceId": resource_id,
            "entitlementCid": record["entitlementCid"],
            "reason": reason[:128],
        ...
        with self._connect() as db:
            db.execute(
                "UPDATE executions SET status='cancelled',usage_cid=?,detail=?,updated_at=? WHERE resource_id=? AND status NOT IN ('succeeded','partial','failed','cancelled','expired','released')",
    ...
    def _transition(self, resource_id: str, status: str, *, allowed: set[str]) -> None:
        ...
        with self._connect() as db:
            changed = db.execute(
                f"UPDATE executions SET status=?,updated_at=? WHERE resource_id=? AND status IN ({placeholders})",
    ...
    def reconcile(self) -> list[dict[str, Any]]:
        with self._connect() as db:
            rows = db.execute(
                "SELECT * FROM executions WHERE status IN ('starting','running') ORDER BY created_at"
            ).fetchall()
...
class PaidAcceleratorService:
    ...
    def __init__(
        ...
        *,
        ...
        self.executions = AcceleratorExecutionStore(
            self.state_dir / "executions.sqlite3", self.artifacts, self.clock_ms
        )
    ...
    def _build_catalog(self) -> dict[str, Any]:
        ...
        document = {
            ...
            "pricingModel": "fixed-compute-tiers",
    ...
    def _scope(
    ...
    ) -> tuple[str, ComputeTier, str, str]:
        tier_name = str(params.get("tier", ""))
        tier = self.config.tiers.get(tier_name)
        if tier is None or operation not in tier.operations:
            raise AcceleratorPaymentError(
                "H_PAYMENT_POLICY_DENIED", "operation is unavailable in the selected tier"
            )
        model, hardware = (
            str(params.get("model", "unspecified")),
            str(params.get("hardware", tier.hardware[0])),
        )
        if not _NAME.fullmatch(model) or not _NAME.fullmatch(hardware):
            raise AcceleratorPaymentError(
                "H_REQUEST_MISMATCH", "invalid model or hardware identifier"
            )
        ...
        if (
            ("*" not in tier.models and model not in tier.models)
            or ("*" not in allowed_models and model not in allowed_models)
            or hardware not in tier.hardware
            or hardware not in allowed_hardware
        ):
            raise AcceleratorPaymentError(
                "H_PAYMENT_POLICY_DENIED", "model or hardware policy denied"
            )
        ...
        if (
            ...
            or requested_units != tier.units
        ):
            raise AcceleratorPaymentError(
                "H_PAYMENT_POLICY_DENIED", "only the fixed tier compute quantity is accepted"
            )
        if (
            ...
            raise AcceleratorPaymentError(
                "H_PAYMENT_POLICY_DENIED", "requested duration exceeds the fixed tier"
            )
        ...
        *,
        ...
        async def start_after_payment() -> Any:
            ...
            if (
                ...
                or ledger_entry.state not in {"settled", "executing"}
                or not ledger_entry.settlement_cid
            ):
                raise AcceleratorPaymentError(
                    "H_RECONCILIATION_REQUIRED",
                    "payment settlement fence is absent",
                    retryable=True,
                )
            record, existed = self.executions.begin(
                context=context,
                operation=operation,
                kind=self.KINDS[operation],
                tier_name=tier_name,
                tier=tier,
                settlement_cid=ledger_entry.settlement_cid,
            ...
            if existed:
                if record["status"] in AcceleratorExecutionStore.TERMINAL:
                    return {
                        "resourceId": record["resourceId"],
                        "jobId": record["resourceId"] if record["kind"] == "job" else None,
                        "reservationId": record["resourceId"]
                        if record["kind"] == "reservation"
                        else None,
                        "entitlementCid": record["entitlementCid"],
                        "usageRecordCid": record["usageRecordCid"],
                        "outcome": record["status"],
                        "recovered": True,
                    }
                raise AcceleratorPaymentError(
                    "H_RECONCILIATION_REQUIRED", "work may already have started", retryable=True
                )
            ...
            try:
                ...
                raise
            outcome = "succeeded"
            if isinstance(value, Mapping) and value.get("outcome") in {
                "succeeded",
                "partial",
                "failed",
            }:
                outcome = str(value["outcome"])
            completed = self.executions.finish(
                record["resourceId"],
                outcome,
                value,
                units=tier.units,
            ...
            envelope = {
                "resourceId": record["resourceId"],
                "entitlementCid": record["entitlementCid"],
                "usageRecordCid": completed["usageRecordCid"],
                "outcome": completed["status"],
            }
            if record["kind"] == "job":
                envelope["jobId"] = record["resourceId"]
            if record["kind"] == "reservation":
                envelope["reservationId"] = record["resourceId"]
            return (
                {**dict(value), **envelope}
        ...
        return await self.runtime.dispatch(
            operation_key, context, start_after_payment, payment=payment
        )
    ...
    def cancel(
        self, resource_id: str, context: RequestContext, *, reason: str = "caller-requested"
    ) -> dict[str, Any]:
        ...
        if not record or record["requestCid"] != context.request_cid:
            raise AcceleratorPaymentError(
                "H_EXECUTION_NOT_FOUND", "execution is unknown to this request"
            )
        ...
        for prefix, evidence_field in (
            ...
            if method == "GET" and path.startswith(prefix):
                ...
                return (
                    (200, {}, artifact)
                    if artifact
                    else (404, {}, {"error": "H_EVIDENCE_NOT_FOUND"})
                )
        for prefix, field in (
            ("/mcp/accelerate/jobs/", "job"),
            ("/mcp/accelerate/reservations/", "reservation"),
        ):
                    ...
                    try:
                        return (
                            ...
                            self.cancel(
                                resource_id,
                                context,
                                reason=str(params.get("reason", "caller-requested")),
                            ),
                        )
                    except AcceleratorPaymentError as error:
                        return (
                            404 if error.code == "H_EXECUTION_NOT_FOUND" else 403,
                            {},
                            {"error": error.code},
                        )
                record = self.executions.get(resource_id)
    ...
    async def reconcile(self) -> dict[str, Any]:
        return {
            "payments": await self.runtime.reconcile(),
            "executions": self.executions.reconcile(),
        }
    ...
    async def diagnostics(self) -> dict[str, Any]:
        ...
        return {
            **base,
            ...
            "fixedComputeTiers": sorted(self.config.tiers),
            "executions": self.executions.diagnostics(),
        }
...
__all__ = [
    "AcceleratePaymentConfig",
    "AcceleratePaymentError",
    "AcceleratorComputeTier",
    "AcceleratorExecutionStore",
