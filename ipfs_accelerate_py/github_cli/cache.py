...
Distributed cache sharing is provided through the canonical MCP++ TaskQueue cache
tools. The older per-cache libp2p stream protocol is intentionally disabled.
"""
...
class GitHubAPICache:
    ...
    def __init__(
        ...
        if self.raw_p2p_cache_requested:
            logger.warning(
                "Legacy raw GitHub cache P2P is disabled; using MCP++ TaskQueue cache when configured"
            )
        ...
        # active distributed cache path is enable_task_p2p_cache.
        self._p2p_host = None
        self._p2p_protocol = None
        self._p2p_peer_exchange_protocol = None
        ...
        # Encryption for the retired raw P2P messages. Kept for tests and
        # compatibility helpers; not used to start a custom transport.
    ...
    def _init_p2p(self) -> None:
        """Legacy raw cache P2P is retired; use MCP++ TaskQueue cache."""
        raise RuntimeError("Legacy raw GitHub cache P2P is disabled; use MCP++ TaskQueue cache")

    async def _enqueue_broadcast(self, cache_key: str, entry: CacheEntry) -> None:
        """Legacy raw broadcast queue is retired."""
        _ = cache_key, entry
        return None

    async def _broadcast_worker(self, recv) -> None:
        """Legacy raw broadcast worker is retired."""
        _ = recv
        return None

    async def _run_p2p_node(self) -> None:
        """Legacy raw cache P2P node is retired."""
        raise RuntimeError("Legacy raw GitHub cache P2P node is disabled")

    async def _bootstrap_and_maintain(self) -> None:
        """Legacy raw cache peer maintenance is retired."""
        return None
    ...
    async def _initial_bootstrap_connect(self) -> None:
        """Legacy raw cache bootstrap is retired."""
    ...
    async def _connect_to_peer(self, peer_addr: str) -> None:
        """Record a legacy peer address without opening a raw connection."""
        if isinstance(peer_addr, str) and self._validate_multiaddr(peer_addr):
            self._p2p_known_peer_addrs.add(peer_addr)
        return None

    async def _periodic_connection_maintenance(self) -> None:
        """Legacy raw cache connection maintenance is retired."""
        return None

    async def _refresh_discovery_and_connect(self) -> None:
        """Legacy raw cache discovery is retired."""
        return None

    async def _ensure_min_connections(self) -> None:
        """Legacy raw cache connection floor is retired."""
        return None
    ...
    async def _peer_exchange_with_peer(self, peer_info: Any) -> None:
        """Legacy raw cache peer exchange is disabled."""
        _ = peer_info
        return None
