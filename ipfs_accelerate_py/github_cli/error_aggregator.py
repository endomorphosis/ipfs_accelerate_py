...
class ErrorAggregator:
    ...
    def start_bundling(self):
        ...
        if self.bundling_thread is None or not self.bundling_thread.is_alive():
            ...
            self.bundling_thread = threading.Thread(
                target=self._bundling_loop, daemon=True, name="ErrorBundlingThread"
            )
    ...
    def collect_peer_errors(self) -> int:
        ...
        try:
            ...
            error_keys = [
                entry["key"] for entry in cache_entries if entry["key"].startswith("error-")
            ]
                            ...
                            try:
                                ...
                                if cached_data:
                                    ...
                                    self.storage.store_file(
                                        temp_file, json.dumps(error_data), pin=False
                                    )
                            except:
                                with open(temp_file, "r") as f:
                                    error_data = json.load(f)
    ...
    def _refresh_existing_issues_cache(self):
        ...
        try:
            ...
            if (
                self.issues_cache_timestamp
                and datetime.now(UTC) - self.issues_cache_timestamp < self.issues_cache_ttl
            ):
                return
            ...
            if self._github_cli:
                try:
                    ...
                    if result.get("success"):
                        ...
                        logger.info(
                            f"✓ Refreshed issues cache (P2P/IPFS cached): {len(self.existing_issues_cache)} issues"
                        )
                        return
                    else:
    ...
    def _create_github_issues(self, errors_to_report: Dict[str, List[Dict]]) -> int:
        ...
        for signature, errors in errors_to_report.items():
            try:
                ...
                # Build issue title
                title = (
                    f"[Auto-Generated] {template_error['type']}: {template_error['message'][:80]}"
                )
                ...
                body_parts.extend(["## Occurrences", ""])
                for error in errors[:5]:  # Show first 5
                    body_parts.append(f"- {error['timestamp']} on peer `{error['peer_id']}`")
                if len(errors) > 5:
                ...
                if self._github_cli:
                    try:
                        ...
                        if result.get("success"):
                            ...
                            logger.warning(
                                f"Cached GitHub CLI failed to create issue: {result.get('stderr')}"
                            )
                    except Exception as e:
                        logger.warning(f"Cached GitHub CLI failed, falling back to direct gh: {e}")
                ...
                if issue_url:
                    ...
                    self.existing_issues_cache[signature] = {"url": issue_url, "title": title}
    ...
    def _create_draft_pr_from_issue(
        self, issue_url: str, error_data: Dict, signature: str
    ) -> Optional[str]:
        """
...
"""
    ...
    def _invoke_copilot_autofix(self, issue_number: str, error_data: Dict, signature: str) -> bool:
        """
...
"""
    ...
    def get_error_statistics(self) -> Dict:
        ...
        return {
            ...
            "last_bundle_time": self.last_bundle_time.isoformat()
            if self.last_bundle_time
            else None,
