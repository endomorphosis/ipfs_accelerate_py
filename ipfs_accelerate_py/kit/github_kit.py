...
class GitHubKit:
    ...
    def __init__(self, gh_path: str = "gh", timeout: int = 30):
        """
    ...
    def _verify_installation(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.gh_path, "--version"], capture_output=True, text=True, timeout=5
            )
    ...
    def _run_command(self, args: List[str], timeout: Optional[int] = None) -> GitHubResult:
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return GitHubResult(success=True, data=data, raw_output=result.stdout)
            else:
                return GitHubResult(
                    success=False, error=result.stderr or result.stdout, raw_output=result.stdout
                )

        except subprocess.TimeoutExpired:
            return GitHubResult(success=False, error=f"Command timed out after {timeout} seconds")
        except Exception as e:
            return GitHubResult(success=False, error=str(e))

    # Repository Operations

    def list_repos(self, owner: Optional[str] = None, limit: int = 30) -> GitHubResult:
        """
        List repositories.
    ...
    def get_repo(self, repo: str) -> GitHubResult:
        ...
        args = [
            ...
            repo,
            "--json",
            "name,description,url,updatedAt,defaultBranch,isPrivate",
        ]
        return self._run_command(args)
    ...
    def list_prs(self, repo: str, state: str = "open", limit: int = 30) -> GitHubResult:
        """
        List pull requests.
    ...
    def get_pr(self, repo: str, number: int) -> GitHubResult:
        ...
        args = [
            ...
            repo,
            "--json",
            "number,title,state,body,url,createdAt,updatedAt,author",
        ]

        return self._run_command(args)

    def create_pr(
        self, repo: str, title: str, body: str, base: str = "main", head: Optional[str] = None
    ) -> GitHubResult:
        """
    ...
    def list_issues(self, repo: str, state: str = "open", limit: int = 30) -> GitHubResult:
        """
        List issues.
    ...
    def create_issue(self, repo: str, title: str, body: str) -> GitHubResult:
        """
        Create an issue.
    ...
    def list_workflow_runs(
        self, repo: str, workflow: Optional[str] = None, limit: int = 20
    ) -> GitHubResult:
        """
        List workflow runs.
    ...
    def get_workflow_run(self, repo: str, run_id: int) -> GitHubResult:
        ...
        args = [
            ...
            repo,
            "--json",
            "databaseId,name,status,conclusion,createdAt,updatedAt,jobs",
        ]
