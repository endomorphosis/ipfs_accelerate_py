...
class DockerKit:
    ...
    def __init__(self, docker_path: str = "docker", timeout: int = 300):
        """
    ...
    def _verify_installation(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.docker_path, "--version"], capture_output=True, text=True, timeout=5
            )
    ...
    def _run_command(
        self, args: List[str], timeout: Optional[int] = None, input_data: Optional[str] = None
    ) -> DockerResult:
        """
        ...
        try:
            result = subprocess.run(
                full_command, capture_output=True, text=True, timeout=timeout, input=input_data
            )
    ...
    def list_containers(
        self, all_containers: bool = False, filters: Optional[Dict[str, str]] = None
    ) -> DockerResult:
        """
    ...
    def stop_container(self, container: str, timeout: int = 10) -> DockerResult:
        """
    ...
    def pull_image(self, image: str, timeout: Optional[int] = None) -> DockerResult:
        """
        Pull a Docker image.
    ...
    def list_images(self, filters: Optional[Dict[str, str]] = None) -> DockerResult:
        """
        List Docker images.
    ...
    def execute_code_in_container(
        self, image: str, code: str, language: str = "python", timeout: Optional[int] = None
    ) -> DockerResult:
        """
