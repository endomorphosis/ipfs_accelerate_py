...
class InferenceServiceConfig:
    ...
    api_backends: List[str] = field(
        default_factory=lambda: ["hf_tgi", "hf_tei", "ollama", "openai_api"]
    )

    # CLI backend configuration
    enable_cli_backends: bool = True
    cli_backends: List[str] = field(default_factory=lambda: ["claude_cli", "openai_cli"])
...
class UnifiedInferenceService:
        ...
        **kwargs: Any,
        ...
        if storage is not None:
            ...
            try:
                ...
                metadata["storage"]["using_fallback"] = bool(
                    getattr(storage, "using_fallback", True)
                )
            except Exception as exc:
    ...
    async def _register_api_backends(self):
        ...
        for backend_name in self.config.api_backends:
            try:
                ...
                module = __import__(
                    f"ipfs_accelerate_py.api_backends.{backend_name}", fromlist=[backend_name]
                )
    ...
    async def _register_cli_backends(self):
        ...
        cli_backend_info = {
            "claude_cli": {"name": "Claude CLI", "tasks": {"text-generation", "code-generation"}},
            "openai_cli": {"name": "OpenAI CLI", "tasks": {"text-generation", "code-generation"}},
        }
    ...
    def _get_backend_capabilities(self, backend_name: str) -> BackendCapabilities:
        ...
        if backend_name in ["hf_tgi", "ollama", "vllm"]:
            ...
            capabilities.supported_tasks = {"text-generation", "text-embedding", "image-generation"}
            capabilities.supports_streaming = True
    ...
    def print_status(self):
        ...
        if self.config.enable_websocket:
            print(f"\n🔌 WebSocket:")
            print(
                f"  Endpoint: ws://{self.config.hf_server_host}:{self.config.hf_server_port}/ws/{{client_id}}"
            )
    ...
    async def main():
        ...
        try:
            ...
            if service.hf_server:
                ...
                uvicorn.run(
                    service.hf_server.app, host=config.hf_server_host, port=config.hf_server_port
                )
            else:
                # Just keep running
