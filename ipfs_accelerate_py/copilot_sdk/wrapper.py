...
class CopilotSDK:
    ...
    def create_session(
        ...
        **kwargs,
        ...
        Returns:
            Session object
        """
        return self._run_async(self._create_session_async(model, tools, streaming, **kwargs))

    async def _send_message_async(
        self, session: Any, prompt: str, use_cache: bool = True, **kwargs
    ) -> Dict[str, Any]:
        ...
        def on_event(event):
            if event.type.value == "assistant.message":
                messages.append(
                    {"type": "message", "content": event.data.content, "timestamp": time.time()}
                )
            elif event.type.value == "assistant.reasoning":
                messages.append(
                    {"type": "reasoning", "content": event.data.content, "timestamp": time.time()}
                )
        ...
        response = {
            ...
            "messages": messages,
            "success": True,
            "model": session._config.get("model")
    ...
    def send_message(
        ...
        """
        return self._run_async(self._send_message_async(session, prompt, use_cache, **kwargs))

    async def _stream_message_async(
        self, session: Any, prompt: str, on_chunk: Optional[Callable[[str], None]] = None, **kwargs
    ) -> Dict[str, Any]:
    ...
    def stream_message(
        ...
        """
        return self._run_async(self._stream_message_async(session, prompt, on_chunk, **kwargs))
    ...
    async def _stop_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "message": "Copilot SDK stopped", "timestamp": time.time()}
        except Exception as e:
