...
- Focuses on what is currently reliable in py-libp2p:
  * TCP multiaddr dialing through the shared libp2p runtime helpers
...
class UniversalConnectivity:
            ...
            def _runner() -> None:
                try:
                    ...
                    try:
                        ...
                        if listen_addrs:
                            ma = make_multiaddr(str(listen_addrs[0]))
    ...
    async def configure_dht(self, host) -> None:
        ...
        if not self.config.enable_dht:
            return
        try:
            self._dht = make_kad_dht(host, mode="server")
    ...
    async def setup_circuit_relay(self, host, relay_addrs: Optional[List[str]] = None) -> None:
        ...
        try:
            ...
            for relay_addr in self.relay_peers:
                try:
                    peer_info = peerinfo_from_multiaddr(relay_addr)
    ...
    def _build_relay_multiaddr(self, peer_addr: str) -> Optional[str]:
        ...
        if not self.relay_peers:
            return None
        try:
            ma = make_multiaddr(peer_addr)
            peer_id = ma.value_for_protocol("p2p")
            if not peer_id:
                return None
            relay = self.relay_peers[0]
            relay_ma = make_multiaddr(relay)
            relay_ma = relay_ma.encapsulate(make_multiaddr("/p2p-circuit"))
            relay_ma = relay_ma.encapsulate(make_multiaddr(f"/p2p/{peer_id}"))
            return str(relay_ma)
        except Exception:
            return None
    ...
    def add_service(self, zeroconf, service_type, name) -> None:
        try:
            ...
            async def _connect():
                ...
                try:
                    peer_info = peerinfo_from_multiaddr(multiaddr)
