...
def fetch_huggingface_repo_structure(
    model_id: str, branch: str = "main", include_ipfs_cids: bool = True
...
class ModelManager:
    ...
    def __init__(
        ...
        self.enable_ipfs = (
            ...
            else os.getenv("ENABLE_IPFS_MODEL_STORAGE", "").lower() in ("1", "true", "yes")
        )
        if self.enable_ipfs:
        ...
        if HAVE_STORAGE_WRAPPER:
            try:
                ...
                if self._storage_wrapper.is_distributed:
                    ...
                    logger.debug(
                        "Model manager using local filesystem (distributed storage disabled)"
                    )
            except Exception as e:
    ...
    def _init_catalog(
        ...
        missing = tuple(name for name in required if not callable(getattr(selected, name, None)))
        if missing:
            raise TypeError(
                "catalog does not implement the canonical facade: %s" % ", ".join(missing)
            )
        ...
        should_project = True if project_legacy_models is None else project_legacy_models
        ...
        if not callable(register):
            raise TypeError("catalog cannot project legacy records without register_source")
        ...
        existing_names = (
            {item.name for item in source_states()} if callable(source_states) else set()
        )
    ...
    def _require_usage_service(self) -> Any:
        ...
        if self._usage_service is None:
            raise UsageServiceUnavailable("usage service is not configured on this ModelManager")
        ...
        try:
            ...
            for column_name in [
                ...
                try:
                    col_type = (
                        "TIMESTAMP"
                        if column_name in {"last_used_at", "revision_created_at"}
                        else "BIGINT"
                        if column_name == "inference_count"
                        else "JSON"
                        if column_name == "serving_config"
                        else "VARCHAR"
                    )
                    self.con.execute(
            ...
            self.con.execute(
                "CREATE INDEX IF NOT EXISTS idx_architecture ON model_metadata(architecture)"
            )
            self.con.execute(
    ...
    @staticmethod
    def _model_metadata_from_json(model_id: str, raw: Any) -> ModelMetadata:
        ...
        for io_field in ("inputs", "outputs"):
            ...
            for value in values:
                ...
                spec = dict(value)
                if "data_type" in spec and not isinstance(spec["data_type"], DataType):
                    data_type = spec["data_type"]
                    if isinstance(data_type, str) and data_type.startswith("DataType."):
                        spec["data_type"] = DataType[data_type.split(".", 1)[1]]
                    else:
        ...
        model_data = {key: value for key, value in model_data.items() if key in known_fields}
    ...
    def _load_from_json(self):
        ...
        if not os.path.exists(self.json_path):
            logger.info(
                f"JSON file {self.json_path} does not exist, starting with empty model registry"
            )
            return
            ...
            if isinstance(records, list):
                ...
                for item in records:
                    if not isinstance(item, dict) or not item.get("model_id"):
                        raise ValueError("model registry list rows require model_id")
        ...
        try:
            ...
            for model_id, metadata in self.models.items():
                ...
                if model_dict["revision_created_at"]:
                    model_dict["revision_created_at"] = model_dict[
                        "revision_created_at"
                    ].isoformat()
            ...
            if self._storage_wrapper and self._storage_wrapper.is_distributed:
                try:
                    ...
                    logger.info(
                        f"Saved {len(self.models)} models to distributed storage (CID: {cid[:16]}...)"
                    )
    ...
    def _record_model_registration(self, metadata: ModelMetadata) -> None:
        ...
        event_data = {
            ...
            if hasattr(metadata.model_type, "value")
            else str(metadata.model_type),
            "model_revision": metadata.model_revision,
            "revision_id": metadata.revision_id,
            "revision_created_at": metadata.revision_created_at.isoformat()
            if metadata.revision_created_at
            else None,
        ...
        if self._datasets_manager:
            try:
                self._datasets_manager.log_event(
                    "model_registered", event_data, level="INFO", category="GENERAL"
                )
            ...
            try:
                self._provenance_logger.log_transformation(
                    ...
                    data={
                        ...
                        "input_types": [
                            inp.data_type.value
                            if hasattr(inp.data_type, "value")
                            else str(inp.data_type)
                        ...
                        "output_types": [
                            out.data_type.value
                            if hasattr(out.data_type, "value")
                            else str(out.data_type)
    ...
    def _record_model_access(self, metadata: ModelMetadata) -> None:
        ...
        if self._datasets_manager:
            try:
                self._datasets_manager.log_event(
                    "model_accessed", event_data, level="INFO", category="GENERAL"
                )
    ...
    def _record_model_usage(self, metadata: ModelMetadata) -> None:
        ...
        if self._datasets_manager:
            try:
                self._datasets_manager.log_event(
                    "model_inference_linked", event_data, level="INFO", category="GENERAL"
                )
    ...
    def store_model_to_ipfs(self, model_path: str, model_id: str = None) -> Optional[str]:
        ...
        try:
            ...
            if self._artifact_storage and path.is_file():
                ...
                cid = self._ipfs_backend.add_path(model_path, recursive=True, pin=True)
    ...
    def _store_artifact_file(
        self, artifact_path: Optional[str], artifact_name: str
    ) -> Optional[str]:
        ...
        if self._artifact_storage:
            try:
                return self._artifact_storage.store(
                    path.read_bytes(), filename=artifact_name, pin=True
                )
            except Exception as e:
    ...
    def _store_artifact_manifest(
        self, metadata: ModelMetadata, artifact_map: Dict[str, Optional[str]]
    ) -> Optional[str]:
    ...
    def add_model_with_ipfs_storage(
        ...
        if store_to_ipfs:
            ...
            artifact_cids["model_cid"] = (
                self._store_artifact_file(model_path, f"{safe_model_id}.model")
                if model_path
                else None
            )
            artifact_cids["config_cid"] = (
                self._store_artifact_file(config_path, f"{safe_model_id}.config")
                if config_path
                else None
            )
            artifact_cids["tokenizer_cid"] = (
                self._store_artifact_file(tokenizer_path, f"{safe_model_id}.tokenizer")
                if tokenizer_path
                else None
            )
            ...
            return CatalogLookupResult(
                ...
                diagnostics=(
                    CatalogDiagnostic(
                        ...
                        message=(
                            "identifier maps to multiple canonical records; lookup failed closed"
                        ),
                        record_type=canonical_type,
                        ambiguous=True,
                    ),
    ...
    def list_usage_limits(
        ...
        *,
        ...
        snapshot = self.usage_snapshot(scope_id, expected_usage_revision=expected_usage_revision)
    ...
    def resolve_for_routing(
        ...
        *routing_policy* is ``off``/omitted; enforcing modes fail closed.
        ...
        if expected_catalog_revision is not None and catalog_revision != expected_catalog_revision:
    ...
    def get_served_model(
        ...
        exact_matches = [
            model
            for model in models
            if (model["id"] == model_id or model.get("transport_model_id") == model_id)
        ]
        ...
        if model_id.lower() in aliases:
            leanstral_matches = [
                ...
                and "leanstral" in str(model.get("transport_model_id") or "").casefold()
            ]
    ...
    def search_models(self, query: str) -> List[ModelMetadata]:
        ...
        for metadata in self.models.values():
            ...
            if (
                query_lower in metadata.model_name.lower()
    ...
    def get_compatible_models(
        self, input_type: DataType, output_type: DataType
    ) -> List[ModelMetadata]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "knowledge_graph_cid": self._knowledge_graph.graph_cid
            if self._knowledge_graph
            else None,
        }
        ...
        for metadata in self.models.values():
            ...
            for inp in metadata.inputs:
                input_type = (
                    inp.data_type.value if hasattr(inp.data_type, "value") else str(inp.data_type)
                )
                stats["common_input_types"][input_type] = (
                    stats["common_input_types"].get(input_type, 0) + 1
                )

            for out in metadata.outputs:
                output_type = (
                    out.data_type.value if hasattr(out.data_type, "value") else str(out.data_type)
                )
                stats["common_output_types"][output_type] = (
                    stats["common_output_types"].get(output_type, 0) + 1
                )
    ...
    def resolve_launch_command(self, model_id: str) -> Optional[List[str]]:
        ...
        ENGINE_TEMPLATES: Dict[str, List[str]] = {
            ...
            "hf_pipeline": [
                "python",
                "-m",
                "ipfs_accelerate_py.hf_model_server.server",
                "--model-id",
                model_id,
            ],
        }
    ...
    def refresh_repository_structure(
        self, model_id: str, branch: str = "main", include_ipfs_cids: bool = True
    ) -> bool:
        """
    ...
    def get_models_with_ipfs_cids(self) -> List[ModelMetadata]:
        ...
        for model in self.models.values():
            if (
                model.repository_structure
                and model.repository_structure.get("ipfs_enabled")
                and any(
                    "ipfs_cid" in f for f in model.repository_structure.get("files", {}).values()
                )
            ):
    ...
    def get_ipfs_gateway_urls(
        self, model_id: str, gateway_base: str = "https://ipfs.io/ipfs/"
    ) -> Dict[str, str]:
        """
    ...
    def get_models_by_pipeline_type(
        self,
        pipeline_type: str,
            ...
            for model_id, metadata in self.models.items():
                ...
                if have_mapper:
                    ...
                    supports_pipeline = PipelineTypeMapper.supports_pipeline(
                        pipeline_type, architecture=metadata.architecture
                    )
                else:
                    ...
                    if "causal" in pipe_lower or "text-generation" in pipe_lower:
                        supports_pipeline = any(
                            x in arch_lower for x in ["gpt", "llama", "mistral", "falcon"]
                        )
                    elif "classification" in pipe_lower:
                        supports_pipeline = any(
                            x in arch_lower for x in ["bert", "roberta", "distilbert"]
                        )
                    ...
                    results.append(
                        {
                            ...
                            "pipeline_types": [
                                pipeline_type
                            ],  # Could be enhanced with full type detection
                            "context_length": metadata.huggingface_config.get(
                    ...
                    results.append(
                        {
                            ...
                            "cost_per_1k_input": api_model.cost_per_1k_tokens.get("input", 0)
                            if api_model.cost_per_1k_tokens
                            else 0,
                            "cost_per_1k_output": api_model.cost_per_1k_tokens.get("output", 0)
                            if api_model.cost_per_1k_tokens
                            else 0,
    ...
    def get_all_pipeline_types(
        self, include_api: bool = True, include_self_hosted: bool = True
    ) -> Set[str]:
        """
        ...
        if include_self_hosted:
            try:
                ...
                for metadata in self.models.values():
                    ...
                    types = PipelineTypeMapper.get_pipeline_types_for_architecture(
        ...
        if include_api:
            try:
                from .api_integrations.model_registry import (
                    get_all_pipeline_types as get_api_pipeline_types,
                )
    ...
    def get_model_recommendations(
        ...
        for model in models:
            ...
            if model["source_type"] == "api" and max_cost_per_1k is not None:
                avg_cost = (
                    model.get("cost_per_1k_input", 0) + model.get("cost_per_1k_output", 0)
                ) / 2
...
def create_model_from_huggingface(
    ...
    if fetch_repo_structure:
        ...
        if repository_structure:
            logger.info(
                f"Successfully fetched repository structure with {repository_structure.get('total_files', 0)} files"
            )
            if repository_structure.get("ipfs_enabled"):
                cid_count = sum(
                    1 for f in repository_structure["files"].values() if "ipfs_cid" in f
                )
                logger.info(f"Generated IPFS CIDs for {cid_count} files")
        else:
...
class VectorDocumentationIndex:
    ...
    def save_index(self):
        ...
        try:
            ...
            data = {"model_name": self.model_name, "documents": []}
    ...
    def load_index(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Loaded index with {len(self.documents)} documents from {self.storage_path}"
            )
            return True
...
class BanditModelRecommender:
    ...
    def provide_feedback(
        self, model_id: str, feedback_score: float, context: RecommendationContext
    ):
        """
        ...
        logger.info(
            f"Updated feedback for {model_id}: avg_reward={arm.average_reward:.3f}, trials={arm.num_trials}"
        )
            ...
            for model in all_models:
                ...
                if context.input_type and hasattr(context.input_type, "value"):
                    input_type_str = (
                        ...
                        if hasattr(context.input_type, "value")
                        else str(context.input_type)
                    )
                    if model.inputs and input_type_str not in ["tokens", "text"]:
                        if not any(
                            hasattr(inp.data_type, "value")
                            and inp.data_type.value == input_type_str
                            for inp in model.inputs
                        ):
                            # If no exact match but it's a language model and we want tokens/text, allow it
                            if model.model_type == ModelType.LANGUAGE_MODEL and input_type_str in [
                                "tokens",
                                "text",
                            ]:
