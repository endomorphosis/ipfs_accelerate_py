...
class DockerAPICache(BaseAPICache):
    ...
    def extract_validation_fields(self, operation: str, data: Any) -> Optional[Dict[str, Any]]:
        ...
        # Container operations - use State and timestamps
            ...
            validation["Id"] = data.get("Id")
            validation["State"] = (
                data.get("State", {}).get("Status") if isinstance(data.get("State"), dict) else None
            )
            validation["Created"] = data.get("Created")
            # Container status changes frequently, so also include:
            validation["Running"] = (
                data.get("State", {}).get("Running")
                if isinstance(data.get("State"), dict)
                else None
            )
            validation["Paused"] = (
                data.get("State", {}).get("Paused") if isinstance(data.get("State"), dict) else None
            )
            validation["Restarting"] = (
                data.get("State", {}).get("Restarting")
                if isinstance(data.get("State"), dict)
                else None
            )
        ...
        # Network operations
            ...
            validation["connected_containers"] = (
                len(containers) if isinstance(containers, dict) else 0
            )
