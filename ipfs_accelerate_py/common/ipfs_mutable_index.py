...
class IPFSMutableCacheIndex:
    ...
    def _start_pubsub_subscriber(self) -> None:
        ...
        self._sub_thread = threading.Thread(
            target=_run, name="ipfs-cache-index-pubsub", daemon=True
        )
    ...
    def _publish_pubsub(
        ...
        try:
            ...
            self._ipfs.pubsub_pub(
                self.pubsub_topic, json.dumps(msg, sort_keys=True, separators=(",", ":"))
            )
        except Exception:
            return
...
def get_global_mutable_index() -> Optional[IPFSMutableCacheIndex]:
    ...
    enable_ipns = _truthy(
        os.environ.get("IPFS_ACCELERATE_CACHE_IPNS_INDEX")
        or os.environ.get("IPFS_DATASETS_PY_CACHE_IPNS_INDEX")
    )
    ...
    with _global_lock:
        ...
        ipns_name = os.environ.get("IPFS_ACCELERATE_CACHE_IPNS_NAME") or os.environ.get(
            "IPFS_DATASETS_PY_CACHE_IPNS_NAME"
        )
        ipns_key = os.environ.get("IPFS_ACCELERATE_CACHE_IPNS_KEY") or os.environ.get(
            "IPFS_DATASETS_PY_CACHE_IPNS_KEY"
        )
        topic = (
            os.environ.get("IPFS_ACCELERATE_CACHE_PUBSUB_TOPIC")
            or os.environ.get("IPFS_DATASETS_PY_CACHE_PUBSUB_TOPIC")
            or "ipfs-accelerate-cache-index"
        )

        publish_min = float(
            os.environ.get("IPFS_ACCELERATE_CACHE_IPNS_PUBLISH_MIN_INTERVAL_S", "30") or "30"
        )
        refresh_min = float(
            os.environ.get("IPFS_ACCELERATE_CACHE_IPNS_REFRESH_MIN_INTERVAL_S", "30") or "30"
        )
        ...
        logger.info(
            "✓ IPFS mutable cache index enabled (ipns=%s, pubsub=%s)", enable_ipns, enable_pubsub
        )
        return _global_index
