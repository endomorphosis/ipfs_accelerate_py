...
class CIDCacheIndex:
    ...
    def add(self, cid: str, operation: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        """
        with self._lock:
            self._cid_to_metadata[cid] = {"operation": operation, "metadata": metadata or {}}
    ...
    def find_by_prefix(self, prefix: str, max_results: int = 100) -> List[str]:
        ...
        """
        with self._lock:
            matches = [cid for cid in self._cid_to_metadata.keys() if cid.startswith(prefix)]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "operation_counts": {op: len(cids) for op, cids in self._operation_to_cids.items()},
            }
    ...
    def save_to_file(self, filepath: Path) -> None:
        ...
        with self._lock:
            data = {
                ...
                "operation_to_cids": {
                    op: list(cids) for op, cids in self._operation_to_cids.items()
                },
            }
