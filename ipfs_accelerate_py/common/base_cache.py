...
class BaseAPICache(ABC):
    ...
    def __init__(
        ...
        if HAVE_STORAGE_WRAPPER:
            try:
                ...
                if self._storage_wrapper.is_distributed:
                    ...
                    logger.debug(
                        f"{self.cache_name} cache using local filesystem (distributed storage unavailable or disabled)"
                    )
            except Exception as e:
        ...
        logger.info(
            f"✓ {self.cache_name} cache initialized (persistence={enable_persistence}, p2p={enable_p2p})"
        )
    ...
    def _task_p2p_remote(self):
        ...
        if not self.enable_p2p:
            return None
        if os.environ.get("IPFS_ACCELERATE_PY_TASK_P2P_CACHE_DISABLE", "").lower() in {
            "1",
            "true",
            "yes",
        }:
    ...
    def _task_p2p_set_one(
        self, *, remote, cache_key: str, payload: Dict[str, Any], ttl_s: Optional[float]
    ) -> None:
        if remote is None or self._p2p_fernet is None:
            return
        try:
            ...
            if encrypted is None:
                return
            p2p_client.cache_set_sync(
                remote=remote, key=remote_key, value=encrypted, ttl_s=ttl_s, timeout_s=10.0
            )
        except Exception:
            return

    def _task_p2p_set(
        self, cache_key: str, payload: Dict[str, Any], ttl_s: Optional[float]
    ) -> None:
        remote = self._task_p2p_remote()
        ...
        for addr in self._task_p2p_replication_multiaddrs():
            try:
                self._task_p2p_set_one(
                    remote=RemoteQueue(peer_id="", multiaddr=addr),
                    cache_key=cache_key,
                    payload=payload,
                    ttl_s=ttl_s,
                )
    ...
    def make_cache_key(self, operation: str, *args, **kwargs) -> str:
        ...
        query = {"operation": operation, "args": list(args), "kwargs": dict(sorted(kwargs.items()))}
        ...
        try:
            ...
            if self._p2p_fernet is not None:
                ...
                if not isinstance(wrapper, dict):
                    return None
                content = json.dumps(
                    wrapper, sort_keys=True, ensure_ascii=False, separators=(",", ":")
                ).encode("utf-8")
            else:
                content = json.dumps(
    ...
    def get(self, operation: str, *args, use_cache: bool = True, **kwargs) -> Optional[Any]:
        """
        Get a cached value.
                        ...
                        try:
                            ...
                            if not remote_entry.is_expired():
                                with self._lock:
                                    self._cache[cache_key] = remote_entry
                                    self._cid_index.add(
                                        cache_key, operation, remote_entry.metadata or {}
                                    )
                                    self._stats["hits"] += 1
                                    self._stats["api_calls_saved"] += 1
                                return remote_entry.data
        ...
        if self.enable_p2p and self._p2p_fernet is not None:
            try:
                ...
                if self._ipfs_payload_pointers or self._mutable_index is not None:
                    ...
                    if (
                        isinstance(ipfs_payload_cid, str)
                        and ipfs_payload_cid
                        and self._mutable_index is not None
                    ):
                        try:
                ...
                if (
                    self._ipfs_payload_pointers
                    and isinstance(ipfs_payload_cid, str)
                    and ipfs_payload_cid
                ):
        ...
        try:
            ...
            if self._storage_wrapper and self._storage_wrapper.is_distributed:
                try:
                    ...
                    logger.debug(
                        f"Failed to save to distributed storage, falling back to local: {e}"
                    )
