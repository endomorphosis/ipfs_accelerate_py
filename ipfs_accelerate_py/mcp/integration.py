...
class _FallbackStandaloneApp:
    ...
    def add_route(
        self, path: str, endpoint: Callable[..., Any], methods: Optional[List[str]] = None
    ) -> None:
...
def setup_extended_handlers(server: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Failed to import ipfs_accelerate_py, extended handlers will not be available"
            )
...
def register_custom_tool(
    server: Any, name: str, function: Callable, description: str, input_schema: Dict[str, Any]
) -> None:
    """
    ...
    try:
        ...
        server.mcp.register_tool(
            name=name, function=function, description=description, input_schema=input_schema
        )
...
def register_custom_resource(server: Any, uri: str, function: Callable, description: str) -> None:
    """
    ...
    try:
        ...
        # Register resource
        server.mcp.register_resource(uri=uri, function=function, description=description)
...
def create_integrated_server(
    ...
    try:
        ...
        integrate_with_ipfs_accelerate(server=server, ipfs_accelerate_module=ipfs_accelerate_module)
