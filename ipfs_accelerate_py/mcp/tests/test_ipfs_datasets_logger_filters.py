...
def test_ipfs_datasets_filter_keeps_errors_and_cutover_warnings() -> None:
    ...
    cutover_record = _make_record(
        logging.WARNING,
        "Legacy MCP facade runtime path is deprecated (D1 warn-only); reason=legacy_fallback.",
    )
...
def test_ipfs_datasets_filter_installs_on_existing_handler() -> None:
    ...
    try:
        ...
        child_logger.warning("Legacy MCP facade runtime path is deprecated (D1 warn-only); reason=legacy_fallback.")
