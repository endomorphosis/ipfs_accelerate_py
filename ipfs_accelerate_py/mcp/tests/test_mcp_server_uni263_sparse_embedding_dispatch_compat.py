#!/usr/bin/env python3
"""UNI-263 sparse embedding dispatch compatibility tests for focused parity coverage."""
...
class TestMCPServerUNI263SparseEmbeddingDispatchCompat(unittest.TestCase):
    ...
    def test_sparse_embedding_dispatch_infers_error_status_from_contradictory_delegate_payloads(
        self,
        mock_wrapper,
    ) -> None:
        class DummyServer:
        ...
        async def _run_flow() -> None:
            with patch.dict(
                ...
                server = create_mcp_server(name="sparse-embedding-dispatch-compat-errors")
                ...
                searched = self._assert_dispatch_success_envelope(
                    await dispatch(
                        ...
                        "sparse_search",
                        {"query": "hello", "collection_name": "docs"},
                    )
                )
