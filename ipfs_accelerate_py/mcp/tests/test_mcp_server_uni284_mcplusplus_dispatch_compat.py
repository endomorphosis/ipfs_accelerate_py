#!/usr/bin/env python3
"""UNI-284 mcplusplus dispatch compatibility tests for focused parity coverage."""
...
class TestMCPServerUNI284McplusplusDispatchCompat(unittest.TestCase):
    ...
    def test_mcplusplus_dispatch_infers_error_status_from_contradictory_delegate_payloads(self, mock_wrapper) -> None:
        class DummyServer:
            def __init__(self):
                self.tools = {}
                self.mcp = None
        ...
        async def _run_flow() -> None:
            with patch.dict(
                ...
                server = create_mcp_server(name="mcplusplus-dispatch-compat-errors")
                ...
                result = self._assert_dispatch_success_envelope(
                    await dispatch(
                        ...
                        {"task_id": "task-1", "include_logs": False, "include_metrics": False},
                    )
                )
