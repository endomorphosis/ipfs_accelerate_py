...
class _NoDispatchServer:
    def __init__(self) -> None:
        self.tools = {}


class _UnknownToolServer:
    def __init__(self) -> None:
        self.tools = {
            "tools_list_tools": {"function": self._list_tools},
            "tools_dispatch": {"function": self._dispatch},
    ...
    async def _list_tools(self):
        return [{"category": "demo", "name": "other_tool"}]
...
class TestMCPServerPolicyDelegationLegacyAdapters(unittest.TestCase):
    ...
    def test_temporal_policy_evaluator_denies_without_permission(self) -> None:
        ...
        result = evaluator.evaluate(
            _DummyIntent(tool="smoke.echo", intent_cid="cid-intent-deny"),
            policy,
            actor="did:model:worker",
        )

        self.assertEqual(result["decision"], "deny")
        self.assertEqual(result["intent_cid"], "cid-intent-deny")
    ...
    def test_temporal_policy_evaluator_allows_with_obligations(self) -> None:
        policy = PolicyObject(
            clauses=[
                ...
                PolicyClause(
                    ...
                    actor="did:model:worker",
                    action="smoke.echo",
                    obligation_deadline="2030-01-01T00:00:00+00:00",
                    metadata={"kind": "audit"},
                ),
            ]
        )
        ...
        result = evaluator.evaluate(
            _DummyIntent(tool="smoke.echo", intent_cid="cid-intent-obligations"),
            policy,
            actor="did:model:worker",
        )

        self.assertEqual(result["decision"], "allow_with_obligations")
        self.assertEqual(result["intent_cid"], "cid-intent-obligations")
        self.assertEqual(len(result["obligations"]), 1)
        self.assertEqual(result["obligations"][0].get("metadata", {}).get("kind"), "audit")

    def test_ucan_delegation_evaluator_rejects_actor_mismatch(self) -> None:
        evaluator = DelegationEvaluator()
        delegation = Delegation(
            cid="cid-leaf",
            issuer="did:user:alice",
        ...
        allowed, reason = evaluator.can_invoke(
            ...
            resource="mcp://tool/smoke.echo",
            ability="invoke",
            actor="did:model:other",
        )
        self.assertFalse(allowed)
        self.assertEqual(reason, "Leaf audience does not match actor")
    ...
    def test_temporal_deontic_server_returns_dispatch_unavailable(self) -> None:
        async def _run() -> None:
            with patch(
                "ipfs_accelerate_py.mcp_server.temporal_deontic_mcp_server.create_server",
                return_value=_NoDispatchServer(),
            ):
                server = TemporalDeonticMCPServer()
                result = await server.call_tool_direct("echo", {"value": "ok"})

            self.assertFalse(result["success"])
            self.assertEqual(result["error_code"], "DISPATCH_UNAVAILABLE")

        anyio.run(_run)

    def test_temporal_deontic_server_returns_unknown_tool(self) -> None:
        async def _run() -> None:
            with patch(
                "ipfs_accelerate_py.mcp_server.temporal_deontic_mcp_server.create_server",
                return_value=_UnknownToolServer(),
            ):
                server = TemporalDeonticMCPServer()
                result = await server.call_tool_direct("echo", {"value": "ok"})
