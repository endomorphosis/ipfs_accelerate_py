#!/usr/bin/env python3
"""UNI-264 vector-store dispatch compatibility tests for focused parity coverage."""
...
class TestMCPServerUNI264VectorStoreDispatchCompat(unittest.TestCase):
    ...
    def test_vector_store_dispatch_infers_error_status_from_contradictory_delegate_payloads(self, mock_wrapper) -> None:
        class DummyServer:
            def __init__(self):
                self.tools = {}
                self.mcp = None
        ...
        async def _run_flow() -> None:
            with patch.dict(
                ...
                native_vector_store_tools._API,
                {"enhanced_vector_search": _contradictory_failure},
                clear=False,
            ):
                server = create_mcp_server(name="vector-store-dispatch-compat-errors")
                ...
                searched = self._assert_dispatch_success_envelope(
                    await dispatch(
                        ...
                        {"collection": "docs", "query_vector": [0.1, 0.2], "top_k": 3},
                    )
                )
