...
class TestDockerMCPTools(unittest.TestCase):
    ...
    def test_execute_docker_container_success(self, mock_execute):
        ...
        mock_execute.return_value = DockerExecutionResult(
            success=True, exit_code=0, stdout="Hello from Docker", stderr="", execution_time=1.5
        )
    ...
    def test_execute_docker_container_failure(self, mock_execute):
        ...
        result = execute_docker_container(image="ubuntu:20.04", command="false")
    ...
    def test_execute_docker_container_with_environment(self, mock_execute):
        ...
        mock_execute.return_value = DockerExecutionResult(
            success=True, exit_code=0, stdout="VAR1=value1", stderr="", execution_time=1.0
        )

        result = execute_docker_container(
            image="ubuntu:20.04", command="env", environment={"VAR1": "value1", "VAR2": "value2"}
        )
    ...
    def test_build_and_execute_github_repo_with_build_args(self, mock_build):
        ...
        mock_build.return_value = DockerExecutionResult(
            success=True, exit_code=0, stdout="Built and executed", stderr="", execution_time=25.0
        )
    ...
    def test_execute_with_payload_script(
        self, mock_exists, mock_unlink, mock_chmod, mock_temp, mock_execute
    ):
        ...
        mock_execute.return_value = DockerExecutionResult(
            success=True, exit_code=0, stdout="Script executed", stderr="", execution_time=0.8
        )
...
class TestDockerToolsEdgeCases(unittest.TestCase):
    ...
    def test_docker_executor_not_available(self):
        ...
        result = execute_docker_container(image="python:3.9", command="echo test")
    ...
    def test_execute_docker_container_exception(self, mock_execute):
        ...
        result = execute_docker_container(image="python:3.9", command="python -c 'print(1)'")
    ...
    def test_execute_with_payload_cleanup(self, mock_execute):
        ...
        mock_execute.return_value = DockerExecutionResult(
            success=True, exit_code=0, stdout="", stderr="", execution_time=1.0
        )
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp:
            with patch("os.unlink") as mock_unlink:
                with patch("os.path.exists", return_value=True):
                    ...
                    execute_with_payload(
                        image="python:3.9", payload="test", payload_path="/app/test"
                    )
