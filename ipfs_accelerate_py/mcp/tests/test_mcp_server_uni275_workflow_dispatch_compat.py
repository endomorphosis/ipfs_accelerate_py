#!/usr/bin/env python3
"""UNI-275 workflow dispatch compatibility tests for focused parity coverage."""
...
class TestMCPServerUNI275WorkflowDispatchCompat(unittest.TestCase):
    ...
    def test_workflow_dispatch_infers_error_status_from_contradictory_delegate_payloads(self, mock_wrapper) -> None:
        ...
        async def _run_flow() -> None:
            with patch.dict(
                ...
                scheduled = self._assert_dispatch_success_envelope(
                    await dispatch(
                        "workflow_tools",
                        "schedule_p2p_workflow",
                        {"workflow_id": "wf-1", "name": "demo", "tags": ["smoke"]},
                    )
                )
                self.assertEqual(scheduled.get("status"), "error")
                self.assertEqual(scheduled.get("error"), "delegate failure")
