#!/usr/bin/env python3
"""UNI-272 CLI dispatch compatibility tests for focused parity coverage."""
...
class TestMCPServerUNI272CliDispatchCompat(unittest.TestCase):
    ...
    def test_cli_dispatch_infers_error_status_from_contradictory_delegate_payloads(self, mock_wrapper) -> None:
        class DummyServer:
            def __init__(self):
                self.tools = {}
                self.mcp = None
        ...
        async def _run_flow() -> None:
            with patch.dict(
                ...
                server = create_mcp_server(name="cli-dispatch-compat-errors")
                ...
                executed = self._assert_dispatch_success_envelope(
                    await dispatch(
                        "cli",
                        "execute_command",
                        {"command": "echo", "args": ["hello"], "timeout_seconds": 3},
                    )
                )
