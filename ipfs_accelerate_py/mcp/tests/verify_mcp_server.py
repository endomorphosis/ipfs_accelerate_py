...
class MCPVerifier:
    ...
    def __init__(self):
        ...
        self.results = {"tools": {}, "resources": {}, "prompts": {}, "summary": {}}
    ...
    async def verify_resources(self, session):
        ...
        try:
            ...
            # Expected resources
            expected_resources = ["ipfs://config", "ipfs://endpoints", "ipfs://models"]
            ...
            print(
                f"  Expected resources found: {len(found_expected_resources)}/{len(expected_resources)}"
            )
            ...
            if "ipfs://config" in resource_uris:
                ...
                try:
                    ...
                    print(
                        f"  Config keys: {', '.join(config.keys()) if isinstance(config, dict) else 'N/A'}"
                    )
                    ...
                    if self._storage:
                        try:
                            cid = self._storage.write_file(
                                config_json, "mcp_server_config.json", pin=False
                            )
    ...
    async def verify_prompts(self, session):
        ...
        try:
            ...
            print(
                f"  Expected prompts found: {len(found_expected_prompts)}/{len(expected_prompts)}"
            )
            if missing_prompts:
    ...
    def prepare_summary(self):
        ...
        completion_percentage = (
            round((total_found / total_expected) * 100, 1) if total_expected > 0 else 0
        )
