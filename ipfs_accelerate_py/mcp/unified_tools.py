...
def register_github_tools(mcp: Any) -> None:
    ...
    mcp.register_tool(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "limit": {
                    "type": "integer",
                    "description": "Maximum number of repos",
                    "default": 30,
                },
            },
        },
    ...
    mcp.register_tool(
        ...
        input_schema={
            "type": "object",
            "properties": {"repo": {"type": "string", "description": "Repository (owner/name)"}},
            "required": ["repo"],
        },
        execution_context="server",
    )
...
def register_docker_tools(mcp: Any) -> None:
    ...
    def docker_list_containers(all_containers: bool = False) -> Dict[str, Any]:
        ...
        return {"success": result.success, "containers": result.data, "error": result.error}
    ...
    mcp.register_tool(
        ...
        input_schema={
            "type": "object",
            "properties": {"container": {"type": "string", "description": "Container ID or name"}},
            "required": ["container"],
        },
        execution_context="worker",
    )
    ...
    mcp.register_tool(
        ...
        input_schema={
            "type": "object",
            "properties": {"image": {"type": "string", "description": "Image name"}},
            "required": ["image"],
        },
        execution_context="worker",
    )
...
def register_hardware_tools(mcp: Any) -> None:
    ...
    def hardware_recommend(
        model_name: str, task: str = "inference", consider_available_only: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        result = kit.recommend_hardware(
            model_name=model_name, task=task, consider_available_only=consider_available_only
        )
        return result
...
def register_runner_tools(mcp: Any) -> None:
    ...
    def runner_provision_for_workflow(repo: str) -> Dict[str, Any]:
        ...
        if not token:
            return {"success": False, "error": "Failed to generate runner token"}
        ...
        return {
            ...
            "data": {"repo": repo, "container_id": container_id, "token_generated": bool(token)},
            "message": f"Provisioned runner for {repo}"
            if container_id
            else "Failed to launch container",
        }
...
def register_network_tools(mcp: Any) -> None:
    ...
    mcp.register_tool(
        ...
        input_schema={
            "type": "object",
            "properties": {"address": {"type": "string", "description": "Peer multiaddr or ID"}},
            "required": ["address"],
        },
    )
    ...
    mcp.register_tool(
        ...
        input_schema={
            "type": "object",
            "properties": {"peer_id": {"type": "string", "description": "Peer ID to disconnect"}},
            "required": ["peer_id"],
        },
    )
