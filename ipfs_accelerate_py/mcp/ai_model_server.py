...
class AIModelMCPServer:
    ...
    def __init__(
        ...
        self.bandit_recommender = BanditModelRecommender(
            model_manager=self.model_manager, storage_path=bandit_storage_path
        )
...
def mock_classify_text(text: str, model_id: str = "bert-base-uncased") -> Dict[str, Any]:
    ...
    return {"prediction": "POSITIVE", "confidence": 0.85, "model_used": model_id}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transport", default="stdio", choices=["stdio", "sse"], help="Transport type"
    )
    parser.add_argument(
        "--host", default="127.0.0.1", help="Host to bind to for network transports"
    )
    parser.add_argument(
        "--port", type=int, default=8000, help="Port to bind to for network transports"
    )
    parser.add_argument(
        "--model-manager-path", default="./models.db", help="Path for model manager storage"
    )
    parser.add_argument(
        "--bandit-storage-path", default="./bandit_data.json", help="Path for bandit algorithm data"
    )
    parser.add_argument(
        "--doc-index-path", default="./doc_index.json", help="Path for documentation index"
    )
    ...
    if HAVE_FASTMCP:
        anyio.run(server.run(transport=args.transport, host=args.host, port=args.port))
    else:
