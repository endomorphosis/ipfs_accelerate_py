...
def main():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for category_name, category in categories.items():
                ...
                for model in models:
                    print(
                        f"    - {model.get('name', 'Unknown')}: {model.get('description', 'No description')}"
                    )
    ...
    try:
        ...
        response = requests.post(f"{base_url}{mcp_path}/tool/test_hardware", json={})

        if response.status_code == 200:
            compatibility = response.json()
            print("✓ Hardware compatibility test completed")
            print(
                f"  - Overall compatibility: {compatibility.get('overall_compatibility', 'Unknown')}"
            )
            ...
            print(
                f"  - Memory compatibility: {compatibility.get('memory_compatibility', 'Unknown')}\n"
            )
        else:
