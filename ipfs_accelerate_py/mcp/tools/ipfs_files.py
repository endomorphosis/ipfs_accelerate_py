...
async def get_ipfs_client_async(ctx: Context) -> Any:
    ...
    try:
        ...
        if (
            hasattr(ctx.request_context, "lifespan_context")
            and hasattr(ctx.request_context.lifespan_context, "ipfs_context")
            and hasattr(ctx.request_context.lifespan_context.ipfs_context, "ipfs_client")
            and ctx.request_context.lifespan_context.ipfs_context.ipfs_client is not None
        ):
...
def register_files_tools(mcp: FastMCP) -> None:
    ...
    async def ipfs_add_file(
        path: str, ctx: Context, wrap_with_directory: bool = False
    ) -> Dict[str, Any]:
        """Add a file to IPFS.
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                ipfs.add_file, path, wrap_with_directory=wrap_with_directory
            )
    ...
    async def ipfs_cat(cid: str, ctx: Context, offset: int = 0, length: int = -1) -> str:
        ...
        try:
            ...
            content = await anyio.to_thread.run_sync(ipfs.cat, cid, offset=offset, length=length)
    ...
    async def ipfs_mkdir(path: str, ctx: Context) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"path": path, "created": False, "error": str(e)}
        ...
        try:
            ...
            try:
                ...
                if cid not in pin_ls.get("Keys", {}):
                    return {"cid": cid, "unpinned": False, "error": "Content is not pinned"}
            except Exception:
            ...
            return {"cid": cid, "unpinned": True, "recursive": recursive}
        except Exception as e:
            await ctx.error(f"Error unpinning content: {str(e)}")
            return {"cid": cid, "unpinned": False, "error": str(e)}

    @mcp.tool()
    async def ipfs_files_write(
        path: str, content: str, ctx: Context, create: bool = True, truncate: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await anyio.to_thread.run_sync(
                ipfs.files_write, path, content_bytes, create=create, truncate=truncate
            )
    ...
    async def ipfs_files_read(path: str, ctx: Context, offset: int = 0, count: int = -1) -> str:
        ...
        try:
            ...
            content = await anyio.to_thread.run_sync(
                ipfs.files_read, path, offset=offset, count=count
            )
