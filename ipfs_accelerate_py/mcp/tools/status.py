...
def register_tools(mcp):
    ...
    def get_server_status() -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Error getting server status: {str(e)}"}
    ...
    def start_session(session_name: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"id": session_id, "name": session["name"], "start_time": session["start_time"]}
        except Exception as e:
            return {"error": f"Error starting session: {str(e)}"}
    ...
    def end_session(session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_cpu_percent = (
                sum(m.get("cpu_percent", 0) for m in session["metrics"]) / len(session["metrics"])
                if session["metrics"]
                else 0
            )
            avg_memory_percent = (
                sum(m.get("memory_percent", 0) for m in session["metrics"])
                / len(session["metrics"])
                if session["metrics"]
                else 0
            )
            ...
            logger.info(
                f"Ended monitoring session {session_id}: {duration_seconds:.2f}s, {session['operations']} operations"
            )
    ...
    def log_operation(
        ...
        try:
            ...
            logger.info(
                f"Logged operation to session {session_id}: {operation_type} in {processing_time:.3f}s"
            )
            ...
            return {"error": f"Error logging operation: {str(e)}"}
    ...
    def get_session(session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Error getting session: {str(e)}"}
