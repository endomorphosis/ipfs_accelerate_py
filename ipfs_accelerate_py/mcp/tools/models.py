...
def search_models_tool(
    ...
    try:
        logger.info(
            f"Searching models: query='{query}', task={task_filter}, hardware={hardware_filter}, limit={limit}"
        )

        hub_scanner = _get_scanner()
        results = hub_scanner.search_models(
            query=query, task_filter=task_filter, hardware_filter=hardware_filter, limit=limit
        )
...
def recommend_models_tool(
    task_type: str, hardware: str = "cpu", performance: str = "balanced", limit: int = 5
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Getting recommendations: task={task_type}, hardware={hardware}, performance={performance}"
        )

        hub_scanner = _get_scanner()
        recommendations = hub_scanner.recommend_models(
            task_type=task_type, hardware=hardware, performance_preference=performance, limit=limit
        )

        return {
            "status": "success",
            "recommendations": recommendations,
            "context": {"task_type": task_type, "hardware": hardware, "performance": performance},
        }
    ...
    try:
        ...
        if hasattr(mcp, "register_tool"):
            ...
            mcp.register_tool(
                ...
                input_schema={
                    ...
                    "properties": {
                        ...
                        "codec": {"type": "string", "description": "CID codec", "default": "raw"},
                        "mh_type": {
                            "type": "string",
            ...
            def load_hf_inference_ipld_from_ipfs(
                cid: str, backend: Optional[str] = None
            ) -> Dict[str, Any]:
