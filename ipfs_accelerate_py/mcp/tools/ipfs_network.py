...
.. deprecated::
    ...
    ``ipfs_accelerate_py.mcp_server.tools.ipfs_network_tools``.  Import from the canonical module instead.
...
def register_network_tools(mcp: FastMCP) -> None:
    ...
    warnings.warn(
        "ipfs_accelerate_py.mcp.tools.ipfs_network.register_network_tools is deprecated. "
        "Use ipfs_accelerate_py.mcp_server.tools.ipfs_network_tools instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    async def ipfs_swarm_connect(addr: str, ctx: Context) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "address": addr, "error": str(e)}
    ...
    async def ipfs_pubsub_pub(topic: str, message: str, ctx: Context) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "topic": topic, "message_length": len(message_bytes)}
        except Exception as e:
        ...
        try:
            ...
            for addr in result.get("Responses", []):
                for address in addr.get("Addrs", []):
                    addresses.append({"peer_id": peer_id, "address": address})
    ...
    @mcp.tool()
    async def ipfs_dht_findprovs(
        cid: str, ctx: Context, num_providers: int = 20
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = await anyio.to_thread.run_sync(
                ipfs.dht_findprovs, cid, num_providers=num_providers
            )

            # Format the providers
            providers = []
            for response in result.get("Responses", []):
                provider = {"id": response.get("ID", ""), "addresses": response.get("Addrs", [])}
