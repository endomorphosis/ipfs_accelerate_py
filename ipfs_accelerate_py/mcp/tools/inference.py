    ...
    if metadata["provenance_cid"] is None and _provenance_logger is not None:
        try:
            metadata["provenance_cid"] = _provenance_logger.log_inference(
                ...
                metadata={"endpoint_id": endpoint_id, "provider": provider}
                if endpoint_id or provider
                else None,
            )
        except Exception as exc:
...
def register_tools(mcp):
        ...
        try:
            ...
            if _ipfs_instance is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"IPFS Accelerate inference failed: {e}. Falling back to MCP implementation."
                    )
            ...
            if not model_info:
                return {"error": f"Model '{model}' not found. Please check the available models."}
            ...
            if provider:
                ...
                try:
                    ...
                    logger.warning(
                        f"Provider routing failed ({provider}): {e}. Falling back to local simulation."
                    )
    ...
    def get_model_list() -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Error getting model list: {str(e)}"}
        ...
        try:
            ...
            if _ipfs_instance is None:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to import ipfs_accelerate_py - falling back to simulation"
                    )
                    ...
                    return {
                        ...
                        "outputs": [
                            f"Simulated distributed inference output for input: {input[:50]}..."
                            for input in inputs
                        ],
                        "elapsed_time": 1.5,
            ...
            # Log the request
            logger.info(
                f"Running distributed inference on model {model} with strategy {sharding_strategy}"
            )
            ...
            if hasattr(_ipfs_instance, "run_distributed_inference"):
                ...
                logger.warning(
                    "IPFS Accelerate instance does not support distributed inference - simulating"
                )
                outputs = [
                    f"Simulated distributed inference output for input: {input[:50]}..."
                    for input in inputs
        ...
        try:
            ...
            if hasattr(_ipfs_instance, "get_distributed_capabilities"):
                ...
                return {
                    ...
                    "supported_strategies": capabilities.get(
                        "supported_strategies", ["tensor_parallel", "pipeline_parallel"]
                    ),
                ...
                if "distributed" in system_info:
                    return {
                        "max_model_size_b": system_info["distributed"].get(
                            "max_model_size_b", 70_000_000_000
                        ),
                        "supported_strategies": system_info["distributed"].get(
                            "supported_strategies", ["tensor_parallel", "pipeline_parallel"]
                        ),
            ...
            # Fallback to simulation
            logger.warning(
                "IPFS Accelerate instance does not provide distributed capabilities - simulating"
            )
            return {
