...
QUEUE_MONITOR = {
    "global_queue": [],
    "endpoint_queues": {},
    "stats": {"total_tasks": 0, "completed_tasks": 0, "failed_tasks": 0, "pending_tasks": 0},
    "endpoint_stats": {},
}
...
def register_tools(mcp):
        ...
        try:
            ...
            for attempt in range(max_retries):
                for preference in model_preferences:
                    provider, model = (
                        preference.split("/", 1) if "/" in preference else ("local", preference)
                    )
    ...
    def register_endpoint(
        ...
        **config,
        ...
        try:
            ...
            return {"status": "error", "error": f"Failed to register endpoint: {str(e)}"}
    ...
    def get_endpoint_status(endpoint_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if endpoint_id:
                ...
                return {"endpoint": ENDPOINT_REGISTRY[endpoint_id], "status": "success"}
            else:
                return {
            ...
            return {"error": f"Failed to get endpoint status: {str(e)}", "status": "error"}
    ...
    def configure_api_provider(
        ...
        try:
            ...
            return {"error": f"Failed to configure provider: {str(e)}", "status": "error"}

    @mcp.tool()
    def search_huggingface_models(
        query: str, task: Optional[str] = None, sort: str = "downloads", limit: int = 10
    ) -> Dict[str, Any]:
        """
    ...
    def get_queue_status() -> Dict[str, Any]:
        ...
        try:
            ...
            if HAVE_CLI_ADAPTERS:
                for endpoint_id, adapter in CLI_ADAPTER_REGISTRY.items():
                    ...
                    endpoint_queues[endpoint_id] = {
                        ...
                        "success_rate": (
                            stats["stats"].get("successes", 0)
                            / stats["stats"].get("requests", 1)
                            * 100
                        )
                        if stats["stats"].get("requests", 0) > 0
                        else 0,
                        "current_task": None,
                    }
            ...
            queue_status = {
                "global_queue": {
                    "total_tasks": QUEUE_MONITOR["stats"]["total_tasks"]
                    + len(QUEUE_MONITOR["global_queue"]),
            ...
            return {"error": f"Failed to get queue status: {str(e)}", "status": "error"}
    ...
    def get_queue_history() -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Failed to get queue history: {str(e)}", "status": "error"}
    ...
    if HAVE_CLI_ADAPTERS:
        ...
        def register_cli_endpoint_tool(
            ...
            **IMPORTANT**: Before registering a CLI endpoint, ensure:
            ...
            try:
                ...
                return {"error": f"Failed to register CLI endpoint: {str(e)}", "status": "error"}
        ...
        def list_cli_endpoints_tool() -> Dict[str, Any]:
            ...
            try:
                ...
                return {"endpoints": endpoints, "count": len(endpoints), "status": "success"}
            except Exception as e:
                logger.error(f"Failed to list CLI endpoints: {e}")
                return {"error": f"Failed to list CLI endpoints: {str(e)}", "status": "error"}
        ...
        def cli_inference(
            ...
            **IMPORTANT**: Before running inference:
            ...
            try:
                ...
                QUEUE_MONITOR["stats"]["failed_tasks"] += 1
                return {"error": f"CLI inference failed: {str(e)}", "status": "error"}
        ...
        def get_cli_providers() -> Dict[str, Any]:
            ...
            """
            return {"providers": CLI_PROVIDERS, "status": "success"}
        ...
        def get_cli_config(cli_type: str) -> Dict[str, Any]:
            ...
            **Use this before registering a CLI endpoint** to understand configuration requirements.
            ...
            try:
                ...
                return {"error": f"Failed to get CLI config: {str(e)}", "status": "error"}
        ...
        def get_cli_install(cli_type: str) -> Dict[str, Any]:
            ...
            **Use this before registering a CLI endpoint** to install the required tool.
            ...
            try:
                ...
                return {"error": f"Failed to get CLI install info: {str(e)}", "status": "error"}
        ...
        def validate_cli_config(endpoint_id: str) -> Dict[str, Any]:
            ...
            try:
                ...
                return {"error": f"Validation failed: {str(e)}", "status": "error"}
        ...
        def get_cli_capabilities(endpoint_id: str) -> Dict[str, Any]:
            ...
            try:
                ...
                return {"error": f"Failed to get capabilities: {str(e)}", "status": "error"}
...
def _run_api_inference(
    provider: str, model: str, prompt: str, task_type: str, **kwargs
) -> Dict[str, Any]:
    ...
    if provider == "openai":
        return {
            "choices": [
                {"message": {"content": f"[OpenAI {model}] Response to: {prompt[:50]}..."}}
            ],
            "model": model,
            "provider": "openai",
        }
        ...
        return {
            "content": [{"text": f"[Anthropic {model}] Response to: {prompt[:50]}..."}],
            "model": model,
            "provider": "anthropic",
        }
