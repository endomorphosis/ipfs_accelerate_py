...
def list_inference_backends(
    backend_type: Optional[str] = None, status: Optional[str] = None, task: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if not HAVE_BACKEND_MANAGER:
        return {"error": "Backend manager not available", "backends": []}
    ...
    try:
        ...
        backends = manager.list_backends(
            backend_type=backend_type_filter, status=status_filter, task=task
        )
            ...
            "backends": [
                {
                    ...
                    "supported_models": list(b.capabilities.supported_models)
                    if b.capabilities.supported_models
                    else None,
                    "protocols": list(b.capabilities.protocols),
                    "hardware_types": list(b.capabilities.hardware_types)
                    if b.capabilities.hardware_types
                    else None,
                    "metrics": {
...
def get_backend_status() -> Dict[str, Any]:
    ...
    """
    if not HAVE_BACKEND_MANAGER:
        return {"error": "Backend manager not available", "status": "unavailable"}
...
def select_backend_for_inference(
    ...
    try:
        ...
        if preferred_types:
            try:
                preferred_backend_types = [BackendType(t.lower()) for t in preferred_types]
        ...
        return {
            ...
            "capabilities": {
                ...
                "models": list(backend.capabilities.supported_models)
                if backend.capabilities.supported_models
                else None,
...
def route_inference_request(
    ...
    try:
        ...
        if backend_id:
            ...
            if not backend:
                return {"error": f"Backend not found: {backend_id}"}
        else:
            backend = manager.select_backend_for_task(task, model)
            if not backend:
                return {"error": f"No suitable backend found for task: {task}"}
...
def get_supported_tasks() -> Dict[str, Any]:
    ...
    """
    if not HAVE_BACKEND_MANAGER:
        return {"error": "Backend manager not available", "tasks": []}
        ...
        for backend in status["backends"]:
            for task in backend["tasks"]:
                if task not in task_backends:
                    task_backends[task] = {"backend_count": 0, "backends": []}
                task_backends[task]["backend_count"] += 1
                task_backends[task]["backends"].append(
                    {"id": backend["id"], "name": backend["name"], "type": backend["type"]}
                )

        return {"total_tasks": len(task_backends), "tasks": task_backends, "timestamp": time.time()}
