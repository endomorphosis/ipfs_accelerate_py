...
def get_basic_hardware_info(include_detailed: bool = False) -> Dict[str, Any]:
    ...
    cpu_info = {"available": True, "name": platform.processor()}
        ...
        try:
            ...
            if hasattr(ipfs_accelerate_py, "hardware_testing"):
                test_results = ipfs_accelerate_py.hardware_testing.test_hardware(
                    accelerator=accelerator, test_level=test_level
                )
...
def perform_basic_hardware_test(
    ...
    Returns:
        Dictionary with test results
    """
    results = {"test_time": "", "test_level": test_level, "results": {}}
    ...
    if accelerator in ["webgpu", "all"]:
        results["results"]["webgpu"] = {
            "status": "unavailable",
            "details": {"reason": "WebGPU can only be tested in a browser environment"},
        }
    ...
    if accelerator in ["webnn", "all"]:
        results["results"]["webnn"] = {
            "status": "unavailable",
            "details": {"reason": "WebNN can only be tested in a browser environment"},
        }
...
def recommend_hardware(
    ...
    try:
        ...
        recommendations = get_basic_hardware_recommendations(
            model_name, task, consider_available_only
        )
...
def get_basic_hardware_recommendations(
    ...
    if task == "inference":
        ...
        if cuda_available and (not consider_available_only or cuda_available):
            ...
            primary_recommendation = "cpu"
            reason = (
                "CPU is the only available option for training (not recommended for large models)"
            )
            settings = {"batch_size": 1, "dtype": "float32", "gradient_accumulation_steps": 16}
