...
def parse_args(args: List[str]) -> argparse.Namespace:
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="IPFS Accelerate MCP Server", prog="python -m mcp")
    ...
    run_parser.add_argument(
        "--host", default="127.0.0.1", help="Host to listen on (for ws and sse transports)"
    )
    run_parser.add_argument(
        "--port", type=int, default=8000, help="Port to listen on (for ws and sse transports)"
    )
