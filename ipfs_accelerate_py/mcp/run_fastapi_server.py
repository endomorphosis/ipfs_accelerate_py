...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Could not import create_standalone_app from ipfs_accelerate_py.mcp.integration. Please check your installation."
            )
            sys.exit(1)
        ...
        logger.info(
            f"API documentation available at http://{args.host}:{args.port}{args.mount_path}/docs"
        )

        run_standalone_app(app=app, host=args.host, port=args.port, verbose=args.verbose)
