...
class CaselawGraphRAGProcessor:
    ...
    def __init__(self):
        ...
        self.knowledge_graph = {"nodes": {}, "edges": [], "node_types": set(), "edge_types": set()}
    ...
    def process_cases(self, dataset_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"GraphRAG processing complete: {stats['total_nodes']} nodes, {stats['total_edges']} edges"
        )
    ...
    def _add_edge(
        self, source: str, target: str, relationship: str, properties: Optional[Dict] = None
    ) -> None:
    ...
    def _detect_case_relationships(self, case: Dict[str, Any]) -> None:
        ...
        for existing_case_id, existing_case in self.knowledge_graph["nodes"].items():
            ...
            if abs(current_year - existing_year) <= 5 and case.get("topic") == existing_case.get(
                "topic"
            ):
...
class LegalEntityExtractor:
    ...
    def extract_entities(self, case: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if case.get("court"):
            entities.append(
                {"type": "Court", "text": case["court"], "canonical_form": case["court"]}
            )

        # Add topic as legal doctrine
        if case.get("topic"):
            entities.append(
                {"type": "Legal_Doctrine", "text": case["topic"], "canonical_form": case["topic"]}
            )
        ...
        for entity_type, pattern in self.patterns.items():
            ...
            for match in matches:
                entities.append(
                    {"type": entity_type, "text": match, "canonical_form": match.strip()}
                )
...
class LegalRelationshipDetector:
    ...
    def detect_relationships(self, case1: Dict[str, Any], case2: Dict[str, Any]) -> List[str]:
        ...
        for rel_type, pattern in self.relationship_patterns.items():
            ...
            if case2.get("title", "").lower() in text1.lower() and re.search(
                pattern, text1, re.IGNORECASE
            ):
