...
class cuda_utils:
    ...
    def benchmark_cuda_inference(self, model, inputs, iterations=5, device=None):
        ...
        try:
            with self.torch.no_grad():
                if hasattr(model, "generate") and callable(model.generate):
                    _ = model.generate(
                        **cuda_inputs if isinstance(cuda_inputs, dict) else cuda_inputs
                    )
                else:
                    _ = model(cuda_inputs)
        ...
        return {
            ...
            "cuda_device": self.torch.cuda.get_device_name(device)
    ...
    def get_implementation_type(self, endpoint=None, tokenizer=None, prefix="", real_impl=None):
        ...
        is_mock = (
            isinstance(endpoint, MagicMock)
            or isinstance(tokenizer, MagicMock)
            or not self.torch.cuda.is_available()
        )
