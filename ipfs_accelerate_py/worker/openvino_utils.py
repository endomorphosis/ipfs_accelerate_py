...
class openvino_utils:
    ...
    def get_openvino_model(self, model_name, model_type=None, device_name=None):
        ...
        huggingface_cache_models_files_dirs_models_model_name = [
            x for x in huggingface_cache_models_files_dirs_models if model_name_convert in x
        ]
        model_src_path = os.path.join(
            huggingface_cache_models, huggingface_cache_models_files_dirs_models_model_name[0]
        )
        ...
        if os.path.exists(model_src_path) and not os.path.exists(model_dst_path):
            try:
                hftokenizer = self.transformers.AutoTokenizer.from_pretrained(
                    model_name, trust_remote_code=True
                )
            except Exception as e:
                print(e)
                hftokenizer = None
            try:
                hfprocessor = self.transformers.AutoProcessor.from_pretrained(
                    model_name, trust_remote_code=True
                )
            except Exception as e:
                print(e)
                hfprocessor = None
            try:
                hfmodel = self.transformers.AutoModel.from_pretrained(
                    model_name, trust_remote_code=True
                ).to("cpu")
            except Exception as e:
                print(e)
            ...
            if hfmodel is not None and "config" in list(dir(hfmodel)):
                if ov_model is None and model_type in genai_model_types:
                    if hfprocessor is not None:
                        ...
                        try:
                            convert = this_hf.openvino_skill_convert(
                                model_name, model_dst_path, model_task, weight_format
                            )
                        except Exception as e:
                            print(e)
                            try:
                                convert = self.openvino_cli_convert(
                                    model_name,
        ...
        if os.path.exists(model_dst_path):
            if model_type == "t5" and model_task == "text2text-generation-with-past":
                ov_model = core.read_model(
                    os.path.join(model_dst_path, "openvino_decoder_with_past_model.xml")
                )
                ov_model = core.compile_model(ov_model)
            elif model_type == "whisper" and model_task == "automatic-speech-recognition":
                ov_model = core.read_model(
                    os.path.join(model_dst_path, model_name.replace("/", "--") + ".xml")
                )
                ov_model = core.compile_model(ov_model)
            else:
                ov_model = core.read_model(
    ...
    def get_optimum_openvino_model(self, model_name, model_type=None, openvino_label=None):
        ...
        if model_type == "bert":
            ...
            try:
                results = OVModelForSequenceClassification.from_pretrained(
                    model_name, compile=False
                )
            except Exception as e:
                results = OVModelForSequenceClassification.from_pretrained(
                    model_dst_path, compile=False
                )
            ...
            try:
                ...
                results = OVModelForTokenClassification.from_pretrained(
                    model_dst_path, compile=False
                )
            ...
            try:
                ...
                results = OVModelForAudioClassification.from_pretrained(
                    model_dst_path, compile=False
                )
            ...
            try:
                ...
                results = OVModelForImageClassification.from_pretrained(
                    model_dst_path, compile=False
                )
    ...
    def get_openvino_pipeline_type(self, model_name, model_type=None):
        ...
        if model_type is not None:
            ...
            if config_model_type is None:
                config_model_type = (
                    config_model_type if config_model_type is not None else model_type
                )
