                ...
                if local > 0 and cpus > 0:
                    if openvino_test and type(openvino_test) != ValueError and model_type != "llama_cpp":
                        ...
                        for openvino_endpoint in openvino_local_endpont_types:
                            ...
                            torch.cuda.empty_cache()
            elif model_type in wav2vec_types:
                if cuda and gpus > 0:
                    if cuda_test and type(cuda_test) != ValueError:
                        for gpu in range(gpus):
                            device = 'cuda:' + str(gpu)
                            cuda_label = device
                            self.local_endpoints[model][cuda_label], self.tokenizer[model][cuda_label], self.endpoint_handler[model][cuda_label], self.queues[model][cuda_label], self.batch_sizes[model][cuda_label] = self.hf_wav2vec.init_cuda( model, device, cuda_label)
                            torch.cuda.empty_cache()
                if local > 0 and cpus > 0:
                    if openvino_test and type(openvino_test) != ValueError and model_type != "llama_cpp":
                        openvino_local_endpont_types = [ x for x in local_endpoint_types if "openvino" in x]
                        for openvino_endpoint in openvino_local_endpont_types:
                            ...
                            openvino_label = "openvino:" + str(ov_count)
                            device = "openvino:" + str(ov_count)
                            self.local_endpoints[model][openvino_label], self.tokenizer[model][openvino_label], self.endpoint_handler[model][openvino_label], self.queues[model][openvino_label], self.batch_sizes[model][openvino_label] = self.hf_wav2vec.init_openvino(
                                model,
                                model_type,
                                device,
                                openvino_label,
                ...
                if local > 0 and cpus > 0:
                    if openvino_test and type(openvino_test) != ValueError and model_type != "llama_cpp":
                        ...
                        for openvino_endpoint in openvino_local_endpont_types:
                            ...
                            torch.cuda.empty_cache()
            elif model_type in whisper_types:
                if cuda and gpus > 0:
                    if cuda_test and type(cuda_test) != ValueError:
                        for gpu in range(gpus):
                            device = 'cuda:' + str(gpu)
