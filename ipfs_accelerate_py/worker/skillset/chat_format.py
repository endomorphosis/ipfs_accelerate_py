...
class chat_format:
    def __init__(self, resources, meta=None):
        ...
        if self._storage:
            try:
                content = self._storage.read_file(templates_path)
                if content:
                    content_str = content if isinstance(content, str) else content.decode()
                    self.templates = yaml.load(content_str, Loader=yaml.Loader)
                    return
            except Exception:
                pass
    ...
    def format_chat_prompt(self, template, messages):
        ...
        final_prompt = (template["round_seperator"] if "round_seperator" in template else "").join(
            blocks
        )
        ...
        if type(messages) == str:
            if "{user" in template["with_system"] or "{assistant" in template["with_system"]:
                if "{" + "system}" in template["with_system"]:
                    index_system_end = template["with_system"].index("{" + "system}") + len(
                        "{" + "system}"
                    )
                    system_prefix = template["with_system"][:index_system_end].split(
                        "{" + "system}"
                    )[0]
                    index_system_prefix_end = template["with_system"].index(system_prefix)

                if (
                    "{" + "user}" in template["with_system"]
                    or "assistant}" in template["with_system"]
                ):
                    index_user_end = template["with_system"].index("{" + "user}")
                    user_prefix = (
                        template["with_system"][index_system_end:index_user_end]
                        .split("{" + "user}")[0]
                        .replace(template["round_seperator"], "")
                    )
                    index_assistant_end = template["with_system"].index("{" + "assistant}") + len(
                        "{" + "assistant}"
                    )
                    assistant_suffix = template["with_system"][index_assistant_end:]
                    assistant_prefix = (
                        template["with_system"][index_user_end:index_assistant_end]
                        .split("{" + "assistant}")[0]
                        .replace(template["round_seperator"], "")
                        .replace("{" + "user}", "")
                        .replace(assistant_suffix, "")
                    )
                    user_suffix = (
                        template["with_system"][index_user_end:index_assistant_end]
                        .replace("{" + "user}", "")
                        .replace(template["round_seperator"], "")
                        .replace(assistant_prefix, "")
                        .replace("{" + "assistant}", "")
                    )
                    user_suffix = (
                        template["with_system"][
                            index_user_end : index_assistant_end
                    ...
                    assistant_prefix = assistant_prefix.replace(user_suffix, "").replace(
                        "{" + "user}", ""
                    )
                ...
                if "system}" in template["with_system"]:
                    system_suffix = (
                        template["with_system"][
                            index_system_prefix_end : index_user_end - len(user_prefix)
                        ]
                        .replace("{" + "system}", "")
                        .replace(template["round_seperator"], "")
                    )
                ...
                system_exists = (
                    messages.find(system_prefix) != -1
                    and messages.find(system_suffix) != -1
                    and messages.find(user_prefix) != 0
                    and messages.find(assistant_prefix) != 0
                )

                if system_exists:
                    system_message_index = messages.find(system_prefix)
                    system = messages[
                        messages.find(system_prefix) + len(system_prefix) : messages.find(
                            system_suffix
                        )
                    ]
                    system_message = messages.replace(system_prefix, "").replace(system_suffix, "")
                    messages = (
                        messages.replace(system_prefix, "")
                        .replace(system_suffix, "")
                ...
                for message in message_list:
                    if (
                        message.find(user_prefix) != -1
                        and message.find(user_suffix) != -1
                        and message.find("</s>") != -1
                    ):
                        user_messages.append(
                            message[
                                message.find(user_prefix) + len(user_prefix) : message.find(
                                    user_suffix
                                )
                            ]
                        )
                    if (
                        message.find(assistant_prefix) != -1
                        and message.find(assistant_suffix) != -1
                        and message.find("</s>") != -1
                    ):
                        assistant_messages.append(
                            message[
                                message.find(assistant_prefix)
                                + len(assistant_prefix) : message.find(assistant_suffix)
                            ]
                        )
                ...
                first_message = (
                    ...
                    if messages.find(user_prefix) == 0
                    else "assistant"
                    if messages.find(assistant_prefix) == 0
                    else None
                )
