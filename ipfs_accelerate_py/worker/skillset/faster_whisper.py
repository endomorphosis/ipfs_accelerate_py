...
class hf_faster_whisper:
    ...
    def runWhisper(self, audio, **kwargs):
        ...
        if os.path.isfile(data):
            ...
            with tempfile.NamedTemporaryFile(suffix=".ogg") as temp_audio:
                audio_segment = AudioSegment.from_file(
                    io.BytesIO(self.header + audio_bytes), format="webm"
                ).export(temp_audio.name, format="ogg")
        ...
        if fragment != None:
            if type(fragment) == str:
                ...
                for token in encodeed_text:
                    self.process_token(
                        token, self.tokens, self.sentences, self.add_sentence_to_list
                    )
        ...
        for segment in segments:
            ...
            for token in encodeed_text:
                this_result = self.process_token(
                    token, self.tokens, self.sentences, self.add_sentence_to_list
                )
        ...
        self.sentences = []
        self.tokens = []
        return {"text": result_sentences, "timestamp": timestamp, "done": True}
