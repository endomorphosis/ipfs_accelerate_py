...
class hf_bert:
    ...
    def __init__(self, resources=None, metadata=None):
        ...
        self.create_apple_text_embedding_endpoint_handler = (
            self.create_apple_text_embedding_endpoint_handler
        )
    ...
    def _create_mock_processor(self):
        ...
        try:
            ...
            def mock_tokenize(
                text, return_tensors="pt", padding=None, truncation=None, max_length=None
            ):
            ...
            class SimpleTokenizer:
                ...
                def __call__(
                    self, text, return_tensors="pt", padding=None, truncation=None, max_length=None
                ):
                    if isinstance(text, str):
                        batch_size = 1
                    else:
    ...
    def _create_mock_endpoint(self, model_name, device_label):
        ...
        try:
            ...
            def mock_forward(**kwargs):
                ...
                sequence_length = kwargs.get("input_ids", kwargs.get("inputs_embeds", None)).shape[
                    1
                ]
    ...
    def init_cpu(self, model_name, device, cpu_label):
        ...
        try:
            ...
            if "transformers" in self.resources and hasattr(
                self.resources["transformers"], "AutoModel"
            ):
                ...
                try:
                    ...
                    print(
                        f"Model type: {config.model_type if hasattr(config, 'model_type') else 'bert'}"
                    )
        ...
        try:
            ...
            if cuda_utils_available and hasattr(self.torch.cuda, "mem_get_info"):
                try:
                    ...
                    batch_size = max(
                        1, min(16, int(free_memory_gb / 0.5))
                    )  # Heuristic: 0.5GB per batch item
            ...
            try:
                ...
                if cuda_utils_available:
                    endpoint = cuda_tools.optimize_cuda_memory(
                        model=endpoint, device=device, use_half_precision=use_half_precision
                    )
                else:
                    # Manual optimization
        ...
        try:
            ...
            if model is None and callable(get_optimum_openvino_model):
                try:
                    ...
                    print(
                        f"(REAL) Successfully loaded OpenVINO model with get_optimum_openvino_model"
                    )
                except Exception as e:
    ...
    def init_apple(self, model, device, apple_label):
        ...
        endpoint_handler = self.create_apple_text_embedding_endpoint_handler(
            endpoint, apple_label, endpoint, tokenizer
        )
    ...
    def init_qualcomm(self, model, device, qualcomm_label):
        ...
        try:
            ...
            endpoint_handler = self.create_qualcomm_text_embedding_endpoint_handler(
                endpoint, qualcomm_label, endpoint, tokenizer
            )
    ...
    def create_cpu_text_embedding_endpoint_handler(
        self, endpoint_model, device, hardware_label, endpoint=None, tokenizer=None
    ):
            ...
            try:
                with self.torch.no_grad():
                    ...
                    masked_hidden = last_hidden.masked_fill(
                        ~tokens["attention_mask"].bool().unsqueeze(-1), 0.0
                    )

                    # Sum and divide by actual token count (excluding padding)
                    average_pool_results = masked_hidden.sum(dim=1) / tokens["attention_mask"].sum(
                        dim=1, keepdim=True
                    )
        ...
        def handler(
            ...
            try:
                ...
                if endpoint is None or not (
                    hasattr(endpoint, "__call__") or hasattr(endpoint, "infer")
                ):
                ...
                try:
                    ...
                    if "attention_mask" in tokens:
                        ...
                        attention_mask = self.torch.ones(
                            last_hidden.shape[:2], dtype=self.torch.bool
                        )
                    ...
                    # Sum and divide by actual token count (excluding padding)
                    average_pool_results = masked_hidden.sum(dim=1) / attention_mask.sum(
                        dim=1, keepdim=True
                    )
                ...
                with self.torch.no_grad():
                    ...
                    if hasattr(self.torch.cuda, "mem_get_info"):
                        try:
                            ...
                            print(
                                f"CUDA memory available before processing: {free_memory_start_gb:.2f}GB"
                            )
                        except Exception as mem_error:
                    ...
                    if hasattr(endpoint, "config") and hasattr(
                        endpoint.config, "max_position_embeddings"
                    ):
                        ...
                        if len(text_input) > batch_size and cuda_utils_available:
                            ...
                            for i, batch in enumerate(batches):
                                ...
                                if "token_type_ids" in batch_tokens:
                                    model_inputs["token_type_ids"] = batch_tokens[
                                        "token_type_ids"
                                    ].to(cuda_device)
                                ...
                                if hasattr(outputs, "last_hidden_state"):
                                    ...
                                    pooled_embeddings = masked_hidden.sum(
                                        dim=1
                                ...
                                if hasattr(self.torch, "cuda") and hasattr(
                                    self.torch.cuda, "empty_cache"
                                ):
                                    self.torch.cuda.empty_cache()
                    ...
                    if hasattr(outputs, "last_hidden_state"):
                        ...
                        # Mean pooling for sentence embeddings
                        pooled_embeddings = masked_hidden.sum(dim=1) / attention_mask.sum(
                            dim=1, keepdim=True
                        )
    ...
    def create_apple_text_embedding_endpoint_handler(
        self, endpoint_model, apple_label, endpoint=None, tokenizer=None
    ):
            ...
            try:
                with self.torch.no_grad():
                    ...
                    if "last_hidden_state" in outputs:
                        ...
                        result = self.torch.tensor(
                            outputs.get("pooler_output", outputs.get("embeddings", None))
                        )
    ...
    def create_qualcomm_text_embedding_endpoint_handler(
        self, endpoint_model, qualcomm_label, endpoint=None, tokenizer=None
    ):
