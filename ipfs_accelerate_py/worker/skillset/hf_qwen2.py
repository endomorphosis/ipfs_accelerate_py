...
class hf_qwen2:
    ...
    def init_cpu(self, model, device, cpu_label):
        ...
        try:
            ...
            endpoint_handler = self.create_cpu_llm_endpoint_handler(
    ...
    def init_qualcomm(self, model, model_type, device, qualcomm_label):
        ...
        try:
            ...
            endpoint_handler = self.create_qualcomm_llm_endpoint_handler(
    ...
    def init_cuda(self, model, device, cuda_label):
        ...
        try:
            ...
            print(e)
            pass
        endpoint_handler = self.create_cuda_llm_endpoint_handler(
    ...
    def init_openvino(
        ...
        endpoint_handler = self.create_openvino_llm_endpoint_handler(
    ...
    def create_cpu_llm_endpoint_handler(
            ...
            with self.torch.no_grad():
                try:
                    ...
                    config = self.transformers.AutoConfig.from_pretrained(
                        endpoint_model, trust_remote_code=True
                    )
    ...
    def create_apple_llm_endpoint_handler(
        self, local_endpoint, local_processor, endpoint_model, apple_label
    ):
        def handler(
            x,
            y=None,
    ...
    def create_qualcomm_llm_endpoint_handler(
        self, local_endpoint, local_processor, endpoint_model, qualcomm_label
    ):
        def handler(
            x,
            y=None,
    ...
    def create_openvino_llm_endpoint_handler(
    ...
    def create_cuda_llm_endpoint_handler(
        self, local_cuda_endpoint, local_cuda_processor, endpoint_model, cuda_label
    ):
        def handler(
            x,
            ...
            with self.torch.no_grad():
                try:
                    ...
                    inputs = local_cuda_processor(prompt, return_tensors="pt").to(
                        cuda_label, self.torch.float16
                    )
                    output = local_cuda_endpoint.generate(**inputs, max_new_tokens=30)
                    result = local_cuda_processor.batch_decode(
                        output, skip_special_tokens=True, clean_up_tokenization_spaces=False
    ...
    def init_qualcomm(self, model, device, qualcomm_label):
        ...
        try:
            ...
            endpoint_handler = self.create_qualcomm_qwen2_endpoint_handler(
    ...
    def create_qualcomm_qwen2_endpoint_handler(
        ...
        def handler(
            ...
            try:
                ...
                if isinstance(text_input, str):
                    ...
                    inputs = {
                        k: v.numpy() if hasattr(v, "numpy") else v for k, v in text_input.items()
                    }
                ...
                # Return result
                return {"generated_text": decoded_output, "model": endpoint_model}
