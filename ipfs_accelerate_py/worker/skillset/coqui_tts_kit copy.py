...
class coqui_tts_kit:
    def __init__(self, resources, meta=None):
        ...
        if os.path.exists(self.custom_model_path) and os.path.isfile(
            self.custom_model_path + "/config.json"
        ):
            ...
            if self.custom_model_path != "":
                try:
                    ...
                    model_path = os.path.join(
                        get_user_data_dir("tts"), self.custom_model_path.replace("/", "--")
                    )
    ...
    def predict_speaker(self, wav_file):
        ...
        with tempfile.NamedTemporaryFile(suffix=".ogg", dir="/tmp") as temp_audio:
            audio_segment = AudioSegment.from_file(io.BytesIO(audio_bytes), format="webm").export(
                temp_audio.name, format="ogg"
            )
    ...
    def get_speakers(self):
        if hasattr(self.model, "speaker_manager") and hasattr(
            self.model.speaker_manager, "speakers"
        ):
            return {
                speaker: {
                    "speaker_embedding": self.model.speaker_manager.speakers[speaker][
                        "speaker_embedding"
                    ]
                    .cpu()
                    .squeeze()
                    .half()
                    .tolist(),
                    "gpt_cond_latent": self.model.speaker_manager.speakers[speaker][
                        "gpt_cond_latent"
                    ]
                    .cpu()
                    .squeeze()
    ...
    def stream_tts(self, input, **kwargs):
        ...
        for chunk in results:
            ...
            if stream:
                yield {"audio": audio, "done": False}

        yield {"audio": "" if stream else final_result, "done": True}
