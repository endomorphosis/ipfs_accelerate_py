...
class hf_llava_next:
    ...
    def __init__(self, resources=None, metadata=None):
        ...
        self.create_cpu_multimodal_endpoint_handler = (
            self.create_cpu_multimodal_endpoint_handler
            if hasattr(self, "create_cpu_multimodal_endpoint_handler")
            else None
        )
        self.create_cuda_multimodal_endpoint_handler = (
            self.create_cuda_multimodal_endpoint_handler
            if hasattr(self, "create_cuda_multimodal_endpoint_handler")
            else None
        )
        self.create_openvino_multimodal_endpoint_handler = (
            self.create_openvino_multimodal_endpoint_handler
            if hasattr(self, "create_openvino_multimodal_endpoint_handler")
            else None
        )
        self.create_apple_multimodal_endpoint_handler = (
            self.create_apple_multimodal_endpoint_handler
            if hasattr(self, "create_apple_multimodal_endpoint_handler")
            else None
        )
        self.create_qualcomm_multimodal_endpoint_handler = (
            self.create_qualcomm_multimodal_endpoint_handler
            if hasattr(self, "create_qualcomm_multimodal_endpoint_handler")
            else None
        )
    ...
    def get_cuda_device(self, device_label="cuda:0"):
        ...
        try:
            ...
            if device_index >= cuda_device_count:
                print(
                    f"Warning: CUDA device index {device_index} out of range (0-{cuda_device_count - 1}), using device 0"
                )
                device_index = 0
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.transformers, "LlavaForConditionalGeneration"):
                    ...
                    print(
                        "LlavaForConditionalGeneration not available, using AutoModelForVision2Seq"
                    )
                ...
                handler = self.create_cuda_multimodal_endpoint_handler(
                    model, processor, model_name, device_label
                )
    ...
    def init_apple(self, model, device, apple_label):
        ...
        try:
            ...
            endpoint_handler = self.create_apple_multimodal_endpoint_handler(
            ...
            return (
                ...
                processor,
                endpoint_handler,
            )  # TODO: Replace with anyio.create_memory_object_stream - AnyioQueue(32), 0
        except Exception as e:
            ...
            try:
                ...
                if image is not None:
                    ...
                    if isinstance(image, list):
                        ...
                        try:
                            ...
                            print(
                                f"Preprocessed {len(pil_images)} images in {preprocess_time:.2f}s"
                            )
                ...
                try:
                    ...
                    result = {
                        ...
                        "metrics": {
                            "tokens_per_second": max_new_tokens / generate_time
                            if generate_time > 0
                            else 0,
                        },
                    }
    ...
    def create_apple_multimodal_endpoint_handler(
            ...
            try:
                ...
                if isinstance(x, str) and y is not None:
                    ...
                    if isinstance(y, str):
                        # Load image
                        image = load_image(y)
                        inputs = processor(text=x, images=image, return_tensors="np", padding=True)
                    elif isinstance(y, list):
                        # Handle multiple images
                        images = [load_image(img_path) for img_path in y]
                        inputs = processor(
                            text=[x] * len(images), images=images, return_tensors="np", padding=True
                        )
                else:
                    inputs = x
