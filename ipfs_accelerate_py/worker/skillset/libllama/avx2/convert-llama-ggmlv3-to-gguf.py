...
class Hyperparameters:
    def __init__(self):
        self.n_vocab = self.n_embd = self.n_mult = self.n_head = self.n_layer = self.n_rot = (
            self.ftype
        ) = 0
        self.n_ff = 0
...
class GGMLV3Model:
    ...
    def validate_header(self, data, offset):
        if (
            bytes(data[offset : offset + 4]) != b"tjgg"
            or struct.unpack("<I", data[offset + 4 : offset + 8])[0] != 3
        ):
...
class GGMLToGGUF:
    ...
    def save(self):
        ...
        gguf_writer = gguf.GGUFWriter(
            self.cfg.output, gguf.MODEL_ARCH_NAMES[gguf.MODEL_ARCH.LLAMA], use_temp_file=False
        )
...
def handle_metadata(cfg, hp):
    ...
    vocab = convert.load_vocab(
        cfg.vocab_dir if cfg.vocab_dir is not None else cfg.model_metadata_dir, cfg.vocabtype
    )
