...
def load_image_tensor(image_file):
    ...
    if isinstance(image_file, str) and (
        ...
        image = Image.open(image_file).convert("RGB")
    image_data = (
        np.array(image.getdata()).reshape(1, image.size[1], image.size[0], 3).astype(np.byte)
    )
...
class hf_xclip:
    ...
    def init_qualcomm(self, model, device, qualcomm_label):
        ...
        try:
            ...
            endpoint_handler = self.create_qualcomm_xclip_endpoint_handler(
    ...
    def init_apple(self, model, device, apple_label):
        ...
        try:
            ...
            processor = self.transformers.XCLIPProcessor.from_pretrained(
            ...
            try:
                ...
                try:
                    ...
                    processor_candidates = [
                        lambda: self.transformers.AutoProcessor.from_pretrained(
                            model, trust_remote_code=True
                        ),
                        lambda: self.transformers.CLIPProcessor.from_pretrained(
                            model, trust_remote_code=True
                        ),
                        lambda: self.transformers.XCLIPProcessor.from_pretrained(
                            model, trust_remote_code=True
                        ),
                    ]
                ...
                try:
                    ...
                    model_candidates = [
                        lambda: self.transformers.AutoModel.from_pretrained(
                            model, trust_remote_code=True
                        ),
                        lambda: self.transformers.CLIPModel.from_pretrained(
                            model, trust_remote_code=True
                        ),
                        lambda: self.transformers.VisionTextDualEncoderModel.from_pretrained(
                            model, trust_remote_code=True
                        ),
                    ]
    ...
    def init_cuda(self, model, device, cuda_label):
        ...
        def create_dummy_components():
            ...
            class DummyProcessor:
                def __call__(self, *args, **kwargs):
                    return {
                        "input_ids": self.torch.zeros((1, 10), dtype=self.torch.long),
                        "attention_mask": self.torch.ones((1, 10), dtype=self.torch.long),
                        "pixel_values": self.torch.zeros(
                            (1, 3, 224, 224), dtype=self.torch.float32
                        ),
                    }
        ...
        try:
            ...
            # Create the handler
            endpoint_handler = self.create_cuda_video_embedding_endpoint_handler(
                endpoint=endpoint, tokenizer=processor, endpoint_model=model, cuda_label=cuda_label
            )
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(huggingface_cache_models):
                    ...
                    model_matches = [
                        x
                        for x in huggingface_cache_models_files_dirs_models
                        if model_name_convert in x
                    ]
                    if model_matches and len(model_matches) > 0:  # Safe list indexing
                        ...
                        model_src_path = os.path.join(
                            huggingface_cache_models, f"models--{model_name_convert}"
                        )
                else:
                ...
                model_dst_path = (
                    os.path.join(model_src_path, "openvino") if model_src_path else None
                )
            ...
            if model_dst_path:
                ...
                if not os.path.exists(model_dst_path):
                    ...
                    if hasattr(self, "openvino_skill_convert"):
                        try:
                            convert = self.openvino_skill_convert(
                                model, model_dst_path, task, weight_format
                            )
            ...
            try:
                processor_result = self.transformers.AutoProcessor.from_pretrained(
                    model, trust_remote_code=True
                )
                ...
                try:
                    if model_src_path:
                        processor_result = self.transformers.AutoProcessor.from_pretrained(
                            model_src_path, trust_remote_code=True
                        )
                        if processor_result is not None:
                            processor = processor_result
                except Exception as e:
                    ...
                    try:
                        ...
                        processor_result = self.transformers.CLIPProcessor.from_pretrained(
                            model, trust_remote_code=True
                        )
                        if processor_result is not None:
                            processor = processor_result
            ...
            if endpoint == dummy_endpoint and get_optimum_openvino_model is not None:
                try:
                    optimum_model_result = get_optimum_openvino_model(
                        model, model_type, openvino_label
                    )
    ...
    def create_cpu_video_embedding_endpoint_handler(
        self, tokenizer, endpoint_model, cpu_label, endpoint=None
    ):
        def handler(
            text=None,
            frames=None,
                    ...
                    with self.torch.no_grad():
                        ...
                        if text is not None:
                            try:
                                ...
                                if hasattr(text_outputs, "text_embeds"):
                                ...
                                elif hasattr(text_outputs, "text_model_output") and hasattr(
                                    text_outputs.text_model_output, "pooler_output"
                                ):
                                    result["text_embedding"] = (
                                        text_outputs.text_model_output.pooler_output
                                    )
                                    print(
                                        "Found text embeddings in text_model_output.pooler_output"
                                    )
                                    ...
                                    print(
                                        "Using mean pooling on last_hidden_state for text embeddings"
                                    )
                                    ...
                                    if "attention_mask" in text_inputs:
                                        ...
                                        result["text_embedding"] = (
                                            text_outputs.last_hidden_state.mean(1)
                                        )
                                else:
                                    ...
                                    for attr_name in dir(text_outputs):
                                        if (
                                            "embed" in attr_name.lower()
                                            and not attr_name.startswith("_")
                                        ):
                                            try:
                                                embed_attr = getattr(text_outputs, attr_name)
                                                if (
                                                    hasattr(embed_attr, "shape")
                                                    and len(embed_attr.shape) >= 2
                                                ):
                                    ...
                                    if not found_embedding:
                                        is_mock = True
                                        print(
                                            "Text embeddings not found in model output, using mock implementation"
                                        )
                            ...
                            try:
                                ...
                                if isinstance(frames, list) and len(frames) > 0:
                                    try:
                                        ...
                                        print(
                                            f"Standard image tokenizer format failed: {standard_error}"
                                        )
                                        try:
                                            # Try alternative format without named parameter
                                            video_inputs = tokenizer(
                                                frames, return_tensors="pt", padding=True
                                            )
                                        except Exception as alt_error:
                                            print(
                                                f"Alternative image tokenizer format failed: {alt_error}"
                                            )
                                            raise
                                    ...
                                    if hasattr(video_outputs, "image_embeds"):
                                    ...
                                    elif hasattr(video_outputs, "vision_model_output") and hasattr(
                                        video_outputs.vision_model_output, "pooler_output"
                                    ):
                                        result["video_embedding"] = (
                                            video_outputs.vision_model_output.pooler_output
                                        )
                                        print(
                                            "Found video embeddings in vision_model_output.pooler_output"
                                        )
                                        ...
                                        # Use mean pooling as a fallback for models that return hidden states
                                        print(
                                            "Using mean pooling on last_hidden_state for video embeddings"
                                        )
                                        result["video_embedding"] = (
                                            video_outputs.last_hidden_state.mean(1)
                                        )
                                    else:
                                        # Try to find any attribute that might contain embeddings
                                        found_embedding = False
                                        for attr_name in dir(video_outputs):
                                            if (
                                                "embed" in attr_name.lower()
                                                or "visual" in attr_name.lower()
                                                or "vision" in attr_name.lower()
                                            ) and not attr_name.startswith("_"):
                                                try:
                                                    ...
                                                    if (
                                                        ...
                                                        print(
                                                            f"Found video embeddings in {attr_name}"
                                                        )
                                                        found_embedding = True
                                                        break
                                                except:
                                        ...
                                        if not found_embedding:
                                            is_mock = True
                                            print(
                                                "Video embeddings not found in model output, using mock implementation"
                                            )
                                else:
                        ...
                        if (
                            ...
                            try:
                                ...
                                if len(text_emb.shape) == 1:
                                    text_emb = text_emb.unsqueeze(
                                        0
                                    )  # Add batch dimension if missing
                                if len(video_emb.shape) == 1:
                                    video_emb = video_emb.unsqueeze(
                                        0
                                    )  # Add batch dimension if missing
                                ...
                                if self.torch.all(text_norm > 0) and self.torch.all(video_norm > 0):
                                    ...
                                    if video_emb_norm.shape[0] == 1:
                                        ...
                                        result["similarity"] = self.torch.matmul(
                                            text_emb_norm, video_emb_norm.transpose(0, 1)
                                        )
                                    else:
                                        # Multiple video embeddings
                                        result["similarity"] = self.torch.matmul(
                                            text_emb_norm, video_emb_norm.T
                                        )

                                    print(
                                        f"Successfully calculated similarity: {result['similarity'].item() if result['similarity'].numel() == 1 else result['similarity'].shape}"
                                    )
                                else:
    ...
    def create_qualcomm_video_embedding_endpoint_handler(
        self, tokenizer, endpoint_model, qualcomm_label, endpoint=None
    ):
        def handler(
            text=None,
            frames=None,
            ...
            has_snpe = (
                ...
                and self.snpe_utils is not None
                and hasattr(self.snpe_utils, "run_inference")
            )
                    ...
                    if inputs:
                        ...
                        if inference_results:
                            ...
                            if "text_embeds" in inference_results and text is not None:
                                result["text_embedding"] = self.torch.tensor(
                                    inference_results["text_embeds"]
                                )

                            if "image_embeds" in inference_results and frames is not None:
                                result["video_embedding"] = self.torch.tensor(
                                    inference_results["image_embeds"]
                                )

                            # Calculate similarity if we have both embeddings
                            if (
                                "text_embeds" in inference_results
                                and "image_embeds" in inference_results
                            ):
    ...
    def create_apple_video_embedding_endpoint_handler(
        self, tokenizer, endpoint_model, apple_label, endpoint=None
    ):
        def handler(
            text=None,
            frames=None,
            ...
            # Flag to track if we're using real implementation or mock
            is_mock = (
                True  # Default to mock for Apple implementation (will update to REAL if successful)
            )
            ...
            has_coreml = (
                ...
                and self.coreml_utils is not None
                and hasattr(self.coreml_utils, "run_inference")
            )
                        ...
                        if outputs:
                            ...
                            if "text_embeds" in outputs and "image_embeds" in outputs:
                                ...
                                if self.torch.all(text_norm > 0) and self.torch.all(image_norm > 0):
                                    ...
                                    # Calculate similarity
                                    result["similarity"] = self.torch.matmul(
                                        text_emb_norm, image_emb_norm.T
                                    )
                                else:
                                    # Fallback for zero norm case
                                    result["similarity"] = self.torch.tensor([[0.8]])
    ...
    def create_cuda_video_embedding_endpoint_handler(
        self, endpoint=None, tokenizer=None, endpoint_model=None, cuda_label=None
    ):
        ...
        def handler(
            ...
            cuda_available = (
                ...
                and hasattr(endpoint, "device")
                and "cuda" in str(endpoint.device)
            )
            ...
            try:
                with torch.no_grad():
                    ...
                    if (
                        ...
                        try:
                            # Process text input
                            text_inputs = tokenizer(text=text, return_tensors="pt", padding=True)

                            # Move inputs to GPU
                            for key in list(
                                text_inputs.keys()
                            ):  # Use list to avoid dict size change issues
                            ...
                            if hasattr(text_output, "text_embeds"):
                                ...
                                print(
                                    "Model output doesn't contain text_embeds attribute, using mock embedding"
                                )
                        ...
                        try:
                            ...
                            if isinstance(frames, list) and len(frames) > 0:
                                ...
                                if hasattr(video_output, "image_embeds"):
                                    ...
                                    # Create mock embedding if structure unexpected
                                    print(
                                        "Model output doesn't contain image_embeds attribute, using mock embedding"
                                    )
    ...
    def create_openvino_video_embedding_endpoint_handler(
        self, endpoint=None, tokenizer=None, endpoint_model=None, openvino_label=None
    ):
        ...
        def handler(
                ...
                try:
                    ...
                    if isinstance(frames, str):
                        ...
                        if os.path.exists(frames):
                            videoreader = self.decord.VideoReader(
                                frames, num_threads=1, ctx=self.decord.cpu(0)
                            )
                        elif "http" in frames:
                            with tempfile.NamedTemporaryFile(suffix=".mp4") as f:
                                ...
                                f.flush()
                                videoreader = self.decord.VideoReader(
                                    f.name, num_threads=1, ctx=self.decord.cpu(0)
                                )

                        if videoreader is not None:
                            videoreader.seek(0)
                            indices = sample_frame_indices(
                                clip_len=32, frame_sample_rate=4, seg_len=len(videoreader)
                            )
            ...
            if video_frames is None and frames is not None:
                ...
                video_frames = self.np.random.randint(
                    0, 255, (32, 224, 224, 3), dtype=self.np.uint8
                )
                is_mock = True
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        new_processed_data = {
                            "input_ids": processed_data.get(
                                "input_ids", self.torch.ones((1, 10), dtype=self.torch.long)
                            ),
                            "attention_mask": processed_data.get(
                                "attention_mask", self.torch.ones((1, 10), dtype=self.torch.long)
                            ),
                            "pixel_values": processed_data.get(
                                "pixel_values",
                                self.torch.zeros((1, 3, 224, 224), dtype=self.torch.float32),
                            ),
                        }
                        ...
                        try:
                            ...
                            if inference_results and hasattr(inference_results, "values"):
                                ...
                                if len(results_list) >= 6:
                                    ...
                                    if text is not None and frames is not None:
                                        return {
                                            ...
                                            "similarity": self.torch.matmul(
                                                ...
                                                (
                                                    video_embeddings
                                                    / video_embeddings.norm(dim=-1, keepdim=True)
                                                ).T,
                                            ),
                                    ...
                                    print(
                                        f"OpenVINO inference results list length insufficient: {len(results_list)}"
                                    )
                                    is_mock = True
                            else:
    ...
    def openvino_skill_convert(
        self, model_name, model_dst_path, task, weight_format, hfmodel=None, hfprocessor=None
    ):
        if hfmodel is None:
            hfmodel = self.transformers.AutoModel.from_pretrained(
                model_name, torch_dtype=self.torch.float16
            )
        ...
        if hfprocessor is not None:
            ...
            with tempfile.NamedTemporaryFile(suffix=".mp4") as f:
                ...
                self.ov.save_model(
                    ov_model, os.path.join(model_dst_path, model_name.replace("/", "--") + ".xml")
                )
                ov_model = self.ov.compile_model(ov_model)
                hfmodel = None
        return ov_model

    def create_qualcomm_xclip_endpoint_handler(
    ...
    def create_apple_multimodal_endpoint_handler(
        ...
        def handler(
            ...
            try:
                ...
                if len(result) == 1 and list(result.keys())[0] in [
                    "text_embedding",
                    "image_embedding",
                ]:
