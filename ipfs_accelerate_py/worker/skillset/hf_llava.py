...
class hf_llava:
    ...
    def __init__(self, resources=None, metadata=None):
        ...
        self.create_openvino_genai_vlm_endpoint_handler = (
            self.create_openvino_genai_vlm_endpoint_handler
        )
    ...
    def _create_mock_processor(self):
        ...
        processor.apply_chat_template = lambda conversation, add_generation_prompt=True: (
            f"User: {conversation[0]['content'][1]['text']} with an image"
        )
        processor.decode = lambda ids, skip_special_tokens=True: (
            "This is a mock response from LLaVA"
        )
        processor.batch_decode = (
            lambda ids, skip_special_tokens=True, clean_up_tokenization_spaces=False: [
                "This is a mock response from LLaVA"
            ]
        )
    ...
    def _create_mock_endpoint(self, model_name, device_label):
        ...
        def mock_handler(text="", image=None):
            ...
            image_info = (
                f"of size {image.size}" if hasattr(image, "size") else "with the provided content"
            )
    ...
    def init_cpu(self, model_name, device, cpu_label):
        ...
        try:
            ...
            endpoint_handler = self.create_cpu_vlm_endpoint_handler(
                endpoint=endpoint, processor=processor, model_name=model_name, cpu_label=cpu_label
            )
    ...
    def init_qualcomm(self, model_name, device, qualcomm_label):
        ...
        try:
            ...
            print(
                f"Successfully initialized LLaVA model '{model_name}' on Qualcomm device {qualcomm_label}"
            )
        ...
        try:
            ...
            if isinstance(self.transformers, type(MagicMock())):
                ...
                processor.batch_decode = MagicMock(
                    return_value=["This is a mock response from LLaVA"]
                )
                ...
                if is_real_impl:
                    try:
                        ...
                        try:
                            if hasattr(self.transformers, "AutoModelForImageTextToText"):
                                endpoint = (
                                    self.transformers.AutoModelForImageTextToText.from_pretrained(
                                ...
                                print(
                                    "AutoModelForImageTextToText not available, trying AutoModelForVision2Seq"
                                )
                                if hasattr(self.transformers, "AutoModelForVision2Seq"):
                                    endpoint = (
                                        self.transformers.AutoModelForVision2Seq.from_pretrained(
                                            model_name, **model_kwargs
                                        )
                                    )
                                else:
            ...
            return (
                ...
                endpoint_handler,
                AnyioQueue(16),
                batch_size if is_real_impl else 1,
            )
    ...
    def init_openvino(
        ...
        device,
        openvino_label,
        get_openvino_genai_pipeline,
        get_optimum_openvino_model,
        get_openvino_model,
        get_openvino_pipeline_type,
        ...
        try:
            ...
            if not os.path.exists(os.path.join(model_dst_path, "openvino_model.xml")):
                print(
                    f"Converting {model_name} to OpenVINO format with {weight_format} precision..."
                )
                openvino_cli_convert(
                    model_name,
            ...
            print(
                f"Successfully initialized LLaVA model '{model_name}' on OpenVINO device {openvino_label}"
            )
            return endpoint, processor, endpoint_handler, AnyioQueue(64), 0
    ...
    def init_apple(self, model_name, device, apple_label):
        ...
        try:
            ...
            print(
                f"Successfully initialized LLaVA model '{model_name}' on Apple Silicon {apple_label}"
            )
    ...
    def create_optimum_vlm_endpoint_handler(
        self, cuda_endpoint_handler, local_cuda_processor, endpoint_model, cuda_label
    ):
        def handler(
            x,
            y,
            cuda_endpoint_handler=cuda_endpoint_handler,
            local_cuda_processor=local_cuda_processor,
    ...
    def create_openvino_genai_vlm_endpoint_handler(
        self, openvino_endpoint_handler, openvino_processor, endpoint_model, openvino_label
    ):
        def handler(
            x,
            y,
            openvino_endpoint_handler=openvino_endpoint_handler,
            openvino_processor=openvino_processor,
            endpoint_model=endpoint_model,
            openvino_label=openvino_label,
        ):
            ...
            try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            response = requests.get(
                                y, timeout=10, headers={"User-Agent": "Mozilla/5.0"}
                            )
                            response.raise_for_status()
                            image = Image.open(BytesIO(response.content)).convert("RGB")
                            image_data = (
                                self.np.array(image.getdata())
                            ...
                            if attempt == max_retries - 1:
                                raise ValueError(
                                    f"Failed to load image from URL after {max_retries} attempts: {y}. Error: {str(e)}"
                                )
                ...
                if x is not None and type(x) == str:
                    ...
                    raise Exception("Invalid input to vlm endpoint handler")
                prompt = x
                output = openvino_endpoint_handler.generate(
                    prompt, image=image_tensor, generation_config=config, streamer=streamer
                )
                # Run model inference
                return output
            except Exception as e:
    ...
    def create_openvino_vlm_endpoint_handler(self, endpoint, processor, model_name, openvino_label):
        ...
        def handler(
            ...
            try:
                            ...
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    if attempt < max_retries - 1:
                                        ...
                                        raise ValueError(
                                            f"Failed to load image from URL after {max_retries} attempts: {image}. Error: {str(last_error)}"
                                        )
                        else:
                ...
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                image_info = (
                    f"of size {image_obj.size}"
                    if image_obj and hasattr(image_obj, "size")
                    else "with the provided content"
                )
    ...
    def create_cuda_vlm_endpoint_handler(
        self, endpoint, processor, model_name, cuda_label, is_real_impl=True
    ):
        ...
        def handler(
            ...
            if (
                ...
                or isinstance(processor, type(MagicMock()))
            ):
            ...
            try:
                ...
                if cuda_device is not None and hasattr(self.torch.cuda, "mem_get_info"):
                    try:
                        free_memory_before, total_memory = self.torch.cuda.mem_get_info(
                            cuda_device.index
                        )
                        gpu_memory_before = {
                    ...
                    if batch_mode and isinstance(text, list):
                        ...
                        for txt in text:
                            if isinstance(txt, str):
                                ...
                                raise ValueError(
                                    f"Unsupported text input type in batch: {type(txt)}"
                                )
                    else:
                ...
                try:
                    ...
                    if not batch_mode:
                        try:
                            ...
                            print(
                                f"Streamer creation failed: {streamer_error}, continuing without streamer"
                            )
                        ...
                        if batch_mode and isinstance(text, list):
                            ...
                            for batch_inputs in inputs_list:
                                batch_output_ids = endpoint.generate(
                                    **batch_inputs, **generation_params
                                )
                                outputs.append(batch_output_ids)
                        else:
                            # Single generation
                            output_ids = endpoint.generate(**inputs, **generation_params)
                    ...
                    if cuda_device is not None and hasattr(self.torch.cuda, "mem_get_info"):
                        try:
                            ...
                            gpu_memory_used = (free_memory_before - free_memory_after) / (
                                1024**3
                            )  # in GB
                        except Exception as mem_error:
