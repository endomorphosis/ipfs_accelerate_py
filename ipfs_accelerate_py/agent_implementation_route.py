...
def _agent_implementation_quota_session_directory(
    *,
    ...
    if verifier_workspace is None:
        # Older Grok clients stored sessions directly under ``sessions``.
        # Keep that compatibility path only when no workspace identity was
        # supplied; it must not weaken a workspace-bound verifier invocation.
        ...
        if (
            ...
            return None
        try:
            # Grok CLI uses JavaScript ``encodeURIComponent`` semantics for
            # the absolute workspace component of its session directory.
            namespace = quote(
        ...
        # The same session identity in the legacy location makes layout
        # selection ambiguous. Fail closed rather than preferring either.
