"""Fail-closed PCPR-066 Accelerate-owned unrelated state change.

Accelerate owns change-event classification and reuse admission. This
module records an unrelated documentation change against the PCPR-061
DatasetsContextPack@1 identity, the PCPR-062 Kit current root, the
PCPR-063 route, the PCPR-064 patch, and the PCPR-065 selected-tests
run without reminting those identities.

The change is documentation-only and outside the LogicProviderProtocol@2
semantic frontier. The impacted cone is empty. Eligible ContextPack,
proof, and test reuse remains PCPR-067. A relevant interface change
remains PCPR-068. Incremental prover binaries missing from the sealed
PATH stay typed unavailable. Model stages are not invoked. A model
assertion cannot complete work.
...
INTERFACE: Final = "AccelerateUnrelatedStateChange@1"
SCHEMA: Final = "ipfs_accelerate_py/assurance/unrelated-state-change@1"
DOCUMENT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/declared-unrelated-state-change@1"
)
CHANGE_SCHEMA: Final = "ipfs_accelerate_py/assurance/unrelated-state-change-event@1"
VERDICT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/unrelated-state-change-verdict@1"
)
CATALOG_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/platform-unrelated-state-change-catalog@1"
)
CATALOG_INTERFACE: Final = "PlatformUnrelatedStateChange@1"
CHANGE_INTERFACE: Final = "UnrelatedStateChangeEvent@1"
...
OBJECTIVE_KIND: Final = "declared_unrelated_state_change"
...
DOCUMENT_JSON_NAME: Final = "reference.unrelated-state-change.json"
DOCUMENT_README_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/UNRELATED_STATE_CHANGE.md"
)
CATALOG_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/platform-unrelated-state-change-catalog.json"
)
SOURCE_DATE_EPOCH: Final = "0"
SOURCE_REPOSITORY: Final = "endomorphosis/ipfs_accelerate_py"
COMPLETED_THROUGH: Final = "unrelated_documentation_classified"
NEXT_AUTHORIZED_STAGE: Final = "eligible_reuse_admission"
CHANGE_KIND: Final = "unrelated_documentation"
...
if PINNED_LOCK_CID != (
    ...
    raise RuntimeError("PCPR-066 lock CID remints PCPR-056")

if OWNER_INTERFACES["SupervisorContextPack"] != OWNER_INTERFACE:
    raise RuntimeError("PCPR-066 remints SupervisorContextPack owner interface")
if OWNER_SCHEMAS["SupervisorContextPack"] != OWNER_SCHEMA:
    raise RuntimeError("PCPR-066 remints SupervisorContextPack owner schema")
if EXECUTION_INVOCATION_INTERFACE != "InvocationRequest@1":
    raise RuntimeError("PCPR-066 remints ExecutionInvocation owner interface")
if EXECUTION_RECEIPT_INTERFACE != "InvocationResult@1":
    raise RuntimeError("PCPR-066 remints ExecutionReceipt owner interface")
...
AUTHORIZED_PATH_PREFIXES: Final[tuple[str, ...]] = (
    ...
    "external/ipfs_kit",
    "artifacts/proof_carrying_platform_qualification_and_release/receipts/PCPR-066.json",
)

UNRELATED_CHANGE_PATHS: Final[tuple[str, ...]] = (
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "test/api/test_agent_supervisor_unrelated_state_change.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "change_is_unrelated_documentation",
        "impacted_cone_empty",
...
DOCUMENT_README: Final = """# PCPR-066 Accelerate unrelated state change

These files record the Accelerate-owned hermetic unrelated documentation
change against the PCPR-061 DatasetsContextPack@1 identity, the PCPR-062
Kit current root, the PCPR-063 deterministic-first route, the PCPR-064
bounded PatchPlan, and the PCPR-065 selected-tests-and-proofs run.
Accelerate owns change-event classification. It does not remint those
identities and does not claim live reuse.

- `cpython312/reference.unrelated-state-change.json` is the declared
  change document. Exact commit and tree are bound by the PCPR-066
  receipt `current_tree_binding`.
- `platform-unrelated-state-change-catalog.json` observes sibling
  Datasets and Kit bindings when present. Sibling source is never
  required.
- The change is documentation-only. The impacted cone is empty.
  LogicProviderProtocol@2 is not reminted. Eligible reuse remains
  PCPR-067. A relevant interface change remains PCPR-068.
...
class AccelerateUnrelatedStateChangeError(Exception):
    """Fail-closed PCPR-066 Accelerate unrelated state change error."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AccelerateUnrelatedStateChangeError(
...
def parse_pyproject_unrelated_state_change_table(text: str) -> dict[str, Any]:
    ...
    if isinstance(tool, dict):
        ...
        if isinstance(accelerate, dict):
            raw = accelerate.get("unrelated-state-change")
...
def refuse_unauthorized_path(path: str) -> str:
    if not path_is_authorized(path):
        raise AccelerateUnrelatedStateChangeError(
            f"path {path} is not an authorized PCPR-066 path"
        )
    return path
...
def refuse_whole_plan_regeneration(*, required: bool) -> None:
    if required:
        raise AccelerateUnrelatedStateChangeError(
            "unrelated documentation cannot require whole-plan regeneration"
        )
...
@dataclass(frozen=True)
class AccelerateUnrelatedStateChangeVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the hermetic unrelated documentation change. Do not write "
            "DuckDB or Quack state. Do not escalate to a model. Do not "
            "treat this as a relevant interface change. PCPR-067 "
            "demonstrates eligible reuse."
        ),
        "reason": (
            "Hermetic unrelated-state-change classification is not live "
...
def produce_unrelated_state_change(
    *,
    ...
    """Produce the hermetic unrelated documentation change. Fail closed."""
    ...
    for relpath in UNRELATED_CHANGE_PATHS:
...
def canonical_change_event() -> dict[str, Any]:
    return produce_unrelated_state_change()
...
def _sibling_binding(path: Path, relative_path: str) -> dict[str, Any]:
    if not path.is_file():
        return {
            ...
            "evidence_kind": "unavailable",
            "reason": (
                "Sibling unrelated-state-change file is not present "
...
def render_declared_change(root: Path | None = None) -> dict[str, Any]:
    ...
    document = {
        ...
        "materialization": {
            "kind": "declared_unrelated_state_change_not_live",
            "admitted": False,
            "live": False,
            "applied": False,
            "duckdb_or_quack_state_written": False,
...
def platform_unrelated_state_change_catalog(
    ...
    datasets_binding = _sibling_binding(
        ...
        / "reference.unrelated-state-change.binding.json",
        relative_path=(
...
def write_unrelated_state_change_files(
    ...
    catalog = platform_unrelated_state_change_catalog(start)
...
def current_head_static_probes(
    ...
    change = document["unrelated_state_change"]
    remint = (
    ...
    probes = [
        _probe(
            ...
            reason=(
                "Committed Accelerate unrelated-state-change matches the generator."
                if verified.get("document_ok") is True
                else "Committed Accelerate unrelated-state-change is missing or drifts."
            ),
        ),
        _probe(
            "pyproject_unrelated_state_change_table",
            ...
            reason=(
                "pyproject.toml declares AccelerateUnrelatedStateChange@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-066 change."
            ),
        ),
        _probe(
            "paths_remain_authorized",
            ...
            reason="Change paths remain inside the declared PCPR-066 allowlist.",
        ),
        _probe(
            "owner_pack_cid_matches_pin",
            document["context_pack"]["pack_cid"] == PINNED_PACK_CID,
        ...
        _probe(
            ...
            and change["relevant_interface_change"] is False
            and list(change["unrelated_change_paths"])
            == list(UNRELATED_CHANGE_PATHS),
            reason="The recorded change is unrelated documentation.",
        ),
        _probe(
            "impacted_cone_empty",
            list(change["impacted_cone"]) == []
            and change["whole_plan_regeneration_required"] is False,
            reason="The unrelated documentation change has an empty impact cone.",
        ),
        _probe(
            "protocol_identity_not_reminted",
        ...
        _probe(
            "whole_plan_regeneration_required",
            False,
            reason="Unrelated documentation does not regenerate the whole plan.",
        ),
        _probe(
            "reuse_claimed_as_demonstrated",
            False,
...
def qualify_unrelated_state_change(
    ...
    *,
    ...
    objective_cid: str,
    idea_digest_cid: str,
) -> AccelerateUnrelatedStateChangeVerdict:
    if not probes:
        raise AccelerateUnrelatedStateChangeError(
    ...
    return AccelerateUnrelatedStateChangeVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def pcpr_066_receipt_promotion(
    verdict: AccelerateUnrelatedStateChangeVerdict,
) -> dict[str, Any]:
...
PINNED_DOCUMENT_CID: Final = (
    "baguqeera3sktt7jdkzb5nr3rmw3bhvpot7v25fa3x4kpf7xmyg4nf2jetwja"
)
PINNED_CATALOG_CID: Final = (
    "baguqeerazzo6vpq6ixmvzl5uljwp4e55ufhjqjnujoh54qgpmw4p54kfh4pa"
)
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerapy5jajcnc2joampnosf4afya3xqgj6t5wimku6dqxlmslerdjtca"
)
...
__all__ = [
    ...
    "ESCALATION_ORDER",
    "AccelerateUnrelatedStateChangeError",
    ...
    "platform_unrelated_state_change_catalog",
    "produce_unrelated_state_change",
