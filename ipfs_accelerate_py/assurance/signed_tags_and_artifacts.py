"""Fail-closed PCPR-055 Accelerate signed tags and artifacts.

Produce a declared signed-tag policy and a declared artifact checksum
manifest for ipfs_accelerate_py. Live GPG/SSH git-tag signatures, cosign
or minisign artifact signatures, and published wheel/sdist/container
identities stay typed unavailable. Signatures and hashes are never
invented. Existing PCPR-053/054 lock, SBOM, and provenance files may be
checksummed because those bytes are present.
...
INTERFACE: Final = "AccelerateSignedTagsAndArtifacts@1"
SCHEMA: Final = "ipfs_accelerate_py/assurance/signed-tags-and-artifacts@1"
TAG_POLICY_SCHEMA: Final = "ipfs_accelerate_py/assurance/declared-signed-tag-policy@1"
CHECKSUM_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/declared-artifact-checksums@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/signed-tags-and-artifacts-verdict@1"
)
CATALOG_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/platform-signed-tags-and-artifacts-catalog@1"
)
CATALOG_INTERFACE: Final = "PlatformSignedTagsAndArtifactsCatalog@1"
...
EVIDENCE_ID: Final = "pcpr/accelerate-signed-tags-and-artifacts@1"
...
CATALOG_RELPATH: Final = "packaging/pcpr/signed/platform-catalog.json"
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "named_git_extras_are_not_signed_release",
...
class SignedTagsAdmissionError(SignedTagsError):
    """Raised when a signed-tag or checksum input is rejected."""
...
def observe_signing_tooling() -> dict[str, Any]:
    """Measure sealed-PATH signing tools. Missing stay unavailable."""
    ...
    return {
        **env,
        "signing_tools": tools,
        "openssl": _which_sealed("openssl"),
        "any_live_tag_signer": False,
        "any_live_artifact_signer": False,
        "gpg_secret_key": typed_unavailable(
            reason=(
                "A sealed-HOME GPG secret key was not admitted. Presence of "
                "gpg on PATH is not a live signing identity."
            )
        ),
        "ssh_signing_key": typed_unavailable(
            reason=(
                "A sealed-HOME SSH signing key was not admitted. Presence of "
                "ssh-keygen on PATH is not a live signing identity."
            )
        ),
...
def parse_pyproject_signed_table(text: str) -> dict[str, Any]:
    ...
    if isinstance(tool, dict):
        ...
        if isinstance(accelerate, dict):
            raw = accelerate.get("signed-tags-and-artifacts")
...
def render_declared_tag_policy(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise SignedTagsError("Accelerate package root was not found")
    ...
    document = {
        ...
        "signing": {
            ...
            "tools": {
                name: path
                for name, path in tooling["signing_tools"].items()
            },
        },
        "commands": {
            "materialize": (
            ...
            "intended_cosign_blob": "unavailable",
...
def write_signed_tags_and_artifacts_files(
    ...
    catalog = platform_signed_catalog(start)
...
def verify_signed_tags_and_artifacts_files(
    ...
    return {
        ...
        "tag_policy_sha256": (
            sha256_bytes(paths["tag_policy"].read_bytes())
            if paths["tag_policy"].is_file()
            else "unavailable"
        ),
        "checksums_sha256": (
            sha256_bytes(paths["checksums"].read_bytes())
...
def _sibling_signed_identity(path: Path, relative_path: str, cid_field: str) -> dict[str, Any]:
    if not path.is_file():
        return {
            ...
            "status": "unavailable",
            "evidence_kind": "unavailable",
            "reason": "Sibling signed-tag file is not present beside this checkout.",
        }
    ...
    return {
        ...
        "tag_kind": payload.get("tag_kind"),
        "checksum_kind": payload.get("checksum_kind"),
        cid_field: payload.get(cid_field),
        "live": False,
        "signed": False,
...
def platform_signed_catalog(start: Path | None = None) -> dict[str, Any]:
    """Bind Datasets, Kit, and Accelerate signed-tag identities."""
    ...
    tag_policy = render_declared_tag_policy(root)
    checksums = render_declared_checksums(root, tag_policy=tag_policy)
    datasets_tag = _sibling_signed_identity(
        parent / "ipfs_datasets" / SIGNED_DIR_RELPATH / TAG_POLICY_JSON_NAME,
        relative_path=f"../ipfs_datasets/{SIGNED_DIR_RELPATH}/{TAG_POLICY_JSON_NAME}",
        cid_field="tag_policy_cid",
    )
    datasets_checksums = _sibling_signed_identity(
        parent / "ipfs_datasets" / SIGNED_DIR_RELPATH / CHECKSUMS_JSON_NAME,
        relative_path=f"../ipfs_datasets/{SIGNED_DIR_RELPATH}/{CHECKSUMS_JSON_NAME}",
        cid_field="checksums_cid",
    )
    kit_tag = _sibling_signed_identity(
        parent / "ipfs_kit" / SIGNED_DIR_RELPATH / TAG_POLICY_JSON_NAME,
        relative_path=f"../ipfs_kit/{SIGNED_DIR_RELPATH}/{TAG_POLICY_JSON_NAME}",
        cid_field="tag_policy_cid",
    )
    kit_checksums = _sibling_signed_identity(
        parent / "ipfs_kit" / SIGNED_DIR_RELPATH / CHECKSUMS_JSON_NAME,
        relative_path=f"../ipfs_kit/{SIGNED_DIR_RELPATH}/{CHECKSUMS_JSON_NAME}",
        cid_field="checksums_cid",
    )
    catalog = {
        ...
        "components": {
            "ipfs_accelerate_py": {
                "tag_policy_path": f"{SIGNED_DIR_RELPATH}/{TAG_POLICY_JSON_NAME}",
                "checksums_path": f"{SIGNED_DIR_RELPATH}/{CHECKSUMS_JSON_NAME}",
                "status": "observed",
                "evidence_kind": "measured",
                "package_name": PACKAGE_NAME,
                "package_version": PACKAGE_VERSION,
            ...
            "ipfs_datasets_py": {
                "tag_policy": datasets_tag,
                "checksums": datasets_checksums,
            },
            "ipfs_kit_py": {
                "tag_policy": kit_tag,
                "checksums": kit_checksums,
            },
        },
        "mutable_main_reference": False,
        "hashes_invented": False,
...
def current_head_static_probes(
    ...
    probes = [
        ...
        _probe(
            ...
            reason=(
                "pyproject.toml declares AccelerateSignedTagsAndArtifacts@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare AccelerateSignedTagsAndArtifacts@1."
            ),
            details={"table": table},
        ),
        _probe(
        ...
        _probe(
            ...
            reason=(
                ...
                "extras may carry source-checkout Git pins; they are not this signed release."
            ),
            details={
                "named_git_extras": list(NAMED_GIT_EXTRAS),
                "named_git_extras_role": NAMED_GIT_EXTRAS_ROLE,
        ...
        _probe(
            ...
            None,
            evidence_kind="unavailable",
            reason="No published wheel identity is bound by this declared checksum manifest.",
        ),
        _probe(
            "published_sdist",
            None,
            evidence_kind="unavailable",
            reason="No published sdist identity is bound by this declared checksum manifest.",
        ),
        _probe(
            "container_digest",
            None,
            evidence_kind="unavailable",
            reason="No container digest is bound by this declared checksum manifest.",
        ),
    ]
    return tuple(probes)
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerasp6qilcrxp3l5kkpa3kc57c3gbgyhskigagbebrymfu4ra5dpclq"
)
