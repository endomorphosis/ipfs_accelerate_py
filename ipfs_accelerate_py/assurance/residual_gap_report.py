"""Fail-closed PCPR-095 Accelerate-owned residual-gap report receipt.

Accelerate owns the hermetic residual-gap report receipt run for the PCPR
...
INTERFACE: Final = "AccelerateResidualGapReport@1"
SCHEMA: Final = "ipfs_accelerate_py/assurance/residual-gap-report-receipt@1"
DOCUMENT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/declared-residual-gap-report-receipt@1"
)
PACKAGE_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/residual-gap-report-inventory@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/residual-gap-report-verdict@1"
)
CATALOG_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/platform-residual-gap-report-catalog@1"
)
CATALOG_INTERFACE: Final = "PlatformResidualGapReport@1"
PACKAGE_INTERFACE: Final = "AuthoritativeResidualGapReport@1"
...
DOCUMENTATION_INTERFACE: Final = "LogicProviderProtocolResidualGapReport@1"
...
OBJECTIVE_KIND: Final = "declared_objective_to_release_residual_gap_report"
...
DOCUMENT_JSON_NAME: Final = "reference.residual-gap-report.json"
DOCUMENT_README_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/RESIDUAL_GAP_REPORT.md"
)
CATALOG_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/platform-residual-gap-report-catalog.json"
)
SOURCE_DATE_EPOCH: Final = "0"
SOURCE_REPOSITORY: Final = "endomorphosis/ipfs_accelerate_py"
COMPLETED_THROUGH: Final = "residual_gap_report"
NEXT_AUTHORIZED_STAGE: Final = "next_bounded_pilot"
CHANGE_KIND: Final = "objective_to_release_residual_gap_report"
PACKAGE_KIND: Final = "hermetic_objective_to_release_residual_gap_report"
CANONICAL_SERVICE: Final = "pcpr-canonical-supervisor-service"
CLIENT_AUTHORITY: Final = "intent_not_authority"
PACKAGE_COMMAND_DIGEST: Final = "pcpr-095-hermetic-residual-gap-report"
...
if PINNED_LOCK_CID != (
    ...
    raise RuntimeError("PCPR-095 lock CID remints PCPR-056")
...
if OWNER_INTERFACES["SupervisorContextPack"] != OWNER_INTERFACE:
    raise RuntimeError("PCPR-095 remints SupervisorContextPack owner interface")
if OWNER_SCHEMAS["SupervisorContextPack"] != OWNER_SCHEMA:
    raise RuntimeError("PCPR-095 remints SupervisorContextPack owner schema")
...
def _gate(
    ...
    *,
    ...
    bounded_action = (
        ...
        "PCPR-096 may recommend a next bounded pilot only."
    )
    return {
        "gate_id": gate_id,
...
if any(not item["bounded"] for item in GATES):
    raise RuntimeError("PCPR-095 gate table contains an unbounded gate")
if {item["family"] for item in GATES} != set(GATE_FAMILIES):
    raise RuntimeError("PCPR-095 gates do not cover every declared family")
if len({item["gate_id"] for item in GATES}) != len(GATES):
    raise RuntimeError("PCPR-095 gate identifiers are not unique")
if any(item["closed_release"] for item in GATES):
    raise RuntimeError("PCPR-095 gates must not claim a closed release")
if any(item["pass_live"] for item in GATES):
    raise RuntimeError("PCPR-095 gates must not claim a live pass")
if any(item["live"] for item in GATES):
    raise RuntimeError("PCPR-095 gates must not claim live execution")
if any(
    item["hidden_failed_stale_partial_simulated_estimated_or_missing"] for item in GATES
):
    raise RuntimeError("PCPR-095 must not hide failed, stale, partial, or missing evidence")
if any(
    not all(item.get(key) for key in ("impact", "evidence", "owner", "precondition", "bounded_action"))
    for item in GATES
):
    raise RuntimeError("PCPR-095 residual must name impact, evidence, owner, precondition, and bounded action")
if any(item.get("successor_campaign") for item in GATES):
    raise RuntimeError("PCPR-095 residual reporting must not create a successor campaign")
...
AUTHORIZED_PATH_PREFIXES: Final[tuple[str, ...]] = (
    ...
    "external/ipfs_kit",
    "artifacts/proof_carrying_platform_qualification_and_release/receipts/PCPR-095.json",
)

HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "test/api/test_agent_supervisor_residual_gap_report.py",
)
...
DOCUMENT_README: Final = """# PCPR-095 Accelerate residual-gap report receipt
...
class AccelerateResidualGapReportError(Exception):
    """Fail-closed PCPR-095 Accelerate residual-gap report receipt error."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AccelerateResidualGapReportError(
...
def parse_pyproject_residual_gap_report_table(text: str) -> dict[str, Any]:
    ...
    if not isinstance(table, dict):
        raise AccelerateResidualGapReportError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        accelerate = tool.get("ipfs_accelerate_py")
        if isinstance(accelerate, dict):
            raw = accelerate.get("residual-gap-report")
...
def refuse_unauthorized_path(path: str) -> str:
    if not path_is_authorized(path):
        raise AccelerateResidualGapReportError(
            f"path {path} is not an authorized PCPR-095 path"
        )
    return path
...
def refuse_live_promotion(*, applied_live: bool) -> None:
    if applied_live:
        raise AccelerateResidualGapReportError(
            "live residual-gap report receipt execution remains typed unavailable"
        )
...
def refuse_self_promotion(*, promoted: bool) -> None:
    if promoted:
        raise AccelerateResidualGapReportError(
            "residual-gap report receipt cannot promote itself"
        )
...
def refuse_non_canonical_objective(idea: str) -> str:
    ...
    if text != REFERENCE_OBJECTIVE_IDEA:
        raise AccelerateResidualGapReportError(
            "residual-gap report receipt must bind the canonical PCPR-060 idea"
        )
    return text
...
def refuse_unbounded_gate(*, unbounded: bool) -> None:
    if unbounded:
        raise AccelerateResidualGapReportError(
            "every residual-gap report receipt must be bounded to a declared family"
        )
...
def hermetic_gate_run_trace() -> dict[str, Any]:
    ...
    return {
        "schema": "ipfs_accelerate_py/assurance/hermetic-residual-gap-report-trace@1",
        "interface": "HermeticResidualGapReportTrace@1",
...
def hermetic_gate_run_idempotency(report_cid: str) -> dict[str, Any]:
    return {
        "schema": "ipfs_accelerate_py/assurance/hermetic-residual-gap-report-idempotency@1",
        "interface": "HermeticResidualGapReportIdempotency@1",
...
@dataclass(frozen=True)
class AccelerateResidualGapReportVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "action": (
            "Keep the hermetic residual-gap report. Keep every gate family evaluated. "
            ...
            "a successor campaign. PCPR-096 recommends the next bounded pilot."
        ),
        "reason": (
            "Hermetic residual-gap report receipt coverage is not a live "
...
def produce_residual_gap_report(
    *,
    ...
    """Produce the hermetic objective-to-release candidate residual-gap report. Fail closed."""
...
def canonical_residual_gap_report() -> dict[str, Any]:
    return produce_residual_gap_report()


def report_cid_of(plan: Mapping[str, Any] | None = None) -> str:
    payload = dict(plan or canonical_residual_gap_report())
...
def reproduce_promotion_ground(gate_id: str) -> dict[str, Any]:
    ...
    if match is None:
        raise AccelerateResidualGapReportError(f"unknown promotion ground {wanted}")
    plan = canonical_residual_gap_report()
...
def reproduce_residual_gap_report() -> dict[str, Any]:
...
def _sibling_binding(path: Path, relative_path: str) -> dict[str, Any]:
    if not path.is_file():
        return {
            ...
            "evidence_kind": "unavailable",
            "reason": (
                "Sibling residual-gap-report binding file is not present "
    ...
    package = payload.get("residual_gap_report")
    return {
        "path": relative_path,
        "status": "observed",
        "evidence_kind": "measured",
...
def render_declared_package(root: Path | None = None) -> dict[str, Any]:
    ...
    reproduction = reproduce_residual_gap_report()
    document = {
        ...
        "materialization": {
            "kind": "declared_residual_gap_report_not_live",
            "admitted": False,
            "live": False,
            "applied": False,
            "duckdb_or_quack_state_written": False,
        ...
        "live_application": typed_unavailable(
            reason=(
                ...
                "Hermetic residual-gap report receipt coverage is not live execution."
            )
        ),
        "live_client": typed_unavailable(
            reason=(
                "Live Python Supervisor.run and live generic MCP remain typed "
                "unavailable. This task only emits a hermetic residual-gap report. "
...
def platform_residual_gap_report_catalog(start: Path | None = None) -> dict[str, Any]:
    ...
    datasets_binding = _sibling_binding(
        ...
        / "reference.residual-gap-report.binding.json",
        relative_path=(
    ...
    catalog = {
        ...
        "gate_run_cid": PINNED_GATE_RUN_CID,
        "promotion_decision_cid": PINNED_PROMOTION_DECISION_CID,
        "report_cid": document["residual_gap_report"]["report_cid"],
...
def write_residual_gap_report_files(start: Path | None = None) -> dict[str, Any]:
    ...
    catalog = platform_residual_gap_report_catalog(start)
...
def verify_residual_gap_report_files(start: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        record_ref = str(result.get("report_cid") or result.get("document_cid") or "residual-gap")
        mirror_work_record(
            catalog_kind="metadata",
            record_kind="residual_gap_report_files",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    package = document["residual_gap_report"]
    trace = package["trace"]
    ...
    probes = [
        _probe(
            ...
            verified["ok"] is True,
            reason=(
                "Declared residual-gap-report files match the generator."
                if verified["ok"]
                else "Declared residual-gap-report files do not match the generator."
            ),
        ),
        _probe(
            "pyproject_residual_gap_report_table",
            table.get("interface") == INTERFACE and table.get("task-id") == PCPR_095_TASK_ID,
            reason=(
                "pyproject.toml declares the PCPR-095 residual-gap-report table."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-095 residual-gap-report table."
            ),
        ),
        _probe(
            "paths_remain_authorized",
            ...
            reason="Residual-gap-report paths remain inside the declared PCPR-095 allowlist.",
        ),
        _probe(
            "canonical_objective_bytes_match",
            package["idea"] == REFERENCE_OBJECTIVE_IDEA
            and package["idea_digest"] == PINNED_IDEA_DIGEST,
            reason="Residual-gap report receipt uses the canonical PCPR-060 idea bytes.",
        ),
        _probe(
            "supervisor_gate_evaluated",
        ...
        _probe(
            ...
            reason="Every declared residual-gap report receipt family is evaluated.",
        ),
        _probe(
            "all_gates_bounded",
            package["all_gates_bounded"] is True and trace["all_gates_bounded"] is True,
            reason="Every recorded residual-gap report receipt is bounded.",
        ),
        _probe(
            "no_hidden_failed_stale_partial_simulated_estimated_or_missing",
        ...
        _probe(
            ...
            and package["next_task_id"] == PCPR_096_TASK_ID,
            reason="Residual-gap report receipt run is produced by PCPR-095.",
        ),
        _probe(
            "gate_run_cid_bound_not_minted",
        ...
        _probe(
            ...
            is True
            and package["next_task_id"] == PCPR_096_TASK_ID,
            reason="PCPR-095 residual-gap report is produced with every residual named.",
        ),
        _probe(
            "all_residuals_named",
        ...
        _probe(
            ...
            and package["next_pilot_deferred"] is True,
            reason="Residual reporting does not create a successor campaign or raise deployment class.",
        ),
        _probe(
            "honest_non_promotion_produced",
            ...
            reason=(
                "PCPR-095 emits an honest non-promotion and does not claim "
        ...
        _probe(
            "live_promotion_represented_as_live",
            False,
            reason="Hermetic residual-gap report receipt coverage is not represented as live.",
        ),
        _probe(
            "closed_release_represented_as_live",
            False,
...
def qualify_residual_gap_report(
    ...
    *,
    ...
    parity_cid: str = PINNED_PARITY_CID,
    bypass_proof_cid: str = PINNED_BYPASS_PROOF_CID,
) -> AccelerateResidualGapReportVerdict:
    if not probes:
        raise AccelerateResidualGapReportError("at least one probe is required")
    ...
    return AccelerateResidualGapReportVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def qualify_current_head_residual_gap_report(
    ...
    return qualify_residual_gap_report(
        ...
        report_cid=str(document["residual_gap_report"]["report_cid"]),
...
def pcpr_095_receipt_promotion(
    verdict: AccelerateResidualGapReportVerdict,
) -> dict[str, Any]:
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeera7y2hjjn3erg6wrnturvponr4nzlwebi3rszfxoyniilbndc4bnwq"
)
if PINNED_REPORT_CID == PINNED_GATE_RUN_CID:
    raise RuntimeError("PCPR-095 report CID remints PCPR-093 gate-run CID")
if PINNED_REPORT_CID == PINNED_PROMOTION_DECISION_CID:
    raise RuntimeError("PCPR-095 report CID remints PCPR-094 promotion-decision CID")
...
__all__ = [
    ...
    "SEALED_PYTHON",
    "TRUSTED_PATH_STAGES",
    "AccelerateResidualGapReportError",
    "all_gate_families_evaluated",
    "all_gates_bounded",
    ...
    "RESIDUALS",
    "platform_residual_gap_report_catalog",
    "produce_residual_gap_report",
...
if __name__ == "__main__":
    ...
    print(document["residual_gap_report"]["report_cid"])
