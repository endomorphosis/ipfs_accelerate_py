"""Fail-closed PCPR-094 Accelerate-owned promotion or non-promotion receipt.

Accelerate owns the hermetic promotion or non-promotion receipt run for the PCPR
...
INTERFACE: Final = "AcceleratePromotionOrNonPromotionReceipt@1"
SCHEMA: Final = "ipfs_accelerate_py/assurance/promotion-or-non-promotion-receipt@1"
DOCUMENT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/declared-promotion-or-non-promotion-receipt@1"
)
PACKAGE_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/promotion-or-non-promotion-inventory@1"
)
VERDICT_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/promotion-or-non-promotion-verdict@1"
)
CATALOG_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/platform-promotion-or-non-promotion-catalog@1"
)
CATALOG_INTERFACE: Final = "PlatformPromotionOrNonPromotion@1"
PACKAGE_INTERFACE: Final = "AuthoritativePromotionOrNonPromotionReceipt@1"
...
OBJECTIVE_KIND: Final = "declared_objective_to_release_promotion_or_non_promotion"
OBJECTIVE_ID: Final = "PCPR-G900"
LANGUAGE: Final = "Python"
OPERATOR_BLOCKING_TASK_ID: Final = "pcpr-094-operator-live-promotion"
DOCUMENT_DIR_RELPATH: Final = "packaging/pcpr/reference-workflow/cpython312"
DOCUMENT_JSON_NAME: Final = "reference.promotion-or-non-promotion.json"
DOCUMENT_README_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/PROMOTION_OR_NON_PROMOTION.md"
)
CATALOG_RELPATH: Final = (
    "packaging/pcpr/reference-workflow/platform-promotion-or-non-promotion-catalog.json"
)
SOURCE_DATE_EPOCH: Final = "0"
SOURCE_REPOSITORY: Final = "endomorphosis/ipfs_accelerate_py"
COMPLETED_THROUGH: Final = "promotion_or_non_promotion_receipt"
NEXT_AUTHORIZED_STAGE: Final = "residual_gap_report"
CHANGE_KIND: Final = "objective_to_release_promotion_or_non_promotion"
PACKAGE_KIND: Final = "hermetic_objective_to_release_promotion_or_non_promotion"
CANONICAL_SERVICE: Final = "pcpr-canonical-supervisor-service"
CLIENT_AUTHORITY: Final = "intent_not_authority"
PACKAGE_COMMAND_DIGEST: Final = "pcpr-094-hermetic-promotion-or-non-promotion"
...
if PINNED_LOCK_CID != (
    ...
    raise RuntimeError("PCPR-094 lock CID remints PCPR-056")
...
if OWNER_INTERFACES["SupervisorContextPack"] != OWNER_INTERFACE:
    raise RuntimeError("PCPR-094 remints SupervisorContextPack owner interface")
if OWNER_SCHEMAS["SupervisorContextPack"] != OWNER_SCHEMA:
    raise RuntimeError("PCPR-094 remints SupervisorContextPack owner schema")
...
if any(not item["bounded"] for item in GATES):
    raise RuntimeError("PCPR-094 gate table contains an unbounded gate")
if {item["family"] for item in GATES} != set(GATE_FAMILIES):
    raise RuntimeError("PCPR-094 gates do not cover every declared family")
if len({item["gate_id"] for item in GATES}) != len(GATES):
    raise RuntimeError("PCPR-094 gate identifiers are not unique")
if any(item["closed_release"] for item in GATES):
    raise RuntimeError("PCPR-094 gates must not claim a closed release")
if any(item["pass_live"] for item in GATES):
    raise RuntimeError("PCPR-094 gates must not claim a live pass")
if any(item["live"] for item in GATES):
    raise RuntimeError("PCPR-094 gates must not claim live execution")
if any(
    item["hidden_failed_stale_partial_simulated_estimated_or_missing"] for item in GATES
):
    raise RuntimeError("PCPR-094 must not hide failed, stale, partial, or missing evidence")
...
AUTHORIZED_PATH_PREFIXES: Final[tuple[str, ...]] = (
    ...
    "external/ipfs_kit",
    "artifacts/proof_carrying_platform_qualification_and_release/receipts/PCPR-094.json",
)

HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "test/api/test_agent_supervisor_promotion_or_non_promotion.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "pyproject_promotion_or_non_promotion_table",
...
DOCUMENT_README: Final = """# PCPR-094 Accelerate promotion or non-promotion receipt
...
class AcceleratePromotionOrNonPromotionError(Exception):
    """Fail-closed PCPR-094 Accelerate promotion or non-promotion receipt error."""


def _text(value: Any, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise AcceleratePromotionOrNonPromotionError(
...
def parse_pyproject_promotion_or_non_promotion_table(text: str) -> dict[str, Any]:
    import tomllib

    table = tomllib.loads(_text(text, "pyproject.toml"))
    if not isinstance(table, dict):
        raise AcceleratePromotionOrNonPromotionError("pyproject.toml must be a table")
    tool = table.get("tool")
    payload: dict[str, Any] = {}
    if isinstance(tool, dict):
        accelerate = tool.get("ipfs_accelerate_py")
        if isinstance(accelerate, dict):
            raw = accelerate.get("promotion-or-non-promotion")
...
def refuse_unauthorized_path(path: str) -> str:
    if not path_is_authorized(path):
        raise AcceleratePromotionOrNonPromotionError(
            f"path {path} is not an authorized PCPR-094 path"
        )
    return path
...
def refuse_database_edit(*, edited: bool) -> None:
    if edited:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt cannot write DuckDB or Quack state"
        )
...
def refuse_live_promotion(*, applied_live: bool) -> None:
    if applied_live:
        raise AcceleratePromotionOrNonPromotionError(
            "live promotion or non-promotion receipt execution remains typed unavailable"
        )
...
def refuse_self_promotion(*, promoted: bool) -> None:
    if promoted:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt cannot promote itself"
        )
...
def refuse_non_canonical_objective(idea: str) -> str:
    ...
    if text != REFERENCE_OBJECTIVE_IDEA:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt must bind the canonical PCPR-060 idea"
        )
    return text
...
def refuse_unbounded_gate(*, unbounded: bool) -> None:
    if unbounded:
        raise AcceleratePromotionOrNonPromotionError(
            "every promotion or non-promotion receipt must be bounded to a declared family"
        )
...
def hermetic_gate_run_trace() -> dict[str, Any]:
    ...
    return {
        "schema": "ipfs_accelerate_py/assurance/hermetic-promotion-or-non-promotion-trace@1",
        "interface": "HermeticPromotionOrNonPromotionTrace@1",
...
def hermetic_gate_run_idempotency(decision_cid: str) -> dict[str, Any]:
    return {
        "schema": "ipfs_accelerate_py/assurance/hermetic-promotion-or-non-promotion-idempotency@1",
        "interface": "HermeticPromotionOrNonPromotionIdempotency@1",
        "first_decision_command_digest": PACKAGE_COMMAND_DIGEST,
        "second_decision_command_digest": PACKAGE_COMMAND_DIGEST,
        "first_decision_cid": decision_cid,
        "second_decision_cid": decision_cid,
        "identical": True,
...
@dataclass(frozen=True)
class AcceleratePromotionOrNonPromotionVerdict:
    schema: str
    interface: str
...
def _operator_blocking_task() -> dict[str, Any]:
    return {
        ...
        "requires": (
            ...
            "promotion or non-promotion receipt is executed as live supervisor work"
        ),
        "action": (
            "Keep the hermetic promotion decision. Keep every gate family evaluated. "
            "Keep failed, stale, partial, simulated, estimated, or missing "
            "evidence visible. Do not write DuckDB or Quack state. Do not "
            "escalate to a model. Do not emit a closed release. PCPR-095 "
            "publishes the residual-gap report."
        ),
        "reason": (
            "Hermetic promotion or non-promotion receipt coverage is not a live "
...
def produce_promotion_or_non_promotion(
    *,
    ...
    """Produce the hermetic objective-to-release candidate promotion decision. Fail closed."""
    ...
    return {
        ...
        "next_task_id": PCPR_095_TASK_ID,
        "prerequisite_task_id": PCPR_093_TASK_ID,
...
def canonical_promotion_or_non_promotion() -> dict[str, Any]:
    return produce_promotion_or_non_promotion()


def decision_cid_of(plan: Mapping[str, Any] | None = None) -> str:
    payload = dict(plan or canonical_promotion_or_non_promotion())
    payload.pop("decision_cid", None)
...
def reproduce_promotion_ground(gate_id: str) -> dict[str, Any]:
    ...
    if match is None:
        raise AcceleratePromotionOrNonPromotionError(f"unknown promotion ground {wanted}")
    plan = canonical_promotion_or_non_promotion()
    run_cid = decision_cid_of(plan)
    return {
        "gate_id": wanted,
        "ok": (
...
def reproduce_promotion_or_non_promotion() -> dict[str, Any]:
...
def _sibling_binding(path: Path, relative_path: str) -> dict[str, Any]:
    if not path.is_file():
        return {
            ...
            "evidence_kind": "unavailable",
            "reason": (
                "Sibling promotion-or-non-promotion binding file is not present "
    ...
    package = payload.get("promotion_or_non_promotion")
    return {
        "path": relative_path,
        "status": "observed",
        "evidence_kind": "measured",
        ...
        or (package.get("decision_cid") if isinstance(package, Mapping) else None),
...
def render_declared_package(root: Path | None = None) -> dict[str, Any]:
    ...
    plan = canonical_promotion_or_non_promotion()
    computed_decision_cid = decision_cid_of(plan)
    idempotency = hermetic_gate_run_idempotency(computed_decision_cid)
    ...
    document = {
        ...
        "context_pack": {
            ...
            "stale": True,
            "rejected": True,
            "survives_promotion_or_non_promotion": True,
            "evidence_kind": "measured",
        },
        "storage": {
        ...
        "promotion_or_non_promotion": {
            **plan,
            "decision_cid": computed_decision_cid,
            "idempotency": idempotency,
            "reproduction": reproduction,
            "produced_by": EXECUTION_OWNER_REPOSITORY,
        },
        "materialization": {
            "kind": "declared_promotion_or_non_promotion_not_live",
            "admitted": False,
            "live": False,
            "applied": False,
            "duckdb_or_quack_state_written": False,
        ...
        "live_application": typed_unavailable(
            reason=(
                ...
                "Hermetic promotion or non-promotion receipt coverage is not live execution."
            )
        ),
        "live_client": typed_unavailable(
            reason=(
                "Live Python Supervisor.run and live generic MCP remain typed "
                "unavailable. This task only emits a hermetic promotion decision. "
        ...
        "source": {
            ...
            "commit": {
                ...
                "evidence_kind": "measured",
                "live": False,
                "field": "PCPR-094 receipt current_tree_binding",
            },
        },
...
def platform_promotion_or_non_promotion_catalog(start: Path | None = None) -> dict[str, Any]:
    ...
    datasets_binding = _sibling_binding(
        ...
        / "reference.promotion-or-non-promotion.binding.json",
        relative_path=(
    ...
    catalog = {
        ...
        "audit_package_cid": PINNED_AUDIT_PACKAGE_CID,
        "gate_run_cid": PINNED_GATE_RUN_CID,
        "decision_cid": document["promotion_or_non_promotion"]["decision_cid"],
...
def write_promotion_or_non_promotion_files(start: Path | None = None) -> dict[str, Any]:
    root = discover_accelerate_root(start)
    if root is None:
        raise AcceleratePromotionOrNonPromotionError(
            "Accelerate package root was not found"
    ...
    catalog = platform_promotion_or_non_promotion_catalog(start)
...
def verify_promotion_or_non_promotion_files(start: Path | None = None) -> dict[str, Any]:
    root = discover_accelerate_root(start)
    if root is None:
        raise AcceleratePromotionOrNonPromotionError(
            "Accelerate package root was not found"
    ...
    try:
        ...
        record_ref = str(result.get("decision_cid") or result.get("document_cid") or "promotion-decision")
        mirror_work_record(
            catalog_kind="metadata",
            record_kind="promotion_or_non_promotion_files",
...
def current_head_static_probes(start: Path | None = None) -> tuple[OutcomeProbe, ...]:
    ...
    document = render_declared_package(root)
    verified = verify_promotion_or_non_promotion_files(root)
    table = parse_pyproject_promotion_or_non_promotion_table(
    ...
    remint = (
        ...
        or package["decision_cid"] != PINNED_DECISION_CID
        or package["decision_cid"] == PINNED_GATE_RUN_CID
    )
    probes = [
        _probe(
            "gate_files_match_generator",
            verified["ok"] is True,
            reason=(
                "Declared promotion-or-non-promotion files match the generator."
                if verified["ok"]
                else "Declared promotion-or-non-promotion files do not match the generator."
            ),
        ),
        _probe(
            "pyproject_promotion_or_non_promotion_table",
            table.get("interface") == INTERFACE and table.get("task-id") == PCPR_094_TASK_ID,
            reason=(
                "pyproject.toml declares the PCPR-094 promotion-or-non-promotion table."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare the PCPR-094 promotion-or-non-promotion table."
            ),
        ),
        _probe(
            "paths_remain_authorized",
            ...
            reason="Promotion-or-non-promotion paths remain inside the declared PCPR-094 allowlist.",
        ),
        _probe(
            "canonical_objective_bytes_match",
            package["idea"] == REFERENCE_OBJECTIVE_IDEA
            and package["idea_digest"] == PINNED_IDEA_DIGEST,
            reason="Promotion or non-promotion receipt uses the canonical PCPR-060 idea bytes.",
        ),
        _probe(
            "supervisor_gate_evaluated",
        ...
        _probe(
            ...
            reason="Every declared promotion or non-promotion receipt family is evaluated.",
        ),
        _probe(
            "all_gates_bounded",
            package["all_gates_bounded"] is True and trace["all_gates_bounded"] is True,
            reason="Every recorded promotion or non-promotion receipt is bounded.",
        ),
        _probe(
            "no_hidden_failed_stale_partial_simulated_estimated_or_missing",
        ...
        _probe(
            ...
            and package["next_task_id"] == PCPR_095_TASK_ID,
            reason="Promotion or non-promotion receipt run is produced by PCPR-094.",
        ),
        _probe(
            "gate_run_cid_bound_not_minted",
            ...
            and package["decision_cid"] != PINNED_GATE_RUN_CID,
        ...
        _probe(
            ...
            reason=(
                "PCPR-094 emits an honest non-promotion and does not claim "
        ...
        _probe(
            ...
            and idempotency["first_decision_cid"] == package["decision_cid"]
            and idempotency["second_decision_cid"] == package["decision_cid"]
            and package["decision_cid"] == PINNED_DECISION_CID,
        ...
        _probe(
            "live_promotion_represented_as_live",
            False,
            reason="Hermetic promotion or non-promotion receipt coverage is not represented as live.",
        ),
        _probe(
            "closed_release_represented_as_live",
            False,
...
def qualify_promotion_or_non_promotion(
    ...
    *,
    ...
    parity_cid: str = PINNED_PARITY_CID,
    bypass_proof_cid: str = PINNED_BYPASS_PROOF_CID,
) -> AcceleratePromotionOrNonPromotionVerdict:
    if not probes:
        raise AcceleratePromotionOrNonPromotionError("at least one probe is required")
    ...
    return AcceleratePromotionOrNonPromotionVerdict(
        schema=VERDICT_SCHEMA,
        interface=INTERFACE,
        promotion_status=promotion_status,
        supervisor_disposition="supervisor_non_promoted",
...
def qualify_current_head_promotion_or_non_promotion(
    start: Path | None = None,
) -> AcceleratePromotionOrNonPromotionVerdict:
    document = render_declared_package(start)
    catalog = platform_promotion_or_non_promotion_catalog(start)
    return qualify_promotion_or_non_promotion(
        current_head_static_probes(start),
        decision_cid=str(document["promotion_or_non_promotion"]["decision_cid"]),
...
def pcpr_094_receipt_promotion(
    verdict: AcceleratePromotionOrNonPromotionVerdict,
) -> dict[str, Any]:
    if verdict.closed_release_outcome is not None:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt must not mint a closed release outcome"
        )
    if verdict.release_claim:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt must not claim a PCPR release"
        )
    if verdict.completion_authoritative:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt completion is not authoritative"
        )
    if verdict.duckdb_or_quack_state_written:
        raise AcceleratePromotionOrNonPromotionError(
            "promotion or non-promotion receipt must not write DuckDB or Quack state"
        )
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeerajm7tzw6kgbd6dj6xmrb24uyabvyf4ffjafwpkjpz346zqcnx4nfa"
)
if PINNED_DECISION_CID == PINNED_GATE_RUN_CID:
    raise RuntimeError("PCPR-094 decision CID remints PCPR-093 gate-run CID")
...
__all__ = [
    ...
    "write_promotion_or_non_promotion_files",
]


if __name__ == "__main__":
    written = write_promotion_or_non_promotion_files()
    document = written["document"]
    catalog = written["catalog"]
    print(document["promotion_or_non_promotion"]["decision_cid"])
