"""Fail-closed PCPR-053 Accelerate dependency locks.

Produce a declared release-profile lock for ipfs_accelerate_py. The lock
binds the exact PEP 508 specs from requirements.txt (the clean-package
runtime authority), forbids mutable Git branches, editable paths, and
local path requires, and records hash/transitive resolution as typed
unavailable until a live sealed-PATH resolver is admitted.
...
INTERFACE: Final = "AccelerateDependencyLocks@1"
SCHEMA: Final = "ipfs_accelerate_py/assurance/dependency-locks@1"
LOCK_SCHEMA: Final = "ipfs_accelerate_py/assurance/dependency-lock@1"
VERDICT_SCHEMA: Final = "ipfs_accelerate_py/assurance/dependency-locks-verdict@1"
CATALOG_SCHEMA: Final = (
    "ipfs_accelerate_py/assurance/platform-dependency-lock-catalog@1"
)
CATALOG_INTERFACE: Final = "PlatformDependencyLockCatalog@1"
...
EVIDENCE_ID: Final = "pcpr/accelerate-dependency-locks@1"
...
PYPROJECT_LOCK_TABLE: Final = "tool.ipfs_accelerate_py.dependency-locks"
...
CATALOG_RELPATH: Final = "packaging/pcpr/locks/platform-catalog.json"
...
HERMETIC_CANDIDATE_SUITES: Final[tuple[str, ...]] = (
    "test/api/test_agent_supervisor_dependency_locks.py",
)
...
REQUIRED_GOOD_PROBE_IDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "inner_requirements_git_is_not_release_lock",
    }
)
...
def parse_requirement_lines(text: str) -> tuple[str, ...]:
    """Parse requirements.txt-style specs. Comments and blanks are dropped."""
    ...
    for raw in (text or "").splitlines():
        ...
        if stripped.startswith(("-r", "--requirement", "-c", "--constraint")):
            raise DependencyLockError(
                "release lock must not include nested requirement includes"
            )
        if " #" in stripped:
...
def parse_pyproject_dependency_locks(text: str) -> dict[str, Any]:
    ...
    if isinstance(tool, dict):
        ...
        if isinstance(accelerate, dict):
            raw = accelerate.get("dependency-locks")
...
def render_release_lock(root: Path | None = None) -> dict[str, Any]:
    ...
    if package_root is None:
        raise DependencyLockError("Accelerate package root was not found")
    requirements = (package_root / "requirements.txt").read_text(encoding="utf-8")
    specs = parse_requirement_lines(requirements)
    ...
    document = {
        ...
        "native_dependencies": {
            "cuda_toolkit": typed_unavailable(
                reason="nvcc is absent from the sealed PATH; CUDA is not this lock."
            ),
            "ipfs_daemon": typed_unavailable(
                reason="ipfs is absent from the sealed PATH; a daemon is not this lock."
            ),
            "duckdb_cli": typed_unavailable(
                reason="duckdb CLI is absent from the sealed PATH."
            ),
        },
        "artifacts": {
        ...
        "commands": {
            "materialize_lock": (
                'python -c "from ipfs_accelerate_py.assurance.dependency_locks '
...
def write_release_lock_files(start: Path | None = None) -> dict[str, Any]:
    ...
    catalog = platform_lock_catalog(start)
...
def _sibling_lock_identity(path: Path, relative_path: str) -> dict[str, Any]:
    if not path.is_file():
        return {
            ...
            "status": "unavailable",
            "evidence_kind": "unavailable",
            "reason": "Sibling lock file is not present beside this checkout.",
        }
    payload = json.loads(path.read_text(encoding="utf-8"))
    hashes = payload.get("hashes") if isinstance(payload, dict) else {}
    hashes_status = (
        hashes.get("status") if isinstance(hashes, dict) else "unavailable"
    )
    return {
        "path": relative_path,
        "status": "observed",
        ...
        "direct_requirement_count": payload.get("direct_requirement_count"),
        "hashes_status": hashes_status,
        "live": False,
        "sha256": sha256_bytes(path.read_bytes()),
    }


def platform_lock_catalog(start: Path | None = None) -> dict[str, Any]:
    """Bind Datasets, Kit, and Accelerate lock identities when present."""

    root = discover_accelerate_root(start)
    if root is None:
        raise DependencyLockError("Accelerate package root was not found")
    parent = root.parent
    accelerate_lock = render_release_lock(root)
    datasets = _sibling_lock_identity(
        parent / "ipfs_datasets" / LOCK_DIR_RELPATH / LOCK_JSON_NAME,
        relative_path=f"../ipfs_datasets/{LOCK_DIR_RELPATH}/{LOCK_JSON_NAME}",
    )
    kit = _sibling_lock_identity(
        parent / "ipfs_kit" / LOCK_DIR_RELPATH / LOCK_JSON_NAME,
        relative_path=f"../ipfs_kit/{LOCK_DIR_RELPATH}/{LOCK_JSON_NAME}",
    )
    catalog = {
        ...
        "components": {
            "ipfs_accelerate_py": {
                "path": f"{LOCK_DIR_RELPATH}/{LOCK_JSON_NAME}",
                "status": "observed",
                "evidence_kind": "measured",
                "package_name": PACKAGE_NAME,
                "package_version": PACKAGE_VERSION,
                "lock_kind": LOCK_KIND,
                "lock_cid": accelerate_lock["lock_cid"],
                "direct_requirement_count": accelerate_lock[
                    "direct_requirement_count"
                ],
                "hashes_status": accelerate_lock["hashes"]["status"],
                "live": False,
            },
...
def current_head_static_probes(
    ...
    inner_path = root / INNER_DEVELOPMENT_REQUIREMENTS
    inner_text = inner_path.read_text(encoding="utf-8") if inner_path.is_file() else ""
    inner_scan = scan_requirement_text(inner_text)
    ...
    probes = [
        ...
        _probe(
            ...
            reason=(
                "pyproject.toml declares AccelerateDependencyLocks@1."
                if table.get("interface") == INTERFACE
                else "pyproject.toml does not declare AccelerateDependencyLocks@1."
            ),
            details={"table": table},
        ),
        _probe(
            ...
            reason="Lock authority remains requirements.txt.",
            details={
        ...
        _probe(
            ...
            reason=(
                ...
                "extras may carry source-checkout Git pins; they are not the release lock."
            ),
            details={
        ...
        _probe(
            ...
            True,
            reason=(
                "ipfs_accelerate_py/requirements.txt may carry source-checkout Git "
                "pins; it is not the release lock."
            ),
            details={
                "inner_vcs": list(inner_scan["vcs"]),
                "release_vcs": list(scanned["vcs"]),
            },
        ),
        _probe(
            "simulated_results_represented_as_live",
...
CURRENT_HEAD_NON_PROMOTION_VERDICT_CID: Final = (
    "baguqeeramphsipll26bz2uddq37mmb6hllng7bkbg2l3rfpr6yz3kqyza5sa"
)
