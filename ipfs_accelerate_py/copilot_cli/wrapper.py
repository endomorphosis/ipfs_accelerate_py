...
class CopilotCLI:
    ...
    def _verify_installation(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["which", self.copilot_path], capture_output=True, text=True, timeout=5
            )
    ...
    def _run_command(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt > 0:
                    logger.debug(
                        f"Retry attempt {attempt}/{max_retries} for command: {' '.join(cmd)}"
                    )
                else:
                ...
                if result.returncode != 0 and any(
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(
                            f"API error detected, retrying in {delay:.2f}s (attempt {attempt + 1}/{max_retries})"
                        )
                        time.sleep(delay)
                        continue
                ...
                if attempt < max_retries:
                    delay = min(base_delay * (2**attempt) + random.uniform(0, 1), max_delay)
                    logger.warning(
                        f"Timeout, retrying in {delay:.2f}s (attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(delay)
                    continue
                ...
                if attempt < max_retries:
                    delay = min(base_delay * (2**attempt) + random.uniform(0, 1), max_delay)
                    logger.warning(
                        f"Error: {e}, retrying in {delay:.2f}s (attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(delay)
                    continue
    ...
    def suggest_command(
        self, prompt: str, shell: Optional[str] = None, use_cache: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if use_cache and self.cache and result["success"]:
            self.cache.put(
                "copilot_suggest", response, ttl=self.cache_ttl, prompt=prompt, shell=shell
            )
