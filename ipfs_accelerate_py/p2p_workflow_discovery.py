...
try:
    from ipfs_accelerate_py.p2p_workflow_scheduler import P2PWorkflowScheduler, P2PTask, WorkflowTag

    HAVE_P2P_SCHEDULER = True
except ImportError:
    try:
        from p2p_workflow_scheduler import P2PWorkflowScheduler, P2PTask, WorkflowTag
...
class P2PWorkflowDiscoveryService:
    ...
    def _taskqueue_enabled(self) -> bool:
        return _truthy_env(
            os.environ.get(
                "IPFS_ACCELERATE_PY_P2P_WORKFLOW_TASKQUEUE",
                os.environ.get("IPFS_DATASETS_PY_P2P_WORKFLOW_TASKQUEUE"),
            ),
            default=True,
        )
    ...
    def _workflow_payload_from_discovery(
        self, discovery: WorkflowDiscovery, *, priority: int
    ) -> Dict[str, Any]:
        return {
    ...
    def _scheduler_snapshot_payload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "merkle_clock": self.scheduler.merkle_clock.to_dict()
            if hasattr(self.scheduler, "merkle_clock")
            else {},
            "pending_tasks": [
                _task_to_dict(t)
                for t in list(getattr(self.scheduler, "pending_tasks", {}).values())
            ],
            "assigned_tasks": [
                _task_to_dict(t)
                for t in list(getattr(self.scheduler, "assigned_tasks", {}).values())
            ],
            "completed_tasks": [
                _task_to_dict(t)
                for t in list(getattr(self.scheduler, "completed_tasks", {}).values())
            ],
        }
    ...
    async def _taskqueue_submit(
        self, *, task_type: str, payload: Dict[str, Any]
    ) -> Optional[Dict[str, str]]:
        if not (
            HAVE_TASKQUEUE_P2P and self._taskqueue_enabled() and self._taskqueue_remote is not None
        ):
            return None
        if taskqueue_submit_task_with_info is None:
            return None
        try:
            return await taskqueue_submit_task_with_info(
                remote=self._taskqueue_remote, task_type=task_type, model_name="", payload=payload
            )
        except Exception as e:
    ...
    def discover_workflows_in_repo(self, owner: str, repo: str) -> List[WorkflowDiscovery]:
        ...
        try:
            ...
            try:
                ...
                result = self.gh.api(
                    f"repos/{owner}/{repo}/contents/.github/workflows", method="GET"
                )
                ...
                for file_info in result:
                    ...
                    try:
                        content_result = self.gh.api(
                            f"repos/{owner}/{repo}/contents/{workflow_path}", method="GET"
                        )
                        ...
                        if content_b64:
                            ...
                            if self._should_process_workflow(tags):
                                ...
                                logger.info(
                                    f"✓ Discovered P2P workflow: {owner}/{repo}/{workflow_name} (tags: {', '.join(tags)})"
                                )
    ...
    def submit_workflow_to_scheduler(self, discovery: WorkflowDiscovery) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"✓ Submitted workflow to P2P scheduler: {task_id} (priority: {priority})"
                )

                # Task A: also submit to TaskQueue over libp2p as a portable payload.
                if (
                    HAVE_TASKQUEUE_P2P
                    and self._taskqueue_enabled()
                    and self._taskqueue_remote is not None
                ):
                    try:
                        import anyio

                        async def _do_submit() -> None:
                            payload = self._workflow_payload_from_discovery(
                                discovery, priority=priority
                            )
                            await self._taskqueue_submit(
                                task_type="p2p-workflow-discovery", payload=payload
                            )
    ...
    def run_discovery_cycle(self) -> Dict[str, Any]:
        ...
        if HAVE_TASKQUEUE_P2P and self._taskqueue_enabled() and self._taskqueue_remote is not None:
            try:
                ...
                async def _do_snap() -> None:
                    await self._taskqueue_submit(
                        task_type="p2p-workflow-scheduler-snapshot",
        ...
        logger.info(
            f"Discovery cycle complete: {len(discoveries)} discovered, {submitted} submitted"
        )
    ...
    def start(self) -> None:
        ...
        while self.running:
            ...
            try:
                ...
                logger.info(
                    f"Status: {stats['discovered']} discovered, {stats['submitted']} submitted"
                )
                logger.info(
                    f"Scheduler: {stats['scheduler']['pending_tasks']} pending, "
...
def main():
    ...
    parser.add_argument("--owner", type=str, help="GitHub owner (user or org) to monitor")
    parser.add_argument(
