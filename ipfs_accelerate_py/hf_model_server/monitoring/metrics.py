...
class PrometheusMetrics:
    ...
    def __init__(self):
        ...
        self.requests_total = Counter(
            "hf_server_requests_total", "Total number of requests", ["model", "endpoint", "status"]
        )
        ...
        # Model metrics
        self.models_loaded = Gauge("hf_server_models_loaded", "Number of models currently loaded")

        self.model_load_duration = Histogram(
            "hf_server_model_load_duration_seconds", "Model loading duration in seconds", ["model"]
        )

        # Cache metrics
        self.cache_hits = Counter("hf_server_cache_hits_total", "Total cache hits", ["cache_type"])

        self.cache_misses = Counter(
            "hf_server_cache_misses_total", "Total cache misses", ["cache_type"]
        )

        # Error metrics
        self.errors_total = Counter(
            "hf_server_errors_total", "Total number of errors", ["model", "error_type"]
        )
        ...
        self.memory_used_mb = Gauge(
            "hf_server_memory_used_mb", "Memory used by loaded models in MB"
        )

        self.memory_limit_mb = Gauge(
            "hf_server_memory_limit_mb", "Memory limit for model cache in MB"
        )
