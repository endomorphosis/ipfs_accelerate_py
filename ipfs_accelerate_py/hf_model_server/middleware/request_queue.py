...
class RequestQueue:
    ...
    def __init__(
        self, max_size: int = 100, timeout_seconds: float = 30.0, enable_priority: bool = True
    ):
        """
        ...
        while True:
            async with self._lock:
                ...
                if self._queue:
                    ...
                    logger.debug(
                        f"Dequeued request: {request.request_id} (queue size: {len(self._queue)})"
                    )
                    return request
...
class QueueManager:
    ...
    def __init__(
        ...
        if enable_per_model_queues:
            ...
            self._global_queue = RequestQueue(
                max_size=default_max_size, timeout_seconds=default_timeout
            )
    ...
    def _get_queue(self, model_id: Optional[str] = None) -> RequestQueue:
        ...
        if model_id not in self._queues:
            self._queues[model_id] = RequestQueue(
                max_size=self.default_max_size, timeout_seconds=self.default_timeout
            )
    ...
    async def dequeue(
        self, model_id: Optional[str] = None, timeout: Optional[float] = None
