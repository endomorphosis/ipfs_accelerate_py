...
class ServerConfig(BaseModel):
    ...
    auto_discover: bool = Field(
        default=True, description="Automatically discover skills on startup"
    )
    ...
    require_auth: bool = Field(
        default=False, description="Require authentication for all endpoints"
    )
    admin_api_key: Optional[str] = Field(
        default=None, description="Admin API key for management endpoints"
    )

    # Rate Limiting (Phase 5)
    enable_rate_limiting: bool = Field(default=False, description="Enable rate limiting")
    default_rate_limit: int = Field(
        default=100, description="Default rate limit (requests per minute)"
    )
    ...
    def from_env(cls) -> "ServerConfig":
        ...
        return cls(
            ...
            enable_circuit_breaker=os.getenv("HF_SERVER_ENABLE_CIRCUIT_BREAKER", "true").lower()
            == "true",
            ...
            enable_rate_limiting=os.getenv("HF_SERVER_ENABLE_RATE_LIMITING", "false").lower()
            == "true",
            enable_request_queue=os.getenv("HF_SERVER_ENABLE_REQUEST_QUEUE", "false").lower()
            == "true",
        )
