...
try:
    from .datasets_integration import is_datasets_available, DatasetsManager, ProvenanceLogger
...
class DatabaseHandler:
        ...
        if HAVE_DATASETS_INTEGRATION and is_datasets_available():
            try:
                ...
                self._datasets_manager = DatasetsManager(
                    {"enable_audit": True, "enable_provenance": True}
                )
    ...
    def store_hardware_detection(
        self, hardware_type: str, available: bool, device_name: str = None, details: Dict = None
    ) -> bool:
        """
    ...
    def generate_report(self, days: int = 7, format: str = "markdown") -> str:
        ...
        try:
            ...
            if (
                ...
                try:
                    cache_key = (
                        f"benchmark_report_{format}_{days}days_{int(datetime.now().timestamp())}"
                    )
    ...
    def _format_markdown_report(self, accel_results, ipfs_ops, hardware, days):
        ...
        if accel_results:
            ...
            for row in accel_results:
                ...
                report.append(
                    f"| {model} | {endpoint} | {accel_type} | {total} | {success_rate:.1f}% | {avg_time_str} |"
                )
        else:
        ...
        if ipfs_ops:
            ...
            for row in ipfs_ops:
                ...
                report.append(
                    f"| {op_type} | {total} | {success_rate:.1f}% | {avg_time_str} | {avg_size_str} |"
                )
        else:
    ...
    def _format_html_report(self, accel_results, ipfs_ops, hardware, days):
        ...
        if accel_results:
            html.append("<table>")
            html.append(
                "    <tr><th>Model</th><th>Endpoint</th><th>Acceleration</th>"
            ...
            for row in accel_results:
                ...
                html.append(
                    f"    <tr><td>{model}</td><td>{endpoint}</td><td>{accel_type}</td>"
        ...
        if ipfs_ops:
            html.append("<table>")
            html.append(
                "    <tr><th>Operation</th><th>Total</th><th>Success Rate</th>"
            ...
            for row in ipfs_ops:
                ...
                html.append(
                    f"    <tr><td>{op_type}</td><td>{total}</td><td>{success_rate:.1f}%</td>"
        ...
        if hardware:
            html.append("<table>")
            html.append(
                "    <tr><th>Hardware</th><th>Detection Count</th><th>Availability Rate</th></tr>"
            )
            ...
            for row in hardware:
                ...
                html.append(
                    f"    <tr><td>{hw_type}</td><td>{count}</td><td>{avail_rate:.1f}%</td></tr>"
                )
