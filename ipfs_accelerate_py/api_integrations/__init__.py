    ...
    def _chat_cached(
        ...
        **kwargs,
        ...
        if not use_cache or not self._cache:
            return getattr(self._api, method_name)(
        ...
        response = getattr(self._api, method_name)(
            messages=messages, model=model, temperature=temperature, **kwargs
        )
...
class CachedGeminiAPI(CachedLLMAPI):
    ...
    def generate_text(
        ...
        **kwargs,
        ...
        return self._completion_cached(
            prompt, model, temperature, use_cache, "generate_text", **kwargs
        )
...
class CachedVLLMAPI(CachedLLMAPI):
    ...
    def generate(
        self, prompt: str, model: str, temperature: float = 0.7, use_cache: bool = True, **kwargs
    ) -> Any:
        """Generation with caching."""
        return self._completion_cached(prompt, model, temperature, use_cache, "generate", **kwargs)
