...
try:
    ...
    if _is_pytest():
        logger.info(
            "sseclient package not found. Streaming will not be available. Install with: pip install sseclient-py"
        )
    else:
        logger.warning(
            "sseclient package not found. Streaming will not be available. Install with: pip install sseclient-py"
...
CHAT_MODELS = {
    ...
    "llama3-70b-8192": {"context_window": 8192, "description": "Meta's LLaMA 3 70B model"},
    "llama3-8b-8192": {"context_window": 8192, "description": "Meta's LLaMA 3 8B model"},
    "mixtral-8x7b-32768": {
    ...
    "mistral-saba-24b": {"context_window": 32768, "description": "Mistral's Saba 24B model"},
...
class APIUsageTracker:
    ...
    def _calculate_cost(self, model: str, prompt_tokens: int, completion_tokens: int) -> float:
        ...
        # Calculate cost
        cost = (prompt_tokens * model_rates["prompt"]) + (
            completion_tokens * model_rates["completion"]
        )
        return cost
...
class groq(BaseAPIBackend):
    def __init__(self, resources=None, metadata=None):
        ...
        if not self.api_key:
            logger.warning(
                "No Groq API key provided. Set GROQ_API_KEY environment variable or provide in metadata."
            )
    ...
    def count_tokens(self, text, model=None):
        ...
        return {"estimated_token_count": token_count, "estimation_method": method, "model": model}
    ...
    def init(self, endpoint_url=None, api_key=None, model_name=None):
        ...
        endpoint_handler = self.create_remote_groq_endpoint_handler(
            ...
            while retries < max_retries:
                try:
                    ...
                    if retries < max_retries - 1:
                        logger.warning(
                            f"Request failed: {str(e)}. Retrying in {retry_delay} seconds (attempt {retries + 1}/{max_retries})..."
                        )
    ...
    def make_stream_request_groq(self, endpoint_url, data, api_key=None, request_id=None):
        ...
        if not SSECLIENT_AVAILABLE:
            raise ValueError(
                "sseclient package is required for streaming. Install with: pip install sseclient-py"
            )
        ...
        if not request_id:
            request_id = (
                f"stream_{int(time.time())}_{hashlib.md5(str(data).encode()).hexdigest()[:8]}"
            )
        ...
        while retries < self.max_retries:
            try:
                response = requests.post(
                    endpoint_url, headers=headers, json=data, stream=True, timeout=90
                )
                ...
                if response.status_code == 401:
                    ...
                    logger.warning(
                        f"Rate limit exceeded (429). Waiting {wait_time} seconds before retry (attempt {retries + 1}/{self.max_retries})."
                    )
                    ...
                    if response.status_code >= 500 and retries < self.max_retries - 1:
                        logger.warning(
                            f"Server error ({response.status_code}): {error_msg}. Retrying in {retry_delay} seconds..."
                        )
                        time.sleep(retry_delay)
                        retries += 1
                        retry_delay = min(retry_delay * self.backoff_factor, self.max_retry_delay)
                ...
                if retries < self.max_retries - 1:
                    logger.warning(
                        f"Streaming request failed: {str(e)}. Retrying in {retry_delay} seconds (attempt {retries + 1}/{self.max_retries})..."
                    )
                    time.sleep(retry_delay)
                    retries += 1
                    retry_delay = min(retry_delay * self.backoff_factor, self.max_retry_delay)
        ...
        raise ValueError(
            f"Stream request failed after {self.max_retries} attempts with exponential backoff"
        )
        ...
        if not self.is_compatible_model(model_name, "chat"):
            ...
            if model_name in ALL_MODELS:
                ...
                if model_info.get("chat_compatible") is False:
                    raise ValueError(
                        f"Model '{model_name}' is not compatible with chat completions. "
                ...
                raise ValueError(
                    f"Model '{model_name}' not found.{suggestion} Available chat models: {', '.join(list(CHAT_MODELS.keys())[:3])}..."
                )
    ...
    def stream_chat(
        ...
        if not SSECLIENT_AVAILABLE:
            logger.warning(
                "Streaming not available (sseclient not installed). Falling back to regular chat."
            )
        ...
        if request_id is None:
            request_id = (
                f"stream_{int(time.time())}_{hashlib.md5(str(messages).encode()).hexdigest()[:8]}"
            )
        ...
        try:
            ...
            accumulated_text = ""
            for chunk in self.make_stream_request_groq(
                endpoint_url, data, self.api_key, request_id
            ):
                if "choices" in chunk and len(chunk["choices"]) > 0:
                    ...
                    if "content" in delta:
                        ...
                        "finish_reason" in chunk["choices"][0]
    ...
    def create_remote_groq_endpoint_handler(self, endpoint_url, api_key=None, model_name=None):
        ...
        def handler(
            ...
            try:
                default_params = {"max_tokens": 128, "temperature": 0.7, "top_p": 0.95}
    ...
    def get_stats(self, endpoint_id=None):
        ...
        if endpoint_id and endpoint_id in self.endpoints:
            ...
            total_requests = (
                sum(e["total_requests"] for e in self.endpoints.values()) if self.endpoints else 0
            )
            successful_requests = (
                sum(e["successful_requests"] for e in self.endpoints.values())
                if self.endpoints
                else 0
            )
            failed_requests = (
                sum(e["failed_requests"] for e in self.endpoints.values()) if self.endpoints else 0
            )
            total_tokens = (
                sum(e["total_tokens"] for e in self.endpoints.values()) if self.endpoints else 0
            )
            input_tokens = (
                sum(e["input_tokens"] for e in self.endpoints.values()) if self.endpoints else 0
            )
            output_tokens = (
                sum(e["output_tokens"] for e in self.endpoints.values()) if self.endpoints else 0
            )
