...
class ollama(BaseAPIBackend):
    ...
    def make_post_request_ollama(
        self, endpoint_url, data, stream=False, request_id=None, priority=None
    ):
        ...
        if at_capacity and getattr(self, "queue_enabled", True):
            result_future = self.queue_with_priority(
                {"func": _execute_request, "request_id": request_id}, priority
            )
            max_wait = 300
    ...
    def chat(
        ...
        **kwargs,
        ...
        data = {"model": model_to_use, "messages": formatted_messages, "stream": False}
        ...
        try:
            ...
            if "prompt_eval_count" in response and "eval_count" in response:
                with self.queue_lock:
                    ...
                    self.usage_stats["total_tokens"] += response.get(
                        "prompt_eval_count", 0
    ...
    def generate(
        self, model=None, prompt=None, max_tokens=None, temperature=None, request_id=None, **kwargs
    ):
        """
    ...
    def stream_chat(
        ...
        **kwargs,
        ...
        # Prepare request data
        data = {"model": model_to_use, "messages": formatted_messages, "stream": True}
        ...
        try:
            response_stream = self.make_post_request_ollama(
                endpoint_url, data, stream=True, request_id=request_id
            )
            ...
            yield {"text": f"Error: {str(e)}", "error": str(e), "done": True, "model": model_to_use}
    ...
    def list_models(self):
        ...
        try:
            response = requests.get(endpoint_url, timeout=self.metadata.get("timeout", 30))
            ...
            for key, value in stats_update.items():
                if key in self.request_stats:
                    if isinstance(self.request_stats[key], dict) and isinstance(value, dict):
                        ...
                        total = self.request_stats["total_response_time"] + stats_update.get(
                            "response_time", 0
                        )
        ...
        report = {
            "summary": {
                ...
                * 100
                ...
                "retry_rate": (stats.get("retried_requests", 0) / stats.get("total_requests", 1))
                * 100
                if stats.get("total_requests", 0) > 0
                else 0,
            },
