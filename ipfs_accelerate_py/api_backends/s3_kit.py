...
class s3_kit(BaseAPIBackend):
    ...
    def _queue_operation(self, operation, **kwargs):
            ...
            while True:
                ...
                try:
                    ...
                    self.track_request_result(
                        False, error_type=type(e).__name__, operation=operation, latency=latency
                    )
                    ...
                    retry_count += 1
                    logger.info(
                        f"Retrying request {request_id} after {delay}s delay (retry {retry_count}/{self.max_retries})"
                    )
                    time.sleep(delay)
        ...
        if at_capacity:
            result_future = self.queue_with_priority(
                {"func": _execute_operation, "request_id": request_id}, self.PRIORITY_NORMAL
            )
    ...
    def upload_file(self, file_path, bucket, key):
        """Upload a file to S3"""
        return self._queue_operation("upload_file", Filename=file_path, Bucket=bucket, Key=key)

    def download_file(self, bucket, key, file_path):
        """Download a file from S3"""
        return self._queue_operation("download_file", Bucket=bucket, Key=key, Filename=file_path)
    ...
    def delete_object(self, bucket, key):
        ...
        return self._queue_operation("delete_object", Bucket=bucket, Key=key)

    def head_object(self, bucket, key):
        """Get object metadata"""
        return self._queue_operation("head_object", Bucket=bucket, Key=key)
    ...
    def create_s3_endpoint_handler(
        self,
        endpoint_url=None,
        access_key=None,
        secret_key=None,
            ...
            with circuit_lock:
                ...
                if endpoint_config["circuit_state"] == "OPEN":
                    ...
                    if now - endpoint_config["last_failure_time"] > self.reset_timeout:
                        logger.info(
                            f"Endpoint {endpoint_config['endpoint']}: Circuit breaker transitioning from OPEN to HALF-OPEN"
                        )
        ...
        def track_endpoint_result(success, error_type=None, operation=None, latency=None):
            with circuit_lock:
                if success:
                    ...
                    if endpoint_config["circuit_state"] == "HALF-OPEN":
                        ...
                        logger.info(
                            f"Endpoint {endpoint_config['endpoint']}: Circuit breaker transitioning from HALF-OPEN to CLOSED"
                        )
                    ...
                    if (
                        endpoint_config["circuit_state"] == "CLOSED"
                        and endpoint_config["failure_count"]
                        >= endpoint_config["circuit_breaker_threshold"]
                    ):
                        # Too many failures, open the circuit
                        logger.warning(
                            f"Endpoint {endpoint_config['endpoint']}: Circuit breaker transitioning from CLOSED to OPEN after {endpoint_config['failure_count']} failures"
                        )
                        ...
                        logger.warning(
                            f"Endpoint {endpoint_config['endpoint']}: Circuit breaker transitioning from HALF-OPEN to OPEN after test request failure"
                        )
                        endpoint_config["circuit_state"] = "OPEN"
            ...
            if self.collect_metrics:
                self.update_metrics(
                    operation=operation, success=success, latency=latency, error_type=error_type
                )
        ...
        def sync_endpoint_handler(operation, **kwargs):
            ...
            with endpoint_lock:
                if endpoint_config["active_requests"] >= endpoint_config["max_concurrent_requests"]:
                    error_msg = (
                        f"Too many concurrent requests for endpoint {endpoint_config['endpoint']}"
                    )
                    logger.warning(error_msg)
            ...
            try:
                    ...
                    try:
                        ...
                        if operation == "upload_file":
                            result = s3_client.upload_file(
                                kwargs.get("file_path"), kwargs.get("bucket"), kwargs.get("key")
                            )
                        elif operation == "download_file":
                            result = s3_client.download_file(
                                kwargs.get("bucket"), kwargs.get("key"), kwargs.get("file_path")
                            )
                            ...
                            result = s3_client.delete_object(
                                Bucket=kwargs.get("bucket"), Key=kwargs.get("key")
                            )
                            ...
                            result = s3_client.generate_presigned_url(
                                "get_object",
                                Params={"Bucket": kwargs.get("bucket"), "Key": kwargs.get("key")},
                        ...
                        track_endpoint_result(success=True, operation=operation, latency=latency)
                        ...
                        if retry_count < endpoint_config["max_retries"]:
                            ...
                            logger.info(
                                f"Retrying {operation} for endpoint {endpoint_config['endpoint']} after {delay}s delay (retry {retry_count + 1}/{endpoint_config['max_retries']})"
                            )
                            ...
                            logger.error(
                                f"Max retries exceeded for {operation} on endpoint {endpoint_config['endpoint']}"
                            )
                ...
                with endpoint_lock:
                    endpoint_config["active_requests"] = max(
                        0, endpoint_config["active_requests"] - 1
                    )
        ...
        async def async_endpoint_handler(operation, **kwargs):
            ...
            try:
                ...
                logger.error(
                    f"Error in async S3 operation {operation} for endpoint {endpoint_config['endpoint']}: {e}"
                )
                return {"error": str(e)}
    ...
    def test_s3_endpoint(self, endpoint_url=None, access_key=None, secret_key=None):
        ...
        try:
            ...
            s3_client = boto3.client(
                "s3", aws_access_key_id=access, aws_secret_access_key=secret, endpoint_url=endpoint
            )
    ...
    def update_metrics(
        self, operation=None, success=True, latency=None, error_type=None, retried=False
    ):
        ...
        with self.stats_lock:
            ...
            if latency:
                current_total = self.request_stats["average_latency"] * (
                    self.request_stats["total_requests"] - 1
                )
                self.request_stats["average_latency"] = (
                    current_total + latency
        ...
        with self.circuit_lock:
            if success:
                ...
                if self.circuit_state == "CLOSED" and self.failure_count >= self.failure_threshold:
                    # Too many failures, open the circuit
                    logger.warning(
                        f"Circuit breaker transitioning from CLOSED to OPEN after {self.failure_count} failures"
                    )
                    ...
                    # Failed during test request, back to open
                    logger.warning(
                        "Circuit breaker transitioning from HALF-OPEN to OPEN after test request failure"
                    )
                    self.circuit_state = "OPEN"
