...
# Try to import datasets integration for API routing tracking
try:
...
openai_api_class = (
    getattr(openai_api, "openai_api", None) if openai_api else create_mock_class("openai_api")
)
    ...
    def __init__(self, resources=None, metadata=None):
        ...
        if "hf_tei" not in self.resources:
            if hf_tei_class is not None:
                ...
                self.resources["hf_tei"] = type(
                    "MockHFTEI",
                    (),
                    {
                        "request_hf_tei_endpoint": lambda *args: None,
                        "test_hf_tei_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "hf_tgi" not in self.resources:
            if hf_tgi_class is not None:
                ...
                # Create a mock implementation
                self.resources["hf_tgi"] = type(
                    "MockHFTGI",
                    (),
                    {
                        "request_tgi_endpoint": lambda *args: None,
                        "test_tgi_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "groq" not in self.resources:
            if groq_class is not None:
                ...
                # Create a mock implementation
                self.resources["groq"] = type(
                    "MockGROQ",
                    (),
                    {
                        "request_groq_endpoint": lambda *args: None,
                        "test_groq_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "llvm" not in self.resources:
            if llvm_class is not None:
                ...
                # Create a mock implementation
                self.resources["llvm"] = type(
                    "MockLLVM",
                    (),
                    {
                        "request_llvm_endpoint": lambda *args: None,
                        "test_llvm_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "ollama" not in self.resources:
            if ollama_class is not None:
                ...
                # Create a mock implementation
                self.resources["ollama"] = type(
                    "MockOllama",
                    (),
                    {
                        "request_ollama_endpoint": lambda *args: None,
                        "test_ollama_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "opea" not in self.resources:
            if opea_class is not None:
                ...
                # Create a mock implementation
                self.resources["opea"] = type(
                    "MockOPEA",
                    (),
                    {
                        "request_opea_endpoint": lambda *args: None,
                        "test_opea_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "ovms" not in self.resources:
            if ovms_class is not None:
                ...
                # Create a mock implementation
                self.resources["ovms"] = type(
                    "MockOVMS",
                    (),
                    {
                        "request_ovms_endpoint": lambda *args: None,
                        "test_ovms_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "openai_api" not in self.resources:
            if openai_api_class is not None:
                ...
                # Create a mock implementation
                self.resources["openai_api"] = type(
                    "MockOpenAI",
                    (),
                    {
                        "request_openai_api_endpoint": lambda *args: None,
                        "test_openai_api_endpoint": lambda *args: None,
                    },
                )()
        ...
        if "s3_kit" not in self.resources:
            if s3_kit_class is not None:
                ...
                # Create a mock implementation
                self.resources["s3_kit"] = type(
                    "MockS3Kit",
                    (),
                    {
                        "request_s3_kit_endpoint": lambda *args: None,
                        "test_s3_kit_endpoint": lambda *args: None,
                    },
                )()
        ...
        for api_type in api_types:
            ...
            try:
                if hasattr(self.resources[api_type], "create_" + api_type + "_endpoint_handler"):
                    get_remote_endpoint_handler = getattr(
                        self.resources[api_type], "create_" + api_type + "_endpoint_handler"
                    )
                    setattr(
                        self,
                        "create_" + api_type + "_endpoint_handler",
                        get_remote_endpoint_handler,
                    )
                else:
                ...
                if hasattr(self.resources[api_type], "make_post_request_" + api_type):
                    get_remote_post_request = getattr(
                        self.resources[api_type], "make_post_request_" + api_type
                    )
                                    ...
                                    if len(endpoint) == 3:
                                        ...
                                        if model == this_model:
                                            ...
                                            if model not in self.resources.get("batch_sizes", {}):
                                                self.resources.setdefault("batch_sizes", {})[
                                                    model
                                                ] = {}
                                            if model not in self.resources.get("queues", {}):
                                                self.resources.setdefault("queues", {})[model] = {}
                                            if model not in self.resources.get(
                                                "endpoint_handler", {}
                                            ):
                                                self.resources.setdefault("endpoint_handler", {})[
                                                    model
                                                ] = {}

                                            # Set up the endpoint
                                            self.resources["batch_sizes"][model][this_endpoint] = 0
                                            self.resources["queues"][model][this_endpoint] = (
                                                anyio.create_memory_object_stream(64)
                                            )
                                            ...
                                            try:
                                                if hasattr(self, "create_endpoint_handler"):
                                                    self.resources["endpoint_handler"][model][
                                                        this_endpoint
                                                    ] = self.create_endpoint_handler(
                                                        model, this_endpoint, context_length
                                                    )
                                                else:
