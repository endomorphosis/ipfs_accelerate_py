...
class vllm(BaseAPIBackend):
    ...
    def init(self, endpoint_url=None, api_key=None, model_name=None, endpoint_type="completion"):
        ...
        if endpoint_type == "chat":
            endpoint_handler = self.create_remote_vllm_chat_endpoint_handler(
                endpoint_url, api_key, model_name
            )
        elif endpoint_type == "streaming":
            endpoint_handler = self.create_remote_vllm_streaming_endpoint_handler(
                endpoint_url, api_key, model_name
            )
        else:
            endpoint_handler = self.create_remote_vllm_endpoint_handler(
                endpoint_url, api_key, model_name
            )
        ...
        return (
            ...
            endpoint_handler,
            self.request_queue,
            self.endpoint_status[endpoint_url],
        )
        ...
        if endpoint_type == "chat":
            ...
            try:
                ...
                if first_chunk is not None:
                    ...
                    print(
                        f"Remote vLLM streaming test failed for {endpoint_label}: No response chunks"
                    )
                    return False
            except Exception as e:
    ...
    def test_vllm_endpoint(
        self, endpoint_url=None, api_key=None, model_name=None, endpoint_type="completion"
    ):
        ...
        if endpoint_type == "chat":
            ...
            try:
                ...
                first_chunk = next(
                    self.make_streaming_request_vllm(endpoint_url, data, api_key), None
                )
            ...
            try:
                ...
                if result and ("text" in result or "choices" in result or "response" in result):
                    ...
                    print(
                        f"Completion test failed for endpoint {endpoint_url}: Invalid response format"
                    )
                    return False
    ...
    def create_vllm_endpoint_handler(
        self, model=None, endpoint=None, endpoint_type="completion", batch=None
    ):
        ...
        if model in self.endpoints:
            for e in self.endpoints[model]:
                ...
                if model in self.registered_models:
                    ...
                    if endpoint_idx >= 0 and endpoint_idx < len(
                        self.registered_models[model]["types"]
                    ):
                        if endpoint_type == self.registered_models[model]["types"][endpoint_idx]:
                            if (
                                e in self.endpoint_status
                                and self.endpoint_status[e] >= incoming_batch_size
                            ):
                                return e
    ...
    def create_remote_vllm_streaming_endpoint_handler(
        self, endpoint_url, api_key=None, model_name=None
    ):
        ...
        def handler(
            ...
            try:
                ...
                if isinstance(prompt_or_messages, list) and all(
                    isinstance(msg, dict) for msg in prompt_or_messages
                ):
