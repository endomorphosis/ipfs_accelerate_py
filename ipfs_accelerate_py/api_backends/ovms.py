...
class ovms(BaseAPIBackend):
    def __init__(self, resources=None, metadata=None):
        ...
        self.api_url = self.metadata.get(
        ...
        self.default_version = self.metadata.get(
            "ovms_version", os.environ.get("OVMS_VERSION", "latest")
        )
        self.default_precision = self.metadata.get(
    ...
    def make_post_request_ovms(
        self, endpoint_url, data, request_id=None, endpoint_id=None, api_key=None
    ):
    ...
    def get_stats(self, endpoint_id=None):
        ...
        if endpoint_id and endpoint_id in self.endpoints:
            ...
            total_processing_time = (
                sum(e["total_processing_time"] for e in self.endpoints.values())
                if self.endpoints
                else 0
            )
            ...
            stats = {
                ...
                "global_queue_size": self.request_queue.qsize()
                if hasattr(self.request_queue, "qsize")
                else len(self.request_queue)
                if isinstance(self.request_queue, list)
                else 0,
    ...
    def make_request_with_endpoint(
        self, endpoint_id, data, model=None, version=None, endpoint_url=None
    ):
    ...
    def test_ovms_endpoint(
        self, endpoint_url=None, model_name=None, endpoint_id=None, request_id=None
    ):
        ...
        if endpoint_id and endpoint_id in self.endpoints:
            ...
            if request_id is None:
                request_id = (
                    f"test_{int(time.time())}_{hashlib.md5(model_name.encode()).hexdigest()[:8]}"
                )
