...
class HuggingFaceModelSearchEngine:
    ...
    def __init__(
        self, model_manager: Optional[ModelManager] = None, cache_dir: str = "./hf_model_cache"
    ):
            ...
            if (
                ...
                try:
                    ...
                    if cache_data_str:
                        ...
                        self.models_cache = {
                            k: HuggingFaceModelInfo(**v) for k, v in cache_data.items()
                        }
                        logger.info(
                            f"Loaded {len(self.models_cache)} models from distributed storage"
                        )
            ...
            if (
                ...
                try:
                    self._storage.write_file(
                        json.dumps(cache_data, indent=2), "hf_models_cache", pin=False
                    )
                    self._storage.write_file(
                        json.dumps(self.search_index, indent=2), "hf_search_index", pin=False
                    )
    ...
    def get_detailed_model_info(
        self, model_id: str, include_repo_structure: bool = True
        ...
        try:
            ...
            if include_repo_structure:
                repo_structure = fetch_huggingface_repo_structure(model_id, include_ipfs_cids=True)
                if repo_structure:
    ...
    def add_model_to_manager(self, model_info: HuggingFaceModelInfo) -> bool:
        ...
        try:
            ...
            model_type = model_type_mapping.get(model_info.pipeline_tag, ModelType.LANGUAGE_MODEL)
            ...
            metadata = ModelMetadata(
                ...
                if model_info.library_name == "transformers"
                else [],
    ...
    def _create_io_specs(
        self, pipeline_tag: Optional[str], config: Optional[Dict[str, Any]]
        ...
        if pipeline_tag == "text-generation":
            ...
            outputs = [
                IOSpec(
                    name="text" if "speech" in pipeline_tag else "classification",
    ...
    def local_search(self, query: str, max_results: int = 50) -> List[HuggingFaceModelInfo]:
        ...
        for model_id, model_info in self.models_cache.items():
            if (
                ...
                or query_lower in model_info.model_name.lower()
                or query_lower in model_info.description.lower()
            ):
    ...
    def get_popular_models(
        self, limit: int = 20, task_filter: Optional[str] = None
    ) -> List[HuggingFaceModelInfo]:
        """
    ...
    def bulk_scrape_models(
        self, model_ids: List[str], add_to_manager: bool = True, include_repo_structure: bool = True
    ) -> Dict[str, bool]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "unique_libraries": len(
                set(m.library_name for m in self.models_cache.values() if m.library_name)
            ),
            "pipeline_tags": {},
            "languages": {},
            "licenses": {},
        ...
        for model in self.models_cache.values():
            if model.pipeline_tag:
                stats["pipeline_tags"][model.pipeline_tag] = (
                    stats["pipeline_tags"].get(model.pipeline_tag, 0) + 1
                )
...
def main():
    ...
    models = search_engine.search_huggingface_models(
        query="gpt", limit=10, filter_dict={"pipeline_tag": "text-generation"}, sort="downloads"
    )
