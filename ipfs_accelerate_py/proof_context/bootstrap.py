...
def _artifact_to_mapping(value: Any) -> dict[str, Any]:
    """Return one stage-artifact mapping across safe module reloads.

    Cold-import qualification deliberately removes and reloads proof-context
    modules. An artifact created by the refreshed lifecycle module is still a
    valid typed artifact, but it is not an instance of bootstrap's earlier
    class object. Bind through the closed ``to_mapping`` contract instead of
    treating that harmless class-identity change as an arbitrary iterable.
    """

    converter = getattr(value, "to_mapping", None)
    if callable(converter):
        return dict(_as_mapping(converter()))
...
class RuntimeSession:
    ...
    def record_from_artifact(
        ...
        mapping = _artifact_to_mapping(artifact)
        identities = LifecycleIdentities.from_mapping(
            _as_mapping(mapping["identities"])
        )
        status = str(mapping.get("status") or "invalid")
        provenance = str(mapping.get("provenance") or "live")
        artifact_cid = str(
            mapping.get("artifact_cid") or identities.repository_state_cid
        )
        ...
        class _Persistence:
            def persist(
                ...
                *,
                ...
                payload = _artifact_to_mapping(artifact)
