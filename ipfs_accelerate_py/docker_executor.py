...
def docker_daemon_available(
    docker_command: str = "docker", timeout_s: float = 3.0, cache_s: float = 5.0
) -> bool:
...
class DockerExecutor:
        ...
        if self._datasets_manager is not None:
            try:
                self._datasets_manager.log_event(
                    ...
                    if result.success
                    else "container_execution_failed",
                    {
                        **artifact_payload,
            ...
            if requested and not retrieved and error:
                materialization_event = (
                    "model_artifact_materialization_failed"
                    if policy in {"required", "strict"}
                    else "model_artifact_materialization_degraded"
                )
                materialization_level = (
                    "ERROR" if materialization_event.endswith("failed") else "WARNING"
                )
                ...
                try:
                    ...
                    logger.debug(
                        "Failed to log Docker model artifact materialization event: %s", exc
                    )
                try:
                    self._datasets_manager.track_provenance(
                        materialization_event, materialization_payload
                    )
                except Exception as exc:
                    logger.debug(
                        "Failed to record Docker model artifact materialization provenance: %s", exc
                    )
    ...
    def _materialize_model_artifact(
    ...
    ) -> tuple[DockerExecutionConfig, Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(artifact, (bytes, bytearray)):
                suffix = (
                    Path(config.model_artifact_mount_path or "/workspace/model_artifact").name
                    or "model_artifact"
                )
    ...
    def _verify_docker_available(self):
        ...
        try:
            result = subprocess.run(
                [self.docker_command, "--version"], capture_output=True, text=True, timeout=5
            )
    ...
    def execute_container(self, config: DockerExecutionConfig) -> DockerExecutionResult:
        """
        ...
        try:
            ...
            if (
                ...
                error = str(
                    model_artifact_metadata.get("error") or "model artifact retrieval failed"
                )
                result = DockerExecutionResult(
                    success=False,
                    exit_code=-1,
    ...
    def build_and_execute_github_repo(
        self, github_config: GitHubDockerConfig, execution_config: DockerExecutionConfig
    ) -> DockerExecutionResult:
        """
        ...
        try:
            ...
            build_cmd = [self.docker_command, "build", "-t", image_tag, "-f", dockerfile_path]
            ...
            result = DockerExecutionResult(
                success=False, exit_code=-1, stdout="", stderr=str(e), error_message=str(e)
            )
            return self._record_execution_artifacts(config=execution_config, result=result)
        finally:
            # Cleanup
    ...
    def pull_image(self, image: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                [self.docker_command, "pull", image], capture_output=True, text=True, timeout=600
            )
