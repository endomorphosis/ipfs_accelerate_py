...
class HuggingFaceHubScanner:
    ...
    def scan_hub(
        ...
        core_results = self.scan_all_models(
            limit=limit, task_filter=task_filter, save_progress_every=save_every
        )
        ...
        comprehensive_results = {
            ...
            "scan_duration": (
                core_results.get("end_time", datetime.now())
                - core_results.get("start_time", datetime.now())
            ).total_seconds(),
            "model_manager_count": len(self.model_manager.list_models())
    ...
    def _get_all_models_list(
        self, limit: Optional[int] = None, task_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while len(models_data) == params["limit"] and (limit is None or len(models) < limit):
                ...
                new_models = [
                    m for m in models_data if m["id"] not in [existing["id"] for existing in models]
                ]
    ...
    def _process_single_model(self, model_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._add_model_to_manager(
                model_id, detailed_info, performance_data, compatibility_data
            )
        ...
        try:
            ...
            if self.search_engine:
                models_info = self.search_engine.search_huggingface_models(
                    query=model_id, limit=1, filter_dict={"model_id": model_id}
                )
    ...
    def _extract_performance_data(
        self, model_data: Dict[str, Any], detailed_info: HuggingFaceModelInfo
    ...
    def _extract_hardware_compatibility(
        self, model_data: Dict[str, Any], detailed_info: HuggingFaceModelInfo
    ...
    def _add_model_to_manager(
        ...
        try:
            ...
            model_metadata = self._convert_to_model_metadata(
                detailed_info, performance_data, compatibility_data
            )
    ...
    def _calculate_initial_reward(
        self, detailed_info: HuggingFaceModelInfo, performance_data: ModelPerformanceData
    ) -> float:
    ...
    def _convert_api_response_to_model_info(
        self, model_data: Dict[str, Any]
    ) -> Optional[HuggingFaceModelInfo]:
        ...
        try:
            ...
            return HuggingFaceModelInfo(
                ...
                library_name=model_data.get(
                    "library_name", model_data.get("libraryName", "transformers")
                ),
    ...
    def _save_scan_progress(self) -> None:
        ...
        try:
            ...
            models_data = {model_id: asdict(info) for model_id, info in self.model_cache.items()}
            ...
            if (
                ...
                try:
                    ...
                    self._storage.write_file(
                        json.dumps(models_data, indent=2, default=str), cache_key, pin=True
                    )
            ...
            if (
                ...
                try:
                    ...
                    self._storage.write_file(
                        json.dumps(performance_data, indent=2, default=str), cache_key, pin=True
                    )
            ...
            if (
                ...
                try:
                    ...
                    self._storage.write_file(
                        json.dumps(compatibility_data, indent=2, default=str), cache_key, pin=True
                    )
            ...
            if (
                ...
                try:
                    ...
                    self._storage.write_file(
                        json.dumps(self.scan_stats, indent=2, default=str), cache_key, pin=False
                    )
    ...
    def _generate_scan_report(self) -> None:
        ...
        if (
            ...
            try:
                ...
                self._storage.write_file(
                    json.dumps(report, indent=2, default=str), cache_key, pin=True
                )
        ...
        logger.info(
            f"Scan completed - {report['model_statistics']['total_models_cached']} models processed"
        )
    ...
    def get_model_recommendations(
        self, context: RecommendationContext, num_recommendations: int = 5
    ) -> List[Dict[str, Any]]:
    ...
    def search_models(
        ...
        for model_id, info in self.model_cache.items():
            ...
            if (not info.description or info.description.strip() == "") and (
                ...
                logger.info(
                    f"Found model {model_id} with empty description and model_card in cache, will refetch"
                )
                needs_refetch = True
                ...
                try:
                    ...
                    if fetched_card and fetched_card.strip():
                        ...
                        if not info.description or info.description.strip() == "":
                            info.description = self._extract_description_from_model_card(
                                fetched_card
                            )
                        # Update cache
                        self.model_cache[model_id] = info
                        logger.info(
                            f"Successfully updated {model_id} with model card and description"
                        )
                        # Save updated cache
                        self._save_scan_progress()
                except Exception as e:
            ...
            if score > 0:
                result = {"model_id": model_id, "score": score, "model_info": asdict(info)}
    ...
    def _search_huggingface_api(
        self, query: str, task_filter: Optional[str] = None, limit: int = 20
    ) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                for model in models_iterator:
                    model_dict = {
                        ...
                        "lastModified": str(model.lastModified)
                ...
                logger.info(
                    f"Retrieved {len(models)} models from HuggingFace Hub API (via huggingface_hub) for query '{query}'"
                )
                return models
            except Exception as e:
                logger.warning(
                    f"Error using huggingface_hub library: {e}, falling back to direct API"
                )
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(models)} models from HuggingFace API (direct) for query '{query}'"
            )
            return models
        except Exception as e:
            logger.warning(
                f"Error searching HuggingFace API (direct): {e}, using static model database"
            )
            # Final fallback: use static database of popular models
            return self._get_static_model_database(query, task_filter, limit)

    def _get_static_model_database(
        self, query: str, task_filter: Optional[str] = None, limit: int = 20
    ) -> List[Dict[str, Any]]:
...
Generally achieves better performance than BERT base on most NLP benchmarks, with typical improvements of 1-2% on tasks like GLUE, SQuAD, etc.""",
        ...
        for model in static_models:
            ...
            if not query_lower or (
                ...
                or query_lower in model["author"].lower()
        ...
        logger.info(
            f"Retrieved {len(filtered_models[:limit])} models from static database for query '{query}'"
        )
    ...
    def _convert_api_model_to_info(self, api_model: Dict[str, Any]) -> HuggingFaceModelInfo:
        ...
        if (description == "" or description.strip() == "") and (
            not model_card or model_card.strip() == ""
        ):
            logger.info(
                f"Empty description and model_card detected for {model_id}, fetching from HuggingFace"
            )
            fetched_card = self._fetch_model_card(model_id)
            if fetched_card and fetched_card.strip():
        ...
        try:
            ...
            if HAVE_HUGGINGFACE_HUB:
                try:
                    ...
                    download_path = snapshot_download(
                        repo_id=model_id, cache_dir=str(download_dir), resume_download=True
                    )
            ...
            try:
                ...
                for sibling in siblings:
                    ...
                    if any(
                        ...
                        for ext in [".json", ".bin", ".safetensors", ".txt", ".model"]
                    ):
                ...
                logger.warning(
                    f"Manual download failed: {manual_error}. Attempting simulated download..."
                )
                ...
                if model_info:
                    ...
                    if (
                        ...
                        try:
                            ...
                            self._storage.write_file(
                                json.dumps(metadata, indent=2), cache_key, pin=False
                            )
                            logger.debug(f"Saved model metadata to distributed storage: {model_id}")
                        except Exception as e:
                            logger.debug(
                                f"Failed to write model metadata to distributed storage: {e}"
                            )
                    ...
                    logger.info(
                        f"Model {model_id} not in static database. Creating placeholder download..."
                    )
            ...
            try:
                ...
                if (
                    ...
                    try:
                        cache_key = (
                            f"scanner_model_metadata_placeholder_{model_id.replace('/', '_')}"
                        )
                        self._storage.write_file(
                            json.dumps(metadata, indent=2), cache_key, pin=False
                        )
                        logger.debug(
                            f"Saved placeholder metadata to distributed storage: {model_id}"
                        )
                    except Exception as storage_e:
                        logger.debug(
                            f"Failed to write placeholder metadata to distributed storage: {storage_e}"
                        )
...
def main():
    ...
    # Initialize scanner
    scanner = HuggingFaceHubScanner(cache_dir=args.cache_dir, max_workers=args.workers)

    # Run scan
    results = scanner.scan_all_models(limit=args.limit, task_filter=args.task_filter)
