...
try:
    ...
    logger.warning(
        "Hardware selector module not available, falling back to basic hardware selection"
    )
...
try:
    ...
    logger.warning(
        "Model performance predictor module not available, falling back to heuristic predictions"
    )
...
class HardwareModelPredictor:
        ...
        if HARDWARE_SELECTOR_AVAILABLE:
            try:
                self.hardware_selector = HardwareSelector(
                    database_path=str(self.benchmark_dir), config_path=self.config_path
                )
    ...
    def predict_optimal_hardware(
        ...
        if available_hardware is None:
            available_hardware = [
                hw for hw, available in self.available_hardware.items() if available
            ]
        ...
        if model_family is None and INTEGRATION_AVAILABLE:
            ...
            if "bert" in model_name_lower or "roberta" in model_name_lower:
                ...
                "t5" in model_name_lower or "gpt" in model_name_lower or "llama" in model_name_lower
            ):
        ...
        if self.hardware_selector is not None:
            try:
                ...
                logger.info(
                    f"Used advanced hardware selector: {recommendation['primary_recommendation']} for {model_name}"
                )
                return recommendation
            except Exception as e:
                logger.warning(
                    f"Advanced hardware selection failed: {e}, falling back to basic selection"
                )
        ...
        if INTEGRATION_AVAILABLE:
            try:
                ...
                integration_result = integrate_hardware_and_model(
                    model_name=model_name, model_family=model_family, hardware_info=filtered_hw
                )
                ...
                result = {
                    ...
                    "fallback_options": [
                        hw
                        for hw in integration_result["compatible_devices"]
                        if hw != integration_result["device"]
                    ],
                ...
                logger.info(
                    f"Used hardware-model integration: {result['primary_recommendation']} for {model_name}"
                )
                return result
            except Exception as e:
                logger.warning(
                    f"Hardware-model integration failed: {e}, falling back to basic selection"
                )
        ...
        model_size_category = (
            "small" if model_size < 100000000 else "medium" if model_size < 1000000000 else "large"
        )
    ...
    def predict_performance(
        ...
        if PREDICTOR_AVAILABLE and self.prediction_models:
            try:
                ...
                logger.warning(
                    f"Advanced performance prediction failed: {e}, falling back to heuristics"
                )
        ...
        if INTEGRATION_AVAILABLE:
            try:
                ...
                for hw in hardware_list:
                    # Check compatibility
                    is_compatible, reason = check_hardware_compatibility(
                        model_family, model_size_tier, hw
                    )
                ...
                logger.warning(
                    f"Hardware-model integration for performance failed: {e}, falling back to basic heuristics"
                )
        ...
        for hw in hardware_list:
            ...
            result["predictions"][hw] = {
                ...
                * 0.004
                * batch_size,  # Rough estimate based on model size
    ...
    def create_hardware_prediction_matrix(
        self,
        ...
        if (
            PREDICTOR_AVAILABLE
            and self.prediction_models
            and hasattr(sys.modules["model_performance_predictor"], "generate_prediction_matrix")
        ):
            try:
                # Convert models to expected format
                model_configs = []
                for model in models:
                    model_configs.append({"name": model["name"], "category": model["family"]})
                ...
                logger.warning(
                    f"Failed to use generate_prediction_matrix: {e}, falling back to manual creation"
                )
    ...
    def visualize_matrix(self, matrix: Dict[str, Any], output_dir: str = "./visualizations"):
        ...
        """
        if PREDICTOR_AVAILABLE and hasattr(
            sys.modules["model_performance_predictor"], "visualize_predictions"
        ):
            try:
                ...
                for metric in ["throughput", "latency_mean", "memory_usage"]:
                    files = visualize_predictions(
                        matrix=matrix, metric=metric, output_dir=output_dir
                    )
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, choices=["inference", "training"], default="inference", help="Mode"
    )
    parser.add_argument(
        "--precision", type=str, choices=["fp32", "fp16", "int8"], default="fp32", help="Precision"
    )
    ...
    predictor = HardwareModelPredictor(
        benchmark_dir=args.benchmark_dir, database_path=args.database, config_path=args.config
    )
    ...
    if args.generate_matrix:
        matrix = predictor.create_hardware_prediction_matrix(
            hardware_platforms=args.hardware, precision=args.precision, mode=args.mode
        )
    ...
    if args.model:
        ...
        print(
            f"  Model Size: {recommendation['model_size_category']} ({recommendation['model_size']} parameters)"
        )
