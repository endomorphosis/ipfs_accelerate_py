...
class SystemLogs:
    ...
    def get_logs(
        ...
        try:
            ...
            # Execute command
            result = subprocess.run(cmd, capture_output=not follow, text=True, check=True)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "total": len(logs),
            "by_level": {"INFO": 0, "WARNING": 0, "ERROR": 0, "CRITICAL": 0, "DEBUG": 0},
            "time_range": "1 hour",
        }
                        ...
                        try:
                            ...
                            if cached_data:
                                ...
                                self._storage.write_file(
                                    content.encode("utf-8"), log_path, pin=False
                                )
                        except Exception:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Access IPFS Accelerate system logs")
    parser.add_argument(
        "--service", default="ipfs-accelerate", help="Service name (default: ipfs-accelerate)"
    )
    parser.add_argument(
        "--lines", "-n", type=int, default=100, help="Number of lines to display (default: 100)"
    )
    parser.add_argument(
        "--since", help='Show logs since time (e.g., "1 hour ago", "30 minutes ago")'
    )
    parser.add_argument(
        "--level",
    ...
    parser.add_argument("--errors", action="store_true", help="Show only errors from last 24 hours")
    ...
    logs = logs_manager.get_logs(
        lines=args.lines, since=args.since, level=args.level, follow=args.follow
    )
