...
def validation_retry_task_block(
    *,
    ...
    validation_command = safe_retry_validation_command(
        ...
        fallback_commands=getattr(source_task, "validation", ()) or (),
    )
    discovery_validation = f"test -f {shlex.quote(str(discovery_path))}"
    if (
        validation_command == discovery_validation
        and discovery_output_path not in outputs
    ):
...
- Retry repair source: {source_task.task_id}
...
def safe_retry_validation_command(
    ...
    *,
    ...
    for fallback in fallback_commands:
        normalized = normalize_validation_command_text(str(fallback or ""))
        if not normalized:
            continue
