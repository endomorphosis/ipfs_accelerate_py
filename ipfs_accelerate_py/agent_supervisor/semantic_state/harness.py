...
def run_closed_loop(record):
    """LGSWF closed-loop refresh used by completed refresh-pipeline tests."""

    from types import MappingProxyType

    from ipfs_accelerate_py.agent_supervisor.semantic_state.work_loop import (
        run_provisional_loop,
    )
    from ipfs_accelerate_py.agent_supervisor.semantic_state.post_merge_refresh import (
        refresh_canonical,
    )

    payload = record if isinstance(record, dict) else {}
    provisional = run_provisional_loop(payload.get("provisional") or {})
    canonical = refresh_canonical(
        payload.get("canonical") or {"accepted_merge": True, "fresh_rescan": True}
    )
    return MappingProxyType({"provisional": provisional, "canonical": canonical})
