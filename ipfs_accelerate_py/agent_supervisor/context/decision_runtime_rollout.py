...
PROOF_DIRECTED_ROLLOUT_REQUIREMENT_ID: Final = "asi-139:proof-directed-runtime-safe-rollout"
...
def _canonical_bytes(value: Any) -> bytes:
    try:
        ...
        raise DecisionRuntimeRolloutError("rollout data must be canonical JSON") from exc
...
def _load_json(value: str | bytes | bytearray, name: str) -> Any:
    def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, item in pairs:
            if key in result:
                raise DecisionRuntimeRolloutError(f"{name} contains duplicate JSON key {key!r}")
            result[key] = item
        return result
...
def _text(value: Any, name: str, *, maximum: int = 512) -> str:
    if not isinstance(value, str) or not value or value != value.strip():
        raise DecisionRuntimeRolloutError(f"{name} must be non-empty canonical text")
...
class DecisionRuntimeRolloutBinding:
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "DecisionRuntimeRolloutBinding":
...
class DecisionRuntimeRolloutPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if len(behaviors) != len(set(behaviors)):
            raise DecisionRuntimeRolloutError("approved behavior IDs must be unique")
        object.__setattr__(self, "approved_behavior_ids", behaviors)
        modes = tuple(sorted({_mode(item) for item in self.approved_modes}, key=lambda x: x.value))
    ...
    def approves(self, behavior_id: str, mode: DecisionRuntimeRolloutMode) -> bool:
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "DecisionRuntimeRolloutPolicy":
        if set(value) != set(cls.__dataclass_fields__):
...
class DecisionRuntimeRolloutEvaluation:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.benchmark, DecisionRuntimeBenchmark):
            raise DecisionRuntimeRolloutError("evaluation benchmark has the wrong type")
    ...
    def from_dict(
        ...
        *,
        ...
        if _canonical_bytes(value) != _canonical_bytes(result.to_dict()):
            raise DecisionRuntimeRolloutError("evaluation does not match producer receipt replay")
        return result
...
class DecisionRuntimeRolloutDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if self.desired_mode is DecisionRuntimeRolloutMode.OFF:
            ...
            raise DecisionRuntimeRolloutError("failed promotion must return to shadow")
        if self.effective_mode is DecisionRuntimeRolloutMode.AUTOMATIC and not self.automatic_ready:
    ...
    def from_dict(
        ...
        *,
        ...
        try:
            ...
            raise DecisionRuntimeRolloutError("rollout decision is missing desired_mode") from exc
...
def evaluate_decision_runtime_rollout(
    ...
    *,
    binding: DecisionRuntimeRolloutBinding,
    policy: DecisionRuntimeRolloutPolicy,
    desired_mode: DecisionRuntimeRolloutMode | str = (DecisionRuntimeRolloutMode.SHADOW),
    current_evaluation: DecisionRuntimeRolloutEvaluation | None = None,
) -> DecisionRuntimeRolloutDecision:
    ...
    if current_evaluation is not None:
        if not isinstance(current_evaluation, DecisionRuntimeRolloutEvaluation):
            raise DecisionRuntimeRolloutError("current_evaluation has the wrong type")
    ...
    *,
    ...
    try:
        ...
        record_ref = str(
            ...
            or getattr(decision, "qualification_report_id", "")
            or "decision-runtime-rollout-verify"
        )
        mirror_work_record(
            catalog_kind="metadata",
...
class DecisionRuntimeControlRequest:
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "DecisionRuntimeControlRequest":
        allowed = {
            "schema",
            "version",
            "action",
        ...
        if (
            value.get("schema") != DECISION_RUNTIME_CONTROL_REQUEST_SCHEMA
            or value.get("version") != DECISION_RUNTIME_ROLLOUT_VERSION
        ):
    ...
    @classmethod
    def from_json(cls, value: str | bytes | bytearray) -> "DecisionRuntimeControlRequest":
...
class DecisionRuntimePublicAPI:
    ...
    def __init__(
        ...
        *,
        ...
        policy: DecisionRuntimeRolloutPolicy,
        current_evaluation: DecisionRuntimeRolloutEvaluation | None = None,
        initial_mode: DecisionRuntimeRolloutMode | str = (DecisionRuntimeRolloutMode.SHADOW),
    ) -> None:
    ...
    def discovery() -> dict[str, Any]:
        ...
        return {
            "schema": "ipfs_accelerate_py/agent-supervisor/decision-runtime-public-api@1",
            "version": DECISION_RUNTIME_ROLLOUT_VERSION,
            "requirement_id": PROOF_DIRECTED_ROLLOUT_REQUIREMENT_ID,
