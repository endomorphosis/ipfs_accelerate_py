...
def verified_consumer_claim(history, *, task_cid, task_alias, revision, body, seed, identity):
    """Reproduce a retained successor's exact native reservation and admission."""
    recovery_revision = seed.get("recovery_control_revision")
    if type(recovery_revision) is not int or revision != recovery_revision + 3:
        return None
    if not isinstance(history, Mapping) or not isinstance(history.get("revisions"), list):
        return None
    rows = history["revisions"]
    if len(rows) != revision:
        return None
    predecessor = rows[recovery_revision]["body"].get("completion_receipt")
    if not isinstance(predecessor, Mapping) or not verified_seed_predecessor(
        history, task_cid=task_cid, task_alias=task_alias, seed=seed, predecessor=predecessor
    ):
        return None
    context = verified_suffix(
        history, task_cid=task_cid, task_alias=task_alias, control_revision=recovery_revision
    )
    if context is None:
        return None
    claim_row, admission_row = rows[-2:]
    if claim_row["status"] != "in_progress" or admission_row != {
        "revision": revision,
        "status": "in_progress",
        "body": body,
    }:
        return None
    claim_body, admission_body = dict(claim_row["body"]), dict(admission_row["body"])
    claim, admission = (
        claim_body.pop("completion_receipt", None),
        admission_body.pop("completion_receipt", None),
    )
    carried = {
        "post_merge_completion_recovery_seed",
        "post_merge_completion_recovery_source_attempt_id",
    ...
    if (
        ...
        or claim_body != context["semantic_body"]
        or admission_body != claim_body
        or set(claim) != CLAIM | carried
        or claim.get("post_merge_completion_recovery_seed") != seed
        or claim.get("post_merge_completion_recovery_source_attempt_id") != seed["attempt_id"]
        or claim.get("operation") != "database_claim"
        or claim.get("claim_phase_schema") != TYPED_DATABASE_CLAIM_RESERVATION_SCHEMA
        or claim.get("claimed_from_revision") != recovery_revision + 1
        or dict(admission)
        != {
            **claim,
            "operation": "database_attempt_admitted",
            "claim_phase_schema": TYPED_DATABASE_ATTEMPT_ADMISSION_SCHEMA,
            "admitted_from_revision": recovery_revision + 2,
            "attempt_execution_phase": "claimed",
            "attempt_execution_revision": 1,
        }
        or any(claim.get(k) != identity.get(k) for k in IDENTITY)
        or any(
            claim.get(k) != context["current_claim"].get(k)
            for k in ROUTE
            | {
                "strict_task_sharding",
                "idle_lane_work_stealing",
        ...
        or any(
            type(claim.get(k)) is not int or claim[k] != context["current_receipt"][k] + 1
            for k in ("attempt_number", "fencing_token", "fence_epoch")
        )
        or any(
            claim.get(k) in {context["current_receipt"][k], seed[k]}
            for k in ("attempt_id", "claim_id", "lease_id")
        )
    ):
        return None
