...
DATABASE_PORTAL_RELOAD_PROJECTION_SCHEMA = (
    "ipfs_accelerate_py/agent-supervisor/database-portal-reload-projection@1"
)
DATABASE_PORTAL_RELOAD_PAGE_LIMIT = 128
DATABASE_PORTAL_RELOAD_MAX_PAGES = 32
_QUACK_OWNER_BINDING_FIELDS = (
    "server_id",
    "store_id",
    "database_uuid",
    "schema_revision",
...
_DATABASE_CLAIM_REQUIRED_FIELDS = frozenset(
    "schema operation task_cid validation_spec_cid attempts_used "
    "max_task_attempts retry_exhausted process_instance_id owner_session_id "
...
class _PinnedReadIntentRepository(IntentRepository):
    """Read-only IntentRepository facade over one already-open transaction.

    ``DatabaseTaskSource`` is the task projection authority, but its ordinary
    repository opens one connection per method.  Reload safety needs a single
    authenticated Quack generation for the complete population read.  This
    facade makes inherited read methods share the caller-owned connection.
    """

    def __init__(self, connection: Any) -> None:
        self.database_path = Path("quack:pinned-read-transaction")
    ...
    def _require_open(self) -> None:
        if self._closed or not self._open:
            raise RuntimeError("pinned intent repository is not open")
...
def database_program_from_cli_namespace(
    ...
    *,
    ...
    env_payload = str(
        environment.get(DATABASE_PROGRAM_JSON_ENV, "") or ""
    ).strip()
...
ATOMIC_CHECKOUT_MUTATION_LEASE_OPERATIONS = frozenset(
    {
        ...
        "fenced_provider_recovery_admission",
...
class PortalImplementationSupervisor:
    ...
    def run_once(self, *, include_refill: bool = True) -> dict[str, Any]:
                        ...
                        if not self._database_portal_reload_projection_is_idle(
                            ...
                            result = {
                                ...
                                "reason": str(
                                    ...
                                    or "database_portal_projection_not_admitted"
                                ),
        ...
        if result.get("maintenance_blocked") is True:
            finish_maintenance(
                "deferred",
                str(result.get("reason") or "maintenance_inconclusive"),
            )
        else:
    ...
    def _run_once_with_maintenance(
        ...
        *,
        ...
        if managed_daemon_launch_lock_held:
            maintenance_kwargs["managed_daemon_launch_lock_held"] = True
        if database_portal_fenced_program is not None:
        ...
        if not self.config.implementation_protected_paths:
            ...
            try:
                ...
                maintenance_kwargs["implementation_maintenance_lease"] = lease
                result = self._run_once_with_maintenance_under_lease(
                    update_maintenance_phase,
                    **maintenance_kwargs,
                )
        ...
        # Database reconciliation owns its task claim, implementation lease,
        # protected-path journal, and checkout mutation lease.  Never lend it
    ...
    def _run_database_portal_guarded_worktree_maintenance(
        self,
        update_maintenance_phase,
        *,
        ...
        active_outer_lease = bool(
            current_lease is not None
            and self._supervisor_checkout_transaction_depth() > 0
        )
        if active_outer_lease:
            if checkout_mutation_lease_state(current_lease) != "current":
                return {
                    "maintenance_blocked": True,
                    "reason": "outer_checkout_lease_not_current",
                    "lock_path": str(lock_path),
                    "database_portal_reload_projection": {},
                }
        elif not self._retained_generated_checkout_lease():
            ...
            if adoption.get("required") and adoption.get("blocked"):
                return {
                    ...
                    "reason": str(
                        adoption.get("reason")
                        or "checkout_mutation_protected_recovery_required"
                    ),
                    "lock_path": str(lock_path),
                    "database_portal_reload_projection": {},
                    "retained_generated_checkout_recovery": adoption,
                }

        retained_checkout = self._retained_generated_checkout_lease()
        lease_owned = False
        if active_outer_lease:
            lease = current_lease
        elif retained_checkout:
            lease = self._current_supervisor_checkout_lease()
            if lease is None or checkout_mutation_lease_state(lease) != "current":
                return {
                    "maintenance_blocked": True,
                    "reason": "retained_checkout_lease_not_current",
                    "lock_path": str(lock_path),
                    "database_portal_reload_projection": {},
                }
        else:
            metadata = self._supervisor_checkout_lock_metadata(
                operation="database_portal_guarded_worktree_maintenance",
            )
            lease, lock_reason, existing_lock = (
                self._acquire_supervisor_checkout_lease(lock_path, metadata)
            )
            if lease is None:
                result: dict[str, Any] = {
                    "maintenance_blocked": True,
            ...
            lease_owned = True
        try:
            update_maintenance_phase("database_portal_reload_projection")
            projection = self._database_portal_reload_projection_fenced(
                database_portal_fenced_program
            )
            if projection.get("defer_maintenance") is True:
                return {
                    "maintenance_blocked": True,
                    "reason": str(
                        projection.get("reason")
                        or "database_portal_projection_inconclusive"
                    ),
                    "database_portal_reload_projection": projection,
                    "lock_path": str(lock_path),
                }
            if retained_checkout:
                # The retained lease is itself the repo-wide mutation fence.
                # Repair it only after the exact DB population proved idle,
                # then require a new projection cycle before any other
                # checkout mutation.
                update_maintenance_phase(
                ...
                retained = bool(retained_recovery.get("retained_lease"))
                return {
                    "maintenance_blocked": True,
                    "reason": (
                        "checkout_mutation_protected_recovery_required"
                        if retained
                        else "retained_checkout_recovered_recheck_required"
                    ),
                    "database_portal_reload_projection": projection,
                    "retained_generated_checkout_recovery": (
                        retained_recovery
            ...
            stale_active_state_repair = (
                self.repair_stale_active_execution_state()
            )
            update_maintenance_phase("completed_leftover_execution")
            completed_leftover_execution = (
                self.release_completed_leftover_execution(
                    _launch_lock_held=True,
                )
            )
            update_maintenance_phase("guarded_dirty_worktree_rescue")
            guarded_dirty_worktree_rescue = (
                self.reconcile_backlogged_worktrees(
                    preacquired_implementation_lock=(
                        implementation_maintenance_lease
                    ),
                    rescue_dirty_only=True,
                )
            )
            ...
            guarded_result = {
                "maintenance_blocked": False,
                "reason": "guarded_supervisor_worktree_mutations_completed",
                "database_portal_reload_projection": projection,
                "guarded_dirty_worktree_rescue": (
                    guarded_dirty_worktree_rescue
                ),
                "retained_generated_checkout_recovery": retained_recovery,
            ...
            if lease_owned:
                self._release_supervisor_checkout_lease(
                    lease,
                    operation=(
                        "database_portal_guarded_worktree_maintenance"
                    ),
                )

        return {
            **guarded_result,
            "reason": "guarded_supervisor_worktree_maintenance_completed",
            "database_portal_daemon_reconciliation_pending": True,
        }

    def _run_database_portal_daemon_reconciliation(
        self,
        ...
        *,
        ...
        def blocked(reason: str, **extra: Any) -> dict[str, Any]:
            return {
                **dict(result),
                "maintenance_blocked": True,
                "reason": reason,
                "database_portal_daemon_reconciliation_pending": False,
                **extra,
            }

        if not managed_daemon_launch_lock_held:
            return blocked("managed_child_quiescence_unproven")
        if (
            database_portal_fenced_program is None
            or database_portal_fenced_program
            is not self.config.database_program
        ):
            return blocked("database_portal_owner_mutation_fence_unproven")
        if (
            self._find_matching_managed_daemon_pid() is not None
            or bool(self._active_agent_worker_processes())
            or self._active_validation_subprocess_exists()
        ):
            return blocked("managed_child_quiescence_changed_between_phases")

        # The supervisor implementation lease was released between phases.
        # Re-project the complete DB claim population from one current Quack
        # generation before allowing the daemon to acquire its own claims.
        update_maintenance_phase("database_portal_daemon_reconciliation_projection")
        projection = self._database_portal_reload_projection_fenced(
            database_portal_fenced_program
        )
        if projection.get("defer_maintenance") is True:
        ...
        if (
            ...
            return blocked(
                "managed_child_quiescence_changed_after_projection",
                database_portal_reload_projection=projection,
            )
        ...
        try:
            ...
            logger.warning(
                "Daemon-owned worktree reconciliation failed closed",
                exc_info=True,
            )
            return blocked(
                "daemon_owned_reconciliation_failed",
                database_portal_reload_projection=projection,
                error_type=type(exc).__name__,
                error=str(exc)[:1000],
            )
        return {
            **dict(result),
            "maintenance_blocked": False,
            "reason": "database_portal_daemon_reconciliation_completed",
            "database_portal_daemon_reconciliation_pending": False,
            "database_portal_daemon_reconciliation_projection": projection,
    ...
    def _run_once_with_maintenance_under_lease(
        ...
        *,
        ...
        if database_guard_required and not managed_daemon_launch_lock_held:
            return {
                "stuck": False,
                "maintenance_blocked": True,
                "reason": "managed_child_quiescence_unproven",
        ...
        if database_guard_required and not database_checkout_guard_held:
            adoption = self._adopt_supervisor_protected_recovery()
            if adoption.get("required") and adoption.get("blocked"):
                return {
                    "stuck": False,
            ...
            if self._retained_generated_checkout_lease():
                # The retained checkout lease is already the exclusive fence.
                # Its exact projection and bounded recovery are handled by the
                # guarded helper; a later cycle will acquire the ordinary
                # supervisor transaction for all remaining mutations.
            ...
            def deferred_checkout(payload: Mapping[str, Any]) -> dict[str, Any]:
                return {
                    ...
                    "checkout_mutation_guard": dict(payload),
                }

            return self._run_generated_board_producer(
                producer="database_portal_guarded_supervisor_maintenance",
                commit_outputs=True,
                operation="database_portal_guarded_supervisor_maintenance",
                callback=lambda: self._run_once_with_maintenance_under_lease(
                    update_maintenance_phase,
                    include_refill=include_refill,
                    implementation_maintenance_lease=(
                        implementation_maintenance_lease
        ...
        if database_guard_required:
            # Retained checkout recovery is a checkout mutation and is handled
            # by the DB guard below only after an exact projection while the
            # same repo-wide lease remains current.
            retained_generated_checkout_recovery: dict[str, Any] = {}
        else:
            # A producer can retain the checkout lease only when its protected
            # outputs could not be proven clean.  Preserve the legacy order
            # for supervisors not using the DB Portal authority.
        ...
        database_portal_daemon_reconciliation_pending = False
        if database_guard_required:
            guarded_worktree_maintenance = (
                self._run_database_portal_guarded_worktree_maintenance(
                    update_maintenance_phase,
                    implementation_maintenance_lease=(
                        implementation_maintenance_lease
                    ),
            ...
            if guarded_worktree_maintenance.get("maintenance_blocked") is True:
                return {
                    "stuck": False,
                    "maintenance_blocked": True,
                    "reason": str(
                        guarded_worktree_maintenance.get("reason") or ""
                    ),
                    "event_log_repair": event_log_repair,
                    "state_file_repair": state_file_repair,
                    "protected_path_guard": protected_path_guard,
                    "retained_generated_checkout_recovery": (
                        guarded_worktree_maintenance.get(
                            "retained_generated_checkout_recovery"
                        )
                        or {}
                    ),
                    ...
                    "guarded_worktree_maintenance": guarded_worktree_maintenance,
                }
            retained_generated_checkout_recovery = dict(
                guarded_worktree_maintenance.get(
                    "retained_generated_checkout_recovery"
                )
                or {}
            )
            stale_worktree_detection = dict(
                guarded_worktree_maintenance.get("stale_worktree_detection")
                or {}
            )
            stale_active_state_repair = dict(
                guarded_worktree_maintenance.get("stale_active_state_repair")
                or {}
            )
            completed_leftover_execution = dict(
                guarded_worktree_maintenance.get(
                    "completed_leftover_execution"
                )
                or {}
            )
            database_portal_daemon_reconciliation_pending = bool(
                guarded_worktree_maintenance.get(
                    "database_portal_daemon_reconciliation_pending"
                )
            )
            worktree_reconciliation = dict(
                guarded_worktree_maintenance.get("worktree_reconciliation")
                or {}
            )
            worktree_reconciliation_replay = dict(
                guarded_worktree_maintenance.get(
                    "worktree_reconciliation_replay"
                )
                or {}
            )
            worktree_cleanup = dict(
                guarded_worktree_maintenance.get("worktree_cleanup") or {}
            )
        else:
            update_maintenance_phase("stale_worktree_detection")
            stale_worktree_detection = self.detect_stale_worktrees()
            ...
            update_maintenance_phase("worktree_reconciliation")
            worktree_reconciliation = self.reconcile_backlogged_worktrees(
                preacquired_implementation_lock=implementation_maintenance_lease,
            )
            update_maintenance_phase("worktree_reconciliation_replay")
            worktree_reconciliation_replay = (
                self.recover_already_merged_reconciliation_candidates(
        ...
        main_checkout_repair = (
            self.repair_main_checkout_merge_state(
                _checkout_lease_held=True,
            )
            if database_guard_required
            else self.repair_main_checkout_merge_state()
        )
    ...
    def _run_forever_loop(self) -> None:
        ...
        self._record_event(
            (
                "supervisor_preflight_maintenance_deferred"
                if preflight.get("maintenance_blocked") is True
                else "supervisor_preflight_maintenance_pass"
            ),
            preflight,
        )
        ...
        while True:
            ...
            try:
                ...
                self._record_event(
                    (
                        "supervisor_loop_recovery_deferred"
                        if recovery.get("maintenance_blocked") is True
                        else "supervisor_loop_recovery_pass"
                    ),
                    {
                        "loop_result": result_payload,
                        "recovery": recovery,
    ...
    def _quiesce_supervised_child_for_control_gate(
        self,
        child: Any,
        *,
        reason: str,
        ...
        """Fence the exact managed child before reload or maintenance reads."""

        if not _launch_lock_held:
            with serialized_lock_update(
                self._managed_daemon_launch_lock_path()
            ):
                return self._quiesce_supervised_child_for_control_gate(
                    child,
                    reason=reason,
                    _launch_lock_held=True,
                )
        ...
        child_alive = bool(child_pid and process_is_running(child_pid))
        result = {
            **dict(cleanup),
            "reason": str(reason),
            "supervised_child_pid": child_pid,
            "supervised_child_alive": child_alive,
        }
        result["quiesced"] = bool(
            cleanup.get("quiesced") is True and not child_alive
        )
        return result
    ...
    def _supervisor_loop_watchdog_decision(
        ...
        if control_plane_status["control_plane_update_pending"]:
            ...
            with serialized_lock_update(
                ...
                if (
                    database_projection.get("defer_reload") is True
                    or residual_active_state
                ):
                    reason = (
                        str(database_projection.get("reason") or "")
        ...
        def defer_before_quiescence(
            ...
            *,
            ...
            detail = {
                ...
                "supervisor_maintenance_deferred_reason": reason,
                "database_portal_projection_reason": str(
                    projection.get("reason") or ""
                ),
                "database_portal_reload_projection": dict(projection),
            }
            self._last_supervisor_maintenance_at = time.monotonic()
            self._set_loop_status_fields(_loop, detail)
            self._record_event(
        ...
        update_maintenance_phase = None
        finish_maintenance = None
        ...
        def perform_quiesced_maintenance(
            ...
            with serialized_lock_update(
                ...
                if quiescence.get("quiesced") is not True:
                    ...
                    self._last_supervisor_maintenance_at = time.monotonic()
                    return SupervisorLoopDecision.keep_running()
                if fenced_program is not None:
                    projection = (
                        self._database_portal_reload_projection_fenced(
                            fenced_program
                        )
                    )
                    state_after_quiescence = PortalTaskState.load(
                        self.config.state_path
                    )
                    residual_active_state = bool(
                        state_after_quiescence.active_task_id
                        or state_after_quiescence.implementation_in_progress
                        or self._active_agent_worker_processes()
                        or self._active_validation_subprocess_exists()
                    )
                    if (
                        ...
                        finish_maintenance("deferred", blocked_reason)
                        return SupervisorLoopDecision.recycle(
                ...
                try:
                    ...
                    logger.warning(
                        "Supervisor maintenance hook failed after child quiescence",
                        exc_info=True,
                    )
                    self._record_event(
                    ...
                    return SupervisorLoopDecision.recycle(
                        "supervisor_maintenance_failed_after_quiescence",
                        detail={"managed_child_quiescence": quiescence},
                    )
                finally:
                    self._last_supervisor_maintenance_at = time.monotonic()
        ...
        if database_portal_configured:
            try:
                ...
                with self._database_portal_reload_mutation_fence() as fenced_program:
                    ...
                    if not self._database_portal_reload_projection_is_idle(
                        ...
                        return defer_before_quiescence(
                            database_projection,
                            reason=str(
                                database_projection.get("reason")
                                or "database_portal_projection_not_admitted"
                    ...
                    (
                        update_maintenance_phase,
                        finish_maintenance,
                    ) = self._begin_supervisor_maintenance_heartbeat(
                        "watchdog",
                        daemon_pid=daemon_pid,
                    )
                ...
                if quiescence.get("quiesced") is True:
                    return SupervisorLoopDecision.recycle(
                        "supervisor_maintenance_failed_after_quiescence",
                        detail={
                            "managed_child_quiescence": quiescence,
                ...
                return defer_before_quiescence(
                    self._database_portal_reload_inconclusive_projection(exc),
                    reason="database_portal_owner_mutation_fence_unavailable",
                )
        else:
            (
            ...
            outcome = perform_quiesced_maintenance(None)

        if isinstance(outcome, SupervisorLoopDecision):
            return outcome
        result = outcome
        assert finish_maintenance is not None
        ...
        return SupervisorLoopDecision.recycle(
            ...
            detail={
                "managed_child_quiescence": quiescence,
                "maintenance_blocked": bool(result.get("maintenance_blocked")),
                "reason": str(result.get("reason") or ""),
            },
        )

    def repair_main_checkout_merge_state(
        self,
        *,
        _checkout_lease_held: bool = False,
    ) -> dict[str, Any]:
        ...
        lease_owned = False
        if _checkout_lease_held:
            lease = self._current_supervisor_checkout_lease()
            if (
                lease is None
                or self._supervisor_checkout_transaction_depth() <= 0
                ...
                result = {
                    ...
                    "reason": "preacquired_checkout_lease_not_current",
                    "lock_path": str(lock_path),
                }
                self._record_event(
    ...
    def release_completed_leftover_execution(
        self,
        *,
        _launch_lock_held: bool = False,
    ) -> dict[str, Any]:
        ...
        if not _launch_lock_held:
            with serialized_lock_update(
                ...
                return self.release_completed_leftover_execution(
                    _launch_lock_held=True,
                )
    ...
    @contextmanager
    def _supervisor_checkout_transaction(
        self,
        lease: CheckoutMutationLease,
    ):
        """Bind an already-published lease to this thread's mutation scope."""

        current = self._current_supervisor_checkout_lease()
        if current is not None and (
            current.lease_id != lease.lease_id
            or current.lock_path != lease.lock_path
            or current.device != lease.device
            or current.inode != lease.inode
        ):
            raise RuntimeError("different checkout mutation lease already bound")
        prior_depth = self._supervisor_checkout_transaction_depth()
        self._checkout_mutation_context.lease = lease
        self._checkout_mutation_context.transaction_depth = prior_depth + 1
        try:
            yield
        finally:
            self._checkout_mutation_context.transaction_depth = prior_depth
            if current is None:
                self._checkout_mutation_context.lease = None
    ...
    def reconcile_backlogged_worktrees(
        ...
        *,
        ...
        rescue_dirty_only: bool = False,
        allow_dirty_rescue: bool = True,
    ) -> dict[str, Any]:
        """Retry clean inactive implementation worktrees before cleanup."""

        if rescue_dirty_only and not allow_dirty_rescue:
            raise ValueError(
                "rescue_dirty_only requires guarded dirty rescue authority"
            )
        ...
        for record in records:
            ...
            cached_entry = (
                None
                if rescue_dirty_only
                else self._worktree_scan_cache_entry(
                    scan_cache,
            ...
            dirty = self._git_status_short(path) if path.exists() else []
            if dirty:
                if not allow_dirty_rescue:
                    skipped.append(
                        {
                            **detail,
                            "reason": "dirty_worktree_requires_guarded_rescue",
                            "status_short": dirty[:20],
                        }
                    )
                    continue
                # Dirty orphan adoption is task-authoritative.  The historical
                # generic rescue helper only staged the outer checkout, which
                # silently missed uncommitted files inside a nested submodule.
                # Resolve the exact current task before any mutation, then let
                # the daemon acquire task/resource fences, run the ordinary
                # proposal scope gate, and recursively commit the candidate.
                # Validation and merge deliberately occur on a later clean pass
                # (after Quack's supervisor maintenance leases are released).
                if reconciliation_daemon is None:
                    reconciliation_daemon = (
                        self._build_worktree_reconciliation_daemon()
                    )
                    (
                ...
                if current_task is None:
                    skip = {
                        **detail,
                        "reason": "dirty_candidate_task_identity_unresolved",
                        "status_short": dirty[:20],
                    }
                    skipped.append(skip)
                    continue
                if str(current_task.status).strip().lower() == "completed":
                    skip = {
                        **detail,
                        "reason": "dirty_candidate_task_already_completed",
                ...
                if not baseline_ref:
                    skip = {
                        **detail,
                        "reason": "dirty_candidate_baseline_unavailable",
                        "task_id": current_task.task_id,
                        "status_short": dirty[:20],
                    }
                    skipped.append(skip)
                    continue
                adoption_result = (
                    reconciliation_daemon.preserve_dirty_worktree_candidate(
                        worktree_path=path,
                        branch_name=branch,
                        task=current_task,
                        baseline_ref=baseline_ref,
                ...
                candidate = {
                    **detail,
                    ...
                    "target_ref": target_ref,
                    "status_short": dirty[:20],
                    "dirty_candidate_adoption": adoption_result,
                }
                candidates.append(candidate)
                processed.append(
                    {
                        **candidate,
                        "head": str(
                            adoption_result.get("candidate_commit") or head
                        ),
                        "merged": False,
                        "reason": str(
                            adoption_result.get("reason")
                            or "dirty_candidate_adoption_deferred"
                        ),
                        "validated_before_merge": False,
                    }
                )
                # Never validate mutable worktree bytes in the same supervisor
                # pass.  The next pass re-enumerates the exact clean commit,
                # reacquires claims, and uses reconcile_validated_worktree_candidate.
                continue
            elif rescue_dirty_only:
                continue
        ...
        result = {
            ...
            "target_signature": target_signature,
            "dry_run": dry_run,
            "rescue_dirty_only": rescue_dirty_only,
            "allow_dirty_rescue": allow_dirty_rescue,
    ...
    def _normalized_quack_owner_binding(
        ...
        """Return one closed live Quack identity, or fail inconclusively."""

        if not isinstance(value, Mapping):
            raise RuntimeError("Quack owner identity is unavailable")
        missing = set(_QUACK_OWNER_BINDING_FIELDS) - set(value)
        if missing:
            raise RuntimeError("Quack owner identity is incomplete")
        result = {name: value[name] for name in _QUACK_OWNER_BINDING_FIELDS}
        for name in (
            "server_id",
            "store_id",
            "database_uuid",
            ...
            if (
                ...
                raise RuntimeError("Quack owner identity is malformed")
        for name in ("schema_revision", "generation"):
            ...
            if (
                ...
                raise RuntimeError("Quack owner generation is malformed")
        return result

    @staticmethod
    def _database_claim_attempt(task: Any) -> Any:
        """Validate the canonical claim receipt and return its exact attempt."""
        ...
        if not isinstance(receipt, Mapping):
            raise RuntimeError("in-progress database task lacks a claim receipt")
        fields = frozenset(str(name) for name in receipt)
        if not _DATABASE_CLAIM_REQUIRED_FIELDS.issubset(fields) or (
            fields - _DATABASE_CLAIM_REQUIRED_FIELDS
            - _DATABASE_CLAIM_OPTIONAL_FIELDS
        ):
            raise RuntimeError("database claim receipt is not a closed record")
        ...
        if (
            ...
            or receipt.get("task_cid") != task_cid
            or not task_cid
        ):
            raise RuntimeError("database claim receipt authority is not exact")

        def required_text(name: str) -> str:
            observed = receipt.get(name)
            if not isinstance(observed, str) or not observed or len(observed) > 1024:
                raise RuntimeError("database claim receipt identity is malformed")
            if any(character in observed for character in "\0\n\r"):
                raise RuntimeError("database claim receipt identity is malformed")
            return observed
        ...
        def required_int(name: str, minimum: int) -> int:
            ...
            if isinstance(observed, bool) or not isinstance(observed, int):
                raise RuntimeError("database claim receipt fence is malformed")
            if observed < minimum:
                raise RuntimeError("database claim receipt fence is malformed")
            return observed
        ...
        text_values = {name: required_text(name) for name in text_fields}
        int_fields = (
            ("attempts_used", 1),
            ("max_task_attempts", 0),
            ("attempt_number", 1),
        ...
        for name in _DATABASE_CLAIM_OPTIONAL_FIELDS:
            ...
            valid = (
                isinstance(observed, bool)
                if name.endswith("_malformed")
                else isinstance(observed, int)
        ...
        return DatabaseTaskAttempt(
            ...
            attempt_number=int_values["attempt_number"],
            owner_session_id=text_values["owner_session_id"],
            fencing_token=int_values["fencing_token"],
            fence_epoch=int_values["fence_epoch"],
            lease_id=text_values["lease_id"],
    ...
    @staticmethod
    def _nested_portal_lifecycle_attempt(
        payload: Mapping[str, Any],
        *,
        task_alias: str,
        ...
        """Select one exact Portal attempt without enumerating lifecycle state."""

        candidates: list[int] = []
        if active_attempt:
            candidates.append(active_attempt)
        for index_name, key in (
            ("implementation_attempts", task_alias),
            ("implementation_attempts_by_cid", canonical_task_cid),
        ):
            values = payload.get(index_name) or {}
            if not isinstance(values, Mapping):
                raise RuntimeError("nested Portal attempt index is malformed")
            raw = values.get(key, 0)
            if isinstance(raw, bool):
                raise RuntimeError("nested Portal attempt index is malformed")
            try:
                observed = int(raw or 0)
            except (TypeError, ValueError) as exc:
                raise RuntimeError(
                    "nested Portal attempt index is malformed"
                ) from exc
            if observed < 0:
                raise RuntimeError("nested Portal attempt index is malformed")
            if observed:
        ...
        if not candidates or len(set(candidates)) != 1:
            raise RuntimeError("nested Portal attempt identity is inconclusive")
        return candidates[0]

    def _database_portal_claim_lifecycle(
        self,
        task_source: Any,
        task: Any,
        attempt: Any,
    ) -> dict[str, Any]:
        """Validate one lane-local binding and exact lifecycle index entry."""
        ...
        bridge = DatabasePortalExecutionBridge(
            ...
            portal_factory=lambda *_args, **_kwargs: (_ for _ in ()).throw(
                RuntimeError("read-only reload projection cannot enter Portal")
            ),
        )
        paths = bridge._paths(attempt)
        try:
            confined_root = paths.root.resolve()
            authority_root = attempt_root.resolve()
        except (OSError, RuntimeError) as exc:
            raise RuntimeError(
                "database attempt path confinement is unavailable"
            ) from exc
        if (
            paths.root.is_symlink()
            or not paths.root.is_dir()
            or confined_root.parent != authority_root
        ):
            raise RuntimeError("database attempt root is not confined")
        for artifact in (paths.binding, paths.task_projection, paths.state):
            try:
                regular = (
                    not artifact.is_symlink()
                    and artifact.is_file()
                    and artifact.resolve().parent == confined_root
                )
            except (OSError, RuntimeError):
                regular = False
            if not regular:
                raise RuntimeError(
                    "database attempt artifact is not a confined regular file"
                )
        selected = bridge.validate_active_attempt_roots((attempt,))
        if selected.get(str(attempt.attempt_id)) != str(paths.binding):
            raise RuntimeError("current database claim lacks its exact binding")
        binding = bridge._read_binding(paths.binding)
        bridge._verify_binding_identity(binding)
        record = bridge._record_for_attempt(task_source, attempt)
        seed = bridge._render_projection(attempt, record)
        expected = bridge._binding(attempt, record, seed)
        if binding != expected:
            raise RuntimeError("database attempt binding is not current")
        if int(binding.get("task_revision") or -1) != int(
            getattr(task, "revision", -2)
        ):
            raise RuntimeError("database attempt binding task revision is stale")
        projection_text = bridge._verify_projection(paths, binding)
        identity = bridge._projection_task_identity(
            paths,
            binding,
            projection_text,
        )
        payload, state_digest = bridge._strict_state_record(paths.state)
        if payload is None:
            raise RuntimeError("database claim lacks exact nested Portal state")
        nested = bridge._verify_nested_state_identity(
            paths,
            binding,
            identity,
            payload=payload,
        ...
        lifecycle_attempt = self._nested_portal_lifecycle_attempt(
            payload,
            task_alias=str(binding["task_alias"]),
            canonical_task_cid=identity["canonical_task_cid"],
            active_attempt=int(nested.get("active_attempt") or 0),
        )
        store = WorktreeLifecycleStore(repo_root=self.config.repo_root)
        lifecycle = store.load_task_attempt(
            canonical_task_cid=identity["canonical_task_cid"],
            task_id=str(binding["task_alias"]),
            attempt=lifecycle_attempt,
        )
        if lifecycle is None:
            raise RuntimeError("database claim lacks exact lifecycle authority")
        expected_repo = str(self.config.repo_root.resolve())
        expected_state = str(paths.root.resolve())
        if (
            lifecycle.record_id != lifecycle.compute_record_id()
            or lifecycle.task_id != str(binding["task_alias"])
            or lifecycle.canonical_task_cid != identity["canonical_task_cid"]
            or lifecycle.attempt != lifecycle_attempt
            or str(Path(lifecycle.repo_root).resolve()) != expected_repo
            or str(Path(lifecycle.state_dir).resolve()) != expected_state
            or (
                self.config.merge_target_branch
                and lifecycle.merge_target != self.config.merge_target_branch
            )
        ):
            raise RuntimeError("database claim lifecycle binding is not exact")
        by_workspace = store.load_workspace(lifecycle.workspace_path)
        if by_workspace != lifecycle:
            raise RuntimeError("database claim lifecycle indexes disagree")
        if nested.get("active") is True and (
            str(Path(str(nested.get("active_worktree_path") or "")).resolve())
            != str(Path(lifecycle.workspace_path).resolve())
            or str(nested.get("active_branch") or "") != lifecycle.branch
        ):
            raise RuntimeError("active nested Portal lifecycle changed workspace")
        return {
            "task_id": str(binding["task_alias"]),
            ...
            "active_phase": str(nested.get("active_phase") or ""),
    ...
    def _database_portal_claim_projection(self, task_source: Any) -> dict[str, Any]:
        ...
        seen: set[str] = set()
        for _page_index in range(DATABASE_PORTAL_RELOAD_MAX_PAGES):
            page = task_source.list_tasks(
                status="in_progress",
                cursor=cursor,
                limit=DATABASE_PORTAL_RELOAD_PAGE_LIMIT,
            )
            page_revision = int(page.revision)
            if revision is None:
                revision = page_revision
            elif revision != page_revision:
                raise RuntimeError("database task projection revision changed")
            for candidate in page.tasks:
                ...
                if (
                    current is None
                    or not callable(current_dict)
                    or not callable(candidate_dict)
                    or current_dict() != candidate_dict()
                ):
                    raise RuntimeError("database task projection changed in read")
                seen.add(task_cid)
        ...
        recovery_count = len(attempts) - active_count
        return {
            "task_source_revision": int(revision or 0),
            "task_ids": [item["task_id"] for item in attempts],
            "attempts": attempts,
        ...
        base.update(
            {
                ...
                "defer_reload": True,
                "defer_maintenance": True,
                "reason": "database_portal_projection_unavailable",
            }
        )
    ...
    def _database_reconciliation_program_environment(
        ...
        program: DatabaseProgramConfig,
        *,
        preserve_live_binding: bool = False,
    ) -> Any:
    ...
    def _cleanup_fenced_provider_migration_branches_locked(
        self,
        *,
        checkout_lease: CheckoutMutationLease | None = None,
        worktree_prune_result: subprocess.CompletedProcess[Any] | None = None,
        ...
        The provider-occurrence verifier is deliberately read-only.  Two
        historical occurrences retained an implementation ref which points
        exactly at their recorded baseline despite having no workspace or
        unique commit.  Only this existing checkout-mutation authority may
        reconcile those refs, and its journal is neither occurrence evidence
        nor a retry credit.

        ``occurrences`` and the manifest arguments are injectable solely for
        hermetic tests.  Production callers always load the operator-owned
        manifest from :mod:`database_portal_bridge` and verify its exact
        content address before considering a ref.
        """

        manifest_revision = "pctdd-provider-recovery-2026-09-02"
        if occurrences is None:
            if getattr(self, "board_namespace", "") != (
            ...
            selected_occurrences = tuple(dict(item) for item in occurrences)
            selected_manifest_schema = str(manifest_schema or "")
            selected_manifest_id = str(manifest_id or "")
        ...
        manifest = {
            ...
            "operator_owned": True,
            "one_shot": True,
            "occurrences": [dict(item) for item in selected_occurrences],
        }
        observed_manifest_id = "sha256:" + hashlib.sha256(
            json.dumps(
                manifest,
                ensure_ascii=False,
                separators=(",", ":"),
        ...
        result: dict[str, Any] = {
            ...
            "schema": (
                "ipfs_accelerate_py/agent-supervisor/"
                "fenced-provider-migration-branch-cleanup@1"
            ),
        ...
        lease_is_current = bool(
            isinstance(checkout_lease, CheckoutMutationLease)
            and isinstance(current_lease, CheckoutMutationLease)
            and checkout_lease.lease_id
            and current_lease.lease_id == checkout_lease.lease_id
            and current_lease.lock_path == checkout_lease.lock_path
            and current_lease.device == checkout_lease.device
            and current_lease.inode == checkout_lease.inode
            and checkout_lease.lock_path == self._repo_merge_lock_path()
            and self._supervisor_checkout_transaction_depth() > 0
            and checkout_mutation_lease_state(checkout_lease) == "current"
            and str(checkout_lease.metadata.get("kind") or "") == "merge"
            and Path(
                str(
                    checkout_lease.metadata.get("repo_root")
                    or checkout_lease.metadata.get("worktree_root")
                    or ""
                )
            ).resolve()
            == self.config.repo_root.resolve()
        ...
        if not lease_is_current:
            result.update(
                {
                    "prerequisites_satisfied": False,
                    "reason": "checkout_mutation_lease_not_current",
                }
            )
            self._record_event(
                "fenced_provider_migration_branch_cleanup",
                result,
            )
            return result
        if (
            worktree_prune_result is None
            or worktree_prune_result.returncode != 0
        ):
            result.update(
                {
                    "prerequisites_satisfied": False,
                    "reason": "worktree_prune_inconclusive",
                    "prune_returncode": (
                        None
                        if worktree_prune_result is None
                        else worktree_prune_result.returncode
                    ),
                }
            )
            self._record_event(
            ...
            return result
        required_aliases = {"PCTDD-006", "PCTDD-007", "PCTDD-034"}
        if (
            not selected_manifest_schema
            or observed_manifest_id != selected_manifest_id
            or len(selected_occurrences) != 3
            or {
            ...
            result.update(
                {
                    "prerequisites_satisfied": False,
                    "reason": "operator_occurrence_manifest_invalid",
                }
            )
            self._record_event(
                "fenced_provider_migration_branch_cleanup",
        ...
        repo_root = self.config.repo_root
        worktree_records, registry_reason = (
            self._strict_git_worktree_records(repo_root)
        )
        if worktree_records is None:
            result.update(
                {
                    "prerequisites_satisfied": False,
        ...
        linked_refs = {
            ...
            if str(record.get("branch") or "")
        }
        branch_results: list[dict[str, Any]] = []
        for occurrence in selected_occurrences:
            task_alias = str(occurrence.get("task_alias") or "")
            task_cid = str(occurrence.get("task_cid") or "")
            branch = str(occurrence.get("branch") or "")
            baseline_ref = str(occurrence.get("baseline_ref") or "")
            workspace_text = str(occurrence.get("workspace_path") or "")
            full_ref = f"refs/heads/{branch}"
            detail: dict[str, Any] = {
                "task_alias": task_alias,
                "task_cid": task_cid,
                "full_ref": full_ref,
                "expected_oid": baseline_ref,
            ...
            if (
                ...
                detail["reason"] = "invalid_manifest_ref_binding"
                ...
                detail.update(
                    {
                        ...
                        "check_ref_returncode": check_ref.returncode,
                    }
                )
                branch_results.append(detail)
                continue
            ...
            if workspace.exists() or workspace.is_symlink():
                detail["reason"] = "workspace_present"
                branch_results.append(detail)
                continue
            ...
            observed_oid = probe.stdout.strip()
            detail["observed_oid"] = observed_oid
            if probe.returncode == 1 and not observed_oid:
                detail.update(
                    {
                        "reconciled": True,
                ...
                branch_results.append(detail)
                continue
            if probe.returncode != 0 or observed_oid != baseline_ref:
                detail.update(
                    {
                        "reason": "ref_target_mismatch",
            ...
            unique = subprocess.run(
                ["git", "rev-list", "--count", f"{baseline_ref}..{full_ref}"],
                cwd=repo_root,
                text=True,
                capture_output=True,
                check=False,
            ...
            try:
                ...
                unique_count = -1
            detail["unique_commit_count"] = unique_count
            if unique.returncode != 0 or unique_count != 0:
                detail.update(
                    {
                        "reason": "unique_commit_check_failed",
                        "unique_commit_returncode": unique.returncode,
                    }
                )
                branch_results.append(detail)
                continue
            ...
            delete = subprocess.run(
                ["git", "update-ref", "-d", full_ref, baseline_ref],
                cwd=repo_root,
                text=True,
                capture_output=True,
                check=False,
            ...
            post_delete_absent = (
                reprobe.returncode == 1 and not reprobe.stdout.strip()
            )
            detail.update(
                {
                    "delete_returncode": delete.returncode,
                    "delete_stderr": delete.stderr[-1000:],
                    "post_delete_absent": post_delete_absent,
                    "deleted": delete.returncode == 0 and post_delete_absent,
                    "reconciled": delete.returncode == 0 and post_delete_absent,
                    "reason": (
                        "deleted_exact_baseline_ref"
                        if delete.returncode == 0 and post_delete_absent
        ...
        result["already_absent_count"] = sum(
            item.get("reason") == "already_absent" for item in branch_results
        )
        result["prerequisites_satisfied"] = bool(
            len(branch_results) == 3
            and all(item.get("reconciled") is True for item in branch_results)
        )
        result["reason"] = (
            "exact_branch_prerequisites_reconciled"
            if result["prerequisites_satisfied"]
            else "exact_branch_prerequisite_unavailable"
        )
        self._record_event(
            "fenced_provider_migration_branch_cleanup",
            result,
    ...
    def _retained_candidate_cleanup_holds(self) -> dict[str, dict[str, Any]]:
        """Load exact PCTDD-005 quarantine events as cleanup exclusions.

        This is preservation authority only.  It cannot approve, merge,
        sanitize, or consume the candidate/credit.  Any duplicate, malformed,
        non-PCTDD, or path-relative occurrence is ignored and follows ordinary
        cleanup policy.
        """
        ...
        event_paths.update(
            path
            for path in self.config.state_dir.parent.glob("*/*_events.jsonl")
            if not path.name.endswith("_supervisor_events.jsonl")
        )
        candidates: dict[str, list[dict[str, Any]]] = {}
        for event_path in sorted(event_paths, key=lambda path: str(path)):
            for event in self._read_jsonl_events(event_path):
                if (
                    str(event.get("type") or "")
                    != "dirty_worktree_candidate_quarantined_retry_issued"
                ...
                credit = event.get("retry_credit")
                if not (
                    PortalImplementationDaemon.
                    _retained_candidate_quarantine_valid(quarantine)
                    and PortalImplementationDaemon.
                    _retained_candidate_retry_credit_valid(
                        credit,
                        quarantine=quarantine,
                    )
                ):
                    continue
                workspace_text = str(
                    quarantine.get("worktree_path") or ""
                )
                workspace = Path(workspace_text)
                if not workspace.is_absolute():
                    continue
                credit_id = str(credit.get("retry_credit_id") or "")
                candidates.setdefault(credit_id, []).append(
                    {
                        "task_id": PCTDD_RETAINED_CANDIDATE_TASK_ID,
                        "task_cid": PCTDD_RETAINED_CANDIDATE_TASK_CID,
                        "worktree_path": workspace_text,
                        "branch": str(quarantine.get("branch") or ""),
                        "candidate_head": str(
                            quarantine.get("candidate_head") or ""
                        ),
        ...
        for records in candidates.values():
            ...
            try:
                ...
                continue
            if key in holds:
                # Two credits may never nominate the same candidate.
                holds.pop(key, None)
                continue
            holds[key] = record
        return holds
    ...
    def _cleanup_backlogged_worktrees_locked(
        ...
        *,
        ...
        retained_candidate_holds = self._retained_candidate_cleanup_holds()
        ...
        for record in records:
            ...
            if not path_resolved.exists():
                ...
                continue
            retained_hold = retained_candidate_holds.get(
                str(path_resolved)
            )
            if retained_hold is not None:
                observed_branch = str(
                    record.get("branch") or ""
                ).removeprefix("refs/heads/")
                observed_head = str(record.get("HEAD") or "")
                if (
                    observed_branch == retained_hold["branch"]
                    and observed_head == retained_hold["candidate_head"]
                ):
                    skipped.append(
                        {
                            "path": str(path),
                            ...
                            "head": observed_head,
                            "reason": (
                                "retained_candidate_quarantine_hold"
                            ),
                            "task_id": retained_hold["task_id"],
                            "task_cid": retained_hold["task_cid"],
                            "retry_credit_id": retained_hold[
                                "retry_credit_id"
                            ],
                            "candidate_diff_fingerprint": retained_hold[
                                "candidate_diff_fingerprint"
                            ],
                            "merge_allowed": False,
    ...
    @staticmethod
    def _strict_git_worktree_records(
        repo_root: Path,
    ) -> tuple[list[dict[str, str]] | None, str]:
        """Read the linked-worktree registry without permissive parse fallback."""
        ...
        if result.returncode != 0:
            return None, "worktree_registry_command_failed"
        raw = result.stdout
        if not isinstance(raw, bytes) or not raw or not raw.endswith(b"\0\0"):
            return None, "worktree_registry_malformed"
        allowed_fields = {
            "worktree",
            "HEAD",
            "branch",
        ...
        records: list[dict[str, str]] = []
        try:
            record_blobs = raw[:-2].split(b"\0\0")
            for record_blob in record_blobs:
                fields = record_blob.split(b"\0")
                if not fields or not fields[0].startswith(b"worktree "):
                    raise ValueError("record does not start with worktree")
                record: dict[str, str] = {}
                for field in fields:
                    if not field:
                        raise ValueError("empty field inside worktree record")
                    key_bytes, separator, value_bytes = field.partition(b" ")
                    key = key_bytes.decode("utf-8", errors="strict")
                    if key not in allowed_fields or key in record:
                        raise ValueError("unknown or duplicate worktree field")
                    value = (
                    ...
                    record[key] = value
                if not record.get("worktree") or not Path(
                    record["worktree"]
                ).is_absolute():
                    raise ValueError("worktree path is not absolute")
                if re.fullmatch(r"[0-9a-f]{40,64}", record.get("HEAD", "")) is None:
                    raise ValueError("worktree HEAD is malformed")
                dispositions = sum(
                    name in record for name in ("branch", "detached", "bare")
                )
                if dispositions != 1:
                    raise ValueError("worktree disposition is ambiguous")
                if "branch" in record and not record["branch"].startswith(
                    "refs/heads/"
                ):
                    raise ValueError("worktree branch is not a local head")
                records.append(record)
        except (UnicodeError, ValueError):
            return None, "worktree_registry_malformed"
        if not records:
            return None, "worktree_registry_empty"
        return records, "worktree_registry_verified"
    ...
    @staticmethod
    def _command_without_exact_owner_session(
        command: Sequence[str],
    ) -> tuple[tuple[str, ...], str] | None:
        """Remove exactly one nonempty owner-session argv pair.

        Owner-session identity is the sole reboot-volatile lane field.  A
        missing flag, duplicate flag, empty value, or any unrelated argv drift
        is never a same-lane predecessor match.
        """
        ...
        if index + 1 >= len(tokens):
            return None
        owner_session = tokens[index + 1]
        if not owner_session or owner_session.startswith("--"):
            return None
    ...
    def _managed_daemon_identity_matches_lane_scope(
        ...
        *,
        ...
        """Match every durable lane field while excluding only owner session."""
        ...
        if (
            set(expected_scope) != set(observed_scope)
    ...
    def _find_matching_managed_daemon_lane_process(
        self,
        *,
        ...
        excluded.add(os.getpid())
        for pid, command_line in self._list_process_details():
            if pid in excluded or not process_is_running(pid):
                continue
            observed_argv = read_process_command_argv(pid)
            observed = (
                self._command_without_exact_owner_session(observed_argv)
            ...
            if observed_argv is not None:
                continue
            try:
                nominated = self._command_without_exact_owner_session(
    ...
    def ensure_managed_daemon_pid_file(self) -> dict[str, Any]:
        ...
        if not pid_marker_present:
            ...
            if database_managed and identity_marker_present:
                ...
                scope_exact = self._managed_daemon_identity_matches_scope(
                    identity,
                    pid=identity_pid,
                )
                lane_exact = self._managed_daemon_identity_matches_lane_scope(
                    identity,
                    pid=identity_pid,
                )
                ...
                if not scope_exact:
                    ...
                    if liveness is not OwnerLiveness.DEAD:
                        return {
                            ...
                            "blocked": True,
                            "reason": (
                                "orphaned_managed_database_identity_"
                                "scope_mismatch"
                            ),
                            "path": str(pid_path),
                            "identity_path": str(identity_path),
                ...
                with serialized_lock_update(
                    ...
                    matching_lane_process = (
                        self._find_matching_managed_daemon_lane_process()
                    )
                    if matching_lane_process is not None:
                        return {
                            "repaired": False,
                            "blocked": True,
                            "reason": (
                                "matching_managed_daemon_lane_ownership_"
                                "unproven"
                            ),
                            "path": str(pid_path),
                            "identity_path": str(identity_path),
                            "pid": int(
                                matching_lane_process.get("pid") or 0
                            ),
        ...
        if identity is not None:
            ...
            if (
                ...
                # PID reuse must be decided under the same launcher lock as
                # marker quarantine.  Re-scan the durable lane without
                # excluding the recorded numeric PID: an old exact birth can
                # be DEAD while that PID now names a live predecessor-owner
                # Quack process.  Such a process retains custody and blocks a
                # duplicate launch.
                with serialized_lock_update(
                    self._managed_daemon_launch_lock_path()
                ):
                    ...
                    try:
                        ...
                        current_pid = 0
                    if current_pid != pid or current_identity != identity:
                        return {
                            "repaired": False,
                            "blocked": True,
                            "reason": (
                                "managed_daemon_identity_advanced_during_"
                                "pid_reuse_reconciliation"
                            ),
                            "path": str(pid_path),
                            "identity_path": str(identity_path),
                            "pid": pid,
                    ...
                    if (
                        supervised_child_identity_liveness(current_identity)
                        is not OwnerLiveness.DEAD
                        ...
                        return {
                            ...
                            "blocked": True,
                            "reason": (
                                "managed_daemon_pid_reuse_liveness_advanced"
                            ),
                            "path": str(pid_path),
                            "identity_path": str(identity_path),
                            "pid": pid,
                        }
                    def current_reused_birth() -> ProcessBirthIdentity | None:
                        try:
                            return read_process_birth(pid)
                        except OSError:
                            return None

                    reused_birth_before = current_reused_birth()
                    if (
                        reused_birth_before is None
                        or reused_birth_before == current_identity.process_birth
                    ):
                        return {
                            "repaired": False,
                            "blocked": True,
                    ...
                    reused_birth_after_scan = current_reused_birth()
                    if (
                        reused_birth_after_scan is None
                        or reused_birth_after_scan != reused_birth_before
                        or not process_is_running(pid)
                    ):
                        return {
                            "repaired": False,
                    ...
                    # Keep the final birth read adjacent to marker mutation.
                    # A second PID reuse after either command scan is a new
                    # authority generation and cannot inherit this cleanup.
                    if current_reused_birth() != reused_birth_before:
                        return {
                            "repaired": False,
                            "blocked": True,
                            "reason": (
