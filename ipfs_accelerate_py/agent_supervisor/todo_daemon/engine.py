...
DEFAULT_CHECKBOX_RE = re.compile(
    r"^(?P<prefix>\s*-\s+\[)(?P<mark>[ xX~!])(?P<suffix>\]\s+)(?P<title>.+)$"
)
...
class Proposal:
    ...
    def valid(self) -> bool:
        return (
            self.applied
            and not self.errors
            and all(result.ok for result in self.validation_results)
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "validation_passed": bool(self.validation_results)
            and all(result.ok for result in self.validation_results),
    ...
    def validate_write_path(self, path: str, *, daemon_label: str = "todo daemon") -> list[str]:
        ...
        try:
            ...
            if self.is_private_write_path(normalized):
                errors.append(
                    f"private/session artifacts may not be generated by {daemon_label} proposals: {normalized}"
                )
        try:
            ...
            record_ref = str(normalized or path or (errors[0] if errors else "write-path"))
            mirror_work_record(
                catalog_kind="filesystem_mtime",
                record_kind="todo_write_path_validation",
    ...
    def has_visible_source_change(self, changed_files: Iterable[str]) -> bool:
        for path in changed_files:
            ...
            if any(normalized.startswith(prefix) for prefix in self.ignored_visible_prefixes):
                continue
            if any(
                normalized.startswith(prefix) for prefix in self.visible_source_prefixes
...
def build_validation_workspace_spec(
    *,
    ...
    return ValidationWorkspaceSpec(
        ...
        external_reference_paths=tuple(
            Path(path) for path in external_reference_paths if Path(path).as_posix()
        ),
        ignore=ignore,
...
def config_validation_workspace_spec(
    ...
    *,
    ...
    stale_seconds = getattr(
        config, str(stale_seconds_field or "worktree_stale_seconds"), default_stale_seconds
    )
    ...
    *,
    ...
    for line in markdown.splitlines():
        ...
        if mark.lower() == "x":
            ...
            status = "blocked"
        tasks.append(
            Task(index=len(tasks) + 1, title=title, status=status, checkbox_id=checkbox_id)
        )
    return tasks
...
def run_command(
    ...
    *,
    ...
    try:
        ...
        return CommandResult(
            tuple(command), 124, stdout or "", (stderr + "\n" + timeout_message).strip()
        )
    ...
    *,
    ...
    try:
        ...
        raise AttributeError(
            f"daemon config does not define {validation_commands_field!r}"
        ) from exc
...
def run_config_validation_commands(
    ...
    *,
    ...
    try:
        timeout_seconds = int(
            getattr(config, str(timeout_seconds_field or "command_timeout_seconds"))
        )
    except AttributeError as exc:
...
def temporary_validation_worktree(spec: ValidationWorkspaceSpec) -> Iterator[Path]:
    ...
    marker.write_text(
        json.dumps(
            worktree_marker_payload(state="initializing", source_root=spec.repo_root),
            indent=2,
            sort_keys=True,
        )
        + "\n",
    ...
    try:
        ...
        marker.write_text(
            json.dumps(
                worktree_marker_payload(state="ready", source_root=spec.repo_root),
                indent=2,
                sort_keys=True,
            )
            + "\n",
...
def verify_promoted_worktree_files(
