...
_DATABASE_PORTAL_ATTEMPT_STATE_DIR = re.compile(r"[0-9a-f]{24}")
...
class WorktreeLifecycleStore:
    ...
    @staticmethod
    def _retry_state_dir_scope_matches(
        recorded_state_dir: str | Path,
        expected_state_dir: str | Path,
    ) -> bool:
        """Return whether two state dirs are one exact or rotated attempt scope.

        Ordinary daemon restarts retain one exact state directory.  Database
        Portal retries intentionally rotate the final, content-derived
        24-hex attempt directory while retaining the exact parent authority.
        No other sibling-directory mismatch is accepted.
        """

        recorded_raw = str(recorded_state_dir or "").strip()
        expected_raw = str(expected_state_dir or "").strip()
        if not recorded_raw or not expected_raw:
            return False
        recorded = Path(normalize_workspace_path(recorded_raw))
        expected = Path(normalize_workspace_path(expected_raw))
        if recorded == expected:
            return True
        return bool(
            recorded.parent == expected.parent
            and recorded.parent.name.endswith("database_portal_attempts")
            and _DATABASE_PORTAL_ATTEMPT_STATE_DIR.fullmatch(recorded.name)
            and _DATABASE_PORTAL_ATTEMPT_STATE_DIR.fullmatch(expected.name)
        )

    def reclaim_exact_dead_task_attempt_for_retry(
        self,
        *,
        task_id: str,
        canonical_task_cid: str,
        ...
        merge_target: str,
        expected_state_dir: str | Path,
        reason: str = "dead_task_attempt_retry_reclaim",
    ) -> WorkspaceLifecycleRecord | None:
        """Terminalize one exact dead task-attempt claim before a retry.

        This is the narrow recovery path for a supervised worker that dies
        during a control-plane reload.  Lease age and a changed attempt state
        directory are never abandonment evidence.  Recovery requires the
        current repository, task CID, task alias, attempt, merge target,
        recorded branch/workspace/state binding, task index, lease, fence,
        record identity, and a PID-reuse-resistant DEAD process birth to all
        verify twice, including once under both durable guards.

        ``None`` means there is no indexed claim to recover (or it is already
        terminal).  Any present but unprovable nonterminal authority raises
        and remains byte-for-byte untouched.
        """

        expected_task_id = str(task_id or "").strip()
        expected_task_cid = str(canonical_task_cid or "").strip()
        if type(attempt) is not int:
            raise WorktreeLifecycleError(
                "retry lifecycle attempt is not an integer"
            )
        expected_attempt = attempt
        expected_target = str(merge_target or "").removeprefix("refs/heads/")
        if (
            ...
            raise WorktreeLifecycleError(
                "retry lifecycle identity is incomplete"
            )
        ...
        if index_payload is None:
            if index_path.exists():
                raise WorktreeLifecycleError(
                    "retry lifecycle task index is malformed"
                )
            return None
        indexed_workspace = index_payload.get("workspace_path")
        if type(indexed_workspace) is not str or not indexed_workspace:
            raise WorktreeLifecycleError(
                "retry lifecycle task index has no workspace"
            )

        current = self._load_strict_workspace_record(indexed_workspace)
        if current.is_terminal:
            return None
        expected_repo = normalize_workspace_path(self.repo_root)
        if (
            current.task_id != expected_task_id
            or current.canonical_task_cid != expected_task_cid
            or current.attempt != expected_attempt
            or not current.repo_root
            or normalize_workspace_path(current.repo_root) != expected_repo
            or current.merge_target.removeprefix("refs/heads/")
            != expected_target
            or not self._retry_state_dir_scope_matches(
                current.state_dir,
                expected_state_dir,
            )
        ):
            raise OwnershipError(
                "retry lifecycle authority is outside the exact task scope"
            )

        proof = self.require_exact_dead_owner(
        ...
        expected_binding = self._normalized_binding(
            ...
            canonical_task_cid=proof.canonical_task_cid,
            attempt=proof.attempt,
            workspace_path=proof.workspace_path,
            branch=proof.branch,
            merge_target=proof.merge_target,
            repo_root=proof.repo_root,
            state_dir=proof.state_dir,
        )
        record_path = self.workspace_path_for(proof.workspace_path)
        with serialized_lock_update(index_path):
            with serialized_lock_update(record_path):
                locked = self._load_strict_workspace_record(
                    proof.workspace_path
                )
                self._require_exact_dead_owner(
                    locked,
                    expected_record_id=proof.record_id,
                    expected_fence=proof.fence,
                    expected_lease_id=proof.lease_id,
                    expected_binding=expected_binding,
                    index_path=index_path,
                    expected_owner=proof.owner,
                )
                now = float(self.clock())
                terminal = replace(
                    locked,
                    state=WorkspaceLifecycleState.TERMINAL,
                    fence=int(locked.fence) + 1,
                    updated_at=now,
                    expires_at=now,
                    terminal_reason=str(
                        reason or "dead_task_attempt_retry_reclaim"
                    ),
                )
                _atomic_write_json(record_path, terminal.to_dict())
                _atomic_write_json(
