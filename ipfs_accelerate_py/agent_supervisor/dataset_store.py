...
"""Schema version for durable per-file scan diagnostic artifacts."""
...
class DatasetScanDetailsArtifact:
    """Content-addressed reference to complete scan diagnostic details.

    Refill receipts should embed :meth:`to_dict`, not the detail rows.  This
    keeps frequently copied supervisor state bounded while leaving every file
    exclusion and parser failure available in the adjacent JSONL artifact.
    """
    ...
    def row_count(self) -> int:
        """Dataset-style alias used by generic artifact consumers."""
...
class ObjectiveDatasetStore:
    ...
    def persist_scan_details(
        ...
        *,
        ...
        """Persist complete per-file scan details as an immutable artifact.

        The JSONL content is authoritative and content addressed.  Reusing a
        logical ``scan_id`` never destroys an earlier payload: it only updates
        an atomic ``latest`` pointer after the new JSONL and manifest are both
        durable.  Detail mappings are projected to JSON-safe values so callers
        may pass paths, datetimes, enums, and dataclass-backed diagnostics.
        """
        ...
        for index, detail in enumerate(details):
            ...
            if not isinstance(projected, dict):  # pragma: no cover - mapping projection invariant
        ...
        # A content-address collision necessarily has identical authoritative
        # bytes.  Rewriting atomically is harmless and also repairs a manually
        # removed/corrupted artifact before publishing the latest pointer.
    ...
    def load_scan_details(
        ...
        """Load complete detail rows from an artifact reference or scan ID.

        Missing or damaged rows return an empty collection, matching the
        defensive cache readers above.  A logical scan ID resolves through its
        atomic latest pointer; an artifact object or mapping resolves the
        immutable content-addressed JSONL directly.
        """
    ...
    def load_scan_details_manifest(
        ...
        """Load the manifest for an immutable reference or latest scan ID."""
...
def _json_compatible(value: Any) -> Any:
    """Return deterministic JSON-safe primitives for artifact persistence."""
