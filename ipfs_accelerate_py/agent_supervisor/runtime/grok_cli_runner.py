...
class _DockerContainerLease:
    ...
    def __init__(
        ...
        *,
        ...
        self.engine_endpoint = _validated_local_docker_engine_endpoint(engine_endpoint)
...
def _run_created_grok_container_with_typed_failure_capture(
    ...
    *,
    ...
    if (
        not isinstance(docker_lease, _DockerContainerLease)
        or docker_lease.docker_bin != docker_bin
        or docker_lease.docker_config != docker_config
        or docker_lease.cidfile != cidfile
    ):
        raise ValueError("Grok Docker start requires its exact native cleanup lease")
...
def _create_bound_provider_container(
    lease: object,
    command: Sequence[str],
    *,
    cwd: Path,
    ...
    """Dispatch only through the exact prepared native cleanup lease.

    The signed network/container builders must separately qualify the native
    anonymous start protocol. An older entrypoint layout cannot acquire that
    authority just because a newer cleanup class is importable.
    """

    if (
        not isinstance(lease, _DockerContainerLease)
        or lease.cleanup_binding_record is None
        or lease._cleanup_binding_identity is None
        or not isinstance(lease._cleanup_binding_value, Mapping)
        or lease._cleanup_binding_value.get("binding_state") != "prepared_no_dispatch"
    ):
        raise ValueError("Docker create requires an admitted native prepared cleanup binding")
    if (
        _DOCKER_PROVIDER_START_SCRIPT not in command
        or "--entrypoint=/bin/sh" not in command
    ):
        raise ValueError(
            "Docker signed launch profile lacks qualified native provider-start custody"
        )
    return lease.create_inert_container(command, cwd=cwd, env=env)
...
def _create_grok_container_and_build_start_command(
    ...
    *,
    ...
    try:
        created = _create_bound_provider_container(
            docker_lease, create_command, cwd=workspace, env=environment,
        )
...
def _run_codex_quota_fallback_in_docker(
    ...
    *,
    ...
    observation = dict(effect_observation or {})
    if (
        set(observation) != set(_DOCKER_EFFECT_OBSERVATION_FIELDS)
        or any(not isinstance(value, str) or not value for value in observation.values())
        or effect_claim is None
        or effect_terminal is None
    ):
        raise ValueError("Codex Docker effect requires exact durable observation and paired callbacks")
    ...
    try:
        ...
        try:
            created = _create_bound_provider_container(
                docker_lease, command, cwd=workspace, env=docker_environment,
            )
        except subprocess.TimeoutExpired as exc:
        ...
        *,
        ...
        try:
            ...
            fallback_returncode = _run_codex_quota_fallback_in_docker(
                ...
                effect_observation=(
                    {name: getattr(invocation_binding, name) for name in _DOCKER_EFFECT_OBSERVATION_FIELDS}
                    if invocation_binding is not None else None
                ),
