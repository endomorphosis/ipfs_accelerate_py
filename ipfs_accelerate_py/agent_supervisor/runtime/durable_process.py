...
def process_cgroup_path(pid: int) -> str:
    """Return the v2 cgroup path of one live process, or empty if unknown."""

    if int(pid) <= 0:
        raise DurableProcessError("process id must be positive")
    try:
        text = Path(f"/proc/{int(pid)}/cgroup").read_text(encoding="ascii")
    except OSError:
        return ""
...
def session_cgroup_owner(pid: int) -> bool:
    """True when the process is still owned by a login-session cgroup."""

    path = process_cgroup_path(pid)
    return "/session-" in path and path.endswith(".scope")


def adopt_pid_into_systemd_user_service(
    pid: int,
    *,
    unit_name: str,
    working_directory: Path,
    ...
    systemd_run_path: Optional[str] = None,
    systemctl_path: Optional[str] = None,
    busctl_path: Optional[str] = None,
    runner: _Runner = subprocess.run,
) -> DurableProcessLaunch:
    """Move one live process into a delegated systemd-user unit.

    This does not exec a new command, mint a store generation, or copy
    environment values.  It exists so a healthy owner started in a session
    cgroup can outlive that session without a successor seal.
    """
    ...
    if target_pid <= 0:
        raise DurableProcessError("process id must be positive")
    if not Path(f"/proc/{target_pid}").is_dir():
        raise DurableProcessError("process is not live")
    current = process_cgroup_path(target_pid)
    service_unit = _normalize_unit_name(unit_name)
    cwd = Path(working_directory).expanduser().resolve()
    output_path = Path(log_path).expanduser().resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if current.endswith(f"/{service_unit}") or current.endswith(f"/{service_unit}/"):
        return DurableProcessLaunch(
            backend="systemd-user",
            unit_name=service_unit,
            pid=target_pid,
    ...
    systemd_run = _resolve_tool("systemd-run", systemd_run_path)
    busctl = _resolve_tool("busctl", busctl_path)
    existing = inspect_systemd_user_service(
        service_unit,
    ...
    if not existing.get("exists"):
        created = _run(
            ...
            [
                ...
                "--property=KillMode=mixed",
                "--property=TimeoutStopSec=90s",
                "--property=SuccessExitStatus=0 143 SIGTERM",
                f"--property=StandardOutput=append:{output_path}",
                f"--property=StandardError=append:{output_path}",
                "--",
                "/bin/true",
        ...
        if created.returncode != 0:
            ...
            raise DurableProcessError(
                "systemd user adopt unit could not be created"
                + (f": {detail}" if detail else "")
            )
    attached = _run(
        runner,
    ...
    if attached.returncode != 0:
        detail = (attached.stderr or attached.stdout or "").strip()
        raise DurableProcessError(
            "systemd user process adopt failed"
            + (f": {detail}" if detail else "")
        )
    adopted = process_cgroup_path(target_pid)
    if service_unit not in adopted:
        raise DurableProcessError(
            "process remained outside the delegated user unit after adopt"
        )
    if not Path(f"/proc/{target_pid}").is_dir():
        raise DurableProcessError("process died during adopt")
    return DurableProcessLaunch(
        backend="systemd-user",
        unit_name=service_unit,
        pid=target_pid,
