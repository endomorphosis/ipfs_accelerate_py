...
UNKNOWN_LIMIT = -1
ADAPTIVE_SCHEDULING_THROUGHPUT_REQUIREMENT_ID = "122080003600146794820964010047426915846"
...
_ADAPTIVE_STAGE_PROFILE_BY_NAME = {item.stage: item for item in ADAPTIVE_STAGE_PROFILES}
...
_CAMPAIGN_STAGE_ADAPTIVE_HINTS = {
    "tokenizer": AdaptiveStageProfile("analysis", "tokenizer", "cpu-medium"),
    "training": AdaptiveStageProfile(
        "inference",
        "training",
        "gpu",
        requires_provider=True,
    ...
    "evaluation": AdaptiveStageProfile("validation", "evaluation", "cpu-validation"),
    "checkpoint": AdaptiveStageProfile(
        "persistence",
        "checkpoint",
        "io-artifact",
        disk_sensitive=True,
    ),
    "corpus": AdaptiveStageProfile(
        "persistence",
        "corpus",
        "io-artifact",
        disk_sensitive=True,
    ),
    "split": AdaptiveStageProfile("analysis", "split", "cpu-medium"),
    "promotion": AdaptiveStageProfile(
        "merge",
        "promotion",
        "git-merge",
        disk_sensitive=True,
    ),
    "publication": AdaptiveStageProfile(
        "persistence",
        "publication",
        "io-artifact",
        disk_sensitive=True,
    ),
    "network": AdaptiveStageProfile(
        "persistence",
        "network",
        "network",
        disk_sensitive=True,
    ),
    "prover": AdaptiveStageProfile("proof", "prover", "cpu-proof-solver"),
}
...
def adaptive_stage_profile(stage: Any) -> AdaptiveStageProfile:
    ...
    if profile is not None:
        return profile
    campaign = _CAMPAIGN_STAGE_ADAPTIVE_HINTS.get(name)
...
CAMPAIGN_RESOURCE_KINDS = tuple(item.value for item in CampaignResourceKind)
...
EXCLUSIVE_AUTHORITIES = tuple(item.value for item in ExclusiveAuthority)
...
PIPELINE_HAZARDS = tuple(item.value for item in PipelineHazard)

_TOKENIZER_CONSUMER_STAGES = frozenset(
    {
        "tokenizer",
        "training",
        "evaluation",
...
_SEALED_INPUT_STAGES = frozenset(
    {
        "training",
        "evaluation",
        "proof",
...
_SAFE_STAGE_OVERLAPS = frozenset(
    {
        frozenset(("analysis", "inference")),
        frozenset(("analysis", "proof")),
        frozenset(("analysis", "validation")),
        frozenset(("analysis", "persistence")),
        frozenset(("analysis", "training")),
        frozenset(("analysis", "evaluation")),
        frozenset(("analysis", "network")),
        frozenset(("analysis", "prover")),
        frozenset(("analysis", "split")),
        frozenset(("inference", "proof")),
        frozenset(("inference", "persistence")),
        frozenset(("inference", "validation")),
        frozenset(("inference", "prover")),
        frozenset(("inference", "network")),
        frozenset(("proof", "persistence")),
        frozenset(("proof", "validation")),
        frozenset(("proof", "training")),
        frozenset(("proof", "network")),
        frozenset(("proof", "evaluation")),
        frozenset(("proof", "prover")),
        frozenset(("validation", "persistence")),
        frozenset(("validation", "network")),
        frozenset(("validation", "training")),
        frozenset(("validation", "prover")),
        frozenset(("persistence", "training")),
        frozenset(("persistence", "evaluation")),
        frozenset(("persistence", "prover")),
        frozenset(("persistence", "network")),
        frozenset(("persistence", "inference")),
        frozenset(("training", "prover")),
        frozenset(("training", "network")),
        frozenset(("evaluation", "prover")),
        frozenset(("evaluation", "network")),
        frozenset(("evaluation", "analysis")),
        frozenset(("network", "prover")),
        frozenset(("network", "split")),
        frozenset(("split", "proof")),
        frozenset(("split", "prover")),
        frozenset(("split", "evaluation")),
        frozenset(("corpus", "analysis")),
        frozenset(("corpus", "proof")),
        frozenset(("execution", "analysis")),
        frozenset(("execution", "persistence")),
        frozenset(("execution", "network")),
    }
)
_SERIAL_STAGES = frozenset(
    {
        "tokenizer",
        "checkpoint",
        "promotion",
...
_STAGE_RESOURCE_KINDS = {
    "analysis": (CampaignResourceKind.CPU.value,),
    "inference": (
        CampaignResourceKind.PROVIDER.value,
        CampaignResourceKind.TOKEN.value,
        CampaignResourceKind.CPU.value,
    ),
    "proof": (CampaignResourceKind.PROVER.value, CampaignResourceKind.CPU.value),
    "prover": (CampaignResourceKind.PROVER.value, CampaignResourceKind.CPU.value),
    "validation": (CampaignResourceKind.CPU.value,),
    "merge": (CampaignResourceKind.CPU.value, CampaignResourceKind.IO.value),
    "persistence": (CampaignResourceKind.IO.value,),
    "execution": (CampaignResourceKind.CPU.value, CampaignResourceKind.IO.value),
    "tokenizer": (CampaignResourceKind.CPU.value,),
    "training": (
        CampaignResourceKind.GPU.value,
        CampaignResourceKind.TOKEN.value,
        CampaignResourceKind.PROVIDER.value,
        CampaignResourceKind.CPU.value,
    ),
    ...
    "checkpoint": (CampaignResourceKind.IO.value, CampaignResourceKind.CPU.value),
    "corpus": (CampaignResourceKind.IO.value,),
    "split": (CampaignResourceKind.CPU.value, CampaignResourceKind.IO.value),
    "promotion": (CampaignResourceKind.CPU.value, CampaignResourceKind.IO.value),
    "publication": (CampaignResourceKind.IO.value, CampaignResourceKind.NETWORK.value),
    "network": (CampaignResourceKind.NETWORK.value, CampaignResourceKind.IO.value),
}
...
_PINNED_HUB_ALLOWLIST = (
    "https://huggingface.co",
    "https://huggingface.co/justicedao",
)

CAMPAIGN_RESOURCE_PROFILE_SPECS = {
    "RP-CPU-S": CampaignResourceProfileSpec(
        profile="RP-CPU-S",
        cpu_count=2,
        memory_bytes=4 * GIB,
        disk_bytes=4 * GIB,
        ...
        resource_kinds=(CampaignResourceKind.CPU.value,),
    ),
    "RP-CPU-M": CampaignResourceProfileSpec(
        profile="RP-CPU-M",
        cpu_count=4,
        memory_bytes=12 * GIB,
        disk_bytes=20 * GIB,
        wall_time_ms=90 * 60 * 1000,
        resource_class="cpu-medium",
        resource_kinds=(CampaignResourceKind.CPU.value,),
    ),
    "RP-IO-PINNED": CampaignResourceProfileSpec(
        profile="RP-IO-PINNED",
        cpu_count=4,
        memory_bytes=12 * GIB,
        disk_bytes=100 * GIB,
        wall_time_ms=4 * 60 * 60 * 1000,
        allows_network=True,
        network_allowlist=_PINNED_HUB_ALLOWLIST,
        resource_class="io-artifact",
        resource_kinds=(
            CampaignResourceKind.CPU.value,
            CampaignResourceKind.IO.value,
            CampaignResourceKind.NETWORK.value,
        ),
    ),
    "RP-PROVER": CampaignResourceProfileSpec(
        profile="RP-PROVER",
        cpu_count=6,
        memory_bytes=16 * GIB,
        disk_bytes=40 * GIB,
        ...
        resource_kinds=(
            CampaignResourceKind.CPU.value,
            CampaignResourceKind.PROVER.value,
        ),
    ),
    "RP-GPU": CampaignResourceProfileSpec(
        profile="RP-GPU",
        cpu_count=8,
        memory_bytes=32 * GIB,
        disk_bytes=250 * GIB,
        ...
        allows_gpu=True,
        allows_provider=True,
        deny_missing_gpu_telemetry=True,
    ...
    "RP-MIXED": CampaignResourceProfileSpec(
        profile="RP-MIXED",
        cpu_count=8,
        memory_bytes=32 * GIB,
        disk_bytes=250 * GIB,
        ...
        network_allowlist=_PINNED_HUB_ALLOWLIST,
        resource_class="cpu-medium",
        resource_kinds=CAMPAIGN_RESOURCE_KINDS,
    ),
}
...
def resource_pool(resource_class: Any) -> str:
    ...
    if normalized == ProofResourceClass.ARTIFACT.value:
        return "artifact"
    if normalized in {"gpu", "gpu-training", "gpu-memory"}:
        return "gpu"
    if normalized in {"network", "https", "hub-pin"}:
        return "network"
    if normalized in {"token", "token-budget"}:
        return "token"
    if normalized in {"prover", "cpu-prover"}:
        return "prover"
    return "cpu-proof"
...
class HostResourceSnapshot:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "HostResourceSnapshot":
        ...
        return cls(
            ...
            cpu_percent=_integer(
                _first(value, ("cpu_percent", "cpu_usage_percent", "cpu"), 0), 0, minimum=0
            ),
            memory_percent=_integer(
                _first(value, ("memory_percent", "memory_usage_percent", "memory"), 0), 0, minimum=0
            ),
            disk_percent=_integer(
                _first(value, ("disk_percent", "disk_usage_percent", "disk"), 0), 0, minimum=0
            ),
            memory_available_bytes=_integer(
                _first(
                    value,
                    ("memory_available_bytes", "available_memory_bytes", "memory_free_bytes"),
                    0,
                ),
                0,
                minimum=0,
            ...
            disk_available_bytes=_integer(
                _first(
                    value, ("disk_available_bytes", "available_disk_bytes", "disk_free_bytes"), 0
                ),
                0,
                minimum=0,
            ),
    ...
    try:
        ...
        try:
            for device_index in range(int(pynvml.nvmlDeviceGetCount())):
                ...
                gpu_memory_available += _integer(getattr(info, "free", 0), 0, minimum=0)
        finally:
            pynvml.nvmlShutdown()
        if gpu_memory_total:
            gpu_memory_percent = max(
                ...
                min(
                    100,
                    100 - gpu_memory_available * 100 // gpu_memory_total,
                ),
            )
    except Exception:
...
class ProviderCapacity:
    ...
    def from_mapping(
        ...
        *,
        ...
        available = _first(
            value, ("available_concurrency", "available_capacity", "free_slots"), None
        )
        if available is not None and not any(
            key in value
            for key in ("max_concurrency", "concurrency_limit", "max_workers", "capacity")
        ):
        ...
        return cls(
            ...
            capabilities=_strings(
                _first(value, ("capabilities", "supported_capabilities", "features"), ())
            ),
            observed_at_ms=_integer(
                _first(value, ("observed_at_ms", "measured_at_ms", "timestamp_ms"), 0), 0, minimum=0
            ),
            retry_after_ms=_integer(
                _first(value, ("retry_after_ms", "backoff_ms", "cooldown_ms"), 0), 0, minimum=0
            ),
        )
...
class LaneResourceRequirements:
    ...
    fairness_key: str = ""
    timeout_ms: int = 0
    input_sealed: bool = True
    trace_identity: str = ""
    expected_trace_identity: str = ""
    checkpoint_key: str = ""
    ...
    mutates_tokenizer: bool = False
    mutates_checkpoint: bool = False
    claimed_resource_kinds: tuple[str, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "input_sealed", _boolean(self.input_sealed, True))
        object.__setattr__(
            self,
            "trace_identity",
            str(self.trace_identity or "").strip(),
        )
        object.__setattr__(
            self,
            "expected_trace_identity",
            str(self.expected_trace_identity or "").strip(),
        )
        object.__setattr__(
            self,
            "checkpoint_key",
            str(self.checkpoint_key or "").strip(),
        )
        object.__setattr__(
            self,
            "tokenizer_identity",
            str(self.tokenizer_identity or "").strip(),
        )
        object.__setattr__(
            self,
            "expected_tokenizer_identity",
            str(self.expected_tokenizer_identity or "").strip(),
        )
        object.__setattr__(
            self,
            "resource_profile",
            str(self.resource_profile or "").strip().upper(),
        )
        object.__setattr__(
            self,
            "exclusive_authorities",
        ...
        object.__setattr__(self, "requires_network", bool(self.requires_network))
        object.__setattr__(self, "requires_gpu", bool(self.requires_gpu))
        object.__setattr__(self, "requires_prover", bool(self.requires_prover))
        object.__setattr__(self, "mutates_tokenizer", bool(self.mutates_tokenizer))
        object.__setattr__(self, "mutates_checkpoint", bool(self.mutates_checkpoint))
        object.__setattr__(
            self,
            "claimed_resource_kinds",
            tuple(
                dict.fromkeys(
                    kind
                    for kind in (
                        normalize_campaign_resource_kind(item)
                        for item in self.claimed_resource_kinds
                    )
                    if kind
                )
            ),
    ...
    def resource_kinds(self) -> tuple[str, ...]:
        profile = stage_admission_profile(self.stage)
        kinds = [
            *self.claimed_resource_kinds,
            *profile.resource_kinds,
        ]
        derived = normalize_campaign_resource_kind(self.resource_class)
        if derived:
            kinds.append(derived)
        if self.requires_gpu or self.gpu_memory_bytes:
            kinds.append(CampaignResourceKind.GPU.value)
        if self.requires_prover:
            kinds.append(CampaignResourceKind.PROVER.value)
        if self.requires_network or self.network_allowlist:
            kinds.append(CampaignResourceKind.NETWORK.value)
        if self.provider_required:
            kinds.append(CampaignResourceKind.PROVIDER.value)
            kinds.append(CampaignResourceKind.TOKEN.value)
        if self.disk_bytes or profile.resource_class == "io-artifact":
            kinds.append(CampaignResourceKind.IO.value)
        return tuple(dict.fromkeys(kind for kind in kinds if kind))

    @property
    def exclusive_authority_keys(self) -> tuple[str, ...]:
        profile = stage_admission_profile(self.stage)
        authorities = [
            *self.exclusive_authorities,
            *profile.exclusive_authorities,
        ]
        if self.mutates_tokenizer or profile.mutates_tokenizer:
            authorities.append(ExclusiveAuthority.TOKENIZER.value)
        if self.mutates_checkpoint or profile.mutates_checkpoint:
            authorities.append(ExclusiveAuthority.CHECKPOINT.value)
        return tuple(dict.fromkeys(authorities))

    @property
    def tokenizer_mutating(self) -> bool:
        return bool(
            self.mutates_tokenizer or stage_admission_profile(self.stage).mutates_tokenizer
        )

    @property
    def checkpoint_mutating(self) -> bool:
        return bool(
            self.mutates_checkpoint or stage_admission_profile(self.stage).mutates_checkpoint
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        payload["exclusive_authorities"] = list(self.exclusive_authorities)
        payload["network_allowlist"] = list(self.network_allowlist)
        payload["claimed_resource_kinds"] = list(self.claimed_resource_kinds)
        payload["provider_required"] = self.provider_required
        payload["resource_kinds"] = list(self.resource_kinds)
        payload["exclusive_authority_keys"] = list(self.exclusive_authority_keys)
        return payload
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "LaneResourceRequirements":
        ...
        provider = (
            str(first("provider_id", "llm_provider", "provider", default="") or "").strip().lower()
        )
        context = _integer(
            first(
                "context_tokens",
        ...
        tokens = _integer(
            first(
                "token_budget", "required_tokens", "estimated_tokens", "max_new_tokens", default=0
            ),
            0,
            minimum=0,
        )
        return cls(
            lane_id=str(
                first("lane_id", "bundle_key", "parallel_lane", "task_cid", default="") or ""
            ),
            stage=normalize_adaptive_stage(
                first("stage", "scheduler_stage", "pipeline_stage", default="execution")
            ),
            resource_class=str(first("resource_class", default="cpu-small") or "cpu-small")
            .strip()
            .lower(),
            required_capabilities=_strings(
                first("required_capabilities", "capabilities", default=())
            ),
            provider_id=provider,
            requires_provider=_boolean(
                first("requires_provider", "requires_llm", default=False), False
            ),
            context_tokens=context,
            token_budget=tokens,
            quota_units=_integer(
                first("quota_units", "quota_cost", "request_cost", default=1), 1, minimum=0
            ),
            memory_bytes=_integer(
                first("memory_bytes", "required_memory_bytes", default=0), 0, minimum=0
            ),
            ...
            disk_bytes=_integer(
                first("disk_bytes", "required_disk_bytes", default=0), 0, minimum=0
            ),
            ...
            * 1000,
            ...
            timeout_ms=_integer(
                first("timeout_ms", "deadline_ms", "wall_time_ms", default=0),
                0,
                minimum=0,
            ),
            input_sealed=_boolean(first("input_sealed", "sealed", default=True), True),
            trace_identity=str(
                first("trace_identity", "trace_cid", "proof_trace_identity", default="")
                or ""
            ),
            expected_trace_identity=str(
                first(
                    "expected_trace_identity",
                    "required_trace_identity",
            ...
            checkpoint_key=str(
                first("checkpoint_key", "checkpoint_id", "checkpoint_lease_key", default="")
                or ""
            ),
            tokenizer_identity=str(
                first(
            ...
            expected_tokenizer_identity=str(
                first(
                    "expected_tokenizer_identity",
                    "required_tokenizer_identity",
                    "frozen_tokenizer_identity",
                    default="",
                )
                or ""
            ),
            exclusive_authorities=_strings(
                first("exclusive_authorities", "authorities", default=())
            ),
            resource_profile=str(
                first("resource_profile", "campaign_resource_profile", default="") or ""
            ),
            network_allowlist=_strings(
                first("network_allowlist", "allowed_network", default=())
            ),
            requires_network=_boolean(
                first("requires_network", "needs_network", default=False),
                False,
            ),
            requires_gpu=_boolean(first("requires_gpu", "needs_gpu", default=False), False),
            requires_prover=_boolean(
                first("requires_prover", "needs_prover", default=False),
                False,
            ),
            mutates_tokenizer=_boolean(
                first("mutates_tokenizer", "tokenizer_mutation", default=False),
                False,
            ),
            mutates_checkpoint=_boolean(
                first("mutates_checkpoint", "checkpoint_mutation", default=False),
                False,
            ),
            claimed_resource_kinds=_strings(
                first("claimed_resource_kinds", "resource_kinds", default=())
            ),
        )
...
class ResourceLeaseBudget:
    ...
    max_processes: int = 1
    max_gpu_concurrency: int = 0
    max_network_concurrency: int = 0
    max_prover_concurrency: int = 0
    max_token_concurrency: int = 0
    wall_time_ms: int = 0
    cpu_time_ms: int = 0
    memory_bytes: int = 0
    gpu_memory_bytes: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            ...
            "max_prover_concurrency",
            "max_token_concurrency",
            "wall_time_ms",
            "cpu_time_ms",
            "memory_bytes",
            "gpu_memory_bytes",
        ...
        *,
        ...
        return ChildResourceLimits(
            ...
            model_token_limit=min(
                item for item in (self.model_token_limit, requirement.token_budget) if item > 0
            )
            if self.model_token_limit and requirement.token_budget
            else (self.model_token_limit or requirement.token_budget),
            provider_quota=min(
                item for item in (self.provider_quota, requirement.quota_units) if item > 0
            )
...
class ResourcePolicy:
    ...
    max_network_concurrency: int = 1
    max_prover_concurrency: int = 0
    network_allowlist: tuple[str, ...] = ()
    allow_unsealed_inputs: bool = False

    def __post_init__(self) -> None:
        if self.max_lanes < 0:
            raise ValueError("max_lanes must be non-negative")
        for name in (
            "cpu_high_watermark_percent",
        ...
        for name in (
            ...
            "maximum_provider_latency_ms",
            "provider_quota_reserve",
            "provider_token_reserve",
            "max_cpu_proof_concurrency",
            "max_model_concurrency",
            "max_artifact_concurrency",
        ...
        if not 0 <= int(self.adaptive_artifact_pressure_high_watermark_percent) <= 100:
            raise ValueError(
                "adaptive_artifact_pressure_high_watermark_percent must be in [0, 100]"
            )
        if self.adaptive_minimum_throughput_multiplier < 1:
            raise ValueError("adaptive_minimum_throughput_multiplier must be positive")
        if self.adaptive_max_duplicate_compute_percent > 100:
            raise ValueError("adaptive_max_duplicate_compute_percent must be in [0, 100]")
        if (
        ...
        object.__setattr__(
            ...
            tuple(
                dict.fromkeys(
                    str(item).strip() for item in (self.network_allowlist or ()) if str(item).strip()
                )
            ),
        )
        object.__setattr__(self, "allow_unsealed_inputs", bool(self.allow_unsealed_inputs))
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ResourcePolicy":
        ...
        return cls(
            max_lanes=_integer(
                _first(value, ("max_lanes", "worker_limit", "max_workers"), defaults.max_lanes),
                defaults.max_lanes,
                minimum=0,
            ),
            cpu_high_watermark_percent=_integer(
                _first(
                    value,
                    ("cpu_high_watermark_percent", "max_cpu_percent"),
                    defaults.cpu_high_watermark_percent,
                ),
                defaults.cpu_high_watermark_percent,
                minimum=0,
            ),
            memory_high_watermark_percent=_integer(
                _first(
                    value,
                    ("memory_high_watermark_percent", "max_memory_percent"),
                    defaults.memory_high_watermark_percent,
                ),
                defaults.memory_high_watermark_percent,
                minimum=0,
            ),
            disk_high_watermark_percent=_integer(
                _first(
                    value,
                    ("disk_high_watermark_percent", "max_disk_percent"),
                    defaults.disk_high_watermark_percent,
                ),
                defaults.disk_high_watermark_percent,
                minimum=0,
            ),
            ...
            minimum_memory_available_bytes=_integer(
                _first(value, ("minimum_memory_available_bytes", "min_memory_available_bytes"), 0),
                0,
                minimum=0,
            ),
            ...
            minimum_disk_available_bytes=_integer(
                _first(value, ("minimum_disk_available_bytes", "min_disk_available_bytes"), 0),
                0,
                minimum=0,
            ),
            maximum_provider_latency_ms=_integer(
                _first(
                    value,
                    ("maximum_provider_latency_ms", "max_provider_latency_ms", "latency_limit_ms"),
                    defaults.maximum_provider_latency_ms,
                ),
                defaults.maximum_provider_latency_ms,
                minimum=0,
            ),
            provider_quota_reserve=_integer(
                _first(value, ("provider_quota_reserve", "quota_reserve"), 0), 0, minimum=0
            ),
            provider_token_reserve=_integer(
                _first(value, ("provider_token_reserve", "token_reserve"), 0), 0, minimum=0
            ),
            ...
            max_gpu_concurrency=_integer(
                _first(value, ("max_gpu_concurrency", "gpu_concurrency"), 1),
                1,
                minimum=0,
            ),
            max_network_concurrency=_integer(
                _first(value, ("max_network_concurrency", "network_concurrency"), 1),
                1,
                minimum=0,
            ),
            max_prover_concurrency=_integer(
                _first(value, ("max_prover_concurrency", "prover_concurrency"), 0),
                0,
                minimum=0,
            ),
            network_allowlist=_strings(
                _first(value, ("network_allowlist", "allowed_network"), ())
            ),
            allow_unsealed_inputs=_boolean(
                _first(value, ("allow_unsealed_inputs", "permit_unsealed"), False),
                False,
            ),
        )
...
class AdaptiveStageCapacity:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        payload["signal_limits"] = {
            str(name): int(value) for name, value in sorted(self.signal_limits.items())
        }
        return payload
...
class AdaptiveStageMetrics:
    ...
    def acceptance_throughput_per_million_ms(self) -> int:
        return self.accepted * 1_000_000 // self.total_duration_ms if self.total_duration_ms else 0

    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["backpressure_reasons"] = {
            str(name): int(value) for name, value in sorted(self.backpressure_reasons.items())
        }
        payload["admission_ratio_millionths"] = self.admission_ratio_millionths
        payload["acceptance_throughput_per_million_ms"] = self.acceptance_throughput_per_million_ms
        return payload
...
class ResourcePoolAdmissionSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.admitted_count + self.backpressured_count != self.scheduled_count:
            raise ValueError(
                "resource pool admitted and backpressured counts must cover every scheduled lane"
            )
        if len(self.fairness_order) != self.scheduled_count:
            raise ValueError("resource pool fairness order must identify every scheduled lane")
        if len(self.fairness_keys) != self.scheduled_count:
            raise ValueError("resource pool fairness keys must identify every scheduled lane")
        if len(self.admitted_lane_ids) != self.admitted_count:
            raise ValueError("resource pool admitted lane identities must match admitted_count")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "admitted_lane_ids": list(self.admitted_lane_ids),
            "backpressure_counts": {
                str(name): int(value) for name, value in sorted(self.backpressure_counts.items())
            },
        }
...
class ResourceScheduleSnapshot:
    ...
    overlap_receipts: tuple[PipelineOverlapReceipt, ...] = ()
    stage_admission_profiles: tuple[StageAdmissionProfile, ...] = ()
    fairness_overlap_receipt: BackpressureFairnessOverlapReceipt | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "adaptive_metrics": (
                self.adaptive_metrics.to_dict() if self.adaptive_metrics is not None else None
            ),
            "active_lease_count": self.active_lease_count,
            "backpressure_counts": {
                str(name): int(value) for name, value in sorted(self.backpressure_counts.items())
            },
            "pool_admissions": [item.to_dict() for item in self.pool_admissions],
            "task_generation": (
                self.task_generation.to_dict() if self.task_generation is not None else None
            ),
            "overlap_receipts": [item.to_dict() for item in self.overlap_receipts],
            "stage_admission_profiles": [
                item.to_dict() for item in self.stage_admission_profiles
            ...
            "fairness_overlap_receipt": (
                self.fairness_overlap_receipt.to_dict()
                if self.fairness_overlap_receipt is not None
                else None
            ),
            "signals": json.loads(json.dumps(dict(self.signals), sort_keys=True)),
        }
...
class ResourceScheduler:
    ...
    def adaptive_stage_capacity(
        ...
        *,
        ...
        artifact_pressure = min(100, max(0, int(artifact_pressure_percent)))
        ...
        if name not in {"merge", "persistence"}:
            artifact_limit = self.policy.adaptive_artifact_pressure_high_watermark_percent
            artifact_target = min(
                artifact_limit,
            ...
            if merge_limit and current_merge_debt:
                signal_limits["merge_debt"] = max(
                    0,
                    resource_limit * max(0, merge_limit - current_merge_debt) // merge_limit,
                )
        resource_limit = min(signal_limits.values(), default=configured)
        minimum = min(
            resource_limit,
        ...
        if profile.gpu_memory_sensitive and snapshot.gpu_memory_total_bytes > 0:
        ...
        if self.policy.adaptive_enabled and resource_limit:
            ...
            if pressure > target:
                ...
                candidate = minimum + (max(0, resource_limit - minimum) * remaining // span)
        ...
        if self.policy.adaptive_enabled:
            with self._metrics_lock:
                ...
                metric = self._stage_metrics.setdefault(name, _MutableStageMetrics())
                if state is None:
                    ...
                    state.last_observed_at_ms = max(state.last_observed_at_ms, observed_at_ms)
                    ...
                    if (
                        continue_recovery
                        and state.recovery_samples >= self.policy.adaptive_recovery_samples
                    ):
                        state.effective_limit = candidate
                        state.recovery_samples = 0
                    ...
                    state.recovery_samples = (
                        state.recovery_samples if state.state == "recovering" else 0
                    )
                effective = min(resource_limit, state.effective_limit)
                hysteresis_state = state.state
                recovery_samples = state.recovery_samples
    ...
    def metrics_snapshot(self, *, observed_at_ms: int | None = None) -> AdaptiveResourceMetrics:
        ...
        with self._metrics_lock:
            for stage, count in lease_stages.items():
                self._stage_metrics.setdefault(stage, _MutableStageMetrics()).active_leases = count
        ...
        *,
        ...
        with self._metrics_lock:
            ...
            if reasons:
                ...
                state.recovery_samples = 0
                state.last_observed_at_ms = max(state.last_observed_at_ms, observed)
                hysteresis_state = "contracted"
            elif state.backpressured:
                hysteresis = self.policy.adaptive_hysteresis_percent
                queue_margin = max(1, queue_limit * hysteresis // 100) if queue_limit else 0
                merge_margin = max(1, merge_limit * hysteresis // 100) if merge_limit else 0
                recovered_low = (
                    (not queue_limit or pending <= max(0, queue_limit - queue_margin))
                    and (not merge_limit or debt <= max(0, merge_limit - merge_margin))
                    and (
                        not artifact_limit
                        or artifact_pressure <= max(0, artifact_limit - hysteresis)
                    )
                ...
                if state.recovery_samples >= self.policy.adaptive_recovery_samples:
    ...
    def fair_work_order(
        self,
        requirements: Iterable[LaneResourceRequirements | Mapping[str, Any]],
    ...
    def select_stealable_work(
        ...
        *,
        ...
        starved = tuple(
            item for item in ordered if starvation_age and item.queue_age_ms >= starvation_age
        )
        ...
        starvation_override = bool(
            starved and selected is starved[0] and selected.stage != home and local
        )
    ...
    def _fair_requirements(
        ...
        for item in requirements:
            grouped.setdefault(item.fairness_key or item.stage, deque()).append(item)
        starvation_age = self.policy.adaptive_starvation_age_ms
        merge_priority_age = self.policy.adaptive_merge_age_ms

        def priority(item: LaneResourceRequirements) -> tuple[Any, ...]:
            starved = bool(starvation_age and item.queue_age_ms >= starvation_age)
            overdue_merge = bool(
                item.stage == "merge"
    ...
    def _host_reasons(
        self, host: HostResourceSnapshot, requirement: LaneResourceRequirements
    ) -> list[str]:
        reasons: list[str] = []
        policy = self.policy
        ...
        if (
            ...
            and host.gpu_memory_percent >= policy.gpu_memory_high_watermark_percent
        ):
        ...
        required_gpu_memory = (
            ...
            if stage_profile.gpu_memory_sensitive or requirement.gpu_memory_bytes
            else 0
        )
        required_disk = (
            max(
        ...
        if required_gpu_memory and host.gpu_memory_available_bytes < required_gpu_memory:
        ...
        if (
            ...
            cpu_extension_compatible = (
                ...
                and any(
                    resource_class.startswith("cpu-") for resource_class in host.resource_classes
                )
            )
            legacy_compatible = (
            ...
            gpu_compatible = requirement.resource_class in {
                "gpu",
                "gpu-training",
                "gpu-memory",
            } and (
                "gpu" in host.capabilities
                or host.gpu_memory_total_bytes > 0
                or requirement.resource_class in host.resource_classes
                or "gpu" in host.resource_classes
            )
            network_compatible = requirement.resource_class in {
                "network",
                "https",
                "hub-pin",
            } and (
                "network" in host.capabilities
                or "https" in host.capabilities
                or requirement.resource_class in host.resource_classes
                or bool(requirement.network_allowlist or self.policy.network_allowlist)
            )
            prover_compatible = requirement.resource_class in {
                "prover",
                "cpu-prover",
            } and (
                "prover" in host.capabilities
                or any(
                    item.startswith("cpu-proof") or item == "prover"
                    for item in host.resource_classes
                )
            )
            if not (
                cpu_extension_compatible
                ...
                or gpu_compatible
                or network_compatible
                or prover_compatible
            ):
    ...
    def _provider_reasons(
        ...
        if (
            provider.context_window_tokens >= 0
            and requirement.context_tokens > provider.context_window_tokens
        ):
    ...
    def _lease_pool_limit(self, pool: str, budget: ResourceLeaseBudget) -> int:
        mapping = {
            "cpu-proof": budget.max_cpu_proof_concurrency,
            "model": budget.max_model_concurrency,
            "artifact": budget.max_artifact_concurrency,
            "gpu": budget.max_gpu_concurrency or budget.max_parallel,
            "network": budget.max_network_concurrency or budget.max_parallel,
            "prover": budget.max_prover_concurrency or budget.max_cpu_proof_concurrency,
            "token": budget.max_token_concurrency or budget.max_model_concurrency,
        }
        return mapping.get(pool, budget.max_parallel)
    ...
    def _pool_limit(self, pool: str) -> int:
        ...
        if pool == "gpu":
            return self.policy.max_gpu_concurrency or 1
        if pool == "network":
            return self.policy.max_network_concurrency or 1
        if pool == "prover":
            return self.policy.max_prover_concurrency or (
                self.policy.max_cpu_proof_concurrency or self.policy.max_lanes
            )
        if pool == "token":
            return self.policy.max_model_concurrency or self.policy.max_lanes
    ...
    @staticmethod
    def _checkpoint_identity(requirement: LaneResourceRequirements) -> str:
        if requirement.checkpoint_key:
            return requirement.checkpoint_key
        if requirement.checkpoint_mutating:
            return ExclusiveAuthority.CHECKPOINT.value
        return ""

    def _effective_network_allowlist(
        self,
        requirement: LaneResourceRequirements,
    ) -> tuple[str, ...]:
        return tuple(
            dict.fromkeys((*self.policy.network_allowlist, *requirement.network_allowlist))
        )
    ...
    def overlap_reasons(
        ...
        *,
        ...
        """Return fail-closed pipeline-overlap and hazard reasons."""

        return self._overlap_reasons(
            requirement,
            host=host,
    ...
    def _overlap_reasons(
        ...
        *,
        host: HostResourceSnapshot,
        active_items: Sequence[LaneResourceRequirements],
    ) -> list[str]:
        reasons: list[str] = []
        profile = stage_admission_profile(requirement.stage)
        if requirement.timeout_ms and requirement.queue_age_ms >= requirement.timeout_ms:
            reasons.append(PipelineHazard.TIMED_OUT.value)
        if (
            profile.requires_sealed_inputs
            and not requirement.input_sealed
            and not self.policy.allow_unsealed_inputs
        ):
            reasons.append(PipelineHazard.UNSEALED_DATA.value)
        if (
            requirement.expected_trace_identity
            and requirement.trace_identity != requirement.expected_trace_identity
        ):
            reasons.append(PipelineHazard.STALE_TRACE.value)
        if (
            requirement.expected_tokenizer_identity
            and requirement.tokenizer_identity != requirement.expected_tokenizer_identity
        ):
            reasons.append(PipelineHazard.INCOMPATIBLE_TOKENIZER_MUTATION.value)
        needs_gpu = requirement.requires_gpu or (
            CampaignResourceKind.GPU.value in requirement.resource_kinds
            and requirement.gpu_memory_bytes
        )
        if needs_gpu and host.gpu_memory_total_bytes <= 0 and host.gpu_memory_available_bytes <= 0:
            reasons.append(PipelineHazard.GPU_TELEMETRY_MISSING.value)
        if requirement.requires_prover:
            prover_ready = (
                "prover" in host.capabilities
                or any(
                    item.startswith("cpu-proof") or item == "prover"
            ...
            if not prover_ready:
                reasons.append(PipelineHazard.PROVER_NOT_ADMITTED.value)
        needs_network = requirement.requires_network or (
            CampaignResourceKind.NETWORK.value in requirement.resource_kinds
            and requirement.stage in {"network", "publication"}
        )
        if requirement.requires_network or needs_network and requirement.stage == "network":
            allowlist = self._effective_network_allowlist(requirement)
            if not allowlist:
                reasons.append(PipelineHazard.NETWORK_DENIED.value)
            elif requirement.network_allowlist and not self._effective_network_allowlist(
                requirement
            ):
                reasons.append(PipelineHazard.NETWORK_NOT_ALLOWLISTED.value)
        if requirement.requires_network and not self._effective_network_allowlist(requirement):
            if PipelineHazard.NETWORK_DENIED.value not in reasons:
                reasons.append(PipelineHazard.NETWORK_DENIED.value)
        candidate_authorities = set(requirement.exclusive_authority_keys)
        candidate_checkpoint = self._checkpoint_identity(requirement)
        for active in active_items:
            if active.lane_id and active.lane_id == requirement.lane_id:
                continue
            shared_authorities = candidate_authorities.intersection(
                active.exclusive_authority_keys
            )
            if shared_authorities:
                if ExclusiveAuthority.CHECKPOINT.value in shared_authorities:
                    reasons.append(PipelineHazard.CHECKPOINT_COLLISION.value)
                if ExclusiveAuthority.TOKENIZER.value in shared_authorities:
                    reasons.append(PipelineHazard.INCOMPATIBLE_TOKENIZER_MUTATION.value)
                reasons.append(PipelineHazard.EXCLUSIVE_AUTHORITY_COLLISION.value)
            active_checkpoint = self._checkpoint_identity(active)
            if (
                candidate_checkpoint
                and active_checkpoint
                and candidate_checkpoint == active_checkpoint
            ):
                reasons.append(PipelineHazard.CHECKPOINT_COLLISION.value)
            if requirement.tokenizer_mutating and (
                active.stage in _TOKENIZER_CONSUMER_STAGES or active.tokenizer_mutating
            ):
                reasons.append(PipelineHazard.INCOMPATIBLE_TOKENIZER_MUTATION.value)
            if active.tokenizer_mutating and (
                requirement.stage in _TOKENIZER_CONSUMER_STAGES
                or requirement.tokenizer_mutating
            ):
                reasons.append(PipelineHazard.INCOMPATIBLE_TOKENIZER_MUTATION.value)
            campaign_overlap = (
                requirement.stage in CAMPAIGN_PIPELINE_STAGES
                or active.stage in CAMPAIGN_PIPELINE_STAGES
                or requirement.exclusive_authority_keys
                or active.exclusive_authority_keys
                or requirement.tokenizer_mutating
                or active.tokenizer_mutating
                or requirement.checkpoint_mutating
                or active.checkpoint_mutating
            )
            if campaign_overlap and not campaign_stages_may_overlap(
                requirement.stage, active.stage
            ):
                reasons.append(PipelineHazard.PROHIBITED_STAGE_OVERLAP.value)
    ...
    def _overlap_receipt(
        ...
        *,
        ...
        active_items: Sequence[LaneResourceRequirements],
        observed_at_ms: int,
    ) -> PipelineOverlapReceipt:
        hazards = tuple(reason for reason in reasons if reason in PIPELINE_HAZARDS)
        overlapping = tuple(
            item
            for item in active_items
            if not (item.lane_id and item.lane_id == requirement.lane_id)
        )
        return PipelineOverlapReceipt(
            lane_id=requirement.lane_id,
            stage=requirement.stage,
            admitted=admitted,
            resource_kinds=requirement.resource_kinds,
            overlapping_lane_ids=tuple(item.lane_id for item in overlapping if item.lane_id),
            overlapping_stages=tuple(dict.fromkeys(item.stage for item in overlapping)),
            exclusive_authorities=requirement.exclusive_authority_keys,
            hazards=hazards,
            reasons=tuple(reasons),
            fairness_key=requirement.fairness_key,
            observed_at_ms=observed_at_ms,
        )
    ...
    def evaluate(
        ...
        *,
        ...
        active_lease_count = sum(1 for item in active_items if item.stage == req.stage)
        decision_signals = {
        ...
        reserved_gpu_memory = sum(item.gpu_memory_bytes for item in active_items)
        ...
        required_gpu_memory = (
            ...
            if stage_profile.gpu_memory_sensitive or req.gpu_memory_bytes
            else 0
        )
        if (
            required_memory
        ...
        if (
            required_gpu_memory
            and host_snapshot.gpu_memory_available_bytes - reserved_gpu_memory < required_gpu_memory
        ...
        pool_occupied = sum(
            item.process_slots for item in active_items if item.resource_pool == req.resource_pool
        )
        ...
        class_occupied = sum(
            item.process_slots for item in active_items if item.resource_class == req.resource_class
        )
        ...
        if CampaignResourceKind.GPU.value in req.resource_kinds:
            gpu_occupied = sum(
                item.process_slots
                for item in active_items
                if CampaignResourceKind.GPU.value in item.resource_kinds
            )
            if gpu_occupied + req.process_slots > (self.policy.max_gpu_concurrency or 1):
                host_reasons.append("gpu_concurrency")
        if CampaignResourceKind.NETWORK.value in req.resource_kinds and (
            req.requires_network or req.stage in {"network", "publication"}
        ):
            network_occupied = sum(
                item.process_slots
                for item in active_items
                if CampaignResourceKind.NETWORK.value in item.resource_kinds
                and (item.requires_network or item.stage in {"network", "publication"})
            )
            if network_occupied + req.process_slots > (self.policy.max_network_concurrency or 1):
                host_reasons.append("network_concurrency")
        stage_occupied = sum(item.process_slots for item in active_items if item.stage == req.stage)
        if self.policy.adaptive_enabled or req.stage in self.policy.stage_concurrency_limits:
            memory_slots = (
                max(
                    0,
                    (host_snapshot.memory_available_bytes - reserved_memory) // req.memory_bytes,
                )
                if req.memory_bytes
                else UNKNOWN_LIMIT
            )
            gpu_slots = (
                max(
                    0,
                    (host_snapshot.gpu_memory_available_bytes - reserved_gpu_memory)
                    // req.gpu_memory_bytes,
                )
            ...
            disk_slots = (
                max(
                    0,
                    (host_snapshot.disk_available_bytes - reserved_disk) // req.disk_bytes,
                )
                if req.disk_bytes
                else UNKNOWN_LIMIT
            )
        ...
        candidates = [
            ...
            if not req.provider_id or item.provider_id == req.provider_id
        ]
        if not candidates:
            reason = (
                "provider_telemetry_unavailable"
                if self.policy.require_provider_telemetry
            ...
            return AdmissionDecision(
                ...
                reserved_gpu_memory_bytes=(
                    req.gpu_memory_bytes if not self.policy.require_provider_telemetry else 0
                ),
                reserved_disk_bytes=req.disk_bytes
                if not self.policy.require_provider_telemetry
                else 0,
        ...
        *,
        ...
        with self._lease_lock:
            ...
            if decision.admitted:
                pool_limit = self._lease_pool_limit(req.resource_pool, lease_budget)
                pool_used = sum(
                    item.process_slots for item in active if item.resource_pool == req.resource_pool
                )
                ...
                gpu_memory_used = sum(item.gpu_memory_bytes for item in active)
                tokens_used = sum(item.token_budget for item in active if item.provider_required)
                quota_used = sum(item.quota_units for item in active if item.provider_required)
                ...
                if pool_used + req.process_slots > pool_limit:
                    extra_reasons.append(f"lease_{req.resource_pool.replace('-', '_')}_concurrency")
                if (
                ...
                if lease_budget.disk_bytes and disk_used + req.disk_bytes > lease_budget.disk_bytes:
                    extra_reasons.append("lease_disk_budget")
                if (
                    lease_budget.gpu_memory_bytes
                    and gpu_memory_used + req.gpu_memory_bytes > lease_budget.gpu_memory_bytes
                ):
                    extra_reasons.append("lease_gpu_memory_budget")
                if (
                    lease_budget.model_token_limit
                    and tokens_used + req.token_budget > lease_budget.model_token_limit
                ):
            ...
            with self._metrics_lock:
                metric = self._stage_metrics.setdefault(req.stage, _MutableStageMetrics())
    ...
    def _host_excluding_accounted_requirements(
        ...
        return replace(
            ...
            available_worker_capacity=max(0, reported_capacity - external_active),
        )
    ...
    def cancel(
        ...
        identity = lease.lease_id if isinstance(lease, ResourceAdmissionLease) else str(lease)
        ...
        with self._lease_lock:
            ...
            if matched is not None:
                ...
                matching_ids = [
                    lease_id for lease_id, item in self._leases.items() if item.lane_id == identity
                ]
            ...
            if not known:
                return False
            released = [self._leases.pop(lease_id) for lease_id in matching_ids]
            lane_id = released[0].lane_id if released else identity
            if lane_id in self._cancelled_lanes:
                return False
            self._cancelled_lanes[lane_id] = normalized_reason
        stages = {item.requirement.stage for item in released} or {"execution"}
        with self._metrics_lock:
            for stage in stages:
                metric = self._stage_metrics.setdefault(stage, _MutableStageMetrics())
                metric.cancelled += 1
                metric.leases_released += sum(
                    1 for item in released if item.requirement.stage == stage
                )
                metric.active_leases = max(
                    0,
                    metric.active_leases
                    - sum(1 for item in released if item.requirement.stage == stage),
                )
        return True
    ...
    def transition(
        ...
        *,
        ...
        with self._lease_lock:
            ...
            others = tuple(
                item.requirement for other_id, item in self._leases.items() if other_id != lease_id
            )
            ...
            for other_id, item in self._leases.items():
                ...
                if other_id == lease_id or not item.provider_id:
                    continue
                reservation = reservations.setdefault(item.provider_id, _ProviderReservation())
                reservation.requests += 1
            ...
            if decision.admitted:
                ...
                total_processes = sum(item.process_slots for item in others) + req.process_slots
                memory = sum(item.memory_bytes for item in others) + req.memory_bytes
                gpu_memory = sum(item.gpu_memory_bytes for item in others) + req.gpu_memory_bytes
                ...
                if budget.gpu_memory_bytes and gpu_memory > budget.gpu_memory_bytes:
        ...
        with self._metrics_lock:
            ...
            new_metric = self._stage_metrics.setdefault(req.stage, _MutableStageMetrics())
            old_metric.active_leases = max(0, old_metric.active_leases - 1)
    ...
    def release_lane(self, lane_id: str) -> int:
        ...
        with self._lease_lock:
            matching = [
                lease_id for lease_id, lease in self._leases.items() if lease.lane_id == lane_id
            ]
    ...
    def schedule(
        ...
        *,
        ...
        path: Path | str = ".",
        active_workers: int = 0,
        active_requirements: Iterable[LaneResourceRequirements | Mapping[str, Any]] = (),
        ...
        if duplicates:
            raise ValueError(
                "duplicate lane_id values are not schedulable: " + ", ".join(duplicates)
            )
        with self._lease_lock:
        ...
        baseline_active = tuple(active_by_id[key] for key in sorted(active_by_id))
        ...
        normalized = tuple(
            replace(
                ...
                active_requests=max(
                    0,
                    provider.active_requests - lease_provider_counts.get(provider.provider_id, 0),
                ),
            )
        ...
        for item in baseline_active:
            ...
            if not provider_id:
                matching = [provider.provider_id for provider in normalized if provider.healthy]
                provider_id = matching[0] if len(matching) == 1 else ""
            if provider_id:
                reservation = reservations.setdefault(provider_id, _ProviderReservation())
                reservation.requests += 1
                reservation.quota += item.quota_units
                reservation.tokens += item.token_budget
        decisions: list[AdmissionDecision] = []
        admitted = 0
        admitted_requirements: list[LaneResourceRequirements] = list(baseline_active)
        ...
        if record_metrics:
            with self._metrics_lock:
                for decision in decisions:
                    metric = self._stage_metrics.setdefault(decision.stage, _MutableStageMetrics())
                    metric.scheduled += 1
                    if decision.admitted:
                        metric.admitted += 1
                    else:
                        metric.backpressured += 1
                        if PipelineHazard.TIMED_OUT.value in decision.reasons:
                            metric.timed_out += 1
        ...
        baseline_processes = sum(item.process_slots for item in baseline_active)
        host_blocked = any(
            reason
            in {
        ...
        host_free = max(
            0,
            total_host_capacity - host_snapshot.active_workers - baseline_processes,
        )
        ...
        if requirements and all(item.provider_required for item in requirements):
            ...
            for provider in normalized:
                reservation = reservations.get(provider.provider_id, _ProviderReservation())
                if (
                    provider.healthy
                    ...
                    provider_free += max(
                        0,
                        provider.available_concurrency - reservation.requests,
                    )
        ...
        backpressure_counts = {
            reason: sum(
                1 for decision in decisions if not decision.admitted and reason in decision.reasons
            )
        ...
        for pool_name in sorted({item.resource_pool for item in requirements}):
            ...
            for _requirement, decision in pool_items:
                if decision.admitted:
                    continue
                for reason in decision.reasons:
                    pool_backpressure[reason] = pool_backpressure.get(reason, 0) + 1
            admitted_pool_lane_ids = tuple(
                decision.lane_id for _requirement, decision in pool_items if decision.admitted
            )
            pool_admissions.append(
                ResourcePoolAdmissionSnapshot(
                    ...
                    backpressured_count=(len(pool_items) - len(admitted_pool_lane_ids)),
                    fairness_order=tuple(decision.lane_id for _requirement, decision in pool_items),
                    fairness_keys=tuple(
                        requirement.fairness_key for requirement, _decision in pool_items
                    ),
        ...
        for item in baseline_active:
            active_by_stage[item.stage] = active_by_stage.get(item.stage, 0) + item.process_slots
        for lease in leases:
            stage = lease.requirement.stage
            active_lease_by_stage[stage] = active_lease_by_stage.get(stage, 0) + 1
        for requirement, decision in zip(requirements, decisions):
            if decision.admitted:
                active_by_stage[requirement.stage] = (
                    active_by_stage.get(requirement.stage, 0) + requirement.process_slots
                )
        stages = sorted(set(queue_counts) | {item.stage for item in baseline_active})
        ...
        stage_capacities = tuple(
            self.adaptive_stage_capacity(
                ...
                merge_age_ms=max(
                    (item.merge_age_ms for item in requirements if item.stage == stage),
                ...
                provider_available_slots=(
                    ...
                    if (
                        stage == "inference"
                        and any(item.stage == stage for item in capacity_members)
                        and all(
        ...
        if len(stage_capacities) == 1:
            baseline_stage_active = sum(item.process_slots for item in baseline_active)
            effective = min(
                effective,
                max(
                    0,
                    stage_capacities[0].effective_limit - baseline_stage_active,
                ),
            )
            available = max(0, effective - admitted)
        ...
        generation = self.task_generation_backpressure(
            ...
            artifact_pressure_percent=signal_payload["artifact_pressure_percent"],
        ...
        observed_at_ms = host_snapshot.observed_at_ms or int(time.time() * 1000)
        overlap_receipts = tuple(
            self._overlap_receipt(
                requirement,
                admitted=decision.admitted,
                reasons=decision.reasons,
                active_items=tuple(
                    item
                    for item in (*baseline_active, *requirements)
                    if item.lane_id != requirement.lane_id
                    and (
                        item.lane_id in {entry.lane_id for entry in baseline_active}
                        or any(
                            other.admitted and other.lane_id == item.lane_id
        ...
        used_stages = tuple(
            dict.fromkeys(
                item.stage for item in (*baseline_active, *requirements)
            )
        )
        stage_profiles = tuple(stage_admission_profile(stage) for stage in used_stages)
        cancelled_lane_ids = tuple(
            decision.lane_id
            for decision in decisions
            if "cancelled" in decision.reasons
        )
        timed_out_lane_ids = tuple(
            decision.lane_id
            for decision in decisions
            if PipelineHazard.TIMED_OUT.value in decision.reasons
        )
        ...
        for decision in decisions:
            for reason in decision.reasons:
                if reason in PIPELINE_HAZARDS:
                    hazard_counts[reason] = hazard_counts.get(reason, 0) + 1
        fairness_overlap = BackpressureFairnessOverlapReceipt(
            observed_at_ms=observed_at_ms,
            fairness_order=tuple(decision.lane_id for decision in decisions),
            admitted_lane_ids=tuple(
                decision.lane_id for decision in decisions if decision.admitted
            ),
            backpressured_lane_ids=tuple(
                decision.lane_id for decision in decisions if not decision.admitted
            ),
            cancelled_lane_ids=cancelled_lane_ids,
            timed_out_lane_ids=timed_out_lane_ids,
            hazard_counts=hazard_counts,
            backpressure_counts=backpressure_counts,
            overlap_receipts=overlap_receipts,
            stage_admission_profiles=stage_profiles,
        )
...
class AdaptiveThroughputRun:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "fixture_ids", tuple(str(item) for item in self.fixture_ids))
        object.__setattr__(
            self,
    ...
    def duplicate_execution_count(self) -> int:
        return max(
            0,
            len(self.executed_fixture_ids) - len(set(self.executed_fixture_ids)),
        )
    ...
    def duplicate_compute_percent_millionths(self) -> int:
        if not self.executed_fixture_ids:
            return 0
        return self.duplicate_execution_count * 100_000_000 // len(self.executed_fixture_ids)
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "AdaptiveThroughputRun":
        return cls(
            ...
            executed_fixture_ids=tuple(str(item) for item in value.get("executed_fixture_ids", ())),
            accepted_fixture_ids=tuple(str(item) for item in value.get("accepted_fixture_ids", ())),
            duration_ms=_integer(value.get("duration_ms"), 0, minimum=0),
            peak_concurrency=_integer(value.get("peak_concurrency"), 0, minimum=0),
        )
...
def _adaptive_benchmark_failure_codes(
    ...
    *,
    ...
    if (
        ...
        < throughput_multiplier * baseline.accepted_count * adaptive.duration_ms
    ):
        failures.append(
...
class AdaptiveThroughputBenchmarkReceipt:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "repository_tree_id", str(self.repository_tree_id).strip())
        object.__setattr__(self, "failure_codes", tuple(str(item) for item in self.failure_codes))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "AdaptiveThroughputBenchmarkReceipt":
        return cls(
            ...
            baseline=AdaptiveThroughputRun.from_mapping(_mapping(value.get("baseline"))),
            adaptive=AdaptiveThroughputRun.from_mapping(_mapping(value.get("adaptive"))),
    ...
    def proved_requirement_ids_for(
        ...
        *,
        ...
        current_policy = (
            policy if isinstance(policy, ResourcePolicy) else ResourcePolicy.from_mapping(policy)
        )
        ...
        valid = (
            ...
            and self.requirement_id == ADAPTIVE_SCHEDULING_THROUGHPUT_REQUIREMENT_ID
...
def benchmark_adaptive_execution(
    fixtures: (Mapping[str, Callable[[], bool]] | Iterable[tuple[str, Callable[[], bool]]]),
    *,
    policy: ResourcePolicy | Mapping[str, Any],
    repository_tree_id: str,
) -> AdaptiveThroughputBenchmarkReceipt:
    ...
    def run(max_workers: int) -> AdaptiveThroughputRun:
        ...
        def invoke(fixture_id: str, callback: Callable[[], bool]) -> tuple[str, bool]:
        ...
        with ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(invoke, fixture_id, callback) for fixture_id, callback in items
            ]
...
class ProofWorkRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "work_kind", normalize_proof_work_kind(self.work_kind))
    ...
    def to_requirement(self) -> LaneResourceRequirements:
        ...
        if self.requires_provider:
            capabilities = tuple(
                f"llm:{item.removeprefix('llm:')}" if not item.startswith("host:") else item
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ProofWorkRequest":
        return cls(
            work_id=str(_first(value, ("work_id", "task_id", "lane_id", "request_id"), "") or ""),
            work_kind=normalize_proof_work_kind(_first(value, ("work_kind", "kind", "stage"), "")),
            provider_id=str(_first(value, ("provider_id", "route_id", "provider"), "") or ""),
            required_capabilities=_strings(
...
class GoalRuntimeResourceScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        provider_capacity_source: (
            ...
            | Callable[
                [],
                Mapping[str, Any] | Iterable[ProviderCapacity | Mapping[str, Any]] | None,
            ]
            | None
        ) = None,
        ...
        if (
            ...
            raise ValueError("max_fallback_concurrency must be a positive integer")
    ...
    def _enqueue(
        ...
        with self._state_lock:
            ...
            self._queues.setdefault(request.resource_class, deque()).append(request.work_id)
            return True
    ...
    def _run_fallback(
        ...
        *,
        ...
        if fallback is None:
            self._forget(request)
            status = ProofWorkStatus.FAILED if primary_error else ProofWorkStatus.BACKPRESSURED
            return self._result(
                request,
                status,
                queued_at_ms=queued_at_ms,
