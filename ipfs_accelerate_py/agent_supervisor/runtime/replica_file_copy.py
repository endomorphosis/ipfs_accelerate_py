#!/usr/bin/env python3
"""Copy a checkpointed native replica without closing a writer-process DB fd.
...
def copy_replica(source: Path, target: Path, *, timeout_seconds: float = COPY_TIMEOUT_SECONDS,
                 max_database_bytes: int = MAX_DATABASE):
    ...
    if (type(max_database_bytes) is not int or not 0 < max_database_bytes <= MAX_DATABASE
            or not 0 < before[2] <= max_database_bytes
            or type(timeout_seconds) not in (int, float) or not math.isfinite(timeout_seconds)
            or not 0 < timeout_seconds <= COPY_TIMEOUT_SECONDS):
        raise ReplicaCopyUnavailable()
    ...
    try:
        with tempfile.TemporaryFile() as output:
            ...
            try:
                child.communicate(packet, timeout=timeout_seconds)
