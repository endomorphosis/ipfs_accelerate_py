...
def strict_event_envelope_fields(event: Mapping[str, Any]) -> frozenset[str]:
    ...
    verifies the entire event hash/chain and its closed payload schema. No field is removed from
    the event or from a content-addressed receipt.
    """
    ...
    if (forbidden and key) or (
        event_coalescing_forbidden(event.get("type", "")) and not forbidden
    ):
