...
def hardware_command(args):
    ...
    if args.hardware_command == "info":
        ...
        result = kit.test_hardware(accelerator=args.accelerator, test_level=args.level)
        print_result(result, args.format)
        ...
        result = kit.recommend_hardware(
            model_name=args.model, task=args.task, consider_available_only=args.available_only
        )
        print_result(result, args.format)
...
def main():
    ...
    list_containers.add_argument(
        "--all", "-a", action="store_true", help="Include stopped containers"
    )
    ...
    hardware_subparsers = hardware_parser.add_subparsers(
        dest="hardware_command", help="Hardware command"
    )
    ...
    hw_recommend.add_argument(
        "--task", default="inference", choices=["inference", "training", "fine-tuning"]
    )
    ...
    runner_start.add_argument(
        "--max-runners", type=int, default=10, help="Maximum concurrent runners"
    )
    runner_start.add_argument(
        "--image", default="myoung34/github-runner:latest", help="Runner Docker image"
    )
    ...
    runner_list_workflows = runner_subparsers.add_parser(
        "list-workflows", help="List workflow queues"
    )

    # Runner list-containers
    runner_list_containers = runner_subparsers.add_parser(
        "list-containers", help="List active runner containers"
    )

    # Runner provision
    runner_provision = runner_subparsers.add_parser("provision", help="Manually provision runners")
    runner_provision.add_argument(
        "--repo", help="Repository (owner/name) - if omitted, provisions for all queues"
    )

    # Runner stop-container
    runner_stop_container = runner_subparsers.add_parser(
        "stop-container", help="Stop a runner container"
    )
    ...
    ipfs_files_subparsers = ipfs_files_parser.add_subparsers(
        dest="ipfs_files_command", help="IPFS files command"
    )
    ...
    ipfs_files_validate = ipfs_files_subparsers.add_parser(
        "validate-cid", help="Validate CID format"
    )
