...
class WorkflowTask:
    ...
    input_mapping: Optional[Dict[str, str]] = (
        None  # Maps output keys from dependencies to input keys
    )
...
class WorkflowStorage:
    ...
    def __init__(self, db_path: str = None):
        if not HAVE_DUCKDB:
            raise RuntimeError(
                "DuckDB is required for workflow storage. Install with: pip install duckdb"
            )
    ...
    def _init_db(self):
        ...
        with duckdb.connect(self.db_path) as conn:
            ...
            try:
                conn.execute(
                    f"COPY (SELECT * FROM workflows) TO '{workflows_parquet}' (FORMAT PARQUET)"
                )
            except Exception:
    ...
    def load_workflow(self, workflow_id: str) -> Optional[Workflow]:
        ...
        with duckdb.connect(self.db_path) as conn:
            ...
            task_results = conn.execute(
                "SELECT * FROM tasks WHERE workflow_id = ? ORDER BY task_id", (workflow_id,)
            ).fetchall()

            task_columns = [
                desc[0] for desc in conn.execute("SELECT * FROM tasks LIMIT 0").description
            ]
            ...
            for task_result in task_results:
                ...
                tasks.append(
                    WorkflowTask(
                        ...
                        if task_row.get("dependencies")
                        else [],
                        input_mapping=json.loads(task_row["input_mapping"])
                        if task_row.get("input_mapping")
                        else {},
                        output_keys=json.loads(task_row["output_keys"])
                        if task_row.get("output_keys")
                        else [],
                        vram_pinned=(
    ...
    def list_workflows(self, status: Optional[str] = None, limit: int = 100) -> List[Workflow]:
        ...
        with duckdb.connect(self.db_path) as conn:
            if status:
                ...
                results = conn.execute(
                    "SELECT workflow_id FROM workflows ORDER BY created_at DESC LIMIT ?", (limit,)
                ).fetchall()
...
class WorkflowEngine:
    ...
    async def execute_task(
        self, workflow: Workflow, task: WorkflowTask, task_results: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _execute_hf_pipeline_task(
        self, task: WorkflowTask, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_text_analysis(
        self, task: WorkflowTask, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_image_analysis(
        self, task: WorkflowTask, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_image_to_text(
        self, task: WorkflowTask, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_generic_inference(
        self, task: WorkflowTask, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def execute_workflow(self, workflow_id: str, stop_signal: threading.Event | None = None):
        ...
        if workflow.status not in [WorkflowStatus.PENDING.value, WorkflowStatus.PAUSED.value]:
            raise ValueError(
                f"Workflow {workflow_id} is not in a runnable state: {workflow.status}"
            )
...
class WorkflowManager:
    ...
    def create_workflow(self, name: str, description: str, tasks: List[Dict[str, Any]]) -> Workflow:
        ...
        workflow = Workflow(
            workflow_id=workflow_id, name=name, description=description, tasks=workflow_tasks
        )
    ...
    def update_workflow(
        ...
        if workflow.status == WorkflowStatus.RUNNING.value:
            raise ValueError(
                f"Cannot update running workflow {workflow_id}. Pause or stop it first."
            )
        ...
        return {
            ...
            "tasks": [
                ...
                },
                {
                    ...
                    "config": {"model": "stable-diffusion-xl", "width": 1024, "height": 1024},
                ...
                },
                {
                    ...
                    "config": {"model": "real-esrgan", "scale": 4},
        ...
        return {
            ...
            "tasks": [
                ...
                },
                {
                    ...
                    "type": "text-to-image",  # HuggingFace pipeline tag
                    "config": {"model": "stable-diffusion-xl", "width": 768, "height": 768},
                    "dependencies": [0],
                    "input_mapping": {"prompt": "0.enhanced_prompt"},
                    "output_keys": ["image"],
                },
                {
                    "name": "Animate to Video",
                    "type": "image-to-video",  # HuggingFace pipeline tag
                    "config": {"model": "animatediff", "duration": 4, "fps": 24},
                    "dependencies": [0, 1],  # Depends on both prompt and image
                    "input_mapping": {"prompt": "0.enhanced_prompt", "image": "1.image"},
            ...
            "tasks": [
                {
                    ...
                    "config": {"model": "nsfw-classifier", "filter_type": "nsfw"},
                    "dependencies": [],
                ...
                },
                {
                    ...
                    "config": {"model": "stable-diffusion-xl", "width": 1024, "height": 1024},
                    "dependencies": [0],
                    "input_mapping": {"prompt": "0.text"},
                    "output_keys": ["image"],
                },
                {
                    "name": "Quality Check",
                    "type": "image-classification",  # HuggingFace pipeline tag
                    "config": {"model": "image-quality-classifier", "threshold": 0.8},
                    "dependencies": [1],
                    "input_mapping": {"image": "1.image"},
        ...
        return {
            ...
            "tasks": [
                ...
                },
                {
                    ...
                    "type": "text-to-image",  # HuggingFace pipeline tag
                    "config": {"model": "stable-diffusion-xl", "width": 1024, "height": 576},
                    "dependencies": [0],
                    "input_mapping": {"prompt": "0.text"},
                    "output_keys": ["image"],
                },
                {
                    ...
                    "config": {"model": "elevenlabs-tts", "voice": "narrator"},
                    "dependencies": [0],
                    "input_mapping": {"text": "0.text"},
                    "output_keys": ["audio"],
                },
                {
                    "name": "Create Final Video",
                    "type": "image-to-video",  # HuggingFace pipeline tag
                    "config": {"model": "animatediff", "duration": 10, "fps": 30},
                    "dependencies": [1, 2],
                    "input_mapping": {"image": "1.image", "audio": "2.audio"},
                    "output_keys": ["video", "video_url"],
                },
            ],
        }
