...
def categorize_error(
    exception: Exception, hardware_type: Optional[str] = None, model_name: Optional[str] = None
) -> BenchmarkError:
    """
    ...
    if (
        "cuda out of memory" in exception_str
        or "insufficient memory" in exception_str
        or "out of memory" in exception_str
    ):
        return MemoryError(hardware_type=hardware_type, model_name=model_name, exception=exception)
    ...
    if isinstance(exception, RuntimeError):
        ...
        if "model" in exception_str and (
            "incompatible" in exception_str or "not supported" in exception_str
        ):
            if model_name and hardware_type:
                return ModelIncompatibleError(
                    model_name=model_name, hardware_type=hardware_type, reason=str(exception)
                )
...
def handle_hardware_unavailable(
    hardware_type: str, model_name: Optional[str] = None, fallback_hardware: Optional[str] = None
    ...
    """
    error = HardwareNotAvailableError(hardware_type=hardware_type, model_name=model_name)
...
def handle_simulated_hardware(
    hardware_type: str, model_name: Optional[str] = None, simulation_reason: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def handle_benchmark_exception(
    exception: Exception,
    hardware_type: Optional[str] = None,
    model_name: Optional[str] = None,
