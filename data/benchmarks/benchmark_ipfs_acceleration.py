...
def benchmark_loading():
    ...
    for module_name in modules:
        ...
        try:
            ...
            results[module_name] = {"status": "Success", "load_time_ms": load_time}
            ...
            for attr_name in component_attrs:
                if hasattr(module, attr_name):
                    ...
                    results[attr_key] = {"status": "Success", "load_time_ms": access_time}
...
def benchmark_basic_operations():
    ...
    try:
        ...
        try:
            ...
            results["config_access"] = {"status": "Success", "time_ms": access_time}
            logger.info(f"Config class accessed in {access_time:.2f} ms")
        except Exception as e:
            results["config_access"] = {"status": "Failed", "error": str(e)}
        ...
        try:
            ...
            results["backends_access"] = {"status": "Success", "time_ms": access_time}
            logger.info(f"Backends class accessed in {access_time:.2f} ms")
        except Exception as e:
            results["backends_access"] = {"status": "Failed", "error": str(e)}
        ...
        try:
            ...
            results["dispatch_access"] = {"status": "Success", "time_ms": access_time}
            logger.info(f"Dispatch function accessed in {access_time:.2f} ms")
        except Exception as e:
            results["dispatch_access"] = {"status": "Failed", "error": str(e)}
            logger.error(f"Error accessing dispatch function: {e}")

    except ImportError:
        logger.error("Failed to import ipfs_accelerate_py for benchmarks")
        results["import"] = {"status": "Failed", "error": "Failed to import ipfs_accelerate_py"}
    except Exception as e:
        logger.error(f"Error in benchmarks: {e}")
        results["general"] = {"status": "Failed", "error": str(e)}
...
def benchmark_parallel_loading(num_threads=5, iterations=3):
    ...
    results = {"threads": num_threads, "iterations": iterations, "thread_results": []}
...
def main():
    ...
    parser.add_argument(
        "--parallel", "-p", action="store_true", help="Run parallel loading benchmark"
    )
    parser.add_argument(
        "--threads", "-t", type=int, default=5, help="Number of threads for parallel benchmark"
    )
    parser.add_argument(
        "--iterations", "-i", type=int, default=3, help="Number of iterations for each thread"
    )
    ...
    loading_success = all(
        r.get("status") == "Success" for r in results["loading_benchmark"].values()
    )
    avg_load_time = sum(
        r.get("load_time_ms", 0)
        for r in results["loading_benchmark"].values()
        if r.get("status") == "Success"
    ) / len(results["loading_benchmark"])
    ...
    if args.parallel and "parallel_benchmark" in results:
        ...
        if "avg_load_time_ms" in parallel:
            ...
            print(
                f"  Min/Max load time: {parallel['min_load_time_ms']:.2f}/{parallel['max_load_time_ms']:.2f} ms"
            )
