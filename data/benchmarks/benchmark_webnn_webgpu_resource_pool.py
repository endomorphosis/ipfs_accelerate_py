...
required_modules = {"selenium": False, "websockets": False, "duckdb": False}
...
class ResourcePoolBenchmarker:
    ...
    def _import_modules(self):
        ...
        try:
            ...
            logger.error(
                "Make sure ipfs_accelerate_impl.py is in the current directory or PYTHONPATH"
            )
            self.ipfs_module = None
        ...
        try:
            ...
            logger.error(
                "Make sure fixed_web_platform/resource_pool_bridge.py is in the current directory or PYTHONPATH"
            )
        ...
        try:
            ...
            logger.error(
                "Make sure run_real_webgpu_webnn_fixed.py is in the current directory or PYTHONPATH"
            )
    ...
    async def detect_real_implementation(self):
        ...
        try:
            ...
            start_success = await web_implementation.start(
                allow_simulation=self.args.allow_simulation
            )
            ...
            if is_real:
                logger.info(
                    f"Real {self.args.platform} implementation detected in {self.args.browser}"
                )
                ...
                logger.warning(
                    f"Using simulation mode - real {self.args.platform} hardware not detected"
                )
    ...
    async def benchmark_single_model(self, model_name):
        ...
        try:
            ...
            if model_type == "text":
                test_content = (
                    "This is a benchmark test of IPFS acceleration with resource pool integration."
                )
            ...
            hardware_preferences = {
                "priority_list": [self.args.platform]
                if self.args.platform != "all"
                else ["webgpu", "webnn", "cpu"],
            ...
            acceleration_config = {
                ...
                "use_firefox_optimizations": (
                    self.args.browser == "firefox"
                    and self.args.optimize_audio
                    and model_type == "audio"
                ),
            }
            ...
            acceleration_result = self.ipfs_module.accelerate(
                model_name, test_content, acceleration_config
            )
    ...
    async def benchmark_concurrent_models(self, model_names):
        ...
        try:
            ...
            if len(models) == 1:
                ...
                if hasattr(primary_model, "run_concurrent") and callable(
                    primary_model.run_concurrent
                ):
            ...
            for i, model_name in enumerate(model_names):
                ...
                acceleration_config = {
                    ...
                    "use_firefox_optimizations": (
                        self.args.browser == "firefox"
                        and self.args.optimize_audio
                        and model_types[i] == "audio"
                    ),
                }

                # Run IPFS acceleration
                ipfs_result = self.ipfs_module.accelerate(
                    model_name, test_contents[i], acceleration_config
                )
    ...
    def _print_result_summary(self, result):
        ...
        print(
            f"Implementation: {'REAL HARDWARE' if result['is_real_implementation'] else 'SIMULATION'}"
        )
        print(
            f"Precision: {result['precision']}-bit{' (mixed)' if result['mixed_precision'] else ''}"
        )
    ...
    async def run_benchmarks(self):
        ...
        try:
            ...
            if self.args.concurrent_models > 1:
                ...
                logger.info(
                    f"Running concurrent benchmark with {len(model_names)} models: {', '.join(model_names)}"
                )
    ...
    def generate_markdown_report(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            if single_model_results:
                ...
                f.write(
                    "| Model | Platform | Browser | Real HW | Precision | Init (ms) | Inference (ms) | IPFS (ms) | Accel. Factor |\n"
                )
                f.write(
                    "|-------|----------|---------|---------|-----------|-----------|----------------|-----------|---------------|\n"
                )
                ...
                sorted_results = sorted(
                    ...
                    key=lambda r: (r.get("model_name", ""), r.get("platform", "")),
                )
                ...
                for result in sorted_results:
                    ...
                    f.write(
                        f"| {model} | {platform} | {browser} | {real} | {precision} | {init_time} | {inference_time} | {ipfs_time} | {accel_factor} |\n"
                    )
            ...
            if concurrent_results:
                ...
                f.write(
                    "| Models | Count | Platform | Browser | Real HW | Init (ms) | Concurrent (ms) | Sequential (ms) | Speedup |\n"
                )
                f.write(
                    "|--------|-------|----------|---------|---------|-----------|-----------------|----------------|--------|\n"
                )
                ...
                for result in concurrent_results:
                    ...
                    f.write(
                        f"| {models} | {count} | {platform} | {browser} | {real} | {init_time} | {concurrent_time} | {sequential_time} | {speedup} |\n"
                    )
            ...
            firefox_audio_results = [
                r
                for r in self.results
                if r.get("browser") == "firefox" and r.get("browser_optimizations", False)
            ]
            ...
            if firefox_audio_results:
                ...
                f.write(
                    "- Uses optimized compute shader workgroup size (256x1x1 vs Chrome's 128x2x1)\n"
                )
...
async def main_async():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark IPFS Acceleration with Resource Pool Integration"
    )
    ...
    # Model options
    parser.add_argument("--model", default="bert-base-uncased", help="Model to benchmark")
    ...
    if args.comprehensive:
        ...
        print(
            "- For audio models (Whisper): Use Firefox browser with WebGPU and audio optimizations"
        )
        print(
            "- For concurrent execution: Use resource pool with 3-4 connections for optimal throughput"
        )
