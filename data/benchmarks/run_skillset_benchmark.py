...
try:
    ...
    from data.benchmarks.benchmark_skillset import (
        SkillsetInferenceBenchmark,
        SkillsetThroughputBenchmark,
    )
...
def main():
    ...
    # Model selection
    parser.add_argument(
        "--model", type=str, default="bert", help="Model to benchmark (use 'all' for all models)"
    )
    parser.add_argument(
        "--random-sample",
        action="store_true",
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate HTML report of benchmark results"
    )
    ...
    benchmark_name = (
        ...
        else "skillset_throughput_benchmark"
    )
    ...
    try:
        ...
        result_file = os.path.join(
            args.output_dir, f"skillset_{args.type}_benchmark_{args.hardware}_{timestamp}.json"
        )
        ...
        if args.type == "inference":
            ...
            logger.info(
                f"Fastest model: {summary.get('fastest_model', 'N/A')} "
                f"({summary.get('fastest_init_time_ms', 0):.2f} ms)"
            )
            logger.info(
                f"Slowest model: {summary.get('slowest_model', 'N/A')} "
...
def generate_html_report(results, args, result_file):
    ...
    report_file = os.path.join(
        report_dir, f"skillset_{args.type}_benchmark_{args.hardware}_{timestamp}.html"
    )
    ...
    if args.type == "inference":
        ...
        if "speedup_over_sequential" in throughput:
            html.append(
                f"                <tr><td>Speedup over Sequential</td><td>{throughput.get('speedup_over_sequential', 0):.2f}x</td></tr>"
            )
    ...
    if args.type == "inference":
        ...
        for model_name, model_data in results.get("models", {}).items():
            ...
            if success:
                ...
                html.extend([f"                <td colspan='3'>{error}</td>"])
        ...
        for model_name, model_data in (
            results.get("throughput", {}).get("model_results", {}).items()
        ):
            success = model_data.get("success", False)
            status_class = "success" if success else "failure"
            status_text = "Success" if success else "Failed"
            ...
            if success:
                ...
                html.append(
                    f"                <td class='{status_class}'>{status_text}: {error}</td>"
                )
