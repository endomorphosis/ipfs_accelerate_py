...
def parse_args():
    ...
    parser.add_argument(
        "--hardware",
        type=str,
        choices=["cpu", "cuda", "rocm", "mps", "openvino", "webnn", "webgpu"],
        required=True,
    ...
    parser.add_argument(
        "--precision", type=str, default="fp32", help="Precision to use (fp32, fp16, int8)"
    )
    parser.add_argument(
        "--iterations", type=int, default=100, help="Number of iterations to run for each test"
    )
    parser.add_argument("--warmup", type=int, default=10, help="Number of warmup iterations")
    parser.add_argument(
        "--device-name", type=str, help="Specific device name (if multiple devices of same type)"
    )
    parser.add_argument(
        "--simulate",
        action="store_true",
    ...
    parser.add_argument("--commit", type=str, help="Git commit hash for this run")
    parser.add_argument("--branch", type=str, help="Git branch for this run")
        ...
        if missing_tables:
            ...
            if schema_script:
                ...
                try:
                    ...
                    logger.error(
                        "Please run scripts/create_benchmark_schema.py to initialize the database schema"
                    )
                    sys.exit(1)
            else:
...
def find_or_create_model(conn, model_name):
    ...
    if existing_model:
        logger.info(
            f"Found existing model: {model_name} (ID: {existing_model[0]}, Family: {existing_model[1]})"
        )
    ...
    logger.info(
        f"Created new model entry: {model_name} (ID: {new_model[0]}, Family: {model_family})"
    )
...
def find_or_create_hardware(conn, hardware_type, device_name=None):
    ...
    if existing_hardware:
        logger.info(
            f"Found existing hardware: {hardware_type} {device_name or ''} (ID: {existing_hardware[0]})"
        )
    ...
    if not device_name:
        ...
        if existing_any:
            logger.info(
                f"Found existing hardware type: {hardware_type} Device: {existing_any[1]} (ID: {existing_any[0]})"
            )
    ...
    logger.info(
        f"Created new hardware entry: {hardware_type} {device_name} (ID: {new_hardware_id})"
    )
...
def update_test_run_completion(conn, run_id, start_time):
    ...
    logger.info(
        f"Updated test run completion (ID: {run_id}, Execution time: {execution_time:.2f}s)"
    )


def run_benchmark(
    model_name, hardware_type, test_case, batch_size, precision, iterations, warmup, simulate=False
):
    """Run a benchmark test and return the results"""
    if simulate:
        logger.info(
            f"Simulating benchmark: {model_name} on {hardware_type}, {test_case}, batch_size={batch_size}"
        )
    ...
    logger.info(
        f"Running benchmark: {model_name} on {hardware_type}, {test_case}, batch_size={batch_size}"
    )
...
def add_performance_result(conn, run_id, model_id, hardware_id, result):
    ...
    logger.info(
        f"  - Latency: {result['average_latency_ms']:.2f} ms, Throughput: {result['throughput_items_per_second']:.2f} items/s"
    )
