...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("benchmark_timing_report.log")],
)
...
def main():
    ...
    parser.add_argument(
        "--api-server", action="store_true", help="Start API server for report data"
    )

    # Configuration options
    parser.add_argument(
        "--db-path", help="Path to benchmark database (defaults to BENCHMARK_DB_PATH env variable)"
    )
    parser.add_argument(
        "--output-dir",
        default="./reports",
    ...
    if not Path(db_path).exists():
        ...
        logger.error(
            "Please specify a valid database path with --db-path or set the BENCHMARK_DB_PATH environment variable"
        )
        return 1
    ...
    if args.generate:
        ...
        output_path = os.path.join(
            args.output_dir, f"benchmark_timing_report_{timestamp}.{args.format}"
        )
        ...
        result_path = report_gen.generate_timing_report(
            output_format=args.format, output_path=output_path, days_lookback=args.days
        )
        ...
        if result_path:
            ...
            latest_link = os.path.join(
                args.output_dir, f"benchmark_timing_report_latest.{args.format}"
            )
            try:
