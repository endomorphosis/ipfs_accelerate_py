...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("comprehensive_benchmark_analysis.log")],
)
...
def generate_performance_ranking(
    models=None, hardware=None, db_path=None, output_dir="./benchmark_results", report_format="html"
):
    """Generate performance ranking report."""
    logger.info("Generating performance ranking report...")

    # Prepare command
    cmd = [sys.executable, "run_performance_ranking.py", "--generate"]
...
def analyze_bottlenecks(
    ...
    # Prepare command
    cmd = [sys.executable, "identify_performance_bottlenecks.py", "--analyze"]
...
def generate_timing_report(db_path=None, output_dir="./benchmark_results", report_format="html"):
    ...
    # Prepare command
    cmd = [sys.executable, "benchmark_timing_report.py", "--generate"]
...
def run_all_analysis(
    ...
    if not skip_benchmarks:
        ...
        results["reports"].append({"type": "benchmark", "output": "See benchmark logs for details"})
    else:
    ...
    if success:
        ...
        results["reports"].append({"type": "performance_ranking", "path": ranking_path})
    else:
    ...
    if success:
        ...
        results["reports"].append({"type": "bottleneck_analysis", "path": bottleneck_path})
    else:
    ...
    success, timing_path = generate_timing_report(
        db_path=db_path, output_dir=output_dir, report_format=report_format
    )

    if success:
        # Add timing report to results
        results["reports"].append({"type": "timing_report", "path": timing_path})
    else:
        logger.warning("Timing report generation failed")

    # Generate summary report
    summary_path = os.path.join(
        output_dir, f"benchmark_analysis_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    # Main command groups
    parser.add_argument(
        "--run-all", action="store_true", help="Run all benchmark and analysis steps"
    )
    parser.add_argument(
        "--skip-benchmarks",
        action="store_true",
        help="Skip benchmarking step and only generate analysis reports",
    )
