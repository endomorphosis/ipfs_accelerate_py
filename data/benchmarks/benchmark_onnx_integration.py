...
def run_benchmark(
    model_path: str, model_config: Dict[str, Any], was_converted: bool, hardware_type: str
) -> Dict[str, Any]:
    """
                ...
                result.update(
                    {
                        ...
                        "onnx_conversion_time": datetime.now().isoformat()
                        if was_converted
                        else None,
...
def check_database_schema() -> bool:
    ...
    try:
        ...
        if not has_schema:
            logger.warning(
                "Database schema does not have ONNX tracking fields. Run onnx_db_schema_update.py to update the schema."
            )

        return has_schema
    except:
        logger.warning(
            "Error checking database schema. Assuming schema does not have ONNX tracking fields."
        )
        return False
...
def get_conversion_config(model_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Base configuration
    config = {"model_type": model_type, "opset_version": 12}
    ...
    if model_type == "bert":
        ...
        config.update(
            {
                "input_shapes": {"batch_size": 1, "feature_size": 80, "sequence_length": 3000},
...
def run_benchmarks_with_onnx_verification(hardware_type: str, models: Optional[List[str]] = None):
    ...
    for model_config in model_configs:
        ...
        try:
            ...
            model_path, was_converted = verify_and_get_onnx_model(
                model_id=model_id, onnx_path=onnx_path, conversion_config=conversion_config
            )
    ...
    # Print summary
    logger.info(
        f"Benchmark complete: {len(benchmark_config['benchmark_results'])} models benchmarked"
    )
...
def main():
    ...
    parser.add_argument(
        "--hardware", type=str, default="cpu", help="Hardware type to run benchmarks on"
    )
    parser.add_argument(
        "--models",
        type=str,
    ...
    # Run benchmarks
    run_benchmarks_with_onnx_verification(hardware_type=args.hardware, models=args.models)
