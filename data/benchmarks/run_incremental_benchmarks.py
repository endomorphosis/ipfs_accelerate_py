...
KEY_MODELS = {
    ...
    "llava_next": {
        "name": "llava-hf/llava-v1.6-mistral-7b",
        "family": "multimodal",
        "modality": "multimodal",
    },
...
class FullHardwareBenchmark:
    ...
    def __init__(
        ...
        self.hardware_platforms = [
            hw for hw in self.hardware_platforms if self.available_hardware.get(hw, False)
        ]
    ...
    def _detect_hardware(self) -> Dict[str, bool]:
        ...
        logger.info(
            f"Available hardware platforms: {', '.join(hw for hw, available in available_hardware.items() if available)}"
        )
    ...
    def run_benchmarks(self):
        ...
        """
        logger.info(
            f"Starting benchmarks for {len(self.models)} models on {len(self.hardware_platforms)} hardware platforms"
        )
        ...
        for model_key, model_info in self.models.items():
            ...
            for hw_type in self.hardware_platforms:
                ...
                compatibility = self._check_compatibility(
                    model_key, model_name, model_family, hw_type
                )
                if not compatibility["is_compatible"]:
                    logger.warning(
                        f"{model_key} is not compatible with {hw_type}: {compatibility['error_message']}"
                    )
                    self.results["benchmark_results"][model_key][hw_type] = {
                        "status": "incompatible",
                    ...
                    if not self.skip_database and run_id:
                        self._store_compatibility_in_db(
                            run_id, model_key, model_name, model_family, hw_type, compatibility
                        )
                ...
                try:
                    ...
                    if (
                        not self.skip_database
                        and os.path.exists("run_benchmark_with_db.py")
                        and run_id
                    ):
                        for batch_size in self.batch_sizes:
                            for test_case in test_cases:
                                ...
                                if result.returncode != 0:
                                    ...
                                    self.results["benchmark_results"][model_key][hw_type][
                                        "test_cases"
                                    ][test_case] = {"status": "failed", "error": result.stderr}
                                else:
                                    logger.info(f"Benchmark completed successfully")
                                    self.results["benchmark_results"][model_key][hw_type][
                                        "test_cases"
                                    ][test_case] = {"status": "completed", "batch_size": batch_size}
                    else:
                        ...
                        for batch_size in self.batch_sizes:
                            for test_case in test_cases:
                                ...
                                batch_result = self._simulate_benchmark(
                                    model_key,
                                    model_name,
                                    model_family,
                                    hw_type,
                                ...
                                if not self.skip_database and run_id:
                                    self._store_simulated_result_in_db(
                                        run_id,
                                        model_key,
                                        model_name,
                                        model_family,
    ...
    def _check_compatibility(
        self, model_key: str, model_name: str, model_family: str, hw_type: str
    ) -> Dict[str, Any]:
        """
        ...
        if hw_type == "cuda":
            return {"is_compatible": True, "compatibility_score": 1.0, "performance_rating": "high"}
        ...
        return {"is_compatible": True, "compatibility_score": 0.8, "performance_rating": "medium"}
    ...
    def _simulate_benchmark(
        ...
        latency_ms = (
            hw_factor["latency"]
            * family_factor["latency"]
            * batch_factor
            * random.uniform(0.8, 1.2)
        )
        throughput = (
            hw_factor["throughput"]
            * family_factor["throughput"]
            * batch_size
            / batch_factor
            * random.uniform(0.8, 1.2)
        )
        memory_mb = (
            hw_factor["memory"] * family_factor["memory"] * batch_factor * random.uniform(0.9, 1.1)
        )
    ...
    def _store_compatibility_in_db(
        self,
        run_id: int,
        model_key: str,
        model_name: str,
        ...
        try:
            ...
            if not model_id or not hardware_id:
                logger.warning(
                    f"Could not find or create model/hardware for {model_name} on {hw_type}"
                )
                return
            ...
            """,
                [
                    ...
                    json.dumps(
                        {"performance_rating": compatibility.get("performance_rating", "unknown")}
                    ),
                ],
            )
    ...
    def _store_simulated_result_in_db(
        self,
        run_id: int,
        model_key: str,
        model_name: str,
        ...
        try:
            ...
            logger.info(
                f"Stored performance result for {model_name} on {hw_type} (batch_size={batch_size}, test_case={test_case})"
            )
    ...
    def _find_or_create_model(self, model_name: str, model_family: str) -> Optional[int]:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
                [model_name, model_family, modality, "huggingface", json.dumps({})],
            )
    ...
    def _find_or_create_hardware(self, hw_type: str) -> Optional[int]:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?)
            """,
                [hw_type, device_name, hw_type.upper(), json.dumps({})],
            )
    ...
    def _generate_compatibility_matrix(self):
        ...
        for model_key, model_info in self.models.items():
            ...
            if model_family not in matrix["model_families"]:
                matrix["model_families"][model_family] = {"hardware_compatibility": {}}
            ...
            for hw_type in self.hardware_platforms:
                ...
                if hw_type in self.results["benchmark_results"].get(model_key, {}):
                    ...
                    if hw_result.get("status") == "completed":
                        ...
                        matrix["model_families"][model_family]["hardware_compatibility"][hw_type][
                            "benchmark_results"
                        ].append({"model_name": model_info["name"], "timestamp": self.timestamp})
                    else:
    ...
    def _generate_report(self) -> Path:
        ...
        with open(report_file, "w") as f:
            ...
            for model_key, model_info in self.models.items():
                f.write(
                    f"| {model_key} | {model_info['name']} | {model_info['family']} | {model_info['modality']} |\n"
                )
            ...
            for model_key, model_info in self.models.items():
                ...
                for m_key, m_info in self.models.items():
                    ...
                    for hw_type in self.hardware_platforms:
                        hw_results = (
                            self.results["benchmark_results"].get(m_key, {}).get(hw_type, {})
                        )
                ...
                for m_key, m_info in self.models.items():
                    ...
                    for hw_type in self.hardware_platforms:
                        ...
                        if hw_results.get("status") == "completed":
                            ...
                            if batch_results:
                                throughputs = [
                                    r.get("throughput", 0) for r in batch_results.values()
                                ]
                                if throughputs:
                                    avg_throughput = sum(throughputs) / len(throughputs)
            ...
            for model_key, model_results in self.results["benchmark_results"].items():
                for hw_type, hw_results in model_results.items():
                    if hw_results.get("status") in ["incompatible", "error"]:
                        has_issues = True
                        f.write(
                            f"- **{model_key}** on **{hw_type}**: {hw_results.get('error', 'Unknown error')}\n"
                        )
            ...
            if not self.skip_database:
                f.write(
                    f"All benchmark results have been stored in the database: `{self.db_path}`\n\n"
                )
                f.write("To query the database, use the following tools:\n\n")
                f.write("```bash\n")
                f.write("# Generate comprehensive performance report\n")
                f.write(
                    f"python scripts/benchmark_db_query.py --report performance --format html --output performance_report.html --db {self.db_path}\n\n"
                )
                f.write("# Generate hardware compatibility matrix\n")
                f.write(
                    f"python scripts/benchmark_db_query.py --report compatibility --format html --output compatibility_matrix.html --db {self.db_path}\n\n"
                )
                f.write("# Compare hardware platforms for a specific model\n")
                f.write(
                    f"python scripts/benchmark_db_query.py --model bert-tiny --metric throughput --compare-hardware --output bert_hardware_comparison.png --db {self.db_path}\n"
                )
                f.write("```\n\n")
            else:
                f.write(
                    "Benchmark results were not stored in the database. To enable database storage, rerun this script without the `--skip-database` flag.\n\n"
                )

            # Footer
            f.write("---\n\n")
            f.write(
                f"Generated by Full Hardware Benchmark Suite on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
    ...
    def _generate_visualizations(self):
        ...
        if not HAS_DEPENDENCIES:
            logger.warning(
                "Visualization dependencies not available. Skipping visualization generation."
            )
            return
        ...
        try:
            ...
            self._plot_family_hardware_performance(
                df, vis_dir, "throughput", "Throughput (items/sec)"
            )
    ...
    def _plot_family_hardware_performance(
        self, df: pd.DataFrame, vis_dir: Path, metric: str, metric_label: str
    ):
        """
    ...
    def _plot_hardware_comparison(self, df: pd.DataFrame, vis_dir: Path):
        ...
        hw_grouped = (
            df.groupby("hardware")
            .agg({"latency_ms": "mean", "throughput": "mean", "memory_mb": "mean"})
            .reset_index()
        )
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, default="./benchmark_db.duckdb", help="Path to the DuckDB database"
    )
    parser.add_argument("--skip-database", action="store_true", help="Skip database storage")
    parser.add_argument(
        "--hardware",
        type=str,
        nargs="+",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode with simulated results"
    )
