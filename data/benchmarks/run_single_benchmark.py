...
DEFAULT_SMALL_MODELS = ["bert-tiny", "t5-small", "vit-tiny", "clip-tiny"]
...
def run_benchmark(
    model: str, hardware_types: List[str], db_path: str, batch_sizes: Optional[List[int]] = None
):
    """
    ...
    cmd = ["python", "benchmark_all_key_models.py", "--specific-models", model, "--hardware"]
...
def update_simulation_status(db_path: str, model: str, hardware_types: List[str]):
    ...
    try:
        ...
        """.format(",".join(["?" for _ in hardware_types])),
            (model, *hardware_types),
        )
        ...
        logger.info(
            f"Updated simulation status for {model} on {', '.join(hardware_types)} to indicate REAL HARDWARE"
        )
    except Exception as e:
...
def run_full_benchmark(
    ...
    if not valid_hardware:
        logger.error(
            f"None of the specified hardware platforms ({', '.join(hardware_types)}) are available."
        )
...
def main():
    ...
    model_group.add_argument(
        "--model-list", type=str, help="Comma-separated list of models to benchmark"
    )
    model_group.add_argument(
        "--small-models", action="store_true", help="Use a predefined set of small models"
    )

    # Hardware options
    parser.add_argument(
        "--hardware", type=str, nargs="+", help="Hardware platforms to benchmark on"
    )
    parser.add_argument(
        "--all-available", action="store_true", help="Use all available hardware platforms"
    )
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        help="Path to the DuckDB database (default: BENCHMARK_DB_PATH env var or ./benchmark_db.duckdb)",
    )
    parser.add_argument(
        "--batch-sizes", type=int, nargs="+", default=[1, 2, 4, 8], help="Batch sizes to test"
    )
