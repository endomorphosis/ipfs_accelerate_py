#!/usr/bin/env python3
"""
Benchmark for the wav2vec2-conformer skillset implementation.

This benchmark measures the performance of the wav2vec2-conformer model implementation
...
@BenchmarkRegistry.register(
    name="wav2vec2-conformer_inference_benchmark",
    category="inference",
    models=["wav2vec2-conformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class Wav2Vec2ConformerInferenceBenchmark(BenchmarkBase):
    """Benchmark for wav2vec2-conformer inference performance."""

    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up wav2vec2-conformer inference benchmark")
        ...
        try:
            ...
            # Import the wav2vec2-conformer skillset
            self.model_name = "wav2vec2-conformer"
            self.model_type_safe = "wav2vec2_conformer"
    ...
    def execute(self):
        ...
        self.logger.info("Executing wav2vec2-conformer inference benchmark")
...
@BenchmarkRegistry.register(
    name="wav2vec2-conformer_throughput_benchmark",
    category="throughput",
    models=["wav2vec2-conformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class Wav2Vec2ConformerThroughputBenchmark(Wav2Vec2ConformerInferenceBenchmark):
    """Benchmark for wav2vec2-conformer throughput performance."""
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing wav2vec2-conformer throughput benchmark")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark wav2vec2-conformer skillset implementation"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    benchmark_name = (
        "wav2vec2-conformer_inference_benchmark"
        if args.type == "inference"
        else "wav2vec2-conformer_throughput_benchmark"
    )
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: wav2vec2-conformer")
