#!/usr/bin/env python3
"""
Benchmark for the autoformer skillset implementation.

This benchmark measures the performance of the autoformer model implementation
...
@BenchmarkRegistry.register(
    name="autoformer_inference_benchmark",
    category="inference",
    models=["autoformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class AutoformerInferenceBenchmark(BenchmarkBase):
    """Benchmark for autoformer inference performance."""

    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up autoformer inference benchmark")
        ...
        try:
            ...
            # Import the autoformer skillset
            self.model_name = "autoformer"
            self.model_type_safe = "autoformer"
    ...
    def execute(self):
        ...
        self.logger.info("Executing autoformer inference benchmark")
...
@BenchmarkRegistry.register(
    name="autoformer_throughput_benchmark",
    category="throughput",
    models=["autoformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class AutoformerThroughputBenchmark(AutoformerInferenceBenchmark):
    """Benchmark for autoformer throughput performance."""
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing autoformer throughput benchmark")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark autoformer skillset implementation")
    parser.add_argument(
        "--type",
        type=str,
    ...
    benchmark_name = (
        "autoformer_inference_benchmark"
        if args.type == "inference"
        else "autoformer_throughput_benchmark"
    )
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: autoformer")
