#!/usr/bin/env python3
"""
Benchmark for the audio-spectrogram-transformer skillset implementation.

This benchmark measures the performance of the audio-spectrogram-transformer model implementation
...
@BenchmarkRegistry.register(
    name="audio-spectrogram-transformer_inference_benchmark",
    category="inference",
    models=["audio-spectrogram-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class AudioSpectrogramTransformerInferenceBenchmark(BenchmarkBase):
    """Benchmark for audio-spectrogram-transformer inference performance."""

    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up audio-spectrogram-transformer inference benchmark")
        ...
        try:
            ...
            # Import the audio-spectrogram-transformer skillset
            self.model_name = "audio-spectrogram-transformer"
            self.model_type_safe = "audio_spectrogram_transformer"
    ...
    def execute(self):
        ...
        self.logger.info("Executing audio-spectrogram-transformer inference benchmark")
    ...
    def process_results(self, raw_results):
        ...
        for batch_key, batch_data in raw_results.get("batch_results", {{}}).items():
            if batch_data.get("error"):
                batch_metrics[batch_key] = {{"success": False, "error": batch_data["error"]}}
                continue
...
@BenchmarkRegistry.register(
    name="audio-spectrogram-transformer_throughput_benchmark",
    category="throughput",
    models=["audio-spectrogram-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class AudioSpectrogramTransformerThroughputBenchmark(AudioSpectrogramTransformerInferenceBenchmark):
    """Benchmark for audio-spectrogram-transformer throughput performance."""
    ...
    def setup(self):
        ...
        self.logger.info(
            f"Throughput benchmark with {{self.concurrent_workers}} concurrent workers"
        )
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing audio-spectrogram-transformer throughput benchmark")
        ...
        try:
            ...
            for i in range(self.concurrent_workers):
                ...
                init_functions.append(
                    create_init_func(init_method, self.model_name, self.hardware.name, i)
                )
            ...
            throughput = self.concurrent_workers / (
                mean_concurrent_time / 1000
            )  # models per second
    ...
    def process_results(self, raw_results):
                ...
                "throughput": {
                    {
                        ...
                        "throughput_models_per_second": concurrent_execution.get(
                            "throughput_models_per_second", 0
                        ),
                    }
                },
        ...
        sequential_time = base_processed.get("fastest_init_time_ms", 0) * raw_results.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark audio-spectrogram-transformer skillset implementation"
    )
    parser.add_argument(
        "--type",
        type=str,
        choices=["inference", "throughput"],
        default="inference",
    ...
    parser.add_argument(
        "--batch-sizes", type=str, default="1,2,4,8", help="Comma-separated list of batch sizes"
    )
    parser.add_argument(
        "--concurrent-workers",
        type=int,
    ...
    parser.add_argument(
        "--measurement-runs", type=int, default=5, help="Number of measurement runs"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    benchmark_name = (
        "audio-spectrogram-transformer_inference_benchmark"
        if args.type == "inference"
        else "audio-spectrogram-transformer_throughput_benchmark"
    )
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: audio-spectrogram-transformer")
            ...
            logger.info(
                f"Throughput: {{throughput.get('throughput_models_per_second', 0):.2f}} models/s"
            )
            if "speedup_over_sequential" in throughput:
                logger.info(
                    f"Speedup over sequential: {{throughput.get('speedup_over_sequential', 0):.2f}}x"
                )
