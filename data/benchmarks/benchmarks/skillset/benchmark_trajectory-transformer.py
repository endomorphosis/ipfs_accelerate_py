#!/usr/bin/env python3
"""
Benchmark for the trajectory-transformer skillset implementation.

This benchmark measures the performance of the trajectory-transformer model implementation
...
@BenchmarkRegistry.register(
    name="trajectory-transformer_inference_benchmark",
    category="inference",
    models=["trajectory-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class TrajectoryTransformerInferenceBenchmark(BenchmarkBase):
    """Benchmark for trajectory-transformer inference performance."""

    def setup(self):
        """Set up benchmark environment."""
        self.logger.info("Setting up trajectory-transformer inference benchmark")
        ...
        try:
            ...
            # Import the trajectory-transformer skillset
            self.model_name = "trajectory-transformer"
            self.model_type_safe = "trajectory_transformer"
    ...
    def execute(self):
        ...
        self.logger.info("Executing trajectory-transformer inference benchmark")
...
@BenchmarkRegistry.register(
    name="trajectory-transformer_throughput_benchmark",
    category="throughput",
    models=["trajectory-transformer"],
    hardware=["cpu", "cuda", "rocm", "openvino", "mps", "qnn"],
)
class TrajectoryTransformerThroughputBenchmark(TrajectoryTransformerInferenceBenchmark):
    """Benchmark for trajectory-transformer throughput performance."""
    ...
    def execute(self):
        """Execute the benchmark."""
        self.logger.info("Executing trajectory-transformer throughput benchmark")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark trajectory-transformer skillset implementation"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    benchmark_name = (
        "trajectory-transformer_inference_benchmark"
        if args.type == "inference"
        else "trajectory-transformer_throughput_benchmark"
    )
    ...
    try:
        ...
        if args.type == "inference":
            ...
            logger.info(f"Model: trajectory-transformer")
