...
try:
    ...
    logger.warning(
        "Warning: Some local modules could not be imported. Functionality may be limited."
    )
...
class VersionManager:
    ...
    def create_version(
        ...
        try:
            ...
            result = conn.execute(
                "SELECT id FROM version_history WHERE version_tag = ?", [version_tag]
            ).fetchone()
...
class TimeSeriesManager:
    ...
    def get_performance_history(
        ...
        try:
            ...
            df = pd.DataFrame(
                result, columns=["timestamp", metric, "version_tag", "batch_size", "test_type"]
            )
...
class RegressionDetector:
            ...
            for row in result:
                ...
                if current_group != group_key:
                    ...
                    if current_group and len(group_data) > window_size:
                        group_regressions = self._analyze_group(
                            group_data, current_group, metric, window_size, min_change
                        )
    ...
    def list_active_regressions(
        self, severity: Optional[str] = None, days: int = 30
    ) -> List[Dict[str, Any]]:
        """
    ...
    def resolve_regression(self, regression_id: int, resolution_note: Optional[str] = None) -> bool:
        ...
        try:
            ...
            result = conn.execute(
                "SELECT id FROM regression_alerts WHERE id = ?", [regression_id]
            ).fetchone()
...
class TrendVisualizer:
    ...
    def visualize_metric_trend(
        ...
        filename = os.path.join(
            output_dir, f"{model_name}_{hardware_type}_{metric}_{timestamp}.png"
        )
    ...
    def create_regression_dashboard(
        self, days: int = 30, limit: int = 10, output_file: Optional[str] = None
    ) -> str:
        """
        ...
        for i, regression in enumerate(regressions):
            ...
            details = (
                f"Change: {regression['percent_change']:.2f}% "
            ...
            plt.text(
                ...
                fontsize=10,
                bbox=dict(
                    facecolor=colors.get(regression["severity"], "white"),
                    alpha=0.2,
                    boxstyle="round",
                ),
            )
    ...
    def create_comparative_dashboard(
        ...
        for i, model_name in enumerate(model_names):
            ...
            for hardware_type in hardware_types:
                df = self.ts_manager.get_performance_history(
                    model_name=model_name, hardware_type=hardware_type, metric=metric, days=days
                )
                ...
                if not df.empty:
                    ...
                    # Plot data
                    plt.plot(
                        df["timestamp"], df[metric], marker="o", linestyle="-", label=hardware_type
                    )
...
This issue was automatically generated by the performance regression detection system.
    ...
    def send_email_notification(
        ...
        try:
            ...
            message["Subject"] = (
                f"[{regression['severity'].upper()}] Performance Regression: {regression['model_name']} on {regression['hardware_type']}"
            )
...
def main():
    ...
    detect_parser.add_argument(
        "--threshold", type=float, default=0.1, help="Regression threshold (0.1 = 10%)"
    )
    detect_parser.add_argument(
        "--window", type=int, default=5, help="Window size for baseline calculation"
    )
    detect_parser.add_argument(
        "--record", action="store_true", help="Record detected regressions in the database"
    )
    ...
    visualize_parser.add_argument(
        "--hide-versions", action="store_true", help="Hide version markers"
    )
    ...
    compare_parser.add_argument(
        "--models", required=True, help="Comma-separated list of model names"
    )
    compare_parser.add_argument(
        "--hardware", required=True, help="Comma-separated list of hardware types"
    )
    compare_parser.add_argument(
        "--metric",
        default="throughput",
    ...
    if args.command == "create-schema":
        ...
        output_file = visualizer.create_regression_dashboard(
            days=args.days, limit=args.limit, output_file=args.output
        )
