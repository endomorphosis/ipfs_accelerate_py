...
class ResultsProcessor:
    ...
    def calculate_statistics(results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "success_count": sum(
                1 for b in results.get("benchmarks", {}).values() if b.get("success", False)
            ),
            "failure_count": sum(
                1 for b in results.get("benchmarks", {}).values() if not b.get("success", False)
            ),
            "total_execution_time": sum(
                b.get("execution_time", 0) for b in results.get("benchmarks", {}).values()
            ),
        }
    ...
    def compare_results(
        ...
        for name in baseline_benchmarks & current_benchmarks:
            ...
            significant_changes = {
                k: v for k, v in metric_comparisons.items() if abs(v["change"]) >= threshold
            }
            ...
            if significant_changes:
                ...
                regressions = {
                    k: v for k, v in significant_changes.items() if not v["is_improvement"]
                }
