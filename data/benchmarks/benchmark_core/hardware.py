    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                core = Core()
                info.update(
                    {"available": True, "version": get_version(), "devices": core.available_devices}
                )
            except ImportError:
                # Alternative import for older versions
                from openvino import Core
                ...
                core = Core()
                info.update({"available": True, "devices": core.available_devices})
...
class QualcommBackend(HardwareBackend):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        if self.is_available():
            info.update({"device": "Qualcomm Neural Processor", "sdk_version": "2.0"})
...
class HardwareManager:
    ...
    def get_hardware(
        self, hardware_name: str, config: Optional[Dict[str, Any]] = None
    ) -> HardwareBackend:
        """
        ...
        for name, available in self._available_hardware.items():
            if available:
                try:
                    ...
                    info[name] = {"name": name, "available": False, "error": str(e)}
            else:
                info[name] = {"name": name, "available": False}
