...
class BenchmarkRunner:
    ...
    def execute(
        self, benchmark_name: str, params: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        benchmark = benchmark_class(hardware=hardware, config=params)
    ...
    def execute_suite(self, suite_config: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        suite_results_path = os.path.join(
            self.output_dir, f"{suite_name.replace(' ', '_')}_{timestamp}.json"
        )
        self.results_collector.save_results(suite_results_path)

        # Generate markdown report
        report_path = os.path.join(
            self.output_dir, f"{suite_name.replace(' ', '_')}_{timestamp}.md"
        )
    ...
    def compare_with_baseline(
        self, current_results_path: str, baseline_results_path: str, threshold: float = 0.05
    ) -> Dict[str, Any]:
        """
...
def ci_entrypoint():
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.05, help="Regression threshold (0.05 = 5%)"
    )
    ...
    runner = BenchmarkRunner(config={"output_dir": args.output})
    ...
    try:
        ...
        if args.benchmark:
            ...
            logger.info(
                f"Suite complete. {suite_result['benchmarks_executed']} benchmarks executed."
            )
            logger.info(
                f"Successful: {suite_result['success_count']}, Failed: {suite_result['failure_count']}"
            )
