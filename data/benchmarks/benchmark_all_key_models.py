...
try:
    ...
    def _handle_unavailable_hardware(hardware_type, model_name, batch_size, reason=None):
        ...
        """
        logger.warning(
            f"Hardware {hardware_type} not available for model {model_name}. Reason: {reason or 'Not detected'}"
        )
...
class KeyModelBenchmarker:
        ...
        try:
                ...
                if hw == "cpu":
                    ...
                    if not hw_available:
                        ...
                        if os.environ.get(f"{hw.upper()}_AVAILABLE") == "1":
                            logger.warning(
                                f"*** WARNING: {hw.upper()} hardware availability forced by environment variable ***"
                            )
                            logger.warning(
                                f"Benchmark results for {hw} will be SIMULATED and will NOT reflect real hardware performance"
                            )
                            hw_available = True
                            hw_simulated = True
                    ...
                    if hw_available and hw_simulated:
                        logger.warning(
                            f"*** SIMULATED MODE: {hw} hardware is being simulated, not real hardware ***"
                        )
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    if cuda_override:
                        logger.warning(
                            "*** WARNING: CUDA forced available by environment variable ***"
                        )
                ...
                if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                    ...
                    if mps_override:
                        logger.warning(
                            "*** WARNING: MPS forced available by environment variable ***"
                        )
            ...
            try:
                ...
                if openvino_override:
                    logger.warning(
                        "*** WARNING: OpenVINO forced available by environment variable ***"
                    )
            ...
            if webnn_simulation:
                ...
                logger.warning(
                    "*** WARNING: WebNN SIMULATION mode enabled via environment variable ***"
                )
            ...
            if webgpu_simulation:
                ...
                logger.warning(
                    "*** WARNING: WebGPU SIMULATION mode enabled via environment variable ***"
                )
            ...
            if qnn_simulation:
                ...
                simulated["qnn"] = True
                logger.warning(
                    "*** WARNING: QNN SIMULATION mode enabled via environment variable ***"
                )
    ...
    def check_implementation_issues(self) -> Dict[str, Dict[str, Any]]:
        ...
        if os.path.exists("CLAUDE.md"):
            try:
                ...
                logger.info(
                    f"Found {sum(len(hw_issues) for hw_issues in issues.values())} implementation issues"
                )
    ...
    def fix_implementation_issues(self) -> Dict[str, Dict[str, Any]]:
        ...
        success_count = sum(
            1
            for model_fixes in fixes.values()
    ...
    def _fix_t5_openvino(self) -> Dict[str, Any]:
        ...
        t5_test_file = None
        possible_paths = ["skills/test_hf_t5.py", "test_hf_t5.py", "modality_tests/test_hf_t5.py"]
        ...
        if not t5_test_file:
            return {"status": "error", "reason": "Could not find T5 test file"}
        ...
        if "openvino" in content.lower() and "mock" not in content.lower():
            return {"status": "skipped", "reason": "T5 already has OpenVINO implementation"}
    ...
    def _fix_clap_openvino(self) -> Dict[str, Any]:
        ...
        if not clap_test_file:
            return {"status": "error", "reason": "Could not find CLAP test file"}
    ...
    def _fix_wav2vec2_openvino(self) -> Dict[str, Any]:
        ...
        if not wav2vec2_test_file:
            return {"status": "error", "reason": "Could not find Wav2Vec2 test file"}
    ...
    def _fix_xclip_web(self, platform: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(web_test_file):
            return {"status": "error", "reason": f"Could not find {web_test_file}"}
    ...
    def _fix_qwen2_implementation(self, hardware: str) -> Dict[str, Any]:
        ...
        if not qwen2_test_file:
            return {"status": "error", "reason": "Could not find Qwen2 test file"}
    ...
    def run_benchmarks(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Running benchmarks for {len(benchmark_models)} models on {len(available_hardware)} hardware platforms"
        )
    ...
    def generate_compatibility_matrix(self) -> Dict[str, Dict[str, str]]:
                    ...
                    for hw_type in self.hardware_platforms:
                        ...
                        for model_name, hw_results in family_data["benchmarks"].items():
                            if hw_type in hw_results:
                                ...
                                if (
                                    hw_metrics.get("status") == "success"
                                    or hw_metrics.get("status") == "completed"
                                ):
                                    success = True
                                    break
        ...
        for model_key, hw_issues in issues.items():
            ...
            for hw, issue in hw_issues.items():
                ...
                elif issue["status"] == "warning" and matrix[family][hw] not in [
                    "incompatible",
                    "not_tested",
                ]:
        ...
        # Save to results
        self.results["compatibility_matrix"] = {"raw": matrix, "display": display_matrix}
        ...
        with open(report_file, "w") as f:
            ...
            if simulated_hw:
                ...
                for hw in simulated_hw:
                    f.write(
                        f"- **{hw}**: SIMULATED (results should not be used for production decisions)\n"
                    )
            ...
            for key, model_info in self.models.items():
                f.write(
                    f"| {key} | {model_info['name']} | {model_info['family']} | {model_info['modality']} |\n"
                )
            ...
            if "compatibility_matrix" in self.results:
                ...
                if simulated_hw:
                    f.write(
                        "**⚠️ WARNING:** Results for simulated hardware platforms should not be used for production decisions or performance comparisons with real hardware.\n\n"
                    )
            else:
                ...
                if issues:
                    ...
                    for model_key, hw_issues in issues.items():
                        for hw, issue in hw_issues.items():
                            ...
                            is_simulated = False
                            if (
                                hasattr(self, "_simulated_hardware")
                                and hw in self._simulated_hardware
                            ):
                                is_simulated = self._simulated_hardware[hw]
                            ...
                            f.write(
                                f"| {model_key} | {hw} | {issue['issue']} | {fix_status} | {sim_status} |\n"
                            )
            ...
            if (
                "benchmark_results" in self.results
                and "performance_benchmarks" in self.results["benchmark_results"]
            ):
                ...
                if simulated_hw:
                    f.write(
                        "**⚠️ WARNING:** The following performance results include SIMULATED hardware platforms. "
                    )
                    f.write(
                        "Values for simulated hardware do not reflect actual performance and should not be used for production decisions.\n\n"
                    )
            ...
            if simulated_hw:
                f.write(
                    "**⚠️ WARNING:** The following recommendations are partially based on SIMULATED hardware data. "
                )
                f.write(
                    "Recommendations for simulated hardware platforms should be validated with real hardware before implementation.\n\n"
                )

                f.write("Simulated hardware platforms in this report:\n")
                for hw in simulated_hw:
                    f.write(
                        f"- **{hw}**: Performance is SIMULATED, not actual hardware measurements\n"
                    )
                f.write("\n")
    ...
    def _generate_recommendations(self) -> Dict[str, List[str]]:
        ...
        if "compatibility_matrix" in self.results:
            ...
            for family, hw_compatibility in matrix.items():
                ...
                if best_hw:
                    hw_str = ", ".join(best_hw)
                    recommendations["Hardware Platform Selection"].append(
                        f"Use {hw_str} for {family} models for best performance"
                    )
        ...
        for key, model_info in self.models.items():
            ...
            if self.use_small_models and key in SMALL_VERSIONS:
                model_recs.add(
                    f"Use {name} as an efficient alternative for {family} tasks in resource-constrained environments"
                )
            ...
            for model_key, hw_issues in issues.items():
                for hw, issue in hw_issues.items():
                    if issue["status"] == "warning" and issue.get("fixable", False):
                        recommendations["Implementation Improvements"].append(
                            f"Complete the implementation of {model_key} on {hw}: {issue['issue']}"
                        )

        # Ongoing monitoring recommendations
        recommendations["Ongoing Monitoring"].append(
    ...
    def run_all(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Complete key model benchmarking workflow completed. Report saved to {report_path}"
        )
    ...
    try:
        ...
        if hasattr(sys.modules[__name__], "_simulated_hardware"):
            ...
            if hasattr(module, "_simulated_hardware") and hw_type in module._simulated_hardware:
                ...
                if is_simulated:
                    simulation_reason = (
                        f"Hardware '{hw_type}' is being simulated (not real hardware)"
                    )
        ...
        hw_metadata = {"source": "benchmark", "is_simulated": is_simulated}
        if simulation_reason:
            hw_metadata["simulation_reason"] = simulation_reason

        hw_id = get_or_create_hardware_platform(hardware_type=hw_type, metadata=hw_metadata)
        ...
        if is_simulated:
            logger.warning(
                f"Stored SIMULATED benchmark result in database for {result.get('model_name', 'unknown')} on {hw_type}"
            )
            logger.warning(f"Simulation reason: {simulation_reason}")
        else:
            logger.info(
                f"Stored benchmark result in database for {result.get('model_name', 'unknown')} on {hw_type}"
            )
...
def main():
    ...
    parser.add_argument(
        "--custom-models",
        type=str,
        help="JSON file with custom models configuration (required if models-set=custom)",
    )
    parser.add_argument(
        "--hardware",
        type=str,
    ...
    parser.add_argument(
        "--batch-sizes", type=int, nargs="+", default=[1, 2, 4, 8, 16], help="Batch sizes to test"
    )
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="Only verify functionality without performance benchmarks",
    )
    parser.add_argument(
        "--benchmark-only",
        action="store_true",
        help="Only run performance benchmarks without verification",
    )
    parser.add_argument("--no-plots", action="store_true", help="Disable plot generation")
    parser.add_argument(
        "--no-compatibility-update", action="store_true", help="Disable compatibility matrix update"
    )
    parser.add_argument(
        "--no-resource-pool", action="store_true", help="Disable ResourcePool for model caching"
    )
    parser.add_argument(
        "--specific-models",
        type=str,
        nargs="+",
        help="Only benchmark specific models (by key) from the selected set",
    )
    ...
    parser.add_argument(
        "--no-db-store", action="store_true", help="Disable storing results in the database"
    )
    parser.add_argument(
        "--visualize-from-db",
        action="store_true",
        help="Generate visualizations from database instead of current run results",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Specific models to benchmark (alternative to --specific-models)",
    )
    ...
    try:
        ...
        logger.info(
            f"Hardware verification results: {len([h for h, v in hardware_status.items() if v])} platforms available"
        )
    ...
    try:
        ...
        elif os.environ.get("BENCHMARK_DB_PATH") and os.path.exists(
            os.environ.get("BENCHMARK_DB_PATH")
        ):
        ...
        logger.warning(
            "benchmark_db_tools module not available, skipping benchmark result verification"
        )
