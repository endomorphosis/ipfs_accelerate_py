...
def verify_hardware_availability() -> Dict[str, bool]:
    ...
    try:
        ...
        hardware_status = {
            ...
            OPENVINO: results["hardware"].get(OPENVINO, False)
            and real_hardware.get(OPENVINO, False),
            QUALCOMM: results["hardware"].get(QUALCOMM, False)
            and real_hardware.get(QUALCOMM, False),
        ...
        logger.error(
            "Could not import hardware_detection module. Make sure the fixes have been applied."
        )
        return {
            "cpu": True,
            "cuda": False,
            ...
            if "is_simulated" in simulation_columns:
                ...
                if null_count > 0:
                    results["errors"].append(
                        f"{null_count} benchmark entries have NULL simulation status"
                    )
...
def verify_benchmark_reports() -> Dict[str, bool]:
    ...
    Returns:
        Dictionary with verification results
    """
    results = {"simulation_warnings_in_reports": False, "simulation_indicators_in_matrix": False}
    ...
    try:
        ...
        results["simulation_warnings_in_reports"] = (
            "SIMULATED" in report_content or "⚠️" in report_content
        )

        # Check for simulation indicators in compatibility matrix
        results["simulation_indicators_in_matrix"] = (
            "⚠️" in report_content and "hardware platforms are being simulated" in report_content
        )
...
def verify_json_outputs() -> Dict[str, bool]:
    ...
    Returns:
        Dictionary with verification results
    """
    results = {"simulation_flags_in_json": False}
    ...
    try:
        ...
        if isinstance(json_data, dict):
            results["simulation_flags_in_json"] = (
                "is_simulated" in json_data
                or "simulation_mode" in json_data
                or "simulated" in json_data
            )
...
def main():
    ...
    parser.add_argument(
        "--hardware-only", action="store_true", help="Only verify hardware detection"
    )
    parser.add_argument("--schema-only", action="store_true", help="Only verify database schema")
    parser.add_argument("--reports-only", action="store_true", help="Only verify benchmark reports")
    parser.add_argument("--json-only", action="store_true", help="Only verify JSON outputs")
    parser.add_argument(
        "--apply-fixes", action="store_true", help="Apply fixes to database schema if needed"
    )
    ...
    if run_all or args.schema_only:
        ...
        if os.path.exists(db_path):
            ...
            print(f"\nDatabase: {db_path}")
            print(
                f"  Schema includes simulation flags: {'✅ YES' if schema_results['database_schema_check'] else '❌ NO'}"
            )
                    ...
                    try:
                        ...
                        if columns_to_add:
                            for column_def in columns_to_add:
                                try:
                                    conn.execute(
                                        f"ALTER TABLE performance_results ADD COLUMN {column_def}"
                                    )
    ...
    if run_all or args.reports_only:
        ...
        print("\nReport Checks:")
        print(
            f"  Simulation warnings in reports: {'✅ YES' if report_results['simulation_warnings_in_reports'] else '❌ NO'}"
        )
        print(
            f"  Simulation indicators in matrix: {'✅ YES' if report_results['simulation_indicators_in_matrix'] else '❌ NO'}"
        )

        reports_check_passed = (
            report_results["simulation_warnings_in_reports"]
            and report_results["simulation_indicators_in_matrix"]
        )
    ...
    if run_all or args.json_only:
        ...
        print("\nJSON Checks:")
        print(
            f"  Simulation flags in JSON files: {'✅ YES' if json_results['simulation_flags_in_json'] else '❌ NO'}"
        )
        ...
        if json_check_passed:
            ...
            print(
                "    Note: This is expected if JSON output is deprecated in favor of database storage"
            )
