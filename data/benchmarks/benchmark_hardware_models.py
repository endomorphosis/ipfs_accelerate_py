...
def store_benchmark_in_database(result, db_path=None):
    ...
    try:
        ...
        hw_id = get_or_create_hardware_platform(
            hardware_type=result.get("hardware", "unknown"), metadata={"source": "benchmark"}
        )
        ...
        logger.info(
            f"Stored benchmark result in database for {result.get('model_name', 'unknown')}"
        )
        return True
    ...
    for batch_size in batch_sizes:
        ...
        try:
            ...
            try:
                ...
                if stdout and (stdout.startswith("{") or stdout.startswith("[")):
                    ...
                    results["batch_sizes"][batch_size] = {
                        ...
                        if len(stdout) > 1000
                        else stdout,  # Truncate long output
                        "timestamp": datetime.now().isoformat(),
                    }
                else:
                    ...
                    results["batch_sizes"][batch_size] = {
                        ...
                        "raw_stderr": process.stderr[:1000]
                    ...
                    throughput_match = re.search(
                        r"throughput[:\s]+(\d+\.?\d*)", stdout, re.IGNORECASE
                    )
                    latency_match = re.search(r"latency[:\s]+(\d+\.?\d*)", stdout, re.IGNORECASE)
                    memory_match = re.search(r"memory[:\s]+(\d+\.?\d*)", stdout, re.IGNORECASE)

                    if throughput_match:
                        results["batch_sizes"][batch_size]["throughput"] = float(
                            throughput_match.group(1)
                        )
                    if latency_match:
                        results["batch_sizes"][batch_size]["latency_ms"] = float(
                            latency_match.group(1)
                        )
                    if memory_match:
                        results["batch_sizes"][batch_size]["memory_usage_mb"] = float(
                ...
                results["batch_sizes"][batch_size] = {
                    ...
                    "execution_time_sec": execution_time,
                    "command": command,
                    "raw_output": process.stdout[:1000]
                    if len(process.stdout) > 1000
                    else process.stdout,  # Truncate long output
                    "raw_stderr": process.stderr[:1000]
                    if len(process.stderr) > 1000
                    else process.stderr,
                    "timestamp": datetime.now().isoformat(),
            ...
            results["batch_sizes"][batch_size] = {
                ...
                "error_output": e.stderr[:1000]
                if len(e.stderr) > 1000
                else e.stderr,  # Truncate long error output
                "command": command,
                "timestamp": datetime.now().isoformat(),
            }
...
def generate_markdown_report(results_list: List[Dict], output_file: str = None) -> str:
    ...
    report = [f"# Hardware Benchmark Report ({timestamp})", "", "## Summary", ""]
    ...
    for category, category_results in categories.items():
        ...
        for result in category_results:
            ...
            for batch_size, batch_results in result.get("batch_sizes", {}).items():
                ...
                report.append(
                    f"| {model} | {hardware} | {batch_size} | {throughput} | {latency} | {memory} |"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark HuggingFace models across hardware platforms"
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--all", action="store_true", help="Benchmark all compatible model-hardware combinations"
    )
    group.add_argument(
        "--model", help="Benchmark a specific model across all compatible hardware platforms"
    )
    group.add_argument(
        "--hardware", help="Benchmark all compatible models on a specific hardware platform"
    )
    group.add_argument(
        "--category", help="Benchmark all models in a category across compatible hardware"
    )
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run a faster, less comprehensive benchmark"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Generate comparison report from benchmark results"
    )
    parser.add_argument(
        "--no-report",
        action="store_false",
        dest="generate_report",
        help="Don't generate a markdown report",
    )
    ...
    if args.all:
            ...
            for hw_key in HARDWARE_PLATFORMS:
                if model_key in HARDWARE_PLATFORMS[hw_key]["compatibility"]:
                    result = run_benchmark(
                        model_key, hw_key, batch_sizes, quick_mode=args.quick, verbose=args.verbose
                    )
        ...
        for hw_key in HARDWARE_PLATFORMS:
            if args.model in HARDWARE_PLATFORMS[hw_key]["compatibility"]:
                result = run_benchmark(
                    args.model, hw_key, batch_sizes, quick_mode=args.quick, verbose=args.verbose
                )
                results_list.append(result)
                if result.get("batch_sizes"):
                    save_benchmark_results(result, args.output_dir)
