...
def check_prerequisites():
    ...
    if not os.path.exists("hardware_benchmark_runner.py"):
        logger.error(
            "hardware_benchmark_runner.py not found. Please ensure it exists in the current directory."
        )
        return False
    ...
    try:
        ...
        logger.warning(
            "Plotting libraries (matplotlib, pandas) not available. Plotting will be disabled."
        )
...
def plot_hardware_comparison(results, plots_dir):
        ...
        for family, models in result["benchmarks"].items():
            for model_name, hw_results in models.items():
                for hw_type, hw_metrics in hw_results.items():
                    if (
                        hw_metrics.get("status") != "completed"
                        or "performance_summary" not in hw_metrics
                    ):
                        continue
    ...
    df_pivot = df.pivot_table(
        index="family", columns="hardware", values="throughput", aggfunc="mean"
    )
    ax = df_pivot.plot(kind="bar", rot=0)
    ...
    if "cpu" in df["hardware"].unique():
        ...
        df_cpu.rename(
            columns={"latency": "cpu_latency", "throughput": "cpu_throughput"}, inplace=True
        )
        ...
        # Plot speedups
        plt.figure(figsize=(12, 8))
        df_pivot = df_speedup.pivot_table(
            index="family", columns="hardware", values="latency_speedup", aggfunc="mean"
        )
        ax = df_pivot.plot(kind="bar", rot=0)
                    ...
                    model_data.append(
                        {
                            ...
                            "latency": perf["latency"]["mean"] * 1000
                            if "latency" in perf and "mean" in perf["latency"]
                            else 0,  # ms
                            "throughput": perf["throughput"]["mean"]
                            if "throughput" in perf and "mean" in perf["throughput"]
...
def plot_batch_size_scaling(results, plots_dir):
        ...
        for family, models in result["benchmarks"].items():
            for model_name, hw_results in models.items():
                for hw_type, hw_metrics in hw_results.items():
                    if (
                        hw_metrics.get("status") != "completed"
                        or "benchmark_results" not in hw_metrics
                    ):
                        continue
                                    ...
                                    batch_data.append(
                                        {
                                            ...
                                            "latency": config_results.get("avg_latency", 0)
                                            * 1000,  # ms
    ...
    if "cuda" in df["hardware"].unique():
        ...
        for family in df_grouped["family"].unique():
            family_data = df_grouped[df_grouped["family"] == family]
            plt.plot(
                family_data["batch_size"],
                family_data["throughput_efficiency"],
                marker="o",
                label=family,
            )
    ...
    for result in results:
        ...
        for family, models in result["benchmarks"].items():
            if family not in compatibility_matrix["model_families"]:
                compatibility_matrix["model_families"][family] = {"hardware_compatibility": {}}

            # Initialize hardware compatibility for this family
            for hw_type in hardware_types:
                if (
                    hw_type
                    not in compatibility_matrix["model_families"][family]["hardware_compatibility"]
                ):
                    compatibility_matrix["model_families"][family]["hardware_compatibility"][
                        hw_type
                    ] = {"compatible": False, "performance_rating": None, "benchmark_results": []}
            ...
            for model_name, hw_results in models.items():
                for hw_type, hw_metrics in hw_results.items():
                    ...
                    if hw_metrics.get("status") == "completed":
                        ...
                        compatibility_matrix["model_families"][family]["hardware_compatibility"][
                            hw_type
                        ]["benchmark_results"].append(benchmark_summary)
                        ...
                        if "error" in hw_metrics:
                            ...
                            ][hw_type]["error"] = hw_metrics["error"]
    ...
    for family, family_data in compatibility_matrix["model_families"].items():
        for hw_type, hw_data in family_data["hardware_compatibility"].items():
            ...
            if benchmark_results:
                ...
                throughputs = [
                    res.get("mean_throughput", 0)
                    for res in benchmark_results
                    if "mean_throughput" in res
                ]
                if throughputs:
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check prerequisites without running benchmarks"
    )
    parser.add_argument(
        "--plot-only", type=str, help="Only generate plots from existing results directory"
    )
    parser.add_argument(
        "--update-matrix-only",
        type=str,
        help="Only update compatibility matrix from existing results directory",
    )
    ...
    if not check_prerequisites():
        logger.error(
            "Prerequisite check failed. Please address the issues before running benchmarks."
        )
        return
