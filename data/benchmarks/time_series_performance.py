...
class TimeSeriesPerformance:
        ...
        if notification_config is None:
            notification_config = {
                ...
                "targets": {
                    "email": [],
                    "slack": "",
                    "github": {"repo": "", "token": "", "labels": ["regression", "performance"]},
                    "webhook": "",
                },
            }
    ...
    def _ensure_schema(self):
        ...
        try:
            ...
            schema_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "db_schema", "time_series_schema.sql"
            )
    ...
    def set_baseline(
        ...
        try:
            ...
            logger.info(
                f"Set baseline with ID {baseline_id} for model_id={model_id}, hardware_id={hardware_id}"
            )
    ...
    def set_all_baselines(
        self, days_lookback: int = 7, min_samples: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for combo in combinations:
                ...
                try:
                    ...
                    logger.error(
                        f"Error setting baseline for model_id={model_id}, hardware_id={hardware_id}: {e}"
                    )
                    baseline_results.append(
                        {
                            "model_id": model_id,
            ...
            logger.info(
                f"Set {len([r for r in baseline_results if r['status'] == 'success'])} baselines"
            )
        ...
        try:
            for reg in regressions:
                ...
                if result:
                    ...
                    logger.warning(
                        f"Could not find performance_id and baseline_id for regression {reg['regression_id']}"
                    )
        except Exception as e:
    ...
    def _send_slack_notification(self, regressions: List[Dict[str, Any]]):
        ...
        logger.info(
            f"Would send Slack message to {webhook_url} about {len(regressions)} regressions"
        )
    ...
    def _send_webhook_notification(self, regressions: List[Dict[str, Any]]):
        ...
        logger.info(
            f"Would send webhook request to {webhook_url} about {len(regressions)} regressions"
        )
    ...
    def _analyze_single_trend(
        ...
        try:
            ...
            trend_result = {
                ...
                "trend_start_date": timestamps[0].isoformat()
                if isinstance(timestamps[0], datetime)
                else timestamps[0],
                "trend_end_date": timestamps[-1].isoformat()
            ...
            logger.error(
                f"Error analyzing trend for model_id={model_id}, hardware_id={hardware_id}: {e}"
            )
            return None
            ...
            for trend in trend_results:
                ...
                """,
                    (
                        ...
                        trend["hardware_id"],
                        metric,
                        trend["trend_start_date"],
                        trend["trend_end_date"],
    ...
    def _generate_markdown_report(
        ...
        for trend in trends:
            trend_counts[trend["trend_direction"]] = (
                trend_counts.get(trend["trend_direction"], 0) + 1
            )
        ...
        if regressions:
            ...
            for reg in sorted(regressions, key=lambda x: abs(x["severity"]), reverse=True):
                ...
                date_str = (
                    reg["test_date"].strftime("%Y-%m-%d")
                    if isinstance(reg["test_date"], datetime)
                    else reg["test_date"]
                )

                lines.append(
                    f"| {reg['model_name']} | {reg['hardware_type']} | {reg['regression_type']} | {severity_str} | {date_str} |"
                )
        else:
        ...
        if trends:
            ...
            for trend in sorted(trends, key=lambda x: abs(x["trend_magnitude"]), reverse=True):
                ...
                lines.append(
                    f"| {trend['model_name']} | {trend['hardware_type']} | {trend['metric']} | {direction_emoji} {trend['trend_direction']} | {magnitude_str} | {confidence_str} |"
                )
        else:
    ...
    def _generate_html_report(
        ...
        if regression_count > 0:
            html.append(
                f"<p>⚠️ <strong>{regression_count} performance regressions detected</strong></p>"
            )
        else:
        ...
        for metric, path in visualization_paths.items():
            if path and os.path.exists(path):
                ...
                html.append(
                    f"<img class='visualization' src='{path}' alt='{metric.capitalize()} Trend'>"
                )
        ...
        if regressions:
            html.append("<table>")
            html.append(
                "<tr><th>Model</th><th>Hardware</th><th>Metric</th><th>Severity</th><th>Date</th></tr>"
            )
        ...
        if trends:
            html.append("<table>")
            html.append(
                "<tr><th>Model</th><th>Hardware</th><th>Metric</th><th>Direction</th><th>Magnitude</th><th>Confidence</th></tr>"
            )
            ...
            for trend in sorted(trends, key=lambda x: abs(x["trend_magnitude"]), reverse=True):
                ...
                html.append(
                    f"<td class='{trend['trend_direction']}'>{direction_emoji} {trend['trend_direction']}</td>"
                )
...
if __name__ == "__main__":
    ...
    record_parser.add_argument(
        "--throughput", type=float, required=True, help="Throughput in items/sec"
    )
    ...
    record_parser.add_argument(
        "--power", type=float, required=True, help="Power consumption in watts"
    )
    ...
    baseline_parser.add_argument(
        "--all", action="store_true", help="Set baselines for all configurations"
    )
    ...
    regression_parser.add_argument(
        "--throughput-threshold", type=float, help="Throughput regression threshold"
    )
    regression_parser.add_argument(
        "--latency-threshold", type=float, help="Latency regression threshold"
    )
    regression_parser.add_argument(
        "--memory-threshold", type=float, help="Memory regression threshold"
    )
    regression_parser.add_argument(
        "--power-threshold", type=float, help="Power regression threshold"
    )
    ...
    trend_parser.add_argument(
        ...
        type=str,
        default="throughput",
        choices=["throughput", "latency", "memory", "power"],
    ...
    report_parser.add_argument(
        "--format", type=str, default="markdown", choices=["markdown", "html"], help="Report format"
    )
    ...
    if args.command == "record":
        ...
        if args.all:
            results = ts_perf.set_all_baselines(
                days_lookback=args.days, min_samples=args.min_samples
            )
            print(f"Set {len([r for r in results if r['status'] == 'success'])} baselines")
            for result in results:
                if result["status"] == "success":
                    print(
                        f"Baseline ID {result['baseline_id']}: {result['model_name']} on {result['hardware_type']}"
                    )
                else:
            ...
            if not all([args.model_id, args.hardware_id, args.batch_size, args.precision]):
                print(
                    "Error: When not using --all, you must specify --model-id, --hardware-id, --batch-size, and --precision"
                )
                exit(1)
        ...
        regressions = ts_perf.detect_regressions(
            model_id=args.model_id, hardware_id=args.hardware_id, days_lookback=args.days
        )
        ...
        if regressions:
            ...
            for reg in regressions:
                print(
                    f"  {reg['model_name']} on {reg['hardware_type']}: {reg['regression_type']} degraded by {reg['severity']:.2f}%"
                )
        else:
        ...
        if trends:
            ...
            for trend in trends:
                print(
                    f"  {trend['model_name']} on {trend['hardware_type']}: {trend['metric']} {trend['trend_direction']} by {trend['trend_magnitude']:.2f}% (confidence: {trend['trend_confidence']:.2f})"
                )
        else:
