...
def test_hardware_detection():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if (
                "webnn_simulation" in capabilities
                and "webgpu_simulation" in capabilities
                and "qualcomm_simulation" in capabilities
            ):
...
def test_database_integration():
    ...
    with tempfile.NamedTemporaryFile(suffix=".duckdb") as temp_db:
        ...
        try:
            ...
            logger.warning(
                "update_db_schema_for_simulation.py not available, skipping schema update"
            )
            ...
            try:
                ...
                if "performance_results" in table_names:
                    ...
                    columns = conn.execute(
                        "DESCRIBE SELECT * FROM performance_results LIMIT 0"
                    ).fetchall()
                    ...
                    logger.warning(
                        f"⚠️ performance_results table not found. Available tables: {', '.join(table_names)}"
                    )
...
def main():
    ...
    parser.add_argument("--hardware-only", action="store_true", help="Only test hardware detection")
    parser.add_argument("--report-only", action="store_true", help="Only test report generation")
    parser.add_argument(
        "--database-only", action="store_true", help="Only test database integration"
    )
