...
try:
    ...
    logger.error(
        "Make sure that the hardware_detection_updates.py and benchmark_error_handling.py files are in the current directory"
    )
    sys.exit(1)
...
def detect_hardware():
    ...
    try:
        ...
        available_hardware = [
            hw for hw, available in hardware_info["hardware"].items() if available
        ]
...
def get_or_create_hardware(conn, hardware_type):
    ...
    try:
        ...
        conn.execute("INSERT INTO hardware_platforms (hardware_type) VALUES (?)", [hardware_type])
...
def store_test_result(conn, test_result, model_id, hardware_id):
    ...
    try:
        ...
        conn.commit()
        logger.info(
            f"Stored test result for model_id={model_id}, hardware_id={hardware_id}, success={test_result.get('success', False)}"
        )
...
def simulate_benchmark(
    model, hardware_type, is_simulated=False, simulation_reason=None, should_fail=False
):
    """
    ...
    if should_fail:
        try:
            ...
            return handle_benchmark_exception(
                e, hardware_type, model_name, is_simulated, simulation_reason
            )
        ...
        performance_metrics.update(
            {
                ...
                or f"Hardware {hardware_type} is being simulated",
            }
        )
    ...
    result = {
        ...
        "details": {"batch_size": 1, "sequence_length": 128},
...
def run_benchmarks(models, hardware_types, conn):
    ...
    for model in models:
        ...
        for hardware_type in hardware_types:
            ...
            if is_simulated:
                simulation_reason = (
                    f"Hardware {hardware_type} is being simulated based on environment settings"
                )
                logger.warning(
                    f"Hardware {hardware_type} is simulated, results will be marked accordingly"
                )
            ...
            result = simulate_benchmark(
                ...
                simulation_reason=simulation_reason,
                should_fail=(
                    model["name"] == "bert-base-uncased" and hardware_type == "cuda"
                ),  # Test error handling
            )
            ...
            if test_id:
                ...
                if "performance_metrics" in result and result.get("success", False):
                    performance_id = store_performance_result(
                        conn, result["performance_metrics"], test_id, model_id, hardware_id
                    )
                    if performance_id:
...
def main():
    ...
    logger.info(
        f"Running benchmarks for {len(models)} models on {len(args.hardware)} hardware types"
    )
    ...
    print("\nBenchmark summary:")
    print(
        f"- Ran {len(results)} benchmarks for {len(models)} models on {len(args.hardware)} hardware types"
    )
    ...
    for hardware_type, count in hardware_counts.items():
        successful_count = len(
            [
                r
                for r in results
                if r.get("hardware_type") == hardware_type and r.get("success", False)
            ]
        )
        simulated_count = len(
            [
                r
                for r in results
                if r.get("hardware_type") == hardware_type and r.get("is_simulated", False)
            ]
        )

        if simulated_count > 0:
            print(
                f"- {hardware_type}: {count} tests, {successful_count} successful, {simulated_count} SIMULATED"
            )
        else:
