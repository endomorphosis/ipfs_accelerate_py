...
def scan_for_problematic_reports(directory, check_json=False):
    ...
    for file_path in md_files:
        ...
        if (
            any(re.search(pattern, content) for pattern in simulation_patterns)
            and "SIMULATION WARNING" not in content
        ):
            if "simulated" in content.lower() and not re.search(
                r"marked as simulated|explicitly simulated|simulation status", content.lower()
            ):
                problematic_files.append(
                    (file_path, "markdown", "Contains simulation results without proper warnings")
                )
    ...
    for file_path in html_files:
        ...
        if (
            ...
            if "simulated" in content.lower() and not re.search(
                ...
                problematic_files.append(
                    (file_path, "html", "Contains simulation results without proper warnings")
                )
    ...
    for file_path in json_files:
        try:
            ...
            if isinstance(data, dict) and any(
                k in data for k in ["results", "benchmarks", "performance", "hardware"]
            ):
                if not any(
                    k in data for k in ["is_simulated", "simulation_status", "simulation_flags"]
                ):
                    problematic_files.append(
                        (file_path, "json", "Contains benchmark data without simulation flags")
                    )
...
def scan_for_simulation_patterns_in_code(directory):
    ...
    simulation_code_patterns = [
        (
            r"def\s+simulate_\w+\s*\([^)]*\)\s*:",
            "Contains simulation method without explicit status tracking",
        ),
...
def fix_report_py_files(directory):
    ...
    for file_path in py_files:
        ...
        if any(re.search(pattern, content) for pattern in report_py_patterns):
            ...
            if (
                "_validate_data_authenticity" not in content
                and "check_simulation_status" not in content
            ):
                # Backup the file
...
def main():
    parser = argparse.ArgumentParser(
        description="Cleanup stale benchmark reports and mark them with warnings."
    )
    parser.add_argument(
        "--scan", action="store_true", help="Scan for problematic reports without modifying them"
    )
    ...
    parser.add_argument(
        "--archive-dir",
        default="/home/barberb/ipfs_accelerate_py/test/archived_stale_reports",
        help="Directory to archive problematic files",
    )
    parser.add_argument(
        "--directory", default="/home/barberb/ipfs_accelerate_py/test", help="Directory to scan"
    )
    parser.add_argument("--check-json", action="store_true", help="Check JSON files as well")
    parser.add_argument(
        "--check-code",
        action="store_true",
        help="Check Python code for outdated simulation methods",
    )
    parser.add_argument(
        "--fix-report-py",
        action="store_true",
        help="Fix report generator Python files to include validation",
    )

    args = parser.parse_args()

    if args.scan or (
        not args.mark and not args.archive and not args.check_code and not args.fix_report_py
    ):
    ...
    if args.check_code:
        ...
        if problematic_code_files:
            print(
                f"Found {len(problematic_code_files)} Python files with outdated simulation methods:"
            )
