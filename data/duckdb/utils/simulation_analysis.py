...
try:
    ...
    logger.warning(
        "Visualization dependencies not available. Install pandas and matplotlib for visualization."
    )
...
def get_simulation_statistics(conn) -> Dict[str, Any]:
    ...
    try:
        ...
        if not has_simulation_columns:
            logger.warning(
                "Database does not have simulation tracking columns. Run apply_simulation_detection_fixes.py first."
            )
            return {"error": "Database schema missing simulation columns"}
        ...
        if not overall_stats:
            return {"error": "No test results found in database"}
        ...
        stats = {
            ...
            "simulation_reasons": [
                {"reason": row[0], "count": row[1]} for row in simulation_reasons
            ],
            "error_categories": [{"category": row[0], "count": row[1]} for row in error_categories],
        }
...
def analyze_simulation_impact(stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Compile analysis
    analysis = {"summary": "", "impact": [], "recommendations": []}
    ...
    if overall["simulated_results"] == 0:
        ...
        analysis["summary"] = (
            f"{simulated_percent}% of benchmark results are from simulated hardware. "
        )

        if simulated_percent > 50:
            analysis["summary"] += (
                "Most results are simulated, which may not reflect real-world performance."
            )
        elif simulated_percent > 20:
            analysis["summary"] += (
                "A significant portion of results are simulated, which may affect overall analysis."
            )
        else:
            analysis["summary"] += (
                "Only a small portion of results are simulated, with minimal impact on overall analysis."
            )
    ...
    for hw in stats["hardware"]:
        if hw["simulated_results"] > 0:
            ...
            impact = {"hardware_type": hw_name, "simulated_percent": sim_percent, "description": ""}

            if sim_percent == 100:
                impact["description"] = (
                    f"All {hw_name} results are simulated, not reflecting actual hardware performance."
                )
                analysis["recommendations"].append(
                    f"Acquire real {hw_name} hardware for accurate benchmarks."
                )
            elif sim_percent > 80:
                impact["description"] = (
                    f"Most {hw_name} results ({sim_percent}%) are simulated, severely limiting accuracy."
                )
                analysis["recommendations"].append(
                    f"Prioritize obtaining real {hw_name} hardware for testing."
                )
            elif sim_percent > 50:
                impact["description"] = (
                    f"Majority of {hw_name} results ({sim_percent}%) are simulated, reducing reliability."
                )
                analysis["recommendations"].append(
                    f"Consider obtaining real {hw_name} hardware for more accurate results."
                )
            else:
                impact["description"] = (
                    f"Some {hw_name} results ({sim_percent}%) are simulated but real hardware data is available."
                )
                analysis["recommendations"].append(
                    f"Flag simulated {hw_name} results in reports or exclude from key analyses."
                )
    ...
    for perf in stats["performance"]:
        ...
        if sim_throughput is not None and real_throughput is not None:
            throughput_diff_percent = abs(
                (sim_throughput - real_throughput) / real_throughput * 100
            )
    ...
    if overall["simulated_results"] > 0:
        ...
        analysis["recommendations"].append(
            "Consider separating simulated and real hardware results in analyses."
        )

        if overall["simulated_percent"] > 50:
            analysis["recommendations"].append(
                "Prioritize obtaining real hardware for more accurate benchmarks."
            )
            analysis["recommendations"].append(
                "Use caution when making decisions based on current benchmark data."
            )
...
def generate_simulation_report(
    stats: Dict[str, Any], analysis: Dict[str, Any], format: str = "text"
) -> str:
    ...
    if format == "json":
        ...
        lines.append(
            f"- **Simulated results:** {overall['simulated_results']} ({overall['simulated_percent']}%)"
        )
        lines.append(
            f"- **Real hardware results:** {overall['real_results']} ({100 - overall['simulated_percent']}%)"
        )
        ...
        for hw in stats["hardware"]:
            lines.append(
                f"| {hw['hardware_type']} | {hw['total_results']} | {hw['simulated_results']} | {hw['real_results']} | {hw['unknown_results']} | {hw['simulated_percent']}% |"
            )
        lines.append("")
        ...
        if stats["performance"]:
            ...
            lines.append(
                "| Hardware Type | Simulated Latency (ms) | Real Latency (ms) | Simulated Throughput | Real Throughput |"
            )
            lines.append(
                "|--------------|------------------------|-------------------|---------------------|-----------------|"
            )
            for perf in stats["performance"]:
                ...
                lines.append(
                    f"| {perf['hardware_type']} | {sim_latency} | {real_latency} | {sim_throughput} | {real_throughput} |"
                )
            lines.append("")
        ...
        if analysis["impact"]:
            ...
            for impact in analysis["impact"]:
                if "metric" in impact:
                    lines.append(
                        f"- **{impact['hardware_type']} ({impact['metric']}):** {impact['description']}"
                    )
                else:
        ...
        if stats["performance"]:
            ...
            for perf in stats["performance"]:
                ...
                if perf["simulated_avg_throughput"] is not None:
                    lines.append(
                        f"  Simulated throughput: {perf['simulated_avg_throughput']} items/s"
                    )
...
def visualize_simulation_status(
    stats: Dict[str, Any], output_path: Optional[str] = None
) -> Optional[str]:
    ...
    axs[0, 0].pie(
        non_zero_sizes, labels=non_zero_labels, autopct="%1.1f%%", shadow=True, startangle=90
    )
    ...
    if not hardware_df.empty:
        ...
        p2 = axs[0, 1].bar(
            x,
            hardware_df["simulated_results"],
            bottom=hardware_df["real_results"],
            label="Simulated",
            color="orange",
            alpha=0.7,
        )
    ...
    if stats["performance"]:
        ...
        if not perf_df.empty:
            ...
            # Plot bars
            axs[1, 0].bar(
                x - width / 2, sim_latency, width, label="Simulated", color="orange", alpha=0.7
            )
            axs[1, 0].bar(
                x + width / 2, real_latency, width, label="Real", color="green", alpha=0.7
            )
            ...
            axs[1, 0].text(
                0.5,
                0.5,
                "No comparative performance data available",
                horizontalalignment="center",
                verticalalignment="center",
            )
    else:
    ...
    if stats["simulation_reasons"]:
        ...
        reason_df["reason"] = reason_df["reason"].apply(
        ...
        axs[1, 1].text(
            0.5,
            0.5,
            "No simulation reason data available",
            horizontalalignment="center",
            verticalalignment="center",
        )
...
def main():
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualization of simulation status"
    )
