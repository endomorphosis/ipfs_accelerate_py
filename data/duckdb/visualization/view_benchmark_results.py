...
def generate_markdown_report(performance_df, hardware_df, output_path):
    ...
    with open(output_path, "w") as f:
        ...
        for _, row in hardware_df.iterrows():
            ...
            f.write(
                f"| {row['hardware_id']} | {row['hardware_type']} | {row['description'] or 'N/A'} | {simulated} | {reason} |\n"
            )
        ...
        for _, row in performance_df.head(20).iterrows():
            simulated = "✅ Yes" if row["is_simulated"] else "❌ No"
            timestamp = (
                row["test_timestamp"].strftime("%Y-%m-%d %H:%M:%S")
                if not pd.isna(row["test_timestamp"])
                else "N/A"
            )

            f.write(
                f"| {row['model_name']} | {row['model_family']} | {row['hardware_type']} | {row['batch_size']} | "
            )
            f.write(
                f"{row['average_latency_ms']:.2f} | {row['throughput_items_per_second']:.2f} | "
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="benchmark_summary.md", help="Output file for the report"
    )
    parser.add_argument(
        "--format",
    ...
    if args.check_simulation:
        ...
        print(
            "UPDATE performance_results SET is_simulated = TRUE, simulation_reason = 'Manual test' WHERE id = X;"
        )
        print(
            "UPDATE hardware_platforms SET is_simulated = TRUE, simulation_reason = 'Hardware not available' WHERE hardware_type = 'X';"
        )
