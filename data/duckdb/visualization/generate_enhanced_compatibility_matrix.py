...
DEFAULT_HARDWARE_PLATFORMS = ["CUDA", "ROCm", "MPS", "OpenVINO", "Qualcomm", "WebNN", "WebGPU"]

# Default modalities
DEFAULT_MODALITIES = ["text", "vision", "audio", "multimodal"]


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Generate comprehensive model compatibility matrix"
    )
    parser.add_argument(
        "--db-path", default="./benchmark_db.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument("--output-dir", default="./docs", help="Output directory for matrix files")
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--all-models", action="store_true", help="Include all models in the database"
    )
    parser.add_argument(
        "--recommendations", action="store_true", help="Include hardware recommendations"
    )
    ...
    try:
        ...
        if model_filter:
            where_clauses.append(
                f"(m.model_family LIKE '%{model_filter}%' OR m.model_type LIKE '%{model_filter}%' OR m.modality LIKE '%{model_filter}%')"
            )
...
def get_performance_data(conn, hardware_platforms, model_filter=None):
    ...
    try:
        ...
        return {"by_family": family_perf, "chart_data": chart_data}
            ...
            if not result.empty:
                ...
                for _, row in result.iterrows():
                    ...
                    recommendations[modality] = {
                        ...
                        "configurations": json.loads(row["recommendation_details"]).get(
                            "configurations", []
                        ),
                    }
...
def main():
    ...
    try:
        ...
        matrix_df = get_model_compatibility_data(
            conn, hardware_platforms, args.filter, args.all_models
        )
        ...
        if args.format in ["markdown", "all"]:
            generate_markdown(
                matrix_df, hardware_platforms, performance_data, recommendations, args
            )
