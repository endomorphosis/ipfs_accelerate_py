...
DEFAULT_METRICS = ["throughput_items_per_second", "average_latency_ms", "peak_memory_mb"]
...
class BenchmarkRegressionDetector:
    ...
    def _compare_with_baseline(
        self, current_data: pd.DataFrame, baseline_data: pd.DataFrame
    ) -> List[Dict[str, Any]]:
        """
        ...
        for _, current_group in current_data.groupby(["model_id", "hardware_id", "batch_size"]):
            ...
            for metric in self.metrics:
                ...
                severity = (
                    "high"
                    if percent_change < -HIGH_THRESHOLD
                    else "medium"
                    if is_regression
                    else "low"
                )
    ...
    def _generate_markdown_report(self) -> str:
        ...
        lines.append(
            "| Model | Hardware | Batch Size | Metric | Current Value | Baseline Mean | % Change | Severity |"
        )
        lines.append(
            "|-------|----------|------------|--------|---------------|---------------|----------|----------|"
        )
        ...
        for reg in self.regressions:
            lines.append(
                f"| {reg['model_name']} | {reg['hardware_type']} | {reg['batch_size']} | "
...
def main():
    ...
    parser.add_argument(
        "--metrics", nargs="+", help=f"Metrics to analyze (default: {', '.join(DEFAULT_METRICS)})"
    )
    parser.add_argument(
        "--format",
