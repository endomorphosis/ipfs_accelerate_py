...
try:
    ...
    print(
        "Error: Could not import TestResultsDBHandler. Make sure test_ipfs_accelerate.py is in the path."
    )
    sys.exit(1)
...
class JSONToDBMigrator:
    ...
    def run_migration(self, directories: List[str], delete_after: bool = False) -> Dict:
        ...
        self.stats["duration_formatted"] = str(
            datetime.fromtimestamp(duration_seconds) - datetime.fromtimestamp(0)
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "-d",
        nargs="+",
        default=["./benchmark_results", "./archived_test_results", "./api_check_results"],
        help="Directories to search for JSON files",
    )
    parser.add_argument("--db-path", help="Path to DuckDB database")
    parser.add_argument(
        "--archive-dir", default="./archived_json_files", help="Directory for archiving JSON files"
    )
    parser.add_argument(
        "--report", "-r", default="migration_report.md", help="Path to write the migration report"
    )
    parser.add_argument(
        "--delete", action="store_true", help="Delete JSON files after successful migration"
    )
