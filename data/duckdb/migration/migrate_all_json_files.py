...
def migrate_json_files(directories, db_path):
    ...
    for directory, files in directories.items():
        ...
        try:
            ...
            if "lock" in error_output.lower():
                logger.error(
                    f"Database lock error migrating files from {directory}. Will try again with a new database."
                )
    ...
    if db_is_locked and successful_directories:
        logger.warning(
            f"Data was saved to {actual_db_path} because the original database was locked."
        )
        logger.warning(
            f"You will need to merge the databases later using benchmark_db_maintenance.py."
        )
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="./benchmark_db.duckdb", help="Path to the DuckDB database file"
    )
    parser.add_argument("--archive", action="store_true", help="Archive JSON files after migration")
    parser.add_argument(
        "--archive-dir", default="./archived_json_files", help="Directory for storing the archive"
    )
    parser.add_argument(
        "--delete",
        action="store_true",
    ...
    parser.add_argument(
        "--exclude-dirs",
        nargs="+",
        default=["node_modules", ".git", "venv", "env"],
        help="Directories to exclude from migration",
    )
    parser.add_argument("--base-dir", default=".", help="Base directory to search for JSON files")
    ...
    logger.info(
        f"Successfully migrated files from {successful_dirs} out of {len(grouped_files)} directories"
    )
