...
try:
    from data.duckdb.ipfs_integration import IPFSConfig, IPFSDBMigration, load_config_from_file
...
Examples:
  ...
  python migrate_to_ipfs.py --db benchmark_db.duckdb --no-backup
    ...
    parser.add_argument("--config", help="Path to IPFS configuration file (JSON)")

    parser.add_argument(
        "--no-backup", action="store_true", help="Skip creating backups before migration"
    )

    parser.add_argument(
        "--no-verify", action="store_true", help="Skip verification after migration"
    )
