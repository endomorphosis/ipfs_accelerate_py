...
class HardwareModelPredictorDuckDBAdapter:
    ...
    def predict_optimal_hardware(
        ...
        store_recommendation: bool = True,
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if primary_hw:
                    primary_mapping = {
                        ...
                        "metadata": {**self.metadata, "recommendation_id": recommendation_id},
                    }
    ...
    def record_recommendation_feedback(
        self, recommendation_id: str, was_accepted: bool, user_feedback: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            recommendations = self.repository.get_recommendations(
                recommendation_id=recommendation_id, limit=1
            )
    ...
    def record_actual_performance(
        ...
        measurement_source: str = "benchmark",
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            prediction = None
            if prediction_id:
                predictions = self.repository.get_predictions(prediction_id=prediction_id, limit=1)
                if predictions:
            ...
            if prediction:
                ...
                for metric, actual_value, predicted_key in [
                    ...
                    if actual_value is not None and prediction.get(predicted_key) is not None:
                        ...
                        relative_error = (
                            absolute_error / abs(actual_value) if abs(actual_value) > 1e-10 else 0
                        )
...
class ModelPerformancePredictorDuckDBAdapter:
    ...
    def store_model(
        ...
        try:
            ...
            # Combine metadata
            combined_metadata = {**self.metadata, **(additional_metadata or {})}
            ...
            if hasattr(model, "feature_importances_") and features:
                self._store_feature_importances(model, model_id, features, method="native")
    ...
    def _store_feature_importances(
        self, model: Any, model_id: str, feature_names: List[str], method: str = "native"
    ) -> None:
        """
        ...
        try:
            ...
            if len(importances) != len(feature_names):
                logger.warning(
                    f"Feature importances length {len(importances)} doesn't match feature names length {len(feature_names)}"
                )
                return
    ...
    def load_model(
        ...
        try:
            # Query for the model
            filters = {"target_metric": target_metric}
    ...
    def predict(
        ...
        try:
            ...
            if not prediction_result:
                logger.warning(
                    f"No prediction result returned for {model_name} on {hardware_platform}"
                )
                return {
                    "model_name": model_name,
                    "hardware_platform": hardware_platform,
            ...
            if store_prediction and self.repository is not None:
                ...
                prediction_id = (
                    f"pred-{int(time.time())}-{hash(model_name)}-{hash(hardware_platform)}"
                )
                    ...
                    for hw in hardware_platforms:
                        ...
                        for batch_size_str, batch_data in hw_predictions.items():
                            ...
                            for precision in precision_options:
                                ...
                                if any(
                                    repo_prediction.get(k) is not None
                                    for k in ["throughput", "latency", "memory_usage"]
                                ):
                                    try:
                                        self.repository.store_prediction(repo_prediction)
