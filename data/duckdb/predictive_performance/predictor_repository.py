...
class DuckDBPredictorRepository:
    ...
    def _initialize_tables(self) -> None:
        ...
        try:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_pred_model_hw ON performance_predictions(model_name, hardware_platform)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_meas_model_hw ON performance_measurements(model_name, hardware_platform)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_mapping_model ON hardware_model_mappings(model_name)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_pred_model_id ON prediction_models(model_id)"
            )
    ...
    def store_prediction(self, prediction: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(prediction.get("timestamp"), str):
                prediction["timestamp"] = datetime.fromisoformat(
            ...
            if not prediction.get("prediction_id"):
                prediction["prediction_id"] = (
                    f"pred-{int(time.time())}-{hash(prediction.get('model_name', ''))}"
                )
    ...
    def store_measurement(self, measurement: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(measurement.get("timestamp"), str):
                measurement["timestamp"] = datetime.fromisoformat(
            ...
            if not measurement.get("measurement_id"):
                measurement["measurement_id"] = (
                    f"meas-{int(time.time())}-{hash(measurement.get('model_name', ''))}"
                )
    ...
    def store_hardware_model_mapping(self, mapping: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(mapping.get("timestamp"), str):
                mapping["timestamp"] = datetime.fromisoformat(
                    mapping["timestamp"].replace("Z", "+00:00")
                )
            ...
            if not mapping.get("mapping_id"):
                mapping["mapping_id"] = (
                    f"map-{int(time.time())}-{hash(mapping.get('model_name', ''))}-{hash(mapping.get('hardware_platform', ''))}"
                )
    ...
    def get_hardware_model_mappings(
        self,
        model_name: Optional[str] = None,
        model_family: Optional[str] = None,
        hardware_platform: Optional[str] = None,
    ...
    def store_prediction_model(self, prediction_model: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(prediction_model.get("timestamp"), str):
                prediction_model["timestamp"] = datetime.fromisoformat(
                    prediction_model["timestamp"].replace("Z", "+00:00")
                )
            ...
            if not prediction_model.get("model_id"):
                prediction_model["model_id"] = (
                    f"model-{int(time.time())}-{hash(prediction_model.get('model_type', ''))}-{hash(prediction_model.get('target_metric', ''))}"
                )
    ...
    def store_prediction_error(self, error: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if (
                "relative_error" not in error
                and "actual_value" in error
                and "predicted_value" in error
            ):
                actual = error.get("actual_value", 0)
                if actual != 0:
                    error["relative_error"] = abs(error.get("predicted_value", 0) - actual) / abs(
                        actual
                    )
                else:
    ...
    def store_recommendation(self, recommendation: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(recommendation.get("timestamp"), str):
                recommendation["timestamp"] = datetime.fromisoformat(
                    recommendation["timestamp"].replace("Z", "+00:00")
                )
            ...
            if not recommendation.get("recommendation_id"):
                recommendation["recommendation_id"] = (
                    f"rec-{int(time.time())}-{hash(recommendation.get('model_name', ''))}"
                )
    ...
    def store_feature_importance(self, importance: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(importance.get("timestamp"), str):
                importance["timestamp"] = datetime.fromisoformat(
                    importance["timestamp"].replace("Z", "+00:00")
                )
        ...
        try:
            ...
            if stats:
                ...
                overall = {
                    ...
                    "metrics": all_metrics,
                    "overall_mean_relative_error": sum(
                        stats[m]["mean_relative_error"] * stats[m]["count"] for m in all_metrics
                    )
                    / sum(stats[m]["count"] for m in all_metrics)
                    if sum(stats[m]["count"] for m in all_metrics) > 0
                    else 0,
                }
    ...
    def generate_sample_data(self, num_models: int = 5) -> None:
        ...
        try:
            ...
            hardware_platforms = ["cpu", "cuda", "rocm", "mps", "openvino", "webgpu", "webnn"]
            ...
            selected_models = np.random.choice(
                model_configs, size=min(num_models, len(model_configs)), replace=False
            )
            ...
            for model_config in selected_models:
                ...
                for i, hw in enumerate(
                    np.random.choice(
                        hardware_platforms, size=min(4, len(hardware_platforms)), replace=False
                    )
                ):
                ...
                for hw in np.random.choice(
                    hardware_platforms, size=min(3, len(hardware_platforms)), replace=False
                ):
                    for batch_size in np.random.choice(batch_sizes, size=2, replace=False):
                        for precision in np.random.choice(precision_options, size=1):
                            ...
                            latency_pred = (
                                base_latency
                                / (hw_factor * precision_factor)
                                * (1 + np.random.normal(0, 0.1))
                            )
                            memory_pred = (
                                base_memory * memory_factor * (1 + np.random.normal(0, 0.05))
                            )
                            ...
                            for metric_name, pred_val, actual_val in [
                                ...
                                relative_error = (
                                    abs(pred_val - actual_val) / abs(actual_val)
                ...
                for target_metric in ["throughput", "latency", "memory_usage"]:
                    ...
                    for hw in np.random.choice(hardware_platforms, size=2, replace=False):
                        ...
                        for i, feature in enumerate(
                            np.random.choice(features, size=len(features), replace=False)
                        ):
                ...
                for _ in range(3):
                    ...
                    recommendation = {
                        ...
                        "user_feedback": np.random.choice(
                            [None, "Works great!", "Too slow", "Memory issues"],
                            p=[0.7, 0.1, 0.1, 0.1],
                        ),
                        "metadata": {
