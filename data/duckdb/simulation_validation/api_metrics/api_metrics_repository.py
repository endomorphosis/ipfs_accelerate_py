    ...
    def store_anomaly(self, anomaly: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(anomaly.get("timestamp"), str):
                anomaly["timestamp"] = datetime.fromisoformat(
    ...
    def generate_sample_data(self, num_records: int = 1000, days_back: int = 30) -> None:
        ...
        try:
            ...
            for _ in range(num_records):
                ...
                response_time = max(
                    0.05, np.random.normal(base_response_time, base_response_time / 3)
                )
            ...
            for _ in range(10):
                ...
                anomaly = {
                    ...
                    "metadata": json.dumps(
                        {
                            "detection_algorithm": np.random.choice(
                                ["z-score", "IQR", "ARIMA", "isolation_forest"]
                            ),
            ...
            for recommendation_type in recommendation_types:
                for priority in range(1, 6):  # 1-5 priority levels
                    if (
                        np.random.rand() > 0.5
                    ):  # 50% chance to generate for each type/priority combo
                        endpoint = np.random.choice(endpoints)
                        model = np.random.choice(models)
                        recommendation = {
                            "timestamp": datetime.now(),
