...
class APIMetricsValidator:
    ...
    def __init__(
        self, repository: Optional[DuckDBAPIMetricsRepository] = None, config: Dict[str, Any] = None
    ):
        """
        ...
        self.validation_thresholds = self.config.get(
    ...
    def validate_data_quality(
        ...
        try:
            ...
            for metric in metrics:
                present_fields = sum(
                    1 for field in required_fields if field in metric and metric[field] is not None
                )
            ...
            for metric in metrics:
                ...
                if "success" in metric and "status_code" in metric:
                    if metric["success"] is False and metric["status_code"] == 200:
                        score -= 0.5
                    elif metric["success"] is True and (
                        metric["status_code"] < 200 or metric["status_code"] >= 300
                    ):
                        score -= 0.5
    ...
    def _generate_data_quality_recommendations(
        self, completeness: float, consistency: float, validity: float, timeliness: float
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_prediction_recommendations(
        ...
        low_accuracy_groups = [
            g for g in group_results if g["accuracy"] < self.validation_thresholds["accuracy"]
        ]
            ...
            for (anom_endpoint, anom_model, anom_metric_type), anoms in anomaly_groups.items():
                ...
                for anomaly in anoms:
                    if "timestamp" in anomaly:
                        anom_windows.append(
                            (anomaly["timestamp"] - window_size, anomaly["timestamp"] + window_size)
                        )
                ...
                results.append(
                    {
                        ...
                        "threshold_met": precision
                        >= self.validation_thresholds["anomaly_precision"],
                    }
                )
    ...
    def validate_recommendation_relevance(
        ...
        try:
            ...
            for (rec_endpoint, rec_model), recs in recommendation_groups.items():
                ...
                for rec in recs:
                    ...
                    if recent_anomalies:
                        ...
                        relevant_anomalies = [
                            ...
                            if (
                                rec_type == "performance_optimization"
                                and a.get("metric_type") == "response_time"
                            )
                            or (
                                rec_type == "reliability_improvement"
                                and a.get("metric_type") == "error_rate"
                            )
                            or (
                ...
                if recent_anomalies and recent_metrics:
                    ...
                    avg_response_times = [
                        ...
                        if m.get("metric_type") == "response_time"
                    ]
                    ...
                    # Calculate coverage
                    if distinct_issues:
                        impact_coverage = len(addressed_issues.intersection(distinct_issues)) / len(
                            distinct_issues
                        )

                # Overall quality score
                weights = {"relevance": 0.4, "actionability": 0.3, "impact_coverage": 0.3}
                ...
                results.append(
                    {
                        ...
                        "threshold_met": overall_quality
                        >= self.validation_thresholds["recommendation_relevance"],
                    }
                )
            ...
            return {
                ...
                "threshold_met": avg_overall_quality
                >= self.validation_thresholds["recommendation_relevance"],
                "group_results": results,
            ...
            recommendations.append(
                {
                    ...
                    "priority": "high"
                    if any(g["relevance_score"] < 0.4 for g in low_relevance_groups)
                    else "medium",
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "priority": "high"
                    if any(g["actionability_score"] < 0.4 for g in low_actionability_groups)
                    else "medium",
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "priority": "high"
                    if any(g["impact_coverage"] < 0.4 for g in low_coverage_groups)
                    else "medium",
                }
            )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "high"
                        if avg_quality
                        < self.validation_thresholds["recommendation_relevance"] * 0.7
                        else "medium",
                    }
                )
    ...
    def generate_validation_report(
        ...
        data_quality = self.validate_data_quality(
            start_time=start_time, end_time=end_time, endpoint=endpoint, model=model
        )
        ...
        overall_score = (
            ...
            + recommendation_relevance.get("overall_quality", 0.0)
            * weights["recommendation_relevance"]
        )
        ...
        if (
            "recommendations" in recommendation_relevance
            and recommendation_relevance["recommendations"]
        ):
        ...
        all_recommendations.sort(
            key=lambda r: priority_map.get(r.get("priority", "low"), 0), reverse=True
        )
