...
class ExecutiveSummaryGenerator(ReportGenerator):
    ...
    def generate_executive_summary(
        ...
        if visualization_paths:
            ...
            limited_viz_paths = dict(
                list(visualization_paths.items())[: self.max_visualization_count]
            )
    ...
    def _extract_key_highlights(self, validation_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if self.executive_level == "c-suite":
            ...
            if hardware_results:
                pass_count = sum(
                    1 for hw in hardware_results.values() if hw.get("status") == "pass"
                )
    ...
    def _get_critical_models(self, model_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sorted_models = sorted(
            model_results.items(), key=lambda x: x[1].get("mape", {}).get("mean", float("inf"))
        )
...
if __name__ == "__main__":
    ...
    generator = ExecutiveSummaryGenerator(output_dir="reports", executive_level="director")
    ...
    comparative_data = {
        "comparison": {
            "mape": {"before": "8.45%", "after": "5.32%", "improvement": 37.04},
            "rmse": {"before": "0.0312", "after": "0.0245", "improvement": 21.47},
        },
        "versions": [
