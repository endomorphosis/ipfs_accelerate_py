...
class TechnicalReportGenerator(ReportGenerator):
    ...
    def __init__(
        ...
        logger.info(
            f"Technical report generator initialized with confidence level {confidence_level}"
        )
    ...
    def generate_technical_report(
        ...
        template_vars["detailed_statistics"] = self._generate_detailed_statistics(
            validation_results
        )

        # Add the technical detailed visualizations section
        if visualization_paths and self.include_visualizations:
            template_vars["technical_visualizations_html"] = (
                self._generate_technical_visualizations_html(visualization_paths)
            )
            template_vars["technical_visualizations_markdown"] = (
                self._generate_technical_visualizations_markdown(visualization_paths)
            )
                    ...
                    for metric in self.detailed_metrics:
                        if metric in hw_data:
                            ...
                            if "status" in hw_data:
                                status_class = (
                                    "positive" if hw_data["status"] == "pass" else "negative"
                                )
                                html += (
                                    f"<td class='{status_class}'>{hw_data['status'].upper()}</td>"
                                )
                            else:
                                html += "<td>N/A</td>"
                    ...
                    for metric in self.detailed_metrics:
                        if metric in model_data:
                            ...
                            if "status" in model_data:
                                status_class = (
                                    "positive" if model_data["status"] == "pass" else "negative"
                                )
    ...
    def _generate_technical_visualizations_html(self, visualization_paths: Dict[str, str]) -> str:
    ...
    def _generate_detailed_statistics(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hardware_results:
            ...
            for hw_id, hw_data in hardware_results.items():
                ...
                for metric in self.detailed_metrics:
                    if metric in hw_data:
                        statistics["summary"]["hardware"][hw_id]["metrics"][metric] = hw_data[
                            metric
                        ]
        ...
        if model_results:
            ...
            for model_id, model_data in model_results.items():
                ...
                for metric in self.detailed_metrics:
                    if metric in model_data:
                        statistics["summary"]["model"][model_id]["metrics"][metric] = model_data[
                            metric
                        ]
            ...
            for dist_name, dist_data in statistics["distributions"].items():
                ...
                if "histogram" in dist_data:
                    ...
                    for i, (bin_start, count) in enumerate(zip(hist["bins"], hist["counts"])):
                        ...
                        bin_label = (
                            f"{bin_start:.2f} - {bin_end:.2f}" if bin_end else f"{bin_start:.2f}+"
                        )
        ...
        if statistics["correlations"]:
            ...
            for corr_name, corr_data in statistics["correlations"].items():
                ...
                if isinstance(corr_data, dict) and "matrix" in corr_data:
                    ...
                    variables = corr_data.get(
                        "variables", [f"Var{i + 1}" for i in range(len(matrix))]
                    )
...
if __name__ == "__main__":
    ...
    validation_results = {
        ...
        "distributions": {
            "MAPE Distribution": {
                "histogram": {"bins": [0, 2, 4, 6, 8, 10], "counts": [5, 18, 15, 8, 4]},
                "statistics": {"skewness": 0.45, "kurtosis": 2.3, "shapiro_p_value": 0.12},
            }
        },
        "correlations": {
            "Metric Correlations": {
