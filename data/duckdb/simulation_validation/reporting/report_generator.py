...
class ReportGenerator:
    ...
    def _load_template(self, report_type: ReportType, format_type: ReportFormat) -> str:
        ...
        default_template_path = os.path.join(
            self.templates_dir, f"{report_type.value}_{format_type.value}.template"
        )
...
Report ID: {{metadata.report_id}}"""
    ...
    def _prepare_template_variables(
        ...
        if visualization_paths and self.include_visualizations:
            ...
            for viz_name, viz_path in visualization_paths.items():
                if os.path.exists(viz_path):
                    visualizations.append({"title": viz_name, "path": viz_path})
        ...
        variables["validation_results_html"] = self._generate_validation_results_html(
            validation_results
        )
        variables["validation_results_markdown"] = self._generate_validation_results_markdown(
            validation_results
        )
        variables["validation_results_text"] = self._generate_validation_results_text(
            validation_results
        )
        ...
        if comparative_data:
            ...
            variables["comparative_analysis_markdown"] = (
                self._generate_comparative_analysis_markdown(comparative_data, show_improvement)
            )
    ...
    def _process_content_for_format(
        self, content: str, format_type: ReportFormat, output_path: str
    ) -> Union[str, bytes]:
    ...
    def _generate_summary_html(self, validation_results: Dict[str, Any]) -> str:
        ...
        if "status" in overall:
            status_class = (
                "improvement-positive" if overall["status"] == "pass" else "improvement-negative"
            )
    ...
    def _generate_validation_results_html(self, validation_results: Dict[str, Any]) -> str:
        ...
        if hardware_results:
            ...
            for hw_id, hw_data in hardware_results.items():
                ...
                html += (
                    f"<td class='{status_class}'>{hw_data.get('status', 'N/A').upper()}</td></tr>"
                )
    ...
    def _generate_comparative_analysis_html(
        self, comparative_data: Dict[str, Any], show_improvement: bool = True
    ) -> str:
...
if __name__ == "__main__":
    ...
    validation_results = {
        "overall": {
            "mape": {"mean": 5.32, "std_dev": 1.23, "min": 2.1, "max": 8.7},
            "rmse": {"mean": 0.0245, "std_dev": 0.0078, "min": 0.0123, "max": 0.0456},
            "status": "pass",
        },
        "hardware_results": {
            "rtx3080": {"mape": {"mean": 4.21}, "rmse": {"mean": 0.0201}, "status": "pass"},
            "a100": {"mape": {"mean": 3.56}, "rmse": {"mean": 0.0189}, "status": "pass"},
        },
        "model_results": {
            "bert-base-uncased": {
                "mape": {"mean": 6.78},
