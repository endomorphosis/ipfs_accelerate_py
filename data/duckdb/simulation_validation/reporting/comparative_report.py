...
class ComparativeReportGenerator(ReportGenerator):
    ...
    def __init__(
        ...
        logger.info(
            f"Comparative report generator initialized with improvement threshold {improvement_threshold}% and regression threshold {regression_threshold}%"
        )
    ...
    def _create_comparison_data(
        ...
        comparison_data = {
            "summary": {"before_label": before_label, "after_label": after_label},
            "comparison": {},
            "details": {"hardware": {}, "model": {}},
            "significant_changes": {"improvements": [], "regressions": []},
        }
        ...
        for metric in metrics_to_compare:
            if metric in overall_before and metric in overall_after:
                ...
                if (
                    isinstance(before_data, dict)
                    and "mean" in before_data
                    and isinstance(after_data, dict)
                    and "mean" in after_data
                ):
                    ...
                    try:
                        ...
                        comparison_data["comparison"][metric] = {
                            ...
                            "is_significant_improvement": improvement >= self.improvement_threshold
                            and improvement_direction == 1,
                            "is_significant_regression": improvement <= -self.regression_threshold
                            and improvement_direction == 1,
                        }

                        # Adjust significance for metrics where higher is better
                        if metric in ["correlation", "ranking"]:
                            comparison_data["comparison"][metric]["is_significant_improvement"] = (
                                improvement >= self.improvement_threshold
                                and improvement_direction == -1
                            )
                            comparison_data["comparison"][metric]["is_significant_regression"] = (
                                improvement <= -self.regression_threshold
                                and improvement_direction == -1
                            )
        ...
        if include_details:
            ...
            for hw_id in all_hardware:
                ...
                comparison_data["details"]["hardware"][hw_id] = {"metrics": {}}
                            ...
                            try:
                                if before_value != 0:
                                    ...
                                    improvement_direction = (
                                        1  # Positive means improved (lower is better)
                                    )
                                else:
                                    improvement = 0
                                    improvement_direction = 0
                                ...
                                if metric in ["correlation", "ranking"]:
                                    ...
                                    improvement_direction = (
                                        -1
                                    )  # Negative means improved (higher is better)
                                ...
                                comparison_data["details"]["hardware"][hw_id]["metrics"][metric] = {
                                    ...
                                    "improvement": improvement,
                                    "improvement_direction": improvement_direction,
                                    "is_significant_improvement": improvement
                                    >= self.improvement_threshold
                                    and improvement_direction == 1,
                                    "is_significant_regression": improvement
            ...
            for model_id in all_models:
                ...
                comparison_data["details"]["model"][model_id] = {"metrics": {}}
    ...
    def _create_trend_analysis(
        ...
        trend_data = {"metrics": {}, "versions": []}

        # Sort historical results by timestamp (newest first)
        sorted_results = sorted(
            historical_results, key=lambda x: x.get("timestamp", ""), reverse=True
        )
            ...
            if len(filtered_values) >= 2:
                ...
                try:
                    ...
                    if abs(slope) < 0.001:
                        ...
                        trend = (
                            "improving"  # Negative slope means decreasing error (for most metrics)
                        )
                    else:
                        trend = (
                            "worsening"  # Positive slope means increasing error (for most metrics)
                        )
                    ...
                    trend_data["metrics"][metric + "_trend"] = {"slope": slope, "direction": trend}
                except (TypeError, ValueError) as e:
    ...
    def _generate_comparative_report_html(
        ...
        html += (
            f"<tr><th>Metric</th><th>{before_label}</th><th>{after_label}</th><th>Change</th></tr>"
        )
        ...
        if "statistical_significance" in comparison_data:
            ...
            html += (
                "<tr><th>Metric</th><th>T-Statistic</th><th>P-Value</th><th>Significant</th></tr>"
            )
        ...
        if (
            "details" in comparison_data
            and comparison_data["details"].get("hardware")
            and comparison_data["details"].get("model")
        ):
            # Hardware details
        ...
        if (
            ...
            for hw_id, hw_data in comparison_data["details"]["hardware"].items():
                ...
                for metric, data in hw_data["metrics"].items():
                    ...
                    markdown += (
                        f"| {metric.upper()} | {before_value} | {after_value} | {change_field} |\n"
                    )
    ...
    def _generate_comparative_report_text(
        ...
        if (
            ...
            for hw_id, hw_data in comparison_data["details"]["hardware"].items():
                ...
                for metric, data in hw_data["metrics"].items():
                    if data.get("is_significant_improvement", False) or data.get(
        ...
        if "trend_analysis" in comparison_data:
            ...
            for metric in ["mape", "rmse", "correlation"]:
                ...
                if trend_key in comparison_data["trend_analysis"]["metrics"]:
                    ...
                    text += (
                        f"{metric.upper()}: Slope {trend_data['slope']:.6f} ({trend_direction})\n"
                    )
...
if __name__ == "__main__":
    ...
    validation_results_before = {
        "overall": {
            "mape": {"mean": 8.45, "std_dev": 1.85, "min": 3.2, "max": 12.1, "sample_size": 50},
            "rmse": {
                "mean": 0.0312,
                "std_dev": 0.0092,
                "min": 0.0145,
    ...
    validation_results_after = {
        "overall": {
            "mape": {"mean": 5.32, "std_dev": 1.23, "min": 2.1, "max": 8.7, "sample_size": 50},
            "rmse": {
                "mean": 0.0245,
                "std_dev": 0.0078,
                "min": 0.0123,
    ...
    version_info = [
        {"version": "1.0.0", "date": "2025-07-15", "description": "Initial version"},
        {"version": "1.1.0", "date": "2025-07-29", "description": "Improved calibration"},
    ]
