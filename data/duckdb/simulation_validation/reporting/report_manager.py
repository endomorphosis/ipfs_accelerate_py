    ...
    def generate_report(
        ...
        if report_type == ReportType.EXECUTIVE_SUMMARY:
            ...
            if "validation_results_before" in comparative_data:
                ...
                logger.warning(
                    "Comparative report requested but no 'validation_results_before' provided"
                )
    ...
    def schedule_report_generation(
        ...
        def job_function():
            try:
                ...
                title = (
                    title_template.format(timestamp=timestamp)
                    if title_template
                    else f"Scheduled Report - {timestamp}"
                )
                description = (
                    description_template.format(timestamp=timestamp)
                    if description_template
                    else f"Automatically generated report at {timestamp}"
                )
        ...
        logger.info(
            f"Scheduled report generation job {job_id} with {schedule_type} schedule: {schedule_value}"
        )
        ...
        def auto_archive_job():
            try:
                ...
                for report in reports_to_archive:
                    logger.info(
                        f"Auto-archiving report {report.get('id')} (age: {(now - datetime.datetime.fromisoformat(report.get('timestamp', ''))).days} days)"
                    )
