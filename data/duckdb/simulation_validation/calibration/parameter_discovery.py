...
class ParameterDiscovery:
    ...
    def discover_parameters(
        ...
        logger.info(
            f"Analyzing {len(numeric_params)} numeric parameters: {list(numeric_params.keys())}"
        )
        ...
        for param in numeric_params:
            ...
            improvement = (
                (baseline_error - min_error) / baseline_error if baseline_error > 0 else 0.0
            )
            ...
            logger.info(
                f"Parameter {param}: sensitivity = {sensitivity}, "
        ...
        sensitive_params = [p for p, data in sensitivity_results.items() if data["is_sensitive"]]
        ...
        improvement = (
            (baseline_error - optimal_error) / baseline_error if baseline_error > 0 else 0.0
        )
        ...
        logger.info(
            f"Parameter discovery completed with {len(sensitive_params)} sensitive parameters"
        )
    ...
    def analyze_parameters_over_time(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for param, history in param_history.items():
                ...
                sensitive_count = sum(
                    1 for result in self.results if param in result.get("sensitive_parameters", [])
                )
...
class AdaptiveCalibrationScheduler:
    ...
    def should_calibrate(
        self, current_error: float = None, drift_value: float = None
    ) -> Tuple[bool, str]:
        """
        ...
        if current_error is not None and current_error > self.error_threshold:
            return (
                True,
                f"Error ({current_error:.4f}) exceeds threshold ({self.error_threshold:.4f})",
            )
    ...
    def record_calibration(
        ...
        calibration = {
            ...
            "improvement": (
                ...
                if error_before and error_after and error_before > 0
                else None
            ),
        ...
        next_time = now + np.timedelta64(
            int(self.schedule_data["current_interval_hours"] * 3600), "s"
        )
    ...
    def _adjust_calibration_interval(self, calibration: Dict[str, Any]) -> None:
        ...
        if calibration.get("improvement") is not None:
            ...
            if improvement < 0.01:  # Less than 1% improvement
                ...
                logger.info(
                    f"Minimal improvement ({improvement:.2%}), increasing interval to {new_interval:.1f} hours"
                )
                ...
                logger.info(
                    f"Significant improvement ({improvement:.2%}), decreasing interval to {new_interval:.1f} hours"
                )
        ...
        if calibration.get("drift_value") is not None:
            ...
            if drift > self.drift_threshold * 2:  # Drift is more than 2x threshold
                ...
                logger.info(
                    f"High drift ({drift:.4f}), decreasing interval to {new_interval:.1f} hours"
                )
    ...
    def analyze_calibration_effectiveness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_error_before = (
                np.mean([e for e in errors_before if e is not None]) if errors_before else None
            )
            avg_error_after = (
                np.mean([e for e in errors_after if e is not None]) if errors_after else None
            )
            avg_improvement = (
                np.mean([i for i in improvements if i is not None]) if improvements else None
            )
            avg_drift = (
                np.mean([d for d in drift_values if d is not None]) if drift_values else None
            )

            # Calculate trends
            improvement_trend = (
                self._detect_trend([i for i in improvements if i is not None])
            ...
            return {
                ...
                "avg_error_before": float(avg_error_before)
                if avg_error_before is not None
                else None,
