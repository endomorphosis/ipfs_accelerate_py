...
class DuckDBCalibrationRepository:
    ...
    def _initialize_tables(self) -> None:
        ...
        try:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_params_calibration_id ON calibration_parameters(calibration_id)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_results_calibration_id ON calibration_results(calibration_id)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_history_timestamp ON calibration_history(timestamp)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_cv_validation_id ON cross_validation_results(validation_id)"
            )
    ...
    def store_parameter(self, parameter: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(parameter.get("timestamp"), str):
                parameter["timestamp"] = datetime.fromisoformat(
                    parameter["timestamp"].replace("Z", "+00:00")
                )
    ...
    def get_parameters(
        self, calibration_id: Optional[str] = None, parameter_names: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def store_calibration_result(self, result: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if (
                "error_reduction_percent" not in result
                and "error_before" in result
                and "error_after" in result
            ):
    ...
    def store_cross_validation_result(self, validation_result: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(validation_result.get("timestamp"), str):
                validation_result["timestamp"] = datetime.fromisoformat(
                    validation_result["timestamp"].replace("Z", "+00:00")
                )
    ...
    def get_cross_validation_results(
        self, validation_id: Optional[str] = None, calibration_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def store_parameter_sensitivity(self, sensitivity: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(sensitivity.get("timestamp"), str):
                sensitivity["timestamp"] = datetime.fromisoformat(
                    sensitivity["timestamp"].replace("Z", "+00:00")
                )
    ...
    def get_parameter_sensitivities(
        self,
    ...
    def store_uncertainty_quantification(self, uncertainty: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(uncertainty.get("timestamp"), str):
                uncertainty["timestamp"] = datetime.fromisoformat(
                    uncertainty["timestamp"].replace("Z", "+00:00")
                )
    ...
    def get_uncertainty_quantifications(
        self,
        analysis_id: Optional[str] = None,
        calibration_id: Optional[str] = None,
        parameter_names: Optional[List[str]] = None,
    ...
    def store_calibration_history(self, history_entry: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(history_entry.get("timestamp"), str):
                history_entry["timestamp"] = datetime.fromisoformat(
                    history_entry["timestamp"].replace("Z", "+00:00")
                )
    ...
    def store_calibration_drift(self, drift: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(drift.get("timestamp"), str):
                drift["timestamp"] = datetime.fromisoformat(
                    drift["timestamp"].replace("Z", "+00:00")
                )
    ...
    def generate_sample_data(self, num_calibrations: int = 5) -> None:
        ...
        try:
            ...
            hardware_platforms = ["cuda", "cpu", "webgpu", "webnn", "openvino"]
            ...
            for i in range(num_calibrations):
                ...
                for param_name in np.random.choice(parameter_names, size=5, replace=False):
                    ...
                    parameter = {
                        ...
                        "metadata": {"original_value": 1.0, "adjustment": value - 1.0},
                    }
                    parameters.append(parameter)
                ...
                for fold in range(5):
                    ...
                    cv_result = {
                        ...
                        "metadata": {"data_split": f"80/20 fold {fold + 1}"},
                    }
                ...
                for idx, param in enumerate(parameters):
                    ...
                    sensitivity_record = {
                        ...
                        "metadata": {"analysis_method": "perturbation"},
                    }
                ...
                history_entry = {
                    ...
                    "status": np.random.choice(
                        ["completed", "failed", "in_progress"], p=[0.8, 0.1, 0.1]
                    ),
                    "metadata": {"generated": True, "date": datetime.now().isoformat()},
                }
                ...
                for j in range(np.random.randint(1, 4)):
                    ...
                    drift_record = {
                        ...
                        "affected_parameters": [
                            p["name"] for p in np.random.choice(parameters, size=2, replace=False)
                        ],
