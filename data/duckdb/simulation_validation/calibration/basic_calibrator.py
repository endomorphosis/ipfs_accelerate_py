...
class BasicCalibrator:
    ...
    def __init__(self, learning_rate: float = 0.1, max_iterations: int = 100):
        ...
        logger.info(
            f"Initialized BasicCalibrator with learning_rate={learning_rate}, "
    ...
    def _calculate_adjustments(
        self, sim_values: List[float], hw_values: List[float]
    ) -> Tuple[float, float]:
        """
        ...
        if len(sim_values) != len(hw_values):
            logger.warning(
                f"Length mismatch: sim_values={len(sim_values)}, hw_values={len(hw_values)}"
            )
    ...
    def _apply_general_calibration(
        self,
        ...
        if "global_scale" in parameters:
            ...
            if abs(total_sim) > 1e-10:
                ...
                adjusted_scale = parameters["global_scale"] * (
                    1.0 + self.learning_rate * (global_scale - 1.0)
                )
    ...
    def evaluate_calibration(
        ...
        for metric_name in (
            set(pre_metrics.keys())
            .intersection(post_metrics.keys())
            .intersection(hw_metrics.keys())
        ):
        ...
        if evaluation["metrics"]:
            avg_pre_mape = np.mean(
                [m["pre_calibration_mape"] for m in evaluation["metrics"].values()]
            )
            avg_post_mape = np.mean(
                [m["post_calibration_mape"] for m in evaluation["metrics"].values()]
            )
            overall_improvement = (
                (avg_pre_mape - avg_post_mape) / avg_pre_mape * 100 if avg_pre_mape > 0 else 0
            )
