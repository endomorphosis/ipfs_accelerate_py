...
class UncertaintyQuantifier:
    ...
    def quantify_parameter_uncertainty(
        ...
        logger.info(
            f"Parameter uncertainty quantification completed for {len(uncertainty)} parameters"
        )
        return result
    ...
    def _generate_parameter_samples(
        self, parameter_uncertainty: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def sensitivity_analysis(
        ...
        """
        logger.info(
            f"Performing sensitivity analysis with perturbation factor {perturbation_factor}"
        )

        # Create baseline parameters (mean values)
        baseline_params = {
            param: metrics["mean"] for param, metrics in parameter_uncertainty.items()
        }
        ...
        for param, metrics in parameter_uncertainty.items():
            ...
            try:
                ...
                sensitivity_value = (
                    abs(error_up - baseline_error) + abs(error_down - baseline_error)
                ) / 2
        ...
        sorted_params = sorted(
            sensitivity.keys(), key=lambda p: sensitivity[p]["relative_sensitivity"], reverse=True
        )
        ...
        for param in sorted_params:
            if sensitivity[param]["sensitivity_level"] in [
                "high",
                "very_high",
            ] and parameter_uncertainty[param]["uncertainty_level"] in ["medium", "high"]:
    ...
    def generate_report(self, result_id: Optional[str] = None, format: str = "text") -> str:
        """
    ...
    def _generate_text_report(self, result: Dict[str, Any]) -> str:
        ...
        if "parameter_uncertainty" in result:
            ...
            for param in sorted_params:
                ...
                lines.append(
                    f"  CI: [{metrics.get('ci_lower', 'N/A'):.6f}, {metrics.get('ci_upper', 'N/A'):.6f}]"
                )
        ...
        if "error_statistics" in result:
            ...
            if "ci_lower" in error_stats and "ci_upper" in error_stats:
                lines.append(
                    f"CI: [{error_stats.get('ci_lower', 'N/A'):.6f}, {error_stats.get('ci_upper', 'N/A'):.6f}]"
                )
            ...
            if "relative_uncertainty" in error_stats:
                lines.append(
                    f"Relative Uncertainty: {error_stats.get('relative_uncertainty', 'N/A'):.4f}"
                )
        ...
        if "reliability" in result:
            ...
            lines.append(
                f"Reliability: {result.get('reliability', 'N/A'):.4f} ({result.get('reliability', 0.0) * 100:.1f}%)"
            )
            lines.append(
                f"Reliability CI: [{result.get('reliability_ci_lower', 'N/A'):.4f}, {result.get('reliability_ci_upper', 'N/A'):.4f}]"
            )
        ...
        if "sensitivity" in result:
            ...
            for param in sorted_params:
                ...
                lines.append(
                    f"  Relative Sensitivity: {metrics.get('relative_sensitivity', 'N/A'):.4f}"
                )
    ...
    def _generate_markdown_report(self, result: Dict[str, Any]) -> str:
        ...
        if "error_statistics" in result:
            ...
            if "uncertainty_level" in error_stats:
                lines.append(
                    f"- **Uncertainty Level:** {error_stats.get('uncertainty_level', 'N/A')}"
                )
