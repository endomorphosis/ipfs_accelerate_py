...
class CalibrationCrossValidator:
    ...
    def cross_validate(
        ...
        if len(simulation_results) != len(hardware_results):
            logger.warning(
                f"Length mismatch: simulation_results={len(simulation_results)}, "
        ...
        if group_by:
            return self._cross_validate_grouped(
                simulation_results, hardware_results, initial_parameters, parameter_bounds, group_by
            )
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(indices)):
            ...
            logger.info(
                f"Fold {fold + 1} train error: {train_error:.6f}, val error: {val_error:.6f}"
            )
        ...
        generalization_gap_percentage = (
            generalization_gap / avg_train_error * 100 if avg_train_error > 0 else 0.0
        )
        ...
        result["improvement"] = float(
            (initial_error - recommended_error) / initial_error * 100 if initial_error > 0 else 0.0
        )
        ...
        logger.info(
            f"Initial error: {initial_error:.6f}, recommended error: {recommended_error:.6f}, "
    ...
    def _create_calibrator(self) -> Union[BasicCalibrator, AdvancedCalibrator]:
        ...
        if self.calibrator_type == "basic":
            ...
            logger.warning(
                f"Unknown calibrator type: {self.calibrator_type}, falling back to BasicCalibrator"
            )
            return BasicCalibrator()
    ...
    def _apply_parameters_to_metric(
        self, sim_values: List[float], parameters: Dict[str, Any], metric_name: str
    ) -> List[float]:
        """
    ...
    def _calculate_recommended_parameters(
        self, parameter_sets: List[Dict[str, Any]], stability: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if avg_gap > 25.0:
                recommendations.append(
                    "High generalization gap detected. Consider using more data or simpler calibration models."
                )

            if avg_improvement < 5.0:
                recommendations.append(
                    "Low improvement detected. Consider adjusting parameter bounds or trying different calibrator types."
                )
            ...
            if len(unique_types) > 1:
                ...
                recommendations.append(
                    f"Based on generalization performance, consider using '{best_type}' calibrator type."
                )
    ...
    def _visualize_single_result(
        ...
        fig.suptitle(
            f"Cross-Validation Results - {result.get('calibrator_type', 'unknown')}", fontsize=16
        )
        ...
        if "improvement" in result:
            ...
            axs[1, 0].text(
                1, values[1] * 1.1, f"{improvement:.2f}% improvement", horizontalalignment="center"
            )
        ...
        if "initial_parameters" in result and "recommended_parameters" in result:
            ...
            for param in params_to_plot:
                if isinstance(initial_params[param], (int, float)) and isinstance(
                    recommended_params[param], (int, float)
                ):
                    param_names.append(param)
                    if abs(initial_params[param]) < 1e-10:
                        ...
                        change_pct = (
                            (recommended_params[param] - initial_params[param])
                            / abs(initial_params[param])
                            * 100
                        )
    ...
    def _visualize_multiple_results(
        ...
        if unique_types:
            ...
            axs[1, 1].text(
                0.5,
                0.5,
                "No calibrator type data available",
                horizontalalignment="center",
                verticalalignment="center",
            )
        ...
        fig.text(
            0.5,
            0.01,
            f"Number of Results: {len(results)} | Date Range: {sorted_timestamps[0][:10]} to {sorted_timestamps[-1][:10]}",
            ha="center",
        )
