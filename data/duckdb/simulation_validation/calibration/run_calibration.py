...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("calibration.log")],
)
...
def create_calibrator(
    ...
    if calibrator_type == "basic":
        ...
        return MultiParameterCalibrator(
            learning_rate=learning_rate, max_iterations=max_iterations, history_file=history_file
        )
...
def run_calibration(
    ...
    # Create calibrator
    calibrator = create_calibrator(calibrator_type, learning_rate, max_iterations, history_file)
...
def run_parameter_discovery(
    ...
    logger.info(
        f"Parameter discovery completed with {len(result['sensitive_parameters'])} sensitive parameters"
    )
    return result
...
def run_cross_validation(
    ...
    if visualization_file and result["status"] == "success":
        cross_validator.visualize_results(output_file=visualization_file, result_id=result["id"])
...
def run_uncertainty_quantification(
    ...
    uncertainty_quantifier = UncertaintyQuantifier(
        confidence_level=confidence_level, n_samples=n_samples, result_file=result_file
    )
    ...
    if report_file:
        report = uncertainty_quantifier.generate_report(
            result_id=sensitivity_result["id"], format=report_format
        )
...
def main():
    ...
    cal_parser.add_argument(
        "--history-file", type=str, default=None, help="File to store calibration history"
    )
    cal_parser.add_argument(
    ...
    disc_parser.add_argument(
        "--result-file", type=str, default=None, help="File to store discovery results"
    )
    ...
    cv_parser.add_argument(
        "--result-file", type=str, default=None, help="File to store validation results"
    )
    cv_parser.add_argument("--group-by", type=str, default=None, help="Field to group results by")
    cv_parser.add_argument(
        "--visualization-file", type=str, default=None, help="File to save visualization"
    )
    ...
    uq_parser.add_argument(
        "--result-file", type=str, default=None, help="File to store uncertainty results"
    )
    uq_parser.add_argument(
        "--error-threshold", type=float, default=None, help="Threshold for reliability estimation"
    )
    uq_parser.add_argument(
        "--perturbation-factor", type=float, default=0.1, help="Factor for parameter perturbation"
    )
    uq_parser.add_argument("--report-file", type=str, default=None, help="File to save report")
    uq_parser.add_argument(
        "--report-format",
        type=str,
        default="markdown",
    ...
    try:
        ...
        if args.command == "calibrate":
            ...
            if (
                ...
                and "recommended_parameters" in result
            ):
