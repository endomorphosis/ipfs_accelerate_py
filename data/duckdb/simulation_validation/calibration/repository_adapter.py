...
class CalibratorDuckDBAdapter:
    ...
    def calibrate(
        ...
        error_before = self._calculate_error(
        ...
        error_after = self._calculate_error(
        ...
        for name, value in calibrated_parameters.items():
            if isinstance(value, (int, float)):
                parameter = {
                    ...
                    "metadata": {"original_value": simulation_parameters.get(name)},
                }
        ...
        result = {
            ...
            "error_reduction_percent": ((error_before - error_after) / error_before * 100)
            if error_before > 0
            else 0.0,
    ...
    def _calculate_error(
        ...
        if hasattr(self.calibrator, "_calculate_error"):
            return self.calibrator._calculate_error(
                simulation_results, hardware_results, parameters
            )
    ...
    def cross_validate(
        ...
        for fold_result in result.get("fold_results", []):
            cv_record = {
                ...
                if "val_error" in fold_result and "train_error" in fold_result
                else None,
                "dataset_id": dataset_id,
            ...
            for name, value in result["recommended_parameters"].items():
                if isinstance(value, (int, float)):
                    parameter = {
                        ...
                        "name": name,
                        "value": value,
                        "uncertainty": result.get("parameter_stability", {})
                        .get(name, {})
                        .get("std", 0.0),
...
class UncertaintyQuantifierDuckDBAdapter:
    ...
    def quantify_parameter_uncertainty(
        self, parameter_sets: List[Dict[str, Any]], calibration_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if result.get("status") != "success":
            logger.warning(
                f"Uncertainty quantification failed: {result.get('error', 'Unknown error')}"
            )
            return result
    ...
    def sensitivity_analysis(
        ...
        for param_name, metrics in result.get("sensitivity", {}).items():
            sensitivity_record = {
                ...
                if param_name in result.get("sorted_parameters", [])
                else None,
