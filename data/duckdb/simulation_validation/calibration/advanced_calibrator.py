...
try:
    ...
    logger.warning(
        "Scikit-learn not available. Bayesian optimization calibration will be disabled."
    )
...
class AdvancedCalibrator(ABC):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized {self.__class__.__name__} with learning_rate={learning_rate}, "
        ...
        event = {
            ...
            "parameter_changes": {
                k: {
                    ...
                    if isinstance(original_parameters.get(k), (int, float))
                    and isinstance(updated_parameters.get(k), (int, float))
                    else None,
                }
    ...
    def analyze_history_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    "increasing_params": [
                        p for p, data in param_trends.items() if data["trend"] == "increasing"
                    ],
                    "decreasing_params": [
                        p for p, data in param_trends.items() if data["trend"] == "decreasing"
                    ],
                    "stable_params": [
                        p for p, data in param_trends.items() if data["trend"] == "stable"
                    ],
                    "oscillating_params": [
                        p for p, data in param_trends.items() if data["trend"] == "oscillating"
                    ],
                },
            }
        except Exception as e:
...
class MultiParameterCalibrator(AdvancedCalibrator):
    ...
    def _perform_calibration(
        ...
        params_to_optimize = [
            k for k, v in updated_parameters.items() if isinstance(v, (int, float))
        ]
        ...
        try:
            if SCIPY_AVAILABLE:
                ...
                self._grid_search_optimization(
                    error_function, params_to_optimize, updated_parameters, initial_values, bounds
                )
        except Exception as e:
...
class BayesianOptimizationCalibrator(AdvancedCalibrator):
    ...
    def __init__(self, n_initial_points: int = 5, exploration_factor: float = 0.1, **kwargs):
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning(
                "Scikit-learn not available. BayesianOptimizationCalibrator will use fallback methods."
            )
    ...
    def _perform_calibration(
        ...
        logger.info(
            f"Optimizing {len(params_to_optimize)} parameters using Bayesian Optimization: {params_to_optimize}"
        )
...
class NeuralNetworkCalibrator(AdvancedCalibrator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not TF_AVAILABLE:
            logger.warning(
                "TensorFlow not available. NeuralNetworkCalibrator will use fallback methods."
            )
    ...
    def _perform_calibration(
        ...
        logger.info(
            f"Optimizing {len(params_to_optimize)} parameters using Neural Network: {params_to_optimize}"
        )
        ...
        try:
            ...
            for iteration in range(self.max_iterations):
                ...
                for i in range(len(current_point_norm)):
                    current_point_norm[i] = max(
                        bounds_norm[i][0], min(bounds_norm[i][1], current_point_norm[i])
                    )
...
class EnsembleCalibrator(AdvancedCalibrator):
    ...
    def __init__(
        self, ensemble_methods: List[str] = None, ensemble_weights: List[float] = None, **kwargs
    ):
        """
        ...
        if ensemble_methods is None:
            ...
            if len(self.ensemble_methods) < len(ensemble_methods):
                logger.warning(
                    f"Some requested ensemble methods are not available: "
        ...
        if ensemble_weights is None:
            ...
            if len(ensemble_weights) != len(self.ensemble_methods):
                logger.warning(
                    f"Length of ensemble_weights ({len(ensemble_weights)}) does not match "
                    f"length of ensemble_methods ({len(self.ensemble_methods)}). "
                    f"Using equal weights."
                )
                self.ensemble_weights = [1.0 / len(self.ensemble_methods)] * len(
        ...
        for method in self.ensemble_methods:
            ...
            self.calibrators[method] = calibrator_class(
                learning_rate=self.learning_rate,
                max_iterations=self.max_iterations
                // len(self.ensemble_methods),  # Divide iterations
    ...
    def _perform_calibration(
        ...
        # Filter parameters to optimize (only numeric parameters)
        params_to_optimize = [
            k for k, v in simulation_parameters.items() if isinstance(v, (int, float))
        ]
        ...
        valid_weights = [
            self.ensemble_weights[self.ensemble_methods.index(m)] for m in valid_methods
        ]
        ...
        if total_inverse == 0:
            ...
            error_weights = [
                w * (e / total_inverse) for w, e in zip(normalized_weights, inverse_errors)
            ]
            # Renormalize
