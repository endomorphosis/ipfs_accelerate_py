...
# Enhanced Restart MCP Server with IPFS Tools
# This script stops any running MCP server and starts a new one with IPFS tools registered
# using the fixed tool registration mechanism
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --debug)
      ...
      ;;
    --use-original)
      USE_FIXED_REGISTRATION=false
      shift
      ;;
    *)
      echo "Unknown option: $1"
      echo "Usage: $0 [--port PORT] [--host HOST] [--debug] [--use-original]"
      exit 1
      ;;
  esac
done
...
pkill -f "python.*mcp.*--port $PORT" || true
sleep 2

# Create a new Python script that will register all tools and start the server
cat > start_mcp_with_ipfs.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x start_mcp_with_ipfs.py

# Start the MCP server with either the fixed registration or original approach
echo "Starting MCP server with IPFS tools on $HOST:$PORT..."
if [ "$USE_FIXED_REGISTRATION" = true ]; then
  echo "Using fixed tool registration mechanism..."
  
  # Start MCP server
  if [ "$DEBUG" = true ]; then
    python start_mcp_with_ipfs.py --port $PORT --host $HOST --debug &
  else
    python start_mcp_with_ipfs.py --port $PORT --host $HOST &
  fi
  
  # Wait for the server to start
  sleep 3
  
  # Apply the fixed tool registration
  echo "Applying fixed tool registration..."
  python fix_mcp_tool_registration.py
else
  echo "Using original tool registration mechanism..."
  if [ "$DEBUG" = true ]; then
    python start_mcp_with_ipfs.py --port $PORT --host $HOST --debug &
  else
    python start_mcp_with_ipfs.py --port $PORT --host $HOST &
...
if pgrep -f "python.*start_mcp_with_ipfs.py.*--port $PORT" > /dev/null; then
  echo "MCP server started successfully!"
  echo "Server is running at: http://$HOST:$PORT"
  echo "SSE endpoint is available at: http://$HOST:$PORT/sse"
