# Guides

These guides describe the maintained user, developer, and operator surfaces of
IPFS Accelerate Python. Historical implementation reports are linked from the
[documentation index](../INDEX.md) but are not used as current API references.

## Start here

- [Getting started](getting-started/README.md)
- [Quick start](QUICKSTART.md)
- [Agent Supervisor Guide](AGENT_SUPERVISOR_GUIDE.md)

## Operate the system

- [Deployment](deployment/README.md)
- [Docker](docker/README.md)
- [P2P](p2p/README.md)
- [GitHub integration](github/README.md)
- [MCP setup](MCP_SETUP_GUIDE.md)
- [Troubleshooting](troubleshooting/faq.md)

## Develop and validate

- [Testing](../development/testing.md)
- [Documentation current state](../development/DOCUMENTATION_CURRENT_STATE.md)
- [Hardware tuning](hardware/overview.md)
- [Infrastructure](infrastructure/README.md)
- [CLI guides](cli/README_CLI.md)
