...
| `ASEH-` | **Agent-supervisor efficiency and state hardening** | Consolidate the existing supervisor path, measure paired token/compute/cost efficiency, and qualify state, routing, ContextPack, recovery, and promotion safety | `agent_supervisor_efficiency_state_hardening.*`, `AGENT_SUPERVISOR_EFFICIENCY_AND_STATE_HARDENING_PLAN.md` |
