...
Wave 3b transition: ASE3-033 protected transition builder and phase authority
...
ASE3-033 productionizes the construction path for every protected phase. Its
ordinary dependency is exactly ASE3-000. Its separate pre-Q integration-freeze
readiness join requires independently reviewed product generations for
ASE3-019, ASE3-030, ASE3-031, ASE3-032, ASE3-023, and ASE3-027, but it does not
depend on their acceptance statuses or future A-phase receipts. Independently
audited and replayed ASE3-033 tooling is already integrated in Q's parent while
ASE3-033 remains todo. Q atomically adds only the Q inventory and changes only
ASE3-033 todo→completed; no other status changes. Every R-or-later parent must
therefore already contain completed ASE3-033 before its candidate is built.

The exact production layout is
...
`test/api/test_agent_supervisor_prompt_v3_transition.py`, and the Q inventory at
`data/agent_supervisor/prompt_only_self_improvement_v3/convergence/protected_acceptance_q_inventory.json`,
plus the `.gitignore` mode correction and the scheduler, roadmap, convergence
validator, and convergence-test integration. Core contains pure schemas and
validation contracts. Merge imports core and receives signer, evidence, and
validator protocols; it never imports entrypoints. The entrypoint facade may
compose merge with `entrypoints.local_profile`, and it plus the CLI exposes or
re-exports exactly `freeze_prompt_v3_product_provenance`,
`build_prompt_v3_root_pin`, `initialize_prompt_v3_reviewer_after_root_pin`,
`build_prompt_v3_provider_authorization`, `canonical_prompt_v3_review_bytes`,
`sign_prompt_v3_operator_artifact`, `build_prompt_v3_phase_candidate`,
`run_prompt_v3_phase_evidence`, `validate_prompt_v3_phase_candidate`,
`publish_prompt_v3_phase_candidate`, `reject_prompt_v3_phase_candidate`,
`observe_prompt_v3_quiescence`, and
`build_prompt_v3_reload_authorization`. The facade also exposes the
A031-acceptance-only `consume_prompt_v3_a031_authorization`,
`append_prompt_v3_a031_failure_attempt`, and
`reauthorize_prompt_v3_p031_attempt` operations, plus the narrow production
verifier `load_verified_prompt_v3_runtime_launch_authority`.

Every independently reviewed product uses
`ipfs_accelerate_py.agent_supervisor.prompt-v3-product-generation@1` with
separate source, replay, and integrated records. Source and clean replay
commits remain nonancestors of Q; the reviewed integrated commit is an ancestor
of Q. The generation order and ordered commit counts are exactly ASE3-019×2,
ASE3-030×2, ASE3-031×1, ASE3-032×1, ASE3-023×3, and ASE3-027×2. For every
ordered commit in every source, replay, and integrated role, the verifier
reconstructs the patch stream with the exact command `git diff --no-ext-diff
--no-textconv --no-renames --binary --full-index <parent> <commit>`. All three
role streams must be byte-identical commit by commit. Each record binds the
exact commit, single parent, tree, changed-path ordering, and every path's Git
mode, raw bytes, and blob ID. The first parent is the role base, each later
parent is the preceding ordered commit, the declared final commit is the last
ordered commit, and an exact task→source/replay/integrated final-commit map
covers all six generations. Each reconstructed stream is stored as strict
standard-Base64 raw bytes plus its SHA-256 digest.
Cross-task or cross-commit evidence is never substitutable. Provenance
preserves each reviewed path's exact mode, including
`ipfs_accelerate_py/llm_router.py` at 100755; 100644 is required only for
builder-created Q/protected artifacts and the separately mandated `.gitignore`
correction.
...
artifacts. The durable object carrier is the sealed
`prompt-v3-product-generation@1` record itself: source and replay verify from
sealed commit fields only (`ordinal`, `commit`, `parent`, `tree`,
`changed_paths`, `files`, `canonical_patch_base64`, `canonical_patch_sha256`)
and do not require live Git object reachability; integrated commits must remain
reachable Q ancestors; ASE3-033 tooling must already be present in Q's parent
tree. When parent and commit objects are present, the verifier still reconstructs
the exact `git diff` stream and binds it to the sealed Base64/SHA-256 values;
otherwise the portable fallback compares sealed canonical patch Base64 with each
file's sealed raw bytes, blob ID, and mode. Ambient ref carriers are forbidden.

Q contains only pre-Q product provenance, stable policy, and the pre-existing
`lifecycle_root_identity_did`. It contains no future-pin sentinel and no
reviewer identity, profile or profile content, lifecycle anchor, signature,
time observation, test observation, capsule output, or generation. R pins the
root through `build_prompt_v3_root_pin`, which accepts only the exact public
root DID, stable policy, and expected repository identity/head/tree, emits
canonical ASCII public bytes through the hardened dirfd projection, verifies
the repository and root authority, and can never read or serialize a private
key, profile content, or future generation. Its only fields are schema, root
DID, stable-policy digest, repository identity/head/tree, and canonical digest,
and it lands as a 0400 public artifact. P019 initializes the reviewer
profile and fresh lifecycle witness only after that root pin and then
constructs the provider authorization. Every
R/P019/A019/A030/P031/A031/A032/A023/027/L/birth step uses a fresh, non-reused
`ipfs_accelerate_py.agent_supervisor.prompt-v3-phase-authority@1`, and every
`ipfs_accelerate_py.agent_supervisor.prompt-v3-phase-receipt@2` binds the exact
authority for its own phase. A019 binds the digest of the actual canonical
provider-authorization@2 bytes, never a planned digest. P031 uses
`ipfs_accelerate_py.agent_supervisor.ase3-031-native-dependency-launch-authorization@2`
with an exact purpose, nonce, not-before, expiry, and durable one-consumer CAS.
That CAS is consumed terminally only by the A031 acceptance preload; ASE3-023
and general post-L runtime cannot reuse P031. Once the native effect starts,
that authorization is terminal. A failed A031 probe appends signed failure
evidence to an immutable
`ipfs_accelerate_py.agent_supervisor.prompt-v3-p031-attempt-ledger@1`, leaves
ASE3-031 todo, and advances through a bounded re-authorization transition with
a fresh P031 nonce and expiry. At most three attempts are permitted, every
failed attempt remains in Git, and retry, overwrite, or erasure is forbidden.
L uses
`ipfs_accelerate_py.agent_supervisor.provider-attempt-daemon-reload@2`; its
prior authorization is canonically nullable and never represented by a
sentinel. Reload@2 carries or derives a distinct fresh per-target-generation
native launch authorization bound to the accepted A031 pin and accepted
A032/A023 gates. Its durable one-consumer CAS is consumed by the scheduler or
PlanRevisionStore before spawn. Recovery may only boundedly adopt a dead,
effectless same-generation launch under that consumed generation authority;
it never replays a provider effect. The ASE3-023 outer scheduler calls
`load_verified_prompt_v3_runtime_launch_authority` to verify the complete
committed Q→L chain plus A031, A032, and ASE3-023 acceptance joins and receive a
strict DTO. The sealed child binds that DTO's digest, chain head/tree, A031 pin,
A032/A023 receipt IDs, L authorization ID, and target generation without
importing the large convergence validator. Runtime L consumption remains owned
by the ASE3-023 PlanRevisionStore, never by the A031 acceptance-only tooling.

`sign_prompt_v3_operator_artifact` accepts only strict ASCII receipt payloads
whose canonical bytes are exactly compatible with
`entrypoints.local_profile.sign_profile_binding`. It calls that adapter with
the expected repository identity/head/tree and profile-generation/authority
bindings, strictly decodes its standard-Base64 result, requires a 64-byte
Ed25519 signature, transcodes it to unpadded base64url, and verifies the
signature with the bound public key before returning it. It reloads the active
profile and generation after signing, requires them to equal the verified
pre-sign profile and generation, and denies any rotation or revocation race.
The receipt mapping must round-trip to byte-identical adapter canonical bytes.
The sanitized execution path rejects the ambient
`AGENT_SUPERVISOR_LOCAL_PROFILE_KEY` variable; only the explicit verified
profile adapter can sign.

The builder starts from an isolated clean checkout of one exact ref and clears
or rejects ambient Git configuration, replacement refs, grafts, repository
redirection, hooks, filters, object/config redirection, and signing behavior.
It uses an alternate index, preserves the exact reviewed mode of every existing
path, writes builder-created Q/protected artifacts as 100644 blobs, constructs
exact one-parent commits, and never reads or mutates the working index or calls
`auto_commit_paths`. The `.gitignore` 100755 entry is corrected to 100644 and
its exact unanchored `core` line becomes `/core`; this preserves the root
core-dump ignore while a `git check-ignore` probe proves the nested
`agent_supervisor/core/protected_acceptance_contracts.py` output is visible.
Candidate validation runs the child validator and exact phase suites against
the candidate tree while one canonical target-ref lease remains held. The
target ref is resolved once and sanitized `git worktree list --porcelain -z`
classifies publication. An unheld ref is advanced only by `git update-ref`
compare-and-swap after a rescue-ref CAS. If the exact target ref is checked out,
the builder must not reject it as detached-only: in its clean holding worktree,
under the same lease, it runs the hook-free and sign-free equivalent of `git -c
core.hooksPath=<owned-empty-directory> -c commit.gpgSign=false -c
tag.gpgSign=false merge --ff-only --no-edit <candidate-commit>`. Before and
after publication it verifies the exact target ref, symbolic HEAD, ref/HEAD
OIDs, tree, index tree, and porcelain-v2 worktree bytes; the candidate must be
a fast-forward and post-state must be clean and equal to the candidate. Failure
restores the target by checked rescue-ref CAS and the checked-out worktree from
the verified rescue under the same lease, re-verifies the exact pre-state, and
retains the rescue ref whenever recovery is incomplete.

Filesystem projection is dirfd-relative and uses `O_NOFOLLOW`, stable owned
regular single-link verification, and group/other-write denial. Public
artifacts land at 0400, private signing keys remain 0600, directories and
runtime projections have group/other write removed, and data plus containing
directories are fsynced around atomic rename. Post-L birth validation resolves
the configured target ref once and obtains one HEAD; it never invokes
`rev-parse HEAD HEAD`. No refill, monitor, provider, scheduler, or other
protected runtime activation is permitted before that exact post-L birth
receipt validates.
