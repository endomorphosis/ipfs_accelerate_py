...
- ASI-058 completion gate integration: `AdaptiveRefinementResult.evaluate_objective_completion` fixes repository-tree identity from the transaction receipt and always evaluates the closed six-clause `NEW_COUNTEREXAMPLE_REFINEMENT_ACCEPTANCE_CRITERIA` population through the canonical goal-completion gate. It does not accept a caller-supplied tree or criterion subset and never promotes the adaptive receipt or formal replanner routing metadata into validation, coverage, analyzer-health, or exhaustion-quorum authority. `test_g098_completion_requires_fresh_complete_current_tree_proof` maps every clause to implementation and validation, submits a fresh passing validation for each, requires an explicitly healthy completion-safe analyzer and two fresh independent healthy exhaustive receipts, proves active-to-provisional-to-verified lifecycle order, and fails closed for a missing criterion, any extra failed or stale proof, implicit or unsafe health, duplicate evidence channels, unhealthy quorum members, or a foreign tree.
...
- Objective validation repair: ASI-054/ASI-055 retain the shared planning validation packet, while ASI-058 owns G098's separate completion-gate integration and literal six-criterion proof map. The current-tree validation runs `python -m pytest test/api/test_agent_supervisor_adaptive_goal_refiner.py -q`; no smaller child goal is needed because the producer, cycle admission boundary, receipt restoration, and completion bridge are exercised cohesively in that suite. G098 remains active and supervisor-actionable until a live evaluation supplies fresh current-tree validation receipts, explicit safe analyzer health, and the configured independent exhaustive quorum and completes the mandatory provisional transition.
- Refinement depth: 2
