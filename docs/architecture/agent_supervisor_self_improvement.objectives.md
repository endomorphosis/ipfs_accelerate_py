...
- Evidence producer bindings: 189057730455837902155591890661235220962=analysis_pipeline.EXACT_TREE_REUSE_REQUIREMENT_ID and the content-addressed ExactTreeReuseEvidence witness emitted only after AnalysisCache returns a fresh exact seven-dimension completion hit, the caller's declared digests have been folded with the actual query, analyzer, AST/retrieval/provider inputs and pipeline policy, the referenced packet artifact passes its digest and content-identity checks, and the packet, stage receipts, lookup state, and witness are rebound to the active request, as asserted by test/api/test_agent_supervisor_analysis_pipeline.py; 184801846437522667882915494501685213497=ipfs_datasets_analysis_provider.IPFS_DATASETS_LAZY_DEGRADATION_REQUIREMENT_ID and a content-addressed IpfsDatasetsProviderDegradationEvidence witness emitted only for a bounded offload request whose policy, lazy-load, capability, or runtime boundary degrades explicitly and returns non-authoritative advisory output with a deterministic local-fallback directive; the witness is bound to the exact request, repository tree, objective revision, operation, result state, and provider policy, as asserted by test/api/test_agent_supervisor_ipfs_datasets_analysis_provider.py
...
- Evidence producer bindings: 314133036252270790078901745919131980427=proposal_validation.NOOP_OR_OUT_OF_SCOPE_FAIL_FAST_REQUIREMENT_ID and the content-addressed ProposalRejectionEvidence carried by ProposalValidationReceipt record the requirement only when a no-op or out-of-scope proposal is rejected before any expensive validation is dispatched; 266404049326363900535699811645710804440=validation_scheduler.TRANSITIVE_IMPACT_REQUIREMENT_ID and the content-addressed TransitiveImpactValidationEvidence carried by ValidationDAGReceipt record the requirement only when impact selection schedules and executes a transitively affected validation that detects the bound seeded defect, as asserted by test/api/test_agent_supervisor_proposal_validation.py and test/api/test_agent_supervisor_validation_dag.py
- Evidence source policy: Producer bindings are routing metadata, not completion evidence. A requirement is covered only by a fresh passing typed receipt from the bound code/test producer with the exact requirement ID, repository tree, objective and policy identities, canonical inputs and result, complete gate trace, and content digest. This heap, todo/task prose, generated discovery reports, an unbound occurrence of an ID, a proposal rejection without proof that expensive dispatch remained closed, and an impact-selection plan without an executed defect-detecting result are non-qualifying sources.
...
- Objective validation repair: ASI-027/ASI-028 are one analysis packet. The joint validation command covers exact-key authority, lazy optional-provider degradation, and sync/async single-flight behavior together. G094 and G095 already separate the authority and optional-capability trust boundaries at depth 2, so a smaller child would duplicate rather than refine the proof.
- Refinement depth: 2
...
- Evidence producer: ipfs_accelerate_py/agent_supervisor/ipfs_datasets_analysis_provider.py IPFS_DATASETS_LAZY_DEGRADATION_REQUIREMENT_ID and the content-addressed IpfsDatasetsProviderDegradationEvidence witness carried by AnalysisProviderResult for explicit disabled, unavailable, or unsupported local fallback. The witness emits the requirement only when request, repository tree, objective revision, operation, result status/reason/health, and content-addressed provider policy are mutually bound; `proves_for` independently checks the active request and policy. This is exercised by test/api/test_agent_supervisor_ipfs_datasets_analysis_provider.py and the integrated fallback cases in test/api/test_agent_supervisor_analysis_pipeline.py.
- Proof obligation map: (1) module import, provider construction, disabled execution, and capabilities perform no optional import (`test_construction_and_capability_declaration_do_not_import`, `test_disabled_provider_is_explicit_without_import`); (2) a missing module and a policy-unsupported operation produce typed content-addressed degradation evidence with an explicit deterministic local fallback and exact requirement ID (`test_missing_optional_module_degrades_explicitly_with_typed_evidence`, `test_unsupported_operation_never_loads_backend`); (3) capability negotiation and requests enforce closed operations, protocol compatibility, supervisor-owned count/query/request/response/reference/time bounds, compact reference projection, deterministic identities, literal booleans, and non-expandable empty policy (`test_supported_backend_receives_bounds_and_remains_advisory`, `test_compact_success_is_deterministic_bounded_and_non_authoritative`, `test_explicit_empty_operation_policy_is_not_expanded`); (4) incompatible, unhealthy, malformed, cancelled, failed, heavy, exceptional, awaitable, and typed backend results all re-enter the same bounded fail-closed projection (`test_capability_degradation_is_typed_bound_and_non_authoritative`, `test_cancelled_and_failed_paths_are_explicit_without_false_authority`, `test_heavy_or_malformed_backend_payload_fails_closed`, `test_backend_exception_is_typed_and_async_facade_leaks_no_coroutine`, `test_typed_backend_result_reenters_bounded_projection`); (5) restored records validate content IDs and fixed non-authority invariants, and a degradation witness cannot be detached or replayed onto another request, tree, operation, status, reason, health, or policy (`test_serialized_invariants_and_content_ids_are_fail_closed`, `test_degradation_evidence_cannot_be_detached_or_replayed`); (6) timed-out non-cooperative executions remain semaphore bounded and yield typed non-authoritative fallback (`test_repeated_timeouts_have_bounded_backend_concurrency`); and (7) explicit degradation remains advisory while healthy local analysis can complete, while a custom provider's foreign identity or authority claim is projected to an advisory failure (`test_optional_provider_degrades_explicitly_without_poisoning_local_result`, `test_optional_provider_rebinding_or_authority_claim_is_rejected_advisory`).
...
- Objective validation repair: ASI-027/ASI-028 share the joint packet validation described by G094. ASI-028 hardened the provider witness against cross-request, cross-tree, cross-result, and cross-policy replay and expanded the shared packet suite from 40 to 48 passing cases. The existing depth-2 children isolate local cache authority from the optional provider boundary, so no additional refinement is required.
- Refinement depth: 2
...
- Evidence producer: ipfs_accelerate_py/agent_supervisor/validation_scheduler.py TRANSITIVE_IMPACT_REQUIREMENT_ID and the content-addressed TransitiveImpactValidationEvidence carried by ValidationDAGReceipt; the receipt binds the current repository tree, objective, validation policy, canonical change and dependency graph, impact-selection trace, complete selected-node population, executed result, seeded-defect identity, fail-fast disposition, and content digest, exercised by test/api/test_agent_supervisor_validation_dag.py and the integrated authority cases in test/api/test_agent_supervisor_semantic_validation_pipeline.py
- Proof obligation map: (1) impact analysis follows deterministic direct and transitive dependency edges from changed code to affected interfaces, tests, and declared validations rather than selecting only same-file tests; (2) the seeded fixture changes an upstream dependency while the observable failure lives in a transitive consumer, and the DAG includes and executes that consumer validation; (3) every required selected node appears exactly once with explicit dependencies and disposition, independent ready nodes may run within bounds, and the detected failure closes every dependent semantic, proof, merge, freshness, and completion gate; (4) bounded diagnostics identify the selected transitive path and the actual failing result without converting an expected adversarial failure into code-completion authority; and (5) TransitiveImpactValidationEvidence and ValidationDAGReceipt reject omitted nodes, unexecuted selection plans, passing or unrelated results, changed graph edges, detached seeded-defect identities, cross-tree or cross-policy replay, altered gate disposition, and forged content digests.
- Evidence source policy: This child goal and its discovery report do not satisfy their own evidence requirement. Only a fresh passing ValidationDAGReceipt from the bound producer/test on the current repository tree and policy qualifies; a static impact graph, selected command list, direct-only failure, unexecuted validation, cached result from another tree, todo/task prose, and a bare requirement ID do not.
- Outputs: ipfs_accelerate_py/agent_supervisor/proposal_validation.py, ipfs_accelerate_py/agent_supervisor/validation_scheduler.py, ipfs_accelerate_py/agent_supervisor/code_proof_obligations.py, ipfs_accelerate_py/agent_supervisor/formal_plan_conformance.py, test/api/test_agent_supervisor_proposal_validation.py, test/api/test_agent_supervisor_validation_dag.py, test/api/test_agent_supervisor_semantic_validation_pipeline.py
- Validation: python -m pytest test/api/test_agent_supervisor_proposal_validation.py test/api/test_agent_supervisor_validation_dag.py test/api/test_agent_supervisor_semantic_validation_pipeline.py -q
- Acceptance: The validation DAG is derived from the canonical changed-file and dependency/interface impact graph; includes every mandatory direct and transitive validation with explicit dependency edges; schedules only ready nodes under bounded parallelism; records selected, executed, failed, blocked, and omitted dispositions deterministically; and fails closed when impact evidence is incomplete or stale. A seeded upstream defect must select and execute its transitively affected consumer test, whose real failure blocks semantic/proof promotion, merge, freshness, and completion authority. The exact requirement ID is emitted only by a tamper-evident receipt binding the current tree, objective, policy, change and graph identities, selected-node population, impact paths, actual defect-detecting result, fail-fast disposition, and content digest.
- Objective validation repair: ASI-031/ASI-032 share the validation-gate packet described by ASI-G100. The joint command runs `python -m pytest test/api/test_agent_supervisor_proposal_validation.py test/api/test_agent_supervisor_validation_dag.py test/api/test_agent_supervisor_semantic_validation_pipeline.py -q` on the current tree, so preflight fail-fast behavior and transitive impact coverage cannot be promoted independently of the common semantic/proof authority boundary. The existing depth-2 children are independently testable and fully separate their proof obligations, so another refinement would duplicate rather than clarify the evidence boundary.
- Refinement depth: 2
- Embedding query: 266404049326363900535699811645710804440
- AST query: TRANSITIVE_IMPACT_REQUIREMENT_ID TransitiveImpactValidationEvidence ValidationDAGReceipt
- Parallel lane: agent-supervisor/self-improvement/validation
- Conflict policy: prefer bundle-local changes; invoke the LLM merge resolver for semantic conflicts
