...
- Evidence producer: ipfs_accelerate_py/agent_supervisor/control_contracts.py CONTROL_DISCOVERY_SAFETY_REQUIREMENT_ID and the content-addressed ControlDiscoveryManifest, ControlDiscoveryRuntimeState, ControlDiscoveryObservation, and ControlDiscoverySafetyEvidence contracts. SupervisorControlService.discovery_manifest, control_cli.agent_cli_discovery_manifest, and native_agent_supervisor_tools.agent_supervisor_discovery_manifest expose static metadata; control_plane.capture_control_discovery_runtime_state records module/process inventories and externally instrumented cumulative counters; CLI dispatch avoids product-runtime construction for the agent namespace and optional AI adapters remain lazy; MCP service resolution is counted and occurs only after tool invocation. The API, unified CLI, and MCP tests construct the qualifying three-surface record.
- Proof obligation map: (1) each manifest derives rather than accepts the complete closed Operation vocabulary and request/result schema identities; (2) each observation re-decodes two manifests for one surface, requires canonical equality, and rejects any before/after change in optional-provider modules, child-process IDs, provider-load attempts, process-start attempts, or service-resolution count; (3) evidence requires exactly one Python, CLI, and MCP observation and a complete CapabilityReport whose provider/process flags are false; (4) Python discovery uses the cached capability report and never calls a backend; (5) CLI parser/help discovery resolves no service and constructs neither IPFSAccelerateCLI nor an optional AI/provider adapter; (6) MCP category, tool, and schema listing remains static and repeated while its cumulative resolution counter stays unchanged; and (7) deserialization re-derives schemas, populations, safety comparisons, current tree/objective/policy binding, and content identity, rejecting omitted surfaces, drift, claimed-only safety, or tampering.
- Evidence source policy: This objective, ASI-047, discovery prose, a bare requirement ID, comments claiming lazy behavior, registration metadata alone, or CapabilityReport.optional_providers_loaded/processes_started alone do not satisfy the requirement. Only a fresh passing ControlDiscoverySafetyEvidence round trip from the bound current-tree validation command qualifies. The evidence must contain repeated observations from all three adapters with independent before/after inventories and counters; a provider or short-lived process hidden by a final snapshot still changes an instrumented cumulative counter and fails.
- Outputs: ipfs_accelerate_py/agent_supervisor/control_contracts.py, ipfs_accelerate_py/agent_supervisor/control_plane.py, ipfs_accelerate_py/agent_supervisor/control_cli.py, ipfs_accelerate_py/cli.py, ipfs_accelerate_py/mcp_server/tools/agent_supervisor_tools, test/api/test_agent_supervisor_control_plane.py, test/test_unified_cli_agent_supervisor.py, test/mcp_server/test_agent_supervisor_tools.py
- Validation: python -m pytest test/api/test_agent_supervisor_control_plane.py test/api/test_agent_supervisor_control_lifecycle.py test/test_unified_cli_agent_supervisor.py test/mcp_server/test_agent_supervisor_tools.py -q
...
- Objective validation repair: G103, G104, and G105 are the three depth-2 partitions of unified control: transport/schema behavior, mutation authority, and discovery isolation. They share one validation matrix so no partition can be promoted by weakening another, but retain distinct content-addressed evidence records. ASI-047 is the completed executable projection for G105. ASI-049/ASI-G128 was subsequently generated as a synthetic depth-3 child which recursively repeats G105's goal and exact evidence term, producer, outputs, and validation command, so it owns no distinct objective obligation. ASI-049 therefore retires that orphan duplicate instead of adding G128 to the heap: `186773143401179107362964063059661378722` remains exclusively G105's content-addressed three-surface discovery-safety receipt. Generated todo, bundle, and vector projections reconcile from this source-of-truth heap, and no smaller or deeper child goal is required.
- Refinement depth: 2
