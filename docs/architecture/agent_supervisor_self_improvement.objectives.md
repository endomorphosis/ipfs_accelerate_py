...
- Objective validation repair: ASI-032 remains the operational G101 producer. ASI-064 is its current depth-2 validation-gate projection (todo vector `237e370a4ddea0a3`, merge key `ac052ce35a764708`, family `objective/ASI-G101`) and closes the previously missing objective-completion bridge. It reruns `python -m pytest test/api/test_agent_supervisor_proposal_validation.py test/api/test_agent_supervisor_validation_dag.py test/api/test_agent_supervisor_semantic_validation_pipeline.py -q` on one current tree, validates the live failed adversarial receipt independently from the passing objective-validation receipts, and keeps proposal, impact, semantic, proof, merge, freshness, and completion boundaries in one replayable chain. This does not replace typed producer evidence or manually mutate supervisor-fed backlog state. G100 and G101 still cleanly partition pre-execution and post-admission obligations at depth 2, so no smaller child goal is needed.
- Refinement depth: 2
