...
- Evidence criterion: Optional ipfs_datasets_py analysis is imported only after a supported bounded offload is requested; unavailable, disabled, unsupported, incompatible, timed-out, failed, or malformed capability/runtime states are explicit and never become supervisor completion authority.
- Evidence producer: ipfs_accelerate_py/agent_supervisor/ipfs_datasets_analysis_provider.py IPFS_DATASETS_LAZY_DEGRADATION_REQUIREMENT_ID and the content-addressed IpfsDatasetsProviderDegradationEvidence witness carried by AnalysisProviderResult for explicit disabled, unavailable, or unsupported local fallback, exercised by test/api/test_agent_supervisor_ipfs_datasets_analysis_provider.py and the integrated fallback case in test/api/test_agent_supervisor_analysis_pipeline.py
- Proof obligation map: (1) module import, provider construction, and capabilities perform no optional import (`test_construction_and_capability_declaration_do_not_import`); (2) a missing module and a policy-unsupported operation produce typed content-addressed degradation evidence with an explicit deterministic local fallback and exact requirement ID (`test_missing_optional_module_degrades_explicitly_with_typed_evidence`, `test_unsupported_operation_never_loads_backend`); (3) capability negotiation and requests enforce closed operations, protocol compatibility, supervisor-owned count/query/request/response/reference/time bounds, compact reference projection, deterministic identities, literal booleans, and non-expandable empty policy (`test_supported_backend_receives_bounds_and_remains_advisory`, `test_compact_success_is_deterministic_bounded_and_non_authoritative`, `test_explicit_empty_operation_policy_is_not_expanded`); (4) heavy, malformed, exceptional, awaitable, and typed backend results all re-enter the same bounded fail-closed projection (`test_heavy_or_malformed_backend_payload_fails_closed`, `test_backend_exception_is_typed_and_async_facade_leaks_no_coroutine`, `test_typed_backend_result_reenters_bounded_projection`); (5) restored capability/result/degradation records validate content IDs and fixed non-authority invariants (`test_serialized_invariants_and_content_ids_are_fail_closed`); and (6) explicit degradation remains advisory while healthy local analysis can complete (`test_optional_provider_degrades_explicitly_without_poisoning_local_result`).
...
- Objective validation repair: ASI-027/ASI-028 share the joint packet validation described by G094. The existing depth-2 children isolate local cache authority from the optional provider boundary, so no additional refinement is required.
- Refinement depth: 2
