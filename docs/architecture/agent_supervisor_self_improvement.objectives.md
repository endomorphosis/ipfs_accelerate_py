...
- Objective validation repair: ASI-G103, ASI-G104, and ASI-G105 form the control packet. G103 owns cross-surface schema/behavior equivalence; G104 owns mutation authorization/idempotency/lease/effect/audit enforcement; G105 owns discovery determinism and provider/process/service isolation. Their joint validation command executes the same current-tree control, lifecycle, CLI, and MCP suites. The obligations are independently receipted at depth 2, so no smaller child is needed.
- Refinement depth: 2
...
- Evidence producer: ipfs_accelerate_py/agent_supervisor/control_contracts.py CONTROL_MUTATION_GUARD_REQUIREMENT_ID and the content-addressed ControlMutationGuardEvidence/MutationGuardExecutionObservation/MutationGuardRejection contracts, backed by OperationRequest cross-field validation and SupervisorControlService authorization freshness, allowlist, lease, effect, audit, and idempotency enforcement. SupervisorControlService.mutation_runtime_state supplies service-owned snapshots of actual mutation dispatch request IDs and successfully persisted mutation audit receipt IDs before execution, after execution, and after replay. test/api/test_agent_supervisor_control_lifecycle.py constructs the qualifying applied/replayed/observed/rejected evidence matrix; test/api/test_agent_supervisor_control_plane.py validates authorization expiry, stale fences, allowlists, idempotency conflicts, restart replay, audit receipts, dry-run non-dispatch, and backend call counts; CLI/MCP tests prove malformed guard variants fail during canonical decoding before service resolution and that transport replay cannot duplicate the backend call.
- Proof obligation map: (1) the request binds operation, roots, repository tree, objective revision, policy revision, caller, expected effects, authorization decision, idempotency scope, lease ID, and fencing epoch; (2) permit authority, authorized effect IDs, identity fields, expiry, and external policy/lease validators are checked before dispatch; (3) dry-run is proposal-only, invokes no mutation backend, applies no effects, and requires no fabricated permit; (4) a real result cannot claim undeclared or reshaped effects, every applied effect receipt must equal the result audit receipt, and the service records that receipt only after audit persistence and idempotency storage succeed; (5) the typed execution observation requires the first call to add exactly the bound request ID and audit receipt ID to service-owned runtime populations and requires replay to leave both populations unchanged; (6) exact idempotent replay returns the prior content identity while changed payload reuse conflicts; and (7) ControlMutationGuardEvidence independently replays each missing guard through OperationRequest, binds the applied result, replay result, execution observation, and rejection matrix to the same current request/tree/objective/policy, and rejects incomplete guards, detached observations, extra dispatch/audit events, non-applied results, missing audits, or changed replay records.
...
- Objective validation repair: ASI-033 is the completed executable projection for the G103/G104 control packet, while ASI-047 independently projects G105 discovery isolation. ASI-048/ASI-G126 was generated earlier as a synthetic depth-3 aggregate over G104/G105, but it owns no distinct evidence term: `184125100306462690646212311073240043804` remains exclusively G104's typed mutation-guard receipt and `186773143401179107362964063059661378722` remains G105's discovery receipt. ASI-048 therefore repairs and retires the orphan duplicate instead of adding G126 to the heap. The joint current-tree validation command still executes all three independent evidence boundaries, and no smaller or deeper child goal is required.
- Refinement depth: 2
