...
- Objective validation repair: G100, G101, and G102 are the three depth-2 partitions of the strict validation gate. G100 owns pre-execution output rejection, G101 owns post-admission transitive impact execution and downstream closure, and G102 owns the semantic proof-to-code-completion authority boundary. Their joint validation command runs the proposal, DAG, and semantic pipeline suites on the same current tree. One end-to-end typed G102 witness expresses the candidate-isolation boundary without crossing either earlier partition, so no smaller child goal is needed.
- Refinement depth: 2
