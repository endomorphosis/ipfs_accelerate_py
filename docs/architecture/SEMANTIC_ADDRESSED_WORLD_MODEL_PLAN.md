...
Status: operator-owned R2 program control. M64 preserves the accepted M63 receipt, event 332, and evidence node 65 while sealing a bounded two-file source-checkout bootstrap repair. Plain execution of the nested SAWM operator previously reached its deferred scheduler import without the repository root on `sys.path`; M64 scopes the exact resolved root to that import and restores the caller's path exactly, while module import and `--help` remain inert. M63 already preserved M62's exact committed controls as a failed pre-authoritative attempt and supplied integer startup-budget values (`30`, `300`, `300`). M64 preserves the same live generation-43 owner, all accepted prior receipts and events, and every task and goal head; it appends only event 333 and evidence node 66 as its source seal. It does not authorize a generation restart, task transition, provider invocation, merge, effect, or completion. A dead or replaced generation-43 owner requires a separately sealed generation-44 successor. M54 remains deny-only and unselected.
...
Accepted M63 source authority: `sha256:17d068e86e8d00b33c686fbb048350c5281e32e68e8b92c8150fff2186c28632` (the committed M62 controls are preserved as a failed pre-authoritative anchor, followed directly by one nine-control-file M63 successor seal).
