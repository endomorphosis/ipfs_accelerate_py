...
Status: operator-owned R2 program control. M59 seals the bounded coordinator launch-gate repair discovered after the accepted M58 restart: the Quack adapter returns exact `DuckDBRow` values, while the detached coordinator compared that list directly with tuples and therefore rejected an otherwise exact authenticated task snapshot. The repair normalizes exactly one row by position, enforces exact width and field types, and retains fail-closed mismatch behavior. M59 preserves M58, its receipt, event 328, the exact live generation-43 owner, every accepted task head, and the failed launch evidence; it appends only the source-seal evidence at event 329. It does not authorize a generation restart, task transition, provider invocation, merge, effect, or completion. A dead or replaced generation-43 owner requires a separately sealed generation-44 successor. M54 remains deny-only and unselected.

M59 source authority: `sha256:7966d5c7bbb909cffe2e6c34c0659d31da9c943b6b11b134c5703f87c5452ef5` (one bounded two-file repair commit followed by one nine-control-file seal; the current control commit identity is deliberately not embedded).
