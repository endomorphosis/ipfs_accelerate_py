# Self-Hosting Qualification v5 Task History
...
- SHQ-008: never launched. A prelaunch receipt-schema correction found that the
  direct process-runner path incorrectly named the pytest-adapter schema and did
  not fully bind the retained stream bytes; it is superseded by the bounded-v6
  G006 projection.
- SHQ-009: never launched because its SHQ-008 dependency was never submitted; it
  is superseded by the bounded-v6 G007 projection.

Neither task was submitted to coordination, leased, assigned a fencing token,
given an implementation worktree, or sent to a model. The canonical task blocks
