...
- Acceptance: Every requested authority is classified; exact baseline identities reproduce; the real objective daemon imports without mocks; materialization is clean-tree bound and emits the exact initial APMC projection; Quack/DuckLake observations are truthfully separated from authority; sibling repositories and pins remain unchanged; the operator-readable baseline seal is present.
