...
- Interfaces: UIUXSourceGate@1, UIUXFormalizationAdapter@1
- Allow concurrent with: LFP-034, LFP-035, LFP-036, LFP-037, LFP-039
- Conflict policy: Own only the LFP-038 source gate and tests; never recreate, copy, or edit ui_ux_ir. Repair the worktree-setup false pin and release LFP-038 without inventing an external checkout.
- Preconditions: LFP-038 retry budget exhausted on worktree_setup; discovery receipt records dependency_missing for a prose false pin.
- Effects: UI/UX source gate and conformance tests land under ipfs_datasets_py; LFP-038 acceptance is satisfied; supervisor can schedule downstream LFP-040+ work.
- Evidence subset: ui ux source gate retry budget worktree setup false pin declaration only
- Symbolic first: true
- LLM context budget bytes: 24000
- Generated by: ipfs_accelerate_py.agent_supervisor.retry-budget-repair@1
- Retry repair source: LFP-038
- Retry failure kind: implementation
- Retry repair discovery: /home/barberb/lift_coding/.worktrees/logic-family-parser-supervisor-runtime/data/agent_supervisor/ipfs_datasets_logic_family_parser/state/discovery/2026-08-09-lfp-050-lfp-038-implementation-retry-budget.md
- Acceptance: Implementation retry-budget repair for LFP-038 is closed: gate outputs exist, conformance tests pass, and LFP-038 is completed so the ready queue is no longer attempt-limited on that single task.
- Embedding query: ui ux source gate retry budget worktree dependency missing external untracked
...
- Estimated tokens: 16000
- Implementation timeout seconds: 7200
- Predicted files: ipfs_datasets_py/ipfs_datasets_py/logic/api.py, ipfs_datasets_py/ipfs_datasets_py/logic/verification_api.py, ipfs_datasets_py/tests/conformance/logic/test_public_api_migration.py
- Interfaces: VerificationAPI@2, CanonicalLogicDiscovery@1
- Allow concurrent with:
- Conflict policy: Own only LFP-044 declared API/migration outputs; preserve current public entry until explicit deprecation gate; do not invent undeclared unit-test modules outside acceptance.
- Preconditions: LFP-044 validation retry budget exhausted on proposal-gate/scope mismatches.
- Effects: Public API dual-read/canonical-write surfaces land under declared outputs; LFP-044 acceptance is satisfied; downstream LFP-045+ can schedule.
- Evidence subset: public api dual read canonical write migration retry budget validation
- Symbolic first: true
- LLM context budget bytes: 24000
- Generated by: ipfs_accelerate_py.agent_supervisor.retry-budget-repair@1
- Retry repair source: LFP-044
- Retry failure kind: validation
- Retry repair discovery: /home/barberb/lift_coding/.worktrees/logic-family-parser-supervisor-runtime/data/agent_supervisor/ipfs_datasets_logic_family_parser/state/discovery/2026-08-09-lfp-051-lfp-044-retry-budget.md
- Acceptance: Validation retry-budget repair for LFP-044 is closed: declared API outputs exist, unit and conformance suites pass, and LFP-044 is completed so the ready queue advances to LFP-045.
- Embedding query: public verification api dual read canonical write migration retry budget
